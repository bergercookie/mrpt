...
// CConfigFileBase
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(CConfigFileBase_read_double_overloads,
                                       read_double, 3, 4)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(CConfigFileBase_read_float_overloads,
                                       read_float, 3, 4)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(CConfigFileBase_read_bool_overloads,
                                       read_bool, 3, 4)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(CConfigFileBase_read_int_overloads,
                                       read_int, 3, 4)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(CConfigFileBase_read_uint64_t_overloads,
                                       read_uint64_t, 3, 4)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(CConfigFileBase_read_string_overloads,
                                       read_string, 3, 4)
...
void export_utils() {
  ...
  // TColorf
  {
    class_<TColorf>("TColorf", init<optional<float, float, float, float>>(
                                   args("r", "g", "b", "alpha"),
  ...
  // CObject
  {
    class_<CObject::Ptr>("CObject::Ptr", "class_name smart pointer type",
                         no_init)
    ...
    class_<CObject, boost::noncopyable>("CObject", no_init)
        .def("clone", &CObject::clone, return_value_policy<manage_new_object>(),
             "Returns a copy of the object, indepently of its class.")
        .def("GetRuntimeClass", &CObject::GetRuntimeClass,
  ...
  // CLoadableOptions
  {
    class_<CLoadableOptions, boost::noncopyable>("CLoadableOptions", no_init)
        .def("loadFromConfigFile", &CLoadableOptions::loadFromConfigFile,
             "This method load the options from a \".ini\"-like file or "
             "memory-stored string list.")
        .def("loadFromConfigFileName",
        ...
        .def("saveToConfigFile", &CLoadableOptions::saveToConfigFile,
             "This method saves the options to a \".ini\"-like file or "
             "memory-stored string list.")
        .def("saveToConfigFileName", &CLoadableOptions::saveToConfigFileName,
             "Behaves like saveToConfigFile, but you can pass directly a file "
             "name and a temporary CConfigFile object will be created "
             "automatically to save the file.")
        .def("dumpToConsole", &CLoadableOptions::dumpToConsole,
             "Just like dumpToTextStream() but sending the text to the console "
             "(std::cout)")
        .def("dumpToTextStream", &CLoadableOptions::dumpToTextStream,
  ...
  // CConfigFileBase
  {
    class_<CConfigFileBase, boost::noncopyable>("CConfigFileBase", no_init)
        .def("getAllSections", &CConfigFileBase::getAllSections,
             "Returns a list with all the section names.")
        .def("getAllKeys", &CConfigFileBase::getAllKeys,
             "Returs a list with all the keys into a section.")
        .def("sectionExists ", &CConfigFileBase::sectionExists,
             "Checks if a given section exists (name is case insensitive).")
        .def("read_double", &CConfigFileBase::read_double,
             CConfigFileBase_read_double_overloads())
        .def("read_float", &CConfigFileBase::read_float,
             CConfigFileBase_read_float_overloads())
        .def("read_bool", &CConfigFileBase::read_bool,
             CConfigFileBase_read_bool_overloads())
        .def("read_int", &CConfigFileBase::read_int,
             CConfigFileBase_read_int_overloads())
        .def("read_uint64_t", &CConfigFileBase::read_uint64_t,
             CConfigFileBase_read_uint64_t_overloads())
        .def("read_string", &CConfigFileBase::read_string,
             CConfigFileBase_read_string_overloads())
        .def("read_string_first_word", &CConfigFileBase::read_string_first_word,
             CConfigFileBase_read_string_first_word_overloads());
  }

  // CConfigFile
  {
    class_<CConfigFile, bases<CConfigFileBase>>("CConfigFile",
                                                init<optional<std::string>>());
  }

  // CSerializable
  {
    class_<CSerializable::Ptr>("CSerializable::Ptr",
                               "class_name smart pointer type", no_init)
        .def("ctx", &CSerializablePtr_get_ctx, return_internal_reference<>())
        .def("ctx", &CSerializablePtr_set_ctx)
        .def("pointer", &CSerializablePtr_pointer,
             return_internal_reference<>());

    class_<CSerializable, boost::noncopyable, bases<CObject>>("CSerializable",
                                                              no_init);
  }
  ...
  // CStream
  {
    class_<CStream, boost::noncopyable>(
        "CStream", "This base class is used to provide a unified interface to "
                   "files,memory buffers.",
        no_init)
        .def("ReadObject", &CStream_ReadObject,
             "Reads an object from stream, its class determined at runtime, "
             "and returns a smart pointer to the object.")
        .def("WriteObject", &CStream_WriteObject,
  ...
  // CStdOutStream
  {
    class_<CStdOutStream, bases<CStream>>(
        "CStdOutStream", "This CStdOutStream derived class allow printing to "
                         "standard out, normally the console text output.",
        init<>());
  }
  ...
  // CFileGZInputStream
  {
    class_<CFileGZInputStream, boost::noncopyable, bases<CStream>>(
        "CFileGZInputStream", "Transparently opens a compressed \"gz\" file "
                              "and reads uncompressed data from it.",
