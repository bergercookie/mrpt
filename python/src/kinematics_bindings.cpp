...
TTwist2D
CVehicleSimul_DiffDriven_getCurrentGTVel(CVehicleSimul_DiffDriven &self) {
  return self.getCurrentGTVel();
}
TTwist2D
CVehicleSimul_DiffDriven_getCurrentGTVelLocal(CVehicleSimul_DiffDriven &self) {
    ...
    class_<CVehicleSimul_DiffDriven>("CVehicleSimul_DiffDriven", init<>())
        .def("setDelayModelParams",
             ...
             CVehicleSimul_DiffDriven_setDelayModelParams_overloads(
                 ...
                 "Change the model of delays used for the orders sent to the "
                 "robot"))
        .def("setOdometryErrors", &CVehicleSimul_DiffDriven::setOdometryErrors,
             CVehicleSimul_DiffDriven_setOdometryErrors_overloads(
                 args("enabled", "Ax_err_bias", "Ax_err_std", "Ay_err_bias",
                      "Ay_err_std", "Aphi_err_bias", "Aphi_err_std"),
                 "Enable/Disable odometry errors. Errors in odometry are "
                 "introduced per millisecond."))
        //			.def("setRealPose",
        //&CVehicleSimul_DiffDriven_setRealPose, "Reset actual robot pose
        //(inmediately, without simulating the movement along time).")
        .def("getCurrentGTPose", &CVehicleSimul_DiffDriven_getRealPose,
             "Returns the instantaneous, ground truth pose in world "
             "coordinates.")
        .def("getCurrentOdometricPose", &CVehicleSimul_DiffDriven_getOdometry,
             " Returns the current pose according to (noisy) odometry.")
        .def("getTime", &CVehicleSimul_DiffDriven::getTime,
             "Get the current simulation time.")
        .def("getCurrentGTVel", &CVehicleSimul_DiffDriven_getCurrentGTVel,
             "Returns the instantaneous, ground truth velocity vector "
             "(vx,vy,omega) in world coordinates.")
        .def("getCurrentGTVelLocal",
             &CVehicleSimul_DiffDriven_getCurrentGTVelLocal,
             "Returns the instantaneous, ground truth velocity vector "
             "(vx,vy,omega) in the robot local frame .")
        .def("setV", &CVehicleSimul_DiffDriven::setV, args("v"),
             "Set actual robot velocity, error-free status of the simulated "
             "robot.")
        .def("setW", &CVehicleSimul_DiffDriven::setW, args("w"),
             "Set actual robot turnrate, error-free status of the simulated "
             "robot.")
        .def("movementCommand", &CVehicleSimul_DiffDriven::movementCommand,
             args("lin_vel", "ang_vel"),
             "Used to command the robot a desired movement (velocities).")
        .def("resetStatus", &CVehicleSimul_DiffDriven::resetStatus,
             "Reset all the simulator variables to 0 (All but current "
             "simulator time).")
        .def("resetTime", &CVehicleSimul_DiffDriven::resetTime,
             "Reset time counter.")
        .def("simulateOneTimeStep",
