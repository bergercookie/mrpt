...
#ifdef ROS_EXTENSIONS
object
CPose3DPDFGaussian_to_ROS_PoseWithCovariance_msg(CPose3DPDFGaussian &self) {
...
// CPosePDFParticles
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(CPosePDFParticles_resetUniform_overloads,
                                       resetUniform, 4, 7)
...
void export_poses() {
  ...
  // CPose2D
  {
    ...
    class_<CPose2D>("CPose2D", init<>())
        ...
        .add_property(
            "x", make_function(CPose2D_get_x,
                               return_value_policy<copy_non_const_reference>()),
            CPose2D_set_x)
        .add_property(
            "y", make_function(CPose2D_get_y,
                               return_value_policy<copy_non_const_reference>()),
            CPose2D_set_y)
        .add_property(
            "phi",
            make_function(CPose2D_get_phi,
                          return_value_policy<copy_non_const_reference>()),
            CPose2D_set_phi)
        ...
        .def("distance2DFrobeniusTo", &CPose2D::distance2DFrobeniusTo,
             "Returns the 2D distance from this pose/point to a 2D pose using "
  ...
  // CPosePDF
  {
    ...
    class_<CPosePDF, boost::noncopyable>("CPosePDF", no_init)
        .def("getMean", &CPosePDF::getMean, "Returns the mean, or mathematical "
                                            "expectation of the probability "
                                            "density distribution (PDF).")
        .def("getCovarianceAndMean", &CPosePDF::getCovarianceAndMean,
             "Returns an estimate of the pose covariance matrix "
             "(STATE_LENxSTATE_LEN cov matrix) and the mean, both at once.")
        .def("saveToTextFile", &CPosePDF::saveToTextFile,
             "Save PDF's particles to a text file. See derived classes for "
             "more information about the format of generated files.")
        .def("copyFrom", &CPosePDF::copyFrom,
             "Copy operator, translating if necesary (for example, between "
             "particles and gaussian representations).")
        .def("bayesianFusion", &CPosePDF::bayesianFusion,
             "Bayesian fusion of two pose distributions (product of two "
             "distributions->new distribution), then save the result in this "
             "object (WARNING: See implementing classes to see classes that "
             "can and cannot be mixtured!).")
        .def("inverse", &CPosePDF::inverse,
  ...
  // CPosePDFGaussian
  {
    ...
    class_<CPosePDFGaussian, bases<CPosePDF>>("CPosePDFGaussian",
                                              init<optional<CPose2D>>())
        .def_readwrite("mean", &CPosePDFGaussian::mean)
        .add_property("cov", CPosePDFGaussian_get_cov,
                      CPosePDFGaussian_set_cov);
  }
  ...
  // CPosePDFParticles
  {
    ...
    class_<CPosePDFParticles, bases<CPosePDF>>("CPosePDFParticles",
                                               init<optional<size_t>>())
        .def("resetUniform", &CPosePDFParticles::resetUniform,
  ...
  // CPose3D
  {
    ...
    class_<CPose3D>("CPose3D", init<optional<CPose2D>>())
        ...
        .add_property(
            "x", make_function(CPose3D_get_x,
                               return_value_policy<copy_non_const_reference>()),
            CPose3D_set_x)
        .add_property(
            "y", make_function(CPose3D_get_y,
                               return_value_policy<copy_non_const_reference>()),
            CPose3D_set_y)
        .add_property(
            "z", make_function(CPose3D_get_z,
                               return_value_policy<copy_non_const_reference>()),
            CPose3D_set_z)
        ...
        .def("setFromValues", &CPose3D::setFromValues,
             ...
             "angles (radians) - This method recomputes the internal rotation "
             "matrix.")
        .def("inverse", &CPose3D::inverse,
             "Convert this pose into its inverse, saving the result in itself.")
  ...
  // CPose3DPDF
  {
    ...
    class_<CPose3DPDF, boost::noncopyable>("CPose3DPDF", no_init)
        .def("jacobiansPoseComposition", &CPose3DPDF::jacobiansPoseComposition,
             "This static method computes the pose composition Jacobians.")
        .def("getMeanVal", &CPose3DPDF::getMeanVal)
        .def("getMean", &CPose3DPDF::getMean,
             "Returns the mean, or mathematical expectation of the probability "
             "density distribution (PDF).")
        .def("getCovariance", getCovariance1,
             "Returns an estimate of the pose covariance matrix "
             "(STATE_LENxSTATE_LEN cov matrix).")
        .def("getCovarianceAndMean", &CPose3DPDF::getCovarianceAndMean,
             "Returns an estimate of the pose covariance matrix "
             "(STATE_LENxSTATE_LEN cov matrix) and the mean, both at once.")
        .def("saveToTextFile", &CPose3DPDF::saveToTextFile,
             "Save PDF's particles to a text file. See derived classes for "
             "more information about the format of generated files.")
        .def("changeCoordinatesReference",
             &CPose3DPDF::changeCoordinatesReference,
             "this = p (+) this. This can be used to convert a PDF from local "
             "coordinates to global, providing the point (newReferenceBase) "
             "from which 'to project' the current pdf. Result PDF substituted "
             "the currently stored one in the object.")
        .def("getCovarianceEntropy", &CPose3DPDF::getCovarianceEntropy,
             "Compute the entropy of the estimated covariance matrix.")
        .def("drawSingleSample", &CPose3DPDF::drawSingleSample,
             "Draws a single sample from the distribution")
        .def("copyFrom", &CPose3DPDF::copyFrom,
             "Copy operator, translating if necesary (for example, between "
             "particles and gaussian representations).")
        .def("bayesianFusion", &CPose3DPDF::bayesianFusion,
             "Bayesian fusion of two pose distributions (product of two "
             "distributions->new distribution), then save the result in this "
             "object (WARNING: See implementing classes to see classes that "
             "can and cannot be mixtured!).")
        .def("inverse", &CPose3DPDF::inverse,
             "Returns a new PDF such as: NEW_PDF = (0,0,0) - THIS_PDF");
  }
  ...
  // CPose3DPDFGaussian
  {
    ...
    class_<CPose3DPDFGaussian, bases<CPose3DPDF>>("CPose3DPDFGaussian",
                                                  init<>())
        .def(init<CPose3DPDFGaussian>())
        .def(init<CPose3D>())
        .def("getMean", &CPose3DPDFGaussian::getMean,
             "Returns the mean, or mathematical expectation of the probability "
             "density distribution (PDF).")
        .def("getCovariance", getCovariance1,
             "Returns an estimate of the pose covariance matrix "
             "(STATE_LENxSTATE_LEN cov matrix).")
        .def("getCovarianceAndMean", &CPose3DPDFGaussian::getCovarianceAndMean,
             "Returns an estimate of the pose covariance matrix "
             "(STATE_LENxSTATE_LEN cov matrix) and the mean, both at once.")
        .def("saveToTextFile", &CPose3DPDFGaussian::saveToTextFile,
             "Save PDF's particles to a text file. See derived classes for "
             "more information about the format of generated files.")
        .def("changeCoordinatesReference",
             &CPose3DPDF::changeCoordinatesReference,
             ...
             "from which 'to project' the current pdf. Result PDF substituted "
             "the currently stored one in the object.")
        .def("getCovarianceEntropy", &CPose3DPDFGaussian::getCovarianceEntropy,
             "Compute the entropy of the estimated covariance matrix.")
        .def("drawSingleSample", &CPose3DPDFGaussian::drawSingleSample,
             "Draws a single sample from the distribution")
        //            .def("copyFrom", &CPose3DPDFGaussian::copyFrom, "Copy
        //            operator, translating if necesary (for example, between
        //            particles and gaussian representations).")
        .def("bayesianFusion", &CPose3DPDF::bayesianFusion,
             "Bayesian fusion of two pose distributions (product of two "
             "distributions->new distribution), then save the result in this "
             "object (WARNING: See implementing classes to see classes that "
             "can and cannot be mixtured!).")
        .def("inverse", &CPose3DPDFGaussian::inverse,
             "Returns a new PDF such as: NEW_PDF = (0,0,0) - THIS_PDF")
        .def("jacobiansPoseComposition",
        ...
        .add_property("cov", CPose3DPDFGaussian_get_cov,
                      CPose3DPDFGaussian_set_cov)
        .def(self + self)
        .def(self - self)
        .def(-self)
        .def(self += self)
  ...
  // CPose3DPDFParticles
  {
    ...
    class_<CPose3DPDFParticles, bases<CPose3DPDF>>("CPose3DPDFParticles",
                                                   init<>())
        .def("particlesCount", &CPose3DPDFParticles::particlesCount,
