...
#define MAKE_FIXED_DOUBLE_MATRIX(rows, cols)                                   \
  class_<CMatrixDouble##rows##cols>(STRINGIFY(CMatrixDouble##rows##cols),      \
                                    init<>())                                  \
...
#define MAKE_FIXED_DOUBLE_MATRIX_GETITEM(rows, cols)                           \
  double CMatrixDouble##rows##cols##_getitem1(CMatrixDouble##rows##cols &self, \
                                              size_t i) {                      \
    return self(i);                                                            \
  }                                                                            \
                                                                               \
  double CMatrixDouble##rows##cols##_getitem2(CMatrixDouble##rows##cols &self, \
                                              const tuple &t) {                \
    extract<int> eir(t[0]);                                                    \
    extract<int> eic(t[1]);                                                    \
...
#define MAKE_FIXED_DOUBLE_MATRIX_SETITEM(rows, cols)                           \
  void CMatrixDouble##rows##cols##_setitem1(CMatrixDouble##rows##cols &self,   \
                                            size_t i, double value) {          \
    self(i) = value;                                                           \
  }                                                                            \
                                                                               \
  void CMatrixDouble##rows##cols##_setitem2(CMatrixDouble##rows##cols &self,   \
                                            const tuple &t, double value) {    \
...
void export_math_stl() {
  ...
  MAKE_VEC(TPose2D)
  MAKE_VEC(TPose3D)
  //     MAKE_VEC(TPose3DQuat) // Compiler complains about something missing on
  //     this type, but I can't figure out what.
}
