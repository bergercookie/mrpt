		...
		scope s =
			class_<CMetricMap, boost::noncopyable, bases<CSerializable>>(
				...
				.def(
					...
					"Returns true if the map is empty/no observation has "
					"been inserted.")
				.def(
		...
		scope s =
			class_<CPointsMap, boost::noncopyable, bases<CMetricMap>>(
				...
				.def(
					...
					"the "
					"map does not change, it only reserves the memory.")
				.def(
					"resize", &CPointsMap::resize,
					"Resizes all point buffers so they can hold the given "
					"number "
					"of points: newly created points are set to default "
					"values, "
				...
				.def(
					...
					"Resizes all point buffers so they can hold the given "
					"number "
					"of points, *erasing* all previous contents and leaving "
					"all "
				...
				.def(
					...
					"index), "
					"*without* checking for out-of-bounds and *without* "
					"calling "
					"mark_as_modified().")
				.def(
				...
				.def(
					...
					"(XYZ, "
					"color,...) as possible from the source map into this one.")
				.def(
					"getPointAllFieldsFast", &CPointsMap::getPointAllFieldsFast,
					"Get all the data fields for one point as a vector: "
					"depending "
					"on the implementation class this can be [X Y Z] or [X Y Z "
					"R G "
					"B], etc...")
				.def(
		...
		scope s =
			class_<CSimplePointsMap, bases<CPointsMap>>(
				...
				.def(
					...
					"coordinated "
					"points. The options in \"insertionOptions\" are "
					"considered in "
					"this method.")
				.def(
		...
		scope s =
			class_<CMultiMetricMap, bases<CMetricMap>>(
				...
				.def(
					...
					"Sets the list of internal map according to the passed "
					"list of "
					"map initializers (Current maps' content will be "
					"deleted!).")
				.def(
				...
				.def(
					...
					"If the map is a simple point map or it's a multi-metric "
					"map "
	...
	// CMultiMetricMapPDF
	{
		...
		scope s = class_<CMultiMetricMapPDF>("CMultiMetricMapPDF", init<>())
					  ...
					  .def(
						  ...
						  "Returns the estimate of the robot pose as a "
						  "particles PDF for "
						  "the instant of time \"timeStep\", from 0 to N-1.")
					  .def(
