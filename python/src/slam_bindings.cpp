		...
		scope s =
			class_<CICP>("CICP", init<CICP::TConfigParams>())
				...
				.def(
					...
					"(COccupancyGridMap2D/CSimplePointsMap): the relative pose "
					"of "
					"m2 with respect to m1. This pose is returned as a PDF "
					"rather "
					"than a single value.")
				.def(
		...
		scope s =
			class_<CMetricMapBuilderWrap, boost::noncopyable>(
				...
				.def(
					...
					"poses) "
					"estimation is dumped to an image file.")
				.def(
		...
		scope s =
			class_<CRangeBearingKFSLAM2D, boost::noncopyable>(
				...
				.def(
					...
					"and "
					"the robot 3D position according to the current filter "
					"state.")
				.def(
		...
		scope s =
			class_<CMonteCarloLocalization2D, boost::noncopyable,
				...
				.def(
					...
					"the "
					"current contents of out_obj, but append new OpenGL "
					"objects to "
					"that list).")
				.def(
