...
// CICP
tuple CICP_AlignPDF1(CICP &self, COccupancyGridMap2D &m1, CSimplePointsMap &m2,
...
tuple CICP_AlignPDF2(CICP &self, CSimplePointsMap &m1, CSimplePointsMap &m2,
                     CPosePDFGaussian &initialEstimationPDF) {
  CPosePDFGaussian posePDF;
  float runningTime;
  CICP::TReturnInfo info;
...
struct CMetricMapBuilderWrap : CMetricMapBuilder, wrapper<CMetricMapBuilder> {
  void initialize(const CSimpleMap &initialMap = CSimpleMap(),
                  CPosePDF *x0 = nullptr) {
  ...
  void processActionObservation(CActionCollection &action,
  ...
  void saveCurrentEstimationToImage(const std::string &file,
...
void CMetricMapBuilderICP_initialize(CMetricMapBuilderICP &self,
                                     CSimpleMap &initialMap,
                                     CPosePDFGaussian &x0) {
  self.initialize(initialMap, &x0);
}

void CMetricMapBuilderRBPF_initialize(CMetricMapBuilderRBPF &self,
                                      CSimpleMap &initialMap,
                                      CPosePDFParticles &x0) {
  self.initialize(initialMap, &x0);
}

CPose3DPDFParticles::Ptr
CMetricMapBuilderRBPF_getCurrentPoseEstimation(CMetricMapBuilderRBPF &self) {
...
tuple CRangeBearingKFSLAM2D_getCurrentState(CRangeBearingKFSLAM2D &self) {
  ...
  self.getCurrentState(out_robotPose, out_landmarksPositions, out_landmarkIDs,
                       out_fullState, out_fullCovariance);
...
CPosePDFGaussian
CRangeBearingKFSLAM2D_getCurrentRobotPose(CRangeBearingKFSLAM2D &self) {
  CPosePDFGaussian out_robotPose;
...
CRangeBearingKFSLAM2D_getAs3DObject(CRangeBearingKFSLAM2D &self) {
...
void CMonteCarloLocalization2D_prediction_and_update_pfStandardProposal(
  ...
  self.prediction_and_update_pfStandardProposal(action.get(), observation.get(),
                                                PF_options);
}
...
CPose2D
CMonteCarloLocalization2D_drawSingleSample(CMonteCarloLocalization2D &self) {
  CPose2D outPart;
...
CPosePDFParticles
CMonteCarloLocalization2D_inverse(CMonteCarloLocalization2D &self) {
  CPosePDFParticles o;
  self.inverse(o);
  return o;
}
...
mrpt::opengl::CSetOfObjects::Ptr
CMonteCarloLocalization2D_getAs3DObject(CMonteCarloLocalization2D &self) {
  mrpt::opengl::CSetOfObjects::Ptr outObj =
      std::make_shared<mrpt::opengl::CSetOfObjects>();
  self.getAs3DObject(outObj);
  return outObj;
...
CPose3D
CMonteCarloLocalization3D_drawSingleSample(CMonteCarloLocalization3D &self) {
  CPose3D outPart;
  self.drawSingleSample(outPart);
  return outPart;
}
...
CPose3DPDFParticles
CMonteCarloLocalization3D_inverse(CMonteCarloLocalization3D &self) {
...
mrpt::opengl::CSetOfObjects::Ptr
CMonteCarloLocalization3D_getAs3DObject(CMonteCarloLocalization3D &self) {
  mrpt::opengl::CSetOfObjects::Ptr outObj =
      std::make_shared<mrpt::opengl::CSetOfObjects>();
  self.getAs3DObject(outObj);
  return outObj;
...
void export_slam() {
  ...
  {
    scope s =
        class_<CICP>("CICP", init<CICP::TConfigParams>())
            .def_readwrite("options", &CICP::options)
            .def("AlignPDF", &CICP_AlignPDF1,
                 "This method computes the PDF of the displacement (relative "
                 "pose) between two maps "
                 "(COccupancyGridMap2D/CSimplePointsMap): the relative pose of "
                 "m2 with respect to m1. This pose is returned as a PDF rather "
                 "than a single value.")
            .def("AlignPDF", &CICP_AlignPDF2,
                 "This method computes the PDF of the displacement (relative "
                 "pose) between two maps (CSimplePointsMap/CSimplePointsMap): "
                 "the relative pose of m2 with respect to m1. This pose is "
                 "returned as a PDF rather than a single value.");

    class_<CICP::TConfigParams, bases<CLoadableOptions>>("TConfigParams",
                                                         init<>())
        .def_readwrite("ICP_algorithm", &CICP::TConfigParams::ICP_algorithm)
        .def_readwrite("onlyClosestCorrespondences",
                       &CICP::TConfigParams::onlyClosestCorrespondences)
        .def_readwrite("onlyUniqueRobust",
                       &CICP::TConfigParams::onlyUniqueRobust)
        .def_readwrite("maxIterations", &CICP::TConfigParams::maxIterations)
        .def_readwrite("minAbsStep_trans",
                       &CICP::TConfigParams::minAbsStep_trans)
        ...
        .def_readwrite("smallestThresholdDist",
                       &CICP::TConfigParams::smallestThresholdDist)
        .def_readwrite("covariance_varPoints",
                       &CICP::TConfigParams::covariance_varPoints)
        .def_readwrite("doRANSAC", &CICP::TConfigParams::doRANSAC)
        .def_readwrite("ransac_minSetSize",
                       &CICP::TConfigParams::ransac_minSetSize)
        .def_readwrite("ransac_maxSetSize",
                       &CICP::TConfigParams::ransac_maxSetSize)
        .def_readwrite("ransac_nSimulations",
        ...
        .def_readwrite("normalizationStd",
                       &CICP::TConfigParams::normalizationStd)
        .def_readwrite("ransac_fuseByCorrsMatch",
                       &CICP::TConfigParams::ransac_fuseByCorrsMatch)
        .def_readwrite("ransac_fuseMaxDiffXY",
                       &CICP::TConfigParams::ransac_fuseMaxDiffXY)
        .def_readwrite("ransac_fuseMaxDiffPhi",
        ...
        .def_readwrite("Axy_aprox_derivatives",
                       &CICP::TConfigParams::Axy_aprox_derivatives)
        .def_readwrite("LM_initial_lambda",
                       &CICP::TConfigParams::LM_initial_lambda)
        .def_readwrite("skip_cov_calculation",
                       &CICP::TConfigParams::skip_cov_calculation)
        //          .def_readwrite("skip_quality_calculation",
        //          &CICP::TConfigParams::skip_quality_calculation)
        .def_readwrite("corresponding_points_decimation",
  ...
  // CMetricMapBuilderICP
  {
    scope s =
        class_<CMetricMapBuilderICP, bases<CMetricMapBuilder>,
            ...
            .def("initialize", &CMetricMapBuilderICP_initialize,
                 "Initialize the method, starting with a known location PDF "
                 "\"x0\"(if supplied, set to nullptr to left unmodified) and a "
                 "given fixed, past map.")
  ...
  // CMetricMapBuilderRBPF
  {
    scope s =
        class_<CMetricMapBuilderRBPF, bases<CMetricMapBuilder>,
            ...
            .def("getCurrentPoseEstimation",
                 &CMetricMapBuilderRBPF_getCurrentPoseEstimation,
                 "Returns a copy of the current best pose estimation as a pose "
                 "PDF.")
            .def("drawCurrentEstimationToImage",
                 &CMetricMapBuilderRBPF::drawCurrentEstimationToImage)
            .def("initialize", &CMetricMapBuilderRBPF_initialize,
                 "Initialize the method, starting with a known location PDF "
                 "\"x0\"(if supplied, set to nullptr to left unmodified) and a "
                 "given fixed, past map.")
            .def("saveCurrentPathEstimationToTextFile",
                 &CMetricMapBuilderRBPF::saveCurrentPathEstimationToTextFile)
            .def("getCurrentJointEntropy",
    ...
    class_<CMetricMapBuilderRBPF::TConstructionOptions,
        ...
        .def_readwrite(
            "insertionLinDistance",
            &CMetricMapBuilderRBPF::TConstructionOptions::insertionLinDistance)
        .def_readwrite(
            "insertionAngDistance",
            &CMetricMapBuilderRBPF::TConstructionOptions::insertionAngDistance)
        .def_readwrite(
            "localizeLinDistance",
            &CMetricMapBuilderRBPF::TConstructionOptions::localizeLinDistance)
        .def_readwrite(
            "localizeAngDistance",
            &CMetricMapBuilderRBPF::TConstructionOptions::localizeAngDistance)
        .def_readwrite("PF_options",
        ...
        .def_readwrite(
            "predictionOptions",
            &CMetricMapBuilderRBPF::TConstructionOptions::predictionOptions);
  }
  ...
  // CRangeBearingKFSLAM2D
  {
    scope s =
        class_<CRangeBearingKFSLAM2D, boost::noncopyable>(
            ...
            .def("reset", &CRangeBearingKFSLAM2D::reset,
                 "Reset the state of the SLAM filter: The map is emptied and "
                 "the robot put back to (0,0,0).")
            .def("processActionObservation",
            ...
            .def("getCurrentState", &CRangeBearingKFSLAM2D_getCurrentState,
                 "Returns the complete mean and cov.")
            .def("getNumberOfLandmarksInTheMap",
                 &CRangeBearingKFSLAM2D::getNumberOfLandmarksInTheMap,
                 "Get number of landmarks in the map.")
            .def("getCurrentRobotPose",
                 &CRangeBearingKFSLAM2D_getCurrentRobotPose,
                 "Returns the mean & 3x3 covariance matrix of the robot 2D "
                 "pose.")
            .def("getAs3DObject", &CRangeBearingKFSLAM2D_getAs3DObject,
                 "Returns a 3D representation of the landmarks in the map and "
                 "the robot 3D position according to the current filter state.")
            .def("loadOptions", &CRangeBearingKFSLAM2D::loadOptions,
    ...
    // TConstructionOptions
    class_<CRangeBearingKFSLAM2D::TOptions, bases<CLoadableOptions>>("TOptions",
                                                                     init<>())
        .def_readwrite("stds_Q_no_odo",
                       &CRangeBearingKFSLAM2D::TOptions::stds_Q_no_odo)
        .def_readwrite("std_sensor_range",
                       &CRangeBearingKFSLAM2D::TOptions::std_sensor_range)
        .def_readwrite("std_sensor_yaw",
        ...
        .def_readwrite("create_simplemap",
  ...
  // TKLDParams
  {
    class_<TKLDParams, bases<CLoadableOptions>>("TKLDParams", init<>())
        ...
        .def_readwrite("KLD_minSamplesPerBin",
                       &TKLDParams::KLD_minSamplesPerBin);
  }

  // TMonteCarloLocalizationParams
  {
    class_<TMonteCarloLocalizationParams>("TMonteCarloLocalizationParams",
                                          init<>())
  ...
  // CMonteCarloLocalization2D
  {
    scope s =
        class_<CMonteCarloLocalization2D, boost::noncopyable,
               bases<CParticleFilterCapable>>("CMonteCarloLocalization2D",
                                              init<optional<size_t>>())
            .def("resetUniformFreeSpace",
            ...
            .def(
                ...
                "Update the m_particles, predicting the posterior of robot "
                "pose and map after a movement command.")
            .def("clear", &CMonteCarloLocalization2D::clear,
                 "Free all the memory associated to m_particles, and set the "
                 "number of parts = 0.")
            .def("copyFrom", &CMonteCarloLocalization2D::copyFrom,
                 "Copy operator, translating if necesary (for example, between "
                 "m_particles and gaussian representations).")
            .def("resetDeterministic",
                 &CMonteCarloLocalization2D::resetDeterministic,
                 CMonteCarloLocalization2D_resetDeterministic_overloads())
            .def("resetUniform", &CMonteCarloLocalization2D::resetUniform,
                 CMonteCarloLocalization2D_resetUniform_overloads())
            .def("getMean", &CMonteCarloLocalization2D_getMean,
                 "Returns an estimate of the pose, (the mean, or mathematical "
                 "expectation of the PDF).")
            .def("getCovarianceAndMean",
                 &CMonteCarloLocalization2D_getCovarianceAndMean,
                 "Returns an estimate of the pose covariance matrix (3x3 cov "
                 "matrix) and the mean, both at once.")
            .def("getParticlePose", &CMonteCarloLocalization2D::getParticlePose,
                 "Returns the pose of the i'th particle.")
            .def("saveToTextFile", &CMonteCarloLocalization2D::saveToTextFile,
                 "Save PDF's m_particles to a text file.")
            .def(
                "size", &CMonteCarloLocalization2D::size,
                "Get the m_particles count (equivalent to \"particlesCount\").")
            .def("changeCoordinatesReference",
                 &CMonteCarloLocalization2D::changeCoordinatesReference,
                 "this = p (+) this.")
            .def("drawSingleSample",
            ...
            .def(self += CPose2D())
            .def("append", &CMonteCarloLocalization2D::append,
                 "Appends (add to the list) a set of m_particles to the "
                 "existing ones, and then normalize weights.")
            .def("inverse", &CMonteCarloLocalization2D_inverse,
                 "Returns a new PDF such as: NEW_PDF = (0,0,0) - THIS_PDF.")
            .def("getMostLikelyParticle",
                 &CMonteCarloLocalization2D::getMostLikelyParticle,
                 "Returns the particle with the highest weight.")
            .def("getAs3DObject", &CMonteCarloLocalization2D_getAs3DObject,
            ...
            .def("readParticlesFromStream",
                 &CMonteCarloLocalization2D_readParticlesFromStream,
                 "Reads the sequence of particles and their weights from a "
                 "stream (requires T implementing CSerializable).")
            .def("getWeights", &CMonteCarloLocalization2D::getWeights,
                 "Returns a vector with the sequence of the logaritmic weights "
                 "of all the samples.")
            .def("getW", &CMonteCarloLocalization2D::getW,
                 "Access to i'th particle (logarithm) weight, where first one "
                 "is index 0.")
            .def("setW", &CMonteCarloLocalization2D::setW,
                 "Modifies i'th particle (logarithm) weight, where first one "
                 "is index 0.")
            .def("particlesCount", &CMonteCarloLocalization2D::particlesCount,
                 "Get the m_particles count.")
            .def("normalizeWeights",
                 &CMonteCarloLocalization2D_normalizeWeights,
                 "Normalize the (logarithmic) weights, such as the maximum "
                 "weight is zero.")
            .def("ESS", &CMonteCarloLocalization2D::ESS,
                 "Returns the normalized ESS (Estimated Sample Size), in the "
                 "range [0,1].")
            .def("performSubstitution",
                 ...
                 "necesary particles only and allowing the number of particles "
                 "to change.")
            .def("prediction_and_update",
                 &CMonteCarloLocalization2D_prediction_and_update,
                 "Returns the normalized ESS (Estimated Sample Size), in the "
                 "range [0,1].")
            .def("performResampling",
            ...
            .def_readwrite("m_particles",
                           &CMonteCarloLocalization2D::m_particles);
  }
  ...
  // CMonteCarloLocalization3D
  {
    scope s =
        class_<CMonteCarloLocalization3D, boost::noncopyable,
               bases<CParticleFilterCapable>>("CMonteCarloLocalization3D",
                                              init<optional<size_t>>())
            .def(
                "prediction_and_update_pfStandardProposal",
            ...
            .def(
                ...
                "Update the m_particles, predicting the posterior of robot "
                "pose and map after a movement command.")
            .def("copyFrom", &CMonteCarloLocalization3D::copyFrom,
                 "Copy operator, translating if necesary (for example, between "
                 "m_particles and gaussian representations).")
            .def("resetDeterministic",
                 &CMonteCarloLocalization3D::resetDeterministic,
                 CMonteCarloLocalization2D_resetDeterministic_overloads())
            .def("getMean", &CMonteCarloLocalization3D_getMean,
                 "Returns an estimate of the pose, (the mean, or mathematical "
                 "expectation of the PDF).")
            .def("getCovarianceAndMean",
                 ...
                 "Returns an estimate of the pose covariance matrix (3x3 cov "
                 "matrix) and the mean, both at once.")
            .def("getParticlePose", &CMonteCarloLocalization3D::getParticlePose,
                 "Returns the pose of the i'th particle.")
            .def("saveToTextFile", &CMonteCarloLocalization3D::saveToTextFile,
                 "Save PDF's m_particles to a text file.")
            .def(
            ...
            .def("drawSingleSample",
                 ...
                 "weights are assumed to be normalized!).")
            .def(self += CPose3D())
            .def("append", &CMonteCarloLocalization3D::append,
                 "Appends (add to the list) a set of m_particles to the "
                 "existing ones, and then normalize weights.")
            .def("inverse", &CMonteCarloLocalization3D_inverse,
                 "Returns a new PDF such as: NEW_PDF = (0,0,0) - THIS_PDF.")
            .def("getMostLikelyParticle",
                 &CMonteCarloLocalization3D::getMostLikelyParticle,
                 "Returns the particle with the highest weight.")
            .def("getAs3DObject", &CMonteCarloLocalization3D_getAs3DObject,
                 "Returns a 3D representation of this PDF (it doesn't clear "
                 "the current contents of out_obj, but append new OpenGL "
                 "objects to that list).")
            .def("getCovarianceEntropy",
                 ...
                 "Compute the entropy of the estimated covariance matrix.")
            .def("clearParticles", &CMonteCarloLocalization3D::clearParticles,
                 "Free the memory of all the particles and reset the array "
                 "\"m_particles\" to length zero.")
            .def("writeParticlesToStream",
            ...
            .def("readParticlesFromStream",
                 ...
                 "Reads the sequence of particles and their weights from a "
                 "stream (requires T implementing CSerializable).")
            .def("getWeights", &CMonteCarloLocalization3D::getWeights,
                 "Returns a vector with the sequence of the logaritmic weights "
                 "of all the samples.")
            .def("getW", &CMonteCarloLocalization3D::getW,
                 "Access to i'th particle (logarithm) weight, where first one "
                 "is index 0.")
            .def("setW", &CMonteCarloLocalization3D::setW,
                 "Modifies i'th particle (logarithm) weight, where first one "
                 "is index 0.")
            .def("particlesCount", &CMonteCarloLocalization3D::particlesCount,
                 "Get the m_particles count.")
            .def("normalizeWeights",
                 &CMonteCarloLocalization3D_normalizeWeights,
                 "Normalize the (logarithmic) weights, such as the maximum "
                 "weight is zero.")
            .def("ESS", &CMonteCarloLocalization3D::ESS,
                 "Returns the normalized ESS (Estimated Sample Size), in the "
                 "range [0,1].")
            .def("performSubstitution",
            ...
            .def_readwrite("m_particles",
                           &CMonteCarloLocalization3D::m_particles);
  }
}
