...
#define MAKE_PTR(class_name)                                                   \
  class_<class_name::Ptr>(STRINGIFY(class_name::Ptr),                          \
      ...
      .def("pointer", &class_name##Ptr_pointer,                                \
           return_internal_reference<>());

#define MAKE_PTR_NAMED(class_name, ptr_name)                                   \
  class_<class_name::Ptr>(STRINGIFY(ptr_name::Ptr),                            \
                          "class_name smart pointer type", no_init)            \
      .def("ctx", &class_name##Ptr_get_ctx, return_internal_reference<>())     \
      .def("ctx", &class_name##Ptr_set_ctx)                                    \
      .def("pointer", &class_name##Ptr_pointer,                                \
...
#define MAKE_CREATE(class_name)                                                \
  .def("Create", std::make_shared<class_name>,                                 \
...
#define MAKE_PTR_CTX(class_name)                                               \
  class_name &class_name##Ptr_get_ctx(class_name::Ptr &self) { return *self; } \
...
template <class T> struct StlListLike {
