...
void TestImageFFT() {
  ...
  // ====================  1  ===================
  IM1.loadFromFile(myDataDir + string("fft2_test_image_patch.jpg"),
                   0);                                            // "Patch"
  IM2.loadFromFile(myDataDir + string("fft2_test_image.jpg"), 0); // Ref. image

  // Method I:
  printf("Computing images correlation %ux%u (FFT)...",
  ...
  for (nTimes = 0; nTimes < N_TIMES; nTimes++) {
    IM2.cross_correlation_FFT(IM1, imgCorr, searchWindow_x, searchWindow_y,
  ...
  // -----------------------------------------
  printf("Computing images correlation %ux%u (OpenCV)...",
         (unsigned)IM1.getWidth(), (unsigned)IM1.getHeight());
  tictac.Tic();
  for (nTimes = 0; nTimes < N_TIMES; nTimes++) {
    IM2.cross_correlation(IM1, u, v, valMax, searchWindow_x, searchWindow_y,
                          searchWindow_lx, searchWindow_ly);
  }
  printf(" Done,%.06fms\n", tictac.Tac() * 1000.0f / N_TIMES);
  ...
  IM2.cross_correlation(IM1, x_max, y_max, corr_max, searchWindow_x,
                        searchWindow_y, searchWindow_lx, searchWindow_ly,
                        &imgCorr2);
...

  {
    ...
    CImage aux(max(IM1.getWidth(), IM2.getWidth()),
               max(IM1.getHeight(), IM2.getHeight()));

    aux.drawImage(0, 0, IM2);
    aux.rectangle(u, v, u - IM1.getWidth() / 2, v - IM1.getHeight() / 2,
                  TColor::white, 2);
...
void TestFFT_2D_real() {
  ...
  printf("Computing 2D FFT of %ux%u...", (unsigned int)A.getRowCount(),
  ...
  printf("Computing 2D IFFT of %ux%u...", (unsigned int)A.getRowCount(),
         (unsigned int)A.getColCount());
  tictac.Tic();
...
void TestFFT_2D_complex() {
  ...
  printf("Computing 2D complex FFT of %ux%u...",
         (unsigned int)DATA_R.getRowCount(),
         (unsigned int)DATA_R.getColCount());
  tictac.Tic();
  ...
  printf("Computing 2D complex IFFT of %ux%u...",
         (unsigned int)DATA_R.getRowCount(),
         (unsigned int)DATA_R.getColCount());
  tictac.Tic();
