...
void TestVideoBuildPyr() {
  ...
  std::vector<COpenGLViewport::Ptr> gl_views(N_OCTAVES);
  {
    ...
    // Assign sizes:
    //  It can be shown mathematically than if we want all viewports to be one
    //  next to each other
    //  horizontally so they fit to the viewport width (="1") and each is the
    ...
    for (size_t i = 0; i < N_OCTAVES; i++) {
      ...
      // cout << "Created viewport " << i << " at X=" << X << " with Width=" <<
      // W << endl;
      X += W;
      W *= 0.5;
    }
  ...
  win.addTextMessage(
      0.51,
      5, // X,Y<=1 means coordinates are factors over the entire viewport area.
  ...
  while (win.isOpen()) {
    ...
    if (obs) {
      if (IS_CLASS(obs, CObservationImage)) {
        ...
        // Update pyramid:
        imgpyr.buildPyramidFast(o->image, // This image is destroyed since we
                                          // are calling the *Fast() version
        ...
        if (do_features) {
          ...
          for (unsigned int level = 0; level < N_OCTAVES; ++level) {
            ...
            CFeatureExtraction::detectFeatures_SSE2_FASTER12(gray_img, feats,
                                                             threshold);
        ...
        win.addTextMessage(
            0.51, 25, // X,Y<=1 means coordinates are factors over the entire
                      // viewport area.
            format("Smooth=%i Grayscale=%i Features=%i", int(do_smooth ? 1 : 0),
                   int(do_grayscale ? 1 : 0), int(do_features ? 1 : 0)),
            TColorf(.8, .8, 0), "sans", 10, // font name & size
            mrpt::opengl::FILL,
            11 // An arbitrary ID to always overwrite the same, previous 2D text
               // message
            );
