...
void stereo2rawlog(bool is_kitti_dataset, const string &src_path0,
                   const string &src_path1, const string &calib_file,
  ...
  */
  ...
  const mrpt::poses::CPose3DQuat l2r_pose(baseline, 0.0, 0.0,
  ...
  for (int i = 0;; i++) {
    ...
    if (is_kitti_dataset) {
      ...
      sTrgImg_L =
          mrpt::format("%s/%s", out_imgs_dir.c_str(), sTrgImgFile_L.c_str());
      sTrgImg_R =
          mrpt::format("%s/%s", out_imgs_dir.c_str(), sTrgImgFile_R.c_str());
    } else {
      // I1_000000.png
      // I2_002570.png
  ...
  try {
    if (argc != 4 && argc != 5) {
      cerr << "Usage:\n"
              ...
              "[OUTPUT_NAME]\n"
              "KITTI seqs:\n"
              " kitti_dataset2rawlog  [PATH_TO_IMAGE_00] [PATH_TO_IMAGE_01] "
