  ...
  try {
    ...
    do {
      ...
      case 'j': // turn left
        srv = c == 'a' ? SRV1 : SRV2;
        cangle += 5;
        mrpt::utils::keep_min(cangle, eNeckBoard.getTruncateFactor() *
                                          90); // Saturation
        eNeckBoard.setAngle(DEG2RAD(cangle), srv);
        std::cout << "Turning servo " << srv + 1 << " 5deg to the left"
                  << std::endl;
        break;
      ...
      case 'l': // turn right
        srv = c == 'd' ? SRV1 : SRV2;
        cangle -= 5;
        mrpt::utils::keep_max(cangle, eNeckBoard.getTruncateFactor() *
                                          (-90)); // Saturation
        eNeckBoard.setAngle(DEG2RAD(cangle), srv);
        std::cout << "Turning servo " << srv + 1 << " 5deg to the right"
                  << std::endl;
        break;
      ...
      case 'h': // complete run
        ...
        eNeckBoard.setAngle(DEG2RAD(90), srv);
        std::this_thread::sleep_for(200ms);
        std::cout << "Performing a complete run in servo " << srv + 1
                  << std::endl;
        break;
      ...
      case 'y': // read angle
        ...
        if (eNeckBoard.getCurrentAngle(nangle), srv)
          std::cout << "Current angle of servo " << srv + 1 << " is "
                    << RAD2DEG(nangle) << "deg" << std::endl;
        else
