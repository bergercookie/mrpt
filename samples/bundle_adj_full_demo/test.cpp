...
/* ===========================================================================
    ...
    PURPOSE: Demonstrate "mrpt::vision::bundle_adj_full" with a set of
                  simulated or real data. If the program is called without
   command
                          line arguments, simulated measurements will be used.
                          To use real data, invoke:
                            bundle_adj_full_demo  <feats.txt> <cam_model.cfg>

                          Where <feats.txt> is a "TSequenceFeatureObservations"
   saved as
                          a text file, and <cam_model.cfg> is a .ini-like file
   with a
                          section named "CAMERA" loadable by
   mrpt::utils::TCamera.
...
void my_BundleAdjustmentFeedbackFunctor(
  ...
  if ((cur_iter % 10) == 0) {
    cout << "[PROGRESS] Iter: " << cur_iter << " avrg err in px: " << avr_err
         << endl;
    cout.flush();
  }
}
...
void bundle_adj_full_demo(const TCamera &camera_params,
  ...
  mrpt::vision::bundle_adj_full(allObs, camera_params, frame_poses,
                                landmark_points, extra_params,
                                &my_BundleAdjustmentFeedbackFunctor);
}
// ---------------------------------------------------------

mrpt::opengl::CSetOfObjects::Ptr
framePosesVecVisualize(const TFramePosesVec &poses, const double len,
  ...
  try {
    ...
    if (argc == 1) {
      ...
      for (double x = -cameraPathLen; x < cameraPathLen; x += camPosesSteps) {
        ...
        p.pitch = 0;
        p.roll = 0;
        // Angles above is for +X pointing to the (0,0,0), but we want instead
        // +Z pointing there, as typical in camera models:
        frame_poses_real.push_back(
            CPose3D(p) + CPose3D(0, 0, 0, DEG2RAD(-90), 0, DEG2RAD(-90)));
      }
      ...
      for (size_t i = 0; i < frame_poses_real.size(); i++) // for each pose
      {
        ...
        for (size_t j = 0; j < landmark_points_real.size(); j++) {
          TPixelCoordf px =
              mrpt::vision::pinhole::projectPoint_no_distortion<false>(
                  camera_params, frame_poses_real[i], landmark_points_real[j]);

          const bool is_outlier =
              (rg.drawUniform(0.0, 1.0) < PROBABILITY_OUTLIERS);
          px.x += rg.drawGaussian1D(0, is_outlier ? STD_PX_ERROR_OUTLIER
                                                  : STD_PX_ERROR);
          px.y += rg.drawGaussian1D(0, is_outlier ? STD_PX_ERROR_OUTLIER
                                                  : STD_PX_ERROR);
          ...
          // Too far?
          const double dist = math::distance(TPoint3D(frame_poses_real[i]),
      ...
      for (size_t i = 0; i < landmark_points_noisy.size(); i++)
        landmark_points_noisy[i] += TPoint3D(rg.drawGaussian1D(0, STD_PT3D),
      ...
      for (size_t i = 0; i < landmark_points.size(); i++)
        landmarks_total_sq_err +=
            square(landmark_points_real[i].distanceTo(landmark_points[i]));
      ...
      if (argc == 3) {
        ...
        mrpt::vision::ba_initial_estimate(allObs, camera_params, frame_poses,
                                          landmark_points);
        cout << "OK\n";

      } else {
        // Load data from 3 files in the same format as used by "eucsbademo" in
        // the SBA library:
...

        {
          ...
          while (fil.getNextLine(ss)) {
            ...
            mrpt::poses::CPose3DQuat p(
                t[0], t[1], t[2],
                mrpt::math::CQuaternionDouble(q[0], q[1], q[2], q[3]));
            // cout << "cam: " << p << endl;
          ...
          frame_poses_noisy =
              frame_poses; // To draw in 3D the initial values as well.
        }

        {
          ...
          while (fil.getNextLine(ss)) {
            ...
            for (size_t i = 0; i < N; i++) {
              ...
              // cout << "feat: " << feat_id << " cam: " << frame_id << " px: "
              // << px.x << "," << px.y << endl;
            }
          }
          ...
          landmark_points_real =
              landmark_points; // To draw in 3D the initial values as well.
        }
    ...
    { // Ground plane:
      CGridPlaneXY::Ptr obj =
          std::make_shared<CGridPlaneXY>(-200, 200, -200, 200, 0, 5);
      obj->setColor(0.7, 0.7, 0.7);
      scene->insert(obj);
    }

    if (!landmark_points_real.empty()) { // Feature points: ground truth
    ...
    if (!landmark_points_noisy.empty()) { // Feature points: noisy
      CPointCloud::Ptr obj = std::make_shared<CPointCloud>();
      obj->setPointSize(4);
