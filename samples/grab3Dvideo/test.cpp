...
void TestDisplay3D() {
  ...
  while (!end && win.isOpen()) {
    ...
    obj1->setLocation(obj1->getPoseX() + cos(obj1->getPoseY() / 2) * 0.05,
    ...
    win.addTextMessage(0.02, 0.98, format("ball#1 pos: %.02f %.02f %.02f ",
                                          obj1->getPoseX(), obj1->getPoseY(),
                                          obj1->getPoseZ()),
                       mrpt::utils::TColorf(0, 0, 1),
                       10, // An arbitrary ID to always overwrite the same,
    ...
    if (img) {
      ...
      img->saveToFile(s);
      printf("Saved frame image to: %s \r", s.c_str()); // "\ r" is to overwrite
                                                        // the same line over
                                                        // and over again..
    }
