...
void clear_memory()
{
	for (map<int, CDisplayWindowPlots*>::iterator iter = Win_map.begin();
		 iter != Win_map.end(); ++iter)
	{
		delete iter->second;
		iter->second = nullptr;
	}
...
void add_VLines()
{
	...
	for (float inc = 0; inc <= 5; inc += 0.05)
	{
		...
		for (size_t i = 0; i < sizeof(Vbaselines) / 4; i++)
		{
			...
			Win_map[1]->plot(
				Vertical_lines_map[Vbaselines[i]], Vline, "k.1", sensor_name);
		}
	}
}
...
void plot_diff_windows(std::map<int, vector<float>>& sensors, bool together)
{
	...
	try
	{
		if (together)  // All sensor on same window
		{
			...
			// Plot
			for (map<int, std::vector<float>>::iterator iter = sensors.begin();
				 iter != sensors.end(); ++iter)
			{
				if (iter->first == 0xFFFF)
				{  // Temperature
					Win_map[0]->plot(sensors[0x0000], iter->second, "b-3");
				}
				else if (iter->first != (0x0000))
				{  // Is not Time
					Win_map[1]->plot(
						sensors[0x0000], iter->second, colors[iter->first],
						format("%i", int(iter->first)));
				}
			}
		}
		else  // Eah sensor has his own window
		{
			if (firstTime)
			{
				...
				int posx = 0;
				int posy = 0;
				for (map<int, std::vector<float>>::iterator iter =
						 sensors.begin();
					 iter != sensors.end(); ++iter)
				{
					if (iter->first != (0x0000))
					{  // Is not Time
						Win_map[iter->first] = new CDisplayWindowPlots(
							format("Sensor: %i", int(iter->first)));
				 ...
				 iter != sensors.end(); ++iter)
			{
				if (iter->first != (0x0000))
				{  // Is not Time
					Win_map[iter->first]->plot(
						sensors[0x0000], sensors[iter->first], "b-3");
				}
			}
		}  // End-if(together)
	}
...
void add_baseline(std::vector<float> time)
{
	try
	{
		...
		for (size_t i = 0; i < sizeof(Hbaselines) / 4; i++)
		{
			...
			string name(sensor_name);
			Win_map[1]->plot(
				time, Baseline_map[Hbaselines[i]], "k.1", sensor_name);
		}
	}
	catch (std::exception& e)
	{
...
int main()
{
	try
	{
		...
		FILE* f_log = os::fopen(
			format(
				"./log_%s.txt", fileNameStripInvalidChars(
									mrpt::system::dateTimeLocalToString(now()))
									.c_str()),
			"wt");
		TTimeStamp timStart = mrpt::system::getCurrentTime();
		std::map<int, std::vector<float>> Sensor_array;
		bool together =
			true;  // True -> Shows all sensor on same window. False ->
		// Each sensor has his own window
		...
		while (!mrpt::system::os::kbhit())
		{
			...
			else
			{
				...
				for (size_t i = 0; i < obs.m_readings.size(); i++)
				{
					...
					Sensor_array[0xFFFF].push_back(
						obs.m_readings[i].temperature);
					if (f_log)
						fprintf(f_log, "%f ", obs.m_readings[i].temperature);

					// E-Nose Sensor's Data
					for (size_t j = 0; j < obs.m_readings[i].sensorTypes.size();
						 j++)
					{
						...
						{  // Not the las item
							...
							{
								Sensor_array[obs.m_readings[i].sensorTypes[j]]
									.push_back(
										obs.m_readings[i]
											.readingsVoltage[j + 1] -
								...
								if (f_log)
									fprintf(
										...
										obs.m_readings[i]
											...
											obs.m_readings[i]
												.readingsVoltage[j]);
								j++;  // skip the next sensor as it has been
								// used already.
							}
							else
							{
