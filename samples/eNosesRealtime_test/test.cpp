...
void add_VLines() {
  ...
  for (float inc = 0; inc <= 5; inc += 0.05) {
    ...
    for (size_t i = 0; i < sizeof(Vbaselines) / 4; i++) {
      ...
      Win_map[1]->plot(Vertical_lines_map[Vbaselines[i]], Vline, "k.1",
                       sensor_name);
    }
  }
}
  ...
  try {
    if (together) // All sensor on same window
    {
      ...
      for (map<int, std::vector<float>>::iterator iter = sensors.begin();
        ...
        if (iter->first == 0xFFFF) { // Temperature
          ...
          Win_map[1]->plot(sensors[0x0000], iter->second, colors[iter->first],
    ...
    } else // Eah sensor has his own window
    {
      if (firstTime) {
        ...
        for (map<int, std::vector<float>>::iterator iter = sensors.begin();
          ...
          if (iter->first != (0x0000)) { // Is not Time
            Win_map[iter->first] =
                new CDisplayWindowPlots(format("Sensor: %i", int(iter->first)));
      ...
      for (map<int, std::vector<float>>::iterator iter = sensors.begin();
           iter != sensors.end(); ++iter) {
        if (iter->first != (0x0000)) { // Is not Time
          Win_map[iter->first]->plot(sensors[0x0000], sensors[iter->first],
                                     "b-3");
        }
      }
    } // End-if(together)
...
int main() {
  try {
    ...
    FILE *f_log = os::fopen(
        format("./log_%s.txt", fileNameStripInvalidChars(
    ...
    bool together = true; // True -> Shows all sensor on same window. False ->
      ...
      if (!eNoses.getObservation(obs)) // NEW OBSERVATION
      {
        ...
        for (size_t i = 0; i < obs.m_readings.size(); i++) {
          ...
          for (size_t j = 0; j < obs.m_readings[i].sensorTypes.size(); j++) {
            ...
            if (j < (obs.m_readings[i].sensorTypes.size() -
                     1)) { // Not the las item
              if (obs.m_readings[i].sensorTypes[j] ==
                  obs.m_readings[i].sensorTypes[j + 1]) {
                Sensor_array[obs.m_readings[i].sensorTypes[j]].push_back(
                ...
                if (f_log)
                  fprintf(f_log, "%f ",
                          obs.m_readings[i].readingsVoltage[j]); // To file
              }
            } else {
              Sensor_array[obs.m_readings[i].sensorTypes[j]].push_back(
                  obs.m_readings[i].readingsVoltage[j]);
