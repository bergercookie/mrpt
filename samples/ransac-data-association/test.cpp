...
// ------------------------------------------------------
void TestRANSAC() {
  mrpt::gui::CDisplayWindow3D win("MRPT example: ransac-data-association", 800,
                                  600);
  ...
  for (size_t i = 0; i < NUM_MAP_FEATS; i++) {
    the_map.setPoint(i, randomGenerator.drawUniform(0, MAP_SIZE_X),
      ...
      std::make_shared<mrpt::opengl::CSetOfLines>();
  {
    ...
    for (size_t i = 0; i < the_map.size(); i++) {
      mrpt::opengl::CText::Ptr gl_txt = std::make_shared<mrpt::opengl::CText>(
  ...
  while (win.isOpen()) {
    ...
    mrpt::tfest::se2_l2_robust(all_correspondences, normalizationStd, params,
                               results);
...

    {
      ...
      win.addTextMessage(5, 5, "Blue: map landmarks | Red: Observations | "
                               "White lines: Found correspondences",
                         mrpt::utils::TColorf(0, 0, 0), "mono", 12,
                         mrpt::opengl::NICE, 0);
      ...
      for (size_t k = 0; k < nObs; k++) {
        ...
        double obs_x, obs_y;
        solution_pose.mean.composePoint(observations[k].x, observations[k].y,
                                        obs_x, obs_y);
        ...
        sqerr += mrpt::math::distanceSqrBetweenPoints<double>(map_x, map_y,
                                                              obs_x, obs_y);
      }

      win.addTextMessage(5, 20, "Ground truth pose    : " + GT_pose.asString(),
                         mrpt::utils::TColorf(0, 0, 0), "mono", 12,
                         mrpt::opengl::NICE, 1);
      win.addTextMessage(
          5, 35,
