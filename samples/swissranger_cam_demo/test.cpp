...
void Test_SwissRanger() {
  ...
  if (cam.isOpen())
    cout << "[Test_SwissRanger] Camera open, serial #"
         << cam.getCameraSerialNumber() << " resolution: " << cam.getColCount()
         << "x" << cam.getRowCount() << " max. range: " << cam.getMaxRange()
         << endl;
  ...
  mrpt::opengl::CTexturedPlane::Ptr gl_img_range =
      std::make_shared<mrpt::opengl::CTexturedPlane>(
...

  {
    ...
    win3D.addTextMessage(30, -10 - 1 * (VW_GAP + VW_HEIGHT), "Range data",
    ...
    win3D.addTextMessage(30, -10 - 2 * (VW_GAP + VW_HEIGHT), "Intensity data",
    ...
    win3D.addTextMessage(30, -10 - 3 * (VW_GAP + VW_HEIGHT),
                         "Intensity data (undistorted)", TColorf(1, 1, 1), 3,
    ...
    viewIntRect->setViewportPosition(5, -10 - 3 * (VW_GAP + VW_HEIGHT),
                                     VW_WIDTH, VW_HEIGHT);
  ...
  while (there_is_obs && !endLoop && win3D.isOpen()) {
    ...
    if (nImgs > 10) {
      win3D.get3DSceneAndLock();
      win3D.addTextMessage(0.01, 0.01, format("%.02f Hz", nImgs / tictac.Tac()),
                           TColorf(0, 1, 1), 100,
                           MRPT_GLUT_BITMAP_HELVETICA_12);
      win3D.unlockAccess3DScene();
      nImgs = 0;
    ...
    win3D.addTextMessage(
        0.08, 0.02,
        format("Keyboard switches: H (hist.equal: %s) | G (convGray: %s) | D "
