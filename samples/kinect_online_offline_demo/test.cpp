...
struct TThreadParam {
  TThreadParam(bool _is_online, const string &_rawlog_file = string(),
               bool _generate_3D_pointcloud_in_this_thread = false)
      : is_online(_is_online), rawlog_file(_rawlog_file),
        generate_3D_pointcloud_in_this_thread(
            _generate_3D_pointcloud_in_this_thread),
        quit(false), Hz(0) {}

  const bool
      is_online; //!< true: live grabbing from the sensor, false: from rawlog
  const string rawlog_file; //!< Only when is_online==false
  const bool generate_3D_pointcloud_in_this_thread; //!< true: populate the 3D
                                                    //!point fields in the
                                                    //!output observation;
                                                    //!false: only RGB and Depth
                                                    //!images.

  volatile bool quit; //!< In/Out variable: Forces the thread to exit or
                      //!indicates an error in the thread that caused it to end.
  volatile double Hz; //!< Out variable: Approx. capturing rate from the thread.

  CObservation3DRangeScan::Ptr
      new_obs; //!< RGB+D (+ optionally, 3D point cloud)
};
    ...
    while (!p.quit) {
      if (p.is_online) {
        ...
        CObservation3DRangeScan::Ptr obs =
            std::make_shared<CObservation3DRangeScan>(); // Smart pointers to
                                                         // observations. Memory
                                                         // pooling is used
                                                         // internally
        ...
        obs3D->load(); // *Important* This is needed to load the range image if
        ...
        if (dataset_prev_tim != INVALID_TIMESTAMP &&
...
#endif
          const double At_actual =
              mrpt::system::timeDifference(my_last_read_obs_tim, now_tim);
          ...
          if (need_to_wait_ms > 0)
            std::this_thread::sleep_for(
                std::chrono::duration<double, std::milli>(need_to_wait_ms));
        }
...
void Test_KinectOnlineOffline(bool is_online,
  ...
  // --------------------------------------------------------
  TThreadParam thrPar(is_online, rawlog_file,
                      false // generate_3D_pointcloud_in_this_thread -> Don't,
                            // we'll do it in this main thread.
                      );
  ...
  opengl::COpenGLViewport::Ptr viewInt; // Extra viewports for the RGB images.
  {
    ...
    win3D.addTextMessage(10, 30 + VW_HEIGHT + 10, "Intensity data",
                         TColorf(1, 1, 1), 2,
                         mrpt::opengl::MRPT_GLUT_BITMAP_HELVETICA_12);

    win3D.addTextMessage(5, 5, format("'o'/'i'-zoom out/in, ESC: quit"),
                         TColorf(0, 0, 1), 110,
  ...
  while (win3D.isOpen() && !thrPar.quit) {
    ...
    if (newObs && newObs->timestamp != INVALID_TIMESTAMP &&
      ...
      win3D.addTextMessage(-100, -30, format("%.02f Hz", thrPar.Hz),
