...
void TestDisplay3D() {
  ...
  // Texts:
  win.addTextMessage(0.01, 0.85, "This is a 2D message", TColorf(1, 1, 1), 0,
  ...
  while (!end && win.isOpen()) {
    ...
    obj1->setLocation(R * cos(W * t) * sin(Q * t), R * sin(W * t),
                      R * cos(W * t) * cos(Q * t));

    // Update the texts on the gl display:
    win.addTextMessage(5, 5, mrpt::format("FPS=%5.02f", win.getRenderingFPS()),
