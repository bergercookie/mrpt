...
void TestDisplay3D() {
  ...
  // Another clone viewport, using absolute coordinates
  {
    ...
    vi->setViewportPosition(/*x px*/ -250, /*y px*/ -250, /*width px*/ 250,
                            /*height px*/ 200); // x,y negative means pixels
                                                // from the top/right, instead
                                                // of from the bottom/left.
    vi->setCloneView("main");
  ...
  // Texts:
  win.addTextMessage(0.01, 0.85, "This is a 2D message", TColorf(1, 1, 1),
                     "sans", 11, mrpt::opengl::NICE, 0);
  ...
  cout << "  P                          -> Enable / disable 'place object' mode"
       << endl;
  cout << endl;
  ...
  while (!end && win.isOpen()) {
    ...
    obj1->setLocation(R1 * cos(W1 * t) * sin(Q1 * t), R1 * sin(W1 * t),
                      R1 * cos(W1 * t) * cos(Q1 * t));

    const double R2 = 6;
    const double W2 = 1.3, Q2 = 7.2;
    opengl::CRenderizable::Ptr obj2 = theScene->getByName("ball_2");
    obj2->setLocation(R2 * cos(W2 * t) * sin(Q2 * t), R2 * sin(W2 * t),
                      R2 * cos(W2 * t) * cos(Q2 * t));

    win.addTextMessage(0.01, 0.85, "This is a 2D message", TColorf(1, 0, 0),
                       "sans", 8, mrpt::opengl::NICE, 0);

    win.addTextMessage(
        0.02, 0.02, // X,Y<=1 means coordinates are factors over the entire
                    // viewport area.
        format("ball#1 pos: %.02f %.02f %.02f ", obj1->getPoseX(),
    ...
    win.addTextMessage(
        5, -15, // |X|,|Y|>1 means absolute coordinates, negative means from the
        ...
        mrpt::opengl::NICE,
        20 // An arbitrary ID to always overwrite the same, previous 2D text
           // message
        );
            ...
            mouse_x, mouse_y)) // See also: getLastMousePositionRay()
    {
      ...
      theScene->getViewport("main")->get3DRayForPixelCoord(mouse_x, mouse_y,
                                                           ray);
