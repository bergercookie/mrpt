...
void printHelp() {
  ...
  cout << "         -r | R: Switch between point cloud and graph representation"
       << endl;
}
...
void keyboardEventOccurred(const pcl::visualization::KeyboardEvent &event,
  ...
  if ((event.getKeySym() == "r" || event.getKeySym() == "R") &&
      event.keyDown()) {
    cout << "r was pressed => change between regular/graph representation"
         << endl;
    graphRepresentation = !graphRepresentation;
  } else if ((event.getKeySym() == "p" || event.getKeySym() == "P") &&
             event.keyDown())
...
PbMapVisualizer::PbMapVisualizer() : cloudViewer("PbMap") {}
  ...
  // Render the data
  {
    ...
    if (graphRepresentation) {
      ...
      for (size_t i = 0; i < pbmap.vPlanes.size(); i++) {
        pcl::PointXYZ center(2 * pbmap.vPlanes[i].v3center[0],
                             2 * pbmap.vPlanes[i].v3center[1],
                             2 * pbmap.vPlanes[i].v3center[2]);
        double radius = 0.1 * sqrt(pbmap.vPlanes[i].areaVoxels);
        //        cout << "radius " << radius << endl;
        sprintf(name, "sphere%u", static_cast<unsigned>(i));
        viz.addSphere(center, radius, ared[i % 10], agrn[i % 10], ablu[i % 10],
                      name);

        if (!pbmap.vPlanes[i].label.empty())
          viz.addText3D(pbmap.vPlanes[i].label, center, 0.1, ared[i % 10],
                        agrn[i % 10], ablu[i % 10], pbmap.vPlanes[i].label);
        else {
        ...
        //        pbmap.vPlanes[i].nearbyPlanes.begin(); it !=
        //        pbmap.vPlanes[i].nearbyPlanes.end(); it++)
        //        {
        ...
        //          pcl::PointXYZ center_it(2*pbmap.vPlanes[*it].v3center[0],
        //          2*pbmap.vPlanes[*it].v3center[1],
        //          2*pbmap.vPlanes[*it].v3center[2]);
        //          viz.addLine (center, center_it, ared[i%10], agrn[i%10],
        //          ablu[i%10], name);
        //        }
        for (map<unsigned, unsigned>::iterator it =
                 pbmap.vPlanes[i].neighborPlanes.begin();
             it != pbmap.vPlanes[i].neighborPlanes.end(); it++) {
          if (it->first > pbmap.vPlanes[i].id)
            break;

          sprintf(name, "commonObs%u_%u", static_cast<unsigned>(i),
                  static_cast<unsigned>(it->first));
          pcl::PointXYZ center_it(2 * pbmap.vPlanes[it->first].v3center[0],
                                  2 * pbmap.vPlanes[it->first].v3center[1],
                                  2 * pbmap.vPlanes[it->first].v3center[2]);
          viz.addLine(center, center_it, ared[i % 10], agrn[i % 10],
                      ablu[i % 10], name);
      ...
      for (size_t i = 0; i < pbmap.vPlanes.size(); i++) {
        ...
        name[0] =
            *(mrpt::format("normal_%u", static_cast<unsigned>(i)).c_str());
        pcl::PointXYZ pt1,
        ...
        viz.addArrow(pt2, pt1, ared[i % 10], agrn[i % 10], ablu[i % 10], false,
                     name);

        if (!plane_i.label.empty())
          viz.addText3D(plane_i.label, pt2, 0.1, ared[i % 10], agrn[i % 10],
                        ablu[i % 10], plane_i.label);
        else {
...
void PbMapVisualizer::Visualize() {
  cloudViewer.runOnVisualizationThread(
      boost::bind(&PbMapVisualizer::viz_cb, this, _1), "viz_cb");
...
// pbmap_visualizer reads a pbmap and its corresponding point-cloud and
// visualizes them
int main(int argc, char **argv) {
  ...
  cout << "Size " << mapViewer.pbmap.globalMapPtr->size() << " "
