...
void printHelp()
{
	...
	cout << "         -r | R: Switch between point cloud and graph "
			"representation"
		 << endl;
}
	...
	const pcl::visualization::KeyboardEvent& event, void* viewer_void)
{
		...
		event.keyDown())
	{
		...
		(event.getKeySym() == "p" || event.getKeySym() == "P") &&
		event.keyDown())
		showPointCloud = !showPointCloud;
	else if (
		(event.getKeySym() == "l" || event.getKeySym() == "L") &&
		event.keyDown())
		showPbMap = !showPbMap;
}
	...
	// Render the data
	{
		...
		if (graphRepresentation)
		{
			...
			for (size_t i = 0; i < pbmap.vPlanes.size(); i++)
			{
				...
				viz.addSphere(
					center, radius, ared[i % 10], agrn[i % 10], ablu[i % 10],
					name);

				if (!pbmap.vPlanes[i].label.empty())
					viz.addText3D(
						pbmap.vPlanes[i].label, center, 0.1, ared[i % 10],
						agrn[i % 10], ablu[i % 10], pbmap.vPlanes[i].label);
				else
				{
					sprintf(name, "P%u", static_cast<unsigned>(i));
					viz.addText3D(
						name, center, 0.1, ared[i % 10], agrn[i % 10],
						ablu[i % 10], name);
				}
				...
				//          pcl::PointXYZ
				//          center_it(2*pbmap.vPlanes[*it].v3center[0],
				...
				{
					if (it->first > pbmap.vPlanes[i].id) break;

					sprintf(
						name, "commonObs%u_%u", static_cast<unsigned>(i),
						static_cast<unsigned>(it->first));
					pcl::PointXYZ center_it(
						2 * pbmap.vPlanes[it->first].v3center[0],
						2 * pbmap.vPlanes[it->first].v3center[1],
						2 * pbmap.vPlanes[it->first].v3center[2]);
					viz.addLine(
						center, center_it, ared[i % 10], agrn[i % 10],
						ablu[i % 10], name);

					sprintf(
						name, "edge%u_%u", static_cast<unsigned>(i),
						static_cast<unsigned>(it->first));
					char commonObs[8];
					sprintf(commonObs, "%u", it->second);
					pcl::PointXYZ half_edge(
						(center_it.x + center.x) / 2,
						(center_it.y + center.y) / 2,
						(center_it.z + center.z) / 2);
					viz.addText3D(
						commonObs, half_edge, 0.05, 1.0, 1.0, 1.0, name);
				}
			}
		}
		else
			...
			for (size_t i = 0; i < pbmap.vPlanes.size(); i++)
			{
				...
				name[0] =
					*(mrpt::format("normal_%u", static_cast<unsigned>(i))
						  .c_str());
				pcl::PointXYZ pt1,
					pt2;  // Begin and end points of normal's arrow for
				// visualization
				pt1 = pcl::PointXYZ(
				...
				viz.addArrow(
					pt2, pt1, ared[i % 10], agrn[i % 10], ablu[i % 10], false,
					name);

				if (!plane_i.label.empty())
					viz.addText3D(
						plane_i.label, pt2, 0.1, ared[i % 10], agrn[i % 10],
...
void PbMapVisualizer::Visualize()
{
	...
	while (!cloudViewer.wasStopped()) std::this_thread::sleep_for(10ms);
}
