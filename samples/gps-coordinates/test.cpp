...
void TestGPS_coords() {
  ...
  cout << "Point 0: lat=" << p0.lat << " lon=" << p0.lon << " alt=" << p0.height
       << endl;
  cout << "Point 1: lat=" << p1.lat << " lon=" << p1.lon << " alt=" << p1.height
       << endl;
  ...
  for (size_t i = 0; i < sizeof(lats) / sizeof(lats[0]); i++) {
    ...
    mrpt::topography::geodeticToUTM(topography::TGeodeticCoords(la, lo, 0),
                                    UTMCoords, zone, band);
