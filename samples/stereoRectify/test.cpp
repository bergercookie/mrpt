...
void TestStereoRectify(int argc, char** argv)
{
	...
	// selected camera.
	cout
		<< "Please, select the input stereo camera or rawlog file (with stereo "
		   "images)...\n";
	...
	CImage img_left_rectified,
		img_right_rectified;  // Declared here to serve as
	// a memory buffer (avoid
	...
	while (win.isOpen())
	{
		...
		win.addTextMessage(
			5, 25,
			format(
				"'r': Switch rectify (Now is: %s) | '+'/'-': Modify alpha "
				"(Now is: %.02f)",
		...
		win.addTextMessage(
			...
			format(
				...
				"Switch "
				"no-disparity (Now is: %s) | 'e': Switch epipolar lines",
