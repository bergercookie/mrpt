...
void TestStereoRectify(int argc, char **argv) {
  ...
  // selected camera.
  cout << "Please, select the input stereo camera or rawlog file (with stereo "
          "images)...\n";
  ...
  CImage img_left_rectified, img_right_rectified; // Declared here to serve as a
                                                  // memory buffer (avoid
  ...
  while (win.isOpen()) {
    ...
    win.addTextMessage(5, 25, format("'r': Switch rectify (Now is: %s) | "
                                     "'+'/'-': Modify alpha (Now is: %.02f)",
                                     enable_rectify ? "ON" : "OFF",
                                     rectifyMap.getAlpha()),
                       TColorf(1, 1, 1), "sans", 15, mrpt::opengl::FILL, 10);
    win.addTextMessage(
        5, 50,
        format("'s': Switch resize output to 320x240 (Now is: %s) | 'c': "
    ...
    if (obs) {
      if (IS_CLASS(obs, CObservationStereoImages)) {
        ...
        if (enable_rectify) {
          ...
          rectifyMap.rectify(o->imageLeft, o->imageRight, img_left_rectified,
                             img_right_rectified);
        ...
        if (enable_draw_epipolar_lines) {
          ...
          for (unsigned int y = 0; y < h; y += LINES_SEP) {
            img_left_rectified.line(0, y, w - 1, y, mrpt::utils::TColor::red,
                                    2);
            img_right_rectified.line(0, y, w - 1, y, mrpt::utils::TColor::red,
                                     2);
          }
        }
        ...
        win.addTextMessage(150, 5, mrpt::system::timeToString(o->timestamp),
      ...
      if (win.keyHit()) {
        ...
        if (key == 's' || key == 'S') {
          rectifyMap.enableResizeOutput(!rectifyMap.isEnabledResizeOutput(),
                                        320, 240);
        }
        if (key == 'c' || key == 'C') {
