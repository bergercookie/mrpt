...
void CostFunction(const vector_double &X, const img_seq_data &last,
                  vector_double &cost) {
  ...
  try {
    ...
    for (i = 0; i < data_img[0].tri_list[t].size(); i++) {
      ...
      xy2[0] /= xy2[2];
      xy2[1] /=
          xy2[2]; // xy2[2]/=xy2[2];			 //SET THE LAST ELEMENT TO
                  // 1.
      ...
      patch2[i] = round(
          (unsigned char)(last.img->imageData[floorS(xy2[0] - 1) +
                                              (floorS(xy2[1]) - 1) *
                                                  data_img[0].img->widthStep]) *
              (floorS(xy2[0]) + 1 - xy2[0]) * (floorS(xy2[1]) + 1 - xy2[1]) +
          (unsigned char)(last.img->imageData[floorS(xy2[0] - 1) +
                                              floorS(xy2[1]) *
                                                  data_img[0].img->widthStep]) *
              (floorS(xy2[0]) + 1 - xy2[0]) * (xy2[1] - floorS(xy2[1])) +
          (unsigned char)(last.img->imageData[floorS(xy2[0]) +
                                              (floorS(xy2[1]) - 1) *
                                                  data_img[0].img->widthStep]) *
              (xy2[0] - floorS(xy2[0])) * (floorS(xy2[1]) + 1 - xy2[1]) +
          (unsigned char)(last.img->imageData[floorS(xy2[0]) +
                                              floorS(xy2[1]) *
                                                  data_img[0].img->widthStep]) *
              (xy2[0] - floorS(xy2[0])) * (xy2[1] - floorS(xy2[1])));
    }
    if (inside_img)
...
double TriangleArea(double ax, double ay, double bx, double by, double cx,
                    double cy) {
  double area;
  area = 0.5 * sqrt((square(ax - bx) + square(ay - by)) *
...
double TriangleArea(CvPoint2D32f a, CvPoint2D32f b, CvPoint2D32f c) {
  double area;
  area =
      0.5 * sqrt((square(a.x - b.x) + square(a.y - b.y)) *
...
bool PointInTriangle(CvPoint2D32f p, CvPoint2D32f a, CvPoint2D32f b,
                     CvPoint2D32f c) {
  double cp1, cp2;
  bool s1, s2, s3;
...
void loadPatches(CFeatureList &Ptos, CMatrix &mesh, CvMat **mask,
                 triangle_list &tri, indices_list &ind, IplImage *ipl) {
  *mask = cvCreateMat(ipl->height, ipl->width,
                      CV_16UC1); // Para cuando mask es un puntero
  cvZero(*mask);
  ...
  for (int i = 1; i <= num_facets;
       i++) // CAMBIAR ESTRUCTURA, A UN UNICO FOR PARA EL RECORRIDO DE LA IMAGEN
  {
    idx1 = mesh(i - 1, 0) - 1;
    idx2 = mesh(i - 1, 1) - 1;
    idx3 = mesh(i - 1, 2) - 1;
...
void followPatch(CMatrixDouble33 &tform, vector_byte &tri, IplImage *ipl) {
  ...
  for (unsigned int i = 0; i < data_img[0].ind_list[t].size(); i++) {
    ...
    tri[i] = round(
        (unsigned char)(ipl->imageData[floorS(xy2[0] - 1) +
                                       (floorS(xy2[1]) - 1) *
                                           data_img[0].img->widthStep]) *
            (floorS(xy2[0]) + 1 - xy2[0]) * (floorS(xy2[1]) + 1 - xy2[1]) +
        (unsigned char)(ipl->imageData[floorS(xy2[0] - 1) +
                                       floorS(xy2[1]) *
                                           data_img[0].img->widthStep]) *
            (floorS(xy2[0]) + 1 - xy2[0]) * (xy2[1] - floorS(xy2[1])) +
        (unsigned char)(ipl->imageData[floorS(xy2[0]) +
                                       (floorS(xy2[1]) - 1) *
                                           data_img[0].img->widthStep]) *
            (xy2[0] - floorS(xy2[0])) * (floorS(xy2[1]) + 1 - xy2[1]) +
        (unsigned char)(ipl->imageData[floorS(xy2[0]) +
...
//	//{
//	//	sprintf( cstr, "%d", i );
//	//	img.textOut( features.getByID(i)->x, features.getByID(i)->y,
//cstr, 255 );
//	//}
...
void detectClusteres(CFeatureList &ptos, CvMat *samples, CvMat *clusters,
                     CvMat *centers, int cluster_count) {
  for (unsigned int i = 0; i < ptos.size(); i++) {
    cvSet2D(samples, i, 0, cvScalar(ptos[i]->x));
    cvSet2D(samples, i, 1, cvScalar(ptos[i]->y));
  }
  cvKMeans2(samples, cluster_count, clusters,
            cvTermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 10, 1.0), 1, 0,
            0, centers);
}
...
namespace math {
template <typename T>
void ransacHomography_fit(const CMatrixTemplateNumeric<T> &allData,
  ...
  try {
    ...
    dett = (-pt1[0] * pt3[1] + pt1[0] * pt2[1] + pt3[1] * pt2[0] -
...
void ransacHomography_distance(
  ...
  if (out_inlierIndices.size() >= 15) {
    // A)Search and store natural vertices in clockwise direction (greatest and
    ...
    ////ESTA FORMA DE BORRAR REPETICIONES GENERA ERRORES "ALEATORIOS" -> HAY QUE
    ///CAMBIARLA
    ...
    double dist,
        max_dist; // dist & max_dist refer to the nearest line of the contour
    bool vert;
    ...
    do {
           ...
           i++) // NO SE EVALUA NADA MAS K EL 1ER PTO
      {
        ...
        point = TPoint2D(allData(2, out_inlierIndices[i]),
                         allData(3, out_inlierIndices[i]));
        if (!poly.contains(point) && !vert) {
          size_t j = 1;
          for (vector<TSegment2D>::const_iterator it = sgs.begin();
               it != sgs.end(); ++it) // NO SE EVALUA NADA MAS K EL 1ER PTO
          {
            TLine2D l = TLine2D(*it);
            dist = -(l.signedDistance(point));
            if (dist > max_dist + 1) // modifica el poligono para ptos
                                     // extremadamente proximos->Evitarlo con
                                     // if( dist > max_dist+1 )
            {
              max_dist = dist;
      ...
      if (max_dist != 0) {
        ...
        vertex.insert(vertex.begin() + seg_id,
                      TPoint2D(allData(2, vertices[seg_id]),
    ...
    img_aux.colorImage(imgColor); // Create a colorimage
    for (size_t j = 0; j < out_inlierIndices.size(); j++)
      imgColor.cross(round(allData(2, out_inlierIndices[j])),
                     round(allData(3, out_inlierIndices[j])), TColor::blue,
                     '+');
    for (size_t j = 0; j < vertices.size() - 1; j++) {
      imgColor.line(allData(2, vertices[j]), allData(3, vertices[j]),
                    allData(2, vertices[j + 1]), allData(3, vertices[j + 1]),
                    255);
      imgColor.textOut(round(allData(2, vertices[j])),
                       round(allData(3, vertices[j])),
                       format("%d", vertices[j]), 255);
    }
    imgColor.line(allData(2, vertices[vertices.size() - 1]),
                  allData(3, vertices[vertices.size() - 1]),
                  allData(2, vertices[0]), allData(3, vertices[0]), 255);
    imgColor.textOut(round(allData(2, vertices[vertices.size() - 1])),
    ...
    //		{
    //			dist = sgs.distance( TPoint2D( allData(2,out_inlierIndices[i]),
    ...
    dist = 0; // dist is now used to check how consistent are the rest of inside
              // points with the homography
    for (size_t i = 0; i < N; i++) {
      ...
      if (i != out_inlierIndices[k]) {
        if (poly.contains(point)) {
          ...
          pt[2] = 1;
          testModels[0].multiply_Ab(pt, pt_r);
          d = sqrt(square(pt_r[0] - allData(2, i)) +
    ...
    if ((more_inlierIndices.size() > 15) && (outliers < 15) &&
      ...
      out_inlierIndices.insert(out_inlierIndices.end(),
                               more_inlierIndices.begin(),
...
/** Return "true" if the selected points are a degenerate (invalid) case.
  */
template <typename T>
bool ransacHomography_degenerate(const CMatrixTemplateNumeric<T> &allData,
  ...
  // dist = r_a*allData.get_unsafe( 2, useIndices[2] ) + r_b*allData.get_unsafe(
  // 3, useIndices[2] ) + r_c;
  // if ( dist < 3 )
  //	return true;
  // else
  ...
  for (;;) {
    ...
    if (this_best_inliers.size() >= min_inliers_for_valid_plane) {
      ...
      if (out_detected_planes.size() == 1) {
        ...
        unsigned int i = 0;
        j = 0;
        other_inliers2.resize(other_inliers.size() +
                              this_best_inliers_r.size());
...
CFeatureList ExtractFeaturesFrame1(IplImage *img,
                                   CFeatureList &f_out) //, double &average_x,
                                                        //double &average_y,
                                                        //double &std_x, double
                                                        //&std_y)
{
  ...
  //f_out[j]->x, f_out[j]->y, f_out[k]->x, f_out[k]->y );
  //					v1 = i; v2 = j; v3 = k;
  //				}
  //			}
  //		}
...
//|		  MATCH FEATURES          |
//---------------------------------
void matchFeatures(CFeatureList &list1, CFeatureList &list2,
                   CMatchedFeatureList &matched_out) {
  // CTicTac	tictac;
...
int main() {
  ...
  try {
    ...
    //		cout<< "Histogram T" <<j << "\taverage: " << media <<"  \tdesviacion:
    //" << desviacion <<endl <<endl;

    //		sprintf( cstr, "%d %.2f", (j+1), desviacion );
    //		im1.textOut( (boot[mesh(j,0)-1]->x + boot[mesh(j,1)-1]->x +
    //boot[mesh(j,2)-1]->x)/3-20, (boot[mesh(j,0)-1]->y + boot[mesh(j,1)-1]->y +
    //boot[mesh(j,2)-1]->y)/3, cstr, 255 );
    //	}
    //	delete cstr;
    ...
    //					patch.cross( feat_patch_ref[t][i]->x, feat_patch_ref[t][i]->y,
    //255, '+' );
    //	}
    //	cvReleaseImage( &eig );
    //	cvReleaseImage( &temp );
    ...
    if (count > 0) {
      cvCalcOpticalFlowPyrLK(
          prev_grey, grey, prev_pyramid, pyramid, points[0], points[1], count,
      ...
      for (i = 0; i < count; i++) {
        distance += sqrt(square(points[0][i].x - points[1][i].x) +
    ...
    CTicTac tictac;
    tictac.Tic();
    ransac_homographies(boot, flow_klt, detected_homographies, DIST_THRESHOLD,
                        min_points, plane_ind);
    ...
    for (j = 0; j < plane_ind.size(); j++) {
      ransac_log << endl
                 ...
                 << endl; // (0,0) <<" " << detected_homographies[0](0,1) <<" "
                          // << detected_homographies[0](0,2) <<endl;
      ...
      for (i = 0; i < plane_ind[j].size(); i++) {
        ...
        // dist = sqrt( square( v_init2[0] - v_init1[0] ) + square(v_init2[1] -
        // v_init1[1] ) );
        v_init3[0] = flow_klt[plane_ind[j][i]]->x;
        v_init3[1] = flow_klt[plane_ind[j][i]]->y;
        v_init3[2] = sqrt(square(v_init2[0] - v_init3[0]) +
    ...
    for (i = 0; i < detected_homographies.size(); i++) {
      ...
      for (j = 0; j < plane_ind[i].size(); j++)
        imgColor.cross(round(flow_klt[plane_ind[i][j]]->x),
    ...
    // CHECK LOCAL PLANARITY (It takes three vertex in a square and verifies if
    // the rest of points in the square fulfill the homography)
    ...
    while (!seed_plane) {
      // Build affine transformation
      D.fill(0);
      dett = (-vs1[0] * vs3[1] + vs1[0] * vs2[1] + vs3[1] * vs2[0] -
      ...
      for (i = 0; i < seed_feat.size(); i++) {
        if (!(i == v1 || i == v2 || i == v3)) {
          ...
          distance += sqrt(square(pto_r[0] - flow_klt[seed_feat[i]->ID]->x) +
                           square(pto_r[1] - flow_klt[seed_feat[i]->ID]->y));
        }
      }
      distance /=
          (seed_feat.size() - 3); // Distancia media de los píxeles del entorno

      if (distance < 2)
        seed_plane = true;

      //			followPatch(TForm, aux.tri_list[t], aux.img);
      ////				followPatch(TForm, aux.tri_list[t],
      //aux.ind_list[t], aux.img);

      correlations(2 * f - 1, t) =
          ncc_vector(data_img[0].tri_list[t], aux.tri_list[t]);
      cout << "Affine-correlation\tFrame: " << f << "\tTriangle: " << t + 1
           << endl
           << correlations(2 * f - 1, t) << endl;

      // DRAW PATCH_TFORM
      patch_tform =
          cvCreateImage(cvSize(data_img[0].img->width, data_img[0].img->height),
    ...
    /*		while(true)
       ...
       im2->getAsIplImage();
                            ...
                            frame = cvQueryFrame(video); frame =
       cvQueryFrame(video); frame = cvQueryFrame(video); frame =
       cvQueryFrame(video); frame = cvQueryFrame(video); //Take one frame each 5
                            ASSERT_(frame);
                            ASSERT_(frame->depth==IPL_DEPTH_8U);
                            ...
                            aux.img = cvCreateImage(cvSize(frame->width,
       ...
       - flow_klt[i]->x ) + square( boot[i]->y - flow_klt[i]->y ) );
                                    ...
                                    for( i = k = 0; i < count; i++ )
                                    {
                                            ...
                                            //cvCircle( image,
       cvPointFrom32f(points[1][i]), 3, CV_RGB(0,255,0), -1, 8,0);
                                    }
                                    //count = k;
                            }
       ...
       flow_klt[mesh(i,2)-1]->y );
                            ...
                            for(it=flow_klt.begin(); it!=flow_klt.end(); it++)
                            {
                                    ...
                                    {
                                            cout<< "Feature " <<(*it)->ID << "
       couldn't be followed." <<endl;
                                            mrpt::system::pause();
                                    }
                            }
                            aux.tri_list.resize( mesh.getRowCount() );
                            aux.ind_list.resize( mesh.getRowCount() );
                            //loadPatches(flow_klt, mesh, &(aux.mask),
       aux.tri_list, aux.ind_list, aux.img);
       ...
       data_img[0].img->depth, 1);
                            ...
                            for(t=0; t<mesh.getRowCount(); t++)
                            {
                                    ...
                                    //{
                                    //	vector_double hist =
       aux1.tri_list[t].histogram( 0, 255, 256, false);

                                    //	double media = 0; double length =
       hist.sumAll(); //=aux1.tri_list[0].size();
                                    //	for( i=0; i<256; i++)
                                    //	{
                                    //		media = media +
       i*hist[i]/length;
       ...
       square(i-media)*(hist[i]/length);
                                    ...
                                    //	desviacion = sqrt(varianza);
                                    //	cout<< "\nHistogram T" <<t <<
       "\taverage: " << media <<"  \tdesviacion: " << desviacion <<endl;
                                    //}//END HISTOGRAM
       ...
       data_img[i].tri_list[t][k]);
                                    //	}
                                    //}
                                    //cvMatchTemplate(image, const CvArr* templ,
       ...
       //This is the square root of minus det(B) -> sqtr(-det(B))
                                    D.set_unsafe(0,0,
       (-po3[t][1]+po2[t][1])/dett);
                                    D.set_unsafe(0,2,
       -(po1[t][1]-po3[t][1])/dett);
                                    D.set_unsafe(0,4,
       (-po2[t][1]+po1[t][1])/dett);
                                    D.set_unsafe(1,0,
       -(po2[t][0]-po3[t][0])/dett);
                                    D.set_unsafe(1,2,
       (po1[t][0]-po3[t][0])/dett);
                                    D.set_unsafe(1,4,
       -(po1[t][0]-po2[t][0])/dett);
                                    D.set_unsafe(2,0,
       ...
       (po1[t][0]*po2[t][1]-po2[t][0]*po1[t][1])/dett);
                                    ...
                                    followPatch(TForm, aux.tri_list[t],
       aux.img);	//				followPatch(TForm, aux.tri_list[t], aux.ind_list[t],
       aux.img);

                                    correlations(2*f-1,t) =
       ncc_vector(data_img[0].tri_list[t], aux.tri_list[t]);
                                    cout<< "Affine-correlation\tFrame: " <<f
       <<"\tTriangle: " <<t+1 <<endl <<correlations(2*f-1,t) <<endl;

                            // DRAW PATCH_TFORM
                                    patch_tform =
       cvCreateImage(cvSize(data_img[0].img->width, data_img[0].img->height),
       data_img[0].img->depth, 1);
                                    for( i=0; i<data_img[0].tri_list[t].size();
       i++ )
                                    {
       ...
       boot[mesh(t,2)-1]->y);
                                    cvLine(patch_tform, pt1, pt2, color, 1, 8,
       0);
                                    cvLine(patch_tform, pt1, pt3, color, 1, 8,
       0);
                                    cvLine(patch_tform, pt3, pt2, color, 1, 8,
       0);
                                    ...
                                    if( t==(mesh.getRowCount()-1) )
                                    {
                                            ...
                                            for( i=0; i<mesh.getRowCount(); i++
                                                    ...
                                                    im2.textOut(
       (boot[mesh(i,0)-1]->x + boot[mesh(i,1)-1]->x +
       boot[mesh(i,2)-1]->x)/3-20, (boot[mesh(i,0)-1]->y + boot[mesh(i,1)-1]->y
       + boot[mesh(i,2)-1]->y)/3-10, format("%.2f", correlations(2*f-1,i) ), 255
       );
       ...
       feat_patch_vid[t][i]->y;
                                    ...
                                    cvCalcOpticalFlowPyrLK( tri_ref[t],
       tri_vid[t], prev_pyramidT, pyramidT,
       ...
       feat_patch_vid[t][i]->x ;
                                            distance2 += square(
       feat_patch_ref[t][i]->x - feat_patch_vid[t][i]->x ) + square(
       feat_patch_ref[t][i]->y - feat_patch_vid[t][i]->y );
                                            patch.line( feat_patch_ref[t][i]->x,
       feat_patch_ref[t][i]->y, feat_patch_vid[t][i]->x,
       feat_patch_vid[t][i]->y, 255 );
                                            patch.cross(
       feat_patch_vid[t][i]->x, feat_patch_vid[t][i]->y, 255, '+' );
                                    }
                                    correlations(2*f,t) = distance2 / countT;
                                    cout<< "distance " <<correlations(2*f,t)
       <<endl;
                                    win3.showImageAndPoints( tri_vid[t],
       feat_patch_vid[t] );
                                    patch.textOut( (boot[mesh(t,0)-1]->x +
       boot[mesh(t,1)-1]->x + boot[mesh(t,2)-1]->x)/3+60, (boot[mesh(t,0)-1]->y
       + boot[mesh(t,1)-1]->y + boot[mesh(t,2)-1]->y)/3, format( "Distance2:
       %f", correlations(2*f,t) ), 255 );
                                    patch.saveToFile( format("D:\\vid%d+p.bmp",
       (t+1) ) );
                                    mrpt::system::pause();

                                    CV_SWAP( tri_ref[t], tri_vid[t], swap_temp
       );		//Reference changes to the previous patch.
                                    //CV_SWAP( pointsT[0], pointsT[1],
       swap_points );
                                    //CV_SWAP( prev_pyramidT, pyramidT,
       swap_temp );
       ...
       Construct Affine Transformation
                                    ...
                                    //	// Increments Vector = [ 0.0001 0.0001
       .5 0.0001 0.0001 .5 0.000001 0.000001 ]
                                    //	increments_x.resize(8, 0.0001 );
                                    //	increments_x[2] = 0.5; increments_x[5]
       = 0.5; increments_x[6] = 0.000001; increments_x[7] = 0.000001;
                                    //	optimal_x.resize(8);
                                    //	//CMyLevMarq::execute(optimal_x,
       initial_x, CostFunction, increments_x, data_img, info  );
                                    //	CTicTac tictac;
                                    //	tictac.Tic();
                                    //	CMyLevMarq::execute(optimal_x,
       initial_x, CostFunction, increments_x, aux, info, false, 200, 1e-3,
       1e-10, 1e-10, false );

                                    //	correlations(2*f,t) = 1 -
       sqrt(info.final_sqr_err);
       ...
       info.iterations_executed <<" iterations\n" <<endl;
                                    ...
                                    //	//pp1[0] = pp1[0]/pp1[2]; pp1[1] =
       pp1[1]/pp1[2];
                                    //	//TForm.multiply_Ab(po2[t],pp2);
                                    //	//pp2[0] = pp2[0]/pp2[2]; pp2[1] =
       pp2[1]/pp2[2];
                                    //	//TForm.multiply_Ab(po3[t],pp3);
                                    //	//pp3[0] = pp3[0]/pp3[2]; pp3[1] =
       pp3[1]/pp3[2];
                                    //	//xx[0] = pp1[0]; xx[1] = pp2[0];
       xx[2] = pp3[0];
                                    //	//yy[0] = pp1[1]; yy[1] = pp2[1];
       yy[2] = pp3[1];
                                    //	////data_img[f].img = (IplImage*)
       im2->getAsIplImage();
                                    //	//win3.showImageAndPoints( aux.img,
       xx, yy, TColor::red );
                                    //	//mrpt::system::pause();
       ...
       triangle.resize(data_img[0].tri_list[t].size());
                                    ...
                                    //	//cout<< ncc( data_img[0].tri_list[t],
       triangle ) <<endl;
       ...
       data_img[0].img->depth, 1);
                                    //	for(i=0;
       i<data_img[0].tri_list[t].size(); i++)
                                    //	{
                                    //
       ...
       boot[mesh(t,1)-1]->y);
                                    //	//pt3 = cvPoint( boot[mesh(t,2)-1]->x,
       boot[mesh(t,2)-1]->y);
                                    //	//cvLine(patch_LM, pt1, pt2, color, 1,
       8, 0);
                                    //	//cvLine(patch_LM, pt1, pt3, color, 1,
       8, 0);
                                    //	//cvLine(patch_LM, pt3, pt2, color, 1,
       8, 0);
                                    //	//
                                    //	//if( t==(mesh.getRowCount()-1) )
                                    //	//{
                                    //	//	im2.loadFromIplImage(patch_LM);
                                    //	//	for( i=0; i<mesh.getRowCount();
       i++ )
                                    //	//	{
                                    //	//		char* cstr = new char
       [20];
       ...
       + boot[mesh(i,2)-1]->y)/3-10, cstr, 255 );
                                    //	//		delete cstr;
                                    //	//	}
                                    //	//	win2.showImageAndPoints( im2,
       boot );		//im2 and boot has different scales, since the origins are 0 & 1
       respectively, therefore the mesh is shifted 1pixel in each axis
                                    //	//	mrpt::system::pause();
                                    //	//} //END DRAWING
