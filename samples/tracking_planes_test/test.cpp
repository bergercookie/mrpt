	...
	vector_size_t& out_inlierIndices)
{
	...
	if (out_inlierIndices.size() >= 15)
	{
		...
		double dist,
			max_dist;  // dist & max_dist refer to the nearest line of the
		// contour
		bool vert;
				 ...
				 i++)  // NO SE EVALUA NADA MAS K EL 1ER PTO
			{
				...
				if (!poly.contains(point) && !vert)
				{
						 ...
						 ++it)  // NO SE EVALUA NADA MAS K EL 1ER PTO
					{
						...
						if (dist >
							max_dist + 1)  // modifica el poligono para ptos
		...
		while (!seed_plane)
		{
			...
			dett =
				(-vs1[0] * vs3[1] + vs1[0] * vs2[1] + vs3[1] * vs2[0] -
				 ...
				 vs3[0] *
					 vs1[1]);  // This is the square root of minus det(B) ->
			// sqtr(-det(B))
		...
		/*		while(true)
		   ...
		   data_img[0].img->height), data_img[0].img->depth, 1);
					...
					for(t=0; t<mesh.getRowCount(); t++)
					{
						...
						//{
						//	vector_double hist = aux1.tri_list[t].histogram( 0,
		   255, 256, false);
		   ...
		   followPatch(TForm, aux.tri_list[t], aux.ind_list[t], aux.img);
						...
						correlations(2*f-1,t) =
		   ncc_vector(data_img[0].tri_list[t],
		   aux.tri_list[t]);
		   ...
		   (boot[mesh(i,0)-1]->y
		   + boot[mesh(i,1)-1]->y + boot[mesh(i,2)-1]->y)/3-10, format("%.2f",
		   correlations(2*f-1,i) ), 255 );

							win2.showImageAndPoints( im2, boot );		//im2
		   and
		   boot has different scales, since the origins are 0 & 1 respectively,
		   therefore the mesh is shifted 1pixel in each axis
							mrpt::system::pause();
						} //END DRAWING
		   ...
		   boot[mesh(t,1)-1]->x
		   + boot[mesh(t,2)-1]->x)/3+60, (boot[mesh(t,0)-1]->y +
		   boot[mesh(t,1)-1]->y + boot[mesh(t,2)-1]->y)/3, format( "Distance2:
		   %f",
		   correlations(2*f,t) ), 255 );
		   ...
		   <<"\tTriangle:
		   " <<t+1 <<endl <<correlations(2*f,t) <<endl;
						//	cout<< "LM-correlation" <<endl <<correlations(2*f,t)
		   <<endl;
		   ...
		   = triangle[i];
						...
						//	//for( i=0; i<data_img[0].tri_list[t].size(); i++ )
						//	//	patch_LM->imageData[data_img[0].ind_list[t][i]]
		   = triangle[i];
						//	//
						//	//pt1 = cvPoint( boot[mesh(t,0)-1]->x,
		   boot[mesh(t,0)-1]->y);
		   ...
		   boot[mesh(i,1)-1]->x + boot[mesh(i,2)-1]->x)/3-20,
		   (boot[mesh(i,0)-1]->y
		   + boot[mesh(i,1)-1]->y + boot[mesh(i,2)-1]->y)/3-10, cstr, 255 );
						//	//		delete cstr;
						//	//	}
