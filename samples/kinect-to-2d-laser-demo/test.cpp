...
void Test_Kinect() {
  ...
  mrpt::opengl::CFrustum::Ptr gl_frustum =
      std::make_shared<mrpt::opengl::CFrustum>(0.2f, 5.0f, 90.0f, 5.0f, 2.0f,
                                               true, true);
      ...
      viewInt; // Extra viewports for the RGB & D images.
  {
    ...
    // Create the Opengl objects for the planar images, as textured planes, each
    // in a separate viewport:
    win3D.addTextMessage(30, -25 - 1 * (VW_GAP + VW_HEIGHT), "Range data",
                         TColorf(1, 1, 1), 1, MRPT_GLUT_BITMAP_HELVETICA_12);
    viewRange = scene->createViewport("view2d_range");
    viewRange->setViewportPosition(5, -10 - 1 * (VW_GAP + VW_HEIGHT), VW_WIDTH,
                                   VW_HEIGHT);

    win3D.addTextMessage(30, -25 - 2 * (VW_GAP + VW_HEIGHT), "Intensity data",
                         TColorf(1, 1, 1), 2, MRPT_GLUT_BITMAP_HELVETICA_12);
    viewInt = scene->createViewport("view2d_int");
    viewInt->setViewportPosition(5, -10 - 2 * (VW_GAP + VW_HEIGHT), VW_WIDTH,
                                 VW_HEIGHT);
  ...
  while (win3D.isOpen() && !thrPar.quit) {
    ...
    if (possiblyNewObs && possiblyNewObs->timestamp != INVALID_TIMESTAMP &&
      ...
      if (last_obs->hasRangeImage) {
        ...
        // Normalize the image
        static CMatrixFloat range2D; // Static to save time allocating the
      ...
      if (last_obs->hasIntensityImage) {
        win3D.get3DSceneAndLock();
        viewInt->setImageView(last_obs->intensityImage); // This is not "_fast"
                                                         // since the intensity
                                                         // image is used below
                                                         // in the coloured
                                                         // point cloud.
      ...
      win3D.addTextMessage(-100, -20, format("%.02f Hz", thrPar.Hz),
                           TColorf(1, 1, 1), "sans", 15, mrpt::opengl::FILL,
                           100);

      win3D.addTextMessage(
          10, 10, "'o'/'i'-zoom out/in, '2'/'3'/'f':show/hide 2D/3D/frustum "
                  "data, mouse: orbit 3D, ESC: quit",
          TColorf(1, 1, 1), "sans", 10, mrpt::opengl::FILL, 110);

      win3D.addTextMessage(
          10, 25, format("Show: 3D=%s 2D=%s Frustum=%s (vert. FOV=%.1fdeg)",
      ...
      if (last_obs_imu && last_obs_imu->dataIsPresent[IMU_X_ACC]) {
        win3D.get3DSceneAndLock();
        win3D.addTextMessage(
            10, 65, format("Acc: x=%.02f y=%.02f z=%.02f",
