...
bool initONI2Stream(openni::Device &device, openni::SensorType sensorType,
                    openni::VideoStream &stream, int w, int h, int fps,
                    openni::PixelFormat format);
bool synchONI2MirrorMode(openni::VideoStream &rgb, openni::VideoStream &depth,
                         bool &isMirrorMode);
...
int main(int argc, char **argv) {
  ...
  try {
    if (argc < 2) {
      ...
      cerr << "Example: " << argv[0] << " /home/jdoe/myrawlog will produce "
                                        "file myrawlog.rawlog in folder "
                                        "/home/jdoe\n";
    ...
    //								Create RGB and Depth
    //channels
    //========================================================================================
    if (initONI2Stream(device, openni::SENSOR_COLOR, rgb, width, height, fps,
                       openni::PIXEL_FORMAT_RGB888) == false) {
      return 1;
    }
    if (initONI2Stream(device, openni::SENSOR_DEPTH, depth, width, height, fps,
    ...
    while ((!system::os::kbhit() && !manual_mode) || manual_mode) {
      ...
      if ((framed.getWidth() != framergb.getWidth()) ||
        ...
        obs.intensityImage.setExternalStorage(out_img_dir + string("/") +
                                              sRGBfile);
...
bool synchONI2MirrorMode(openni::VideoStream &rgb, openni::VideoStream &depth,
  ...
  for (int i = 0; i < SIZE; ++i) {
    if (streams[i]->isPropertySupported(openni::STREAM_PROPERTY_MIRRORING) ==
        false) {
      printf("%s:openni::STREAM_PROPERTY_MIRRORING is not supported on %s.\n"
             "  We assume this is MS Kinect and taken images are inverted to "
             "right and left.\n",
