...
//   f: R^5   => R^3
void myFun1(const CArrayDouble<5> &x, const double &user_param,
            CArrayDouble<3> &y) {
...
void Test_SUT() {
  ...
  const double x0cov[] = {0.049400,  0.011403,  -0.006389, 0.008132,  -0.008595,
  ...
  for (int i = 0; i < 3; i++) {
    winHistos[i] = std::make_shared<mrpt::gui::CDisplayWindowPlots>(
...
void aux_posequat2poseypr(const CArrayDouble<7> &x, const double &dummy,
                          CArrayDouble<6> &y) {
  const CPose3DQuat p(x[0], x[1], x[2],
...
void TestCalibrate_pose2quat() {
  ...
  static const bool elements_do_wrapPI[6] = {false, false, false, true,
                                             true,  true}; // xyz yaw pitch roll
  ...
  mrpt::math::transform_gaussian_unscented(x_mean, o.cov, aux_posequat2poseypr,
                                           dummy, y_mean, p_ypr.cov,
                                           elements_do_wrapPI,
                                           1e-3, // alpha
                                           0,    // K
                                           2.0   // beta
