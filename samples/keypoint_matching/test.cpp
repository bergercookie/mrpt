...
string myDataDir(MRPT_EXAMPLES_BASE_DIRECTORY +
                 string("keypoint_matching/imgs/"));
...
void TestExtractMatchProjectAndPaint() {
  ...
  CGridPlaneXY::Ptr gridXY =
      std::make_shared<CGridPlaneXY>(-10, 10, -10, 10, 0, 1);
...
void TestMatchFeatures(bool showMatches) {
  ...
  T = tictac.Tac();
  cout << "[SAD] Matches found: " << mHarris_SAD.size() << " in " << T * 1000.0f
       << " ms " << endl;
  cout << "***************************************************" << endl;
  ...
  if (showMatches) {
    CDisplayWindow winHarrisSAD, winHarrisNCC, winFASTSAD, winFASTNCC, winSIFT,
        winSURF;
    ...
    winHarrisNCC.showImagesAndMatchedPoints(imL, imR, mHarris,
                                            TColor(0, 0, 255));
    winHarrisSAD.showImagesAndMatchedPoints(imL, imR, mHarris_SAD,
                                            TColor(0, 0, 255));
    ...
    winFASTSAD.showImagesAndMatchedPoints(imL, imR, mFAST_SAD,
                                          TColor(0, 255, 0));
    winFASTNCC.showImagesAndMatchedPoints(imL, imR, mFAST_CC,
                                          TColor(0, 255, 0));
...
void TestMatchingComparative() {
  ...
  for (it1 = list1.begin(); it1 != list1.end(); ++it1) {
    ...
    copyjoinimage.line((*it1)->x + imW, 0, (*it1)->x + imW, imH,
                       TColor::green); // Horiz
    copyjoinimage.line(0, (*it1)->y, imW + imW, (*it1)->y,
                       TColor::green); // Epipolar
    copyjoinimage.drawCircle((*it1)->x, (*it1)->y, 4, TColor::green,
                             2); // Keypoint
    ...
    for (it2 = list2.begin(); it2 != list2.end(); ++it2) {
      if (fabs((*it1)->y - (*it2)->y) <= 1.0 && (*it1)->x > (*it2)->x) {
        ...
        double ncc;
        size_t u, v;
        mrpt::vision::openCV_cross_correlation((*it1)->patch, (*it2)->patch, u,
                                               v, ncc);
        if (ncc > maxncc) {
          maxncc = ncc;
          idxncc = cnt;
        ...
        copyjoinimage.drawCircle((*it2)->x + imW, (*it2)->y, 4, TColor::blue,
                                 2); // Keypoint
        ...
        copyjoinimage.filledRectangle(rx0, ry0, rx1, ry1,
                                      TColor(150, 150, 150));
        ...
        copyInfoImage.textOut(px, imH2 - 29, format("%.2f", siftd),
                              TColor::blue);
        copyInfoImage.textOut(px, imH2 - 17, format("%.2f", surfd),
                              TColor::blue);

        cnt++;
      } // end if
    }   // end for it2
    copyInfoImage.textOut(80 + idxsad * 50, imH2 - 53, format("%.2f", minsad),
                          TColor::green);
    copyInfoImage.textOut(80 + idxncc * 50, imH2 - 41, format("%.2f", maxncc),
                          TColor::green);
    copyInfoImage.textOut(80 + idxsiftd * 50, imH2 - 29,
                          format("%.2f", minsiftd), TColor::green);
    copyInfoImage.textOut(80 + idxsurfd * 50, imH2 - 17,
                          format("%.2f", minsurfd), TColor::green);
...
int main(int argc, char **argv) {
  try {
    if (argc == 1) {
      ...
      cerr << "Options:" << endl;
      cerr << " -match [-s]: TestMatchFeatures (if -s is set, final matches in "
      ...
      if (!strcmp(argv[1], "-match")) {
        ...
        cerr << "Usage: " << argv[0] << endl;
        cerr << "Options:" << endl;
        cerr << " -match [-s]: TestMatchFeatures (if -s is set, final matches "
