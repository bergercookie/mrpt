...
void insertRandomPoints_uniform(const size_t N, opengl::CPointCloud::Ptr &gl,
                                const TPoint3D &p_min, const TPoint3D &p_max) {
  for (size_t i = 0; i < N; i++)
    gl->insertPoint(random::randomGenerator.drawUniform(p_min.x, p_max.x),
                    random::randomGenerator.drawUniform(p_min.y, p_max.y),
                    random::randomGenerator.drawUniform(p_min.z, p_max.z));
}

void insertRandomPoints_screw(const size_t N, opengl::CPointCloud::Ptr &gl,
                              const TPoint3D &p_start, const TPoint3D &p_end) {
...
void insertRandomPoints_gauss(const size_t N, opengl::CPointCloud::Ptr &gl,
...
void TestOctreeRenderHugePointCloud() {
  ...
  for (int XX = -10; XX <= 10; XX++) {
    ...
    for (int YY = -10; YY <= 10; YY++) {
      ...
      insertRandomPoints_screw(1e4, gl_pointcloud,
  ...
  while (win.isOpen() && !end) {
    ...
    string s = mrpt::format(
        "FPS=%5.02f | Rendered points=%.02e/%.02e (%.02f%%) | Visib.oct.nodes: "
        "%u/%u",
    ...
    win.addTextMessage(5, 5, s, TColorf(1, 1, 1), 0,
                       MRPT_GLUT_BITMAP_HELVETICA_18);
    win.addTextMessage(5, 35, "'b': switch bounding-boxes visible, 'q': quit",
