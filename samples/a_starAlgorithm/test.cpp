...
public:
  /**
    * When a class derives from CAStarAlgorithm, its constructor should include
   * all the data that define the specific problem.
    */
  CAStarExample(size_t goal) : N(goal) {}
  /**
    * The following five methods must be implemented to use the algorithm:
    */
  virtual bool isSolutionEnded(
      const CCoinDistribution &s) { // True if the solution is complete.
    return s.money() == N;
  }
  virtual bool isSolutionValid(
      const CCoinDistribution &s) { // True if the solution is valid.
    return s.money() <= N;
  }
  virtual void generateChildren(
      const CCoinDistribution &s,
      vector<CCoinDistribution> &sols) { // Get all the children of a solution.
  ...
  virtual double
  getHeuristic(const CCoinDistribution &
                   s) { // Heuristic cost of the remaining part of the solution.
    // Check the documentation of CAStarAlgorithm to know which characteristics
    // does this function need to comply.
    return static_cast<double>(N - s.money()) / 19.0;
  }
  virtual double
  getCost(const CCoinDistribution &s) { // Known cost of the partial solution.
...
int main(int argc, char **argv) {
  for (;;) {
    ...
    if (1 !=
        scanf("%10s", text)) // GCC warning: scanf -> return cannot be ignored!
    {
      printf("Please, input a positive integer.\n\n");
      continue;
    }
    if (strlen(text) == 1 && (text[0] == 'e' || text[0] == 'E'))
      break;
    int val = atoi(text);
    if (val <= 0) {
      printf("Please, input a positive integer.\n\n");
    ...
    case 0:
      printf("No solution has been found. Either the number is too small, or "
             "the time elapsed has exceeded 15 seconds.\n\n");
      break;
    case 1:
