...
// Load config:
  ...
  if (1) {
    ...
    task.ai.physicalChannelCount = 8; // Must be the number of channels encoded
                                      // in the "physicalChannel" string.
  ...
  if (0) {
    ...
    task.ao.physicalChannelCount = 1; // Must be the number of channels encoded
                                      // in the "physicalChannel" string.
    task.ao.minVal = -10;
    task.ao.maxVal = 10;
  ...
  if (0) {
    ...
    daq.writeAnalogOutputTask(1, sizeof(volt_values) / sizeof(volt_values[0]),
                              volt_values, 0.100, true);
  }
  ...
  while (!mrpt::system::os::kbhit()) {
    std::vector<mrpt::obs::CObservationRawDAQ::Ptr> readObs;
    bool hardError;
    ...
    if (!readObs.empty()) {
      ...
      for (size_t i = 0; i < readObs.size(); i++) {
        ...
        const size_t nSamplPerChan =
            readObs[i]->AIN_double.size() / readObs[i]->AIN_channel_count;
