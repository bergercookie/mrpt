        ...
        if (IS_CLASS(o, CObservationGasSensors)) {
          ...
          if (obs->sensorLabel == sensorLabel) {
            ...
            if (sensorType == 0) { // compute the mean
              raw_reading =
                  math::mean(obs->m_readings[enoseID].readingsVoltage);
            } else {
              ...
              if (indexMonitoredSensor == -1) {
                ...
                for (indexMonitoredSensor = 0;
                     ...
                     (int)obs->m_readings[enoseID].sensorTypes.size();
                     indexMonitoredSensor++) {
              ...
              if (indexMonitoredSensor <
                  (int)obs->m_readings[enoseID].sensorTypes.size()) {
                raw_reading = obs->m_readings[enoseID].readingsVoltage.at(
                    indexMonitoredSensor);
              } else // Sensor especified not found, compute default mean value
              {
                cout << "sensorType not found. Computing the mean value"
                     << endl;
                raw_reading =
                    math::mean(obs->m_readings[enoseID].readingsVoltage);
              }
            ...
            mrpt::poses::CPose3D MOXmodel_pose = mrpt::poses::CPose3D(
            ...
            have_estimation = MOSmodel.get_GasDistribution_estimation(
