...
string myInitFile(MRPT_EXAMPLES_BASE_DIRECTORY +
    ...
    if (detected.size() > 0) {
      for (unsigned int i = 0; i < detected.size(); i++) {
        ...
        if (showEachDetectedFace) {
          ...
          o->getZoneAsObs(face, obj->m_y, obj->m_y + obj->m_height, obj->m_x,
                          obj->m_x + obj->m_width);
          ...
          if (o->hasPoints3D) {
            ...
            if (!o->hasConfidenceImage) {
              ...
              unsigned int i = 0;
              for (unsigned int j = 0; j < face.confidenceImage.getHeight();
                   j++)
                for (unsigned int k = 0; k < face.confidenceImage.getWidth();
                     k++, i++) {
        ...
        o->intensityImage.rectangle(obj->m_x, obj->m_y, obj->m_x + obj->m_width,
                                    obj->m_y + obj->m_height,
                                    TColor(255, 0, 0));
  ...
  while (win.isOpen()) {
    ...
    if (IS_CLASS(obs, CObservationImage)) {
      ...
      for (unsigned int i = 0; i < detected.size(); i++) {
        ...
        o->image.rectangle(obj->m_x, obj->m_y, obj->m_x + obj->m_width,
      ...
      for (unsigned int i = 0; i < detected.size(); i++) {
        ...
        CDetectable2D::Ptr obj =
            std::dynamic_pointer_cast<CDetectable2D>(detected[i]);
        o->imageRight.rectangle(obj->m_x, obj->m_y, obj->m_x + obj->m_width,
                                obj->m_y + obj->m_height, TColor(255, 0, 0));
      }
...
void TestImagesFaceDetection(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    for (unsigned int i = 0; i < detected.size(); i++) {
      ...
      CDetectable2D::Ptr obj =
          std::dynamic_pointer_cast<CDetectable2D>(detected[i]);
      img.rectangle(obj->m_x, obj->m_y, obj->m_x + obj->m_width,
                    obj->m_y + obj->m_height, TColor(255, 0, 0));
    }
...
void BatchMode() {
  ...
  for (size_t i = 0; i < rawlogs.size(); i++) {
    ...
    faceDetector.debug_returnResults(falsePositives[i], ignored[i],
                                     falsePositivesDeleted, realFacesDeleted);
...
void TestPrepareDetector() {
  ...
  if (classifierType == 0) // Haar
    cfg.write("CascadeClassifier", "cascadeFileName",
              OPENCV_SRC_DIR +
  ...
  if (batchMode) {
    ...
    for (size_t i = 0; i < numRawlogs; i++) {
      cfg.read_vector(rawlogs[i], "falsePositives", vector_uint(),
                      falsePositives[i]);
