...
void addEdge(TNodeID from, TNodeID to,
...
double myDijkstraWeight(const CMyDijkstra::graph_t &g, const TNodeID from,
                        const TNodeID to, const CMyDijkstra::edge_t &edge) {
...
void TestDijkstra() {
  ...
  for (size_t j = 0; j < N_VERTEX; j++) {
    CPose2D p(randomGenerator.drawUniform(-NODES_XY_MAX, NODES_XY_MAX),
  ...
  for (TNodeID i = 0; i < N_VERTEX && win.isOpen(); i++) {
    ...
    win.setWindowTitle(format("Dijkstra path %u->%u",
                              static_cast<unsigned int>(SOURCE_NODE),
    ...
    cout
        << "Press any key to show next shortest path, close window to end...\n";
    win.waitForKey();
  }
