...
void TestCapture_FlyCapture2_stereo() {
  cout << " FlyCapture2 version: " << CImageGrabber_FlyCapture2::getFC2version()
       << std::endl;
  ...
  capture_left.open(cam_options_left,
                    false /*only open, don't start grabbing*/);
  ...
  capture_right.open(cam_options_right,
                     false /*only open, don't start grabbing*/);
...
// Open both cameras simultaneously:
...
#else
  capture_right.startCapture(); // Will not start until a strobe is got from the
                                // "master" camera:
  ...
  while (!mrpt::system::os::kbhit()) {
    ...
    cout.flush();
    if (winL.isOpen())
      winL.showImage(obsL->image);
    if (winR.isOpen())
      winR.showImage(obsR->image);
