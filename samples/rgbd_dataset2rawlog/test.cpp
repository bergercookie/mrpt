...
find_closest(const CONTAINER &data,
...
void rgbd2rawlog(const string &src_path, const string &out_name) {
  ...
  obs.range_is_depth = true; // Kinect style: ranges are actually depth values,
  ...
  obs.relativePoseIntensityWRTDepth = mrpt::poses::CPose3D(
      ...
      mrpt::utils::DEG2RAD(-90)); // No translation between rgb & range cameras,
                                  // and rotation only due to XYZ axes
                                  // conventions.
  ...
  if (!only_ir && !only_rgb) {
    for (map<double, string>::const_iterator it_list_rgb = list_rgb.begin();
      ...
      // This is not the most efficient solution in the world, but...come on!
      // it's just a rawlog converter tool!
      ...
      if (At > (1. / KINECT_FPS) * .5) {
        cout << "\nWarning: Discarding observation for too separated RGB/D "
                "timestamps: "
             << At * 1e3 << " ms\n";
      } else {
        ...
        obs.intensityImage.loadFromFile(src_path + string("/") +
                                        it_list_rgb->second);
        ...
        if (!depth_img.loadFromFile(src_path + string("/") +
                                    it_list_depth->second))
          throw std::runtime_error(string("Error loading depth image!: ") +
                                   it_list_depth->second);
        ...
        for (unsigned int row = 0; row < h; row++) {
          const uint16_t *ptr =
              reinterpret_cast<const uint16_t *>(depth_img.get_unsafe(0, row));
