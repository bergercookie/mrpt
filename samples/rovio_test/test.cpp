...
// ... we search it into the vector
        {
                for( itVec2 = list2.begin(); itVec2 != list2.end(); itVec2++)
                {
                        ...
                        {
                                std::pair<CFeature::Ptr,CFeature::Ptr> mPair(
itVec1, itVec2 );
...
*/
CMatchedFeatureList comp2Matched_lists(CMatchedFeatureList &list1,
                                       CMatchedFeatureList &list2) {
...
void matching(data_st *data) {
  ...
  tictac.Tic();
  mrpt::vision::utils::matchFeatures2(data->featsHarris, data->featsHarris2,
                                      data->Harris_matched, opt);
  ...
  // opt.F=(~data->FM_Harris2ways);
  tictac.Tic();
  mrpt::vision::utils::matchFeatures2(data->featsHarris2, data->featsHarris,
                                      data->Harris_matched_inv, opt);
...
int main() {
  ...
  string
      MenuJoystick =
          ...
          "	|\n"
          "|3->Head to the middle		4->Head down		"
          "3&4->Head up	|\n"
          "|1&2->Go Home			Up&3->Go Home and Dock	"
          "1&3->Get Report	|\n"
          "--------------------------------------------------------------------"
          "---\n";
  string
      MenuKeyboard = "---------------------------Keyboard "
                     ...
                     "|'h'->Gohome			'j'->menuJY		"
                     "'r'->menuKB	|\n"
                     "|'l'->ShowPosition		'n'->ShowEnc		"
                     "'k'->Get Report	|\n"
                     "|'f'->Ext Features		'b'->Match Feat		"
                     "'*'->FMatrix	|\n"
                     "|				'.'->EXIT		"
                     "		|\n"
                     "---------------------------------------------------------"
    ...
    if (errormsg.empty()) {
      ...
      do {
        ...
        case 'f': // Take Features
          act = 0;
          output = false;
          data.features_taken = false; // this var checks if the function
        ...
        case 'b': // Match Features without epipolar restriction
          act = 0;
          output = false;
          data.matching_done = false; // this var checks if the function
                                      // TestExtractFeatures has finished
          data.matching_type = complete_match;
          matching_hd = std::thread(matching, &data);
          break;
        case '1': // Change the matching to complete matching, get all the
        ...
        case '*': // Calculate Fundamental Matrix
          act = 0;
          output = false;
          data.fm_calculated = false; // this var checks if the function
                                      // TestExtractFeatures has finished
