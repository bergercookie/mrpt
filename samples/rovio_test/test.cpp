...
CDisplayWindow win("Video"), wind1,
	wind2;  // wind3,wind4,wind5,wind6; 	//open a window to show the video
...
{
	string field_name[12] = {"Packet length",
							 "Not Used",
							 "Left Wheel:Dir Rotation",
							 "Left Wheel:Ticks",
							 "Right Wheel:Dir Rotation",
							 "Right Wheel:Ticks",
							 "Rear Wheel:Dir Rotation",
							 "Rear Wheel:Ticks",
							 "Not used",
							 "Head Position",
							 "Batery",
	...
	for (int i = 0; i <= 11; i++)
	{
		...
		{
			cout << " = ";
			binary(arr[i]);  // print binary value
		}
		cout << endl;
	}
	cout << "\n-------------------------------------------\n";
...
void goThere(data_st& Pdata, string& response, string& errormsg)
{
	...
	do
	{
		...
		while (angle > 0.6 || angle < (-0.6))
		{
			if (angle > 0.6) Pdata.Robot.Moving(18, 6, 10, response, errormsg);
			if (angle < (-0.6))
				Pdata.Robot.Moving(18, 5, 10, response, errormsg);
		...
		if (distance < 500 && theta_d < 0.6 && theta_d > (-0.6)) target = true;
	} while (!target);
}
...
void JoystickControl(data_st* data)
{
	...
	while (joy.getJoystickPosition(0, x, y, z, buttons))
	{
		...
		if ((buttons[3] && buttons[2]) || (buttons[0] && buttons[1]) ||
			(y < -0.5 && buttons[2]) || (buttons[0] && buttons[2]))
		{
...
/*
CMatchedFeatureList comp2Matched_lists2(CMatchedFeatureList &list1,
CMatchedFeatureList &list2)
{
	CMatchedFeatureList final_list;
	CMatchedFeatureList::iterator			itVec1, itVec2;				//
We set an iterator for the vector
	for( itVec1 = list1.begin(); itVec1 != list1.end(); itVec1++)		// ...
we search it into the vector
	{
...
*/
CMatchedFeatureList comp2Matched_lists(
	CMatchedFeatureList& list1, CMatchedFeatureList& list2)
{
	CMatchedFeatureList final_list = list1;
	CMatchedFeatureList::iterator itVec1,
		itVec2;  // We set an iterator for the vector
	for (itVec1 = final_list.begin();
		 itVec1 != final_list.end();)  // ... we search it into the vector
	{
		for (itVec2 = list2.begin(); itVec2 != list2.end(); itVec2++)
		{
			if (itVec1->second->ID == itVec2->first->ID) break;
		}  // end for itVec2
		if (itVec2 == list2.end())
...
void matching(data_st* data)
{
	...
	tictac.Tic();
	mrpt::vision::utils::matchFeatures2(
		data->featsHarris, data->featsHarris2, data->Harris_matched, opt);
	...
	tictac.Tic();
	mrpt::vision::utils::matchFeatures2(
		data->featsHarris2, data->featsHarris, data->Harris_matched_inv, opt);
	cout << "Detected " << data->Harris_matched_inv.size()
		 << " Harris matches 'INV' in " << endl;
	cout << format("  %.03fms", tictac.Tac() * 1000) << endl;
	data->Harris_matched_inv.saveToTextFile("Harris_matches_inv.txt");

	// Only matches found in both ways are kept in a new file (Detect matches in
	// only one way and delete it)
	data->Harris_matched2ways =
		comp2Matched_lists(data->Harris_matched, data->Harris_matched_inv);
...
void matching2(data_st* data)
{
	...
	// Detect matches in only one way and delete it (Only matches found in both
	// ways are kept in a new file)
	data->Harris_matched2ways =
		comp2Matched_lists(data->Harris_matched, data->Harris_matched_inv);
	cout << data->Harris_matched2ways.size() << " Harris matches in both ways\n"
		 << endl;
	...
	//SIFT
		...
		tictac.Tic();
		mrpt::vision::utils::matchFeatures2(data->featsSIFT_Hess,
	data->featsSIFT_Hess2, data->SIFT_matched, opt);
		cout << "Detected " << data->SIFT_matched.size() << " SIFT matches in "
	<<
	endl;
		cout << format("  %.03fms",tictac.Tac()*1000) << endl;
	...
	//SURF
		...
		opt.matching_method=TMatchingOptions::mmDescriptorSURF;
		tictac.Tic();
		mrpt::vision::utils::matchFeatures2(data->featsSURF, data->featsSURF2,
	data->SURF_matched, opt);
		cout << "Detected " << data->SURF_matched.size() << " SURF matches in "
	<<
	endl;
		cout << format("  %.03fms",tictac.Tac()*1000) << endl;
...
void getFMat(data_st* data)
{
	...
	else
	{
		...
		for (i = 0, it = data->Harris_matched.begin();
			 it != data->Harris_matched.end(); it++, i++)
		{
		...
		int fm_count = cvFindFundamentalMat(
			points1, points2, fundamental_matrix, CV_FM_RANSAC, 1, 0.99,
			status);
		...
		if (fm_count != 0)
		{
			...
			for (i = 0; i < 3; i++)
			{
				...
				{
					data->FM_Harris(i, j) =
						cvGetReal2D(fundamental_matrix, i, j);
	...
	else
	{
		...
		/* Fill the points here ... */
		for (i = 0, it = data->Harris_matched_inv.begin();
			 it != data->Harris_matched_inv.end(); it++, i++)
		{
			points1->data.fl[i * 2] = it->first->x;
			points1->data.fl[i * 2 + 1] = it->first->y;
			points2->data.fl[i * 2] = it->second->x;
	...
	else
	{
		...
		/* Fill the points here ... */
		for (i = 0, it = data->Harris_matched2ways.begin();
			 it != data->Harris_matched2ways.end(); it++, i++)
		{
			points1->data.fl[i * 2] = it->first->x;
			points1->data.fl[i * 2 + 1] = it->first->y;
			points2->data.fl[i * 2] = it->second->x;
	...
	// SIFT
		...
		else
		{
			...
			// Fill the points here ...
			for(i=0, it=data->SIFT_matched.begin();
	it!=data->SIFT_matched.end();
	it++,i++)
			{
				points1->data.fl[i*2] = it->first->x;
				points1->data.fl[i*2+1] = it->first->y;
	...
	// SURF
		...
		else
		{
			...
			// Fill the points here ...
			for(i=0, it=data->SURF_matched.begin();
	it!=data->SURF_matched.end();
	it++,i++)
			{
				points1->data.fl[i*2] = it->first->x;
				points1->data.fl[i*2+1] = it->first->y;
...
void getFMat_from_txt(data_st* data)
{
	...
	else
	{
		...
		int fm_count = cvFindFundamentalMat(
			points1, points2, fundamental_matrix, CV_FM_RANSAC, 2, 0.8, status);
		...
		if (fm_count != 0)
		{
			...
			cout << "\n";
			mat.open("FM_Harris.txt");
			if (!mat.is_open()) cout << "Fundamental Matrix file not opened";
			for (i = 0; i < 3; i++)
			{
				for (j = 0; j < 3; j++)
				{
	...
	else
	{
		...
		{
			cout << "\n\nFUNDAMENTAL MATRIX SIFT:\n\n";
			for (i = 0; i < point_count; i++) cout << status->data.ptr[i];
			cout << "\n";
			mat.open("FM_SIFT.txt");
			for (i = 0; i < 3; i++)
			{
...
// void getAffineMat(CMatrixDouble16 first, CMatrixDouble16 second, CvMat
// affine)
//{
...
void FeaturesStream(void* Pdata)
{
	...
	int k = 2;
	string pictname, fharrisname, fharrismatch, fharrismatch_inv,
		fharrismatch2ways, FM1, FM2, FM_;
...
void Tracking(void* Pdata)
{
	...
	while (data->pict2.loadFromFile(format("img%i.jpg", (k + 1))))
	{
		...
		//	for(it=data->featsHarris2.begin(); it!=data->featsHarris2.end();
		// it++)
		//	{
		//		if((*it)->x < 0)
		//			(*it).
...
int main()
{
	...
	data.features_taken = false, data.matching_done = false,
	data.fm_calculated = false;
	// string line;
		...
		if (errormsg.empty())
		{
			...
			do
			{
				...
				switch (decision)
				{
					...
					case 'i':  // fordward
						act = 7;
						cout << "Running path Fordward mode" << endl;
						break;
					case 'o':  // backward
						act = 8;
						cout << "Running path Backward mode" << endl;
						break;
					// Speed control
					case '-':
						act = 0;
					...
					case 'v':
						...
						else  // close the window
						{
							cout << "VIDEO OFF" << endl;
							data.showvideo =
								!data.showvideo;  // this kill the thread
					...
					case 'f':  // Take Features
						...
						output = false;
						data.features_taken =
							false;  // this var checks if the function
						// TestExtractFeatures has finished
					...
					case 'b':  // Match Features without epipolar restriction
						...
						break;
					case '1':  // Change the matching to complete matching, get
						// all the
						// correspondences using epipolar restriction
						act = 0;
						output = false;
						data.matching_done =
							false;  // this var checks if the function
					...
					case '/':  // Calculate Fundamental Matrix
						...
						break;
					case '2':  // Calculate the Translation & Rotation Matrix
						// for the
						// camera
						act = 0;
						output = false;
					...
					case '3':  // Get the camera's intrinsic matrix
						act = 0;
						output = false;
						data.intrinsic_matrix.loadFromTextFile(
					...
					case '7':  // Rectify images
						intrinsic_matrix2.loadFromTextFile(
							"intrinsic_matrix.txt");
						distortion_matrix2.loadFromTextFile(
						...
						{
							Picture.rectifyImageInPlace(
								intrinsic_matrix2, distortion2);
