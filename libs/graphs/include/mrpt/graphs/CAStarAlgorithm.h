   ...
   public:
		...
		double maxComputationTime = HUGE_VAL)
	{
		...
		while (!partialSols.empty())
		{
			...
			// then also
			// is every solution in the set. So the algorithm returns
			// immediately.
			...
			// So, if
			// the solution is complete, the optimal solution and the upper
			// bound are
			// updated.
			...
			// is
			// included in the set only if it's valid and it's not yet present
			// in the
			// set.
