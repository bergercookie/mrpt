...
public:
  ...
  /**
    * Client code must implement this method.
    * Returns true if the given solution is acceptable, that is, doesn't violate
   * the problem logic.
    */
  virtual bool isSolutionValid(const T &sol) = 0;
  /**
    * Client code must implement this method.
    * Given a partial solution, returns all its children solution, regardless of
   * their validity or completeness.
    */
...
public:
  /**
    * Finds the optimal solution for a problem, using the A* algorithm. Returns
   * whether an optimal solution was actually found.
    * Returns 0 if no solution was found, 1 if an optimal solution was found and
   * 2 if a (possibly suboptimal) solution was found but the time lapse ended.
    */
  int getOptimalSolution(const T &initialSol, T &finalSol,
    ...
    // The best known solution is set to the upper bound (positive infinite, if
    // there is no given parameter).
    ...
    while (!partialSols.empty()) {
      ...
      if (time.Tac() >= maxComputationTime)
        return found ? 2 : 0;
      typename std::multimap<double, T>::iterator it = partialSols.begin();
      double tempCost = it->first;
      // If the minimum estimated cost is higher than the upper bound, then also
      // is every solution in the set. So the algorithm returns immediately.
      if (tempCost >= currentOptimal)
        return found ? 1 : 0;
      T tempSol = it->second;
      partialSols.erase(it);
      // At this point, the solution cost is lesser than the upper bound. So, if
      // the solution is complete, the optimal solution and the upper bound are
      // updated.
      ...
      // If the solution is not complete, check for its children. Each one is
      // included in the set only if it's valid and it's not yet present in the
      // set.
      ...
      for (typename std::vector<T>::const_iterator it2 = children.begin();
        ...
        if (isSolutionValid(*it2)) {
          ...
          typename std::pair<typename std::multimap<double, T>::const_iterator,
          ...
          if (!alreadyPresent)
            partialSols.insert(std::pair<double, T>(getTotalCost(*it2), *it2));
        }
    }
