...
template <class graph_t> struct graph_ops {
  static void write_VERTEX_line(const TNodeID id, const mrpt::poses::CPose2D &p,
                                std::ofstream &f) {
    //  VERTEX2 id x y phi
    f << "VERTEX2 " << id << " " << p.x() << " " << p.y() << " " << p.phi();
  }
  static void write_VERTEX_line(const TNodeID id, const mrpt::poses::CPose3D &p,
                                std::ofstream &f) {
  ...
  static void write_EDGE_line(const TPairNodeIDs &edgeIDs,
                              const CPosePDFGaussianInf &edge,
                              std::ofstream &f) {
    //  EDGE2 from_id to_id Ax Ay Aphi inf_xx inf_xy inf_yy inf_pp inf_xp inf_yp
    // **CAUTION** TORO docs say "from_id" "to_id" in the opposite order, but it
    // seems from the data that this is the correct expected format.
  ...
  static void write_EDGE_line(const TPairNodeIDs &edgeIDs,
                              ...
                              std::ofstream &f) {
    //  EDGE3 from_id to_id Ax Ay Az Aroll Apitch Ayaw inf_11 inf_12 .. inf_16
    //  inf_22 .. inf_66
    // **CAUTION** In the TORO graph format angles are in the RPY order vs.
    // MRPT's YPR.
    // **CAUTION** TORO docs say "from_id" "to_id" in the opposite order, but it
    ...
    f << "EDGE3 " << edgeIDs.first << " " << edgeIDs.second << " "
      ...
      << " " << edge.cov_inv(0, 0) << " " << edge.cov_inv(0, 1) << " "
      << edge.cov_inv(0, 2) << " " << edge.cov_inv(0, 5) << " "
      << edge.cov_inv(0, 4) << " " << edge.cov_inv(0, 3) << " "
      << edge.cov_inv(1, 1) << " " << edge.cov_inv(1, 2) << " "
      << edge.cov_inv(1, 5) << " " << edge.cov_inv(1, 4) << " "
      << edge.cov_inv(1, 3) << " " << edge.cov_inv(2, 2) << " "
      << edge.cov_inv(2, 5) << " " << edge.cov_inv(2, 4) << " "
      << edge.cov_inv(2, 3) << " " << edge.cov_inv(5, 5) << " "
      << edge.cov_inv(5, 4) << " " << edge.cov_inv(5, 3) << " "
      << edge.cov_inv(4, 4) << " " << edge.cov_inv(4, 3) << " "
      << edge.cov_inv(3, 3) << endl;
  }
  static void write_EDGE_line(const TPairNodeIDs &edgeIDs,
                              const CPosePDFGaussian &edge, std::ofstream &f) {
  ...
  // =================================================================
  static void save_graph_of_poses_to_text_file(const graph_t *g,
    ...
    for (typename graph_t::const_iterator it = g->begin(); it != g->end(); ++it)
      if (it->first.first != it->first.second) // Ignore self-edges, typically
                                               // from importing files with
                                               // EQUIV's
  ...
  // =================================================================
  static void save_graph_of_poses_to_binary_file(const graph_t *g,
  ...
  // =================================================================
  static void read_graph_of_poses_from_binary_file(graph_t *g,
  ...
  // =================================================================
  static void load_graph_of_poses_from_text_file(graph_t *g,
                                                 const std::string &fil) {
    ...
    set<string> alreadyWarnedUnknowns; // for unknown line types, show a warning
                                       // to cerr just once.

    // First, empty the graph:
    g->clear();

    // Determine if it's a 2D or 3D graph, just to raise an error if loading a
    // 3D graph in a 2D one, since
    ...
    map<TNodeID, TNodeID> lstEquivs; // List of EQUIV entries: NODEID ->
                                     // NEWNODEID. NEWNODEID will be always the
                                     // lowest ID number.
    ...
    while (filParser.getNextLine(s)) {
      ...
      if (!(s >> key) || key.empty())
        THROW_EXCEPTION(
            format("Line %u: Can't read string for entry type in: '%s'",
                   lineNum, lin.c_str()));
      ...
      if (mrpt::system::strCmpI(key, "EQUIV")) {
        ...
        if (!(s >> id1 >> id2))
          THROW_EXCEPTION(
              format("Line %u: Can't read id1 & id2 in EQUIV line: '%s'",
                     lineNum, lin.c_str()));
    ...
    while (filParser.getNextLine(s)) {
      ...
      if (strCmpI(key, "VERTEX2") || strCmpI(key, "VERTEX") ||
        ...
        if (!(s >> id >> p2D.x >> p2D.y >> p2D.phi))
          THROW_EXCEPTION(format("Line %u: Error parsing VERTEX2 line: '%s'",
                                 lineNum, lin.c_str()));

        // Make sure the node is new:
        if (g->nodes.find(id) != g->nodes.end())
          THROW_EXCEPTION(
              format("Line %u: Error, duplicated verted ID %u in line: '%s'",
        ...
        if (g->nodes.find(id) == g->nodes.end()) {
          typedef
              typename CNetworkOfPoses<CPOSE>::constraint_t::type_value pose_t;
          pose_t &newNode = g->nodes[id];
          newNode =
              pose_t(CPose2D(p2D)); // Convert to mrpt::poses::CPose3D if needed
        }
      } else if (strCmpI(key, "VERTEX3")) {
        if (!graph_is_3D)
          THROW_EXCEPTION(
              format("Line %u: Try to load VERTEX3 into a 2D graph: '%s'",
                     lineNum, lin.c_str()));
        ...
        // **CAUTION** In the TORO graph format angles are in the RPY order vs.
        // MRPT's YPR.
        if (!(s >> id >> p3D.x >> p3D.y >> p3D.z >> p3D.roll >> p3D.pitch >>
              p3D.yaw))
          THROW_EXCEPTION(format("Line %u: Error parsing VERTEX3 line: '%s'",
                                 lineNum, lin.c_str()));
        ...
        if (g->nodes.find(id) == g->nodes.end()) {
          g->nodes[id] =
              typename CNetworkOfPoses<CPOSE>::constraint_t::type_value(
                  CPose3D(p3D)); // Auto converted to CPose2D if needed
        }
      } else if (strCmpI(key, "VERTEX_SE3:QUAT")) {
        if (!graph_is_3D)
          THROW_EXCEPTION(format(
              "Line %u: Try to load VERTEX_SE3:QUAT into a 2D graph: '%s'",
              lineNum, lin.c_str()));
        ...
        if (!(s >> id >> p3D.x >> p3D.y >> p3D.z >> p3D.qx >> p3D.qy >>
              p3D.qz >> p3D.qr))
          THROW_EXCEPTION(
              format("Line %u: Error parsing VERTEX_SE3:QUAT line: '%s'",
                     lineNum, lin.c_str()));
        ...
        if (g->nodes.find(id) == g->nodes.end()) {
          g->nodes[id] =
              typename CNetworkOfPoses<CPOSE>::constraint_t::type_value(CPose3D(
                  CPose3DQuat(p3D))); // Auto converted to CPose2D if needed
        }
        ...
        if (!(s >> from_id >> to_id))
          THROW_EXCEPTION(format("Line %u: Error parsing EDGE2 line: '%s'",
                                 lineNum, lin.c_str()));
        ...
        if (from_id !=
            to_id) // Don't load self-edges! (probably come from an EQUIV)
        {
          TPose2D Ap_mean;
          ...
          TPosePDFHelper<CPOSE>::copyFrom2D(
              newEdge, CPosePDFGaussianInf(CPose2D(Ap_mean), Ap_cov_inv));
          g->insertEdge(from_id, to_id, newEdge);
        }
      } else if (strCmpI(key, "EDGE3")) {
        if (!graph_is_3D)
          THROW_EXCEPTION(
              format("Line %u: Try to load EDGE3 into a 2D graph: '%s'",
                     lineNum, lin.c_str()));
        ...
        TNodeID to_id, from_id;
        if (!(s >> from_id >> to_id))
          THROW_EXCEPTION(format("Line %u: Error parsing EDGE3 line: '%s'",
                                 lineNum, lin.c_str()));
            ...
            to_id) // Don't load self-edges! (probably come from an EQUIV)
        {
          ...
          // **CAUTION** In the TORO graph format angles are in the RPY order
          // vs. MRPT's YPR.
          if (!(s >> Ap_mean.x >> Ap_mean.y >> Ap_mean.z >> Ap_mean.roll >>
                Ap_mean.pitch >> Ap_mean.yaw))
            THROW_EXCEPTION(format("Line %u: Error parsing EDGE3 line: '%s'",
                                   lineNum, lin.c_str()));

          // **CAUTION** Indices are shuffled to the change YAW(3) <-> ROLL(5)
          // in the order of the data.
          if (!(s >> Ap_cov_inv(0, 0) >> Ap_cov_inv(0, 1) >> Ap_cov_inv(0, 2) >>
                Ap_cov_inv(0, 5) >> Ap_cov_inv(0, 4) >> Ap_cov_inv(0, 3) >>
                Ap_cov_inv(1, 1) >> Ap_cov_inv(1, 2) >> Ap_cov_inv(1, 5) >>
                Ap_cov_inv(1, 4) >> Ap_cov_inv(1, 3) >> Ap_cov_inv(2, 2) >>
                Ap_cov_inv(2, 5) >> Ap_cov_inv(2, 4) >> Ap_cov_inv(2, 3) >>
                Ap_cov_inv(5, 5) >> Ap_cov_inv(5, 4) >> Ap_cov_inv(5, 3) >>
                Ap_cov_inv(4, 4) >> Ap_cov_inv(4, 3) >> Ap_cov_inv(3, 3))) {
          ...
          TPosePDFHelper<CPOSE>::copyFrom3D(
              newEdge, CPose3DPDFGaussianInf(CPose3D(Ap_mean), Ap_cov_inv));
          g->insertEdge(from_id, to_id, newEdge);
        }
        ...
        if (!(s >> from_id >> to_id))
          THROW_EXCEPTION(
              format("Line %u: Error parsing EDGE_SE3:QUAT line: '%s'", lineNum,
                     lin.c_str()));
            ...
            to_id) // Don't load self-edges! (probably come from an EQUIV)
        {
          TPose3DQuat Ap_mean;
          mrpt::math::CMatrixDouble66 Ap_cov_inv;
          if (!(s >> Ap_mean.x >> Ap_mean.y >> Ap_mean.z >> Ap_mean.qx >>
                Ap_mean.qy >> Ap_mean.qz >> Ap_mean.qr))
            THROW_EXCEPTION(
                format("Line %u: Error parsing EDGE_SE3:QUAT line: '%s'",
                       lineNum, lin.c_str()));
          ...
          TPosePDFHelper<CPOSE>::copyFrom3D(
              newEdge,
              CPose3DPDFGaussianInf(CPose3D(CPose3DQuat(Ap_mean)), Ap_cov_inv));
          g->insertEdge(from_id, to_id, newEdge);
        }
      } else if (strCmpI(key, "EQUIV")) {
        // Already read in the 1st pass.
      } else { // Unknown entry: Warn the user just once:
        if (alreadyWarnedUnknowns.find(key) == alreadyWarnedUnknowns.end()) {
          ...
          cerr << "[CNetworkOfPoses::loadFromTextFile] " << fil << ":"
               << lineNum << ": Warning: unknown entry type: " << key << endl;
        }
      }
    } // end while
  ...
  //  mean
  //   & covariance (or information matrix) corresponding to the Bayesian fusion
  //   of all the Gaussians.
  // --------------------------------------------------------------------------------
  static size_t graph_of_poses_collapse_dup_edges(graph_t *g) {
    ...
    for (TEdgeIterator itEd = g->edges.begin(); itEd != g->edges.end();
      ...
      const pair<TNodeID, TNodeID> arc_id =
          make_pair(std::min(itEd->first.first, itEd->first.second),
  ...
  static void graph_of_poses_dijkstra_init(graph_t *g) {
    ...
    struct VisitorComputePoses : public dijkstra_t::tree_graph_t::Visitor {
      ...
      virtual void OnVisitNode(
          ...
          const typename dijkstra_t::tree_graph_t::Visitor::tree_t::TEdgeInfo
              &edge_to_child,
        ...
        if ((!edge_to_child.reverse && !m_g->edges_store_inverse_poses) ||
            (edge_to_child.reverse &&
             m_g->edges_store_inverse_poses)) { // pose_child = p_parent (+)
                                                // p_delta
          m_g->nodes[child_id].composeFrom(m_g->nodes[parent_id],
                                           edge_to_child.data->getPoseMean());
        } else { // pose_child = p_parent (+) [(-)p_delta]
          m_g->nodes[child_id].composeFrom(m_g->nodes[parent_id],
                                           -edge_to_child.data->getPoseMean());
        }
      }
    ...
    if (!empty_node_annots) {
      for (typename graph_t::global_poses_t::const_iterator poses_cit =
        ...
        nodeID_to_annots.insert(
            make_pair(poses_cit->first, poses_cit->second.getCopyOfAnnots()));
      }
    }
  ...
  static inline double auxMaha2Dist(VEC &err, const CPosePDFGaussianInf &p) {
    ...
    return mrpt::math::multiply_HCHt_scalar(err,
                                            p.cov_inv); // err^t*cov_inv*err
  }
  template <class VEC>
  ...
  // These two are for simulating maha2 distances for non-PDF types: fallback to
  // squared-norm:
  template <class VEC>
  ...
  static inline double auxMaha2Dist(VEC &err, const mrpt::poses::CPose3D &p) {
    ...
    math::wrapToPiInPlace(err[4]);
    math::wrapToPiInPlace(err[5]);
    return square(err[0]) + square(err[1]) + square(err[2]) + square(err[3]) +
           square(err[4]) + square(err[5]);
  }

  static inline double auxEuclid2Dist(const mrpt::poses::CPose2D &p1,
                                      const mrpt::poses::CPose2D &p2) {
    return square(p1.x() - p2.x()) + square(p1.y() - p2.y()) +
           square(mrpt::math::wrapToPi(p1.phi() - p2.phi()));
  }
  static inline double auxEuclid2Dist(const mrpt::poses::CPose3D &p1,
  ...
  static double graph_edge_sqerror(
      ...
      const typename mrpt::graphs::CDirectedGraph<
          typename graph_t::constraint_t>::edges_map_t::const_iterator &itEdge,
    ...
    typename graph_t::global_poses_t::const_iterator itPoseTo =
        g->nodes.find(to_id);
    ASSERTMSG_(itPoseFrom != g->nodes.end(),
               format("Node %u doesn't have a global pose in 'nodes'.",
                      static_cast<unsigned int>(from_id)))
    ASSERTMSG_(itPoseTo != g->nodes.end(),
               format("Node %u doesn't have a global pose in 'nodes'.",
    ...
    if (ignoreCovariances) { // Square Euclidean distance: Just use the mean
                             // values, ignore covs.
