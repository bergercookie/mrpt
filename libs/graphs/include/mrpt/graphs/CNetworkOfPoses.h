 ...
 *		- \a edges  (in the base class
 *mrpt::graphs::CDirectedGraph::edges): A map from pairs of node ID -> pose
 *constraints.
 ...
 *		- CPOSE: The type of the edges, which hold a relative pose (2D/3D,
 *just a value or a Gaussian, etc.)
 *		- MAPS_IMPLEMENTATION: Can be either
 *mrpt::utils::map_traits_stdmap or mrpt::utils::map_traits_map_as_vector.
 *Determines the type of the list of global poses (member \a nodes).
 *
 * \sa mrpt::graphslam
...
public:
  /** @name Typedef's
      @{ */
  typedef mrpt::graphs::CDirectedGraph<CPOSE, EDGE_ANNOTATIONS>
      BASE; //!< The base class "CDirectedGraph<CPOSE,EDGE_ANNOTATIONS>" */
  typedef CNetworkOfPoses<CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS,
                          EDGE_ANNOTATIONS>
      self_t; //!< My own type

  typedef CPOSE constraint_t; //!< The type of PDF poses in the contraints
                              //!(edges) (=CPOSE template argument)
  typedef NODE_ANNOTATIONS node_annotations_t; //!< The extra annotations in
                                               //!nodes, apart from a \a
                                               //!constraint_no_pdf_t
  typedef EDGE_ANNOTATIONS edge_annotations_t; //!< The extra annotations in
                                               //!edges, apart from a \a
                                               //!constraint_t

  typedef MAPS_IMPLEMENTATION maps_implementation_t; //!< The type of map's
                                                     //!implementation
                                                     //!(=MAPS_IMPLEMENTATION
                                                     //!template argument)
  typedef typename CPOSE::type_value constraint_no_pdf_t; //!< The type of edges
                                                          //!or their means if
                                                          //!they are PDFs (that
                                                          //!is, a simple "edge"
                                                          //!value)
  ...
  struct global_pose_t : public constraint_no_pdf_t, public NODE_ANNOTATIONS {
    typedef
        typename CNetworkOfPoses<CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS,
    ...
    inline bool operator==(const global_pose_t &other) const {
      return (static_cast<const constraint_no_pdf_t>(*this) ==
   ...
   *   If an unknown entry is found, a warning is dumped to std::cerr (only once
   * for each unknown keyword).
   *   An exception will be raised if trying to load a 3D graph into a 2D class
   * (in the opposite case, missing 3D data will default to zero).
   *
   ...
   * a 2D graph.
   */
  inline void loadFromTextFile(const std::string &fileName,
                               bool collapse_dup_edges = true) {
    detail::graph_ops<self_t>::load_graph_of_poses_from_text_file(this,
                                                                  fileName);
    if (collapse_dup_edges)
      this->collapseDuplicatedEdges();
  }
   ...
   */
  inline void
  getAs3DObject(mrpt::opengl::CSetOfObjects::Ptr object,
  ...
  inline size_t collapseDuplicatedEdges() {
    return detail::graph_ops<self_t>::graph_of_poses_collapse_dup_edges(this);
  }

  /** Computes the overall square error from all the pose constraints (edges)
   * with respect to the global poses in \a nodes
   *  If \a ignoreCovariances is false, the squared Mahalanobis distance will be
   * computed instead of the straight square error.
  ...
  double getGlobalSquareError(bool ignoreCovariances = true) const {
    ...
    for (typename BASE::edges_map_t::const_iterator itEdge =
      ...
      sqErr += detail::graph_ops<self_t>::graph_edge_sqerror(this, itEdge,
                                                             ignoreCovariances);
    return sqErr;
  }
  ...
  void extractSubGraph(const std::set<TNodeID> &node_IDs, self_t *sub_graph,
    ...
    ASSERTMSG_(sub_graph, "\nInvalid pointer to a CNetworkOfPoses instance is "
                          "given. Exiting..\n");
    sub_graph->clear();
    ...
    for (std::set<TNodeID>::const_iterator node_IDs_it = node_IDs_real.begin();
    ...
    if (!auto_expand_set && !is_fully_connected_graph) {
      ...
      // as long as the graph is unconnected (as indicated by Dijkstra) add a
      // virtual edge between
      ...
      while (!dijkstra_runs_successfully) {
        try {
          ...
          // mrpt::math::getSTLContainerAsString(unconnected_nodeIDs) << endl;
          ...
          for (typename global_poses_t::const_iterator n_it =
            ...
            for (typename std::set<TNodeID>::const_iterator uncon_it =
          ...
          if (is_single_island) { // single island
            ...
            // | mainland                       |                            |
            // island                       |
            // | <low nodeIDs>  mainland_highest|  --- <virtual_edge> --->>  |
            // island_lowest <high nodeIDs> |
            ...
            // add a virtual edge between the last group before the mainland and
            // the mainland
            ...
            // cout << "last_island: " <<
            // getSTLContainerAsString(vec_of_islands.back()) << endl;
            // cout << "mainland: " << getSTLContainerAsString(mainland) <<
            // endl;
            this->connectGraphPartitions(sub_graph, vec_of_islands.back(),
                                         mainland);
          }
        }
      }
   ...
   *
   * \param[in] other Graph (of the same type) that is to be integrated with own
   * graph.
  ...
  inline void mergeGraph(
    ...
    for (nodes_cit_t n_cit = this->nodes.begin(); n_cit != this->nodes.end();
         ++n_cit) {
    ...
    // cout << "====================" << endl;
    for (nodes_cit_t n_cit = other.nodes.begin(); n_cit != other.nodes.end();
         ++n_cit) {
   ...
   */
  inline static void connectGraphPartitions(self_t *sub_graph,
    ...
    ASSERTMSG_(*groupA.rend() < *groupB.rbegin() ||
   ...
   */
  inline double
  getEdgeSquareError(const typename BASE::edges_map_t::const_iterator &itEdge,
                     bool ignoreCovariances = true) const {

    return detail::graph_ops<self_t>::graph_edge_sqerror(this, itEdge,
                                                         ignoreCovariances);
  }
   ...
   * \a nodes
   */
  double getEdgeSquareError(const mrpt::utils::TNodeID from_id,
    ...
    ASSERTMSG_(itEdge != BASE::edges.end(),
               format("Request for edge %u->%u that doesn't exist in graph.",
...
private:
   ...
   */
  inline static void addVirtualEdge(self_t *graph, const TNodeID &from,
                                    const TNodeID &to) {
...
template <class CPOSE, class MAPS_IMPLEMENTATION, class NODE_ANNOTATIONS,
          class EDGE_ANNOTATIONS>
mrpt::utils::CStream &
operator<<(mrpt::utils::CStream &out,
...
template <class CPOSE, class MAPS_IMPLEMENTATION, class NODE_ANNOTATIONS,
          class EDGE_ANNOTATIONS>
mrpt::utils::CStream &
operator>>(mrpt::utils::CStream &in,
...
typedef CNetworkOfPoses<mrpt::poses::CPose2D, mrpt::utils::map_traits_stdmap>
    CNetworkOfPoses2D; //!< The specialization of CNetworkOfPoses for poses of
                       //!type CPose2D (not a PDF!), also implementing
                       //!serialization.
typedef CNetworkOfPoses<mrpt::poses::CPose3D, mrpt::utils::map_traits_stdmap>
    CNetworkOfPoses3D; //!< The specialization of CNetworkOfPoses for poses of
                       //!type mrpt::poses::CPose3D (not a PDF!), also
                       //!implementing serialization.
typedef CNetworkOfPoses<mrpt::poses::CPosePDFGaussian,
                        mrpt::utils::map_traits_stdmap>
    CNetworkOfPoses2DCov; //!< The specialization of CNetworkOfPoses for poses
                          //!of type CPosePDFGaussian, also implementing
                          //!serialization.
typedef CNetworkOfPoses<mrpt::poses::CPose3DPDFGaussian,
                        mrpt::utils::map_traits_stdmap>
    CNetworkOfPoses3DCov; //!< The specialization of CNetworkOfPoses for poses
                          //!of type CPose3DPDFGaussian, also implementing
                          //!serialization.
typedef CNetworkOfPoses<mrpt::poses::CPosePDFGaussianInf,
                        mrpt::utils::map_traits_stdmap>
    CNetworkOfPoses2DInf; //!< The specialization of CNetworkOfPoses for poses
                          //!of type CPosePDFGaussianInf, also implementing
                          //!serialization.
typedef CNetworkOfPoses<mrpt::poses::CPose3DPDFGaussianInf,
                        mrpt::utils::map_traits_stdmap>
    CNetworkOfPoses3DInf; //!< The specialization of CNetworkOfPoses for poses
                          //!of type CPose3DPDFGaussianInf, also implementing
                          //!serialization.
