...
namespace mrpt {
namespace graphs {
using mrpt::utils::TNodeID; //!< Make available this typedef in this namespace
                            //!too
using mrpt::utils::TPairNodeIDs; //!< Make available this typedef in this
                                 //!namespace too
...
struct TGraphvizExportParams {
  bool mark_edges_as_not_constraint; //!< If true (default=false), an
                                     //!"[constraint=false]" will be added to
                                     //!all edges (see Graphviz docs).
  std::map<TNodeID, std::string> node_names; //!< If provided, these textual
                                             //!names will be used for naming
                                             //!the nodes instead of their
                                             //!numeric IDs given in the edges.
  std::map<TNodeID, std::string> node_props; //!< If provided, an extra line
                                             //!will be added setting Graphviz
                                             //!properties for each node, e.g.
                                             //!to set a node position, use the
                                             //!string "pos = \"x,y\""
...
public:
  ...
  typedef typename mrpt::aligned_containers<TPairNodeIDs, edge_t>::multimap_t
      edges_map_t; //!< The type of the member \a edges
  ...
  inline CDirectedGraph(const edges_map_t &obj)
      : edges(obj) {} //!< Copy constructor from a multimap<pair< >, >
   ...
   */
  inline void insertEdge(TNodeID from_nodeID, TNodeID to_nodeID,
  ...
  inline void insertEdgeAtEnd(TNodeID from_nodeID, TNodeID to_nodeID,
                              const edge_t &edge_value) {
    MRPT_ALIGN16 typename edges_map_t::value_type entry(
        std::make_pair(from_nodeID, to_nodeID), edge_value);
  ...
  edge_t &getEdge(TNodeID from_nodeID, TNodeID to_nodeID) {
    ...
    if (it == edges.end())
      THROW_EXCEPTION(format("Edge %u->%u does not exist",
                             (unsigned)from_nodeID, (unsigned)to_nodeID))
    else
      return it->second;
  }
  ...
  std::pair<iterator, iterator> getEdges(TNodeID from_nodeID,
                                         TNodeID to_nodeID) {
    return edges.equal_range(std::make_pair(from_nodeID, to_nodeID));
  }
  /** Return a pair<first,last> of const iterators to the range of edges between
   * two given nodes.  \sa getEdge */
  std::pair<const_iterator, const_iterator> getEdges(TNodeID from_nodeID,
                                                     TNodeID to_nodeID) const {
    return edges.equal_range(std::make_pair(from_nodeID, to_nodeID));
  }
   ...
   * \sa getAllNodes( std::set<TNodeID> &lstNode_IDs) */
  ...
  /** Return the list of all neighbors of "nodeID", by creating a list of their
   * node IDs. \sa getAdjacencyMatrix */
  void getNeighborsOf(const TNodeID nodeID,
                      std::set<TNodeID> &neighborIDs) const {
    neighborIDs.clear();
    for (typename edges_map_t::const_iterator it = edges.begin();
         it != edges.end(); ++it) {
  ...
  /** Return a map from node IDs to all its neighbors (that is, connected nodes,
  ...
  void getAdjacencyMatrix(MAP_NODEID_SET_NODEIDS &outAdjacency,
  ...
  /** Save the graph in a Graphviz (.dot files) text format; useful for quickly
   * rendering the graph with "dot"
    * \return false on any error */
  bool
  saveAsDot(std::ostream &o,
    ...
    for (const_iterator it = begin(); it != end(); ++it) {
      ...
      o << " \"" << s1 << "\" -> \"" << s2 << "\"";
      if (p.mark_edges_as_not_constraint)
        o << " [constraint=false]";
      o << ";\n";
    }
    return !((o << "}\n").fail());
  ...
  /** \overload */
  bool
  saveAsDot(const std::string &fileName,
            const TGraphvizExportParams &p = TGraphvizExportParams()) const {
