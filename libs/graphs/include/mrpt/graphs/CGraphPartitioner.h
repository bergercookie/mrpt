...
public:
   ...
   * square matrix, where element W<sub>ij</sub> is the "likelihood" between
   * nodes "i" and "j", and typically W<sub>ii</sub> = 1.
   * \param out_parts		 [OUT] An array of partitions, where each partition
   * is represented as a vector of indexs for nodes.
   ...
   * 0.5*(W<sub>ij</sub>+W<sub>ji</sub>). Set to false if matrix is known to be
   * simetric.
   * \param useSpectralBisection [IN] If set to true (default) a quick spectral
   * bisection will be used. If set to false, a brute force, exact finding of
   * the min-cut is performed.
   ...
   * \exception Throws a std::logic_error if an invalid matrix is passed.
   */
  static void SpectralBisection(GRAPH_MATRIX &in_A, vector_uint &out_part1,
                                vector_uint &out_part2, num_t &out_cut_value,
   ...
   * \exception Throws a std::logic_error if an invalid matrix is passed.
   */
  static void exactBisection(GRAPH_MATRIX &in_A, vector_uint &out_part1,
                             vector_uint &out_part2, num_t &out_cut_value,
                             bool forceSimetry = true);

  /** Returns the normaliced cut of a graph, given its adjacency matrix A and a
   * bisection:
    */
  static num_t nCut(const GRAPH_MATRIX &in_A, const vector_uint &in_part1,
