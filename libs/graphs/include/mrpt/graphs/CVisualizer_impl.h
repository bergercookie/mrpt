...
void CVisualizer<
    ...
    EDGE_ANNOTATIONS>::getAs3DObject(mrpt::opengl::CSetOfObjects::Ptr &object,
...
void CVisualizer<
    CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS,
    EDGE_ANNOTATIONS>::drawGroundGrid(mrpt::opengl::CSetOfObjects::Ptr &object,
                                      const mrpt::utils::TParametersDouble
                                          *viz_params /*=NULL*/) const {
  using namespace mrpt::opengl;
  using namespace mrpt::utils;
  ...
  for (typename GRAPH_T::global_poses_t::const_iterator n_it =
    ...
    const CPose3D p =
        CPose3D(n_it->second); // Convert to 3D from whatever its real type.
...
void CVisualizer<
    CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS,
    EDGE_ANNOTATIONS>::drawNodePoints(mrpt::opengl::CSetOfObjects::Ptr &object,
                                      const mrpt::utils::TParametersDouble
                                          *viz_params /*=NULL*/) const {
  ASSERTMSG_(viz_params, "Pointer to viz_params was not provided.");
...
void CVisualizer<
    CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS,
    EDGE_ANNOTATIONS>::drawNodeCorners(mrpt::opengl::CSetOfObjects::Ptr &object,
                                       const mrpt::utils::TParametersDouble
                                           *viz_params /*=NULL*/) const {
  using namespace mrpt::opengl;
  using namespace mrpt::utils;
  ...
  for (typename GRAPH_T::global_poses_t::const_iterator n_it =
    ...
    // Convert to 3D from whatever its real type.  CSetOfObjects::Ptr gl_corner
    // = show_node_corners ?
...
void CVisualizer<CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS,
  ...
  const TColor col8bit(edge_rel_poses_color & 0xffffff,
                       edge_rel_poses_color >> 24);
  ...
  for (typename GRAPH_T::const_iterator edge_it = m_graph.begin();
    ...
    if (n_it != m_graph.nodes.end()) {
      ...
      mrpt::opengl::CSimpleLine::Ptr gl_line =
          mrpt::opengl::CSimpleLine::Create(0, 0, 0, edge_pose_pt.x(),
                                            edge_pose_pt.y(), edge_pose_pt.z());
...
void CVisualizer<
    CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS,
    EDGE_ANNOTATIONS>::drawEdges(mrpt::opengl::CSetOfObjects::Ptr &object,
                                 const mrpt::utils::TParametersDouble
                                     *viz_params /*=NULL*/) const {
  using namespace mrpt::opengl;
  using namespace mrpt::utils;
  ...
  for (typename GRAPH_T::const_iterator edge_it = m_graph.begin();
    ...
    if (n_it1 != m_graph.nodes.end() &&
        n_it2 != m_graph.nodes.end()) { // both nodes found?
      const CPose3D p1 = CPose3D(n_it1->second);
      const CPose3D p2 = CPose3D(n_it2->second);
      gl_edges->appendLine(mrpt::math::TPoint3D(p1.x(), p1.y(), p1.z()),
