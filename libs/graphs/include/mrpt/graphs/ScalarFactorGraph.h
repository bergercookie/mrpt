...
public:
  ...
  struct GRAPHS_IMPEXP FactorBase {
    virtual ~FactorBase();
    virtual double evaluateResidual()
        const = 0; //!< Return the residual/error of this observation.
    virtual double getInformation()
        const = 0; //!< Return the inverse of the variance of this constraint
  };

  /** Simple, scalar (1-dim) constraint (edge) for a GMRF */
  struct GRAPHS_IMPEXP UnaryFactorVirtualBase : public FactorBase {
    size_t node_id;
    virtual void evalJacobian(double &dr_dx) const = 0; //!< Returns the
                                                        //!derivative of the
                                                        //!residual wrt the node
                                                        //!value
  };
  ...
  struct GRAPHS_IMPEXP BinaryFactorVirtualBase : public FactorBase {
    ...
    virtual void evalJacobian(double &dr_dxi,
                              double &dr_dxj) const = 0; //!< Returns the
                                                         //!derivative of the
                                                         //!residual wrt the
                                                         //!node values
  };
  ...
  bool eraseConstraint(const FactorBase &c); //!< Removes a constraint. Return
  ...
  void updateEstimation(
      Eigen::VectorXd &solved_x_inc, //!< Output increment of the current
                                     //!estimate. Caller must add this vector to
                                     //!current state vector to obtain the
                                     //!optimal estimation.
      Eigen::VectorXd *solved_variances = nullptr //!< If !=nullptr, the
