...
void CGraphPartitioner<GRAPH_MATRIX, num_t>::RecursiveSpectralPartition(
  ...
  if (verbose)
    std::cout << format("Cut:%u=%u+%u,nCut=%.02f->", (unsigned int)nodeCount,
                        (unsigned int)p1.size(), (unsigned int)p2.size(),
                        cut_value);
  ...
  if (cut_value > threshold_Ncut || p1.size() < minSizeClusters ||
    ...
    if (recursive) {
      ...
      RecursiveSpectralPartition(A_1, p1_parts, threshold_Ncut, forceSimetry,
                                 useSpectralBisection, recursive,
                                 minSizeClusters);
      ...
      RecursiveSpectralPartition(A_2, p2_parts, threshold_Ncut, forceSimetry,
                                 useSpectralBisection, recursive,
                                 minSizeClusters);
