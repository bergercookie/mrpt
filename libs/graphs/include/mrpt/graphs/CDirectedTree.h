 ...
 * \ingroup mrpt_graphs_grp
  */
template <class TYPE_EDGES = uint8_t> class CDirectedTree {
public:
  struct TEdgeInfo {
    TNodeID id;      //!< The ID of the child node.
    bool reverse;    //!< True if edge direction is child->parent, false if it's
                     //!parent->child.
    TYPE_EDGES data; //!< User data for this edge.

    /** Edge constructor from data */
    TEdgeInfo(TNodeID child_id_, bool direction_child_to_parent = false,
  ...
  struct Visitor {
    ...
    /** Virtual method to be implemented by the user and which will be called
     * during the visit to a graph with visitDepthFirst or visitBreadthFirst
      *  Specifically, the method will be called once for each <b>edge</b> in
     * the tree.
     ...
     * "edge_to_child.id" (root node is at 0, its children are at 1, etc.).
      */
    virtual void OnVisitNode(const TNodeID parent,
   ...
   * from the visit), invoking a user-provided function for each node/edge. \sa
   * visitBreadthFirst */
  void visitDepthFirst(const TNodeID root, Visitor &user_visitor,
    ...
    if (itChildren == edges_to_children.end())
      return; // No children
    ...
    for (typename TListEdges::const_iterator itEdge = children.begin();
      ...
      visitDepthFirst(itEdge->id, user_visitor,
                      next_depth_level); // Recursive depth-first call.
    }
  }

  /** Breadth-first visit of all children nodes of a given root (itself excluded
   * from the visit), invoking a user-provided function for each node/edge. \sa
   * visitDepthFirst */
  void visitBreadthFirst(const TNodeID root, Visitor &user_visitor,
                         const size_t root_depth_level = 0) const {
    const size_t next_depth_level = root_depth_level + 1;
    typename TMapNode2ListEdges::const_iterator itChildren =
        edges_to_children.find(root);
  ...
  /** Return a text representation of the tree spanned in a depth-first view, as
   * in this example:
    *  \code
    *    0
    *     -> 1
  ...
  std::string getAsTextDescription() const {
    ...
    struct CMyVisitor
      ...
      virtual void
      OnVisitNode(const TNodeID parent,
