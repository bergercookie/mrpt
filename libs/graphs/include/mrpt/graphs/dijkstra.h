...
public:
   ...
   */
  void
  getUnconnectedNodeIDs(std::set<mrpt::utils::TNodeID> *set_nodeIDs) const {
...
protected:
  ...
  // Private typedefs:
  typedef typename MAPS_IMPLEMENTATION::template map<TNodeID, std::set<TNodeID>>
      list_all_neighbors_t; //!< A std::map (or a similar container according to
                            //!MAPS_IMPLEMENTATION) with all the neighbors of
                            //!every node.
...
public:
  ...
  typedef TYPE_GRAPH graph_t; //!< The type of the graph, typically a
                              //!mrpt::graphs::CDirectedGraph<> or any other
                              //!derived class
  typedef typename graph_t::edge_t edge_t; //!< The type of edge data in graph_t
  typedef std::list<TPairNodeIDs>
      edge_list_t; //!< A list of edges used to describe a path on the graph
   ...
   * graph
   */
  CDijkstra(const graph_t &graph, const TNodeID source_node_ID,
            double (*functor_edge_weight)(const graph_t &graph,
                                          const TNodeID id_from,
                                          const TNodeID id_to,
                                          const edge_t &edge) = nullptr,
            void (*functor_on_progress)(const graph_t &graph,
                                        size_t visitedCount) = nullptr)
    ...
    */
    ...
    if (m_lstNode_IDs.find(source_node_ID) == m_lstNode_IDs.end()) {
      THROW_EXCEPTION_FMT("Cannot find the source node_ID=%lu in the graph",
    ...
    do { // The algorithm:
      ...
      // make sure we have found the next nodeID from the available non-visited
      // distances
      if (u == INVALID_NODEID) {
        ...
        std::string err_str = mrpt::format("Graph is not fully connected!");
        throw mrpt::graphs::detail::NotConnectedGraph(nodeIDs_unconnected,
                                                      err_str);
      }

      // Update distance (for possible future reference...) and remove this node
      // from "non-visited":
      ...
      for (std::set<TNodeID>::const_iterator itNei = neighborsOfU.begin();
        ...
        // the "edge_ui" may be searched here or a bit later, so the "bool" var
        // will tell us.
        ...
        if (!functor_edge_weight)
          ...
          edge_ui_weight =
              (*functor_edge_weight)(graph, edge_ui->first.first,
                                     edge_ui->first.second, edge_ui->second);
          edge_ui_found = true;
        }

        if ((min_d + edge_ui_weight) <
            m_distances[i].dist) { // the [] creates the entry if needed
  ...
  inline double getNodeDistanceToRoot(const TNodeID id) const {
    ...
    if (it == m_distances.end())
      THROW_EXCEPTION("Node was not found in the graph when running Dijkstra");
   ...
   * \sa getTreeGraph
   */
  void getShortestPathTo(const TNodeID target_node_ID,
                         edge_list_t &out_path) const {
  ...
  /** Returns a tree representation of the graph, as determined by the Dijkstra
   * shortest paths from the root node.
   * Note that the annotations on each edge in the tree are "const pointers" to
   * the original graph edge data, so
  ...
  void getTreeGraph(tree_graph_t &out_tree) const {
    ...
    for (typename id2pairIDs_map_t::const_iterator itArcs = m_prev_arc.begin();
      ...
      ASSERTMSG_(
          ...
          format("Edge %u->%u is in Dijkstra paths but not in original graph!",
