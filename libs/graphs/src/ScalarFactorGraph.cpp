...
void ScalarFactorGraph::updateEstimation(
    Eigen::VectorXd &solved_x_inc, //!< Output increment of the current
                                   ...
                                   //!current state vector to obtain the optimal
                                   //!estimation.
    Eigen::VectorXd *solved_variances //!< If !=nullptr, the covariance of the
                                      //!estimate will be stored here.
    ) {
  ...
  if (solved_variances) {
    ...
    for (int l = n - 1; l >= 0; l--) {
      if (!(l % show_progress_steps))
        MRPT_LOG_DEBUG_FMT("Computing variance %6.02f%%... \r",
                           (100.0 * (n - l - 1)) / n);
