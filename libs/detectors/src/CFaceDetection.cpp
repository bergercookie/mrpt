...
void CFaceDetection::init(const mrpt::utils::CConfigFileBase &cfg) {
  ...
  m_testsOptions.regionsTest_sumDistThreshold_top =
      cfg.read_double("FaceDetection", "regionsTest_sumDistThreshold_top", 0.5);
...
void CFaceDetection::detectObjects_Impl(const mrpt::obs::CObservation *obs,
  ...
  if ((IS_CLASS(obs, CObservation3DRangeScan)) && (localDetected.size() > 0)) {
    ...
    CObservation3DRangeScan *o =
        static_cast<CObservation3DRangeScan *>(const_cast<CObservation *>(obs));
    ...
    if (o->hasPoints3D) {
      ...
      for (unsigned int i = 0; i < localDetected.size(); i++) {
        ...
        if (m_options.multithread) {
          ...
          // First check if we can adjust a plane to detected region as face, if
          // yes it isn't a face!
          if (m_options.useCovFilter &&
              !checkIfFacePlaneCov(&m_lastFaceDetected)) {
            deleteDetected.push_back(i);
            remove = true;
          } else if (m_options.useRegionsFilter &&
                     !checkIfFaceRegions(&m_lastFaceDetected)) {
            deleteDetected.push_back(i);
            remove = true;
          } else if ((m_options.useSizeDistanceRelationFilter ||
                      m_options.useDiagonalDistanceFilter) &&
...
bool CFaceDetection::checkIfFacePlane(CObservation3DRangeScan *face) {
  ...
  for (size_t i = 0; i < N; i++)
    points[i] = TPoint3D(face->points3D_x.at(i), face->points3D_y.at(i),
  ...
  for (unsigned int j = 0; j < faceHeight; j++) {
    for (unsigned int k = 0; k < faceWidth; k++) {
      ...
      if (region.get_unsafe(j, k) &&
          (((!confidence) ||
            ((confidence) && (*(face->confidenceImage.get_unsafe(k, j, 0)) >
                              m_options.confidenceThreshold) &&
...
bool CFaceDetection::checkIfFaceRegions(CObservation3DRangeScan *face) {
  ...
  if (end == 0)
    end = faceWidth - 1; // Check if the end has't changed
  if (end < 3 * (faceWidth / 4))
    end = 3 * (faceWidth / 4); // To avoid spoiler
  if (start == faceWidth)
    start = 0; // Check if the start has't changed
  if (start > faceWidth / 4)
    start = faceWidth / 4; // To avoid spoiler
  ...
  experimental_calcHist(face->intensityImage, start, 0, end,
                        ceil(faceHeight * 0.1), hist);
  ...
  for (unsigned int r = 0; r < faceHeight; r++) {
    for (unsigned int c = 0; c < faceWidth; c++, cont++) {
      if ((r >= upLimit) && (r <= downLimit) && (region.get_unsafe(r, c)) &&
        ...
        TPoint3D point(face->points3D_x[cont], face->points3D_y[cont],
        ...
        if (row == 0 && col == 0)
          regions2[0].push_back(TPoint3D(face->points3D_x[cont],
                                         face->points3D_y[cont],
                                         face->points3D_z[cont]));
        else if (row == 0 && col == 1)
          regions2[1].push_back(TPoint3D(face->points3D_x[cont],
                                         face->points3D_y[cont],
                                         face->points3D_z[cont]));
        else if (row == 0 && col == 2)
          regions2[2].push_back(TPoint3D(face->points3D_x[cont],
                                         face->points3D_y[cont],
                                         face->points3D_z[cont]));
        else if (row == 1 && col == 0)
          regions2[3].push_back(TPoint3D(face->points3D_x[cont],
                                         face->points3D_y[cont],
                                         face->points3D_z[cont]));
        else if (row == 1 && col == 1)
          regions2[4].push_back(TPoint3D(face->points3D_x[cont],
                                         face->points3D_y[cont],
                                         face->points3D_z[cont]));
        else if (row == 1 && col == 2)
          regions2[5].push_back(TPoint3D(face->points3D_x[cont],
                                         face->points3D_y[cont],
                                         face->points3D_z[cont]));
        else if (row == 2 && col == 0)
          regions2[6].push_back(TPoint3D(face->points3D_x[cont],
                                         face->points3D_y[cont],
                                         face->points3D_z[cont]));
        else if (row == 2 && col == 1)
          regions2[7].push_back(TPoint3D(face->points3D_x[cont],
                                         face->points3D_y[cont],
                                         face->points3D_z[cont]));
        else
          regions2[8].push_back(TPoint3D(face->points3D_x[cont],
                                         face->points3D_y[cont],
                                         face->points3D_z[cont]));
      }
    }
  ...
  if (regions2[0].size() > 0) {
    ...
    nth_element(oldPointsX1.begin(), oldPointsX1.begin() + middle1,
  ...
  if (regions2[2].size() > 0) {
    ...
    nth_element(oldPointsX2.begin(), oldPointsX2.begin() + middle2,
  ...
  size_t res = checkRelativePosition(meanPos[1][0], meanPos[1][2],
                                     meanPos[1][1], dist[0]);
  res += res && checkRelativePosition(meanPos[2][0], meanPos[2][2],
                                      meanPos[2][1], dist[1]);
  res += res && checkRelativePosition(meanPos[0][0], meanPos[0][2],
                                      meanPos[0][1], dist[2]);
  res += res && checkRelativePosition(meanPos[0][0], meanPos[2][2],
                                      meanPos[1][1], dist[3]);
  res += res && checkRelativePosition(meanPos[2][0], meanPos[0][2],
                                      meanPos[1][1], dist[4]);
...
size_t CFaceDetection::checkRelativePosition(const TPoint3D &p1,
                                             const TPoint3D &p2,
                                             const TPoint3D &p, double &dist) {
  double x1 = -p1.y;
  double y1 = p1.x;
  ...
  for (unsigned int i = y1; i <= y2; i++) {
    ...
    for (unsigned int j = x1; j <= x2; j++, cont++) {
      if (*(face->confidenceImage.get_unsafe(j, i, 0)) >
        ...
        total++;
        points.push_back(TPoint3D(face->points3D_x[cont],
                                  face->points3D_y[cont],
                                  face->points3D_z[cont]));
      }
    }
  ...
  for (unsigned int i = y1; i <= y2; i++) {
    ...
    for (unsigned int j = x1; j <= x2; j++, cont++) {
      if (valids.get_unsafe(i, j)) {
        // experimental_calcDiagDist( face, i, j, faceWidth, faceHeight, valids,
        // distance );

        distance = 0;
        if ((i + 1 <= y2) && (j + 1 <= x2)) {
          if (valids.get_unsafe(i + 1, j + 1)) {
            TPoint3D p1(face->points3D_x[cont], face->points3D_y[cont],
                        face->points3D_z[cont]);
            offsetIndex = cont + faceWidth + 1;
            distance = p1.distanceTo(TPoint3D(face->points3D_x[offsetIndex],
            ...
            while (validOffset) {
              if ((i + offset <= y2) && (j + offset <= x2)) {
                if (valids.get_unsafe(i + offset, j + offset)) {
                  ...
                  distance =
                      p1.distanceTo(TPoint3D(face->points3D_x[offsetIndex],
                                             face->points3D_y[offsetIndex],
                                             face->points3D_z[offsetIndex]));
                  break;
                }
...
void CFaceDetection::experimental_viewFacePointsAndEigenVects(
  ...
  TPoint3D E1(eigenVect.get_unsafe(0, 0), eigenVect.get_unsafe(0, 1),
              eigenVect.get_unsafe(0, 2));
  TPoint3D E2(eigenVect.get_unsafe(1, 0), eigenVect.get_unsafe(1, 1),
              eigenVect.get_unsafe(1, 2));
  TPoint3D E3(eigenVect.get_unsafe(2, 0), eigenVect.get_unsafe(2, 1),
              eigenVect.get_unsafe(2, 2));
  ...
  CArrow::Ptr arrow1 =
      std::make_shared<CArrow>(center.x, center.y, center.z, p1.x, p1.y, p1.z);
  CArrow::Ptr arrow2 =
      std::make_shared<CArrow>(center.x, center.y, center.z, p2.x, p2.y, p2.z);
  CArrow::Ptr arrow3 =
      std::make_shared<CArrow>(center.x, center.y, center.z, p3.x, p3.y, p3.z);
...
void CFaceDetection::experimental_viewRegions(const vector<TPoint3D> regions[9],
  ...
  scene->insert(std::make_shared<mrpt::opengl::CGridPlaneXY>());
  scene->insert(
      std::make_shared<mrpt::opengl::CAxis>(-5, -5, -5, 5, 5, 5, 2.5, 3, true));
...
void CFaceDetection::experimental_showMeasurements() {
  ...
  if (m_measure.lessEigenVals.size() > 0) {
    ...
    double minEigenVal = *min_element(m_measure.lessEigenVals.begin(),
                                      m_measure.lessEigenVals.end());
    double maxEigenVal = *max_element(m_measure.lessEigenVals.begin(),
                                      m_measure.lessEigenVals.end());
    ...
    if (m_measure.saveMeasurementsToFile) {
      f << endl
        << "Statistical data about eigen values calculated of regions detected "
           "as faces"
        << endl;
  ...
  if (m_measure.sumDistances.size() > 0) {
    ...
    double minSumDist = *min_element(m_measure.sumDistances.begin(),
                                     m_measure.sumDistances.end());
    double maxSumDist = *max_element(m_measure.sumDistances.begin(),
                                     m_measure.sumDistances.end());
  ...
  if (m_measure.errorEstimations.size() > 0) {
    ...
    double minEstimationErr = *min_element(m_measure.errorEstimations.begin(),
                                           m_measure.errorEstimations.end());
    double maxEstimationErr = *max_element(m_measure.errorEstimations.begin(),
                                           m_measure.errorEstimations.end());
