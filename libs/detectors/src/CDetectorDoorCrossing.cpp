...
CDetectorDoorCrossing::CDetectorDoorCrossing()
    : COutputLogger("CDetectorDoorCrossing"), options(), lastObs(), entropy(),
      lastEntropy(), lastEntropyValid(false) {
  clear();
}
...
/*---------------------------------------------------------------
                                                process
  ---------------------------------------------------------------*/
void CDetectorDoorCrossing::process(CActionRobotMovement2D &in_poseChange,
                                    CSensoryFrame &in_sf,
  ...
  for (i = 0; i < options.windowSize; i++) {
    ...
    ASSERT_(
        act->GetRuntimeClass()->derivedFrom(CLASS_ID(CActionRobotMovement2D)))
