...
public:
  ...
  void debug_returnResults(const vector_uint &falsePositives,
                           const vector_uint &ignore,
                           unsigned int &falsePositivesDeleted,
                           unsigned int &realFacesDeleted);

private:
  std::thread m_thread_checkIfFaceRegions;     //!< Thread that execute
                                               //!checkIfFaceRegions filter
  std::thread m_thread_checkIfFacePlaneCov;    //!< Thread that execute
                                               //!checkIfFacePlaneCov filter
  std::thread m_thread_checkIfDiagonalSurface; //!< Thread that execute
                                               //!checkIfDiagonalSurface filter
  ...
  bool m_checkIfDiagonalSurface_res; //!< Save result of checkIfDiagonalSurface
                                     //!filter

  bool m_end_threads; //!< Indicates to all threads that must finish their
                      //!execution

  std::promise<void> m_enter_checkIfFaceRegions;  //!< Indicates to
                                                  //!thread_checkIfFaceRegions
                                                  //!that exist a new face to
                                                  //!analyze
  std::promise<void> m_enter_checkIfFacePlaneCov; //!< Indicates to
                                                  //!thread_checkIfFacePlaneCov
                                                  //!that exist a new face to
                                                  //!analyze
  std::promise<void>
      m_enter_checkIfDiagonalSurface; //!< Indicates to
                                      //!thread_checkIfDiagonalSurface that
                                      //!exist a new face to analyze

  std::promise<void> m_leave_checkIfFaceRegions;  //!< Indicates to main thread
                                                  //!that
                                                  //!thread_checkIfFaceRegions
                                                  //!has been completed analisis
                                                  //!of the last face detected
  std::promise<void> m_leave_checkIfFacePlaneCov; //!< Indicates to main thread
                                                  //!that
                                                  //!thread_checkIfFacePlaneCov
                                                  //!has been completed analisis
                                                  //!of the last face detected
  std::promise<void>
      m_leave_checkIfDiagonalSurface; //!< Indicates to main thread that
                                      //!thread_checkIfDiagonalSurface has been
                                      //!completed analisis of the last face
                                      //!detected
  ...
  size_t checkRelativePosition(const mrpt::math::TPoint3D &p1,
  ...
  void experimental_viewFacePointsScanned(const std::vector<float> &xs,
  ...
  void
  experimental_viewRegions(const std::vector<mrpt::math::TPoint3D> regions[9],
                           const mrpt::math::TPoint3D meanPos[3][3]);

  // Segmentation methods
  void experimental_segmentFace(const mrpt::obs::CObservation3DRangeScan &face,
                                mrpt::math::CMatrixTemplate<bool> &region);

  // Histogram methods
  void experimental_calcHist(const mrpt::utils::CImage &face, const size_t &c1,
                             const size_t &r1, const size_t &c2,
                             const size_t &r2,
