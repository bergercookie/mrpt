 ...
 */
class DETECTORS_IMPEXP CDetectorDoorCrossing
...
public:
  ...
  struct DETECTORS_IMPEXP TDoorCrossingOutParams {
    TDoorCrossingOutParams()
        : enoughtInformation(false), doorCrossingLikelihood(0),
          informationGain(0), cumulativeTurning(0), pointsMap(){};
    ...
    /** The cumulative turning of the robot in radians for the movements in the
     * "window"
      */
   ...
   * the robot pose since last observation to this one.
    ...
    */
  void process(mrpt::obs::CActionRobotMovement2D &in_poseChange,
