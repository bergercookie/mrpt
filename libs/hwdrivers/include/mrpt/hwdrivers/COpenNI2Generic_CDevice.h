...
private:
  ...
  public:
    CStream(openni::Device &device, openni::SensorType type,
    ...
    virtual bool getFrame(openni::VideoFrameRef &frame, uint64_t &timestamp,
    ...
    static Ptr create(openni::Device &device, openni::SensorType type,
                      openni::PixelFormat format, std::ostream &log,
                      bool verbose);
  };
  ...
  inline void setPixel(const openni::RGB888Pixel &src, mrpt::utils::CImage &rgb,
                       int x, int y) {
    rgb.setPixel(x, y, (src.r << 16) + (src.g << 8) + src.b);
  }
  inline void setPixel(const openni::DepthPixel &src,
                       mrpt::math::CMatrix &depth, int x, int y) {
...
public:
  CDevice(const openni::DeviceInfo &info, openni::PixelFormat rgb,
  ...
  bool getNextFrameRGB(mrpt::utils::CImage &img, uint64_t &timestamp,
                       bool &there_is_obs, bool &hardware_error);
  bool getNextFrameD(mrpt::math::CMatrix &img, uint64_t &timestamp,
                     bool &there_is_obs, bool &hardware_error);
  bool getNextFrameRGBD(mrpt::obs::CObservation3DRangeScan &obs,
                        bool &there_is_obs, bool &hardware_error);
  ...
  static Ptr create(const openni::DeviceInfo &info, openni::PixelFormat rgb,
