...
/** A class for interfacing Intersense Inertial Measuring Units (IMUs).
  ...
  *
  *		In order to record all this information within the
  *'rawMeasurements' vector in mrpt::obs::CObservationIMU, some of it had to be
  *stored in positions which weren't intended for the stored data (marked with
  **):
  *		- Euler angles --> rawMeasurements[IMU_YAW],
  *rawMeasurements[IMU_PITCH], rawMeasurements[IMU_ROLL]
  ...
  *rawMeasurements[IMU_Y_VEL], rawMeasurements[IMU_Z_VEL]
  ...
  *		Be careful with this when using the grabbed
  *mrpt::obs::CObservationIMU data.
  *
  ...
  *    pose_x		= 0					; [double] Sensor 3D position
  *relative to the robot (meters)
  *    pose_y		= 0
  *    pose_z		= 0
  *    pose_yaw		= 0					; [double] Angles in
  *degrees
  *    pose_pitch	= 0
  *    pose_roll	= 0
  *
  *	   sensitivity	= 10				; [int] Sensor sensitivity
  *(see API documentation)
  *	   enhancement	= 2					; [int] Enhancement mode
  *(see API documentation)
  *	   prediction	= 0					; [int] Prediction mode
  *(see API documentation)
  *	   useBuffer	= 0					; [bool] {0,1} (unused by
  *now) Whether or not use a buffer for storing old data (see API documentation)
  *
  *  \endcode
