									  ...
									  public CGenericSensor
{
   ...
   public:
	...
	/** Requests a command of "change address" for a given SRF10 device.
	  *  currentAddress and newAddress are the I2C addresses in the range 0 to
	 * 15
	 * (mapped to 0xE0 to 0xFE internally).
	  * \return true on success, false on communications errors or device not
	 * found.
	  */
   ...
   protected:
	...
	/** The maximum range in meters, used for the internal device timer (value
	 * between 4cm and 11m).
	  */
	float m_maxRange;
	...
	/** The individual gains of the sonars, indexed by their I2C addresses
	 * [0,15].
	  *  Up to 16 devices, but you can put any number of devices (from 1 to 16).
	  */
	...
	/** Sends the configuration (max range, gain,...) to the USB board. Used
	 * internally after a successfull connection.
	  * \return true on success, false on communications errors or device not
	 * found.
	  */
