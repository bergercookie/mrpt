...
protected:
  ...
  std::string m_COM_port;  //!< If not an empty string (default), will open that
                           //!serial port, otherwise will try to open USB FTDI
                           //!device "m_usbSerialNumber"
  unsigned int m_COM_baud; //!< Default=115200

  // Only one of these two streams will be !=nullptr and open for each specific
  // eNose board!
  std::unique_ptr<CInterfaceFTDI>
      m_stream_FTDI; //!< FTDI comms pipe (when not in serial port mode)
...
public:
  ...
  /** If not an empty string, will open that serial port, otherwise will try to
   * open USB FTDI device "m_usbSerialNumber"
