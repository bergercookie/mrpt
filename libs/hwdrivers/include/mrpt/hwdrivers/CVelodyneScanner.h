 ...
 * https://github.com/ros-drivers/velodyne
  ...
  */
class HWDRIVERS_IMPEXP CVelodyneScanner
    : public mrpt::hwdrivers::CGenericSensor {
  DEFINE_GENERIC_SENSOR(CVelodyneScanner)
public:
  static short int
      VELODYNE_DATA_UDP_PORT; //!< Default: 2368. Change it if required.
  static short int
      VELODYNE_POSITION_UDP_PORT; //!< Default: 8308. Change it if required.
  ...
  struct HWDRIVERS_IMPEXP TModelPropertiesFactory {
    ...
    static std::string
    getListKnownModels(); //!< Return human-readable string: "`VLP16`,`XXX`,..."
  };
...
protected:
  ...
  std::string
      m_pcap_input_file; //!< Default: "" (do not operate from an offline file)
  std::string
      m_pcap_output_file; //!< Default: "" (do not dump to an offline file)
  mrpt::poses::CPose3D m_sensorPose;
  mrpt::obs::VelodyneCalibration m_velodyne_calib; //!< Device calibration file
                                                   //!(supplied by vendor in an
                                                   //!XML file)
  ...
  unsigned int m_pcap_read_count; //!< number of pkts read from the file so far
                                  //!(for debugging)
  bool m_pcap_read_once;          //!< Default: false
  bool m_pcap_read_fast;          //!< (Default: false) If false, will use
                                  //!m_pcap_read_full_scan_delay_ms
  double m_pcap_read_full_scan_delay_ms; //!< (Default:100 ms) delay after each
                                         //!full scan read from a PCAP log
...
public:
   ...
   * "not based on satellite time". 30 secs, with typical velodyne clock drifts,
   * means a ~1.7 ms typical drift. */
  ...
  bool loadCalibrationFile(
      const std::string &
          velodyne_xml_calib_file_path); //!< Returns false on error. \sa
  ...
  /** Switches the LASER on/off (saves energy when not measuring) (via HTTP post
  * interface).
  * Can be called at any instant, before or after initialize().
  * Requires setting a device IP address.
  * \return false on error*/
   ...
   * packets/second (!=scans/second). Checkout Velodyne user manual if in doubt.
    *
    * \param[out] outScan Upon return, an empty smart pointer will be found here
   * if no new data was available. Otherwise, a valid scan.
    * \param[out] outGPS  Upon return, an empty smart pointer will be found here
   * if no new GPS data was available. Otherwise, a valid GPS reading.
    * \return true if no error ocurred (even if there was no new observation).
   * false if any communication error occurred.
    */
  bool getNextObservation(mrpt::obs::CObservationVelodyneScan::Ptr &outScan,
   ...
   * with a call to loadConfig.
    * Velodyne specifics: this method sets up the UDP listening sockets, so all
   * relevant params MUST BE SET BEFORE calling this.
   ...
   * each kind of packets, or INVALID_TIMESTAMP if timeout ocurred waiting for a
   * packet.
...
private:
  ...
  static mrpt::system::TTimeStamp
  internal_receive_UDP_packet(platform_socket_t hSocket, uint8_t *out_buffer,
                              const size_t expected_packet_size,
                              const std::string &filter_only_from_IP);

  bool internal_read_PCAP_packet(mrpt::system::TTimeStamp &data_pkt_time,
...
{
template <> struct TEnumTypeFiller<hwdrivers::CVelodyneScanner::model_t> {
...
template <> struct TEnumTypeFiller<hwdrivers::CVelodyneScanner::return_type_t> {
