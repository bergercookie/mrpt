 ...
 * messages, not only serial ports.
  ...
  */
class HWDRIVERS_IMPEXP CGPSInterface : public mrpt::utils::COutputLogger,
...
public:
  ...
  bool isGPS_connected(); //!< Returns true if communications work, i.e. if some
                          //!message has been received.
  bool isGPS_signalAcquired(); //!< Returns true if the last message from the
                               //!GPS indicates that the signal from sats has
                               //!been acquired.

  /** \name Set-up and configuration
    * @{ */
  void setSerialPortName(const std::string &COM_port); //!< Set the serial port
                                                       //!to use (COM1, ttyUSB0,
                                                       //!etc).
  std::string getSerialPortName()
      const; //!< Get the serial port to use (COM1, ttyUSB0, etc).

  void setParser(PARSERS parser); //!< Select the parser for incomming data,
                                  //!among the options enumerated in \a
                                  //!CGPSInterface
   ...
   * that object allocated during the entire life of this object.
    ...
    */
  void bindStream(mrpt::utils::CStream *external_stream,
  ...
  /** If set to non-empty, RAW GPS serial data will be also dumped to a separate
   * file. */
   ...
   * documentation of CGPSInterface
    * \return true if some new data field has been correctly parsed and inserted
   * into out_obs
    */
  static bool parse_NMEA(const std::string &cmd_line,
                         mrpt::obs::CObservationGPS &out_obs,
                         const bool verbose = false);

  /** Gets the latest GGA command or an empty string if no newer GGA command was
   * received since the last call to this method.
...
protected:
  ...
  bool OnConnectionShutdown(); //!< Like OnConnectionEstablished() for sending
                               //!optional shutdown commands

  bool legacy_topcon_setup_commands();

  mrpt::utils::CStream *m_data_stream; //!< Typically a CSerialPort created by
                                       //!this class, but may be set externally.
  ...
  /** Set Advanced Input Mode for the primary port.
     ...
     port that it's used for the commands.
          The RTK correction stream must be re-packaged into a special frame
     with prefix ">>" */
...
private:
  mrpt::utils::circular_buffer<uint8_t>
      m_rx_buffer; //!< Auxiliary buffer for readings
  PARSERS m_parser;
  ...
  /** \name Legacy support for TopCon RTK configuration
    * @{ */
  std::string m_JAVAD_rtk_src_port;  //!< If not empty, will send a cmd
                                     //!"set,/par/pos/pd/port,...". Example
                                     //!value: "/dev/ser/b"
  unsigned int m_JAVAD_rtk_src_baud; //!< Only used when "m_JAVAD_rtk_src_port"
                                     //!is not empty
  std::string m_JAVAD_rtk_format; //!< Only used when "m_JAVAD_rtk_src_port" is
                                  //!not empty: format of RTK corrections:
                                  //!"cmr", "rtcm", "rtcm3", etc.

  bool m_topcon_useAIMMode; //!< Use this mode for receive RTK corrections from
                            //!a external source through the primary port
  bool m_topcon_AIMConfigured; //!< Indicates if the AIM has been properly set
                               //!up.
  double m_topcon_data_period; //!< The period in seconds which the data should
                               //!be provided by the GPS
  void JAVAD_sendMessage(
      const char *str,
      bool waitForAnswer =
          true); //!< Private auxiliary method. Raises exception on error.
                 /** @} */

  /** Returns true if the COM port is already open, or try to open it in other
   * case.
    * \return true if everything goes OK, or false if there are problems opening
   * the port. */
  bool tryToOpenTheCOM();

  void parseBuffer(); //!< Process data in "m_buffer" to extract GPS messages,
                      //!and remove them from the buffer.

  void flushParsedMessagesNow(); //!< Queue out now the messages in \a
                                 //!m_just_parsed_messages, leaving it empty
  mrpt::obs::CObservationGPS
      m_just_parsed_messages; //!< A private copy of the last received gps datum
...
namespace utils {
template <> struct TEnumTypeFiller<hwdrivers::CGPSInterface::PARSERS> {
