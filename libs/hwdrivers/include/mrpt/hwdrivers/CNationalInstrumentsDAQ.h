...
public:
  ...
  /** Setup and launch the DAQ tasks, in parallel threads.
    * Access to grabbed data with CNationalInstrumentsDAQ::readFromDAQ() or the
   * standard CGenericSensor::doProcess() */
   ...
   * CGenericSensor::doProcess() interface.
    *  No observations may be returned if there are not samples enough yet from
   * any task.
    */
  void
  readFromDAQ(std::vector<mrpt::obs::CObservationRawDAQ::Ptr> &outObservations,
              bool &hardwareError);
   ...
   * channels in the task when it was initiated.
    */
  void writeAnalogOutputTask(size_t task_index, size_t nSamplesPerChannel,
                             const double *volt_values, double timeout,
   ...
   * channels in the task when it was initiated.
    */
  void writeDigitalOutputTask(size_t task_index, bool line_value,
                              double timeout);
  ...
  struct HWDRIVERS_IMPEXP TaskDescription {
    ...
    double samplesPerSecond;     //!< Sample clock config: samples per second.
                                 //!Continuous (infinite) sampling is assumed.
    std::string sampleClkSource; //!< Sample clock source: may be empty (default
                                 //!value) for some channels.
    uint32_t bufferSamplesPerChannel; //!< (Default=0) From NI's docs: The
                                      //!number of samples the buffer can hold
                                      //!for each channel in the task. Zero
                                      //!indicates no buffer should be
                                      //!allocated. Use a buffer size of 0 to
                                      //!perform a hardware-timed operation
                                      //!without using a buffer.
    uint32_t samplesPerChannelToRead; //!< (Default=1000) The number of samples
                                      //!to grab at once from each channel.
    std::string taskLabel;            //!< (Default="task###")

    struct HWDRIVERS_IMPEXP desc_ai_t {
      desc_ai_t()
          : terminalConfig("DAQmx_Val_NRSE"), minVal(-10), maxVal(10),
            physicalChannelCount(0) {}

      std::string physicalChannel, terminalConfig;
      double minVal, maxVal;
      unsigned int physicalChannelCount; //!< *IMPORTANT* This must be the total
                                         //!number of channels listed in
                                         //!"physicalChannel" (e.g. 4 for
                                         //!"Dev1/ai0:3")
    } ai;                                //!< Analog inputs
    ...
    struct HWDRIVERS_IMPEXP desc_ci_lin_encoder_t {
      desc_ci_lin_encoder_t()
          : ZidxEnable(false), ZidxVal(0), distPerPulse(0.1), initialPos(0) {}
      ...
      double distPerPulse;
      double initialPos;
    } ci_lin_encoder; //!< Counter: uses a linear encoder to measure linear
                      //!position

    struct HWDRIVERS_IMPEXP desc_ci_ang_encoder_t {
      desc_ci_ang_encoder_t()
          : ZidxEnable(false), ZidxVal(0), pulsesPerRev(512), initialAngle(0),
            decimate(1), decimate_cnt(0) {}
    ...
    } ci_ang_encoder; //!< Counter: uses an angular encoder to measure angular
                      //!position

    struct HWDRIVERS_IMPEXP desc_co_pulses_t {
      desc_co_pulses_t()
          : idleState("DAQmx_Val_Low"), initialDelay(0), freq(1000),
            dutyCycle(0.5) {}
  ...
  /** Publicly accessible vector with the list of tasks to be launched upon call
   * to CNationalInstrumentsDAQ::initialize().
...
private:
  std::vector<mrpt::obs::CObservationRawDAQ::Ptr>
      m_nextObservations; //!< A buffer for doProcess
  ...
  struct TInfoPerTask {
    ...
    TaskDescription task; //!< A copy of the original task description that
