...
public:
  ...
  /** Establish Connection with Rovio and log in its system: Important, fill out
   * "options" members *BEFORE* calling this method.
    *  \exception std::runtime On errors
    */
  void initialize(); // string &errormsg_out, std::string
                     // url_out="150.214.109.134", std::string user_out="admin",
                     // std::string password_out="investigacion");
  ...
  /** Returns the next frame from Rovio's live video stream, after starting the
   * live streaming with retrieve_video()
    * \return False on error
   ...
   * live video streaming.
    ...
    * \sa captureImageSync
    */
  bool captureImageAsync(mrpt::utils::CImage &out_img,
                         bool recttified); // string pict_name,

  bool isVideoStreamming()
      const; //!< Return true if video is streaming correctly \sa retrieve_video
