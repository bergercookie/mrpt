...
public:
  ...
  /** Retrieves the next frame from the video source, raising an exception on
    ...
    *		- mrpt::obs::CObservationImage (For normal cameras or video
    *sources)
  ...
  /** Functor type */
  typedef void (*TPreSaveUserHook)(const mrpt::obs::CObservation::Ptr &obs,
                                   void *user_ptr);
...
protected:
  ...
  bool m_sr_save_confidence;    //!< Save the estimated confidence 2D image
                                //!(default: false)
  ...
  bool m_kinect_save_intensity_img; //!< Save the 2D intensity image (default:
                                    //!true)
...
private:
  ...
  std::unique_ptr<CImageGrabber_OpenCV>
      m_cap_cv; //!< The OpenCV capture object.
  std::unique_ptr<CImageGrabber_dc1394>
      m_cap_dc1394; //!< The dc1394 capture object.
  std::unique_ptr<CImageGrabber_FlyCapture2>
      m_cap_flycap; //!< The FlyCapture2 object
  std::unique_ptr<CImageGrabber_FlyCapture2> m_cap_flycap_stereo_l,
      m_cap_flycap_stereo_r; //!< The FlyCapture2 object for stereo pairs
  ...
  std::unique_ptr<CFFMPEG_InputStream>
      m_cap_ffmpeg; //!< The FFMPEG capture object
  std::unique_ptr<mrpt::utils::CFileGZInputStream>
      m_cap_rawlog; //!< The input file for rawlogs
  std::unique_ptr<CSwissRanger3DCamera>
      m_cap_swissranger;                         //!< SR 3D camera object.
  ...
  mrpt::gui::CDisplayWindow::Ptr m_preview_win1,
      m_preview_win2; //!< Normally we'll use only one window, but for stereo
                      //!images we'll use two of them.

  /** @name Stuff related to working threads to save images to disk
      @{ */
  unsigned int m_external_image_saver_count; //!< Number of working threads.
                                             //!Default:1, set to 2 in quad
                                             //!cores.
  ...
  std::vector<TListObservations> m_toSaveList; //!< The queues of objects to be
                                               //!returned by getObservations,
                                               //!one for each working thread.
  void thread_save_images(unsigned int my_working_thread_index); //!< Thread to
                                                                 //!save images
                                                                 //!to files.
