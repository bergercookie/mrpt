...
public:
  ...
  * \exception std::exception On communication errors or a different serial port
  * already open.
  */
  ...
  * \exception std::exception On communication errors
  */
  void setConfig(int baudRate, int parity = 0, int bits = 8, int nStopBits = 1,
                 bool enableFlowControl = false);

  /** Changes the timeouts of the port, in milliseconds.
  * \exception std::exception On communication errors
  */
  void setTimeouts(int ReadIntervalTimeout, int ReadTotalTimeoutMultiplier,
                   int ReadTotalTimeoutConstant,
                   int WriteTotalTimeoutMultiplier,
                   int WriteTotalTimeoutConstant);

  /** Implements the virtual method responsible for reading from the stream -
   * Unlike CStream::ReadBuffer, this method will not raise an exception on zero
   * bytes read, as long as there is not any fatal error in the communications.
    * \exception std::exception On communication errors
   */
   ...
   * eol are found.
    * \param eol_chars A line reception is finished when one of these characters
   * is found. Default: LF (10), CR (13).
   ...
   * ocurred, false on a valid read.
    ...
    * \exception std::exception On communication errors
    */
  std::string ReadString(const int total_timeout_ms = -1,
