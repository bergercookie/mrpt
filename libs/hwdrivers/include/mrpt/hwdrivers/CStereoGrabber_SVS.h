...
struct HWDRIVERS_IMPEXP TCaptureOptions_SVS {
  TCaptureOptions_SVS(int _frame_width = 640, int _frame_height = 480,
                      double _framerate = 30, int _NDisp = 64,
                      int _Corrsize = 15, int _LR = false, int _Thresh = 10,
                      int _Unique = 13, int _Horopter = 0,
                      int _SpeckleSize = 100, bool _procesOnChip = true,
                      bool _calDisparity = true);

  int frame_width, frame_height; //!< Capture resolution (Default: 640x480)

  bool getRectified; //!< Indicates if the STOC camera must capture rectified
                     //!images (Default: true -> rectified)
...
public:
  ...
  /** Constructor: */
  CStereoGrabber_SVS(int cameraIndex = 0, const TCaptureOptions_SVS &options =
                                              TCaptureOptions_SVS());
   ...
   *
   *  NOTICE: (1) That the member "CObservationStereoImages::refCameraPose" must
   * be
...
static_assert(!std::is_copy_constructible<CStereoGrabber_SVS>::value &&
