...
struct HWDRIVERS_IMPEXP TCaptureOptions_DUO3D {
  ...
  bool m_capture_rectified; //!< (Default = true) Rectify images. Rectification
                            //!map must be provided \sa m_rectify_map_filename.
  bool m_calibration_from_file; //!< (Default = true) Get calibration
                                //!information from files provided by DUO3D
                                //!Calibration App.
                                /** @} */

  /** @name Files specification
    * @{ */
  std::string m_rectify_map_filename; //!< Rectification map file provided by
                                      //!DUO3D Calibration App (YML format).
  std::string m_intrinsic_filename;   //!< Intrinsic parameters file provided by
                                      //!DUO3D Calibration App (YML format).
  std::string m_extrinsic_filename;   //!< Extrinsic parameters file provided by
                                      //!DUO3D Calibration App (YML format).
                                      /** @} */
  ...
  /** Loads all the options from a config file.
    ...
    * \code
    * [sectionName]
    *		image_width   			= 640			// [int]	x
    *Resolution
    *		image_height  			= 480			// [int]	y
    *Resolution
    *		fps						= 30			// [int]	Frames per second (<=
    *30)
    *		exposure				= 50			// [int]	Exposure value
    *(1..100)
    *		led						= 0				// [int]	Led intensity (only
    *for some device models) (1..100).
    *		gain					= 50			// [int]	Camera gain
    *(1..100)
    *		capture_rectified 		= false			// [bool]	Rectify captured
    *images
    *		capture_imu 			= true			// [bool]	Capture IMU data from
    *DUO3D device (if available)
    ...
    *parameters file. This filename should contain a substring _RWWWxHHH_ with
    *WWW being the image width and HHH the image height, as provided by the
    ...
    *file. This filename should contain a substring _RWWWxHHH_ with WWW being
    *the image width and HHH the image height, as provided by the calibration
    *application.
    *
    * \endcode
  ...
  TYMLReadResult
  m_camera_int_params_from_yml(const std::string &_file_name = std::string());
  TYMLReadResult
  m_camera_ext_params_from_yml(const std::string &_file_name = std::string());
  TYMLReadResult
  m_rectify_map_from_yml(const std::string &_file_name = std::string());
...
/** This "software driver" implements the communication protocol for interfacing
  ...
  *		exposure				= 50			// [int]
  *Exposure value (1..100)
  *		led						= 0				// [int]
  *Led intensity (only for some device models) (1..100).
  *		gain					= 50			// [int]	Camera
  *gain (1..100)
  *		capture_rectified 		= false			// [bool]
  *Rectify captured images
  *		capture_imu 			= true			// [bool]
  *Capture IMU data from DUO3D device (if available)
  *		calibration_from_file	= true			// [bool]	Use
  *YML calibration files provided by calibration application supplied with DUO3D
  *device
  *	  	intrinsic_filename		= ""			// [string]
  *Intrinsic parameters file. This filename should contain a substring
  *_RWWWxHHH_ with WWW being the image width and HHH the image height, as
  *provided by the calibration application.
  *		extrinsic_filename		= ""			// [string]
  *Extrinsic parameters file. This filename should contain a substring
  *_RWWWxHHH_ with WWW being the image width and HHH the image height, as
  *provided by the calibration application.
  *		rectify_map_filename	= ""			// [string]
  *Rectification map file. This filename should contain a substring _RWWWxHHH_
  *with WWW being the image width and HHH the image height, as provided by the
  *calibration application.
  *
...
public:
   ...
   * grabbing data.
    ...
    */
  void open(const TCaptureOptions_DUO3D &options,
            const bool startCapture = true);

  /** Start the actual data capture of the camera. Must be called after open(),
  ...
  /** Specific laser scanner "software drivers" must process here new data from
   * the I/O stream, and, if a whole scan has arrived, return it.
    *  This method will be typically called in a different thread than other
   * methods, and will be called in a timely fashion.
    */
  void getObservations(mrpt::obs::CObservationStereoImages &outObservation_img,
                       mrpt::obs::CObservationIMU &outObservation_imu,
                       bool &there_is_img, bool &there_is_imu);
