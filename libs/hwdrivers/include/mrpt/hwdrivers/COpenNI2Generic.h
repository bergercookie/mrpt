...
public:
  ...
  COpenNI2Generic(int width, int height, float fps = 30.0f,
                  bool open_streams_now = true); //!< Ctor. \sa start()
  ...
  void getNextFrameRGB(mrpt::utils::CImage &rgb_img, uint64_t &timestamp,
   ...
   * there_is_obs=true).
    ...
    *  \param hardware_error True on hardware/comms error.
    *  \param sensor_id The index of the sensor accessed. */
  void getNextFrameD(mrpt::math::CMatrix &depth_img, uint64_t &timestamp,
                     bool &there_is_obs, bool &hardware_error,
                     unsigned sensor_id = 0);
   ...
   * there_is_obs=true).
    ...
    * \sa doProcess
    */
  void getNextFrameRGBD(mrpt::obs::CObservation3DRangeScan &out_obs,
                        bool &there_is_obs, bool &hardware_error,
                        unsigned sensor_id = 0);
   ...
   * set before calling this) - users may also call initialize(), which in turn
   * calls this method.
  ...
  *  are not among the connected devices. This function also fills a vector with
  * the serial numbers of the connected
  *  OpenNI2 sensors (this requires openning the sensors which are still closed
  * to read their serial)
  */
  unsigned int
  ...
  */
  ...
  bool getDeviceIDFromSerialNum(const unsigned int SerialRequired,
                                int &sensor_id)
      const;    //!< Get the ID of the device corresponding to 'SerialRequired'.
  bool start(); //!< Open all sensor streams (normally called automatically at
                //!constructor, no need to call it manually). \return false on
                //!error \sa kill() to close
  void kill();  //!< Kill the OpenNI2 driver \sa start()
  bool isOpen(const unsigned sensor_id)
      const; //!< Whether there is a working connection to the sensor
  void close(unsigned sensor_id = 0); //!< Close the connection to the sensor
                                      //!(no need to call it manually unless
                                      //!desired for some reason, since it's
                                      //!called at destructor
  int getNumDevices()
      const; //!< The number of available devices at initialization
  ...
  bool getColorSensorParam(mrpt::utils::TCamera &param,
                           unsigned sensor_id = 0) const;
  bool getDepthSensorParam(mrpt::utils::TCamera &param,
                           unsigned sensor_id = 0) const;
