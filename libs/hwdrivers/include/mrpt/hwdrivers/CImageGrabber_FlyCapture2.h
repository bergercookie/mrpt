...
struct HWDRIVERS_IMPEXP TCaptureOptions_FlyCapture2
{
	 ...
	 * given
	 * input trigger signal is detected. Refer to PGR docs. */
	 ...
	 * [sectionName]
	 * # Camera selection:
	 * // (Default=0) If open_by_guid==false, will open the i'th camera based
	 *on this 0-based index.
	 * camera_index = 0
	 * // (Default=false) Set to true to force opening a camera by its GUID, in
	 *\a camera_guid
	 * open_by_guid = false
	 * // GUID of the camera to open, only when open_by_guid==true. Hexadecimal
	 * // blocks separated by dashes ("-")
	 * camera_guid = 11223344-55667788-99AABBCC-DDEEFF00
	 *
	 * # Camera settings:
	 * // (Default="", which means default) A string with a video mode, from
	 ...
	 * /// (default=0.0) Sensor gain, if gain_auto=false
	 * #gain_dB;
	 * // (default=0) Obtain images as stereo pairs with Flycapture2
	 * flycap_stereo_mode = 1
	 * // (default=0) Rectify stereo images (needs Triclops installed)
	 * flycap_get_rectified = 1
	 * // (default=640) Width for output rectified images
	 * flycap_rect_width = 320
	 * // (default=480) Height for output rectified images
	 * flycap_rect_height = 240
	 * \endcode
	 *
	 * \note All parameter names may have an optional prefix, set with the
	 * "prefix" parameter.*For example, if prefix = "LEFT_", the expected
	 * variable name "camera_index" in the config section will be
	 * "LEFT_camera_index", and so on.
	 */
