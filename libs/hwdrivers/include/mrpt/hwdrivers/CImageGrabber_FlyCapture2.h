...
struct HWDRIVERS_IMPEXP TCaptureOptions_FlyCapture2 {
  ...
  /** @name Camera to open
    * @{ */
  unsigned int camera_index; //!< (Default=0) If open_by_guid==false, will open
                             //!the i'th camera based on this 0-based index.
  bool open_by_guid; //!< (Default=false) Set to true to force opening a camera
                     //!by its GUID, in \a camera_guid
  unsigned int camera_guid[4]; //!< GUID of the camera to open, only when
                               //!open_by_guid==true.
                               /** @} */
  ...
  /** @name Camera settings
    * @{ */
  std::string
      videomode; //!< (Default="", which means default) A string with a video
                 //!mode, from the list available in
                 //![FlyCapture2::VideoMode](http://www.ptgrey.com/support/downloads/documents/flycapture/Doxygen/html/),
                 //!eg. "VIDEOMODE_640x480Y8", etc.
  std::string
      framerate; //!< (Default="", which means default) A string with a
                 //!framerate, from the list available in
                 //![FlyCapture2::FrameRate](http://www.ptgrey.com/support/downloads/documents/flycapture/Doxygen/html/),
                 //!eg. "FRAMERATE_30", etc.
  std::string
      grabmode; //!< (Default="BUFFER_FRAMES") A string with a grab mode, from
                //!the list available in
                //![FlyCapture2::GrabMode](http://www.ptgrey.com/support/downloads/documents/flycapture/Doxygen/html/)
  unsigned int numBuffers; //!< (Default=30) Number of images that can be stored
                           //!in the buffer, if enabled with grabMode.
  int grabTimeout;         //!< (Default=5000) Time in milliseconds that
                   //!RetrieveBuffer() and WaitForBufferEvent() will wait for an
                   //!image before timing out and returning.

  bool trigger_enabled; //!< (default=false) Enable non-free-running mode, only
                        //!capturing when a given input trigger signal is
                        //!detected. Refer to PGR docs.
  ...
  bool strobe_enabled; //!< (default=false) Enable the generation of a strobe
                       //!signal in GPIO. Refer to PGR docs.
  ...
  float strobe_duration; //!< (default=1.0) Pulse durationin ms. Refer to PGR
                         //!docs.

  bool autoexposure_auto;  //!< (default=true)
  bool autoexposure_onOff; //!< (default=true) Activate this feature
  bool autoexposure_abs;   //!< (default=true) Numeric mode (absolute or integer
                           //!values)
  float autoexposure_EV;   //!< (default=0.0) Exposure Value, if
                           //!autoexposure_auto=false
  ...
  bool stereo_mode;   //!< (default=false) Obtain images as stereo pairs with
                      //!Flycapture2
  bool get_rectified; //!< (default=false) Rectify stereo images (needs Triclops
                      //!installed)
  unsigned int rect_width; //!< (default=640) Width for output rectified images
  unsigned int
      rect_height; //!< (default=480) Height for output rectified images
                   /** @} */
  ...
  /** Loads all the options from a config file.
    ...
    * [sectionName]
    * # Camera selection:
    * camera_index = 0      // (Default=0) If open_by_guid==false, will open the
   * i'th camera based on this 0-based index.
    * open_by_guid = false  // (Default=false) Set to true to force opening a
   * camera by its GUID, in \a camera_guid
    * camera_guid  = 11223344-55667788-99AABBCC-DDEEFF00  // GUID of the camera
   * to open, only when open_by_guid==true. Hexadecimal blocks separated by
   * dashes ("-")
    *
    * # Camera settings:
    * videomode   = VIDEOMODE_640x480Y8 // (Default="", which means default) A
   * string with a video mode, from the list available in
   * [FlyCapture2::VideoMode](http://www.ptgrey.com/support/downloads/documents/flycapture/Doxygen/html/),
   * eg. "VIDEOMODE_640x480Y8", etc.
    * framerate   = FRAMERATE_30        // (Default="", which means default) A
   * string with a framerate, from the list available in
   * [FlyCapture2::FrameRate](http://www.ptgrey.com/support/downloads/documents/flycapture/Doxygen/html/),
   * eg. "FRAMERATE_30", etc.
    * grabmode    = BUFFER_FRAMES       // (Default="BUFFER_FRAMES") A string
   * with a grab mode, from the list available in
   * [FlyCapture2::GrabMode](http://www.ptgrey.com/support/downloads/documents/flycapture/Doxygen/html/)
    * numBuffers  = 30                  // (Default=30) Number of images that
   * can be stored in the buffer, if enabled with grabMode.
    * grabTimeout = 5000                // (Default=5000) Time in milliseconds
   * that RetrieveBuffer() and WaitForBufferEvent() will wait for an image
   * before timing out and returning.
    *
    * trigger_enabled = false // (default=false) Enable non-free-running mode,
   * only capturing when a given input trigger signal is detected. Refer to PGR
   * docs.
    ...
    *
    * strobe_enabled   = false // (default=false) Enable the generation of a
   * strobe signal in GPIO. Refer to PGR docs.
    ...
    * #strobe_duration  = 1.0   // (default=1.0) Pulse durationin ms. Refer to
   * PGR docs.
    *
    * #autoexposure_auto; //!< (default=true)
    * #autoexposure_abs;  //!< (default=true) Numeric mode (absolute or integer
   * values)
    * #autoexposure_onoff;//!< (default=true) Activate this feature
    * #autoexposure_EV;   //!< (default=0.0) Exposure Value, if
   * autoexposure_auto=false
    * #shutter_auto     = false   // (default=true)
    * #shutter_abs;       //!< (default=true) Numeric mode (absolute or integer
   * values)
    * #shutter_time_ms  = 4.0     // (default=4.0) Shutter time, if
   * shutter_auto=false
    * #gain_auto;         //!< (default=true)
    * #gain_abs;          //!< (default=true) Numeric mode (absolute or integer
   * values)
    * #gain_dB;           //!< (default=0.0) Sensor gain, if gain_auto=false
    *
    * flycap_stereo_mode	= 1	// (default=0) Obtain images as stereo pairs with
   * Flycapture2
    * flycap_get_rectified	= 1	// (default=0) Rectify stereo images (needs
   * Triclops installed)
    * flycap_rect_width	= 320	// (default=640) Width for output rectified
   * images
    * flycap_rect_height	= 240	// (default=480) Height for output rectified
   * images
    *
    * \endcode
    * \note All parameter names may have an optional prefix, set with the
   * "prefix" parameter.
    *  For example, if prefix="LEFT_", the expected variable name "camera_index"
   * in the config section will be "LEFT_camera_index", and so on.
    */
  void loadOptionsFrom(const mrpt::utils::CConfigFileBase &configSource,
...
protected:
  void *m_camera; //!< Opaque pointer to the FlyCapture2::Camera object. nullptr
                  //!if no camera is grabbing.
  void *m_camera_info; //!< Opaque pointer to the FlyCapture2::CameraInfo
                       //!object. nullptr if no camera is grabbing.
  void *m_img_buffer;  //!< Opaque pointer to the FlyCapture2::Image, used as a
                       //!temporary buffer and to avoid mem alloc/reallocs.
  void *m_triclops; //!< Opaque pointer to the TriclopsContext objetc. nullptr
...
public:
  ...
  CImageGrabber_FlyCapture2 &
  operator=(const CImageGrabber_FlyCapture2 &) = delete;
   ...
   * already opened.
    * NOTE: This method only works with Firewire cameras, not with USB3 or GigaE
   * ones (as confirmed by PGR support service).
    * \sa startCapture
    */
  static void startSyncCapture(int numCameras,
                               const CImageGrabber_FlyCapture2 **cameras_array);

  /** Stop capture. */
  void stopCapture();

  /** Stop capture and closes the opened camera, if any. Called automatically on
   * object destruction. */
  void close();
  ...
  /** Grab stereo image from the camera. This method blocks until the next frame
   * is captured.
   * \return false on any error. */
...
static_assert(!std::is_copy_constructible<CImageGrabber_FlyCapture2>::value &&
