...
public:
  CFFMPEG_InputStream(); //!< Default constructor, does not open any video
                         //!source at startup
   ...
   * "c:\a.mpeg") and also IP cameras (e. "rtsp://a.b.c.d/live.sdp").
    *  However, note that there is currently no support for user/password in IP
   * access.
   ...
   * success.
    */
  bool openURL(const std::string &url, bool grab_as_grayscale = false,
               bool verbose = false);
  ...
  double getVideoFPS() const; //!< Get the frame-per-second (FPS) of the video
                              //!source, or "-1" if the video is not open.
