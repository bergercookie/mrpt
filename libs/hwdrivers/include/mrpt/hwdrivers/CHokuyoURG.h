...
public:
  ...
  struct TSensorInfo {
    ...
    int scan_first, scan_last, scan_front; //!< First, last, and front step of
                                           //!the scanner angular span.
    int motor_speed_rpm;                   //!< Standard motor speed, rpm.
  };

private:
  int m_firstRange,
      m_lastRange; //!< The first and last ranges to consider from the scan.
  ...
  std::string m_lastSentMeasCmd; //!< The last sent measurement command (MDXXX),
                                 //!including the last 0x0A.
  ...
  /** Ask to the device, and print to the debug stream, details about the sensor
   * model.
  ...
  /** Waits for a response from the device.
    * \return false on any error
    */
  bool receiveResponse(const char *sentCmd_forEchoVerification,
                       char &rcv_status0, char &rcv_status1, char *rcv_data,
                       int &rcv_dataLength);

  /** Assures a minimum number of bytes in the input buffer, reading from the
   * serial port only if required.
    * \return false if the number of bytes are not available, even after trying
   * to fetch more data from the serial port.
    */
...
public:
  ...
  /** Enables the scanning mode (which may depend on the specific laser device);
   * this must be called before asking for observations to assure that the
   * protocol has been initializated.
   ...
   * automatically when this class is first used to request data from the laser.
   */
...
protected:
  ...
  double m_reduced_fov; //!< Used to reduce artificially the interval of scan
                        //!ranges.

  std::string m_com_port; //!< If set to non-empty, the serial port will be
                          //!attempted to be opened automatically when this
                          //!class is first used to request data from the laser.

  std::string m_ip_dir; //!< If set to non-empty and m_port_dir too, the program
                        //!will try to connect to a Hokuyo using Ethernet
                        //!communication
  unsigned int m_port_dir; //!< If set to non-empty and m_ip_dir too, the
                           //!program will try to connect to a Hokuyo using
  ...
  uint32_t m_timeStartUI;    //!< Time of the first data packet, for
                             //!synchronization purposes.
  int m_timeStartSynchDelay; //!< Counter to discard to first few packets before
                             //!setting the correspondence between device and
                             //!computer timestamps.
