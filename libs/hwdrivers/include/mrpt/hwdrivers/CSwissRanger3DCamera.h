...
/** A class for grabing "range images" from a MESA imaging SwissRanger 3D
  ...
  *
  * NOTES:
  *		- This class requires a vendor specific driver installed in the
  *system in order to build MRPT with support for this sensor. Download and
  *install the driver from: http://www.mesa-imaging.ch/drivers.php
  *		- The intensity channel (grayscale image) is converted from 16bit
  *to standard 8bit-per-pixel using a logarithmic, modified A-law compression.
  *This allows exploiting the full dynamic range of the sensor and provides
  *quite good results.
  *
  ...
  *    save_3d            = true			// Save the 3D point cloud
  *(default: true)
  *    save_range_img     = true			// Save the 2D range image
  *(default: true)
  *    save_intensity_img = true			// Save the 2D intensity image
  *(default: true)
  *    save_confidence    = true			// Save the estimated confidence
  *2D image (default: false)
  *
  *    enable_img_hist_equal = false		// Enable intensity image
  *histogram equalization (default: false)
  *    enable_median_filter  = true			// Enable median filter in
  *range data (default: true)
  *    enable_mediancross_filter = false	// Enable median cross-filter
  *(default: false)
  *    enable_conv_gray      = false		// Enable intensity image scale
  *with range (default: false)
  ...
  */
class HWDRIVERS_IMPEXP CSwissRanger3DCamera
...
public:
  ...
  bool open(); //!< return false on error - Called automatically from
               //!initialize(), no need normally for the user to call this.
  void close();

  bool isOpen() const; //!< whether the camera is open and comms work ok. To be
...
protected:
  ...
  bool m_save_confidence; //!< Save the estimated confidence 2D image (default:
                          //!false)
