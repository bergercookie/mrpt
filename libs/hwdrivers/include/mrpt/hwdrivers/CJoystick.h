...
private:
  ...
  /** Using an event system we only have deltas, need to keep the whole joystick
   * state (Linux only) */
...
public:
   ...
   * second 1, etc... See CJoystick::getJoysticksCount to discover the number of
   ...
   * \param buttons Each element will hold true if buttons are pressed. The size
   * of the vector will be set automatically to the number of buttons.
   ...
   * \sa setLimits
   */
  bool getJoystickPosition(int nJoy, float &x, float &y, float &z,
                           std::vector<bool> &buttons, int *raw_x_pos = nullptr,
                           int *raw_y_pos = nullptr, int *raw_z_pos = nullptr);
 ...
 */
  ...
  void setLimits(int x_min = 0, int x_max = 0xFFFF, int y_min = 0,
                 int y_max = 0xFFFF, int z_min = 0, int z_max = 0xFFFF);
#else
  void setLimits(int x_min = -32767, int x_max = 32767, int y_min = -32767,
