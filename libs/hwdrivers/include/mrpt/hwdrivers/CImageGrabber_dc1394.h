...
struct HWDRIVERS_IMPEXP TCaptureOptions_dc1394 {
  TCaptureOptions_dc1394()
      : frame_width(640), frame_height(480), framerate(FRAMERATE_15),
        color_coding(COLOR_CODING_YUV422), mode7(-1), shutter(-1), gain(-1),
        gamma(-1), brightness(-1), exposure(-1), sharpness(-1),
        white_balance(-1), shutter_mode(-1), gain_mode(-1), gamma_mode(-1),
        brightness_mode(-1), exposure_mode(-1), sharpness_mode(-1),
        white_balance_mode(-1), deinterlace_stereo(false), trigger_power(-1),
        trigger_mode(-1), trigger_source(-1), trigger_polarity(-1),
        ring_buffer_size(15) {}
  ...
  int mode7; //!< -1: Normal mode, i>=0: use MODE7_i, then frame_width/height
             //!and color_coding are ignored.
...
public:
   ...
   * \param cameraGUID Set the camera GUID to open, or 0 to open the first found
   * camera.
   * \param cameraUnit (Ignored if cameraGUID=0). The number of camera to open
   * within the device with the given GUID: In a stereo camera this may be 0 or
   * 1. Normally this is 0.
  ...
  */
  ...
  /** Changes the capture properties (brightness, gain, shutter, etc)
    * The frame size, framerate, and color_coding fields in options are ignored
   * since they can be only set at construction time.
   ...
   * available yet. Ensure trigger before getObservation() or take into account
   * that this call may block.
