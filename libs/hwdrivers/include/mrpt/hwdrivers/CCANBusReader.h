 ...
 * Valid values: 25,50,100 (Default=50)
  ...
  */
class HWDRIVERS_IMPEXP CCANBusReader : public mrpt::utils::COutputLogger,
...
private:
  bool tryToOpenComms(std::string *err_msg = nullptr); //!< Tries to open the
                                                       ...
                                                       //!already open.
  bool waitContinuousSampleFrame(uint8_t &out_prio, uint8_t &out_pdu_format,
                                 uint8_t &out_pdu_spec,
                                 uint8_t &out_src_address,
                                 uint8_t &out_data_length, uint16_t &out_pgn,
                                 std::vector<uint8_t> &out_data,
                                 std::vector<char> &out_raw_frame);

  bool
  sendCANBusReaderSpeed();  //!< Sends the specified speed to the CAN Converter.
  ...
  bool sendCommandToCANReader(const uint8_t *cmd, const uint16_t cmd_len,
                              bool wait = true);
...
public:
  ...
  /** Enables/Disables the addition of a timestamp according to the arrival time
   * to the converter (default=false)
