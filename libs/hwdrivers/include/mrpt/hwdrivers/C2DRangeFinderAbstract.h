...
private:
  ...
  mrpt::obs::CObservation2DRangeScan::Ptr
      m_nextObservation; //!< A dynamic object used as buffer in doProcess

  mrpt::obs::CObservation2DRangeScan::TListExclusionAreasWithRanges
      m_lstExclusionPolys; //!< A list of optional exclusion polygons, in
                           //!coordinates relative to the vehicle, that is,
                           //!taking into account the "sensorPose".
  std::vector<std::pair<double, double>> m_lstExclusionAngles; //!< A list of
                                                               //!pairs of
                                                               //!angles
                                                               //!<init,end>
                                                               //!such as all
  ...
  bool m_showPreview; //!< If true, shows a 3D window with a preview of the
                      //!grabber data
...
protected:
  ...
  /** Should be call by derived classes at "loadConfig" (loads exclusion areas
    ...
    */
  void loadCommonParams(const mrpt::utils::CConfigFileBase &configSource,
  ...
  /** Must be called inside the capture method to allow optional GUI preview of
   * scans */
...
public:
  ...
  /** Get the last observation from the sensor, if available, and unmarks it as
   * being "the last one" (thus a new scan must arrive or subsequent calls will
   * find no new observations).
    */
  void getObservation(bool &outThereIsObservation,
   ...
   * proccessing of incoming data. It can be run in a different thread safely.
    */
  virtual void
  doProcessSimple(bool &outThereIsObservation,
                  mrpt::obs::CObservation2DRangeScan &outObservation,
