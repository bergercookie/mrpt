...
struct TCaptureCVOptions {
  TCaptureCVOptions()
      : frame_width(0), frame_height(0), gain(0), ieee1394_fps(0),
        ieee1394_grayscale(false) {}

  int frame_width,
      frame_height; //!< (All cameras) Capture resolution (0: Leave the default)
  double gain;      //!< (All cameras) Camera gain (0: Leave the default)
  double ieee1394_fps; //!< (IEEE1394 cameras) Frame rate for the capture (0:
                       //!Leave the default).
  bool ieee1394_grayscale; //!< (IEEE1394 cameras) Whether to grab grayscale
...
public:
   ...
   * \param cameraType Can be any value of TCameraType, or CAMERA_CV_AUTODETECT
