...
public:
  ...
  /** Turns the servo up to the specified angle (in radians in the range -pi,pi,
   * other values will be saturated to the maximum or the mininum)
  ...
  /** Turns the servo up to the specified angle (in radians in the range -pi,pi)
   * filtered by average with the last N specified angles.
    * \param Angle: the new desired angle to turn.
    * \param Servo: the id of the servo to move (in our ATMEGA16, from 0 to 2).
    * \param Fast: indicates if the servo must reach the angle at maximum speed
    * \return Whether or not the procedure succeded.
    */
  bool setAngleWithFilter(double angle, const uint8_t servo = 0,
                          bool fast = false);
...
protected:
  std::string m_usbSerialNumber; //!< A copy of the device serial number (to
                                 //!open the USB FTDI chip).
  double m_MaxValue; //!< The value set in the ICR register within the ATMEGA16
                     //!controller.
  double m_TruncateFactor; //!< The range of turn of the servo will be truncated
                           //!to "+-m_truncate_factor*(pi/2)".
  std::deque<double> m_PrevAngles; //!< A vector containing the last N angles
                                   //!which where passed to the servo (for
                                   //!averaging)
  unsigned int
      m_NumPrevAngles; //!< Number of previous angles to store for averaging
  ...
  bool setRegisterValue(const uint16_t value, const uint8_t servo = 0,
                        bool fast = false);
  bool setRegisterValueAndSpeed(const uint16_t value, const uint8_t servo,
