...
/** A class for interfacing XSens 3rd generation Inertial Measuring Units
  ...
  *	   COM_port_LIN	= /dev/ttyUSB0       ; COM PORT in LINUX (optional.
  *If not provided, the system will search the connected port)
  *	   COM_port_WIN	= COM1               ; COM PORT in Windows
  *(optional. If not provided, the system will search the connected port)
  *	   baudRate	                         ; Baudrate for communicating
  *with the COM port (mandatory for Linux)
  *						                    (for Windows, if
  *COM_port_WIN is not provided, this value is ignored)
  *  \endcode
...
protected:
  ...
  // CSerialPort		m_serial_port;			//!< The serial port
  // connection
