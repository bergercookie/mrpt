...
/** A class for grabing "range images", intensity images (either RGB or IR) and
  ...
  *datasets within a robot with more sensors.
  *		- There is no built-in threading support, so if you use this class
  *manually (not with-in rawlog-grabber),
  *			the ideal would be to create a thread and continuously request
  *data from that thread (see mrpt::system::createThread ).
  *		- The intensity channel default to the RGB images, but it can be
  *changed with setVideoChannel() to read the IR camera images (useful for
  *calibrating).
  *		- There is a built-in support for an optional preview of the data
  *on a window, so you don't need to even worry on creating a window to show
  *them.
  *		- This class relies on an embedded version of libfreenect (you do
  *NOT need to install it in your system). Thanks guys for the great job!
  *
  ...
  *<code> MRPT/scripts/51-kinect.rules </code> in
  *<code>/etc/udev/rules.d/</code> to allow access to all users.
  *		- Windows:
...
public:
  typedef float TDepth2RangeArray[KINECT_RANGES_TABLE_LEN]; //!< A typedef for
                                                            //!an array that
                                                            //!converts raw
                                                            //!depth to ranges
                                                            //!in meters.
  ...
  /** To be called  at a high rate (>XX Hz), this method populates the internal
   * buffer of received observations.
    *  This method is mainly intended for usage within rawlog-grabber or similar
   * programs.
   ...
   * there_is_obs=true).
    ...
    * \sa doProcess
    */
  void getNextObservation(mrpt::obs::CObservation3DRangeScan &out_obs,
                          bool &there_is_obs, bool &hardware_error);

  /** \overload
    * \note This method also grabs data from the accelerometers, returning them
   * in out_obs_imu
    */
  void getNextObservation(mrpt::obs::CObservation3DRangeScan &out_obs,
  ...
  /** Set the sensor index to open (if there're several sensors attached to the
   * computer); default=0 -> the first one. */
  ...
  /** Get a reference to the array that convert raw depth values (10 or 11 bit)
   * into ranges in meters, so it can be read or replaced by the user.
    *  If you replace it, remember to set the first and last entries (index 0
   * and KINECT_RANGES_TABLE_LEN-1) to zero, to indicate that those are invalid
   * ranges.
    */
...
protected:
  ...
  size_t m_preview_window_decimation; //!< If preview is enabled, only show 1
                                      //!out of N images.
  ...
  mrpt::poses::CPose3D
      m_relativePoseIntensityWRTDepth; //!< See
                                       //!mrpt::obs::CObservation3DRangeScan for
                                       //!a diagram of this pose

  int m_initial_tilt_angle; //!< Set Kinect tilt to an initial deegre (it should
                            //!be take in account in the sensor pose by the
                            //!user)
...
namespace utils {
template <> struct TEnumTypeFiller<hwdrivers::CKinect::TVideoChannel> {
