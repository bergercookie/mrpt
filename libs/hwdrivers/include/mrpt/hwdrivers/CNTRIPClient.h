...
/** A client for NTRIP (HTTP) sources of differential GPS corrections from
  ...
  *radio.
  *  Usage:
  *		- To open the server, invoke "open" with the proper parameters.
  *Then use "stream_data" to read the read data.
  *		- To obtain a list of all the mountpoints available at a given
  *NTRIP Caster, call "retrieveListOfMountpoints" (it's a static method).
  *
...
public:
  ...
  struct HWDRIVERS_IMPEXP TMountPoint {
    ...
    TMountPoint()
        : carrier(0), latitude(0), longitude(0), needs_nmea(false),
          net_ref_stations(false), authentication('B'), pay_service(false),
          stream_bitspersec(0) {}
  };

  typedef std::list<TMountPoint>
      TListMountPoints; //!< Used in CNTRIPClient::retrieveListOfMountpoints
  ...
  struct HWDRIVERS_IMPEXP NTRIPArgs {
    ...
    /** Default params */
    NTRIPArgs()
        : server("www.euref-ip.net"), port(2101), user(""), password(""),
          mountpoint() {}
  };
...
protected:
  ...
  mutable bool
      m_thread_do_process; //!< Will be "true" between "open" and "close"
  ...
  mrpt::synch::MT_buffer
      m_upload_data; //!< Buffer for data to be sent back to the server
...
public:
  ...
  /** Connect to a given NTRIP caster and get the list of all available
    ...
    *  Note that the authentication parameters "auth_user" and "auth_pass" will
    *be left empty in most situations, since LISTING the Caster normally doesn't
    ...
    */
  static bool
  retrieveListOfMountpoints(TListMountPoints &out_list, std::string &out_errmsg,
