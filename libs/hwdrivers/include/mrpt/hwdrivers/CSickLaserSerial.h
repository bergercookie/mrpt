...
private:
  ...
  bool tryToOpenComms(std::string *err_msg = nullptr); //!< Tries to open the
                                                       //!com port and setup all
                                                       //!the LMS protocol.
                                                       //!Returns true if OK or
                                                       //!already open.
  bool waitContinuousSampleFrame(std::vector<float> &ranges,
                                 unsigned char &LMS_status, bool &is_mm_mode);

  bool LMS_setupSerialComms(); //!< Assures laser is connected and operating at
                               //!38400, in its case returns true.
  bool LMS_setupBaudrate(int baud); //!< Send a command to change the LMS comms
                                    //!baudrate, return true if ACK is OK. baud
                                    //!can be: 9600, 19200, 38400, 500000
  bool LMS_statusQuery(); //!< Send a status query and wait for the answer.
                          //!Return true on OK.
  ...
  bool SendCommandToSICK(const uint8_t *cmd,
                         const uint16_t cmd_len); //!< Send
                                                  //!header+command-data+crc and
                                                  //!waits for ACK. Return false
                                                  //!on error.
  ...
  CSerialPort *m_mySerialPort; //!< Will be !=nullptr only if I created it, so I
                               //!must destroy it at the end.
  int m_com_baudRate;          //!< Baudrate: 9600, 38400, 500000
  unsigned int m_nTries_connect; //!< Default = 1
  unsigned int m_nTries_current;
  bool m_skip_laser_config; //!< If true, doesn't send the initialization
                            //!commands to the laser and go straight to
                            //!capturing
