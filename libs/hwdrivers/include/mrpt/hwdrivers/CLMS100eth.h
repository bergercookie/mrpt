...
*C2DRangeFinderAbstract::bindIO.
        *   Connection is established when user call the turnOn() method. You
*can pass to the class's constructor the LMS100 's ip address and port.
...
*them, so
        *    <b>before using this class</b>, you must "pre-configure" your
*scanner with the SICK's software "SOAP" (this software ships with the device),
...
*memory.
        ...
        * \endcode
        *
        * The sensor pose on the vehicle could be loaded from an ini
*configuration file with :
        *  \code
        *  PARAMETERS IN THE ".INI"-LIKE CONFIGURATION STRINGS:
        * -------------------------------------------------------
        *   [supplied_section_name]
        *	  ip_address = 192.168.0.50 ;a string wich is the SICK's ip
*adress (default is 192.168.0.1)
        *   TCP_port = 1234			; an integer value : the tcp ip
*port on wich the sick is listening (default is 2111).
...
public:
  /** Constructor.
          * Note that there is default arguments, here you can customize IP
   * Adress and TCP Port of your device.
          */
  CLMS100Eth(std::string _ip = std::string("192.168.0.1"),
   ...
   * hardwareError will be set to true.
          ...
          *
          * \exception This method throw exception if the frame received from
   * the LMS 100 contain the following bad parameters :
          *  * Status is not OK
