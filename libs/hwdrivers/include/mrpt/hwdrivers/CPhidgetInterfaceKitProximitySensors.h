...
* (phidget's part number : 1101),or a MaxBotix EZ-1 sonar (phidget's part number :
* 1118).
 ...
 * process_rate = 100 						// Integer value in Hz
* (common to all sensors), default value is 50Hz.
 * displayRecapitulativeInformations = true	// default value = false.
 * serialNumber = 12345						// The interface kit serial
* number (Integer value), default value is -1.
 * sensor1 = SHARP-30cm						// sharp InfraRed sensor
* 30cm range (string value). capital to convert raw data to range data (in
* meters).
 * pose1_x = 0								// position on the robot
* (float value in meters)
 * pose1_y = 0
 * pose1_z = 0.5
 * pose1_yaw = -45.0     					// Angles in degrees (float
* value).
 * pose1_pitch = 0
 * pose1_roll = 0
 *  //...
 * sensorn = EZ1							// Maxbotix Ultrasound
* sonar
 * posen_x = 0
 * // ...
 * \endcode
...
* the constructor will block during
 * in the worst case 200ms, if the board isn't found, an exception will be
* thrown.
 * mrpt::obs::CObservation returned by this class is a CObservationRange.
* CObservationrange::minSensorDistance will be the minimum of the minimum of the
* sensor distances, e.g if you plug to the interface
 * kit a GP2D12 (min range 4 cm) and a GP2Y0A21 (min range 8 cm), then
* CObservationrange::minSensorDistance = min(0.04,0.08) = 0.04. Respectively for
* the maximal range.
 * \endlatexonly
...
class HWDRIVERS_IMPEXP CPhidgetInterfaceKitProximitySensors
...
public:
  /** Constructor
    * \param serialNumber The board's serial number. Set -1 to choose the first
   * available board
    */
...
private:
  /** An 8 dimension vector of boolean value wich store the presence or abscence
   * of a sensor on the phidget interface kit board.
    */
