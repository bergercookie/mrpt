...
/** A generic interface for a wide-variety of sensors designed to be used in the
  ...
  *mind:
  *		- Object constructor
  *		- CGenericSensor::loadConfig: The following parameters are common
  *to all sensors in rawlog-grabber (they are automatically loaded by
  *rawlog-grabber) - see each class documentation for additional parameters:
  *			- "process_rate": (Mandatory) The rate in Hertz (Hz) at which
  *the sensor thread should invoke "doProcess".
  *			- "max_queue_len": (Optional) The maximum number of objects in
  *the observations queue (default is 200). If overflow occurs, an error message
  *will be issued at run-time.
  *			- "grab_decimation": (Optional) Grab only 1 out of N
  *observations captured by the sensor (default is 1, i.e. do not decimate).
...
public:
   ...
   * operation.
    * Default: disabled unless the environment variable "MRPT_HWDRIVERS_VERBOSE"
   * is set to "1" during object creation.
    */
  ...
  /** Register a class into the internal list of "CGenericSensor" descendents.
    ...
    *
    *  Can be used as "CGenericSensor::registerClass( SENSOR_CLASS_ID(CMySensor)
   * );" if
...
private:
  ...
  TListObservations
      m_objList; //!< The queue of objects to be returned by getObservations

  /** Used in registerClass */
  typedef std::map<std::string, const TSensorClassId *>
      registered_sensor_classes_t;
  static registered_sensor_classes_t &
  get_registered_sensor_classes(); //!< Access to singleton
...
protected:
  ...
  size_t m_grab_decimation;  //!< If set to N>=2, only 1 out of N observations
                             //!will be saved to m_objList.
  std::string m_sensorLabel; //!< See CGenericSensor

  /** @} */

  size_t
      m_grab_decimation_counter; //!< Used when "m_grab_decimation" is enabled
  ...
  std::string m_path_for_external_images; //!< The path where to save off-rawlog
                                          //!images: empty means save images
                                          //!embedded in the rawlog.
  std::string m_external_images_format;   //!< The extension
                                          //!("jpg","gif","png",...) that
                                        //!determines the format of images saved
                                        //!externally \sa
                                        //!setPathForExternalImages
  unsigned int m_external_images_jpeg_quality; //!< For JPEG images, the quality
                                               //!(default=95%).
  // ======================================

  /** This method must be called by derived classes to enqueue a new observation
    in the list to be returned by getObservations.
    ...
    */
  void
  appendObservations(const std::vector<mrpt::utils::CSerializable::Ptr> &obj);
...
public:
  /** Creates a sensor by a name of the class.
    *  Typically the user may want to create a smart pointer around the returned
   * pointer, whis is made with:
    *  \code
  ...
  /** Just like createSensor, but returning a smart pointer to the newly created
   * sensor object. */
  static inline CGenericSensor::Ptr
  createSensorPtr(const std::string &className) {
  ...
  /** Returns a list of enqueued objects, emptying it (thread-safe). The objects
   * must be freed by the invoker.
    */
  void getObservations(TListObservations &lstObjects);

  /**  Set the path where to save off-rawlog image files (will be ignored in
   * those sensors where this is not applicable).
    *  An  empty string (the default value at construction) means to save images
   * embedded in the rawlog, instead of on separate files.
...
static_assert(!std::is_copy_constructible<CGenericSensor>::value &&
