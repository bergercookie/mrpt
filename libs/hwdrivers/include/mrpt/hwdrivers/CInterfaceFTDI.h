...
std::ostream HWDRIVERS_IMPEXP &operator<<(std::ostream &o,
                                          const TFTDIDevice &d);
 ...
 *		- Windows: "FT2XX.DLL" and the device drivers (see FTDI
 *website).
 *		- Linux: "libusb.so" (quite standard!), and "libftdi.so" only if
 *linking against the dynamic library.
 *
 ...
 *		- 16/FEB/2007: Integration into the MRPT framework. Support for
 *device serial numbers. JLBC
...
public:
   ...
   * for a few ms, data is not sent to the PC waiting for possible more data, to
   * save USB trafic. */
  void SetLatencyTimer(unsigned char latency_ms);

  /** Change read & write timeouts, in milliseconds. */
  void SetTimeouts(unsigned long dwReadTimeout_ms,
...
protected:
  ...
  void ftdi_read(void *lpvBuffer, unsigned long dwBuffSize,
                 unsigned long *lpdwBytesRead);
  void ftdi_write(const void *lpvBuffer, unsigned long dwBuffSize,
...
private:
  ...
  typedef FT_STATUS(__stdcall *PtrToOpenEx)(void *, unsigned long,
                                            unsigned long *);
  ...
  typedef FT_STATUS(__stdcall *PtrToRead)(unsigned long, void *, unsigned long,
                                          unsigned long *);
  PtrToRead m_pRead;

  typedef FT_STATUS(__stdcall *PtrToWrite)(unsigned long, const void *,
  ...
  typedef FT_STATUS(__stdcall *PtrToSetTimeouts)(unsigned long, unsigned long,
                                                 unsigned long);
  PtrToSetTimeouts m_pSetTimeouts;

  typedef FT_STATUS(__stdcall *PtrToGetQueueStatus)(unsigned long,
                                                    unsigned long *);
  PtrToGetQueueStatus m_pGetQueueStatus;

  typedef FT_STATUS(__stdcall *PtrToSetLatencyTimer)(unsigned long,
                                                     unsigned char);
  ...
  void recursive_fill_list_devices(void *usb_device_structure,
