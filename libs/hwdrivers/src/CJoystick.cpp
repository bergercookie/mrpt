...
/*---------------------------------------------------------------
 ...
 is not present.
 ---------------------------------------------------------------*/
bool CJoystick::getJoystickPosition(int nJoy, float &x, float &y, float &z,
                                    std::vector<bool> &buttons, int *raw_x_pos,
                                    int *raw_y_pos, int *raw_z_pos) {
  MRPT_START
#ifdef MRPT_OS_WINDOWS
  JOYINFO jinfo;
  ...
  if (raw_x_pos)
    *raw_x_pos = jinfo.wXpos;
  if (raw_y_pos)
    *raw_y_pos = jinfo.wYpos;
  if (raw_z_pos)
    *raw_z_pos = jinfo.wZpos;
  ...
  if (m_joy_index == nJoy && m_joy_fd != -1) {
    ...
    if ((m_joy_fd = open(format("/dev/input/js%i", nJoy).c_str(), O_RDONLY)) <
        0)
      return false;
...
*/
void CJoystick::setLimits(int x_min, int x_max, int y_min, int y_max, int z_min,
                          int z_max) {
  m_x_max = x_max;
  m_x_min = x_min;
