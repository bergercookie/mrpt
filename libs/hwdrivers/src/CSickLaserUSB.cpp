...
void CSickLaserUSB::doProcessSimple(
  ...
  for (size_t i = 0; i < ranges.size(); i++) {
    ...
    outObservation.setScanRangeValidity(i,
                                        (ranges[i] <= outObservation.maxRange));
  }
...
void CSickLaserUSB::loadConfig_sensorSpecific(
  ...
  m_serialNumber = configSource.read_string(iniSection, "SICKUSB_serialNumber",
                                            m_serialNumber);
  m_sensorPose =
...
-------------------------------------------------------------*/
bool CSickLaserUSB::waitContinuousSampleFrame(vector<float> &out_ranges_meters,
  ...
  while (nFrameBytes < (lenghtField = (6 + (buf[2] | (buf[3] << 8)))) +
    ...
    if (nFrameBytes < 4)
      ...
      nBytesToRead =
          (5 /* for 32bit timestamp + end-flag */ + lenghtField) - nFrameBytes;

    try {
      nRead = m_usbConnection->ReadSync(buf + nFrameBytes, nBytesToRead);
    } catch (std::exception &e) {
      // Disconnected?
      MRPT_LOG_ERROR_FMT("[CSickLaserUSB::waitContinuousSampleFrame] "
...
// endl;
  ...
  if (CRC_packet != CRC) {
    const string s =
        format("[CSickLaserUSB::waitContinuousSampleFrame] bad CRC len=%u "
               "nptns=%u: %i != %i\n",
