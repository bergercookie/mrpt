...
CCANBusReader::CCANBusReader()
    : mrpt::utils::COutputLogger("CCANBusReader"), m_com_port(),
      m_mySerialPort(nullptr), m_com_baudRate(57600), m_nTries_connect(1),
      m_nTries_current(0), m_canbus_speed(250000), m_canreader_timestamp(false),
...
void CCANBusReader::doProcess() {
  mrpt::obs::CObservationCANBusJ1939::Ptr obs =
      std::make_shared<mrpt::obs::CObservationCANBusJ1939>();
...
void CCANBusReader::doProcessSimple(
  ...
  if (!waitContinuousSampleFrame(out_prio, out_pdu_format, out_pdu_spec,
                                 out_src_address, out_data_length, out_pgn,
                                 out_data, out_raw_frame))
    return;
...
bool CCANBusReader::tryToOpenComms(std::string *err_msg) {
  ...
  try {
    ...
    // open the CAN channel. If true, at this point, frames should be poping out
    // the CAN Bus
...
bool CCANBusReader::waitContinuousSampleFrame(
  ...
  if (buf[nFrameBytes - 1] != 0x0D) {
    cout << format("[CCANBusReader::waitContinuousSampleFrame] expected 0x0D "
...
bool CCANBusReader::waitForVersion(uint16_t timeout, bool printOutVersion) {
  ...
  if (m_received_frame_buffer[nBytes - 1] != 0x0D) {
    printf("[CCANBusReader::waitForVersion] Error: expected 0x0D final byte, "
           "received %x\n",
...
bool CCANBusReader::waitIncomingFrame(uint16_t timeout) {
    ...
    if (m_mySerialPort->Read(&b, 1)) {
      ...
      if (nBytes == 10)
        dlc = 2 * uint8_t(hexCharToInt(
                      m_received_frame_buffer[9])); // here is the number of
                                                    // BYTES of data -> 2 hex
                                                    // values for byte
    }
    if (tictac.Tac() >= maxTime)
  ...
  if (m_received_frame_buffer[10U + dlc] != 0x0D) {
    printf("[CCANBusReader::waitIncomingFrame] Error: expected 0x0D as final "
           "flag, received %x\n",
...
bool CCANBusReader::sendCommandToCANReader(const uint8_t *cmd,
                                           const uint16_t cmd_len, bool wait) {
