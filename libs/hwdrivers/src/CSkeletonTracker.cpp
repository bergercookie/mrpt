...
CSkeletonTracker::CSkeletonTracker()
    : m_skeletons_ptr(nullptr), m_userTracker_ptr(nullptr), m_timeStartUI(0),
      m_timeStartTT(0), m_sensorPose(), m_nUsers(0), m_showPreview(false),
      m_toutCounter(0) {
...
#else
  THROW_EXCEPTION("MRPT has been compiled with 'BUILD_OPENNI2'=OFF or "
                  "'BUILD_NITE2'=OFF, so this class cannot be used.");
#endif
}
...
void CSkeletonTracker::processPreviewNone() {
  ...
  if (m_showPreview) {
    if (!m_win) {
      ...
      m_win = std::make_shared<mrpt::gui::CDisplayWindow3D>(caption, 800, 600);
    ...
    if (m_win && m_win->isOpen()) {
      COpenGLScene::Ptr &scene = m_win->get3DSceneAndLock();
      {
        ...
        if (!scene->getByName("dummy")) {
          ...
          CSetOfObjects::Ptr dummy = std::make_shared<CSetOfObjects>();
          dummy->setName("dummy");
          dummy->setPose(math::TPose3D(0, 0, 0, 0, 0, DEG2RAD(-90)));
          {
            // head
            CSphere::Ptr part = std::make_shared<CSphere>(HEAD_RADIUS);
          ...
          }
          {
            // left arm 0
            CCylinder::Ptr part =
                std::make_shared<CCylinder>(ARM_RADIUS, ARM_RADIUS, ARM_LENGTH);
            part->setColor(1, 1, 1, ALPHA_CH);
            part->setPose(math::TPose3D(-BODY_RADIUS, 0,
                                        0.5 * BODY_LENGTH - ARM_RADIUS, 0,
                                        DEG2RAD(-90), 0));
            dummy->insert(part);
          }
          {
            ...
            CCylinder::Ptr part =
                std::make_shared<CCylinder>(ARM_RADIUS, ARM_RADIUS, ARM_LENGTH);
            part->setColor(1, 1, 1, ALPHA_CH);
            part->setPose(math::TPose3D(-BODY_RADIUS - ARM_LENGTH + ARM_RADIUS,
                                        0, 0.5 * BODY_LENGTH - ARM_RADIUS, 0, 0,
                                        0));
            dummy->insert(part);
          }
          {
          ...
          }
          {
            // left leg
            CCylinder::Ptr part =
                std::make_shared<CCylinder>(LEG_RADIUS, LEG_RADIUS, LEG_LENGTH);
            part->setColor(1, 1, 1, ALPHA_CH);
            part->setPose(math::TPose3D(-BODY_RADIUS + LEG_RADIUS, 0,
                                        -(0.5 * BODY_LENGTH + LEG_LENGTH), 0, 0,
                                        0));
            dummy->insert(part);
          }
          {
        ...
        CSetOfObjects::Ptr body =
            std::dynamic_pointer_cast<CSetOfObjects>(scene->getByName("body"));
        ASSERT_(body)
  ...
  if (m_showPreview) {
    ...
    if (m_win && m_win->isOpen()) {
      ...
      CSetOfObjects::Ptr dummy =
          std::dynamic_pointer_cast<CSetOfObjects>(scene->getByName("dummy"));
      if (dummy)
...

      {
        ...
        for (int i = 0; i < NUM_JOINTS; ++i) {
          ...
          CSphere::Ptr s = std::dynamic_pointer_cast<CSphere>(
              body->getByName(jointNames[i]));
          s->setPose(
              mrpt::math::TPose3D(j.x * 1e-3, j.y * 1e-3, j.z * 1e-3, 0, 0, 0));
          s->setColor(std::min(1.0, 2 * (1 - j.conf)),
        ...
        CSetOfLines::Ptr lines =
            std::dynamic_pointer_cast<CSetOfLines>(body->getByName("lines"));
        ASSERT_(lines)
        ...
        for (int i = 0; i < NUM_LINES; ++i) {
          ...
          lines->appendLine(s0->getPoseX(), s0->getPoseY(), s0->getPoseZ(),
...
void CSkeletonTracker::doProcess() {
  ...
  for (int i = 0; i < m_nUsers; ++i) {
    ...
    if (user.isNew()) {
      ...
      CObservationSkeleton::Ptr obs = std::make_shared<CObservationSkeleton>();
