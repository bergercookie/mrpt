...
CNationalInstrumentsDAQ::TInfoPerTask::TInfoPerTask()
    : taskHandle(0), must_close(false), is_closed(false), new_obs_available(0),
      task() {}
...
#define MY_LOAD_HERE_CONFIG_VAR(variableName, variableType, targetVariable,    \
...
#define MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(variableName, variableType,         \
                                           targetVariable, configFileObject,   \
                                           sectionNameStr)                     \
  {                                                                            \
    try {                                                                      \
      targetVariable = configFileObject.read_##variableType(                   \
      ...
      THROW_EXCEPTION(format("Value for '%s' not found in config file",        \
...
void CNationalInstrumentsDAQ::loadConfig_sensorSpecific(
  ...
  if (!nTasks) {
    std::cerr << "[CNationalInstrumentsDAQ] Warning: Number of tasks is zero. "
                 "No datalogging will be done.\n";
  }
  ...
  for (unsigned int i = 0; i < nTasks; i++) {
    ...
    MY_LOAD_HERE_CONFIG_VAR(sTask + string(".samplesPerSecond"), double,
                            t.samplesPerSecond, cfg, sect)
    MY_LOAD_HERE_CONFIG_VAR(sTask + string(".samplesPerChannelToRead"), double,
                            t.samplesPerChannelToRead, cfg, sect)
    MY_LOAD_HERE_CONFIG_VAR(sTask + string(".sampleClkSource"), string,
                            t.sampleClkSource, cfg, sect)
    MY_LOAD_HERE_CONFIG_VAR(sTask + string(".bufferSamplesPerChannel"), double,
    ...
    for (size_t j = 0; j < lstStrChanns.size(); j++) {
      if (strCmpI(lstStrChanns[j], "ai")) {
        ...
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(
            sTask + string(".ai.physicalChannel"), string, t.ai.physicalChannel,
            cfg, sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(
            sTask + string(".ai.physicalChannelCount"), uint64_t,
            t.ai.physicalChannelCount, cfg, sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".ai.terminalConfig"),
                                           string, t.ai.terminalConfig, cfg,
                                           sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".ai.minVal"), double,
                                           t.ai.minVal, cfg, sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".ai.maxVal"), double,
                                           t.ai.maxVal, cfg, sect)
      } else if (strCmpI(lstStrChanns[j], "ao")) {
        t.has_ao = true;
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(
            sTask + string(".ao.physicalChannel"), string, t.ao.physicalChannel,
            cfg, sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(
            sTask + string(".ao.physicalChannelCount"), uint64_t,
            t.ao.physicalChannelCount, cfg, sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".ao.minVal"), double,
                                           t.ao.minVal, cfg, sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".ao.maxVal"), double,
                                           t.ao.maxVal, cfg, sect)
      } else if (strCmpI(lstStrChanns[j], "di")) {
        t.has_di = true;
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".di.line"), string,
                                           t.di.line, cfg, sect)
      } else if (strCmpI(lstStrChanns[j], "do")) {
        t.has_do = true;
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".do.line"), string,
                                           t.douts.line, cfg, sect)
      } else if (strCmpI(lstStrChanns[j], "ci_period")) {
        t.has_ci_period = true;
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".ci_period.counter"),
                                           string, t.ci_period.counter, cfg,
                                           sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".ci_period.minVal"),
                                           double, t.ci_period.minVal, cfg,
                                           sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".ci_period.maxVal"),
                                           double, t.ci_period.maxVal, cfg,
                                           sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".ci_period.units"),
                                           string, t.ci_period.units, cfg, sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".ci_period.edge"),
                                           string, t.ci_period.edge, cfg, sect)
        MY_LOAD_HERE_CONFIG_VAR(sTask + string(".ci_period.measTime"), double,
                                t.ci_period.measTime, cfg, sect)
        MY_LOAD_HERE_CONFIG_VAR(sTask + string(".ci_period.divisor"), int,
        ...
        MY_LOAD_HERE_CONFIG_VAR(sTask + string(".ci_count_edges.initialCount"),
                                int, t.ci_count_edges.initialCount, cfg, sect)
        ...
        MY_LOAD_HERE_CONFIG_VAR(sTask + string(".ci_ang_encoder.decimate"), int,
                                t.ci_ang_encoder.decimate, cfg, sect)

      } else if (strCmpI(lstStrChanns[j], "co_pulses")) {
        t.has_co_pulses = true;
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".co_pulses.counter"),
                                           string, t.co_pulses.counter, cfg,
                                           sect)
        MY_LOAD_HERE_CONFIG_VAR(sTask + string(".co_pulses.idleState"), string,
                                t.co_pulses.idleState, cfg, sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(
            sTask + string(".co_pulses.initialDelay"), double,
            t.co_pulses.initialDelay, cfg, sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(sTask + string(".co_pulses.freq"),
                                           double, t.co_pulses.freq, cfg, sect)
        MY_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(
            sTask + string(".co_pulses.dutyCycle"), double,
            t.co_pulses.dutyCycle, cfg, sect)
      } else {
        THROW_EXCEPTION_FMT("Unknown channel type '%s'! See the docs of "
...
void CNationalInstrumentsDAQ::initialize() {
  ...
  for (size_t i = 0; i < task_definitions.size(); i++) {
    ...
    ipt.task = tf; // Save a copy of the task info for the thread to have all
                   // the needed info
    ...
    try {
      ...
      if (tf.has_ai) {
        ASSERTMSG_(
            ...
            "ai.physicalChannelCount is zero! Please, define it correctly.")

        MRPT_DAQmx_ErrChk(MRPT_DAQmxCreateAIVoltageChan(
            taskHandle, tf.ai.physicalChannel.c_str(), nullptr,
            daqmx_defstr2num(tf.ai.terminalConfig), tf.ai.minVal, tf.ai.maxVal,
      ...
      if (tf.has_ao) {
        ...
        MRPT_DAQmx_ErrChk(MRPT_DAQmxCreateAOVoltageChan(
            taskHandle, tf.ao.physicalChannel.c_str(), nullptr, tf.ao.minVal,
            tf.ao.maxVal, DAQmx_Val_Volts, nullptr));
      }
      if (tf.has_di) {
        MRPT_DAQmx_ErrChk(MRPT_DAQmxCreateDIChan(
            taskHandle, tf.di.line.c_str(), nullptr, DAQmx_Val_ChanPerLine));
      }
      if (tf.has_do) {
        MRPT_DAQmx_ErrChk(MRPT_DAQmxCreateDOChan(
            taskHandle, tf.douts.line.c_str(), nullptr, DAQmx_Val_ChanPerLine));
      }
      if (tf.has_ci_period) {
      ...
      if (tf.has_ci_lin_encoder) {
        MRPT_DAQmx_ErrChk(MRPT_DAQmxCreateCILinEncoderChan(
            ...
            tf.ci_lin_encoder.distPerPulse, tf.ci_lin_encoder.initialPos,
            nullptr));
      }
      if (tf.has_ci_ang_encoder) {
        MRPT_DAQmx_ErrChk(MRPT_DAQmxCreateCIAngEncoderChan(
            ...
            tf.ci_ang_encoder.pulsesPerRev, tf.ci_ang_encoder.initialAngle,
            nullptr));
      }
      if (tf.has_co_pulses) {
        MRPT_DAQmx_ErrChk(MRPT_DAQmxCreateCOPulseChanFreq(
            taskHandle, tf.co_pulses.counter.c_str(), nullptr, DAQmx_Val_Hz,
            daqmx_defstr2num(tf.co_pulses.idleState), tf.co_pulses.initialDelay,
            tf.co_pulses.freq, tf.co_pulses.dutyCycle));
      }

      // Seems to be needed to avoid an errors avoid like:
      // " Onboard device memory overflow. Because of system and/or
      // bus-bandwidth limitations, the driver could not read data from the
      // device fast enough to keep up with the device throughput."
      if (tf.has_ai || tf.has_di || tf.has_ci_period || tf.has_ci_count_edges ||
          tf.has_ci_pulse_width || tf.has_ci_lin_encoder ||
          tf.has_ci_ang_encoder) {
        // sample rate:
        ...
        MRPT_DAQmx_ErrChk(MRPT_DAQmxCfgSampClkTiming(
            ...
            DAQmx_Val_Rising, DAQmx_Val_ContSamps, tf.samplesPerChannelToRead));

        MRPT_DAQmx_ErrChk(
            MRPT_DAQmxCfgInputBuffer(taskHandle, tf.bufferSamplesPerChannel));
      }
      ...
      if (tf.has_ao) {
        ...
        //	// Output buffer MUST have some data before starting the task:
        //write 0s:
        //	vector<double> d;
      ...
      // Add a large timeout, just in case the writing thread dies unexpectedly
      // so the reader doesn't hang on:
      ...
      ipt.hThread =
          std::thread(&CNationalInstrumentsDAQ::grabbing_thread, this, ipt);
      ...
      std::cerr << "[CNationalInstrumentsDAQ] Error while creating tasks. "
                   "Closing other tasks before returning...\n";
      this->stop();
...
void CNationalInstrumentsDAQ::stop() {
  ...
  if (m_verbose)
    cout << "[CNationalInstrumentsDAQ::stop] Waiting for grabbing threads to "
            "end...\n";
  for (list<TInfoPerTask>::iterator it = m_running_tasks.begin();
       it != m_running_tasks.end(); ++it) {
...
void CNationalInstrumentsDAQ::grabbing_thread(TInfoPerTask &ipt) {
  ...
  try {
    TaskHandle &taskHandle = *reinterpret_cast<TaskHandle *>(&ipt.taskHandle);
    if (m_verbose)
      cout << "[CNationalInstrumentsDAQ::grabbing_thread] Starting thread for "
              "task "
           << ipt.taskHandle << "\n";
    ...
    while (!ipt.must_close) {
      ...
      if (ipt.task.has_ai) {
        ...
        const uint32_t totalSamplesToRead =
            ipt.task.ai.physicalChannelCount * ipt.task.samplesPerChannelToRead;
      ...
      if (ipt.task.has_ci_ang_encoder || ipt.task.has_ci_lin_encoder) {
        ...
        dBuf.resize(totalSamplesToRead);
        int32 pointsReadPerChan = -1;
        if ((err = MRPT_DAQmxReadCounterF64(taskHandle, totalSamplesToRead,
                                            timeout, &dBuf[0], dBuf.size(),
                                            &pointsReadPerChan, nullptr)) < 0 &&
            err != DAQmxErrorSamplesNotYetAvailable) {
          MRPT_DAQmx_ErrChk(err)
        } else if (pointsReadPerChan > 0) {
      ...
      if (there_are_data) {
        ...
        // std::this_thread::sleep_for(1ms); // This seems to be needed to allow
        // all objs to be sent to the recv thread
      } else {
        std::this_thread::sleep_for(1ms);
      }
...
void CNationalInstrumentsDAQ::writeDigitalOutputTask(size_t task_index,
  ...
  int err = 0;
  if (err = MRPT_DAQmxWriteDigitalLines(taskHandle, nSamplesPerChannel, FALSE,
...
CNationalInstrumentsDAQ::TaskDescription::TaskDescription()
    : has_ai(false), has_ao(false), has_di(false), has_do(false),
      has_ci_period(false), has_ci_count_edges(false),
      has_ci_pulse_width(false), has_ci_lin_encoder(false),
      has_ci_ang_encoder(false), has_co_pulses(false), samplesPerSecond(1000.0),
      bufferSamplesPerChannel(200000), samplesPerChannelToRead(1000) {}
