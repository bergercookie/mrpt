	...
	const mrpt::utils::CConfigFileBase& cfg, const std::string& sect)
{
	...
	{
		std::cerr
			<< "[CNationalInstrumentsDAQ] Warning: Number of tasks is zero. "
...
void CNationalInstrumentsDAQ::initialize()
{
	...
	for (size_t i = 0; i < task_definitions.size(); i++)
	{
		...
		ipt.task =
			tf;  // Save a copy of the task info for the thread to have all
		// the needed info
		...
		catch (std::exception const& e)
		{
			...
			std::cerr
				<< "[CNationalInstrumentsDAQ] Error while creating tasks. "
