...
std::string GUID2Str(const GUID &ifaceGuid) {
  ...
  iRet = StringFromGUID2(ifaceGuid, (LPOLESTR)&GuidString,
...
std::vector<PWLAN_AVAILABLE_NETWORK> ListNetworksW(PWLAN_INTERFACE_INFO iface,
  ...
  // interface
  dwResult = WlanGetAvailableNetworkList((HANDLE)hClient, &ifaceGuid, 0,
                                         nullptr, &pBssList);
 ...
 * format)
                 * \exception std::exception In case there is a failure
                 * \return PWLAN_AVAILABLE_NETWORK returns a handle to the
 * network
                 */
PWLAN_AVAILABLE_NETWORK GetNetworkW(HANDLE hClient, const std::string &ssid,
...
std::vector<std::string> CWirelessPower::ListInterfaces() {
  ...
  // Run the command line: get the info frim /proc/net/wireless and cut out the
...
std::vector<std::string> CWirelessPower::ListNetworks() {
  ...
  // must be done as a superuser, so the command is executed with sudo. Usually
  // it should ask for the password only the first time.
...
bool CWirelessPower::getObservation(
  ...
  try {
    ...
    cerr
        << "[CWirelessPower::getObservation] Returning false due to exception: "
        << endl;
    cerr << e.what() << endl;
    return false;
  }
...
void CWirelessPower::doProcess() {
  ...
  mrpt::obs::CObservationWirelessPower::Ptr outObservation =
      std::make_shared<mrpt::obs::CObservationWirelessPower>();
...
void CWirelessPower::loadConfig_sensorSpecific(
  ...
  guid = configSource.read_string(
      iniSection, "guid", "",
      true); // in the case of Linux, the "GUID" is the interface name (wlanX)
