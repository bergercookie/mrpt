...
C2DRangeFinderAbstract::C2DRangeFinderAbstract()
    : mrpt::utils::COutputLogger("C2DRangeFinderAbstract"), m_lastObservation(),
      m_lastObservationIsNew(false), m_hardwareError(false),
      m_nextObservation(), m_showPreview(false), m_stream(nullptr) {}
...
void C2DRangeFinderAbstract::loadCommonParams(
  ...
  for (;;) {
    ...
    configSource.read_vector(iniSection, format("exclusionZone%u_x", N),
                             vector<double>(0), x);
    configSource.read_vector(iniSection, format("exclusionZone%u_y", N),
                             vector<double>(0), y);
    configSource.read_vector(iniSection, format("exclusionZone%u_z", N++),
                             vector<double>(0), z_range);
