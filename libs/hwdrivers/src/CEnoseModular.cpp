...
bool CEnoseModular::getObservation(mrpt::obs::CObservationGasSensors &obs) {
  try {
    ...
    if (msg.content.size() > 0) {
      ...
      if (enose_poses_x.size() != 0) {
        newRead.eNosePoseOnTheRobot = CPose3D(
            ...
            enose_poses_yaw[0], enose_poses_pitch[0], enose_poses_roll[0]);
      } else
      ...
      for (size_t idx = 0; idx < numSensors; idx++) {
        ...
        // );
        memcpy(&sensorType_temp, &msg.content[idx * 3 + 1],
        ...
        newRead.readingsVoltage.push_back((msg.content[idx * 3 + 3] * 0.6f) /
                                          255.0f);
      }
    ...
    cerr << "[CEnoseModular::getObservation] Returning false due to exception: "
         << endl;
    cerr << e.what() << endl;
    return false;
  } catch (...) {
