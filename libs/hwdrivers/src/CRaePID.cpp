...
bool CRaePID::tryToOpenTheCOM() {
  ...
  try {
    ...
    std::cerr << "[CRaePID::tryToOpenTheCOM] Error opening or configuring the "
                 "serial port:"
              << std::endl
              << e.what();
    COM.close();
...
void CRaePID::doProcess() {
  ...
  appendObservation(mrpt::obs::CObservationGasSensors::Ptr(
...
bool CRaePID::errorStatus(std::string &errorString) {
      ...
      (strcmp(errors_text[1].c_str(), "0") == 0)) // no error
  {
    ...
    // By the moment, return the raw error; note that if necessary a detailed
    // description of the error can be obtained analyzing the two error strings
    // separately
    errorString = reading;
    return true;
  }
