...
/*-------------------------------------------------------------
                                        Constructor
 -------------------------------------------------------------*/
CImageGrabber_OpenCV::CImageGrabber_OpenCV(int cameraIndex,
...
//  if it was CAMERA_CV_AUTODETECT
  ...
  if (options.gain != 0) {
    if (cvSetCaptureProperty(M_CAPTURE, CV_CAP_PROP_GAIN, options.gain) < 1)
      cerr << "[CImageGrabber_OpenCV] Warning: Could not set the capturing "
              "gain property!"
           << endl;
  }
  ...
  if (cameraType == CAMERA_CV_DC1394) {
    if (options.frame_height != 0 && options.frame_width != 0) {
      ...
      else if (options.frame_height == 640 && options.frame_width == 480 &&
      ...
      if (cvMode1394 > 0) {
        ...
        cerr << "[CImageGrabber_OpenCV] Warning: Not valid combination of "
                "width x height x color mode for OpenCV/IEEE1394 interface"
             << endl;
    }
    ...
    if (cvSetCaptureProperty(M_CAPTURE, CV_CAP_PROP_FPS, options.ieee1394_fps) <
        1)
      cerr << "[CImageGrabber_OpenCV] Warning: Could not set the fps property!"
           << endl;
  }
  ...
  if (cameraType == CAMERA_CV_AUTODETECT || cameraType == CAMERA_CV_VFL ||
    ...
    if (options.frame_width != 0 && options.frame_height != 0) {
      ...
      cvSetCaptureProperty(M_CAPTURE, CV_CAP_PROP_FRAME_WIDTH,
                           options.frame_width);
      if (cvSetCaptureProperty(M_CAPTURE, CV_CAP_PROP_FRAME_HEIGHT,
                               options.frame_height) < 1)
        cerr << "[CImageGrabber_OpenCV] Warning: Could not set the frame width "
                "& height property!"
             << endl;
    }
  }
