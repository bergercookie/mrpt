...
template <typename T> inline T *&SAFE_DELETE(T *&p) {
  if (nullptr != p)
    delete (p);
  p = nullptr;
  return (p);
...
template <typename T> struct QuickList {
  T ini;
  T end;
  QuickList() {
...
template <typename T> struct QuickListMutexed : QuickList<T> {
protected:
...
public:
  ...
  ~QuickListMutexed() { /*this->~QuickListMutexed()*/
    mutex.Enter();
    mutex.Leave();
  };
  const bool Empty() const {
...
transfer_wrapper *libusbemu_get_transfer_wrapper(libusb_transfer *transfer) {
  ...
  char *off_address(raw_address - sizeof(void *) -
                    2 * sizeof(transfer_wrapper *) -
                    sizeof(QuickList<transfer_wrapper> *));
  return ((transfer_wrapper *)off_address);
}

libusb_device *libusbemu_register_device(libusb_context *ctx,
...
int libusbemu_setup_transfer(transfer_wrapper *wrapper) {
  ...
  case LIBUSB_TRANSFER_TYPE_ISOCHRONOUS:
    ret = usb_isochronous_setup_async(handle, &context, transfer->endpoint,
                                      transfer->iso_packet_desc[0].length);
    break;
  case LIBUSB_TRANSFER_TYPE_CONTROL:
  // libusb-0.1 does not actually support asynchronous control transfers, but
  // this should be
  // very easy to emulate if necessary: just stick the transfer in a special
  // list and then
  // libusb_handle_events() check if the list is empty or not; if it is not
  // empty, a thread
  // is created temporarily just to deal with such control transfer requests
  // until the list
  // becomes eventually empty again and the thread terminates.
  case LIBUSB_TRANSFER_TYPE_BULK:
  case LIBUSB_TRANSFER_TYPE_INTERRUPT:
  // these transfer types are not being used by libfreenect. they should be
  // fairly simple to
