...
struct QuickList
{
	...
	{
		T* head(nullptr);
		if (!Empty()) head = ini.next;
		return (head);
	}
	T* Last() const
	{
		T* last(nullptr);
		if (!Empty()) last = end.prev;
		return (last);
	}
	const bool Member(T* node) const { return (this == node->list); }
	static T* Prev(T* node)
	{
		T* prev(nullptr);
		if (nullptr != node->prev->prev) prev = node->prev;
		return (prev);
	}
	static T* Next(T* node)
	{
		T* next(nullptr);
		if (nullptr != node->next->next) next = node->next;
		return (next);
	}
	...
	{
		if (Orphan(node)) return;
		node->list->Remove(node);
	}
	static const bool Orphan(T* node) { return (nullptr == node->list); }
};
...
struct QuickListMutexed : QuickList<T>
{
   protected:
	...
	mutable QuickEvent
		chomp;  // signals whether there is (or not) transfers in the list

   public:
	QuickListMutexed(){};
	QuickListMutexed(const QuickListMutexed& rhs) : QuickList<T>(rhs){};
	~QuickListMutexed()
	{ /*this->~QuickListMutexed()*/
		mutex.Enter();
		mutex.Leave();
	};
	const bool Empty() const
	...
	const bool Remove(T* node)
	{
		...
		if (QuickList<T>::Empty()) chomp.Reset();
		mutex.Leave();
		return (removed);
	}
	...
	void Wait() const { chomp.Wait(); }
	const bool Check() const { return (chomp.Check()); }
};
...
void libusbemu_report_error(const char* file, const int line, const char* msg)
{
	...
	fprintf(
		stderr, "ERROR in libusbemu -- source file '%s' at line %d -- %s\n",
		file, line, msg);
}
...
transfer_wrapper* libusbemu_get_transfer_wrapper(libusb_transfer* transfer)
{
	...
	char* off_address(
		raw_address - sizeof(void*) - 2 * sizeof(transfer_wrapper*) -
		sizeof(QuickList<transfer_wrapper>*));
	return ((transfer_wrapper*)off_address);
}

libusb_device* libusbemu_register_device(
	libusb_context* ctx, struct usb_device* dev)
{
	RAIIMutex lock(ctx->mutex);
	// register the device (if not already there) ...
	libusb_device dummy = {ctx, dev, 0, nullptr, nullptr};
	libusb_context::TMapDevices::iterator it =
		ctx->devices.insert(std::make_pair(dev, dummy)).first;
...
void libusbemu_unregister_device(libusb_device* dev)
{
	...
	if (0 == dev->refcount)
	{
		...
		{
			libusb_device::TMapIsocTransfers& allTransfers(
				*(dev->isoTransfers));
			while (!allTransfers.empty())
			{
				libusb_device::TMapIsocTransfers::iterator it(
					allTransfers.begin());
				libusb_device::TListTransfers& listTransfers(
					it->second.listTransfers);
...
int libusbemu_setup_transfer(transfer_wrapper* wrapper)
{
	...
	// paranoid check...
	if (nullptr != context) return (LIBUSB_ERROR_OTHER);
	...
	switch (transfer->type)
	{
		case LIBUSB_TRANSFER_TYPE_ISOCHRONOUS:
			ret = usb_isochronous_setup_async(
				handle, &context, transfer->endpoint,
				transfer->iso_packet_desc[0].length);
			break;
		case LIBUSB_TRANSFER_TYPE_CONTROL:
		// libusb-0.1 does not actually support asynchronous control transfers,
		// but
		// this should be
		// very easy to emulate if necessary: just stick the transfer in a
		// special
		// list and then
		// libusb_handle_events() check if the list is empty or not; if it is
		// not
		// empty, a thread
		// is created temporarily just to deal with such control transfer
		// requests
		// until the list
		// becomes eventually empty again and the thread terminates.
		case LIBUSB_TRANSFER_TYPE_BULK:
		case LIBUSB_TRANSFER_TYPE_INTERRUPT:
		// these transfer types are not being used by libfreenect. they should
		// be
		// fairly simple to
		// emulate with libusb-0.1 since it already provides support for them.
		// usb_bulk_setup_async(translate(transfer->dev_handle), &context,
		// transfer->endpoint);
		// usb_interrupt_setup_async(translate(transfer->dev_handle), &context,
		// transfer->endpoint);
		default:
