...
private:
  ...
  template <typename F> struct ThreadWrapper {
    struct State {
      F *routine;
      void *params;
      bool release;
...
public:
  ...
  inline QuickThread(F *proc, void *params, const bool auto_release = false)
    ...
    hThread = CreateThread(nullptr, 0, &ThreadWrapper<F>::Thunk, (LPVOID)&state,
                           0, nullptr);
...
struct EventList {
  ...
  int WaitAnyUntilTimeout(const unsigned int milliseconds) {
    ...
    if (nHandles > 0)
      ret =
          WaitForMultipleObjects(nHandles, &m_vHandles[0], FALSE, milliseconds);
  ...
  const bool WaitAllUntilTimeout(const unsigned int milliseconds) {
    ...
    const unsigned int nHandles(m_vHandles.size());
    if (nHandles > 0)
      waited = (WAIT_TIMEOUT != WaitForMultipleObjects(nHandles, &m_vHandles[0],
                                                       FALSE, milliseconds));
    mutex.Leave();
    return (waited);
  }
