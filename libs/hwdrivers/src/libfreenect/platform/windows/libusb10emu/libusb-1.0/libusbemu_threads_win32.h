...
struct QuickEvent
{
   ...
   public:
	...
	inline ~QuickEvent() { CloseHandle(hEvent); }
	...
	{
		return (
			WAIT_OBJECT_0 == WaitForSingleObject(hEvent, (DWORD)milliseconds));
	}
	inline void Wait() { WaitUntilTimeout(INFINITE); }
};
...
struct QuickThread
{
   private:
	...
	// The wrapper is also capable of cleaning up itself upon thread
	// termination.
	...
	struct ThreadWrapper
	{
		...
		{
			State state =
				*((State*)lpParameter);  // clone state (no heap alloc!)
			...
			if (state.release) delete (state.instance);
   ...
   public:
	template <typename F>
	inline QuickThread(F* proc, void* params, const bool auto_release = false)
		: hThread(nullptr)
	{
		...
		// thunk function can signal back when it is done cloning the state;
		// this
		...
		hThread = CreateThread(
			nullptr, 0, &ThreadWrapper<F>::Thunk, (LPVOID)&state, 0, nullptr);
	...
	static inline QuickThread Myself() { return (QuickThread()); }
	...
	static inline void Sleep(int milliseconds) { ::Sleep(milliseconds); }
...
struct QuickMutex
{
   ...
   public:
	inline QuickMutex() { InitializeCriticalSection(&cs); }
	inline ~QuickMutex() { DeleteCriticalSection(&cs); }
	inline const bool TryEnter() { return (0 != TryEnterCriticalSection(&cs)); }
...
struct EventList
{
	...
	{
		mutex.Enter();
		std::vector<QuickEvent*>::iterator it1 =
			std::find(m_vEvents.begin(), m_vEvents.end(), poEvent);
		m_vEvents.erase(it1);
		std::vector<HANDLE>::iterator it2 =
			std::find(m_vHandles.begin(), m_vHandles.end(), poEvent->hEvent);
	...
	int WaitAnyUntilTimeout(const unsigned int milliseconds)
	{
		...
		if (nHandles > 0)
			ret = WaitForMultipleObjects(
				nHandles, &m_vHandles[0], FALSE, milliseconds);
		if (ret - WAIT_OBJECT_0 < nHandles) index = (int)(ret - WAIT_OBJECT_0);
		mutex.Leave();
		return (index);
	}

	int WaitAny() { return (WaitAnyUntilTimeout(INFINITE)); }
	int CheckAny() { return (WaitAnyUntilTimeout(0)); }
	...
	const bool WaitAll() { return (WaitAllUntilTimeout(INFINITE)); }
