...
int libusb_init(libusb_context **context) {
  usb_init();
  LIBUSB_DEBUG_CMD(const usb_version *version = usb_get_version();
                   fprintf(stdout, "libusb-win32: dll version %d.%d.%d.%d | "
                                   "driver (libusb0.sys) version %d.%d.%d.%d\n",
...
ssize_t libusb_get_device_list(libusb_context *ctx, libusb_device ***list) {
  ...
  // libusb_device** is better understood as libusb_device*[], which is an array
  // of
  ...
  devlist = new libusb_device
      *[ctx->devices.size() + 1]; // +1 is for a finalization mark
...
int libusb_get_string_descriptor(libusb_device_handle *dev_handle,
  ...
  RAIIMutex lock(dev_handle->dev->ctx->mutex);
  int bytes = usb_get_string(dev_handle->handle, (int)desc_index, (int)langid,
...
int libusb_get_string_descriptor_ascii(libusb_device_handle *dev_handle,
                                       ...
                                       int length) {
  RAIIMutex lock(dev_handle->dev->ctx->mutex);
  int bytes = usb_get_string_simple(dev_handle->handle, (int)desc_index,
                                    (char *)data, (size_t)length);
  if (bytes < 0) {
    LIBUSBEMU_ERROR_LIBUSBWIN32();
  }
...
int libusb_control_transfer(libusb_device_handle *dev_handle,
  ...
  int bytes_transferred =
      usb_control_msg(dev_handle->handle, bmRequestType, bRequest, wValue,
                      wIndex, (char *)data, wLength, timeout);
...
int libusb_bulk_transfer(libusb_device_handle *dev_handle, uint8_t endpoint,
  ...
  if (endpoint & LIBUSB_ENDPOINT_IN) { // Device to Host
    bytes_transferred = usb_bulk_read(dev_handle->handle, endpoint,
                                      (char *)data, length, timeout);
  } else { // Host to Device
    bytes_transferred = usb_bulk_write(dev_handle->handle, endpoint,
                                       (char *)data, length, timeout);
  }
  if (bytes_transferred < 0) {
...
struct libusb_transfer *libusb_alloc_transfer(int iso_packets) {
  ...
  memset(transfer.iso_packet_desc, 0,
...
void libusb_fill_iso_transfer(struct libusb_transfer *transfer,
                              ...
                              int length, int num_iso_packets,
...
int libusb_submit_transfer(struct libusb_transfer *transfer) {
  ...
  int ret = usb_submit_async(wrapper->usb, (char *)transfer->buffer,
                             transfer->length);
  if (ret < 0) {
...
int libusb_handle_events_timeout(libusb_context *ctx, struct timeval *timeout) {
  ...
  if (timeout == nullptr)
    ...
    libusbemu_handle_isochronous(ctx, (timeout->tv_sec * 1000) +
...
static void (*PreprocessTransfer)(libusb_transfer *,
                                  const int)(PreprocessTransferFreenect);
...
int ReapTransfer(transfer_wrapper *, unsigned int,
                 libusb_device::TListTransfers *);
...
int ReapThreadProc(void *params) {
  ...
  while (!listTransfers.Empty() || !listReadyLocal.Empty()) {
    ...
    // if there are no pending transfers, wait the ready ones to be delivered
      ...
      LIBUSB_DEBUG_CMD(fprintf(stdout, "ReapThreadProc(): no pending "
                                       "transfers, sleeping until "
                                       "delivery...\n"));
      if (!boDeliverRequested) {
        doneDelivering.Reset();
        wannaDeliver.Signal();
...
int ReapTransfer(transfer_wrapper *wrapper, unsigned int timeout,
