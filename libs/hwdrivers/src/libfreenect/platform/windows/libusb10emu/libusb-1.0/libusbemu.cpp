...
// until VC71 (MSVC2003) the human-readable map/dump mode flag is
// CRTDBG_MAP_ALLOC:
...
int libusb_init(libusb_context** context)
{
	usb_init();
	LIBUSB_DEBUG_CMD(
		const usb_version* version = usb_get_version(); fprintf(
			stdout,
			"libusb-win32: dll version %d.%d.%d.%d | driver (libusb0.sys) "
			"version %d.%d.%d.%d\n",
			version->dll.major, version->dll.minor, version->dll.micro,
			version->dll.nano, version->driver.major, version->driver.minor,
	...
	if (context == nullptr) context = &default_context;
...
void libusb_exit(libusb_context* ctx)
{
	...
	while (!ctx->devices.empty())
	{
		...
		{
			// a simple "while(!device.handles->empty())" loop is impossible
			// here
			...
			// count reaches zero), rendering "device.handles" to a corrupt
			// state.
			...
			{
				libusb_device::TMapHandles::iterator itHandle =
					device.handles->begin();
...
ssize_t libusb_get_device_list(libusb_context* ctx, libusb_device*** list)
{
	...
	// libusb_device*** is the C equivalent to libusb_device**& in C++; such
	// declaration
	// allows the scope of this function libusb_get_device_list() to write on a
	// variable
	// of type libusb_device** declared within the function scope of the caller.
	// libusb_device** is better understood as libusb_device*[], which is an
	// array
	// of
	// pointers of type libusb_device*, each pointing to a struct that holds
	// actual data.
	usb_find_busses();
	usb_find_devices();
	...
	libusb_device* dev, struct libusb_device_descriptor* desc)
{
	...
	// plain copy of one descriptor on to another: this is a safe operation
	// because
	// usb_device_descriptor and libusb_device_descriptor have the same
	// signature
...
int libusb_open(libusb_device* dev, libusb_device_handle** handle)
{
	...
	if (nullptr == dev->handles) dev->handles = new libusb_device::TMapHandles;

	libusb_device_handle_t dummy = {dev, usb_handle};
	libusb_device::TMapHandles::iterator it =
		dev->handles->insert(std::make_pair(usb_handle, dummy)).first;
	*handle = &(it->second);
...
int libusb_get_string_descriptor(
	libusb_device_handle* dev_handle, uint8_t desc_index, uint16_t langid,
	unsigned char* data, int length)
{
	RAIIMutex lock(dev_handle->dev->ctx->mutex);
	int bytes = usb_get_string(
		dev_handle->handle, (int)desc_index, (int)langid, (char*)data,
		(size_t)length);
	if (bytes < 0)
	{
		LIBUSBEMU_ERROR_LIBUSBWIN32();
...
{
	RAIIMutex lock(dev_handle->dev->ctx->mutex);
	int bytes = usb_get_string_simple(
		dev_handle->handle, (int)desc_index, (char*)data, (size_t)length);
	if (bytes < 0)
	{
		LIBUSBEMU_ERROR_LIBUSBWIN32();
	}
...
int libusb_set_interface_alt_setting(
	libusb_device_handle* dev, int interface_number, int alternate_setting)
{
	RAIIMutex lock(dev->dev->ctx->mutex);
...
{
	RAIIMutex lock(dev->dev->ctx->mutex);
	// according to the official libusb-win32 usb_set_configuration()
	// documentation:
...
int libusb_control_transfer(
	libusb_device_handle* dev_handle, uint8_t bmRequestType, uint8_t bRequest,
	uint16_t wValue, uint16_t wIndex, unsigned char* data, uint16_t wLength,
	unsigned int timeout)
{
	// in libusb-1.0 a timeout of zero it means 'wait indefinitely'; in
	// libusb-0.1, a timeout of zero means 'return immediatelly'!
	timeout = (0 == timeout) ? 60000 : timeout;  // wait 60000ms (60s = 1min) if
	// the transfer is supposed to
	// wait indefinitely...
	int bytes_transferred = usb_control_msg(
...
int libusb_bulk_transfer(
	libusb_device_handle* dev_handle, uint8_t endpoint, uint8_t* data,
	int length, int* transferred, unsigned int timeout)
{
	// in libusb-1.0 a timeout of zero it means 'wait indefinitely'; in
	// libusb-0.1, a timeout of zero means 'return immediately'!
	timeout = (0 == timeout) ? 60000 : timeout;  // wait 60000ms (60s = 1min) if
	// the transfer is supposed to
	// wait indefinitely...
	...
	{  // Device to Host
		bytes_transferred = usb_bulk_read(
			dev_handle->handle, endpoint, (char*)data, length, timeout);
	}
	else
	{  // Host to Device
		bytes_transferred = usb_bulk_write(
			dev_handle->handle, endpoint, (char*)data, length, timeout);
	}
	if (bytes_transferred < 0)
	{
...
void libusb_free_transfer(struct libusb_transfer* transfer)
{
	...
	//  is non-nullptr, this function will also free the transfer buffer using
	//  the
...
int libusb_submit_transfer(struct libusb_transfer* transfer)
{
	...
	if (nullptr == wrapper->usb) libusbemu_setup_transfer(wrapper);

	libusbemu_clear_transfer(wrapper);

	int ret = usb_submit_async(
		wrapper->usb, (char*)transfer->buffer, transfer->length);
	if (ret < 0)
	{
	...
	libusb_device::TMapIsocTransfers& isoTransfers(
		*transfer->dev_handle->dev->isoTransfers);
	libusb_device::TMapIsocTransfers::iterator it =
		isoTransfers.find(transfer->endpoint);
	if (isoTransfers.end() == it)
	{
		libusb_device::isoc_handle dummy = {libusb_device::TListTransfers(),
											nullptr};
		it = isoTransfers.insert(std::make_pair(transfer->endpoint, dummy))
				 .first;
	}
...
int libusb_cancel_transfer(struct libusb_transfer* transfer)
{
	...
	if (ret != 0) LIBUSBEMU_ERROR_LIBUSBWIN32();
	// 0 on success
...
int libusbemu_handle_isochronous(
	libusb_context* ctx, const unsigned int milliseconds)
{
	if (ctx == nullptr) ctx = default_context;
	...
	if (-1 != index)
	{
		...
		while (-1 != (index = ctx->hWantToDeliverPool.CheckAny()))
		{
			...
			hDoneDeliveringPoolLocal.AttachEvent(
				ctx->hDoneDeliveringPool[index]);
		}
...
int libusb_handle_events(libusb_context* ctx)
{
	...
	if (failguard::Abort()) return (LIBUSB_ERROR_INTERRUPTED);
...
int libusb_handle_events_timeout(libusb_context* ctx, struct timeval* timeout)
{
	...
	if (timeout == nullptr)
		...
		libusbemu_handle_isochronous(
			ctx, (timeout->tv_sec * 1000) + (timeout->tv_usec / 1000));
...
static void (*PreprocessTransfer)(libusb_transfer*, const int)(
	PreprocessTransferFreenect);
...
int ReapTransfer(
	transfer_wrapper*, unsigned int, libusb_device::TListTransfers*);
...
int ReapThreadProc(void* params)
{
	...
	libusb_device::TMapIsocTransfers& isocTransfers =
		*(dev_handle->dev->isoTransfers);
	...
	// isochronous I/O must be handled in high-priority! (at least
	// TIME_CRITICAL)
	...
	while (!listTransfers.Empty() || !listReadyLocal.Empty())
	{
		...
		if (!listReadyLocal.Empty())
		{
			...
			// delivery request already signaled; wait for the delivery
			// permission
		...
		// if there are no pending transfers, wait the ready ones to be
		// delivered
		else
		{
			LIBUSB_DEBUG_CMD(
				fprintf(
					stdout,
					"ReapThreadProc(): no pending transfers, sleeping until "
					"delivery...\n"));
			if (!boDeliverRequested)
			{
				doneDelivering.Reset();
	...
	LIBUSB_DEBUG_CMD(if (boAbort) fprintf(stderr, "Thread loop aborted.\n"););
...
int ReapTransfer(
	transfer_wrapper* wrapper, unsigned int timeout,
	libusb_device::TListTransfers* lstReady)
{
	...
	if (read >= 0)
	{
		...
		// since the client will allegedly not read from this field, we'll be
		// using
		// it here just to simplify the emulation implementation, more
		// specifically
		...
		// two possibilities here: either deliver the transfer now or postpone
		// the
		// delivery to keep it in sync with libusb_handle_events(); in the
		// latter
...
#undef ETIMEOUT
		...
		switch (read)
		{
			...
			case ETIMEOUT:
				// when usb_reap_async_nocancel() returns ETIMEOUT, then either:
				// (a) the timeout indeed expired: in this case the transfer
				// should
				//     remain as the head of the transfer list (do not remove
				//     the node)
				//     and silently return without calling back the client (or
				//     perhaps
				//     set the transfer status to LIBUSB_TRANSFER_TIMED_OUT and
				//     then
				//     call back - MORE INVESTIGATION REQUIRED)
				// (b) the transfer was cancelled: in this case the transfer
				// should be
				//     removed from the list and reported back through the
				//     callback.
				...
				break;
			case EINVAL:
				// I guess -22 is returned if one attempts to reap a context
				// that does
				// not exist anymore (one that has already been deleted)
				LIBUSBEMU_ERROR_LIBUSBWIN32();
				break;
			case EIO:
				// Error code -5 seems to be triggered when the device is
				// lost...
				LIBUSBEMU_ERROR_LIBUSBWIN32();
				libusb_device::TListTransfers::RemoveNode(wrapper);
				...
				break;
			default:
				// I have not stumbled into any other negative values coming
				// from the
				// usb_reap_async_nocancel()... Anyway, cancel seems to be a
				// simple yet
...
void PreprocessTransferFreenect(libusb_transfer* transfer, const int read)
{
	...
	packet_stream* pktstrm =
		(transfer->endpoint == 0x81) ? &dev->video : &dev->depth;
	...
	LIBUSB_DEBUG_CMD(if (remaining > 0) {
		fprintf(stdout, "%d remaining out of %d\n", remaining, read);
		if (remaining == read) fprintf(stdout, "no bytes consumed!\n");
	});
}
