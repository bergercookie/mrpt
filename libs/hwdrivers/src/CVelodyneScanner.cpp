...
std::string CVelodyneScanner::TModelPropertiesFactory::getListKnownModels() {
  ...
  for (CVelodyneScanner::model_properties_list_t::const_iterator it =
    ...
    s += mrpt::format(
        "`%s`,",
        mrpt::utils::TEnumType<CVelodyneScanner::model_t>::value2name(it->first)
            .c_str());
  return s;
}

CVelodyneScanner::CVelodyneScanner()
    : m_model(CVelodyneScanner::VLP16), m_pos_packets_min_period(0.5),
      m_pos_packets_timing_timeout(30.0), m_device_ip(""), m_pcap_verbose(true),
      m_last_pos_packet_timestamp(INVALID_TIMESTAMP), m_pcap(nullptr),
      m_pcap_out(nullptr), m_pcap_dumper(nullptr), m_pcap_bpf_program(nullptr),
      m_pcap_file_empty(true), m_pcap_read_once(false), m_pcap_read_fast(false),
      m_pcap_read_full_scan_delay_ms(100), m_pcap_repeat_delay(0.0),
      m_hDataSock(INVALID_SOCKET), m_hPositionSock(INVALID_SOCKET),
      m_last_gps_rmc_age(INVALID_TIMESTAMP), m_lidar_rpm(0),
...
void CVelodyneScanner::loadConfig_sensorSpecific(
  ...
  MRPT_LOAD_HERE_CONFIG_VAR(pcap_read_full_scan_delay_ms, double,
                            m_pcap_read_full_scan_delay_ms, cfg, sect);
  MRPT_LOAD_HERE_CONFIG_VAR(pcap_repeat_delay, double, m_pcap_repeat_delay, cfg,
                            sect);
  MRPT_LOAD_HERE_CONFIG_VAR(pos_packets_timing_timeout, double,
                            m_pos_packets_timing_timeout, cfg, sect);
  MRPT_LOAD_HERE_CONFIG_VAR(pos_packets_min_period, double,
                            m_pos_packets_min_period, cfg, sect);
...
bool CVelodyneScanner::getNextObservation(
  ...
  try {
    ...
    bool rx_all_ok = this->receivePackets(data_pkt_timestamp, rx_pkt,
                                          pos_pkt_timestamp, rx_pos_pkt);
    ...
    if (data_pkt_timestamp != INVALID_TIMESTAMP) {
      ...
      if (!m_rx_scan) {
        m_rx_scan = std::make_shared<mrpt::obs::CObservationVelodyneScan>();
...

        {
          ...
          if (it != lstModels.end()) { // Model params:
      ...
      if (m_rx_scan->scan_packets.empty()) {
        ...
        if (m_last_gps_rmc.fields.validity_char == 'A' &&
            mrpt::system::timeDifference(m_last_gps_rmc_age,
                                         data_pkt_timestamp) <
          ...
          // take the date and time from the GPS, then modify minutes and
          // seconds from data pkt:
          ...
          tim_parts.minute =
              rx_pkt.gps_timestamp /*us from top of hour*/ / 60000000ul;
          tim_parts.second =
              (rx_pkt.gps_timestamp /*us from top of hour*/ % 60000000ul) *
              1e-6;
...
void CVelodyneScanner::initialize() {
  ...
  if (m_velodyne_calib.empty()) {
    ...
    m_velodyne_calib = VelodyneCalibration::LoadDefaultCalibration(
        mrpt::utils::TEnumType<CVelodyneScanner::model_t>::value2name(m_model));
    if (m_velodyne_calib.empty())
      THROW_EXCEPTION("Could not find default calibration data for the given "
                      "LIDAR `model` name. Please, specify a valid `model` or "
                      "load a valid XML configuration file first.");
  }

  // online vs off line operation??
  // -------------------------------
  if (m_pcap_input_file.empty()) { // Online
    ...
    if (INVALID_SOCKET == (m_hDataSock = socket(PF_INET, SOCK_DGRAM, 0)))
      THROW_EXCEPTION(
          format("Error creating UDP socket:\n%s",
    ...
    if (int(INVALID_SOCKET) ==
        ::bind(m_hDataSock, (struct sockaddr *)(&bindAddr), sizeof(sockaddr)))
    ...
    if (int(INVALID_SOCKET) == ::bind(m_hPositionSock,
    ...
    // Build PCAP filter:
    {
      ...
      static std::string sMsgError =
          "[CVelodyneScanner] Error calling pcap_compile: "; // This is to avoid
                                                             // the ill-formed
                                                             // signature of
                                                             // pcap_error()
                                                             // accepting
                                                             ...
                                                             // "const char*"...
                                                             // sigh
      if (pcap_compile(reinterpret_cast<pcap_t *>(m_pcap),
...
#else
    THROW_EXCEPTION("Velodyne: Reading from PCAP requires building MRPT with "
                    "libpcap support!");
#endif
  }
...
bool CVelodyneScanner::receivePackets(
  ...
  if (m_pcap) {
    ret =
        internal_read_PCAP_packet(data_pkt_timestamp, (uint8_t *)&out_data_pkt,
...
// Optional PCAP dump:
      ...
      !m_pcap_out) // 1st time: create output file
  {
    ...
    sFilePostfix +=
        format("%04u-%02u-%02u_%02uh%02um%02us", (unsigned int)parts.year,
               (unsigned int)parts.month, (unsigned int)parts.day,
               (unsigned int)parts.hour, (unsigned int)parts.minute,
               (unsigned int)parts.second);
    const string sFileName =
        m_pcap_output_file +
        mrpt::system::fileNameStripInvalidChars(sFilePostfix) + string(".pcap");

    m_pcap_out = pcap_open_dead(DLT_EN10MB, 65535);
    ASSERTMSG_(m_pcap_out != nullptr,
               "Error creating PCAP live capture handle");

    printf("\n[CVelodyneScanner] Writing to PCAP file \"%s\"\n",
           sFileName.c_str());
    if ((m_pcap_dumper = pcap_dump_open(reinterpret_cast<pcap_t *>(m_pcap_out),
                                        sFileName.c_str())) == nullptr) {
      THROW_EXCEPTION_FMT("Error creating PCAP live dumper: '%s'", errbuf);
    }
#else
    THROW_EXCEPTION("Velodyne: Writing PCAP files requires building MRPT with "
                    "libpcap support!");
#endif
  }

  if (m_pcap_out && m_pcap_dumper && (data_pkt_timestamp != INVALID_TIMESTAMP ||
    ...
    if (data_pkt_timestamp != INVALID_TIMESTAMP) {
      ...
      memcpy(&(packetBuffer[0]) + 42, (uint8_t *)&out_data_pkt,
             CObservationVelodyneScan::PACKET_SIZE);
    ...
    if (pos_pkt_timestamp != INVALID_TIMESTAMP) {
      ...
      memcpy(&(packetBuffer[0]) + 42, (uint8_t *)&out_pos_pkt,
             CObservationVelodyneScan::POS_PACKET_SIZE);
      pcap_dump((u_char *)this->m_pcap_dumper, &header, &(packetBuffer[0]));
    }
  }
...
// (done AFTER saving the pckg as is to pcap above)
  ...
  if (pos_pkt_timestamp != INVALID_TIMESTAMP) {
    if (m_last_pos_packet_timestamp != INVALID_TIMESTAMP &&
        mrpt::system::timeDifference(m_last_pos_packet_timestamp,
                                     pos_pkt_timestamp) <
...
mrpt::system::TTimeStamp CVelodyneScanner::internal_receive_UDP_packet(
  ...
  if (hSocket == INVALID_SOCKET)
    THROW_EXCEPTION("Error: UDP socket is not open yet! Have you called "
                    "initialize() first?");
  ...
  while (true) {
    ...
    // uses a non-interruptible std::this_thread::sleep_for(ms) when waiting for
    // data,
    ...
    int nbytes = recvfrom(hSocket, (char *)&out_buffer[0], expected_packet_size,
    ...
    std::cerr << "[CVelodyneScanner] Warning: incomplete Velodyne packet read: "
              << nbytes << " bytes\n";
  }
...
bool CVelodyneScanner::internal_read_PCAP_packet(
  ...
  while (true) {
    int res;
    if ((res = pcap_next_ex(reinterpret_cast<pcap_t *>(m_pcap), &header,
                            &pkt_data)) >= 0) {
      ++m_pcap_read_count;
      ...
      if (pcap_offline_filter(
              ...
              pkt_data) == 0) {
        if (m_verbose)
          std::cout << "[CVelodyneScanner] DEBUG: Filtering out packet #"
      ...
      if (udp_dst_port == CVelodyneScanner::VELODYNE_POSITION_UDP_PORT) {
        if (m_verbose)
          std::cout << "[CVelodyneScanner] DEBUG: Packet #" << m_pcap_read_count
                    << " in PCAP file is POSITION pkt.\n";
        memcpy(out_pos_buffer, pkt_data + 42,
               CObservationVelodyneScan::POS_PACKET_SIZE);
        ...
        memcpy(out_data_buffer, pkt_data + 42,
               CObservationVelodyneScan::PACKET_SIZE);
        data_pkt_time = tim; // success
        return true;
      } else {
        std::cerr << "[CVelodyneScanner] ERROR: Packet " << m_pcap_read_count
                  << " in PCAP file passed the filter but does not match "
                     "expected UDP port numbers! Skipping it.\n";
      }
    }

    if (m_pcap_file_empty) // no data in file?
    {
      fprintf(stderr, "[CVelodyneScanner] Maybe the PCAP file is empty? Error "
                      "%d reading Velodyne packet: `%s`\n",
    ...
    if (m_pcap_repeat_delay > 0.0) {
      if (m_pcap_verbose)
        printf("[CVelodyneScanner] INFO: end of file reached -- delaying %.3f "
               "seconds.\n",
               m_pcap_repeat_delay);
