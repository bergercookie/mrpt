...
void CBoardENoses::loadConfig_sensorSpecific(
  ...
  configSource.read_vector(iniSection, "enose_poses_x", vector<float>(0),
                           enose_poses_x, true);
  configSource.read_vector(iniSection, "enose_poses_y", vector<float>(0),
                           enose_poses_y, true);
  configSource.read_vector(iniSection, "enose_poses_z", vector<float>(0),
                           enose_poses_z, true);

  configSource.read_vector(iniSection, "enose_poses_yaw", vector<float>(0),
                           enose_poses_yaw, true);
  configSource.read_vector(iniSection, "enose_poses_pitch", vector<float>(0),
                           enose_poses_pitch, true);
  configSource.read_vector(iniSection, "enose_poses_roll", vector<float>(0),
                           enose_poses_roll, true);
...
bool CBoardENoses::getObservation(mrpt::obs::CObservationGasSensors &obs) {
  try {
    ...
    if (msg.content.size() > 0) {
      ...
      memcpy(&readings[0], &msg.content[0],
      ...
      for (size_t i = 0; i < NumberOfChambers; i++) {
        ...
        if (i < enose_poses_x.size()) {
          newRead.eNosePoseOnTheRobot = CPose3D(
              ...
              enose_poses_yaw[i], enose_poses_pitch[i], enose_poses_roll[i]);
        } else
        ...
        for (size_t idx = 0; idx < wordsPereNose / 2; idx++) {
              ...
              0x0000) // not empty slot
          {
            ...
            if (readings[i * wordsPereNose + 2 * idx + 2] == 0xFFFF) {
              ...
              newRead.temperature =
                  ((int16_t)readings[i * wordsPereNose + 2 * idx + 3]) / 32.0f;
            } else // Is a gas sensors
            {
              ...
              newRead.readingsVoltage.push_back(
                  (readings[i * wordsPereNose + 2 * idx + 3] * 5.0f) / 4096.0f);
            }
          }
      ...
      // Set Timestamp
      uint32_t *p = (uint32_t *)&readings[readings.size() - 2]; // Get readings
                                                                // time from
                                                                // frame (always
                                                                // last 2 words)
    ...
    cerr << "[CBoardENoses::getObservation] Returning false due to exception: "
         << endl;
    cerr << e.what() << endl;
    return false;
  } catch (...) {
