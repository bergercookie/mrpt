...
CNTRIPClient::CNTRIPClient()
    : m_thread(), m_thread_exit(false), m_thread_do_process(false),
      m_waiting_answer_connection(false), m_answer_connection(connError),
      m_args() {
...
bool CNTRIPClient::open(const NTRIPArgs &params, string &out_errmsg) {
  ...
  case connError:
    out_errmsg =
        format("Error trying to connect to server '%s'", params.server.c_str());
    return false;
  case connUnauthorized:
    out_errmsg =
        format("Authentication failed for server '%s'", params.server.c_str());
    return false;
  ...
  try {
    ...
    while (!m_thread_exit) {
      ...
      if (!my_sock.isConnected()) {
        ...
        // We are not disconnected yet, it's a good thing... anyway, check the
        // answer code:
        if (!buf.empty()) {
          string resp;
