...
void CImpinjRFID::loadConfig_sensorSpecific(
  ...
  /*		pose_x_1 =
     configSource.read_float(iniSection,"pose_x_1",0,true);
                  pose_y_1 =
     configSource.read_float(iniSection,"pose_y_1",0,true);
                  pose_z_1 =
     configSource.read_float(iniSection,"pose_z_1",0,true);
                  pose_roll_1 =
     ...
     configSource.read_float(iniSection,"pose_yaw_1",0,true);
                  ...
                  pose_x_2 =
     configSource.read_float(iniSection,"pose_x_2",0,true);
                  pose_y_2 =
     configSource.read_float(iniSection,"pose_y_2",0,true);
                  pose_z_2 =
     configSource.read_float(iniSection,"pose_z_2",0,true);
                  pose_roll_2 =
...
bool CImpinjRFID::getObservation(mrpt::obs::CObservationRFID &obs) {
  try {
    ...
    while (client->readAsync(msg, 34, 100) > 0) {
      ...
      if (!ant_port_ptr) {
        std::cerr << "[CImpinjRFID::getObservation] Unexpected format in "
                     "sensor data! (skipping).\n";
        continue;
      }
      ...
      obs.tag_readings.resize(obs.tag_readings.size() +
                              1); // Alloc space for one more tag obs
      mrpt::obs::CObservationRFID::TTagReading &new_tag =
          *obs.tag_readings
               .rbegin(); // Get a reference to the latest new tag structure
...
void CImpinjRFID::doProcess() {
  ...
  mrpt::obs::CObservationRFID::Ptr obs =
      std::make_shared<mrpt::obs::CObservationRFID>();
