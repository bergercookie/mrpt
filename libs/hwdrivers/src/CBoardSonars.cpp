...
void CBoardSonars::loadConfig_sensorSpecific(
  ...
  // Some parameters ...
  m_usbSerialNumber = configSource.read_string(iniSection, "USB_serialNumber",
                                               m_usbSerialNumber, true);
  ...
  // Sonar firing order ...
  configSource.read_vector(iniSection, "firingOrder", m_firingOrder,
                           m_firingOrder, true);
  ...
  for (itSonar = m_firingOrder.begin(); itSonar != m_firingOrder.end();
       ++itSonar) {
    configSource.read_vector(iniSection, format("pose%i", *itSonar), aux, aux,
                             true); // Get the sonar poses
    m_sonarPoses[*itSonar] =
        mrpt::math::TPose3D(aux[0], aux[1], aux[2], DEG2RAD((float)aux[3]),
    ...
    if (receiveMessage(msgRx)) {
      ...
      for (size_t i = 0; i < data.size() / 2; i++) {
        ...
        if (sonar_range_cm != 0xFFFF && sonar_idx < 16) {
          ...
          obsRange.sensorPose =
              m_sonarPoses[sonar_idx]; // mrpt::poses::CPose3D(); // sonar_idx
