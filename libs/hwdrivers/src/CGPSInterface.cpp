...
CGPSInterface::CGPSInterface()
    : mrpt::utils::COutputLogger("CGPSInterface"),
      ...
      m_data_stream_cs(nullptr), m_data_stream_is_external(false),
      m_customInit(), m_rx_buffer(0x10000), m_parser(CGPSInterface::AUTO),
      m_raw_dump_file_prefix(), m_COMname(), m_COMbauds(4800),
      m_sensorLabelAppendMsgType(true), m_GPS_comsWork(false),
      m_last_timestamp(INVALID_TIMESTAMP), m_custom_cmds_delay(0.1),
      m_custom_cmds_append_CRLF(true),

      m_JAVAD_rtk_src_port(), m_JAVAD_rtk_src_baud(0),
      m_JAVAD_rtk_format("cmr"), m_topcon_useAIMMode(false),
      m_topcon_AIMConfigured(false), m_topcon_data_period(0.2) // 20 Hz
{
  m_sensorLabel = "GPS";
}
...
void CGPSInterface::loadConfig_sensorSpecific(
  ...
  // new custom cmds:
  m_custom_cmds_delay = configSource.read_float(iniSection, "custom_cmds_delay",
                                                m_custom_cmds_delay);
  ...
  m_JAVAD_rtk_src_baud = configSource.read_int(iniSection, "JAVAD_rtk_src_baud",
                                               m_JAVAD_rtk_src_baud);
  m_JAVAD_rtk_format = configSource.read_string(iniSection, "JAVAD_rtk_format",
                                                m_JAVAD_rtk_format);

  m_topcon_useAIMMode = configSource.read_bool(iniSection, "JAVAD_useAIMMode",
                                               m_topcon_useAIMMode);
  m_topcon_data_period =
      1.0 /
      configSource.read_double(iniSection, "outputRate", m_topcon_data_period);
}
...
void CGPSInterface::bindStream(mrpt::utils::CStream *external_stream,
                               std::mutex *csOptionalExternalStream) {
  if (!m_data_stream_is_external) {
    delete m_data_stream;
    m_data_stream = nullptr;
  }
...
void CGPSInterface::setSerialPortName(const std::string &COM_port) {
  ...
  if (m_data_stream_is_external)
    THROW_EXCEPTION("Cannot change serial port name: an external stream has "
                    "been already bound manually.")
...
/* -----------------------------------------------------
                                getSerialPortName
----------------------------------------------------- */
std::string CGPSInterface::getSerialPortName() const { return m_COMname; }
...
bool CGPSInterface::tryToOpenTheCOM() {
  ...
  if (serial) {
    ...
    if (m_verbose)
      cout << "[CGPSInterface] Opening " << m_COMname << " @ " << m_COMbauds
           << endl;
...
/* -----------------------------------------------------
                                isGPS_connected
----------------------------------------------------- */
bool CGPSInterface::isGPS_connected() { return m_GPS_comsWork; }
...
void CGPSInterface::doProcess() {
  ...
  try {
    ...
    if (!m_raw_dump_file_prefix.empty() &&
        !m_raw_output_file.fileOpenCorrectly()) {
      // 1st time open:
      ...
      if (m_verbose)
        std::cout << "[CGPSInterface] Creating RAW dump file: `" << sFileName
                  << "`\n";
    ...
    // ERROR:
    MRPT_LOG_ERROR("[CGPSInterface::doProcess] Error reading stream of data: "
  ...
  if (!m_customInit.empty()) { // "Advanced" (RTK,mmGPS) device  (kept for
    ...
    if (m_last_timestamp == INVALID_TIMESTAMP) {
      if (m_verbose)
        cout << "[CGPSInterface] Initial timestamp: "
             << mrpt::system::timeToString(m_just_parsed_messages.timestamp)
             << endl;
      // Check if the initial timestamp seems to be OK (not a spurio one)
      TTimeStamp tmNow = mrpt::system::now();
      const double tdif =
          mrpt::system::timeDifference(m_just_parsed_messages.timestamp, tmNow);
      if (tdif >= 0 && tdif < 7500 /*Up to two hours*/)
        m_last_timestamp = m_just_parsed_messages.timestamp;
      else {
        if (m_verbose)
          cout << "[CGPSInterface] Warning: The initial timestamp seems to be "
                  "wrong! : "
               << tdif << endl;
      }
    } // end-if
      ...
      const double time_diff = mrpt::system::timeDifference(
          m_last_timestamp, m_just_parsed_messages.timestamp);
      if (time_diff < 0 || time_diff > 300) // Assert that the current timestamp
                                            // is after the previous one and not
                                            // more than 5 minutes later ->
                                            // remove spurious
      {
        if (m_verbose)
          cout << "[CGPSInterface ] Bad timestamp difference" << endl;
        return;
      }

      if (time_diff - m_topcon_data_period > 0.25 * m_topcon_data_period) {
        if (m_verbose)
          cout << "[CGPSInterface] WARNING: According to the timestamps, we "
                  "probably skipped one frame!"
               << endl;
      }

      // a. These GPS data have both synched RMC and GGA data
      // don't append observation until we have both data
      do_append_obs = (m_just_parsed_messages.has_GGA_datum &&
...
void CGPSInterface::parseBuffer() {
  if (m_parser == CGPSInterface::NONE)
    return; // Dont try to parse data

  // Only one parser selected?
  ptr_parser_t parser_ptr = nullptr;
  switch (m_parser) {
  case CGPSInterface::NMEA:
    parser_ptr = &CGPSInterface::implement_parser_NMEA;
    break;
  ...
  if (parser_ptr) {
    ...
    // AUTO mode --------
    const std::list<CGPSInterface::ptr_parser_t> &all_parsers =
        TParsersRegistry::getInstance().all_parsers;
...
void CGPSInterface::JAVAD_sendMessage(const char *str, bool waitForAnswer) {
  ...
  if (written != len)
    throw std::runtime_error(
        format("Error sending command: '%s'", str).c_str());
  ...
  while (bad_counter < 10) {
    ...
    if (nRead < 3)
      throw std::runtime_error(
          format("ERROR: Invalid response '%s' for command '%s'", buf, str));
...
bool CGPSInterface::OnConnectionEstablished() {
  ...
  for (size_t i = 0; i < m_setup_cmds.size(); i++) {
    ...
    try {
      ...
      std::cerr << "[CGPSInterface::OnConnectionEstablished] Error sending "
                   "setup cmds: "
                << e.what() << std::endl;
      return false;
    }
...
bool CGPSInterface::setJAVAD_AIM_mode() {
  ...
  if (!os::_strcmpi(m_customInit.c_str(), "JAVAD") ||
    ...
    if (m_JAVAD_rtk_format == "cmr") {
      ...
      JAVAD_sendMessage(
          format("%%%%set,/par/cur/term/jps/1,{rtcm3,-1,y,%s}\r\n",
                 m_JAVAD_rtk_src_port.c_str())
      ...
      cout << "Unknown RTK corrections format. Only supported: CMR, RTCM or "
              "RTCM3"
           << endl;
      return false;
    }
...
bool CGPSInterface::legacy_topcon_setup_commands() {
  ...
  if (m_topcon_useAIMMode) {
    if (m_verbose)
      cout << "[CGPSInterface] Using Advanced Input Mode";
  ...
  if (m_topcon_useAIMMode) {
    if (m_verbose)
      cout << "[CGPSInterface::OnConnectionEstablished] JAVAD/TopCon commands "
              "sent successfully with AIM."
           << endl;
  } else {
    if (m_verbose)
      cout << "[CGPSInterface::OnConnectionEstablished] JAVAD/TopCon commands "
