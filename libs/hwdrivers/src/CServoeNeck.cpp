...
// const double MAX_VALUE = 10000;					// ICR value in the
// ATMEGA16
...
CServoeNeck::CServoeNeck()
    : m_usbSerialNumber("eNeck001"), m_MaxValue(10000), m_TruncateFactor(0.5),
      m_PrevAngles(0), m_NumPrevAngles(5) {
...
unsigned int CServoeNeck::angle2RegValue(const double angle /* rad */) {
  ...
  // return ( (uint16_t)( (m_MaxValue/20)*(2*angle/M_PI+1.5) ) );			//
  // equation: v = s*(a-a0) + v0 where s = 450/pi; a0 = 0 and v0 = 750
  // return ( (uint16_t)( (900/M_PI)*nangle + 750 ) );			// equation: v =
...
double CServoeNeck::regValue2angle(const uint16_t value) {
  ...
  // return ( -M_PI*0.5*(20*value/m_MaxValue-1.5) );					// equation:
  // angle = (pi/2)*(20*OCR/ICR-1.5)
  // return ( -M_PI*0.25+(M_PI/1800)*(value-1050) );					// equation:
  // angle = (pi/2)*(20*OCR/ICR-1.5)
  // return ( M_PI*0.5*(20*value/m_MaxValue-1.5) );					// equation:
  // angle = (pi/2)*(20*OCR/ICR-1.5)
...
/*-------------------------------------------------------------
                                        setRegisterValue
-------------------------------------------------------------*/
bool CServoeNeck::setRegisterValue(const uint16_t value, const uint8_t servo,
                                   bool fast) {
  try {
    if (!isOpen())
      return false;
...
/*-------------------------------------------------------------
                                        setAngleAndSpeed
-------------------------------------------------------------*/
bool CServoeNeck::setAngleAndSpeed(double angle, const uint8_t servo,
                                   const uint8_t speed) {
...
/*-------------------------------------------------------------
                                        setAngleWithFilter
-------------------------------------------------------------*/
bool CServoeNeck::setAngleWithFilter(double angle, const uint8_t servo,
                                     bool fast) {
  double nangle = 0;
