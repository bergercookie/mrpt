...
COpenNI2Sensor::COpenNI2Sensor()
    : m_sensorPoseOnRobot(), m_preview_window(false),
      ...
      m_relativePoseIntensityWRTDepth(0, 0, 0, DEG2RAD(-90), DEG2RAD(0),
                                      DEG2RAD(-90)),
      m_user_device_number(0), m_serial_number(0) {
...
#if MRPT_HAS_OPENNI2
  try {
    if (getConnectedDevices() <= 0) { // Check and list the available devices.
                                      // If there is at least one device
                                      // connected, open the first in the list.
      return;
    }
    {
      if (m_serial_number != 0) {
        openDeviceBySerial(m_serial_number);
        if (getDeviceIDFromSerialNum(m_serial_number, m_user_device_number) ==
            false) {
          THROW_EXCEPTION(
              mrpt::format("Failed to find sensor_id from serial number(%d).",
                           m_serial_number))
        }
      } else
    ...
    if (isOpen(m_user_device_number) == false) {
      THROW_EXCEPTION(mrpt::format("Failed to open OpenNI2 device(%d).",
                                   m_user_device_number))
    }
    /* If camera parameter is not read from ini file, we get the parameters from
     * OpenNI2. */
    if (isValidParameter(m_cameraParamsDepth) == false) {
      if (getDepthSensorParam(m_cameraParamsDepth, m_user_device_number) ==
          false) {
