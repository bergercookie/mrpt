...
bool CRovio::getRovioState(CRovio::TRovioState& status)
{
	...
	general_command(
		1, response, errormsg);  // Get report from Rovio to response
	...
	if (error.empty())
	{
		...
		for (int i = 0; i <= 11; i++)
		{
			...
			if ((i == 2) || (i == 4) ||
				...
				l_value =
					((l_value >> 1) &
					 0x1);  // This extracts the last but one bit
			// of l_value which sustitutes
			// l_value
			a_enc[i] = l_value;
		}
		...
		if (a_enc[2])
			this->encoders.left +=
				a_enc[3];  // Esta esto bien asi? o deberia usar
