                          ...
                          // string password_out)
{
  ...
  http_get(format("http://%s/rev.cgi?Cmd=nav&action=1", options.IP.c_str()),
...
bool CRovio::path_management(int act, const string &path_name) {
  string response, errormsg;
  string command;
  command = format("http://%s/rev.cgi?Cmd=nav&action=%i&name=%s",
                   options.IP.c_str(), act, path_name.c_str());
  http_get(command, response, errormsg, 80, options.user, options.password);
  return errormsg.empty();
}
...
bool CRovio::pathRename(const string &current_name, const string &new_name) {
  string response, errormsg;
  string command =
      format("http://%s/rev.cgi?Cmd=nav&action=11&name=%s&newname=%s",
                            ...
                            // getLastImage() ask for it, and so on.
{
  try {
    ...
    const bool open_ok =
        in_video.openURL(video_url, false /*grayscale*/, false /* verbose */);
    ...
    while (!m_videothread_must_exit) {
      ...
      if (in_video.retrieveFrame(obs->image)) {
        ...
        cout << "[CRovio::thread_video] Warning: the program doesn't receive "
                "any image\n";
      }
...
bool CRovio::getEncoders(CRovio::TEncoders &encoders) // Revisar esto
{
  ...
  // Wheel:Ticks","Rear Wheel:Dir Rotation","Rear Wheel:Ticks","Not used","Head
  // Position","Batery","Config Status"};
  size_t length;
  ...
  if (error.empty()) {
    ...
    for (int i = 0; i <= 11; i++) {
      ...
      if ((i == 2) || (i == 4) ||
        ...
        l_value = ((l_value >> 1) & 0x1); // This extracts the last but one bit
                                          // of l_value which sustitutes l_value
      a_enc[i] = l_value;
    }
    // Upload the encoders value
    if (a_enc[2])
      this->encoders.left += a_enc[3]; // Esta esto bien asi? o deberia usar
                                       // encoders como parametro de entrada
    else
