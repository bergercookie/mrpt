...
void CRoboPeakLidar::doProcessSimple(
  ...
  if (op_result == RESULT_OK) {
    ...
    if (op_result == RESULT_OK) {
      ...
      outObservation.resizeScanAndAssign(angle_compensate_nodes_count, 0,
                                         false);
...
bool CRoboPeakLidar::getDeviceHealth() const {
  ...
  if (IS_OK(op_result)) {
    ...
    if (healthinfo.status != RPLIDAR_STATUS_OK) {
      fprintf(stderr, "[CRoboPeakLidar] Error, rplidar internal error "
...
bool CRoboPeakLidar::checkCOMMs() {
  ...
  // make connection...
  if (IS_FAIL(RPLIDAR_DRV->connect(m_com_port.c_str(),
                                   (_u32)m_com_port_baudrate))) {
    fprintf(
        stderr,
  ...
  if (m_verbose) {
    printf("[CRoboPeakLidar] Connection established:\n"
           ...
           (unsigned int)devinfo.hardware_version, (unsigned int)devinfo.model);
