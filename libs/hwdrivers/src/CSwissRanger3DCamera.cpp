...
CSwissRanger3DCamera::CSwissRanger3DCamera()
    : m_sensorPoseOnRobot(), m_save_3d(true), m_save_range_img(true),
      m_save_intensity_img(true), m_save_confidence(false),

      m_enable_img_hist_equal(false), m_enable_median_filter(true),
      m_enable_mediancross_filter(false), m_enable_conv_gray(false),
      m_enable_denoise_anf(true),

      m_open_from_usb(true), m_usb_serial(0), m_ip_address("192.168.2.14"),
      m_rows(0), m_cols(0), m_cam_serial_num(0), m_maxRange(5),
...
#if !MRPT_HAS_SWISSRANGE
  THROW_EXCEPTION("MRPT was compiled without support for SwissRanger 3D "
                  "cameras! Rebuild it.")
#endif
}
...
void CSwissRanger3DCamera::loadConfig_sensorSpecific(
  ...
  m_save_confidence =
      configSource.read_bool(iniSection, "save_confidence", m_save_confidence);
  ...
  m_enable_conv_gray = configSource.read_bool(iniSection, "enable_conv_gray",
                                              m_enable_conv_gray);
      ...
      if (this->m_save_intensity_img) {
        ...
        if (!m_path_for_external_images.empty()) {
          const string filName =
              ...
              format("_INT_%f.%s", (double)timestampTotime_t(obs.timestamp),
                     m_external_images_format.c_str());
          obs.intensityImage.saveToFile(m_path_for_external_images +
                                            string("/") + filName,
                                        m_external_images_jpeg_quality);
    ...
    case ImgEntry::IT_CONF_MAP: {
      if (this->m_save_confidence) {
        ...
        if (!m_path_for_external_images.empty()) {
          const string filName =
              fileNameStripInvalidChars(trim(m_sensorLabel)) +
              format("_CONF_%f.%s", (double)timestampTotime_t(obs.timestamp),
                     m_external_images_format.c_str());
          obs.confidenceImage.saveToFile(m_path_for_external_images +
                                             string("/") + filName,
                                         m_external_images_jpeg_quality);
