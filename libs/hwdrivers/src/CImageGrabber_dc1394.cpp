	  ...
	  m_options(options)
{
	...
	else
	{
		...
		{
			if (list->ids[i].guid == cameraGUID &&
				list->ids[i].unit == cameraUnit)
			{
				m_dc1394camera =
				...
				{
					dc1394_camera_free_list(list);
					cerr << "[CImageGrabber_dc1394] ERROR: Failed to "
							"initialize camera "
							"with GUID "
						 << list->ids[0].guid << "\n";
					return;
		...
		{
			dc1394_camera_free_list(list);
			cerr << "[CImageGrabber_dc1394] ERROR: Camera with GUID="
				 << cameraGUID << " and UNIT=" << cameraUnit << " not found.\n";
			return;
		}
	}
	dc1394_camera_free_list(list);
	...
	if (options.mode7 >= 0)
	{
		...
		if (verbose)
			cout << "[CImageGrabber_dc1394] Mode is mode7: " << options.mode7
				 << endl;
	}
	else
	{
#define TEST_MODE(W, H, COLORMODEL)                                         \
	...
	// This checking only assures that m_desired_mode is inside
	// dc1394video_mode_t
	// enum range
	if (err != DC1394_SUCCESS)
	{
...
#undef SET_TRIGGER
	...
	if (verbose)
	{
		...
		if ((err = dc1394_feature_get_all(THE_CAMERA, &features)) ==
			DC1394_SUCCESS)
...
#else
	THROW_EXCEPTION(
		"[CImageGrabber_dc1394] ERROR: MRPT compiled with "
	...
	mrpt::obs::CObservationImage& out_observation)
{
	...
	else
	{
		...
		{
			cerr
				<< "[CImageGrabber_dc1394] ERROR: Could not deinterlace stereo "
				   "images: "
				<< err << endl;
			return false;
		}
	...
	mrpt::obs::CObservationStereoImages& out_observation)
{
	...
	{
		THROW_EXCEPTION(
			"Call to getObservation(stereo) but the camera was not set as "
			"stereo!");
	}
	else
	{
...
#define SET_MODE(feat, FEAT)                                                   \
	...
	{                                                                          \
		err = dc1394_feature_set_mode(                                         \
			...
			static_cast<dc1394feature_mode_t>(                                 \
				DC1394_FEATURE_MODE_MIN + options.feat##_mode));               \
		DC1394_WRN(                                                            \
			err,                                                               \
			"[CImageGrabber_dc1394::changeCaptureOptions] Could not set feat " \
			"mode");                                                           \
	}
...
#define SET_VALUE(feat, FEAT)                                                  \
	...
	{                                                                          \
		err = dc1394_feature_set_value(                                        \
			THE_CAMERA, DC1394_FEATURE_##FEAT, options.feat);                  \
		DC1394_WRN(                                                            \
			err,                                                               \
			"[CImageGrabber_dc1394::changeCaptureOptions] Could not set feat " \
			"value");                                                          \
