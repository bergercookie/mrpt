...
CImageGrabber_dc1394::CImageGrabber_dc1394(
    ...
    : m_bInitialized(false), m_dc1394_lib_context(nullptr),
      m_dc1394camera(nullptr), m_options(options) {
  MRPT_START
  ...
  if (err != DC1394_SUCCESS) {
    cerr << "[CImageGrabber_dc1394] ERROR: Failed to enumerate cameras (Maybe "
            "your user has no rights to access IEEE1394?)."
         << endl;
    return;
  }
  ...
  if (!cameraGUID) {
    ...
    if (!THE_CAMERA) {
      ...
      cerr << "[CImageGrabber_dc1394] ERROR: Failed to initialize camera with "
              "GUID "
           << list->ids[0].guid << "\n";
      return;
    }
    ...
    for (uint32_t i = 0; i < list->num; i++) {
      if (list->ids[i].guid == cameraGUID && list->ids[i].unit == cameraUnit) {
        m_dc1394camera =
            dc1394_camera_new_unit(THE_CONTEXT, cameraGUID, cameraUnit);
        if (!THE_CAMERA) {
          dc1394_camera_free_list(list);
          cerr << "[CImageGrabber_dc1394] ERROR: Failed to initialize camera "
                  "with GUID "
               << list->ids[0].guid << "\n";
          return;
        }
    ...
    if (!m_dc1394camera) {
      dc1394_camera_free_list(list);
      cerr << "[CImageGrabber_dc1394] ERROR: Camera with GUID=" << cameraGUID
           << " and UNIT=" << cameraUnit << " not found.\n";
      return;
    }
  }
  dc1394_camera_free_list(list);
...
#define TEST_MODE(W, H, COLORMODEL)                                            \
  else if (options.frame_width == W && options.frame_height == H &&            \
  ...
  for (uint32_t i = 0; i < modes.num; i++) {
    ...
    case DC1394_VIDEO_MODE_1600x1200_YUV422:
      mode = "1600x1200_YUV422";
      break;
    case DC1394_VIDEO_MODE_1600x1200_RGB8:
      mode = "1600x1200_RGB8";
      break;
    case DC1394_VIDEO_MODE_1600x1200_MONO8:
      mode = "1600x1200_MONO8";
      break;
    case DC1394_VIDEO_MODE_1280x960_MONO16:
      mode = "1280x960_MONO16";
      break;
    ...
    case DC1394_VIDEO_MODE_FORMAT7_7:
      ...
      break;
    default:
      cerr << "[CImageGrabber_dc1394] ERROR: Requested video mode is not valid."
           << endl;
      return;
    }
    if (modes.modes[i] == m_desired_mode)
      valid_video_mode = true;
    if (verbose) {
      if (modes.modes[i] == m_desired_mode)
        cout << mode << " (*)" << endl;
      else
  ...
  // This checking only assures that m_desired_mode is inside dc1394video_mode_t
  // enum range
  if (err != DC1394_SUCCESS) {
  ...
  default:
    cerr << "[CImageGrabber_dc1394] ERROR: Requested framerate is not valid."
         << endl;
    return;
  }
  ...
  err = dc1394_capture_setup(THE_CAMERA, SIZE_RING_BUFFER,
  ...
  uint32_t iso_chan;
  if ((err = dc1394_video_get_iso_channel(THE_CAMERA, &iso_chan)) ==
      DC1394_SUCCESS)
    if (verbose)
      cout << "ISO Channel: " << iso_chan << endl;

  dc1394speed_t iso_speed;
  if ((err = dc1394_video_get_iso_speed(THE_CAMERA, &iso_speed)) ==
      DC1394_SUCCESS)
    if (verbose)
...
#define SET_TRIGGER(opt, OPT, TYPE)                                            \
  if (options.trigger_##opt >= 0) {                                            \
    err = dc1394_external_trigger_set_##opt(                                   \
        THE_CAMERA, static_cast<dc1394trigger_##opt##_t>(                      \
                        DC1394_TRIGGER_##TYPE##_MIN + options.trigger_##opt)); \
    DC1394_WRN(err, "[CImageGrabber_dc1394::changeCaptureOptions] Could not "  \
                    "set trigger opt");                                        \
  }
  ...
  if (options.trigger_power >= 0) {
    err = dc1394_external_trigger_set_power(
        THE_CAMERA, dc1394switch_t(options.trigger_power));
    DC1394_WRN(err, "[CImageGrabber_dc1394::changeCaptureOptions] Could not "
                    "set trigger power");
  }
#undef SET_TRIGGER
...
#else
  THROW_EXCEPTION("[CImageGrabber_dc1394] ERROR: MRPT compiled with "
                  "MRPT_HAS_LIBDC1394_2=0 !");
#endif
  MRPT_END
}
...
CImageGrabber_dc1394::~CImageGrabber_dc1394() {
  ...
  if (THE_CAMERA) {
    ...
    // Release BW:
    //		uint32_t val;
    //		if (dc1394_video_get_bandwidth_usage(THE_CAMERA, &val) ==
    //DC1394_SUCCESS)
    //			 dc1394_iso_release_bandwidth(THE_CAMERA, val);
    //		if ( dc1394_video_get_iso_channel(THE_CAMERA, &val) ==
    //DC1394_SUCCESS)
...
/*-------------------------------------------------------------
                                        get the image - MONO
 -------------------------------------------------------------*/
bool CImageGrabber_dc1394::getObservation(
  ...
  // dc1394error_t err=dc1394_capture_dequeue(THE_CAMERA,
  ...
  if (!m_options.deinterlace_stereo) {
     ...
     *-----------------------------------------------------------------------*/
    // dc1394_get_image_size_from_video_mode(THE_CAMERA, m_desired_mode, &width,
    // &height);
    ...
    if ((err = dc1394_deinterlace_stereo(frame->image, imageBuf, width,
                                         2 * height)) != DC1394_SUCCESS) {
      cerr << "[CImageGrabber_dc1394] ERROR: Could not deinterlace stereo "
              "images: "
           << err << endl;
      return false;
    }
    ...
    if ((err = dc1394_bayer_decoding_8bit(
      ...
      cerr << "[CImageGrabber_dc1394] ERROR: Could not apply Bayer conversion: "
           << err << endl;
      return false;
    }
  ...
  if (err != DC1394_SUCCESS) {
    cerr << "[CImageGrabber_dc1394] ERROR: Could not enqueue the ring buffer "
            "frame"
         << endl;
    return false;
  }
...
bool CImageGrabber_dc1394::getObservation(
  ...
  // get frame from ring buffer:
  dc1394error_t err =
      dc1394_capture_dequeue(THE_CAMERA, DC1394_CAPTURE_POLICY_WAIT, &frame);
  if (err != DC1394_SUCCESS) {
    cerr << "[CImageGrabber_dc1394] ERROR: Could not capture a frame" << endl;
    return false;
  }
  ...
  if (!m_options.deinterlace_stereo) {
    ...
    out_observation.imageRight.loadFromMemoryBuffer(
        width, height, true, imageBufRGB + width * height * 3); // Right cam.
...
#define SET_MODE(feat, FEAT)                                                   \
  if (options.feat##_mode >= 0) {                                              \
    err = dc1394_feature_set_mode(                                             \
        THE_CAMERA, DC1394_FEATURE_##FEAT,                                     \
        static_cast<dc1394feature_mode_t>(DC1394_FEATURE_MODE_MIN +            \
                                          options.feat##_mode));               \
    DC1394_WRN(err, "[CImageGrabber_dc1394::changeCaptureOptions] Could not "  \
                    "set feat mode");                                          \
  }
...
#define SET_VALUE(feat, FEAT)                                                  \
  if (options.feat >= 0) {                                                     \
    err = dc1394_feature_set_value(THE_CAMERA, DC1394_FEATURE_##FEAT,          \
                                   options.feat);                              \
    DC1394_WRN(err, "[CImageGrabber_dc1394::changeCaptureOptions] Could not "  \
                    "set feat value");                                         \
  }
...
bool CImageGrabber_dc1394::setSoftwareTriggerLevel(bool level) {
  ...
  DC1394_WRN(err, "[CImageGrabber_dc1394::setSoftwareTriggerLevel] Could not "
                  "set software trigger level");
...
void CImageGrabber_dc1394::enumerateCameras(TCameraInfoList &out_list) {
  ...
  try {
    ...
    if (!lib_context)
      throw std::runtime_error("[CImageGrabber_dc1394] ERROR: Failed to "
                               "enumerate cameras (Maybe your user has no "
                               "rights to access IEEE1394?).");
    ...
    for (unsigned int i = 0; i < list->num; i++) {
      ...
      if (!cam)
        throw std::runtime_error(
            format("[CImageGrabber_dc1394] ERROR: Failed to query camera with "
                   "GUID %u\n",
