...
CNTRIPEmitter::CNTRIPEmitter()
    : m_client(), m_com_port(""), m_com_bauds(38400),
      m_transmit_to_server(true), m_rate_count(0) {}
...
void CNTRIPEmitter::doProcess() {
  ...
  if (!buf.empty()) {
    ...
    if (m_raw_output_file_stream.is_open()) {
      m_raw_output_file_stream.write(reinterpret_cast<const char *>(&buf[0]),
                                     buf.size());
    }
  }
...
void CNTRIPEmitter::initialize() {
  ...
  if (!m_raw_output_file_prefix.empty()) {
    const string fil = mrpt::system::fileNameStripInvalidChars(
        ...
        string(".bin"));
    m_raw_output_file_stream.open(fil.c_str(),
  ...
  if (!m_client.open(m_ntrip_args, errstr))
    THROW_EXCEPTION_FMT("ERROR trying to connect to NTRIP caster: %s",
                        errstr.c_str());
}
...
void CNTRIPEmitter::loadConfig_sensorSpecific(
  ...
  ASSERTMSG_(!m_raw_output_file_prefix.empty() || !m_com_port.empty(),
             "At least one of either raw file output or serial COM file must "
