...
CInterfaceFTDI::CInterfaceFTDI() : m_readBuffer(4096) {
  ...
  if (ret)
    THROW_EXCEPTION("There was a problem initializing ftdi_context.");

  // Save in member:
  m_ftdi_context = static_cast<void *>(newCtx);
#else
  THROW_EXCEPTION("MRPT has been compiled without FTDI support. Please, "
                  "reconfigure and recompile MRPT.")
#endif
...
void CInterfaceFTDI::OpenBySerialNumber(const std::string &serialNumber) {
  ...
  if (!myDev)
    THROW_EXCEPTION_FMT("USB device with serial number '%s' not found.",
                        serialNumber.c_str());
...
void CInterfaceFTDI::recursive_fill_list_devices(void *usb_device_structure,
  ...
  if (dev->descriptor.idProduct && dev->descriptor.idVendor) {
    ...
    if (hUSB) {
      ...
      if ((strLen = usb_get_string_simple(hUSB, dev->descriptor.iManufacturer,
                                          manufacturer,
    ...
    if (hUSB) {
      ...
      if ((strLen = usb_get_string_simple(hUSB, dev->descriptor.iProduct,
    ...
    if (hUSB) {
      char serial[300];
      if ((strLen = usb_get_string_simple(hUSB, dev->descriptor.iSerialNumber,
...
/*-------------------------------------------------------------
                                        ftdi_read
-------------------------------------------------------------*/
void CInterfaceFTDI::ftdi_read(void *lpvBuffer, unsigned long dwBuffSize,
...
/*-------------------------------------------------------------
                                        ftdi_write
-------------------------------------------------------------*/
void CInterfaceFTDI::ftdi_write(const void *lpvBuffer, unsigned long dwBuffSize,
...
/*-------------------------------------------------------------
                                        OpenByDescription
-------------------------------------------------------------*/
std::ostream &mrpt::hwdrivers::operator<<(std::ostream &o,
                                          const TFTDIDevice &d) {
