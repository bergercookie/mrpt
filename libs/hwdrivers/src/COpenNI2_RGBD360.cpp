...
COpenNI2_RGBD360::COpenNI2_RGBD360()
    : m_sensorPoseOnRobot(), m_preview_window(false),
      ...
      m_grab_rgb(true), m_grab_depth(true), m_grab_3D_points(true) {
...
* \sa doProcess
*/
void COpenNI2_RGBD360::getNextObservation(CObservationRGBD360 &out_obs,
                                          bool &there_is_obs,
                                          bool &hardware_error) {
#if MRPT_HAS_OPENNI2
  CTicTac tictac;
  ...
  for (unsigned sensor_id = 0; sensor_id < (unsigned int)NUM_SENSORS;
    ...
    getNextFrameRGB(newObs.intensityImages[sensor_id],
                    newObs.timestamps[sensor_id], there_is_obs, hardware_error,
                    sensor_id);
    getNextFrameD(newObs.rangeImages[sensor_id], newObs.timestamps[sensor_id],
    ...
    if (m_preview_window) {
      if (out_obs.hasRangeImage) {
        if (++m_preview_decim_counter_range > m_preview_window_decimation) {
          ...
          CMatrixFloat r =
              out_obs.rangeImages[sensor_id] * float(1.0 / this->m_maxRange);
      ...
      if (out_obs.hasIntensityImage) {
        if (++m_preview_decim_counter_rgb > m_preview_window_decimation) {
          ...
          if (!m_win_int[sensor_id]) {
            m_win_int[sensor_id] = std::make_shared<mrpt::gui::CDisplayWindow>(
                "Preview INTENSITY");
      ...
      if (m_win_range[sensor_id])
        m_win_range[sensor_id].reset();
      if (m_win_int[sensor_id])
        m_win_int[sensor_id].reset();
    }
  }
