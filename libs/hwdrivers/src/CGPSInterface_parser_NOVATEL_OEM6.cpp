...
bool CGPSInterface::implement_parser_NOVATEL_OEM6(
  ...
  // Short header?
  const bool is_short_hdr = peek_buffer[0] == nv_oem6_short_header_t::SYNCH0 &&
  ...
  if (is_short_hdr) {
    ...
    m_rx_buffer.pop_many(reinterpret_cast<uint8_t *>(&buf[sizeof(hdr)]),
                         hdr.msg_len + 4 /*crc*/);
    ...
    if (!CObservationGPS::GPS_time_to_UTC(hdr.week, hdr.ms_in_week * 1e-3,
                                          num_leap_seconds,
