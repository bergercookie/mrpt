...
bool CGPSInterface::implement_parser_NMEA(
  ...
  if (peek_buffer[0] != '$' || peek_buffer[1] != 'G' || peek_buffer[2] != 'P') {
    ...
    if (line_is_ended) {
      ...
      if (CGPSInterface::parse_NMEA(line, m_just_parsed_messages,
                                    false /*verbose*/)) {
        ...
        if (m_verbose)
          std::cerr << "[CGPSInterface::implement_parser_NMEA] Line of unknown "
                       "format ignored: `"
                    << line << "`\n";
      }
      return true;
...
/* -----------------------------------------------------
                                        parse_NMEA
----------------------------------------------------- */
bool CGPSInterface::parse_NMEA(const std::string &s,
                               mrpt::obs::CObservationGPS &out_obs,
                               const bool verbose) {
  static mrpt::system::TTimeStamp last_known_date =
      mrpt::system::now(); // For building complete date+time in msgs without a
                           // date.
  ...
  mrpt::system::tokenize(s, "*,\t\r\n", lstTokens,
                         false /* do not skip blank tokens */);
  if (lstTokens.size() < 3)
    return false;
  ...
  if (lstTokens[0] == "$GPGGA" && lstTokens.size() >= 13) {
    ...
    token = lstTokens[7];
    if (!token.empty())
      rmc.fields.speed_knots = atof(token.c_str());

    // Direction:
    token = lstTokens[8];
    if (!token.empty())
      rmc.fields.direction_degrees = atof(token.c_str());
