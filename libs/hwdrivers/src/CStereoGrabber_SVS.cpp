...
/*-------------------------------------------------------------
                                        Constructor
 -------------------------------------------------------------*/
CStereoGrabber_SVS::CStereoGrabber_SVS(int cameraIndex,
                                       const TCaptureOptions_SVS &options)
    : m_bInitialized(false), m_videoObject(nullptr), m_stereoImage(nullptr),
      m_disparityParams(nullptr), m_resolutionX(options.frame_width),
      m_resolutionY(options.frame_height),
      m_procesOnChip(options.m_procesOnChip),
      m_calDisparity(options.m_calDisparity), m_options(options) {
#if MRPT_HAS_SVS
...
bool CStereoGrabber_SVS::getStereoObservation(
  ...
  if ((m_stereoImage =
      ...
      static_cast<svsStereoImage *>(m_stereoImage)
          ->haveImages) // 500 ms timeout //TODO adjust timeout with framerate
  {
    ...
    if (m_procesOnChip) {
      IplImage *ImageDisparity =
          cvCreateImage(cvSize(m_resolutionX, m_resolutionY), IPL_DEPTH_8U, 1);
      ...
      ASSERT_(
          ...
          ImageDisparity
              ->width); // JL: The code below assumes image_width == widthStep
      ...
      // Create the object to be return (it will have a fresh timestamp if it's
      // created now and here):
      CObservationStereoImages ret_obj(
          cvCloneImage(ImageLeft), // Create a new IplImage* which will be owned
                                   // by the observation object.
...
// All ok
  ...
  // construction.
  return false; // This shouldn't actually be never reached, just to please the
                // compiler.
#endif
}
...
/*-------------------------------------------------------------
 ...
 -------------------------------------------------------------*/
TCaptureOptions_SVS::TCaptureOptions_SVS(int _frame_width, int _frame_height,
                                         double _framerate, int _NDisp,
                                         int _Corrsize, int _LR, int _Thresh,
                                         int _Unique, int _Horopter,
                                         int _SpeckleSize, bool _procesOnChip,
                                         bool _calDisparity) {
