...
#if MRPT_HAS_OPENNI2
bool setONI2StreamMode(openni::VideoStream &stream, int w, int h, int fps,
...
COpenNI2Generic::COpenNI2Generic()
    : m_width(640), m_height(480), m_fps(30.0f),
#if MRPT_HAS_OPENNI2
      m_rgb_format(openni::PIXEL_FORMAT_RGB888),
      m_depth_format(openni::PIXEL_FORMAT_DEPTH_1_MM),
#endif // MRPT_HAS_OPENNI2
      m_verbose(false), m_grab_image(true), m_grab_depth(true),
...
COpenNI2Generic::COpenNI2Generic(int width, int height, float fps,
...
int COpenNI2Generic::getConnectedDevices() {
  ...
  for (std::set<int>::const_iterator it = newDevices.begin(),
    ...
    CDevice::Ptr device =
        CDevice::create(info, (openni::PixelFormat)m_rgb_format,
...
void COpenNI2Generic::open(unsigned sensor_id) {
  ...
  if (m_verbose)
    printf("[COpenNI2Generic] DBG: [%s] about to call vDevices[%d]->open()\n",
...
unsigned int COpenNI2Generic::openDevicesBySerialNum(
  ...
  for (unsigned sensor_id = 0; sensor_id < vDevices.size(); sensor_id++) {
    ...
    if (m_verbose)
      printf("[COpenNI2Generic::openDevicesBySerialNum] checking device with "
             "serial '%d'\n",
             serialNum);
    ...
    int height = m_height;
    if (m_verbose)
      printf("[COpenNI2Generic] DBG: [%s] about to call "
    ...
    num_open_dev++;
    if (m_verbose)
      printf("[COpenNI2Generic] DBG: [%s] now has %d devices open\n",
...
unsigned int
COpenNI2Generic::openDeviceBySerial(const unsigned int SerialRequired) {
...
void COpenNI2Generic::getNextFrameRGB(mrpt::utils::CImage &rgb_img,
                                      uint64_t &timestamp, bool &there_is_obs,
                                      bool &hardware_error,
                                      unsigned sensor_id) {
#if MRPT_HAS_OPENNI2
  // Sensor index validation.
  ...
  if (vDevices[sensor_id]->getNextFrameRGB(rgb_img, timestamp, there_is_obs,
...
void COpenNI2Generic::getNextFrameD(mrpt::math::CMatrix &depth_img,
  ...
  if (vDevices[sensor_id]->getNextFrameD(depth_img, timestamp, there_is_obs,
                                         hardware_error) == false) {
    showLog(mrpt::format("[%s]\n", __FUNCTION__));
    showLog(mrpt::format(" Error [%d]th Sensor.\n", sensor_id));
    showLog(std::string(" ") + vDevices[sensor_id]->getLog() + "\n");
...
void COpenNI2Generic::getNextFrameRGBD(
  ...
  if (vDevices[sensor_id]->getNextFrameRGBD(out_obs, there_is_obs,
                                            hardware_error) == false) {
    showLog(mrpt::format("[%s]\n", __FUNCTION__));
    showLog(mrpt::format(" Error [%d]th Sensor.\n", sensor_id));
    showLog(std::string(" ") + vDevices[sensor_id]->getLog() + "\n");
...
bool setONI2StreamMode(openni::VideoStream &stream, int w, int h, int fps,
                       openni::PixelFormat format) {
  // std::cout << "[COpenNI2Generic] Ask mode: " << w << "x" << h << " " << fps
  // << " fps. format " << format << std::endl;
  bool found = false;
...
//
COpenNI2Generic::CDevice::CDevice(const openni::DeviceInfo &info,
                                  openni::PixelFormat rgb,
  ...
  m_streams[DEPTH_STREAM] =
      CStream::create(m_device, openni::SENSOR_DEPTH, depth, m_log, m_verbose);
}
...
bool COpenNI2Generic::CDevice::synchMirrorMode() {
  ...
  for (int i = 0; i < STREAM_TYPE_SIZE; ++i) {
    ...
    if (mirror_support == false) {
      ...
      m_log << " We assume this is MS Kinect and taken images are inverted to "
               "right and left."
            << std::endl;
      // In this case, getMirroringEnabled() method always returns false. So we
      // cannot confirm whether the images are inverted or not.
      m_mirror = true;
      break;
    }
  }
...
bool COpenNI2Generic::CDevice::startStreams() {
  ...
  for (int i = 0; i < STREAM_TYPE_SIZE; ++i) {
    ...
    if (m_streams[i]->start() == false) {
      if (m_verbose)
        printf("  [%s] m_streams[%d]->start() returned FALSE!\n", __FUNCTION__,
               i);
    } else {
      num_ok++;
    }
    if (m_verbose)
      printf("  [%s] m_streams[%d]->start() returned TRUE\n", __FUNCTION__, i);
  }
  if (m_verbose)
    printf("  [COpenNI2Generic::CDevice::startStreams()] %d streams were "
           "started.\n",
           num_ok);
  return num_ok > 0;
  MRPT_END
...
bool COpenNI2Generic::CDevice::open(int w, int h, int fps) {
  ...
  for (int i = 0; i < STREAM_TYPE_SIZE; ++i) {
    ...
    if (m_streams[i]->open(w, h, fps) == false) {
      if (m_verbose)
        printf("   [%s] m_streams[%d]->open() returned FALSE\n", __FUNCTION__,
               i);
      return false;
    }
    if (m_verbose)
      printf("   [%s] m_streams[%d]->open() returned OK\n", __FUNCTION__, i);
  }
  ...
  if (m_device.isImageRegistrationModeSupported(
    ...
    // SEB		if(m_device.setImageRegistrationMode(openni::IMAGE_REGISTRATION_OFF)
    // != openni::STATUS_OK){
...
bool COpenNI2Generic::CDevice::getNextFrameRGB(mrpt::utils::CImage &img,
                                               uint64_t &timestamp,
                                               bool &there_is_obs,
                                               bool &hardware_error) {
  MRPT_START
  ...
  if (m_streams[COLOR_STREAM]->getFrame(frame, timestamp, there_is_obs,
                                        hardware_error) == false) {
    return false;
  }
...
bool COpenNI2Generic::CDevice::getNextFrameD(mrpt::math::CMatrix &img,
                                             uint64_t &timestamp,
                                             bool &there_is_obs,
                                             bool &hardware_error) {
  MRPT_START
  ...
  openni::VideoFrameRef frame;
  if (m_streams[DEPTH_STREAM]->getFrame(frame, timestamp, there_is_obs,
                                        hardware_error) == false) {
    return false;
  }
...
bool COpenNI2Generic::CDevice::getNextFrameRGBD(
  ...
  for (int i = 0; i < STREAM_TYPE_SIZE; ++i) {
    if (!m_streams[i] || !m_streams[i]->isValid())
      continue;
    if (m_streams[i]->getFrame(frame[i], tm, there_is_obs, hardware_error) ==
        false) {
      return false;
    }
...
COpenNI2Generic::CDevice::Ptr
COpenNI2Generic::CDevice::create(const openni::DeviceInfo &info,
                                 openni::PixelFormat rgb,
                                 openni::PixelFormat depth, bool verbose) {
...
//
COpenNI2Generic::CDevice::CStream::CStream(openni::Device &device,
                                           openni::SensorType type,
                                           openni::PixelFormat format,
                                           std::ostream &log, bool verbose)
    : m_log(log), m_device(device), m_strName("Unknown"), m_type(type),
      m_format(format), m_verbose(verbose) {
...
void COpenNI2Generic::CDevice::CStream::setCloseRange(int &value) {
  if (m_verbose)
    printf("      [CDevice::CStream::setCloseRange] entry with value=%d\n",
           value);
  m_stream.setProperty(XN_STREAM_PROPERTY_CLOSE_RANGE, value);
  if (m_verbose)
    printf("      [CDevice::CStream::setCloseRange] returned from "
           "mstream.setProperty()\n");
...
bool COpenNI2Generic::CDevice::CStream::open(int w, int h, int fps) {
  ...
  if (m_type != openni::SENSOR_COLOR && m_type != openni::SENSOR_DEPTH &&
      m_type != openni::SENSOR_IR) { // SEB added IR
    m_log << "[" << __FUNCTION__ << "]" << std::endl
          << " Unknown SensorType -> " << m_type << std::endl;
    return false;
  }
  if (m_verbose)
    printf("      [COpenNI2Generic::CDevice::CStream::open] opening sensor "
  ...
  if (m_verbose)
    printf("returned OK from stream.create()\n");
  openni::VideoMode options = m_stream.getVideoMode();
  m_log << "[" << __FUNCTION__ << "]" << std::endl;
  m_log << " " << m_strName << std::endl;
  m_log << " "
        << mrpt::format("Initial resolution (%d, %d) FPS %d Format %d",
  ...
  m_log << " "
        << mrpt::format("-> (%d, %d) FPS %d Format %d",
                        options.getResolutionX(), options.getResolutionY(),
                        options.getFps(), options.getPixelFormat())
        << std::endl;
  if (m_verbose)
    printf("      [COpenNI2Generic::CDevice::CStream::open] returning TRUE\n");
  return true;
}
...
COpenNI2Generic::CDevice::CStream::create(openni::Device &device,
                                          openni::SensorType type,
                                          openni::PixelFormat format,
                                          std::ostream &log, bool verbose) {
...
bool COpenNI2Generic::CDevice::CStream::getFrame(openni::VideoFrameRef &frame,
                                                 uint64_t &timestamp,
                                                 bool &there_is_obs,
                                                 bool &hardware_error) {
  there_is_obs = false;
