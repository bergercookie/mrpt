...
//! create?
#ifdef _WIN32
  m_handle =
      _wfopen(filename.toStdWString().c_str(), L"w+b"); // open for update (r/w)
#else
  ...
  if (_wfullpath(fullpath, filename.toStdWString().c_str(),
...
/*! \brief Delete the given data from the file.
        \details The function erases the given data from the file at the given
   write position. This
   ...
   checked for validity upon function exit.
...
/*! \brief Find a string of bytes in the file
        \details The function searches from the current read position until the
   given \c needle is
        found. If the needle is not found, XsResultValue::NOT_FOUND is returned.
   The function
        will update the seek position to the first character of the found
   needle.
        \param needleV	The byte string to find.
        \param pos	The position where \a needleV was found. This will
...
*/
XsFilePos IoInterfaceFile::getFileSize(void) const { return m_fileSize; }
...
/*! \brief Insert the given data into the file.
        \details The function writes the given data to the file at the current
   write position. This
...
/*! \brief Open a file.
        ...
        \param createNew When true, the file will be created if it doesn't exist
   yet
        \param readOnly When true, the file will be marked as read only for
   %IoInterfaceFile,
                        preventing accidental writes to the file.
        \returns XRV_OK if the file was opened successfully
        \sa createFile
*/
XsResultValue IoInterfaceFile::open(const XsString &filename, bool createNew,
                                    bool readOnly) {
  if (m_handle)
    return m_lastResult = XRV_ALREADYOPEN;
  ...
  if (readOnly)
#ifdef _WIN32
    m_handle = _wfopen(filename.toStdWString().c_str(),
                       L"rb"); // open for read only (r)
#else
...
/*! \copydoc IoInterface::readData
        \note This function reads exactly the number of bytes as requested from
   the file unless the end
...
/*! \copydoc IoInterface::writeData
        \note The function writes the given data to the file at the current
   write position.
*/
...
*/
XsFilePos IoInterfaceFile::getReadPosition(void) const { return m_readPos; }

/*! \brief Return the current write position.
        \returns The current write position.
*/
XsFilePos IoInterfaceFile::getWritePosition(void) const { return m_writePos; }
...
*/
bool IoInterfaceFile::isOpen(void) const { return m_handle != nullptr; }

/*! \brief Return whether the file is readonly or not.
        \returns true if the file is readonly
*/
bool IoInterfaceFile::isReadOnly(void) const { return !isOpen() || m_readOnly; }
