...
/*! \brief Find a string of bytes in the file
   ...
   given \c needle is
	found. If the needle is not found, XsResultValue::NOT_FOUND is returned. The
   function
	will update the seek position to the first character of the found needle.
	\param needleV	The byte string to find.
	\param pos	The position where \a needleV was found. This will point to
   the first character
...
XsResultValue IoInterfaceFile::find(const XsByteArray& needleV, XsFilePos& pos)
{
	...
	pos = 0;
	if (needleLength == 0) return m_lastResult = XRV_OK;
...
/*! \brief Open a file.
   ...
   %IoInterfaceFile,
	...
	\sa createFile
*/
XsResultValue IoInterfaceFile::open(
	const XsString& filename, bool createNew, bool readOnly)
{
	if (m_handle) return m_lastResult = XRV_ALREADYOPEN;
...
#ifdef _WIN32
		m_handle = _wfopen(
			filename.toStdWString().c_str(), L"rb");  // open for read only (r)
#else
...
#ifdef _WIN32
		m_handle = _wfopen(
			filename.toStdWString().c_str(), L"r+b");  // open for update (r/w)
#else
...
#else
			m_handle =
				fopen(filename.c_str(), "w+b");  // create for update (r/w)
#endif
		else
		{
#ifdef _WIN32
	...
	if (m_handle == nullptr) return m_lastResult = XRV_INPUTCANNOTBEOPENED;
...
XsResultValue IoInterfaceFile::readData(XsSize maxLength, XsByteArray& data)
{
	...
	m_readPos += length;
	if (length < maxLength) data.pop_back(maxLength - length);
	return m_lastResult = XRV_OK;
}
...
*/
XsResultValue IoInterfaceFile::readTerminatedData(
	XsSize maxLength, unsigned char terminator, XsByteArray& bdata)
{
	...
	while (!feof(m_handle) && !ferror(m_handle))
	{
		...
		if (length >= maxLength) return m_lastResult = XRV_OK;
...
/*! \copydoc IoInterface::writeData
   ...
   position.
*/
XsResultValue IoInterfaceFile::writeData(
	const XsByteArray& data, XsSize* written)
{
	...
	if (length == 0) return m_lastResult = XRV_OK;
