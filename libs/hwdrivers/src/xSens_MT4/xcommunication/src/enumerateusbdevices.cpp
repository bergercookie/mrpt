...
bool xsEnumerateUsbDevices(XsPortInfoList &ports) {
  ...
  deviceInfo = SetupDiGetClassDevs(&guid, nullptr, nullptr,
  ...
  for (DWORD dwIndex = 0; port == 0; ++dwIndex) {
    BOOL bRet = SetupDiEnumDeviceInterfaces(deviceInfo, nullptr, &guid, dwIndex,
                                            &interfaceData);
    if (!bRet) {
      if (GetLastError() == ERROR_NO_MORE_ITEMS)
        break;
    } else {
      if (!SetupDiGetDeviceInterfaceDetail(deviceInfo, &interfaceData, nullptr,
      ...
      bResult = SetupDiGetDeviceInterfaceDetailA(deviceInfo, &interfaceData,
                                                 detailData, length,
  ...
  for (ssize_t i = 0; i < deviceCount; i++) {
    ...
    unsigned char serialNumber[256];
    result = libUsb.get_string_descriptor_ascii(handle, desc.iSerialNumber,
                                                serialNumber, 256);

    if (desc.idVendor == ATMEL_VENDOR_ID &&
        desc.idProduct == ATMEL_BORROWED_PRODUCT_ID) {
      unsigned char productName[256];
      result = libUsb.get_string_descriptor_ascii(handle, desc.iProduct,
                                                  productName, 256);
