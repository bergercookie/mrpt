...
void XsUdev::initLibrary() {
  ...
  if (m_libraryLoader->isLoaded()) {
    ...
    m_uDev.enumerate_unref = (uDEV_enumerate_unref *)m_libraryLoader->resolve(
...
/*! \brief Drop a reference of the udev library context.
        ...
        If the refcount reaches zero, the resources of the context will be
   released.
*/
...
/*! \brief Drop a reference of a udev device.
        ...
        If the refcount reaches zero, the resources of the device will be
   released.
...
*/
int XsUdev::enumerate_add_match_subsystem(struct udev_enumerate *udev_enumerate,
...
*/
udev_list_entry *
XsUdev::list_entry_get_next(struct udev_list_entry *list_entry) {
...
*/
udev_list_entry *
XsUdev::enumerate_get_list_entry(struct udev_enumerate *udev_enumerate) {
...
/*! \brief Drop a reference of an enumeration context.
        ...
        If the refcount reaches zero, all resources of the enumeration context
   will be released.
...
*/
udev_device *XsUdev::device_new_from_syspath(struct udev *udev,
                                             const char *syspath) {
...
/*! \brief Find the next parent device, and fill in information from the sys
   device and the udev database entry.
        ...
        Returned device is not referenced. It is attached to the child device,
...
udev_device *XsUdev::device_get_parent_with_subsystem_devtype(
  ...
  if (m_uDev.device_get_parent_with_subsystem_devtype)
    return m_uDev.device_get_parent_with_subsystem_devtype(udev_device,
                                                           subsystem, devtype);
  else
    return nullptr;
}
...
/*! \brief Get a sys attribute value
   ...
   the same value and not open the attribute again.
        ...
        \return the content of a sys attribute file, or nullptr if there is no
   sys attribute value.
*/
const char *XsUdev::device_get_sysattr_value(struct udev_device *udev_device,
                                             const char *sysattr) {
