...
/*! \class PacketStamper
	...
	\details This class can analyze a data packet and create a proper packet id
   for it.
*/
...
/*! \brief Create 64 bit counter for a packet.
	\details Wrap when new XsDataPacket is too far away from the previous
   XsDataPacket in time.
	...
	\param highestPacket The highest packet available for the current device, it
   will be updated if
		the new counter is higher than the stored value.
	\returns The computed counter for the packet.
*/
int64_t PacketStamper::stampPacket(
	XsDataPacket& pack, XsDataPacket& highestPacket)
{
	//	int did = 0;
	...
	//		did = sdi.m_deviceId;
	//		JLDEBUG(gJournal, "XsensDeviceAPI", "%s [%08x] SDI interval
	//(%d-%d)\n", __FUNCTION__, sdi.m_deviceId, sdi.m_firstFrameNumber,
	// sdi.m_lastFrameNumber);
	//	}

	//! \todo This could be a (couple of) milliseconds too late, this should be
	//! set as soon as the source message arrives: mantis 7157
	pack.setTimeOfArrival(XsTimeStamp::now());
	int64_t newCounter, lastCounter = -1;

	if (!highestPacket.empty()) lastCounter = highestPacket.packetId().msTime();

	if (pack.containsPacketCounter())
		newCounter =
			calculateLargePacketCounter(pack.packetCounter(), lastCounter);
	else if (pack.containsSampleTimeFine())
	{
		if (pack.containsSampleTimeCoarse())
			newCounter = (int64_t)pack.sampleTime64();
		else
			newCounter = calculateLargeSampleTime(
				(int32_t)pack.sampleTimeFine(), lastCounter);
	}
	else if (pack.containsPacketCounter8())
		newCounter =
			calculateLargePacketCounter8(pack.packetCounter8(), lastCounter);
	else
		newCounter = lastCounter + 1;

	//	JLDEBUG(gJournal, "XsensDeviceAPI", "%s [%08x] old = %I64d new = %I64d
	// diff = %I64d\n", __FUNCTION__, did, lastCounter, newCounter,
	//(newCounter-lastCounter));

	pack.setPacketId(newCounter);
	if (newCounter > lastCounter) highestPacket = pack;

	return newCounter;
}

/*! \brief Calculate the new large packet counter value based on \a frameCounter
   and the \a lastCounter
...
*/
int64_t PacketStamper::calculateLargePacketCounter8(
	int64_t frameCounter, int64_t lastCounter)
{
	...
	if (dt < SC8BOUNDARY_HALF) return lastCounter + dt;  // normal increment

	return lastCounter + dt - SC8BOUNDARY;  // negative wraparound
}

/*! \brief Calculate the new large sample counter value based on \a frameCounter
   and the \a lastCounter
	...
	\note If lastCounter < 0, returns frameCounter
*/
int64_t PacketStamper::calculateLargePacketCounter(
	int64_t frameCounter, int64_t lastCounter)
{
	if (lastCounter < 0)
	{
	...
	if (dt < MTSCBOUNDARY_HALF) return lastCounter + dt;  // normal increment

	return lastCounter + dt - MTSCBOUNDARY;  // negative wraparound
}

/*! \brief Calculate the new large sample time value based on \a frameTime and
   the \a lastTime
...
*/
int64_t PacketStamper::calculateLargeSampleTime(
	int64_t frameTime, int64_t lastTime)
{
	...
	if (dt < (864000000 / 2)) return lastTime + dt;  // normal increment
