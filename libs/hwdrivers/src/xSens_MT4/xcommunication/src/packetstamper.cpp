...
/*! \class PacketStamper
        ...
        \details This class can analyze a data packet and create a proper packet
   id for it.
*/
...
/*! \brief Create 64 bit counter for a packet.
   ...
   XsDataPacket in time.
        ...
        \param pack The XsDataPacket that needs its 64-bit sample counter
   updated
        \param highestPacket The highest packet available for the current
   device, it will be updated if
...
int64_t PacketStamper::stampPacket(XsDataPacket &pack,
  ...
  if (pack.containsPacketCounter())
    ...
    if (pack.containsSampleTimeCoarse())
      ...
      newCounter =
          calculateLargeSampleTime((int32_t)pack.sampleTimeFine(), lastCounter);
