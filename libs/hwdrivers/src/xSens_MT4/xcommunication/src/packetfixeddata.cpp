...
PacketFixedData::PacketFixedData()
    : m_infoList(nullptr), m_formatList(nullptr), m_idList(nullptr),
      m_xm(false), m_itemCount(0) {
...
/*! \brief Sized constructor, creates an object with room for \a count device's
   worth of data
        ...
        \param count The number of devices whose metadata is stored in the
   object
*/
PacketFixedData::PacketFixedData(uint16_t count)
    : m_infoList(nullptr), m_formatList(nullptr), m_idList(nullptr),
      m_xm(false), m_itemCount(count) {
