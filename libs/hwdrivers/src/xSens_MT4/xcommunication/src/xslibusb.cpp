...
void XsLibUsb::initLibrary() {
  ...
  if (m_libraryLoader->isLoaded()) {
    ...
    m_libUsb.unref_device =
        (libUSB_unref_device *)m_libraryLoader->resolve("libusb_unref_device");
    ...
    m_libUsb.get_bus_number = (libUSB_get_bus_number *)m_libraryLoader->resolve(
    ...
    m_libUsb.bulk_transfer = (libUSB_bulk_transfer *)m_libraryLoader->resolve(
...
/*! \brief Initialize libusb. This function must be called before calling any
   other libusb function.
        ...
        If you do not provide an output location for a context pointer, a
   default
...
/*! \brief Deinitialize libusb. Should be called after closing all open devices
   and before your application terminates.
        \param ctx the context to deinitialize, or nullptr for the default
   context
*/
...
/*! \brief Open a device and obtain a device handle. A handle allows you to
   ...
   available to you through libusb_get_device().
        ...
        \param handle output location for the returned device handle pointer.
...
*/
int XsLibUsb::kernel_driver_active(libusb_device_handle *dev,
...
*/
int XsLibUsb::attach_kernel_driver(libusb_device_handle *dev,
                                   int interface_number) {
...
/*! \brief Detach a kernel driver from an interface. If successful, you will
   ...
   is not available
        ...
        \see libusb_kernel_driver_active()
*/
int XsLibUsb::detach_kernel_driver(libusb_device_handle *dev,
                                   int interface_number) {
...
/*! \brief Claim an interface on a given device handle.
   ...
   on any of its endpoints.
        ...
        \param dev a device handle
        \param interface_number the \a bInterfaceNumber of the interface you
   wish to claim
        \returns 0 on success
        \returns LIBUSB_ERROR_NOT_FOUND if the requested interface does not
   exist
...
/*! \brief Release an interface previously claimed with
   ...
   sent
        ...
        \param dev a device handle
        \param interface_number the \a bInterfaceNumber of the
   previously-claimed interface
        ...
        \returns another LIBUSB_ERROR code on other failure
*/
int XsLibUsb::release_interface(libusb_device_handle *dev,
                                int interface_number) {
...
/*! \brief Free a configuration descriptor obtained from
   libusb_get_active_config_descriptor() or libusb_get_config_descriptor().
        ...
        It is safe to call this function with a nullptr config parameter, in
...
/*! \brief Get the underlying device for a handle.
        ...
        This function does not modify the reference count of the returned
   device,
...
/*! \brief Get the USB device descriptor for a given device.
   ...
   memory.
        ...
        \returns 0 on success or a LIBUSB_ERROR code on failure
*/
int XsLibUsb::get_device_descriptor(libusb_device *dev,
...
/*! \brief Returns a list of USB devices currently attached to the system.
   ...
   LIBUSB_ERROR code to errors encountered by the backend.
*/
...
/*! \brief Frees a list of devices previously discovered using
   libusb_get_device_list().
        If the unref_devices parameter is set, the reference count of each
...
/*! \brief Retrieve a string descriptor in C style ASCII.
   ...
   supported by the device.
        ...
        \returns number of bytes returned in data, or LIBUSB_ERROR code on
   failure
*/
int XsLibUsb::get_string_descriptor_ascii(libusb_device_handle *dev,
                                          uint8_t desc_index,
...
*/
int XsLibUsb::bulk_transfer(libusb_device_handle *dev_handle,
                            unsigned char endpoint, unsigned char *data,
                            int length, int *actual_length,
                            unsigned int timeout) {
  if (m_libUsb.bulk_transfer)
    return m_libUsb.bulk_transfer(dev_handle, endpoint, data, length,
