...
/*! \class XsDataPacket
        ...
        This class is used by XDA for easy access to data contained in a
   message. It analyzes its internal
        XsMessage upon contruction to give access to the individual contained
   pieces of data.
...
void XsDataPacket_assignFromXsLegacyDataPacket(
  ...
  if (pack->containsCalibratedGyroscopeData(index)) {
    ...
    hacket->message().setDataShort(XDI_RateOfTurn | di,
                                   hacket->message().getDataSize());
    hacket->message().setDataByte(3 * hacket->getFPValueSize(di),
                                  hacket->message().getDataSize());
    hacket->itemCount()++;
  ...
  if (pack->containsOrientationEuler(index))
    hacket->setOrientationEuler(pack->orientationEuler(index),
                                (format.m_outputSettings & XOS_Coordinates_Ned)
                                    ? XDI_CoordSysNed
                                    : XDI_CoordSysNwu);
  if (pack->containsOrientationMatrix(index))
    hacket->setOrientationMatrix(pack->orientationMatrix(index),
                                 (format.m_outputSettings & XOS_Coordinates_Ned)
                                     ? XDI_CoordSysNed
                                     : XDI_CoordSysNwu);
  ...
  if (pack->containsVelocity(index))
    hacket->setVelocity(pack->velocity(index),
                        (format.m_outputSettings & XOS_Coordinates_Ned)
                            ? XDI_CoordSysNed
                            : XDI_CoordSysNwu);
  ...
  if (pack->containsMtwSdiData(index)) {
    ...
    {
      // Special copy to preserve watermarking
      hacket->message().setDataShort(XDI_DeltaQ | di,
                                     hacket->message().getDataSize());
      hacket->message().setDataByte(4 * hacket->getFPValueSize(di),
                                    hacket->message().getDataSize());
      hacket->itemCount()++;
      hacket->message().setDataBuffer(
      ...
      hacket->message().setDataShort(XDI_DeltaV | di,
                                     hacket->message().getDataSize());
      hacket->message().setDataByte(3 * hacket->getFPValueSize(di),
                                    hacket->message().getDataSize());
      hacket->itemCount()++;
    ...
    hacket->setFrameRange(
        XsRange((int)mtwsdi.m_firstFrameNumber, (int)mtwsdi.m_lastFrameNumber));
