...
public:
  ...
  */
  ...
  /*! \brief A context manager for libusb
    ...
    For predictable operation with libusb, it is recommended to use at least one
    context per library.
  */
  class UsbContext {
  public:
  ...
  */
  XsResultValue libusbErrorToXrv(int libusbError,
...
DWORD usbReadThreadFunc(void *obj) {
  ...
  d->m_running = true;
  xsSetThreadPriority(xsGetCurrentThreadHandle(),
                      XS_THREAD_PRIORITY_HIGHEST); // lint !e534
...
void UsbInterfacePrivate::threadFunc() {
  ...
  for (m_readIdx = 0; m_readIdx < (m_oCount - 1); ++m_readIdx) {
    ...
    m_winUsb.ReadPipe(m_usbHandle[1], m_bulkInPipe, m_fixedBuffer[m_readIdx],
  ...
  while (run) {
    ...
    default:
...
#endif
      {
        ...
        int idx = m_readIdx;
        DWORD dataRead = 0;
        if (!m_winUsb.GetOverlappedResult(m_usbHandle[0], &m_overlapped[idx],
                                          &dataRead, FALSE)) {
          // error
          ...
          case ERROR_IO_INCOMPLETE:
            // JLDEBUG(gJournal, "m_winUsb.GetOverlappedResult resulted in
            // acceptable windows error " << err);
            break;

          default:
            JLALERT(gJournal,
                    "m_winUsb.GetOverlappedResult resulted in windows error "
                        << err);
            run = false;
            break;
          }
          ...
          JLTRACE(gJournal, "m_winUsb.GetOverlappedResult resulted in "
...
/*! \brief Close the USB communication port.
        ...
        \note Linux:\n
        If a kernel driver was detached when communication with the device
   started,
        attach it again. No guarantee is given that udev will pick up on it
   though.
*/
...
// lint --e{534}
  ...
  for (int i = 0; i < d->m_interfaceCount; i++) {
    ...
    while (result != LIBUSB_SUCCESS) {
      result =
          UsbInterfacePrivate::getContextManager().m_libUsb.release_interface(
              d->m_deviceHandle, i);
      if (result == LIBUSB_SUCCESS) {
        UsbInterfacePrivate::getContextManager().m_libUsb.attach_kernel_driver(
            d->m_deviceHandle, i);
      }
    }
  }
...
XsResultValue UsbInterface::open(const XsPortInfo &portInfo, uint32_t,
                                 uint32_t) {
  d->m_endTime = 0;
...
#else
  JLDEBUG(gJournal, "Open usb port " << portInfo.usbBus() << ":"
                                     << portInfo.usbAddress());
#endif

  if (isOpen()) {
    JLALERT(gJournal, "Port " << portInfo.portName().toStdString()
                              << " already open");
...
#ifdef USE_WINUSB
  d->m_deviceHandle =
      CreateFileA(portInfo.portName().c_str(), GENERIC_WRITE | GENERIC_READ,
  ...
  if (result) {
    result = d->m_winUsb.GetAssociatedInterface(d->m_usbHandle[0], 0,
                                                &d->m_usbHandle[1]);
  } else {
#ifdef XSENS_DEBUG
    DWORD err = GetLastError();
  ...
  for (int k = 0; k < 2; k++) {
    ...
    if (result) {
      ...
      result = d->m_winUsb.QueryInterfaceSettings(d->m_usbHandle[k], 0,
                                                  &interfaceDescriptor);
    }
    if (result) {
      for (int i = 0; i < interfaceDescriptor.bNumEndpoints; i++) {
        result =
            d->m_winUsb.QueryPipe(d->m_usbHandle[k], 0, (UCHAR)i, &pipeInfo);
  ...
  for (int i = 0; i < listLength && device == nullptr; ++i) {
    ...
    if (UsbInterfacePrivate::getContextManager().m_libUsb.get_bus_number(dev) !=
            bus ||
        UsbInterfacePrivate::getContextManager().m_libUsb.get_device_address(
            dev) != address)
      continue;

    libusb_device_descriptor desc;
    result =
        UsbInterfacePrivate::getContextManager().m_libUsb.get_device_descriptor(
            dev, &desc);
    if (result != LIBUSB_SUCCESS)
      break;
    ...
    for (uint8_t ifCount = 0;
      ...
      for (uint8_t altsettingCount = 0;
           altsettingCount < configDesc->interface[ifCount].num_altsetting;
           altsettingCount++) {
        const libusb_endpoint_descriptor *endpoints =
            configDesc->interface[ifCount].altsetting[altsettingCount].endpoint;
        int inEndpoint = -1, outEndpoint = -1;
        for (uint8_t i = 0; i < configDesc->interface[ifCount]
                                    .altsetting[altsettingCount]
                                    .bNumEndpoints;
             i++) {
          if ((endpoints[i].bmAttributes & LIBUSB_TRANSFER_TYPE_MASK) !=
          ...
          case LIBUSB_ENDPOINT_IN:
            inEndpoint =
                endpoints[i].bEndpointAddress & LIBUSB_ENDPOINT_ADDRESS_MASK;
            break;
    ...
    UsbInterfacePrivate::getContextManager().m_libUsb.free_config_descriptor(
        configDesc);
  ...
  UsbInterfacePrivate::getContextManager().m_libUsb.free_device_list(deviceList,
                                                                     1);
  if (result != LIBUSB_SUCCESS) {
  ...
  for (int i = 0; i < d->m_interfaceCount; i++) {
    result =
        UsbInterfacePrivate::getContextManager().m_libUsb.kernel_driver_active(
            handle, i);
    if (result > 0)
      result = UsbInterfacePrivate::getContextManager()
    ...
    if (result == LIBUSB_SUCCESS)
      result =
          UsbInterfacePrivate::getContextManager().m_libUsb.claim_interface(
              handle, i);
    if (result != LIBUSB_SUCCESS) {
      for (int j = 0; j < i; j++) {
        while (result != LIBUSB_SUCCESS) {
...
/*! \brief Read data from the USB port and put it into the data buffer.
   ...
   (non-blocking) and
        ...
        \returns XRV_OK if no error occurred. It can be that no data is
   available and XRV_OK will be
                        returned. Check data.size() for the number of bytes that
   were read.
*/
...
/*! \brief Read data from the serial port and put it into the data buffer.
   ...
   (non-blocking) and
        ...
        \param data			Pointer to a buffer that will store
   the received data.
        \param length		The number of bytes placed into \c data.
        \returns XRV_OK if no error occurred. It can be that no data is
   available and XRV_OK will be
                        returned. Check *length for the number of bytes that
   were read.
*/
XsResultValue UsbInterface::readData(const XsSize maxLength, void *data,
                                     XsSize *length) {
  JLTRACE(gJournal, "maxLength=" << maxLength << ", data=0x" << data
                                 << ", length=0x" << length);
  XsSize ln;
  if (length == nullptr)
    length = &ln;
  ...
  JLTRACE(gJournal, "returned success, read "
                        << *length << " of " << maxLength
  ...
  JLTRACE(gJournal, "bulk read returned: " << d->libusbErrorToString(res)
                                           << ". " << actual
                                           << " bytes received");
...
#ifdef USE_WINUSB
  JLDEBUG(gJournal, "Request to set timeout to "
                        << ms << " ms overridden, setting to 0 ms instead");
  ms = 0; // no timeout ever
  UCHAR enable = FALSE;

  d->m_winUsb.SetPipePolicy(d->m_usbHandle[1], d->m_bulkInPipe,
                            IGNORE_SHORT_PACKETS, sizeof(UCHAR),
                            &enable); // lint !e534
  d->m_winUsb.SetPipePolicy(d->m_usbHandle[1], d->m_bulkInPipe,
                            PIPE_TRANSFER_TIMEOUT, sizeof(ULONG),
                            &ms); // lint !e534
...
void UsbInterface::setRawIo(bool enable) {
  ...
  d->m_winUsb.SetPipePolicy(d->m_usbHandle[1], d->m_bulkInPipe, RAW_IO,
                            sizeof(UCHAR), &rawIo); // lint !e534
#else
  (void)enable;
#endif
  d->m_lastResult = XRV_OK;
...
bool UsbInterface::getRawIo(void) {
  ...
  d->m_winUsb.GetPipePolicy(d->m_usbHandle[1], d->m_bulkInPipe, RAW_IO,
                            &someSize, &rawIo); // lint !e534
  return (rawIo != 0);
#else
  return false;
#endif
...
/*! \brief Wait for data to arrive or a timeout to occur.
        \details The function waits until \c maxLength data is available or
   until a timeout occurs.
        The function returns success if data is available or
   XsResultValue::TIMEOUT if a
   ...
   timeout stored
        ...
        \param data A buffer that will be filled with the read data. It must be
   able to contain at
                        least \a maxLength bytes.
        \param length An optional pointer to storage for the actual number of
   bytes read.
        \returns XRV_OK if the requested data was read
*/
XsResultValue UsbInterface::waitForData(const XsSize maxLength, void *data,
                                        XsSize *length) {
  JLTRACE(gJournal, "timeout=" << d->m_timeout << ", data=" << data
                               << ", length=" << length);
...
/*! \brief Write the data to the USB port.
        ...
        \param written An optional pointer to storage for the actual number of
...
*/
XsResultValue UsbInterface::writeData(const XsSize length, const void *data,
                                      XsSize *written) {
  XsSize bytes;
  if (written == nullptr)
    written = &bytes;
...
#ifdef USE_WINUSB
  ULONG dataWritten;
  d->m_winUsb.WritePipe(d->m_usbHandle[1], d->m_bulkOutPipe, (uint8_t *)data,
                        (ULONG)length, &dataWritten,
                        nullptr); // lint !e534
  ...
  *written = 0;
  while (*written < length) {
    ...
    *written += actual;
    if (result != LIBUSB_SUCCESS) {
      JLALERT(gJournal, "bulk write failed: " << d->libusbErrorToString(result)
                                              << ". " << actual
                                              << " bytes sent");
      return (d->m_lastResult = d->libusbErrorToXrv(result));
    }
