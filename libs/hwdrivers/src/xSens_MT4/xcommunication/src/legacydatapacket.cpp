...
#define FORMAT_DOUBLE                                                          \
  ((m_fixedData->m_formatList[index].m_outputSettings & XOS_Dataformat_Mask) | \
...
/*!	\struct LegacyDataPacket
        \brief Contains an MTData XsMessage and supports functions for
   extracting contained data.
*/

/*! \brief Construct a new %LegacyDataPacket without data. The %LegacyDataPacket
 * will be invalid. */
LegacyDataPacket::LegacyDataPacket()
    : m_fixedData(new PacketFixedData), m_lastFoundId(0), m_lastFoundIndex(0),
      m_rtc(0), m_toa(0), m_packetId(0), m_triggerIn1(0), m_triggerIn2(0) {
...
/*! \brief Construct a new %LegacyDataPacket with room for data from \a count
   ...
   standalone MT. This matters
                                because the sample counter is in a different
   place.
*/
LegacyDataPacket::LegacyDataPacket(uint16_t count, bool xbus)
    : m_fixedData(new PacketFixedData(count)), m_lastFoundId(0),
      m_lastFoundIndex(0), m_rtc(0), m_toa(0), m_packetId(0), m_triggerIn1(0),
      m_triggerIn2(0)
...
LegacyDataPacket::LegacyDataPacket(const LegacyDataPacket &other)
    : m_fixedData(0), m_lastFoundId(0), m_lastFoundIndex(0), m_rtc(0), m_toa(0),
      m_packetId(0), m_triggerIn1(0), m_triggerIn2(0)
...
*/
const LegacyDataPacket &LegacyDataPacket::
operator=(const LegacyDataPacket &pack) {
...
/*! \brief Returns the packet info for the \a index'th device in the packet.
        ...
        \param index The index of the device whose packet info should be
   returned
...
/*! \brief Return the frame counter (previously: sample counter) of the packet
        \details For SDI data, this function will return the Last Frame Number
   in the SDI interval.
...
/*! \brief Returns the index of the fixed data with id \a dev
        \details If the result is not -1, it can be used as the \a index
...
/*! \brief Returns the data format of the device with the given \a index
        \param index The index of the device whose data format should be
   returned
...
/*! \brief Sets the data format of the device with the given \a index to \a
   outputMode and \a outputSettings
        \param outputMode The output mode of the device (see low level
...
/*! \brief Sets the xbus flag
        \details The xbus flag determines where the packet counter is stored in
   the message. Setting
        this value incorrectly can lead to corrupted data reads.
        \param xbus The desired setting
        \param convert When set to true, the sample counter will be moved from
   its old place to the new
        place.
*/
void LegacyDataPacket::setXbusSystem(bool xbus, bool convert) {
  ...
  if (xbus != m_fixedData->m_xm) {
    if (convert) {
      ...
      if (xbus) {
        // new version is Xbus, so old version is regular format -> place
        // timestamp at start of packet and remove from all individual data
        // units
        ...
        // new version is regular, so old version is Xbus format -> place
        // timestamp at end of all individual data units and remove from start
        // of packet
...
void LegacyDataPacket::updateInfoList() {
  ...
  for (uint16_t i = 0; i < m_fixedData->m_itemCount; ++i) {
    ...
    if (m_fixedData->m_formatList[i].m_outputMode & XOM_GpsPvt_Pressure) {
      ...
      if ((m_fixedData->m_formatList[i].m_outputSettings & XOS_NoGpsInGpsPvt) ==
          0) {
...
XsUShortVector LegacyDataPacket::rawAcceleration(int32_t index) const {
  ...
  if (containsRawAcceleration(index))
    for (uint16_t i = 0; i < 3; ++i)
      buffer[i] =
          m_msg.getDataShort(m_fixedData->m_infoList[index].m_rawAcc + (2 * i));
...
XsUShortVector LegacyDataPacket::rawGyroscopeData(int32_t index) const {
  ...
  if (containsRawGyroscopeData(index))
    for (uint16_t i = 0; i < 3; ++i)
      buffer[i] =
          m_msg.getDataShort(m_fixedData->m_infoList[index].m_rawGyr + (2 * i));
...
XsUShortVector LegacyDataPacket::rawMagneticField(int32_t index) const {
  ...
  if (containsRawMagneticField(index))
    for (uint16_t i = 0; i < 3; ++i)
      buffer[i] =
          m_msg.getDataShort(m_fixedData->m_infoList[index].m_rawMag + (2 * i));
...
bool LegacyDataPacket::containsRawTemperature(int32_t index,
                                              int channel) const {
  if (dataSize(index) == 0)
    return false;
...
bool LegacyDataPacket::setRawTemperature(const uint16_t temp, int32_t index,
                                         int channel) {
  if (dataSize(index) == 0)
    return false;
  if (channel >= XS_MAX_TEMPERATURE_CHANNELS)
...
bool LegacyDataPacket::containsRawData(int32_t index) const {
  if (dataSize(index) == 0)
    return false;
  return (m_fixedData->m_infoList[index].m_rawData !=
          XS_DATA_ITEM_NOT_AVAILABLE);
}
bool LegacyDataPacket::setRawData(const XsScrData &data, int32_t index) {
  ...
  for (uint16_t i = 0; i < (9 + rawTemperatureChannelCount(index)); ++i)
    m_msg.setDataShort(bare[i],
                       m_fixedData->m_infoList[index].m_rawData + (2 * i));
  return true;
}
...
XsGpsPvtData LegacyDataPacket::gpsPvtData(int32_t index) const {
  ...
  if (containsGpsPvtData(index)) {
    ...
    buffer.m_pressureAge =
        m_msg.getDataByte(m_fixedData->m_infoList[index].m_gpsPvtPressureAge);
    ...
    for (uint16_t i = 0; i < 10; ++i) {
      ...
      if (m_fixedData->m_infoList[index].m_gpsPvtGpsData !=
          XS_DATA_ITEM_NOT_AVAILABLE) {
        bareln[i] =
            m_msg.getDataLong(m_fixedData->m_infoList[index].m_gpsPvtGpsData +
                              (4 * i)); // lint !e661 !e662
      } else {
        bareln[i] = 0;
      }
    ...
    if (m_fixedData->m_infoList[index].m_gpsPvtGpsAge !=
        XS_DATA_ITEM_NOT_AVAILABLE) {
      buffer.m_gpsAge =
          m_msg.getDataByte(m_fixedData->m_infoList[index].m_gpsPvtGpsAge);
    } else {
      buffer.m_gpsAge = 0;
    }
  } else
...
bool LegacyDataPacket::setGpsPvtData(const XsGpsPvtData &data, int32_t index) {
  ...
  // update
  m_msg.setDataShort(data.m_pressure,
                     m_fixedData->m_infoList[index].m_gpsPvtPressure);
  m_msg.setDataByte(data.m_pressureAge,
                    m_fixedData->m_infoList[index].m_gpsPvtPressureAge);

  // lon,lat,height,hacc,vacc,veln,vele,veld
  int32_t *bareln = (int32_t *)&data.m_itow;
  for (uint16_t i = 0; i < 10; ++i)
    m_msg.setDataLong(bareln[i],
                      m_fixedData->m_infoList[index].m_gpsPvtGpsData +
                          (4 * i)); // lint !e661 !e662

  // gpsAge
  m_msg.setDataByte(data.m_gpsAge,
                    m_fixedData->m_infoList[index].m_gpsPvtGpsAge);
  return true;
}
...
XsPressure LegacyDataPacket::pressure(int32_t index) const {
  ...
  if (containsPressure(index)) {
    ...
    // pressure data
    buffer.m_pressure =
        (double)m_msg.getDataShort(m_fixedData->m_infoList[index].m_wBaroMeter);
    // pressAge
    buffer.m_pressureAge =
        (m_msg.getDataByte(m_fixedData->m_infoList[index].m_wAidingData) ? 0
                                                                         : 255);
  } else
    buffer = XsPressure();
  return buffer;
...
bool LegacyDataPacket::setPressure(const XsPressure &data, int32_t index) {
  ...
  // update
  m_msg.setDataShort((uint16_t)data.m_pressure,
                     m_fixedData->m_infoList[index].m_gpsPvtPressure);
  m_msg.setDataByte(data.m_pressureAge,
                    m_fixedData->m_infoList[index].m_gpsPvtPressureAge);
...
MtwSdiData LegacyDataPacket::mtwSdiData(int32_t index) const {
  ...
  if (containsMtwSdiData(index)) {
    ...
    buffer.m_firstFrameNumber =
        m_msg.getDataShort(m_fixedData->m_infoList[index].m_wFirstFrameNumber);
    buffer.m_lastFrameNumber =
        m_msg.getDataShort(m_fixedData->m_infoList[index].m_wLastFrameNumber);
    m_msg.getDataFPValue(CHECKIFDOUBLE(m_wCurrentBias),
                         &buffer.m_currentBias[0],
                         m_fixedData->m_infoList[index].m_wCurrentBias, 3);
    buffer.m_aidingData =
        (m_msg.getDataByte(m_fixedData->m_infoList[index].m_wAidingData) == 1)
            ? true
            : false;
    buffer.m_barometer =
        m_msg.getDataShort(m_fixedData->m_infoList[index].m_wBaroMeter) / 50.0;
    buffer.m_rssi =
        (int8_t)m_msg.getDataByte(m_fixedData->m_infoList[index].m_wRssi);

    m_msg.getDataFPValue(CHECKIFDOUBLE(m_wOrientationIncrement),
                         &buffer.m_orientationIncrement[0],
                         m_fixedData->m_infoList[index].m_wOrientationIncrement,
                         4);
    ...
    m_msg.getDataFPValue(CHECKIFDOUBLE(m_wMagnetoMeter),
                         &buffer.m_magnetoMeter[0],
...
bool LegacyDataPacket::setMtwSdiData(const MtwSdiData &data, int32_t index) {
  if (m_fixedData->m_infoList[index].m_mtwSdiData ==
    ...
    m_fixedData->m_infoList[index].m_wOrientationIncrement =
        (uint16_t)(m_fixedData->m_infoList[index].m_mtwSdiData + 9 + 3 * ds);
    m_fixedData->m_infoList[index].m_wVelocityIncrement =
        (uint16_t)(m_fixedData->m_infoList[index].m_mtwSdiData + 9 + 7 * ds);
    m_fixedData->m_infoList[index].m_wAidingData =
        (uint16_t)(m_fixedData->m_infoList[index].m_mtwSdiData + 9 + 10 * ds);
    m_fixedData->m_infoList[index].m_wBaroMeter =
        (uint16_t)(m_fixedData->m_infoList[index].m_mtwSdiData + 10 + 10 * ds);
    m_fixedData->m_infoList[index].m_wMagnetoMeter =
        (uint16_t)(m_fixedData->m_infoList[index].m_mtwSdiData + 12 + 10 * ds);
    m_fixedData->m_infoList[index].m_wRssi =
        (uint16_t)(m_fixedData->m_infoList[index].m_mtwSdiData + 12 + 13 * ds);
    m_fixedData->m_infoList[index].m_size += (uint16_t)(13 + 13 * ds);
  }
  // update
  m_msg.setDataLong(data.m_deviceId.toInt(),
                    m_fixedData->m_infoList[index].m_wClientId);
  m_msg.setDataByte(data.m_timeSync,
                    m_fixedData->m_infoList[index].m_wTimeSync);
  m_msg.setDataShort(data.m_firstFrameNumber,
                     m_fixedData->m_infoList[index].m_wFirstFrameNumber);
  m_msg.setDataShort(data.m_lastFrameNumber,
                     m_fixedData->m_infoList[index].m_wLastFrameNumber);
  m_msg.setDataFPValue(CHECKIFDOUBLE(m_wCurrentBias), data.m_currentBias.data(),
                       m_fixedData->m_infoList[index].m_wCurrentBias, 3);
  m_msg.setDataFPValue(CHECKIFDOUBLE(m_wOrientationIncrement),
                       data.m_orientationIncrement.data(),
                       m_fixedData->m_infoList[index].m_wOrientationIncrement,
                       4);
  m_msg.setDataFPValue(CHECKIFDOUBLE(m_wVelocityIncrement),
                       data.m_velocityIncrement.data(),
                       m_fixedData->m_infoList[index].m_wVelocityIncrement, 3);
  m_msg.setDataByte((data.m_aidingData == true) ? 1 : 0,
                    m_fixedData->m_infoList[index].m_wAidingData);
  m_msg.setDataShort((uint16_t)(XsMath_doubleToLong(data.m_barometer * 50)),
                     m_fixedData->m_infoList[index].m_wBaroMeter);
  m_msg.setDataFPValue(CHECKIFDOUBLE(m_wMagnetoMeter),
                       data.m_magnetoMeter.data(),
                       m_fixedData->m_infoList[index].m_wMagnetoMeter, 3);
...
double LegacyDataPacket::temperature(int32_t index, int channel) const {
  if (containsTemperature(index, channel))
    return m_msg.getDataFPValue(CHECKIFDOUBLE(m_temp[channel]),
...
bool LegacyDataPacket::setTemperature(const double &temp, int32_t index,
                                      int channel) {
  if (dataSize(index) == 0)
    return false;
  if (channel >= XS_MAX_TEMPERATURE_CHANNELS)
  ...
  // update
  m_msg.setDataFPValue(CHECKIFDOUBLE(m_temp[channel]), temp,
                       m_fixedData->m_infoList[index].m_temp[channel]);
  return true;
}
...
XsVector LegacyDataPacket::calibratedAcceleration(int32_t index) const {
  ...
  if (containsCalibratedAcceleration(index))
    m_msg.getDataFPValue(CHECKIFDOUBLE(m_calAcc), &buffer[0],
...
bool LegacyDataPacket::setCalibratedAcceleration(const XsVector &vec,
  ...
  // update
  m_msg.setDataFPValue(CHECKIFDOUBLE(m_calAcc), vec.data(),
...
XsVector LegacyDataPacket::calibratedGyroscopeData(int32_t index) const {
  ...
  if (containsCalibratedGyroscopeData(index))
    m_msg.getDataFPValue(CHECKIFDOUBLE(m_calGyr), &buffer[0],
...
bool LegacyDataPacket::setCalibratedGyroscopeData(const XsVector &vec,
  ...
  // update
  m_msg.setDataFPValue(CHECKIFDOUBLE(m_calGyr), vec.data(),
...
XsVector LegacyDataPacket::calibratedMagneticField(int32_t index) const {
  ...
  if (containsCalibratedMagneticField(index))
    m_msg.getDataFPValue(CHECKIFDOUBLE(m_calMag), &buffer[0],
...
bool LegacyDataPacket::setCalibratedMagneticField(const XsVector &vec,
  ...
  // update
  m_msg.setDataFPValue(CHECKIFDOUBLE(m_calMag), vec.data(),
...
XsCalibratedData LegacyDataPacket::calibratedData(int32_t index) const {
  ...
  if (containsCalibratedData(index)) {
    ...
    if (m_fixedData->m_infoList[index].m_calAcc == XS_DATA_ITEM_NOT_AVAILABLE)
      ...
      m_msg.getDataFPValue(CHECKIFDOUBLE(m_calAcc), bare,
                           m_fixedData->m_infoList[index].m_calAcc, 3);
    ...
    if (m_fixedData->m_infoList[index].m_calGyr == XS_DATA_ITEM_NOT_AVAILABLE)
      ...
      m_msg.getDataFPValue(CHECKIFDOUBLE(m_calGyr), bare,
                           m_fixedData->m_infoList[index].m_calGyr, 3);
    ...
    if (m_fixedData->m_infoList[index].m_calMag == XS_DATA_ITEM_NOT_AVAILABLE)
      ...
      m_msg.getDataFPValue(CHECKIFDOUBLE(m_calMag), bare,
                           m_fixedData->m_infoList[index].m_calMag, 3);
  } else {
    buffer.m_acc.zero();
    buffer.m_gyr.zero();
...
bool LegacyDataPacket::containsCalibratedData(int32_t index) const {
  if (dataSize(index) == 0)
    return false;
  return (m_fixedData->m_infoList[index].m_calData !=
          XS_DATA_ITEM_NOT_AVAILABLE);
}
...
bool LegacyDataPacket::setCalibratedData(const XsCalibratedData &data,
  ...
  if (m_fixedData->m_infoList[index].m_calData == XS_DATA_ITEM_NOT_AVAILABLE ||
      !ISDOUBLE(m_calData)) {
    // add
  ...
  if (m_fixedData->m_infoList[index].m_calAcc != XS_DATA_ITEM_NOT_AVAILABLE)
    m_msg.setDataFPValue(CHECKIFDOUBLE(m_calAcc), bare,
                         m_fixedData->m_infoList[index].m_calAcc, 3);
  bare += 3;
  if (m_fixedData->m_infoList[index].m_calGyr != XS_DATA_ITEM_NOT_AVAILABLE)
    m_msg.setDataFPValue(CHECKIFDOUBLE(m_calGyr), bare,
                         m_fixedData->m_infoList[index].m_calGyr, 3);
  bare += 3;
  if (m_fixedData->m_infoList[index].m_calMag != XS_DATA_ITEM_NOT_AVAILABLE)
    m_msg.setDataFPValue(CHECKIFDOUBLE(m_calMag), bare,
                         m_fixedData->m_infoList[index].m_calMag, 3);
...
XsQuaternion LegacyDataPacket::orientationQuaternion(int32_t index) const {
  ...
  if (containsOrientationQuaternion(index))
    m_msg.getDataFPValue(CHECKIFDOUBLE(m_oriQuat), &buffer[0],
...
bool LegacyDataPacket::setOrientationQuaternion(const XsQuaternion &data,
  ...
  if (m_fixedData->m_infoList[index].m_oriQuat == XS_DATA_ITEM_NOT_AVAILABLE ||
    ...
    double *bare = (double *)&data;
    m_msg.setDataFPValue((m_fixedData->m_formatList[index].m_outputSettings &
                          XOS_Dataformat_Mask) |
                             XOS_Dataformat_Double,
                         bare, m_fixedData->m_infoList[index].m_oriQuat,
                         numValues);
    return true;
  }
  // update
  m_msg.setDataFPValue(CHECKIFDOUBLE(m_oriQuat), data.data(),
...
XsEuler LegacyDataPacket::orientationEuler(int32_t index) const {
  ...
  if (containsOrientationEuler(index)) {
    ...
    m_msg.getDataFPValue(CHECKIFDOUBLE(m_oriEul), &buffer[0],
...
bool LegacyDataPacket::setOrientationEuler(const XsEuler &data, int32_t index) {
  ...
  // update
  m_msg.setDataFPValue(CHECKIFDOUBLE(m_oriEul), data.data(),
  ...
  if (containsOrientationMatrix(index)) {
    ...
    for (int32_t i = 0; i < 3; ++i)
      for (int32_t j = 0; j < 3; ++j, k += ds)
        buffer.setValue(j, i, m_msg.getDataFPValue(
...
bool LegacyDataPacket::setOrientationMatrix(const XsMatrix &data,
  ...
  for (int32_t i = 0; i < 3; ++i)
    for (int32_t j = 0; j < 3; ++j, k += ds)
      m_msg.setDataFPValue(CHECKIFDOUBLE(m_oriMat), data.value(j, i),
                           m_fixedData->m_infoList[index].m_oriMat + k);
  return true;
}
...
XsVector LegacyDataPacket::positionLLA(int32_t index) const {
  ...
  if (containsPositionLLA(index))
    m_msg.getDataFPValue(CHECKIFDOUBLE(m_posLLA), &buffer[0],
...
bool LegacyDataPacket::setPositionLLA(const XsVector &data, int32_t index) {
  ...
  // update
  m_msg.setDataFPValue(CHECKIFDOUBLE(m_posLLA), data.data(),
...
XsVector LegacyDataPacket::velocity(int32_t index) const {
  ...
  if (containsVelocity(index))
    m_msg.getDataFPValue(CHECKIFDOUBLE(m_velNEDorNWU), &buffer[0],
...
bool LegacyDataPacket::setVelocity(const XsVector &data, int32_t index) {
  ...
  // update
  m_msg.setDataFPValue(CHECKIFDOUBLE(m_velNEDorNWU), data.data(),
...
uint32_t LegacyDataPacket::status(int32_t index, bool *outIsDetailed) const {
  ...
  if (containsStatus(index)) {
    if (m_fixedData->m_infoList[index].m_detailedStatus ==
      ...
      *outIsDetailed = true;
      uint32_t tmp =
          m_msg.getDataLong(m_fixedData->m_infoList[index].m_detailedStatus);
      return tmp;
    }
  } else {
    return 0;
...
bool LegacyDataPacket::setUtcTime(const XsUtcTime &data, int32_t index) {
  ...
  for (uint16_t i = 0; i < 6; ++i)
    m_msg.setDataByte(bareByte[i], m_fixedData->m_infoList[index].m_utcMonth +
                                       i); // lint !e661 !e662
...
XsVector LegacyDataPacket::freeAcceleration(int32_t index) const {
  ...
  if (containsFreeAcceleration(index))
    m_msg.getDataFPValue(CHECKIFDOUBLE(m_acc_g), &buffer[0],
...
bool LegacyDataPacket::setFreeAcceleration(const XsVector &g, int32_t index) {
  ...
  // update
  m_msg.setDataFPValue(CHECKIFDOUBLE(m_acc_g), g.data(),
...
bool LegacyDataPacket::containsTriggerIndication(int channelID /*= 0*/) const {
  switch (channelID) {
  case 0:
    return (m_triggerIn1.msTime() == 0 && m_triggerIn2.msTime() == 0) ? false
                                                                      : true;
  case 1:
