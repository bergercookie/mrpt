...
XsResultValue SerialInterface::close(void) { return closeLive(); }
//! \brief Close the serial communication port.
XsResultValue SerialInterface::closeLive(void)
{
#ifdef LOG_RX_TX
	if (rx_log != nullptr) fclose(rx_log);
	if (tx_log != nullptr) fclose(tx_log);
	rx_log = nullptr;
	tx_log = nullptr;
#endif
	if (!isOpen()) return m_lastResult = XRV_NOPORTOPEN;

	m_lastResult = XRV_OK;
#ifdef _WIN32
	if (::FlushFileBuffers(m_handle))
	{
		// read all data before closing the handle, a Flush is not enough for
		// FTDI
		// devices unfortunately
		// we first need to set the COMM timeouts to instantly return when no
		// more
		// data is available
		COMMTIMEOUTS cto;
	...
	if (!::CloseHandle(m_handle)) m_lastResult = XRV_ERROR;
	m_handle = INVALID_HANDLE_VALUE;
#else
	flushData();
	::close(m_handle);
...
*/
XsResultValue SerialInterface::escape(
	const XsControlLine mask, const XsControlLine state)
{
...
/*! \brief Open a communication channel to the given port info.
	\details If the baudrate in \a portInfo is set to XBR_Invalid, the baud rate
   is automatically
				detected if possible.
	\param portInfo The details of the port that should be opened. Depending on
   the type of interface,
				parts of this parameter may be ignored.
	\param readBufSize The size of the read buffer in bytes (if appliccable to
   the device)
	\param writeBufSize The size of the write buffer in bytes (if appliccable to
   the device)
	\returns XRV_OK if the device was opened successfully
*/
XsResultValue SerialInterface::open(
	const XsPortInfo& portInfo, uint32_t readBufSize, uint32_t writeBufSize)
{
	m_endTime = 0;

	JLDEBUG(
		gJournal, "port " << portInfo.portName().toStdString() << " at "
	...
	if (setTimeout(20)) fail = m_lastResult;

	// Other initialization functions
	if (!EscapeCommFunction(m_handle, SETDTR))  // Set DTR (Calibration sensors
		// need DTR to startup, won't hurt
		// otherwise
		fail = XRV_ERROR;
	...
	// Check if the file is already opened by someome else (other
	// thread/process)
	...
	if (tcgetattr(m_handle, &m_commState) != 0) return XRV_ERROR;

	// Set baudrate.
	if (cfsetispeed(&m_commState, portInfo.baudrate()) != 0) return XRV_ERROR;

	if (cfsetospeed(&m_commState, portInfo.baudrate()) != 0) return XRV_ERROR;
	...
	if (tcgetattr(m_handle, &checkCommState) != 0) return XRV_ERROR;
		...
		(m_commState.c_cc[VTIME] != checkCommState.c_cc[VTIME]))
	{
		JLDEBUG(
			gJournal,
			"commstates do not match, which is OK for USB connected MkIV "
			"devices");
	}
	...
	JLDEBUG(
		gJournal, "Port " << portInfo.portName().toStdString() << " opened");
	return (m_lastResult = XRV_OK);
}
...
XsResultValue SerialInterface::readData(XsSize maxLength, XsByteArray& data)
{
	...
	BOOL rres =
		::ReadFile(m_handle, data.data(), (DWORD)maxLength, &length, nullptr);
	...
	if (length == 0) return (m_lastResult = XRV_TIMEOUT);
#else
	fd_set fd;
	fd_set err;
	timeval timeout;
	...
	if (length > 0)
	{
		if (rx_log == nullptr)
		{
			...
			sprintf(
				fname, "rx_%s_%d.log", devname + 1,
				XsBaud::rateToNumeric(m_baudrate));
#endif
...
XsResultValue SerialInterface::setTimeout(const uint32_t ms)
{
	...
	if (!GetCommTimeouts(
			m_handle, &commTimeouts))  // Fill CommTimeouts structure
		return m_lastResult = XRV_ERROR;
	...
	if (!SetCommTimeouts(
			m_handle, &commTimeouts))  // Set CommTimeouts structure
		return m_lastResult = XRV_ERROR;
#else
	...
	if (isOpen()) tcsetattr(m_handle, TCSANOW, &m_commState);
#endif
	return (m_lastResult = XRV_OK);
}
...
/*! \brief Wait for data to arrive or a timeout to occur.
   ...
   stored
	...
	\returns XRV_OK if \a maxLength bytes were read, XRV_TIMEOUT if less was
   read, XRV_TIMEOUTNODATA if nothing was read
*/
XsResultValue SerialInterface::waitForData(XsSize maxLength, XsByteArray& data)
{
	...
	while ((data.size() < maxLength) &&
		   (XsTime_getTimeOfDay(nullptr, nullptr) <= eTime))
	{
		XsByteArray raw;
	...
	JLTRACE(
		gJournal, "Read " << data.size() << " of " << maxLength << " bytes");
...
*/
XsResultValue SerialInterface::writeData(
	const XsByteArray& data, XsSize* written)
{
	XsSize bytes;
	...
	*written = 0;
	...
	DWORD lwritten = 0;
	if (WriteFile(
			m_handle, data.data(), (DWORD)data.size(), &lwritten, nullptr) == 0)
		return (m_lastResult = XRV_ERROR);

	*written = lwritten;
#else
	ssize_t result = write(m_handle, (const void*)data.data(), data.size());
	if (result < 0) return (m_lastResult = XRV_ERROR);
...
/* This function is only available on Windows Vista and higher.
   When a read action hangs, this function can cancel IO operations from another
   thread.
*/
