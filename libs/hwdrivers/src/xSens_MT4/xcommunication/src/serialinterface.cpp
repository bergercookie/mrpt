...
XsResultValue SerialInterface::closeLive(void) {
  ...
  if (::FlushFileBuffers(m_handle)) {
    // read all data before closing the handle, a Flush is not enough for FTDI
    // devices unfortunately
    // we first need to set the COMM timeouts to instantly return when no more
    // data is available
    COMMTIMEOUTS cto;
...
/*! \brief Manipulate the Serial control lines
   ...
   win32.
        \param mask		Indicates which lines are to be manipulated
   and which should be
                                        left alone.
        \param state	Contains the new state of the control lines.
        \returns XRV_OK if the function succeeded
*/
XsResultValue SerialInterface::escape(const XsControlLine mask,
                                      const XsControlLine state) {
...
/*! \copydoc IoInterface::flushData
        \note This function tries to send and receive any remaining data
   immediately
...
/*! \brief Open a communication channel to the given port info.
        \details If the baudrate in \a portInfo is set to XBR_Invalid, the baud
   ...
   on the type of interface,
        ...
        \param readBufSize The size of the read buffer in bytes (if appliccable
   to the device)
        \param writeBufSize The size of the write buffer in bytes (if
   appliccable to the device)
...
XsResultValue SerialInterface::open(const XsPortInfo &portInfo,
  ...
  JLDEBUG(gJournal, "port " << portInfo.portName().toStdString() << " at "
  ...
  if (m_handle == INVALID_HANDLE_VALUE) {
    JLDEBUG(gJournal, "Port " << portInfo.portName().toStdString()
                              << " cannot be opened");
...
XsResultValue SerialInterface::readData(XsSize maxLength, XsByteArray &data) {
  ...
  if (length > 0) {
    if (rx_log == nullptr) {
      ...
      sprintf(fname, "rx_%s_%d.log", devname + 1,
...
#ifdef _WIN32
  DWORD lwritten = 0;
  if (WriteFile(m_handle, data.data(), (DWORD)data.size(), &lwritten,
                nullptr) == 0)
    return (m_lastResult = XRV_ERROR);
...
#else
      char *devname = strrchr(m_portname, '/');
      sprintf(fname, "tx_%s_%d.log", devname + 1,
              XsBaud::rateToNumeric(m_baudrate));
#endif
      makeFilenameUnique(fname);
