...
/*! \class ProtocolHandler
	...
	This class' purpose is to get valid messages according to its protocol from
   the raw
	data that is supplied to it. The default implementation (ProtocolHandler)
   implements
...
/*! \brief Compute the expected message size given a possibly incomplete message
   ...
   function
	will return the minimum size of the message given the information that is
   available
*/
int expectedMessageSize(const unsigned char* buffer, int sz)
{
	...
	{
		if (sz < 6)
			return XS_EXTLENCODE +
				   XS_LEN_MSGEXTHEADERCS;  // typical minimum size at
...
/*! \copydoc IProtocolHandler::findMessage
   ...
   without garbage, the scan
	is implemented in a rather naive and simple way. If we can expect lots of
   garbage in the data
	stream, this should probably be looked into.
*/
MessageLocation ProtocolHandler::findMessage(
	XsMessage& rcv, const XsByteArray& raw) const
{
	...
	for (int pre = 0; pre < bufferSize; ++pre)
	{
		if (buffer[pre] == XS_PREAMBLE)
		{
			...
			if (rv.m_startPos == -1) rv.m_startPos = (int32_t)pre;
			int remaining =
				bufferSize -
				pre;  // remaining bytes in buffer INCLUDING preamble

			if (remaining < XS_LEN_MSGHEADERCS)
			{
				JLTRACE(gJournal, "Not enough header data read");
				if (rv.m_startPos != pre) continue;
			...
			{
				// found 'valid' message that isn't actually valid... happens
				// inside GPS
				// raw data
			...
			JLTRACE(
				gJournal, "Bytes in buffer=" << remaining
											 << ", full target = " << target);
			...
			// we have read enough data to fulfill our target so we'll try to
			// parse
			// the message
			...
			if (rcv.loadFromString(msgStart, (uint16_t)target))
			{
				...
				rv.m_startPos =
					pre;  // we do this again here because this may not be
				// the first preamble encountered (the check for
				// -1 at the start of the loop is necessary)
				return rv;
			}

			// we could not read the message, clear message and try next
			// preamble
			rcv.clear();
			if (rv.m_startPos == pre)
			{
				rv.m_startPos = -1;
				JLALERT(
					gJournal, "Invalid checksum"
								  << std::hex << std::setfill('0')
								  << " First bytes " << std::setw(2)
								  << (int)msgStart[0] << " " << std::setw(2)
