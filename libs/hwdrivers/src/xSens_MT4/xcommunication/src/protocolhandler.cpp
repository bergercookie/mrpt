...
/*! \class ProtocolHandler
        ...
        This class' purpose is to get valid messages according to its protocol
   from the raw
        data that is supplied to it. The default implementation
   (ProtocolHandler) implements
        the Xsens message protocol. To use a different protocol, overload the
   findMessage function.

        The class is intended to be state-less with respect to the data it
   handles.
*/
...
/*! \brief Compute the expected message size given a possibly incomplete message
        \details When analyzing a message with incomplete size information, the
   function
        will return the minimum size of the message given the information that
   is available
*/
int expectedMessageSize(const unsigned char *buffer, int sz) {
  const XsMessageHeader *hdr = (const XsMessageHeader *)buffer;
  if (sz < 4)
    return XS_LEN_MSGHEADERCS; // no size information available at all, return a
                               // minimum message

  // we have size information
  if (hdr->m_length == XS_EXTLENCODE) {
    if (sz < 6)
      return XS_EXTLENCODE + XS_LEN_MSGEXTHEADERCS; // typical minimum size at
                                                    // which point extended size
                                                    // is needed

    return XS_LEN_MSGEXTHEADERCS +
...
/*! \copydoc IProtocolHandler::findMessage
        \todo Since the assumption is that we receive a stream of valid messages
   without garbage, the scan
        is implemented in a rather naive and simple way. If we can expect lots
...
MessageLocation ProtocolHandler::findMessage(XsMessage &rcv,
  ...
  for (int pre = 0; pre < bufferSize; ++pre) {
    if (buffer[pre] == XS_PREAMBLE) {
      ...
      int remaining =
          bufferSize - pre; // remaining bytes in buffer INCLUDING preamble
      ...
      if (hdr->m_length == XS_EXTLENCODE) {
        ...
        // found 'valid' message that isn't actually valid... happens inside GPS
        // raw data
      ...
      JLTRACE(gJournal, "Bytes in buffer=" << remaining
                                           << ", full target = " << target);
      ...
      if (remaining < target) {
        ...
        JLTRACE(gJournal, "Not enough data read: " << remaining << " / "
                                                   << target);
        if (rv.m_size == 0)
          rv.m_size = -target;
        continue;
      ...
      // we have read enough data to fulfill our target so we'll try to parse
      // the message
      ...
      if (rcv.loadFromString(msgStart, (uint16_t)target)) {
        JLTRACE(gJournal,
                "OK, size = " << (int)rcv.getTotalMessageSize() << std::hex
                              << std::setfill('0') << " First bytes "
                              << std::setw(2) << (int)msgStart[0] << " "
                              << std::setw(2) << (int)msgStart[1] << " "
                              << std::setw(2) << (int)msgStart[2] << " "
                              << std::setw(2) << (int)msgStart[3] << " "
                              << std::setw(2) << (int)msgStart[4] << std::dec
                              << std::setfill(' '));
        rv.m_size = (int)rcv.getTotalMessageSize();
        rv.m_startPos = pre; // we do this again here because this may not be
                             // the first preamble encountered (the check for -1
                             // at the start of the loop is necessary)
        return rv;
      }
      ...
      if (rv.m_startPos == pre) {
        ...
        JLALERT(gJournal,
                "Invalid checksum"
                    << std::hex << std::setfill('0') << " First bytes "
                    << std::setw(2) << (int)msgStart[0] << " " << std::setw(2)
                    << (int)msgStart[1] << " " << std::setw(2)
                    << (int)msgStart[2] << " " << std::setw(2)
                    << (int)msgStart[3] << " " << std::setw(2)
                    << (int)msgStart[4] << std::dec << std::setfill(' '));
      }
    }
  }
...
/*! \brief Returns the minimum size of a valid message of this protocol
 * including preambles and checksums */
int ProtocolHandler::minimumMessageSize() const {
  return XS_LEN_MSGHEADERCS; // minimum size of xsens xbus protocol message
}

/*! \brief Returns the maximum size of a valid message of this protocol
 * including preambles and checksums */
int ProtocolHandler::maximumMessageSize() const {
  return XS_LEN_MSGEXTHEADERCS +
         XS_MAXDATALEN; // maximum size of xsens xbus protocol message
}
...
int ProtocolHandler::composeMessage(XsByteArray &raw, const XsMessage &msg) {
  if (msg.getTotalMessageSize() <
      5) // minimum size of an xsens message including envelope is 5 bytes
    return -1;
