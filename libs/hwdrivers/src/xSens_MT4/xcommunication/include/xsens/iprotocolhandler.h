...
public:
  ...
  /*! \brief Find the first message in the \a raw byte stream
          \details This function scans \a raw for a sequence of bytes that it
     can convert into an
          %XsMessage object. It returns the location and total byte size of the
     message so that the
          caller can remove those bytes from the stream. The return value can
     also describe that a
  ...
  */
  virtual MessageLocation findMessage(XsMessage &rcv,
