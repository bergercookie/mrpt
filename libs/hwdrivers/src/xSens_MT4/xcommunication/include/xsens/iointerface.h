...
struct IoInterface
{
	...
	*/
	...
	/*! \brief Write the data contained in \a data to the device
		...
		\param written An optional %XsSize value that will receive the number of
	   bytes that were
		actually written.
		\returns XRV_OK if the data was written successfully
	*/
	virtual XsResultValue writeData(
		const XsByteArray& data, XsSize* written = nullptr) = 0;
	...
	/*! \brief Read at most \a maxLength bytes from the device into \a data
	   ...
	   device type and timeout
		settings, the function may return with less than this number of bytes
	   read.
		\param data A buffer that will contain the read data.
		\returns XRV_OK if all data was read successfully, XRV_TIMEOUT if some
	   data was read, but not
