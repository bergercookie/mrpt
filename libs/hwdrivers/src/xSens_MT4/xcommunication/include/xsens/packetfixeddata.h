...
/*! \brief Contains offset information about data in the packet
        \details All items are initialized to \a XS_DATA_ITEM_NOT_AVAILABLE and
   set to their proper
        value by LegacyDataPacket::updateInfoList and/or the LegacyDataPacket
   update functions.
*/
struct PacketInfo {
  ...
  uint16_t m_rawTemp[XS_MAX_TEMPERATURE_CHANNELS]; //!< Offset of raw
                                                   //!temperature data. Note
                                                   //!that usually only the
                                                   //!first item is used.
  uint16_t m_temp[XS_MAX_TEMPERATURE_CHANNELS];    //!< Offset of calibrated
                                                //!temperature data. Note that
                                                //!usually only the first item
                                                //!is used.
  ...
  uint16_t
      m_gpsPvtHacc; //!< Offset of raw GPS horizontal accuracy estimate data
  ...
  uint16_t m_wFirstFrameNumber; //!< Offset of MTw SDI first frame number in
                                //!interval data
  uint16_t m_wLastFrameNumber;  //!< Offset of MTw SDI last frame number in
                                //!interval data
  uint16_t m_wCurrentBias;      //!< Offset of MTw SDI gyroscope bias data
  uint16_t
      m_wOrientationIncrement; //!< Offset of MTw SDI orientation increment data
  ...
  uint16_t m_doubleBoundary;     //!< Boundary where the original data format is
                                 //!ignored and values are stored in double
                                 //!precision
...
struct PacketFixedData {
  ...
  PacketInfo *m_infoList; //!< Contains information about data in the packet and
