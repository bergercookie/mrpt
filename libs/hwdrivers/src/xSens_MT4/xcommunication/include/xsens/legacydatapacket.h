...
struct LegacyDataPacket
{
   public:
	...
	bool setDataFormat(
		XsOutputMode outputMode, XsOutputSettings outputSettings,
		int32_t index = 0);
	...
	*/
	...
	/*! \brief Return the Status component of a data item.
		\param index The index of the item of which the data should be returned.
		\param[out] outIsDetailed pointer to boolean. After call, the boolean is
	   true if the status is detailed
