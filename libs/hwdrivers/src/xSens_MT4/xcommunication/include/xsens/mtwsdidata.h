...
struct MtwSdiData {
  ...
  uint16_t m_firstFrameNumber; //!< The first frame number of the SDI interval.
                               //!The time of the interval is [first, last)
  uint16_t m_lastFrameNumber;  //!< The last frame number of the SDI interval.
                               //!The time of the interval is [first, last)
  XsVector3 m_currentBias; //!< The gyroscope bias used during the SDI interval
  XsQuaternion m_orientationIncrement; //!< The orientation increment (delta Q)
                                       //!over the interval
  XsVector3 m_velocityIncrement; //!< The velocity increment (delta V) over the
                                 //!interval
  ...
  int8_t
      m_rssi; //!< The Received Signal Strength Indication (RSSI) of the message
