	...
	*/
	...
	/*! \brief This function resizes the contained string to the desired size,
	   while retaining its contents
		\param count The desired size of the string. This excludes the
	   terminating
	   0 character.
		\sa XsString_resize
	*/
...
struct XsString : public XsStringType
{
	...
	//! \brief Constructs an XsInt64Array that references the data supplied in
	//! \a
	//! ref
	...
	{
		if (!src.empty()) XsString_assign(this, src.size() + 1, src.c_str());
	}
