...
XSTYPES_DLL_API void XsString_assign(XsString *thisPtr, XsSize count,
                                     const char *src);
...
XSTYPES_DLL_API void XsString_erase(XsString *thisPtr, XsSize index,
                                    XsSize count);
...
struct XsStringType : public XsArrayImpl<char, g_xsStringDescriptor, XsString> {
  ...
  /*! \brief Returns the number of items currently in the array, excluding the
     terminating 0
  ...
  */
  ...
  inline XsSize reserved() const { return m_reserved ? m_reserved - 1 : 0; }
  ...
  /*! \brief Insert \a count \a items at \a index in the string
      ...
      \param index The index to use for inserting. Anything beyond the end of
     the string (ie. -1) will
  ...
  */
  ...
  /*! \brief Assign the characters in \a src to the string
      \details This function is resizes the string to fit \a count characters
     and copies those from \a src.
      \param count The number of characters in src. If this value is 0 and \a
     scr is not 0, the value is
                  determined automatically by looking through src until a
     terminating 0 is found.
      \param src The source string to copy from. If this is 0 and \a count is
     not 0, the string will be
  ...
  */
  ...
  /*! \brief This function resizes the contained string to the desired size,
     while retaining its contents
      \param count The desired size of the string. This excludes the terminating
     0 character.
      \sa XsString_resize
  */
  inline void resize(XsSize count) { XsString_resize((XsString *)this, count); }
...
struct XsString : public XsStringType {
  ...
  //! \brief Constructs an XsInt64Array that references the data supplied in \a
  //! ref
  ...
  inline bool operator!=(char const *str) const { return !(*this == str); }
