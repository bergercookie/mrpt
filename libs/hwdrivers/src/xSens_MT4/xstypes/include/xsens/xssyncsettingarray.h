...
XsSyncSettingArray_construct(XsSyncSettingArray *thisPtr, XsSize count,
                             struct XsSyncSetting const *src);
#else
} // extern "C"
#endif
...
struct XsSyncSettingArray
  ...
  inline explicit XsSyncSettingArray(XsSize sz = 0,
                                     XsSyncSetting const *src = 0)
      : ArrayImpl(sz, src) {}
  ...
  //! \brief Constructs an XsSyncSettingArray that references the data supplied
  //! in \a ref
  inline explicit XsSyncSettingArray(XsSyncSetting *ref, XsSize sz,
