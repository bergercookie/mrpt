...
#ifndef __cplusplus
#define XSSYNCSETTINGSARRAY_INITIALIZER \
	XSARRAY_INITIALIZER(&g_xsSyncSettingArrayDescriptor)
...
XSTYPES_DLL_API void XsSyncSettingArray_construct(
	XsSyncSettingArray* thisPtr, XsSize count, struct XsSyncSetting const* src);
#else
}  // extern "C"
#endif
...
struct XsSyncSettingArray
	: public XsArrayImpl<XsSyncSetting, g_xsSyncSettingArrayDescriptor,
						 XsSyncSettingArray>
{
	//! \brief Constructs an XsSyncSettingArray
	inline explicit XsSyncSettingArray(
		XsSize sz = 0, XsSyncSetting const* src = 0)
		: ArrayImpl(sz, src)
	{
	}
	...
	//! \brief Constructs an XsSyncSettingArray that references the data
	//! supplied
	//! in \a ref
	inline explicit XsSyncSettingArray(
		XsSyncSetting* ref, XsSize sz, XsDataFlags flags = XSDF_None)
		: ArrayImpl(ref, sz, flags)
	{
	}

#ifndef XSENS_NOITERATOR
	//! \brief Constructs an XsSyncSettingArray with the array bound by the
