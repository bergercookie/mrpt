...
/*! \brief This object describes how to treat the data in an array.
        \details Ususally there is one static instance per type of array that
   will be used by all
...
protected:
  ...
  void (*itemSwap)(void *a, void *b); //!< The function to use for swapping the
                                      //!data of two array items. \param a
                                      //!Pointer to first item to swap. \param b
                                      //!Pointer to second item to swap.
  void (*itemConstruct)(void *e); //!< The function to use for constructing a
                                  //!new array item. May be 0 for simple types.
                                  //!\param e Pointer to item to construct.
  void (*itemCopyConstruct)(void *e, void const *s); //!< The function to use
                                                     //!for constructing a new
                                                     //!array item with a source
                                                     //!initializer. This may
                                                     //!not be 0. \param e
                                                     //!Pointer to item to
                                                     //!construct. \param s
                                                     //!Pointer to source item
                                                     //!to copy from.
  void (*itemDestruct)(void *e); //!< The function to use for destructing a
                                 //!array item. May be 0 for simple types.
                                 //!\param e Pointer to item to destruct.
  void (*itemCopy)(void *to, void const *from); //!< The function to use for
                                                //!copying the data of \a from
                                                //!to \a to. \param to Pointer
                                                //!to item to copy to. \param
                                                //!from Pointer to item to copy
                                                //!from.
  int (*itemCompare)(void const *a,
                     void const *b); //!< The function to use for comparing two
                                     //!items. \param a Left hand side of
                                     //!comparison. \param b Right hand side of
                                     //!comparison. \returns The function will
                                     //!return 0 when the items are equal. When
                                     //!greater/less comparison is possible, the
                                     //!function should return < 0 if a < b and
                                     //!> 0 if a > b.
};
...
XsArray_construct(void *thisPtr, XsArrayDescriptor const *const descriptor,
                  XsSize count, void const *src);
XSTYPES_DLL_API void XsArray_copyConstruct(void *thisPtr, void const *src);
XSTYPES_DLL_API void XsArray_destruct(void *thisPtr);
XSTYPES_DLL_API void XsArray_assign(void *thisPtr, XsSize count,
                                    void const *src);
...
XSTYPES_DLL_API void XsArray_insert(void *thisPtr, XsSize index, XsSize count,
                                    void const *src);
...
struct XsArray {
  ...
  inline XsArray(XsArrayDescriptor const *descriptor, XsSize count = 0,
                 void const *src = 0)
  ...
  //! \brief Creates a array that references the data supplied in \a ref without
  //! allocating the data itself
  inline explicit XsArray(XsArrayDescriptor const *descriptor, void *ref,
                          XsSize count, XsDataFlags flags)
      : m_data(ref), m_size(count), m_reserved(count), m_flags(flags),
...
/*! \brief A C++ wrapper for XsArray, this is a template class for the actual
   specialized classes
        ...
        \tparam D The descriptor to use for this specific array implementation.
   This must be statically allocated and its lifetime must encompass the
   lifetime of objects that use it.
        \tparam I The class that inherits from the XsArrayImpl. Some functions
   (such as the streaming operator) require the inheriting type to be returned
...
struct XsArrayImpl : protected XsArray { // MRPT
  ...
  inline explicit XsArrayImpl<T, D, I>(T *ref, XsSize sz,
...
protected:
  ...
  template <ptrdiff_t F, typename R, typename Derived> struct IteratorImplBase {
  public:
...
public:
  ...
  /*! \brief STL-style const_iterator to the first data item past the end of the
   * array */
  ...
  */
  ...
  /*! \brief Adds \a item to the end of the array \sa XsArray_insert \param item
   * The item to append to the array. */
  inline void push_back(T const &item) { insert(&item, (XsSize)-1, 1); }
  /*! \brief Removes \a count items from the end of the array \sa XsArray_erase
   * \param count The number items to remove */
  ...
  /*! \brief Removes \a count items from the start of the array \param count The
   * number items to remove \sa XsArray_erase */
  ...
  */
  ...
  /*! \brief Removes \a count items from the array starting at \a index. \param
   * index The index of the first item to remove. \param count The number of
   * items to remove. */
  inline void erase(XsSize index, XsSize count = 1) {
    XsArray_erase(this, index, count);
  }
#ifndef XSENS_NOITERATOR
  /*! \brief Removes the item at iterator \a it. \details \param it The item to
   * remove. \returns An iterator pointing to the next item after the erased
   * item. */
  ...
  /*! \brief Set the size of the array to \a count. \details The contents of the
   * array after this operation are undefined. \param count The desired new size
   * fo the array. \sa XsArray_assign \sa reserve \sa resize */
  ...
  /*! \brief Assignment operator, copies \a other into this, overwriting the old
   * contents \param other The array to copy from \returns A reference to this
   * \sa XsArray_copy */
  ...
  /*! \brief Returns whether the array is empty. \details This differs slightly
   * from a straight check for size() != 0 in that it also works for fixed-size
   * XsArrays. \returns true if the array is empty. */
...
#endif
  /*! \brief Swap the contents of the array with those of \a other. \param other
   * The array to swap contents with. \sa XsArray_swap*/
...
*/
  ...
  inline int find(T const &needle) const { return XsArray_find(this, &needle); }
#ifndef XSENS_NOITERATOR
  /*! \brief Returns the linear index of \a it in the array
          \param it The iterator to analyze
          \returns The linear forward index of the item pointed to by \a it. If
     \a it points to before the
...
protected: // MRPT
           /*! \internal
              ...
              specified in \a descriptor
                   \param descriptor The array descriptor that contains the size of a
              array item
