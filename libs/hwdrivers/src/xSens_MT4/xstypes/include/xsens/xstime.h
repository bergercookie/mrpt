...
XSTYPES_DLL_API void XsTime_getDateAsWString(wchar_t *dest,
                                             struct tm const *date);
XSTYPES_DLL_API void XsTime_getTimeAsWString(wchar_t *dest,
...
inline int64_t getDateTime(tm *date = 0) { return XsTime_getDateTime(date); }
...
inline void msleep(uint32_t ms) { XsTime_msleep(ms); }

//! \copydoc XsTime_udelay
inline void udelay(uint32_t us) { XsTime_udelay(us); }

//! \copydoc XsTime_initializeTime
inline void initializeTime() { XsTime_initializeTime(); }
