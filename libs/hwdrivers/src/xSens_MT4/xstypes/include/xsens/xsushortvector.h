...
#ifndef __cplusplus
#define XSUSHORTVECTOR_INITIALIZER                                             \
  {                                                                            \
    { 0, 0, 0 }                                                                \
  }
#else
...
struct XsUShortVector {
  ...
  //! \brief Constructor that creates the vector with all components set to
  //! given values \a v1 \a v2 and \a v3
  inline XsUShortVector(unsigned short v1, unsigned short v2,
                        unsigned short v3) {
    m_data[0] = v1;
    m_data[1] = v2;
    m_data[2] = v3;
  ...
  //! \brief Constructor that creates the vector and initializes it with data
  ...
  //! \brief Assignment operator copies the data from the \a other vector to
  //! this vector
  ...
  //! \brief Comparison operator, returns true if the contents of the \a other
  //! vector match those of this vector
  ...
  inline XsSize size() const { return 3; }

  //! \brief Return a value from the vector (needed to allow generated C# access
  //! to these elements)
  inline unsigned short at(int index) { return m_data[index]; }
