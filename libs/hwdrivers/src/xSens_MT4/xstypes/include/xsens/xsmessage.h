...
XSTYPES_DLL_API void XsMessage_load(XsMessage *thisPtr, XsSize msgSize,
...
XSTYPES_DLL_API const uint8_t *XsMessage_constData(XsMessage const *thisPtr,
                                                   XsSize offset);
XSTYPES_DLL_API const uint8_t *
XsMessage_getMessageStart(XsMessage const *thisPtr);
...
XSTYPES_DLL_API const uint8_t *XsMessage_getDataBuffer(XsMessage const *thisPtr,
                                                       XsSize offset);
XSTYPES_DLL_API void XsMessage_setDataByte(XsMessage *thisPtr, uint8_t value,
                                           XsSize offset);
XSTYPES_DLL_API void XsMessage_setDataShort(XsMessage *thisPtr, uint16_t value,
                                            XsSize offset);
XSTYPES_DLL_API void XsMessage_setDataLong(XsMessage *thisPtr, uint32_t value,
                                           XsSize offset);
XSTYPES_DLL_API void XsMessage_setDataFloat(XsMessage *thisPtr, float value,
                                            XsSize offset);
XSTYPES_DLL_API void XsMessage_setDataDouble(XsMessage *thisPtr, double value,
                                             XsSize offset);
XSTYPES_DLL_API void XsMessage_setDataF1220(XsMessage *thisPtr, double value,
                                            XsSize offset);
XSTYPES_DLL_API void XsMessage_setDataFP1632(XsMessage *thisPtr, double value,
                                             XsSize offset);
XSTYPES_DLL_API void XsMessage_setDataBuffer(XsMessage *thisPtr,
                                             const uint8_t *buffer, XsSize size,
                                             XsSize offset);
...
XSTYPES_DLL_API const XsMessageHeader *
XsMessage_getConstHeader(XsMessage const *thisPtr);
...
XSTYPES_DLL_API void XsMessage_insertData(XsMessage *thisPtr, XsSize count,
                                          XsSize offset);
XSTYPES_DLL_API void XsMessage_deleteData(XsMessage *thisPtr, XsSize count,
                                          XsSize offset);
XSTYPES_DLL_API void XsMessage_getDataFPValues(XsMessage const *thisPtr,
                                               uint64_t outputSettings,
                                               double *dest, XsSize offset,
                                               XsSize numValues);
...
XsMessage_getDataFPValuesById(XsMessage const *thisPtr,
                              XsDataIdentifier dataIdentifier, double *dest,
                              XsSize offset, XsSize numValues);
...
struct XsMessageHeader {
  ...
  uint8_t m_busId;     //!< \brief The bus ID \sa XS_BID_MASTER XS_BID_BROADCAST
                       //!XS_BID_MT
  uint8_t m_messageId; //!< \brief The message ID \sa XsXbusMessageId
  uint8_t m_length; //!< \brief The length of the message \details A length of
                    //!255 means extended length is used
  // lint --e{18}
  //! \brief Contains optional extended length of message and first byte of data
  //! buffer
  union LengthData {
    //! \brief Contains extended length information and first byte of data
    //! buffer if normal length is 255
    struct ExtendedLength {
      ...
      } m_length; //!< \brief Extended length, only valid if normal length is
                  //!255
      uint8_t m_data[1]; //!< \brief The first byte of the data buffer, the data
                         //!buffer is always at least 1 byte since it has to
                         //!contain the checksum, but it can be bigger.
    } m_extended; //!< \brief The extended length, only valid if normal length
                  //!is 255
    uint8_t m_data[1]; //!< \brief The first byte of the data buffer if length <
                       //!255, the data buffer is always at least 1 byte since
                       //!it has to contain the checksum, but it can be bigger.
...
struct XsMessage {
#ifdef __cplusplus
  /*! \brief Create a XsMessage object with the given data length and message
     Id.

          The function allocates enough memory to hold an entire message with
     the given
          data length.
          \param msgId		The message Id that will be assigned to the
     m_messageId field.
          \param dataLength	The length of the data in the message.
  */
  explicit XsMessage(XsXbusMessageId msgId = XMID_InvalidMessage,
                     XsSize dataLength = 0)
      : m_autoUpdateChecksum(1), m_checksum(0) {
  ...
  /*! \brief Create a message from the given source string
          ...
          This is done through a simple memory copy. The number of bytes copied
     is taken
  ...
  */
  ...
  XsMessage(const XsMessage &src)
      : m_message(src.m_message),
        m_autoUpdateChecksum(src.m_autoUpdateChecksum), m_checksum(0) {
    updateChecksumPtr();
  }

  //! Destroy the message
  ~XsMessage() { XsMessage_destruct(this); }

  //! \brief Clear all data in the message
  void clear(void) { XsMessage_destruct(this); }

  /*! \brief Test if this message is empty

    \returns true if this message is empty, false otherwise
  */
  bool empty(void) const { return 0 != XsMessage_empty(this); }
  ...
  */
  ...
  // void setDataFPValue(const uint64_t outputSettings, const double data, const
  // XsSize offset = 0);
  // void setDataFPValue(const uint64_t outputSettings, const double *data,
  // XsSize offset, const uint16_t numValues);
  // void setDataFPValue(const XsDataIdentifier dataIdentifier, const double
  // data, const XsSize offset = 0);
  // void setDataFPValue(const XsDataIdentifier dataIdentifier, const double
  ...
  */
  ...
  void getDataFPValue(uint64_t outputSettings, double *dest, XsSize offset = 0,
                      XsSize numValues = 1) const {
    XsMessage_getDataFPValues(this, outputSettings, dest, offset, numValues);
  }

  /*! \brief Return current data value as a double(64 bits), after converting it
    from float, double, FP1632 or FP1220 depending on outputSettings

    \param outputSettings the output settings containing the current
    floating/fixed point settings
  ...
  */
  ...
  void setDataFPValue(uint64_t outputSettings, const double *data,
                      XsSize offset = 0, XsSize numValues = 1) {
    XsMessage_setDataFPValues(this, outputSettings, data, offset, numValues);
  }

  /*! \brief Write a floating/fixed point value into to the data buffer,
     conversion depends on outputSettings
  ...
  */
  ...
  void getDataFPValue(XsDataIdentifier dataIdentifier, double *dest,
                      XsSize offset = 0, XsSize numValues = 1) const {
    XsMessage_getDataFPValuesById(this, dataIdentifier, dest, offset,
                                  numValues);
  }

  /*! \brief Return current data values as double, conversion depends on
     outputSetting
  ...
  */
  double getDataFPValue(XsDataIdentifier dataIdentifier,
                        XsSize offset = 0) const {
    double tmp;
  ...
  void setDataFPValue(XsDataIdentifier dataIdentifier, const double *data,
                      XsSize offset = 0, XsSize numValues = 1) {
    XsMessage_setDataFPValuesById(this, dataIdentifier, data, offset,
                                  numValues);
  }
  ...
  /*! \brief Write a floating/fixed point value into to the data buffer,
    ...
    \param offset Offset in the data buffer from where to start writing.
  */
  void setDataFPValue(XsDataIdentifier dataIdentifier, double data,
                      XsSize offset = 0) {
...
private:
  void updateChecksumPtr() {
    *((uint8_t **)&m_checksum) =
        &m_message[XsMessage_getTotalMessageSize(this) - 1];
  }
  ...
  uint8_t *const m_checksum; //! Points to the checksum to speed up automatic
                             //! checksum updates
};
