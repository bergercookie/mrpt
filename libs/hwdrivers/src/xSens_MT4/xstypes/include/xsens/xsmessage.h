...
struct XsMessageHeader
{
	...
	uint8_t m_messageId;
	/** \brief The length of the message \details A length of 255 means extended
	 * length is used */
	uint8_t m_length;
	// lint --e{18}
	//! \brief Contains optional extended length of message and first byte of
	//! data
	//! buffer
	union LengthData {
		...
		struct ExtendedLength
		{
			...
			/** \brief The first byte of the data buffer, the data buffer is
			 * always at
			 * least 1 byte since it has to contain the checksum, but it can be
			 * bigger. */
			uint8_t m_data[1];
			/** \brief The extended length, only valid if normal length is 255
			 */
		} m_extended;
		/** \brief The first byte of the data buffer if length < 255, the data
		 * buffer is always at least 1 byte since it has to contain the
		 * checksum,
...
struct XsMessage
{
	...
	*/
	explicit XsMessage(
		XsXbusMessageId msgId = XMID_InvalidMessage, XsSize dataLength = 0)
	...
	*/
	...
	{
		const XsMessageHeader* hdr = XsMessage_getConstHeader(this);
		if (!hdr) return XMID_InvalidMessage;
	...
	*/
	...
	{
		if (this != &src) XsMessage_copy(this, &src);
		return *this;
	}
	...
	void getDataFPValue(
		uint64_t outputSettings, double* dest, XsSize offset = 0,
		XsSize numValues = 1) const
	{
		XsMessage_getDataFPValues(
			this, outputSettings, dest, offset, numValues);
	}

	/*! \brief Return current data value as a double(64 bits), after converting
	  it
	...
	*/
	...
	void setDataFPValue(
		uint64_t outputSettings, const double* data, XsSize offset = 0,
		XsSize numValues = 1)
	{
		XsMessage_setDataFPValues(
			this, outputSettings, data, offset, numValues);
	}
	...
	*/
	...
	void getDataFPValue(
		XsDataIdentifier dataIdentifier, double* dest, XsSize offset = 0,
		XsSize numValues = 1) const
	{
		XsMessage_getDataFPValuesById(
			this, dataIdentifier, dest, offset, numValues);
	}
	...
	*/
	double getDataFPValue(
		XsDataIdentifier dataIdentifier, XsSize offset = 0) const
	{
		double tmp;
	...
	void setDataFPValue(
		XsDataIdentifier dataIdentifier, const double* data, XsSize offset = 0,
		XsSize numValues = 1)
	{
		XsMessage_setDataFPValuesById(
			this, dataIdentifier, data, offset, numValues);
	}
	...
	*/
	void setDataFPValue(
		XsDataIdentifier dataIdentifier, double data, XsSize offset = 0)
	{
