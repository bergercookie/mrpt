...
class XsException : public std::exception
{
   public:
	...
	XsException(XsException const& e)
		: m_code(e.m_code), m_description(e.m_description)
	{
	}

	/*! \brief Initializing constructor
		\details This constructor uses the value in \a err and the supplied \a
	   description to create a full
		text for when the user requests what() or text()
		\param err The error code that the exception should report
		\param description A description of the error. The constructor prefixes
	   this with a textual
							description of the error code unless prefix is
	   false.
		\param prefix Whether to prefix the description with a textual
	   description
	   of the error code or not (default is yes)
	*/
	XsException(
		XsResultValue err, XsString const& description, bool prefix = true)
		: m_code(err), m_description(description)
	{
		if (prefix && (m_code != XRV_OK))
		{
			...
			if (!m_description.empty()) rv << "\tInfo: ";
	...
	*/
	explicit XsException(XsString const& description)
		: m_code(XRV_ERROR), m_description(description)
	{
	}
	...
	inline XsResultValue code() const throw() { return m_code; }
	//! \brief Returns a description of the error that occurred as a char const*
	inline char const* what() const throw() { return m_description.c_str(); }
	//! \brief Returns a description of the error that occurred as a XsString
	inline XsString const& text() const throw() { return m_description; }
