...
XSTYPES_DLL_API void XsByteArray_construct(XsByteArray *thisPtr, XsSize count,
                                           uint8_t const *src);
...
#define XsByteArray_fromString(str, copy)                                      \
  XsArray_assign(copy, str->m_size ? str->m_size : 1,                          \
                 str->m_size ? str->m_data : "\0")
#define XsByteArray_swap(a, b) XsArray_swap(a, b)
#define XsByteArray_erase(thisPtr, index, count)                               \
...
struct XsByteArray
  ...
  //! ref
  inline explicit XsByteArray(uint8_t *ref, XsSize sz,
