...
#endif
  XsReal XSCCONST
      m_fixedData[3]; //!< Fixed size storage for the components in the vector
...
public:
  ...
  XsVector3(XsVector3 const &other)
      : XsVector(other, m_fixedData, 3, XSDF_FixedSize) {}

  //! \brief Constructs a vector3 from an \a other XsVector
  XsVector3(XsVector const &other)
      : XsVector(other, m_fixedData, 3, XSDF_FixedSize) {}

  //! \brief Constructs a vector3 using the values \a x, \a y, \a z
  XsVector3(XsReal x, XsReal y, XsReal z)
      : XsVector(m_fixedData, 3, XSDF_FixedSize) {
    m_fixedData[0] = x;
    m_fixedData[1] = y;
    m_fixedData[2] = z;
  }
