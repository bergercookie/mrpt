...
struct XsTimeStamp {
  ...
  /*! \brief Get the time of day component of the stored timestamp in seconds as
  ...
  */
  ...
  /*! \brief Get the sum of the current and the given \a other timestamp. \param
   * other The value to add to this \returns The added timestamp values */
  ...
  /*! \brief Get the result of subtracting the given \a other timestamp from the
   * current timestamp. */
  ...
  /*! \brief Test if the given \a other timestamp is larger than or equal to the
   * current timestamp. */
  inline bool operator>=(const XsTimeStamp &other) const {
    return m_msTime >= other.m_msTime;
  }
  ...
  /*! \brief Test if the given \a other is smaller than or equal to the current
   * timestamp. */
  inline bool operator<=(int other) const { return m_msTime <= other; }
  ...
  /*! \brief Returns the number of seconds elapsed since the epoch as stored in
   * the XsTimeStamp */
  inline int64_t secondTime() const { return m_msTime / 1000; }

  /*! \brief Returns the millisecond part of the time (in the range 0-999) */
  inline int32_t milliSecondPart() const { return (int32_t)(m_msTime % 1000); }
