...
#ifndef __cplusplus
#define XSDEVICEIDARRAY_INITIALIZER \
	XSARRAY_INITIALIZER(&g_xsDeviceIdArrayDescriptor)
XSARRAY_STRUCT(XsDeviceIdArray, XsDeviceId);
typedef struct XsDeviceIdArray XsDeviceIdArray;

XSTYPES_DLL_API void XsDeviceIdArray_construct(
	XsDeviceIdArray* thisPtr, XsSize count, XsDeviceId const* src);
#else
}  // extern "C"
#endif
						 ...
						 XsDeviceIdArray>
{
	...
	inline XsDeviceIdArray(XsDeviceIdArray const& other) : ArrayImpl(other) {}
	//! \brief Constructs an XsDeviceIdArray that references the data supplied
	//! in
	//! \a ref
	inline explicit XsDeviceIdArray(
		XsDeviceId* ref, XsSize sz, XsDataFlags flags = XSDF_None)
		: ArrayImpl(ref, sz, flags)
	{
	}
#ifndef XSENS_NOITERATOR
	//! \brief Constructs an XsDeviceIdArray with the array bound by the
	//! supplied
