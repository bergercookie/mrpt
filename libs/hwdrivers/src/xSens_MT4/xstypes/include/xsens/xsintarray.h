...
//	XsArrayImpl<int,g_xsIntArrayDescriptor>(XsSize sz = 0, int const* src =
//0);
...
XSTYPES_DLL_API void XsIntArray_construct(XsIntArray *thisPtr, XsSize count,
                                          int const *src);
#else
} // extern "C"
#endif
...
#ifdef __cplusplus
struct XsIntArray
    : public XsArrayImpl<int, g_xsIntArrayDescriptor, XsIntArray> {
  ...
  inline XsIntArray(XsIntArray const &other) : ArrayImpl(other) {}

  //! \brief Constructs an XsIntArray that references the data supplied in \a
  //! ref
  inline explicit XsIntArray(int *ref, XsSize sz, XsDataFlags flags = XSDF_None)
      : ArrayImpl(ref, sz, flags) {}

#ifndef XSENS_NOITERATOR
  //! \brief Constructs an XsIntArray with the array bound by the supplied
