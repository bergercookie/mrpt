...
enum XsStatusFlag {
  ...
  ,
  XSF_OrientationValid = 0x02 //!< Is set when the computed orientation is
                              //!valid. The orientation may be invalid during
                              //!startup or when the sensor data is clipping
                              //!during violent (for the device) motion
  ,
  XSF_GpsValid = 0x04 //!< Is set when the device has a GPS receiver and the
                      //!receiver says that there is a GPS position fix.

  ,
  XSF_NoRotationMask = 0x18 //!< If all of these flags are set, the No Rotation
                            //!algorithm is running
  ,
  XSF_NoRotationAborted = 0x10 //!< If only this flag is set (out of the
                               //!XSF_NoRotationMask) then the No Rotation
                               //!algorithm was aborted
  ,
  XSF_NoRotationSamplesRejected =
      0x08 //!< If only this flag is set (out of the XSF_NoRotationMask) then
           //!the No Rotation algorithm is running but has rejected samples
  ,
  XSF_NoRotationRunningNormally = 0x18 //!< If all these flags are set (out of
                                       //!the XSF_NoRotationMask) then the No
  ...
  ,
  XSF_SyncIn =
      0x00200000 //!< When set indicates a sync-in event has been triggered
  ,
  XSF_SyncOut =
      0x00400000 //!< When set Indicates a sync-out event has been generated
...
/*! \brief Status flag bit offsets
        \details Sometimes (rarely) it is necessary to know the bit offset
   instead of the bit mask (ie when
