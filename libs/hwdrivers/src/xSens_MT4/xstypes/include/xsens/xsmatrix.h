...
XSTYPES_DLL_API void XsMatrix_ref(XsMatrix *thisPtr, XsSize rows, XsSize cols,
                                  XsSize stride, XsReal *buffer,
                                  XsDataFlags flags);
XSTYPES_DLL_API void XsMatrix_construct(XsMatrix *thisPtr, XsSize rows,
                                        XsSize cols, XsSize stride,
                                        const XsReal *src, XsSize srcStride);
XSTYPES_DLL_API void XsMatrix_assign(XsMatrix *thisPtr, XsSize rows,
                                     XsSize cols, XsSize stride,
                                     const XsReal *src, XsSize srcStride);
...
XSTYPES_DLL_API void XsMatrix_multiplyScalar(const XsMatrix *thisPtr,
                                             XsReal scalar, XsMatrix *dest);
XSTYPES_DLL_API XsSize XsMatrix_offset(const XsMatrix *thisPtr, XsSize row,
                                       XsSize column);
XSTYPES_DLL_API XsReal XsMatrix_value(const XsMatrix *thisPtr, XsSize row,
                                      XsSize column);
XSTYPES_DLL_API void XsMatrix_setValue(XsMatrix *thisPtr, XsSize row,
                                       XsSize column, XsReal value);
XSTYPES_DLL_API int XsMatrix_dimensionsMatch(const XsMatrix *thisPtr,
                                             XsSize rows, XsSize columns);
...
struct XsMatrix {
  ...
  const XsSize m_stride; //!< Number of items per row in memory (usually equal
                         //!to cols but not always)
...
public:
  /*! \brief Initialize an XsMatrix object with the specified number of \a rows
   * and \a cols */
  inline explicit XsMatrix(XsSize rows = 0, XsSize cols = 0, XsSize strde = 0,
                           const XsReal *dat = 0)
  ...
  /*! \brief Initialize an XsMatrix object that references the data passed in \a
   * ref. \a rows, \a cols and \a stride can be used to specify the layout of
   * the data */
  inline explicit XsMatrix(XsReal *ref, XsSize rows, XsSize cols, XsSize stride,
                           XsDataFlags flags = XSDF_None)
      : m_data(ref), m_rows(rows), m_cols(cols), m_stride(stride),
        m_flags(flags) {}

  /*! \brief Initialize a copy of \a other in an XsMatrix object that references
   * the data passed in \a ref. \a rows, \a cols and \a stride can be used to
   * specify the layout of the data */
  inline explicit XsMatrix(const XsMatrix &other, XsReal *ref, XsSize rows,
                           XsSize cols, XsSize stride,
                           XsDataFlags flags = XSDF_None)
      : m_data(ref), m_rows(rows), m_cols(cols), m_stride(stride),
        m_flags(flags) {
  ...
  /*! \brief Return the stride of the matrix.
     ...
     defines the number of items
          in a row in the data buffer. This is always greater than or equal to
     the number of columns.
          Especially for matrices that reference a part of another matrix this
     may differ from the
          cols() value.
