...
bool CFFMPEG_InputStream::openURL(const std::string &url,
                                  bool grab_as_grayscale, bool verbose) {
#if MRPT_HAS_FFMPEG
...
// Open video file
...
#else
  if (av_open_input_file(&ctx->pFormatCtx, url.c_str(), nullptr, 0, nullptr) !=
      0)
#endif
  ...
  if (
    ...
    std::cerr
        << "[CFFMPEG_InputStream::openURL] Couldn't find stream information: "
        << url << std::endl;
    return false;
  }
  ...
  if (ctx->videoStream == -1) {
    std::cerr << "[CFFMPEG_InputStream::openURL] Didn't find a video stream: "
              << url << std::endl;
    return false;
  }
...
// Open codec
...
#endif
  {
    std::cerr << "[CFFMPEG_InputStream::openURL] Could not open codec: " << url
              << std::endl;
    return false;
  }
  ...
  if (ctx->pFrameRGB == nullptr || ctx->pFrame == nullptr) {
    std::cerr << "[CFFMPEG_InputStream::openURL] Could not alloc memory for "
                 "frame buffers: "
              << url << std::endl;
    return false;
  }
...
// Determine required buffer size and allocate buffer
  ...
  avpicture_fill((AVPicture *)ctx->pFrameRGB, &ctx->buffer[0],
                 m_grab_as_grayscale ? // BGR vs. RGB for OpenCV
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(55, 00, 0)
                     AV_PIX_FMT_GRAY8
                                     : AV_PIX_FMT_BGR24,
...
// Decode video frame
    ...
    (LIBAVCODEC_VERSION_MAJOR == 52 && LIBAVCODEC_VERSION_MINOR >= 72)
      avcodec_decode_video2(ctx->pCodecCtx, ctx->pFrame, &frameFinished,
                            &packet);
#else
      avcodec_decode_video(ctx->pCodecCtx, ctx->pFrame, &frameFinished,
      ...
      if (frameFinished) {
        ...
        ctx->img_convert_ctx = sws_getCachedContext(
            ctx->img_convert_ctx, ctx->pCodecCtx->width, ctx->pCodecCtx->height,
            ctx->pCodecCtx->pix_fmt, ctx->pCodecCtx->width,
        ...
        sws_scale(ctx->img_convert_ctx, ctx->pFrame->data,
        ...
        // ctx->pCodecCtx->width << "x" << ctx->pCodecCtx->height << std::endl;
        ...
        out_img.loadFromMemoryBuffer(ctx->pCodecCtx->width,
                                     ctx->pCodecCtx->height,
