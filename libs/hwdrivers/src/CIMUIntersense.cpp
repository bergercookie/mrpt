...
CIMUIntersense::CIMUIntersense()
    : m_handles_ptr(nullptr), m_timeStartUI(0), m_timeStartTT(0),
      m_sensorPose(), m_nSensors(0), m_sensitivity(10), m_enhancement(2),
      m_prediction(0), m_useBuffer(false), m_toutCounter(0) {
...
#else
  THROW_EXCEPTION("MRPT has been compiled with 'BUILD_INTERSENSE'=OFF, so this "
...
void CIMUIntersense::doProcess() {
  ...
  for (int i = 0; i < m_nSensors; ++i) {
    ...
    // mrpt::system::TTimeStamp AtDO = mrpt::system::secondsToTimestamp( AtUI *
    // 1e-6 /* Board time is usec */ );
  ...
  if (openSuccess < 1)
    ...
    for (int i = 0; i < ISD_MAX_TRACKERS; ++i) {
      if (isense_handles[i] > 0) {
        ...
        Bool res_ok =
            ISD_GetStationConfig(isense_handles[i],
        ...
        if (!res_ok) {
          ...
          res_ok =
              ISD_SetStationConfig(isense_handles[i],
                                   &station_info[i], // & station,
                                   i + 1 /*from 1 to ISD_MAX_TRACKERS*/, FALSE);
