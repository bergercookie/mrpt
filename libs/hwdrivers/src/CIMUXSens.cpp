...
-------------------------------------------------------------*/
CIMUXSens::CIMUXSens()
    : m_COMbauds(0), m_com_port(), m_timeStartUI(0), m_timeStartTT(0),
      m_sensorPose(), m_cmt3_ptr(nullptr), m_deviceId_ptr(nullptr),
      m_toutCounter(0) {
...
#else
  THROW_EXCEPTION("MRPT has been compiled with 'BUILD_XSENS_MT3'=OFF, so this "
  ...
  do {
    ...
    if (res == XRV_TIMEOUT) {
      if (++m_toutCounter > 3) {
        ...
        std::cerr
            << "[CIMUXSens::doProcess()] Error: No data available [XRV_TIMEOUT]"
            << std::endl;
      }
    } // end if XRV_TIMEOUT
    ...
    if (res == XRV_TIMEOUTNODATA) {
      ...
      //			if( cmt3.isPortOpen() )
      //				cmt3.closePort();
      //			std::cerr << "[CIMUXSens::doProcess()] Error: No
      //data available [XRV_TIMEOUTNODATA]" << std::endl;
...
bool CIMUXSens::searchPortAndConnect() {
  ...
  else // Port defined by user in .ini file
  {
    ...
    strcpy(pInfo.m_portName,
           m_com_port
               .c_str()); // m_portNr		= (unsigned char)m_com_port;
  ...
  std::cout
      << "Device ID at busId 1: " << (long)deviceId
...
void CIMUXSens::initialize() {
  ...
  if (res != XRV_OK) {
    ...
    std::cerr << "An error ocurred when setting the device to measurement mode"
              << std::endl;
    return;
  }
