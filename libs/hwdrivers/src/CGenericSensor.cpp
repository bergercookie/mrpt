...
CGenericSensor::CGenericSensor()
    : m_process_rate(0), m_max_queue_len(200), m_grab_decimation(0),
      m_sensorLabel("UNNAMED_SENSOR"), m_grab_decimation_counter(0),
      m_state(ssInitializing), m_verbose(false), m_path_for_external_images(),
      m_external_images_format("png"), m_external_images_jpeg_quality(95) {
...
void CGenericSensor::appendObservations(
  ...
  if (++m_grab_decimation_counter >= m_grab_decimation) {
    ...
    for (size_t i = 0; i < objs.size(); i++) {
      ...
      } else if (obj->GetRuntimeClass()->derivedFrom(CLASS_ID(CObservation))) {
...
void CGenericSensor::loadConfig(const mrpt::utils::CConfigFileBase &cfg,
  ...
  m_process_rate = cfg.read_double(sect, "process_rate",
                                   0); // Leave it to 0 so rawlog-grabber can
                                       // detect if it's not set by the user.
