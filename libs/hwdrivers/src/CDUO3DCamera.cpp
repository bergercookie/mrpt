...
//#	endif
#endif
MRPT_TODO("FIXME: Try to remove the pragma lib above ==> "
          "TARGET_LINK_LIBRARIES() in hwdrivers/CMakeLists.txt, so it can work "
          "seamlessly on Linux in the future")
...
TCaptureOptions_DUO3D::TCaptureOptions_DUO3D()
    : m_img_width(640), m_img_height(480), m_fps(30), m_exposure(50), m_led(25),
      m_gain(0), m_capture_imu(false), m_capture_rectified(false),
      m_calibration_from_file(true), m_rectify_map_filename(""),
      m_intrinsic_filename(""), m_extrinsic_filename(""),
...
TCaptureOptions_DUO3D::m_rectify_map_from_yml(const string &_file_name) {
  ...
  const size_t found =
      aux.find(mrpt::format("_R%dx%d_", this->m_img_width, this->m_img_height));
...
TCaptureOptions_DUO3D::TYMLReadResult
TCaptureOptions_DUO3D::m_camera_ext_params_from_yml(const string &_file_name) {
#if MRPT_HAS_OPENCV
  const string file_name =
...
TCaptureOptions_DUO3D::TYMLReadResult
TCaptureOptions_DUO3D::m_camera_int_params_from_yml(const string &_file_name) {
#if MRPT_HAS_OPENCV
  const string file_name =
...
void TCaptureOptions_DUO3D::loadOptionsFrom(
  ...
  m_capture_rectified = configSource.read_bool(iniSection, "capture_rectified",
                                               m_capture_rectified);
  m_capture_imu =
...
#else
  THROW_EXCEPTION("MRPT has been compiled with 'MRPT_BUILD_DUO3D'=OFF, so this "
...
void CDUO3DCamera::open(const TCaptureOptions_DUO3D &options,
  ...
  if (this->m_options.m_calibration_from_file) {
    ...
    if (res == TCaptureOptions_DUO3D::yrr_EMPTY)
      cout << "[CDUO3DCamera] Warning: Some of the intrinsic params could not "
              "be read (size=0). Check file content."
           << endl;
    ...
    if (res == TCaptureOptions_DUO3D::yrr_EMPTY)
      cout << "[CDUO3DCamera] Warning: Some of the extrinsic params could not "
              "be read (size!=3x3). Check file content."
           << endl;
    else if (res == TCaptureOptions_DUO3D::yrr_NAME_NON_CONSISTENT)
    ...
    if (this->m_options.m_capture_rectified) {
      if (!this->m_options.m_rectify_map_filename.empty()) {
        ...
        if (res == TCaptureOptions_DUO3D::yrr_EMPTY)
          cout << "[CDUO3DCamera] Warning: Rectification map could not be read "
                  "(size==0). Check file content."
               << endl;
        else if (res == TCaptureOptions_DUO3D::yrr_NAME_NON_CONSISTENT)
          cout << "[CDUO3DCamera] Warning: Rectification map filename is not "
                  "consistent with image size. Are you using the correct "
                  "calibration?. Rectification map set to zero."
               << endl;
        ...
        cout << "[CDUO3DCamera] Warning: Calibration information is set to be "
                "read from a file, but the file was not specified. Unrectified "
                "images will be grabbed."
             << endl;
      }
    } // end-if
  }   // end-if
  ...
  if (!EnumerateResolutions(&ri, 1, this->m_options.m_img_width,
                            this->m_options.m_img_height, binning,
                            this->m_options.m_fps))
  ...
  cout << "[CDUO3DCamera::open] DUO3DCamera name: " << name << " (v" << version
       << ")" << endl;
...
/*-------------------------------------------------------------
                                                getObservations
-------------------------------------------------------------*/
void CDUO3DCamera::getObservations(CObservationStereoImages &outObservation_img,
  ...
  if (this->m_options.m_capture_imu) {
    if (!reinterpret_cast<PDUOFrame>(m_pframe_data)->accelerometerPresent) {
      cout << "[CDUO3DCamera] Warning: This device does not provide IMU data. "
              "No IMU observations will be created."
           << endl;
