...
#endif
MRPT_TODO(
	"FIXME: Try to remove the pragma lib above ==> TARGET_LINK_LIBRARIES() in "
	"hwdrivers/CMakeLists.txt, so it can work seamlessly on Linux in the "
	"future")
		...
		if (this->m_options.m_capture_rectified)
		{
			if (!this->m_options.m_rectify_map_filename.empty())
			{
				...
				if (res == TCaptureOptions_DUO3D::yrr_EMPTY)
					...
					cout << "[CDUO3DCamera] Warning: Rectification map "
							"filename is not "
							"consistent with image size. Are you using the "
							"correct "
			...
			else
			{
				cout << "[CDUO3DCamera] Warning: Calibration information is "
						"set to be "
						"read from a file, but the file was not specified. "
						"Unrectified "
