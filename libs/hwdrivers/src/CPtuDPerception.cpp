...
bool CPtuDPerception::enableLimitsQ(bool &enable) {
  ...
  if (!transmit("L") || !receive("L", response))
    return false;
...
bool CPtuDPerception::verboseQ(bool &mode) {
  ...
  if (!transmit("F") || !receive("F", response))
    return false;
...
bool CPtuDPerception::echoModeQ(bool &mode) {
  ...
  if (!transmit("E") || !receive("E", response))
    return false;
...
bool CPtuDPerception::resolution(void) {
  ...
  if ((!transmit("PR")) || (!receive("PR", response)))
    return false;
  panResolution = DEG2RAD(convertToDouble(response) / 3600);

  if ((!transmit("TR")) || (!receive("TR", response)))
    return false;
...
bool CPtuDPerception::radQuerry(char axis, char command, double &rad) {
  ...
  if ((!transmit(sTrans)) || (!receive(sTrans, response)))
    return false;
...
bool CPtuDPerception::scan(char axis, int tWait, float initial, float final,
                           double radPre) {
