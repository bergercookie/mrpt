...
CCameraSensor::CCameraSensor()
    : mrpt::utils::COutputLogger("CCameraSensor"), m_sensorPose(),
      m_grabber_type("opencv"), m_capture_grayscale(false),
      m_cv_camera_index(0), m_cv_camera_type("CAMERA_CV_AUTODETECT"),
      m_cv_options(),
      // ---
      m_dc1394_camera_guid(0), m_dc1394_camera_unit(0), m_dc1394_options(),
      m_preview_decimation(0), m_preview_reduction(1),
      // ---
      m_bumblebee_dc1394_camera_guid(0), m_bumblebee_dc1394_camera_unit(0),
      m_bumblebee_dc1394_framerate(15),
      // ---
      m_svs_camera_index(0), m_svs_options(),
      // ---
      m_sr_open_from_usb(true), m_sr_save_3d(true), m_sr_save_range_img(true),
      m_sr_save_intensity_img(true), m_sr_save_confidence(true),
      // ---
      m_kinect_save_3d(true), // These options are also used for OpenNI2 grabber
      m_kinect_save_range_img(true), m_kinect_save_intensity_img(true),
      m_kinect_video_rgb(true),
      // ---
      m_fcs_start_synch_capture(false),
      // ---
      m_img_dir_url(""), m_img_dir_left_format("imL_%04d.jpg"),
      m_img_dir_right_format("imR_%04d.jpg"), m_img_dir_start_index(0),
      m_img_dir_end_index(100), m_img_dir_is_stereo(true), m_img_dir_counter(0),
      // ---
      m_external_images_own_thread(false), m_camera_grab_decimator(0),
      m_camera_grab_decimator_counter(0), m_preview_counter(0),
      m_external_image_saver_count(std::thread::hardware_concurrency()),
      m_threadImagesSaverShouldEnd(false), m_hook_pre_save(nullptr),
...
void CCameraSensor::initialize() {
  ...
  if (m_grabber_type == "opencv") {
    // OpenCV driver:
    mrpt::utils::CTypeSelector camera_type("CAMERA_CV_AUTODETECT,CAMERA_CV_"
                                           "DC1394,CAMERA_CV_VFL,CAMERA_CV_VFW,"
                                           "CAMERA_CV_MIL",
    ...
    m_cap_cv.reset(new CImageGrabber_OpenCV(m_cv_camera_index, TCameraType(idx),
                                            m_cv_options));
    ...
    m_cap_dc1394.reset(
        new CImageGrabber_dc1394(m_dc1394_camera_guid, m_dc1394_camera_unit,
                                 m_dc1394_options, true /* verbose */));
    ...
    cout << format("[CCameraSensor::initialize] bumblebee_libdc1394 camera: "
    ...
    m_cap_svs.reset(new CStereoGrabber_SVS(m_svs_camera_index, m_svs_options));
    ...
    if (!m_cap_ffmpeg->openURL(m_ffmpeg_url, m_capture_grayscale)) {
      m_state = CGenericSensor::ssError;
      THROW_EXCEPTION_FMT("Error opening FFmpeg stream: %s",
                          m_ffmpeg_url.c_str())
    }
    ...
    m_cap_kinect->setVideoChannel(m_kinect_video_rgb
    ...
    m_cap_openni2->enableGrab3DPoints(
        m_kinect_save_3d); // It uses the same options as the Kinect grabber
    ...
    if (!m_cap_rawlog->open(m_rawlog_file)) {
      m_state = CGenericSensor::ssError;
      THROW_EXCEPTION_FMT("Error opening rawlog file: %s",
                          m_rawlog_file.c_str())
    }
    // File open OK.
    ...
    try {
      ...
      cout << "[CCameraSensor::initialize] PGR FlyCapture2 stereo camera: "
              "Openning LEFT camera...\n";
      m_cap_flycap_stereo_l->open(m_flycap_stereo_options[0],
                                  false /* don't start grabbing */);

      cout << "[CCameraSensor::initialize] PGR FlyCapture2 stereo camera: "
              "Openning RIGHT camera...\n";
      m_cap_flycap_stereo_r->open(m_flycap_stereo_options[1],
                                  false /* don't start grabbing */);
    ...
    THROW_EXCEPTION_FMT("Unknown 'grabber_type' found: %s",
                        m_grabber_type.c_str())
...
void CCameraSensor::loadConfig_sensorSpecific(
  ...
  MRPT_LOAD_HERE_CONFIG_VAR(preview_decimation, int, m_preview_decimation,
                            configSource, iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(preview_reduction, int, m_preview_reduction,
                            configSource, iniSection)
  ...
  m_cv_options.frame_width = configSource.read_int(iniSection, "cv_frame_width",
                                                   m_cv_options.frame_width);
  ...
  m_cv_options.ieee1394_fps =
      configSource.read_double(iniSection, "cv_fps", m_cv_options.ieee1394_fps);
  ...
  // dc1394 options:
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_camera_guid, uint64_t, m_dc1394_camera_guid,
                            configSource, iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_camera_unit, int, m_dc1394_camera_unit,
                            configSource, iniSection)

  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_frame_width, int,
                            m_dc1394_options.frame_width, configSource,
                            iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_frame_height, int,
                            m_dc1394_options.frame_height, configSource,
                            iniSection)

  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_mode7, int, m_dc1394_options.mode7,
                            configSource, iniSection)

  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_shutter, int, m_dc1394_options.shutter,
                            configSource, iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_gain, int, m_dc1394_options.gain,
                            configSource, iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_gamma, int, m_dc1394_options.gamma,
                            configSource, iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_brightness, int, m_dc1394_options.brightness,
                            configSource, iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_exposure, int, m_dc1394_options.exposure,
                            configSource, iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_sharpness, int, m_dc1394_options.sharpness,
                            configSource, iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_white_balance, int,
                            m_dc1394_options.white_balance, configSource,
                            iniSection)

  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_shutter_mode, int,
                            m_dc1394_options.shutter_mode, configSource,
                            iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_gain_mode, int, m_dc1394_options.gain_mode,
                            configSource, iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_gamma_mode, int, m_dc1394_options.gamma_mode,
                            configSource, iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_brightness_mode, int,
                            m_dc1394_options.brightness_mode, configSource,
                            iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_exposure_mode, int,
                            m_dc1394_options.exposure_mode, configSource,
                            iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_sharpness_mode, int,
                            m_dc1394_options.sharpness_mode, configSource,
                            iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_white_balance_mode, int,
                            m_dc1394_options.white_balance_mode, configSource,
                            iniSection)

  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_trigger_power, int,
                            m_dc1394_options.trigger_power, configSource,
                            iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_trigger_mode, int,
                            m_dc1394_options.trigger_mode, configSource,
                            iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_trigger_source, int,
                            m_dc1394_options.trigger_source, configSource,
                            iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_trigger_polarity, int,
                            m_dc1394_options.trigger_polarity, configSource,
                            iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(dc1394_ring_buffer_size, int,
                            m_dc1394_options.ring_buffer_size, configSource,
                            iniSection)

  // Bumblebee_dc1394 options:
  MRPT_LOAD_HERE_CONFIG_VAR(bumblebee_dc1394_camera_guid, uint64_t,
                            m_bumblebee_dc1394_camera_guid, configSource,
                            iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(bumblebee_dc1394_camera_unit, int,
                            m_bumblebee_dc1394_camera_unit, configSource,
                            iniSection)
  MRPT_LOAD_HERE_CONFIG_VAR(bumblebee_dc1394_framerate, double,
                            m_bumblebee_dc1394_framerate, configSource,
                            iniSection)

  // SVS options:
  m_svs_camera_index =
      configSource.read_int(iniSection, "svs_camera_index", m_svs_camera_index);
  ...
  m_svs_options.m_Corrsize = configSource.read_int(iniSection, "svs_Corrsize",
  ...
  m_svs_options.m_Horopter = configSource.read_int(iniSection, "svs_Horopter",
  ...
  m_rawlog_camera_sensor_label = mrpt::system::trim(configSource.read_string(
      iniSection, "rawlog_camera_sensor_label", m_rawlog_camera_sensor_label));
  ...
  m_sr_save_range_img =
      configSource.read_bool(iniSection, "sr_grab_range", m_sr_save_range_img);
  ...
  m_kinect_video_rgb = configSource.read_bool(iniSection, "kinect_video_rgb",
                                              m_kinect_video_rgb);
  ...
  m_flycap_stereo_options[0].loadOptionsFrom(configSource, iniSection,
                                             "fcs_LEFT_");
  m_flycap_stereo_options[1].loadOptionsFrom(configSource, iniSection,
                                             "fcs_RIGHT_");
  ...
  if (it_fps == map_fps.end())
    THROW_EXCEPTION_FMT(
        "ERROR: DC1394 framerate seems to be not a valid number: %f", the_fps);
...
#define ADD_COLOR_MAP(c) map_color[#c] = c;
  ...
  if (it_color == map_color.end())
    THROW_EXCEPTION_FMT("ERROR: Color coding seems not to be valid : '%s'",
  ...
  m_external_images_jpeg_quality =
      configSource.read_int(iniSection, "external_images_jpeg_quality",
                            m_external_images_jpeg_quality);
  ...
  m_external_image_saver_count =
      configSource.read_int(iniSection, "external_images_own_thread_count",
                            m_external_image_saver_count);
...
void CCameraSensor::getNextFrame(vector<CSerializable::Ptr> &out_obs) {
  ...
  if (m_cap_cv) {
    ...
    if (!m_cap_cv->getObservation(*obs)) { // Error
      m_state = CGenericSensor::ssError;
    ...
    obs = std::make_shared<CObservationImage>();
    if (!m_cap_dc1394->getObservation(*obs)) { // Error
      m_state = CGenericSensor::ssError;
      THROW_EXCEPTION("Error grabbing image");
    } else
      capture_ok = true;
    ...
    if (!there_is_obs || hardware_error) { // Error
      m_state = CGenericSensor::ssError;
    ...
    // If we have an "MRPT_CCAMERA_KINECT_TIMEOUT_MS" environment variable, use
    // that timeout instead:
    const char *envVal = getenv("MRPT_CCAMERA_KINECT_TIMEOUT_MS");
    if (envVal)
      max_timeout = atoi(envVal) * 0.001;
    ...
    if (!m_cap_ffmpeg->retrieveFrame(obs->image)) { // Error
      m_state = CGenericSensor::ssError;
      THROW_EXCEPTION("Error grabbing image");
    } else
      capture_ok = true;
    ...
    std::string auxL =
        format("%s/%s", m_img_dir_url.c_str(), m_img_dir_left_format.c_str());
    if (m_img_dir_is_stereo) {
      ...
      std::string auxR = format("%s/%s", m_img_dir_url.c_str(),
                                m_img_dir_right_format.c_str());
    ...
    while (!obs && !stObs && !obs3D) {
      *m_cap_rawlog >> newObs;
      ...
      if (obs || stObs || obs3D) {
        // We must convert externally stored images into "normal in-memory"
        // images.
        ...
        if (stObs && stObs->hasImageDisparity &&
          ...
          stObs->imageDisparity.loadFromFile(
              stObs->imageDisparity.getExternalStorageFileAbsolutePath());
    ...
    if (!ok1 || !ok2) {
      ...
      if (std::abs(At) > 0.1) {
        cout << "[CCamera, flycap_stereo] Warning: Too large delay between "
                "left & right images: "
             << At << " sec.\n";
      }
    ...
    THROW_EXCEPTION("There is no initialized camera driver: has 'initialize()' "
                    "been called?")
  }
  ...
  if (m_capture_grayscale) {
    if (obs) {
      if (obs->image.isColor())
        obs->image.grayscaleInPlace();
    } else if (stObs) {
      if (stObs->imageLeft.isColor())
        stObs->imageLeft.grayscaleInPlace();
      ...
      if (stObs->hasImageDisparity && stObs->imageDisparity.isColor())
        stObs->imageDisparity.grayscaleInPlace(); // Shouldn't happen, but...
    } else if (obs3D) {
  ...
  // External storage?
  bool delayed_insertion_in_obs_queue = false; // If true, we'll return nothing,
                                               // but the observation will be
    ...
    {          // Stereo obs  -------
      if (m_external_images_own_thread) {
        ...
        const string filNameL =
            ...
            format("_L_%f.%s", (double)timestampTotime_t(stObs->timestamp),
                   m_external_images_format.c_str());
        const string filNameR =
            fileNameStripInvalidChars(trim(m_sensorLabel)) +
            format("_R_%f.%s", (double)timestampTotime_t(stObs->timestamp),
                   m_external_images_format.c_str());
        const string filNameD =
            fileNameStripInvalidChars(trim(m_sensorLabel)) +
            format("_D_%f.%s", (double)timestampTotime_t(stObs->timestamp),
                   m_external_images_format.c_str());
        // cout << "[CCameraSensor] Saving " << filName << endl;
        stObs->imageLeft.saveToFile(m_path_for_external_images + string("/") +
                                        filNameL,
                                    m_external_images_jpeg_quality);
        stObs->imageLeft.setExternalStorage(filNameL);

        if (stObs->hasImageRight) {
          stObs->imageRight.saveToFile(m_path_for_external_images +
                                           string("/") + filNameR,
                                       m_external_images_jpeg_quality);
          stObs->imageRight.setExternalStorage(filNameR);
        }
        if (stObs->hasImageDisparity) {
          stObs->imageDisparity.saveToFile(m_path_for_external_images +
                                               string("/") + filNameD,
                                           m_external_images_jpeg_quality);
          stObs->imageDisparity.setExternalStorage(filNameD);
        }
      }
    } else if (obs) { // Monocular image obs  -------
      if (m_external_images_own_thread) {
        m_csToSaveList.lock();
        ...
        string filName =
            ...
            format("_%f.%s", (double)timestampTotime_t(obs->timestamp),
                   m_external_images_format.c_str());
        // cout << "[CCameraSensor] Saving " << filName << endl;
        obs->image.saveToFile(m_path_for_external_images + string("/") +
                                  filName,
                              m_external_images_jpeg_quality);
    ...
    if (++m_preview_counter > m_preview_decimation) {
      ...
      if (m_preview_win1->isOpen()) {
        ...
        if (m_preview_reduction >= 2) {
          ...
          CImage auxImg;
          img->scaleImage(auxImg, w / m_preview_reduction,
      ...
      if (m_preview_win2 && m_preview_win2->isOpen() && stObs &&
        ...
        if (m_preview_reduction >= 2) {
          ...
          CImage auxImg;
          stObs->imageRight.scaleImage(auxImg, w / m_preview_reduction,
                                       h / m_preview_reduction, IMG_INTERP_NN);
      ...
      if (m_preview_win2 && m_preview_win2->isOpen() && stObs &&
        ...
        if (m_preview_reduction >= 2) {
          ...
          CImage auxImg;
          stObs->imageDisparity.scaleImage(auxImg, w / m_preview_reduction,
                                           h / m_preview_reduction,
                                           IMG_INTERP_NN);
          m_preview_win2->showImage(auxImg);
        } else
  ...
  if (delayed_insertion_in_obs_queue) {
    ...
    if (stObs)
      out_obs.push_back(CObservation::Ptr(stObs));
    if (obs)
      out_obs.push_back(CObservation::Ptr(obs));
    if (obs3D)
      out_obs.push_back(CObservation::Ptr(obs3D));
  }
  return;
}
...
void CCameraSensor::setSoftwareTriggerLevel(bool level) {
  if (m_cap_dc1394) {
    if (!m_cap_dc1394->setSoftwareTriggerLevel(level)) { // Error
      m_state = CGenericSensor::ssError;
...
void CCameraSensor::setPathForExternalImages(const std::string &directory) {
  if (!mrpt::system::createDirectory(directory)) {
    THROW_EXCEPTION_FMT(
        "Error: Cannot create the directory for externally saved images: %s",
        directory.c_str())
  }
...
CCameraSensor::Ptr mrpt::hwdrivers::prepareVideoSourceFromUserSelection() {
  ...
  if (!mrpt::gui::WxSubsystem::createOneInstanceMainThread()) {
    std::cerr << "[mrpt::hwdrivers::prepareVideoSourceFromUserSelection] Error "
                 "initiating Wx subsystem."
              << std::endl;
      ...
      if (m_hook_pre_save) {
        if (IS_DERIVED(i->second, CObservation)) {
          mrpt::obs::CObservation::Ptr obs =
              std::dynamic_pointer_cast<mrpt::obs::CObservation>(i->second);
