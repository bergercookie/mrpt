...
void CHokuyoURG::doProcessSimple(
  ...
  m_state = ssWorking;
  if (!receiveResponse(m_lastSentMeasCmd.c_str(), rcv_status0, rcv_status1,
  ...
  if (expectedSize != rcv_dataLength) {
    MRPT_LOG_DEBUG_FMT("[CHokuyoURG::doProcess] ERROR: Expecting %u data "
                       "bytes, received %u instead!\n",
                       expectedSize, rcv_dataLength);
    hardwareError = true;
    return;
  }
  // Delay the sync of timestamps due to instability in the constant rate during
...
void CHokuyoURG::loadConfig_sensorSpecific(
  ...
  if (!m_ip_dir.empty()) {
    ASSERTMSG_(m_port_dir, "A TCP/IP port number `PORT_DIR` must be specified "
...
bool CHokuyoURG::setHighBaudrate() {
  ...
  // Receive response:
  if (!receiveResponse(cmd, rcv_status0, rcv_status1, rcv_data,
                       rcv_dataLength)) {
...
/*-------------------------------------------------------------
                                                receiveResponse
-------------------------------------------------------------*/
bool CHokuyoURG::receiveResponse(const char *sentCmd_forEchoVerification,
                                 char &rcv_status0, char &rcv_status1,
                                 char *rcv_data, int &rcv_dataLength) {
  if (!checkCOMisOpen())
    return false;
...
bool CHokuyoURG::displaySensorInfo(TSensorInfo *out_data) {
  ...
  if (out_data) {
    ...
    if (nullptr != (ptr = strstr(rcv_data, "DMAX:")))
      ...
      cerr << "[CHokuyoURG::displayVersionInfo] Parse error: didn't find DMAX"
           << endl;

    if (nullptr != (ptr = strstr(rcv_data, "DMIN:")))
      out_data->d_min = 0.001 * atoi(ptr + 5);
    else
      cerr << "[CHokuyoURG::displayVersionInfo] Parse error: didn't find DMIN"
           << endl;

    if (nullptr != (ptr = strstr(rcv_data, "ARES:")))
      out_data->scans_per_360deg = atoi(ptr + 5);
    else
      cerr << "[CHokuyoURG::displayVersionInfo] Parse error: didn't find ARES"
           << endl;

    if (nullptr != (ptr = strstr(rcv_data, "SCAN:")))
      out_data->motor_speed_rpm = atoi(ptr + 5);
    else
      cerr << "[CHokuyoURG::displayVersionInfo] Parse error: didn't find SCAN"
           << endl;

    if (nullptr != (ptr = strstr(rcv_data, "AMIN:")))
      out_data->scan_first = atoi(ptr + 5);
    else
      cerr << "[CHokuyoURG::displayVersionInfo] Parse error: didn't find AMIN"
           << endl;
    if (nullptr != (ptr = strstr(rcv_data, "AMAX:")))
      out_data->scan_last = atoi(ptr + 5);
    else
      cerr << "[CHokuyoURG::displayVersionInfo] Parse error: didn't find AMAX"
           << endl;
    if (nullptr != (ptr = strstr(rcv_data, "AFRT:")))
      out_data->scan_front = atoi(ptr + 5);
    else
      cerr << "[CHokuyoURG::displayVersionInfo] Parse error: didn't find AFRT"
           << endl;
  ...
  if (m_stream) {
    ...
    if (!m_ip_dir.empty() && m_port_dir) {
      ...
      if (COM != nullptr) {
        ...
        cerr << "[CHokuyoURG] Socket connection lost! trying to reconnect..."
             << endl;
    ...
    if (m_com_port.empty() && m_ip_dir.empty() && !m_port_dir) {
      THROW_EXCEPTION("No stream bound to the laser nor COM serial port or ip "
                      "and port provided in 'm_com_port','m_ip_dir' and "
                      "'m_port_dir'");
    }
    ...
    if (!m_ip_dir.empty()) {
      ...
      if (!theCOM->isConnected()) {
        cerr << "[CHokuyoURG] Cannot connect with the server '" << m_com_port
             << "'" << endl;
        delete theCOM;
        return false;
      }
...
void CHokuyoURG::purgeBuffers() {
  ...
  } else // Socket connection
  {
    ...
    if (to_read) {
      ...
      if (nRead != to_read)
        THROW_EXCEPTION("Error in purge buffers: read and expected number of "
                        "bytes are different.");
