...
CSerialPort::CSerialPort(const string &portName, bool openNow)
    : m_serialName(portName), m_totalTimeout_ms(0), m_interBytesTimeout_ms(0),
#ifdef MRPT_OS_WINDOWS
      hCOM(nullptr)
#else
...
CSerialPort::CSerialPort()
    : m_serialName(), m_totalTimeout_ms(0), m_interBytesTimeout_ms(0),
#ifdef MRPT_OS_WINDOWS
      hCOM(nullptr)
#else
      hCOM(-1) // Not connected
...
void CSerialPort::open() {
...
#ifdef MRPT_OS_WINDOWS
  // Check name:
  if (!m_serialName.size())
    THROW_EXCEPTION("Serial port name is empty!!")
  ...
  if (INVALID_HANDLE_VALUE ==
      (hCOM = CreateFileA(m_serialName.c_str(), // Serial Port name
                          ...
                          OPEN_EXISTING, 0, 0))) {
    hCOM = nullptr;
    THROW_EXCEPTION_FMT("Error trying to open serial port: %s",
                        m_serialName.c_str());
  }
  ...
  if (!m_serialName.size())
    THROW_EXCEPTION("Serial port name is empty!!")
  if (m_serialName[0] != '/')
    m_serialName = string("/dev/") + m_serialName;
  ...
  if (-1 == (hCOM = ::open(m_serialName.c_str(), O_RDWR | O_NOCTTY | O_NDELAY)))
    THROW_EXCEPTION_FMT("Error trying to open the serial port %s!!",
                        m_serialName.c_str());
  ...
  if (tcsetattr(hCOM, TCSANOW, &port_settings) < 0)
    THROW_EXCEPTION_FMT("Cannot set the new config to the serial port: %s",
                        strerror(errno));
...
/* -----------------------------------------------------
                setConfig
   ----------------------------------------------------- */
void CSerialPort::setConfig(int baudRate, int parity, int bits, int nStopBits,
  ...
  // Port must be open!
  if (!isOpen())
    THROW_EXCEPTION("The serial port is not open");
  ...
  if (special_rate) {
    ...
    if (actual_rate != baudRate)
      cout << "[CSerialPort::setConfig] Setting custom baud rate to "
           << actual_rate << ", the closer I can make to " << baudRate << endl;
#else
  ...
  if ((cfsetispeed(&port_settings, BR) < 0) ||
    ...
    THROW_EXCEPTION_FMT("Cannot change baudRate in setting structure: %s",
                        strerror(errno));
  ...
  if (tcgetattr(hCOM, &port_settings_verif) < 0)
    THROW_EXCEPTION_FMT("Cannot get the settings to verify: %s",
                        strerror(errno));
...
void CSerialPort::setTimeouts(int ReadIntervalTimeout,
  ...
  timeouts.ReadTotalTimeoutConstant =
      ReadTotalTimeoutConstant; // Milisegundos de espera en cada operacion de
                                // recepcion
 ...
 * eol are found.
  */
std::string CSerialPort::ReadString(const int total_timeout_ms,
                                    bool *out_timeout, const char *eol_chars) {
  MRPT_TRY_START
  ...
  if (out_timeout)
    *out_timeout = false; // Will be set to true on timeout
  ...
  while (total_timeout_ms < 0 || (m_timer.Tac() * 1e3 < total_timeout_ms)) {
    ...
    if (actuallyRead) { // Append to string, if it's not a control char:
    ...
    int waiting_bytes = 0;
    if (ioctl(hCOM, FIONREAD, &waiting_bytes) < 0) {
      if (errno == EIO) { // The port has been disconnect (for USB ports)
        this->close();
    ...
    if (waiting_bytes > 0) {
      ...
      if (nRead) { // Append to string, if it's not a control char:
        if (!strchr(eol_chars, buf[0]))
          receivedStr.push_back(buf[0]);
        else { // end of string!
          return receivedStr;
      ...
      // we decide to move the sleep here to satisfy realtime requirement in the
      // case where we are waiting a n-length string at a frequency
      // greater than 1/n...
...
size_t CSerialPort::Write(const void *Buffer, size_t Count) {
  ...
  do {
    ...
    num_of_bytes_written = write(hCOM, reinterpret_cast<const char *>(Buffer) +
                                           total_bytes_written,
                                 Count - total_bytes_written);
    // cout << "wr: " << num_of_bytes_written << " tot: " << total_bytes_written
    // << " of " << Count << " err: " << errno << endl;
  ...
  if (num_of_bytes_written <
    ...
    THROW_EXCEPTION_FMT("Error writing data to the serial port: %s",
                        strerror(errno));
