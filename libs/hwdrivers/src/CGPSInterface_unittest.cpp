...
TEST(CGPSInterface, parse_NMEA_GGA) {
  ...
  {
    const char *test_cmd = "$GPGGA,101830.00,3649.76162994,N,00224.53709052,W,"
                           "2,08,1.1,9.3,M,47.4,M,5.0,0120*58";
    ...
    EXPECT_NEAR(msg->fields.longitude_degrees, -(002 + 24.53709052 / 60.0),
                1e-10);
...
TEST(CGPSInterface, parse_NMEA_ZDA) {
  ...
  EXPECT_TRUE(msg->fields.UTCTime.sec == 13.0); // Replaced from EXPECT_EQ() to
                                                // avoid a "bus error" in a
                                                // gtest template under armhf.
}
