...
protected:
  ...
  uint8_t calcChecksum(void) const {
    return computeChecksum(((uint8_t *)m_buffer) + 1,
                           getTotalMessageSize() - 2);
  }
...
public:
  ...
  /*! \brief Create a Message object with the given data length and message Id.
     ...
     the given
          data length.
          \param msgId	The message Id that will be assigend to the
     m_messageId field.
          \param length	The length of the data in the message. This value
     is stored in
                                          \c m_createdLength as well as \c
     m_length or \c m_extendedLength.
          \param maxLength	The maximum data length that can be stored in
     the structure.
  */
  Message(const uint8_t msgId = 0, const uint16_t length = 0,
  ...
  /*! \brief Create a message from the given source string
     ...
     the structure.
  */
  Message(const uint8_t *source, const uint16_t size,
          const uint16_t maxLength = CMT_MAXMSGLEN);
  ...
  */
  ...
  /*! \brief Return current data value as double, conversion depends on
     ...
     start reading.
  */
  double getDataFPValue(const uint64_t outputSettings,
  ...
  */
  void getDataFPValue(double *dest, const uint64_t outputSettings,
                      uint16_t offset, const int16_t numValues) const;
  ...
  */
  ...
  /*! \brief Write a string of bytes into the data buffer.
          ...
          \param offset	An optional offset in the data buffer from where to
     start writing.
          \param count	An optional number of bytes to write, if set to 0
     (not set), as
                                          many bytes as will fit into the buffer
     from the given point will
                                          be written.
  */
  ...
  */
  ...
  /*! \brief Write a double (64 bits) into the data buffer, after converting it
     to F1220.
  ...
  */
  void setDataFPValue(const uint64_t outputSettings, const double *data,
                      uint16_t offset, const uint16_t numValues);
