...
#define CMT_CHECKASSERT \
	if (m_checkvar != CMT_CHECKVAL) throw "Message assertion failed!";
#else
#define CMT_CHECKVAR
#define CMT_CHECKVAL
...
class Message
{
   protected:
	...
	uint8_t calcChecksum(void) const
	{
		return computeChecksum(
			((uint8_t*)m_buffer) + 1, getTotalMessageSize() - 2);
	}
   ...
   public:
	...
	/*! \brief Create a Message object with the given data length and message
	   Id.

		The function allocates enough memory to hold an entire message with the
	   given
		data length.
		\param msgId	The message Id that will be assigend to the m_messageId
	   field.
		\param length	The length of the data in the message. This value is
	   stored
	   in
						\c m_createdLength as well as \c m_length or \c
	   m_extendedLength.
		\param maxLength	The maximum data length that can be stored in the
	   structure.
	*/
	Message(
		const uint8_t msgId = 0, const uint16_t length = 0,
	...
	*/
	...
	/*! \brief Return the current value of the data as an unsigned byte (8
	   bits).
	...
	*/
	...
	/*! \brief Return the current value of the data as a double, converting it
	   from FP 12.20
	...
	*/
	...
	/*! \brief Return current data value as double, conversion depends on
	   outputSettings.
	...
	*/
	double getDataFPValue(
		const uint64_t outputSettings, const uint16_t offset = 0) const;
	...
	*/
	const uint8_t* getMessageStart(void) const { return (uint8_t*)m_buffer; }
	/*! \brief Return the length of the message buffer.

		The function returns the total size of the message, including the
	   checksum. This
		is in effect the number of bytes that would be transferred if the
	   message
	   were to
		be sent over a communications channel.
	*/
	uint16_t getTotalMessageSize(void) const;
	//! Compute the checksum and compare it with the stored checksum. Equal is
	//! ok.
	...
	*/
	...
	/*! \brief Compute the checksum field and fill it.
	   ...
	   have
		somehow managed to mess it up, this function can be used to recompute
	   it.
	*/
	...
	/*! \brief Write a string of bytes into the data buffer.
		...
		\param offset	An optional offset in the data buffer from where to
	   start
	   writing.
		\param count	An optional number of bytes to write, if set to 0 (not
	   set),
	   as
						many bytes as will fit into the buffer from the given
	   point will
						be written.
	*/
	void setDataBuffer(
		const uint8_t* data, const uint16_t offset = 0,
	...
	*/
	...
	/*! \brief Write a double (64 bits) into the data buffer, after converting
	   it
	   to F1220.
	...
	*/
	void setDataFPValue(
		const uint64_t outputSettings, const double data,
	...
	*/
	...
	//! Remove a number of bytes from the message (this will reduce the message
	//! size)
	void deleteData(uint16_t size, uint16_t offset = 0);
	//! Insert a number of bytes into the message (this will increase the
	//! message
	//! size)
