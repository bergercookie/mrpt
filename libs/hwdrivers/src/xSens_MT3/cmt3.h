...
class Cmt3
{
   ...
   protected:
	...
	/** The sample frequency of the port, computed from sampling period and
	 * output
	 * skip factor. */
   ...
   protected:
	 ...
	 * possible
	 * and indicates that the devId was not found. */
   ...
   public:
	 ...
	 * a
	 * value between 0 and 255 that indicates the voltage of the batteries. The
	 * scale is not linear and the values should not be used as an absolute
	 ...
	 * sensors
	 * attached to the Xbus Master and the used output options. */
	XsensResultValue getBatteryLevel(uint8_t& level);
	/** \brief Get the baudrate used by a port. \details This function returns
	 * the
	 * baud rate at which the port is currently connected. The function will
	 ...
	 * (1)
	 * or not (0). When it is switched off, the attached MT devices have no
	 * power
	 * and communication with them is not possible. Before going to measurement
	 * mode, use setBusPowerState to restore power. \see setBusPowerState \note
	 ...
	 * \details
	 * Any manipulation of the object should be done through Cmt3. Cmt3's
	 * integrity is not guaranteed if the Cmt2f object is manipulated directly.
	 * \see refreshCache */
	Cmt2f* getCmt2f(void);
	 ...
	 * Cmt3's
	 * integrity is not guaranteed if the Cmt2s object is manipulated directly.
	 * \see refreshCache */
	Cmt2s* getCmt2s(void);
	 ...
	 * is
	 * cached by Cmt3, so it is available in measurement mode. */
	 ...
	 * of
	 * bytes that are in a data message as sent by the given device. */
	 ...
	 * When
	 * no devices are connected, a 0 ID is supplied. */
	 ...
	 * device
	 * mode from the cached configuration, so it is available in measurement
	 * mode.
	 * \see refreshCache */
	 ...
	 * and
	 * output settings. \see setDeviceMode \note This function actually reads
	 * the
	 ...
	 * Orientation
	 * data. The eMTs data is quite large, but it is cached. The first request
	 * should be done in configuration mode, but following requests can be done
	 * in
	 * measurement mode. When requesting eMTS data for a single sensor, the
	 * buffer
	 * should be at least CMT_EMTS_SIZE bytes long. When using
	 * CMT_DID_BROADCAST,
	 * the eMTS data of all connected sensors is placed into the buffer
	 * sequentially. In the latter case, the buffer should be able to hold at
	 * least sensorcount * CMT_EMTS_SIZE bytes. */
	 ...
	 * returns
	 * the XsensResultValue of the last problem reported by hardware (if any).
	 * Hardware problems are all 'error' messages returned by a sensor. \param
	 * did
	 * If any problems were found, the responsible device ID will be returned in
	 * this parameter. \see clearHwError */
	 ...
	 * reset
	 * the hardware error code reported by getHwError. \see getHwError */
	 ...
	 * function
	 * will return the current read position in the open log file in bytes from
	 * the start. \note The read and write positions of log files are completely
	 * independent of each other. \remarks To change the read position, either
	 * use
	 * resetLogFileReadPos or manipulate the log file through getCmt2f. \see
	 ...
	 * range
	 * is -pi to +pi. The magnetic declination is used in the sensor fusion
	 * process to determine the output orientation. \see getHeading setHeading
	 ...
	 * the
	 * product code. This buffer should be at least 20 bytes. \remarks The
	 * product
	 * code is NOT necessarily NULL-terminated. If it is less than 20 characters
	 * it will be, but if the product code is 20 characters it won't be. The
	 * recommended method is to create a buffer that is 21 bytes long and set
	 * the
	 * last byte to 0 before calling this function. \note This function is only
	 ...
	 * determines
	 * the sample frequency of the device from the cached sample rate and skip
	 * factor and returns it. For devices connected to an Xbus Master, the
	 * values
	 * used by the Xbus Master are returned. \see setDeviceMode getDeviceMode */
	 ...
	 * connected
	 * by a serial connection. In most cases this is the same as getBaudrate,
	 * but
	 * when an Xbus Master is connected by a Bluetooth connection, it doesn't
	 * have
	 * to be. \see setSerialBaudrate setBaudrate getBaudrate \note This function
	 ...
	 * be
	 * used for sensors connected to an Xbus Master. \see setSyncInSettings
	 ...
	 * \details
	 * This function retrieves the current inbound synchronization mode of the
	 * MT
	 * device. This function does not work for Xbus Masters and should not be
	 * used
	 * for sensors connected to an Xbus Master. \see setSyncInMode
	 * setSyncInSettings getSyncInSettings getSyncInSkipFactor
	 * setSyncInSkipFactor
	 * getSyncInOffset setSyncInOffset getSyncOutSettings setSyncOutSettings
	 ...
	 * not be used for sensors connected to an Xbus Master. \see setSyncInOffset
	 * setSyncInSettings getSyncInSettings getSyncInSkipFactor
	 * setSyncInSkipFactor
	 * getSyncOutSettings setSyncOutSettings getSyncMode setSyncMode */
	 ...
	 * mode
	 * of the MT device. This function does not work for Xbus Masters and should
	 * not be used for sensors connected to an Xbus Master. \see setSyncOutMode
	 ...
	 * should not be used for sensors connected to an Xbus Master. \see
	 * setSyncOutPulseWidth setSyncOutSettings getSyncOutSettings setSyncOutMode
	 ...
	 * factor of the MT device. This function does not work for Xbus Masters and
	 * should not be used for sensors connected to an Xbus Master. \see
	 * stSyncOutSkipFactor setSyncOutSettings getSyncOutSettings setSyncOutMode
	 ...
	 * mode messages. This function can be used to determine the current config
	 * mode timeout in ms. \see setTimeoutConfig getTimeoutMeasurement
	 ...
	 * will
	 * retrieve the most recent UTC time. In config mode, the time returned will
	 * be requested diectly from the GPS subsystem, with several layers of
	 ...
	 * estimated
	 * (with a very high precision) for the remaining samples. So in config
	 * mode,
	 * the time will probably jitter more than in measurement mode. */
	 ...
	 * This
	 * function places the sensors in configuration mode. \note This function
	 * has
	 ...
	 * no
	 * effect when reading from a log file */
...
//! the given COM port number. \details This
//! function is first passed through to the
//! Cmt2s object. Then, the device settings
//! are retrieved and stored locally. This
...
//! gotoConfig. \see gotoConfig closePort
	 ...
	 * to
	 * decide whether a readDataPacket should be called or for example
	 * getBatteryLevel or getGpsStatus. When the peek function is not used and
	 * for
	 * example getBatteryLevel is called, all messages between the current read
	 * position and the first battery level message in the file will be skipped.
	 * An alternative would be to get the read position, call the desired
	 * function
	 * (get battery level) and restore the read position, but then the moment at
	 * which the (battery level, UTC time, satellite info, etc) data becomes
	...
	XsensResultValue requestData(
		Packet* pack);  //!\brief Request a data message
	 ...
	 * setting
	 * to the device, perform the CMT_RESETORIENTATION_STORE operation while in
	 * Config mode after having performed the appropriate orientation reset in
	 * measurement mode. */
	 ...
	 * function
	 * to change the baudrate of the device. This actually tries to change the
	 * baud rate of the current connection. When reconnect is set to true, the
	 * device receives a Reset message and the port is reopened at the new
	 * baudrate. Otherwise, make sure to perform a Reset manually, since the new
	 * baudrate will not be set until a Reset has been performed. \note To
	 * change
	 * the baudrate of the serial connection while connected via Bluetooth, use
	 * setSerialBaudrate. \see getBaudrate setSerialBaudrate getSerialBaudrate
	 ...
	 * function
	 * sets the state of the bluetooth communication to on or off. \note This
	 ...
	 * still
	 * keeping a connection to the system. However, there is a relatively long
	 * startup time when restoring power as the sensors are reinitialized. \note
	 * You will need to perform an initBus and possibly a refreshCache call
	 * after
	 * switching the power back on with this function. \note This function is
	 * only
	 * available in configuration mode. \see initBus refreshCache */
	 ...
	 * is
	 * set to true. The function will automatically update only the part of the
	 * device mode that is relevant for the device. So it is possible to
	 * configure
	 * all devices, including an Xbus Master with the same mode (only the Xbus
	 * Master will update its period, while the Motion Trackers will update
	 * their
	 * output mode and settings). \note This function is only available in
	 * configuration mode. \see getDeviceMode getDeviceMode2 setDeviceMode2 */
	 ...
	 * their
	 * output mode and settings). \note This function is only available in
	 * configuration mode. \see getDeviceMode2 getDeviceMode setDeviceMode */
	 ...
	 * handles
	 * errors. See the low-level communication documentation for more details.
	 ...
	 * in
	 * configuration mode. \see getLocationId */
	 ...
	 * to
	 * +pi. The magnetic declination is used in the sensor fusion process to
	 * determine the output orientation. \note This function is only available
	 * in
	 * configuration mode. \see getHeading setHeading getMagneticDeclination */
	 ...
	 * \details
	 * This function sets the current inbound synchronization settings of the MT
	 * device (sync mode, skip factor and offset). This function does not work
	 * for
	 * Xbus Masters and should not be used for sensors connected to an Xbus
	 * Master. \note This function is only available in configuration mode. \see
	 * getSyncInSettings getSyncOutSettings setSyncOutSettings getSyncMode
	 * setSyncMode */
	 ...
	 * This
	 * function sets the current inbound synchronization mode of the MT device.
	 * This function does not work for Xbus Masters and should not be used for
	 * sensors connected to an Xbus Master. \see getSyncInMode setSyncInSettings
	 * getSyncInSettings getSyncInSkipFactor setSyncInSkipFactor getSyncInOffset
	 * setSyncInOffset getSyncOutSettings setSyncOutSettings getSyncMode
	 * setSyncMode */
	 ...
	 * device. This function does not work for Xbus Masters and should not be
	 * used
	 * for sensors connected to an Xbus Master. \see getSyncInOffset
	 * setSyncInSettings getSyncInSettings getSyncInSkipFactor
	 * setSyncInSkipFactor
	 * getSyncOutSettings setSyncOutSettings getSyncMode setSyncMode */
	 ...
	 * available
	 * in configuration mode. \see getSyncMode getSyncInSettings
	 * setSyncInSettings
	 ...
	 * \details
	 * This function sets the current outbound synchronization settings of the
	 * MT
	 * device (sync mode, skip factor, offset and pulse width). This function
	 * does
	 ...
	 * mode. \see setSyncOutSettings getSyncInSettings setSyncInSettings
	 * getSyncMode setSyncMode */
	 ...
	 * device. This function does not work for Xbus Masters and should not be
	 * used
	 * for sensors connected to an Xbus Master. \see getSyncOutMode
	 * setSyncOutSettings getSyncOutSettings getSyncOutPulseWidth
	 * setSyncOutPulseWidth getSyncOutSkipFactor setSyncOutSkipFactor
	 * getSyncOutOffset setSyncOutOffset getSyncInSettings setSyncInSettings
	 * getSyncMode setSyncMode */
	 ...
	 * device. This function does not work for Xbus Masters and should not be
	 * used
	 * for sensors connected to an Xbus Master. \see getSyncOutOffset
	 ...
	 * since
	 * configuration messages can take longer to process than measurement mode
	 * messages. This function can be used to set the config mode timeout in ms.
	 * \note This function is only available in configuration mode. \see
	 ...
	 * timeout in ms. \note This function is only available in configuration
	 * mode.
	 * \see getTimeoutMeasurement getTimeoutConfig setTimeoutConfig setTimeout
	 */
	 ...
	 * the
	 * dual mode output mode of the Xbus Master. The dual output mode of the
	 * Xbus
	 * Master defines whether it will send data on the serial connection (at the
	 * same baud rate as the bluetooth connection) when it is connected via
	 * Bluetooth. When set to 0, the serial communication is disabled, data is
	 * sent over the serial bus. \note When dual-mode is enabled, the Xbus
	 * Master
	 * will NOT receive commands on the serial bus. \note This function is only
	 * available in configuration mode. \see getXmDualOutputMode */
	 ...
	 * When
	 * both a file and a port are open, the file parameter can be used to
	 * indicate
	 * what data source to read from (true is file, false is port). \note This
	 * function is only available in configuration mode. */
	XsensResultValue refreshCache(const bool file = false);
	/** \brief Wait for a data message to arrive. \details The function waits
	 * for
	 * a data message to arrive or until a timeout occurs. The function will
	 * also
	 ...
	 * be
	 * connected to a communication port or the function will fail. \param
	 ...
	 * true
	 * (default), Cmt3 will immediately start logging incoming messages to the
	 * file, otherwise setLogMode must be called first before logging is
	 * started.
	 * \remarks If a writable file of the same name already exists in the target
	 * location, it will be overwritten without warning. \see closeLogFile
	 * isLogFileOpen openLogFile setLogMode \note This function is available in
	 * configuration AND in measurement mode. */
	 ...
	 * after
	 * closing if it is not read-only. This is mostly useful for temporary
	 ...
	 * name
	 * will be checked against the name of the open log file as well. \return
	 * true
	 * if the log file is open, false if it isn't \see closeLogFile
	 * createLogFile
	 ...
	 * open.
	 * It is recommended to use a fully qualified path+filename. \note This
	 * function is only available in configuration mode. \see closeLogFile
	 ...
	 * possible
	 * to create a log file while a port is open. In some rare cases it may be
	 * required to log data and read it back while the port remains open and
	 * then
	 * continue logging in the same file. This is not recommended. Instead, log
	 * to
	 * a file, close it and then open it in another Cmt3 object. However, if you
	 * must do this, then this function can be used to specify that you want to
	 * switch between reading from the log file (true) or port (false). */
	XsensResultValue setDataSource(bool readFromFile);
	/** \brief Set the logging mode. \details This function sets the logging
	 * mode
	 * to enabled (true) or disabled (false). When enabled, all received
	 * messages
	 * are logged to the file. \remarks Some information that is cached on the
	 * host PC will generate faked messages in the log file. Eg.
	 * getConfiguration
	 * and some others. */
	XsensResultValue setLogMode(bool active);
	/** \brief Restart reading from the start of the open log file. \details
	 * This
	 * function resets the read position to the start of the open log file. Only
	 * the read position is affected, the write position remains the same. \see
	 * openLogFile */
	...
	//! log file. \details This function can be used to
	//! add custom messages to a log file. \note A
	 ...
	 * Tracker.
	 * See the supplied documentation for more information about scenarios.
	 * \param
	 * scenarios A buffer for storing the available scenarios. The supplied
	 * buffer
	 * should be able to contain at least 6 scenarios, but for future devices a
	 * larger buffer is advised. Use CMT_MAX_SCENARIOS_IN_MT+1. The list is
	 * terminated by a scenario that has type 0. \remarks The type of any of the
	 * returned scenarios can be safely supplied to the setScenario function.
	 * \see
	 ...
	 * device
	 * when outputting orientation and/or position data. \see
	 ...
	 * in
	 * the list supplied by getAvailableScenarios. \see getAvailableScenarios
	 * getScenario \note This function is only available in configuration mode.
	 */
	 ...
	 * absolute
	 * acceleration from measured acceleration. \see setGravityMagnitude \note
	 * This function is only available in configuration mode. */
	 ...
	 * a
	 * custom value in m/s2. The default value of 9.812687357684514m/s2 should
	 * be
	 * fine for most places on earth, but in some cases the gravity may be
	 ...
	 * risk.
	 * \see getGravityMagnitude \remarks In future versions, the MTi-G may be
	 * able
	 * to estimate the local gravity magnitude on its own. \note This function
	 * is
	 ...
	 * to
	 * retrieve the vector currently used as the GPS lever arm in meters. The
	 * GPS
	 * lever arm is the relative position of the GPS antenna to the MTi-G unit.
	 * The arm is specified in the object coordinate system. See the manual for
	 * more information on coordinate systems, alignment resets and the lever
	 * arm.
	 * \see setGpsLeverArm \note This function is only available in
	 * configuration
	 ...
	 * to
	 * set the vector currently used as the GPS lever arm. The GPS lever arm is
	 * the relative position of the GPS antenna to the MTi-G unit. The arm is
	 ...
	 * non-volatile
	 * memory. \details This function allows you to store some critical
	 * components
	 * of the internal XKF filter state to non-volatile memory. The stored
	 * settings will be used to initialize the filter whenever the sensor is
