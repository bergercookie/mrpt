...
/*! \brief Memory releasing janitor class
	...
	This class can be used to make sure that the associated object is deleted
   when the
	janitor leaves scope.
*/
template <class T>
...
/*! \brief Class function calling janitor class
	...
	This class can be used to make sure that the given class function is called
   when the
	janitor leaves scope.
*/
...
class JanitorClassFunc
{
   ...
   public:
	JanitorClassFunc<T, R>(
		T& control, t_func_JanitorClasssFunc func, bool enabl = true)
...
/*! \brief Function calling janitor class
   ...
   given
	object when the janitor leaves scope. Take care that the object is not of a
   type that
...
/*! \brief Log / printf-like function calling janitor class
	...
	This class can be used to make sure that the given printf-like function is
   called with the
	supplied parameter when the janitor leaves scope. Take care that the object
   is not of a type that
	is destroyed before the function unrolling begins.
*/
...
class JanitorLogFunc
{
   ...
   public:
	JanitorLogFunc<T, C, R>(
		t_func_JanitorLogFunc func, const char* str, T& control,
		bool enable = true)
	...
	~JanitorLogFunc<T, C, R>()
	{
		if (m_enabled) (*m_funcJF)(m_str, (C)m_control);
	}
...
class JanitorFuncStdCall
{
   ...
   public:
	JanitorFuncStdCall<T, R>(
		t_func_JanitorFuncStdCall func, T& control, bool enabl = true)
