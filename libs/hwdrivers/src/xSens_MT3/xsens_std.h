...
typedef enum /*! \cond NODOXYGEN */ _XsensResultValue_ /*! \endcond */ {
  ...
  ,
  XRV_MEASUREMENTFAIL1 = 24 //!< Timer overflow - period too short to collect
                            //!all data from Motion Trackers
  ,
  XRV_MEASUREMENTFAIL2 =
      25 //!< Motion Tracker responds with other than SlaveData message
  ,
  XRV_MEASUREMENTFAIL3 = 26 //!< Total bytes of data of Motion Trackers
  ...
  ,
  XRV_UNEXPECTEDMSG = 263 //!< Unexpected message received (e.g. no acknowledge
                          //!message received)
  ,
  ...
  ,
  XRV_COULDNOTREADSETTINGS = 271 //!< A required settings file could not be
  ...
  ,
  XRV_VERSION_PROBLEM = 301 //!< The object has an unrecognized version, so it's
                            //!not safe to perform the operation
} XsensResultValue;
