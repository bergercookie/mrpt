...
//#	define XSENS_LIST_THROW throw(...)   // JLBC: Changed for MRPT in Linux
//in Debug
...
private:
  void operator=(const List &list); //!< intentionally NOT implemented due to
                                    //!ambiguous nature
...
public:
  ...
  template <typename TB> void appendCopy(const TB &item);
  //! Adds a related item to the end of the list, using the T = TR operator.
  template <typename TR> void appendRelated(const TR &item);
  ...
  //! Retrieves the item at the given index. An index beyond the end returns the
  //! first item.
  ...
  template <typename T2> void twinSortAscending(List<T2> &twin);
  //! Finds an item in an unsorted list (walk over all items) using the T::==
  //! operator
  template <typename TB> uint32_t find(const TB &item) const;
  //! Finds an item in an unsorted list (walk over all items) using the T::==
  //! operator on dereferenced list items
  template <typename TB> uint32_t findDeref(const TB &item) const;
  //! Finds an item in a sorted list (binary search) using the T::== and T::<
  //! operators
  template <typename TB> uint32_t findSorted(const TB &item) const;
  //! Finds an item in a sorted list (binary search) using the T::== and T::<
  //! operators on dereferenced list items
  template <typename TB> uint32_t findSortedDeref(const TB &item) const;
  ...
  //! TB(*source[i])
  template <typename TB> void isDeepCopyOf(const List<T> &source);
...
private:
  void operator=(const IntList &list); //!< intentionally NOT implemented due to
                                       //!ambiguous nature
public:
  //! Standard constructor, creates an empty list with some room for items.
  ...
  void setIncremental(const uint32_t start, const uint32_t end,
                      const int32_t step);
