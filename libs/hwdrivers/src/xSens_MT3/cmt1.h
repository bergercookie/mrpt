...
private:
  ...
  */
  ...
  //! Custom, user supplied parameter for the OnBytesReceived callback function,
...
public:
  ...
  /*! \brief Manipulate the Serial control lines
          ...
          The function manipulates the serial control lines that are indicated
     by the
          mask parameter. Note that only the DTR and RTS lines can be set by
     win32.
          \param mask		Indicates which lines are to be manipulated and
     which should be
                                          left alone.
          \param state	Contains the new state of the control lines.
  */
  XsensResultValue escape(const CmtControlLine mask,
  ...
  */
  ...
  /*! \brief Open a communcation channel to the given serial port name.
          ...
          If the baudrate is set to 0, the baud rate is automatically detected.
     If possible.
  */
  ...
  */
  ...
  /*! \brief Read data from the serial port and put it into the data buffer.
     ...
     m_readBuffer to
          fulfill the request, the data parameter is first filled and the port
     is polled
          afterwards.
          \param maxLength	The maximum amount of data read.
          \param data			Pointer to a buffer that will store the
     received data.
  ...
  */
  ...
  XsensResultValue setCallbackFunction(CmtCallbackSelector tp, int32_t instance,
                                       CmtCallbackFunction func, void *param);
  /*! \brief Set the default timeout value to use in blocking operations.
     ...
     operations.
          If the value is set to or from 0, the low-level serial port settings
     may be
...
public:
  ...
  */
  ...
  /*! \brief Delete the given data from the file.
     ...
     position. This
          ...
          The write position is not changed and the read position	is
     checked for validity.
  */
  XsensResultValue deleteData(const CmtFilePos start, const uint32_t length);
  /*! \brief Find a string of bytes in the file

          The function searches from the current read position until the given
     \c needle is
     ...
     needle.
          ...
          \param pos			Out: The position where the needle was
     found. This will point
                                                  to the first character of the
     found needle.
  */
  XsensResultValue find(const void *needle, const uint32_t needleLength,
                        CmtFilePos &pos);
  /*! \brief Flush all data to be written.
          This function writes any remaining data immediately and does not
     return
          until this is done.
  */
  XsensResultValue flushData(void);
  ...
  /*! \brief Retrieve the filename that was last successfully opened.
          ...
          \param filename	A buffer for storing the filename. The buffer
     should be able
                          to hold the filename. A safe size is to make it at
     least 256 bytes.
  */
  ...
  */
  ...
  //! Open a file.
  XsensResultValue open(const char *filename, const bool create,
                        const bool readOnly);
  //! Open a file using a unicode filename.
  XsensResultValue open(const wchar_t *filename, const bool create,
                        const bool readOnly);
  /*! \brief Read data from the file and put it into the data buffer.
     ...
     file.
          ...
          \param data			Pointer to a buffer that will store the
     read data.
          \param length		pointer to a variable that will store the
     number of bytes
                                                  that were actually read. The
     parameter may be NULL.
  */
  XsensResultValue readData(const uint32_t maxLength, void *data,
                            uint32_t *length);
  /*! \brief Read data from the file and put it into the data buffer.
     ...
     encountered.
          \param length		The actual number of bytes read, including
     the terminator
                                                  character, if encountered.
  */
  XsensResultValue readData(const uint32_t maxLength, const char terminator,
