...
#ifndef _CMT_DLL_IMPORT
  //! default constructor, initializes to the given (default) MT settings
  CmtSyncInSettings(const uint16_t mode = 0, const uint16_t skip = 0,
...
#ifndef _CMT_DLL_IMPORT
  //! default constructor, initializes to the given (default) MT settings
  CmtSyncOutSettings(const uint16_t mode = 0, const uint16_t skip = 0,
                     const uint32_t offset = 0,
...
struct CmtUtcTime {
  ...
  uint8_t m_minute;
  uint8_t m_second;
  uint8_t m_valid; //!< When set to 1, the time is valid, when set to 2, the
                   //!time part is valid, but the date may not be valid. when
                   //!set to 0, the time is invalid and should be ignored.
};
...
struct CmtDeviceMode {
  ...
  /*! \brief Compute the period and skip factor.
          ...
          This function computes the period and skipFactor fields from the
     stored
          m_sampleFrequency field. The maximum error in the frequency is
     approximately
          0.4%, which occurs at 510Hz (= actually 512 Hz). In general, the
     higher
...
struct CmtDeviceMode2 {
  ...
  CmtDeviceMode2(const CmtOutputMode mode = CMT_DEFAULT_OUTPUT_MODE,
      ...
      : m_outputMode(mode), m_outputSettings(settings), m_period(period),
        m_skip(skip) {}
  ...
  */
  ...
  This function does the reverse of the getPeriodAndSkipFactor function, storing
  the
...
struct CmtScenario {
  uint8_t m_type;    //!< The type of the scenario. When set to 255 in an
                     //!operation, the 'current' scenario is used.
  uint8_t m_version; //!< The version of the scenario.
  char m_label[CMT_LEN_SCENARIOLABEL + 1]; //!< The label of the scenario.
  // bool m_inSensor;							//!< When set to true, the scenario is
  // available in the sensor
  char m_filterType; //!< The type of filter this scenario is intended for '3':
                     //!XKF-3, '6': XKF-6. \note The value is a character, so
                     //!XKF-3 is '3', which is hex 0x33
};

#define CMT_MAX_OBJECTS 20

enum CmtCallbackSelector {
  CMT_CALLBACK_ONMEASUREMENTPREPARE = 0, //!< Callback function, called right
                                         //!before sending a GotoMeasurement
                                         //!message
  CMT_CALLBACK_ONMEASUREMENTSTART = 1,   //!< Callback function, called right
                                         //!after successfully switching to
                                         //!Measurement mode
  CMT_CALLBACK_ONMEASUREMENTSTOP = 2,    //!< Callback function, called right
                                      //!before switching from Measurement mode
                                      //!to Config mode
  CMT_CALLBACK_ONPOSTPROCESS =
      3, //!< Callback function, called when a full data bundle is available and
         //!has been processed by the CMT. The first void* parameter supplied to
         //!this function can be handed as the bundle parameter in cmtData...
         //!functions to manipulate the newly received bundle.
  CMT_CALLBACK_ONBYTESRECEIVED =
      4, //!< Callback function, called when bytes have been read from a port
  CMT_CALLBACK_ONMESSAGERECEIVED = 5, //!< Callback function, called when a full
                                      //!message has been received from a port
  CMT_CALLBACK_ONMESSAGESENT = 6      //!< Callback function, called when a full
                                      //!message has been sent by a port
};
...
//#ifdef _WIN32
typedef XsensResultValue(__cdecl *CmtCallbackFunction)(int32_t,
                                                       CmtCallbackSelector,
                                                       void *, void *);
//#else
