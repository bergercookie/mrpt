...
/*! \brief Mid-level serial communication class.
	...
	The class uses CMT level 1, but does not inherit from it. If software needs
   to access
...
class Cmt2s
{
   private:
	...
	/*! \brief The message received function.
		...
		This function is automatically called every time a complete message was
	   read from the
		connected COM port.
	*/
	...
	*/
	...
	/*! \brief Read a message from the COM port.
		...
		The function reads data from the embedded Cmt1s object. The data is then
	   converted
	...
	*/
	...
	/*! \brief Wait for a message to arrive.
		...
		If the msgId parameter is set to a value other than 0, the function will
	   wait
		until a message has arrived with that particular msgId.

		\note msgId 0 is the ReqDeviceID MID, but that is an outgoing only
	   message. It is
		illogical to wait for a message that will never be sent to the host. So
	   0
	   is a
	...
	*/
	XsensResultValue waitForMessage(
...
class Cmt2f
{
   ...
   protected:
	...
	/** When set to true, the file is read-only and attempts to write to it will
	 * fail */
	bool m_readOnly;
