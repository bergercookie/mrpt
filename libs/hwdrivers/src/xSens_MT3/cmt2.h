...
//! Find a valid message in the given buffer. If nothing is found, the function
//! returns -1. Otherwise the index of the first character of the message is
//! returned.
...
/*! \brief Mid-level serial communication class.
        ...
        The class uses CMT level 1, but does not inherit from it. If software
   needs to access
        the level 1 component, it needs to be done through the getCmt1s()
   function.
*/
class Cmt2s {
private:
  ...
  /*! \brief The message received function.
          ...
          This function is automatically called every time a complete message
     was read from the
          connected COM port.
  */
...
protected:
  ...
  //! Buffer for reading data until a valid message is read. Should be rarely
  //! used.
...
public:
  ...
  /*! \brief Return a reference to the embedded Cmt1s object.
          ...
          Any manipulation of the object should be done through Cmt2s. Cmt2s
     integrity is
  ...
  */
  ...
  /*! \brief Read a message from the COM port.
          ...
          The function reads data from the embedded Cmt1s object. The data is
     then converted
          into a Message object.
          If an error occurred, a nullptr pointer is returned and the error code
     can be
  ...
  */
  ...
  /*! \brief Set the default timeout value to use in blocking operations.
          ...
          This function sets the level 2 timeout value. The L1 value is set to
     half the given
          timeout value.
  */
  ...
  /*! \brief Wait for a message to arrive.
          ...
          If the msgId parameter is set to a value other than 0, the function
     will wait
          until a message has arrived with that particular msgId.

          \note msgId 0 is the ReqDeviceID MID, but that is an outgoing only
     message. It is
          illogical to wait for a message that will never be sent to the host.
     So 0 is a
  ...
  */
  XsensResultValue waitForMessage(Message *rcv, const uint8_t msgId,
                                  uint32_t timeoutOverride,
                                  bool acceptErrorMessage);
  /*! \brief Send a message over the COM port.

          The function attempts to write the message over the connected COM
     port.
...
/*! \brief The mid-level file communication class.
        ...
        The class uses CMT level 1, but does not inherit from it. If software
   needs to access
        the level 1 component, it needs to be done through the getCmt1f()
   function.
*/
class Cmt2f {
private:
...
protected:
  ...
  bool m_readOnly; //!< When set to true, the file is read-only and attempts to
...
public:
  ...
  //! Read the next message from the file, when msgId is non-zero, the first
  //! matching message will be returned
