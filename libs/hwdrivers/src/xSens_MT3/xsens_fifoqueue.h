...
/*! \brief A FIFO queue with limited length (cyclic).
        ...
        The class is based on the STL queue class, but has a limited size. If
   more items are
        inserted than would fit, the oldest item is overwritten. The class can
   only handle
        pointer types.
*/
template <class T, bool E = true> class FifoQueue {
private:
#if !defined(QUEUELOG)
...
public:
  ...
  template <bool E2> FifoQueue(const FifoQueue<T, E2> &q) {
    QUEUELOG("[%p]FifoQueue.new (copy), T=%s, size=%d, count=%d, del=%d\n",
             this, typeid(T).name(), q.m_maxCount, q.m_currentCount,
  ...
  template <bool E2> FifoQueue<T, E> &operator=(const FifoQueue<T, E2> &q) {
    QUEUELOG("[%p]FifoQueue.operator =, T=%s, max=%d, count=%d, smax=%d, "
             "scount=%d\n",
...
/*! \brief A FIFO queue with limited length (cyclic).
   ...
   only handle
        non-pointer types.
*/
template <class T> class FifoQueueBasic {
private:
#if !defined(QUEUELOG)
  // write to screen/debug-stream
  void QUEUELOG(const char *str, ...) {
...
public:
  ...
  FifoQueueBasic<T> &operator=(const FifoQueueBasic<T> &q) {
    QUEUELOG("[%p]FifoQueueBase.operator =, T=%s, max=%d, count=%d, smax=%d, "
             "scount=%d\n",
             this, typeid(T).name(), m_maxCount, m_currentCount, q.m_maxCount,
             q.m_currentCount);
    if (m_maxCount != q.m_maxCount) {
