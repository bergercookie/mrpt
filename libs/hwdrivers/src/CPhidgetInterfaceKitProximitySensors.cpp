...
#else
  THROW_EXCEPTION("MRPT Was compiled without the CPhidget support. Recompile "
                  "MRPT to use this class")
#endif
}
...
void CPhidgetInterfaceKitProximitySensors::loadConfig_sensorSpecific(
  ...
  // process_rate = 100 						// Hz (common to all
  // sensors)
  ...
  for (int i = 1; i <= 8; i++) {
    ...
    string sensorType = configSource.read_string(iniSection, sensorNKeyName,
                                                 string("UNPLUGGED"));
...
void CPhidgetInterfaceKitProximitySensors::initialize(void) {
  ...
  int err =
      CPhidget_waitForAttachment(*((CPhidgetHandle *)(m_carteInterfaceKit)),
    ...
    if (m_sensorIsPlugged[i]) {
      ...
      int err = CPhidgetInterfaceKit_getSensorValue(
          *((CPhidgetInterfaceKitHandle *)(m_carteInterfaceKit)), i,
          &sensorValue);
      if (err > 0) {
        string error("Error durring acquiering sensor value on channel : %d",
                     i);
        THROW_EXCEPTION(error);
      }
      switch (m_sensorType[i]) {
