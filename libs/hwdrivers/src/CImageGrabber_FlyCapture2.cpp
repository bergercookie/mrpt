...
template <typename T> struct fc2_str_val {
  const char *str;
  T val;
};
...
#define GET_CONV_TABLE(type)                                                   \
  vector<fc2_str_val<type>> fc2_vals_gen(type) {                               \
    ...
    vector<fc2_str_val<type>> vec(&fc2_##type##_table[0],                      \
                                  &fc2_##type##_table[n]);                     \
    return vec;                                                                \
  }
...
template <typename T> T fc2_defstr2num(const std::string &str) {
...
template <typename T> const char *fc2_defnum2str(const T &val) {
  vector<fc2_str_val<T>> fc2_vals = fc2_vals_gen(T());
  size_t i = static_cast<int>(val);
  if (i < fc2_vals.size())
    return fc2_vals[i].str;
  else
    THROW_EXCEPTION_FMT("Error: Unknown FlyCapture2 enum: %i",
                        static_cast<int>(val))
}
#endif
...
TCaptureOptions_FlyCapture2::TCaptureOptions_FlyCapture2()
    : camera_index(0), open_by_guid(false),
      videomode(), //("VIDEOMODE_640x480Y8"),
      framerate(), // ("FRAMERATE_30"),
      grabmode("BUFFER_FRAMES"), numBuffers(30), grabTimeout(-1),
      trigger_enabled(false), trigger_polarity(0), trigger_source(0),
      trigger_mode(0), strobe_enabled(false), strobe_source(0),
      strobe_polarity(0), strobe_delay(0.0f), strobe_duration(1.0f),
      autoexposure_auto(true), autoexposure_onOff(true), autoexposure_abs(true),
      autoexposure_EV(0.0f), shutter_auto(true), shutter_abs(true),
      shutter_time_ms(4.0f), gain_auto(true), gain_abs(true), gain_dB(0.0f),
      stereo_mode(false), get_rectified(false), rect_width(640),
      rect_height(480) {
...
void TCaptureOptions_FlyCapture2::loadOptionsFrom(
  ...
  trigger_enabled =
      cfg.read_bool(sect, prefix + string("trigger_enabled"), trigger_enabled);
  trigger_polarity =
      cfg.read_int(sect, prefix + string("trigger_polarity"), trigger_polarity);
  trigger_source =
  ...
  strobe_duration =
      cfg.read_float(sect, prefix + string("strobe_duration"), strobe_duration);

  autoexposure_auto = cfg.read_bool(sect, prefix + string("autoexposure_auto"),
                                    autoexposure_auto);
  autoexposure_onOff = cfg.read_bool(
      sect, prefix + string("autoexposure_onOFf"), autoexposure_onOff);
  autoexposure_abs = cfg.read_bool(sect, prefix + string("autoexposure_abs"),
                                   autoexposure_abs);
  autoexposure_EV =
      cfg.read_float(sect, prefix + string("autoexposure_EV"), autoexposure_EV);
  ...
  shutter_time_ms =
      cfg.read_float(sect, prefix + string("shutter_time_ms"), shutter_time_ms);
...
CImageGrabber_FlyCapture2::CImageGrabber_FlyCapture2()
    : m_camera(nullptr), m_camera_info(nullptr), m_img_buffer(nullptr),
      m_triclops(nullptr) {
#if MRPT_HAS_FLYCAPTURE2
...
void CImageGrabber_FlyCapture2::open(const TCaptureOptions_FlyCapture2 &options,
  ...
  if (m_options.open_by_guid) {
    ...
    if (m_options.camera_index >= numCameras)
      THROW_EXCEPTION(
          mrpt::format("Error: camera_index to open is '%u', but only '%u' "
  ...
  cout << mrpt::format("[CImageGrabber_FlyCapture2::open] Opening camera with "
                       "GUID= %08X-%08X-%08X-%08X...\n",
                       guid.value[0], guid.value[1], guid.value[2],
                       guid.value[3]);
    ...
    if (!m_options.stereo_mode) {
      ...
      if (!isSupported) {
        ...
        THROW_EXCEPTION(mrpt::format(
            "Camera mode '%s' + '%s' is not supported by this camera. Current "
            "mode is %d, current rate is %d.",
      ...
      */
      ...
      te = triclopsSetResolution(*(TRI_CONTEXT), m_options.rect_height,
                                 m_options.rect_width);
...

  {
    ...
    fe = FC2_CAM->GetVideoModeAndFrameRate(&curVidMode, &curVidRate);
    if (fe == PGRERROR_OK)
      cout << mrpt::format("[CImageGrabber_FlyCapture2::open] Current camera "
                           "mode is %s, current rate is %s.\n",
  ...
  if (fe == PGRERROR_OK) {
    if (eii.frameCounter.available)
      eii.frameCounter.onOff = true;
    if (eii.timestamp.available)
      eii.timestamp.onOff = true;
    if (eii.exposure.available)
      eii.exposure.onOff = true;
    if (eii.brightness.available)
      eii.brightness.onOff = true;
...
void CImageGrabber_FlyCapture2::startSyncCapture(
  ...
  if (!cam_ptrs.empty()) {
    FlyCapture2::Error error =
        FlyCapture2::Camera::StartSyncCapture(cam_ptrs.size(), &cam_ptrs[0]);
    CHECK_FC2_ERROR(error)
  }
#else
  THROW_EXCEPTION("MRPT compiled without support for FlyCapture2")
...
std::string CImageGrabber_FlyCapture2::getFC2version() {
  ...
  return mrpt::format("%d.%d.%d.%d", fc2Version.major, fc2Version.minor,
...
#if MRPT_HAS_FLYCAPTURE2
  if (!m_camera) {
    std::cerr << "[CImageGrabber_FlyCapture2::getObservation] Camera is not "
                 "opened. Call open() first.\n";
    return false;
  }
  try {
    FlyCapture2::Error error;
    ...
    // Decode image:
    error = image.Convert(is_color ? PIXEL_FORMAT_BGR : PIXEL_FORMAT_MONO8,
                          FC2_BUF_IMG);
    CHECK_FC2_ERROR(error)
    ...
    out_observation.image.loadFromMemoryBuffer(img_cols, img_rows, is_color,
                                               FC2_BUF_IMG->GetData());
...
bool CImageGrabber_FlyCapture2::getObservation(
  ...
  try {
    ...
    if (fterr != Fc2Triclops::ERRORTYPE_OK) {
      Fc2Triclops::handleFc2TriclopsError(fterr,
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      if (m_options.get_rectified) // If rectified
      {
        ...
        te = triclopsBuildPackedTriclopsInput(
            rgbuImage.GetCols(), rgbuImage.GetRows(), rgbuImage.GetStride(),
        ...
        te = triclopsRectifyPackedColorImage(
            *(TRI_CONTEXT), i == 0 ? TriCam_RIGHT : TriCam_LEFT,
            const_cast<TriclopsInput *>(&triclopsColorInput), &rectPackColImg);
        CHECK_TRICLOPS_ERROR(te)
