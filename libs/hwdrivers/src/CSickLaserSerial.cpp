...
CSickLaserSerial::CSickLaserSerial()
    : m_mm_mode(false), m_scans_FOV(180), m_scans_res(50), m_com_port(),
      m_mySerialPort(nullptr), m_com_baudRate(38400), m_nTries_connect(1),
      m_nTries_current(0), m_skip_laser_config(false) {
...
void CSickLaserSerial::loadConfig_sensorSpecific(
  ...
  m_sensorPose =
      CPose3D(configSource.read_float(iniSection, "pose_x", 0),
  ...
  m_skip_laser_config = configSource.read_bool(iniSection, "skip_laser_config",
                                               m_skip_laser_config);
  ...
  try {
    if (!m_stream) {
      ...
      if (!m_com_port.empty()) {
        ...
        throw std::logic_error("ERROR: No serial port attached with bindIO, "
                               "neither it set with 'setSerialPort'");
    }
...
bool CSickLaserSerial::waitContinuousSampleFrame(
  ...
  while (nFrameBytes < (lengthField = (6 + (buf[2] | (buf[3] << 8))))) {
    ...
    try {
      ...
      // Disconnected?
      MRPT_LOG_ERROR_FMT("[CSickLaserSerial::waitContinuousSampleFrame] "
                         "Disconnecting due to comms error: %s\n",
                         e.what());
  ...
  if (CRC_packet != CRC) {
    cerr << format("[CSickLaserSerial::waitContinuousSampleFrame] bad CRC "
                   "len=%u nptns=%u: %i != %i",
...
bool CSickLaserSerial::LMS_setupSerialComms() {
  ASSERT_(m_com_baudRate == 9600 || m_com_baudRate == 38400 ||
  ...
  for (size_t reps = 0; !detected_rate && reps < m_nTries_connect; reps++) {
    ...
    for (size_t i = 0; !detected_rate && i < sizeof(rates) / sizeof(rates[0]);
         i++) {
  ...
  if (!this->LMS_setupBaudrate(m_com_baudRate))
    RET_ERROR("error");
...
bool CSickLaserSerial::LMS_waitIncomingFrame(uint16_t timeout) {
  ...
  if (4U + lengthField + 2U != nBytes) {
    printf("[CSickLaserSerial::LMS_waitIncomingFrame] Error: expected %u "
           "bytes, received %u\n",
  ...
  // Check CRC
  uint16_t CRC = mrpt::utils::compute_CRC16(m_received_frame_buffer,
                                            4 + lengthField, CRC16_GEN_POL);
  uint16_t CRC_packet = m_received_frame_buffer[4 + lengthField + 0] |
                        (m_received_frame_buffer[4 + lengthField + 1] << 8);
  if (CRC_packet != CRC) {
    printf("[CSickLaserSerial::LMS_waitIncomingFrame] Error in CRC: rx: "
...
bool CSickLaserSerial::SendCommandToSICK(const uint8_t *cmd,
  ...
  for (int k = 0; k < NTRIES; k++) {
    if (toWrite != COM->Write(cmd_full, toWrite)) {
      cout << "[CSickLaserSerial::SendCommandToSICK] Error writing data to "
              "serial port."
           << endl;
      return false;
    }
