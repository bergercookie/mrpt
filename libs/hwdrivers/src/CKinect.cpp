	...
	const std::string& iniSection)
{
	...
	{
		std::cout
			<< "[CKinect::loadConfig_sensorSpecific] Warning: Ignoring error "
	...
	bool& hardware_error)
{
		   ...
		   mrpt::system::now() < (tim0 + max_wait))
	{
		// Got a new frame?
		if ((!m_grab_image ||
			 m_tim_latest_rgb != 0) &&  // If we are NOT grabbing
			// RGB or we are and
			// there's a new frame...
			(!m_grab_depth ||
			 m_tim_latest_depth != 0)  // If we are NOT grabbing
			// Depth or we are and
			// there's a new frame...
			)
		{
		...
		(m_grab_depth && m_tim_latest_depth == 0))
	{
		...
		m_latest_obs_cs
			.lock();  // Important: if system is running slow, etc. we
		// cannot tell for sure that the depth buffer is
		// not beeing filled right now:
