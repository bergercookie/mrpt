...
/*-------------------------------------------------------------
 ...
 -------------------------------------------------------------*/
CKinect::CKinect()
    : m_sensorPoseOnRobot(), m_preview_window(false),
      m_preview_window_decimation(1), m_preview_decim_counter_range(0),
      ...
      m_tim_latest_depth(0), m_tim_latest_rgb(0),
#endif
      m_relativePoseIntensityWRTDepth(0, -0.02, 0, DEG2RAD(-90), DEG2RAD(0),
                                      DEG2RAD(-90)),
      m_initial_tilt_angle(360), m_user_device_number(0), m_grab_image(true),
      m_grab_depth(true), m_grab_3D_points(true), m_grab_IMU(true) {
...
void CKinect::loadConfig_sensorSpecific(
  ...
  const mrpt::poses::CPose3D twist(0, 0, 0, DEG2RAD(-90), DEG2RAD(0),
                                   DEG2RAD(-90));

  mrpt::utils::TStereoCamera sc;
  sc.leftCamera = m_cameraParamsDepth; // Load default values so that if we fail
                                       // to load from cfg at least we have some
                                       // reasonable numbers.
  ...
  try {
    ...
    std::cout << "[CKinect::loadConfig_sensorSpecific] Warning: Ignoring error "
  ...
  m_user_device_number =
      configSource.read_int(iniSection, "device_number", m_user_device_number);
  ...
  m_initial_tilt_angle = configSource.read_int(iniSection, "initial_tilt_angle",
...
void rgb_cb(freenect_device *dev, void *img_data, uint32_t timestamp) {
  ...
  if (obj->getVideoChannel() == CKinect::VIDEO_CHANNEL_RGB) {
    ...
    obs.intensityImage.resize(frMode.width, frMode.height, CH_RGB,
    ...
    const cv::Mat src_img_bayer(frMode.height, frMode.width, CV_8UC1, img_data,
                                frMode.width);

    cv::Mat dst_img_RGB = cv::cvarrToMat(obs.intensityImage.getAs<IplImage>(),
...
void CKinect::open() {
  ...
  if (freenect_open_device(f_ctx, f_dev_ptr, m_user_device_number) < 0)
    THROW_EXCEPTION_FMT("Error opening Kinect sensor with index: %d",
                        m_user_device_number)
  ...
  const freenect_frame_mode desiredFrMode = freenect_find_video_mode(
      ...
      m_video_channel == VIDEO_CHANNEL_IR
          ...
          : FREENECT_VIDEO_BAYER // FREENECT_VIDEO_RGB: Use Bayer instead so we
                                 // can directly decode it here
      );
...
void CKinect::getNextObservation(mrpt::obs::CObservation3DRangeScan &_out_obs,
  ...
  // Reset these timestamp flags so if they are !=0 in the next call we're sure
  // they're new frames.
  ...
  while (freenect_process_events(f_ctx) >= 0 &&
    ...
    // Got a new frame?
    if ((!m_grab_image || m_tim_latest_rgb != 0) && // If we are NOT grabbing
                                                    // RGB or we are and there's
                                                    // a new frame...
        (!m_grab_depth || m_tim_latest_depth != 0)  // If we are NOT grabbing
                                                    // Depth or we are and
                                                    // there's a new frame...
        ) {
  ...
  if ((m_grab_image && m_tim_latest_rgb != 0) &&
    ...
    m_latest_obs_cs.lock(); // Important: if system is running slow, etc. we
                            // cannot tell for sure that the depth buffer is not
                            // beeing filled right now:
    m_latest_obs.rangeImage.setSize(m_cameraParamsDepth.nrows,
                                    m_cameraParamsDepth.ncols);
  ...
  if (m_preview_window) {
    if (_out_obs.hasRangeImage) {
      if (++m_preview_decim_counter_range > m_preview_window_decimation) {
        ...
        if (!m_win_range) {
          m_win_range =
              std::make_shared<mrpt::gui::CDisplayWindow>("Preview RANGE");
    ...
    if (_out_obs.hasIntensityImage) {
      if (++m_preview_decim_counter_rgb > m_preview_window_decimation) {
        ...
        if (!m_win_int) {
          m_win_int =
              std::make_shared<mrpt::gui::CDisplayWindow>("Preview INTENSITY");
...
void CKinect::setPathForExternalImages(const std::string &directory) {
  ...
  //externally saved images: %s",directory.c_str() )
  //	}
