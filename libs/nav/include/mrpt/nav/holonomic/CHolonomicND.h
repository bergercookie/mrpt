...
public:
  ...
  /** The set of posible situations for each trajectory. (mrpt::utils::TEnumType
  ...
  struct NAV_IMPEXP TOptions : public mrpt::utils::CLoadableOptions {
    ...
    std::vector<double> factorWeights; //!< Vector of 4 weights: [0]=Free space,
                                       //![1]=Dist. in sectors, [2]=Closer to
                                       //!target (Euclidean), [3]=Hysteresis
  ...
  TOptions options; //!< Parameters of the algorithm (can be set manually or
                    //!loaded from CHolonomicND::initialize or
...
private:
  ...
  /**  Find gaps in the obtacles.
    */
  void gapsEstimator(const std::vector<double> &obstacles,
                     const mrpt::math::TPoint2D &in_target, TGapArray &gaps);

  /** Search the best gap.
    */
  void searchBestGap(const std::vector<double> &in_obstacles,
                     const double in_maxObsRange, const TGapArray &in_gaps,
  ...
  /** Fills in the representative sector field in the gap structure:
    */
  void calcRepresentativeSectorForGap(TGap &gap,
                                      const mrpt::math::TPoint2D &target,
                                      const std::vector<double> &obstacles);

  /** Evaluate each gap:
    */
  void evaluateGaps(const std::vector<double> &in_obstacles,
                    const double in_maxObsRange, const TGapArray &in_gaps,
                    const unsigned int TargetSector, const float TargetDist,
                    std::vector<double> &out_gaps_evaluation);

}; // end of CHolonomicND
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CHolonomicND,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CLogFileRecord_ND,
...
namespace utils {
template <> struct TEnumTypeFiller<nav::CHolonomicND::TSituations> {
  typedef nav::CHolonomicND::TSituations enum_t;
  static void fill(bimap<enum_t, std::string> &m_map) {
    m_map.insert(nav::CHolonomicND::SITUATION_TARGET_DIRECTLY,
    ...
    m_map.insert(nav::CHolonomicND::SITUATION_NO_WAY_FOUND,
                 "SITUATION_NO_WAY_FOUND");
  }
};
} // End of namespace
