...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CLogFileRecord_VFF,
...
public:
   ...
   * or default values if set to NULL. */
  ...
  struct NAV_IMPEXP TOptions : public mrpt::utils::CLoadableOptions {
    double TARGET_SLOW_APPROACHING_DISTANCE; //!< For stopping gradually
                                             //!(Default: 0.10)
    double TARGET_ATTRACTIVE_FORCE; //!< Dimension-less (may have to be tuned
                                    //!depending on the density of obstacle
                                    //!sampling) (Default: 20)
  ...
  TOptions options; //!< Parameters of the algorithm (can be set manually or
                    //!loaded from CHolonomicVFF::initialize or
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CHolonomicVFF,
