...
public:
  /**  Initialize the parameters of the navigator, from some configuration file,
   * or default values if set to nullptr */
  ...
  struct NAV_IMPEXP TOptions : public mrpt::utils::CLoadableOptions {
    double TOO_CLOSE_OBSTACLE; //!< Directions with collision-free distances
                               //!below this threshold are not elegible.
    double TARGET_SLOW_APPROACHING_DISTANCE; //!< Start to reduce speed when
                                             //!closer than this to target  [m]
    double OBSTACLE_SLOW_DOWN_DISTANCE;      //!< Start to reduce speed when
                                        //!clearance is below this value ([0,1]
                                        //!ratio wrt obstacle reference/max
                                        //!distance)
    double HYSTERESIS_SECTOR_COUNT;    //!< Range of "sectors" (directions) for
                                       //!hysteresis over successive timesteps
    std::vector<double> factorWeights; //!< See docs above
    std::vector<int32_t> factorNormalizeOrNot; //!< 0/1 to normalize factors.
    std::vector<std::vector<int32_t>> PHASE_FACTORS; //!< Factor indices [0,4]
                                                     //!for the factors to
                                                     //!consider in each phase
                                                     //!1,2,...N of the movement
                                                     //!decision (Defaults:
                                                     //!`PHASE1_FACTORS=0 1 2`,
                                                     //!`PHASE2_FACTORS=`3 4`)
    std::vector<double> PHASE_THRESHOLDS; //!< Phase 1,2,N-1... scores must be
                                          //!above this relative range threshold
                                          //![0,1] to be considered in phase 2
                                          //!(Default:`0.75`)

    bool LOG_SCORE_MATRIX; //!< (default:false, to save space)

    double clearance_threshold_ratio; //!<  Ratio [0,1], times path_count, gives
                                      //!the minimum number of paths at each
                                      //!side of a target direction to be
                                      //!accepted as desired direction
    double gap_width_ratio_threshold; //!<  Ratio [0,1], times path_count, gives
                                      //!the minimum gap width to accept a
                                      //!direct motion towards target.
  ...
  TOptions options; //!< Parameters of the algorithm (can be set manually or
                    //!loaded from CHolonomicFullEval::initialize or
                    //!options.loadFromConfigFile(), etc.)
...
private:
  ...
  mrpt::math::CMatrixD m_dirs_scores; //!< Individual scores for each direction:
                                      //!(i,j), i (row) are directions, j (cols)
                                      //!are scores. Not all directions may have
                                      //!evaluations, in which case a "-1" value
                                      //!will be found.
  ...
  void evalSingleTarget(unsigned int target_idx, const NavInput &ni,
                        EvalOutput &eo); //!< Evals one single target of the
                                         //!potentially many of them in NavInput
};                                       // end of CHolonomicFullEval

DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CHolonomicFullEval,
...
public:
  ...
  double evaluation;
  mrpt::math::CMatrixD dirs_scores; //!< Individual scores for each direction:
                                    //!(i,j), i (row) are directions, j (cols)
                                    //!are scores. Not all directions may have
                                    //!evaluations, in which case a "-1" value
                                    //!will be found.
  int32_t selectedTarget; //!< Normally = 0. Can be >0 if multiple targets
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CLogFileRecord_FullEval,
