...
/** Clearance information for one particular PTG and one set of obstacles.
        ...
        * - Repeatedly call CParameterizedTrajectoryGenerator::updateClearance()
 * for each 2D obstacle point.
        *
        *  \ingroup nav_tpspace
        */
...
public:
  ...
  /** Initializes the container to allocate `decimated_num_paths` entries, as a
   * decimated
   ...
   * origin to that specific spot.
    */
  double getClearance(uint16_t k, double TPS_query_distance,
                      bool integrate_over_path) const;
  void renderAs3DObject(mrpt::opengl::CMesh &mesh, double min_x, double max_x,
                        double min_y, double max_y, double cell_res,
                        bool integrate_over_path) const;
  ...
  inline const dist2clearance_t &
  get_path_clearance_decimated(size_t decim_k) const {
    return m_raw_clearances[decim_k];
  }
...
protected:
  ...
  size_t m_actual_num_paths; // The decimated number of paths is implicit in
                             // raw_clearances.size()
