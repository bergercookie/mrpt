 ...
 *  \ingroup nav_tpspace
 */
class NAV_IMPEXP CPTG_DiffDrive_alpha
    : public CPTG_DiffDrive_CollisionGridBased {
  DEFINE_SERIALIZABLE(CPTG_DiffDrive_alpha)
public:
  CPTG_DiffDrive_alpha() : cte_a0v(0), cte_a0w(0) {}
  CPTG_DiffDrive_alpha(const mrpt::utils::CConfigFileBase &cfg,
  ...
  void ptgDiffDriveSteeringFunction(float alpha, float t, float x, float y,
                                    float phi, float &v,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CPTG_DiffDrive_alpha,
