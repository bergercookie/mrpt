...
struct NAV_IMPEXP TCPoint {
  TCPoint() {}
  TCPoint(const float x_, const float y_, const float phi_, const float t_,
...
mrpt::utils::CStream NAV_IMPEXP &operator<<(mrpt::utils::CStream &o,
                                            const mrpt::nav::TCPoint &p);
mrpt::utils::CStream NAV_IMPEXP &operator>>(mrpt::utils::CStream &i,
                                            mrpt::nav::TCPoint &p);
...
public:
  ...
  virtual void ptgDiffDriveSteeringFunction(float alpha, float t, float x,
                                            float y, float phi, float &v,
                                            float &w) const = 0;
  ...
  virtual bool inverseMap_WS2TP(double x, double y, int &out_k, double &out_d,
  ...
  directionToMotionCommand(uint16_t k) const override;
  virtual mrpt::kinematics::CVehicleVelCmd::Ptr
  ...
  void getPathPose(uint16_t k, uint32_t step,
                   mrpt::math::TPose2D &p) const override;
  double getPathDist(uint16_t k, uint32_t step) const override;
  bool getPathStepForDist(uint16_t k, double dist,
                          uint32_t &out_step) const override;
  ...
  void updateTPObstacle(double ox, double oy,
                        std::vector<double> &tp_obstacles) const override;
  void updateTPObstacleSingle(double ox, double oy, uint16_t k,
                              double &tp_obstacle_k) const override;
...
protected:
  ...
  void internal_initialize(const std::string &cacheFilename = std::string(),
   ...
   * trajectories */
  void simulateTrajectories(float max_time, float max_dist, unsigned int max_n,
                            float diferencial_t, float min_dist,
  ...
  public:
    CCollisionGrid(float x_min, float x_max, float y_min, float y_max,
                   float resolution, CPTG_DiffDrive_CollisionGridBased *parent)
        : mrpt::utils::CDynamicGrid<TCollisionCell>(x_min, x_max, y_min, y_max,
                                                    resolution),
          m_parent(parent) {}
    ...
    bool saveToFile(mrpt::utils::CStream *fil,
        ...
        const; //!< Save to file, true = OK
    bool loadFromFile(mrpt::utils::CStream *fil,
                      const mrpt::math::CPolygon
                          &current_robotShape); //!< Load from file,  true = OK

    /** For an obstacle (x,y), returns a vector with all the pairs (a,d) such as
     * the robot collides */
    const TCollisionCell &getTPObstacle(const float obsX,
                                        const float obsY) const;

    /** Updates the info into a cell: It updates the cell only if the distance d
      *for the path k is lower than the previous value:
            ...
            */
    void updateCellInfo(const unsigned int icx, const unsigned int icy,
                        const uint16_t k, const float dist);
