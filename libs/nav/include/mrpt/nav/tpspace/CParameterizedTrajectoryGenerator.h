...
enum PTG_collision_behavior_t {
  COLL_BEH_BACK_AWAY =
      0, //!< Favor getting back from too-close (almost collision) obstacles.
  COLL_BEH_STOP //!< Totally dissallow any movement if there is any too-close
                //!(almost collision) obstacles.
};
 ...
 *	- 19/JUL/2009: Simplified to use only STL data types, and created the
...
public:
  CParameterizedTrajectoryGenerator();         //!< Default ctor. Must call
                                               //!`loadFromConfigFile()` before
                                               //!initialization
  ...
  /** The class factory for creating a PTG from a list of parameters in a
    ...
    *derived classes)
    *
    * `ptgClassName` can be any PTG class name which has been registered as any
    *other
    * mrpt::utils::CSerializable class.
    *
    * \exception std::logic_error On invalid or missing parameters.
    */
  static CParameterizedTrajectoryGenerator *
  CreatePTG(const std::string &ptgClassName,
   ...
   *  @{ */
  virtual std::string getDescription() const = 0; //!< Gets a short textual
                                                  //!description of the PTG and
                                                  //!its parameters
...
protected:
  ...
  virtual void
  internal_initialize(const std::string &cacheFilename = std::string(),
...
public:
   ...
   * 0-based index).
    * \param[out] out_d Trajectory distance, normalized such that D_max becomes
   * 1.
    *
    * \return true if the distance between (x,y) and the actual trajectory point
   * is below the given tolerance (in meters).
    */
  virtual bool inverseMap_WS2TP(double x, double y, int &out_k,
  ...
  directionToMotionCommand(uint16_t k) const = 0;
  ...
  struct NAV_IMPEXP TNavDynamicState {
    mrpt::math::TTwist2D
        curVelLocal; //!< Current vehicle velocity (local frame of reference)
    mrpt::math::TPose2D relTarget; //!< Current relative target location
    double
        targetRelSpeed; //!< Desired relative speed [0,1] at target. Default=0
...
protected:
  /** Invoked when `m_nav_dyn_state` has changed; gives the PTG the opportunity
   * to react and parameterize paths depending on the instantaneous conditions
   */
...
public:
  ...
  virtual void getPathPose(uint16_t k, uint32_t step,
                           mrpt::math::TPose2D &p) const = 0;
  ...
  * \sa getPathStepCount() */
  ...
  /** Access path `k` ([0,N-1]=>[-pi,pi] in alpha): largest step count for which
   * the traversed distance is < `dist`
    * \param[in] dist Distance in pseudometers (real distance, NOT normalized to
   * [0,1] for [0,refDist])
   ...
   * `out_step`.
    * \sa getPathStepCount(), getAlphaValuesCount() */
  virtual bool getPathStepForDist(uint16_t k, double dist,
                                  uint32_t &out_step) const = 0;
   ...
   * "pseudometers", un-normalized).
    * \param [in] ox Obstacle point (X), relative coordinates wrt origin of the
   * PTG.
    * \param [in] oy Obstacle point (Y), relative coordinates wrt origin of the
   * PTG.
    * \note The length of tp_obstacles is not checked for efficiency since this
   * method is potentially called thousands of times per
    *  navigation timestap, so it is left to the user responsibility to provide
   * a valid buffer.
    * \note `tp_obstacles` must be initialized with initTPObstacle() before
   * call.
    */
  virtual void updateTPObstacle(double ox, double oy,
                                std::vector<double> &tp_obstacles) const = 0;

  /** Like updateTPObstacle() but for one direction only (`k`) in TP-Space.
   * `tp_obstacle_k` must be initialized with initTPObstacleSingle() before call
   * (collision-free ranges, in "pseudometers", un-normalized). */
  virtual void updateTPObstacleSingle(double ox, double oy, uint16_t k,
                                      double &tp_obstacle_k) const = 0;
  ...
  /** Only for PTGs supporting supportVelCmdNOP(): this is the maximum time (in
   * seconds) for which the path
    * can be followed without re-issuing a new velcmd. Note that this is only an
  ...
  /** Returns the actual distance (in meters) of the path, discounting possible
   * circular loops of the path (e.g. if it comes back to the origin).
    * Default: refDistance */
  ...
  virtual double evalPathRelativePriority(uint16_t k,
  ...
  virtual bool isPointInsideRobotShape(const double x,
  ...
  virtual double evalClearanceToRobotShape(const double ox,
                                           const double oy) const = 0;

  /** @} */ // --- end of virtual methods

  /** To be invoked by the navigator *before* each navigation step, to let the
   * PTG to react to changing dynamic conditions. * \sa onNewNavDynamicState(),
   * m_nav_dyn_state  */
  ...
  static std::string
      OUTPUT_DEBUG_PATH_PREFIX; //!< The path used as defaul output in, for
                                //!example, debugDumpInFiles.
                                //!(Default="./reactivenav.logs/")

  /** Must be called after setting all PTG parameters and before requesting
   * converting obstacles to TP-Space, inverseMap_WS2TP(), etc. */
  void initialize(const std::string &cacheFilename = std::string(),
                  const bool verbose = true);
  /** This must be called to de-initialize the PTG if some parameter is to be
   * changed. After changing it, call initialize again */
  void deinitialize();
  /** Returns true if `initialize()` has been called and there was no errors, so
   * the PTG is ready to be queried for paths, obstacles, etc. */
   ...
   * tp-obstacles (collision-free ranges, in "pseudometers", un-normalized). \sa
   * updateTPObstacle()  */
   ...
   * meters).
    */
  virtual void
  renderPathAsSimpleLine(const uint16_t k, mrpt::opengl::CSetOfLines &gl_obj,
   ...
   * `./reactivenav.logs/PTGs/PTG%i_{x,y,phi,d}.txt`
    * Text files are loadable from MATLAB/Octave, and can be visualized with the
   * script `[MRPT_DIR]/scripts/viewPTG.m`
   ...
   *   - `${sKeyPrefix}num_paths`: The number of different paths in this family
   * (number of discrete `alpha` values).
   *   - `${sKeyPrefix}ref_distance`: The maximum distance in PTGs [meters]
   *   - `${sKeyPrefix}score_priority`: When used in path planning, a
   * multiplying factor (default=1.0) for the scores for this PTG. Assign values
   * <1 to PTGs with low priority.
   */
  virtual void loadFromConfigFile(const mrpt::utils::CConfigFileBase &cfg,
                                  const std::string &sSection) override;
  virtual void saveToConfigFile(mrpt::utils::CConfigFileBase &cfg,
  ...
  void initClearanceDiagram(ClearanceDiagram &cd) const; //!< Must be called to
                                                         //!resize a CD to its
                                                         //!correct size, before
                                                         //!calling
                                                         //!updateClearance()
   ...
   * coordinates relative
    ...
    */
  void updateClearance(const double ox, const double oy,
                       ClearanceDiagram &cd) const;
  void updateClearancePost(ClearanceDiagram &cd,
                           const std::vector<double> &TP_obstacles) const;

protected:
  double refDistance;
  uint16_t m_alphaValuesCount; //!< The number of discrete values for "alpha"
                               //!between -PI and +PI.
  double m_score_priority;
  uint16_t m_clearance_num_points;      //!< Number of steps for the
                                        //!piecewise-constant approximation of
                                        //!clearance from TPS distances [0,1]
                                        //!(Default=5) \sa updateClearance()
  uint16_t m_clearance_decimated_paths; //!< Number of paths for the decimated
                                        //!paths analysis of clearance
  TNavDynamicState m_nav_dyn_state;     //!< Updated before each nav step by
  uint16_t m_nav_dyn_state_target_k;    //!< Update in updateNavDynamicState(),
                                        //!contains the path index (k) for the
                                        //!target.
   ...
   * "pseudometers", un-normalized, for some "k" direction.
    * \param inout_tp_obs The target where to store the new TP-Obs distance, if
   * it fulfills the criteria determined by the collision behavior.
    */
  void internal_TPObsDistancePostprocess(const double ox, const double oy,
...
public:
  /** Evals the robot clearance for each robot pose along path `k`, for the real
  * distances in
  * the key of the map<>, then keep in the map value the minimum of its current
  * stored clearance,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CParameterizedTrajectoryGenerator,
                                             mrpt::utils::CSerializable,
                                             NAV_IMPEXP)

using TListPTGPtr =
    std::vector<mrpt::nav::CParameterizedTrajectoryGenerator::Ptr>; //!< A list
                                                                    //!of PTGs
                                                                    //!(smart
                                                                    //!pointers)
...
public:
  ...
  /** @name Robot shape
    * @{ **/
  /** Robot shape must be set before initialization, either from ctor params or
   * via this method. */
  void setRobotShape(const mrpt::math::CPolygon &robotShape);
  const mrpt::math::CPolygon &getRobotShape() const { return m_robotShape; }
  double getMaxRobotRadius() const override;
  virtual double evalClearanceToRobotShape(const double ox,
                                           const double oy) const override;
  /** @} */
  bool isPointInsideRobotShape(const double x, const double y) const override;
  void add_robotShape_to_setOfLines(mrpt::opengl::CSetOfLines &gl_shape,
                                    const mrpt::poses::CPose2D &origin =
                                        mrpt::poses::CPose2D()) const override;

protected:
  virtual void internal_processNewRobotShape() = 0; //!< Will be called whenever
                                                    //!the robot shape is set /
                                                    //!updated
  mrpt::math::CPolygon m_robotShape;
  double m_robotMaxRadius;
  void loadShapeFromConfigFile(const mrpt::utils::CConfigFileBase &source,
