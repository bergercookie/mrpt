...
public:
   ...
   * (default=0.4); Notice
    *  that full grid resolution is used in path finding, this is only a way to
   * reduce the
    *  amount of redundant information to be returned.
    */
  float minStepInReturnedPath;

  float robotRadius; //!< The aproximate robot radius used in the planification.
                     //!Default is 0.35m
   ...
   * "map".
    * \param target	[IN] The desired target pose for the robot, in coordinates
   * of "map".
    * \param path		[OUT] The found path, in global coordinates relative to
   * "map".
    * \param notFount	[OUT] Will be true if no path has been found.
    * \param maxSearchPathLength [IN] The maximum path length to search for, in
   * meters (-1 = no limit)
    *
    * \sa robotRadius
    *
    ...
    */
  void computePath(const mrpt::maps::COccupancyGridMap2D &theMap,
