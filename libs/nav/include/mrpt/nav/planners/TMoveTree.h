...
template <class node_t> struct PoseDistanceMetric;
...
public:
  struct node_t : public NODE_TYPE_DATA {
    mrpt::utils::TNodeID node_id; //!< Duplicated ID (it's also in the
                                  //!map::iterator->first), but put here to make
                                  //!it available in path_t
    mrpt::utils::TNodeID
        parent_id; //!< INVALID_NODEID for the root, a valid ID otherwise
    EDGE_TYPE *edge_to_parent; //!< NULL for root, a valid edge otherwise
    node_t(mrpt::utils::TNodeID node_id_, mrpt::utils::TNodeID parent_id_,
           EDGE_TYPE *edge_to_parent_, const NODE_TYPE_DATA &data)
        : NODE_TYPE_DATA(data), node_id(node_id_), parent_id(parent_id_),
  ...
  typedef
      typename MAPS_IMPLEMENTATION::template map<mrpt::utils::TNodeID, node_t>
          node_map_t;               //!< Map: TNode_ID => Node info
  ...
  void insertNodeAndEdge(const mrpt::utils::TNodeID parent_id,
    ...
    // node:
    m_nodes[new_child_id] =
        node_t(new_child_id, parent_id, &edges_of_parent.back().data,
               new_child_node_data);
  }

  /** Insert a node without edges (should be used only for a tree root node) */
  void insertNode(const mrpt::utils::TNodeID node_id,
   ...
   * from a `target_node` towards the root
    ...
    */
  void backtrackPath(const mrpt::utils::TNodeID target_node,
                     path_t &out_path) const {
    out_path.clear();
    typename node_map_t::const_iterator it_src = m_nodes.find(target_node);
    if (it_src == m_nodes.end())
      throw std::runtime_error("backtrackPath: target_node not found in tree!");
...
struct NAV_IMPEXP TMoveEdgeSE2_TP {
  ...
  mrpt::math::TPose2D end_state;  //!< state in SE2 as 2D pose (x, y, phi)  -
                                  //!\note: it is not possible to initialize a
                                  //!motion without a state
  double cost;   //!< cost associated to each motion, this should be defined by
                 //!the user according to a spefic cost function
  ...
  TMoveEdgeSE2_TP(const mrpt::utils::TNodeID parent_id_,
                  const mrpt::math::TPose2D end_pose_)
      : parent_id(parent_id_), end_state(end_pose_), cost(0.0), ptg_index(0),
        ptg_K(0), ptg_dist(0.0) // these are all PTGs parameters
  {}
...
template <> struct PoseDistanceMetric<TNodeSE2> {
  bool cannotBeNearerThan(const TNodeSE2 &a, const TNodeSE2 &b,
...
template <> struct PoseDistanceMetric<TNodeSE2_TP> {
  bool cannotBeNearerThan(const TNodeSE2_TP &a, const TNodeSE2_TP &b,
                          const double d) const {
    if (std::abs(a.state.x - b.state.x) > d)
      return true;
    if (std::abs(a.state.y - b.state.y) > d)
  ...
  double distance(const TNodeSE2_TP &src, const TNodeSE2_TP &dst) const {
    ...
    relPose.inverseComposeFrom(mrpt::poses::CPose2D(dst.state),
                               mrpt::poses::CPose2D(src.state));
    ...
    if (tp_point_is_exact)
      ...
      return std::numeric_limits<double>::max(); // not in range: we can't
...
typedef TMoveTree<TNodeSE2_TP, TMoveEdgeSE2_TP> TMoveTreeSE2_TP; //!< tree data
                                                                 //!structure
                                                                 //!for planning
                                                                 //!in SE2
                                                                 //!within
