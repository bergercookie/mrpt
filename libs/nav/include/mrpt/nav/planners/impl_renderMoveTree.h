...
void PlannerTPS_VirtualBase::renderMoveTree(
  ...
  double xyzcorners_scale; // Size of XYZ corners (scaled to vehicle dimensions)
  {
    ...
    if (!params.robot_shape.empty()) {
      gl_veh_shape->appendLine(params.robot_shape[0].x, params.robot_shape[0].y,
                               0, params.robot_shape[1].x,
                               params.robot_shape[1].y, 0);
      for (size_t i = 2; i <= params.robot_shape.size(); i++) {
        ...
        gl_veh_shape->appendLineStrip(params.robot_shape[idx].x,
      ...
      for (int i = 0; i <= NUM_VERTICES; i++) {
        ...
        gl_veh_shape->appendLine(R * cos(ang), R * sin(ang), 0, R * cos(angn),
                                 R * sin(angn), 0);
      }
  ...
  // Determine the up-to-now best solution, so we can highlight the best path so
  // far:
  typename tree_t::path_t best_path;
  if (options.highlight_path_to_node_id != INVALID_NODEID)
    result.move_tree.backtrackPath(options.highlight_path_to_node_id,
                                   best_path);
  ...
  // The starting pose vehicle shape must be inserted independently, because the
  // rest are edges and we draw the END pose of each edge:
  {
  ...
  // Existing nodes & edges between them:
  {
    ...
    for (typename tree_t::node_map_t::const_iterator itNode = lstNodes.begin();
      ...
      if (node.parent_id != INVALID_NODEID) {
        ...
        mrpt::opengl::CSetOfLines::Ptr obj =
            std::make_shared<mrpt::opengl::CSetOfLines>();
        obj->setPose(mrpt::poses::CPose3D(parent_state)); // Points are relative
                                                          // to this pose: let
                                                          // OpenGL to deal with
                                                          // the coords.
                                                          // composition
  ...
  if (options.draw_obstacles && options.local_obs_from_nearest_pose &&
    ...
    obj->setPose(*options.x_nearest_pose); // Points are relative to this pose:
                                           // let OpenGL to deal with the
                                           // coords. composition
  ...
  if (!options.log_msg.empty()) {
    mrpt::opengl::CText3D::Ptr gl_txt = std::make_shared<mrpt::opengl::CText3D>(
