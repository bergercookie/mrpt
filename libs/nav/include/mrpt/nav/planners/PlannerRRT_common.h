...
struct TPlannerInputTempl {
  ...
  world_limits_t world_bbox_min, world_bbox_max; //!< Bounding box of the world,
                                                 //!used to draw uniform random
                                                 //!pose samples
  mrpt::maps::CSimplePointsMap
      obstacles_points; //!< World obstacles, as a point cloud
};

template <typename tree_t> struct TPlannerResultTempl {
  ...
  double path_cost; //!< Total cost of the best found path (cost ~~ Euclidean
                    //!distance)
  mrpt::utils::TNodeID
      best_goal_node_id; //!< The ID of the best target node in the tree
  std::set<mrpt::utils::TNodeID>
      acceptable_goal_node_ids; //!< The set of target nodes within an
                                //!acceptable distance to target (including
                                //!`best_goal_node_id` and others)
  tree_t move_tree; //!< The generated motion tree that explores free space
                    //!starting at "start"

  TPlannerResultTempl()
      : success(false), computation_time(0),
...
struct NAV_IMPEXP RRTEndCriteria {
  double acceptedDistToTarget; //!< Maximum distance from a pose to target to
                               //!accept it as a valid solution (meters). (Both
                               //!acceptedDistToTarget & acceptedAngToTarget
                               //!must be satisfied)
  double acceptedAngToTarget; //!< Maximum angle from a pose to target to accept
                              //!it as a valid solution (rad).  (Both
                              //!acceptedDistToTarget & acceptedAngToTarget must
                              //!be satisfied)

  double maxComputationTime; //!< In seconds. 0 means no limit until a solution
                             //!is found.
  double minComputationTime; //!< In seconds. 0 means the first valid path will
                             //!be returned. Otherwise, the algorithm will try
                             //!to refine and find a better one.

  RRTEndCriteria()
      : acceptedDistToTarget(0.1),
        acceptedAngToTarget(mrpt::utils::DEG2RAD(180)), maxComputationTime(0.0),
...
struct NAV_IMPEXP RRTAlgorithmParams {
  ...
  *  config file/text as a single 2xN matrix in MATLAB format, first row are Xs,
  * second are Ys, e.g:
  *  \code
  ...
  */
  ...
  double goalBias;  //!< Probabily of picking the goal as random target (in
                    //![0,1], default=0.05)
  double maxLength; //!< (Very sensitive parameter!) Max length of each edge
                    //!path (in meters, default=1.0)
  double minDistanceBetweenNewNodes; //!< Minimum distance [meters] to nearest
                                     //!node to accept creating a new one
                                     //!(default=0.10). (Any of
                                     //!minDistanceBetweenNewNodes and
                                     //!minAngBetweenNewNodes must be satisfied)
  double minAngBetweenNewNodes;      //!< Minimum angle [rad] to nearest node to
                                //!accept creating a new one (default=15 deg)
                                //!(Any of minDistanceBetweenNewNodes and
                                //!minAngBetweenNewNodes must be satisfied)
  bool ptg_verbose; //!< Display PTG construction info (default=true)

  size_t save_3d_log_freq; //!< Frequency (in iters) of saving tree state to
                           //!debug log files viewable in SceneViewer3D
                           //!(default=0, disabled)
...
public:
  ...
  RRTAlgorithmParams
      params; //!< Parameters specific to this path solver algorithm
  ...
  struct NAV_IMPEXP TRenderPlannedPathOptions {
    mrpt::utils::TNodeID highlight_path_to_node_id; //!< Highlight the path from
                                                    //!root towards this node
                                                    //!(usually, the target)
    size_t draw_shape_decimation; //!< (Default=1) Draw one out of N vehicle
                                  //!shapes along the highlighted path
    ...
    double xyzcorners_scale; //!< A scale factor to all XYZ corners (default=0,
                             //!means auto determien from vehicle shape)
    bool highlight_last_added_edge; //!< (Default=false)
    double
        ground_xy_grid_frequency; //!< (Default=10 meters) Set to 0 to disable
    ...
    double vehicle_shape_z;    //!< (Default=0.01) Height (Z coordinate) for the
                               //!vehicle shapes. Helps making it in the "first
                               //!plane"
    double vehicle_line_width; //!< Robot line width for visualization - default
                               //!2.0
    ...
    TRenderPlannedPathOptions()
        : highlight_path_to_node_id(INVALID_NODEID), draw_shape_decimation(1),
          x_rand_pose(NULL), x_nearest_pose(NULL),
          local_obs_from_nearest_pose(NULL), new_state(NULL),
          xyzcorners_scale(0), highlight_last_added_edge(false),
          ground_xy_grid_frequency(10.0), color_vehicle(0xFF, 0x00, 0x00, 0xFF),
          ...
          color_optimal_edge(0x00, 0x00, 0x00), width_last_edge(3.f),
          width_normal_edge(1.f), width_optimal_edge(4.f),
          point_size_obstacles(5), point_size_local_obstacles(5),
          vehicle_shape_z(0.01), vehicle_line_width(2.0), draw_obstacles(true),
          log_msg_position(0, 0, 0), log_msg_scale(0.2) {}

    virtual ~TRenderPlannedPathOptions() {}
  };

  template <typename node_pose_t, typename world_limits_t, typename tree_t>
  void renderMoveTree(mrpt::opengl::COpenGLScene &scene,
...
protected:
  ...
  void
  spaceTransformer(const mrpt::maps::CSimplePointsMap &in_obstacles,
