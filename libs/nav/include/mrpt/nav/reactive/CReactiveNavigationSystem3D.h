...
*  \ingroup nav_reactive
*/
class NAV_IMPEXP CReactiveNavigationSystem3D
    : public CAbstractPTGBasedReactive {
public:
...
public:
  ...
  virtual CParameterizedTrajectoryGenerator *getPTG(size_t i) override {
    ...
    return m_ptgmultilevel[i].PTGs[0]; // Return for the 0'th level (ptgs are
...
private:
  ...
  //					PRIVATE	VARIABLES
  // ------------------------------------------------------
  mrpt::maps::CSimplePointsMap m_WS_Obstacles_unsorted; //!< The unsorted set of
                                                        //!obstacles from the
                                                        //!sensors
  std::vector<mrpt::maps::CSimplePointsMap>
      m_WS_Obstacles_inlevels; //!< One point cloud per 2.5D robot-shape-slice,
                               //!coordinates relative to the robot local frame
