...
public:
  ...
  getPTG(size_t i) const override {
    ASSERT_(i < PTGs.size());
    return PTGs[i];
  }
  ...
  struct NAV_IMPEXP TReactiveNavigatorParams
    ...
    double min_obstacles_height,
        max_obstacles_height; // The range of "z" coordinates for obstacles to
                              // be considered
...
private:
  std::vector<CParameterizedTrajectoryGenerator *>
      PTGs; //!< The list of PTGs to use for navigation
  ...
  bool
  implementSenseObstacles(mrpt::system::TTimeStamp &obs_timestamp) override;

protected:
  mrpt::math::CPolygon m_robotShape; //!< The robot 2D shape model. Only one of
                                     //!`robot_shape` or
                                     //!`robot_shape_circular_radius` will be
                                     //!used in each PTG
  double m_robotShapeCircularRadius; //!< Radius of the robot if approximated as
                                     //!a circle. Only one of `robot_shape` or
                                     //!`robot_shape_circular_radius` will be
                                     //!used in each PTG
  ...
  mrpt::maps::CSimplePointsMap m_WS_Obstacles; //!< The obstacle points, as seen
                                               //!from the local robot frame.
  mrpt::maps::CSimplePointsMap m_WS_Obstacles_original; //!< Obstacle points,
                                                        //!before filtering (if
                                                        //!filtering is
                                                        //!enabled).
  // See docs in parent class
