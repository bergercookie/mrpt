 ...
 */
class NAV_IMPEXP CNavigatorManualSequence
    : public mrpt::nav::CAbstractNavigator {
public:
  ...
  void initialize() override; //!< Must be called for loading collision grids,
                              //!etc. before invoking any navigation command
  /** @} */
  ...
  struct TVelCmd {
    mrpt::kinematics::CVehicleVelCmd::Ptr
        cmd_vel; //!< all with the same meaning than in
                 //!CRobot2NavInterface::changeSpeeds()
  };

  std::map<double, TVelCmd>
      programmed_orders; //!< map [time_in_secs_since_beginning] -> orders.
