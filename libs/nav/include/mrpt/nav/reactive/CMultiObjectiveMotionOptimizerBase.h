...
public:
  static CMultiObjectiveMotionOptimizerBase::Ptr
  Factory(const std::string
              &className) noexcept; //!< Class factory from C++ class name
  ...
  struct NAV_IMPEXP TResultInfo {
    ...
    std::vector<double>
        final_evaluation; //!< The final evaluation score for each candidate
    std::vector<std::string> log_entries; //!< Optionally, debug logging info
                                          //!will be stored here by the
                                          //!implementor classes
  };

  /** The main entry point for the class: returns the 0-based index of the best
   * of the N motion candidates in `movs`.
    * If no valid one is found, `-1` will be returned.
    */
  int decide(const std::vector<mrpt::nav::TCandidateMovementPTG> &movs,
  ...
  struct NAV_IMPEXP TParamsBase : public mrpt::utils::CLoadableOptions {
    ...
    /** A list of `name` -> mathematical expression (in the format of the exprtk
     * library) for
      * the list of "score" factors to evaluate.
      */
    std::map<std::string, std::string> formula_score;

    /** A list of exprtk expressions for conditions that any candidate movement
     * must
    ...
    /** List of score names (as defined in the key of `formula_score`) that must
     * be normalized
  ...
  virtual void clear(); //!< Resets the object state; use if the parameters
                        //!change, so they are re-read and applied.
...
private:
  ...
  virtual int
  impl_decide(const std::vector<mrpt::nav::TCandidateMovementPTG> &movs,
              TResultInfo &extra_info) = 0;

  TParamsBase &m_params_base;

  std::map<std::string, mrpt::math::CRuntimeCompiledExpression>
      m_score_exprs; //!< score names -> score compiled expressions
  std::vector<mrpt::math::CRuntimeCompiledExpression> m_movement_assert_exprs;
  std::map<std::string, double> m_expr_vars;
};
DEFINE_MRPT_OBJECT_POST_CUSTOM_LINKAGE(CMultiObjectiveMotionOptimizerBase,
                                       NAV_IMPEXP)
}
}
