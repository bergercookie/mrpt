...
struct NAV_IMPEXP TWaypoint {
  ...
  std::string target_frame_id; //!< (Default="map") Frame ID in which target is
                               //!given. Optional, use only for submapping
                               //!applications.

  double allowed_distance; //!< [Must be set by the user] How close should the
                           //!robot get to this waypoint for it to be considered
                           //!reached.

  /** [Default=true] Whether it is allowed to the navigator to proceed to a more
   * advanced waypoint
    * in the sequence if it determines that it is easier to skip this one (e.g.
   * it seems blocked by dynamic obstacles).
  ...
  bool isValid() const; //!< Check whether all the minimum mandatory fields have
  ...
  static const double INVALID_NUM; //!< The default value of fields (used to
                                   //!detect non-set values)
};
...
struct NAV_IMPEXP TWaypointSequence {
   ...
   */
  void
  getAsOpenglVisualization(mrpt::opengl::CSetOfObjects &obj,
...
struct NAV_IMPEXP TWaypointStatus : public TWaypoint {
  bool reached; //!< Whether this waypoint has been reached already (to within
                //!the allowed distance as per user specifications) or skipped.
  bool skipped; //!< If `reached==true` this boolean tells whether the waypoint
                //!was physically reached (false) or marked as reached because
                //!it was skipped (true).
  mrpt::system::TTimeStamp timestamp_reach; //!< Timestamp of when this waypoint
                                            //!was reached.
                                            //!(Default=INVALID_TIMESTAMP means
                                            //!not reached so far)
  int counter_seen_reachable; //!< (Initialized to 0 automatically) How many
                              //!times this waypoint has been seen as
                              //!"reachable" before it being the current active
                              //!waypoint.
...
struct NAV_IMPEXP TWaypointStatusSequence {
  std::vector<TWaypointStatus>
      waypoints; //!< Waypoints parameters and status (reached, skipped, etc.)
  mrpt::system::TTimeStamp
      timestamp_nav_started; //!< Timestamp of user navigation command.
  bool final_goal_reached;   //!< Whether the final waypoint has been reached
                             //!successfuly.
  /** Index in `waypoints` of the waypoint the navigator is currently trying to
   * reach.
  ...
  mrpt::math::TPose2D last_robot_pose; //!< Robot pose at last time step (has
                                       //!INVALID_NUM fields upon
                                       //!initialization)
