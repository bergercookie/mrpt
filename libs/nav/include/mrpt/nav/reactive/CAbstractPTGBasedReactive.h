...
public:
  ...
  protected:
    virtual bool
    isEqual(const CAbstractNavigator::TNavigationParamsBase &o) const override;
  };
   ...
   * the required interfaces to read from and control a robot.
    ...
    * \param[in] enableLogFile Set to true to enable creation of navigation log
   * files, useful for inspection and debugging.
    */
   ...
   * CAbstractHolonomicReactiveMethod
          */
  void setHolonomicMethod(const std::string &method,
  ...
  struct NAV_IMPEXP TAbstractPTGNavigatorParams
    ...
    std::string holonomic_method; //!< C++ class name of the holonomic
                                  //!navigation method to run in the transformed
                                  //!TP-Space
    std::string motion_decider_method; //!< C++ class name of the motion chooser

    std::string ptg_cache_files_directory; //!< (Default: ".")
    double ref_distance;    //!< Maximum distance up to obstacles will be
                            //!considered (D_{max} in papers).
    double speedfilter_tau; //!< Time constant (in seconds) for the low-pass
    ...
    */
    ...
    double max_distance_predicted_actual_path; //!< Max distance [meters] to
                                               //!discard current PTG and issue
                                               //!a new vel cmd (default= 0.05)
    double min_normalized_free_space_for_ptg_continuation; //!< Min normalized
                                                           //!dist [0,1] after
                                                           //!current pose in a
                                                           //!PTG continuation
                                                           //!to allow it.
    ...
    mrpt::kinematics::CVehicleVelCmd::TVelCmdParams
        robot_absolute_speed_limits; //!< Params related to speed limits.
    bool enable_obstacle_filtering;
    bool evaluate_clearance;                       //!< Default: false
    double max_dist_for_timebased_path_prediction; //!< Max dist [meters] to use
                                                   //!time-based path prediction
                                                   //!for NOP evaluation.

    virtual void loadFromConfigFile(const mrpt::utils::CConfigFileBase &c,
                                    const std::string &s) override;
    virtual void saveToConfigFile(mrpt::utils::CConfigFileBase &c,
  ...
  virtual void loadConfigFile(
      const mrpt::utils::CConfigFileBase &c) override; // See base class docs!
   ...
   * construction)
          *  When enabled, a report will be dumped to std::cout upon
   * destruction.
          * \sa getTimeLogger
          */
  ...
  virtual size_t getPTG_count()
      const = 0; //!< Returns the number of different PTGs that have been setup
  virtual CParameterizedTrajectoryGenerator *
  getPTG(size_t i) = 0; //!< Gets the i'th PTG
  virtual const CParameterizedTrajectoryGenerator *
  getPTG(size_t i) const = 0; //!< Gets the i'th PTG
  ...
  /** Changes the current, global (honored for all PTGs) robot speed limits, via
   * returning a reference to a structure that holds those limits */
  mrpt::kinematics::CVehicleVelCmd::TVelCmdParams &
  changeCurrentRobotSpeedLimits() {
    return params_abstract_ptg_navigator.robot_absolute_speed_limits;
  }

  void setTargetApproachSlowDownDistance(const double dist); //!< Changes this
                                                             //!parameter in all
                                                             //!inner holonomic
                                                             //!navigator
                                                             //!instances [m].
  double getTargetApproachSlowDownDistance() const; //!< Returns this parameter
                                                    //!for the first inner
                                                    //!holonomic navigator
                                                    //!instances  [m] (should be
...
protected:
  ...
  std::vector<CAbstractHolonomicReactiveMethod::Ptr>
      m_holonomicMethod; //!< The holonomic navigation algorithm (one object per
                         //!PTG, so internal states are maintained)
  std::unique_ptr<mrpt::utils::CStream> m_logFile;
  mrpt::utils::CStream *m_prev_logfile; //!< The current log file stream, or
                                        //!nullptr if not being used
  ...
  mrpt::kinematics::CVehicleVelCmd::Ptr
      m_last_vel_cmd; //!< Last velocity commands
  ...
  mrpt::utils::CTimeLogger
      m_timelogger; //!< A complete time logger \sa enableTimeLog()
  ...
  mrpt::math::LowPassFilter_IIR1 meanExecutionPeriod; //!< Runtime estimation of
  ...
  virtual bool
  impl_waypoint_is_reachable(const mrpt::math::TPoint2D &wp_local_wrt_robot)
  ...
  virtual bool
  implementSenseObstacles(mrpt::system::TTimeStamp &obs_timestamp) = 0;
  bool STEP2_SenseObstacles();

  /** Builds TP-Obstacles from Workspace obstacles for the given PTG.
    * "out_TPObstacles" is already initialized to the proper length and maximum
   * collision-free distance for each "k" trajectory index.
    * Distances are in "pseudo-meters". They will be normalized automatically to
   * [0,1] upon return. */
  ...
  virtual double
  generate_vel_cmd(const TCandidateMovementPTG &in_movement,
  ...
  void preDestructor(); //!< To be called during children destructors to assure
                        //!thread-safe destruction, and free of shared objects.
  virtual void onStartNewNavigation() override;

  bool m_closing_navigator; //!< Signal that the destructor has been called, so
                            //!no more calls are accepted from other threads
  ...
  struct TInfoPerPTG {
    ...
    std::vector<double> TP_Obstacles; //!< One distance per discretized alpha
                                      //!value, describing the "polar plot" of
                                      //!TP obstacles.
    ClearanceDiagram clearance;       //!< Clearance for each path
  };

  std::vector<TInfoPerPTG> m_infoPerPTG; //!< Temporary buffers for working with
                                         //!each PTG during a navigationStep()
  ...
  struct NAV_IMPEXP TSentVelCmd {
    ...
    mrpt::system::TTimeStamp
        tim_send_cmd_vel;  //!< Timestamp of when the cmd was sent
    TRobotPoseVel poseVel; //!< Robot pose & velocities and timestamp of when it
                           //!was queried
    double colfreedist_move_k; //!< TP-Obstacles in the move direction at the
                               //!instant of picking this movement
    bool was_slowdown;
    double
        speed_scale; //!< [0,1] scale of the raw cmd_vel as generated by the PTG
...
private:
  ...
  std::unique_ptr<TNavigationParams> m_copy_prev_navParams; //!< A copy of
                                                            //!last-iteration
                                                            //!navparams, used
                                                            //!to detect changes
