...
public:
  ...
  struct NAV_IMPEXP TParams
    ...
    std::string scalar_score_formula; //!< A formula that takes all/a subset of
                                      //!scores and generates a scalar global
                                      //!score.
...
protected:
  ...
  int impl_decide(const std::vector<mrpt::nav::TCandidateMovementPTG> &movs,
                  TResultInfo &extra_info) override;
};
DEFINE_MRPT_OBJECT_POST_CUSTOM_LINKAGE(CMultiObjMotionOpt_Scalarization,
                                       NAV_IMPEXP)
}
}
