...
public:
  ...
  struct NAV_IMPEXP TInfoPerPTG {
    ...
    mrpt::math::CVectorFloat TP_Obstacles; //!< Distances until obstacles, in
                                           //!"pseudometers", first index for
                                           //!-PI direction, last one for PI
                                           //!direction.
    std::vector<mrpt::math::TPoint2D>
        TP_Targets;                //!< Target(s) location in TP-Space
    mrpt::math::TPoint2D TP_Robot; //!< Robot location in TP-Space: normally
                                   //!(0,0), except during "NOP cmd vel" steps
    double timeForTPObsTransformation,
        timeForHolonomicMethod; //!< Time, in seconds.
    double desiredDirection,
        desiredSpeed;  //!< The results from the holonomic method.
    double evaluation; //!< Final score of this candidate
    mrpt::utils::TParametersDouble evalFactors; //!< Evaluation factors
    CHolonomicLogFileRecord::Ptr
        HLFR; //!< Other useful info about holonomic method execution.
    mrpt::nav::CParameterizedTrajectoryGenerator::Ptr ptg; //!< Only for the
                                                           //!FIRST entry in a
                                                           //!log file, this
                                                           //!will contain a
                                                           //!copy of the PTG
  ...
  mrpt::aligned_containers<TInfoPerPTG>::vector_t infoPerPTG; //!< The info for
                                                              //!each applied
                                                              //!PTG: must
                                                              //!contain "nPTGs
                                                              //!*
  ...
  *	- "curPoseAndVel":  Time of querying robot pose and velocities.
  ...
  std::map<std::string, std::string>
      additional_debug_msgs; //!< Additional debug traces
  mrpt::maps::CSimplePointsMap WS_Obstacles,
      WS_Obstacles_original; //!< The WS-Obstacles
  mrpt::math::TPose2D robotPoseLocalization,
      robotPoseOdometry; //!< The robot pose (from odometry and from the
                         //!localization/SLAM system).
  ...
  std::vector<mrpt::math::TPose2D>
      WS_targets_relative; //!< The relative location of target(s) in Workspace.

  mrpt::kinematics::CVehicleVelCmd::Ptr cmd_vel; //!< The final motion command
                                                 //!sent to robot, in "m/sec"
                                                 //!and "rad/sec".
  mrpt::kinematics::CVehicleVelCmd::Ptr cmd_vel_original; //!< Motion command as
                                                          //!comes out from the
                                                          //!PTG, before scaling
                                                          //!speed limit
                                                          //!filtering.
  mrpt::math::TTwist2D cur_vel; //!< The actual robot velocities in global (map)
                                //!coordinates, as read from sensors, in "m/sec"
                                //!and "rad/sec".
  mrpt::math::TTwist2D cur_vel_local; //!< The actual robot velocities in local
                                      //!(robot) coordinates, as read from
                                      //!sensors, in "m/sec" and "rad/sec".

  mrpt::math::CVectorFloat robotShape_x,
      robotShape_y; //!< The robot shape in WS. Used by PTGs derived from
                    //!mrpt::nav::CPTG_RobotShape_Polygonal
  double robotShape_radius; //!< The circular robot radius. Used by PTGs derived
                            //!from mrpt::nav::CPTG_RobotShape_Circular

  // "NOP motion command" mode variables:
  int16_t ptg_index_NOP; //!< Negative means no NOP mode evaluation, so the rest
                         //!of "NOP variables" should be ignored.
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CLogFileRecord,
