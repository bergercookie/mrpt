...
public:
  /** The struct for configuring navigation requests to CWaypointsNavigator and
   * derived classes. */
  ...
  /** Get a copy of the control structure which describes the progress status of
   * the waypoint navigation. */
  virtual void
  getWaypointNavStatus(TWaypointStatusSequence &out_nav_status) const;
  ...
  * the point is out of range for the existing PTGs, etc. */
  ...
  struct NAV_IMPEXP TWaypointsNavigatorParams
    ...
    int min_timesteps_confirm_skip_waypoints; //!< How many times shall a future
                                              //!waypoint be seen as reachable
                                              //!to skip to it (Default: 1)
    double waypoint_angle_tolerance;     //!< [rad] Angular error tolerance for
                                         //!waypoints with an assigned heading
                                         //!(Default: 5 deg)
    double rel_speed_for_stop_waypoints; //!< [0,1] Relative speed when aiming
                                         //!at a stop-point waypoint
                                         //!(Default=0.10)
    int multitarget_look_ahead; //!< >=0 number of waypoints to forward to the
                                //!underlying navigation engine, to ease
                                //!obstacles avoidance when a waypoint is
                                //!blocked (Default=0 : none).
...
protected:
  TWaypointStatusSequence m_waypoint_nav_status; //!< The latest waypoints
                                                 //!navigation command and the
                                                 //!up-to-date control status.
   ...
   * there is a free path to
    * the given point; `false` otherwise: if way is blocked or there is missing
   * information, the point is out of range, etc. */
  ...
  virtual void waypoints_navigationStep(); //!< The waypoints-specific part of
                                           //!navigationStep()

private:
  bool m_was_aligning; //!< Whether the last timestep was "is_aligning" in a
                       //!waypoint with heading
  bool m_is_aligning;
};
}
}
