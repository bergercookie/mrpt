...
class NAV_IMPEXP CAbstractNavigator : public mrpt::utils::COutputLogger
{
   public:
	...
	struct NAV_IMPEXP TargetInfo
	{
		 ...
		 * "slow
		 * down" approaching target only if this is "==.0". Intermediary values
		 * will
		 * be honored only by the higher-level navigator, based on straight-line
		 * Euclidean distances. */
		...
		: public mrpt::utils::CLoadableOptions
	{
		 ...
		 * target
		 * is blocked to abort navigation with an error. */
