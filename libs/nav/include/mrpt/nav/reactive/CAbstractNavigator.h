...
public:
  ...
  struct NAV_IMPEXP TargetInfo {
    mrpt::math::TPose2D target_coords; //!< Coordinates of desired target
                                       //!location. Heading may be ignored by
                                       //!some reactive implementations.
    std::string target_frame_id; //!< (Default="map") Frame ID in which target
                                 //!is given. Optional, use only for submapping
                                 //!applications.
    float targetAllowedDistance; //!< (Default=0.5 meters) Allowed distance to
                                 //!target in order to end the navigation.
    bool targetIsRelative;       //!< (Default=false) Whether the \a target
                           //!coordinates are in global coordinates (false) or
                           //!are relative to the current robot pose (true).
    double targetDesiredRelSpeed; //!< (Default=.05) Desired relative speed (wrt
                                  //!maximum speed), in range [0,1], of the
                                  //!vehicle at target. Holonomic nav methods
                                  //!will perform "slow down" approaching target
                                  //!only if this is "==.0". Intermediary values
                                  //!will be honored only by the higher-level
                                  //!navigator, based on straight-line Euclidean
                                  //!distances.
    bool targetIsIntermediaryWaypoint; // !< (Default=false) If true, event
                                       // callback `sendWaypointReachedEvent()`
                                       // will be called instead of
                                       // `sendNavigationEndEvent()`

    TargetInfo();
    std::string
    getAsText() const; //!< Gets navigation params as a human-readable format
  ...
  struct NAV_IMPEXP TNavigationParamsBase {
    ...
    virtual std::string getAsText()
        const = 0; //!< Gets navigation params as a human-readable format
    virtual TNavigationParamsBase *clone() const = 0;

  protected:
    friend bool NAV_IMPEXP operator==(const TNavigationParamsBase &,
  ...
  struct NAV_IMPEXP TNavigationParams : public TNavigationParamsBase {
    ...
    virtual std::string getAsText()
        const override; //!< Gets navigation params as a human-readable format
  ...
  virtual void
  initialize() = 0; //!<  Must be called before any other navigation command
  virtual void navigationStep(); //!< This method must be called periodically in
                                 //!order to effectively run the navigation

  /** Navigation request to a single target location. It starts a new
   * navigation.
    * \param[in] params Pointer to structure with navigation info (its contents
   * will be copied, so the original can be freely destroyed upon return if it
   ...
   * targets in a frame ID
    * different than the one in which robot odometry is given (both IDs default
   * to `"map"`).
  ...
  struct NAV_IMPEXP TAbstractNavigatorParams
    ...
    double dist_to_target_for_sending_event; //!< Default value=0, means use the
                                             //!"targetAllowedDistance" passed
                                             //!by the user in the navigation
                                             //!request.
    double alarm_seems_not_approaching_target_timeout; //!< navigator timeout
                                                       //!(seconds) [Default=30
                                                       //!sec]
    double dist_check_target_is_blocked; //!< (Default value=0.6) When closer
                                         //!than this distance, check if the
                                         //!target is blocked to abort
...
private:
  ...
  bool m_navigationEndEventSent; //!< Will be false until the navigation end is
                                 //!sent, and it is reset with each new command
...
protected:
   ...
   * m_curPoseVel accordingly.
    * If an error is returned by the user callback, first, it calls robot.stop()
   * ,then throws an std::runtime_error exception. */
   ...
   * navigation, timeout reaching destination, etc.
    * `call_virtual_nav_method` can be set to false to avoid calling the virtual
   * method performNavigationStep()
    */
  virtual void
  performNavigationStepNavigating(bool call_virtual_nav_method = true);

  /** Stops the robot and set navigation state to error */
  void doEmergencyStop(const std::string &msg);

  virtual bool changeSpeeds(const mrpt::kinematics::CVehicleVelCmd
                                &vel_cmd); //!< Default: forward call to
                                           //!m_robot.changeSpeed(). Can be
                                           //!overriden.
  virtual bool changeSpeedsNOP();          //!< Default: forward call to
                                           //!m_robot.changeSpeedsNOP(). Can be
                                           //!overriden.
  virtual bool stop(bool isEmergencyStop); //!< Default: forward call to
                                           //!m_robot.stop(). Can be overriden.
  ...
  /** Checks whether the robot shape, when placed at the given pose (relative to
  * the current pose),
  ...
  * Default implementation: always returns false. */
  ...
  std::unique_ptr<TNavigationParams>
      m_navigationParams; //!< Current navigation parameters
  ...
  struct NAV_IMPEXP TRobotPoseVel {
    ...
    mrpt::math::TPose2D rawOdometry; //!< raw odometry (frame does not match to
                                     //!"pose", but is expected to be smoother
                                     //!in the short term).
  ...
  TRobotPoseVel m_curPoseVel; //!< Current robot pose (updated in
  ...
  mrpt::poses::CPose2DInterpolator m_latestPoses,
      m_latestOdomPoses; //!< Latest robot poses (updated in
                         //!CAbstractNavigator::navigationStep() )

  mrpt::utils::CTimeLogger
      m_timlog_delays; //!< Time logger to collect delay-related stats
...
bool NAV_IMPEXP operator==(const CAbstractNavigator::TNavigationParamsBase &,
                           const CAbstractNavigator::TNavigationParamsBase &);
}

// Specializations MUST occur at the same namespace:
namespace utils {
template <> struct TEnumTypeFiller<mrpt::nav::CAbstractNavigator::TState> {
