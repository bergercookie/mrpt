...
public:
  ...
  /** Get the current pose and velocity of the robot. The implementation should
  * not take too much time to return,
  *   so if it might take more than ~10ms to ask the robot for the instantaneous
  * data, it may be good enough to
  ...
  */
  virtual bool getCurrentPoseAndSpeeds(
      mrpt::math::TPose2D &curPose, //!< (output) The latest robot pose
                                    //!(typically from a mapping/localization
                                    //!module), in world coordinates. (x,y:
                                    //!meters, phi: radians)
      mrpt::math::TTwist2D &curVelGlobal,  //!< (output) The latest robot
                                           //!velocity vector, in world
                                           //!coordinates. (vx,vy: m/s, omega:
                                           //!rad/s)
      mrpt::system::TTimeStamp &timestamp, //!< (output) The timestamp for the
                                           //!read pose and velocity values. Use
                                           //!mrpt::system::now() unless you
                                           //!have something more accurate.
      mrpt::math::TPose2D &curOdometry,    //!< (output) The latest robot raw
                                           //!odometry pose; may have long-time
                                           //!drift should be more locally
                                        //!consistent than curPose (x,y: meters,
                                        //!phi: radians)
      std::string &frame_id //!< (output) ID of the coordinate frame for
                            //!curPose. Default is not modified is "map". [Only
                            //!for future support to submapping,etc.]
      ) = 0;
   ...
   * implemented in a particular robotic platform) started with startWatchdog()
   * \return false on any error.
   * \sa startWatchdog
   */
  virtual bool
  changeSpeeds(const mrpt::kinematics::CVehicleVelCmd &vel_cmd) = 0;

  /** Just like changeSpeeds(), but will be called when the last velocity
   * command is still the preferred solution,
    * so there is no need to change that past command. The unique effect of this
   * callback would be resetting the watchdog timer.
    * \return false on any error.
  ...
  getAlignCmd(const double relative_heading_radians);
   ...
   * `false` if it was actually "skipped".
    */
  virtual void sendWaypointReachedEvent(int waypoint_index,
  ...
  /** Callback: Target seems to be blocked by an obstacle. This event is invoked
   * before ending navigation with an ERROR state and another call to
   * sendWaySeemsBlockedEvent(). */
  virtual void sendCannotGetCloserToBlockedTargetEvent();

  /** @} */

  /** Returns the number of seconds ellapsed since the constructor of this class
   * was invoked, or since
    * the last call of resetNavigationTimer(). This will be normally wall-clock
   * time, except in simulators where this method will return simulation time.
   */
