	...
	*/
	virtual bool getCurrentPoseAndSpeeds(
		/** (output) The latest robot pose (typically from a
		   ...
		   module), in world coordinates. (x,y: meters, phi: radians) */
		...
		/** (output) The latest robot raw odometry pose; may have long-time
		   drift
		   should be more locally consistent than curPose (x,y: meters, phi:
		   radians) */
	...
	/** Callback: Target seems to be blocked by an obstacle. This event is
	 * invoked
