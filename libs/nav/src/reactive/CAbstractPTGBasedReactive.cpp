...
void CAbstractPTGBasedReactive::performNavigationStep()
{
	...
	try
	{
		...
		const size_t nTargets =
			targets.size();  // Normally = 1, will be >1 if we
		...
		for (auto& t : targets)
		{
			if (t.target_frame_id != m_curPoseVel.pose_frame_id)
			{
				...
				t.target_frame_id =
					m_curPoseVel
						.pose_frame_id;  // Now the coordinates are in the same
				// frame than robot pose
			}
		}
		...
		vector<TCandidateMovementPTG> candidate_movs(
			nPTGs +
			1);  // the last extra one is for the evaluation of "NOP motion
		// command" choice.
		...
		else
		{
				...
				new_vel_cmd->isStopCmd())
			{
				MRPT_LOG_DEBUG(
					...
					"robot.stop()");
				this->stop(
					true /* emergency */);  // don't call doEmergencyStop() here
	...
	catch (...)
	{
		doEmergencyStop(
			...
			"and "
			"finishing navigation due to untyped exception.");
	}
}
	...
	const mrpt::math::TPose2D& rel_cur_pose_wrt_last_vel_cmd_NOP)
{
	...
	ipf.targets.assign(
		relTargets.size(), PTGTarget());  // Init to default, invalid state
	...
	mrpt::utils::CConfigFileBase& c, const std::string& s) const
{
	...
	MRPT_SAVE_CONFIG_VAR_COMMENT(
		holonomic_method, string(
							  "C++ class name of the holonomic navigation "
