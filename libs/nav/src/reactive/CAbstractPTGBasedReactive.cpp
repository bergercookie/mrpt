...
CAbstractPTGBasedReactive::CAbstractPTGBasedReactive(
    ...
    : CWaypointsNavigator(react_iterf_impl), m_holonomicMethod(),
      m_prev_logfile(nullptr), m_enableKeepLogRecords(false),
      m_enableConsoleOutput(enableConsoleOutput), m_init_done(false),
      ...
      m_closing_navigator(false), m_WS_Obstacles_timestamp(INVALID_TIMESTAMP),
      m_infoPerPTG_timestamp(INVALID_TIMESTAMP), m_navlogfiles_dir(sLogDir) {
...
void CAbstractPTGBasedReactive::enableLogFile(bool enable) {
  ...
  try {
    ...
    if (!enable) {
      ...
      while (!free_name) {
        nFile++;
        sprintf(aux, "%s/log_%03u.reactivenavlog", m_navlogfiles_dir.c_str(),
                nFile);
      ...
      MRPT_LOG_DEBUG(mrpt::format(
          "[CAbstractPTGBasedReactive::enableLogFile] Logging to file `%s`\n",
          aux));
    }
...
void CAbstractPTGBasedReactive::setHolonomicMethod(
  ...
  for (size_t i = 0; i < nPTGs; i++) {
    ...
    if (!m_holonomicMethod[i])
      THROW_EXCEPTION_FMT("Non-registered holonomic method className=`%s`",
                          method.c_str());
...
void CAbstractPTGBasedReactive::performNavigationStep() {
  // Security tests:
  if (m_closing_navigator)
    return; // Are we closing in the main thread?
  if (!m_init_done)
  ...
  newLogRec.infoPerPTG.resize(
      nPTGs + 1); /* +1: [N] is the "NOP cmdvel" option; not to be present in
                     all log entries. */
  ...
  // which PTGs are we using:
  {
    if (m_logFile && m_logFile.get() != m_prev_logfile) // Only the first time
    {
      ...
      for (size_t i = 0; i < nPTGs; i++) {
        ...
        // collision grid.
        // Let's just store the parameters of each PTG by serializing it, so
        ...
        newLogRec.infoPerPTG[i].ptg = std::dynamic_pointer_cast<
            mrpt::nav::CParameterizedTrajectoryGenerator>(buf.ReadObject());
      }
    }
  }
  ...
  try {
    ...
    // Semantic is: any of the target is good. If several targets are reachable,
    // head to latest one.
    ...
    const size_t nTargets = targets.size(); // Normally = 1, will be >1 if we
                                            // want the robot local planner to
                                            // be "smarter" in skipping dynamic
                                            // obstacles.
    ASSERT_(nTargets >= 1);
    ...
    if (!STEP2_SenseObstacles()) {
      doEmergencyStop("Error while loading and sorting the obstacles. Robot "
                      "will be stopped.");
      if (fill_log_record) {
    ...
    // Round #1: As usual, pure reactive, evaluate all PTGs and all directions
    // from scratch.
    // =========
    ...
    if (params_abstract_ptg_navigator.use_delays_model) {
      ...
      */
      ...
      if (std::abs(timoff_pose2sense) > 1.25)
        MRPT_LOG_WARN_FMT("timoff_pose2sense=%e is too large! Path "
                          "extrapolation may be not accurate.",
                          timoff_pose2sense);
      if (std::abs(timoff_pose2VelCmd) > 1.25)
        MRPT_LOG_WARN_FMT("timoff_pose2VelCmd=%e is too large! Path "
                          "extrapolation may be not accurate.",
                          timoff_pose2VelCmd);
    ...
    for (auto &t : targets) {
      if (t.target_frame_id != m_curPoseVel.pose_frame_id) {
        if (m_frame_tf == nullptr) {
          THROW_EXCEPTION_FMT(
              "Different frame_id's but no frame_tf object attached!: "
              "target_frame_id=`%s` != pose_frame_id=`%s`",
              t.target_frame_id.c_str(), m_curPoseVel.pose_frame_id.c_str());
        }
        mrpt::math::TPose2D robot_frame2map_frame;
        m_frame_tf->lookupTransform(t.target_frame_id,
                                    m_curPoseVel.pose_frame_id,
                                    robot_frame2map_frame);

        MRPT_LOG_DEBUG_FMT("frame_tf: target_frame_id=`%s` as seen from "
                           "pose_frame_id=`%s` = %s",
        ...
        t.target_frame_id = m_curPoseVel.pose_frame_id; // Now the coordinates
    ...
    std::transform(targets.begin(), targets.end(), // in
    ...
    CParameterizedTrajectoryGenerator::TNavDynamicState
        ptg_dynState; //!< Allow PTGs to be responsive to target location,
                      //!dynamics, etc.
    ...
    vector<TCandidateMovementPTG> candidate_movs(nPTGs + 1); // the last extra
                                                             // one is for the
                                                             // evaluation of
                                                             // "NOP motion
                                                             // command" choice.
    ...
    for (size_t indexPTG = 0; indexPTG < nPTGs; indexPTG++) {
      ...
      // The picked movement in TP-Space (to be determined by holonomic method
      // below)
      TCandidateMovementPTG &cm = candidate_movs[indexPTG];

      ASSERT_(m_navigationParams);
      build_movement_candidate(
          ptg, indexPTG, relTargets, rel_pose_PTG_origin_wrt_sense, ipf, cm,
    ...
    const bool can_do_nop_motion =
        (m_lastSentVelCmd.isValid() && !target_changed_since_last_iteration &&
         last_sent_ptg && last_sent_ptg->supportVelCmdNOP()) &&
        (NOP_not_too_old =
             (NOP_At = mrpt::system::timeDifference(
                  m_lastSentVelCmd.tim_send_cmd_vel, tim_start_iteration)) <
             (NOP_max_time = last_sent_ptg->maxTimeInVelCmdNOP(
    ...
    if (!NOP_not_too_old) {
      newLogRec.additional_debug_msgs["PTG_cont"] =
          mrpt::format("PTG-continuation not allowed: previous command "
                       "timed-out (At=%.03f > Max_At=%.03f)",
                       NOP_At, NOP_max_time);
    }
    ...
    if (can_do_nop_motion) {
      ...
      const mrpt::system::TTimeStamp tim_send_cmd_vel_corrected =
          mrpt::system::timestampAdd(m_lastSentVelCmd.tim_send_cmd_vel,
      ...
      // smoother than particle filter-based localization data, more accurate in
      // the middle/long term,
      // but not in the short term:
      mrpt::math::TPose2D robot_pose_at_send_cmd, robot_odom_at_send_cmd;
      bool valid_odom, valid_pose;

      m_latestOdomPoses.interpolate(tim_send_cmd_vel_corrected,
                                    robot_odom_at_send_cmd, valid_odom);
      m_latestPoses.interpolate(tim_send_cmd_vel_corrected,
                                robot_pose_at_send_cmd, valid_pose);
      ...
      if (valid_odom && valid_pose) {
        ...
        std::transform(
            ...
            [robot_pose_at_send_cmd](const CAbstractNavigator::TargetInfo &e) {
              return e.target_coords - robot_pose_at_send_cmd;
            });
        ASSERT_EQUAL_(relTargets_NOPs.size(), targets.size());

        rel_pose_PTG_origin_wrt_sense_NOP =
            robot_odom_at_send_cmd - (m_curPoseVel.rawOdometry + relPoseSense);
        rel_cur_pose_wrt_last_vel_cmd_NOP =
        ...
        if (fill_log_record) {
          ...
          newLogRec.additional_debug_msgs["robot_odom_at_send_cmd(interp)"] =
        ...
        build_movement_candidate(
            ...
            *m_holonomicMethod[m_lastSentVelCmd.ptg_index], tim_start_iteration,
            *m_navigationParams, rel_cur_pose_wrt_last_vel_cmd_NOP);
    ...
    // If the selected PTG is (N+1), it means the NOP cmd. vel is selected as
    // the best alternative, i.e. do NOT send any new motion command.
    ...
    if (best_is_NOP_cmdvel) {
      ...
      if (!this->changeSpeedsNOP()) {
        doEmergencyStop("\nERROR calling changeSpeedsNOP()!! Stopping robot "
                        "and finishing navigation\n");
        if (fill_log_record) {
          STEP8_GenerateLogRecord(newLogRec, relTargets, best_ptg_idx,
                                  m_robot.getEmergencyStopCmd(), nPTGs,
                                  best_is_NOP_cmdvel,
                                  rel_cur_pose_wrt_last_vel_cmd_NOP,
                                  rel_pose_PTG_origin_wrt_sense_NOP,
      ...
      // Make sure the dynamic state of a NOP cmd has not overwritten the state
      // for a "regular" PTG choice:
      for (size_t i = 0; i < nPTGs; i++) {
        getPTG(i)->updateNavDynamicState(ptg_dynState);
      }
      ...
      if (selectedHolonomicMovement) {
        CTimeLoggerEntry tle(m_timelogger,
      ...
      if (!new_vel_cmd /* which means best_PTG_eval==.0*/ ||
        ...
        MRPT_LOG_DEBUG("Best velocity command is STOP (no way found), calling "
                       "robot.stop()");
        this->stop(true /* emergency */); // don't call doEmergencyStop() here
                                          // since that will stop navigation
                                          // completely
        ...
        // Update delay model:
        const double timoff_sendVelCmd =
            mrpt::system::timeDifference(tim_start_iteration, tim_send_cmd_vel);
    ...
    if (period_tim > 1.5 * meanExecutionPeriod.getLastOutput()) {
      MRPT_LOG_WARN_FMT("Timing warning: Suspicious executionPeriod=%.03f ms "
                        "is far above the average of %.03f ms",
                        1e3 * period_tim,
                        meanExecutionPeriod.getLastOutput() * 1e3);
    }
    ...
    doEmergencyStop(std::string("[CAbstractPTGBasedReactive::"
                                "performNavigationStep] Stopping robot and "
                                "finishing navigation due to exception:\n") +
                    std::string(e.what()));
  } catch (...) {
    doEmergencyStop("[CAbstractPTGBasedReactive::performNavigationStep] "
                    "Stopping robot and finishing navigation due to untyped "
                    "exception.");
  }
}
...
void CAbstractPTGBasedReactive::STEP8_GenerateLogRecord(
  ...
  // --------------------------------------
  {
    mrpt::utils::CTimeLoggerEntry tle(m_timelogger,
                                      "navigationStep.write_log_file");
    if (m_logFile)
      (*m_logFile) << newLogRec;
  }
...
void CAbstractPTGBasedReactive::calc_move_candidate_scores(
  ...
  // const double   target_dir    = (TP_Target.x!=0 || TP_Target.y!=0) ? atan2(
  // TP_Target.y, TP_Target.x) : 0.0;
  ...
  // input ones.
  // Policy: use the target whose TP-Space direction is closer to this candidate
  // direction:
  size_t selected_trg_idx = 0;
  {
    ...
    for (size_t i = 0; i < TP_Targets.size(); i++) {
      const double angdist = std::abs(
          mrpt::math::angDistance(TP_Targets[i].target_alpha, cm.direction));
  ...
  if (m_navigationParams &&
      ...
      !cm.PTG->supportSpeedAtTarget() // If the PTG is able to handle the
                                      // slow-down on its own, dont change speed
                                      // here
      ) {
    ...
    const double f = std::min(
        1.0, Vf + target_WS_d * (1.0 - Vf) / TARGET_SLOW_APPROACHING_DISTANCE);
    if (f < cm.speed) {
      newLogRec.additional_debug_msgs["PTG_eval.speed"] =
          mrpt::format("Relative speed reduced %.03f->%.03f based on Euclidean "
                       "nearness to target.",
                       cm.speed, f);
      cm.speed = f;
    }
  ...
  double &move_cur_d = cm.props["move_cur_d"] =
      0; // current robot path normalized distance over path (0 unless in a NOP
         // cmd)
  ...
  colfree = in_TPObstacles[move_k]; // we'll next substract here the
                                    // already-traveled distance, for NOP motion
                                    // candidates.

  // Special case for NOP motion cmd:
  // consider only the empty space *after* the current robot pose, which is not
  // at the origin.
  ...
  if (this_is_PTG_continuation) {
    ...
    const double maxD =
        params_abstract_ptg_navigator.max_dist_for_timebased_path_prediction;
    if (std::abs(rel_cur_pose_wrt_last_vel_cmd_NOP.x) > maxD ||
        std::abs(rel_cur_pose_wrt_last_vel_cmd_NOP.y) > maxD) {
      is_exact = cm.PTG->inverseMap_WS2TP(rel_cur_pose_wrt_last_vel_cmd_NOP.x,
                                          rel_cur_pose_wrt_last_vel_cmd_NOP.y,
                                          cur_k, cur_norm_d);
    } else {
      // Use time:
      is_time_based = true;
      ...
      const double NOP_At =
          ...
          mrpt::system::timeDifference(m_lastSentVelCmd.tim_send_cmd_vel,
                                       tim_start_iteration);
      ...
      cur_norm_d =
          cm.PTG->getPathDist(cur_k, cur_ptg_step) / cm.PTG->getRefDistance();
      {
    ...
    if (!is_time_based) {
      ...
      if (ok1) {
        ...
        WS_point_is_unique =
            WS_point_is_unique && cm.PTG->isBijectiveAt(move_k, predicted_step);
        newLogRec.additional_debug_msgs["PTG_eval.bijective"] =
            mrpt::format("isBijectiveAt(): k=%i step=%i -> %s", (int)cur_k,
        ...
        newLogRec.additional_debug_msgs["PTG_eval.PTGcont"] = mrpt::format(
            "mismatchDistance=%.03f cm", 1e2 * predicted2real_dist);

        if (predicted2real_dist > params_abstract_ptg_navigator
        ...
        cm.speed = -0.01; // this enforces a 0 global evaluation score
        newLogRec.additional_debug_msgs["PTG_eval"] =
            "PTG-continuation not allowed, couldn't get PTG step for cur. "
            "robot pose.";
        return;
      }
    }

    // Path following isn't perfect: we can't be 100% sure of whether the robot
    // followed exactly
    // the intended path (`kDirection`), or if it's actually a bit shifted, as
    // reported in `cur_k`.
    ...
    // Only discount free space if there was a real obstacle, not the "end of
    // path" due to limited refDistance.
    if (colfree < 0.99) {
      colfree -= cur_norm_d;
    }
  ...
  if (cm.PTG->supportVelCmdNOP()) {
    ...
    if (typeid(*ptr1) == typeid(*ptr2)) {
      ASSERT_EQUAL_(m_last_vel_cmd->getVelCmdLength(),
                    desired_cmd->getVelCmdLength());

      double simil_score = 0.5;
      for (size_t i = 0; i < desired_cmd->getVelCmdLength(); i++) {
        const double scr = exp(-std::abs(desired_cmd->getVelCmdElement(i) -
  ...
  clearance = in_clearance.getClearance(move_k, target_d_norm * 1.01,
                                        false /* spot, dont interpolate */);
  ...
  if (cm.PTG && cm.speed > .0) // for valid cases only
  {
    ...
    if (valid_step) {
      ...
      if (this_is_PTG_continuation) {
        ...
        // Note that hm.speed above scales the overall path time using the
        // current speed scale, not the exact
        // integration over the past timesteps. It's an approximation, probably
        // good enough...
...
bool CAbstractPTGBasedReactive::impl_waypoint_is_reachable(
  ...
  if (m_infoPerPTG.size() < N || m_infoPerPTG_timestamp == INVALID_TIMESTAMP ||
      mrpt::system::timeDifference(m_infoPerPTG_timestamp,
                                   mrpt::system::now()) > 0.5)
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    if (tp_obs.size() != ptg->getPathCount())
      continue; // May be this PTG has not been used so far? (Target out of
                // domain,...)
...
void CAbstractPTGBasedReactive::build_movement_candidate(
  ...
  ipf.targets.assign(relTargets.size(),
                     PTGTarget()); // Init to default, invalid state
  ...
  if (!any_TPTarget_is_valid) {
    ...
    // -----------------------------------------------------------------------------
    {
      ...
      STEP3_WSpaceToTPSpace(indexPTG, ipf.TP_Obstacles, ipf.clearance,
      ...
      for (size_t i = 0; i < Ki; i++)
        ipf.TP_Obstacles[i] *= _refD;

      timeForTPObsTransformation = tictac.Tac();
      if (m_timelogger.isEnabled())
        m_timelogger.registerUserMeasure("navigationStep.STEP3_WSpaceToTPSpace",
    ...
    if (!this_is_PTG_continuation) {
      ...
      // Take into account the future robot pose after NOP motion iterations to
      // slow down accordingly *now*
      if (ptg->supportVelCmdNOP()) {
        const double v = mrpt::math::hypot_fast(m_curPoseVel.velLocal.vx,
                                                m_curPoseVel.velLocal.vy);
        const double d = v * ptg->maxTimeInVelCmdNOP(kDirection);
        obsFreeNormalizedDistance =
            std::min(obsFreeNormalizedDistance,
                     std::max(0.90, obsFreeNormalizedDistance - d));
      }

      double velScale = 1.0;
      ASSERT_(params_abstract_ptg_navigator.secure_distance_end >
      ...
      if (obsFreeNormalizedDistance <
        ...
        if (obsFreeNormalizedDistance <=
          ...
          velScale = (obsFreeNormalizedDistance -
                      params_abstract_ptg_navigator.secure_distance_start) /
                     (params_abstract_ptg_navigator.secure_distance_end -
                      params_abstract_ptg_navigator.secure_distance_start);
      }
      ...
      if (m_timelogger.isEnabled())
        m_timelogger.registerUserMeasure("navigationStep.STEP4_HolonomicMethod",
                                         timeForHolonomicMethod);
    } else {
      // "NOP cmdvel" case: don't need to re-run holo algorithm, just keep the
      // last selection:
    ...
    // ---------------------------------------------------------------------
    {
      ...
      calc_move_candidate_scores(
          ...
          this_is_PTG_continuation, rel_cur_pose_wrt_last_vel_cmd_NOP, indexPTG,
          tim_start_iteration);
  ...
  if (fill_log_record) {
    ...
    if (!this_is_PTG_continuation)
      ...
      ipp.PTG_desc =
          mrpt::format("NOP cmdvel (prev PTG idx=%u)",
                       static_cast<unsigned int>(m_lastSentVelCmd.ptg_index));

    metaprogramming::copy_container_typecasting(ipf.TP_Obstacles,
                                                ipp.TP_Obstacles);
...
void CAbstractPTGBasedReactive::TAbstractPTGNavigatorParams::loadFromConfigFile(
  ...
  MRPT_LOAD_CONFIG_VAR_CS(min_normalized_free_space_for_ptg_continuation,
                          double);
...
void CAbstractPTGBasedReactive::TAbstractPTGNavigatorParams::saveToConfigFile(
  ...
  MRPT_SAVE_CONFIG_VAR_COMMENT(
      holonomic_method, string("C++ class name of the holonomic navigation "
                               "method to run in the transformed TP-Space.\n") +
                            lstHoloStr);
  ...
  MRPT_SAVE_CONFIG_VAR_COMMENT(ref_distance, "Maximum distance up to obstacles "
                                             "will be considered (D_{max} in "
                                             "papers).");
  MRPT_SAVE_CONFIG_VAR_COMMENT(speedfilter_tau,
                               "Time constant (in seconds) for the low-pass "
                               "filter applied to kinematic velocity commands "
                               "(default=0: no filtering)");
  MRPT_SAVE_CONFIG_VAR_COMMENT(secure_distance_start,
                               "In normalized distance [0,1], start/end of a "
                               "ramp function that scales the holonomic "
                               "navigator output velocity.");
  MRPT_SAVE_CONFIG_VAR_COMMENT(secure_distance_end,
                               "In normalized distance [0,1], start/end of a "
                               "ramp function that scales the holonomic "
                               "navigator output velocity.");
  MRPT_SAVE_CONFIG_VAR_COMMENT(use_delays_model,
                               "Whether to use robot pose inter/extrapolation "
                               "to improve accuracy (Default:false)");
  MRPT_SAVE_CONFIG_VAR_COMMENT(max_distance_predicted_actual_path,
                               "Max distance [meters] to discard current PTG "
                               "and issue a new vel cmd (default= 0.05)");
  MRPT_SAVE_CONFIG_VAR_COMMENT(min_normalized_free_space_for_ptg_continuation,
                               "Min normalized dist [0,1] after current pose "
                               "in a PTG continuation to allow it.");
  MRPT_SAVE_CONFIG_VAR_COMMENT(
...
CAbstractPTGBasedReactive::TAbstractPTGNavigatorParams::
    ...
    : holonomic_method(), ptg_cache_files_directory("."), ref_distance(4.0),
      speedfilter_tau(0.0), secure_distance_start(0.05),
      secure_distance_end(0.20), use_delays_model(false),
      max_distance_predicted_actual_path(0.15),
      min_normalized_free_space_for_ptg_continuation(0.2),
      robot_absolute_speed_limits(), enable_obstacle_filtering(true),
      evaluate_clearance(false), max_dist_for_timebased_path_prediction(2.0) {}
...
void CAbstractPTGBasedReactive::loadConfigFile(
  ...
  // Load my params:
  params_abstract_ptg_navigator.loadFromConfigFile(c,
...
void CAbstractPTGBasedReactive::saveConfigFile(
  ...
  params_abstract_ptg_navigator.saveToConfigFile(c,
                                                 "CAbstractPTGBasedReactive");
