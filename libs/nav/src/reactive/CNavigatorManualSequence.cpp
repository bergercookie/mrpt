...
void CNavigatorManualSequence::loadConfigFile(
  ...
  for (size_t i = 0; i < lstKeys.size(); i++) {
    ...
    ASSERTMSG_(toks.size() > 2,
               std::string("Wrong format while parsing "
                           "CNavigatorManualSequence cfg file in entry: ") +
                   lstKeys[i]);
    ...
    case 2:
      krc.cmd_vel =
          std::make_shared<mrpt::kinematics::CVehicleVelCmd_DiffDriven>();
      break;
    case 4:
      krc.cmd_vel = std::make_shared<mrpt::kinematics::CVehicleVelCmd_Holo>();
      break;
    default:
