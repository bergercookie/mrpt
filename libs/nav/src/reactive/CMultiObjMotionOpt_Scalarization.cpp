...
IMPLEMENTS_MRPT_OBJECT(CMultiObjMotionOpt_Scalarization,
...
void CMultiObjMotionOpt_Scalarization::saveConfigFile(
  ...
  parameters.CMultiObjectiveMotionOptimizerBase::TParamsBase::saveToConfigFile(
      c, "CMultiObjectiveMotionOptimizerBase"); // call base-member with special
                                                // argument
...
int CMultiObjMotionOpt_Scalarization::impl_decide(
  ...
  if (m_expr_scalar_vars.empty()) {
    ...
    try {
      m_expr_scalar_formula.compile(parameters.scalar_score_formula,
                                    m_expr_scalar_vars);
    } catch (std::exception &) {
      m_expr_scalar_vars.clear();
      throw; // rethrow
...
void CMultiObjMotionOpt_Scalarization::TParams::saveToConfigFile(
  ...
  // TParamsBase members are already dealt elsewhere.
  MRPT_SAVE_CONFIG_VAR_COMMENT(scalar_score_formula,
                               "A formula that takes all/a subset of scores "
                               "and generates a scalar global score.");
}
