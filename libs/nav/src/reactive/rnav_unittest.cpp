...
void run_rnav_test(const std::string &sFilename, const std::string &sHoloMethod,
                   ...
                   const TPoint2D &block_obstacle_rightbottom = TPoint2D(0,
                                                                         0)) {
  using namespace std;
  ...
  grid.setSize(world_topleft.x, world_rightbottom.x, world_rightbottom.y,
  ...
  struct MyDummyRobotIF : public CRobot2NavInterfaceForSimulator_DiffDriven {
    ...
    MyDummyRobotIF(mrpt::kinematics::CVehicleSimul_DiffDriven &sim,
                   mrpt::maps::COccupancyGridMap2D &grid)
    ...
    bool senseObstacles(mrpt::maps::CSimplePointsMap &obstacles,
      ...
      getCurrentPoseAndSpeeds(curPose, curVel, pose_tim, odomPose,
                              pose_frame_id);
  ...
  EXPECT_LT((TPoint2D(robot_simul.getCurrentGTPose()) - nav_target).norm(),
            0.4);
...
TEST(CReactiveNavigationSystem, no_obstacle_nav_FullEval) {
  run_rnav_test<mrpt::nav::CReactiveNavigationSystem>(
      "reactive2d_config.ini", "CHolonomicFullEval", no_obs_trg, no_obs_topleft,
      no_obs_bottomright);
}
...
TEST(CReactiveNavigationSystem3D, no_obstacle_nav_FullEval) {
  run_rnav_test<mrpt::nav::CReactiveNavigationSystem3D>(
      "reactive3d_config.ini", "CHolonomicFullEval", no_obs_trg, no_obs_topleft,
      no_obs_bottomright);
}

TEST(CReactiveNavigationSystem, with_obstacle_nav_VFF) {
  run_rnav_test<mrpt::nav::CReactiveNavigationSystem>(
      "reactive2d_config.ini", "CHolonomicVFF", with_obs_trg, with_obs_topleft,
...
TEST(CReactiveNavigationSystem3D, with_obstacle_nav_VFF) {
  run_rnav_test<mrpt::nav::CReactiveNavigationSystem3D>(
      "reactive3d_config.ini", "CHolonomicVFF", with_obs_trg, with_obs_topleft,
      with_obs_bottomright, obs_tl, obs_br);
}
