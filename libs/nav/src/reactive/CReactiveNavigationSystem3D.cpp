...
CReactiveNavigationSystem3D::CReactiveNavigationSystem3D(
    ...
    : CAbstractPTGBasedReactive(react_iterf_impl, enableConsoleOutput,
                                enableLogToFile, logFileDirectory) {}
...
void CReactiveNavigationSystem3D::saveConfigFile(
  ...
  MRPT_SAVE_CONFIG_VAR_COMMENT(HEIGHT_LEVELS,
...
void CReactiveNavigationSystem3D::loadConfigFile(
  ...
  for (unsigned int i = 1; i <= num_levels; i++) {
    ...
    c.read_vector(s, format("LEVEL%d_VECTORX", i), vector<float>(0), xaux,
                  false);
    c.read_vector(s, format("LEVEL%d_VECTORY", i), vector<float>(0), yaux,
                  false);
  ...
  for (unsigned int j = 1; j <= num_ptgs; j++) {
    for (unsigned int i = 1; i <= m_robotShape.size(); i++) {
      MRPT_LOG_INFO_FMT("[loadConfigFile] Generating PTG#%u at level %u...", j,
                        i);
      const std::string sPTGName =
          c.read_string(s, format("PTG%d_TYPE", j), "", true);
      CParameterizedTrajectoryGenerator *ptgaux =
          CParameterizedTrajectoryGenerator::CreatePTG(sPTGName, c, s,
                                                       format("PTG%d_", j));
  ...
  if (m_PTGsMustBeReInitialized) {
    ...
    for (unsigned int j = 0; j < m_ptgmultilevel.size(); j++) {
      for (unsigned int i = 0; i < m_robotShape.size(); i++) {
        ...
        MRPT_LOG_INFO_FMT("[loadConfigFile] Initializing PTG#%u.%u... (`%s`)",
                          j, i,
        ...
        m_ptgmultilevel[j].PTGs[i]->initialize(
            format(
                ...
                params_abstract_ptg_navigator.ptg_cache_files_directory.c_str(),
                i, j),
  ...
  for (size_t j = 0; j < nPts; j++) {
    ...
    for (size_t idxH = 0; idxH < nSlices; ++idxH) {
      ...
      if (zs[j] < h) {
        ...
        // just don't account for it, because we don't know its mapping into
        // TP-Obstacles anyway...
        if (xs[j] > -OBS_MAX_XY && xs[j] < OBS_MAX_XY && ys[j] > -OBS_MAX_XY &&
            ys[j] < OBS_MAX_XY)
...
void CReactiveNavigationSystem3D::STEP3_WSpaceToTPSpace(
  ...
  for (size_t j = 0; j < m_robotShape.size(); j++) {
    ...
    for (size_t obs = 0; obs < nObs; obs++) {
      ...
      m_ptgmultilevel[ptg_idx].PTGs[j]->updateTPObstacle(ox, oy,
                                                         out_TPObstacles);
      if (eval_clearance) {
        m_ptgmultilevel[ptg_idx].PTGs[j]->updateClearance(ox, oy,
                                                          out_clearance);
      }
    }
  }
...
void CReactiveNavigationSystem3D::loggingGetWSObstaclesAndShape(
  ...
  for (unsigned int i = 0; i < m_WS_Obstacles_inlevels.size(); i++)
    out_log.WS_Obstacles.insertAnotherMap(&m_WS_Obstacles_inlevels[i],
                                          CPose3D(0, 0, 0));
  ...
  for (size_t idxH = 0; idxH < nSlices; ++idxH) {
    ...
    for (size_t i = 0; i < 1 /* assume all PTGs share the same robot shape! */;
      ...
      for (size_t obs = 0; obs < nObs; obs++) {
        ...
        relative_robot_pose.inverseComposePoint(mrpt::math::TPoint2D(gox, goy),
                                                lo);
