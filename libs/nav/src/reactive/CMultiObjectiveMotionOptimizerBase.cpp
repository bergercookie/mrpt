...
IMPLEMENTS_VIRTUAL_MRPT_OBJECT(CMultiObjectiveMotionOptimizerBase, CObject,
                               mrpt::nav)
...
int CMultiObjectiveMotionOptimizerBase::decide(
      ...
      for (const auto &f : m_params_base.formula_score) {
        ...
        // expressions:
        {
          ...
          if (it != m_expr_vars.end()) {
            THROW_EXCEPTION_FMT("Error: Expression name `%s` already exists as "
                                "an input variable.",
                                f.first.c_str());
          }
          // Add it:
  ...
  for (const auto &sScoreName : m_params_base.scores_to_normalize) {
    ...
    // Normalize:
    if (maxScore <= 0) // all scores=0... let's decide that all are equal, so
                       // normalized to "1"
    {
  ...
  for (unsigned int mov_idx = 0; mov_idx < movs.size(); ++mov_idx) {
    ...
    bool assert_failed = false;
    {
      for (auto &ma : m_movement_assert_exprs) {
        ...
        if (val == 0) {
          assert_failed = true;
          extra_info.log_entries.emplace_back(std::move(
              mrpt::format("[CMultiObjectiveMotionOptimizerBase] mov_idx=%u "
                           "ASSERT failed: `%s`",
...
void CMultiObjectiveMotionOptimizerBase::TParamsBase::loadFromConfigFile(
  ...
  // Load: formula_score
  {
    ...
    for (;; idx++) {
      ...
      if (none && idx == 1)
        THROW_EXCEPTION_FMT("Expect at least a first `%s` and `%s` pair "
                            "defining one score in section `[%s]`",
...
void CMultiObjectiveMotionOptimizerBase::TParamsBase::saveToConfigFile(
...

  {
    const std::string sComment =
        ...
        "i=1,...,N\n"
        "# Each one defines one of the scores that will be evaluated for each "
        "candidate movement.\n"
        "# Multiobjective optimizers will then use those scores to select the "
        "best candidate, \n"
        "# possibly using more parameters that follow below.\n";
    c.write(s, "dummy", "", mrpt::utils::MRPT_SAVE_NAME_PADDING,
    ...
    for (const auto &p : this->formula_score) {
      ...
      const std::string sKeyName = mrpt::format("score%i_name", idx),
                        sKeyValue = mrpt::format("score%i_formula", idx);
      c.write(s, sKeyName, p.first, mrpt::utils::MRPT_SAVE_NAME_PADDING,
              mrpt::utils::MRPT_SAVE_VALUE_PADDING);
      c.write(s, sKeyValue, p.second, mrpt::utils::MRPT_SAVE_NAME_PADDING,
              mrpt::utils::MRPT_SAVE_VALUE_PADDING);
    }
  }
  ...
  // Load: movement_assert
  {
    const std::string sComment =
        "\n"
        "# Next follows a list of `movement_assert%i` exprtk expressions for "
        "i=1,...,N\n"
