...
CLogFileRecord::CLogFileRecord()
    : nPTGs(0), nSelectedPTG(-1), robotPoseLocalization(0, 0, 0),
      robotPoseOdometry(0, 0, 0), relPoseSense(0, 0, 0), relPoseVelCmd(0, 0, 0),
      WS_targets_relative(), cur_vel(0, 0, 0), cur_vel_local(0, 0, 0),
      robotShape_radius(.0), ptg_index_NOP(-1), ptg_last_k_NOP(0),
  ...
  if (version)
    *version = 26;
    ...
    for (i = 0; i < n; i++) {
      ...
      out << m;
      if (m)
        out.WriteBuffer((const void *)&(*infoPerPTG[i].TP_Obstacles.begin()),
      ...
      // removed in v23: out << evaluation_org << evaluation_priority; // added
      // in v21
      out << infoPerPTG[i].HLFR;

      // Version 9: Removed security distances. Added optional field with PTG
      // info.
    ...
    out << n;
    if (n) {
      out.WriteBuffer((const void *)&(*robotShape_x.begin()),
                      n * sizeof(robotShape_x[0]));
      out.WriteBuffer((const void *)&(*robotShape_y.begin()),
  ...
  case 26: {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (version >= 9) // Extra PTG info
      {
        ...
        if (there_is_ptg_data)
          ipp.ptg =
              std::dynamic_pointer_cast<CParameterizedTrajectoryGenerator>(
                  in.ReadObject());
      }
    ...
    if (version < 6) {
      ...
      prevV.resize(n);
      if (n)
        in.ReadBufferFixEndianness(&(*prevV.begin()), n);

      in >> n;
      prevW.resize(n);
      if (n)
        in.ReadBufferFixEndianness(&(*prevW.begin()), n);

      in >> n;
      prevSelPTG.resize(n);
      if (n)
        in.ReadBufferFixEndianness(&(*prevSelPTG.begin()), n);
    }
    ...
    } else { // Default values for old versions:
      cur_vel = mrpt::math::TTwist2D(0, 0, 0);
    }
