...
void CWaypointsNavigator::navigateWaypoints(
  ...
  // The main loop navigationStep() will iterate over waypoints and send them to
  // navigate()
  MRPT_END
}
...
void CWaypointsNavigator::waypoints_navigationStep() {
  ...
  {
    mrpt::utils::CTimeLoggerEntry tle(m_timlog_delays,
    ...
    if (wps.waypoints.empty() || wps.final_goal_reached) {
          ...
          if (wp.target_heading == TWaypoint::INVALID_NUM) {
            ...
            if (std::abs(ang_err) <=
              ...
              if (!m_was_aligning) {
                ...
                mrpt::kinematics::CVehicleVelCmd::Ptr align_cmd =
                    m_robot.getAlignCmd(ang_err);

                MRPT_LOG_INFO_FMT(
                    "[CWaypointsNavigator::navigationStep] Trying to align to "
                    "heading: %.02f deg. "
                ...
                if (align_cmd) {
                  ...
                  consider_wp_reached =
                      true; // this robot does not support "in place" alignment
                }
              } else {
                MRPT_LOG_THROTTLE_INFO_FMT(
                    0.5, "[CWaypointsNavigator::navigationStep] Waiting for "
                         "the robot to get aligned: current_heading=%.02f deg "
      ...
      // 2) More advanced policy: if available, use children class methods to
      // decide
      //     which is the best candidate for the next waypoint, if we can skip
      //     current one:
      if (!wps.final_goal_reached && wps.waypoint_index_current_goal >= 0 &&
        ...
        for (int idx = wps.waypoint_index_current_goal;
          ...
          if (idx < 0)
            continue;
          if (wps.waypoints[idx].reached)
            continue;

          // Is it reachable?
          mrpt::math::TPoint2D wp_local_wrt_robot;
          robot_pose.inverseComposePoint(wps.waypoints[idx].target,
                                         wp_local_wrt_robot);

          if (params_waypoints_navigator.max_distance_to_allow_skip_waypoint >
                  0 &&
          ...
          if (is_reachable) {
            // Robustness filter: only skip to a future waypoint if it is seen
            // as "reachable" during
          ...
          if (!wps.waypoints[idx].allow_skip)
            break; // Do not keep trying, since we are now allowed to skip this
                   // one.
        }
      ...
      if (wps.waypoint_index_current_goal >= 0 &&
        ...
        // Send the current targets + "multitarget_look_ahead" additional ones
        // to help the local planner.
        ...
        for (int wp_idx = wps.waypoint_index_current_goal;
          ...
          ti.target_coords.phi =
              (wp.target_heading != TWaypoint::INVALID_NUM ? wp.target_heading
                                                           : .0);
          ...
          ti.targetDesiredRelSpeed =
              (is_final_wp || (wp.target_heading != TWaypoint::INVALID_NUM))
                  ? params_waypoints_navigator.rel_speed_for_stop_waypoints
                  : 1.;

          // For backwards compat. with single-target code, write single target
          // info too for the first, next, waypoint:
        ...
        MRPT_LOG_DEBUG_STREAM("[CWaypointsNavigator::navigationStep] Active "
                              "waypoint changed. Current status:\n"
...
void CWaypointsNavigator::loadConfigFile(
...
void CWaypointsNavigator::saveConfigFile(
...
void mrpt::nav::CWaypointsNavigator::TWaypointsNavigatorParams::
    loadFromConfigFile(const mrpt::utils::CConfigFileBase &c,
                       const std::string &s) {
...
void mrpt::nav::CWaypointsNavigator::TWaypointsNavigatorParams::
    saveToConfigFile(mrpt::utils::CConfigFileBase &c,
                     const std::string &s) const {
  MRPT_SAVE_CONFIG_VAR_COMMENT(
      max_distance_to_allow_skip_waypoint,
      "Max distance to `foresee` waypoints [meters]. (<0: unlimited)");
  MRPT_SAVE_CONFIG_VAR_COMMENT(min_timesteps_confirm_skip_waypoints,
                               "Min timesteps a `future` waypoint must be seen "
                               "as reachable to become the active one.");
  MRPT_SAVE_CONFIG_VAR_DEGREES_COMMENT(
      ...
      "(Default: 5 deg)");
  MRPT_SAVE_CONFIG_VAR_COMMENT(rel_speed_for_stop_waypoints,
                               "[0,1] Relative speed when aiming at a "
                               "stop-point waypoint (Default=0.10)");
  MRPT_SAVE_CONFIG_VAR_COMMENT(multitarget_look_ahead,
                               ">=0 number of waypoints to forward to the "
                               "underlying navigation engine, to ease "
                               "obstacles avoidance when a waypoint is blocked "
                               "(Default=0 : none)");
}
...
CWaypointsNavigator::TWaypointsNavigatorParams::TWaypointsNavigatorParams()
    : max_distance_to_allow_skip_waypoint(-1.0),
      ...
      rel_speed_for_stop_waypoints(0.10), multitarget_look_ahead(0) {}

bool CWaypointsNavigator::checkHasReachedTarget(const double targetDist) const {
  return (!m_navigationParams->target.targetIsIntermediaryWaypoint) &&
