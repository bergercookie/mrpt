...
bool CRobot2NavInterface::changeSpeedsNOP() {
  MRPT_LOG_THROTTLE_INFO(10.0, "[changeSpeedsNOP] Doing nothing : not "
                               "implemented in user's derived class.");
  return true;
}

mrpt::kinematics::CVehicleVelCmd::Ptr
CRobot2NavInterface::getAlignCmd(const double relative_heading_radians) {
  return mrpt::kinematics::CVehicleVelCmd::Ptr();
}

bool CRobot2NavInterface::startWatchdog(float T_ms) {
  MRPT_LOG_INFO_FMT("[startWatchdog] Period=%.03f ms. Doing nothing: not "
                    "implemented in user's derived class.",
                    T_ms);
  return true;
}
...
void CRobot2NavInterface::sendNavigationStartEvent() {
  MRPT_LOG_INFO("[sendNavigationStartEvent] Doing nothing: not implemented in "
                "user's derived class.");
}
void CRobot2NavInterface::sendNavigationEndEvent() {
  MRPT_LOG_INFO("[sendNavigationEndEvent] Doing nothing: not implemented in "
                "user's derived class.");
}
...
void CRobot2NavInterface::sendNavigationEndDueToErrorEvent() {
  MRPT_LOG_INFO("[sendNavigationEndDueToErrorEvent] Doing nothing: not "
                "implemented in user's derived class.");
}
void CRobot2NavInterface::sendWaySeemsBlockedEvent() {
  MRPT_LOG_INFO("[sendWaySeemsBlockedEvent] Doing nothing: not implemented in "
                "user's derived class.");
}
void CRobot2NavInterface::sendApparentCollisionEvent() {
  MRPT_LOG_INFO("[sendApparentCollisionEvent] Doing nothing: not implemented "
                "in user's derived class.");
}
void CRobot2NavInterface::sendCannotGetCloserToBlockedTargetEvent() {
  MRPT_LOG_INFO("[sendCannotGetCloserToBlockedTargetEvent] Doing nothing: not "
                "implemented in user's derived class.");
}
