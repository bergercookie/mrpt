...
void CReactiveNavigationSystem::loadConfigFile(
  ...
  ASSERTMSG_(xs.size() == ys.size(),
             "Config parameters `RobotModel_shape2D_xs` and "
             "`RobotModel_shape2D_ys` must have the same length!");
  if (!xs.empty()) {
    math::CPolygon shape;
  ...
  for (unsigned int n = 0; n < PTG_COUNT; n++) {
    ...
    PTGs[n] = CParameterizedTrajectoryGenerator::CreatePTG(sPTGName, c, sectCfg,
                                                           format("PTG%u_", n));
  }
...
void CReactiveNavigationSystem::STEP1_InitPTGs() {
  if (m_PTGsMustBeReInitialized) {
    ...
    for (unsigned int i = 0; i < PTGs.size(); i++) {
      ...
      logFmt(mrpt::utils::LVL_INFO, "[CReactiveNavigationSystem::STEP1_"
                                    "InitPTGs] Initializing PTG#%u (`%s`)...",
             i, PTGs[i]->getDescription().c_str());

      // Polygonal robot shape?
      {
        mrpt::nav::CPTG_RobotShape_Polygonal *ptg =
            dynamic_cast<mrpt::nav::CPTG_RobotShape_Polygonal *>(PTGs[i]);
        if (ptg)
...
bool CReactiveNavigationSystem::implementSenseObstacles(
  ...
  try {
    ...
    return ret;
    // Note: Clip obstacles by "z" axis coordinates is more efficiently done in
...
void CReactiveNavigationSystem::TReactiveNavigatorParams::saveToConfigFile(
  ...
  MRPT_SAVE_CONFIG_VAR_COMMENT(min_obstacles_height,
                               "Minimum `z` coordinate of obstacles to be "
                               "considered fo collision checking");
  MRPT_SAVE_CONFIG_VAR_COMMENT(max_obstacles_height,
                               "Maximum `z` coordinate of obstacles to be "
                               "considered fo collision checking");
}
