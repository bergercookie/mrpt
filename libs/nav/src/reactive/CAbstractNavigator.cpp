		...
		case SUSPENDED:
			try
			{
				...
				if (m_lastNavigationState == NAVIGATING)
				{
					...
					// the
					// "state", so we have more flexibility
	...
	mrpt::utils::CConfigFileBase& c, const std::string& s) const
{
	...
	MRPT_SAVE_CONFIG_VAR_COMMENT(
		...
		"abort "
		"navigation with an error. [Default=0.6 m]");
}
	...
	bool call_virtual_nav_method)
{
	try
	{
		...
		if (m_lastNavigationState == IDLE) m_robot.sendNavigationStartEvent();
