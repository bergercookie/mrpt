...
CAbstractNavigator::TargetInfo::TargetInfo()
    : target_coords(0, 0, 0), target_frame_id("map"),
      targetAllowedDistance(0.5), targetIsRelative(false),
      targetDesiredRelSpeed(.05), targetIsIntermediaryWaypoint(false) {}
...
bool mrpt::nav::CAbstractNavigator::TargetInfo::
operator==(const TargetInfo &o) const {
...
CAbstractNavigator::TRobotPoseVel::TRobotPoseVel()
    : pose(0, 0, 0), velGlobal(0, 0, 0), velLocal(0, 0, 0),
      rawOdometry(0, 0, 0), timestamp(INVALID_TIMESTAMP), pose_frame_id() {}

/*---------------------------------------------------------------
                                                        Constructor
  ---------------------------------------------------------------*/
CAbstractNavigator::CAbstractNavigator(CRobot2NavInterface &react_iterf_impl)
    : mrpt::utils::COutputLogger("MRPT_navigator"), m_lastNavigationState(IDLE),
      m_navigationEndEventSent(false), m_navigationState(IDLE),
      m_robot(react_iterf_impl), m_frame_tf(nullptr), m_curPoseVel(),
      m_last_curPoseVelUpdate_robot_time(-1e9), m_latestPoses(),
      m_latestOdomPoses(),
...
void
    CAbstractNavigator::setFrameTF(mrpt::poses::FrameTransformer<2> *frame_tf) {
  m_frame_tf = frame_tf;
}
...
void CAbstractNavigator::navigationStep() {
  ...
  case SUSPENDED:
    try {
      ...
      if (m_lastNavigationState == NAVIGATING) {
        ...
        // this->stop();  stop() is called by the method switching the "state",
  ...
  case NAV_ERROR:
    try {
      ...
      if (m_lastNavigationState == NAVIGATING) {
        MRPT_LOG_ERROR("[CAbstractNavigator::navigationStep()] Stoping "
...
void CAbstractNavigator::navigate(
  ...
  ASSERT_(params->target.targetDesiredRelSpeed >= .0 &&
...
void CAbstractNavigator::updateCurrentPoseAndSpeeds() {
  ...
  if (m_last_curPoseVelUpdate_robot_time >= .0) {
    ...
    if (last_call_age < MIN_TIME_BETWEEN_POSE_UPDATES) {
      MRPT_LOG_THROTTLE_DEBUG_FMT(5.0, "updateCurrentPoseAndSpeeds: ignoring "
                                       "call, since last call was only %f ms "
                                       "ago.",
                                  last_call_age * 1e3);
...

  {
    ...
    if (!m_robot.getCurrentPoseAndSpeeds(
      ...
      MRPT_LOG_ERROR("ERROR calling m_robot.getCurrentPoseAndSpeeds, stopping "
                     "robot and finishing navigation");
      throw std::runtime_error("ERROR calling m_robot.getCurrentPoseAndSpeeds, "
                               "stopping robot and finishing navigation");
    }
  }
  ...
  while (m_latestPoses.size() > 1 &&
         mrpt::system::timeDifference(m_latestPoses.begin()->first,
                                      m_latestPoses.rbegin()->first) >
             PREVIOUS_POSES_MAX_AGE) {
    m_latestPoses.erase(m_latestPoses.begin());
  }
  while (m_latestOdomPoses.size() > 1 &&
         mrpt::system::timeDifference(m_latestOdomPoses.begin()->first,
                                      m_latestOdomPoses.rbegin()->first) >
             PREVIOUS_POSES_MAX_AGE) {
...
void CAbstractNavigator::TAbstractNavigatorParams::saveToConfigFile(
  ...
  MRPT_SAVE_CONFIG_VAR_COMMENT(dist_to_target_for_sending_event,
                               "Default value=0, means use the "
                               "`targetAllowedDistance` passed by the user in "
  ...
  MRPT_SAVE_CONFIG_VAR_COMMENT(dist_check_target_is_blocked,
                               "When closer than this distance, check if the "
                               "target is blocked to abort navigation with an "
                               "error. [Default=0.6 m]");
}
...
void CAbstractNavigator::performNavigationStepNavigating(
  ...
  try {
    if (m_lastNavigationState != NAVIGATING) {
      MRPT_LOG_INFO("[CAbstractNavigator::navigationStep()] Starting "
                    "Navigation. Watchdog initiated...\n");
      if (m_navigationParams)
        MRPT_LOG_DEBUG(mrpt::format(
            "[CAbstractNavigator::navigationStep()] Navigation Params:\n%s\n",
    ...
    if (m_navigationParams) {
      ...
      if (targetDist < m_badNavAlarm_minDistTarget) {
        ...
        if (mrpt::system::timeDifference(m_badNavAlarm_lastMinDistTime,
      ...
      // Check if the target seems to be at reach, but it's clearly occupied by
      // obstacles:
      if ((!m_navigationParams->target.targetIsIntermediaryWaypoint ||
           m_navigationParams->target.targetDesiredRelSpeed < 1) &&
          targetDist < params_abstract_navigator.dist_check_target_is_blocked) {
        const auto rel_trg =
            m_navigationParams->target.target_coords - m_curPoseVel.pose;
        const bool is_col = checkCollisionWithLatestObstacles(rel_trg);
        if (is_col) {
          MRPT_LOG_WARN(
              "Target seems to be blocked by obstacles. Aborting navigation.");
          m_navigationState = NAV_ERROR;
