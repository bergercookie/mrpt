...
IMPLEMENTS_SERIALIZABLE(CHolonomicVFF, CAbstractHolonomicReactiveMethod,
                        mrpt::nav)
...
void CHolonomicVFF::navigate(const NavInput &ni, NavOutput &no) {
  ...
  if (m_enableApproachTargetSlowDown) {
    const double targetNearnessFactor =
        std::min(1.0, trg.norm() / (options.TARGET_SLOW_APPROACHING_DISTANCE /
                                    ptg_ref_dist));
...
void CHolonomicVFF::writeToStream(mrpt::utils::CStream &out,
                                  int *version) const {
  if (version)
    *version = 0;
  else {
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CLogFileRecord_VFF::writeToStream(mrpt::utils::CStream &out,
                                       int *version) const {
...
void CHolonomicVFF::TOptions::saveToConfigFile(mrpt::utils::CConfigFileBase &c,
                                               const std::string &s) const {
  MRPT_START;

  MRPT_SAVE_CONFIG_VAR_COMMENT(TARGET_SLOW_APPROACHING_DISTANCE,
                               "For stopping gradually");
  MRPT_SAVE_CONFIG_VAR_COMMENT(TARGET_ATTRACTIVE_FORCE,
                               "Dimension-less (may have to be tuned depending "
                               "on the density of obstacle sampling)");
