...
IMPLEMENTS_SERIALIZABLE(CHolonomicND, CAbstractHolonomicReactiveMethod,
                        mrpt::nav)
...
void CHolonomicND::navigate(const NavInput &ni, NavOutput &no) {
  ...
  // Select best gap:
  searchBestGap(ni.obstacles, 1.0 /* max obs range*/, gaps, trg, selectedSector,
...
void CHolonomicND::searchBestGap(
  ...
  if (caseD1) {
    ...
    evaluateGaps(obstacles, maxObsRange, in_gaps, target_sector, target_dist,
                 gaps_evaluation);

    log.gaps_eval = gaps_evaluation;

    // D2: is there any gap "beyond" the target (and not too far away)?   (Not
    // used)
    // ----------------------------------------------------------------
  ...
  const unsigned int sec_ini =
      std::max(min_risk_eval_sector, risk_eval_nsectors < out_selDirection
...
/*---------------------------------------------------------------
                                                Evaluate each gap
  ---------------------------------------------------------------*/
void CHolonomicND::evaluateGaps(const std::vector<double> &obstacles,
                                const double maxObsRange, const TGapArray &gaps,
                                const unsigned int target_sector,
  ...
  for (unsigned int i = 0; i < gaps.size(); i++) {
    ...
    const float d = min3(obstacles[gap->representative_sector], maxObsRange,
                         0.95 * target_dist);
    ...
    double factor1;
    if (mrpt::utils::abs_diff(gap->representative_sector, target_sector) <= 1 &&
        target_dist < 1)
    ...
    double dist_eucl =
        math::minimumDistanceFromPointToSegment(target_x, target_y, // Point
                                                0, 0, x, y,         // Segment
    ...
    if (m_last_selected_sector != std::numeric_limits<unsigned int>::max()) {
      unsigned int dist = mrpt::utils::abs_diff(m_last_selected_sector,
                                                gap->representative_sector);
    ...
    if (obstacles[gap->representative_sector] <
      ...
      out_gaps_evaluation[i] = (options.factorWeights[0] * factor1 +
...
CHolonomicND::TOptions::TOptions()
    : // Default values:
      ...
      WIDE_GAP_SIZE_PERCENT(0.25), RISK_EVALUATION_SECTORS_PERCENT(0.10),
      RISK_EVALUATION_DISTANCE(0.4), MAX_SECTOR_DIST_FOR_D2_PERCENT(0.25),
...
void CHolonomicND::TOptions::loadFromConfigFile(
  ...
  MRPT_LOAD_CONFIG_VAR(RISK_EVALUATION_SECTORS_PERCENT, double, source,
                       section);
  ...
  MRPT_LOAD_CONFIG_VAR(TARGET_SLOW_APPROACHING_DISTANCE, double, source,
                       section);

  source.read_vector(section, "factorWeights", std::vector<double>(),
                     factorWeights, true);
...
void CHolonomicND::TOptions::saveToConfigFile(mrpt::utils::CConfigFileBase &c,
  ...
  MRPT_SAVE_CONFIG_VAR_COMMENT(RISK_EVALUATION_DISTANCE,
                               "In normalized ps-meters [0,1]");
  MRPT_SAVE_CONFIG_VAR_COMMENT(TOO_CLOSE_OBSTACLE, "For stopping gradually");
  MRPT_SAVE_CONFIG_VAR_COMMENT(TARGET_SLOW_APPROACHING_DISTANCE,
                               "In normalized ps-meters");

  ASSERT_EQUAL_(factorWeights.size(), 4)
  c.write(s, "factorWeights",
          mrpt::format("%.2f %.2f %.2f %.2f", factorWeights[0],
                       factorWeights[1], factorWeights[2], factorWeights[3]),
          WN, WV, "[0]=Free space, [1]=Dist. in sectors, [2]=Closer to target "
