...
void ClearanceDiagram::renderAs3DObject(mrpt::opengl::CMesh &mesh, double min_x,
                                        double max_x, double min_y,
                                        double max_y, double cell_res,
    ...
    for (int iY = 0; iY < nY; iY++) {
      ...
      if (x != 0 || y != 0) {
        ...
        const uint16_t actual_k =
            CParameterizedTrajectoryGenerator::alpha2index(alpha,
                                                           m_actual_num_paths);
...
ClearanceDiagram::dist2clearance_t &
ClearanceDiagram::get_path_clearance(size_t actual_k) {
  return m_raw_clearances[real_k_to_decimated_k(actual_k)];
}

const ClearanceDiagram::dist2clearance_t &
ClearanceDiagram::get_path_clearance(size_t actual_k) const {
  return m_raw_clearances[real_k_to_decimated_k(actual_k)];
}
...
double ClearanceDiagram::getClearance(uint16_t actual_k, double dist,
                                      bool integrate_over_path) const {
  ...
  double res = 0;
  int avr_count = 0; // weighted avrg: closer to query points weight more than
                     // at path start.
