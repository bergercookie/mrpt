	...
	for (unsigned int i = 0; i < nDirs; i++)
	{
		...
		if ((endpt_dist_to_target_norm > target_dist &&
			...
			min_dist_target_along_path >
				1.05 * std::min(
						   target_dist,
						   endpt_dist_to_target_norm)  // the path does
			// not get any
			// closer to trg
		...
		// -----------------------------------------------------
		{
			scores[2] = std::sqrt(
				1.01 -
				endpt_dist_to_target_norm);  // the 1.01 instead of 1.0 is to
			// be 100% sure we don't get a
	...
	mrpt::utils::CConfigFileBase& c, const std::string& s) const
{
	...
	c.write(
		...
		"(Euclidean), "
		"[3]=Hysteresis, [4]=clearance along path");
	c.write(
	...
	for (unsigned int i = 0; i < PHASE_FACTORS.size(); i++)
	{
		c.write(
			...
			"be "
			"considered in next phase (Default:`0.75`)");
		c.write(
