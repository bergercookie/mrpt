...
void CHolonomicFullEval::evalSingleTarget(unsigned int target_idx,
  ...
  for (unsigned int i = 0; i < nDirs; i++) {
    ...
    {
      scores[2] =
          std::sqrt(1.01 - endpt_dist_to_target_norm); // the 1.01 instead of
                                                       // 1.0 is to be 100% sure
                                                       // we don't get a domain
                                                       // error in sqrt()
    }
  ...
  if (best_eval == .0) {
    ...
    const double targetNearnessFactor =
        m_enableApproachTargetSlowDown
            ? std::min(1.0, ni.targets[best_trg_idx].norm() /
                                (options.TARGET_SLOW_APPROACHING_DISTANCE /
                                 ptg_ref_dist))
            : 1.0;

    const double obs_dist =
        ni.obstacles[evals[best_trg_idx].best_k]; // Was: min with obs_clearance
                                                  // too.
...
CHolonomicFullEval::TOptions::TOptions()
    : // Default values:
      ...
      TARGET_SLOW_APPROACHING_DISTANCE(0.60), OBSTACLE_SLOW_DOWN_DISTANCE(0.15),
      HYSTERESIS_SECTOR_COUNT(5), LOG_SCORE_MATRIX(false),
      clearance_threshold_ratio(0.05), gap_width_ratio_threshold(0.25) {
...
void CHolonomicFullEval::TOptions::saveToConfigFile(
  ...
  MRPT_SAVE_CONFIG_VAR_COMMENT(TOO_CLOSE_OBSTACLE,
                               "Directions with collision-free distances below "
                               "this threshold are not elegible.");
  MRPT_SAVE_CONFIG_VAR_COMMENT(
      TARGET_SLOW_APPROACHING_DISTANCE,
      "Start to reduce speed when closer than this to target.");
  MRPT_SAVE_CONFIG_VAR_COMMENT(OBSTACLE_SLOW_DOWN_DISTANCE,
                               "Start to reduce speed when clearance is below "
                               "this value ([0,1] ratio wrt obstacle "
                               "reference/max distance)");
  MRPT_SAVE_CONFIG_VAR_COMMENT(HYSTERESIS_SECTOR_COUNT,
                               "Range of `sectors` (directions) for hysteresis "
                               "over successive timesteps");
  ...
  MRPT_SAVE_CONFIG_VAR_COMMENT(gap_width_ratio_threshold,
                               "Ratio [0,1], times path_count, gives the "
                               "minimum gap width to accept a direct motion "
                               "towards target.");
  ...
  for (unsigned int i = 0; i < PHASE_FACTORS.size(); i++) {
    c.write(s, mrpt::format("PHASE%u_THRESHOLD", i + 1), PHASE_THRESHOLDS[i],
            WN, WV, "Phase scores must be above this relative range threshold "
                    "[0,1] to be considered in next phase (Default:`0.75`)");
