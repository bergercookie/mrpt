...
IMPLEMENTS_VIRTUAL_SERIALIZABLE(CParameterizedTrajectoryGenerator,
                                CSerializable, mrpt::nav)

CParameterizedTrajectoryGenerator::CParameterizedTrajectoryGenerator()
    : refDistance(.0), m_alphaValuesCount(0), m_score_priority(1.0),
      m_clearance_num_points(5), m_clearance_decimated_paths(15),
...
void CParameterizedTrajectoryGenerator::loadFromConfigFile(
  ...
  MRPT_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(num_paths, uint64_t, m_alphaValuesCount,
                                       cfg, sSection);
  MRPT_LOAD_CONFIG_VAR_NO_DEFAULT(refDistance, double, cfg, sSection);
  MRPT_LOAD_HERE_CONFIG_VAR(score_priority, double, m_score_priority, cfg,
                            sSection);
  MRPT_LOAD_HERE_CONFIG_VAR(clearance_num_points, double,
                            m_clearance_num_points, cfg, sSection);
  MRPT_LOAD_HERE_CONFIG_VAR(clearance_decimated_paths, double,
                            m_clearance_decimated_paths, cfg, sSection);
  ...
  MRPT_LOAD_HERE_CONFIG_VAR(vxi, double, m_nav_dyn_state.curVelLocal.vx, cfg,
                            sSection);
  MRPT_LOAD_HERE_CONFIG_VAR(vyi, double, m_nav_dyn_state.curVelLocal.vy, cfg,
                            sSection);
  MRPT_LOAD_HERE_CONFIG_VAR_DEGREES(
      wi, double, m_nav_dyn_state.curVelLocal.omega, cfg, sSection);

  MRPT_LOAD_HERE_CONFIG_VAR(reltrg_x, double, m_nav_dyn_state.relTarget.x, cfg,
                            sSection);
  MRPT_LOAD_HERE_CONFIG_VAR(reltrg_y, double, m_nav_dyn_state.relTarget.y, cfg,
                            sSection);
  MRPT_LOAD_HERE_CONFIG_VAR_DEGREES(
      reltrg_phi, double, m_nav_dyn_state.relTarget.phi, cfg, sSection);

  MRPT_LOAD_HERE_CONFIG_VAR(target_rel_speed, double,
                            m_nav_dyn_state.targetRelSpeed, cfg, sSection);
}
void CParameterizedTrajectoryGenerator::saveToConfigFile(
  ...
  cfg.write(sSection, "num_paths", m_alphaValuesCount, WN, WV,
  ...
  cfg.write(sSection, "score_priority", m_score_priority, WN, WV,
            "When used in path planning, a multiplying factor (default=1.0) "
            "for the scores for this PTG. Assign values <1 to PTGs with low "
            "priority.");
  cfg.write(sSection, "clearance_num_points", m_clearance_num_points, WN, WV,
            "Number of steps for the piecewise-constant approximation of "
            "clearance (Default=5).");
  cfg.write(
  ...
  // Optional params, for debugging only
  cfg.write(sSection, "vxi", m_nav_dyn_state.curVelLocal.vx, WN, WV,
            "(Only for debugging) Current robot velocity vx [m/s].");
  cfg.write(sSection, "vyi", m_nav_dyn_state.curVelLocal.vy, WN, WV,
            "(Only for debugging) Current robot velocity vy [m/s].");
  cfg.write(sSection, "wi",
            mrpt::utils::RAD2DEG(m_nav_dyn_state.curVelLocal.omega), WN, WV,
            "(Only for debugging) Current robot velocity omega [deg/s].");

  cfg.write(sSection, "reltrg_x", m_nav_dyn_state.relTarget.x, WN, WV,
            "(Only for debugging) Relative target x [m].");
  cfg.write(sSection, "reltrg_y", m_nav_dyn_state.relTarget.y, WN, WV,
            "(Only for debugging) Relative target y [m].");
  cfg.write(sSection, "reltrg_phi",
            mrpt::utils::RAD2DEG(m_nav_dyn_state.relTarget.phi), WN, WV,
            "(Only for debugging) Relative target phi [deg].");

  cfg.write(sSection, "target_rel_speed", m_nav_dyn_state.targetRelSpeed, WN,
            WV, "(Only for debugging) Desired relative speed at target [0,1]");
...
double
CParameterizedTrajectoryGenerator::index2alpha(uint16_t k,
...
uint16_t
CParameterizedTrajectoryGenerator::alpha2index(double alpha,
                                               const unsigned int num_paths) {
  mrpt::math::wrapToPi(alpha);
  int k = mrpt::utils::round(0.5 * (num_paths * (1.0 + alpha / M_PI) - 1.0));
  if (k < 0)
    k = 0;
  if (k >= static_cast<int>(num_paths))
    k = num_paths - 1;
  return (uint16_t)k;
}
...
void CParameterizedTrajectoryGenerator::renderPathAsSimpleLine(
  ...
  for (size_t n = 0; n < nPointsInPath; n++) {
    const double d =
        this->getPathDist(k, n); // distance thru path "k" until timestep "n"

    // Draw the TP only until we reach the target of the "motion" segment:
    if (max_path_distance >= 0.0 && d >= max_path_distance)
      break;
...
bool CParameterizedTrajectoryGenerator::debugDumpInFiles(
  ...
  fy << "% PTG data file for 'y'. Each row is the trajectory for a different "
        "'alpha' parameter value."
     << endl;
  fp << "% PTG data file for 'phi'. Each row is the trajectory for a different "
        "'alpha' parameter value."
     << endl;
...
void CParameterizedTrajectoryGenerator::updateNavDynamicState(
  ...
  // changed
  // may imply a significant computational cost if paths need to be re-evaluated
  // on the fly, etc.
  // so the cost of the comparison here is totally worth:
  if (force_update || m_nav_dyn_state != newState) {
    ASSERT_(newState.targetRelSpeed >= .0 &&
    ...
    if (this->supportSpeedAtTarget()) {
      ...
      // restrictive.
      this->inverseMap_WS2TP(m_nav_dyn_state.relTarget.x,
                             m_nav_dyn_state.relTarget.y, target_k,
                             target_norm_d, 1.0 /*large tolerance*/);
...
void CParameterizedTrajectoryGenerator::internal_TPObsDistancePostprocess(
  ...
  // Handle the special case of obstacles *inside* the robot at the begining of
  // the PTG path:
...
void CParameterizedTrajectoryGenerator::updateClearance(
  ...
  for (uint16_t decim_k = 0; decim_k < cd.get_decimated_num_paths();
       decim_k++) {
    const auto real_k = cd.decimated_k_to_real_k(decim_k);
    this->evalClearanceSingleObstacle(ox, oy, real_k,
                                      cd.get_path_clearance_decimated(decim_k));
  }
}
...
void CParameterizedTrajectoryGenerator::evalClearanceSingleObstacle(
  ...
  for (auto &e : inout_realdist2clearance) {
    ...
    if (had_collision) {
      ...
      // it does not make sense to evaluate the clearance of a pose which is not
      // reachable:
      inout_clearance = .0;
      continue;
    }
...
bool CParameterizedTrajectoryGenerator::TNavDynamicState::
operator==(const TNavDynamicState &o) const {
