...
IMPLEMENTS_SERIALIZABLE(CPTG_DiffDrive_C, CParameterizedTrajectoryGenerator,
                        mrpt::nav)
...
void CPTG_DiffDrive_C::writeToStream(mrpt::utils::CStream &out,
                                     int *version) const {
  if (version) {
    *version = 0;
    return;
...
void CPTG_DiffDrive_C::ptgDiffDriveSteeringFunction(float alpha, float t,
...
bool CPTG_DiffDrive_C::inverseMap_WS2TP(double x, double y, int &k_out,
                                        double &d_out,
