...
IMPLEMENTS_SERIALIZABLE(CPTG_Holo_Blend, CParameterizedTrajectoryGenerator,
                        mrpt::nav)
...
static double calc_trans_distance_t_below_Tramp_abc_numeric(double T, double a,
                                                            double b,
                                                            double c) {
  PERFORMANCE_BENCHMARK;
...
double CPTG_Holo_Blend::calc_trans_distance_t_below_Tramp(double k2, double k4,
                                                          double vxi,
                                                          double vyi,
                                                          double t) {
  /*
...
void CPTG_Holo_Blend::loadFromConfigFile(
  ...
  MRPT_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(T_ramp_max, double, T_ramp_max, cfg,
                                       sSection);
...
void CPTG_Holo_Blend::saveToConfigFile(mrpt::utils::CConfigFileBase &cfg,
  ...
  cfg.write(sSection, "T_ramp_max", T_ramp_max, WN, WV,
            "Max duration of the velocity interpolation since a vel_cmd is "
            "issued [s].");
  ...
  cfg.write(sSection, "expr_W", expr_W, WN, WV,
            "Math expr for |omega| (disregarding the sign, only the module) as "
            "a function of `dir`,`V_MAX`,`W_MAX`,`T_ramp_max`.");
  cfg.write(sSection, "expr_T_ramp", expr_T_ramp, WN, WV,
...
std::string CPTG_Holo_Blend::getDescription() const {
  return mrpt::format("PTG_Holo_Blend_Tramp=%.03f_Vmax=%.03f_Wmax=%.03f",
                      T_ramp_max, V_MAX, W_MAX);
}
...
bool CPTG_Holo_Blend::inverseMap_WS2TP(double x, double y, int &out_k,
                                       double &out_d,
...
CPTG_Holo_Blend::directionToMotionCommand(uint16_t k) const {
...
size_t CPTG_Holo_Blend::getPathStepCount(uint16_t k) const {
  ...
  if (!getPathStepForDist(k, this->refDistance, step)) {
    THROW_EXCEPTION_FMT("Could not solve closed-form distance for k=%u",
...
void CPTG_Holo_Blend::getPathPose(uint16_t k, uint32_t step,
                                  mrpt::math::TPose2D &p) const {
...
bool CPTG_Holo_Blend::getPathStepForDist(uint16_t k, double dist,
                                         uint32_t &out_step) const {
  PERFORMANCE_BENCHMARK;
  ...
  if (dist >= dist_trans_T_ramp) {
    ...
    if (std::abs(k2) < eps && std::abs(k4) < eps) {
      ...
      if (std::abs(b) < eps && std::abs(c) < eps) {
        ...
        // dist = (t/2 + b/(4*a))*(a*t^2 + b*t + c)^(1/2) - (b*c^(1/2))/(4*a) +
        // (log((b/2 + a*t)/a^(1/2) + (a*t^2 + b*t + c)^(1/2))*(- b^2/4 +
        // a*c))/(2*a^(3/2)) - (log((b + 2*a^(1/2)*c^(1/2))/(2*a^(1/2)))*(-
        // b^2/4 + a*c))/(2*a^(3/2))
        // dist =
        ...
        t_solved = T_ramp * 0.6; // Initial value for starting interation inside
                                 // the valid domain of the function
                                 // t=[0,T_ramp]
        for (int iters = 0; iters < 10; iters++) {
          double err =
              calc_trans_distance_t_below_Tramp_abc(t_solved, a, b, c) - dist;
          const double diff =
...
void CPTG_Holo_Blend::updateTPObstacleSingle(double ox, double oy, uint16_t k,
                                             double &tp_obstacle_k) const {
  ...
  // but there are cases in which we will have valid collisions for t>T_ramp but
  // other valid ones
  // for t<T_ramp as well, so the only SAFE way to detect shortest distances is
  // to check over increasing values of "t".
...
void CPTG_Holo_Blend::internal_initialize(const std::string &cacheFilename,
                                          const bool verbose) {
  ...
  m_expr_T_ramp.compile(expr_T_ramp, std::map<std::string, double>(),
                        "expr_T_ramp");
