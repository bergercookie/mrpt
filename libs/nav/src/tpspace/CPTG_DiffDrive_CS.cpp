...
IMPLEMENTS_SERIALIZABLE(CPTG_DiffDrive_CS, CParameterizedTrajectoryGenerator,
                        mrpt::nav)
...
void CPTG_DiffDrive_CS::saveToConfigFile(mrpt::utils::CConfigFileBase &cfg,
...
void CPTG_DiffDrive_CS::writeToStream(mrpt::utils::CStream &out,
                                      int *version) const {
  if (version) {
    *version = 0;
    return;
...
void CPTG_DiffDrive_CS::ptgDiffDriveSteeringFunction(float alpha, float t,
                                                     float x, float y,
                                                     float phi, float &v,
                                                     float &w) const {
  MRPT_UNUSED_PARAM(x);
