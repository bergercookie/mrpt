...
TEST(NavTests, PTGs_tests) {
  ...
  for (unsigned int n = 0; n < PTG_COUNT; n++) {
    ...
    const string sPTGName =
        cfg.read_string("PTG_UNIT_TESTS", format("PTG%u_Type", n), "", true);
  ...
  for (unsigned int n = 0; n < PTG_COUNT; n++) {
    ...
    // TEST: inverseMap_WS2TP
    {
      ...
      for (double tx = -refDist * 0.5; !skip_this_ptg && tx < refDist * 0.5;
           tx += 0.1) {
        for (double ty = -refDist * 0.5; !skip_this_ptg && ty < refDist * 0.5;
             ty += 0.1) {
          if (std::abs(tx) < 1e-2 && std::abs(ty) < 1e-2)
            continue; // TP-Space does not include the WS point (0,0) in its
                      // domain

          const double tolerance_dist =
              std::max(0.10, 10.0 * std::sqrt(tx * tx + ty * ty) * M_PI * 2 /
                                 ptg->getPathCount());
          ...
          if (valid && normalized_d < 1.0) {
            ...
            uint32_t step;
            bool step_ok =
                ptg->getPathStepForDist(k, normalized_d * refDist, step);
    ...
    // TEST: TP_obstacles
    {
      ...
      for (double ox = -refDist * 0.5; !skip_this_ptg && ox < refDist * 0.5;
           ox += 0.1) {
        for (double oy = -refDist * 0.5; !skip_this_ptg && oy < refDist * 0.5;
             oy += 0.1) {
