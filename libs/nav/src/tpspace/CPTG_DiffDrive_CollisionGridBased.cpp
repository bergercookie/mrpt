...
CPTG_DiffDrive_CollisionGridBased::CPTG_DiffDrive_CollisionGridBased()
    : V_MAX(.0), W_MAX(.0), turningRadiusReference(.10), m_resolution(0.05),
      m_stepTimeDuration(0.01), m_collisionGrid(-1, 1, -1, 1, 0.5, this) {}
...
void CPTG_DiffDrive_CollisionGridBased::loadFromConfigFile(
  ...
  MRPT_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(resolution, double, m_resolution, cfg,
                                       sSection);
  MRPT_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(v_max_mps, double, V_MAX, cfg, sSection);
  MRPT_LOAD_HERE_CONFIG_VAR_DEGREES_NO_DEFAULT(w_max_dps, double, W_MAX, cfg,
                                               sSection);
...
void CPTG_DiffDrive_CollisionGridBased::saveToConfigFile(
  ...
  cfg.write(sSection, "resolution", m_resolution, WN, WV,
            "Resolution of the collision-check look-up-table [m].");
  cfg.write(sSection, "v_max_mps", V_MAX, WN, WV,
            "Maximum linear velocity for trajectories [m/s].");
  cfg.write(sSection, "w_max_dps", mrpt::utils::RAD2DEG(W_MAX), WN, WV,
...
mrpt::utils::CStream &mrpt::nav::operator<<(mrpt::utils::CStream &o,
                                            const mrpt::nav::TCPoint &p) {
  o << p.x << p.y << p.phi << p.t << p.dist << p.v << p.w;
  return o;
}
mrpt::utils::CStream &mrpt::nav::operator>>(mrpt::utils::CStream &i,
                                            mrpt::nav::TCPoint &p) {
...
void CPTG_DiffDrive_CollisionGridBased::simulateTrajectories(
  ...
  const float radio_max_robot = 1.0f; // Arbitrary "robot radius", only to
                                      // determine the spacing of points under
                                      // pure rotation
  ...
  try {
    for (unsigned int k = 0; k < m_alphaValuesCount; k++) {
      ...
      float x = .0f, y = .0f, phi = .0f, v = .0f, w = .0f, _x = .0f, _y = .0f,
            _phi = .0f;

      // Sliding window with latest movement commands (for the optional low-pass
      // filtering):
    ...
    // Save accelerations
    if (out_max_acc_v)
      *out_max_acc_v = max_acc_lin;
    if (out_max_acc_w)
      *out_max_acc_w = max_acc_ang;

    // --------------------------------------------------------
    // Build the speeding-up grid for lambda function:
    // --------------------------------------------------------
    const TCellForLambdaFunction defaultCell;
    m_lambdaFunctionOptimizer.setSize(x_min - 0.5f, x_max + 0.5f, y_min - 0.5f,
                                      y_max + 0.5f, 0.25f, &defaultCell);
    ...
    std::cout << format("[CPTG_DiffDrive_CollisionGridBased::"
                        "simulateTrajectories] Simulation aborted: unexpected "
                        "exception!\n");
  }
}
...
CPTG_DiffDrive_CollisionGridBased::directionToMotionCommand(uint16_t k) const {
  float v, w;
...
void CPTG_DiffDrive_CollisionGridBased::CCollisionGrid::updateCellInfo(
  ...
  if (itK == cell->end()) { // New entry:
    cell->push_back(std::make_pair(k, dist));
  } else { // Only update that "k" if the distance is shorter now:
    if (dist < itK->second)
      itK->second = dist;
  }
}
...
bool CPTG_DiffDrive_CollisionGridBased::loadColGridsFromFile(
  ...
  try {
    ...
    if (n != 1)
      return false; // Incompatible (old) format, just discard and recompute.
...
bool CPTG_DiffDrive_CollisionGridBased::CCollisionGrid::loadFromFile(
  ...
  try {
    ...
    *f >> serialized_version;
      ...
      *f >> stored_shape;
      ...
      const bool shapes_match =
          (stored_shape.size() == current_robotShape.size() &&
           std::equal(stored_shape.begin(), stored_shape.end(),
    ...
    case 1:
    default:
      // Unknown version: Maybe we are loading a file from a more recent version
      // of MRPT? Whatever, we can't read it: It's safer just to re-generate the
      // PTG data
      return false;
    };
...
#define READ_DOUBLE_CHECK_IT_MATCHES_STORED(_VAR)                              \
  {                                                                            \
    ...
    *f >> ff;                                                                  \
    ...
    // OK, all parameters seem to be exactly the same than when we precomputed
    // the table: load it.
    // v1 was:  *f >> m_map;
    uint32_t N;
    *f >> N;
...
bool CPTG_DiffDrive_CollisionGridBased::inverseMap_WS2TP(
    ...
    double tolerance_dist) const {
  using mrpt::math::square;

  ASSERTMSG_(m_alphaValuesCount > 0,
...
void CPTG_DiffDrive_CollisionGridBased::setRefDistance(const double refDist) {
  ASSERTMSG_(m_trajectory.empty(), "Changing reference distance not allowed in "
                                   "this class after initialization!");
...
void CPTG_DiffDrive_CollisionGridBased::internal_initialize(
  ...
  ASSERTMSG_(m_robotShape.size() >= 3,
  ...
  if (verbose)
    cout << "Initializing PTG '" << cacheFilename << "'...";

  // Simulate paths:
  const float min_dist = 0.015f;
  simulateTrajectories(100,                         // max.tim,
  ...
  // ----------------------------------------------------------------------------
  m_collisionGrid.setSize(-refDistance, refDistance, -refDistance, refDistance,
                          m_resolution);
  ...
  if (loadColGridsFromFile(cacheFilename, m_robotShape)) {
    ...
    m_collisionGrid.setSize(-refDistance, refDistance, -refDistance,
                            refDistance, m_collisionGrid.getResolution());
    ...
    for (size_t k = 0; k < Ki; k++) {
      ...
      for (size_t n = 0; n < (nPoints - 1); n++) {
        ...
        mrpt::math::TPoint2D bb_min(std::numeric_limits<double>::max(),
                                    std::numeric_limits<double>::max());
        mrpt::math::TPoint2D bb_max(-std::numeric_limits<double>::max(),
                                    -std::numeric_limits<double>::max());
