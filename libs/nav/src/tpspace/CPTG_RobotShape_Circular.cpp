...
void CPTG_RobotShape_Circular::saveToConfigFile(
  ...
  cfg.write(sSection, "robot_radius", m_robotRadius, WN, WV,
            "Robot radius [m].");
}
...
void CPTG_RobotShape_Circular::add_robotShape_to_setOfLines(
  ...
  for (int i = 0; i < N; i++) {
    origin.composePoint(R * cos(i * 2 * M_PI / (N - 1)),
                        R * sin(i * 2 * M_PI / (N - 1)), 0, shap_x[i],
                        shap_y[i], shap_z[i]);
  }
  // Draw a "radius" to identify the "forward" orientation (phi=0)
  gl_shape.appendLine(origin.x(), origin.y(), .0, shap_x[0], shap_y[0],
                      shap_z[0]);
  ...
  gl_shape.appendLine(origin.x() - R * 0.02, origin.y(), .0,
                      origin.x() + R * 0.02, origin.y(), .0);
  gl_shape.appendLine(origin.x(), origin.y() - R * 0.02, .0, origin.x(),
                      origin.y() + R * 0.02, .0);
}
...
double
CPTG_RobotShape_Circular::evalClearanceToRobotShape(const double ox,
