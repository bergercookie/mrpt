...
IMPLEMENTS_SERIALIZABLE(CPTG_DiffDrive_CC, CParameterizedTrajectoryGenerator,
                        mrpt::nav)
...
void CPTG_DiffDrive_CC::saveToConfigFile(mrpt::utils::CConfigFileBase &cfg,
...
void CPTG_DiffDrive_CC::writeToStream(mrpt::utils::CStream &out,
                                      int *version) const {
  if (version) {
    *version = 0;
    return;
...
void CPTG_DiffDrive_CC::ptgDiffDriveSteeringFunction(float alpha, float t,
                                                     float x, float y,
                                                     float phi, float &v,
                                                     float &w) const {
