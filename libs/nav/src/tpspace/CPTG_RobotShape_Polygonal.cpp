...
void CPTG_RobotShape_Polygonal::saveToConfigFile(
  ...
  for (unsigned int i = 0; i < m_robotShape.size(); i++) {
    ...
    cfg.write(sSection, sPtx, m_robotShape[i].x, WN, WV,
              "Robot polygonal shape, `x` [m].");
    cfg.write(sSection, sPty, m_robotShape[i].y, WN, WV,
...
void CPTG_RobotShape_Polygonal::add_robotShape_to_setOfLines(
  ...
  if (N >= 2) {
    ...
    for (int i = 0; i < N; i++) {
      origin.composePoint(m_robotShape[i].x, m_robotShape[i].y, 0, shap_x[i],
                          shap_y[i], shap_z[i]);
    }

    gl_shape.appendLine(shap_x[0], shap_y[0], shap_z[0], shap_x[1], shap_y[1],
                        shap_z[1]);
...
double
CPTG_RobotShape_Polygonal::evalClearanceToRobotShape(const double ox,
