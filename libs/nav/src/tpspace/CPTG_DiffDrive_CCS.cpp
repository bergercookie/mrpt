...
IMPLEMENTS_SERIALIZABLE(CPTG_DiffDrive_CCS, CParameterizedTrajectoryGenerator,
                        mrpt::nav)
...
void CPTG_DiffDrive_CCS::saveToConfigFile(mrpt::utils::CConfigFileBase &cfg,
...
void CPTG_DiffDrive_CCS::writeToStream(mrpt::utils::CStream &out,
                                       int *version) const {
  if (version) {
    *version = 0;
    return;
...
void CPTG_DiffDrive_CCS::ptgDiffDriveSteeringFunction(float alpha, float t,
                                                      float x, float y,
                                                      float phi, float &v,
                                                      float &w) const {
