...
void PlannerRRT_SE2_TPS::loadConfig(const mrpt::utils::CConfigFileBase &ini,
                                    const std::string &sSect) {
 ...
 * 'target' */
void PlannerRRT_SE2_TPS::solve(const PlannerRRT_SE2_TPS::TPlannerInput &pi,
  ...
  if (result.move_tree.getAllNodes().empty()) {
    ...
    result.move_tree.insertNode(result.move_tree.root,
                                TNodeSE2_TP(pi.start_pose));
  }
    ...
    for (size_t idxPTG = 0; idxPTG < nPTGs; ++idxPTG) {
      ...
      if (x_nearest_id == INVALID_NODEID) {
        ...
        if (params.save_3d_log_freq > 0 &&
          ...
          scene.saveToFile(
              mrpt::format("./rrt_log_trees/rrt_log_%03u_%06u.3Dscene",
      ...
      int k_rand; // k_rand is the index of target_alpha in PTGs corresponding
                  // to a specific d_rand
      // bool tp_point_is_exact =
      m_PTGs[idxPTG]->inverseMap_WS2TP(x_rand_rel.x(), x_rand_rel.y(), k_rand,
                                       d_rand);
      d_rand *= m_PTGs[idxPTG]
                    ->getRefDistance(); // distance to target, in "real meters"

      float d_free;
      // bool local_obs_ok = false; // Just for 3D log files: indicates whether
      // obstacle points have been recomputed
      ...
      const double MAX_DIST_FOR_OBSTACLES =
          1.5 * m_PTGs[idxPTG]->getRefDistance(); // Maximum Euclidean distance
                                                  // (radius) for considering
                                                  // obstacles around the
                                                  // current robot pose

      ASSERT_ABOVE_(m_PTGs[idxPTG]->getRefDistance(),
                    1.1 * max_veh_radius); // Make sure the PTG covers at least
                                           // a bit more than the vehicle
                                           // shape!! (should be much, much
                                           // higher)

      {
        CTimeLoggerEntry tle(m_timelogger,
                             "PT_RRT::solve.changeCoordinatesReference");
        transformPointcloudWithSquareClipping(pi.obstacles_points, m_local_obs,
                                              CPose2D(x_nearest_node.state),
                                              MAX_DIST_FOR_OBSTACLES);
        // local_obs_ok=true;
      }
      {
        CTimeLoggerEntry tle(m_timelogger, "PT_RRT::solve.SpaceTransformer");
        spaceTransformerOneDirectionOnly(
            k_rand, m_local_obs, m_PTGs[idxPTG].get(), MAX_DIST_FOR_OBSTACLES,
            TP_Obstacles_k_rand);
      }
      ...
      double d_new = std::min(
          D_max, d_rand); // distance of the new candidate state in TP-space
...
// mrpt::poses::CPose2D *log_new_state_ptr=NULL; // For graphical logs only
      ...
      sLogTxt += mrpt::format(
          ...
          static_cast<unsigned int>(idxPTG), tp_point_is_exact ? 'Y' : 'N',
          d_free, d_rand, d_new);
      sLogTxt +=
          mrpt::format(" nearest:%s\n", x_nearest_pose.asString().c_str());
#endif
      ...
      if (d_free >= d_new) {
        ...
        mrpt::math::wrapToPiInPlace(
            rel_pose.phi); // wrap to [-pi,pi] -->avoid out of bounds errors

        // [Algo `tp_space_rrt`: Line 15]: pose composition
        // ------------------------------------------------------------
        const mrpt::poses::CPose2D new_state_rel(rel_pose);
        mrpt::poses::CPose2D new_state =
            x_nearest_pose + new_state_rel; // compose the new_motion as the
                                            // last nmotion and the new state
        ...
        if (!is_acceptable_goal) // Only check for nearby nodes if this is not a
                                 // solution!
        {
          ...
          new_nearest_id = result.move_tree.getNearestNode(
              new_state_node, distance_evaluator_se2, &new_nearest_dist,
              &result.acceptable_goal_node_ids);
          m_timelogger.leave("TMoveTree::getNearestNode");

          if (new_nearest_id != INVALID_NODEID) {
            // Also check angular distance:
            const double new_nearest_ang = std::abs(mrpt::math::angDistance(
                new_state.phi(), result.move_tree.getAllNodes()
                                     .find(new_nearest_id)
                                     ->second.state.phi));
            accept_this_node =
                (new_nearest_dist >= params.minDistanceBetweenNewNodes ||
        ...
        if (!accept_this_node) {
          ...
          if (new_nearest_id != INVALID_NODEID) {
            sLogTxt +=
                mrpt::format(" -> new node NOT accepted for closeness to: %s\n",
    ...
    if (!candidate_new_nodes.empty()) {
      ...
      result.move_tree.insertNodeAndEdge(best_edge.parent_id, new_child_id,
      ...
      const double goal_ang = std::abs(
          mrpt::math::angDistance(best_edge.end_state.phi, pi.goal_pose.phi));
      ...
      if (is_acceptable_goal) // Don't waste time computing path length if it
