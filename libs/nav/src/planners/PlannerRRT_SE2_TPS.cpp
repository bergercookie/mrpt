	...
	for (;;)
	{
		...
		for (size_t idxPTG = 0; idxPTG < nPTGs; ++idxPTG)
		{
			...
			const double MAX_DIST_FOR_OBSTACLES =
				1.5 *
				m_PTGs[idxPTG]->getRefDistance();  // Maximum Euclidean distance
			...
			ASSERT_ABOVE_(
				...
				1.1 *
					max_veh_radius);  // Make sure the PTG covers at least a bit
			// more than the vehicle shape!! (should be
			// much, much higher)
...
// mrpt::poses::CPose2D *log_new_state_ptr=NULL; // For graphical logs only
			...
			if (d_free >= d_new)
			{
				...
				mrpt::poses::CPose2D new_state =
					x_nearest_pose +
					new_state_rel;  // compose the new_motion as the
