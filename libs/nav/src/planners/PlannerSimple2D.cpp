...
PlannerSimple2D::PlannerSimple2D()
    : occupancyThreshold(0.5f), minStepInReturnedPath(0.4f),
      robotRadius(0.35f) {}

/*---------------------------------------------------------------
                                                computePath
  ---------------------------------------------------------------*/
void PlannerSimple2D::computePath(const COccupancyGridMap2D &theMap,
...
#define CELL_TARGET 0xFFFE
  ...
  do {
      ...
      for (x = range_x_min; x < range_x_max; x++) {
        if (grid[x + row] == CELL_EMPTY) {
          ...
          if (minNeigh < CELL_EMPTY && maxNeigh > CELL_EMPTY) {
            ...
            longestPathInCellsMetric =
                max(longestPathInCellsMetric, CELL_EMPTY - minNeigh);
            ...
            longestPathInCellsMetric =
                max(longestPathInCellsMetric, maxNeigh - CELL_EMPTY);
    ...
    if (maxSearchPathLength > 0)
      if (theMap.getResolution() * longestPathInCellsMetric * 0.5f >
        ...
        //Path exceeded desired length! (length=%f m)\n", theMap.getResolution()
        //* longestPathInCellsMetric * 0.5f);
        notFound = true;
      }
  ...
  while ((v = grid[x + size_x * y]) != CELL_ORIGIN) {
    // Add cell to the path (in inverse order, now we go backward!!) Later is
    // will be reversed
