...
RRTAlgorithmParams::RRTAlgorithmParams()
    : robot_shape_circular_radius(0.30), ptg_cache_files_directory("."),
      goalBias(0.05), maxLength(1.0), minDistanceBetweenNewNodes(0.10),
      minAngBetweenNewNodes(mrpt::utils::DEG2RAD(15)), ptg_verbose(true),
      save_3d_log_freq(0) {
  ...
  for (size_t i = 0; i < m_PTGs.size(); i++) {
    ...
    // Polygonal robot shape?
    {
      mrpt::nav::CPTG_DiffDrive_CollisionGridBased *diff_ptg =
          dynamic_cast<mrpt::nav::CPTG_DiffDrive_CollisionGridBased *>(
              m_PTGs[i].get());
      if (diff_ptg) {
        ASSERTMSG_(!poly_robot_shape.empty(),
    ...
    // Circular robot shape?
    {
      mrpt::nav::CPTG_RobotShape_Circular *ptg =
          dynamic_cast<mrpt::nav::CPTG_RobotShape_Circular *>(m_PTGs[i].get());
      if (ptg) {
        ASSERTMSG_(params.robot_shape_circular_radius > 0,
                   "No circular robot shape specified, and PTG requires one!");
        ptg->setRobotShapeRadius(params.robot_shape_circular_radius);
      }
    }

    m_PTGs[i]->initialize(mrpt::format("%s/TPRRT_PTG_%03u.dat.gz",
...
void PlannerTPS_VirtualBase::internal_loadConfig_PTG(
  ...
  // polygonal shape
  {
    ...
    if (!sShape.empty()) {
      ...
      if (!mShape.fromMatlabStringFormat(sShape))
        THROW_EXCEPTION_FMT("Error parsing robot_shape matrix: '%s'",
                            sShape.c_str());
  ...
  for (unsigned int n = 0; n < PTG_COUNT; n++) {
    ...
    m_PTGs.push_back(CParameterizedTrajectoryGenerator::Ptr(
        CParameterizedTrajectoryGenerator::CreatePTG(sPTGName, ini, sSect,
                                                     format("PTG%u_", n))));
  }
}
...
void PlannerTPS_VirtualBase::transformPointcloudWithSquareClipping(
  ...
  // We can safely discard the rest of obstacles, since they cannot be converted
  ...
  for (size_t obs = 0; obs < nObs; obs++) {
    ...
    if (std::abs(gx - asSeenFrom.x()) > MAX_DIST_XY ||
      ...
      continue; // ignore this obstacle: anyway, I don't know how to map it to
                // TP-Obs!
...
void PlannerTPS_VirtualBase::spaceTransformer(
  ...
  try {
    ...
    // Leave distances in out_TPObstacles un-normalized ([0,1]), so they just
    // represent real distances in meters.
...
void PlannerTPS_VirtualBase::spaceTransformerOneDirectionOnly(
  ...
  try {
    ...
    for (size_t obs = 0; obs < nObs; obs++) {
      ...
      in_PTG->updateTPObstacleSingle(ox, oy, tp_space_k_direction,
                                     out_TPObstacle_k);
    }
