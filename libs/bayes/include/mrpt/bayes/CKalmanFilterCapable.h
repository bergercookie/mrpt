...
struct TKF_options : public mrpt::utils::CLoadableOptions {
  TKF_options(mrpt::utils::VerbosityLevel &verb_level_ref)
      : method(kfEKFNaive), verbosity_level(verb_level_ref), IKF_iterations(5),
        enable_profiler(false), use_analytic_transition_jacobian(true),
  ...
  void loadFromConfigFile(const mrpt::utils::CConfigFileBase &iniFile,
    ...
    MRPT_LOAD_CONFIG_VAR(use_analytic_transition_jacobian, bool, iniFile,
                         section);
    MRPT_LOAD_CONFIG_VAR(use_analytic_observation_jacobian, bool, iniFile,
                         section);
    MRPT_LOAD_CONFIG_VAR(debug_verify_analytic_jacobians, bool, iniFile,
                         section);
    MRPT_LOAD_CONFIG_VAR(debug_verify_analytic_jacobians_threshold, double,
                         iniFile, section);
  }
  ...
  int IKF_iterations;   //!< Number of refinement iterations, only for the IKF
                        //!method.
  bool enable_profiler; //!< If enabled (default=false), detailed timing
                        //!information will be dumped to the console thru a
                        //!CTimerLog at the end of the execution.
  bool use_analytic_transition_jacobian; //!< (default=true) If true,
                                         //!OnTransitionJacobian will be called;
                                         //!otherwise, the Jacobian will be
                                         //!estimated from a numeric
                                         //!approximation by calling several
                                         //!times to OnTransitionModel.
  bool use_analytic_observation_jacobian; //!< (default=true) If true,
                                          //!OnObservationJacobians will be
                                          //!called; otherwise, the Jacobian
                                          //!will be estimated from a numeric
                                          //!approximation by calling several
                                          //!times to OnObservationModel.
  bool debug_verify_analytic_jacobians;   //!< (default=false) If true, will
                                        //!compute all the Jacobians numerically
                                        //!and compare them to the analytical
                                        //!ones, throwing an exception on
                                        //!mismatch.
  double debug_verify_analytic_jacobians_threshold; //!< (default-1e-2) Sets the
                                                    //!threshold for the
 ...
 *	- OBS_SIZE: The dimension of each observation (eg, 2 for pixel
 *coordinates, 3 for 3D coordinates,etc).
 *	- FEAT_SIZE: The dimension of the features in the system state (the
 *"map"), or 0 if not applicable (the default if not implemented).
...
public:
  ...
  typedef KFTYPE
      kftype; //!< The numeric type used in the Kalman Filter (default=double)
  typedef CKalmanFilterCapable<VEH_SIZE, OBS_SIZE, FEAT_SIZE, ACT_SIZE, KFTYPE>
      KFCLASS; //!< My class, in a shorter name!
  ...
  inline void getLandmarkMean(size_t idx, KFArray_FEAT &feat) const {
    ...
    ::memcpy(&feat[0], &m_xkk[VEH_SIZE + idx * FEAT_SIZE],
             FEAT_SIZE * sizeof(m_xkk[0]));
  }
  /** Returns the covariance of the idx'th landmark (not applicable to non-SLAM
   * problems).
    * \exception std::exception On idx>= getNumberOfLandmarksInTheMap()
    */
  inline void getLandmarkCov(size_t idx, KFMatrix_FxF &feat_cov) const {
    m_pkk.extractMatrix(VEH_SIZE + idx * FEAT_SIZE, VEH_SIZE + idx * FEAT_SIZE,
                        feat_cov);
  }
...
protected:
   ...
   * updated as usual (this is to allow numeric approximation of Jacobians).
    */
  virtual void OnTransitionModel(const KFArray_ACT &in_u, KFArray_VEH &inout_x,
  ...
  /** Implements the transition Jacobian \f$ \frac{\partial f}{\partial x} \f$
    ...
    *  The returned matrix must be \f$V \times V\f$ with V being either the size
   * of the whole state vector (for non-SLAM problems) or VEH_SIZE (for SLAM
   * problems).
    */
   ...
   * state vector while estimating the Jacobian.
    */
  virtual void
  OnTransitionJacobianNumericGetIncrements(KFArray_VEH &out_increments) const {
   ...
   * the application to reduce the number of covariance landmark predictions to
   * be made.
   ...
   * [0,getNumberOfLandmarksInTheMap()-1] that should be predicted.
    * \note This is not a pure virtual method, so it should be implemented only
   * if desired. The default implementation returns a vector with all the
   ...
   * observation for the given problem.
    ...
    */
  virtual void
  OnObservationModel(const mrpt::vector_size_t &idx_landmarks_to_predict,
   ...
   * zero and the expected output is for the whole state vector.
    ...
    */
  virtual void OnObservationJacobians(const size_t &idx_landmark_to_predict,
                                      KFMatrix_OxV &Hx,
   ...
   * topology of the individual scalar components (eg, angles).
    */
  virtual void OnSubstractObservationVectors(KFArray_OBS &A,
...
public:
   ...
   * y_n=y(x,z_n) \f$.
    * \param out_dyn_dxv The \f$F \times V\f$ Jacobian of the inv. sensor model
   * wrt the robot pose \f$ \frac{\partial y_n}{\partial x_v} \f$.
    * \param out_dyn_dhn The \f$F \times O\f$ Jacobian of the inv. sensor model
   * wrt the observation vector \f$ \frac{\partial y_n}{\partial h_n} \f$.
    *
    *  - O: OBS_SIZE
    *  - V: VEH_SIZE
    *  - F: FEAT_SIZE
  ...
  /** If applicable to the given problem, this method implements the inverse
   * observation model needed to extend the "map" with a new "element".
    *  The uncertainty in the new map feature comes from two parts: one from the
   * vehicle uncertainty (through the out_dyn_dxv Jacobian),
    *   and another from the uncertainty in the observation itself. By default,
   * out_use_dyn_dhn_jacobian=true on call, and if it's left at "true",
    *   the base KalmanFilter class will compute the uncertainty of the landmark
   * relative position from out_dyn_dhn.
  ...
  /** If applicable to the given problem, do here any special handling of adding
   * a new landmark to the map.
   ...
   * state vector (0:just after the vehicle state, 1: after that,...). Save this
   * number so data association can be done according to these indices.
    * \sa OnInverseObservationModel
    */
  virtual void OnNewLandmarkAddedToMap(const size_t in_obsIdx,
...
public:
  CKalmanFilterCapable()
      : mrpt::utils::COutputLogger("CKalmanFilterCapable"),
        ...
        m_user_didnt_implement_jacobian(true) {} //!< Default constructor
  virtual ~CKalmanFilterCapable() {}             //!< Destructor

  mrpt::utils::CTimeLogger &getProfiler() { return m_timLogger; }

  TKF_options
      KF_options; //!< Generic options for the Kalman Filter algorithm itself.
...
private:
  ...
  typename mrpt::aligned_containers<KFMatrix_OxV>::vector_t
      Hxs; //!< The vector of all partial Jacobians dh[i]_dx for each prediction
  typename mrpt::aligned_containers<KFMatrix_OxF>::vector_t Hys; //!< The vector
                                                                 //!of all
                                                                 //!partial
                                                                 //!Jacobians
                                                                 //!dh[i]_dy[i]
...
private:
  ...
  friend void detail::addNewLandmarks(
      CKalmanFilterCapable<VEH_SIZEb, OBS_SIZEb, FEAT_SIZEb, ACT_SIZEb, KFTYPEb>
          &obj,
      ...
      const vector_int &data_association,
      const typename CKalmanFilterCapable<VEH_SIZEb, OBS_SIZEb, FEAT_SIZEb,
                                          ACT_SIZEb, KFTYPEb>::KFMatrix_OxO &R);
}; // end class
...
namespace utils {
template <> struct TEnumTypeFiller<bayes::TKFMethod> {
