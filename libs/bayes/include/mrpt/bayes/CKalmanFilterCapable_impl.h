						  ...
						  KFTYPE>::runOneKalmanIteration()
{
	...
	bool skipPrediction =
		false;  // Whether to skip the prediction step (in SLAM
	...
	if (!skipPrediction)
	{
		...
		{  // Numeric approximation:
			...
			if (KF_options.debug_verify_analytic_jacobians)
			{
				...
				{
					std::cerr
						<< "[KalmanFilter] ERROR: User analytical transition "
						   "Jacobians are wrong: \n"
						<< " Real dfv_dxv: \n"
						<< dfv_dxv << "\n Analytical dfv_dxv:\n"
						<< dfv_dxv_gt << "Diff:\n"
	...
	do
	{
		if (!missing_predictions_to_add.empty())
		{
			...
			MRPT_LOG_WARN_STREAM(
				...
				<< nNew << " LMs were not correctly predicted "
						   "by OnPreComputingPredictions().");
		...
		for (size_t i = first_new_pred; i < N_pred; ++i)
		{
			...
			m_user_didnt_implement_jacobian =
				false;  // Set to true by the default
		...
		switch (KF_options.method)
		{
			...
			case kfIKFFull:
			{
				...
				const size_t N_upd =
					(FEAT_SIZE == 0)
						? 1
						:  // Non-SLAM problems: Just one observation
						...
						{  // SLAM problems:
							...
							for (size_t i = 0; i < data_association.size(); ++i)
							{
								...
								ASSERTMSG_(
									...
									"recommend the "
									"prediction of a landmark which has been "
									"actually "
									"observed!")
								// TODO: In these cases, extend the prediction
								// right now instead
								// of launching an exception... or is this a bad
								// idea??
				...
				for (size_t obsIdx = 0; obsIdx < Z.size(); obsIdx++)
				{
					...
					if (doit)
					{
						...
						ASSERTMSG_(
							...
							"prediction "
							"of a landmark which has been actually observed!")
