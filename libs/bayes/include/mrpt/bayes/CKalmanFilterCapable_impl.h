...
void CKalmanFilterCapable<VEH_SIZE, OBS_SIZE, FEAT_SIZE, ACT_SIZE,
  ...
  // Sanity check:
  if (FEAT_SIZE) {
    ASSERTDEB_((((m_xkk.size() - VEH_SIZE) / FEAT_SIZE) * FEAT_SIZE) ==
               (m_xkk.size() - VEH_SIZE))
  }
  ...
  bool skipPrediction = false; // Whether to skip the prediction step (in SLAM
                               // this is desired for the first iteration...)
  ...
  if (!skipPrediction) {
    ...
    m_user_didnt_implement_jacobian = false; // Set to true by the default
                                             // method if not reimplemented in
                                             // base class.
    ...
    if (m_user_didnt_implement_jacobian ||
        ...
        KF_options.debug_verify_analytic_jacobians) { // Numeric approximation:
      ...
      if (KF_options.debug_verify_analytic_jacobians) {
        ...
        if ((dfv_dxv - dfv_dxv_gt).array().abs().sum() >
          ...
          std::cerr << "[KalmanFilter] ERROR: User analytical transition "
                       "Jacobians are wrong: \n"
                    << " Real dfv_dxv: \n"
                    << dfv_dxv << "\n Analytical dfv_dxv:\n"
                    << dfv_dxv_gt << "Diff:\n"
                    << (dfv_dxv - dfv_dxv_gt) << "\n";
          THROW_EXCEPTION("ERROR: User analytical transition Jacobians are "
                          "wrong (More details dumped to cerr)")
        }
      }
    }
    ...
    for (size_t i = 0; i < N_map; i++) {
      aux =
          dfv_dxv * Eigen::Block<typename KFMatrix::Base, VEH_SIZE, FEAT_SIZE>(
  ...
  OnObservationModel(mrpt::math::sequenceStdVec<size_t, 1>(0, N_map),
                     all_predictions);
  ...
  vector_int
      data_association; // -1: New map feature.>=0: Indexes in the state vector

  // The next loop will only do more than one iteration if the heuristic in
  // OnPreComputingPredictions() fails,
  //  which will be detected by the addition of extra landmarks to predict into
  ...
  do {
    if (!missing_predictions_to_add.empty()) {
      ...
      MRPT_LOG_WARN_STREAM("[KF] *Performance Warning*: "
    ...
    for (size_t i = first_new_pred; i < N_pred; ++i) {
      ...
      if (m_user_didnt_implement_jacobian ||
          ...
          KF_options
              .debug_verify_analytic_jacobians) { // Numeric approximation:
        ...
        OnObservationJacobiansNumericGetIncrements(xkk_veh_increments,
                                                   feat_increments);

        mrpt::math::estimateJacobian(
            x_vehicle, &KF_aux_estimate_obs_Hx_jacobian, xkk_veh_increments,
            std::pair<KFCLASS *, size_t>(this, lm_idx), Hx);
        // The state vector was temporarily modified by KF_aux_estimate_*,
        // restore it:
        ...
        // The state vector was temporarily modified by KF_aux_estimate_*,
        // restore it:
        ::memcpy(&m_xkk[lm_idx_in_statevector], &x_feat[0],
        ...
        if (KF_options.debug_verify_analytic_jacobians) {
          ...
          if ((Hx - Hx_gt).array().abs().sum() >
              KF_options.debug_verify_analytic_jacobians_threshold) {
            std::cerr << "[KalmanFilter] ERROR: User analytical observation Hx "
                         "Jacobians are wrong: \n"
                      << " Real Hx: \n"
                      << Hx << "\n Analytical Hx:\n"
                      << Hx_gt << "Diff:\n"
                      << Hx - Hx_gt << "\n";
            THROW_EXCEPTION("ERROR: User analytical observation Hx Jacobians "
                            "are wrong (More details dumped to cerr)")
          }
          if ((Hy - Hy_gt).array().abs().sum() >
              KF_options.debug_verify_analytic_jacobians_threshold) {
            std::cerr << "[KalmanFilter] ERROR: User analytical observation Hy "
                         "Jacobians are wrong: \n"
                      << " Real Hy: \n"
                      << Hy << "\n Analytical Hx:\n"
                      << Hy_gt << "Diff:\n"
                      << Hy - Hy_gt << "\n";
            THROW_EXCEPTION("ERROR: User analytical observation Hy Jacobians "
                            "are wrong (More details dumped to cerr)")
          }
        }
      }
    ...
    if (FEAT_SIZE > 0) { // SLAM-like problem:
      const Eigen::Block<const typename KFMatrix::Base, VEH_SIZE, VEH_SIZE> Px(
          m_pkk, 0, 0); // Covariance of the vehicle pose

      for (size_t i = 0; i < N_pred; ++i) {
        const size_t lm_idx_i = predictLMidxs[i];
        const Eigen::Block<const typename KFMatrix::Base, FEAT_SIZE, VEH_SIZE>
            Pxyi_t(m_pkk, VEH_SIZE + lm_idx_i * FEAT_SIZE, 0); // Pxyi^t

        // Only do j>=i (upper triangle), since S is symmetric:
        for (size_t j = i; j < N_pred; ++j) {
          const size_t lm_idx_j = predictLMidxs[j];
          // Sij block:
          Eigen::Block<typename KFMatrix::Base, OBS_SIZE, OBS_SIZE> Sij(
              S, OBS_SIZE * i, OBS_SIZE * j);

          const Eigen::Block<const typename KFMatrix::Base, VEH_SIZE, FEAT_SIZE>
              Pxyj(m_pkk, 0, VEH_SIZE + lm_idx_j * FEAT_SIZE);
          const Eigen::Block<const typename KFMatrix::Base, FEAT_SIZE,
                             FEAT_SIZE>
              Pyiyj(m_pkk, VEH_SIZE + lm_idx_i * FEAT_SIZE,
    ...
    } else { // Not SLAM-like problem: simply S=H*Pkk*H^t + R
      ASSERTDEB_(N_pred == 1)
    ...
    OnGetObservationsAndDataAssociation(Z, data_association, // Out
                                        all_predictions, S, predictLMidxs,
                                        R // In
                                        );
    ASSERTDEB_(data_association.size() == Z.size() ||
    ...
    if (FEAT_SIZE != 0) {
      for (size_t i = 0; i < data_association.size(); ++i) {
        ...
        const size_t assoc_idx_in_pred =
            mrpt::utils::find_in_vector(assoc_idx_in_map, predictLMidxs);
    ...
    first_new_pred =
        N_pred; // If we do another loop, start at the begin of new predictions
  ...
  if (!Z.empty()) {
    ...
    case kfIKFFull: {
      ...
      mapIndicesForKFUpdate.resize(std::distance(
          ...
          std::remove_copy_if(data_association.begin(), data_association.end(),
                              mapIndicesForKFUpdate.begin(),
                              bind1st(equal_to<int>(), -1))));

      const size_t N_upd =
          (FEAT_SIZE == 0) ? 1 : // Non-SLAM problems: Just one observation for
                                 // the entire system.
      ...
      {
        for (size_t IKF_iteration = 0; IKF_iteration < nKF_iterations;
             IKF_iteration++) {
          // Compute ytilde = OBS - PREDICTION
          KFVector ytilde(OBS_SIZE * N_upd);
          size_t ytilde_idx = 0;

          // TODO: Use a Matrix view of "dh_dx_full" instead of creating a copy
          // into "dh_dx_full_obs"
          dh_dx_full_obs.zeros(N_upd * OBS_SIZE,
                               VEH_SIZE + FEAT_SIZE * N_map); // Init to zeros.
          KFMatrix S_observed; // The KF "S" matrix: A re-ordered, subset,
                               // version of the prediction S:

          if (FEAT_SIZE != 0) { // SLAM problems:
            ...
            for (size_t i = 0; i < data_association.size(); ++i) {
              ...
              const size_t assoc_idx_in_pred =
                  mrpt::utils::find_in_vector(assoc_idx_in_map, predictLMidxs);
              ASSERTMSG_(assoc_idx_in_pred != string::npos,
                         "OnPreComputingPredictions() didn't recommend the "
                         "prediction of a landmark which has been actually "
                         "observed!")
              // TODO: In these cases, extend the prediction right now instead
              // of launching an exception... or is this a bad idea??
              ...
              //										=
              //										dh_dx_full.block
              //(assoc_idx_in_pred*OBS_SIZE,0, OBS_SIZE, row_len);

              Eigen::Block<typename KFMatrix::Base, OBS_SIZE, VEH_SIZE>(
                  dh_dx_full_obs, S_idxs.size(), 0) = Hxs[assoc_idx_in_pred];
              Eigen::Block<typename KFMatrix::Base, OBS_SIZE, FEAT_SIZE>(
              ...
              OnSubstractObservationVectors(
                  ytilde_i, all_predictions[predictLMidxs[assoc_idx_in_pred]]);
              for (size_t k = 0; k < OBS_SIZE; k++)
          ...
          } else { // Non-SLAM problems:
            ASSERT_(Z.size() == 1 && all_predictions.size() == 1)
            ASSERT_(dh_dx_full_obs.getRowCount() == OBS_SIZE &&
          ...
          if (nKF_iterations == 1) {
            ...
            m_xkk = xkk_0;
            K.multiply_Ab((ytilde - HAx_column), m_xkk,
              ...
              for (size_t c = 0; c < stat_len; c++) {
                if (r == c)
                  aux_K_dh_dx.get_unsafe(r, c) =
                      -aux_K_dh_dx.get_unsafe(r, c) + kftype(1);
                else
                  aux_K_dh_dx.get_unsafe(r, c) = -aux_K_dh_dx.get_unsafe(r, c);
              }
            }
      ...
      for (size_t obsIdx = 0; obsIdx < Z.size(); obsIdx++) {
        ...
        if (doit) {
          ...
          const size_t idx_off =
              VEH_SIZE +
              idxInTheFilter * FEAT_SIZE; // The offset in m_xkk & Pkk.

          // Compute just the part of the Jacobian that we need using the
          // current updated m_xkk:
          ...
          // Jacobians:
          // dh_dx: already is (N_pred*OBS_SIZE) x (VEH_SIZE + FEAT_SIZE *
          // N_pred )
          //         with N_pred = |predictLMidxs|

          const size_t i_idx_in_preds =
              mrpt::utils::find_in_vector(idxInTheFilter, predictLMidxs);
          ASSERTMSG_(i_idx_in_preds != string::npos,
                     "OnPreComputingPredictions() didn't recommend the "
                     "prediction of a landmark which has been actually "
                     "observed!")
...
//
...
#if defined(_DEBUG)
            {
              // This algorithm is applicable only if the scalar component of
              // the sensor noise are INDEPENDENT:
              for (size_t a = 0; a < OBS_SIZE; a++)
                for (size_t b = 0; b < OBS_SIZE; b++)
                  if (a != b)
                    if (R(a, b) != 0)
                      THROW_EXCEPTION("This KF algorithm assumes independent "
                                      "noise components in the observation "
                                      "(matrix R). Select another KF "
                                      "algorithm.")
            }
#endif
            // R:
            ...
            for (size_t k = 0; k < FEAT_SIZE; k++) {
              ...
              for (size_t q = 0; q < FEAT_SIZE; q++)
                accum += Hy.get_unsafe(j, q) *
                         m_pkk.get_unsafe(idx_off + q, idx_off + k);
  ...
  MRPT_LOG_DEBUG(mrpt::format(
      ...
      "%.2fms\n",
      static_cast<unsigned int>(getNumberOfLandmarksInTheMap()), 1e3 * tim_pred,
      1e3 * tim_pred_obs, 1e3 * tim_obs_DA, 1e3 * tim_update));
  MRPT_END
...
void CKalmanFilterCapable<VEH_SIZE, OBS_SIZE, FEAT_SIZE, ACT_SIZE, KFTYPE>::
    KF_aux_estimate_trans_jacobian(const KFArray_VEH &x,
...
template <size_t VEH_SIZE, size_t OBS_SIZE, size_t FEAT_SIZE, size_t ACT_SIZE,
          typename KFTYPE>
void CKalmanFilterCapable<VEH_SIZE, OBS_SIZE, FEAT_SIZE, ACT_SIZE, KFTYPE>::
    KF_aux_estimate_obs_Hx_jacobian(const KFArray_VEH &x,
...
template <size_t VEH_SIZE, size_t OBS_SIZE, size_t FEAT_SIZE, size_t ACT_SIZE,
          typename KFTYPE>
void CKalmanFilterCapable<VEH_SIZE, OBS_SIZE, FEAT_SIZE, ACT_SIZE, KFTYPE>::
    KF_aux_estimate_obs_Hy_jacobian(const KFArray_FEAT &x,
                                    const std::pair<KFCLASS *, size_t> &dat,
                                    KFArray_OBS &out_x) {
  vector_size_t idxs_to_predict(1, dat.second);
  vector_KFArray_OBS prediction;
  ...
  // Overwrite (temporarily!) the affected part of the state vector:
  ::memcpy(&dat.first->m_xkk[lm_idx_in_statevector], &x[0],
...
void addNewLandmarks(
  ...
  typedef CKalmanFilterCapable<VEH_SIZE, OBS_SIZE, FEAT_SIZE, ACT_SIZE, KFTYPE>
      KF;
  ...
  for (size_t idxObs = 0; idxObs < Z.size(); idxObs++) {
    ...
    if (data_association[idxObs] < 0) // Not in the map yet!
    {
      ...
      ASSERTDEB_(FEAT_SIZE > 0)
      ASSERTDEB_(0 == ((obj.internal_getXkk().size() - VEH_SIZE) %
      ...
      obj.OnInverseObservationModel(Z[idxObs], yn, dyn_dxv, dyn_dhn,
                                    dyn_dhn_R_dyn_dhnT, use_dyn_dhn_jacobian);

      // And let the application do any special handling of adding a new feature
      // to the map:
      ...
      obj.internal_getXkk().conservativeResize(obj.internal_getXkk().size() +
                                               FEAT_SIZE);
      ...
      // Append to Pkk:
      // --------------------
      ASSERTDEB_(obj.internal_getPkk().getColCount() == idx &&
      ...
      for (q = 0; q < nLMs; q++) {
        ...
        obj.internal_getPkk().extractMatrix(0, VEH_SIZE + q * FEAT_SIZE,
                                            P_x_yq);

        typename KF::KFMatrix_FxF P_cross(mrpt::math::UNINITIALIZED_MATRIX);
        P_cross.multiply(dyn_dxv, P_x_yq);

        obj.internal_getPkk().insertMatrix(idx, VEH_SIZE + q * FEAT_SIZE,
                                           P_cross);
        obj.internal_getPkk().insertMatrixTranspose(VEH_SIZE + q * FEAT_SIZE,
                                                    idx, P_cross);
...
template <size_t VEH_SIZE, size_t OBS_SIZE, size_t FEAT_SIZE, size_t ACT_SIZE,
          typename KFTYPE>
inline bool isMapEmpty(const CKalmanFilterCapable<VEH_SIZE, OBS_SIZE, FEAT_SIZE,
                                                  ACT_SIZE, KFTYPE> &obj) {
...
template <size_t VEH_SIZE, size_t OBS_SIZE, size_t ACT_SIZE, typename KFTYPE>
inline bool
isMapEmpty(const CKalmanFilterCapable<VEH_SIZE, OBS_SIZE, 0 /*FEAT_SIZE*/,
                                      ACT_SIZE, KFTYPE> &obj) {
  return true;
}
