...
struct MapExecutor {
  ...
  template <typename OP> static void run(const CMultiMetricMap &_mmm, OP &op) {
    MRPT_START
    CMultiMetricMap &mmm =
...
struct MapVectorClearer {
  template <typename T> inline void operator()(T &container) {
    container.clear();
  }
};
...
struct MapComputeLikelihood {
  ...
  MapComputeLikelihood(const CMultiMetricMap &m, const CObservation *_obs,
                       const CPose3D &_takenFrom, double &_total_log_lik)
      : obs(_obs), takenFrom(_takenFrom), total_log_lik(_total_log_lik) {
    total_log_lik = 0;
  }

  template <typename PTR> inline void operator()(PTR &ptr) {
...
struct MapCanComputeLikelihood {
  ...
  MapCanComputeLikelihood(const CMultiMetricMap &m, const CObservation *_obs,
                          bool &_can)
      : obs(_obs), can(_can) {
    can = false;
  }
...
struct MapInsertObservation {
  ...
  MapInsertObservation(const CMultiMetricMap &m, const CObservation *_obs,
...
void CMultiMetricMap::setListOfMaps(
  ...
  if (initializers != nullptr) {
    ...
    for (TSetOfMetricMapInitializers::const_iterator it = initializers->begin();
...
Insert the observation information into this map.
 ---------------------------------------------------------------*/
bool CMultiMetricMap::internal_insertObservation(const CObservation *obs,
...
void CMultiMetricMap::determineMatching2D(
  ...
  MRPT_START
  ASSERTMSG_(m_pointsMaps.size() == 1,
             "There is not exactly 1 points maps in the multimetric map.");
  m_pointsMaps[0]->determineMatching2D(otherMap, otherMapPose, correspondences,
                                       params, extraResults);
  MRPT_END
}
...
void CMultiMetricMap::saveMetricMapRepresentationToFile(
  ...
  for (size_t idx = 0; idx < maps.size(); idx++) {
    ...
    fil += format("_%s_%02u", m->GetRuntimeClass()->className,
