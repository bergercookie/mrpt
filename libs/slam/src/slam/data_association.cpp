...
struct TAuxDataRecursiveJCBB {
  size_t nPredictions, nObservations,
      length_O; //!< Just to avoid recomputing them all the time.
...
double joint_pdf_metric(const CMatrixTemplateNumeric<T> &Z_observations_mean,
  ...
  // ----------------------------------------------------------------------
  // Mean:
  // The same for the vector of "errors" or "innovation" between predictions and
  // observations:
  // ----------------------------------------------------------------------
  Eigen::Matrix<T, Eigen::Dynamic, 1> innovations(N * info.length_O);
  T *dst_ptr = &innovations[0];
  for (map<size_t, size_t>::const_iterator it = info.currentAssociation.begin();
       it != info.currentAssociation.end(); ++it) {
...
template <> bool isCloser<metricMaha>(const double v1, const double v2) {
  return v1 < v2;
}

template <> bool isCloser<metricML>(const double v1, const double v2) {
  return v1 > v2;
}
...
void JCBB_recursive(
  ...
  if (curObsIdx >= info.nObservations) {
    if (info.currentAssociation.size() > results.associations.size()) {
      ...
      results.distance =
          joint_pdf_metric<T, METRIC>(Z_observations_mean, Y_predictions_mean,
                                      Y_predictions_cov, info, results);
    } else if (!info.currentAssociation.empty() &&
               info.currentAssociation.size() == results.associations.size()) {
      // The same # of features matched than the previous best one... decide by
      // better distance:
      const double d2 =
          joint_pdf_metric<T, METRIC>(Z_observations_mean, Y_predictions_mean,
                                      Y_predictions_cov, info, results);
  ...
  } else // A normal iteration:
  {
      ...
      if ((info.currentAssociation.size() + potentials) >=
        ...
        if (results.indiv_compatibility(predIdx, obsIdx)) {
          ...
          if (!already_asigned) {
            ...
            JCBB_recursive<T, METRIC>(Z_observations_mean, Y_predictions_mean,
    ...
    if ((info.currentAssociation.size() + potentials) >=
      ...
      results.nNodesExploredInJCBB++;
      JCBB_recursive<T, METRIC>(Z_observations_mean, Y_predictions_mean,
                                Y_predictions_cov, results, info,
                                curObsIdx + 1);
    }
  }
}
...
void mrpt::slam::data_association_full_covariance(
  ...
  std::vector<double> kd_result_distances(DAT_ASOC_USE_KDTREE ? N_KD_RESULTS
                                                              : 0);
  ...
  results.indiv_distances.fill(metric == metricMaha
  ...
  for (size_t j = 0; j < nObservations; ++j) {
    if (!DAT_ASOC_USE_KDTREE) {
      ...
      for (size_t i = 0; i < nPredictions; ++i) {
        ...
        Y_predictions_cov.extractMatrix(pred_cov_idx, pred_cov_idx, length_O,
                                        length_O, pred_i_cov);
        ...
        // d2,ml);
        mrpt::math::mahalanobisDistance2AndLogPDF(diff_means_i_j, pred_i_cov,
                                                  d2, ml);
      ...
      kd_tree->query(&kd_queryPoint[0], N_KD_RESULTS, &kd_result_indices[0],
                     &kd_result_distances[0]);

      // Only compute the distances for these ones:
      for (size_t w = 0; w < N_KD_RESULTS; w++) {
        const size_t i = kd_result_indices[w]; // This is the index of the
                                               // prediction in
                                               // "predictions_mean"
        ...
        if (d2 > 6 * chi2thres)
          break; // Since kd-tree returns the landmarks by distance order, we
                 // can skip the rest
  ...
  case assocNN: {
    ...
    // -------------------------------------------------------------------
    typedef multimap<
        double, pair<observation_index_t, multimap<double, prediction_index_t>>>
        TListAllICs;
  ...
  case assocJCBB: {
    ...
    if (metric == metricMaha)
      JCBB_recursive<CMatrixDouble::Scalar, metricMaha>(
          Z_observations_mean, Y_predictions_mean, Y_predictions_cov, results,
          info, 0);
    else
...
void mrpt::slam::data_association_independent_predictions(
  ...
  CMatrixDouble Y_predictions_cov_full(length_O * nPredictions,
                                       length_O * nPredictions);
  CMatrixDouble COV_i(length_O, length_O);
  for (size_t i = 0; i < nPredictions; i++) {
    const size_t idx = i * length_O;
    Y_predictions_cov_stacked.extractSubmatrix(idx, idx + length_O - 1, 0,
                                               length_O - 1, COV_i);
    Y_predictions_cov_full.insertMatrix(idx, idx, COV_i);
  }

  data_association_full_covariance(
      Z_observations_mean, Y_predictions_mean, Y_predictions_cov_full, results,
      method, metric, chi2quantile, DAT_ASOC_USE_KDTREE, predictions_IDs,
