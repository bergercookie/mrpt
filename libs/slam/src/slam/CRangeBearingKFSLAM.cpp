...
CRangeBearingKFSLAM::CRangeBearingKFSLAM()
    : options(), m_action(), m_SF(), m_IDs(), mapPartitioner(), m_SFs(),
      m_lastPartitionSet() {
  reset();
}
...
void CRangeBearingKFSLAM::reset() {
  ...
  // Use SF-based matching (faster & easier for bearing-range observations with
  // ID).
 ...
 * filter.
 */
void CRangeBearingKFSLAM::OnTransitionModel(const KFArray_ACT &u,
                                            KFArray_VEH &xv,
...
void CRangeBearingKFSLAM::OnTransitionJacobian(KFMatrix_VxV &F) const {
  ...
  CPose3DQuatPDF::jacobiansPoseComposition(robotPose, // x
                                           theIncr,   // u
                                           F,         // df_dx,
                                           df_du);
...
void CRangeBearingKFSLAM::OnObservationModel(
  ...
  ASSERTMSG_(obs, "*ERROR*: This method requires an observation of type "
                  "CObservationBearingRange")
  const CPose3DQuat sensorPoseOnRobot = CPose3DQuat(obs->sensorLocationOnRobot);

  /* -------------------------------------------
 Equations, obtained using matlab, of the relative 3D position of a landmark
 (xi,yi,zi), relative
  ...
  Hy : dh_dyi   -> Jacobian of the observation model wrt each landmark mean
 position
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    const TPoint3D mapEst(m_xkk[vehicle_size + row_in + 0],
                          m_xkk[vehicle_size + row_in + 1],
                          m_xkk[vehicle_size + row_in + 2]);

    // Generate Range, yaw, pitch
    // ---------------------------------------------------
    sensorPoseAbs.sphericalCoordinates(mapEst,
...
void CRangeBearingKFSLAM::OnGetObservationsAndDataAssociation(
  ...
  size_t row;
  for (row = 0, itObs = obs->sensedData.begin(); itObs != obs->sensedData.end();
       ++itObs, ++row) {
    // Fill one row in Z:
    Z[row][0] = itObs->range;
    Z[row][1] = itObs->yaw;
    ...
    for (row = 0, itObs = obs->sensedData.begin(),
      ...
      if (itObs->landmarkID < 0)
        ...
        if ((itID = m_IDs.find_key(itObs->landmarkID)) != m_IDs.end())
          *itDA = itID->second; // This row in Z corresponds to the i'th map
  ...
  if (obs_idxs_needing_data_assoc.empty()) {
    ...
    if (nPredictions) {
      ...
      mrpt::slam::data_association_full_covariance(
          ...
          m_last_data_association.Y_pred_covs, m_last_data_association.results,
          options.data_assoc_method, options.data_assoc_metric,
          options.data_assoc_IC_chi2_thres,
          true, // Use KD-tree
          m_last_data_association.predictions_IDs, options.data_assoc_IC_metric,
...
void CRangeBearingKFSLAM::OnNormalizeStateVector() {
  ...
  const double T = std::sqrt(square(m_xkk[3]) + square(m_xkk[4]) +
                             square(m_xkk[5]) + square(m_xkk[6]));
...
void CRangeBearingKFSLAM::TOptions::loadFromConfigFile(
  ...
  std_sensor_yaw = DEG2RAD(source.read_float(section, "std_sensor_yaw_deg",
                                             RAD2DEG(std_sensor_yaw)));
  std_sensor_pitch = DEG2RAD(source.read_float(section, "std_sensor_pitch_deg",
                                               RAD2DEG(std_sensor_pitch)));

  std_odo_z_additional =
      source.read_float(section, "std_odo_z_additional", std_odo_z_additional);
...
CRangeBearingKFSLAM::TOptions::TOptions()
    : stds_Q_no_odo(get_vehicle_size(), 0), std_sensor_range(0.01f),
      std_sensor_yaw(DEG2RAD(0.2f)), std_sensor_pitch(DEG2RAD(0.2f)),
      std_odo_z_additional(0), doPartitioningExperiment(false),
      quantiles_3D_representation(3), partitioningMethod(0),
      data_assoc_method(assocNN), data_assoc_metric(metricMaha),
      data_assoc_IC_chi2_thres(0.99), data_assoc_IC_metric(metricMaha),
      data_assoc_IC_ml_threshold(0.0), create_simplemap(false),
...
void CRangeBearingKFSLAM::OnInverseObservationModel(
  ...
  CPose3DQuatPDF::jacobiansPoseComposition(
      robotPose, sensorPoseOnRobot, dsensorabs_dvehpose, dsensorabs_dsenrelpose,
      &sensorPoseAbs);
  ...
  const TPoint3D yn_rel_sensor(hn_r * chn_y * chn_p, hn_r * shn_y * chn_p,
                               -hn_r * shn_p);
  ...
  */
  ...
  sensorPoseAbs.composePoint(yn_rel_sensor.x, yn_rel_sensor.y,
  ...
  // dyn_dxv =
  dyn_dxv.multiply_AB(jacob_dyn_dsensorabs,
                      dsensorabs_dvehpose); // dsensorabs_dsenrelpose);
...
void CRangeBearingKFSLAM::getAs3DObject(
  ...
  for (size_t i = 0; i < nLMs; i++) {
    ...
    m_pkk.extractMatrix(get_vehicle_size() + get_feature_size() * i,
                        get_vehicle_size() + get_feature_size() * i,
                        get_feature_size(), get_feature_size(), COV);
    pointGauss.cov = COV;
...
double CRangeBearingKFSLAM::computeOffDiagonalBlocksApproximationError(
  ...
  ASSERT_((get_vehicle_size() + nLMs * get_feature_size()) ==
          fullCov.getColCount());
  ...
  for (i = 0; i < nLMs; i++) {
    for (size_t j = i + 1; j < nLMs; j++) {
      ...
      if (0 == math::countCommonElements(landmarksMembership[i],
                                         landmarksMembership[j])) {
  ...
  return sumOffBlocks /
         H.block(get_vehicle_size(), get_vehicle_size(),
...
void CRangeBearingKFSLAM::reconsiderPartitionsNow() {
  ...
  vector<vector_uint>
      partitions; // A different buffer for making this thread-safe some day...
...
void CRangeBearingKFSLAM::saveMapAndPath2DRepresentationAsMATLABFile(
  ...
  for (size_t i = 0; i < nLMs; i++) {
    ...
    os::fprintf(f, "%s", math::MATLAB_plotCovariance2D(cov, mean, stdCount,
                                                       styleLandmarks)
                             .c_str());
  }
