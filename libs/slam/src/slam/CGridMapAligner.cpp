...
* \param m2			[IN] The second map. The pose of this map
respect to m1 is to be estimated.
...
* \param info			[OUT] See derived classes for details, or
nullptr if it isn't needed.
*
* \sa CPointsMapAlignmentAlgorithm
  ---------------------------------------------------------------*/
CPosePDF::Ptr
CGridMapAligner::AlignPDF(const mrpt::maps::CMetricMap *mm1,
                          const mrpt::maps::CMetricMap *mm2,
                          const CPosePDFGaussian &initialEstimationPDF,
                          float *runningTime, void *info) {
  MRPT_START

  switch (options.methodSelection) {
  case CGridMapAligner::amCorrelation:
    return AlignPDF_correlation(mm1, mm2, initialEstimationPDF, runningTime,
                                info);
  ...
  case CGridMapAligner::amRobustMatch:
    ...
    return AlignPDF_robustMatch(mm1, mm2, initialEstimationPDF, runningTime,
                                info);
...
CPosePDF::Ptr CGridMapAligner::AlignPDF_robustMatch(
  ...
  ASSERT_(options.methodSelection == CGridMapAligner::amRobustMatch ||
  ...
  } else if (IS_CLASS(mm1, COccupancyGridMap2D) &&
  ...
  m_grid_feat_extr.extractFeatures(*m1, *lm1, N1, options.feature_descriptor,
                                   options.feature_detector_options);
  m_grid_feat_extr.extractFeatures(*m2, *lm2, N2, options.feature_descriptor,
                                   options.feature_detector_options);
  ...
  if (nLM1 < 2 || nLM2 < 2) {
    ...
    if (options.debug_show_corrs) {
      // auxWin = CDisplayWindowPlots::Ptr( new CDisplayWindowPlots("Individual
      // corr.") );
      std::cerr << "Warning: options.debug_show_corrs has no effect since MRPT "
                   "0.9.1\n";
    }

    for (size_t idx1 = 0; idx1 < nLM1; idx1++) {
      // CVectorFloat  	corrs_indiv;
      vector<pair<size_t, float>> corrs_indiv; // (index, distance); Index is
      ...
      for (size_t idx2 = 0; idx2 < nLM2; idx2++) {
        float minDist;
        minDist = lm1->landmarks.get(idx1)->features[0]->descriptorDistanceTo(
      ...
      for (size_t w = 0; w < nBestToKeep; w++) {
        if (corrs_indiv[w].second <= thres_max &&
          ...
          outInfo.correspondences_dists_maha.push_back(
              TReturnInfo::TPairPlusDistance(idx1, corrs_indiv[w].first,
                                             corrs_indiv[w].second));
        }
      }
    ...
    if (options.save_feat_coors) {
      ...
      for (std::map<size_t, std::set<size_t>>::iterator it =
        ...
        lm1->landmarks.get(it->first)->features[0]->getFirstDescriptorAsMatrix(
            descriptor1);
        ...
        img_compose.filledRectangle(0, 0, img_compose.getWidth() - 1,
        ...
        for (j = 0, it_j = it->second.begin(); j < nF; ++j, ++it_j) {
          ...
          lm2->landmarks.get(*it_j)->features[0]->getFirstDescriptorAsMatrix(
              descriptor2);
    ...
    if (!mrpt::tfest::se2_l2(correspondences, noRobustEst)) {
      ...
      MRPT_LOG_INFO(mrpt::format("[CGridMapAligner] Overall estimation(%u "
                                 "corrs, total: %u): (%.03f,%.03f,%.03fdeg)\n",
      ...
      if (options.methodSelection == CGridMapAligner::amRobustMatch) {
        ...
        mrpt::tfest::se2_l2_robust(correspondences, options.ransac_SOG_sigma_m,
                                   tfest_params, tfest_result);
        ...
        *pdf_SOG = tfest_result.transformation;
        ...
        unsigned int ransac_nSimulations = 10; // It doesn't matter actually,
                                               // since will be changed in the
                                               // first loop
        ...
        const unsigned int max_trials =
            (nCorrs * (nCorrs - 1) / 2) * 5; // "*5" is just for safety...

        unsigned int iter =
            0; // Valid iterations (those passing the first mahalanobis test)
        unsigned int trials =
            0; // counter of all iterations, including "iter" + failing ones.
        while (iter < ransac_nSimulations &&
               trials < max_trials) // ransac_nSimulations can be dynamic
        {
          trials++;
          ...
          for (TMapMatchingsToPoseMode::iterator itOldHyps = sog_modes.begin();
               itOldHyps != sog_modes.end(); ++itOldHyps) {
            if (itOldHyps->first.contains(all_corrs[idx1]) &&
                itOldHyps->first.contains(all_corrs[idx2])) {
              // Increment weight:
              itOldHyps->second.log_w =
                  std::log(std::exp(itOldHyps->second.log_w) + 1.0);
              is_new_hyp = false;
              break;
            }
          }
          ...
          // Maintain a list of already used landmarks IDs in both maps to avoid
          // repetitions:
          ...
          do // Incremently incorporate inliers:
          {
            ...
            // The computed cov is "normalized", i.e. must be multiplied by
            // std^2_xy
...
//#define SHOW_CORRS
            ...
            for (size_t j = 0; j < nLM2; j++) {
              ...
              pdf_M2_j.cov.get_unsafe(0, 0) = pdf_M2_j.cov.get_unsafe(1, 1) =
                  square(options.ransac_SOG_sigma_m);

#ifdef SHOW_CORRS
              win.plotEllipse(pdf_M2_j.mean.x(), pdf_M2_j.mean.y(),
                              pdf_M2_j.cov, 2, "b-",
              ...
              lm1_pnts.kdTreeNClosestPoint2DIdx(
                  pdf_M2_j.mean.x(), pdf_M2_j.mean.y(), N_KDTREE_SEARCHED,
                  matches_idx, matches_dist);

              // And for each one, compute the product-integral:
              for (size_t u = 0; u < matches_idx.size(); u++) {
                if (used_landmarks1[matches_idx[u]])
                  continue;
...
// And now compute the product integral:
            ...
            if (best_pair_d2 < chi2_thres_dim2) {
              ...
              // if (feat_dist< options.threshold_max)
              {
                ...
                lm2_pnts.getPoint(best_pair_ij.second, p2_j_localx,
                ...
                tentativeSubSet.push_back(mrpt::utils::TMatchingPair(
                    best_pair_ij.first, best_pair_ij.second, p1_i_localx,
                    p1_i_localy, 0,             // MAP1
          ...
          if (largestConsensusCorrs.size() < ninliers) {
            ...
            const double fracinliers =
                ninliers / static_cast<double>(std::min(nLM1, nLM2));
            double pNoOutliers =
                1 - pow(fracinliers,
            ...
            // Number of
            ransac_nSimulations =
                log(1 - probability_find_good_model) / log(pNoOutliers);
            ...
            //ransac_nSimulations << "  pNoOutliers=" << pNoOutliers << "
        ...
        pdf_SOG->clear();
        for (TMapMatchingsToPoseMode::const_iterator s = sog_modes.begin();
             s != sog_modes.end(); ++s) {
        ...
        MRPT_LOG_INFO(mrpt::format("[CGridMapAligner] amModifiedRANSAC: %u SOG "
                                   "modes merged to %u in %.03fsec\n",
                                   (unsigned int)nB, (unsigned int)nA,
                                   merge_clock_tim));
      ...
      if (options.debug_save_map_pairs) {
        ...
        for (TMapMatchingsToPoseMode::const_iterator s = sog_modes.begin();
          ...
          COccupancyGridMap2D::saveAsEMFTwoMapsWithCorrespondences(
              format("__debug_corrsGrid_%05u.emf", NN), m1, m2, s->first);
      ...
      if (multimap1 && multimap2 && !multimap1->m_pointsMaps.empty() &&
          !multimap2->m_pointsMaps.empty() && multimap1->m_pointsMaps[0] &&
          multimap2->m_pointsMaps[0]) {
        ...
        for (CPosePDFSOG::iterator i = pdf_SOG->begin(); i != pdf_SOG->end();
             ++cnt) {
          CPosePDF::Ptr icp_est =
              icp.Align(pnts1.get(), pnts2.get(), (i)->mean, nullptr, &icpInfo);
        ...
        pdf_SOG->mergeModes(options.maxKLd_for_merge, false);
        MRPT_LOG_DEBUG_STREAM("[CGridMapAligner] "
                              << pdf_SOG->size()
...
CPosePDF::Ptr CGridMapAligner::AlignPDF_correlation(
  ...
  map2_mod.setSize(m1->getXMin(), m1->getXMax(), m1->getYMin(), m1->getYMax(),
                   m1->getResolution());
  ...
  for (phi = phiMin; phi < phiMax; phi += phiResolution) {
    ...
    // --------------------------------------------------------
    CPose2D v2(pivotPt_x - cos(phi) * map2width_2,
    ...
    map1_img.cross_correlation_FFT(map2_img, outCrossCorr, -1, -1, -1, -1,
...
CGridMapAligner::TConfigParams::TConfigParams()
    : methodSelection(CGridMapAligner::amModifiedRANSAC),
      ...
      feature_descriptor(descPolarImages), feature_detector_options(),

      ransac_minSetSizeRatio(0.20f), ransac_SOG_sigma_m(0.10f),
      ransac_mahalanobisDistanceThreshold(6.0f), ransac_chi2_quantile(0.99),
      ransac_prob_good_inliers(0.9999), featsPerSquareMeter(0.015f),
      threshold_max(0.15f), threshold_delta(0.10f), min_ICP_goodness(0.30f),
      max_ICP_mahadist(10.0), maxKLd_for_merge(0.9),

      save_feat_coors(false), debug_show_corrs(false),
...
void CGridMapAligner::TConfigParams::loadFromConfigFile(
  ...
  MRPT_LOAD_CONFIG_VAR_CAST(methodSelection, int, TAlignerMethod, iniFile,
                            section)
  ...
  MRPT_LOAD_CONFIG_VAR_NO_DEFAULT(ransac_minSetSizeRatio, float, iniFile,
                                  section)
  MRPT_LOAD_CONFIG_VAR_NO_DEFAULT(ransac_mahalanobisDistanceThreshold, float,
                                  iniFile, section)
  MRPT_LOAD_CONFIG_VAR_NO_DEFAULT(ransac_chi2_quantile, double, iniFile,
                                  section)
  MRPT_LOAD_CONFIG_VAR_NO_DEFAULT(ransac_prob_good_inliers, double, iniFile,
                                  section)
  ...
  MRPT_LOAD_CONFIG_VAR_CAST_NO_DEFAULT(feature_descriptor, int, TDescriptorType,
                                       iniFile, section)
  feature_detector_options.loadFromConfigFile(iniFile, section);
}

CPose3DPDF::Ptr
CGridMapAligner::Align3DPDF(const mrpt::maps::CMetricMap *m1,
