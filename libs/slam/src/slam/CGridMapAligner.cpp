	...
	void* info)
{
	...
	else
	{
		...
		if (options.debug_show_corrs)
		{
			...
			// CDisplayWindowPlots("Individual
			// corr.") );
			std::cerr
				<< "Warning: options.debug_show_corrs has no effect since MRPT "
				   "0.9.1\n";
		}
		...
		for (size_t idx1 = 0; idx1 < nLM1; idx1++)
		{
			...
			vector<pair<size_t, float>>
				corrs_indiv;  // (index, distance); Index is
