...
CRejectionSamplingRangeOnlyLocalization::
    CRejectionSamplingRangeOnlyLocalization()
    : m_z_robot(0), m_sigmaRanges(0.10f), m_oldPose(), m_drawIndex(0),
      m_dataPerBeacon() {}
...
void CRejectionSamplingRangeOnlyLocalization::RS_drawFromProposal(
  ...
  if (m_dataPerBeacon.size() == 0)
    THROW_EXCEPTION("There is no information from which to draw samples!! Use "
                    "'setParams' with valid data!");

  ASSERT_(m_drawIndex < m_dataPerBeacon.size());

  float ang =
      randomGenerator.drawUniform(m_dataPerBeacon[m_drawIndex].minAngle,
  ...
  mrpt::math::TPoint2D on(m_dataPerBeacon[m_drawIndex].sensorOnRobot.x,
...
double CRejectionSamplingRangeOnlyLocalization::RS_observationLikelihood(
  ...
  for (size_t i = 0; i < m_dataPerBeacon.size(); i++) {
    ...
    CPoint3D P(x + CPoint3D(m_dataPerBeacon[i].sensorOnRobot.x,
                            m_dataPerBeacon[i].sensorOnRobot.y, 0));

    if (i != m_drawIndex)
      // Evalute:
      lik *=
          exp(-0.5 * square(m_dataPerBeacon[i].radiusAtRobotPlane -
...

{
  ...
  // Minimum radius:
  std::deque<mrpt::obs::CObservationBeaconRanges::TMeasurement>::const_iterator
      it;
  size_t i;
  ...
  for (i = 0, it = observation.sensedData.begin();
    ...
    if (lm != nullptr) {
      ...
      if (data.radiusAtRobotPlane > 0) {
        ...
        // Keep max/min:
        xMin = min(xMin, data.beaconPosition.x - (data.radiusAtRobotPlane +
                                                  4 * m_sigmaRanges + 1.0));
        xMax = max(xMax, data.beaconPosition.x + (data.radiusAtRobotPlane +
                                                  4 * m_sigmaRanges + 1.0));
        yMin = min(yMin, data.beaconPosition.y - (data.radiusAtRobotPlane +
                                                  4 * m_sigmaRanges + 1.0));
        yMax = max(yMax, data.beaconPosition.y + (data.radiusAtRobotPlane +
                                                  4 * m_sigmaRanges + 1.0));
      } else {
        printf("\nWARNING: Beacon range is shorter than distance between the "
               "robot and the beacon in the Z axis!!!: Ignoring this "
               "measurement\n");
      }
    }
  } // end for
  ...
  if (autoCheckAngleRanges && m_dataPerBeacon.size() > 1) {
    // Build the grid:
    // Each cell in the grid is a vector of bools, each one indicating whether
    // some samples from the i'th beacon falls there.
    ...
    for (i = 0; i < m_dataPerBeacon.size(); i++) {
      for (float a = -M_PIf; a <= M_PIf; a += Aa) {
        ...
        R = max(R, 0.0f);
        cell = grid.cellByPos(m_dataPerBeacon[i].beaconPosition.x + cos(a) * R,
