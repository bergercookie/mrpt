 ...
 * filter.
 */
void CRangeBearingKFSLAM2D::OnTransitionModel(const KFArray_ACT &u,
                                              KFArray_VEH &xv,
...
void CRangeBearingKFSLAM2D::OnObservationModel(
  ...
  /* -------------------------------------------
 Equations, obtained using matlab, of the relative 2D position of a landmark
 (xi,yi), relative
...
void CRangeBearingKFSLAM2D::OnObservationJacobians(
 ...
 position
  ...
  // ---------------------------------------------------
  const kftype EXP1 =
      -2 * yi * y0s * cphi0 - 2 * yi * y0 + 2 * xi * y0s * sphi0 - 2 * xi * x0 -
      2 * xi * x0s * cphi0 - 2 * yi * x0s * sphi0 + 2 * y0s * y0 * cphi0 -
      2 * y0s * x0 * sphi0 + 2 * y0 * x0s * sphi0 + square(x0) +
      2 * x0s * x0 * cphi0 + square(x0s) + square(y0s) + square(xi) +
      square(yi) + square(y0);
...
void CRangeBearingKFSLAM2D::OnGetObservationsAndDataAssociation(
  ...
  if (obs_idxs_needing_data_assoc.empty()) {
...
#if STATS_EXPERIMENT // DEBUG: Generate statistic info
    {
      ...
      vector_int::iterator itDA;
      size_t idx_obs = 0;
      for (itDA = data_association.begin(); itDA != data_association.end();
           ++itDA, ++idx_obs) {
        if (*itDA == -1)
          continue;
...
CRangeBearingKFSLAM2D::TOptions::TOptions()
    : stds_Q_no_odo(3, 0), std_sensor_range(0.1f),
      std_sensor_yaw(DEG2RAD(0.5f)), quantiles_3D_representation(3),
      create_simplemap(false), data_assoc_method(assocNN),
      data_assoc_metric(metricMaha), data_assoc_IC_chi2_thres(0.99),
      data_assoc_IC_metric(metricMaha), data_assoc_IC_ml_threshold(0.0)
...
void CRangeBearingKFSLAM2D::OnPreComputingPredictions(
  ...
  for (size_t i = 0; i < prediction_means.size(); i++)
                        ...
                        // predictions are out of range, so just generate all of
                        // them:
