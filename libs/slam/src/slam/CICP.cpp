		...
		if (!options.skip_cov_calculation && nCorrespondences)
		{
			switch (options.ICP_covariance_method)
			{
				...
				case icpCovFiniteDifferences:
				{
					...
					// not singular,
					// while not changing its covariance
					// significantly.
	...
	const CPosePDFGaussian& initialEstimationPDF, TReturnInfo& outInfo)
{
	...
	CPose2D lastMeanPose;
	std::vector<float> other_xs_trans,
		other_ys_trans;  // temporary container of
	// "other" map (map2)
	// transformed by "q"
	...
	if (!m2->isEmpty())
	{
		...
		do
		{
			...
			else
			{
				...
				// ----------------------------------------------------------------------
				dJ_dq.setSize(
					3, nCorrespondences);  // The jacobian of the error
				// function wrt the transformation
				// q
