...
<code>CPose2D(0,0,0)</code> for example.
* \param pdf			[IN/OUT] A pointer to a CPosePDF pointer,
initially set to nullptr for this method to create the object. For greater
efficiency, this object can be left undeleted and passed again to the method.
When <b>not used anymore remember to delete it</b> using <code>delete
pdf;</code>
...
CPosePDF::Ptr CICP::AlignPDF(const mrpt::maps::CMetricMap *m1,
  ...
  case icpLevenbergMarquardt:
    ...
    break;
  default:
    THROW_EXCEPTION_FMT("Invalid value for ICP_algorithm: %i",
...
CICP::TConfigParams::TConfigParams()
    : ICP_algorithm(icpClassic), ICP_covariance_method(icpCovFiniteDifferences),

      onlyClosestCorrespondences(true), onlyUniqueRobust(false),
      maxIterations(40), minAbsStep_trans(1e-6f), minAbsStep_rot(1e-6f),
      thresholdDist(0.75f), thresholdAng(DEG2RAD(0.15f)), ALFA(0.50f),
      smallestThresholdDist(0.10f), covariance_varPoints(square(0.02f)),
      doRANSAC(false),

      ransac_minSetSize(3), ransac_maxSetSize(20), ransac_nSimulations(100),
      ransac_mahalanobisDistanceThreshold(3.0f), normalizationStd(0.02f),
      ransac_fuseByCorrsMatch(true), ransac_fuseMaxDiffXY(0.01f),
      ransac_fuseMaxDiffPhi(DEG2RAD(0.1f)),

      kernel_rho(0.07f), use_kernel(true), Axy_aprox_derivatives(0.05f),

      LM_initial_lambda(1e-4f),

      skip_cov_calculation(false), skip_quality_calculation(true),
...
void CICP::TConfigParams::loadFromConfigFile(
  ...
  ICP_algorithm =
      iniFile.read_enum<TICPAlgorithm>(section, "ICP_algorithm", ICP_algorithm);
  ...
  thresholdAng = DEG2RAD(iniFile.read_float(section.c_str(), "thresholdAng_DEG",
  ...
  MRPT_LOAD_CONFIG_VAR(ransac_mahalanobisDistanceThreshold, float, iniFile,
                       section);
  ...
  ransac_fuseMaxDiffPhi =
      DEG2RAD(iniFile.read_float(section.c_str(), "ransac_fuseMaxDiffPhi_DEG",
...
CPosePDF::Ptr CICP::ICP_Method_Classic(
  ...
  if (!m2->isEmpty()) {
    ...
    do {
      ...
      matchParams.angularDistPivotPoint =
          TPoint3D(gaussPdf->mean.x(), gaussPdf->mean.y(),
                   0); // Pivot point for angular measurements

      m1->determineMatching2D(m2,             // The other map
    ...
    if (!options.skip_cov_calculation && nCorrespondences) {
      ...
      case icpCovFiniteDifferences: {
        ...
        size_t i;
        for (i = 0, it = correspondences.begin(); i < nCorrespondences;
             ++i, ++it) {
          float other_x_trans =
          ...
          q1 = kernel(square(it->this_x - w1) +
                          square(it->this_y - other_y_trans),
                      rho2);

          w2 = other_x_trans;
          q2 = kernel(square(it->this_x - w2) +
                          square(it->this_y - other_y_trans),
                      rho2);

          w3 = other_x_trans + Axy;
          q3 = kernel(square(it->this_x - w3) +
                          square(it->this_y - other_y_trans),
                      rho2);
          ...
          q1 = kernel(square(it->this_x - other_x_trans) +
                          square(it->this_y - w1),
                      rho2);

          w2 = other_y_trans;
          q2 = kernel(square(it->this_x - other_x_trans) +
                          square(it->this_y - w2),
                      rho2);

          w3 = other_y_trans + Axy;
          q3 = kernel(square(it->this_x - other_x_trans) +
                          square(it->this_y - w3),
                      rho2);
        ...
        for (i = 0; i < 3; i++)
          DDt(i, i) += 1e-6; // Just to make sure the matrix is not singular,
                             // while not changing its covariance significantly.

        DDt.inv(gaussPdf->cov);
      } break;
      default:
        THROW_EXCEPTION_FMT("Invalid value for ICP_covariance_method: %i",
    ...
    if (!nCorrespondences || options.skip_quality_calculation) {
      ...
      // Compute a crude estimate of the quality of scan matching at this local
      // minimum:
      // -----------------------------------------------------------------------------------
      ...
      outInfo.quality =
          -max(EX1 - E0, max(EX2 - E0, max(EY1 - E0, EY2 - E0))) / (E0 + 1e-1);
    }
  ...
  if (options.doRANSAC) {
    ...
    mrpt::tfest::se2_l2_robust(correspondences, options.normalizationStd,
                               params, results);
...
/*----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------*/
CPosePDF::Ptr CICP::ICP_Method_LM(const mrpt::maps::CMetricMap *mm1,
  ...
  CPose2D lastMeanPose;
  std::vector<float> other_xs_trans, other_ys_trans; // temporary container of
                                                     // "other" map (map2)
                                                     // transformed by "q"
  ...
  if (!m2->isEmpty()) {
    ...
    do {
      ...
      if (!nCorrespondences) {
        ...
        // Levenberg-Marquardt
        // ----------------------------------------------------------------------
        dJ_dq.setSize(3, nCorrespondences); // The jacobian of the error
                                            // function wrt the transformation q
        ...
        double A, B;
        const double Axy =
            options.Axy_aprox_derivatives; // For approximating the derivatives

        // Compute at once the square errors for each point with the current "q"
        // and the transformed points:
        std::vector<float> sq_errors;
        correspondences.squareErrorVector(q, sq_errors, other_xs_trans,
                                          other_ys_trans);
...
//#define ICP_DISTANCES_TO_LINE
          ...
          m1->kdTreeTwoClosestPoint2D(*other_x_trans,
                                      *other_y_trans, // The query
                                      x1, y1,         // Closest point #1
                                      x2, y2,         // Closest point #2
                                      d1, d2);

          w1 = *other_x_trans - Axy;
          q1 = math::closestSquareDistanceFromPointToLine(w1, *other_y_trans,
                                                          x1, y1, x2, y2);
          q1 = kernel(q1, rho2);

          w2 = *other_x_trans;
          q2 = math::closestSquareDistanceFromPointToLine(w2, *other_y_trans,
                                                          x1, y1, x2, y2);
          q2 = kernel(q2, rho2);

          w3 = *other_x_trans + Axy;
          q3 = math::closestSquareDistanceFromPointToLine(w3, *other_y_trans,
                                                          x1, y1, x2, y2);
          q3 = kernel(q3, rho2);
#endif
          // interpolate
          ...
          q1 = math::closestSquareDistanceFromPointToLine(*other_x_trans, w1,
                                                          x1, y1, x2, y2);
          q1 = kernel(q1, rho2);
#else
          q1 = kernel(square(it->this_x - *other_x_trans) +
          ...
          // );
          // q2= kernel( square(it->this_x - *other_x_trans)+ square( it->this_y
          // - w2 ),  rho2 );

          w3 = *other_y_trans + Axy;
#ifdef ICP_DISTANCES_TO_LINE
          q3 = math::closestSquareDistanceFromPointToLine(*other_x_trans, w3,
                                                          x1, y1, x2, y2);
          q3 = kernel(q3, rho2);
#else
          q3 = kernel(square(it->this_x - *other_x_trans) +
        ...
        while (keepIteratingLM && ++nLMiters < maxLMiters) {
          ...
          dJ_dq.multiply_Ab(
              Eigen::Map<Eigen::VectorXf>(&sq_errors[0], sq_errors.size()),
              dJsq);
          ...
          correspondences.squareErrorVector(
              q_new, new_sq_errors, new_other_xs_trans, new_other_ys_trans);
          ...
          keepIteratingLM = fabs(LM_delta[0]) > options.minAbsStep_trans ||
                            fabs(LM_delta[1]) > options.minAbsStep_trans ||
                            fabs(LM_delta[2]) > options.minAbsStep_rot;

          if (improved) {
            // If resids have gone down, keep change and make lambda smaller by
            // factor of 10
            lambda /= 10;
            q = q_new;
            OSE_initial = OSE_new;
...
CPose3DPDF::Ptr CICP::ICP3D_Method_Classic(
  ...
  if (!m2->isEmpty()) {
    ...
    do {
      matchParams.angularDistPivotPoint =
          TPoint3D(gaussPdf->mean.x(), gaussPdf->mean.y(), gaussPdf->mean.z());

      // ------------------------------------------------------
      //		Find the matching (for a points map)
      // ------------------------------------------------------
      m1->determineMatching3D(m2,             // The other map
                              gaussPdf->mean, // The other map pose
                              correspondences, matchParams, matchExtraResults);
      ...
      if (!nCorrespondences) {
        ...
        double transf_scale;
        mrpt::tfest::se3_l2(correspondences, estPoseQuat, transf_scale,
        ...
        if (!(fabs(lastMeanPose.x() - gaussPdf->mean.x()) >
              ...
              fabs(lastMeanPose.z() - gaussPdf->mean.z()) >
                  options.minAbsStep_trans ||
              fabs(math::wrapToPi(lastMeanPose.yaw() - gaussPdf->mean.yaw())) >
                  options.minAbsStep_rot ||
              fabs(math::wrapToPi(lastMeanPose.pitch() -
                                  gaussPdf->mean.pitch())) >
                  options.minAbsStep_rot ||
              fabs(
                  math::wrapToPi(lastMeanPose.roll() - gaussPdf->mean.roll())) >
                  options.minAbsStep_rot)) {
          matchParams.maxDistForCorrespondence *= options.ALFA;
          matchParams.maxAngularDistForCorrespondence *= options.ALFA;
          if (matchParams.maxDistForCorrespondence <
