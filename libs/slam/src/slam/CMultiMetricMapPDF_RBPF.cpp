...
void KLF_loadBinFromParticle(
  ...
  if (currentParticleValue != nullptr)
    ...
    {
      outBin.bins[i].x =
          round(currentParticleValue->robotPath[i].x / opts.KLD_binSize_XY);
      outBin.bins[i].y =
          round(currentParticleValue->robotPath[i].y / opts.KLD_binSize_XY);
      outBin.bins[i].phi =
          round(currentParticleValue->robotPath[i].yaw / opts.KLD_binSize_PHI);
    }
...
struct TAuxRangeMeasInfo {
  TAuxRangeMeasInfo()
      : sensorLocationOnRobot(), sensedDistance(0), beaconID(INVALID_BEACON_ID),
        nGaussiansInMap(0) {}
...
void CMultiMetricMapPDF::prediction_and_update_pfAuxiliaryPFStandard(
  ...
  PF_SLAM_implementation_pfAuxiliaryPFStandard<mrpt::slam::detail::TPoseBin2D>(
      actions, sf, PF_options, options.KLD_params);
...
void CMultiMetricMapPDF::prediction_and_update_pfOptimalProposal(
  ...
  //  Prediction:
  //
  //  Compute a new mean and covariance by sampling around the mean of the input
  //  "action"
  // --------------------------------------------------------------------------------------
  printf(" 1) Prediction...");
  M = m_particles.size();
  ...
  for (i = 0, partIt = m_particles.begin(); partIt != m_particles.end();
    ...
    if (options.pfOptimalProposal_mapSelection == 0 ||
      ...
      } else if (options.pfOptimalProposal_mapSelection == 3) // Map of points
      {
      ...
      // Use ICP to align to each particle's map:
      {
        CPosePDF::Ptr alignEst =
            icp.Align(map_to_align_to, &localMapPoints,
      ...
      if (i == particleWithHighestW) {
        newInfoIndex =
            1 - icpInfo.goodness; // newStaticPointsRatio; //* icpInfo.goodness;
      }
      ...
      if (icpInfo.goodness < options.ICPGlobalAlign_MinQuality && SFs.size()) {
        printf("[rbpf-slam] Warning: gridICP[%u]: %.02f%% -> Using odometry "
               "instead!\n",
...
//mrpt::math::wrapToPi( initialPoseEstimation.yaw()-icpEstimation.mean.phi() )
//);
...
//Iters=%u\n",icpInfo.goodness,icpInfo.nIterations);
      ...
      randomGenerator.drawGaussianMultivariate(rndSamples,
                                               finalEstimatedPoseGauss.cov);
      // Add noise:
      finalPose.setFromValues(finalPose.x() + rndSamples[0],
                              finalPose.y() + rndSamples[1], finalPose.z(),
      ...
      if (!robotActionSampler.isPrepared()) {
        ...
        // THROW_EXCEPTION("There is no odometry & the initial beacon map is
        // empty: RO-SLAM has no solution -> ABORTED!!!");

        if (!beacMap->size()) {
          // First iteration only...
          cerr << "[RO-SLAM] Optimal filtering without map & odometry...this "
                  "message should appear only the first iteration!!"
               << endl;
        } else {
          // To make RO-SLAM to have a solution, arbitrarily fix one of the
          // beacons so
      ...
      for (CSensoryFrame::const_iterator itObs = sf->begin();
        ...
        if (IS_CLASS((*itObs), CObservationBeaconRanges)) {
          const CObservationBeaconRanges *obs =
              static_cast<const CObservationBeaconRanges *>(itObs->get());
          deque<CObservationBeaconRanges::TMeasurement>::const_iterator
              itRanges;
          for (itRanges = obs->sensedData.begin();
            ...
            for (CBeaconMap::iterator itBeacs = beacMap->begin();
              ...
              if ((itBeacs)->m_ID == itRanges->beaconID) {
                ...
                ASSERT_((itBeacs)->m_typePDF == CBeacon::pdfGauss ||
      ...
      sort(lstObservedRanges.begin(), lstObservedRanges.end(),
      ...
      if (methodSOGorGrid) {
        ...
        //       at least one well-localized beacon at the beginning, or the
          ...
          for (deque<TAuxRangeMeasInfo>::iterator itObs =
            ...
            if ((itBeacs)->m_ID == itObs->beaconID) {
              ...
              if (!fusedObsModels.size()) {
                ...
                tempFused.bayesianFusion(fusedObsModels, newObsModel,
...

              {
                ...
                for (it = fusedObsModels.begin(); it != fusedObsModels.end();
                     it++)
                  max_w =
                      max(max_w, (it)->log_w); // keep the maximum mode weight
        ...
        cout << "fusedObsModels # of modes: " << fusedObsModels.m_modes.size()
        << endl;
        printf("ESS: %f\n",fusedObsModels.ESS() );
        ...
        do {
          CPosePDFGrid *pdfGrid =
              new CPosePDFGrid(grid_min_x, grid_max_x, grid_min_y, grid_max_y,
                        ...
                        lik *= exp(
                            ...
                            square(
                                ...
                                (it)->val.mean.distance2DTo(
                                    grid_x + itObs->sensorLocationOnRobot.x(),
                                    grid_y +
                                        itObs->sensorLocationOnRobot.y())) /
                            sensorSTD2);
                      }
                    } break;
                    default:
      ...
      if (!beacMap->size()) {
        ...
        // cout << "Final cov was:\n" << fusedObsModels.getEstimatedCovariance()
        // << endl << endl;

        ASSERT_(
            firstEstimateRobotHeading !=
            std::numeric_limits<float>::max()) // Make sure it was initialized

        finalPose.setFromValues(newDrawnPosition.x(), newDrawnPosition.y(),
                                newDrawnPosition.z(), firstEstimateRobotHeading,
                                0, 0);
      }
