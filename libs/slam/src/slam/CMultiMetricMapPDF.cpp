...
CMultiMetricMapPDF::CMultiMetricMapPDF(
    ...
    : averageMap(mapsInitializers), averageMapIsUpdated(false), SFs(),
      SF2robotPath(), options(), newInfoIndex(0) {
...
void CMultiMetricMapPDF::getEstimatedPosePDF(
  ...
  getEstimatedPosePDFAtTime(m_particles[0].d->robotPath.size() - 1,
                            out_estimation);
}
...
/*---------------------------------------------------------------
                                                writeToStream
  ---------------------------------------------------------------*/
void CRBPFParticleData::writeToStream(mrpt::utils::CStream &out,
                                      int *version) const {
...
void CMultiMetricMapPDF::rebuildAverageMap() {
  ...
  for (part = m_particles.begin(); part != m_particles.end(); ++part)
    part->d->mapTillNow.m_gridMaps[0]->resizeGrid(min_x, max_x, min_y, max_y,
                                                  0.5f, false);
  ...
  // CHECK:
  for (part = m_particles.begin(); part != m_particles.end(); ++part) {
    ASSERT_(part->d->mapTillNow.m_gridMaps[0]->getSizeX() ==
            averageMap.m_gridMaps[0]->getSizeX());
    ASSERT_(part->d->mapTillNow.m_gridMaps[0]->getSizeY() ==
...
void CMultiMetricMapPDF::getPath(size_t i,
                                 std::deque<math::TPose3D> &out_path) const {
  if (i >= m_particles.size())
...
CMultiMetricMapPDF::getCurrentMostLikelyMetricMap() const {
...
void CMultiMetricMapPDF::saveCurrentPathEstimationToTextFile(
  ...
  for (CParticleList::iterator it = m_particles.begin();
    ...
    for (size_t i = 0; i < it->d->robotPath.size(); i++) {
      ...
      os::fprintf(f, "%.04f %.04f %.04f %.04f %.04f %.04f ", p.x, p.y, p.z,
...
CMultiMetricMapPDF::TPredictionParams::TPredictionParams()
    : pfOptimalProposal_mapSelection(0), ICPGlobalAlign_MinQuality(0.70f),
...
void CMultiMetricMapPDF::TPredictionParams::loadFromConfigFile(
  ...
  pfOptimalProposal_mapSelection =
      iniFile.read_int(section, "pfOptimalProposal_mapSelection",
