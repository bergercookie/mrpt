...
CMetricMapBuilderRBPF::CMetricMapBuilderRBPF(
    ...
    : mapPDF(initializationOptions.PF_options,
      ...
      odoIncrementSinceLastLocalization(), odoIncrementSinceLastMapUpdate() {
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
CMetricMapBuilderRBPF &CMetricMapBuilderRBPF::
operator=(const CMetricMapBuilderRBPF &src) {
  if (this == &src) {
    return *this;
  }
  mapPDF = src.mapPDF;
...
void CMetricMapBuilderRBPF::processActionObservation(
  ...
  MRPT_LOG_DEBUG(mrpt::format("do_map_update=%s do_localization=%s",
                              do_map_update ? "YES" : "NO",
  ...
  if (do_localization) {
    ...
    if (isLoggingLevelVisible(LVL_INFO)) {
      ...
      MRPT_LOG_INFO_STREAM("New pose=" << estPos << std::endl
                                       << "New ESS:" << mapPDF.ESS()
                                       << std::endl);
...
MRPT_TODO("Split initialize() in two, one generic without pose, one with "
          "particles-based PDF");

/*---------------------------------------------------------------
                                        initialize
  ---------------------------------------------------------------*/
void CMetricMapBuilderRBPF::initialize(const CSimpleMap &initialMap,
                                       CPosePDF *x0) {
  // Enter critical section (updating map)
  enterCriticalSection();

  MRPT_LOG_INFO_STREAM("[initialize] Called with " << initialMap.size()
                                                   << " nodes in fixed map\n");
  if (x0) {
...
CMetricMapBuilderRBPF::getCurrentlyBuiltMetricMap() const {
...
void CMetricMapBuilderRBPF::drawCurrentEstimationToImage(utils::CCanvas *img) {
  ...
  bool alreadyCopiedImage = false;
  {
    ...
    if (obj)
      obj->resize(currentMetricMapEstimation->m_gridMaps[0]->getSizeX(),
                  currentMetricMapEstimation->m_gridMaps[0]->getSizeY(), 1,
                  true);
  }
  ...
  for (i = 0; i <= M; i++) {
    if (i != bestPath || i == M) {
      ...
      for (size_t j = 0; j < nPoses; j++) {
        ...
        // Draw line:
        img->line(x1, round((imgHeight - 1) - y1), x2,
...
CMetricMapBuilderRBPF::TConstructionOptions::TConstructionOptions()
    : insertionLinDistance(1.0f), insertionAngDistance(DEG2RAD(30)),
      localizeLinDistance(0.4f), localizeAngDistance(DEG2RAD(10)), PF_options(),
      mapsInitializers(), predictionOptions(),
