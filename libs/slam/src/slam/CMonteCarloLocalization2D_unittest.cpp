...
void run_test_pf_localization(CPose2D &meanPose, CMatrixDouble33 &cov) {
  ...
  // Mandatory entries:
  iniFile.read_vector(iniSectionName, "particles_count", vector_int(1, 0),
                      particles_count, /*Fail if not found*/ true);
  string RAWLOG_FILE = iniFile.read_string(iniSectionName, "rawlog_file", "",
  ...
  for (vector_int::iterator itNum = particles_count.begin();
    ...
    for (int repetition = 0; repetition < NUM_REPS; repetition++) {
      ...
      // -----------------------------
      tictac.Tic();
      if (!iniFile.read_bool(iniSectionName, "init_PDF_mode", false,
                             /*Fail if not found*/ true))
        pdf.resetUniformFreeSpace(
            ...
            DEG2RAD(iniFile.read_float(iniSectionName, "init_PDF_min_phi_deg",
                                       -180)),
            DEG2RAD(iniFile.read_float(iniSectionName, "init_PDF_max_phi_deg",
                                       180)));
      else
        pdf.resetUniform(
            iniFile.read_float(iniSectionName, "init_PDF_min_x", 0, true),
...
TEST(MonteCarlo2D, RunSampleDataset) {
  ...
  for (int op = 0; op < 3; op++) {
    ...
    cout << "\n*Warning: Test failed. Will give it another chance, since after "
            "all it's nondeterministic!\n";
  }
