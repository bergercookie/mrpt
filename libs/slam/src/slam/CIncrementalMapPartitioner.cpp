...
CIncrementalMapPartitioner::CIncrementalMapPartitioner()
    : COutputLogger("CIncrementalMapPartitioner"), options(),
      m_individualFrames(), m_individualMaps(), m_A(0, 0), m_last_partition(),
      m_last_last_partition_are_new_ones(false), m_modified_nodes() {
  clear();
}
...
CIncrementalMapPartitioner::TOptions::TOptions()
    : partitionThreshold(1.0f), gridResolution(0.10f),
      minDistForCorrespondence(0.20f), minMahaDistForCorrespondence(2.0f),
      forceBisectionOnly(false), useMapMatching(true),
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
unsigned int
CIncrementalMapPartitioner::addMapFrame(const CSensoryFrame::Ptr &frame,
                                        const CPosePDF::Ptr &robotPose) {
  MRPT_START
  return addMapFrame(frame,
                     CPose3DPDF::Ptr(CPose3DPDF::createFrom2D(*robotPose)));
  MRPT_END
}
...
CIncrementalMapPartitioner::addMapFrame(const CSensoryFrame::Ptr &frame,
  ...
  // JLBC,17/AGO/2006: "m_individualMaps" were created from the robot pose, but
  // it is
  //   more convenient now to save them as the robot being at (0,0,0).

  // frame->insertObservationsInto(newMetricMap.m_pointsMaps[0]);
  newMetricMap.m_pointsMaps[0]->copyFrom(*frame->buildAuxPointsMap<CPointsMap>(
...
void CIncrementalMapPartitioner::updatePartitions(
  ...
  for (i = 0; i < n; i++)
    m_last_partition[i] = partitions[i];
  ...
  for (size_t i = 0; i < m_individualFrames.size(); i++) {
    ...
    for (size_t j = i + 1; j < m_individualFrames.size(); j++) {
      ...
      if (SSO_ij > 0.01) {
        ...
        lin->setLineCoords(i_mean.x(), i_mean.y(), i_mean.z(), j_mean.x(),
                           j_mean.y(), j_mean.z());
...
/*---------------------------------------------------------------
                                        readFromStream
  ---------------------------------------------------------------*/
void CIncrementalMapPartitioner::readFromStream(mrpt::utils::CStream &in,
                                                int version) {
  switch (version) {
  case 0: {
    in >> m_individualFrames >> m_individualMaps >> m_A >> m_last_partition >>
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CIncrementalMapPartitioner::writeToStream(mrpt::utils::CStream &out,
                                               int *version) const {
  if (version)
    *version = 0;
  else {
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
unsigned int
CIncrementalMapPartitioner::addMapFrame(const CSensoryFrame &frame,
                                        const CPose3DPDF &robotPose3D) {
  return addMapFrame(CSensoryFrame::Ptr(new CSensoryFrame(frame)),
