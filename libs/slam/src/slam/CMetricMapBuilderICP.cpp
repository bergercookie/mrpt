...
CMetricMapBuilderICP::TConfigParams::TConfigParams(
    ...
    : matchAgainstTheGrid(false), insertionLinDistance(1.0),
      insertionAngDistance(DEG2RAD(30)), localizationLinDistance(0.20),
      localizationAngDistance(DEG2RAD(30)), minICPgoodnessToAccept(0.40),
      verbosity_level(parent_verbosity_level), mapInitializers() {}
...
void CMetricMapBuilderICP::processObservation(const CObservation::Ptr &obs) {
  ...
  if (metricMap.m_pointsMaps.empty() && metricMap.m_gridMaps.empty())
    throw std::runtime_error("Neither grid maps nor points map: Have you "
                             "called initialize() after setting "
                             "ICP_options.mapInitializers?");
  ...
  if (IS_CLASS(obs, CObservationOdometry)) {
    ...
    m_lastPoseEst.processUpdateNewOdometry(
        odo->odometry, odo->timestamp, odo->hasVelocities, odo->velocityLocal);
    ...
    if (pose_before_valid) {
      ...
      MRPT_LOG_DEBUG_STREAM(
          "processObservation(): obs is CObservationOdometry, new post_after="
          << pose_after);
    }
  } // end it's odometry
  else {
    ...
    TPose2D initialEstimatedRobotPose;
    {
      ...
      if (obs->timestamp != INVALID_TIMESTAMP) {
        MRPT_LOG_DEBUG("processObservation(): extrapolating pose from latest "
                       "pose and new observation timestamp...");
        if (!m_lastPoseEst.getCurrentEstimate(
                initialEstimatedRobotPose, robotVelLocal, robotVelGlobal,
                obs->timestamp)) { // couldn't had a good extrapolation
                                   // estimate... we'll have to live with the
                                   // latest pose:
          m_lastPoseEst.getLatestRobotPose(initialEstimatedRobotPose);
          MRPT_LOG_WARN("processObservation(): new pose extrapolation failed, "
    ...
    const bool we_skip_ICP_pose_correction =
        ...
        m_distSinceLastICP.lin < std::min(ICP_options.localizationLinDistance,
                                          ICP_options.insertionLinDistance) &&
        m_distSinceLastICP.ang < std::min(ICP_options.localizationAngDistance,
                                          ICP_options.insertionAngDistance);

    MRPT_LOG_DEBUG_STREAM("processObservation(): skipping ICP pose correction "
                          "due to small odometric displacement? : "
    ...
    if (ICP_options.matchAgainstTheGrid && !metricMap.m_gridMaps.empty()) {
      ...
      ASSERTMSG_(metricMap.m_pointsMaps.size(),
    ...
    if (!we_skip_ICP_pose_correction) {
      ...
      //  2) In any case, insert the observation if the minimum distance has
      //  been satisfaced.
      // --------------------------------------------------------------------------------------
      ...
      if (ICP_options.matchAgainstTheGrid && !metricMap.m_gridMaps.empty() &&
        ...
        if (IS_CLASS(obs, CObservation2DRangeScan)) {
          ...
          if (std::abs(metricMap.m_gridMaps[0]->insertionOptions.mapAltitude -
      ...
      if (can_do_icp) {
        ...
        CPosePDF::Ptr pestPose = ICP.Align(
            matchWith,                                       // Map 1
            &sensedPoints,                                   // Map 2
            mrpt::poses::CPose2D(initialEstimatedRobotPose), // a first gross
                                                             // estimation of
                                                             // map 2 relative
                                                             // to map 1.
        ...
        if (icpReturn.goodness > ICP_options.minICPgoodnessToAccept) {
          ...
          m_lastPoseEst.processUpdateNewPoseLocalization(TPose2D(pEst2D.mean),
                                                         obs->timestamp);
          ...
          MRPT_LOG_INFO_STREAM("processObservation: previousPose="
                               << previousKnownRobotPose << "-> currentPose="
                               << pEst2D.getMeanVal() << std::endl);
          MRPT_LOG_INFO(
              format("[CMetricMapBuilderICP]   Fit:%.1f%% Itr:%i In %.02fms \n",
                     icpReturn.goodness * 100, icpReturn.nIterations,
                     1000 * runningTime));
        } else {
          MRPT_LOG_WARN_STREAM("Ignoring ICP of low quality: "
        ...
        MRPT_LOG_WARN_STREAM(
            "Cannot do ICP: empty pointmap or not suitable gridmap...\n");
      }
    ...
    //    either: (i) this was a good match or (ii) this is the first time for
    //    this sensor.
    // ----------------------------------------------------------
    ...
    // We need to always insert ALL the observations at the beginning until the
    // first one
    //  that actually insert some points into the map used as a reference, since
    //  otherwise
    //  we'll not be able to do ICP against an empty map!!
    if (matchWith && matchWith->isEmpty())
      update = true;

    MRPT_LOG_DEBUG_STREAM("update map: "
                          << (update ? "YES" : "NO")
    ...
    if (options.enableMapUpdating && update) {
      ...
      if (!anymap_update)
        MRPT_LOG_WARN_STREAM(
            "**No map was updated** after inserting an observation of type `"
      ...
      MRPT_LOG_INFO_STREAM("Map updated OK. Done in "
                           << mrpt::system::formatTimeInterval(tictac.Tac())
                           << std::endl);
    }
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CMetricMapBuilderICP::processActionObservation(CActionCollection &action,
                                                    CSensoryFrame &in_SF) {
  // 1) process action:
  CActionRobotMovement2D::Ptr movEstimation =
      action.getBestMovementEstimation();
  if (movEstimation) {
    m_auxAccumOdometry.composeFrom(m_auxAccumOdometry,
                                   movEstimation->poseChange->getMeanVal());
...
/*---------------------------------------------------------------
                                                initialize
  ---------------------------------------------------------------*/
void CMetricMapBuilderICP::initialize(const CSimpleMap &initialMap,
                                      CPosePDF *x0) {
  MRPT_START
  ...
  if (x0)
    m_lastPoseEst.processUpdateNewPoseLocalization(x0->getMeanVal(),
                                                   mrpt::system::now());
...
CMetricMapBuilderICP::getCurrentlyBuiltMetricMap() const {
  return &metricMap;
}
...
void CMetricMapBuilderICP::TDist::updateDistances(
