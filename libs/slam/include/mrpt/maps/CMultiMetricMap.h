 ...
 *		- mrpt::maps::COccupancyGridMap2D: 2D, <b>horizontal</b>  laser
 *range scans, at different altitudes.
 *		- mrpt::maps::COctoMap: For 3D occupancy grids of variable
 *resolution, with octrees (based on the library `octomap`).
 *		- mrpt::maps::CColouredOctoMap: The same than above, but nodes can
 *store RGB data appart from occupancy.
 *		- mrpt::maps::CLandmarksMap: For visual landmarks,etc...
 *		- mrpt::maps::CGasConcentrationGridMap2D: For gas concentration
 *maps.
 *		- mrpt::maps::CWirelessPowerGridMap2D: For wifi power maps.
 *		- mrpt::maps::CBeaconMap: For range-only SLAM.
 *		- mrpt::maps::CHeightGridMap2D: For elevation maps of height for
 *each (x,y) location (Digital elevation model, DEM)
 *		- mrpt::maps::CHeightGridMap2D_MRF: DEMs as Markov Random Field
 *(MRF)
 *		- mrpt::maps::CReflectivityGridMap2D: For maps of "reflectivity"
 *for each (x,y) location.
 *		- mrpt::maps::CColouredPointsMap: For point map with color.
 *		- mrpt::maps::CWeightedPointsMap: For point map with weights
 *(capable of "fusing").
 *
 ...
 *std::make_shared<mrpt::maps::CSimplePointsMap>();
...
protected:
  void deleteAllMaps(); //!< Deletes all maps and clears the internal lists of
                        //!maps (with clear_unique(), so user copies remain
                        //!alive)
...
public:
   ...
   * pointer-based downcast to access maps by their actual type.
    * You can directly manipulate this list. Helper methods to initialize it are
   * described in the docs of CMultiMetricMap
    */
  TListMaps maps;
  ...
  /** Returns the i'th observation of a given class (or of a descendant class),
   * or nullptr if there is no such observation in the array.
    *  Example:
    * \code
  ...
  /** Takes a const ref of a STL non-associative container of smart pointers at
   * construction and exposes an interface
  ...
  struct ProxyFilterContainerByClass {
    ...
    ProxyFilterContainerByClass(
        ...
        : m_source() {} // m_source init in parent copy ctor
    ...
    ProxyFilterContainerByClass(
        ...
        : m_source() {} // m_source init in parent copy ctor
    ProxyFilterContainerByClass<SELECTED_CLASS_PTR, CONTAINER> &
    operator=(ProxyFilterContainerByClass<SELECTED_CLASS_PTR, CONTAINER> &&o) {
      return *this;
    } // Do nothing, we must keep refs to our own parent
        ...
        if (dynamic_cast<const_ptr_t>(it->get()))
          if (cnt++ == index) {
            return std::dynamic_pointer_cast<
                typename SELECTED_CLASS_PTR::element_type>(it->get_ptr());
          }
      throw std::out_of_range("Index is out of range");
    }
    template <typename ELEMENT> void push_back(const ELEMENT &element) {
  ...
  /** A proxy like ProxyFilterContainerByClass, but it directly appears as if
    * it was a single smart pointer (empty if no matching object is found in the
   * container) */
  template <class SELECTED_CLASS_PTR, class CONTAINER>
  struct ProxySelectorContainerByClass {
    ...
    ProxySelectorContainerByClass<SELECTED_CLASS_PTR, CONTAINER> &operator=(
        const ProxySelectorContainerByClass<SELECTED_CLASS_PTR, CONTAINER> &o) {
      return *this;
    } // Do nothing, we must keep refs to our own parent
    ProxySelectorContainerByClass(
        ProxySelectorContainerByClass<SELECTED_CLASS_PTR, CONTAINER> &&)
  ...
  ProxyFilterContainerByClass<mrpt::maps::CSimplePointsMap::Ptr, TListMaps>
      m_pointsMaps; //!< STL-like proxy to access this kind of maps in \ref maps
  ProxyFilterContainerByClass<mrpt::maps::COccupancyGridMap2D::Ptr, TListMaps>
      m_gridMaps; //!< STL-like proxy to access this kind of maps in \ref maps
  ProxyFilterContainerByClass<mrpt::maps::COctoMap::Ptr, TListMaps>
      m_octoMaps; //!< STL-like proxy to access this kind of maps in \ref maps
  ProxyFilterContainerByClass<mrpt::maps::CColouredOctoMap::Ptr, TListMaps>
      m_colourOctoMaps; //!< STL-like proxy to access this kind of maps in \ref
                        //!maps
  ProxyFilterContainerByClass<mrpt::maps::CGasConcentrationGridMap2D::Ptr,
                              TListMaps>
      m_gasGridMaps; //!< STL-like proxy to access this kind of maps in \ref
                     //!maps
  ProxyFilterContainerByClass<mrpt::maps::CWirelessPowerGridMap2D::Ptr,
                              TListMaps>
      m_wifiGridMaps; //!< STL-like proxy to access this kind of maps in \ref
                      //!maps
  ProxyFilterContainerByClass<mrpt::maps::CHeightGridMap2D::Ptr, TListMaps>
      m_heightMaps; //!< STL-like proxy to access this kind of maps in \ref maps
  ProxyFilterContainerByClass<mrpt::maps::CHeightGridMap2D_MRF::Ptr, TListMaps>
      m_heightMRFMaps; //!< STL-like proxy to access this kind of maps in \ref
                       //!maps
  ProxyFilterContainerByClass<mrpt::maps::CReflectivityGridMap2D::Ptr,
                              TListMaps>
      m_reflectivityMaps; //!< STL-like proxy to access this kind of maps in
                          //!\ref maps
  ProxySelectorContainerByClass<mrpt::maps::CColouredPointsMap::Ptr, TListMaps>
      m_colourPointsMap; //!< Proxy that looks like a smart pointer to the first
                         //!matching object in \ref maps
  ProxySelectorContainerByClass<mrpt::maps::CWeightedPointsMap::Ptr, TListMaps>
      m_weightedPointsMap; //!< Proxy that looks like a smart pointer to the
                           //!first matching object in \ref maps
  ProxySelectorContainerByClass<mrpt::maps::CLandmarksMap::Ptr, TListMaps>
      m_landmarksMap; //!< Proxy that looks like a smart pointer to the first
                      //!matching object in \ref maps
  ProxySelectorContainerByClass<mrpt::maps::CBeaconMap::Ptr, TListMaps>
      m_beaconMap; //!< Proxy that looks like a smart pointer to the first
                   //!matching object in \ref maps

  /** @} */

  /** Constructor.
   * \param initializers One internal map will be created for each entry in this
   * "TSetOfMetricMapInitializers" struct.
  ...
  void
  setListOfMaps(const mrpt::maps::TSetOfMetricMapInitializers *initializers);
  /** \overload */
  void
  setListOfMaps(const mrpt::maps::TSetOfMetricMapInitializers &initializers) {
  ...
  bool isEmpty() const override; //!< Returns true if all maps returns true to
                                 //!their isEmpty() method, which is
                                 //!map-dependent. Read the docs of each map
                                 //!class
  ...
  /** See the definition in the base class: Calls in this class become a call to
   * every single map in this set. */
  float
  ...
  /** This method is called at the end of each "prediction-update-map insertion"
   * cycle within "mrpt::slam::CMetricMapBuilderRBPF::processActionObservation".
    *  This method should normally do nothing, but in some cases can be used to
   * free auxiliary cached variables.
    */
  ...
  /** If the map is a simple point map or it's a multi-metric map that contains
   * EXACTLY one simple point map, return it.
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CMultiMetricMap,
