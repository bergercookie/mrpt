...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CRBPFParticleData,
...
private:
  ...
  mrpt::maps::CSimpleMap
      SFs; //!< The SFs and their corresponding pose estimations
  std::vector<uint32_t> SF2robotPath; //!< A mapping between indexes in the SFs
                                      //!to indexes in the robot paths from
                                      //!particles.
...
public:
  ...
  struct SLAM_IMPEXP TPredictionParams : public utils::CLoadableOptions {
     ...
     *   3: Pointsmap -> Uses Scan matching-based approximation with a map of
     * points (based on Stachniss' work)
     *  Default = 0
     */
    int pfOptimalProposal_mapSelection;

    /** [prediction stage][pf optimal proposal only] If
     * useICPGlobalAlign_withGrid=true, this is the minimum quality ratio [0,1]
     * of the alignment such as it will be accepted. Otherwise, raw odometry is
     * used for those bad cases (default=0.7).
      */
    float ICPGlobalAlign_MinQuality;

    /** [update stage] If the likelihood is computed through the occupancy grid
     * map, then this structure is passed to the map when updating the particles
     * weights in the update stage.
      */
    COccupancyGridMap2D::TLikelihoodOptions update_gridMapLikelihoodOptions;

    mrpt::slam::TKLDParams KLD_params;

    mrpt::slam::CICP::TConfigParams icp_params; //!< ICP parameters, used only
                                                //!when "PF_algorithm=2" in the
                                                //!particle filter.
  ...
  /** Clear all elements of the maps, and restore all paths to a single starting
   * pose */
  void clear(const mrpt::poses::CPose2D &initialPose);
  void clear(const mrpt::poses::CPose3D &initialPose); //!< \overload

  /** Returns the estimate of the robot pose as a particles PDF for the instant
   * of time "timeStep", from 0 to N-1.
   * \sa getEstimatedPosePDF
   */
  void getEstimatedPosePDFAtTime(
      size_t timeStep, mrpt::poses::CPose3DPDFParticles &out_estimation) const;

  /** Returns the current estimate of the robot pose, as a particles PDF.
   * \sa getEstimatedPosePDFAtTime
   */
  void
  getEstimatedPosePDF(mrpt::poses::CPose3DPDFParticles &out_estimation) const;

  /** Returns the weighted averaged map based on the current best estimation. If
   * you need a persistent copy of this object, please use
   ...
   * poses along the path, where entropy of each pose estimation is computed as
   * the entropy of the gaussian approximation covariance.
    */
  ...
  /** A logging utility: saves the current path estimation for each particle in
   * a text file (a row per particle, each 3-column-entry is a set [x,y,phi],
   * respectively).
    */
...
private:
  ...
  float newInfoIndex; //!< An index [0,1] measuring how much information an
                      //!observation aports to the map (Typ. threshold=0.07)
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CMultiMetricMapPDF,
                                             mrpt::utils::CSerializable,
                                             SLAM_IMPEXP)
