...
public:
   ...
   */
  virtual void
  processActionObservation(mrpt::obs::CActionCollection &action,
  ...
  virtual const mrpt::maps::CMultiMetricMap *
  getCurrentlyBuiltMetricMap() const = 0;
   ...
   * is dumped to an image file.
    ...
    */
  virtual void saveCurrentEstimationToImage(const std::string &file,
                                            bool formatEMF_BMP = true) = 0;
  ...
  void saveCurrentMapToFile(const std::string &fileName,
                            bool compressGZ = true) const;

  /** Options for the algorithm */
  struct SLAM_IMPEXP TOptions {
    TOptions(mrpt::utils::VerbosityLevel &verb_level_ref)
        : verbosity_level(verb_level_ref), enableMapUpdating(true),
          debugForceInsertion(false), alwaysInsertByClass() {}

    mrpt::utils::VerbosityLevel &verbosity_level;
    bool enableMapUpdating;   //!< Enable map updating, default is true.
    bool debugForceInsertion; //!< Always insert into map. Default is false:
                              //!detect if necesary.
