...
public:
  ...
  struct SLAM_IMPEXP TConfigParams : public mrpt::utils::CLoadableOptions {
    ...
    /** (default:false) Match against the occupancy grid or the points map? The
     * former is quicker but less precise. */
    bool matchAgainstTheGrid;

    double insertionLinDistance; //!< Minimum robot linear (m) displacement for
                                 //!a new observation to be inserted in the map.
    double insertionAngDistance; //!< Minimum robot angular (rad, deg when
                                 //!loaded from the .ini) displacement for a new
                                 //!observation to be inserted in the map.
    double localizationLinDistance; //!< Minimum robot linear (m) displacement
                                    //!for a new observation to be used to do
                                    //!ICP-based localization (otherwise,
                                    //!dead-reckon with odometry).
    double localizationAngDistance; //!< Minimum robot angular (rad, deg when
                                    //!loaded from the .ini) displacement for a
                                    //!new observation to be used to do
                                    //!ICP-based localization (otherwise,
                                    //!dead-reckon with odometry).

    double minICPgoodnessToAccept; //!< Minimum ICP goodness (0,1) to accept the
                                   //!resulting corrected position (default:
                                   //!0.40)
     ...
     * loadFromConfigFile(), see documentation of TSetOfMetricMapInitializers.
      */
    mrpt::maps::TSetOfMetricMapInitializers mapInitializers;
  };

  TConfigParams
      ICP_options; //!< Options for the ICP-SLAM application \sa ICP_params
  CICP::TConfigParams
      ICP_params; //!< Options for the ICP algorithm itself \sa ICP_options
  ...
  /** Sets the "current map file", thus that map will be loaded if it exists or
   * a new one will be created if it does not, and the updated map will be save
   * to that file when destroying the object.
    */
   ...
   */
  void processActionObservation(mrpt::obs::CActionCollection &action,
  ...
  /** Fills "out_map" with the set of "poses"-"sensory-frames", thus the so far
   * built map */
  ...
  getCurrentlyBuiltMetricMap() const override;
  ...
  void saveCurrentEstimationToImage(const std::string &file,
                                    bool formatEMF_BMP = true) override;
...
private:
  ...
  mrpt::poses::CRobot2DPoseEstimator
      m_lastPoseEst; //!< Last pose estimation (Mean)
  mrpt::math::CMatrixDouble33
      m_lastPoseEst_cov; //!< Last pose estimation (covariance)
  ...
  mrpt::aligned_containers<std::string, TDist>::map_t
      m_distSinceLastInsertion; //!< Indexed by sensor label.
  bool m_there_has_been_an_odometry;

  void
  accumulateRobotDisplacementCounters(const mrpt::poses::CPose2D &new_pose);
