...
class SLAM_IMPEXP CMetricMapBuilderICP : public mrpt::slam::CMetricMapBuilder
{
   public:
	...
	struct SLAM_IMPEXP TConfigParams : public mrpt::utils::CLoadableOptions
	{
		...
		TConfigParams& operator=(
			const TConfigParams& other);  // Copy assignment
		 ...
		 * displacement
		 * for a new observation to be inserted in the map. */
		double insertionAngDistance;
		/** Minimum robot linear (m) displacement for a new observation to be
		 * used
		 * to do ICP-based localization (otherwise, dead-reckon with odometry).
		 */
		double localizationLinDistance;
		/** Minimum robot angular (rad, deg when loaded from the .ini)
		 * displacement
		 * for a new observation to be used to do ICP-based localization
		 * (otherwise,
	 ...
	 * or
	 * a new one will be created if it does not, and the updated map will be
	 * save
