  ...
  {
    {
      ...
      if (robotMovement2D) {
        ...
        if (!m_accumRobotMovement2DIsValid) { // First time:
      ...
      } else // If there is no 2D action, look for a 3D action:
      {
        ...
        if (robotMovement3D) {
          ...
          if (!m_accumRobotMovement3DIsValid)
            ...
            m_accumRobotMovement3D += robotMovement3D->poseChange;
          // This "+=" takes care of all the Jacobians, etc... You MUST love
          // C++!!! ;-)
    ...
    mrpt::poses::CPose3D motionModelMeanIncr;
    {
      ...
      if (robotMovement2D) {
        ...
        if (robotMovement3D) {
          ...
          THROW_EXCEPTION("Action list does not contain any "
  ...
  if (sf) {
    ...
    for (size_t i = 0; i < M; i++) {
      ...
      const double obs_log_likelihood =
          PF_SLAM_computeObservationLikelihoodForParticle(PF_options, i, *sf,
                                                          partPose2);
...
double PF_implementation<PARTICLE_TYPE, MYSELF>::
  ...
  for (size_t q = 0; q < N; q++) {
    ...
    vectLiks[q] = indivLik;
    if (indivLik > maxLik) { // Keep the maximum value:
...
double PF_implementation<PARTICLE_TYPE, MYSELF>::
  ...
  if (!PF_options.pfAuxFilterStandard_FirstStageWeightsMonteCarlo) {
    ...
    x_predict.composeFrom(oldPose,
                          *static_cast<const mrpt::poses::CPose3D *>(action));
...
void PF_implementation<PARTICLE_TYPE, MYSELF>::
  ...
  // ----------------------------------------------------------------------
  if (!PF_SLAM_implementation_gatherActionsCheckBothActObs<BINTYPE>(actions,
                                                                    sf))
  ...
  if (USE_OPTIMAL_SAMPLING &&
    ...
    me->logStr(
        ...
        mrpt::format("[prepareFastDrawSample] max      (log) = %10.06f\n",
                     math::maximum(m_pfAuxiliaryPFOptimal_estimatedProb)));
    me->logStr(
        mrpt::utils::LVL_DEBUG,
        mrpt::format("[prepareFastDrawSample] max-mean (log) = %10.06f\n",
                     -math::mean(m_pfAuxiliaryPFOptimal_estimatedProb) +
                         math::maximum(m_pfAuxiliaryPFOptimal_estimatedProb)));
    me->logStr(
        mrpt::utils::LVL_DEBUG,
        mrpt::format("[prepareFastDrawSample] max-min  (log) = %10.06f\n",
  ...
  if (!PF_options.adaptiveSampleSize) {
    ...
    // ------------------------------------------------------------------------------
    // 2.1) PRELIMINARY STAGE: Build a list of pairs<TPathBin,vector_uint> with
    // the
    //      indexes of m_particles that fall into each multi-dimensional-path
    //      bins
    //      //The bins will be saved into "stateSpaceBinsLastTimestep", and the
    //      list
    ...
    for (partIt = me->m_particles.begin(), partIndex = 0;
      ...
      BINTYPE p;
      KLF_loadBinFromParticle<PARTICLE_TYPE, BINTYPE>(p, KLD_options,
                                                      partIt->d.get());
    ...
    // double	maxLik = math::maximum(m_pfAuxiliaryPFOptimal_maxLikelihood); //
    // For normalization purposes only

    // The desired dynamic number of m_particles (to be modified dynamically
    // below):
    const size_t minNumSamples_KLD =
        std::max((size_t)KLD_options.KLD_minSampleSize,
                 (size_t)round(KLD_options.KLD_minSamplesPerBin *
    ...
    std::random_shuffle(permutationPathsAuxVector.begin(),
    ...
    do // "N" is the index of the current "new particle":
    {
      ...
      if (doResample) {
        ...
        // Assure that at least one particle per "discrete-path" is taken (if
        // the
        ...
        if (permutationPathsAuxVector.size()) {
          ...
          permutationPathsAuxVector.resize(permutationPathsAuxVector.size() -
                                           1);

          const size_t idx =
              mrpt::random::randomGenerator.drawUniform32bit() %
              stateSpaceBinsLastTimestepParticles[idxBinSpacePath].size();
          ...
          oldPartIdxsStillNotPropragated.erase(
              std::find(oldPartIdxsStillNotPropragated.begin(),
                        oldPartIdxsStillNotPropragated.end(), k));
        } else {
          // Select a particle from the previous set with a UNIFORM
          // distribution,
          // in such a way we will assign each particle the updated weight
          // accounting
          // for its last weight.
          // The first "old_N" m_particles will be drawn using a uniform random
          // selection
          // without repetitions:
          //
          // Select a index from "oldPartIdxsStillNotPropragated" and remove it
          // from the list:
      ...
      // -----------------------------------------------------------------------------
      // Look for the bin "p" into "stateSpaceBins": If it is not yet into the
      // set,
...
void PF_implementation<PARTICLE_TYPE, MYSELF>::
  ...
  // ADD-ON: If the 'm_pfAuxiliaryPFOptimal_estimatedProb[k]' is **extremelly**
  // low relative to the other m_particles,
  ...
  while (((USE_OPTIMAL_SAMPLING ? m_pfAuxiliaryPFOptimal_estimatedProb[k]
    ...
    k = mrpt::random::randomGenerator.drawUniform32bit() %
        me->m_particles.size();
    me->logStr(mrpt::utils::LVL_DEBUG, "[PF_SLAM_aux_perform_one_rejection_"
                                       "sampling_step] Warning: Discarding "
                                       "very unlikely particle.");
  }
  ...
  if (PF_SLAM_implementation_skipRobotMovement()) {
    ...
    } else { // Optimal APF with rejection sampling:
      // Rejection-sampling:
      ...
      do {
        ...
        if (PF_options.pfAuxFilterOptimal_MLE &&
            !m_pfAuxiliaryPFOptimal_maxLikMovementDrawHasBeenUsed
                [k]) { // No! first take advantage of a good drawn value, but
                       // only once!!
        ...
        out_newPose.composeFrom(
            oldPose, movementDraw); // out_newPose = oldPose + movementDraw;
        ...
        double ratioLikLik =
            std::exp(poseLogLik - m_pfAuxiliaryPFOptimal_maxLikelihood[k]);
    ...
    // And its weight:
    if (USE_OPTIMAL_SAMPLING) { // Optimal PF
      if (doResample)
        out_newParticleLogWeight = 0; // By definition of our optimal PF, all
                                      // samples have identical weights.
      else {
      ...
      if (doResample)
        ...
        out_newParticleLogWeight = weightFact + me->m_particles[k].log_w;
    }
  }
  // Done.
