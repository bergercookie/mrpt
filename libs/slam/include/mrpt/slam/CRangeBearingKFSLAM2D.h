...
public:
   ...
   * out_landmarksPositions) gives the corresponding landmark ID.
    ...
    *  \param out_fullCovariance The full (3+2M)x(3+2M) covariance matrix of the
   * filter.
    * \sa getCurrentRobotPose
    */
  void
  getCurrentState(mrpt::poses::CPosePDFGaussian &out_robotPose,
  ...
  struct SLAM_IMPEXP TOptions : utils::CLoadableOptions {
    ...
    mrpt::math::CVectorFloat stds_Q_no_odo; //!< A 3-length vector with the std.
                                            //!deviation of the transition model
                                            //!in (x,y,phi) used only when there
                                            //!is no odometry (if there is odo,
                                            //!its uncertainty values will be
                                            //!used instead); x y: In meters,
                                            //!phi: radians (but in degrees when
                                            //!loading from a configuration
                                            //!ini-file!)
    float std_sensor_range,
        std_sensor_yaw; //!< The std. deviation of the sensor (for the matrix R
                        //!in the kalman filters), in meters and radians.
...
protected:
  ...
  void getLandmarkIDsFromIndexInStateVector(
      std::map<unsigned int, mrpt::maps::CLandmark::TLandmarkID> &out_id2index)
      const {
