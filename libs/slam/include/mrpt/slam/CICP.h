...
enum TICPCovarianceMethod {
  icpCovLinealMSE = 0,    //!< Use the covariance of the optimal registration,
                          //!disregarding uncertainty in data association
  icpCovFiniteDifferences //!< Covariance of a least-squares optimizer (includes
...
public:
  ...
  public:
    ...
    /** @name Algorithms selection
        @{ */
    TICPAlgorithm
        ICP_algorithm; //!< The algorithm to use (default: icpClassic). See
                       //!http://www.mrpt.org/tutorials/programming/scan-matching-and-icp/
                       //!for details
    TICPCovarianceMethod ICP_covariance_method; //!< The method to use for
    ...
    /** @name Correspondence-finding criteria
        @{ */
    bool onlyClosestCorrespondences; //!< The usual approach: to consider only
                                     //!the closest correspondence for each
                                     //!local point (Default to true)
    bool onlyUniqueRobust; //! Apart of "onlyClosestCorrespondences=true", if
                           //! this option is enabled only the closest
                           //! correspondence for each reference point will be
                           //! kept (default=false).
                           /** @} */
    ...
    float minAbsStep_trans;     //!< If the correction in all translation
                            //!coordinates (X,Y,Z) is below this threshold (in
                            //!meters), iterations are terminated (Default:1e-6)
    float minAbsStep_rot; //!< If the correction in all rotation coordinates
                          //!(yaw,pitch,roll) is below this threshold (in
                          //!radians), iterations are terminated (Default:1e-6)
    /** @} */

    float thresholdDist, thresholdAng; //!< Initial threshold distance for two
                                       //!points to become a correspondence.
    float ALFA; //!< The scale factor for threshold everytime convergence is
                //!achieved.
    float smallestThresholdDist; //!< The size for threshold such that
                                 //!iterations will stop, since it is considered
                                 //!precise enough.

    /** This is the normalization constant \f$ \sigma^2_p \f$ that is used to
     * scale the whole 3x3 covariance.
      *  This has a default value of \f$ (0.02)^2 \f$, that is, a 2cm sigma.
      *  See paper: J.L. Blanco, J. Gonzalez-Jimenez, J.A. Fernandez-Madrigal,
     * "A Robust, Multi-Hypothesis Approach to Matching Occupancy Grid Maps",
     * Robotica, vol. 31, no. 5, pp. 687-701, 2013.
      */
    float covariance_varPoints;

    bool doRANSAC; //!< Perform a RANSAC step, mrpt::tfest::se2_l2_robust(),
                   //!after the ICP convergence, to obtain a better estimation
                   //!of the pose PDF.
    ...
    float normalizationStd; //!< RANSAC-step option: The standard deviation in
                            //!X,Y of landmarks/points which are being matched
                            //!(used to compute covariances in the SoG)
    bool ransac_fuseByCorrsMatch;
    float ransac_fuseMaxDiffXY, ransac_fuseMaxDiffPhi;
    /** @} */

    float kernel_rho; //!< Cauchy kernel rho, for estimating the optimal
                      //!transformation covariance, in meters (default = 0.07m)
    bool use_kernel;  //!< Whether to use kernel_rho to smooth distances, or use
                      //!distances directly (default=true)
    float Axy_aprox_derivatives; //!< [LM method only] The size of the
                                 //!perturbance in x & y used to estimate the
                                 //!Jacobians of the square error (default=0.05)
    float LM_initial_lambda;     //!< [LM method only] The initial value of the
                             //!lambda parameter in the LM method (default=1e-4)

    bool skip_cov_calculation;     //!< Skip the computation of the covariance
                                   //!(saves some time) (default=false)
    bool skip_quality_calculation; //!< Skip the (sometimes) expensive
                                   //!evaluation of the term 'quality' at ICP
                                   //!output (Default=true)

    /** Decimation of the point cloud being registered against the reference one
     * (default=5) - set to 1 to have the older (MRPT <0.9.5) behavior
      *  of not approximating ICP by ignoring the correspondence of some points.
     * The speed-up comes from a decimation of the number of KD-tree queries,
  ...
  struct SLAM_IMPEXP TReturnInfo {
    ...
    unsigned short
        nIterations; //!< The number of executed iterations until convergence
    float goodness;  //!< A goodness measure for the alignment, it is a [0,1]
                     //!range indicator of percentage of correspondences.
    float quality;   //!< A measure of the 'quality' of the local minimum of the
                   //!sqr. error found by the method. Higher values are better.
                   //!Low values will be found in ill-conditioned situations
                   //!(e.g. a corridor)
  };

  /** An implementation of CMetricMapsAlignmentAlgorithm for the case of a point
   * maps and a occupancy grid/point map.
   *
   ...
   * derived class or a mrpt::slam::COccupancyGrid2D class)
   * \param m2			[IN] The second map. (MUST BE A mrpt::poses::CPointsMap
   ...
   * \param info			[OUT] A pointer to a CICP::TReturnInfo, or nullptr if
   * it isn't needed.
   *
  ...
  AlignPDF(const mrpt::maps::CMetricMap *m1, const mrpt::maps::CMetricMap *m2,
  ...
  Align3DPDF(const mrpt::maps::CMetricMap *m1, const mrpt::maps::CMetricMap *m2,
...
protected:
  ...
  mrpt::poses::CPosePDF::Ptr
  ICP_Method_Classic(const mrpt::maps::CMetricMap *m1,
                     const mrpt::maps::CMetricMap *m2,
                     const mrpt::poses::CPosePDFGaussian &initialEstimationPDF,
                     TReturnInfo &outInfo);
  mrpt::poses::CPosePDF::Ptr
  ICP_Method_LM(const mrpt::maps::CMetricMap *m1,
                const mrpt::maps::CMetricMap *m2,
                const mrpt::poses::CPosePDFGaussian &initialEstimationPDF,
                TReturnInfo &outInfo);
...
namespace utils {
template <> struct TEnumTypeFiller<slam::TICPAlgorithm> {
...
template <> struct TEnumTypeFiller<slam::TICPCovarianceMethod> {
