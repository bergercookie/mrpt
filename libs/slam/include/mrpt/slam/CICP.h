   ...
   public:
	...
	class SLAM_IMPEXP TConfigParams : public utils::CLoadableOptions
	{
	   public:
		 ...
		 * this
		 * threshold (in meters), iterations are terminated (Default:1e-6) */
		float minAbsStep_trans;
		/** If the correction in all rotation coordinates (yaw,pitch,roll) is
		 * below
		 * this threshold (in radians), iterations are terminated (Default:1e-6)
		 */
		 ...
		 * Fernandez-Madrigal,
		 * "A Robust, Multi-Hypothesis Approach to Matching Occupancy Grid
		 * Maps",
		 ...
		 * landmarks/points
		 * which are being matched (used to compute covariances in the SoG) */
		 ...
		 * estimate
		 * the Jacobians of the square error (default=0.05) */
		 ...
		 * points.
		 * The speed-up comes from a decimation of the number of KD-tree
		 * queries,
	...
	struct SLAM_IMPEXP TReturnInfo
	{
		 ...
		 * found
		 * by the method. Higher values are better. Low values will be found in
		 * ill-conditioned situations (e.g. a corridor) */
		float quality;
	};
