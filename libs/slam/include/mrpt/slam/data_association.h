...
enum TDataAssociationMetric {
  ...
  metricML //!< Matching likelihood (See TDataAssociationMetric for a paper
           //!explaining this metric)
};

typedef size_t
    observation_index_t; //!< Used in mrpt::slam::TDataAssociationResults
typedef size_t
    prediction_index_t; //!< Used in mrpt::slam::TDataAssociationResults

/** The results from mrpt::slam::data_association
  */
struct SLAM_IMPEXP TDataAssociationResults {
  TDataAssociationResults()
      : associations(), distance(0), indiv_distances(0, 0),
        indiv_compatibility(0, 0), indiv_compatibility_counts(),
   ...
   * "Z_observations"), its association in the predictions, as the row index in
   * the "Y_predictions_mean" input (or it's mapping to a custom ID, if it was
   * provided).
  ...
  double distance; //!< The Joint Mahalanobis distance or matching likelihood of
                   //!the best associations found.

  /** Individual mahalanobis distances (or matching likelihood, depending on the
   * selected metric) between predictions (row indices) & observations (column
   * indices).
    *  Indices are for the appearing order in the arguments "Y_predictions_mean"
   * & "Z_observations", they are NOT landmark IDs.
    */
  mrpt::math::CMatrixDouble indiv_distances;
  mrpt::math::CMatrixBool indiv_compatibility; //!< The result of a chi2 test
                                               //!for compatibility using
                                               //!mahalanobis distance - Indices
                                               //!are like in "indiv_distances".
  vector_uint indiv_compatibility_counts;      //!< The sum of each column of
                                          //!indiv_compatibility, that is, the
                                          //!number of compatible pairings for
                                          //!each observation.

  size_t nNodesExploredInJCBB; //!< Only for the JCBB method,the number of
                               //!recursive calls expent in the algorithm.
};
...
namespace utils {
template <> struct TEnumTypeFiller<slam::TDataAssociationMethod> {
...
template <> struct TEnumTypeFiller<slam::TDataAssociationMetric> {
