...
public:
   ...
   * orientation.
   *   The meaning of some parameters and the kind of the maps to be aligned are
   ...
   * \sa CICP
   */
  mrpt::poses::CPosePDF::Ptr Align(const mrpt::maps::CMetricMap *m1,
   ...
   * orientation.
           ...
           *   is returned as a PDF rather than a single value.
           *
           *  \note This method can be configurated with a "options" structure
   * in the implementation classes.
           *
           * \param m1			[IN] The first map (MUST BE A COccupancyGridMap2D
   * derived class)
           * \param m2			[IN] The second map. (MUST BE A CPointsMap
   * derived class) The pose of this map respect to m1 is to be estimated.
           * \param initialEstimationPDF	[IN] An initial gross estimation for
   * the displacement.
           * \param runningTime	[OUT] A pointer to a container for obtaining
   * the algorithm running time in seconds, or nullptr if you don't need it.
   ...
   * nullptr if it isn't needed.
           ...
           * \sa CICP
           */
  virtual mrpt::poses::CPosePDF::Ptr
   ...
   * \sa CICP
   */
  mrpt::poses::CPose3DPDF::Ptr Align3D(const mrpt::maps::CMetricMap *m1,
                                       const mrpt::maps::CMetricMap *m2,
   ...
   * nullptr if it isn't needed.
           ...
           * \sa CICP
           */
  virtual mrpt::poses::CPose3DPDF::Ptr
