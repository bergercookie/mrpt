...
public:
   ...
   * noise".
    * \param robot_z The height of the robot on the floor (default=0). Note that
   * the beacon sensor on the robot may be at a different height, according to
   * data within the observation object.
   ...
   * will be launch, since there is no information to generate samples.
    */
  bool setParams(const mrpt::maps::CLandmarksMap &beaconsMap,
...
protected:
  ...
  /** Returns the NORMALIZED observation likelihood (linear, not exponential!!!)
   * at a given point of the state space (values in the range [0,1]).
    */
  ...
  /** The index in "m_dataPerBeacon" used to draw samples (the rest will be used
  ...
  struct SLAM_IMPEXP TDataPerBeacon {
    TDataPerBeacon()
        : sensorOnRobot(), beaconPosition(), radiusAtRobotPlane(0), minAngle(0),
          maxAngle(0) {}
