...
private:
  ...
  COccupancyGridMapFeatureExtractor
      m_grid_feat_extr; //!< Grid map features extractor
public:
  ...
  public:
    ...
    /** RANSAC-step options:
      ...
      */
    float ransac_minSetSizeRatio; //!< The ratio of landmarks that must be
                                  //!inliers to accepto an hypotheses (typ:
                                  //!0.20)
    float ransac_SOG_sigma_m;     //!< The square root of the uncertainty
                              //!normalization variance var_m (see papers...)
    ...
    double ransac_chi2_quantile; //!< [amModifiedRANSAC method only] The
                                 //!quantile used for chi-square thresholding
                                 //!(default=0.99)

    double ransac_prob_good_inliers; //!< Probability of having a good inliers
                                     //!(def:0,9999), used for automatic number
                                     //!of iterations
    float featsPerSquareMeter; //!< Features extraction from grid map: How many
                               //!features to extract
    float threshold_max; //!< Correspondences are considered if their distances
                         //!are below this threshold (in the range [0,1])
                         //!(default=0.15).
    float threshold_delta; //!< Correspondences are considered if their
                           //!distances to the best match are below this
                           //!threshold (in the range [0,1]) (default=0.15).

    float min_ICP_goodness; //!< The minimum goodness (0-1) of the post-matching
                            //!ICP to accept a hypothesis as good (default=0.30)
    double max_ICP_mahadist; //!< The maximum Mahalanobis distance between the
                             //!initial and final poses in the ICP not to
                             //!discard the hypothesis (default=10)
    double maxKLd_for_merge; //!< Maximum KL-divergence for merging modes of the
                             //!SOG (default=0.9)

    bool save_feat_coors;  //!< DEBUG - Dump all feature correspondences in a
                           //!directory "grid_feats"
    bool debug_show_corrs; //!< DEBUG - Show graphs with the details of each
                           //!feature correspondences
    bool debug_save_map_pairs; //!< DEBUG - Save the pair of maps with all the
                               //!pairings.
  ...
  struct SLAM_IMPEXP TReturnInfo {
    ...
    /** A goodness measure for the alignment, it is a [0,1] range indicator of
     * percentage of correspondences.
     */
    float goodness;

    /** The "brute" estimation from using all the available correspondences,
     * provided just for comparison purposes (it is not the robust estimation,
     * available as the result of the Align method).
      */
    ...
    /** The landmarks of each map (the indices of these landmarks correspond to
     * those in "correspondences")  */
    ...
    std::vector<TPairPlusDistance> correspondences_dists_maha; //!< Mahalanobis
                                                               //!distance for
                                                               //!each potential
                                                               //!correspondence

    std::vector<double> icp_goodness_all_sog_modes; //!< The ICP goodness of all
                                                    //!potential SOG modes at
                                                    //!the stage "sog2", thus
                                                    //!before the removing of
                                                    //!"bad" ICP matches.
  };
   ...
   *mrpt::maps::CMultiMetricMap class)
...
namespace utils {
template <> struct TEnumTypeFiller<slam::CGridMapAligner::TAlignerMethod> {
