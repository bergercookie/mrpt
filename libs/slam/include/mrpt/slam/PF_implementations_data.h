...
protected:
  ...
  mrpt::poses::CPoseRandomSampler
      m_movementDrawer; //!< Used in al PF implementations. \sa
                        //!PF_SLAM_implementation_gatherActionsCheckBothActObs
  mutable mrpt::math::CVectorDouble
      m_pfAuxiliaryPFOptimal_estimatedProb; //!< Auxiliary variable used in the
                                            //!"pfAuxiliaryPFOptimal" algorithm.
  mutable mrpt::math::CVectorDouble
      m_pfAuxiliaryPFStandard_estimatedProb; //!< Auxiliary variable used in the
                                             //!"pfAuxiliaryPFStandard"
                                             //!algorithm.
  mutable mrpt::math::CVectorDouble
      m_pfAuxiliaryPFOptimal_maxLikelihood; //!< Auxiliary variable used in the
                                            //!"pfAuxiliaryPFOptimal" algorithm.
  mutable std::vector<mrpt::math::TPose3D>
      m_pfAuxiliaryPFOptimal_maxLikDrawnMovement; //!< Auxiliary variable used
                                                  //!in the
  ...
  template <class BINTYPE> // Template arg. actually not used, just to allow
                           ...
                           PF_SLAM_particlesEvaluator_AuxPFStandard(
                               const mrpt::bayes::CParticleFilter::
                                   TParticleFilterOptions &PF_options,
...
public:
   ...
   * samples by another new set.
    *  The method uses pointers to make fast copies the first time each particle
   * is duplicated, then
  ...
  virtual void PF_SLAM_implementation_replaceByNewParticleSet(
    ...
    // For efficiency, just copy the "CParticleData" from the old particle into
    // the
    ...
    // Update the particle with the new pose: this part is caller-dependant and
    ...
    // Free those old m_particles not being copied into the new ones: not needed
    // since use of smart ptr.
