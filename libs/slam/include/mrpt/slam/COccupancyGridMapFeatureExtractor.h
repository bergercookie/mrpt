...
public:
  ...
  /** Computes a set of distinctive landmarks from an occupancy grid, and store
   * them (previous content is not erased!) into the given landmarks map.
...
protected:
  void OnEvent(const mrpt::utils::mrptEvent &e); //!< This will receive the
                                                 //!events from maps in order to
                                                 //!purge the cache.
