...
public:
  typedef mrpt::math::TPoint3D
      landmark_point_t; //!< Either mrpt::math::TPoint2D or mrpt::math::TPoint3D
  ...
  void reset(); //!< Reset the state of the SLAM filter: The map is emptied and
   ...
   */
  void processActionObservation(mrpt::obs::CActionCollection::Ptr &action,
                                mrpt::obs::CSensoryFrame::Ptr &SF);

  /** Returns the complete mean and cov.
    *  \param out_robotPose The mean and the 7x7 covariance matrix of the robot
   * 6D pose
   ...
   * out_landmarksPositions) gives the corresponding landmark ID.
    ...
    *  \param out_fullCovariance The full (7+3M)x(7+3M) covariance matrix of the
   * filter.
    * \sa getCurrentRobotPose
    */
  void
  getCurrentState(mrpt::poses::CPose3DQuatPDFGaussian &out_robotPose,
   ...
   * filter.
    ...
    */
  inline void
  getCurrentState(mrpt::poses::CPose3DPDFGaussian &out_robotPose,
                  std::vector<mrpt::math::TPoint3D> &out_landmarksPositions,
                  std::map<unsigned int, mrpt::maps::CLandmark::TLandmarkID>
                      &out_landmarkIDs,
                  mrpt::math::CVectorDouble &out_fullState,
    ...
    this->getCurrentState(q, out_landmarksPositions, out_landmarkIDs,
   ...
   * rotation as a quaternion).
    ...
    */
  void
  getCurrentRobotPose(mrpt::poses::CPose3DQuatPDFGaussian &out_robotPose) const;
   ...
   * rotation as 3 angles).
    ...
    */
  inline void
  getCurrentRobotPose(mrpt::poses::CPose3DPDFGaussian &out_robotPose) const {
    mrpt::poses::CPose3DQuatPDFGaussian q(mrpt::math::UNINITIALIZED_QUATERNION);
  ...
  struct SLAM_IMPEXP TOptions : utils::CLoadableOptions {
    ...
    /** Additional std. dev. to sum to the motion model in the z axis (useful
     * when there is only 2D odometry and we want to put things hard to the
     * algorithm) (default=0) */
    float std_odo_z_additional;

    /** If set to true (default=false), map will be partitioned using the method
     * stated by partitioningMethod */
    ...
    double data_assoc_IC_chi2_thres; //!< Threshold in [0,1] for the chi2square
                                     //!test for individual compatibility
                                     //!between predictions and observations
                                     //!(default: 0.99)
    TDataAssociationMetric data_assoc_IC_metric; //!< Whether to use mahalanobis
                                                 //!(->chi2 criterion) vs.
                                                 //!Matching likelihood.
    double data_assoc_IC_ml_threshold; //!< Only if data_assoc_IC_metric==ML,
                                       //!the log-ML threshold (Default=0.0)

    bool create_simplemap; //!< Whether to fill m_SFs (default=false)

    bool force_ignore_odometry; //!< Whether to ignore the input odometry and
                                //!behave as if there was no odometry at all
                                //!(default: false)
  } options;
  ...
  struct SLAM_IMPEXP TDataAssocInfo {
    ...
    /** Map from the 0-based index within the last observation and the landmark
       0-based index in the map (the robot-map state vector)
  ...
  /** Return the partitioning of the landmarks in clusters accoring to the last
   * partition.
    *  Note that the same landmark may appear in different clusters (the
   * partition is not in the space of landmarks)
    *  Only if options.doPartitioningExperiment = true
    *  \param landmarksMembership The i'th element of this vector is the set of
   * clusters to which the i'th landmark in the map belongs to (landmark index
   * != landmark ID !!).
    * \sa getLastPartition
    */
  ...
  /** For testing only: returns the partitioning as "getLastPartitionLandmarks"
   * but as if a fixed-size submaps (size K) were have been used.
    */
  ...
  /** Save the current state of the filter (robot pose & map) to a MATLAB script
   * which displays all the elements in 2D
    */
...
protected:
   ...
   * landmarks in the map.
    ...
    */
  void
  OnPreComputingPredictions(const vector_KFArray_OBS &in_all_prediction_means,
   ...
   * method if a landmark is actually being added to the map.
    */
  void OnInverseObservationModel(const KFArray_OBS &in_z, KFArray_FEAT &out_yn,
   ...
   * number so data association can be done according to these indices.
    ...
    */
  void OnNewLandmarkAddedToMap(const size_t in_obsIdx,
