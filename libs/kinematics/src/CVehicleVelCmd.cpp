...
std::string mrpt::kinematics::CVehicleVelCmd::asString() const {
  ...
  for (size_t i = 0; i < getVelCmdLength(); i++) {
    s += mrpt::format("%s=%.03f ", getVelCmdDescription(i).c_str(),
                      getVelCmdElement(i));
  }
  s += ")";
  return s;
...
CVehicleVelCmd &CVehicleVelCmd::operator=(const CVehicleVelCmd &other) {
  ...
  ASSERTMSG_(typeid(*this) == typeid(other),
...
void CVehicleVelCmd::TVelCmdParams::loadConfigFile(
  ...
  MRPT_LOAD_HERE_CONFIG_VAR_DEGREES_NO_DEFAULT(robotMax_W_degps, double,
                                               robotMax_W_radps, cfg, section);
...
void CVehicleVelCmd::TVelCmdParams::saveToConfigFile(
  ...
  MRPT_SAVE_CONFIG_VAR_COMMENT(robotMax_V_mps,
                               "Max. linear speed (m/s) [Default=-1 (not set), "
                               "will raise exception if needed and not set]");
  MRPT_SAVE_CONFIG_VAR_DEGREES_COMMENT("robotMax_W_degps", robotMax_W_radps,
                                       "Max. angular speed (deg/s) [Default=-1 "
                                       "(not set), will raise exception if "
                                       "needed and not set]");
  MRPT_SAVE_CONFIG_VAR_COMMENT(robotMinCurvRadius,
                               "Min. radius of curvature of paths (m) "
