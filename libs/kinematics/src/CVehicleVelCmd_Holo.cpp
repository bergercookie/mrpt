...
CVehicleVelCmd_Holo::CVehicleVelCmd_Holo()
    : vel(.0), dir_local(.0), ramp_time(.0), rot_speed(.0) {}

CVehicleVelCmd_Holo::CVehicleVelCmd_Holo(double vel, double dir_local,
                                         double ramp_time, double rot_speed)
    : vel(vel), dir_local(dir_local), ramp_time(ramp_time),
      rot_speed(rot_speed) {}
...
double CVehicleVelCmd_Holo::cmdVel_limits(
  ...
  ASSERTMSG_(params.robotMax_V_mps >= .0,
             "[CVehicleVelCmd_Holo] `robotMax_V_mps` must be set to valid "
             "values: either assign values programatically or call "
             "loadConfigFile()");

  double f = 1.0;
  if (vel > params.robotMax_V_mps)
    f = params.robotMax_V_mps / vel;

  vel *= f;       // |(vx,vy)|
  rot_speed *= f; // rot_speed
  // ramp_time: leave unchanged
  // Blending with "beta" not required, since the ramp_time already blends cmds
  // for holo robots.
