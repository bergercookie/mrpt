...
IMPLEMENTS_SERIALIZABLE(CVehicleVelCmd_DiffDriven, CVehicleVelCmd,
                        mrpt::kinematics)
...
std::string
CVehicleVelCmd_DiffDriven::getVelCmdDescription(const int index) const {
  switch (index) {
  case 0:
    return "lin_vel";
    break;
...
double CVehicleVelCmd_DiffDriven::cmdVel_limits(
  ...
  } else // there is a non-zero translational component
  {
    ...
    lin_vel =
        beta * lin_vel + (1 - beta) * prevcmd->lin_vel; // blend new v value
    ang_vel =
...
double CVehicleVelCmd_DiffDriven::filter_max_vw(double &v, double &w,
