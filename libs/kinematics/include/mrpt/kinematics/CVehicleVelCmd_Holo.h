...
public:
  ...
  double dir_local; //!<: direction, **relative** to the current robot heading
                    //!(radians). 0 means forward.
  double ramp_time; //!<: Blending time between current and target time.
  double rot_speed; //!<: (rad/s) rotational speed for rotating such as the
                    //!robot slowly faces forward.

  CVehicleVelCmd_Holo();
  CVehicleVelCmd_Holo(double vel, double dir_local, double ramp_time,
                      double rot_speed);
  ...
  double cmdVel_limits(const mrpt::kinematics::CVehicleVelCmd &prev_vel_cmd,
                       const double beta, const TVelCmdParams &params) override;
};
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CVehicleVelCmd_Holo,
                                             CVehicleVelCmd, KINEMATICS_IMPEXP)
