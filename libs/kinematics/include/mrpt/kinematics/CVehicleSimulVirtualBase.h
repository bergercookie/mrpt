...
public:
  ...
  /** Returns the current pose according to (noisy) odometry \sa
   * setOdometryErrors */
  ...
  template <typename T> void setCurrentOdometricPose(const T &pose) {
    m_odometry = mrpt::math::TPose2D(pose);
  }
  ...
  const mrpt::math::TTwist2D &getCurrentOdometricVel() const {
    return m_odometric_vel;
  }
  /** Returns the instantaneous, odometric velocity vector (vx,vy,omega) in the
   * robot local frame */
  ...
  /** Sends a velocity command to the robot. The number of components and their
   * meaning depends
    * on the vehicle-kinematics derived class */
  virtual void sendVelCmd(const CVehicleVelCmd &cmd_vel) = 0;
  /** Gets an empty velocity command object that can be queried to find out the
   * number of velcmd components,... */
  virtual CVehicleVelCmd::Ptr getVelCmdType() const = 0;

  /** Enable/Disable odometry errors. Errors in odometry are 1 sigma Gaussian
   * values per second */
  void setOdometryErrors(bool enabled, double Ax_err_bias = 1e-3,
                         double Ax_err_std = 10e-3, double Ay_err_bias = 1e-3,
...
protected:
  ...
  /** @} */
  double m_firmware_control_period; //!< The period at which the low-level
                                    //!controller updates velocities (Default:
                                    //!0.5 ms)
