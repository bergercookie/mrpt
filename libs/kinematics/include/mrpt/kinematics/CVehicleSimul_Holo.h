...
public:
   ...
   *   \param[in] rot_speed Rotational speed while there is heading error and to
   * this constant (rad/s)
   */
  void sendVelRampCmd(double vel, double dir, double ramp_time,
                      double rot_speed);
...
private:
   ...
   *  @{ */
  struct TVelRampCmd {
    double issue_time; //!< time when the cmd was issued. (<0: invalid, means
                       //!there are no pending cmds to execute)
