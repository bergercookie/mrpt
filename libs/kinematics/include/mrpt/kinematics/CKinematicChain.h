...
struct KINEMATICS_IMPEXP TKinematicLink {
  ...
  double a; //!< Distance along the common normal (in the same direction than
            //!the new X_{i+1})
  double alpha; //!< Rotation along X_{i+1} to transform Z_i into Z_{i+1}

  bool is_prismatic; //!< "false": Is revolute ("q_i" is "theta"), "true": is
                     //!prismatic ("q_i" is "d")

  TKinematicLink(double _theta, double _d, double _a, double _alpha,
                 bool _is_prismatic)
      : theta(_theta), d(_d), a(_a), alpha(_alpha),
        is_prismatic(_is_prismatic) {}
  TKinematicLink() : theta(0), d(0), a(0), alpha(0), is_prismatic(false) {}
};

KINEMATICS_IMPEXP mrpt::utils::CStream &operator>>(mrpt::utils::CStream &in,
                                                   TKinematicLink &o);
KINEMATICS_IMPEXP mrpt::utils::CStream &operator<<(mrpt::utils::CStream &out,
...
private:
  mutable std::vector<mrpt::opengl::CRenderizable::Ptr>
      m_last_gl_objects; //!< Smart pointers to the last objects for each link,
                         //!as returned in getAs3DObject(), for usage within
                         //!update3DObject()
...
public:
  ...
  void addLink(double theta, double d, double a, double alpha,
               bool is_prismatic);
  ...
  /** Can be used to define a first degree of freedom along a +Z axis which does
   * not coincide with the global +Z axis. */
   ...
   * values, which
    * are interpreted as rotations (radians) or displacements (meters) depending
   * on links being "revolute" or "prismatic".
   ...
   * in the format of recomputeAllPoses()
    * \sa update3DObject
    */
  void getAs3DObject(mrpt::opengl::CSetOfObjects::Ptr &inout_gl_obj,
   ...
   * in the format of recomputeAllPoses()
    */
  void update3DObject(mrpt::aligned_containers<mrpt::poses::CPose3D>::vector_t
                          *out_all_poses = nullptr) const;

  /** Go thru all the links of the chain and compute the global pose of each
   * link. The "ground" link pose "pose0" defaults to the origin of coordinates,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CKinematicChain,
