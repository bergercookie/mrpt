...
/** The base class of 3D objects that can be directly rendered through OpenGL.
  ...
  *mainly:
  *		- A name (m_name): A name that can be optionally asigned to objects
  *for easing its reference.
...
protected:
  ...
  mrpt::poses::CPose3D m_pose; //!< 6D pose wrt the parent coordinate reference.
                               //!This class automatically holds the cached 3x3
                               //!rotation matrix for quick load into opengl
                               //!stack.
  float m_scale_x, m_scale_y,
      m_scale_z;  //!< Scale components to apply to the object (default=1)
...
public:
  ...
  CRenderizable &setPose(const mrpt::poses::CPose3D &o); //!< Set the 3D pose
                                                         //!from a
                                                         //!mrpt::poses::CPose3D
                                                         //!object (return a ref
                                                         //!to this)
  CRenderizable &setPose(const mrpt::poses::CPose2D &o); //!< Set the 3D pose
                                                         //!from a
                                                         //!mrpt::poses::CPose3D
                                                         //!object (return a ref
                                                         //!to this)
  CRenderizable &setPose(const mrpt::math::TPose3D &o);  //!< Set the 3D pose
                                                         //!from a
                                                         //!mrpt::math::TPose3D
                                                         //!object (return a ref
                                                         //!to this)
  CRenderizable &setPose(const mrpt::math::TPose2D &o);  //!< Set the 3D pose
                                                         //!from a
                                                         //!mrpt::math::TPose3D
                                                         //!object (return a ref
                                                         //!to this)
  CRenderizable &
  setPose(const mrpt::poses::CPoint3D &o); //!< Set the 3D pose from a
                                           //!mrpt::poses::CPose3D object
                                           //!(return a ref to this)
  CRenderizable &
  setPose(const mrpt::poses::CPoint2D &o); //!< Set the 3D pose from a
                                           //!mrpt::poses::CPose3D object
                                           //!(return a ref to this)

  mrpt::math::TPose3D
  getPose() const; //!< Returns the 3D pose of the object as TPose3D
  ...
  inline double getPoseX() const {
    return m_pose.x();
  } //!< Translation relative to parent coordinate origin.
  inline double getPoseY() const {
    return m_pose.y();
  } //!< Translation relative to parent coordinate origin.
  inline double getPoseZ() const {
    return m_pose.z();
  } //!< Translation relative to parent coordinate origin.
  ...
  inline double getPoseYawRad() const {
    return m_pose.yaw();
  } //!< Rotation relative to parent coordinate origin, in radians.
  inline double getPosePitchRad() const {
    return m_pose.pitch();
  } //!< Rotation relative to parent coordinate origin, in radians.
  inline double getPoseRollRad() const {
    return m_pose.roll();
  } //!< Rotation relative to parent coordinate origin, in radians.

  inline double getColorR() const {
    return m_color.R / 255.;
  } //!< Color components in the range [0,1]
  inline double getColorG() const {
    return m_color.G / 255.;
  } //!< Color components in the range [0,1]
  inline double getColorB() const {
    return m_color.B / 255.;
  } //!< Color components in the range [0,1]
  inline double getColorA() const {
    return m_color.A / 255.;
  } //!< Color components in the range [0,1]

  inline uint8_t getColorR_u8() const {
    return m_color.R;
  } //!< Color components in the range [0,255]
  inline uint8_t getColorG_u8() const {
    return m_color.G;
  } //!< Color components in the range [0,255]
  inline uint8_t getColorB_u8() const {
    return m_color.B;
  } //!< Color components in the range [0,255]
  inline uint8_t getColorA_u8() const {
    return m_color.A;
  } //!< Color components in the range [0,255]
  ...
  CRenderizable &
  setColor(const mrpt::utils::TColorf
               &c) //!< Changes the default object color \return a ref to this
  {
    return setColor_u8(mrpt::utils::TColor(c.R * 255.f, c.G * 255.f,
                                           c.B * 255.f, c.A * 255.f));
  }
  ...
  inline const mrpt::utils::TColor &getColor_u8() const {
    return m_color;
  ...
  /** Simulation of ray-trace, given a pose. Returns true if the ray effectively
   * collisions with the object (returning the distance to the origin of the ray
   * in "dist"), or false in other case. "dist" variable yields undefined
   * behaviour when false is returned
    */
   ...
   * by renderTextBitmap().
    ...
    */
  static int textBitmapWidth(const std::string &str,
   ...
   * numbers in the range [0,1].
    ...
    */
  static void
  renderTextBitmap(int screen_x, int screen_y, const std::string &str,
                   float color_r = 1, float color_g = 1, float color_b = 1,
                   mrpt::opengl::TOpenGLFont font =
                       mrpt::opengl::MRPT_GLUT_BITMAP_TIMES_ROMAN_24);

  /** Evaluates the bounding box of this object (including possible children) in
   * the coordinate frame of the object parent. */
  virtual void getBoundingBox(mrpt::math::TPoint3D &bb_min,
                              mrpt::math::TPoint3D &bb_max) const = 0;
...
using CListOpenGLObjects = std::deque<CRenderizable::Ptr>
    ...
    DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CRenderizable,
 ...
 * with the existing one.
  * \sa setPose */
OPENGL_IMPEXP CRenderizable::Ptr &operator<<(CRenderizable::Ptr &r,
