...
public:
  ...
  /**  Loads a scene from a file in any supported file.
    * \exception std::runtime_error On any error during loading or importing the
   * file.
    */
  ...
  struct TInfoPerTexture {
    size_t id_idx; //!< indices in \a m_textureIds. string::npos for
                   //!non-initialized ones.
    mrpt::utils::CImage::Ptr img_rgb, img_alpha;
    TInfoPerTexture() : id_idx(std::string::npos) {}
  };

  CAssimpModel();
  virtual ~CAssimpModel(); //!< Private, virtual destructor: only can be deleted
                           //!from smart pointers
private:
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CAssimpModel,
