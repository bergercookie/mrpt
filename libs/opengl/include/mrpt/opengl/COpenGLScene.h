...
public:
  ...
  /**
    * Inserts a set of objects into the scene, in the given viewport ("main" by
   * default). Any iterable object will be accepted.
  ...
  /** Returns the viewport with the given name, or nullptr if it does not exist;
   * note that the default viewport is named "main" and initially occupies the
   * entire rendering area.
    */
  COpenGLViewport::Ptr
  getViewport(const std::string &viewportName = std::string("main")) const;
  ...
  /** If disabled (default), the SceneViewer application will ignore the camera
   * of the "main" viewport and keep the viewport selected by the user by hand;
   * otherwise, the camera in the "main" viewport prevails.
    * \sa followCamera
    */
   ...
   * pointer) if not found.
    */
  CRenderizable::Ptr
  getByName(const std::string &str,
            const std::string &viewportName = std::string("main"));
  ...
  inline void visitAllObjects(FUNCTOR functor,
                              const EXTRA_PARAM &userParam) const {
