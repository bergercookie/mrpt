...
public:
  ...
  void getPlaneLimits(float &xmin, float &xmax, float &ymin,
                      float &ymax) const {
    xmin = m_xMin;
    xmax = m_xMax;
    ymin = m_yMin;
  ...
  /** Class factory  */
  static CGridPlaneXY::Ptr Create(float xMin, float xMax, float yMin,
                                  float yMax, float z = 0, float frequency = 1,
                                  float lineWidth = 1.3f,
                                  bool antiAliasing = true);

  /** Constructor  */
  CGridPlaneXY(float xMin = -10, float xMax = 10, float yMin = -10,
               float yMax = 10, float z = 0, float frequency = 1,
               float lineWidth = 1.3f, bool antiAliasing = true);

  /** Private, virtual destructor: only can be deleted from smart pointers */
  virtual ~CGridPlaneXY() {}
};
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CGridPlaneXY,
