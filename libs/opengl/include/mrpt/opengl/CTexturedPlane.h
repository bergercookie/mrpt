...
protected:
  ...
  mutable std::vector<mrpt::math::TPolygonWithPlane>
      tmpPoly; //!< Used for ray-tracing
...
public:
  ...
  void setTextureCornerCoords(float tex_x_min, float tex_x_max, float tex_y_min,
                              float tex_y_max) {
   ...
   * plane. */
  inline void getPlaneCorners(float &xMin, float &xMax, float &yMin,
                              float &yMax) const {
    xMin = m_xMin;
    xMax = m_xMax;
    yMin = m_yMin;
  ...
  /** Class factory  */
  static CTexturedPlane::Ptr Create(float x_min, float x_max, float y_min,
                                    float y_max);
  ...
  /** Constructor
    */
  CTexturedPlane(float x_min = -1, float x_max = 1, float y_min = -1,
                 float y_max = 1);

  /** Private, virtual destructor: only can be deleted from smart pointers */
  virtual ~CTexturedPlane();
};
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CTexturedPlane, CTexturedObject,
                                             OPENGL_IMPEXP)
