...
CSetOfObjects::Ptr
graph_visualize(const GRAPH_T &g,
  ...
  if (show_ground_grid) {
    ...
    for (typename GRAPH_T::global_poses_t::const_iterator itNod =
      ...
      const CPose3D p =
          CPose3D(itNod->second); // Convert to 3D from whatever its real type.
    ...
    CGridPlaneXY::Ptr grid = std::make_shared<CGridPlaneXY>(
  ...
  if (show_edge_rel_poses) {
    ...
    for (typename GRAPH_T::const_iterator itEd = g.begin(); itEd != g.end();
      ...
      const TNodeID node_id_start =
          g.edges_store_inverse_poses ? itEd->first.second : itEd->first.first;
      ...
      if (itNod != g.nodes.end()) {
        ...
        mrpt::opengl::CSimpleLine::Ptr gl_line =
            std::make_shared<mrpt::opengl::CSimpleLine>(
                0, 0, 0, edge_pose_pt.x(), edge_pose_pt.y(), edge_pose_pt.z());
