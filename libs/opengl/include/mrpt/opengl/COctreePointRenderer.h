 ...
 * http://www.mrpt.org/Efficiently_rendering_point_clouds_of_millions_of_points
  * \ingroup mrpt_opengl_grp
  */
template <class Derived> class COctreePointRenderer {
public:
  /** Default ctor */
  COctreePointRenderer()
      : m_octree_has_to_rebuild_all(true), m_visible_octree_nodes(0),
...
protected:
  ...
  void octree_render(const mrpt::opengl::gl_utils::TRenderInfo &ri) const {
    ...
    for (size_t i = 0; i < m_render_queue.size(); i++) {
      ...
      octree_derived().render_subset(node.all, node.pts,
                                     m_render_queue[i].render_area_sqpixels);
    }
  }

  void octree_getBoundingBox(mrpt::math::TPoint3D &bb_min,
...
private:
  /** The structure for each octree spatial node. Each node can either be a leaf
   * of has 8 children nodes.
  ...
  struct OPENGL_IMPEXP TNode {
    TNode()
        : bb_min(std::numeric_limits<float>::max(),
                 ...
                 std::numeric_limits<float>::max()),
          bb_max(-std::numeric_limits<float>::max(),
                 -std::numeric_limits<float>::max(),
                 -std::numeric_limits<float>::max()) {}

    bool is_leaf; //!< true: it's a leaf and \a pts has valid indices; false: \a
                  //!children is valid.
    ...
    std::vector<size_t>
        pts;  //!< Point indices in the derived class that fall into this node.
    bool all; //!< true: All elements in the reference object; false: only those
              //!in \a pts

    // Fields used if is_leaf=false
    mrpt::math::TPoint3Df center; //!< [is_leaf=false] The center of the node,
                                  //!whose coordinates are used to decide
                                  //!between the 8 children nodes.
    size_t child_id[8]; //!< [is_leaf=false] The indices in \a m_octree_nodes of
                        //!the 8 children.
  ...
  struct OPENGL_IMPEXP TRenderQueueElement {
    ...
    float render_area_sqpixels; //!< The approximate size of the octree on the
                                //!screen (squared pixels).
  };
  mutable std::vector<TRenderQueueElement> m_render_queue; //!< The list of
                                                           //!elements that
                                                           //!really are visible
                                                           //!and will be
                                                           //!rendered.
  ...
  typename mrpt::aligned_containers<TNode>::deque_t
      m_octree_nodes; //!< First one [0] is always the root node
  ...
  /** Render a given node. */
  void octree_recursive_render(size_t node_idx,
    ...
    if (!corners_are_all_computed) {
      for (int i = 0; i < 8; i++) {
        // project point:
        ri.projectPointPixels(node.getCornerX(i), node.getCornerY(i),
                              node.getCornerZ(i), cr_px[i].x, cr_px[i].y,
                              cr_z[i]);
      }
    }

    mrpt::utils::TPixelCoordf px_min(std::numeric_limits<float>::max(),
                                     std::numeric_limits<float>::max()),
        px_max(-std::numeric_limits<float>::max(),
               -std::numeric_limits<float>::max());
    if (!trust_me_youre_visible) {
      ...
      // If all 8 corners are way out of the screen (and all "cr_z" have the
      // same sign),
    ...
    if (node.is_leaf) { // Render this leaf node:
    ...
    } else { // Render children nodes:
      // If ALL my 8 corners are within the screen, tell our children that they
      //  won't need to compute anymore, since all of them and their children
      //  are visible as well:
      ...
      if (!trust_me_youre_visible) // Trust my parent... otherwise:
      {
        for (int i = 0; i < 8; i++) {
          if (!(cr_px[i].x >= 0 && cr_px[i].y >= 0 &&
                cr_px[i].x < ri.vp_width && cr_px[i].y < ri.vp_height)) {
      ...
      if (children_are_all_visible_for_sure) {
        ...
        const float approx_child_area =
            trust_me_youre_visible ? approx_area_sqpixels / 8.0f
                                   : std::abs(px_min.x - px_max.x) *
                                         std::abs(px_min.y - px_max.y) / 8.0f;

        for (int i = 0; i < 8; i++)
          this->octree_recursive_render(node.child_id[i], ri, child_cr_px,
                                        child_cr_z, true, true,
                                        approx_child_area);
      } else {
#ifdef __clang__
...
#pragma clang diagnostic ignored "-Wunused-variable"
        ...
        // Precompute the 19 (3*9-8) intermediary points so children don't have
        // to compute them several times:
        const mrpt::math::TPoint3Df p_Xm_Ym_Zm(node.bb_min.x, node.bb_min.y,
                                               node.bb_min.z); // 0
        const mrpt::math::TPoint3Df p_X0_Ym_Zm(node.center.x, node.bb_min.y,
                                               node.bb_min.z);
        const mrpt::math::TPoint3Df p_Xp_Ym_Zm(node.bb_max.x, node.bb_min.y,
                                               node.bb_min.z); // 1
        const mrpt::math::TPoint3Df p_Xm_Y0_Zm(node.bb_min.x, node.center.y,
                                               node.bb_min.z);
        const mrpt::math::TPoint3Df p_X0_Y0_Zm(node.center.x, node.center.y,
                                               node.bb_min.z);
        const mrpt::math::TPoint3Df p_Xp_Y0_Zm(node.bb_max.x, node.center.y,
                                               node.bb_min.z);
        const mrpt::math::TPoint3Df p_Xm_Yp_Zm(node.bb_min.x, node.bb_max.y,
                                               node.bb_min.z); // 2
        const mrpt::math::TPoint3Df p_X0_Yp_Zm(node.center.x, node.bb_max.y,
                                               node.bb_min.z);
        const mrpt::math::TPoint3Df p_Xp_Yp_Zm(node.bb_max.x, node.bb_max.y,
                                               node.bb_min.z); // 3

        const mrpt::math::TPoint3Df p_Xm_Ym_Z0(node.bb_min.x, node.bb_min.y,
                                               node.center.z);
        const mrpt::math::TPoint3Df p_X0_Ym_Z0(node.center.x, node.bb_min.y,
                                               node.center.z);
        const mrpt::math::TPoint3Df p_Xp_Ym_Z0(node.bb_max.x, node.bb_min.y,
                                               node.center.z);
        const mrpt::math::TPoint3Df p_Xm_Y0_Z0(node.bb_min.x, node.center.y,
                                               node.center.z);
        const mrpt::math::TPoint3Df p_X0_Y0_Z0(node.center.x, node.center.y,
                                               node.center.z);
        const mrpt::math::TPoint3Df p_Xp_Y0_Z0(node.bb_max.x, node.center.y,
                                               node.center.z);
        const mrpt::math::TPoint3Df p_Xm_Yp_Z0(node.bb_min.x, node.bb_max.y,
                                               node.center.z);
        const mrpt::math::TPoint3Df p_X0_Yp_Z0(node.center.x, node.bb_max.y,
                                               node.center.z);
        const mrpt::math::TPoint3Df p_Xp_Yp_Z0(node.bb_max.x, node.bb_max.y,
                                               node.center.z);

        const mrpt::math::TPoint3Df p_Xm_Ym_Zp(node.bb_min.x, node.bb_min.y,
                                               node.bb_max.z); // 4
        const mrpt::math::TPoint3Df p_X0_Ym_Zp(node.center.x, node.bb_min.y,
                                               node.bb_max.z);
        const mrpt::math::TPoint3Df p_Xp_Ym_Zp(node.bb_min.x, node.bb_min.y,
                                               node.bb_max.z); // 5
        const mrpt::math::TPoint3Df p_Xm_Y0_Zp(node.bb_min.x, node.center.y,
                                               node.bb_max.z);
        const mrpt::math::TPoint3Df p_X0_Y0_Zp(node.center.x, node.center.y,
                                               node.bb_max.z);
        const mrpt::math::TPoint3Df p_Xp_Y0_Zp(node.bb_max.x, node.center.y,
                                               node.bb_max.z);
        const mrpt::math::TPoint3Df p_Xm_Yp_Zp(node.bb_min.x, node.bb_max.y,
                                               node.bb_max.z); // 6
        const mrpt::math::TPoint3Df p_X0_Yp_Zp(node.center.x, node.bb_max.y,
                                               node.bb_max.z);
        const mrpt::math::TPoint3Df p_Xp_Yp_Zp(node.bb_max.x, node.bb_max.y,
                                               node.bb_max.z); // 7
...
#define PROJ_SUB_NODE(POSTFIX)                                                 \
  ...
  ri.projectPointPixels(p_##POSTFIX.x, p_##POSTFIX.y, p_##POSTFIX.z,           \
                        px_##POSTFIX.x, px_##POSTFIX.y, depth_##POSTFIX);
...
#define DO_RECURSE_CHILD(INDEX, SEQ0, SEQ1, SEQ2, SEQ3, SEQ4, SEQ5, SEQ6,      \
                         SEQ7)                                                 \
  {                                                                            \
    ...
    this->octree_recursive_render(node.child_id[INDEX], ri, child_cr_px,       \
                                  child_cr_z);                                 \
  }

        //                     0         1         2         3          4
        //                     5        6         7
        DO_RECURSE_CHILD(0, Xm_Ym_Zm, X0_Ym_Zm, Xm_Y0_Zm, X0_Y0_Zm, Xm_Ym_Z0,
                         X0_Ym_Z0, Xm_Y0_Z0, X0_Y0_Z0)
        DO_RECURSE_CHILD(1, X0_Ym_Zm, Xp_Ym_Zm, X0_Y0_Zm, Xp_Y0_Zm, X0_Ym_Z0,
                         Xp_Ym_Z0, X0_Y0_Z0, Xp_Y0_Z0)
        DO_RECURSE_CHILD(2, Xm_Y0_Zm, X0_Y0_Zm, Xm_Yp_Zm, X0_Yp_Zm, Xm_Y0_Z0,
                         X0_Y0_Z0, Xm_Yp_Z0, X0_Yp_Z0)
        DO_RECURSE_CHILD(3, X0_Y0_Zm, Xp_Y0_Zm, X0_Yp_Zm, Xp_Yp_Zm, X0_Y0_Z0,
                         Xp_Y0_Z0, X0_Yp_Z0, Xp_Yp_Z0)
        DO_RECURSE_CHILD(4, Xm_Ym_Z0, X0_Ym_Z0, Xm_Y0_Z0, X0_Y0_Z0, Xm_Ym_Zp,
                         X0_Ym_Zp, Xm_Y0_Zp, X0_Y0_Zp)
        DO_RECURSE_CHILD(5, X0_Ym_Z0, Xp_Ym_Z0, X0_Y0_Z0, Xp_Y0_Z0, X0_Ym_Zp,
                         Xp_Ym_Zp, X0_Y0_Zp, Xp_Y0_Zp)
        DO_RECURSE_CHILD(6, Xm_Y0_Z0, X0_Y0_Z0, Xm_Yp_Z0, X0_Yp_Z0, Xm_Y0_Zp,
                         X0_Y0_Zp, Xm_Yp_Zp, X0_Yp_Zp)
        DO_RECURSE_CHILD(7, X0_Y0_Z0, Xp_Y0_Z0, X0_Yp_Z0, Xp_Yp_Z0, X0_Y0_Zp,
...
#pragma clang diagnostic pop
  ...
  void internal_octree_assure_uptodate() {
    if (!m_octree_has_to_rebuild_all)
      return;
  ...
  //  will only happen
  //  for the root node)
  void internal_recursive_split(const size_t node_id,
    ...
    if (N <= mrpt::global_settings::OCTREE_RENDER_MAX_POINTS_PER_NODE) {
        ...
        if (p.z < c.z) {
          if (p.y < c.y) {
            if (p.x < c.x)
              ...
              m_octree_nodes[children_idx_base + 1].pts.push_back(i);
          } else {
            if (p.x < c.x)
              m_octree_nodes[children_idx_base + 2].pts.push_back(i);
            else
              m_octree_nodes[children_idx_base + 3].pts.push_back(i);
          }
        } else {
          if (p.y < c.y) {
            if (p.x < c.x)
              ...
              m_octree_nodes[children_idx_base + 5].pts.push_back(i);
          } else {
            if (p.x < c.x)
              m_octree_nodes[children_idx_base + 6].pts.push_back(i);
            else
              m_octree_nodes[children_idx_base + 7].pts.push_back(i);
          }
        }
      }
      ...
      // Clear list of elements (they're now in our children):
      {
        ...
        node.pts.swap(
            emptyVec); // This is THE way of really clearing a std::vector
      }
...
public:
   ...
   * \sa octree_get_nonempty_node_count */
   ...
   * rebuild the entire node tree (for example, after modifying the point cloud
   * or any global octree parameter) */
  inline void octree_mark_as_outdated() { m_octree_has_to_rebuild_all = true; }

  /** Returns a graphical representation of all the bounding boxes of the octree
   * (leaf) nodes.
  ...
  void octree_get_graphics_boundingboxes(
    ...
    for (size_t i = 0; i < m_octree_nodes.size(); i++) {
      ...
      gl_box->setBoxCorners(mrpt::math::TPoint3D(node.bb_min),
