...
protected:
  ...
  bool m_projectiveModel;   //!< If set to true (default), camera model is
                            //!projective, otherwise, it's orthogonal.
  float m_projectiveFOVdeg; //!< Field-of-View in degs, only when
                            //!projectiveModel=true (default=30 deg).
  bool m_6DOFMode; //!< If set to true, camera pose is used when rendering the
                   //!viewport
...
public:
  ...
  template <class POSEORPOINT> void setPointingAt(const POSEORPOINT &p) {
    m_pointingX = p.x();
    m_pointingY = p.y();
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CCamera, CRenderizable,
                                             OPENGL_IMPEXP)
