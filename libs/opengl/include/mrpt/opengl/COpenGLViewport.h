...
/** A viewport within a COpenGLScene, containing a set of OpenGL objects to
  ...
  *setCloneView()
  *		- Image mode: It renders an image (e.g. from a video stream)
  *efficiently using a textued quad. See \a setImageView().
  *
...
public:
  ...
  /** Set this viewport as a clone of some other viewport, given its name - as a
   * side effect, current list of internal OpenGL objects is cleared.
  ...
  /** If set to true, and setCloneView() has been called, this viewport will be
   * rendered using the camera of the cloned viewport.
    */
  ...
  /** Change the viewport position and dimension on the rendering window.
    ...
    *in a factor of the width/height.
    *		- If in [-1,0[, the size is computed such as the right/top border
    *ends up in the given coordinate, interpreted as a factor (e.g. -1: up to
    *the end of the viewport, -0.5: up to the middle of it).
    *		- If <-1 the size is computed such as the right/top border ends up in
    ...
    */
  void setViewportPosition(const double x, const double y, const double width,
                           const double height);
   ...
   * allow the user to pick and select 3D objects by clicking onto the 2D image.
    *  \param x_coord Horizontal coordinate with the usual meaning (0:left of
   * the viewport, W-1: right border).
    *  \param y_coord Horizontal coordinate with the usual meaning (0:top of the
   * viewport, H-1: right border).
   ...
   * viewports do not extend to the whole window size.
    ...
    */
  void
  get3DRayForPixelCoord(const double x_coord, const double y_coord,
  ...
  /** Insert a new object into the list.
    *  The object MUST NOT be deleted, it will be deleted automatically by this
   * object when not required anymore.
    */
    ...
    for (CListOpenGLObjects::const_iterator it = m_objects.begin();
      ...
      if (*it &&
        ...
        typename T::Ptr o = std::dynamic_pointer_cast<T>(
            std::dynamic_pointer_cast<CSetOfObjects>(*it)->getByClass<T>(ith));
        if (o)
          return o;
      }
    }
...
protected:
  /** Constructor, invoked from COpenGLScene only.
    */
  COpenGLViewport(COpenGLScene *parent = nullptr,
  ...
  utils::safe_ptr<COpenGLScene>
      m_parent;                 //!< The scene that contains this viewport.
  ...
  double m_view_x, m_view_y, m_view_width,
      m_view_height; //!< The viewport position [0,1]
  double m_clip_min,
      m_clip_max; //!< The min/max clip depth distances (default: 0.1 - 10000)
  bool m_custom_backgb_color;
  mrpt::utils::TColorf
      m_background_color; //!< used only if m_custom_backgb_color
  ...
  mrpt::utils::CImage::Ptr m_imageview_img; //!< The image to display, after
                                            //!calling \a setImageView()

  struct TLastProjectiveMatrixInfo {
    TLastProjectiveMatrixInfo()
        : eye(0, 0, 0), pointing(0, 0, 0), up(0, 0, 0), viewport_width(640),
          viewport_height(480), FOV(30), azimuth(0), elev(0), zoom(1),
          is_projective(true) {}
    ...
    size_t viewport_width, viewport_height; //!< In pixels. This may be smaller
                                            //!than the total render window.
    float FOV;                              //!< FOV in degrees.
    float azimuth, elev; //!< Camera elev & azimuth, in radians.
    float zoom;
    bool is_projective; // true: projective, false: ortho
  };
  mutable TLastProjectiveMatrixInfo m_lastProjMat; //!< Info updated with each
                                                   //!"render()" and used in
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(COpenGLViewport,
...
/**
  ...
  */
inline COpenGLViewport::Ptr &operator<<(COpenGLViewport::Ptr &s,
 ...
 * chaining.
  * \sa mrpt::opengl::COpenGLViewport::insert
  */
inline COpenGLViewport::Ptr &
operator<<(COpenGLViewport::Ptr &s, const std::vector<CRenderizable::Ptr> &v) {
