...
public:
  ...
  /**
    * Ray Tracing. Will always return false, since objects of this class are not
   * intended to have geometric properties.
    * \sa mrpt::opengl::CRenderizable
    */
   ...
   * openGL flags.
    * \throw std::logic_error if the number of vertices is not an exact multiple
   * of the chunk size.
    */
  static COpenGLStandardObject::Ptr
  Create(_GLENUM t, const std::vector<mrpt::math::TPoint3D> &v, uint32_t cs = 0,
  ...
  /**
    ...
    */
  COpenGLStandardObject(_GLENUM t, const std::vector<mrpt::math::TPoint3D> &v,
                        uint32_t cs, const std::vector<_GLENUM> &en)
  ...
  COpenGLStandardObject()
      : type(0), vertices(std::vector<mrpt::math::TPoint3D>(0)), chunkSize(0),
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(COpenGLStandardObject,
