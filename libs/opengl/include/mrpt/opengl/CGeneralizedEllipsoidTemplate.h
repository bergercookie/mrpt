...
public:
  typedef mrpt::math::CMatrixFixedNumeric<double, DIM, DIM>
      cov_matrix_t; //!< The type of fixed-size covariance matrices for this
                    //!representation
  typedef mrpt::math::CMatrixFixedNumeric<double, DIM, 1>
      mean_vector_t; //!< The type of fixed-size vector for this representation
   ...
   * ellipsoid - Notice that the
    *  covariance determines the uncertainty in the parameter space, which would
   * be transformed by derived function
    */
  template <typename MATRIX, typename VECTOR>
  void setCovMatrixAndMean(const MATRIX &new_cov, const VECTOR &new_mean) {
    MRPT_START
    ASSERT_(new_cov.getColCount() == new_cov.getRowCount() &&
  ...
  /** Set the number of segments of the surface/curve (higher means with greater
   * resolution) */
  ...
  void render_dl() const override {
    ...
    if (m_needToRecomputeEigenVals) {
      ...
      if (fabs(d) < 1e-20 || d != d) // Note: "d!=d" is a great test for invalid
                                     // numbers, don't remove!
      {
        // All zeros:
        m_U.setZero(DIM, DIM);
      } else {
    ...
    if (eig_ok) {
      ...
      detail::generalizedEllipsoidPoints<DIM>(Uscaled, m_mean, params_pts,
      ...
      m_bb_min = mrpt::math::TPoint3D(std::numeric_limits<double>::max(),
                                      std::numeric_limits<double>::max(), 0);
      m_bb_max = mrpt::math::TPoint3D(-std::numeric_limits<double>::max(),
                                      -std::numeric_limits<double>::max(), 0);
  ...
  /** Ray tracing
    */
  virtual bool traceRay(const mrpt::poses::CPose3D &o,
...
protected:
   ...
   * transformation, a list of points
    *  defining an ellipsoid in parameter space into their corresponding points
   * in 2D/3D space.
    */
  virtual void
  transformFromParameterSpace(const std::vector<array_point_t> &params_pts,
  ...
  float m_quantiles; //!< The number of "sigmas" for drawing the
                     //!ellipse/ellipsoid (default=3)
  float m_lineWidth; //!< The line width for 2D ellipses or 3D wireframe
                     //!ellipsoids (default=1)
  uint32_t
      m_numSegments; //!< Number of segments in 2D/3D ellipsoids (default=10)
  ...
  void thisclass_readFromStream(mrpt::utils::CStream &in) {
    ...
    switch (version) {
    case 0: {
      in >> m_cov >> m_mean >> m_quantiles >> m_lineWidth >> m_numSegments;
      m_needToRecomputeEigenVals = true;
    } break;
    default:
  ...
  CGeneralizedEllipsoidTemplate()
      : m_needToRecomputeEigenVals(true), m_quantiles(3.f), m_lineWidth(1.f),
        m_numSegments(50), m_bb_min(0, 0, 0), m_bb_max(0, 0, 0) {}
