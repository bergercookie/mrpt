...
public:
  void setAxisLimits(float xmin, float ymin, float zmin, float xmax, float ymax,
                     float zmax);
  ...
  void
  setTextScale(float f); //!< Changes the size of text labels (default:0.25)
  float getTextScale() const;
  void setTextLabelOrientation(int axis, float yaw_deg, float pitch_deg,
                               float roll_deg); //!< axis: {0,1,2}=>{X,Y,Z}
  void
  getTextLabelOrientation(int axis, float &yaw_deg, float &pitch_deg,
                          float &roll_deg) const; //!< axis: {0,1,2}=>{X,Y,Z}

  void enableTickMarks(bool v = true);
  void enableTickMarks(bool show_x, bool show_y, bool show_z);

  /** Class factory  */
  static CAxis::Ptr Create(float xmin, float ymin, float zmin, float xmax,
                           float ymax, float zmax, float frecuency = 1,
                           float lineWidth = 3, bool marks = false);
  ...
  /** Constructor */
  CAxis(float xmin = -1.0f, float ymin = -1.0f, float zmin = -1.0f,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CAxis, CRenderizableDisplayList,
                                             OPENGL_IMPEXP)
