...
public:
  ...
  /**
    ...
    */
  inline void appendLine(float x0, float y0, float z0, float x1, float y1,
                         float z1) {
    appendLine(mrpt::math::TSegment3D(mrpt::math::TPoint3D(x0, y0, z0),
  ...
  //! \overload
  template <class U> inline void appendLineStrip(const U &point) {
    ASSERT_(!this->empty())
  ...
  /**
    ...
    * \sa appendLine
    */
  inline void setLineByIndex(size_t index, double x0, double y0, double z0,
                             double x1, double y1, double z1) {
    setLineByIndex(index,
                   mrpt::math::TSegment3D(mrpt::math::TPoint3D(x0, y0, z0),
                                          mrpt::math::TPoint3D(x1, y1, z1)));
    CRenderizableDisplayList::notifyChange();
  }
  /**
    ...
    * \sa getLineByIndex
    */
  void getLineByIndex(size_t index, double &x0, double &y0, double &z0,
                      double &x1, double &y1, double &z1) const;

  /** Class factory */
  static CSetOfLines::Ptr
  Create(const std::vector<mrpt::math::TSegment3D> &sgms,
  ...
  // Iterator management
  typedef std::vector<mrpt::math::TSegment3D>::iterator
      iterator; //!< Iterator to the set.
  typedef std::vector<mrpt::math::TSegment3D>::reverse_iterator
      reverse_iterator; //!< Iterator to the set.
  ...
  CSetOfLines(const std::vector<mrpt::math::TSegment3D> &sgms,
              bool antiAliasing = true);
  /** Private, virtual destructor: only can be deleted from smart pointers. */
  virtual ~CSetOfLines() {}
};
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CSetOfLines,
...
/** Inserts a segment into the list. Allows call chaining.
  ...
  */
template <>
inline CSetOfLines::Ptr &operator<<(CSetOfLines::Ptr &l,
