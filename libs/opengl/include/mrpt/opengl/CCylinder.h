...
protected:
  /**
    * Cylinder's radii. If mBaseRadius==mTopRadius, then the object is an actual
   * cylinder. If both differ, it's a truncated cone. If one of the radii is
   * zero, the object is a cone.
    */
  ...
  float mHeight;
  /**
    * Implementation parameters on which depend the number of actually rendered
   * polygons.
    */
...
public:
  ...
  /**
    ...
    */
  CCylinder()
      : mBaseRadius(1), mTopRadius(1), mHeight(1), mSlices(10), mStacks(10),
        mHasTopBase(true), mHasBottomBase(true){};
  /**
  ...
  CCylinder(const float baseRadius, const float topRadius,
            const float height = 1, const int slices = 10,
            const int stacks = 10)
      : mBaseRadius(baseRadius), mTopRadius(topRadius), mHeight(height),
        mSlices(slices), mStacks(stacks), mHasTopBase(true),
        mHasBottomBase(true){};
  /**
    * Destructor.
    */
...
private:
  /**
    * Gets the radius of the circunference located at certain height, returning
   * false if the cylinder doesn't get that high.
    */
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CCylinder,
