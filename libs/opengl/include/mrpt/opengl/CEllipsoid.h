...
protected:
  ...
  math::CMatrixD m_cov; //!< The 2x2 or 3x3 covariance matrix that will
                        //!determine the aspect of the ellipsoid.
  bool m_drawSolid3D;   //!< If set to true (default), a whole ellipsoid surface
                      //!will be drawn, or if set to "false" it will be drawn as
                      //!a "wireframe".
  float m_quantiles; //!< The number of "sigmas" for drawing the
                     //!ellipse/ellipsoid (default=3)
  unsigned int
      m_2D_segments; //!< The number of segments of a 2D ellipse (default=20)
  unsigned int m_3D_segments; //!< The number of segments of a 3D ellipse (in
                              //!both "axis") (default=20)
...
public:
  void setCovMatrix(const mrpt::math::CMatrixDouble &m,
                    int resizeToSize = -1); //!< Set the 2x2 or 3x3 covariance
                                            //!matrix that will determine the
                                            //!aspect of the ellipsoid (if
                                            //!resizeToSize>0, the matrix will
                                            //!be cut to the square matrix of
                                            //!the given size)
  ...
  } //!< If set to true (default), a whole ellipsoid surface will be drawn, or
  ...
  /** Constructor
    */
  CEllipsoid()
      : m_eigVal(), m_eigVec(), m_prevComputedCov(), m_cov(2, 2),
        m_drawSolid3D(true), m_quantiles(3), m_2D_segments(20),
        m_3D_segments(20), m_lineWidth(1.0), m_bb_min(0, 0, 0),
        m_bb_max(0, 0, 0) {}
  /** Private, virtual destructor: only can be deleted from smart pointers */
  virtual ~CEllipsoid() {}
};
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CEllipsoid,
