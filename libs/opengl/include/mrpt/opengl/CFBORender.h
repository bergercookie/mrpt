...
public:
   ...
   * windows already exist with an associated OpenGL context. If left to false,
   * a hidden GLUT window will be created.
    */
  ...
  /** Render the scene and get the rendered rgb image. Resizes the image buffer
     if it
          is necessary.
    */
  void getFrame(const COpenGLScene &scene, mrpt::utils::CImage &image);

  /** Render the scene and get the rendered rgb image. Does not resize the image
     buffer.
