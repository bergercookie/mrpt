...
protected:
  ...
  mutable bool
      m_enableTransparency; //!< Of the texture using "m_textureImageAlpha"
  mutable int r_width,
      r_height; //!< Size of the texture image, rounded up to next power of 2
  mutable int m_pad_x_right, m_pad_y_bottom; //!< The size of the fill in pixels
                                             //!in the textured image, w.r.t the
                                             //!image passed by the user.
  ...
  virtual void
  render_texturedobj() const = 0; //!< Must be implemented by derived classes
...
public:
  ...
  /** VERY IMPORTANT: If you use a multi-thread application, you MUST call this
   * from the same thread that will later destruct the object in order to the
   * OpenGL texture memory to be correctly deleted.
    *  Calling this method more than once has no effects. If you use one thread,
   * this method will be automatically called when rendering, so there is no
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CTexturedObject,
