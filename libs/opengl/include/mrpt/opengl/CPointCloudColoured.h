...
public:
  struct TPointColour {
    inline TPointColour() {}
    inline TPointColour(float _x, float _y, float _z, float _R, float _G,
                        float _B)
...
public:
  /** Constructor
    */
  CPointCloudColoured()
      : m_points(), m_pointSize(1), m_pointSmooth(false),
        m_last_rendered_count(0), m_last_rendered_count_ongoing(0) {}
  /** Private, virtual destructor: only can be deleted from smart pointers */
  virtual ~CPointCloudColoured() {}

  void markAllPointsAsNew(); //!< Do needed internal work if all points are new
                             //!(octree rebuilt,...)
...
public:
  ...
  /** Like \a setPoint() but does not check for index out of bounds */
  inline void setPoint_fast(const size_t i, const float x, const float y,
                            const float z) {
  ...
  void render_subset(const bool all, const std::vector<size_t> &idxs,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CPointCloudColoured, CRenderizable,
                                             OPENGL_IMPEXP)
...
MRPT_DECLARE_TTYPENAME_NAMESPACE(CPointCloudColoured::TPointColour,
                                 mrpt::opengl)
}
 ...
 * mrpt_adapters_grp*/
template <> class PointCloudAdapter<mrpt::opengl::CPointCloudColoured> {
private:
...
public:
  ...
  inline void setPointXYZ_RGBf(const size_t idx, const coords_t x,
    ...
    m_obj.setPoint_fast(
        idx, mrpt::opengl::CPointCloudColoured::TPointColour(x, y, z, r, g, b));
  }
  ...
  inline void setPointXYZ_RGBu8(const size_t idx, const coords_t x,
                                ...
                                const uint8_t b) {
    m_obj.setPoint_fast(idx, mrpt::opengl::CPointCloudColoured::TPointColour(
