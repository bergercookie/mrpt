...
protected:
  ...
  bool *m_is_quad;        //!< Pointer storing whether a face is a quad (1) or a
                          //!triangle (0)
  f_verts *m_face_verts;  //!< Pointer storing the vertices that compose each
                          //!face. Size: 4 x num_faces (4 for the possible max
                          //!number - quad)
  coord3D *m_vert_coords; //!< Pointer storing the coordinates of the vertices.
  ...
  mrpt::utils::TColormap m_colorMap; // Not used yet. I leave it here in case I
...
public:
  ...
  /** Load a 3D mesh. The arguments indicate:
      - num_verts: Number of vertices of the mesh
          ...
          - verts_per_face: An array (pointer) with the number of vertices of
     each face. The elements must be set either to 3 (triangle) or 4 (quad).
    ...
    */
  void loadMesh(unsigned int num_verts, unsigned int num_faces,
                int *verts_per_face, int *face_verts, float *vert_coords);

  /** Load a 3D mesh. The arguments indicate:
          - num_verts: Number of vertices of the mesh
          - num_faces: Number of faces of the mesh
          - is_quad: A binary array (Eigen) saying whether the face is a quad
     (1) or a triangle (0)
          - face_verts: An array (Eigen) with the vertices of each face. For
     every column (face), each row contains the num of a vertex. The fourth does
     not need
            to be filled if the face is a triangle.
          - vert_coords: An array (Eigen) with the coordinates of each vertex.
     For every column (vertex), each row contains the xyz coordinates of the
     vertex.
   */
  void loadMesh(unsigned int num_verts, unsigned int num_faces,
  ...
  /** Class factory
    */
  static CMesh3D::Ptr Create(bool enableTransparency, bool enableShowEdges,
  ...
  /** Constructor */
  CMesh3D(bool enableTransparency = false, bool antiAliasing = false,
          bool enableShowEdges = true, bool enableShowFaces = true,
          bool enableShowVertices = false);
  /** Private, virtual destructor: only can be deleted from smart pointers  */
  virtual ~CMesh3D();
};
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CMesh3D, CRenderizableDisplayList,
                                             OPENGL_IMPEXP)
