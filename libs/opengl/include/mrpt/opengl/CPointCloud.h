...
public:
  ...
  /** Set the list of (X,Y,Z) point coordinates, all at once, from three vectors
  ...
  /** Set the list of (X,Y,Z) point coordinates, DESTROYING the contents of the
   * input vectors (via swap) */
  void setAllPointsFast(std::vector<float> &x, std::vector<float> &y,
                        std::vector<float> &z) {
    this->clear();
    m_xs.swap(x);
    m_ys.swap(y);
  ...
  inline const std::vector<float> &getArrayX() const {
    return m_xs;
  } //!< Get a const reference to the internal array of X coordinates
  inline const std::vector<float> &getArrayY() const {
    return m_ys;
  } //!< Get a const reference to the internal array of Y coordinates
  inline const std::vector<float> &getArrayZ() const {
    return m_zs;
  ...
  inline void setPoint_fast(size_t i, const float x, const float y,
                            const float z) {
    m_xs[i] = x;
    m_ys[i] = y;
    m_zs[i] = z;
  ...
  void setGradientColors(const mrpt::utils::TColorf &colorMin,
...
private:
  mutable float m_min, m_max, m_max_m_min,
      m_max_m_min_inv; //!< Buffer for min/max coords when m_colorFromDepth is
                       //!true.
  mutable mrpt::utils::TColorf m_col_slop,
      m_col_slop_inv; //!< Color linear function slope
  mutable bool m_minmax_valid;

  mrpt::utils::TColorf m_colorFromDepth_min,
      m_colorFromDepth_max; //!< The colors used to interpolate when
                            //!m_colorFromDepth is true.

  inline void internal_render_one_point(size_t i) const;
};
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CPointCloud, CRenderizable,
                                             OPENGL_IMPEXP)
