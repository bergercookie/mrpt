...
struct OPENGL_IMPEXP TRenderInfo {
  int vp_x, vp_y, vp_width,
      vp_height; //!< Rendering viewport geometry (in pixels)
  Eigen::Matrix<float, 4, 4, Eigen::ColMajor>
      proj_matrix; //!< The 4x4 projection matrix
  Eigen::Matrix<float, 4, 4, Eigen::ColMajor>
      model_matrix; //!< The 4x4 model transformation matrix
  ...
  /** Computes the normalized coordinates (range=[0,1]) on the current rendering
   * viewport of a
    ...
    */
  void projectPoint(float x, float y, float z, float &proj_x, float &proj_y,
                    float &proj_z_depth) const {
    const Eigen::Matrix<float, 4, 1, Eigen::ColMajor> proj =
        full_matrix * Eigen::Matrix<float, 4, 1, Eigen::ColMajor>(x, y, z, 1);
    proj_x = proj[3] ? proj[0] / proj[3] : 0;
    proj_y = proj[3] ? proj[1] / proj[3] : 0;
    proj_z_depth = proj[2];
  }

  /** Exactly like projectPoint but the (x,y) projected coordinates are given in
   * pixels instead of normalized coordinates. */
  void projectPointPixels(float x, float y, float z, float &proj_x_px,
                          float &proj_y_px, float &proj_z_depth) const {
...
void OPENGL_IMPEXP renderTriangleWithNormal(const mrpt::math::TPoint3D &p1,
                                            const mrpt::math::TPoint3D &p2,
                                            const mrpt::math::TPoint3D &p3);
void OPENGL_IMPEXP renderTriangleWithNormal(const mrpt::math::TPoint3Df &p1,
...
void OPENGL_IMPEXP renderQuadWithNormal(const mrpt::math::TPoint3Df &p1,
                                        const mrpt::math::TPoint3Df &p2,
                                        const mrpt::math::TPoint3Df &p3,
...
void OPENGL_IMPEXP renderMessageBox(
    ...
    const mrpt::utils::TColor &text_col = mrpt::utils::TColor(255, 255, 255,
                                                              220),
...
mrpt::utils::TPixelCoordf OPENGL_IMPEXP
glDrawText(const std::string &text, const double textScale,
...
/// \note This functions comes from libcvd (LGPL,
/// http://www.edwardrosten.com/cvd/ )
mrpt::utils::TPixelCoordf OPENGL_IMPEXP glGetExtends(const std::string &text,
