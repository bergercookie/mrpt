...
public:
  ...
  static CColorBar::Ptr
  Create(const mrpt::utils::TColormap colormap, //!< The colormap to represent.
         double width, double height,           //!< size of the color bar
         double min_col, double max_col, //!< limits for [0,1] colormap indices
         double min_value,
         double max_value, //!< limits for values associated to extreme colors
         const std::string &label_format =
             std::string("%7.02f"),   //!< sprintf-like format string for values
  ...
  void setColorAndValueLimits(double col_min, double col_max, double value_min,
                              double value_max);
  ...
  CColorBar(const mrpt::utils::TColormap colormap =
                mrpt::utils::cmGRAYSCALE, //!< The colormap to represent.
            double width = 0.2,
            double height = 1.0, //!< size of the color bar
            double min_col = .0,
            double max_col = 1.0, //!< limits for [0,1] colormap indices
            double min_value = .0,
            double max_value =
                1.0, //!< limits for values associated to extreme colors
            const std::string &label_format = std::string(
                "%7.02f"), //!< sprintf-like format string for values
            double label_font_size = .05 //!< Label text font size
            );
};
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CColorBar,
