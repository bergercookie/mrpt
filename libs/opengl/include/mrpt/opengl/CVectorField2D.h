...
public:
  ...
  /**
    * Set the coordinates of the grid on where the vector field will be drawn by
   * setting its center and the cell size.
    * The number of cells is marked by the content of xcomp and ycomp.
    * \sa xcomp, ycomp
    */
  void setGridCenterAndCellSize(const float center_x, const float center_y,
   ...
   * using x-y max and min values.
    */
  void setGridLimits(const float xmin, const float xmax, const float ymin,
                     const float ymax) {
    xMin = xmin;
    xMax = xmax;
    yMin = ymin;
  ...
  /**
    * Adjust the vector field in the scene (vectors magnitude) according to the
   * grid size.
    */
  ...
  /**
    * Class factory
    */
  static CVectorField2D::Ptr Create(const mrpt::math::CMatrixFloat &Matrix_x,
                                    const mrpt::math::CMatrixFloat &Matrix_y,
  ...
  CVectorField2D(mrpt::math::CMatrixFloat Matrix_x,
                 mrpt::math::CMatrixFloat Matrix_y, float xmin = -1,
                 float xmax = 1, float ymin = -1, float ymax = 1);
  /** Private, virtual destructor: only can be deleted from smart pointers. */
  virtual ~CVectorField2D() {}
};
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CVectorField2D,
