 ...
 * any spatial position.
        *  This opengl object has been created to represent scene flow, and
 * hence both the vector field and
        *  the coordinates of the points at which the vector field is
 * represented are stored in matrices because
        *  they are computed from intensity and depth images.
        *  \sa opengl::COpenGLScene
        *
        *  <div align="center">
        *  <table border="0" cellspan="4" cellspacing="4" style="border-width:
...
protected:
  ...
  mrpt::math::CMatrix
      x_p; //!< X coordinate of the points at which the vector field is plotted
  mrpt::math::CMatrix
      y_p; //!< Y coordinate of the points at which the vector field is plotted
  mrpt::math::CMatrix
      z_p; //!< Z coordinate of the points at which the vector field is plotted
  ...
  mrpt::utils::TColor
      m_still_color; //!< Color associated to fields with null module
  mrpt::utils::TColor m_maxspeed_color; //!< Color associated to fields whose
                                        //!module is equal or larger than
                                        //!'m_maxspeed'
  float m_maxspeed; //!< Value of the module of the motion field which will
                    //!correspond to 'm_maxspeed_color'
...
public:
  ...
  /**
          ...
          */
  inline void setPointColor(const float R, const float G, const float B,
                            const float A = 1) {
  ...
  /**
          ...
          */
  inline void setVectorFieldColor(const float R, const float G, const float B,
                                  const float A = 1) {
  ...
  /**
          * Get the motion field min and max colors (colormap) in the range
   * [0,1]
          */
  inline void getVectorFieldColor(mrpt::utils::TColorf Cmin,
                                  mrpt::utils::TColorf Cmax) const {
    Cmin = m_still_color / 255;
    Cmax = m_maxspeed_color / 255;
  }

  /**
          * Set the motion field min and max colors (colormap) in the range
   * [0,1]
          */
  inline void setMotionFieldColormap(const float Rmin, const float Gmin,
                                     const float Bmin, const float Rmax,
                                     const float Gmax, const float Bmax,
                                     const float Amin = 1,
                                     const float Amax = 1) {
    m_still_color =
  ...
  /**
          * Get the vector field in three independent matrices: Matrix_x,
   * Matrix_y and Matrix_z.
          */
  void getVectorField(mrpt::math::CMatrixFloat &Matrix_x,
  ...
  void getVectorField(Eigen::MatrixXf &Matrix_x, Eigen::MatrixXf &Matrix_y,
   ...
   * plotted: Coord_x, Coord_y and Coord_z.
          */
  void getPointCoordinates(mrpt::math::CMatrixFloat &Coord_x,
  ...
  void getPointCoordinates(Eigen::MatrixXf &Coord_x, Eigen::MatrixXf &Coord_y,
  ...
  /**
          ...
          */
  void setVectorField(mrpt::math::CMatrixFloat &Matrix_x,
                      mrpt::math::CMatrixFloat &Matrix_y,
                      mrpt::math::CMatrixFloat &Matrix_z) {
    ASSERT_((Matrix_x.getRowCount() == Matrix_y.getRowCount()) &&
            (Matrix_x.getRowCount() == Matrix_z.getRowCount()))
    ASSERT_((Matrix_x.getColCount() == Matrix_y.getColCount()) &&
  ...
  void setVectorField(Eigen::MatrixXf &Matrix_x, Eigen::MatrixXf &Matrix_y,
  ...
  /**
          * Set the coordinates of the points at which the vector field is
   * plotted with Matrix_x, Matrix_y and Matrix_z.
          */
  void setPointCoordinates(mrpt::math::CMatrixFloat &Matrix_x,
                           mrpt::math::CMatrixFloat &Matrix_y,
                           mrpt::math::CMatrixFloat &Matrix_z) {
    ASSERT_((Matrix_x.getRowCount() == Matrix_y.getRowCount()) &&
            (Matrix_x.getRowCount() == Matrix_z.getRowCount()))
  ...
  void setPointCoordinates(Eigen::MatrixXf &Matrix_x, Eigen::MatrixXf &Matrix_y,
                           Eigen::MatrixXf &Matrix_z) {
    ASSERT_((Matrix_x.getRowCount() == Matrix_y.getRowCount()) &&
            (Matrix_x.getRowCount() == Matrix_z.getRowCount()))
    ASSERT_((Matrix_x.getColCount() == Matrix_y.getColCount()) &&
  ...
  /**
          * Class factory
          */
  static CVectorField3D::Ptr Create(const mrpt::math::CMatrixFloat x_vf_ini,
  ...
  CVectorField3D(mrpt::math::CMatrixFloat x_vf_ini,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CVectorField3D,
