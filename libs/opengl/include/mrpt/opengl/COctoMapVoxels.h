...
/** A flexible renderer of voxels, typically from a 3D octo map (see
  ...
  *		- A grid representation of all cubes, as simple lines
  *(occupied/free, leafs/nodes,... whatever). See:
  *			- showGridLines()
  *			- setGridLinesColor()
  *			- setGridLinesWidth()
  *			- push_back_GridCube()
  *		- A number of <b>voxel collections</b>, drawn as cubes each having
  *a different color (e.g. depending on the color scheme in the original
...
public:
  ...
  enum visualization_mode_t {
    COLOR_FROM_HEIGHT, //!< Color goes from black (at the bottom) to the chosen
                       //!color (at the top)
    COLOR_FROM_OCCUPANCY, //!< Color goes from black (occupied voxel) to the
                          //!chosen color (free voxel)
    TRANSPARENCY_FROM_OCCUPANCY,    //!< Transparency goes from opaque (occupied
                                    //!voxel) to transparent (free voxel).
    TRANS_AND_COLOR_FROM_OCCUPANCY, //!< Color goes from black (occupaid voxel)
                                    //!to the chosen color (free voxel) and they
                                    //!are transparent
  ...
  struct OPENGL_IMPEXP TVoxel {
    ...
    TVoxel() {}
    TVoxel(const mrpt::math::TPoint3D &coords_, const double side_length_,
  ...
  struct OPENGL_IMPEXP TGridCube {
    ...
    TGridCube() {}
    TGridCube(const mrpt::math::TPoint3D &min_,
...
public:
  ...
  /** Shows/hides the voxels (voxel_set is a 0-based index for the set of voxels
   * to modify, e.g. VOXEL_SET_OCCUPIED, VOXEL_SET_FREESPACE) */
  ...
  /** Manually changes the bounding box (normally the user doesn't need to call
   * this) */
  void setBoundingBox(const mrpt::math::TPoint3D &bb_min,
                      const mrpt::math::TPoint3D &bb_max);
  ...
  inline const TVoxel &getVoxel(const size_t set_index,
   ...
   * have access to the library [mrpt-maps].
    */
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(COctoMapVoxels,
