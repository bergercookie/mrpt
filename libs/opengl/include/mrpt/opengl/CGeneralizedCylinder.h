...
public:
  ...
  public:
    ...
    double normal[3];
    /**
      * Given a polygon with 4 already positions allocated, this method fills it
     * with the quadrilateral points.
    ...
    /**
      ...
      */
    TQuadrilateral(const mrpt::math::TPoint3D &p1,
...
protected:
  /** Cylinder's axis. It's represented as a pose because it holds the angle to
   * get to the next pose. */
...
public:
  /**
    ...
    */
  static CGeneralizedCylinder::Ptr
  Create(const std::vector<mrpt::math::TPoint3D> &axis,
  ...
  /**
    ...
    */
  inline void
  getAxis(mrpt::aligned_containers<mrpt::poses::CPose3D>::vector_t &a) const {
    a = axis;
  }
  /**
    * Set the axis points.
  ...
  /**
    ...
    */
  void getClosedSection(size_t index1, size_t index2,
                        CPolyhedron::Ptr &poly) const;
  /**
  ...
  /**
    * Adds another visible section at the end of the cylinder. The cylinder must
   * have an invisible section to display.
    * \throw std::logic_error if there is no section to add to the displaying
   * set.
    * \sa
  ...
  /**
    * Gets the axis pose of the first visible section, returning false if there
   * is no such pose.
    */
...
private:
  /**
    * Updates the axis, transforming each point into a pose pointing to the next
   * section.
    */
  void
  generatePoses(const std::vector<mrpt::math::TPoint3D> &pIn,
   ...
   * the section to be actually rendered.
    */
  void getMeshIterators(const std::vector<TQuadrilateral> &m,
...
public:
  ...
  CGeneralizedCylinder()
      : axis(), generatrix(), mesh(), meshUpToDate(false), polysUpToDate(false),
        closed(false), fullyVisible(true) {}
  /**
    * Constructor with axis and generatrix.
    */
  CGeneralizedCylinder(const std::vector<mrpt::math::TPoint3D> &a,
                       const std::vector<mrpt::math::TPoint3D> &g)
      : generatrix(g), mesh(), meshUpToDate(false), polysUpToDate(false),
        closed(false), fullyVisible(true) {
    generatePoses(a, axis);
  }
  /**
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CGeneralizedCylinder,
