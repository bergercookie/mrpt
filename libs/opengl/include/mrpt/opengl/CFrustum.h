...
public:
  ...
  static CFrustum::Ptr Create(float near_distance, float far_distance,
                              float horz_FOV_degrees, float vert_FOV_degrees,
                              float lineWidth = 1.5f, bool draw_lines = true,
  ...
  void setHorzFOVAsymmetric(const float fov_horz_left_degrees,
                            const float fov_horz_right_degrees);
  /** Changes vertical FOV (asymmetric) */
  void setVertFOVAsymmetric(const float fov_vert_down_degrees,
  ...
  CFrustum(float near_distance, float far_distance, float horz_FOV_degrees,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CFrustum, CRenderizableDisplayList,
                                             OPENGL_IMPEXP)
}
}
#endif
