...
/** A renderizable object suitable for rendering with OpenGL's display lists.
  ...
  *primitives as usual. They'll be saved in a display list transparently.
  *
...
private:
  mutable unsigned int m_dl; //!< Display list ID, for derived classes that want
                             //!to use it (it's automatically deleted and freed
                             //!on destruction of this base class).
  mutable bool m_dl_recreate; //!< If using display lists, this is true when the
                              //!list must be updated (the object changes, it's
                              //!the first rendering, etc...).

protected:
  /** @name Methods accesible or implemented by derived classes
      @{ */

  /** Must be called to notify that the object has changed (so, the display list
   * must be updated) */
  ...
  /** Optional: If the object has some state in which creating a display list is
   * NOT preferred over direct rendering,
...
public:
  ...
  /** Render the object, regenerating the display list if needed, otherwise just
   * calling it. */
  ...
  inline CRenderizable &setColor_u8(uint8_t R, uint8_t G, uint8_t B,
                                    uint8_t A = 255) {
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CRenderizableDisplayList,
                                             CRenderizable, OPENGL_IMPEXP)
