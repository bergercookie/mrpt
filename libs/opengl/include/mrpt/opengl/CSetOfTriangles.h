...
public:
  ...
  inline void insertTriangles(const InputIterator &begin,
  ...
  /**
    * Reserves memory for certain number of triangles, avoiding multiple memory
   * allocation calls.
    */
  ...
  /**
    * Inserts a set of triangles, given in a container of either TTriangle's or
   * TPolygon3D
    * \sa insertTriangle
    */
  template <class CONTAINER> inline void insertTriangles(const CONTAINER &c) {
   ...
   * last triangle.
    */
  inline const_reverse_iterator rbegin() const { return m_triangles.rbegin(); }
  /**
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CSetOfTriangles,
...
/** Inserts a triangle into the list. Allows call chaining.
  ...
  */
template <>
inline CSetOfTriangles::Ptr &operator<<(CSetOfTriangles::Ptr &s,
