...
void gl_utils::renderSetOfObjects(const CListOpenGLObjects &objectsToRender) {
  ...
  try {
    for (itP = objectsToRender.begin(); itP != objectsToRender.end(); ++itP) {
      ...
      // It's more efficient to prepare the 4x4 matrix ourselves and load it
      // directly into opengl stack:
      ...
      glMultMatrixd(m); // Multiply so it's composed with the previous, current
                        // MODELVIEW matrix
      ...
      // Set color:
      glColor4f(it->getColorR(), it->getColorG(), it->getColorB(),
                it->getColorA());
...
void gl_utils::renderTriangleWithNormal(const mrpt::math::TPoint3D &p1,
...
void gl_utils::renderTriangleWithNormal(const mrpt::math::TPoint3Df &p1,
...
void gl_utils::renderQuadWithNormal(const mrpt::math::TPoint3Df &p1,
                                    const mrpt::math::TPoint3Df &p2,
                                    const mrpt::math::TPoint3Df &p3,
...
#if MRPT_HAS_OPENGL_GLUT
  while (*str)
    glutBitmapCharacter(fontStyle, *(str++));
#else
 ...
 * renderTextBitmap().
  * \sa renderTextBitmap
  */
int gl_utils::textBitmapWidth(const std::string &str,
                              mrpt::opengl::TOpenGLFont font) {
#if MRPT_HAS_OPENGL_GLUT
  if (str.empty())
    return 0;
  return glutBitmapLength(aux_mrptfont2glutfont(font),
...
/*---------------------------------------------------------------
                                        renderTextBitmap
  ---------------------------------------------------------------*/
void CRenderizable::renderTextBitmap(int screen_x, int screen_y,
                                     const std::string &str, float color_r,
...
void gl_utils::renderMessageBox(
  ...
  gl_utils::glDrawText(text, text_scale, text_style, text_spacing,
                       text_kerning);
...
mrpt::utils::TPixelCoordf gl_utils::glDrawText(const std::string &text,
  ...
  case NICE:
    ...
    break;
  default:
    THROW_EXCEPTION("Invalid text style value.");
  }
  ...
  return mrpt::utils::TPixelCoordf(textScale * max_total,
                                   textScale * (lines + 1) * spacing);
#else
  MRPT_UNUSED_PARAM(text);
...
mrpt::utils::TPixelCoordf gl_utils::glGetExtends(const std::string &text,
                                                 const double textScale,
                                                 double spacing,
                                                 double kerning) {
#if MRPT_HAS_OPENGL_GLUT
