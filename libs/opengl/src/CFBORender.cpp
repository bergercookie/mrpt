...
/*---------------------------------------------------------------
                                                Constructor
---------------------------------------------------------------*/
CFBORender::CFBORender(unsigned int width, unsigned int height,
...
// In win32 we have to load the pointers to the functions:
  ...
  glTexImage2D(texTarget, 0, GL_RGB, m_width, m_height, 0, GL_RGB,
  ...
  //'unbind' the frambuffer object, so subsequent drawing ops are not drawn into
  //the FBO.
...
void CFBORender::getFrame2(const COpenGLScene &scene, CImage &buffer) {
  ...
  glClearColor(m_default_bk_color.R, m_default_bk_color.G, m_default_bk_color.B,
  ...
  // glPixelStore() etc.
  glReadPixels(0, 0, m_width, m_height, GL_BGR_EXT, GL_UNSIGNED_BYTE,
               buffer(0, 0));
