...
void CEllipsoid::setCovMatrix(const mrpt::math::CMatrixDouble &m,
      ...
      d != d) // Note: "d!=d" is a great test for invalid numbers, don't remove!
  {
    ...
    if (m_cov.eigenVectors(m_eigVec, m_eigVal)) {
      ...
      // Do the scale at render to avoid recomputing the m_eigVal for different
      // m_quantiles
    } else {
      m_eigVec.zeros(3, 3);
      m_eigVal.zeros(3, 3);
...
void CEllipsoid::setCovMatrix(const mrpt::math::CMatrixFloat &m,
                              int resizeToSize) {
  CRenderizableDisplayList::notifyChange();
...
void CEllipsoid::getBoundingBox(mrpt::math::TPoint3D &bb_min,
