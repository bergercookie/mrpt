...
void CTextMessageCapable::render_text_messages(const int w, const int h) const {
  ...
  for (std::map<size_t, mrpt::opengl::T2DTextData>::const_iterator it =
    ...
    // If (x,y) \in [0,1[, it's interpreted as a ratio, otherwise, as an actual
    ...
    // This means this is a vectorized font, so just copy the parameters set by
    // the user:
    ...
    default:
      std::cerr << "[CTextMessageCapable::render_text_messages] Invalid value "
                   "for TOpenGLFont\n";
      break;
    };
    ...
    if (it->second.draw_shadow) {
      ...
      glColor3f(it->second.shadow_color.R, it->second.shadow_color.G,
                it->second.shadow_color.B);
      mrpt::opengl::gl_utils::glSetFont(font_name);
      mrpt::opengl::gl_utils::glDrawText(it->second.text, font_size, font_style,
                                         font_spacing, font_kerning);
