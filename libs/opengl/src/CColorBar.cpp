...
CColorBar::CColorBar(
    ...
    : m_colormap(colormap), m_width(width), m_height(height),
      m_label_format(label_format), m_min_col(min_col), m_max_col(max_col),
      m_min_value(min_value), m_max_value(max_value),
      m_label_font_size(label_font_size), m_disable_depth_test(true) {}
...
CColorBar::Ptr CColorBar::Create(
  ...
  return CColorBar::Ptr(new CColorBar(colormap, width, height, min_col, max_col,
                                      min_value, max_value, label_format,
                                      label_font_size));
}
...
void CColorBar::setColorAndValueLimits(double col_min, double col_max,
...
void CColorBar::render_dl() const {
  ...
  if (m_disable_depth_test)
    glDisable(GL_DEPTH_TEST); // colobars are typically displayed on-top of the
                              // rest of objects!
...
void CColorBar::getBoundingBox(mrpt::math::TPoint3D &bb_min,
