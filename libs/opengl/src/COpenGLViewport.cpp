...
COpenGLViewport::COpenGLViewport(COpenGLScene *parent, const string &name)
    : m_camera(), m_parent(parent), m_isCloned(false), m_isClonedCamera(false),
      m_clonedViewport(), m_name(name), m_isTransparent(false),
      m_borderWidth(0), m_view_x(0), m_view_y(0), m_view_width(1),
      m_view_height(1), m_clip_min(0.1), m_clip_max(10000),
      m_custom_backgb_color(false), m_background_color(0.6f, 0.6f, 0.6f),
      m_isImageView(false), m_imageview_img(), m_objects(),
      // OpenGL settings:
      m_OpenGL_enablePolygonNicest(true), m_lights() {
...
/*--------------------------------------------------------------
                                        setViewportPosition
  ---------------------------------------------------------------*/
void COpenGLViewport::setViewportPosition(const double x, const double y,
                                          const double width,
                                          const double height) {
  MRPT_START
...
/*--------------------------------------------------------------
                                        getViewportPosition
  ---------------------------------------------------------------*/
void COpenGLViewport::getViewportPosition(double &x, double &y, double &width,
                                          double &height) {
  x = m_view_x;
  y = m_view_y;
  width = m_view_width;
...
void COpenGLViewport::render(const int render_width,
  ...
  try {
    ...
    else if (m_view_width < 0) { // Negative numbers: Specify the right side
                                 // coordinates instead of the width:
      if (m_view_width >= -1)
    ...
    else if (m_view_height < 0) { // Negative numbers: Specify the right side
                                  // coordinates instead of the width:
      if (m_view_height >= -1)
    ...
    if (!m_isTransparent) { // Clear color & depth buffers:
                            // Save?
      ...
      // Restore old colors:
      if (m_custom_backgb_color)
        glClearColor(old_colors[0], old_colors[1], old_colors[2],
                     old_colors[3]);
    } else { // Clear depth buffer only:
      ...
      if (m_imageview_img) // should be ALWAYS true, but just in case!
      {
        ...
        if (img_w != 0 && img_h != 0) {
          ...
          const bool is_RGB_order = (!::strcmp(
              img->getChannelsOrder(), "RGB")); // Reverse RGB <-> BGR order?
          ...
          glDrawPixels(img_w, img_h, img_format, img_type,
                       img->get_unsafe(0, 0));
...
// done.
      ...
      if (m_isCloned) { // Clone: render someone's else objects.
      ...
      } else { // Normal case: render our own objects:
      ...
      if (myCamera->m_projectiveModel) {
        gluPerspective(myCamera->m_projectiveFOVdeg, vw / double(vh),
                       m_clip_min, m_clip_max);
        CRenderizable::checkOpenGLError();
      } else {
      ...
      if (myCamera->is6DOFMode()) {
        // In 6DOFMode eye is set viewing towards the direction of the positive
        // Z axis
        // Up is set as Y axis
        ...
        gluLookAt(pose.x(), pose.y(), pose.z(), at.x(), at.y(), at.z(),
        ...
        gluLookAt(m_lastProjMat.eye.x, m_lastProjMat.eye.y, m_lastProjMat.eye.z,
                  m_lastProjMat.pointing.x, m_lastProjMat.pointing.y,
                  m_lastProjMat.pointing.z, m_lastProjMat.up.x,
                  m_lastProjMat.up.y, m_lastProjMat.up.z);
        CRenderizable::checkOpenGLError();
      }
  ...
  THROW_EXCEPTION("The MRPT has been compiled with MRPT_HAS_OPENGL_GLUT=0! "
                  "OpenGL functions are not implemented");
#endif
}
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void COpenGLViewport::writeToStream(mrpt::utils::CStream &out,
                                    int *version) const {
  if (version)
    *version = 3;
  else {
...
void COpenGLViewport::readFromStream(mrpt::utils::CStream &in, int version) {
  ...
  case 3: {
    // Load data:
    in >> m_camera >> m_isCloned >> m_isClonedCamera >> m_clonedViewport >>
        m_name >> m_isTransparent >> m_borderWidth >> m_view_x >> m_view_y >>
        m_view_width >> m_view_height;
    ...
    if (version >= 1) {
      in >> m_custom_backgb_color >> m_background_color.R >>
          m_background_color.G >> m_background_color.B >> m_background_color.A;
    } else {
...
void COpenGLViewport::get3DRayForPixelCoord(
  ...
  ASSERTDEB_(m_lastProjMat.viewport_height > 0 &&
             m_lastProjMat.viewport_width > 0)
  ...
  if (!m_lastProjMat.is_projective) {
    ...
    const TPoint3D l(tan(ang_horz), tan(ang_vert),
...
void COpenGLViewport::internal_setImageView_fast(const mrpt::utils::CImage &img,
                                                 bool is_fast) {
...
void COpenGLViewport::getBoundingBox(mrpt::math::TPoint3D &bb_min,
  ...
  bb_min = TPoint3D(std::numeric_limits<double>::max(),
                    std::numeric_limits<double>::max(),
                    std::numeric_limits<double>::max());
  bb_max = TPoint3D(-std::numeric_limits<double>::max(),
                    -std::numeric_limits<double>::max(),
                    -std::numeric_limits<double>::max());

  for (CListOpenGLObjects::const_iterator it = m_objects.begin();
       it != m_objects.end(); ++it) {
    TPoint3D child_bbmin(std::numeric_limits<double>::max(),
                         std::numeric_limits<double>::max(),
                         std::numeric_limits<double>::max());
    TPoint3D child_bbmax(-std::numeric_limits<double>::max(),
                         -std::numeric_limits<double>::max(),
                         -std::numeric_limits<double>::max());
