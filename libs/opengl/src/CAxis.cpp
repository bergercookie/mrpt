...
CAxis::CAxis(float xmin, float ymin, float zmin, float xmax, float ymax,
             float zmax, float frecuency, float lineWidth, bool marks)
    : m_xmin(xmin), m_ymin(ymin), m_zmin(zmin), m_xmax(xmax), m_ymax(ymax),
      m_zmax(zmax), m_frequency(frecuency), m_lineWidth(lineWidth),
      m_textScale(0.25f) {
  for (int i = 0; i < 3; i++)
    m_marks[i] = marks;
...
CAxis::Ptr CAxis::Create(float xmin, float ymin, float zmin, float xmax,
                         float ymax, float zmax, float frecuency,
                         float lineWidth, bool marks) {
  return CAxis::Ptr(new CAxis(xmin, ymin, zmin, xmax, ymax, zmax, frecuency,
                              lineWidth, marks));
}
...
void CAxis::render_dl() const {
  ...
  if (m_marks[1]) {
    ...
    for (float i = m_ymin; i <= m_ymax; i = i + m_frequency) {
      if (std::abs(i) > 1e-4) { // Dont draw the "0" more than once
        os::sprintf(n, 50, "%.02f", i);
        glPushMatrix();
...
void CAxis::getBoundingBox(mrpt::math::TPoint3D &bb_min,
...
void CAxis::setAxisLimits(float xmin, float ymin, float zmin, float xmax,
...
void CAxis::setTextLabelOrientation(int axis, float yaw_deg, float pitch_deg,
                                    float roll_deg) {
...
void CAxis::getTextLabelOrientation(int axis, float &yaw_deg, float &pitch_deg,
