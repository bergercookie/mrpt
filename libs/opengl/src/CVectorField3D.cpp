...
CVectorField3D::CVectorField3D(CMatrixFloat x_vf_ini, CMatrixFloat y_vf_ini,
                               CMatrixFloat z_vf_ini, CMatrixFloat x_p_ini,
                               CMatrixFloat y_p_ini, CMatrixFloat z_p_ini)
    : m_LineWidth(1.0), m_pointSize(1.0), m_antiAliasing(true),
      m_colorFromModule(false), m_showPoints(true) {
  x_vf = x_vf_ini;
  y_vf = y_vf_ini;
...
void CVectorField3D::render_dl() const {
  ...
  if (m_showPoints) {
    glBegin(GL_POINTS);
    glColor4ub(m_point_color.R, m_point_color.G, m_point_color.B,
               m_point_color.A);
  ...
  if (m_colorFromModule == false) {
    ...
    for (unsigned int i = 0; i < x_vf.getColCount(); i++)
      for (unsigned int j = 0; j < x_vf.getRowCount(); j++) {
        glVertex3f(x_p(j, i), y_p(j, i), z_p(j, i));
        glVertex3f(x_p(j, i) + x_vf(j, i), y_p(j, i) + y_vf(j, i),
                   z_p(j, i) + z_vf(j, i));
      }
  } else {
    for (unsigned int i = 0; i < x_vf.getColCount(); i++)
      for (unsigned int j = 0; j < x_vf.getRowCount(); j++) {
        // Compute color
        const float module =
            sqrt(square(x_vf(j, i)) + square(y_vf(j, i)) + square(z_vf(j, i)));
        if (module > m_maxspeed)
          glColor4ub(m_maxspeed_color.R, m_maxspeed_color.G, m_maxspeed_color.B,
                     m_maxspeed_color.A);
        else {
          const float R = (m_maxspeed - module) * m_still_color.R / m_maxspeed +
                          module * m_maxspeed_color.R / m_maxspeed;
          const float G = (m_maxspeed - module) * m_still_color.G / m_maxspeed +
                          module * m_maxspeed_color.G / m_maxspeed;
          const float B = (m_maxspeed - module) * m_still_color.B / m_maxspeed +
                          module * m_maxspeed_color.B / m_maxspeed;
          const float A = (m_maxspeed - module) * m_still_color.A / m_maxspeed +
  ...
  //ycomp(j,i)*ycomp(j,i));
  //			const float ang = ::atan2(ycomp(j,i), xcomp(j,i)) -
  //1.5708;
  //			glVertex3f( -sin(ang)*0.866*tri_side + xMin+i*x_cell_size +
  //xcomp(j,i), cos(ang)*0.866*tri_side + yMin+j*y_cell_size + ycomp(j,i), 0);
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CVectorField3D::writeToStream(mrpt::utils::CStream &out,
                                   int *version) const {
  if (version)
    *version = 0;
  else {
...
void CVectorField3D::getBoundingBox(mrpt::math::TPoint3D &bb_min,
