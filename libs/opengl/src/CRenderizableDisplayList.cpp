...
IMPLEMENTS_VIRTUAL_SERIALIZABLE(CRenderizableDisplayList, CRenderizable,
                                mrpt::opengl)

// TAuxDLData: needed since it seems we must delete display lists from the same
// thread we create them....
...
void CRenderizableDisplayList::render() const {
  ...
  if (should_skip_display_list_cache()) {
    // The object is in a state where caching a display list is not preferred,
    // so render directly:
    render_dl();
  } else {
    // We must delete pending dl's in the same thread we create them, so, let's
    ...
    // Call the list:
    glCallList(m_dl);
    glGetError(); // Clear any error flags that may remain: this is because it
                  // seems FBO rendering lead to "errors" but it actually
                  // renders OK...
  }
#endif
}
