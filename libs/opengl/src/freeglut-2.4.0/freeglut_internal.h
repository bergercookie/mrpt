...
/*
 * The deinitialize function gets called on glutMainLoop() end. It should clean
 * up
...
#if TARGET_HOST_WIN32 || TARGET_HOST_WINCE
LRESULT CALLBACK
	fgWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
GLboolean fgSetupPixelFormat(
	SFG_Window* window, GLboolean checkOnly, unsigned char layer_type);
#endif
 ...
 */
SFG_Window* fgCreateWindow(
	SFG_Window* parent, const char* title, int x, int y, int w, int h,
	GLboolean gameMode, GLboolean isMenu);
void fgSetWindow(SFG_Window* window);
void fgOpenWindow(
	SFG_Window* window, const char* title, int x, int y, int w, int h,
...
void fgEnumSubWindows(
	SFG_Window* window, FGCBenumerator enumCallback,
...
GLboolean fgCheckActiveMenu(
	SFG_Window* window, int button, GLboolean pressed, int mouse_x,
	int mouse_y);
