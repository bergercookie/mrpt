...
void get_bounding_box_for_node(const aiScene *sc, const aiNode *nd,
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CAssimpModel::writeToStream(mrpt::utils::CStream &out,
                                 int *version) const {
  if (version)
    *version = 0;
  else {
...
void CAssimpModel::getBoundingBox(mrpt::math::TPoint3D &bb_min,
...
void apply_material(
  ...
  if (AI_SUCCESS ==
    ...
    // bind texture
    std::map<std::string, CAssimpModel::TInfoPerTexture>::const_iterator it =
        textureIdMap.find(texPath.data);
...
void recursive_render(
  ...
  for (; n < nd->mNumMeshes; ++n) {
    ...
    apply_material(sc->mMaterials[mesh->mMaterialIndex], textureIds,
                   textureIdMap);
...
void load_textures(
  ...
  for (unsigned int m = 0; m < scene->mNumMaterials; m++) {
    ...
    for (;;) {
      ...
      if (texFound == AI_SUCCESS) {
        ...
        ipt.id_idx = std::string::npos; // fill map with textures, pointers
                                        // still nullptr yet
        texIndex++;
      } else
        break;
  ...
  for (int i = 0; i < numTextures; i++) {
    ...
    bool load_ok;
    if (mrpt::system::lowerCase(mrpt::system::extractFileExtension(fileloc)) ==
        string("tga")) {
    ...
    if (load_ok) {
      // success = ilConvertImage(IL_RGB, IL_UNSIGNED_BYTE); /* Convert every
      // colour component into unsigned byte. If your image contains alpha
...
interpolation for minifying filter */
      ...
      const bool is_RGB_order = (!::strcmp(
          img_rgb->getChannelsOrder(), "RGB")); // Reverse RGB <-> BGR order?
      ...
      glPixelStorei(GL_UNPACK_ROW_LENGTH,
                    img_rgb->getRowStride() / nBytesPerPixel);
      glTexImage2D(GL_TEXTURE_2D, 0 /*level*/, 3 /* RGB components */, width,
      ...
      const std::string sError = mrpt::format(
          "[CAssimpModel] Couldn't load texture image: '%s'", fileloc.c_str());
      cout << sError << endl;
#ifdef _MSC_VER
