...
CPointCloud::CPointCloud()
    : m_colorFromDepth(CPointCloud::colNone), m_xs(), m_ys(), m_zs(),
      m_pointSize(1), m_pointSmooth(false), m_last_rendered_count(0),
      m_last_rendered_count_ongoing(0), m_min(0), m_max(0), m_max_m_min(0),
      m_max_m_min_inv(0), m_minmax_valid(false), m_colorFromDepth_min(0, 0, 0),
...
void CPointCloud::render() const {
  ...
  glBegin(GL_POINTS);
  glColor4ub(m_color.R, m_color.G, m_color.B,
...
inline void CPointCloud::internal_render_one_point(size_t i) const {
  ...
  if (m_colorFromDepth != colNone && m_max_m_min > 0) {
    ...
    glColor4f(m_colorFromDepth_min.R + f * m_col_slop_inv.R,
...
void CPointCloud::render_subset(const bool all, const std::vector<size_t> &idxs,
                                const float render_area_sqpixels) const {
#if MRPT_HAS_OPENGL_GLUT

  const size_t N = (all ? m_xs.size() : idxs.size());
  const size_t decimation = mrpt::utils::round(std::max(
      1.0f,
      static_cast<float>(
          N /
...
void CPointCloud::insertPoint(float x, float y, float z) {
  ...
  // JL: TODO note: Well, this can be clearly done much more efficiently but...I
  // don't have time! :-(
  markAllPointsAsNew();
}
 ...
 */
void CPointCloud::setPoint(size_t i, const float x, const float y,
                           const float z) {
#ifdef _DEBUG
...
/*---------------------------------------------------------------
                                        setGradientColors
---------------------------------------------------------------*/
void CPointCloud::setGradientColors(const mrpt::utils::TColorf &colorMin,
