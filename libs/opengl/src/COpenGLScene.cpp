...
COpenGLScene &COpenGLScene::operator=(const COpenGLScene &obj) {
  if (this != &obj) {
    ...
    for_each(m_viewports.begin(), m_viewports.end(),
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void COpenGLScene::writeToStream(mrpt::utils::CStream &out,
                                 int *version) const {
  if (version)
    *version = 1;
  else {
...
void COpenGLScene::readFromStream(mrpt::utils::CStream &in, int version) {
  ...
  case 0: {
    ...
    for_each(view->m_objects.begin(), view->m_objects.end(),
...
/*---------------------------------------------------------------
                                                        getByName
  ---------------------------------------------------------------*/
CRenderizable::Ptr COpenGLScene::getByName(const string &str,
...
/*--------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
COpenGLViewport::Ptr
COpenGLScene::getViewport(const std::string &viewportName) const {
  MRPT_START
  for (TListViewports::const_iterator it = m_viewports.begin();
       it != m_viewports.end(); ++it)
    if ((*it)->m_name == viewportName)
