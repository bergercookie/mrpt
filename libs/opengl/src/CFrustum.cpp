...
void CFrustum::render_dl() const {
  ...
  if (m_draw_planes) {
    ...
    glColor4ub(m_planes_color.R, m_planes_color.G, m_planes_color.B,
               m_planes_color.A);
...
// Ctors
CFrustum::CFrustum()
    : m_min_distance(0.1f), m_max_distance(1.f),
      m_fov_horz_left(mrpt::utils::DEG2RAD(45)),
      m_fov_horz_right(mrpt::utils::DEG2RAD(45)),
      m_fov_vert_down(mrpt::utils::DEG2RAD(30)),
      m_fov_vert_up(mrpt::utils::DEG2RAD(30)), m_draw_lines(true),
      m_draw_planes(false), m_lineWidth(1.5f),
...
CFrustum::CFrustum(float near_distance, float far_distance,
                   float horz_FOV_degrees, float vert_FOV_degrees,
                   float lineWidth, bool draw_lines, bool draw_planes)
    : m_min_distance(near_distance), m_max_distance(far_distance),
      ...
      m_draw_lines(draw_lines), m_draw_planes(draw_planes),
      m_lineWidth(lineWidth), m_planes_color(0xE0, 0x00, 0x00, 0x50) // RGBA
{}
...
// setters:
void CFrustum::setNearFarPlanes(const float near_distance,
...
void CFrustum::setHorzFOVAsymmetric(const float fov_horz_left_degrees,
...
void CFrustum::setVertFOVAsymmetric(const float fov_vert_down_degrees,
...
void CFrustum::getBoundingBox(mrpt::math::TPoint3D &bb_min,
