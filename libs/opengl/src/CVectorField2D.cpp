...
CVectorField2D::CVectorField2D()
    : xcomp(0, 0), ycomp(0, 0), xMin(-1.0), xMax(1.0), yMin(-1.0), yMax(1.0),
...
CVectorField2D::CVectorField2D(CMatrixFloat Matrix_x, CMatrixFloat Matrix_y,
...
void CVectorField2D::render_dl() const {
  ...
  for (unsigned int i = 0; i < xcomp.getColCount(); i++)
    for (unsigned int j = 0; j < xcomp.getRowCount(); j++) {
      glVertex3f(xMin + i * x_cell_size, yMin + j * y_cell_size, 0);
      glVertex3f(xMin + i * x_cell_size + xcomp(j, i),
  ...
  for (unsigned int i = 0; i < xcomp.getColCount(); i++)
    for (unsigned int j = 0; j < xcomp.getRowCount(); j++) {
      const float tri_side =
          0.25 * sqrt(xcomp(j, i) * xcomp(j, i) + ycomp(j, i) * ycomp(j, i));
      const float ang = ::atan2(ycomp(j, i), xcomp(j, i)) - 1.5708;
      glVertex3f(
          -sin(ang) * 0.866 * tri_side + xMin + i * x_cell_size + xcomp(j, i),
          cos(ang) * 0.866 * tri_side + yMin + j * y_cell_size + ycomp(j, i),
          0);
      glVertex3f(
          cos(ang) * 0.5 * tri_side + xMin + i * x_cell_size + xcomp(j, i),
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CVectorField2D::writeToStream(mrpt::utils::CStream &out,
                                   int *version) const {
  if (version)
    *version = 0;
  else {
...
void CVectorField2D::getBoundingBox(mrpt::math::TPoint3D &bb_min,
  ...
  for (unsigned int i = 0; i < xcomp.getColCount(); i++)
    for (unsigned int j = 0; j < xcomp.getRowCount(); j++) {
      ...
      if (-sin(ang) * 0.866 * tri_side + xMin + i * x_cell_size + xcomp(j, i) <
          bb_min.x)
        bb_min.x =
            -sin(ang) * 0.866 * tri_side + xMin + i * x_cell_size + xcomp(j, i);

      if (cos(ang) * 0.866 * tri_side + yMin + j * y_cell_size + ycomp(j, i) <
          bb_min.y)
        bb_min.y =
            cos(ang) * 0.866 * tri_side + yMin + j * y_cell_size + ycomp(j, i);
