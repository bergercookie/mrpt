...
CGridPlaneXY::Ptr CGridPlaneXY::Create(float xMin, float xMax, float yMin,
                                       float yMax, float z, float frequency,
                                       float lineWidth, bool antiAliasing) {
  return CGridPlaneXY::Ptr(new CGridPlaneXY(
      xMin, xMax, yMin, yMax, z, frequency, lineWidth, antiAliasing));
}
/** Constructor  */
CGridPlaneXY::CGridPlaneXY(float xMin, float xMax, float yMin, float yMax,
                           float z, float frequency, float lineWidth,
                           bool antiAliasing)
    : m_xMin(xMin), m_xMax(xMax), m_yMin(yMin), m_yMax(yMax), m_plane_z(z),
      m_frequency(frequency), m_lineWidth(lineWidth),
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CGridPlaneXY::writeToStream(mrpt::utils::CStream &out,
                                 int *version) const {
...
void CGridPlaneXY::getBoundingBox(mrpt::math::TPoint3D &bb_min,
