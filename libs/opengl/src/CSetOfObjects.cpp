...
void CSetOfObjects::clear() {
  m_objects.clear(); // clear the list and delete objects (if there are no more
                     // copies out there!)
}
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CSetOfObjects::writeToStream(mrpt::utils::CStream &out,
                                  int *version) const {
  if (version)
    *version = 0;
  else {
...
bool CSetOfObjects::traceRay(const mrpt::poses::CPose3D &o,
                             double &dist) const {
...
CRenderizable &CSetOfObjects::setColor_u8(const mrpt::utils::TColor &c) {
  for_each(m_objects.begin(), m_objects.end(),
           FSetColor(m_color.R = c.R, m_color.G = c.G, m_color.B = c.B,
                     m_color.A = c.A));
  return *this;
}
...
void CSetOfObjects::getBoundingBox(mrpt::math::TPoint3D &bb_min,
