...
CMesh3D::CMesh3D(bool enableTransparency, bool antiAliasing,
                 bool enableShowEdges, bool enableShowFaces,
                 bool enableShowVertices)
    : m_enableTransparency(enableTransparency), m_antiAliasing(antiAliasing),
      m_showEdges(enableShowEdges), m_showFaces(enableShowFaces),
      m_showVertices(enableShowVertices), m_computeNormals(true),
      m_lineWidth(2.f), m_pointSize(6.f), m_colorMap(mrpt::utils::cmHOT) {
  m_color.R = 1.f;
  m_color.G = 0.f;
  m_color.B = 0.f;
  m_color.A = 1.f;
...
void CMesh3D::loadMesh(unsigned int num_verts, unsigned int num_faces,
                       int *verts_per_face, int *face_verts,
                       float *vert_coords) {
  ...
  if (m_computeNormals) {
    ...
    for (unsigned int f = 0; f < num_faces; f++) {
      ...
      if (m_is_quad[f]) {
        const float vec1[3] = {m_vert_coords[v3][0] - m_vert_coords[v1][0],
                               m_vert_coords[v3][1] - m_vert_coords[v1][1],
                               m_vert_coords[v3][2] - m_vert_coords[v1][2]};
        const float vec2[3] = {m_vert_coords[v4][0] - m_vert_coords[v2][0],
        ...
        const float vec1[3] = {m_vert_coords[v2][0] - m_vert_coords[v1][0],
                               m_vert_coords[v2][1] - m_vert_coords[v1][1],
                               m_vert_coords[v2][2] - m_vert_coords[v1][2]};
        const float vec2[3] = {m_vert_coords[v3][0] - m_vert_coords[v1][0],
                               m_vert_coords[v3][1] - m_vert_coords[v1][1],
                               m_vert_coords[v3][2] - m_vert_coords[v1][2]};
        m_normals[f][0] = vec1[1] * vec2[2] - vec1[2] * vec2[1];
        m_normals[f][1] = vec1[2] * vec2[0] - vec1[0] * vec2[2];
...
void CMesh3D::render_dl() const {
  ...
  //continuously is very slow (or not)
  //---------------------------------------------------------------------------------------------------------
    ...
    for (unsigned int f = 0; f < m_num_faces; f++) {
      ...
      if (m_is_quad[f]) {
        ...
        for (int i = 0; i < 4; i++) {
          ...
          glVertex3f(m_vert_coords[vert_ind][0], m_vert_coords[vert_ind][1],
  ...
  if (m_showEdges) {
    ...
    for (unsigned int f = 0; f < m_num_faces; f++) {
      ...
      for (int i = 0; i < num_vert - 1; i++) {
        ...
        glVertex3f(m_vert_coords[v_0][0], m_vert_coords[v_0][1],
                   m_vert_coords[v_0][2]);
        glVertex3f(m_vert_coords[v_1][0], m_vert_coords[v_1][1],
                   m_vert_coords[v_1][2]);
      }
...
void CMesh3D::getBoundingBox(mrpt::math::TPoint3D &bb_min,
  ...
  if (m_num_verts == 0)
    printf("\n The mesh is empty and has no size. The returned information has "
           "no meaning.");
  else {
