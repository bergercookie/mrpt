...
COctoMapVoxels::COctoMapVoxels()
    : m_enable_lighting(false), m_enable_cube_transparency(true),
      m_showVoxelsAsPoints(false), m_showVoxelsAsPointsSize(3.0f),
      m_show_grids(false), m_grid_width(1.0f),
...
void COctoMapVoxels::setBoundingBox(const mrpt::math::TPoint3D &bb_min,
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void COctoMapVoxels::writeToStream(mrpt::utils::CStream &out,
                                   int *version) const {
  if (version)
    *version = 2;
  else {
...
void COctoMapVoxels::getBoundingBox(mrpt::math::TPoint3D &bb_min,
...
void COctoMapVoxels::sort_voxels_by_z() {
  for (size_t i = 0; i < m_voxel_sets.size(); i++) {
    std::sort(m_voxel_sets[i].voxels.begin(), m_voxel_sets[i].voxels.end(),
              &sort_voxels_z);
  }
}
