...
CSphere::Ptr CSphere::Create(float radius, int nDivsLongitude,
                             int nDivsLatitude) {
...
bool CSphere::traceRay(const mrpt::poses::CPose3D &o, double &dist) const {
  ...
  // The sphere's equation is invariant to rotations (but not to translations),
  // and we can take advantage of this;
  // we'll simply transform the center and then compute the beam's points whose
  // distance to that transformed point
...
void CSphere::getBoundingBox(mrpt::math::TPoint3D &bb_min,
