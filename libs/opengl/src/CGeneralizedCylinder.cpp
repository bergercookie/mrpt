...
IMPLEMENTS_SERIALIZABLE(CGeneralizedCylinder, CRenderizableDisplayList,
                        mrpt::opengl)
...
inline void createMesh(const CMatrixTemplate<TPoint3D> &pointsMesh, size_t R,
                       size_t C,
  ...
  for (size_t i = 0; i < R; i++)
    for (size_t j = 0; j < C; j++)
      mesh.push_back(CGeneralizedCylinder::TQuadrilateral(
          pointsMesh(i, j), pointsMesh(i, j + 1), pointsMesh(i + 1, j + 1),
          pointsMesh(i + 1, j)));
}
...
void CGeneralizedCylinder::updateMesh() const {
  ...
  if (closed && genX.size() > 2)
    genX.push_back(genX[0]);
  size_t G = genX.size();
  mesh.clear();
  if (A > 1 && G > 1) {
...
void CGeneralizedCylinder::writeToStream(mrpt::utils::CStream &out,
                                         int *version) const {
  if (version)
    *version = 1;
  else {
...
void CGeneralizedCylinder::readFromStream(mrpt::utils::CStream &in,
                                          int version) {
  switch (version) {
  case 0: {
...
void generatePolygon(CPolyhedron::Ptr &poly, const vector<TPoint3D> &profile,
                     const CPose3D &pose) {
  math::TPolygon3D p(profile.size());
  for (size_t i = 0; i < profile.size(); i++)
    pose.composePoint(profile[i].x, profile[i].y, profile[i].z, p[i].x, p[i].y,
                      p[i].z);
...
bool CGeneralizedCylinder::getFirstVisibleSectionPose(CPose3D &p) {
  if (fullyVisible)
    return getFirstSectionPose(p);
  if (getVisibleSections() <= 0)
    return false;
  p = axis[firstSection];
  return true;
}
...
void CGeneralizedCylinder::getBoundingBox(mrpt::math::TPoint3D &bb_min,
