...
IMPLEMENTS_SERIALIZABLE(CEllipsoidInverseDepth3D, CRenderizableDisplayList,
                        mrpt::opengl)
...
void CEllipsoidInverseDepth3D::transformFromParameterSpace(
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    const double range = inv_range < 0 ? m_underflowMaxRange
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CEllipsoidInverseDepth3D::writeToStream(mrpt::utils::CStream &out,
                                             int *version) const {
  if (version)
    *version = 0;
  else {
...
/*---------------------------------------------------------------
        Implements the reading from a CStream capability of
                CSerializable objects
  ---------------------------------------------------------------*/
void CEllipsoidInverseDepth3D::readFromStream(mrpt::utils::CStream &in,
                                              int version) {
  switch (version) {
  case 0: {
