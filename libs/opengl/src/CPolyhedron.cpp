...
void insertCupola(size_t numBaseEdges, double angleShift, double baseRadius,
                  double edgeLength, bool isRotated, bool isUpwards,
  ...
  double h =
      sqrt(square(edgeLength) - square(baseRadius * cos(M_PI / numBaseEdges) -
                                       minorRadius * cos(M_PI / edges2)));
  double height = verts[base].z + (isUpwards ? h : -h);
  angleShift +=
      M_PI / edges2 + (isRotated ? -M_PI / numBaseEdges : M_PI / numBaseEdges);
  ...
  if (edges2 >= 3)
    faces.push_back(cBase);
}
void insertRotunda(double angleShift, double baseRadius, bool isRotated,
                   bool isUpwards, size_t base, vector<TPoint3D> &verts,
                   vector<CPolyhedron::TPolyhedronFace> &faces) {
...
inline bool faceContainsEdge(const CPolyhedron::TPolyhedronFace &f,
...
bool searchForFace(const vector<CPolyhedron::TPolyhedronFace> &fs, uint32_t v1,
...
bool searchForEdge(const vector<CPolyhedron::TPolyhedronEdge> &es, uint32_t v1,
                   uint32_t v2, size_t &where) {
...
double getHeight(const TPolygon3D &p, const TPoint3D &c) {
  ...
  for (size_t i = 1; i < N; i++)
    if (abs(mrpt::math::distance(p[i], c) - r) >= mrpt::math::geometryEpsilon)
      throw std::logic_error("There is a non-regular polygon.");
    else if (abs(mrpt::math::distance(p[i], p[(i + 1) % N]) - l) >=
...
double
CPolyhedron::TPolyhedronEdge::length(const vector<TPoint3D> &vertices) const {
...
double CPolyhedron::TPolyhedronFace::area(const vector<TPoint3D> &vs) const {
  ...
  for (size_t i = 1; i < N - 1; i++)
    res += sqrt(square(d[i - 1].mod * d[i].mod) -
                square(dotProduct<3, double>(d[i - 1], d[i])));
  return res / 2;
}

void CPolyhedron::TPolyhedronFace::getCenter(const vector<TPoint3D> &vrts,
                                             TPoint3D &p) const {
  p.x = p.y = p.z = 0.0;
...
CPolyhedron::CPolyhedron(const vector<TPoint3D> &vertices,
...
CPolyhedron::Ptr CPolyhedron::CreateCubicPrism(double x1, double x2, double y1,
                                               double y2, double z1,
                                               double z2) {
...
CPolyhedron::Ptr
CPolyhedron::CreatePyramid(const vector<TPoint2D> &baseVertices,
                           double height) {
  uint32_t n = baseVertices.size();
  if (baseVertices.size() < 3)
    throw std::logic_error("Not enought vertices");
  vector<TPoint3D> verts;
  vector<TPolyhedronFace> faces;
...
CPolyhedron::Ptr
CPolyhedron::CreateDoublePyramid(const vector<TPoint2D> &baseVertices,
...
CPolyhedron::Ptr
CPolyhedron::CreateTruncatedPyramid(const vector<TPoint2D> &baseVertices,
  ...
  for (uint32_t i = 0; i < n; i++) {
    ...
    verts[i + n] =
        TPoint3D(baseVertices[i].x * ratio, baseVertices[i].y * ratio, height);
    uint32_t ii = (i + 1) % n;
    f.vertices[0] = i;
    f.vertices[1] = ii;
    f.vertices[2] = ii + n;
...
CPolyhedron::Ptr
CPolyhedron::CreateCustomAntiprism(const vector<TPoint2D> &bottomBase,
...
CPolyhedron::Ptr CPolyhedron::CreateParallelepiped(const TPoint3D &base,
                                                   const TPoint3D &v1,
                                                   const TPoint3D &v2,
                                                   const TPoint3D &v3) {
...
CPolyhedron::Ptr
CPolyhedron::CreateBifrustum(const vector<TPoint2D> &baseVertices,
                             double height1, double ratio1, double height2,
                             double ratio2) {
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    verts[i + N2].z = -height2; // This is not an error. This way, two positive
                                // heights produce an actual bifrustum.
  }
...
CPolyhedron::Ptr CPolyhedron::CreateTrapezohedron(uint32_t numBaseEdges,
  ...
  if (numBaseEdges < 3)
    throw std::logic_error("Not enough vertices");
  if (basesDistance == 0 || baseRadius == 0)
    return CreateEmpty();
...
CPolyhedron::Ptr CPolyhedron::CreateJohnsonSolidWithConstantBase(
  ...
  double antiPrismHeight =
      sqrt(square(edgeLength) -
  ...
  for (vector<pair<double, size_t>>::const_iterator it =
    ...
    generateShiftedBase(numBaseEdges, baseRadius, it->first - mHeight,
                        semi * it->second, verts);
  ...
  for (size_t i = 0; i < nParts; i++) {
    ...
    case UPWARDS_CUPOLA:
      ...
      insertCupola(numBaseEdges, basePositionInfo.rbegin()->second * semi,
    ...
    case DOWNWARDS_CUPOLA:
      ...
      insertCupola(numBaseEdges, basePositionInfo[0].second * semi, baseRadius,
                   edgeLength, false, false, endBase, verts, faces);
      break;
    case ROTATED_UPWARDS_CUPOLA:
      // Upwards-pointing, slightly rotated, cupola. There must be an even
      // amount of vertices.
      insertCupola(numBaseEdges, basePositionInfo.rbegin()->second * semi,
    ...
    case ROTATED_DOWNWARDS_CUPOLA:
      // Downwards-pointing, slightly rotated, cupola. There must be an even
      // amount of vertices.
      insertCupola(numBaseEdges, basePositionInfo[0].second * semi, baseRadius,
    ...
    case UPWARDS_ROTUNDA:
      // Upwards-pointing pentagonal rotunda. Only for bases of exactly 10
      // vertices.
      insertRotunda(basePositionInfo.rbegin()->second * semi, baseRadius, false,
                    true, initialBase, verts, faces);
      break;
    case DOWNWARDS_ROTUNDA:
      // Downwards-pointing pentagonal rotunda. Only for bases of exactly 10
      // vertices.
      insertRotunda(basePositionInfo[0].second * semi, baseRadius, false, false,
                    endBase, verts, faces);
      break;
    case ROTATED_UPWARDS_ROTUNDA:
      // Upwards-pointing, slightly rotated, pentagonal rotunda. Only for bases
      // of exactly 10 vertices.
      insertRotunda(basePositionInfo.rbegin()->second * semi, baseRadius, true,
                    true, initialBase, verts, faces);
      break;
    case ROTATED_DOWNWARDS_ROTUNDA:
      // Downwards-pointing, slightly rotated, pentagonal rotunda. Only for
      // bases of exactly 10 vertices.
      insertRotunda(basePositionInfo[0].second * semi, baseRadius, true, false,
                    endBase, verts, faces);
      break;
    default:
...
double CPolyhedron::getVolume() const {
  // TODO. Calculate as set of pyramids whose apices are situated in the center
  // of the polyhedron (will work only with convex polyhedrons).
  // Pyramid volume=V=1/3*base area*height. Height=abs((A-V)·N), where A is the
  // apex, V is any other vertex, N is the base's normal vector and (·) is the
  // scalar product.
  TPoint3D center;
  getCenter(center);
  double res = 0;
...
void CPolyhedron::getSetOfPolygonsAbsolute(
  ...
  for (size_t i = 0; i < N; i++)
    pose.composePoint(mVertices[i], nVerts[i]);
  transform(mFaces.begin(), mFaces.end(), vec.begin(),
...
void CPolyhedron::makeConvexPolygons() {
  ...
  for (vector<TPolygon3D>::const_iterator it = polys.begin(); it != polys.end();
       ++it)
...
void CPolyhedron::getCenter(TPoint3D &center) const {
  size_t N = mVertices.size();
  if (N == 0)
    throw new std::logic_error("There are no vertices");
...
CPolyhedron::Ptr CPolyhedron::CreateRandomPolyhedron(double radius) {
  ...
  case 23:
    return CreateArchimedeanRegularPrism(
        (mrpt::random::randomGenerator.drawUniform32bit() % 10) + 3, radius);
  case 24:
    return CreateArchimedeanRegularAntiprism(
        (mrpt::random::randomGenerator.drawUniform32bit() % 10) + 3, radius);
  case 25:
    return CreateJohnsonSolidWithConstantBase(
        ((mrpt::random::randomGenerator.drawUniform32bit() % 4) << 1) + 4,
        radius, "C+");
  case 26:
    return CreateJohnsonSolidWithConstantBase(
        ((mrpt::random::randomGenerator.drawUniform32bit() % 4) << 1) + 4,
  ...
  case 32:
    return CreateCatalanTrapezohedron(
        (mrpt::random::randomGenerator.drawUniform32bit() % 5) + 3, radius);
  case 33:
...
CPolyhedron::Ptr CPolyhedron::getDual() const {
  // This methods builds the dual of a given polyhedron, which is assumed to be
  // centered in (0,0,0), using polar reciprocation.
  ...
  for (vector<TPolyhedronFace>::iterator it = faces.begin(); it != faces.end();
       ++it) {
    ...
    for (size_t i = 0; i < NV; i++) {
      ...
      for (size_t j = 1; j < sf - 1; j++) {
        ...
        for (;;) {
          ...
          if (!((e1.v1 == i || e1.v2 == i) && (e2.v1 == i || e2.v2 == i)))
            THROW_EXCEPTION("En algo te has equivocado, chaval.");
          if (searchForFace(mFaces, i, (e1.v1 == i) ? e1.v2 : e1.v1,
                            (e2.v1 == i) ? e2.v2 : e2.v1))
            break;
          uint32_t tmpV = f[j];
          f.erase(f.begin() + j);
    ...
    for (size_t i = 0; i < NV; i++) {
      ...
      for (size_t j = 1; j < sf - 1; j++) {
        ...
        for (;;) {
          ...
          if (!((e1.v1 == i || e1.v2 == i) && (e2.v1 == 1 || e2.v2 == i)))
            THROW_EXCEPTION("En algo te has equivocado, chaval.");
          if (searchForFace(mFaces, i, (e1.v1 == i) ? e1.v2 : e1.v1,
                            (e2.v1 == i) ? e2.v2 : e2.v1))
            break;
...
CPolyhedron::Ptr CPolyhedron::cantellate(double factor) const {
  ...
  for (size_t i = 0; i < NF; i++) {
    ...
    for (size_t j = 0; j < oPS; j++) {
      ...
      size_t edge;
      searchForEdge(mEdges, curr, mFaces[i].vertices[(j + oPS - 1) % oPS],
                    edge);
...
void CPolyhedron::generateBase(uint32_t numBaseEdges, double baseRadius,
...
void CPolyhedron::generateShiftedBase(uint32_t numBaseEdges, double baseRadius,
...
void CPolyhedron::updatePolygons() const {
  ...
  transform(mFaces.begin(), mFaces.end(), tempPolygons.begin(),
...
bool CPolyhedron::checkConsistence(const vector<TPoint3D> &vertices,
                                   const vector<TPolyhedronFace> &faces) {
  size_t N = vertices.size();
  if (vertices.size() > 0)
    for (vector<TPoint3D>::const_iterator it = vertices.begin();
         it != vertices.end() - 1; ++it)
      for (vector<TPoint3D>::const_iterator it2 = it + 1; it2 != vertices.end();
           ++it2)
        if (*it == *it2)
          return false;
...
/*CPolyhedron::Ptr CPolyhedron::CreateCuboctahedron(double radius)	{
        ...
        verts.push_back(TPoint3D(-d,0,-d));
        TPolyhedronFace f;
        static uint32_t faces3[]={0,1,4, 0,2,5, 1,3,6, 2,3,7, 8,9,4, 8,10,5,
9,11,6, 10,11,7};
        static uint32_t faces4[]={0,1,3,2, 8,9,11,10, 0,4,8,5, 1,4,9,6,
2,5,10,7, 3,6,11,7};
...
CPolyhedron::Ptr CPolyhedron::CreateRhombicuboctahedron(double radius,
                                                        bool type) {
  return CreateJohnsonSolidWithConstantBase(
      8, radius / sqrt(1 + square(sin(M_PI / 8))), type ? "C-PRC+" : "GC-PRC+",
      3);
}
CPolyhedron::Ptr CPolyhedron::CreateIcosidodecahedron(double radius,
                                                      bool type) {
  return CreateJohnsonSolidWithConstantBase(10, radius, type ? "GR-R+" : "R-R+",
                                            1);
}
...
CPolyhedron::Ptr CPolyhedron::CreateCubicPrism(const TPoint3D &p1,
                                               const TPoint3D &p2) {
  return CreateCubicPrism(p1.x, p2.x, p1.y, p2.y, p1.z, p2.z);
}
CPolyhedron::Ptr
CPolyhedron::CreateFrustum(const vector<TPoint2D> &baseVertices, double height,
                           double ratio) {
  return CreateTruncatedPyramid(baseVertices, height, ratio);
}
CPolyhedron::Ptr
CPolyhedron::CreateCustomPrism(const vector<TPoint2D> &baseVertices,
                               double height) {
...
CPolyhedron::Ptr CPolyhedron::CreateRegularAntiprism(uint32_t numBaseEdges,
                                                     double baseRadius,
                                                     double height) {
  return CreateCustomAntiprism(generateBase(numBaseEdges, baseRadius),
                               generateShiftedBase(numBaseEdges, baseRadius),
                               height);
}
...
CPolyhedron::Ptr CPolyhedron::CreateRegularDoublePyramid(uint32_t numBaseEdges,
                                                         ...
                                                         double height1,
                                                         double height2) {
  return CreateDoublePyramid(generateBase(numBaseEdges, baseRadius), height1,
                             height2);
}
CPolyhedron::Ptr
CPolyhedron::CreateArchimedeanRegularPrism(uint32_t numBaseEdges,
                                           double baseRadius) {
  return CreateJohnsonSolidWithConstantBase(numBaseEdges, baseRadius, "PR");
}
CPolyhedron::Ptr
CPolyhedron::CreateArchimedeanRegularAntiprism(uint32_t numBaseEdges,
                                               double baseRadius) {
...
CPolyhedron::Ptr CPolyhedron::CreateRegularTruncatedPyramid(
  ...
  return CreateTruncatedPyramid(generateBase(numBaseEdges, baseRadius), height,
                                ratio);
}
...
CPolyhedron::Ptr
CPolyhedron::CreateRegularBifrustum(uint32_t numBaseEdges, double baseRadius,
                                    double height1, double ratio1,
                                    double height2, double ratio2) {
  return CreateBifrustum(generateBase(numBaseEdges, baseRadius), height1,
                         ratio1, height2, ratio2);
}
...
CPolyhedron::Ptr
CPolyhedron::CreateNoCheck(const vector<TPoint3D> &vertices,
                           const vector<TPolyhedronFace> &faces) {
