...
struct _player_texture {
  ...
  float scale_x, scale_y; // scale the texcoords, as OpenGL thinks in TEX_XSIZE
                          // and TEX_YSIZE
};
    ...
    if (!mesh->user.d) {
...

      {
        ...
        Lib3dsVector *normalL =
            (Lib3dsVector *)malloc(3 * sizeof(Lib3dsVector) * mesh->faces);
        ...
        for (p = 0; p < mesh->faces; ++p) {
          ...
          if (mat != oldmat) {
            if (mat) {
              ...
              /*
                            if (mat->texture1_map.name[0]) {		// texture
              map?
                              Lib3dsTextureMap *tex = &mat->texture1_map;
                              if (!tex->user.p) {		// no player texture
              yet?
                                char texname[1024];
                                pt = (Player_texture*) malloc(sizeof(*pt));
                                                tex->user.p = pt;
                                snprintf(texname, sizeof(texname), "%s/%s",
              datapath, tex->name);
              ...
              #else
                                pt->bitmap = nullptr;
                                fputs("3dsplayer: Warning: No image loading
              support, skipping texture.\n", stderr);
              #endif // USE_SDL
                                if (pt->bitmap) {	// could image be
              loaded ?
                                  // this OpenGL texupload code is incomplete
              format-wise!
                                                      // to make it complete,
              examine SDL_surface->format and
                                  //  tell us @lib3ds.sf.net about your
              improvements :-)
                                  //int upload_format = GL_RED; // safe choice,
              shows errors
              #ifdef USE_SDL
                                  int bytespp =
              ...
              #ifdef	DEBUG
                                  printf("Uploading texture to OpenGL, id %d, at
              %d bytepp\n",
                                    pt->tex_id, bytespp);
              #endif
                                  if (pt->bitmap->format->palette) {
                                    ...
                                    if (bytespp == 1) upload_format =
              GL_LUMINANCE;
                                    else if (bytespp == 3) upload_format =
              GL_RGB;
                                    else if (bytespp == 4) upload_format =
              GL_RGBA;
                                  }
            ...
            for (i = 0; i < 3; ++i) {
              ...
              if (tex_mode) {
                glTexCoord2f(mesh->texelL[f->points[i]][1] * pt->scale_x,
                             pt->scale_y -
                                 mesh->texelL[f->points[i]][0] * pt->scale_y);
              }
...
void C3DSScene::loadFrom3DSFile(const std::string &filepath) {
  ...
  lib3ds_file_bounding_box_of_nodes(file, LIB3DS_TRUE, LIB3DS_FALSE,
                                    LIB3DS_FALSE, bmin, bmax);
...
void C3DSScene::getBoundingBox(mrpt::math::TPoint3D &bb_min,
