...
IMPLEMENTS_VIRTUAL_SERIALIZABLE(CTexturedObject, CRenderizableDisplayList,
                                mrpt::opengl)
...
CTexturedObject::CTexturedObject()
    : m_glTextureName(0), m_texture_is_loaded(false),
...
#endif
  data.resize(len);
  return ((unsigned char *)(((POINTER_TYPE)&data[0]) &
                            (~((POINTER_TYPE)0x0F)))) +
         0x10;
}
...
void CTexturedObject::loadTextureInOpenGL() const {
  ...
  try {
    ...
    //  occupancy grid maps, such as we want those "big pixels" to be clearly
    //  visible ;-)
    ...
    // Assure that the images do not overpass the maximum dimensions allowed by
    // OpenGL:
    // ------------------------------------------------------------------------------------
    GLint texSize;
...
#ifdef TEXTUREOBJ_PROFILE_MEM_ALLOC
    {
      const std::string sSec =
          mrpt::format("opengl_texture: load %ix%i %s %stransp", width, height,
...
// --------------------------------------
        ...
        const bool is_RGB_order =
            (!::strcmp(m_textureImage.getChannelsOrder(),
        ...
        glTexImage2D(GL_TEXTURE_2D, 0 /*level*/, 4 /* RGB components */, width,
      ...
      if (m_enableTransparency) {
        ...
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
        glPixelStorei(GL_UNPACK_ROW_LENGTH, width);
        glTexImage2D(GL_TEXTURE_2D, 0 /*level*/, 2 /* RGB components */, width,
                     height, 0 /*border*/, img_format, img_type, dataAligned);
        checkOpenGLError();
        glPixelStorei(GL_UNPACK_ROW_LENGTH, 0); // Reset
        ...
        glTexImage2D(GL_TEXTURE_2D, 0 /*level*/, 1 /* RGB components */, width,
                     height, 0 /*border*/, img_format, img_type,
                     m_textureImage(0, 0));
        checkOpenGLError();
        glPixelStorei(GL_UNPACK_ROW_LENGTH, 0); // Reset
...
void CTexturedObject::render_pre() const {
  ...
  // This will load and/or select our texture, only if "m_texture_is_loaded" is
  // false
  loadTextureInOpenGL();
  MRPT_END
#endif
