...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CSetOfTriangles::writeToStream(mrpt::utils::CStream &out,
                                    int *version) const {
...
bool CSetOfTriangles::traceRay(const mrpt::poses::CPose3D &o,
                               double &dist) const {
  if (!polygonsUpToDate)
    updatePolygons();
...
&dist,const float xb[3],const float yb[3],const float zb[3])	{
        ...
        float z[3];
        for (int i=0;i<3;i++)
transf.composePoint(xb[i],yb[i],zb[i],x[i],y[i],z[i]);

        //If the triangle is parallel to the beam, no collision is posible.
        //The triangle is parallel to the beam if the projection of the triangle
to the YZ plane results in a line.
        float lCoefs[3];
        ...
        //M=|p2|=|x2 y2 z2|
        //  \p3/ \x3 y3 z3/
        //If M has rank 3, then (p1,p2,p3) conform a plane which does not
contain the origin (0,0,0).
...
which contains the origin.
        ...
        //N=/p2-p1\ = /x1 y1 z1\ .
        //  \p3-p1/   \x3 y3 z3/
        //Given that the rank of M is 2, if the rank of N is still 2 then
(p1,p2,p3) conform a plane; either, a line.
        float mat[9];
        for (int i=0;i<3;i++)	{
                mat[3*i]=x[i];
                mat[3*i+1]=y[i];
        ...
        switch (M.rank())	{
                case 3:
                        //M's rank is 3, so the triangle is inside a plane which
doesn't pass through (0,0,0).
                        //This plane's equation is Ax+By+Cz+1=0. Since the point
we're searching for verifies y=0 and z=0, we
                        //only need to compute A (x=-1/A). We do this using
Cramer's method.
                        ...
                        break;
                case 2:
                        //if N's rank is 2, the triangle is inside a plane
containing (0,0,0).
                        ...
                        if (CMatrixTemplateNumeric<float>(2,3,mat2).rank()==2)
dist=0;
                        else return false;
                        break;
                default:
...
plane, but we need to check if this
        //point is actually inside the triangle. We do this by projecting the
scene into a <x=constant> plane, so
        //that the triangle is defined by three 2D lines and the beam is the
point (y,z)=(0,0).

        //For each pair of points, we compute the line that they conform, and
then we check if the other point's
        //sign in that line's equation equals that of the origin (that is, both
points are on the same side of the line).
        //If this holds for each one of the three possible combinations, then
the point is inside the triangle.
        //Furthermore, if any of the three equations verify f(0,0)=0, then the
point is in the verge of the line, which
...
((lCoefs[0]*y[0]+lCoefs[1]*z[0]+lCoefs[2])>0)==(lCoefs[2]>0);
}
*/
...
void CSetOfTriangles::getBoundingBox(mrpt::math::TPoint3D &bb_min,
                                     mrpt::math::TPoint3D &bb_max) const {
  bb_min = mrpt::math::TPoint3D(std::numeric_limits<double>::max(),
                                std::numeric_limits<double>::max(),
                                std::numeric_limits<double>::max());
  bb_max = mrpt::math::TPoint3D(-std::numeric_limits<double>::max(),
                                -std::numeric_limits<double>::max(),
                                -std::numeric_limits<double>::max());
...
void CSetOfTriangles::insertTriangles(const CSetOfTriangles::Ptr &p) {
  ...
  m_triangles.insert(m_triangles.end(), p->m_triangles.begin(),
                     p->m_triangles.end());
  polygonsUpToDate = false;
  CRenderizableDisplayList::notifyChange();
}
