...
// Ctor:
CLight::CLight()
    : light_ID(0), constant_attenuation(1.f), linear_attenuation(0.f),
      quadratic_attenuation(0.f), spot_exponent(0.f), spot_cutoff(180.f) {
...
void CLight::writeToStream(mrpt::utils::CStream &out) const {
  ...
  out << light_ID << color_ambient[0] << color_ambient[1] << color_ambient[2]
      << color_ambient[3] << color_diffuse[0] << color_diffuse[1]
      << color_diffuse[2] << color_diffuse[3] << color_specular[0]
      << color_specular[1] << color_specular[2] << color_specular[3]
      << position[0] << position[1] << position[2] << position[3]
      << direction[0] << direction[1] << direction[2] << constant_attenuation
      << linear_attenuation << quadratic_attenuation << spot_exponent
      << spot_cutoff;
}
...
void CLight::readFromStream(mrpt::utils::CStream &in) {
  ...
  switch (version) {
  case 0:
    in >> light_ID >> color_ambient[0] >> color_ambient[1] >>
        color_ambient[2] >> color_ambient[3] >> color_diffuse[0] >>
        color_diffuse[1] >> color_diffuse[2] >> color_diffuse[3] >>
        color_specular[0] >> color_specular[1] >> color_specular[2] >>
        color_specular[3] >> position[0] >> position[1] >> position[2] >>
        position[3] >> direction[0] >> direction[1] >> direction[2] >>
        constant_attenuation >> linear_attenuation >> quadratic_attenuation >>
        spot_exponent >> spot_cutoff;
    break;
  default:
