...
IMPLEMENTS_SERIALIZABLE(COpenGLStandardObject, CRenderizableDisplayList,
                        mrpt::opengl)
...
void COpenGLStandardObject::render_dl() const {
  ...
  // This line won't take any effect if GL_BLEND is not enabled, so it's safe to
  // always execute it.
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void COpenGLStandardObject::writeToStream(mrpt::utils::CStream &out,
                                          int *version) const {
  if (version)
    *version = 1;
  else {
...
/*---------------------------------------------------------------
        Implements the reading from a CStream capability of
                CSerializable objects
  ---------------------------------------------------------------*/
void COpenGLStandardObject::readFromStream(mrpt::utils::CStream &in,
                                           int version) {
  switch (version) {
  case 1: {
...
bool COpenGLStandardObject::traceRay(const mrpt::poses::CPose3D &o,
                                     double &dist) const {
  MRPT_UNUSED_PARAM(o);
...
void COpenGLStandardObject::getBoundingBox(mrpt::math::TPoint3D &bb_min,
