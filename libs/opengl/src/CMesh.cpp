...
CMesh::CMesh(bool enableTransparency, float xMin, float xMax, float yMin,
             float yMax)
    : m_textureImage(0, 0), m_enableTransparency(enableTransparency),
      m_colorFromZ(false), m_isWireFrame(false), m_isImage(false), Z(0, 0),
      mask(0, 0), U(0, 0), V(0, 0), C(0, 0), C_r(0, 0), C_g(0, 0), C_b(0, 0),
      m_colorMap(mrpt::utils::cmHOT), m_modified_Z(true),
...
void CMesh::updateTriangles() const {
  ...
  vertex_normals.assign((1 + cols) * (1 + rows),
  ...
  for (size_t iX = 0; iX < rows - 1; iX++)
    for (size_t iY = 0; iY < cols - 1; iY++) {
      if (useMask && (!mask(iX, iY) || !mask(iX + 1, iY + 1)))
        continue;
      ...
      if (!useMask || mask(iX + 1, iY)) {
        ...
        // Compute normal of this triangle, and add it up to the 3 neighboring
        // vertices:
        // A = P1 - P0, B = P2 - P0
        ...
        const TPoint3D this_normal(ay * bz - az * by, az * bx - ax * bz,
                                   ax * by - ay * bx);
        ...
        actualMesh.push_back(
            std::pair<CSetOfTriangles::TTriangle, TTriangleVertexIndices>(tri,
                                                                          tvi));
...
void CMesh::render_dl() const {
  ...
  glEnable(GL_NORMALIZE); // So the GPU normalizes the normals instead of doing
                          // it in the CPU
  glEnable(GL_COLOR_MATERIAL);
  glShadeModel(GL_SMOOTH);
  if (!trianglesUpToDate)
    updateTriangles();
  if (!m_isWireFrame)
...
void CMesh::assignImageAndZ(
  ...
  ASSERT_((img.getWidth() == static_cast<size_t>(in_Z.cols())) &&
...
void CMesh::setUV(const mrpt::math::CMatrixTemplateNumeric<float> &in_U,
...
bool CMesh::traceRay(const mrpt::poses::CPose3D &o, double &dist) const {
  if (!trianglesUpToDate || !polygonsUpToDate)
    updatePolygons();
...
createPolygonFromTriangle(const std::pair<CSetOfTriangles::TTriangle,
                                          CMesh::TTriangleVertexIndices> &p) {
...
void CMesh::updatePolygons() const {
  ...
  tmpPolys.resize(N);
  transform(actualMesh.begin(), actualMesh.end(), tmpPolys.begin(),
            createPolygonFromTriangle);
  polygonsUpToDate = true;
  CRenderizableDisplayList::notifyChange();
}

void CMesh::getBoundingBox(mrpt::math::TPoint3D &bb_min,
