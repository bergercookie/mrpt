...
CRenderizable::CRenderizable()
    : m_name(), m_show_name(false), m_color(255, 255, 255, 255), m_pose(),
      m_scale_x(1), m_scale_y(1), m_scale_z(1), m_visible(true) {}
...
unsigned int CRenderizable::getNewTextureNumber() {
  ...
  while (ret != 0 && booker.freeTextureNames[ret]) {
    ...
    if (++tries >= MAX_GL_TEXTURE_IDS)
      THROW_EXCEPTION_FMT(
          "Maximum number of textures (%u) excedeed! (are you deleting them?)",
...
void CRenderizable::releaseTextureName(unsigned int i) {
  ...
  // "glDeleteTextures" seems not to be neeeded, since we do the reservation of
  // texture names by our own.
}
...
void CRenderizable::readFromStreamRender(mrpt::utils::CStream &in) {
  ...
  // Read signature:
  union {
    uint8_t magic_signature[2 + 2];  // (the extra 4 bytes will be used only for
                                     // the old format)
    uint32_t magic_signature_uint32; // So we can interpret the 4bytes above as
                                     // a 32bit number cleanly.
  };
  ...
  if (is_new_format) {
    ...
    default:
      THROW_EXCEPTION_FMT("Can't parse CRenderizable standard data field: "
                          "corrupt data stream or format in a newer MRPT "
                          "format? (serialization version=%u)",
    ...
    m_color = mrpt::utils::TColor(col.R / 255, col.G / 255, col.B / 255,
                                  col.A / 255); // For some stupid reason,
                                                // colors were saved multiplied
                                                // by 255... (facepalm)
    ...
    m_pose.setYawPitchRoll(DEG2RAD(yaw_deg), DEG2RAD(pitch_deg),
                           DEG2RAD(roll_deg));
  }
}
...
CRenderizable &CRenderizable::setPose(
