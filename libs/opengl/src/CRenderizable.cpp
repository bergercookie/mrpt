...
void CRenderizable::readFromStreamRender(mrpt::utils::CStream& in)
{
	...
	union {
		uint8_t
			magic_signature[2 + 2];  // (the extra 4 bytes will be used only for
		// the old format)
		uint32_t
			magic_signature_uint32;  // So we can interpret the 4bytes above as
		...
		switch (serialization_version)
		{
			...
			default:
				THROW_EXCEPTION_FMT(
					...
					"data "
					"stream or format in a newer MRPT format? (serialization "
					"version=%u)",
	...
	else
	{
		...
		m_color = mrpt::utils::TColor(
			...
			col.A /
				255);  // For some stupid reason, colors were saved multiplied
		// by 255... (facepalm)
