...
void CCylinder::render_dl() const {
  ...
  if (mHasBottomBase)
    gluDisk(obj, 0, mBaseRadius, mSlices, 1);
...
bool solveEqn(double a, double b, double c,
              double &t) { // Actually, the b from the quadratic equation is the
                           // DOUBLE of this. But this way, operations are
                           // simpler.
  if (a < 0) {
    a = -a;
    b = -b;
...
bool CCylinder::traceRay(const mrpt::poses::CPose3D &o, double &dist) const {
  ...
  lin.unitarize(); // By adding this line, distance from any point of the line
                   // to its base is exactly equal to the "t".
  if (abs(lin.director[2]) < geometryEpsilon) {
    if (!reachesHeight(lin.pBase.z))
      return false;
    float r;
    return getRadius(static_cast<float>(lin.pBase.z), r)
               ? solveEqn(square(lin.director[0]) + square(lin.director[1]),
  ...
  if (mHasBottomBase && (tZ0 = -lin.pBase.z / lin.director[2]) > 0) {
    nDist = sqrt(square(lin.pBase.x + tZ0 * lin.director[0]) +
  ...
  if (mBaseRadius == mTopRadius) {
    if (solveEqn(square(lin.director[0]) + square(lin.director[1]),
...
void CCylinder::getBoundingBox(mrpt::math::TPoint3D &bb_min,
