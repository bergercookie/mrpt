...
bool tfest::se3_l2_robust(
  ...
  ASSERTMSG_(d >= n, "Minimum number of points to be considered a good set is "
  ...
  for (size_t iterations = 0; iterations < max_it; iterations++) {
    ...
    if (cSet.size() < n) {
      if (params.verbose)
        std::cerr << "[tfest::se3_l2_robust] Iter " << iterations
                  << ": It was not possible to find the min no of (compatible) "
                     "matching pairs.\n";
      continue; // Try again
    }

    CPose3DQuat mbOutQuat;
    const bool res = mrpt::tfest::se3_l2(mbInliers, mbOutQuat, scale,
                                         params.forceScaleToUnity);
    if (!res) {
      std::cerr << "[tfest::se3_l2_robust] tfest::se3_l2() returned false for "
                   "tentative subset during RANSAC iteration!\n";
      continue;
    }
    ...
    for (size_t k = n; k < N; k++) {
      ...
      const bool res = mrpt::tfest::se3_l2(mbInliers, csOutQuat, scale,
                                           params.forceScaleToUnity);
      mbInliers.erase(mbInliers.end() - 1); // Erase

      if (!res) {
        std::cerr << "[tfest::se3_l2_robust] tfest::se3_l2() returned false "
                     "for tentative subset during RANSAC iteration!\n";
        continue;
      }
      ...
      if (fabs(mbOut_vec[0] - csOut.x()) < th[0] &&
        ...
        // cout << " It " << iterations << " - RANSAC Outlier Detected: " << k
        // << endl;
      }
    } // end 'inner' for
    ...
    if (cSet.size() >= d) {
      ...
      const bool res =
          mrpt::tfest::se3_l2(cSetInliers, cIOutQuat, scale,
                              params.forceScaleToUnity); // Compute output
      ASSERTMSG_(res, "tfest::se3_l2() returned false for tentative subset "
                      "during RANSAC iteration!")

      // Compute error for consensus_set
      const CPose3D cIOut = CPose3D(cIOutQuat);
      const double err = std::sqrt(
          square(mbOut_vec[0] - cIOut.x()) + square(mbOut_vec[1] - cIOut.y()) +
          square(mbOut_vec[2] - cIOut.z()) +
          square(mbOut_vec[3] - cIOut.yaw()) +
          square(mbOut_vec[4] - cIOut.pitch()) +
          square(mbOut_vec[5] - cIOut.roll()) + square(mbOut_vec[6] - scale));
