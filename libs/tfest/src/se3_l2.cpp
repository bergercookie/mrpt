...
//		ct_others = (1/n)*sum{i}( pLi )		ct_others = { cLx, cLy,
//cLz }
//		ct_this = (1/n)*sum{i}( pRi )		ct_this = { cRx, cRy, cRz
//}
//
// 2. Substract centroids from the point coordinates:
//		pLi' = pLi - ct_others					pLi' = {
//pLix', pLiy', pLiz' }
//		pRi' = pRi - ct_this					pRi' = { pRix',
//pRiy', pRiz' }
//
// 3. For each pair of coordinates (correspondences) compute the nine possible
// products:
//		pi1 = pLix'*pRix'		pi2 = pLix'*pRiy'		pi3 =
//pLix'*pRiz'
//		pi4 = pLiy'*pRix'		pi5 = pLiy'*pRiy'		pi6 =
//pLiy'*pRiz'
//		pi7 = pLiz'*pRix'		pi8 = pLiz'*pRiy'		pi9 =
//pLiz'*pRiz'
//
// 4. Compute S components:
//		Sxx = sum{i}( pi1 )		Sxy = sum{i}( pi2 )		Sxz
//= sum{i}( pi3 )
//		Syx = sum{i}( pi4 )		Syy = sum{i}( pi5 )		Syz
//= sum{i}( pi6 )
//		Szx = sum{i}( pi7 )		Szy = sum{i}( pi8 )		Szz
//= sum{i}( pi9 )
//
// 5. Compute N components:
//			[ Sxx+Syy+Szz	Syz-Szy			Szx-Sxz
//Sxy-Syx		 ]
//			[ Syz-Szy		Sxx-Syy-Szz		Sxy+Syx
//Szx+Sxz		 ]
//		N = [ Szx-Sxz		Sxy+Syx			-Sxx+Syy-Szz
//Syz+Szy		 ]
...
bool se3_l2_internal(
  ...
  N.set_unsafe(0, 0,
               S.get_unsafe(0, 0) + S.get_unsafe(1, 1) + S.get_unsafe(2, 2));
  ...
  ASSERTDEB_(fabs(sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2] + v[3] * v[3]) -
                  1.0) < 0.1);
  ...
  TPoint3D pp;
  out_transform.composePoint(ct_others.x, ct_others.y, ct_others.z, pp.x, pp.y,
                             pp.z);
  pp *= s;
...
bool tfest::se3_l2(const std::vector<mrpt::math::TPoint3D> &in_points_this,
  ...
  return se3_l2_internal(points_this, points_other, out_transform, out_scale,
                         forceScaleToUnity);
}

bool tfest::se3_l2(const mrpt::utils::TMatchingPairList &corrs,
                   mrpt::poses::CPose3DQuat &out_transform, double &out_scale,
                   bool forceScaleToUnity) {
