...
/*---------------------------------------------------------------
 ...
 This works as follows:
        - Repeat "results.ransac_iters" times:
                - Randomly pick TWO correspondences from the set
 "in_correspondences".
                - Compute the associated rigid transformation.
                - For "ransac_maxSetSize" randomly selected correspondences,
 test for "consensus" with the current group:
                        - If if is compatible (ransac_maxErrorXY,
...
//#define DO_PROFILING
  ...
  for (TMatchingPairList::const_iterator matchIt = in_correspondences.begin();
       matchIt != in_correspondences.end(); ++matchIt) {
    if (!hasCorrThis[matchIt->this_idx] && !hasCorrOther[matchIt->other_idx]) {
  ...
  if (use_dynamic_iter_number) {
    ASSERT_(params.probability_find_good_model > 0 &&
       ...
       iter_idx++) // results.ransac_iters can be dynamic
  {
    ...
    if (params.ransac_algorithmForLandmarks) {
      ...
      // For points: Do not repeat the corrs, and take the number of corrs as
      // weights
    }
...
// ------------------------------------------------------------------------------------------
    ...
    for (unsigned int j = 0;
      ...
      if (subSet.size() < 2) {
        // ------------------------------------------------------------------------------------------------------
        // If we are within the first two correspondences, just add them to the
        // subset:
        // ------------------------------------------------------------------------------------------------------
        ...
        if (subSet.size() == 2) {
          ...
          const double corrs_dist1 = mrpt::math::distanceBetweenPoints(
              subSet[0].this_x, subSet[0].this_y, subSet[1].this_x,
              subSet[1].this_y);

          const double corrs_dist2 = mrpt::math::distanceBetweenPoints(
              subSet[0].other_x, subSet[0].other_y, subSet[1].other_x,
              subSet[1].other_y);
          ...
          if (is_acceptable) {
            ...
            // Additional filter:
            //  If the correspondences as such the transformation has a high
            //  ambiguity, we discard it!
            is_acceptable =
                (referenceEstimation.cov(2, 2) < square(DEG2RAD(5.0f)));
          }
        ...
        referenceEstimation.composePoint(
            mrpt::math::TPoint2D(corr_j.other_x, corr_j.other_y), pt_this);

        const double maha_dist =
            pt_this.mahalanobisDistanceToPoint(corr_j.this_x, corr_j.this_y);
...
// else -> Test failed
    ...
    // Compute the RMSE of this matching and the corresponding transformation
    // (only if we'll use this value below)
    ...
    if (has_to_eval_RMSE) {
      ...
      for (size_t k = 0; k < subSet.size(); k++) {
        double gx, gy;
        referenceEstimation.mean.composePoint(subSet[k].other_x,
                                              subSet[k].other_y, gx, gy);
    ...
    if (subSet.size() >= params.ransac_minSetSize) {
      // If this subset was previously added to the SOG, just increment its
      // weight
      //  and do not add a new mode:
      int indexFound = -1;

      // JLBC Added DEC-2007: An alternative (optional) method to fuse Gaussian
      // modes:
      if (!params.ransac_fuseByCorrsMatch) {
        ...
        for (size_t i = 0; i < results.transformation.size(); i++) {
          double diffXY = results.transformation.get(i).mean.distanceTo(
              referenceEstimation.mean);
          double diffPhi =
              fabs(math::wrapToPi(results.transformation.get(i).mean.phi() -
    ...
    if (largest_consensus_yet < ninliers) {
      ...
      if (use_dynamic_iter_number) {
        ...
        results.ransac_iters =
            log(1 - params.probability_find_good_model) / log(pNoOutliers);

        results.ransac_iters =
            std::max(results.ransac_iters, params.ransac_min_nSimulations);

        if (params.verbose)
          cout << "[tfest::RANSAC] Iter #" << iter_idx
               << ":est. # iters=" << results.ransac_iters
               << " pNoOutliers=" << pNoOutliers << " #inliers: " << ninliers
               << endl;
      }
    }
