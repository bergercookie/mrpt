 ...
 * enabled from CMake)
  ...
  */
bool TFEST_IMPEXP
se2_l2(const mrpt::utils::TMatchingPairList &in_correspondences,
...
struct TFEST_IMPEXP TSE2RobustParams {
  ...
  unsigned int ransac_nSimulations; //!< (Default = 0) If set to 0, an adaptive
                                    //!algorithm is used to determine the number
                                    //!of iterations, such as a good model is
                                    //!found with a probability p=0.999, or that
                                    //!passed as the parameter
                                    //!probability_find_good_model
  /** (Default = true)  If true, the weight of Gaussian modes will be increased
  * when an exact match in the
  *   subset of correspondences for the modes is found. Otherwise, an
  * approximate method is used as test by just looking at the
  *   resulting X,Y,PHI means. Threshold in this case are: ransac_fuseMaxDiffXY,
  * ransac_fuseMaxDiffPhi */
  ...
  bool ransac_algorithmForLandmarks;  //!< (Default = true) Use Mahalanobis
                                      //!distance (true) or Euclidean dist
                                      //!(false)
  double probability_find_good_model; //!< (Default = 0.999) See parameter
                                      //!ransac_nSimulations. When using
                                      //!`probability_find_good_model`, the
                                      //!minimum number of iterations can be set
                                      //!with `ransac_min_nSimulations`
  unsigned int ransac_min_nSimulations; //!< (Default = 1500) See parameter
                                        //!probability_find_good_model
  double max_rmse_to_end; //!< Stop searching for solutions when the RMSE of one
                          //!solution is below this threshold. Special value "0"
                          //!means "auto", which employs "2*normalizationStd".
  ...
  void *
      user_individual_compat_callback_userdata; //!< User data to be passed to
                                                //!user_individual_compat_callback()

  /** Default values */
  TSE2RobustParams()
      : ransac_minSetSize(3), ransac_maxSetSize(20),
        ransac_mahalanobisDistanceThreshold(3.0), ransac_nSimulations(0),
        ransac_fuseByCorrsMatch(true), ransac_fuseMaxDiffXY(0.01),
        ransac_fuseMaxDiffPhi(mrpt::utils::DEG2RAD(0.1)),
        ransac_algorithmForLandmarks(true), probability_find_good_model(0.999),
        ransac_min_nSimulations(1500), max_rmse_to_end(0), verbose(false) {}
};

/** Output placeholder for se2_l2_robust() */
struct TFEST_IMPEXP TSE2RobustResult {
  mrpt::poses::CPosePDFSOG transformation; //!< The output as a set of
                                           //!transformations (sum of Gaussians)
  mrpt::utils::TMatchingPairList
      largestSubSet;         //!< the largest consensus sub-set
