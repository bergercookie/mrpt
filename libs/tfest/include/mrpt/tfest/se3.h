 ...
 * mrpt::scanmatching::HornMethod()
  ...
  */
bool TFEST_IMPEXP
se3_l2(const mrpt::utils::TMatchingPairList &in_correspondences,
 ...
 * have identical length).
  */
bool TFEST_IMPEXP
se3_l2(const std::vector<mrpt::math::TPoint3D> &in_points_this,
...
struct TFEST_IMPEXP TSE3RobustParams {
  unsigned int ransac_minSetSize; //!< (Default=5)  The minimum amount of points
                                  //!in a set to start a consensus set. \sa
                                  //!ransac_maxSetSizePct
  unsigned int ransac_nmaxSimulations; //!< (Default=50) The maximum number of
                                       //!iterations of the RANSAC algorithm
  double ransac_maxSetSizePct; //!< (Default=0.5) The minimum ratio (0.0 - 1.0)
                               //!of the input set that is considered to be
                               //!inliers. *Important*: The minimum size of a
                               //!consensus set to be accepted will be
                               //!"INPUT_CORRESPONDENCES*ransac_maxSetSizePct".
  double ransac_threshold_lin; //!< (Default=0.05) The maximum distance in X,Y,Z
                               //!for a solution to be considered as matching a
                               //!candidate solution (In meters)
  double ransac_threshold_ang; //!< (Default=1 deg) The maximum angle
                               //!(yaw,pitch,roll) for a solution to be
                               //!considered as matching a candidate solution
                               //!(In radians)
  double ransac_threshold_scale; //!< (Default=0.03) The maximum difference in
                                 //!scale for a solution to be considered as
                                 //!matching a candidate solution
                                 //!(dimensionless)
  ...
  TSE3RobustParams()
      : ransac_minSetSize(5), ransac_nmaxSimulations(50),
        ransac_maxSetSizePct(0.5), ransac_threshold_lin(0.05),
        ransac_threshold_ang(mrpt::utils::DEG2RAD(1)),
        ransac_threshold_scale(0.03), forceScaleToUnity(true), verbose(false) {}
};
...
struct TFEST_IMPEXP TSE3RobustResult {
  ...
  double scale; //!< The estimated scale of the rigid transformation (should be
                //!very close to 1.0)
  mrpt::vector_int inliers_idx; //!< Indexes within the `in_correspondences`
 ...
 * mrpt::scanmatching::leastSquareErrorRigidTransformation6DRANSAC()
  ...
  */
bool TFEST_IMPEXP
se3_l2_robust(const mrpt::utils::TMatchingPairList &in_correspondences,
