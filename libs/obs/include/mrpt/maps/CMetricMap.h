 ...
 *	  - mrpt::obs::mrptEventMetricMapClear: Upon call of the ::clear()
 *method.
...
private:
  ...
  virtual bool internal_canComputeObservationLikelihood(
    ...
    return true; // Unless implemented otherwise, assume we can always compute
                 // the likelihood.
  }
  ...
  virtual void OnPostSuccesfulInsertObs(
      const mrpt::obs::CObservation *) { /* Default: do nothing */
  }
...
public:
   ...
   * \exception std::exception Some internal steps in invoked methods can raise
   * exceptions on invalid parameters, etc...
   */
  void
  loadFromProbabilisticPosesAndObservations(const mrpt::maps::CSimpleMap &Map);
   ...
   * system, or NULL (default) if you want to use the origin.
   *
   * \sa CObservation::insertObservationInto
   */
  bool insertObservation(const mrpt::obs::CObservation *obs,
                         const mrpt::poses::CPose3D *robotPose = NULL);

  /** A wrapper for smart pointers, just calls the non-smart pointer version.
   * See: \ref maps_observations  */
  bool insertObservationPtr(const mrpt::obs::CObservation::Ptr &obs,
                            const mrpt::poses::CPose3D *robotPose = NULL);
   ...
   */
  double computeObservationLikelihood(const mrpt::obs::CObservation *obs,
   ...
   */
  virtual bool
  canComputeObservationLikelihood(const mrpt::obs::CObservation *obs) const;
   ...
   */
  double computeObservationsLikelihood(const mrpt::obs::CSensoryFrame &sf,
                                       const mrpt::poses::CPose2D &takenFrom);
   ...
   */
  bool
  canComputeObservationsLikelihood(const mrpt::obs::CSensoryFrame &sf) const;
  ...
  virtual void
  saveMetricMapRepresentationToFile(const std::string &filNamePrefix) const = 0;

  /** Returns a 3D object representing the map.
    * \sa genericMapParams, TMapGenericParams::enableSaveAs3DObject */
  virtual void
  getAs3DObject(mrpt::opengl::CSetOfObjects::Ptr &outObj) const = 0;
   ...
   * free auxiliary cached variables.
    */
  virtual void
  auxParticleFilterCleanUp() { /* Default implementation: do nothing. */
  }
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CMetricMap,
