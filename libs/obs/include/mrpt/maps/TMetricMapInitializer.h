...
struct OBS_IMPEXP TMetricMapInitializer : public mrpt::utils::CLoadableOptions {
  ...
  using Ptr = std::shared_ptr<TMetricMapInitializer>; //!< Smart pointer to
...
protected:
  ...
  const mrpt::utils::TRuntimeClassId::Ptr metricMapClassType; //!< Derived
                                                              //!classes set
                                                              //!this to
                                                              //!CLASS_ID(<
                                                              //!class >) where
  ...
  virtual void
  dumpToTextStream_map_specific(mrpt::utils::CStream &out) const = 0;
...
public:
  ...
  template <typename MAP_DEFINITION> void push_back(const MAP_DEFINITION &o) {
  ...
  /** Loads the configuration for the set of internal maps from a textual
    ...
    *  The format of the ini file is defined in utils::CConfigFile. The list of
    *maps and their options
    ...
    *maps are defined in textual form by the same name of the corresponding C++
