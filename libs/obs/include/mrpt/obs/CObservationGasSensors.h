...
public:
  ...
  struct OBS_IMPEXP TObservationENose {
    ...
    std::vector<float> readingsVoltage; //!< The set of readings (in volts) from
                                        //!the array of sensors (size of
                                        //!"sensorTypes" is the same that the
                                        //!size of "readingsVoltage")

    /** The kind of sensors in the array (size of "sensorTypes" is the same that
      *the size of "readingsVoltage")
    ...
    bool hasTemperature; //!< Must be true for "temperature" to contain a valid
                         //!measurement
    float temperature;   //!< Sensed temperature in Celcius (valid if
                         //!hasTemperature=true only)
    bool isActive; //!< True if the input to this chamber/enose is poluted air,
                   //!False if clean air
  };
  ...
  /** Declares a class within "CObservationGasSensors" that represents a set of
   * gas concentration readings from the modelation of a MOS gas sensor
   * readings.
  ...
  public:
    ...
    /** @name MOS-model parameters
      *  @{  */
    size_t winNoise_size; //!< The size of the mobile average window used to
                          //!reduce noise on sensor reagings.
    int decimate_value; //!< [useMOSmodel] The decimate frecuency applied after
                        //!noise filtering
     ...
     * readings  */
    bool get_GasDistribution_estimation(float &reading,
                                        mrpt::system::TTimeStamp &timestamp);

  protected:
    /** The content of each m_lastObservations in the estimation when using the
     * option : MOS_MODEl (useMOSmodel =1)
            */
    ...
    TdataMap last_Obs, temporal_Obs; //!< The content of each m_lastObservations
                                     //!in the estimation when using the option
                                     //!: MOS_MODEl (useMOSmodel =1)
    std::vector<TdataMap> m_antiNoise_window; //!< Vector to temporally store
                                              //!and averge readings to reduce
                                              //!noise
    std::ofstream
        *m_debug_dump;       //!< Ofstream to save to file option "save_maplog"
    uint16_t decimate_count; //!< Decimate value for oversampled enose readings
    double
        fixed_incT;  //!< To force e-nose samples to have fixed time increments
    bool first_incT; //!< To force e-nose samples to have fixed time increments
    float min_reading;    //!< Minimum reading value till the moment, used as
                          //!approximation to baeline level
    ...
    /** Estimates the gas concentration based on readings and sensor model
      */
    void inverse_MOSmodeling(const float &reading,
                             const mrpt::system::TTimeStamp &timestamp);

    /** Reduce noise by averaging with a mobile window of specific size
     * (winNoise_size)
      */
    void noise_filtering(const float &reading,
                         const mrpt::system::TTimeStamp &timestamp);

    /** Save the gas distribution estiamtion into a log file for offline
     * representation
    */
    void save_log_map(const mrpt::system::TTimeStamp &timestamp,
                      const float &reading, const float &estimation,
                      const float &tau);

  }; // End of CMOSmodel class def.

}; // End of class def.
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservationGasSensors,
