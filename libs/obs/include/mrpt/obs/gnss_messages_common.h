...
struct OBS_IMPEXP gnss_message {
	...
	/** Dumps the contents of the observation in a human-readable form to a given output stream \sa dumpToConsole() */
	...
	/** Dumps a header for getAllFieldValues() \return false if not implemented for this message type */
	virtual bool getAllFieldDescriptions( std::ostream &o ) const { return false; } 
	/** Dumps a line with the sequence of all field values (without a line feed at the end). \sa getAllFieldDescriptions() \return false if not implemented for this message type */
