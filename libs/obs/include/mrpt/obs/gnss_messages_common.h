...
struct OBS_IMPEXP gnss_message {
  ...
  void writeToStream(mrpt::utils::CStream &out)
      const; //!< Save to binary stream. Launches an exception upon error
  void readFromStream(mrpt::utils::CStream &in); //!< Load from binary stream
                                                 //!into this existing object.
                                                 //!Launches an exception upon
                                                 //!error.

  bool isOfType(const gnss_message_type_t type_id) const;
  template <class MSG_CLASS> bool isOfClass() const {
    return isOfType(MSG_CLASS::msg_type);
  }

  static gnss_message *
  readAndBuildFromStream(mrpt::utils::CStream &in); //!< Load from binary stream
                                                    //!and creates object
                                                    //!detecting its type (class
                                                    //!factory). Launches an
                                                    //!exception upon error
  static gnss_message *
  Factory(const gnss_message_type_t
              msg_id); //!< Creates message \return nullptr on unknown msg type
  static bool
  FactoryKnowsMsgType(const gnss_message_type_t msg_id); //!< Returns true if
                                                         //!Factory() has a
                                                         //!registered
  ...
  virtual void
  dumpToStream(mrpt::utils::CStream &out) const = 0; //!< Dumps the contents of
                                                     //!the observation in a
                                                     //!human-readable form to a
                                                     //!given output stream \sa
                                                     //!dumpToConsole()
  ...
  const std::string &
  getMessageTypeAsString() const; //!< Returns "NMEA_GGA", etc.
  virtual ~gnss_message() {}

protected:
  virtual void internal_writeToStream(mrpt::utils::CStream &out)
      const = 0; //!< Save to binary stream. Launches an exception upon error
  virtual void internal_readFromStream(
      mrpt::utils::CStream
          &in) = 0; //!< Save to binary stream. Launches an exception upon error
};
...
public:
  gnss_message_ptr(); //!< Ctor (default: nullptr pointer)
  ...
  gnss_message_ptr &
  operator=(const gnss_message_ptr &o); // Makes a copy of the pointee
  ...
  void set(gnss_message *p); //!< Replaces the pointee with a new pointer. Its
                             //!memory now belongs to this object, do not free
                             //!manually.
};
...
struct OBS_IMPEXP UTC_time {
  ...
  UTC_time();
  mrpt::system::TTimeStamp getAsTimestamp(const mrpt::system::TTimeStamp &date)
      const; //!< Build an MRPT timestamp with the hour/minute/sec of this
             //!structure and the date from the given timestamp.
  ...
  void writeToStream(mrpt::utils::CStream &out)
      const; //!< Save to binary stream. Launches an exception upon error
  void readFromStream(mrpt::utils::CStream &in); //!< Save to binary stream.
                                                 //!Launches an exception upon
                                                 //!error
};
