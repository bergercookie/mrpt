...
public:
  ...
  struct OBS_IMPEXP TMeasurement {
    mrpt::poses::CPose3D
        pose;   //!< The observed feature SE(3) pose with respect to the sensor
    int32_t id; //!< The feature ID, or INVALID_LANDMARK_ID if unidentified
                //!(default)
    mrpt::math::CMatrixDouble66 inf_matrix; //!< The inverse of the observation
                                            //!covariance matrix (default:all
                                            //!zeros)
  ...
  mrpt::aligned_containers<TMeasurement>::deque_t
      sensedFeatures; //!< The list of observed features

  mrpt::poses::CPose3D
      sensorPose; //!< The pose of the sensor on the robot/vehicle
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservation6DFeatures,
