...
struct OBS_IMPEXP Message_NMEA_GGA : public gnss_message {
  ...
  struct OBS_IMPEXP content_t {
    ...
    double latitude_degrees;  //!< The measured latitude, in degrees (North:+ ,
                              //!South:-)
    double longitude_degrees; //!< The measured longitude, in degrees (East:+ ,
                              //!West:-)
    uint8_t fix_quality; //!< NMEA standard values: 0 = invalid, 1 = GPS fix
                         //!(SPS), 2 = DGPS fix, 3 = PPS fix, 4 = Real Time
                         //!Kinematic, 5 = Float RTK, 6 = estimated (dead
                         //!reckoning) (2.3 feature), 7 = Manual input mode, 8 =
                         //!Simulation mode */
    double altitude_meters;  //!< The measured altitude, in meters (A).
    double geoidal_distance; //!< Undulation: Difference between the measured
                             //!altitude and the geoid, in meters (B).
    double orthometric_altitude; //!< The measured orthometric altitude, in
                                 //!meters (A)+(B).
    double corrected_orthometric_altitude; //!< The corrected (only for TopCon
                                           //!mmGPS) orthometric altitude, in
                                           //!meters mmGPS(A+B).
    uint32_t satellitesUsed; //!< The number of satelites used to compute this
                             //!estimation.
    bool thereis_HDOP; //!< This states whether to take into account the value
                       //!in the HDOP field.
    float HDOP; //!< The HDOP (Horizontal Dilution of Precision) as returned by
                //!the sensor.
  ...
  void
  dumpToStream(mrpt::utils::CStream &out) const override; // See docs in base
  ...
  inline TGEODETICCOORDS getOrthoAsStruct() const {
    return TGEODETICCOORDS(fields.latitude_degrees, fields.longitude_degrees,
   ...
   * structure (requires linking against mrpt-topography)
          *   Call as: getAsStruct<TGeodeticCoords>(); */
  template <class TGEODETICCOORDS> inline TGEODETICCOORDS getAsStruct() const {
    return TGEODETICCOORDS(fields.latitude_degrees, fields.longitude_degrees,
...
struct OBS_IMPEXP Message_NMEA_RMC : public gnss_message {
  ...
  struct OBS_IMPEXP content_t {
    ...
    uint8_t date_day, date_month,
        date_year; //!< Date: day (1-31), month (1-12), two-digits year (00-99)
    double magnetic_dir;   //!< Magnetic variation direction (East:+, West:-)
    char positioning_mode; //!< 'A': Autonomous, 'D': Differential, 'N': Not
                           //!valid, 'E': Estimated, 'M': Manual
    content_t();
  };
  content_t fields; //!< Message content, accesible by individual fields

  mrpt::system::TTimeStamp getDateAsTimestamp() const; //!< Build an MRPT
                                                       //!timestamp with the
                                                       //!year/month/day of this
                                                       //!observation.
...
struct OBS_IMPEXP Message_NMEA_ZDA : public gnss_message {
  ...
  mrpt::system::TTimeStamp getDateTimeAsTimestamp() const; //!< Build an MRPT
                                                           //!UTC timestamp with
                                                           //!the year/month/day
                                                           //!+ hour/minute/sec
                                                           //!of this
