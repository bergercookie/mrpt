...
public:
  ...
  /** Constructor from "IplImage*" images, which could be NULL.
    *  The fields hasImageDisparity and hasImageRight will be set to true/false
   * depending on them being !=nullptr.
  ...
  /** Image from the right camera, only contains a valid image if hasImageRight
   * == true. \sa areImagesRectified() */
  ...
  bool hasImageDisparity; //!< Whether imageDisparity actually contains data
                          //!(Default upon construction: false)
  bool hasImageRight; //!< Whether imageRight actually contains data  (Default
                      //!upon construction: true)

  /** Parameters for the left/right cameras: individual intrinsic and distortion
   * parameters of the cameras.
    * See the <a href="http://www.mrpt.org/Camera_Parameters" >tutorial</a> for
  ...
  mrpt::poses::CPose3DQuat
      cameraPose; //!< The pose of the LEFT camera, relative to the robot.
  ...
  /** Populates a TStereoCamera structure with the parameters in \a leftCamera,
   * \a rightCamera and \a rightCameraPose \sa areImagesRectified() */
  ...
  void swap(CObservationStereoImages &o); //!< Do an efficient swap of all data
                                          //!members of this object with "o".

}; // End of class def.
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservationStereoImages,
