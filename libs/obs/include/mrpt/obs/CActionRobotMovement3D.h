...
public:
  ...
  struct OBS_IMPEXP TMotionModelOptions {
    ...
    struct OBS_IMPEXP TOptions_6DOFModel {
      ...
      /** Options for the 6DOFModel model  which generates a CPosePDFParticles
       * object an then create from that  CPosePDFGaussian object  in poseChange
       */
      uint32_t nParticlesCount;
      float a1, a2, a3, a4, a5, a6, a7, a8, a9, a10;
      /** An additional noise added to the 6DOF model (std. dev. in meters and
       * radians). */
   ...
   * Carlo Localization 4th Workshop on Planning, Perception and Navigation for
   * Intelligent Vehicles, IROS, 2012
   * \sa computeFromOdometry
   */
  void
  computeFromOdometry_model6DOF(const mrpt::poses::CPose3D &odometryIncrement,
  ...
  /** The velocity of the robot in each of 6D: v_x,v_y,v_z,v_yaw,v_pitch,v_roll
   * (linear in meters/sec and angular in rad/sec).
    */
  mrpt::math::CVectorFloat velocities;

}; // End of class def.
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CActionRobotMovement3D, CAction,
                                             OBS_IMPEXP)
