...
public:
  ...
  /** Insert all the observations in this SF into a metric map or any kind (see
   ...
   */
  bool
  insertObservationsInto(mrpt::maps::CMetricMap *theMap,
  ...
  /** You can use "sf+=obs;" to add the observation "obs" to the "sf1". Objects
   * are copied, using the smart pointer, thus the original pointer can be
   * safely deleted next.
    * \sa moveFrom
   */
   ...
   * smart pointer type:
    ...
    * \sa begin, size
    */
  template <typename T> T getObservationByIndexAs(const size_t &idx) const {
   ...
   * (0=first).
    ...
    * \sa begin, size
    */
  CObservation::Ptr getObservationBySensorLabel(const std::string &label,
   ...
   * (0=first), and as a different smart pointer type:
    ...
    */
  template <typename T>
  T getObservationBySensorLabelAs(const std::string &label,
                                  const size_t &idx = 0) const {
    return std::dynamic_pointer_cast<typename T::element_type>(
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CSensoryFrame,
