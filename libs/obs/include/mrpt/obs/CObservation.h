...
#define INVALID_BEACON_ID                                                      \
  (-1) //!< Used for CObservationBeaconRange, CBeacon, etc. \ingroup
       //!mrpt_obs_grp
#define INVALID_LANDMARK_ID                                                    \
  (-1) //!< Used for CObservationBearingRange::TMeasurement::beaconID and
       //!others. \ingroup mrpt_obs_grp
...
protected:
  void swap(CObservation &o); //!< Swap with another observation, ONLY the data
                              //!defined here in the base class CObservation.
                              //!It's protected since it'll be only called from
                              //!child classes that should know what else to
                              //!swap appart from these common data.

public:
  /** @name Data common to any observation
      @{ */
  mrpt::system::TTimeStamp timestamp; //!< The associated UTC time-stamp. Where
                                      //!available, this should contain the
                                      //!accurate satellite-based timestamp of
                                      //!the sensor reading. \sa
                                      //!getOriginalReceivedTimeStamp(),
                                      //!getTimeStamp()
  std::string sensorLabel; //!< An arbitrary label that can be used to identify
                           //!the sensor.
   ...
   * satellite (or otherwise) accurate UTC timing of readings, this contains the
   * computer-based timestamp of reception, which may be slightly different than
   * \a timestamp \sa getTimeStamp()  */
  virtual mrpt::system::TTimeStamp getOriginalReceivedTimeStamp() const {
    return timestamp;
  }
  /** @} */

  CObservation(); //!< Constructor: It sets the initial timestamp to current
                  //!time
  ...
  * \endcode
  * \param theMap The map where this observation is to be inserted: the map will
  *be updated.
  ...
  */
  ...
  inline bool
  insertObservationInto(METRICMAP *theMap,
                        const mrpt::poses::CPose3D *robotPose = nullptr) const {
    return theMap->insertObservation(this, robotPose);
  }

  /** A general method to retrieve the sensor pose on the robot.
  *  Note that most sensors will return a full (6D) CPose3D, but see the derived
  * classes for more details or special cases.
  * \sa setSensorPose
  */
  ...
  *  Note that most sensors will use the full (6D) CPose3D, but see the derived
  * classes for more details or special cases.
  * \sa getSensorPose
  */
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservation,
