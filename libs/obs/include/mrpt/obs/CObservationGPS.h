...
public:
  ...
  mrpt::system::TTimeStamp originalReceivedTimestamp; //!< The local
                                                      //!computer-based
   ...
   * mrpt::obs::gnss::gnss_message. If another message of the same type exists,
   * it is overwritten. */
  template <class MSG_CLASS> void setMsg(const MSG_CLASS &msg) {
    messages[static_cast<gnss::gnss_message_type_t>(MSG_CLASS::msg_type)].set(
        new MSG_CLASS(msg));
  }
  /** Returns true if the list \a CObservationGPS::messages contains one of the
   * requested type. \sa mrpt::obs::gnss::gnss_message_type_t,
   * CObservationGPS::getMsgByType() */
  bool hasMsgType(const gnss::gnss_message_type_t type_id) const;
  /** Like \a hasMsgType() but allows querying for message classes, from any of
   * those derived from mrpt::obs::gnss::gnss_message \sa
   * CObservationGPS::hasMsgType(),  */
  template <class MSG_CLASS> bool hasMsgClass() const {
    return hasMsgType(
        static_cast<gnss::gnss_message_type_t>(MSG_CLASS::msg_type));
  }
  /** Returns a pointer to the message in the list CObservationGPS::messages of
   * the requested type. Users normally would prefer using
   * CObservationGPS::getMsgByClass()
  ...
  getMsgByType(const gnss::gnss_message_type_t type_id);
  /** \overload */
  const mrpt::obs::gnss::gnss_message *
  getMsgByType(const gnss::gnss_message_type_t type_id) const;
   ...
   * Please, check existence before calling this method with
   * CObservationGPS::hasMsgClass()
    * \sa mrpt::obs::gnss::gnss_message_type_t, CObservationGPS::getMsgByType(),
   * CObservationGPS::hasMsgType() */
  template <class MSG_CLASS> MSG_CLASS &getMsgByClass() {
    message_list_t::iterator it = messages.find(
        static_cast<gnss::gnss_message_type_t>(MSG_CLASS::msg_type));
    ASSERTMSG_(it != messages.end(),
               mrpt::format("[CObservationGPS::getMsgByClass] Cannot find any "
  ...
  template <class MSG_CLASS> const MSG_CLASS *getMsgByClassPtr() const {
    message_list_t::const_iterator it = messages.find(
        static_cast<gnss::gnss_message_type_t>(MSG_CLASS::msg_type));
    return it == messages.end() ? dynamic_cast<MSG_CLASS *>(nullptr)
                                : dynamic_cast<MSG_CLASS *>(it->second.get());
  }

  void dumpToStream(mrpt::utils::CStream &out) const; //!< Dumps the contents of
                                                      //!the observation in a
                                                      //!human-readable form to
                                                      //!a given output stream
                                                      //!\sa dumpToConsole(),
                                                      //!getDescriptionAsText()
  void dumpToConsole(std::ostream &o = std::cout) const; //!< Dumps the contents
                                                         //!of the observation
                                                         //!in a human-readable
                                                         //!form to an
                                                         //!std::ostream
                                                         //!(default=console)
  void
  clear(); //!< Empties this observation, clearing the container \a messages
  ...
  /** @name Deprecated, backwards compatible (MRPT <1.4.0) data and types
    * @{ */
  typedef gnss::UTC_time
      TUTCTime; //!< Deprecated, kept for backwards compatibility
  typedef gnss::Message_TOPCON_PZS
      TGPSDatum_PZS; //!< Deprecated, kept for backwards compatibility
  typedef gnss::Message_TOPCON_SATS
      TGPSDatum_SATS; //!< Deprecated, kept for backwards compatibility
  typedef gnss::Message_NMEA_GGA
      TGPSDatum_GGA; //!< Deprecated, kept for backwards compatibility
  typedef gnss::Message_NMEA_RMC
      TGPSDatum_RMC; //!< Deprecated, kept for backwards compatibility
  ...
  struct internal_msg_test_proxy {
    ...
    internal_msg_test_proxy<MSG_TYPE> &
    operator=(const internal_msg_test_proxy<MSG_TYPE> &) {
      return *this;
    }
  ...
  internal_msg_test_proxy<gnss::NMEA_GGA> has_GGA_datum; //!< Evaluates as a
                                                         //!bool; true if the
                                                         //!corresponding field
                                                         //!exists in \a
                                                         //!messages.
  internal_msg_test_proxy<gnss::NMEA_RMC> has_RMC_datum; //!< Evaluates as a
                                                         //!bool; true if the
                                                         //!corresponding field
                                                         //!exists in \a
                                                         //!messages.
  internal_msg_test_proxy<gnss::TOPCON_PZS> has_PZS_datum; //!< Evaluates as a
                                                           //!bool; true if the
                                                           //!corresponding
                                                           //!field exists in \a
                                                           //!messages.
  internal_msg_test_proxy<gnss::TOPCON_SATS> has_SATS_datum; //!< Evaluates as a
                                                             //!bool; true if
                                                             //!the
                                                             //!corresponding
                                                             //!field exists in
  ...
  static bool GPS_time_to_UTC(
      ...
      mrpt::system::TTimeStamp &utc_out /**< [out] UTC timestamp */); //!<
                                                                      ...
                                                                      //!input
                                                                      //!data
  static bool GPS_time_to_UTC(uint16_t gps_week, double gps_sec,
                              const int leap_seconds_count,
                              mrpt::system::TTimeParts &utc_out); //!< \overload
                                                                  /** @} */
}; // End of class def.
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservationGPS, CObservation,
                                             OBS_IMPEXP)
