...
void project3DPointsFromDepthImageInto(
  ...
  src_obs.resizePoints3DVectors(
      WH); // This is to make sure points3D_idxs_{x,y} have the expected sizes.
  ...
  if (src_obs.range_is_depth) {
    ...
    if (projectParams.PROJ3D_USE_LUT) {
      ...
      if (filterParams.rangeMask_min) { // sanity check:
        ASSERT_EQUAL_(filterParams.rangeMask_min->cols(),
                      src_obs.rangeImage.cols());
        ASSERT_EQUAL_(filterParams.rangeMask_min->rows(),
                      src_obs.rangeImage.rows());
      }
      if (filterParams.rangeMask_max) { // sanity check:
        ASSERT_EQUAL_(filterParams.rangeMask_max->cols(),
                      src_obs.rangeImage.cols());
        ASSERT_EQUAL_(filterParams.rangeMask_max->rows(),
                      src_obs.rangeImage.rows());
      }
#if MRPT_HAS_SSE2
      if ((W & 0x07) == 0 && projectParams.USE_SSE2)
        do_project_3d_pointcloud_SSE2(
            ...
            src_obs.points3D_idxs_y, filterParams, projectParams.MAKE_DENSE);
      else
        do_project_3d_pointcloud(
            H, W, kys, kzs, src_obs.rangeImage, pca, src_obs.points3D_idxs_x,
            src_obs.points3D_idxs_y, filterParams,
            projectParams
                .MAKE_DENSE); // if image width is not 8*N, use standard method
#else
  ...
  if (src_obs.hasIntensityImage) {
    ...
    for (size_t i = 0; i < nPts; i++) {
      int img_idx_x,
          img_idx_y; // projected pixel coordinates, in the RGB image plane
      ...
      if (isDirectCorresp) {
        ...
        // Project point, which is now in "pca" in local coordinates wrt the
        // depth camera, into the intensity camera:
        ...
        if (pt_wrt_color[2]) {
          img_idx_x =
              mrpt::utils::round(cx + fx * pt_wrt_color[0] / pt_wrt_color[2]);
          img_idx_y =
              mrpt::utils::round(cy + fy * pt_wrt_color[1] / pt_wrt_color[2]);
      ...
      if (pointWithinImage) {
        if (hasColorIntensityImg) {
          const uint8_t *c =
              src_obs.intensityImage.get_unsafe(img_idx_x, img_idx_y, 0);
          pCol.R = c[2];
          pCol.G = c[1];
          pCol.B = c[0];
        } else {
  ...
  if (projectParams.takeIntoAccountSensorPoseOnRobot ||
    ...
    if (projectParams.robotPoseInTheWorld)
      transf_to_apply.composeFrom(*projectParams.robotPoseInTheWorld,
...
inline void do_project_3d_pointcloud_SSE2(
  ...
  for (int r = 0; r < H; r++) {
    ...
    const float *Dgt_ptr = !filterParams.rangeMask_min
                               ? nullptr
                               : &filterParams.rangeMask_min->coeffRef(r, 0);
    const float *Dlt_ptr = !filterParams.rangeMask_max
                               ? nullptr
    ...
    for (int c = 0; c < W_4; c++) {
      ...
      if (!filterParams.rangeMask_min &&
          !filterParams.rangeMask_max) { // No filter: just skip D=0 points
        valid_range_mask = nz_mask;
      } else {
        if (!filterParams.rangeMask_min ||
            !filterParams.rangeMask_max) { // Only one filter
          if (filterParams.rangeMask_min) {
            const __m128 Dmin = _mm_load_ps(Dgt_ptr);
            valid_range_mask =
                _mm_and_ps(_mm_cmpgt_ps(D, Dmin), _mm_cmpgt_ps(Dmin, D_zeros));
          } else {
            const __m128 Dmax = _mm_load_ps(Dlt_ptr);
            valid_range_mask =
                _mm_and_ps(_mm_cmplt_ps(D, Dmax), _mm_cmpgt_ps(Dmax, D_zeros));
          }
          valid_range_mask =
              _mm_and_ps(valid_range_mask, nz_mask); // Filter out D=0 points
        } else {
          // We have both: D>Dmin and D<Dmax conditions, with XOR to optionally
          // invert the selection:
          const __m128 Dmin = _mm_load_ps(Dgt_ptr);
          const __m128 Dmax = _mm_load_ps(Dlt_ptr);

          const __m128 gt_mask = _mm_cmpgt_ps(D, Dmin);
          const __m128 lt_mask =
              _mm_and_ps(_mm_cmplt_ps(D, Dmax), nz_mask); // skip points at zero
          ...
          valid_range_mask = _mm_or_ps(valid_range_mask,
                                       _mm_and_ps(_mm_cmpeq_ps(Dmin, D_zeros),
