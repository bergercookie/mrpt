...
//		void project3DPointsFromDepthImageInto(CObservationRGBD360
//&src_obs, POINTMAP &dest_pointcloud, const bool
//takeIntoAccountSensorPoseOnRobot, const mrpt::poses::CPose3D *
//robotPoseInTheWorld, const bool PROJ3D_USE_LUT);
//	}
...
public:
  ...
  mrpt::math::CMatrix rangeImages[NUM_SENSORS]; //!< If hasRangeImage=true, a
                                                //!matrix of floats with the
                                                //!range data as captured by the
                                                //!camera (in meters) \sa
                                                //!range_is_depth

  void
  rangeImage_setSize(const int HEIGHT, const int WIDTH,
                     const unsigned sensor_id); //!< Similar to calling
                                                //!"rangeImage.setSize(H,W)" but
                                                //!this method provides memory
                                                //!pooling to speed-up the
                                                //!memory allocation.

  bool hasIntensityImage; //!< true means the field intensityImage contains
                          //!valid data
  mrpt::utils::CImage intensityImages[NUM_SENSORS]; //!< If
                                                    //!hasIntensityImage=true, a
                                                    //!color or gray-level
                                                    //!intensity image of the
                                                    //!same size than
                                                    //!"rangeImage"
  mrpt::utils::TCamera sensorParamss[NUM_SENSORS]; //!< Projection parameters of
                                                   //!the 8 RGBD sensor.
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservationRGBD360, CObservation,
                                             OBS_IMPEXP)
