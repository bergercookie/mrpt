...
protected:
  std::deque<mrpt::utils::poly_ptr_ptr<CAction::Ptr>>
      m_actions; //!< The robot "actionss"
...
public:
   ...
   * element.
    */
  typedef std::deque<mrpt::utils::poly_ptr_ptr<CAction::Ptr>>::iterator
      iterator;

  /** You can use CActionCollection::begin to get a iterator to the first
   * element.
    */
  typedef std::deque<mrpt::utils::poly_ptr_ptr<CAction::Ptr>>::const_iterator
      const_iterator;
  ...
  /** Returns a iterator pointing to the end of the list: this is an example of
    *usage:
    * \code
    *   CActionCollection  acts;
    *   ...
  ...
  /** Access to the i'th action of a given class, or a nullptr smart pointer if
    there is no action of that class in the list.
    *  Example:
    * \code
  ...
  /** Returns the best pose increment estimator in the collection, based on the
   * determinant of its pose change covariance matrix.
   ...
   * specified type.
    * \return The estimation, or nullptr if none is available.
    */
  CActionRobotMovement2D::Ptr
  getMovementEstimationByType(CActionRobotMovement2D::TEstimationMethod method);
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CActionCollection,
