...
struct OBS_IMPEXP T3DPointsProjectionParams {
  bool takeIntoAccountSensorPoseOnRobot; //!< (Default: false) If false, local
                                         //!(sensor-centric) coordinates of
                                         //!points are generated. Otherwise,
                                         //!points are transformed with \a
                                         //!sensorPose. Furthermore, if
                                         //!provided, those coordinates are
                                         //!transformed with \a
                                         //!robotPoseInTheWorld
  const mrpt::poses::CPose3D
      *robotPoseInTheWorld; //!< (Default: nullptr) Read
                            //!takeIntoAccountSensorPoseOnRobot
  bool PROJ3D_USE_LUT;      //!< (Default:true) [Only used when
                       //!`range_is_depth`=true] Whether to use a Look-up-table
                       //!(LUT) to speed up the conversion. It's thread safe in
                       //!all situations <b>except</b> when you call this method
                       //!from different threads <b>and</b> with different
                       //!camera parameter matrices. In all other cases, it is a
                       //!good idea to left it enabled.
  bool USE_SSE2;   //!< (Default:true) If possible, use SSE2 optimized code.
  bool MAKE_DENSE; //!< (Default:true) set to false if you want to preserve the
                   //!organization of the point cloud
  T3DPointsProjectionParams()
      : takeIntoAccountSensorPoseOnRobot(false), robotPoseInTheWorld(nullptr),
        PROJ3D_USE_LUT(true), USE_SSE2(true), MAKE_DENSE(true) {}
};
/** Used in CObservation3DRangeScan::convertTo2DScan() */
struct OBS_IMPEXP T3DPointsTo2DScanParams {
  std::string sensorLabel;     //!< The sensor label that will have the newly
                               //!created observation.
  double angle_sup, angle_inf; //!< (Default=5 degrees) [Only if
                               //!use_origin_sensor_pose=false] The upper &
                               //!lower half-FOV angle (in radians).
  double z_min, z_max;         //!< (Default:-inf, +inf) [Only if
                       //!use_origin_sensor_pose=true] Only obstacle points with
                       //!Z coordinates within the range [z_min,z_max] will be
                       //!taken into account.
  double oversampling_ratio; //!< (Default=1.2=120%) How many more laser scans
                             //!rays to create (read docs for
                             //!CObservation3DRangeScan::convertTo2DScan()).

  /** (Default:false) If `false`, the conversion will be such that the 2D
   * observation pose on the robot coincides with that in the original 3D range
   * scan.
 ...
 *sensor):
 *		- range_is_depth=true  -> Kinect-like ranges: entries of \a
 *rangeImage are distances along the +X axis
 *		- range_is_depth=false -> Ranges in \a rangeImage are actual
 *distances in 3D.
 *
...
protected:
  bool m_points3D_external_stored; //!< If set to true, m_points3D_external_file
                                   //!is valid.
  std::string
      m_points3D_external_file; //!< 3D points are in
                                //!CImage::IMAGES_PATH_BASE+<this_file_name>

  bool m_rangeImage_external_stored; //!< If set to true,
                                     //!m_rangeImage_external_file is valid.
  std::string
      m_rangeImage_external_file; //!< rangeImage is in
                                  //!CImage::IMAGES_PATH_BASE+<this_file_name>
...
public:
  ...
  /** @name Delayed-load manual control methods.
      @{ */
  /** Makes sure all images and other fields which may be externally stored are
   * loaded in memory.
    *  Note that for all CImages, calling load() is not required since the
   * images will be automatically loaded upon first access, so load() shouldn't
   * be needed to be called in normal cases by the user.
   ...
   * rangeImage) and the depth camera camera parameters (\a cameraParams).
    *  There exist two set of formulas for projecting the i'th point, depending
   * on the value of "range_is_depth".
    *   In all formulas below, "rangeImage" is the matrix of ranges and the
   * pixel coordinates are (r,c).
    *
    *  1) [range_is_depth=true] With "range equals depth" or "Kinect-like depth
   * mode": the range values
   ...
   * into the RGB image using \a cameraParamsIntensity.
    *
    *  By default the local (sensor-centric) coordinates of points are directly
   * stored into the local map, but if indicated so in \a
   * takeIntoAccountSensorPoseOnRobot
    *  the points are transformed with \a sensorPose. Furthermore, if provided,
   * those coordinates are transformed with \a robotPoseInTheWorld
    *
  ...
  inline void project3DPointsFromDepthImageInto(
      POINTMAP &dest_pointcloud, const T3DPointsProjectionParams &projectParams,
  ...
  /** Convert this 3D observation into an "equivalent 2D fake laser scan", with
   * a configurable vertical FOV.
    *
   ...
   * make sure we don't leave "gaps" unseen by the virtual "2D laser".
    *
    *  All obstacles within a frustum are considered and the minimum distance is
   * kept in each direction.
   ...
   * having range data,
    *  i.e. hasRangeImage must be true. It's not needed to have RGB data nor the
   * raw 3D point clouds
  ...
  std::vector<float> points3D_x, points3D_y,
      points3D_z; //!< If hasPoints3D=true, the (X,Y,Z) coordinates of the 3D
                  //!point cloud detected by the camera. \sa
                  //!resizePoints3DVectors
  std::vector<uint16_t> points3D_idxs_x,
      points3D_idxs_y; //!< //!< If hasPoints3D=true, the (x,y) pixel
                       //!coordinates for each (X,Y,Z) point in \a points3D_x,
                       //!points3D_y, points3D_z
  ...
  void points3D_convertToExternalStorage(
      ...
      const std::string &use_this_base_dir); //!< Users won't normally want to
                                             //!call this, it's only used from
                                             //!internal MRPT programs. \sa
                                             //!EXTERNALS_AS_TEXT
                                             /** @} */
  ...
  mrpt::math::CMatrix rangeImage; //!< If hasRangeImage=true, a matrix of floats
                                  //!with the range data as captured by the
                                  //!camera (in meters) \sa range_is_depth
  bool range_is_depth; //!< true: Kinect-like ranges: entries of \a rangeImage
                       //!are distances along the +X axis; false: Ranges in \a
                       //!rangeImage are actual distances in 3D.

  void rangeImage_setSize(const int HEIGHT,
                          const int WIDTH); //!< Similar to calling
                                            //!"rangeImage.setSize(H,W)" but
                                            //!this method provides memory
                                            //!pooling to speed-up the memory
                                            //!allocation.
                                            /** @} */
  ...
  void
  rangeImage_getExternalStorageFileAbsolutePath(std::string &out_path) const;
  ...
  bool hasIntensityImage; //!< true means the field intensityImage contains
                          //!valid data
  mrpt::utils::CImage intensityImage; //!< If hasIntensityImage=true, a color or
                                      //!gray-level intensity image of the same
                                      //!size than "rangeImage"
  TIntensityChannelID intensityImageChannel; //!< The source of the
                                             //!intensityImage; typically the
                                             //!visible channel \sa
                                             //!TIntensityChannelID
                                             /** @} */
  ...
  /** \name Confidence "channel"
    * @{ */
  bool hasConfidenceImage; //!< true means the field confidenceImage contains
                           //!valid data
  mrpt::utils::CImage confidenceImage; //!< If hasConfidenceImage=true, an image
                                       //!with the "confidence" value [range
                                       //!0-255] as estimated by the capture
                                       //!drivers.
                                       /** @} */

  /** \name Pixel-wise classification labels (for semantic labeling, etc.)
    * @{ */
  /** Returns true if the field CObservation3DRangeScan::pixelLabels contains a
   * non-NULL smart pointer.
    * To enhance a 3D point cloud with labeling info, just assign an appropiate
   * object to \a pixelLabels
    */
  ...
  struct OBS_IMPEXP TPixelLabelInfoBase {
    using Ptr = std::shared_ptr<
        TPixelLabelInfoBase>; //!< Used in CObservation3DRangeScan::pixelLabels
    ...
    /** Resizes the matrix pixelLabels to the given size, setting all bitfields
     * to zero (that is, all pixels are assigned NONE category). */
    ...
    /** For the pixel(row,col), removes its classification into the category \a
     * label_idx, which may be in the range 0 to 7
      * Note that 0 is a valid label index, it does not mean "no label" \sa
     * setLabel, unsetAll */
    virtual void unsetLabel(const int row, const int col,
                            uint8_t label_idx) = 0;
    ...
    virtual bool checkLabel(const int row, const int col,
                            uint8_t label_idx) const = 0;

    void writeToStream(mrpt::utils::CStream &out) const;
    static TPixelLabelInfoBase *
    readAndBuildFromStream(mrpt::utils::CStream &in);
    ...
    const uint8_t BITFIELD_BYTES; //!< Minimum number of bytes required to hold
                                  //!MAX_NUM_DIFFERENT_LABELS bits.
  ...
  struct TPixelLabelInfo : public TPixelLabelInfoBase {
    ...
    /** Automatically-determined integer type of the proper size such that all
     * labels fit as one bit (max: 64)  */
    typedef typename mrpt::utils::uint_select_by_bytecount<BYTES_REQUIRED>::type
        bitmask_t;

    /** Each pixel may be assigned between 0 and MAX_NUM_LABELS-1 'labels' by
      * setting to 1 the corresponding i'th bit [0,MAX_NUM_LABELS-1] in the byte
     * in pixelLabels(r,c).
      * That is, each pixel is assigned an 8*BITFIELD_BYTES bit-wide bitfield of
     * possible categories.
      * \sa hasPixelLabels
      */
    ...
    bool checkLabel(const int row, const int col,
                    uint8_t label_idx) const override {
  ...
  /** All information about pixel labeling is stored in this (smart pointer to)
   * structure; refer to TPixelLabelInfo for details on the contents
    * User is responsible of creating a new object of the desired data type. It
   * will be automatically (de)serialized no matter its specific type. */
  ...
  mrpt::utils::TCamera
      cameraParams; //!< Projection parameters of the depth camera.
  mrpt::utils::TCamera cameraParamsIntensity; //!< Projection parameters of the
                                              //!intensity (graylevel or RGB)
                                              //!camera.

  /** Relative pose of the intensity camera wrt the depth camera (which is the
   * coordinates origin for this observation).
    *  In a SwissRanger camera, this will be (0,0,0,-90deg,0,-90deg) since both
   * cameras coincide.
  ...
  float maxRange; //!< The maximum range allowed by the device, in meters (e.g.
                  //!8.0m, 5.0m,...)
  mrpt::poses::CPose3D sensorPose; //!< The 6D pose of the sensor on the robot.
  float stdError; //!< The "sigma" error of the device in meters, used while
                  //!inserting the scan in an occupancy grid.
  ...
  void swap(CObservation3DRangeScan &o); //!< Very efficient method to swap the
                                         //!contents of two observations.
  /** Extract a ROI of the 3D observation as a new one. \note PixelLabels are
   * *not* copied to the output subimage. */
  void getZoneAsObs(CObservation3DRangeScan &obs, const unsigned int &r1,
                    const unsigned int &r2, const unsigned int &c1,
   ...
   * point cloud. It's 1cm for SwissRanger SR4000.
    ...
    */
  static double
  recoverCameraCalibrationParameters(const CObservation3DRangeScan &in_obs,
  ...
  static TCached3DProjTables m_3dproj_lut; //!< 3D point cloud projection
                                           //!look-up-table \sa
                                           //!project3DPointsFromDepthImage

}; // End of class def.
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservation3DRangeScan,
                                             CObservation, OBS_IMPEXP)
