...
#pragma pack(push, 1)
  ...
  *  bank.  The device returns three times as many upper bank blocks. */
  ...
  struct OBS_IMPEXP TVelodyneRawPacket {
    ...
    uint8_t
        laser_return_mode; //!< 0x37: strongest, 0x38: last, 0x39: dual return
  ...
  struct OBS_IMPEXP TVelodynePositionPacket {
    ...
    uint32_t unused2;
    char NMEA_GPRMC[72 + 234]; //!< the full $GPRMC message, as received by
                               //!Velodyne, terminated with "\r\n\0"
  };
#pragma pack(pop)

  double minRange, maxRange; //!< The maximum range allowed by the device, in
                             //!meters (e.g. 100m). Stored here by the driver
                             //!while capturing based on the sensor model.
  mrpt::poses::CPose3D sensorPose; //!< The 6D pose of the sensor on the
                                   //!robot/vehicle frame of reference
  std::vector<TVelodyneRawPacket> scan_packets; //!< The main content of this
                                                //!object: raw data packet from
                                                //!the LIDAR. \sa point_cloud
  mrpt::obs::VelodyneCalibration
      calibration; //!< The calibration data for the LIDAR device. See
                   //!mrpt::hwdrivers::CVelodyneScanner and
                   //!mrpt::obs::VelodyneCalibration for details.
  mrpt::system::TTimeStamp
      originalReceivedTimestamp; //!< The local computer-based timestamp based
                                 //!on the reception of the message in the
                                 //!computer. \sa has_satellite_timestamp,
                                 //!CObservation::timestamp in the base class,
                                 //!which should contain the accurate
                                 //!satellite-based UTC timestamp.
  bool has_satellite_timestamp;  //!< If true, CObservation::timestamp has been
                                 //!generated from accurate satellite clock.
                                 //!Otherwise, no GPS data is available and
                                 //!timestamps are based on the local computer
                                 //!clock.

  mrpt::system::TTimeStamp
  getOriginalReceivedTimeStamp() const override; // See base class docs

  /** See \a point_cloud and \a scan_packets */
  struct OBS_IMPEXP TPointCloud {
    std::vector<float> x, y, z;
    std::vector<uint8_t> intensity; //!< Color [0,255]
    std::vector<mrpt::system::TTimeStamp>
        timestamp; //!< Timestamp for each point (if
                   //!`generatePerPointTimestamp`=true in
                   //!`TGeneratePointCloudParameters`), or empty vector if not
                   //!populated (default).
    std::vector<float> azimuth; //!< Original azimuth of each point (if
                                //!`generatePerPointAzimuth`=true, empty
                                //!otherwise )
  ...
  struct OBS_IMPEXP TGeneratePointCloudParameters {
    double minAzimuth_deg; //!< Minimum azimuth, in degrees (Default=0). Points
                           //!will be generated only the the area of interest
                           //![minAzimuth, maxAzimuth]
    double maxAzimuth_deg; //!< Minimum azimuth, in degrees (Default=360).
                           //!Points will be generated only the the area of
                           //!interest [minAzimuth, maxAzimuth]
    float minDistance,
        maxDistance; //!< Minimum (default=1.0f) and maximum (default: Infinity)
                     //!distances/ranges for a point to be considered. Points
                     //!must pass this (application specific) condition and also
                     //!the minRange/maxRange values in CObservationVelodyneScan
                     //!(sensor-specific).
    ...
    float nROI_x_min, nROI_x_max, nROI_y_min, nROI_y_max, nROI_z_min,
        nROI_z_max;
    float isolatedPointsFilterDistance; //!< (Default:2.0 meters) Minimum
                                        //!distance between a point and its two
                                        //!neighbors to be considered an invalid
                                        //!point.

    bool filterByROI;  //!< Enable ROI filter (Default:false): add points inside
                       //!a given 3D box
    bool filterBynROI; //!< Enable nROI filter (Default:false): do NOT add
                       //!points inside a given 3D box
    bool filterOutIsolatedPoints; //!< (Default:false) Simple filter to remove
                                  //!spurious returns (e.g. Sun reflected on
                                  //!large water extensions)
    bool dualKeepStrongest, dualKeepLast; //!< (Default:true) In VLP16 dual
                                          //!mode, keep both or just one of the
                                          //!returns.
    bool generatePerPointTimestamp; //!< (Default:false) If `true`, populate the
                                    //!vector timestamp
    bool generatePerPointAzimuth;   //!< (Default:false) If `true`, populate the
                                    //!vector azimuth
  ...
  struct OBS_IMPEXP TGeneratePointCloudSE3Results {
    ...
    size_t num_correctly_inserted_points; //!< Number of points for which a
                                          //!valid interpolated SE(3) pose could
                                          //!be determined
   ...
   * CObservationVelodyneScan::setSensorPose()
    * \param[out] out_points The generated points, in the same coordinates frame
   * than \a vehicle_path. Points are APPENDED to the list, so prevous contents
   * are kept.
  ...
  void
  getDescriptionAsText(std::ostream &o) const override; // See base class docs

}; // End of class def.
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservationVelodyneScan,
