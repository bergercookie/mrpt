...
#pragma pack(pop)
	...
	struct OBS_IMPEXP TGeneratePointCloudParameters
	{
		 ...
		 * only
		 * the the area of interest [minAzimuth, maxAzimuth] */
		 ...
		 * distances/ranges
		 * for a point to be considered. Points must pass this (application
		 * specific) condition and also the minRange/maxRange values in
		 * CObservationVelodyneScan (sensor-specific). */
	 ...
	 * frame
	 * than \a vehicle_path. Points are APPENDED to the list, so prevous
	 * contents
	 * are kept.
