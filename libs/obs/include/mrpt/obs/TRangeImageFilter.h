...
struct OBS_IMPEXP TRangeImageFilterParams {
  ...
  TRangeImageFilterParams()
      : rangeCheckBetween(true), rangeMask_min(nullptr),
...
struct OBS_IMPEXP TRangeImageFilter {
  ...
  inline bool do_range_filter(size_t r, size_t c,
                              const float D) const; //!< Returns true if the
                                                    //!point (r,c) with depth D
                                                    //!passes all filters.
...
// ======== Implementation ========
bool TRangeImageFilter::do_range_filter(size_t r, size_t c,
                                        const float D) const {
  // Filters:
  if (D <= .0f)
    return false;
