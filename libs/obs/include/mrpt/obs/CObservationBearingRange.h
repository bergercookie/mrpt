...
public:
  ...
  float minSensorDistance, maxSensorDistance; //! Information about the sensor:
                                              //! Ranges, in meters (0: there is
                                              //! no limits)
  float fieldOfView_yaw; //!< Information about the sensor: The "field-of-view"
                         //!of the sensor, in radians (for yaw ).
  float fieldOfView_pitch; //!< Information about the sensor: The
                           //!"field-of-view" of the sensor, in radians (for
                           //!pitch ).
  ...
  struct OBS_IMPEXP TMeasurement {
    ...
    /** The sensed landmark direction, in radians, measured as the yaw (azimuth)
     * and pitch (negative elevation).
      *  Set pitch to zero for 2D sensors.
      * See mrpt::poses::CPose3D for a definition of the 3D angles.
      */
    float yaw, pitch;

    /** The ID of the sensed beacon, or INVALID_LANDMARK_ID (-1) if the sensor
     * does not identify the landmark. */
    int32_t landmarkID;
  ...
  /** Taken into account only if validCovariances=false: the standard deviation
   * of the sensor noise model for range,yaw and pitch (in meters and radians).
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservationBearingRange,
