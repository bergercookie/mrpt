...
typedef std::pair<mrpt::system::TTimeStamp, CObservation::Ptr>
    TTimeObservationPair; //!< For usage with CRawlog classes.
typedef std::multimap<mrpt::system::TTimeStamp, CObservation::Ptr>
    TListTimeAndObservations; //!< For usage with CRawlog classes.

/** This class stores a rawlog (robotic datasets) in one of two possible
 *formats:
 *		- Format #1: A sequence of actions and observations. There is a
 *sequence of objects, where each one can be of one type:
 *			- An action:	Implemented as a CActionCollection object,
 *the actuation of the robot (i.e. odometry increment).
 *			- Observations: Implemented as a CSensoryFrame, refering to
 *a set of robot observations from the same pose.
 *		- Format #2: A sequence of actions and observations. There is a
 *sequence of objects, where each one can be of one type:
 *
...
public:
  void getCommentText(std::string &t)
      const; //!< Returns the block of comment text for the rawlog
  std::string
  getCommentText() const; //!< Returns the block of comment text for the rawlog
  void setCommentText(const std::string &t); //!< Changes the block of comment
                                             //!text for the rawlog
  void getCommentTextAsConfigFile(mrpt::utils::CConfigFileMemory &memCfg)
      const; //!< Saves the block of comment text for the rawlog into the passed
             //!config file object.
  ...
  /** Add a set of actions to the sequence, using a smart pointer to the object
   * to add.
    * \sa addActions, addObservationsMemoryReference,
   ...
   * case the method stops reading on EOF of an unrecogniced class name.
    *  - Only if `non_obs_objects_are_legal` is true, any `CSerializable` object
   * is allowed in the log file. Otherwise, the read stops on classes different
   * from the ones listed in the item above.
  ...
  /** Saves the contents to a rawlog-file, compatible with RawlogViewer (As the
   * sequence of internal objects).
    *  The file is saved with gz-commpressed if MRPT has gz-streams.
    * \returns It returns false if any error is found while writing/creating the
   * target file.
    */
  ...
  /** Returns the i'th element in the sequence, as being actions, where index=0
   * is the first object.
   ...
   * index=0 is the first object.
    *  If it is not an CSensoryFrame, it throws an exception. Do neighter modify
   * nor delete the returned pointer.
   ...
   * index=0 is the first object.
    *  If it is not an CObservation, it throws an exception. Do neighter modify
   * nor delete the returned pointer.
   ...
   * ascending order (which should be the normal situation).
    ...
    */
  void
  findObservationsByClassInRange(mrpt::system::TTimeStamp time_start,
   ...
   * data from the origin (after calling this, the original object will have no
   ...
   * rawlog entry read, for logging or monitoring purposes.
    ...
    */
  static bool readActionObservationPair(mrpt::utils::CStream &inStream,
                                        CActionCollection::Ptr &action,
                                        CSensoryFrame::Ptr &observations,
                                        size_t &rawlogEntry);

  /** Reads a consecutive pair action/sensory_frame OR an observation, depending
    *of the rawlog format, from the rawlog opened at some input stream.
    *   Previous contents of action and observations are discarded (using
    *stlplus::smart_ptr::reset), and
    *    at exit they contain the new objects read from the rawlog file.
    *
  ...
  /** Gets the next consecutive pair action / observation from the rawlog loaded
   * into this object.
    *   Previous contents of action and observations are discarded (using
   * stlplus::smart_ptr::reset), and
    *    at exit they contain the new objects read from the rawlog file.
   ...
   * rawlog entry read, for logging or monitoring purposes.
    ...
    */
  bool getActionObservationPair(CActionCollection::Ptr &action,
                                CSensoryFrame::Ptr &observations,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CRawlog,
