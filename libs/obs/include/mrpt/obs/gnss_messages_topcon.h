...
struct OBS_IMPEXP Message_TOPCON_PZS : public gnss_message {
  ...
  double longitude_degrees; //!< The measured longitude, in degrees (East:+ ,
                            //!West:-)
  double height_meters; //!< ellipsoidal height from N-beam [m] perhaps weighted
                        //!with regular gps
  double
      RTK_height_meters; //!< ellipsoidal height [m] without N-beam correction
  ...
  double cartesian_x, cartesian_y,
      cartesian_z; //!< Only if hasCartesianPosVel is true
  double cartesian_vx, cartesian_vy,
      cartesian_vz; //!< Only if hasCartesianPosVel is true
  ...
  uint8_t stats_rtk_fix_progress; //!< [0,100] %, only in modes other than RTK
                                  //!FIXED.
...
struct OBS_IMPEXP Message_TOPCON_SATS : public gnss_message {
  ...
  mrpt::vector_byte USIs; //!< The list of USI (Universal Sat ID) for the
                          //!detected sats (See GRIL Manual, pag 4-31).
  mrpt::vector_signed_byte
      ELs; //!< Elevation (in degrees, 0-90) for each satellite in USIs.
  mrpt::vector_signed_word
      AZs; //!< Azimuth (in degrees, 0-360) for each satellite in USIs.
