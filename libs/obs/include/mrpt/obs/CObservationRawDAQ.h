...
public:
  ...
  inline CObservationRawDAQ()
      : AIN_channel_count(0), AIN_interleaved(true), sample_rate(0) {}
  /** Destructor */
  ...
  std::
      vector<uint16_t>
          AIN_16bits; /** Readings from 16-bit analog input (ADCs) channels
                         (vector length=channel count) in ADC units. */
  std::
      vector<uint32_t>
          AIN_32bits; /** Readings from 32-bit analog input (ADCs) channels
                         (vector length=channel count) in ADC units. */
  std::vector<float> AIN_float; /** Readings from analog input (ADCs) channels
                                   (vector length=channel count) in Volts. */
  std::vector<double>
      AIN_double; /** Readings from analog input (ADCs) channels (vector
                     length=channel count) in Volts. */

  uint16_t AIN_channel_count; //!< How many different ADC channels are present
                              //!in the AIN_* vectors.
  bool AIN_interleaved; //!< Whether the channels are interleaved (A0 A1 A2 A0
                        //!A1 A2...) or not (A0 A0 A0 A1 A1 A1 A2 A2 A2...) in
                        //!the AIN_* vectors.
  ...
  std::vector<uint16_t>
      AOUT_16bits; /** Present output values for 16-bit analog output (DACs)
                      channels (vector length=channel count) in DAC units.*/
  ...
  std::
      vector<uint8_t>
          DIN; /** Readings from digital inputs; each byte stores 8 digital
                  inputs, or 8-bit port. */
  std::vector<uint8_t>
      DOUT; /** Present digital output values; each byte stores 8 digital
               inputs, or 8-bit port. */

  std::vector<uint32_t>
      CNTRIN_32bits; /** Readings from ticks counters, such as quadrature
                        encoders. (vector length=channel count) in ticks. */
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservationRawDAQ, CObservation,
                                             OBS_IMPEXP)
