...
public:
  ...
  mrpt::poses::CPose2D
      odometry; //!< The absolute odometry measurement (IT IS NOT INCREMENTAL)

  bool hasEncodersInfo; //!< "true" means that "encoderLeftTicks" and
                        //!"encoderRightTicks" contain valid values.
  /** For differential-driven robots: The ticks count for each wheel in ABSOLUTE
   * VALUE (IT IS NOT INCREMENTAL) (positive means FORWARD, for both wheels);
   * \sa hasEncodersInfo  */
  int32_t encoderLeftTicks, encoderRightTicks;

  bool hasVelocities; //!< "true" means that `velocityLocal` contains valid
                      //!values.
  mrpt::math::TTwist2D velocityLocal; //!< Velocity, in the robot (local) frame
                                      //!of reference (+X=forward).
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservationOdometry, CObservation,
                                             OBS_IMPEXP)
