...
//  OBS_IMPEXP, mrpt_xxx_EXPORTS

/*   The macros below for DLL import/export are required for Windows only.
    Mostly all the definitions in this file are copied or at least obsd
     on the file wx/dlimpexp.h, written by Vadim Zeitlin and published
         under the wxWindows licence.
*/
#if defined(MRPT_OS_WINDOWS)
#define MRPT_OBS_EXPORT __declspec(dllexport)
#define MRPT_OBS_IMPORT __declspec(dllimport)
#else
#define MRPT_OBS_EXPORT
#define MRPT_OBS_IMPORT
#endif
...
#if defined(mrpt_obs_EXPORTS) /* Building the DLL */
#define OBS_IMPEXP MRPT_OBS_EXPORT
#else /* Using the DLL */
#define OBS_IMPEXP MRPT_OBS_IMPORT
#endif
