...
public:
  ...
  float sensorConeApperture; //!< Cone aperture of each ultrasonic beam, in
                             //!radians.

  struct OBS_IMPEXP TMeasurement {
    TMeasurement() : sensorID(0), sensorPose(), sensedDistance(0) {}

    /** Some kind of sensor ID which identifies it on the bus (if applicable, 0
     * otherwise)
      */
    uint16_t sensorID;
    ...
    /** The measured range, in meters (or a value of 0 if there was no detected
     * echo).
      */
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservationRange, CObservation,
                                             OBS_IMPEXP)
} // End of namespace
} // End of namespace
#endif
