...
public:
   ...
   * cache.getSinCosForScan( scan );
    ...
    */
  const TSinCosValues &
  getSinCosForScan(const CObservation2DRangeScan &scan) const;
  /** \overload */
  const TSinCosValues &
  getSinCosForScan(const T2DScanProperties &scan_prop) const;

private:
  mutable std::map<T2DScanProperties, TSinCosValues>
      m_cache; //!< The cache of known scans and their sin/cos tables.
};
