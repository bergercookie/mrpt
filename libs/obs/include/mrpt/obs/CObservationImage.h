...
public:
  /** Constructor.
   * \param iplImage An OpenCV "IplImage*" object with the image to be loaded in
   * the member "image", or nullptr (default) for an empty image.
   *
   */
  ...
  mrpt::utils::CImage image; //!< The image captured by the camera, that is, the
                             //!main piece of information of this observation.

  /** Computes the rectified (un-distorted) image, using the embeded distortion
   * parameters.
    */
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservationImage, CObservation,
                                             OBS_IMPEXP)
