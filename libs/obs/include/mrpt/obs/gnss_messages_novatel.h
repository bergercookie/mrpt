...
enum nv_solution_status_t : uint32_t {
  ...
  TEST_DIST,    //!< test distance exceeded (max of 3 rejections if distance >
                //!10km)
  ...
  PENDING = 18, //!< when a fix position command is entered, the receiver
                //!computes its own position and determines if the fixed
                //!position is valid
  INVALID_FIX //!< the fixed position entered using the fix position command is
              //!not valid
};
...
enum nv_ins_status_type_t : uint32_t {
  INS_INACTIVE = 0, // IMU logs are present, but the alignment routine has not
  ...
  INS_HIGH_VARIANCE = 2, // The INS solution is in navigation mode but the
                         ...
                         // threshold.
  INS_SOLUTION_GOOD =
      3, // The INS filter is in navigation mode and the INS solution is good.
  INS_SOLUTION_FREE = 6, // The INS filter is in navigation mode and the GNSS
                         // solution is suspected to be in error.
  INS_ALIGNMENT_COMPLETE = 7, // The INS filter is in navigation mode, but not
                              // enough vehicle dynamics have been experienced
                              // for the system to be within specifications.
...
uint32_t week;
double seconds_in_week;
double pos_cov[9]; //!< Position covariance matrix in local level frame (metres
                   //!squared) xx,xy,xz,yx,yy,yz,zx,zy,zz
double att_cov[9]; //!< Attitude covariance matrix of the SPAN frame to the
                   //!local level frame.  (deg sq) xx,xy,xz,yx,yy,yz,zx,zy,zz
double vel_cov[9]; //!< Velocity covariance matrix in local level frame.
