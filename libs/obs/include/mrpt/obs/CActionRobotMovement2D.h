...
public:
  ...
  mrpt::utils::poly_ptr_ptr<mrpt::poses::CPosePDF::Ptr>
      poseChange; //!< The 2D pose change probabilistic estimation.
  /** This is the raw odometry reading, and only is used when "estimationMethod"
   * is "TEstimationMethod::emOdometry" */
  mrpt::poses::CPose2D rawOdometryIncrementReading;
  TEstimationMethod estimationMethod; //!< This fields indicates the way in
                                      //!which this estimation was obtained.

  bool hasEncodersInfo; //!< If "true" means that "encoderLeftTicks" and
                        //!"encoderRightTicks" contain valid values.
  ...
  bool hasVelocities; //!< If "true" means that "velocityLin" and "velocityAng"
                      //!contain valid values.
  mrpt::math::TTwist2D velocityLocal; //!< If "hasVelocities"=true, the robot
                                      //!velocity in local (robot frame, +X
                                      //!forward) coordinates.
  ...
  struct OBS_IMPEXP TMotionModelOptions {
    ...
    struct OBS_IMPEXP TOptions_ThrunModel {
       ...
       * representation (anyway you can draw as many samples as you want through
       * CActionRobotMovement2D::drawSingleSample)  */
      ...
      /** An additional noise added to the thrun model (std. dev. in meters and
       * radians). */
   ...
   * according to the given motion model (speed and encoder ticks information is
   * not modified).
   ...
   * the private sampling functions (see "see also" next).
    ...
    */
  void computeFromOdometry(const mrpt::poses::CPose2D &odometryIncrement,
                           const TMotionModelOptions &options);

  /** If "hasEncodersInfo"=true, this method updates the pose estimation
   * according to the ticks from both encoders and the passed parameters, which
   * is passed internally to the method "computeFromOdometry" with the last used
   * PDF options (or the defualt ones if not explicitly called by the user).
    *
  ...
  /** Using this method instead of "poseChange->drawSingleSample()" may be more
...
protected:
  ...
  /** Computes the PDF of the pose increment from an odometry reading, using the
   * motion model from Thrun's book.
   * This model is discussed in "Probabilistic Robotics", Thrun, Burgard, and
   * Fox, 2006, pp.136.
   * \sa computeFromOdometry
   */
  void
  computeFromOdometry_modelThrun(const mrpt::poses::CPose2D &odometryIncrement,
                                 const TMotionModelOptions &o);
  ...
  /** Internal use
    */
  void
  fastDrawSingleSample_modelGaussian(mrpt::poses::CPose2D &outSample) const;
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CActionRobotMovement2D, CAction,
                                             OBS_IMPEXP)
