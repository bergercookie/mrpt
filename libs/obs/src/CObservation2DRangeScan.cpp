...
CObservation2DRangeScan::CObservation2DRangeScan()
    : m_scan(), m_intensity(), m_validRange(), m_has_intensity(false),
      scan(m_scan),             // proxy ctor
      intensity(m_intensity),   // proxy ctor
      validRange(m_validRange), // proxy ctor
      aperture(M_PIf), rightToLeft(true), maxRange(80.0f), sensorPose(),
      stdError(0.01f), beamAperture(0), deltaPitch(0), m_cachedMap() {}
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CObservation2DRangeScan::writeToStream(mrpt::utils::CStream &out,
                                            int *version) const {
  if (version)
    *version = 7;
  else {
...
void CObservation2DRangeScan::truncateByDistanceAndAngle(float min_distance,
  ...
  for (itScan = m_scan.begin(), itValid = m_validRange.begin(), k = 0;
    ...
    if (min_height != 0 || max_height != 0) {
      ...
      if (*itScan < min_distance || ang > max_angle || x > h - min_height ||
          x < h - max_height)
        *itValid = false;
    } // end if
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CObservation2DRangeScan::readFromStream(mrpt::utils::CStream &in,
                                             int version) {
  switch (version) {
  case 0:
  case 1:
...
void CObservation2DRangeScan::filterByExclusionAngles(
  ...
  for (vector<pair<double, double>>::const_iterator itA = angles.begin();
    ...
    if (ap_idx_ini > (int)sizeRangeScan)
      ap_idx_ini = sizeRangeScan - 1;
    if (ap_idx_end > (int)sizeRangeScan)
...
void CObservation2DRangeScan::internal_buildAuxPointsMap(
  ...
  if (!ptr_internal_build_points_map_from_scan2D)
    throw std::runtime_error("[CObservation2DRangeScan::buildAuxPointsMap] "
                             "ERROR: This function needs linking against "
                             "mrpt-maps.\n");
