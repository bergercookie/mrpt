...
void CSensoryFrame::readFromStream(mrpt::utils::CStream &in, int version) {
  ...
  case 2: {
    ...
    for_each(m_observations.begin(), m_observations.end(),
...
void CSensoryFrame::eraseByIndex(const size_t &idx) {
  MRPT_START
  if (idx >= size())
    THROW_EXCEPTION_FMT("Index %u out of range.", static_cast<unsigned>(idx));
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
CObservation::Ptr
CSensoryFrame::getObservationByIndex(const size_t &idx) const {
  MRPT_START
  if (idx >= size())
    THROW_EXCEPTION_FMT("Index %u out of range.", static_cast<unsigned>(idx));
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
CObservation::Ptr
CSensoryFrame::getObservationBySensorLabel(const std::string &label,
...
void CSensoryFrame::moveFrom(CSensoryFrame &sf) {
  copy(sf.m_observations.begin(), sf.m_observations.end(),
...
void CSensoryFrame::internal_buildAuxPointsMap(const void *options) const {
  if (!ptr_internal_build_points_map_from_scan2D)
    throw std::runtime_error("[CSensoryFrame::buildAuxPointsMap] ERROR: This "
                             "function needs linking against mrpt-maps.\n");
