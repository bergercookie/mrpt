  ...
  if (strStartsI(line, "ROBOTLASER")) {
    ...
    for (size_t i = 0; i < nRanges; i++) {
      ...
      obsLaser->setScanRangeValidity(
          i,
          (obsLaser->scan[i] >= obsLaser->maxRange || obsLaser->scan[i] <= 0));
    }
    ...
    if (!(S >> obsLaser->sensorLabel >> nRanges))
      ...
      // Params:
      {
        ...
        if (line[0] == 'F') { // front:
          ...
          resolutionDeg =
              atof(global_log_params
                       .getWithDefaultVal("laser_front_laser_resolution", "0.5")
                       .c_str());
        } else if (line[0] == 'R') { // rear:
          maxRange = atof(global_log_params
                              .getWithDefaultVal("robot_rear_laser_max", "81.0")
                              .c_str());
          resolutionDeg =
              atof(global_log_params
                       .getWithDefaultVal("laser_rear_laser_resolution", "0.5")
                       .c_str());
        }
