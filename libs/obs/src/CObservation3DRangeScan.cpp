...
struct CObservation3DRangeScan_Points_MemPoolParams {
  ...
  inline bool
  isSuitable(const CObservation3DRangeScan_Points_MemPoolParams &req) const {
    return WH >= req.WH;
  }
};
struct CObservation3DRangeScan_Points_MemPoolData {
  ...
  std::vector<uint16_t> idxs_x,
      idxs_y; //!< for each point, the corresponding (x,y) pixel coordinates
};
...
struct CObservation3DRangeScan_Ranges_MemPoolParams {
  ...
  inline bool
  isSuitable(const CObservation3DRangeScan_Ranges_MemPoolParams &req) const {
...
CObservation3DRangeScan::CObservation3DRangeScan()
    : m_points3D_external_stored(false), m_rangeImage_external_stored(false),
      hasPoints3D(false), hasRangeImage(false), range_is_depth(true),
      hasIntensityImage(false), intensityImageChannel(CH_VISIBLE),
      hasConfidenceImage(false), pixelLabels(), // Start without label info
      cameraParams(), cameraParamsIntensity(),
      relativePoseIntensityWRTDepth(0, 0, 0, DEG2RAD(-90), DEG2RAD(0),
                                    DEG2RAD(-90)),
      maxRange(5.0f), sensorPose(), stdError(0.01f) {}
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CObservation3DRangeScan::writeToStream(mrpt::utils::CStream &out,
                                            int *version) const {
  if (version)
    *version = 8;
  else {
    ...
    out << hasPoints3D;
    if (hasPoints3D) {
      ASSERT_(points3D_x.size() == points3D_y.size() &&
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CObservation3DRangeScan::readFromStream(mrpt::utils::CStream &in,
                                             int version) {
  switch (version) {
  case 0:
  case 1:
  ...
  case 8: {
    ...
    if (version >= 1) {
      ...
      if (version >= 2) {
        ...
        if (version >= 4) {
          in >> cameraParamsIntensity >> relativePoseIntensityWRTDepth;
        } else {
    ...
    pixelLabels.reset(); // Remove existing data first (_unique() is to leave
                         // alive any user copies of the shared pointer).
    if (version >= 7) {
...
void CObservation3DRangeScan::points3D_convertToExternalStorage(
  ...
  // Use "use_this_base_dir" in "*_getExternalStorageFileAbsolutePath()" instead
  // of CImage::IMAGES_PATH_BASE
...
void cost_func(const CVectorDouble &par, const TLevMarData &d,
               CVectorDouble &err) {
  ...
  for (size_t r = 0; r < nR; r += CALIB_DECIMAT) {
    for (size_t c = 0; c < nC; c += CALIB_DECIMAT) {
      ...
      TPoint3D p(obs.points3D_x[idx] + d.z_offset, obs.points3D_y[idx],
                 obs.points3D_z[idx]);
...
double CObservation3DRangeScan::recoverCameraCalibrationParameters(
  ...
  ASSERT_(obs.points3D_x.size() == obs.points3D_y.size() &&
  ...
  TMyLevMar lm;
  lm.execute(optimal_x, initial_x, &mrpt::obs::detail::cost_func, increments_x,
...
void CObservation3DRangeScan::getZoneAsObs(CObservation3DRangeScan &obs,
  ...
  if (hasConfidenceImage)
    confidenceImage.extract_patch(obs.confidenceImage, c1, r1, c2 - c1,
                                  r2 - r1);
...
void CObservation3DRangeScan::resizePoints3DVectors(const size_t WH) {
  ...
  if (pool) {
    ...
    if (mem_block) { // Take the memory via swaps:
...
void CObservation3DRangeScan::convertTo2DScan(
  ...
  if (!this->hasRangeImage) { // Nothing to do!
  ...
  // for data
  //        from OpenNI, and will be in the future for libfreenect in MRPT, but
  //        it's
  //        not implemented yet (as of Mar 2012), so this is an approximation in
  //        that case.
  ...
  out_scan2d.resizeScanAndAssign(nLaserRays, 2.0 * this->maxRange,
  ...
  if (!sp.use_origin_sensor_pose) {
    ...
    // Go thru columns, and keep the minimum distance (along the +X axis, not 3D
    // distance!)
    ...
    for (size_t i = 0; i < nLaserRays; i++, ang += A_ang) {
      ...
      const size_t c = std::min(
          static_cast<size_t>(std::max(0.0, cx + fx * tan_ang)), nCols - 1);
      ...
      if (any_valid) {
        ...
        out_scan2d.setScanRange(i, closest_range *
                                       std::sqrt(1.0 + tan_ang * tan_ang));
      }
    } // end for columns
  } else {
    // Algorithm 2: project to 3D and reproject (for a different sensorPose at
    // the origin)
    // ------------------------------------------------------------------------
...
void CObservation3DRangeScan::getDescriptionAsText(std::ostream &o) const {
  ...
  if (hasIntensityImage) {
    ...
    // Channel?
    o << "Source channel: "
      << mrpt::utils::TEnumType<CObservation3DRangeScan::TIntensityChannelID>::
...
void CObservation3DRangeScan::TPixelLabelInfoBase::writeToStream(
  ...
  // 1st: Save number MAX_NUM_DIFFERENT_LABELS so we can reconstruct the object
  // in the class factory later on.
  out << BITFIELD_BYTES;
...
CObservation3DRangeScan::TPixelLabelInfoBase::readAndBuildFromStream(
  ...
  case 1: {
    ...
    // Hand-made class factory. May be a good solution if there will be not too
...
T3DPointsTo2DScanParams::T3DPointsTo2DScanParams()
    : angle_sup(mrpt::utils::DEG2RAD(5)), angle_inf(mrpt::utils::DEG2RAD(5)),
      ...
      z_max(std::numeric_limits<double>::max()), oversampling_ratio(1.2),
