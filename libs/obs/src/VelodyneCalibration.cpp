...
VelodyneCalibration::PerLaserCalib::PerLaserCalib()
    : azimuthCorrection(.0), verticalCorrection(.0), distanceCorrection(.0),
      verticalOffsetCorrection(.0), horizontalOffsetCorrection(.0),
      sinVertCorrection(.0), cosVertCorrection(1.0),
      sinVertOffsetCorrection(.0), cosVertOffsetCorrection(1.0) {}
...
bool VelodyneCalibration::internal_loadFromXMLNode(void *node) {
  ...
  for (int i = 0;; ++i) {
    ...
    if (node_points_item.isEmpty())
      break;

    XMLNode node_px = node_points_item.getChildNode("px");
    if (node_px.isEmpty())
      throw std::runtime_error("Cannot find XML node: 'points_::item::px'");
    ...
    }
    {
      ...
      if (node.isEmpty())
        throw std::runtime_error("Cannot find XML node: "
...
bool VelodyneCalibration::loadFromXMLText(
  ...
  try {
    ...
    if (results.error != eXMLErrorNone) {
      cerr << "[VelodyneCalibration::loadFromXMLText] Error parsing XML "
              "content: "
           << XMLNode::getError(results.error) << " at line " << results.nLine
           << ":" << results.nColumn << endl;
      return false;
    }
...
bool VelodyneCalibration::loadFromXMLFile(
  ...
  try {
    XMLResults results;
    XMLNode root = XMLNode::parseFile(velodyne_calibration_xml_filename.c_str(),
                                      nullptr, &results);
...
// name is unknown.
const VelodyneCalibration &
VelodyneCalibration::LoadDefaultCalibration(const std::string &lidar_model) {
  // Cached calib data?
