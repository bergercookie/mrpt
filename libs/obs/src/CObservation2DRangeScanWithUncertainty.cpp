...
CObservation2DRangeScanWithUncertainty::TEvalParams::TEvalParams()
    : prob_outliers(0.5), prob_lost_ray(0.3), max_prediction_std_dev(1.0),
...
double CObservation2DRangeScanWithUncertainty::evaluateScanLikelihood(
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    const double likGauss = mrpt::math::normalPDF(
        otherScanRange, rangesMean[i], std::sqrt(prediction_total_var));
    double pi;
    if (otherScan.scan[i] > rangesMean[i]) {
      ...
      pi =
          std::max(likGauss, std::min(1.0, params.prob_outliers *
                                               otherScanRange / rangesMean[i]));
    }
