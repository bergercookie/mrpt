...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CObservationRawDAQ::writeToStream(mrpt::utils::CStream &out,
                                       int *version) const {
  if (version)
    *version = 0;
  else {
...
void CObservationRawDAQ::readFromStream(mrpt::utils::CStream &in, int version) {
  switch (version) {
  case 0: {
    in >> sensorLabel >> timestamp >> sample_rate >> AIN_8bits >> AIN_16bits >>
        AIN_32bits >> AIN_float >> AIN_double >> AIN_channel_count >>
        AIN_interleaved >> AOUT_8bits >> AOUT_16bits >> AOUT_float >>
        AOUT_double >> DIN >> DOUT >> CNTRIN_32bits >> CNTRIN_double;
  } break;
...
#define RAWDAQ_SHOW_FIRSTS(_VEC)                                               \
  ...
  if (!_VEC.empty()) {                                                         \
    for (size_t i = 1; i <= std::min(_VEC.size(), static_cast<size_t>(10));    \
         i++)                                                                  \
      cout << _VEC[i - 1] << " ";                                              \
    cout << " ... ";                                                           \
  }                                                                            \
