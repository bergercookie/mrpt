...
nv_oem6_header_t::nv_oem6_header_t()
    : hdr_len(0), msg_id(0), msg_type(0), port_addr(0), msg_len(0),
      seq_number(0), idle_percent(0), time_status(0), week(0), ms_in_week(0),
      receiver_status(0), reserved(0), receiver_sw_version(0) {
...
#define DEF_TYPESTR(_NAME) val2str[nv_oem6_solution_status::_NAME] = #_NAME;
    ...
    DEF_TYPESTR(INSUFFICIENT_OBS) DEF_TYPESTR(NO_CONVERGENCE)
        DEF_TYPESTR(SINGULARITY) DEF_TYPESTR(COV_TRACE) DEF_TYPESTR(TEST_DIST)
            DEF_TYPESTR(COLD_START) DEF_TYPESTR(V_H_LIMIT) DEF_TYPESTR(VARIANCE)
                DEF_TYPESTR(RESIDUALS) DEF_TYPESTR(DELTA_POS)
                    DEF_TYPESTR(NEGATIVE_VAR) DEF_TYPESTR(INTEGRITY_WARNING)
                        DEF_TYPESTR(INS_INACTIVE) DEF_TYPESTR(INS_ALIGNING)
                            DEF_TYPESTR(INS_BAD) DEF_TYPESTR(IMU_UNPLUGGED)
                                DEF_TYPESTR(PENDING) DEF_TYPESTR(INVALID_FIX)
#undef DEF_TYPESTR
  }
...
#define DEF_TYPESTR(_NAME) val2str[nv_oem6_position_type::_NAME] = #_NAME;
    DEF_TYPESTR(NONE)
    DEF_TYPESTR(FIXEDPOS) DEF_TYPESTR(FIXEDHEIGHT) DEF_TYPESTR(Reserved)
        DEF_TYPESTR(FLOATCONV) DEF_TYPESTR(WIDELANE) DEF_TYPESTR(NARROWLANE)
            DEF_TYPESTR(DOPPLER_VELOCITY) DEF_TYPESTR(SINGLE)
                DEF_TYPESTR(PSRDIFF) DEF_TYPESTR(WAAS) DEF_TYPESTR(PROPAGATED)
                    DEF_TYPESTR(OMNISTAR) DEF_TYPESTR(L1_FLOAT) DEF_TYPESTR(
                        IONOFREE_FLOAT) DEF_TYPESTR(NARROW_FLOAT)
                        DEF_TYPESTR(L1_INT) DEF_TYPESTR(WIDE_INT)
                            DEF_TYPESTR(NARROW_INT) DEF_TYPESTR(RTK_DIRECT_INS)
                                DEF_TYPESTR(INS) DEF_TYPESTR(INS_PSRSP)
...
#define DEF_TYPESTR(_NAME) val2str[nv_oem6_ins_status_type::_NAME] = #_NAME;
    ...
    DEF_TYPESTR(INS_ALIGNING) DEF_TYPESTR(INS_HIGH_VARIANCE)
        DEF_TYPESTR(INS_SOLUTION_GOOD) DEF_TYPESTR(INS_SOLUTION_FREE)
...
void generic_dump_BESTPOS(const Message_NV_OEM6_BESTPOS::content_t &fields,
  ...
  out.printf(" Longitude: %.09f deg (std dev: %.06f m)  Latitude: %.09f deg "
             "(std dev: %.06f m)\n",
             fields.lon, fields.lon_sigma, fields.lat, fields.lat_sigma);
  out.printf(" Height (sea level): %.06f m (std dev: %.06f m) Undulation: "
             "%.04f m   (Sum: %.04f m)\n",
...
bool Message_NV_OEM6_BESTPOS::getAllFieldValues(std::ostream &o) const {
  o << mrpt::format(
      "%u.%08u %u %u %.09f %.09f %.06f %.04f %.06f %.06f %.06f %.03f %.03f %u "
      "%u",
...
void Message_NV_OEM6_INSCOVS::dumpToStream(mrpt::utils::CStream &out) const {
  ...
  out.printf(" GPS week: %u  ms in week: %u\n", (unsigned)fields.header.week,
             (unsigned)(fields.header.ms_in_week));
  out.printf(" Position cov: %9.03f %9.03f %9.03f  %9.03f %9.03f %9.03f  "
             "%9.03f %9.03f %9.03f\n",
  ...
  out.printf(" Attitude cov: %9.03f %9.03f %9.03f  %9.03f %9.03f %9.03f  "
             "%9.03f %9.03f %9.03f\n",
  ...
  out.printf(" Velocity cov: %9.03f %9.03f %9.03f  %9.03f %9.03f %9.03f  "
             "%9.03f %9.03f %9.03f\n",
...
void generic_getFieldValues_MARKTIME(
  ...
  o << mrpt::format("%u.%08u %u %lu %f %f", (unsigned)fields.header.week,
                    (unsigned)(fields.header.ms_in_week),
                    (unsigned)fields.clock_status, (long unsigned)fields.week,
                    fields.week_seconds, fields.utc_offset);
}
