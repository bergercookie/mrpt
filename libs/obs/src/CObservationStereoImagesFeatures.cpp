...
IMPLEMENTS_SERIALIZABLE(CObservationStereoImagesFeatures, CObservation,
                        mrpt::obs)
...
void CObservationStereoImagesFeatures::saveFeaturesToTextFile(
  ...
  for (it = theFeatures.begin(); it != theFeatures.end(); ++it)
    file << format("%u %.2f %.2f %.2f %.2f\n", it->ID, it->pixels.first.x,
                   it->pixels.first.y, it->pixels.second.x,
                   it->pixels.second.y);
...
void CObservationStereoImagesFeatures::writeToStream(mrpt::utils::CStream &out,
  ...
  if (version)
    *version = 0;
    ...
    out << (uint32_t)theFeatures
               .size(); // Write the number of items within the feature list
...
void CObservationStereoImagesFeatures::getDescriptionAsText(
  ...
  o << cameraPoseOnRobot.getHomogeneousMatrixVal() << cameraPoseOnRobot << endl;
  ...
  o << endl
    << format(" Image size: %ux%u pixels\n", (unsigned int)cameraLeft.ncols,
              (unsigned int)cameraLeft.nrows);
  o << endl
    << format(" Number of features in images: %u\n",
