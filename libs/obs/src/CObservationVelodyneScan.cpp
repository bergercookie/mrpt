...
CObservationVelodyneScan::TGeneratePointCloudParameters::
    ...
    : minAzimuth_deg(0.0), maxAzimuth_deg(360.0), minDistance(1.0f),
      ...
      ROI_z_max(+std::numeric_limits<float>::max()), nROI_x_min(.0f),
      nROI_x_max(.0f), nROI_y_min(.0f), nROI_y_max(.0f), nROI_z_min(.0f),
      nROI_z_max(.0f), isolatedPointsFilterDistance(2.0f), filterByROI(false),
      filterBynROI(false), filterOutIsolatedPoints(false),
      dualKeepStrongest(true), dualKeepLast(true),
      generatePerPointTimestamp(false), generatePerPointAzimuth(false) {}
...
CObservationVelodyneScan::CObservationVelodyneScan()
    : minRange(1.0), maxRange(130.0), sensorPose(),
  ...
  if (version)
    *version = 1;
    ...
    }
    {
      ...
      out << N;
      if (N)
        out.WriteBuffer(&calibration.laser_corrections[0],
...
void CObservationVelodyneScan::readFromStream(mrpt::utils::CStream &in,
  ...
  case 1: {
    ...
    in >> minRange >> maxRange >> sensorPose;
    {
      ...
      if (N)
        in.ReadBuffer(&scan_packets[0], sizeof(scan_packets[0]) * N);
    }
    {
      uint32_t N;
      in >> N;
      ...
      if (N)
        in.ReadBuffer(&calibration.laser_corrections[0],
                      sizeof(calibration.laser_corrections[0]) * N);
    }
...
double VLP16AdjustTimeStamp(int firingblock, int dsr,
                            int firingwithinblock) //!< [us]
{
...
struct PointCloudStorageWrapper {
  ...
  virtual void add_point(double pt_x, double pt_y, double pt_z,
...
static void velodyne_scan_to_pointcloud(
  ...
  // The LUT contains sin/cos values for angles in this order: [180deg ... 0 deg
  // ... -180 deg]
  ...
  for (size_t iPkt = 0; iPkt < scan.scan_packets.size(); iPkt++) {
    ...
    // Take the median rotational speed as a good value for interpolating the
    // missing azimuths:
    int median_azimuth_diff;
    {
      // In dual return, the azimuth rate is actually twice this estimation:
      const int nBlocksPerAzimuth =
          (raw->laser_return_mode == CObservationVelodyneScan::RETMODE_DUAL)
              ? 2
              : 1;
      std::vector<int> diffs(CObservationVelodyneScan::BLOCKS_PER_PACKET -
                             nBlocksPerAzimuth);
      for (int i = 0;
           i < CObservationVelodyneScan::BLOCKS_PER_PACKET - nBlocksPerAzimuth;
           ++i) {
      ...
      std::nth_element(diffs.begin(),
                       diffs.begin() +
                           CObservationVelodyneScan::BLOCKS_PER_PACKET / 2,
         ...
         block++) // Firings per packet
    {
      ...
      if ((num_lasers != 64 &&
           CObservationVelodyneScan::UPPER_BANK != raw->blocks[block].header) ||
          (raw->blocks[block].header != CObservationVelodyneScan::UPPER_BANK &&
           raw->blocks[block].header != CObservationVelodyneScan::LOWER_BANK)) {
        cerr << "[CObservationVelodyneScan] skipping invalid packet: block "
             << block << " header value is " << raw->blocks[block].header;
        continue;
      }
      ...
      for (int dsr = 0, k = 0; dsr < SCANS_PER_FIRING; dsr++, k++) {
        ...
        // In dual return, if the distance is equal in both ranges, ignore one
        // of them:
        ...
        if (params.filterOutIsolatedPoints) {
          ...
          if (!pass_filter)
            continue; // Filter out this point
        }

        // Azimuth correction: correct for the laser rotation as a function of
        // timing during the firings
        ...
        case 16: {
          if (raw->laser_return_mode ==
            ...
            timestampadjustment =
                VLP16AdjustTimeStamp(block / 2, laserId, firingWithinBlock);
            nextblockdsr0 = VLP16AdjustTimeStamp(block / 2 + 1, 0, 0);
            blockdsr0 = VLP16AdjustTimeStamp(block / 2, 0, 0);
          } else {
            timestampadjustment =
                VLP16AdjustTimeStamp(block, laserId, firingWithinBlock);
        ...
        case 64:
          break;
        default: { THROW_EXCEPTION("Error: unhandled LIDAR model!") }
        };
        ...
        // Insert point:
        out_pc.add_point(pt.x, pt.y, pt.z,
...
void CObservationVelodyneScan::generatePointCloud(
  ...
  struct PointCloudStorageWrapper_Inner : public PointCloudStorageWrapper {
    ...
    PointCloudStorageWrapper_Inner(CObservationVelodyneScan &me,
                                   const TGeneratePointCloudParameters &p)
        : me_(me), params_(p) {
      // Reset point cloud:
      me_.point_cloud.clear();
    }
    void add_point(double pt_x, double pt_y, double pt_z, uint8_t pt_intensity,
                   const mrpt::system::TTimeStamp &tim,
                   const float azimuth) override {
      ...
      if (params_.generatePerPointAzimuth) {
        const int azimuth_corrected =
            ((int)round(azimuth)) %
            CObservationVelodyneScan::ROTATION_MAX_UNITS;
        me_.point_cloud.azimuth.push_back(azimuth_corrected *
                                          ROTATION_RESOLUTION);
      }
    }
  };
...
void CObservationVelodyneScan::generatePointCloudAlongSE3Trajectory(
    ...
    const TGeneratePointCloudParameters &params) {
  // Pre-alloc mem:
  out_points.reserve(out_points.size() +
                     scan_packets.size() * BLOCKS_PER_PACKET * SCANS_PER_BLOCK +
                     16);
  ...
  struct PointCloudStorageWrapper_SE3_Interp : public PointCloudStorageWrapper {
    ...
    PointCloudStorageWrapper_SE3_Interp(
        ...
        std::vector<mrpt::math::TPointXYZIu8> &out_points,
        TGeneratePointCloudSE3Results &results_stats)
        : me_(me), vehicle_path_(vehicle_path), out_points_(out_points),
          results_stats_(results_stats), last_query_tim_(INVALID_TIMESTAMP),
          last_query_valid_(false) {}
    void add_point(double pt_x, double pt_y, double pt_z, uint8_t pt_intensity,
                   const mrpt::system::TTimeStamp &tim,
                   const float azimuth) override {
      // Use a cache since it's expected that the same timestamp is queried
  ...
  PointCloudStorageWrapper_SE3_Interp my_pc_wrap(*this, vehicle_path,
                                                 out_points, results_stats);
  velodyne_scan_to_pointcloud(*this, params, my_pc_wrap);
}
