...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CObservationGasSensors::writeToStream(mrpt::utils::CStream &out,
                                           int *version) const {
  if (version)
    *version = 5;
  else {
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CObservationGasSensors::readFromStream(mrpt::utils::CStream &in,
                                            int version) {
  switch (version) {
  case 2:
  case 3:
...
CObservationGasSensors::CMOSmodel::CMOSmodel()
    : winNoise_size(30), decimate_value(6), a_rise(0), b_rise(0), a_decay(0),
      b_decay(0), save_maplog(false), last_Obs(), temporal_Obs(),
      m_debug_dump(nullptr), decimate_count(1), fixed_incT(0), first_incT(true),
      min_reading(10), first_iteration(true) {}
...
bool CObservationGasSensors::CMOSmodel::get_GasDistribution_estimation(
  ...
  try {
    ...
    inverse_MOSmodeling(m_antiNoise_window[winNoise_size / 2].reading_filtered,
    ...
    if (save_maplog)
      save_log_map(last_Obs.timestamp, last_Obs.reading, last_Obs.estimation,
                   last_Obs.tau);
...
void CObservationGasSensors::CMOSmodel::inverse_MOSmodeling(
  ...
  try {
    ...
    if (!first_iteration) {
      // Assure the samples are provided at constant rate (important for the
      // correct gas distribution estimation)
      double incT = mrpt::system::timeDifference(last_Obs.timestamp, timestamp);

      if ((incT > 0) & (!first_incT)) { // not the same sample due to
      ...
      if (incT > 0)
        // Initially there may come repetetive values till m_antiNoise_window is
        // full populated.
        last_Obs.estimation =
            ((reading - last_Obs.reading) * last_Obs.tau / incT) + reading;
      else
...
void CObservationGasSensors::getDescriptionAsText(std::ostream &o) const {
  ...
  for (size_t j = 0; j < m_readings.size(); j++) {
    ...
    ASSERT_(m_readings[j].readingsVoltage.size() ==
