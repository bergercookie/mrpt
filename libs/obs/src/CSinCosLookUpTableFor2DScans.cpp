...
CSinCosLookUpTableFor2DScans::getSinCosForScan(
  ...
  if (it != m_cache.end()) { // Found in the cache:
    return it->second;
  } else {
    ...
    // Make sure the allocated memory at least have room for 4 extra double's at
    // the end,
    //  for the case we use these buffers for SSE2 optimized code. If the final
    //  values are uninitialized it doesn't matter.
