...
CObservationBearingRange::CObservationBearingRange()
    : minSensorDistance(0), maxSensorDistance(0), fieldOfView_yaw(DEG2RAD(180)),
      fieldOfView_pitch(DEG2RAD(90)), sensorLocationOnRobot(), sensedData(),
      validCovariances(false), sensor_std_range(0), sensor_std_yaw(0),
      sensor_std_pitch(0) {}

/*---------------------------------------------------------------
  Implements the writing to a CStream capability of CSerializable objects
 ---------------------------------------------------------------*/
void CObservationBearingRange::writeToStream(mrpt::utils::CStream &out,
                                             int *version) const {
  if (version)
    *version = 3;
  else {
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CObservationBearingRange::readFromStream(mrpt::utils::CStream &in,
                                              int version) {
  switch (version) {
  case 0:
  case 1:
  ...
  case 3: {
    ...
    for (i = 0; i < n; i++) {
      in >> sensedData[i].range >> sensedData[i].yaw >> sensedData[i].pitch >>
...
void CObservationBearingRange::debugPrintOut() {
  ...
  for (i = 0; i < n; i++)
    printf("[CObservationBearingRange::debugPrintOut] \tID[%i]: y:%fdeg "
           "p:%fdeg range: %f\n",
...
void CObservationBearingRange::getDescriptionAsText(std::ostream &o) const {
  ...
  o << sensorLocationOnRobot.getHomogeneousMatrixVal() << sensorLocationOnRobot
    << endl
    << endl;
  ...
  for (size_t q = 0; q < sensedData.size(); q++) {
    ...
    o << format("   %10.03f  %10.03f %10.03f        ", sensedData[q].range,
