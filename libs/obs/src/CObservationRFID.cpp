...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CObservationRFID::writeToStream(mrpt::utils::CStream &out,
                                     int *version) const {
  ...
  if (version)
    *version = 4;
    ...
    // (Fields are dumped in separate for loops for backward compatibility with
    // old serialization versions)
...
void CObservationRFID::readFromStream(mrpt::utils::CStream &in, int version) {
  ...
  case 4: {
    ...
    // (Fields are read in separate for loops for backward compatibility with
    // old serialization versions)
