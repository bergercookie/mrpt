...
CObservationGPS::CObservationGPS()
    : sensorPose(), originalReceivedTimestamp(INVALID_TIMESTAMP),
      has_satellite_timestamp(false), messages(), has_GGA_datum(messages),
      has_RMC_datum(messages), has_PZS_datum(messages),
...
void CObservationGPS::readFromStream(mrpt::utils::CStream &in, int version) {
  ...
  case 9: {
    ...
    if (has_GGA_datum_) {
      ...
      if (version >= 9) {
        in >> GGA_datum.geoidal_distance >> GGA_datum.orthometric_altitude >>
    ...
    if (version >= 5) {
      ...
      if (has_PZS_datum_) {
        ...
        in >> PZS_datum.latitude_degrees >> PZS_datum.longitude_degrees >>
            PZS_datum.height_meters >> PZS_datum.RTK_height_meters >>
            PZS_datum.PSigma >> PZS_datum.angle_transmitter >> PZS_datum.nId >>
            PZS_datum.Fix >> PZS_datum.TXBattery >> PZS_datum.RXBattery >>
            PZS_datum.error;
        // extra data?
        if (version >= 6) {
          in >> PZS_datum.hasCartesianPosVel >> PZS_datum.cartesian_x >>
              PZS_datum.cartesian_y >> PZS_datum.cartesian_z >>
              PZS_datum.cartesian_vx >> PZS_datum.cartesian_vy >>
              PZS_datum.cartesian_vz >> PZS_datum.hasPosCov >>
              PZS_datum.pos_covariance >> PZS_datum.hasVelCov >>
              PZS_datum.vel_covariance >> PZS_datum.hasStats >>
...
void CObservationGPS::dumpToConsole(std::ostream &o) const {
  ...
  if (memStr.getTotalBytesCount()) {
    o.write((const char *)memStr.getRawBufferData(),
...
void CObservationGPS::getDescriptionAsText(std::ostream &o) const {
  ...
  o << "  (as time_t): " << std::fixed << std::setprecision(5)
    << mrpt::system::timestampTotime_t(originalReceivedTimestamp) << std::endl;
...
bool TIMECONV_GetUTCTimeFromJulianDate(
    const double julian_date, //!< Number of days since noon Universal Time Jan
                              //!1, 4713 BCE (Julian calendar) [days]
...
bool CObservationGPS::GPS_time_to_UTC(uint16_t gps_week, double gps_sec,
...
bool CObservationGPS::GPS_time_to_UTC(uint16_t gps_week, double gps_sec,
  ...
  if (gps_sec < 0.0 || gps_sec > 604800.0)
    return false;
  if (!TIMECONV_GetJulianDateFromGPSTime(gps_week, gps_sec, leap_seconds_count,
                                         &julian_date))
    return false;
