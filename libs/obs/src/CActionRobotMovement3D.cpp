...
CActionRobotMovement3D::CActionRobotMovement3D()
    : poseChange(), rawOdometryIncrementReading(), estimationMethod(emOdometry),
      motionModelConfiguration(), hasVelocities(6, false), velocities(6) {
  velocities.assign(.0);
}
...
void CActionRobotMovement3D::computeFromOdometry_model6DOF(
  ...
  // F. Sacchi, D. G. Sorrenti An effective 6DoF motion model for 3D-6DoF Monte
  ...
  /*
     ...
     dPitch=pitch(t)-pitch(t-1); dYaw=yaw(t)-yaw(t-1)
          ...
          Model the error:
                          ...
                          trans_draw = trans + sample( a4 * trans + a5 * yaw2 +
     a6 * (roll + pitch2 ) )
  ...
  */
  ...
  for (size_t i = 0; i < o.mm6DOFModel.nParticlesCount; i++) {
    ...
    double new_yaw = Ayaw1_draw + Ayaw2_draw +
