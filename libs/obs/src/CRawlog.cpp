...
void CRawlog::addActions(CActionCollection &actions) {
  m_seqOfActObs.push_back(
      std::dynamic_pointer_cast<CSerializable>(actions.duplicateGetSmartPtr()));
}
...
CActionCollection::Ptr CRawlog::getAsAction(size_t index) const {
  ...
  if (obj->GetRuntimeClass() == CLASS_ID(CActionCollection))
    ...
    THROW_EXCEPTION_FMT("Element at index %i is not a CActionCollection",
                        (int)index);
  MRPT_END
}
...
CObservation::Ptr CRawlog::getAsObservation(size_t index) const {
  ...
  if (obj->GetRuntimeClass()->derivedFrom(CLASS_ID(CObservation)))
    ...
    THROW_EXCEPTION_FMT("Element at index %i is not a CObservation",
                        (int)index);
  MRPT_END
}
...
CSensoryFrame::Ptr CRawlog::getAsObservations(size_t index) const {
  ...
  if (obj->GetRuntimeClass()->derivedFrom(CLASS_ID(CSensoryFrame)))
    ...
    THROW_EXCEPTION_FMT("Element at index %i is not a CSensoryFrame",
                        (int)index);
  MRPT_END
}
...
bool CRawlog::loadFromRawLogFile(const std::string &fileName,
  ...
  while (keepReading) {
    ...
    } catch (mrpt::utils::CExceptionEOF &) { // EOF, just finish the loop
      keepReading = false;
...
void CRawlog::remove(size_t first_index, size_t last_index) {
  ...
  if (first_index >= m_seqOfActObs.size() || last_index >= m_seqOfActObs.size())
    THROW_EXCEPTION("Index out of bounds")
  m_seqOfActObs.erase(m_seqOfActObs.begin() + first_index,
...
bool CRawlog::readActionObservationPair(CStream &inStream,
...
std::string CRawlog::detectImagesDirectory(const std::string &str) {
  ...
  else if (mrpt::system::fileExists(temptative_img_path =
