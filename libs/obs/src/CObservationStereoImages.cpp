...
CObservationStereoImages::CObservationStereoImages(void *iplImageLeft,
                                                   ...
                                                   bool ownMemory)
    : imageLeft(UNINITIALIZED_IMAGE), imageRight(UNINITIALIZED_IMAGE),
  ...
  case 5: {
    ...
    if (version >= 2) {
      ...
      rightCameraPose = CPose3DQuat(
          0.10f, 0, 0,
          mrpt::math::CQuaternionDouble(1, 0, 0, 0)); // For version 1 to 5
    ...
    if (version >= 3 && version < 5) // For versions 3 & 4
    {
      ...
      leftCamera.focalLengthMeters = rightCamera.focalLengthMeters =
          0.002; // For version 0, 1 & 2 (from version 5, this parameter is
                 // included in the TCamera objects)
