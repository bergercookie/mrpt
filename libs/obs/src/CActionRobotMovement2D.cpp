...
CActionRobotMovement2D::CActionRobotMovement2D()
    : poseChange(std::make_shared<CPosePDFGaussian>()),
      rawOdometryIncrementReading(), estimationMethod(emOdometry),
      hasEncodersInfo(false), encoderLeftTicks(0), encoderRightTicks(0),
      hasVelocities(false), velocityLocal(.0, .0, .0),
      motionModelConfiguration(), m_fastDrawGauss_Z(), m_fastDrawGauss_M() {}
...
void CActionRobotMovement2D::readFromStream(mrpt::utils::CStream &in,
  ...
  case 7: {
    ...
    if (estimationMethod == emOdometry) {
      ...
      // Re-build the PDF:
      computeFromOdometry(rawOdometryIncrementReading,
  ...
  case 2: {
    ...
    if (estimationMethod == emOdometry) {
      ...
      //				TMotionModelOptions_OLD_VERSION_2
      //dummy;
      //				in.ReadBuffer( &dummy,
...
/*---------------------------------------------------------------
                                        computeFromEncoders
 ---------------------------------------------------------------*/
void CActionRobotMovement2D::computeFromEncoders(double K_left, double K_right,
                                                 double D) {
  if (hasEncodersInfo) {
    const double As =
...
CActionRobotMovement2D::TMotionModelOptions::TMotionModelOptions()
    : modelSelection(CActionRobotMovement2D::mmGaussian), gaussianModel(),
      thrunModel() {
...
void CActionRobotMovement2D::computeFromOdometry_modelGaussian(
  ...
  C_ODO(0, 0) = square(o.gaussianModel.minStdXY + o.gaussianModel.a1 * Al +
                       o.gaussianModel.a2 * fabs(odometryIncrement.phi()));
  C_ODO(1, 1) = square(o.gaussianModel.minStdXY + o.gaussianModel.a1 * Al +
                       o.gaussianModel.a2 * fabs(odometryIncrement.phi()));
  C_ODO(2, 2) = square(o.gaussianModel.minStdPHI + o.gaussianModel.a3 * Al +
...
void CActionRobotMovement2D::computeFromOdometry_modelThrun(
  ...
  // ---------------------------------------------------------------------------------------------
  // The following is an implementation from Thrun et al.'s book  (Probabilistic
  // Robotics),
...
void CActionRobotMovement2D::drawSingleSample_modelThrun(
  ...
  float Arot1 = (rawOdometryIncrementReading.y() != 0 ||
                 rawOdometryIncrementReading.x() != 0)
                    ? atan2(rawOdometryIncrementReading.y(),
  ...
  // Output:
  outSample.x(Atrans_draw * cos(Arot1_draw) +
              motionModelConfiguration.thrunModel.additional_std_XY *
                  randomGenerator.drawGaussian1D_normalized());
  outSample.y(Atrans_draw * sin(Arot1_draw) +
              motionModelConfiguration.thrunModel.additional_std_XY *
                  randomGenerator.drawGaussian1D_normalized());
  outSample.phi(Arot1_draw + Arot2_draw +
                motionModelConfiguration.thrunModel.additional_std_phi *
                    randomGenerator.drawGaussian1D_normalized());
...
void CActionRobotMovement2D::fastDrawSingleSample_modelGaussian(
  ...
  outSample = CPose2D(m_fastDrawGauss_M.x() + rndVector[0],
