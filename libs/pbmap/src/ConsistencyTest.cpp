...
Eigen::Matrix4f
ConsistencyTest::initPose(std::map<unsigned, unsigned> &matched_planes) {
  ...
  //    sumError += error;
  //    cout << "errorRot " << it->first << " " << it->second << " is " << error
  //    << endl;
  //  }
  ...
  for (map<unsigned, unsigned>::iterator it = matched_planes.begin();
       it != matched_planes.end(); it++) {
    float trans_error = (PBMSource.vPlanes[it->first].d -
  ...
  // Form SE3 transformation matrix. This matrix maps the model into the current
...
// Transformation from Source to Target
Eigen::Matrix4f
ConsistencyTest::estimatePose(std::map<unsigned, unsigned> &matched_planes) {
  if (matched_planes.size() < 3) {
    cout << "Insuficient matched planes " << matched_planes.size() << endl;
    return Eigen::Matrix4f::Identity();
  }
  ...
  for (map<unsigned, unsigned>::iterator it = matched_planes.begin();
    ...
    float trans_error = (PBMSource.vPlanes[it->first].d -
                         PBMTarget.vPlanes[it->second].d); //+n*t
    //  cout << it->first << " area " << PBMSource.vPlanes[it->first].areaHull
    //  << endl;
    //    accum_error2 += trans_error * trans_error;
    //    hessian += PBMTarget.vPlanes[it->second].v3normal *
    //    PBMTarget.vPlanes[it->second].v3normal.transpose();
...
Eigen::Matrix4f
ConsistencyTest::initPose2D(std::map<unsigned, unsigned> &matched_planes) {
  // Calculate rotation
  Matrix3f normalCovariances = Matrix3f::Zero();
  for (map<unsigned, unsigned>::iterator it = matched_planes.begin();
       it != matched_planes.end(); it++)
  ...
  for (map<unsigned, unsigned>::iterator it = matched_planes.begin();
       it != matched_planes.end(); it++) {
    if (PBMSource.vPlanes[it->first].bFromStructure) // The certainty in center
                                                     // of structural planes is
                                                     // too low
      continue;
...
Eigen::Matrix4f
ConsistencyTest::getRTwithModel(std::map<unsigned, unsigned> &matched_planes) {
  assert(matched_planes.size() >= 3);
  Eigen::Matrix4f rigidTransf =
  ...
  while (nIter < 10 && improveRate < 0.999) {
    ...
    updatePose << CMatUpdate(0, 0), CMatUpdate(0, 1), CMatUpdate(0, 2),
        CMatUpdate(0, 3), CMatUpdate(1, 0), CMatUpdate(1, 1), CMatUpdate(1, 2),
        CMatUpdate(1, 3), CMatUpdate(2, 0), CMatUpdate(2, 1), CMatUpdate(2, 2),
        CMatUpdate(2, 3), 0, 0, 0, 1;
...
CMatrixDouble getAlignment(const CMatrixDouble &matched_planes) {
  ...
  for (unsigned i = 0; i < 3; i++) {
    Vector3f n_i = Vector3f(matched_planes(0, i), matched_planes(1, i),
                            matched_planes(2, i));
    Vector3f n_ii = Vector3f(matched_planes(4, i), matched_planes(5, i),
                             matched_planes(6, i));
...
void ransac3Dplane_distance(const CMatrixDouble &planeCorresp,
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    const Eigen::Vector3f n_ii =
        Rotation * Eigen::Vector3f(planeCorresp(4, i), planeCorresp(5, i),
                                   planeCorresp(6, i));
    const float d_error =
        fabs((planeCorresp(7, i) - translation.dot(n_i)) - planeCorresp(3, i));
    const float angle_error = (n_i.cross(n_ii)).norm();

    if (d_error < distanceThreshold)
      if (angle_error < distanceThreshold) // Warning: this threshold has a
                                           // different dimension
...
bool ransac3Dplane_degenerate(const CMatrixDouble &planeCorresp,
  ...
  const Eigen::Vector3f n_1 = Eigen::Vector3f(planeCorresp(0, useIndices[0]),
                                              planeCorresp(1, useIndices[0]),
                                              planeCorresp(2, useIndices[0]));
  const Eigen::Vector3f n_2 = Eigen::Vector3f(planeCorresp(0, useIndices[1]),
                                              planeCorresp(1, useIndices[1]),
                                              planeCorresp(2, useIndices[1]));
  const Eigen::Vector3f n_3 = Eigen::Vector3f(planeCorresp(0, useIndices[2]),
                                              planeCorresp(1, useIndices[2]),
...
Eigen::Matrix4f ConsistencyTest::estimatePoseRANSAC(
  ...
  ransac_executer.execute(planeCorresp, ransacPlaneAlignment_fit,
                          ransac3Dplane_distance, ransac3Dplane_degenerate, 0.2,
