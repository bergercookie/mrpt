...
*/
bool SubgraphMatcher::evalUnaryConstraints(Plane &plane1, Plane &plane2,
                                           PbMap &trgPbMap, bool useStructure) {
...
// double tCondition1, tCondition2, tCondition3;
  ...
  //++nCheckConditions;
  if (plane1.bFromStructure &&
      plane2.bFromStructure) { // cout << "True: both structure\n";
    return true;
  }
  ...
  if (plane1.bFullExtent && plane2.bFullExtent) {
    ...
    } else if (rel_areas < configLocaliser.area_threshold_inv ||
               rel_areas > configLocaliser.area_threshold) {
#if _VERBOSE
...
bool SubgraphMatcher::evalUnaryConstraintsOdometry(Plane &plane1, Plane &plane2,
                                                   PbMap &trgPbMap,
                                                   bool useStructure) {
// Eigen::Vector3f color_dif = plane1.v3colorNrgb - plane2.v3colorNrgb;
// if(plane1.id==6 && plane2.id==8)
...
bool SubgraphMatcher::evalUnaryConstraints2D(Plane &plane1, Plane &plane2,
                                             PbMap &trgPbMap,
                                             bool useStructure) {
// Eigen::Vector3f color_dif = plane1.v3colorNrgb - plane2.v3colorNrgb;
// if(plane1.id==6 && plane2.id==8)
...
*/
bool SubgraphMatcher::evalBinaryConstraints(Plane &Ref, Plane &neigRef,
                                            Plane &Check, Plane &neigCheck) {
  ...
  if (neigRef.areaHull < 2 && neigCheck.areaHull < 2) {
    double dif_height =
        fabs(Ref.v3normal.dot(neigRef.v3center - Ref.v3center) -
    ...
    float threshold_dist_dependent = configLocaliser.height_threshold *
                                     (Ref.v3center - neigRef.v3center).norm() *
                                     0.4;
    float height_threshold =
        std::max(configLocaliser.height_threshold, threshold_dist_dependent);
...
//    if(dif_height > configLocaliser.height_threshold){
  ...
  if (Ref.areaHull < 2 && Check.areaHull < 2) {
    double dif_height2 =
        fabs(neigRef.v3normal.dot(Ref.v3center - neigRef.v3center) -
...
//    if(dif_height > configLocaliser.height_threshold){
  ...
  // Normal
  double dif_normal =
      fabs(RAD2DEG(acos(Ref.v3normal.dot(neigRef.v3normal)) -
                   acos(Check.v3normal.dot(neigCheck.v3normal))));
  //  if( dif_normal > configLocaliser.angle_threshold ){
  if (dif_normal > std::max(configLocaliser.angle_threshold,
  ...
  if (configLocaliser.use_completeness) {
    if (RefBothFull && CheckBothFull) {
      ...
      //++nCheckConditions;
    } else if (!Ref.bFromStructure && !Check.bFromStructure &&
    ...
    if (Ref.areaVoxels < 1 && neigCheck.areaVoxels < 1 &&
        Check.areaVoxels < 1 &&
        neigRef.areaVoxels < 1) { // Use the restriction only when all the
                                  // planes involved are smaller than 1m2
      if (rel_dist_centers < configLocaliser.dist_threshold_inv ||
          rel_dist_centers > configLocaliser.dist_threshold) {
#if _VERBOSE
...
void SubgraphMatcher::exploreSubgraphTreeR(set<unsigned> &sourcePlanes,
                                           set<unsigned> &targetPlanes,
  ...
  while (!sourcePlanes.empty()) {
...
#if _VERBOSE
    cout << "Compare " << *it1 << " Compare Compare Compare Compare Compare "
         << endl;
#endif
    ...
    //        matched.size() << " prev winner " << winnerMatch.size() << endl;
    //      #endif
    //      return;
    //    }
    ...
    for (set<unsigned>::iterator it2 = targetPlanes.begin();
      ...
      //      subgraphTrg->pPBM->vPlanes[*it2], *subgraphTrg->pPBM, false )
      //      )//(FloorPlane != -1 && FloorPlaneMap != -1) ? true : false ) )
      if (hashUnaryConstraints[*it1][*it2] !=
          1) //(FloorPlane != -1 && FloorPlaneMap != -1) ? true : false ) )
        continue;
      ...
      //    cout << "Match edge\n";
      // If this point is reached, the planes it1 and it2 are candidates to be
      // the same
...
#if _VERBOSE
    cout << "End branch recursive search. matched " << matched.size() << " A "
         << areaMatched << " prev winner " << winnerMatch.size() << " A "
         << areaWinnerMatch << endl;
#endif
...
SubgraphMatcher::compareSubgraphs(Subgraph &subgraphSource,
                                  Subgraph &subgraphTarget, const int option) {
  // cout << "SubgraphMatcher::compareSubgraphs... \n";
  subgraphSrc = &subgraphSource;
  subgraphTrg = &subgraphTarget;
  map<unsigned, unsigned> matched;
  ...
  if (option == 0) // Default subgraph matcher
    ...
    for (set<unsigned>::iterator it1 = sourcePlanes.begin();
      ...
      for (set<unsigned>::iterator it2 = targetPlanes.begin();
           it2 != targetPlanes.end(); it2++)
        hashUnaryConstraints[*it1][*it2] =
            (evalUnaryConstraintsOdometry(subgraphSrc->pPBM->vPlanes[*it1],
                                          subgraphTrg->pPBM->vPlanes[*it2],
                                          *subgraphTrg->pPBM, false)
                 ? 1
                 : 0);
  else if (option == 2) // Default graph matcher restricted to planar movement
                        // (fix plane x=const)
    for (set<unsigned>::iterator it1 = sourcePlanes.begin();
         it1 != sourcePlanes.end(); it1++)
      for (set<unsigned>::iterator it2 = targetPlanes.begin();
           it2 != targetPlanes.end(); it2++)
        hashUnaryConstraints[*it1][*it2] =
            (evalUnaryConstraints2D(subgraphSrc->pPBM->vPlanes[*it1],
                                    subgraphTrg->pPBM->vPlanes[*it2],
                                    *subgraphTrg->pPBM, false)
                 ? 1
                 : 0);
  else if (option == 3) // Odometry graph matcher restricted to planar movement
                        // (fix plane x=const)
    for (set<unsigned>::iterator it1 = sourcePlanes.begin();
         it1 != sourcePlanes.end(); it1++)
      for (set<unsigned>::iterator it2 = targetPlanes.begin();
           it2 != targetPlanes.end(); it2++)
        hashUnaryConstraints[*it1][*it2] =
            (evalUnaryConstraintsOdometry2D(subgraphSrc->pPBM->vPlanes[*it1],
                                            subgraphTrg->pPBM->vPlanes[*it2],
                                            *subgraphTrg->pPBM, false)
                 ? 1
                 : 0);
