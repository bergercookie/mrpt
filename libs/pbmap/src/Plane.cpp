...
float Plane::compute2DPolygonalArea() {
  ...
  // cos(theta), where theta is the angle between the polygon and the projected
  // plane
...
void Plane::getPlaneC1C2C3() {
  ...
  for (unsigned i = 0; i < planePointCloudPtr->size(); i++) {
    c1[i] = atan2((double)planePointCloudPtr->points[i].r,
                  (double)max(planePointCloudPtr->points[i].g,
                              planePointCloudPtr->points[i].b));
    c2[i] = atan2((double)planePointCloudPtr->points[i].g,
                  (double)max(planePointCloudPtr->points[i].r,
                              planePointCloudPtr->points[i].b));
    c3[i] = atan2((double)planePointCloudPtr->points[i].b,
                  (double)max(planePointCloudPtr->points[i].r,
                              planePointCloudPtr->points[i].g));
  }
}
...
void Plane::calcPlaneHistH() {
  ...
  for (unsigned i = 0; i < planePointCloudPtr->size(); i++) {
    ...
    if (fV < 0.01) {
      //      fH = 72; // Histogram has 72 bins for Hue values, and 2 bins more
      //      for black and white
      H[72]++;
      continue;
    } else
    ...
    {                     // Make sure its not pure black.
      ...
      if (fS < 0.2) {
        //        fH = 73; // Histogram has 72 bins for Hue values, and 2 bins
        //        more for black and white
        H[73]++;
        continue;
      }

      //      float ANGLE_TO_UNIT = 1.0f / (6.0f * fDelta);	// Make the Hues
      //      between 0.0 to 1.0 instead of 6.0
      float ANGLE_TO_UNIT = 12.0f / fDelta; // 12 bins
      if (iMax ==
          planePointCloudPtr->points[i].r) { // between yellow and magenta.
        fH = (fG - fB) * ANGLE_TO_UNIT;
      } else if (iMax ==
                 planePointCloudPtr->points[i].g) { // between cyan and yellow.
...
void Plane::calcMainColor2() {
  ...
  size_t countPix = 0;
  size_t stepColor =
      std::max(planePointCloudPtr->size() / 2000,
               static_cast<size_t>(
  ...
  for (size_t i = 0; i < planePointCloudPtr->size(); i += stepColor) {
    ...
    if (sumRGB != 0) {
      ...
      // cout << "color " << color[countPix][0] << " " << color[countPix][1] <<
      // " " << color[countPix][2] << " " << color[countPix][3] << endl;
      ++countPix;
    }
  }
...
bool Plane::isPlaneNearby(Plane &plane_nearby, const float distThreshold) {
  ...
  for (unsigned i = 1; i < polygonContourPtr->size(); i++)
    for (unsigned j = 1; j < plane_nearby.polygonContourPtr->size(); j++)
      if ((diffPoints(polygonContourPtr->points[i],
  ...
  for (unsigned i = 1; i < polygonContourPtr->size(); i++)
    for (unsigned j = 1; j < plane_nearby.polygonContourPtr->size(); j++)
      if (dist3D_Segment_to_Segment2(
              Segment(polygonContourPtr->points[i],
                      polygonContourPtr->points[i - 1]),
              Segment(plane_nearby.polygonContourPtr->points[j],
...
bool Plane::isSamePlane(Plane &plane_nearby, const float &cosAngleThreshold,
 ...
 * If the planes are the same they are merged in this and the function returns
 * true. Otherwise it returns false.*/
bool Plane::isSamePlane(Eigen::Matrix4f &Rt, Plane &plane_,
  ...
  pcl::transformPointCloud(*plane_.polygonContourPtr, *plane.polygonContourPtr,
                           Rt);
...
void Plane::mergePlane(Plane &same_plane_patch) {
  ...
  v3normal = (inliers.size() * v3normal +
  ...
  *planePointCloudPtr = mergeCloud;
  ...
  inliers.insert(inliers.end(), same_plane_patch.inliers.begin(),
...
void Plane::mergePlane2(Plane &same_plane_patch) {
  ...
  *same_plane_patch.polygonContourPtr += *polygonContourPtr;
  ...
  d = -v3normal.dot(v3center);
  // std::cout << d << "area " << areaHull << " center " << v3center.transpose()
  // << " elongation " << elongation << " v3PpalDir " << v3PpalDir.transpose()
  // << std::endl;
