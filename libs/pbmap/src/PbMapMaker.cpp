...
struct config_pbmap {
  ...
  int graph_mode; // This var selects the condition to create edges in the
                  // graph, either proximity of planar patches or co-visibility
                  // in a single frame

  // [semantics]
  bool inferStructure; // Infer if the planes correspond to the floor, ceiling
                       // or walls
...
void readConfigFile(const string &config_file_name) {
  ...
  configPbMap.intensity_threshold =
      config_file.read_float("map_construction", "intensity_threshold", 255.0);
  ...
  if (configPbMap.detect_loopClosure)
    configPbMap.config_localiser =
        config_file.read_string("localisation", "config_localiser", "", true);
  ...
  configPbMap.path_save_registered_cloud =
      config_file.read_string("serialize", "path_save_registered_cloud",
...
PbMapMaker::PbMapMaker(const string &config_file)
    : cloudViewer("Cloud Viewer"), mpPbMapLocaliser(nullptr),
      m_pbmaker_must_stop(false), m_pbmaker_finished(false) {
  // Load parameters
...
bool PbMapMaker::arePlanesNearby(Plane &plane1, Plane &plane2,
  ...
  for (unsigned i = 1; i < plane1.polygonContourPtr->size(); i++)
    for (unsigned j = 1; j < plane2.polygonContourPtr->size(); j++)
      if ((diffPoints(plane1.polygonContourPtr->points[i],
  ...
  for (unsigned i = 1; i < plane1.polygonContourPtr->size(); i++)
    for (unsigned j = 1; j < plane2.polygonContourPtr->size(); j++)
      if (dist3D_Segment_to_Segment2(
              Segment(plane1.polygonContourPtr->points[i],
                      plane1.polygonContourPtr->points[i - 1]),
              Segment(plane2.polygonContourPtr->points[j],
  ...
  for (unsigned i = 1; i < plane1.polygonContourPtr->size(); i++)
    if (plane2.v3normal.dot(
      ...
      if (isInHull(plane1.polygonContourPtr->points[i],
  ...
  for (unsigned j = 1; j < plane2.polygonContourPtr->size(); j++)
    if (plane1.v3normal.dot(
      ...
      if (isInHull(plane2.polygonContourPtr->points[j],
...
void PbMapMaker::checkProximity(Plane &plane, float proximity) {
  for (unsigned i = 0; i < mPbMap.vPlanes.size(); i++) {
    ...
    if (arePlanesNearby(plane, mPbMap.vPlanes[i],
                        proximity)) // If the planes are closer than proximity
                                    // (in meters), then mark them as neighbors
    {
...
void PbMapMaker::watchProperties(set<unsigned> &observedPlanes,
                                 Plane &observedPlane) {
  cout << "PbMapMaker::watchProperties..." << configPbMap.color_threshold << " "
       << configPbMap.intensity_threshold << " " << configPbMap.hue_threshold
       << "\n";
  // Watch properties
  //  double tCondition;
...
//    timeCompareElong.push_back( (pcl::getTime() - tCondition)*1e6 );
      ...
      double dif_C1C2C3 =
          (observedPlane.v3colorC1C2C3 - observedPlane.prog_C1C2C3[i]).norm();
      ...
      if (fabs(observedPlane.dominantIntensity -
      ...
      // Hue histogram
      double hist_dist =
          BhattacharyyaDist(observedPlane.hist_H, observedPlane.prog_hist_H[i]);
...
//    timeCompareElong.push_back( (pcl::getTime() - tCondition)*1e6 );
      ...
      // prevPlane.dominantIntensity << " " << configPbMap.intensity_threshold;
      // cout << " " << rejectIntensity_F << " " << acceptIntensity_F << endl;

      if (fabs(observedPlane.dominantIntensity - prevPlane.dominantIntensity) >
          configPbMap.intensity_threshold)
        rejectIntensity_F++;
      else
        acceptIntensity_F++;
...
void PbMapMaker::saveInfoFiles() {
  ...
  file << configPbMap.area_THRESHOLD << " " << rejectAreaT << " " << acceptAreaT
       << " " << rejectAreaF << " " << acceptAreaF << endl;
  file.close();
  ...
  file << configPbMap.intensity_threshold << " " << rejectColor_T << " "
       << acceptColor_T << " " << rejectColor_F << " " << acceptColor_F << endl;
  file.close();

  results_file = "results/HueRestriction.txt";
  file.open(results_file.c_str(), ios::app);
  file << configPbMap.hue_threshold << " " << rejectHistH_T << " "
       << acceptHistH_T << " " << rejectHistH_F << " " << acceptHistH_F << endl;
  file.close();
#endif
}
...
void PbMapMaker::detectPlanesCloud(
  ...
  for (size_t i = 0; i < regions.size(); i++) {
    ...
    plane.v3normal =
        poseKF.block(0, 0, 3, 3) * Vector3f(model_coefficients[i].values[0],
    ...
    cout << "Area plane region " << plane.areaVoxels << " of Chull "
         << plane.areaHull << " of polygon " << plane.compute2DPolygonalArea()
         << endl;
#endif

    // Check whether this region correspond to the same plane as a previous one
    // (this situation may happen when there exists a small discontinuity in the
    // observation)
    bool isSamePlane = false;
    for (size_t j = 0; j < detectedPlanes.size(); j++)
      if (areSamePlane(detectedPlanes[j], plane, configPbMap.max_cos_normal,
  ...
  observedPlanes.clear();
  {
    ...
    for (size_t i = 0; i < detectedPlanes.size(); i++) {
      ...
      {
        if (areSamePlane(mPbMap.vPlanes[j], detectedPlanes[i],
                                                           ...
                                                           // the same
        {
          ...
          //          {
          //            cout << " ID " << mPbMap.vPlanes[j].id << " obs " <<
          //            mPbMap.vPlanes[j].numObservations;
          //            cout << " areaVoxels " << mPbMap.vPlanes[j].areaVoxels
          //            << " areaVoxels " << mPbMap.vPlanes[j].areaHull;
          //            cout << " ratioXY " << mPbMap.vPlanes[j].elongation << "
          //            structure " << mPbMap.vPlanes[j].bFromStructure << "
          //            label " << mPbMap.vPlanes[j].label;
          ...
          //            it=mPbMap.vPlanes[j].neighborPlanes.begin(); it !=
          //            mPbMap.vPlanes[j].neighborPlanes.end(); it++)
          //              cout << it->first << " ";
          //            cout << "\n CommonObservations: ";
          //            for(map<unsigned,unsigned>::iterator
          //            it=mPbMap.vPlanes[j].neighborPlanes.begin(); it !=
          ...
          //            mPbMap.vPlanes[j].polygonContourPtr->size(); jj++)
          //              cout << "\t" <<
          ...
          //          }
          //          {
          ////            cout << " ID " << detectedPlanes[i].id << " obs " <<
          ///detectedPlanes[i].numObservations;
          ////            cout << " areaVoxels " << detectedPlanes[i].areaVoxels
          ///<< " areaVoxels " << detectedPlanes[i].areaHull;
          ////            cout << " ratioXY " << detectedPlanes[i].elongation <<
          ///" structure " << detectedPlanes[i].bFromStructure << " label " <<
          ...
          //            for(unsigned jj=0; jj <
          //            detectedPlanes[i].polygonContourPtr->size(); jj++)
          //              cout << "\t" <<
...
#ifdef _VERBOSE
          cout << "Previous plane " << mPbMap.vPlanes[j].id << " area "
               << mPbMap.vPlanes[j].areaVoxels << " of polygon "
               << mPbMap.vPlanes[j].compute2DPolygonalArea() << endl;
#endif

          if (observedPlanes.count(mPbMap.vPlanes[j].id) ==
              0) // If this plane has already been observed through a previous
                 // partial plane in this same keyframe, then we must not
                 // account twice in the observation count
          {
            mPbMap.vPlanes[j].numObservations++;
            ...
            for (set<unsigned>::iterator it = observedPlanes.begin();
              ...
              if (mPbMap.vPlanes[j].neighborPlanes.count(*it)) {
                ...
                mPbMap.vPlanes[*it].neighborPlanes
                    [mPbMap.vPlanes[j].id]++; // j = mPbMap.vPlanes[j]
              } else {
          ...
          for (size_t k = j + 1; k < numPrevPlanes;
            ...
            if (areSamePlane(mPbMap.vPlanes[j], mPbMap.vPlanes[k],
                             configPbMap.max_cos_normal,
                             configPbMap.max_dist_center_plane,
                             configPbMap.proximity_threshold)) // The planes are
                                                               // merged if they
                                                               // are the same
            {
              ...
              for (set<unsigned>::iterator it =
                   ...
                   it != mPbMap.vPlanes[k].nearbyPlanes.end(); it++)
              ...
              for (map<unsigned, unsigned>::iterator it =
                   ...
                   it != mPbMap.vPlanes[k].neighborPlanes.end(); it++)
              ...
              for (size_t h = 0; h < numPrevPlanes; h++) {
                ...
                for (set<unsigned>::iterator it =
                     ...
                     it != mPbMap.vPlanes[h].nearbyPlanes.end(); it++)
                ...
                for (map<unsigned, unsigned>::iterator it =
                         mPbMap.vPlanes[h].neighborPlanes.begin();
                     it != mPbMap.vPlanes[h].neighborPlanes.end(); it++)
                  if (it->first > mPbMap.vPlanes[k].id) {
                    mPbMap.vPlanes[h].neighborPlanes[it->first - 1] =
                        it->second;
...
#ifdef _VERBOSE
              cout << "MERGE TWO PREVIOUS PLANES WHEREBY THE INCORPORATION OF "
                      "A NEW REGION \n";
#endif
            }
      ...
      if (!isSamePlane) {
        ...
        if (configPbMap.makeClusters) {
          ...
          clusterize->groups[clusterize->currentSemanticGroup].push_back(
              detectedPlanes[i].id);
        }
        ...
        // Update proximity graph
        checkProximity(detectedPlanes[i],
                       configPbMap.proximity_neighbor_planes); // Detect
                                                               // neighbors with
                                                               // max separation
                                                               // of 1.0 meters
      ...
      -1) // Verify that the observed planes centers are above the floor
  {
    ...
    for (set<unsigned>::reverse_iterator it = observedPlanes.rbegin();
      ...
      if (mPbMap.vPlanes[mPbMap.FloorPlane].v3normal.dot(
        ...
        if (mPbMap.vPlanes[mPbMap.FloorPlane].v3normal.dot(
                mPbMap.vPlanes[*it].v3normal) > 0.99) //(cos 8.1º = 0.99)
        {
...
bool PbMapMaker::areSamePlane(Plane &plane1, Plane &plane2,
...
void PbMapMaker::mergePlanes(Plane &updatePlane, Plane &discardPlane) {
  ...
  *updatePlane.planePointCloudPtr +=
      *discardPlane.planePointCloudPtr; // Add the points of the new detection
                                        // and perform a voxel grid

  // Filter the points of the patch with a voxel-grid. This points are used only
  // for visualization
...
void PbMapMaker::viz_cb(pcl::visualization::PCLVisualizer &viz) {
...

  {
    ...
    // Render the data
    {
      ...
      //            viz.addText3D (name, center, 0.1, ared[i%10], agrn[i%10],
      //            ablu[i%10], name);
      //          }
      //
      //          // Draw edges
      ...
      //                break;
      //
      //              sprintf (name, "commonObs%u_%u", static_cast<unsigned>(i),
      //              static_cast<unsigned>(*it));
      //              pcl::PointXYZ center_it(2*mPbMap.vPlanes[*it].v3center[0],
      //              2*mPbMap.vPlanes[*it].v3center[1],
      //              2*mPbMap.vPlanes[*it].v3center[2]);
      //              viz.addLine (center, center_it, ared[i%10], agrn[i%10],
      //              ablu[i%10], name);
      //            }
      //          else
      //            for(map<unsigned,unsigned>::iterator it =
      ...
      { // Regular representation
        ...
        if (mpPbMapLocaliser != nullptr)
          if (mpPbMapLocaliser->alignedModelPtr) {
            if (!viz.updatePointCloud(mpPbMapLocaliser->alignedModelPtr,
                                      "model"))
              viz.addPointCloud(mpPbMapLocaliser->alignedModelPtr, "model");
          }

        sprintf(name, "PointCloud size %u",
        ...
        for (size_t i = 0; i < mPbMap.vPlanes.size(); i++) {
          ...
          pcl::PointXYZ pt1,
              pt2; // Begin and end points of normal's arrow for visualization
          pt1 = pcl::PointXYZ(plane_i.v3center[0], plane_i.v3center[1],
                              plane_i.v3center[2]);
          pt2 =
              pcl::PointXYZ(plane_i.v3center[0] + (0.5f * plane_i.v3normal[0]),
                            plane_i.v3center[1] + (0.5f * plane_i.v3normal[1]),
                            plane_i.v3center[2] + (0.5f * plane_i.v3normal[2]));
          viz.addArrow(pt2, pt1, ared[i % 10], agrn[i % 10], ablu[i % 10],
                       false, name);
          ...
          //            pcl::PointXYZ pt3 = pcl::PointXYZ ( plane_i.v3center[0]
          //            + (0.2f * plane_i.v3PpalDir[0]),
          //                                                plane_i.v3center[1]
          //                                                + (0.2f *
          //                                                plane_i.v3PpalDir[1]),
          //                                                plane_i.v3center[2]
          //                                                + (0.2f *
          //                                                plane_i.v3PpalDir[2]));
          //            viz.addArrow (pt3, plane_i.pt1, ared[i%10], agrn[i%10],
          //            ablu[i%10], false, name);
          //          }

          //          if( !plane_i.label.empty() )
          //            viz.addText3D (plane_i.label, pt2, 0.1, ared[i%10],
          //            agrn[i%10], ablu[i%10], plane_i.label);
          //          else
          {
            sprintf(name, "n%u", static_cast<unsigned>(i));
            //            sprintf (name, "n%u_%u", static_cast<unsigned>(i),
            //            static_cast<unsigned>(plane_i.semanticGroup));
            viz.addText3D(name, pt2, 0.1, ared[i % 10], agrn[i % 10],
                          ablu[i % 10], name);
          }

          //          sprintf (name, "planeRaw_%02u", static_cast<unsigned>(i));
          //          viz.addPointCloud (plane_i.planeRawPointCloudPtr, name);//
          //          contourPtr, planePointCloudPtr, polygonContourPtr
          ...
          //          if(plane_i.bDominantColor)
          {
            ...
            //          <PointT> color (plane_i.planePointCloudPtr, red[i%10],
            //          grn[i%10], blu[i%10]);
            ////          pcl::visualization::PointCloudColorHandlerCustom
            ///<PointT> color (plane_i.planePointCloudPtr,
            ///red[plane_i.semanticGroup%10], grn[plane_i.semanticGroup%10],
            ...
            viz.setPointCloudRenderingProperties(
                pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 10, name);
            //          }
          }
          //          else
          //            viz.addPointCloud (plane_i.planePointCloudPtr, name);//
          //            contourPtr, planePointCloudPtr, polygonContourPtr

          //          sprintf (name, "planeBorder_%02u",
          //          static_cast<unsigned>(i));
          //          pcl::visualization::PointCloudColorHandlerCustom <PointT>
          //          color2 (plane_i.contourPtr, 255, 255, 255);
          //          viz.addPointCloud (plane_i.contourPtr, color2, name);//
          //          contourPtr, planePointCloudPtr, polygonContourPtr
          ...
          //            static_cast<unsigned>(i));
          //            pcl::visualization::PointCloudColorHandlerCustom
          //            <PointT> color4 (mPbMap.edgeCloudPtr, 255, 255, 0);
          //            viz.addPointCloud (mPbMap.edgeCloudPtr, color4, name);//
          //            contourPtr, planePointCloudPtr, polygonContourPtr
          //            viz.setPointCloudRenderingProperties
          ...
          //            viz.addLine (mPbMap.edgeCloudPtr->points.front(),
          //            mPbMap.edgeCloudPtr->points.back(), ared[3], agrn[3],
          //            ablu[3], name);
          //          }

          sprintf(name, "approx_plane_%02d", int(i));
          viz.addPolygon<PointT>(plane_i.polygonContourPtr, 0.5 * red[i % 10],
                                 0.5 * grn[i % 10], 0.5 * blu[i % 10], name);
        }

        // if(configPbMap.makeClusters)
        //  for(map<unsigned, std::vector<unsigned> >::iterator
        //  it=clusterize->groups.begin(); it != clusterize->groups.end(); it++)
        ...
        //      pcl::visualization::PointCloudColorHandlerCustom <PointT> color
        //      (plane_i.planePointCloudPtr, red[planeID%10], grn[planeID%10],
        //      blu[planeID%10]);
        //      viz.addPointCloud (plane_i.planePointCloudPtr, color, name);//
        //      contourPtr, planePointCloudPtr, polygonContourPtr
        //      viz.setPointCloudRenderingProperties
