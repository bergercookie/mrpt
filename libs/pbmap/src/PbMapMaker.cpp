...
struct config_pbmap
{
	...
	// [semantics]
	bool
		inferStructure;  // Infer if the planes correspond to the floor, ceiling
	// or walls
...
void PbMapMaker::checkProximity(Plane& plane, float proximity)
{
	for (unsigned i = 0; i < mPbMap.vPlanes.size(); i++)
	{
		...
		if (arePlanesNearby(
				plane, mPbMap.vPlanes[i], proximity))  // If the planes
		// are closer
		// than proximity
		// (in meters),
		// then mark them
	...
	double distThreshold, double angleThreshold, double minInliersF)
{
	...
	for (size_t i = 0; i < regions.size(); i++)
	{
		...
		// one
		// (this situation may happen when there exists a small discontinuity in
		// the
		// observation)
		...
		for (size_t j = 0; j < detectedPlanes.size(); j++)
			if (areSamePlane(
					...
					configPbMap
						.proximity_threshold))  // The planes are merged if
			// they are the same
			{
				isSamePlane = true;
		...
		for (size_t i = 0; i < detectedPlanes.size(); i++)
		{
				 ...
				 j++, itPlane++)  // numPrevPlanes
			{
				...
				// they are the same
				{
					...
					//            cout << " areaVoxels " <<
					//            mPbMap.vPlanes[j].areaVoxels
					//            << " areaVoxels " <<
					//            mPbMap.vPlanes[j].areaHull;
					//            cout << " ratioXY " <<
					//            mPbMap.vPlanes[j].elongation << "
					//            structure " <<
					//            mPbMap.vPlanes[j].bFromStructure << "
					...
					////            cout << " areaVoxels " <<
					/// detectedPlanes[i].areaVoxels
					///<< " areaVoxels " << detectedPlanes[i].areaHull;
					////            cout << " ratioXY " <<
					/// detectedPlanes[i].elongation <<
					///" structure " << detectedPlanes[i].bFromStructure << "
					/// label " <<
			...
			if (!isSamePlane)
			{
				...
				checkProximity(
					detectedPlanes[i],
					configPbMap
						.proximity_neighbor_planes);  // Detect neighbors with
				// max separation of 1.0
				// meters
...
void PbMapMaker::mergePlanes(Plane& updatePlane, Plane& discardPlane)
{
	...
	*updatePlane.planePointCloudPtr +=
		*discardPlane
			 .planePointCloudPtr;  // Add the points of the new detection
			...
			{  // Regular representation
				...
				for (size_t i = 0; i < mPbMap.vPlanes.size(); i++)
				{
					...
					//          if(plane_i.bDominantColor)
					{
						...
						//          viz.addPointCloud
						//          (plane_i.planePointCloudPtr, color,
						//          name);
				...
				//      pcl::visualization::PointCloudColorHandlerCustom
				//      <PointT> color
				//      (plane_i.planePointCloudPtr, red[planeID%10],
				//      grn[planeID%10],
				//      blu[planeID%10]);
