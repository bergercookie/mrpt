...
bool PlaneInferredInfo::searchTheFloor(Eigen::Matrix4f &poseSensor,
  ...
  // The angle between the Y axis of the camera and the normal of the plane will
  // normally be around: a) -1 for the floor b) 1 for the ceiling c) 0 for the
  // walls
 ...
 * limits.
*/
bool PlaneInferredInfo::isPlaneCutbyImage(vector<int> &planeIndices,
...
bool PlaneInferredInfo::isSurroundingBackground(
  ...
  // Find in the frame the 2D position of the convex hull vertex by brute force
  // // Innefficient
    ...
    for (unsigned j = 0; j < planeIndices.size(); j++)
      if (frame->points[planeIndices[j]].x ==
        ...
        polyImg.push_back(pair<unsigned, unsigned>(
            planeIndices[j] % frame->width, planeIndices[j] / frame->width));
        break;
      }
  ...
  for (unsigned i = 1; i < polyImg.size(); i++) {
    Eigen::Vector2f dir;
    double normDir = sqrt(double((polyImg[i].first - center.first) *
    ...
    //  cout << "polyImg[i] " << polyImg[i].first << " " << polyImg[i].second <<
    //  endl;
    //  cout << "dir " << dir << endl;
  ...
  for (unsigned i = 1; i < outerPolygon.size(); i++) {
    double edgeLenght = std::sqrt(
        static_cast<double>((outerPolygon[i][0] - outerPolygon[i - 1][0]) *
    ...
    for (unsigned j = 0; j < edgeLenght; j++) {
      ...
      double dist2Plane =
          plane.v3normal.dot(getVector3fromPointXYZ(outerPt) - plane.v3center);
      if (dist2Plane > 0.1)
        return false;
    }
  }
