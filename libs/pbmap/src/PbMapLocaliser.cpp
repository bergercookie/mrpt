...
PbMapLocaliser::PbMapLocaliser(PbMap &mPbM, const string &config_file)
    : mPbMap(mPbM), m_pbMapLocaliser_must_stop(false),
...
void PbMapLocaliser::LoadPreviousPbMaps(std::string fileMaps) {
  ...
  while (!configFile.eof()) {
    ...
    //  cout << "PbMap loaded has " << previousPbMap.vPlanes.size() << " planes"
    //  << endl;
  }
...
void PbMapLocaliser::compareSubgraphNeighbors(SubgraphMatcher &matcher) {
  ...
  for (map<unsigned, unsigned>::iterator it = bestMatch.begin();
    ...
    if (matcher.configLocaliser.graph_mode == 0) // Nearby neighbors - Check the
                                                 // 2nd order neighbors of the
                                                 // matched planes
      for (set<unsigned>::iterator it1 =
        ...
        for (set<unsigned>::iterator it2 =
          ...
          if (!matcher.evalUnaryConstraints(
                  ...
                  matchedPbMap, false)) //(FloorPlane != -1 && FloorPlaneMap !=
                                        //-1) ? true : false ) )
            continue;
          for (map<unsigned, unsigned>::iterator it_matched = bestMatch.begin();
               it_matched != bestMatch.end(); it_matched++)
            if (!matcher.evalBinaryConstraints(
                    mPbMap.vPlanes[*it1], mPbMap.vPlanes[it_matched->first],
    ...
    else // if(matcher.configLocaliser.graph_mode == 1)// Co-visible neighbors -
         // Check the 2nd order neighbors of the matched planes
      for (map<unsigned, unsigned>::iterator it1 =
        ...
        for (map<unsigned, unsigned>::iterator it2 =
                 matchedPbMap.vPlanes[it->second].neighborPlanes.begin();
             it2 != matchedPbMap.vPlanes[it->second].neighborPlanes.end();
             it2++) {
          if (matcher.subgraphTrg->subgraphPlanesIdx.count(it2->first))
            continue;

          if (!matcher.evalUnaryConstraints(
                  mPbMap.vPlanes[it1->first], matchedPbMap.vPlanes[it2->first],
                  matchedPbMap, false)) //(FloorPlane != -1 && FloorPlaneMap !=
                                        //-1) ? true : false ) )
            continue;
          for (map<unsigned, unsigned>::iterator it_matched = bestMatch.begin();
...
bool PbMapLocaliser::searchPlaneContext(Plane &searchPlane) {
  ...
  // REVISAR: Esto cambia el codigo anterior (antes la vecindad actual contenia
  // todos los vecinos de los planos observados
  ...
  for (size_t mapId = 0; mapId < previousPbMaps.size(); mapId++) {
    ...
    for (size_t i = 0; i < prevPbMap.vPlanes.size(); i++) {
      ...
      //  cout << "\nsource size " << currentSubgraph.subgraphPlanesIdx.size()
      //  << endl;
      //  cout << "target size " << targetSubgraph.subgraphPlanesIdx.size() <<
      //  endl;
        ...
        if (planeRecognitionLUT.count(winnerPbMap.vPlanes[it->second].label)) {
          ...
          if (bestMatch.size() >=
            ...
            mPbMap.vPlanes[it->first].label =
                winnerPbMap.vPlanes[it->second].label;
            planeRecognitionLUT[winnerPbMap.vPlanes[it->second].label] =
                pair<int, double>(mPbMap.vPlanes[it->first].id,
                                  bestMatch.size());
          } else
            mPbMap.vPlanes[it->first].label = "";
        } else {
          ...
          planeRecognitionLUT[winnerPbMap.vPlanes[it->second].label] =
              pair<int, double>(mPbMap.vPlanes[it->first].id, bestMatch.size());
          mPbMap.vPlanes[it->first].label =
              winnerPbMap.vPlanes[it->second].label;
        }
      }
...
void PbMapLocaliser::run() {
  ...
  while (!m_pbMapLocaliser_must_stop && !placeFound) {
    ...
    if (vQueueObservedPlanes.empty()) // if(sQueueObservedPlanes.empty())
      ...
      while (!vQueueObservedPlanes.empty()) {
        // Do not consider searching planes which do not have more than 2
        // neighbor planes (too much uncertainty)
        if (matcher.configLocaliser.graph_mode == 0) // Nearby neighbors
        {
          //        cout << "vecinos1 " <<
          //        mPbMap.vPlanes[vQueueObservedPlanes[0]].nearbyPlanes.size()
          //        << endl;
          if (mPbMap.vPlanes[vQueueObservedPlanes[0]].nearbyPlanes.size() <
              matcher.configLocaliser.min_planes_recognition) {
        ...
        } else // matcher.configLocaliser.graph_mode = 1
        {
          ...
          //        << endl;
          if (mPbMap.vPlanes[vQueueObservedPlanes[0]].neighborPlanes.size() <
              matcher.configLocaliser.min_planes_recognition) {
            vQueueObservedPlanes.erase(vQueueObservedPlanes.begin());
            continue;
          }
        ...
        if (searchPlaneContext(mPbMap.vPlanes[vQueueObservedPlanes[0]])) {
          ...
          std::cout << "PLACE FOUND. Search took "
                    << double(search_plane_end - search_plane_start) << " s\n";
          //          #endif
          break;
        }
        ...
        if (timeLocalizer.count(mPbMap.vPlanes[vQueueObservedPlanes[0]]
          ...
          timeLocalizer[mPbMap.vPlanes[vQueueObservedPlanes[0]]
                            .neighborPlanes.size()]
              .push_back(double(search_plane_end - search_plane_start));
  ...
  for (std::map<unsigned, vector<double>>::iterator it = timeLocalizer.begin();
    ...
    cout << "Size " << it->first << " time " << sum_times / it->second.size()
         << endl;
