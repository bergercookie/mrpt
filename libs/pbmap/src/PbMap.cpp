...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
PbMap::PbMap()
    : FloorPlane(-1), globalMapPtr(new pcl::PointCloud<pcl::PointXYZRGBA>()),
...
void PbMap::MergeWith(PbMap &pbm, Eigen::Matrix4f &T) {
  ...
  for (size_t i = 0; i < pbm.vPlanes.size(); i++) {
    ...
    pcl::transformPointCloud(*plane.polygonContourPtr, *plane.polygonContourPtr,
                             T);

    pcl::transformPointCloud(*plane.planePointCloudPtr,
                             *plane.planePointCloudPtr, T);
