...
public:
  ...
  /*!Check if the input plane fulfill some heuristics and so we can infer some
     ...
     a wall, the ceiling, in the world. It also makes use of the current sensor
     pose to verify some assumptions. // Modificar con un define esto ultimo
    */
  ...
  /*!Check if the input planar patch (after segmentation) is cut by the image.
    ...
    "heightSampledImage"
  */
  bool isPlaneCutbyImage(std::vector<int> &planeIndices,
   ...
   * the plane
  */
  bool isSurroundingBackground(Plane &plane,
                               pcl::PointCloud<pcl::PointXYZRGBA>::Ptr &frame,
                               std::vector<int> &planeIndices,
                               unsigned threshold);

  /*!Check if the input plane represents completely a 3D planar surface. It uses
    some heuristics to mark a plane as "Complete" when its area does not grow
    after
