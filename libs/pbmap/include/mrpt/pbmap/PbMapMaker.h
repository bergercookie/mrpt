...
/*! This class construct the PbMap extracting planar segments from Range images,
 * which pose must be also provided.
 *  The range images and their poses are communicated with the object
 * frameQueue.
...
public:
  /*!PbMapMaker's constructor sets some threshold for plane segmentation and map
     growing from a configuration file (or default).
  ...
  /*!frameQueue is a vector containing the frameRGBDandPose (range image + pose)
   * to be processed.*/
...
private:
  /*!Find planar patches in the input organised point cloud "pointCloudPtr_arg",
  and update the PbMap with them (it update previous planes and
  initialize new ones when they are first observed), the input pose "poseInv" is
  used to place the current observations into a common frame of
  reference. Different thresholds are used to control the plane segmentation:
   ...
   - "angleThreshold" defines the maximum angle between an inlier's normal and
  the plane's normal
   - "minInliersF" defines the minimum number of inliers as a fraction of the
  total number of points in the input cloud
   */
  void detectPlanesCloud(pcl::PointCloud<PointT>::Ptr &pointCloudPtr_arg,
                         Eigen::Matrix4f &poseKF, double distThreshold,
                         double angleThreshold, double minInliersF);

  /*!Returns true when the closest distance between the patches "plane1" and
   * "plane2" is under distThreshold.*/
  bool arePlanesNearby(Plane &plane1, Plane &plane2, const float distThreshold);

  /*!Check for new graph connections of the input plane. These connections are
  stablished when the minimum distance between two patches is under
  ...
  bool areSamePlane(Plane &plane1, Plane &plane2,
                    const float &cosAngleThreshold, const float &distThreshold,
...
protected:
  ...
  // Color paper
  void watchProperties(std::set<unsigned> &observedPlanes,
                       Plane &observedPlane);
  void saveInfoFiles();
  // Unary
  ...
  float rejectIntensity_F, acceptIntensity_F, rejectIntensity_T,
      acceptIntensity_T;
