...
class PBMAP_IMPEXP PbMapMaker
{
   public:
	...
	/*!frameQueue is a vector containing the frameRGBDandPose (range image +
	 * pose)
	 * to be processed.*/
   ...
   private:
	/*!Find planar patches in the input organised point cloud
	"pointCloudPtr_arg",
	and update the PbMap with them (it update previous planes and
	initialize new ones when they are first observed), the input pose "poseInv"
	is
	 ...
	 */
	void detectPlanesCloud(
		pcl::PointCloud<PointT>::Ptr& pointCloudPtr_arg,
		Eigen::Matrix4f& poseKF, double distThreshold, double angleThreshold,
		double minInliersF);

	/*!Returns true when the closest distance between the patches "plane1" and
	 * "plane2" is under distThreshold.*/
	bool arePlanesNearby(
