...
public:
  ...
  double calcAlignmentError(std::map<unsigned, unsigned> &matched_planes,
                            Eigen::Matrix4f &rigidTransf);

  /*!Return an initial guess for the rigid transformation which aligns two
  matched places.
  The translation is calculated from the planes centroids and the rotation from
  the alignment of the plane's normals.*/
  Eigen::Matrix4f initPose(std::map<unsigned, unsigned> &matched_planes);
  Eigen::Matrix4f estimatePose(
      std::map<unsigned, unsigned> &matched_planes); // Weighted with the area
  bool estimatePoseWithCovariance(std::map<unsigned, unsigned> &matched_planes,
                                  Eigen::Matrix4f &rigidTransf,
  ...
  This function iteratively minimizes the alignment error of the matched planes
  wrt the rigid transformation.*/
  ...
  Eigen::Matrix4f
  estimatePoseRANSAC(std::map<unsigned, unsigned> &matched_planes);
