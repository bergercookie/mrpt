...
public:
  ...
  bool evalUnaryConstraints(Plane &plane1, Plane &plane2, PbMap &trgPbMap,
                            bool useStructure = false);
  bool evalUnaryConstraints2D(Plane &plane1, Plane &plane2, PbMap &trgPbMap,
                              bool useStructure = false);
  bool evalUnaryConstraintsOdometry(Plane &plane1, Plane &plane2,
                                    PbMap &trgPbMap, bool useStructure = false);
  bool evalUnaryConstraintsOdometry2D(Plane &plane1, Plane &plane2,
                                      PbMap &trgPbMap,
                                      bool useStructure = false);

  /*!Check if the two pair of planes plane1-plane2 ans planeA-planeB fulfill the
  same geometric relationship, and so,
  if they are candidates to be the same planes.*/
  bool evalBinaryConstraints(Plane &plane1, Plane &plane2, Plane &planeA,
                             Plane &planeB);
  bool evalBinaryConstraintsOdometry(Plane &plane1, Plane &plane2,
  ...
  void exploreSubgraphTreeR(std::set<unsigned> &evalRef,
                            std::set<unsigned> &evalCheck,
                            std::map<unsigned, unsigned> &matched);
  void exploreSubgraphTreeR_Area(std::set<unsigned> &evalRef,
                                 std::set<unsigned> &evalCheck,
                                 std::map<unsigned, unsigned> &matched);
  ...
  compareSubgraphs(Subgraph &subgraphSource, Subgraph &subgraphTarget,
