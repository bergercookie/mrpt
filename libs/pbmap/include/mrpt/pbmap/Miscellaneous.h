...
template <class pointPCL> Eigen::Vector3f getVector3fromPointXYZ(pointPCL &pt) {
...
Eigen::Matrix<dataType, 3, 1> compose(Eigen::Matrix<dataType, 4, 4> &pose,
...
/*!Compose two poses.*/
template <class dataType>
Eigen::Matrix<dataType, 4, 4> compose(Eigen::Matrix<dataType, 4, 4> &pose1,
...
bool PBMAP_IMPEXP isInHull(PointT &point3D,
...
// shift
template <typename dataType>
Eigen::Vector4f getMultiDimMeanShift_color(std::vector<Eigen::Vector4f> &data,
                                           dataType &stdDevHist,
  ...
  while (2 * dataTemp.size() > size && shift > convergence) {
    //  std::cout << "iteration " << iteration_counter << " Std " << stdDevHist
    //  << " maxEig " << svd.singularValues().maxCoeff() << std::endl;
    for (typename std::vector<Eigen::Vector4f>::iterator it = dataTemp.begin();
         it != dataTemp.end();) {
      //        cout << "CHeck\n";
...
template <typename dataType>
dataType
getHistogramMeanShift(std::vector<dataType> &data, double range,
