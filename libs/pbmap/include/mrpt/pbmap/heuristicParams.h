...
struct PBMAP_IMPEXP config_heuristics
{
	...
	/*! Global parameter indicating the minimum required plane matches for place
	 * recognition*/
	unsigned min_planes_recognition;  // Minimum number of planes to accept a
	// match between two neighborhoods of planes
	bool use_structure;  // Use inferred knowledge
	bool use_completeness;  // Use inferred knowledge

	// [unary]
	/*! Unary constraint threshold to limit the difference of distances between
	 * two pairs camera-plane (to be used in odometry, i.e. when the two cameras
	 * are nearby */
	float dist_d;
	float angle;
	...
	float area_half_threshold,
		area_half_threshold_inv;  // Constraint with "half
	// confidence": one plana
	// is fully observed
	...
	// [binary]
	/*! Binary constraint threshold to limit the difference of distances between
	 * the centers of two pair of planes. This threashold should be loose if the
	 * plane boundaries are not known, which is the most common situation */
	float dist_threshold, dist_threshold_inv;
	float
		angle_threshold;  // Binary constraint: the angle between the normals of
	// two pair of planes
	float height_threshold;  // Binary constraint: Height from one plane wrt a
	// neighbor (in meters)
	float
		height_threshold_parallel;  // Binary constraint: Height from one plane
	// wrt a neighbor (in meters) for complete
	// marked planes
	...
	void load_params(const std::string& config_file_name)
	{
		...
		// global
		path_prev_pbmaps =
			config_file.read_string("global", "path_prev_pbmaps", "", false);
		//  std::cout << path_prev_pbmaps << std::endl;
		use_structure = config_file.read_bool("global", "use_structure", true);
		use_completeness =
			config_file.read_bool("global", "use_completeness", true);
		min_planes_recognition =
			config_file.read_int("global", "min_planes_recognition", 4);
		...
		color_threshold =
			config_file.read_float("unary", "color_threshold", 0.09);
		intensity_threshold =
			config_file.read_float("unary", "intensity_threshold", 175.0);
		hue_threshold = config_file.read_float("unary", "hue_threshold", 0.3);
		//      colorDev_threshold =
		//      config_file.read_float("unary","colorDev_threshold",0.005);
		area_threshold = config_file.read_float("unary", "area_threshold", 3.0);
		area_threshold_inv = 1 / area_threshold;
		area_full_threshold =
			config_file.read_float("unary", "area_full_threshold", 1.6);
		area_full_threshold_inv = 1 / area_full_threshold;
		area_half_threshold =
			config_file.read_float("unary", "area_half_threshold", 2.0);
		area_half_threshold_inv = 1 / area_half_threshold;
		elongation_threshold =
			config_file.read_float("unary", "elongation_threshold", 2.9);
		elongation_threshold_inv = 1 / elongation_threshold;

		// Binary constraints
		dist_threshold =
			config_file.read_float("binary", "dist_threshold", 2.0);
		dist_threshold_inv = 1 / dist_threshold;
		angle_threshold =
			config_file.read_float("binary", "angle_threshold", 7.0);
		height_threshold =
			config_file.read_float("binary", "height_threshold", 0.2);
		height_threshold_parallel =
			config_file.read_float("binary", "height_threshold_parallel", 0.2);
		cos_angle_parallel =
			config_file.read_float("binary", "cos_angle_parallel", 0.985);
	};
