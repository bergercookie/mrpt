...
private:
  ...
  */
  void buildProximityMatrix() {
    ...
    for (unsigned i = 0; i < neighborGroups.size(); i++) {
      ...
      planesVicinity_order.insert(planesVicinity_order.end(),
                                  groups[neighborGroups[i]].begin(),
    ...
    for (unsigned i = 0; i < planesVicinity_order.size(); i++) {
      ...
      for (unsigned j = i + 1; j < planesVicinity_order.size(); j++) {
        ...
        if (mPbMap.vPlanes[plane_i].neighborPlanes.count(plane_j)) {
          ...
          float sso = (float)mPbMap.vPlanes[plane_i].neighborPlanes[plane_j] /
                      std::min(mPbMap.vPlanes[plane_i].numObservations,
  ...
  */
  void arrangeNewGroups(std::vector<mrpt::vector_uint> &parts) {
    ...
    // cout << endl;
    for (unsigned i = 0; i < neighborGroups.size(); i++)
      group_limits.push_back(group_limits.back() +
                             groups[neighborGroups[i]].size());
    // cout << "group_limits: ";
    //  for(unsigned i=0; i < group_limits.size(); i++)
    //    cout << group_limits[i] << " ";
    // cout << endl;
      ...
      for (unsigned j = 0; j < parts[i].size(); j++) {
        if (i < neighborGroups.size()) {
          if (parts[i][j] < group_limits[i] ||
              parts[i][j] >=
                  group_limits[i + 1]) // The plane has changed the group
          {
            for (unsigned k = 0; k < neighborGroups.size() && i != k; k++)
              if (parts[i][j] >= group_limits[k] ||
                  parts[i][j] <
                      group_limits[k + 1]) // The plane has changed the group
              {
                assert(groups[neighborGroups[k]]
                             [parts[i][j] - group_limits[k]] ==
                       planesVicinity_order[parts[i][j]]);
                //                cout << parts[i][j] << " swithces to group "
          ...
          newGroups[groups.size() + i - neighborGroups.size()].push_back(
              planesVicinity_order[parts[i][j]]);
          mPbMap.vPlanes[planesVicinity_order[parts[i][j]]].semanticGroup =
              groups.size() + i - neighborGroups.size();
          //          cout << parts[i][j] << " swithces to NEW group " <<
          //          groups.size()+i-neighborGroups.size() << endl;
        }
      }
    }
...
public:
  ...
  */
  int evalPartition(std::set<unsigned> &observedPlanes) {
    ...
    std::vector<mrpt::vector_uint> parts; // Vector of vectors to keep the KFs
                                          // index of the different partitions
                                          // (submaps)
    mrpt::graphs::CGraphPartitioner<
        mrpt::math::CMatrix>::RecursiveSpectralPartition(connectivity_matrix,
                                                         parts, 0.8, false,
                                                         true, true, 1);

    //    cout << "Time RecursiveSpectralPartition " << time.Tac()*1000 << "ms"
    //    << endl;
