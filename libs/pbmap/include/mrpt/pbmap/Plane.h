...
public:
  Plane()
      : elongation(1.0), bFullExtent(false), bFromStructure(false),
  ...
  */
  ...
  void
  calcConvexHullandParams(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr &pointCloud,
                          std::vector<size_t> &indices = DEFAULT_VECTOR);
...
//      * \param normal the plane normal
  ...
  bool isSamePlane(Plane &plane, const float &cosAngleThreshold,
                   const float &distThreshold, const float &proxThreshold);

  bool isSamePlane(Eigen::Matrix4f &Rt, Plane &plane_,
  ...
  */
  ...
  float d;
  Eigen::Matrix4f information; // Fisher information matrix (the inverse of the
                               // plane covariance)
  float curvature;
  ...
  */
  ...
  std::vector<float> hist_H; // Normalized, Saturated Hue histogram (including 2
                             // bins for black and white)

  std::vector<double> prog_area;
  std::vector<double> prog_elongation; // This is the reatio between the lengths
                                       // of the plane in the two principal
                                       // directions
