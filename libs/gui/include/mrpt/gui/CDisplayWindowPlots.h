...
public:
  ...
  typedef void (*TCallbackMenu)(int menuID, float cursor_x, float cursor_y,
                                void *userParam); //!< Type for the callback
                                                  //!function used in
                                                  //!setMenuCallback
...
protected:
  ...
  void internal_plot(mrpt::math::CVectorFloat &x, mrpt::math::CVectorFloat &y,
                     const std::string &lineFormat,
                     const std::string &plotName);
  template <typename VECTOR1, typename VECTOR2>
  void internal_plot_interface(const VECTOR1 &x, const VECTOR2 &y,
                               const std::string &lineFormat,
                               const std::string &plotName) {
  ...
  void internal_plot_interface(const VECTOR1 &y, const std::string &lineFormat,
                               const std::string &plotName) {
...
public:
  /** Constructor
   */
  CDisplayWindowPlots(const std::string &windowCaption = std::string(),
                      unsigned int initialWidth = 350,
                      unsigned int initialHeight = 300);

  /** Class factory returning a smart pointer */
  static CDisplayWindowPlots::Ptr
  Create(const std::string &windowCaption,
  ...
  */
  ...
  /** Adds a new layer with a 2D plot based on two vectors of X and Y points,
    ...
    *coincides with an already existing plot: in this case the X & Y points are
    *used to update this existing layer (this also applies to using the default
    *plot name).
  ...
  inline void plot(const std::vector<T1> &x, const std::vector<T2> &y,
  ...
  inline void plot(const std::vector<T1> &x,
  ...
  inline void plot(const Eigen::MatrixBase<Derived1> &x,
  ...
  //! \overload
  template <typename T>
  void plot(const std::vector<T> &y,
            const std::string &lineFormat = std::string("b-"),
            const std::string &plotName = std::string("plotXY")) {
  ...
  void plot(const Eigen::MatrixBase<Derived> &y,
            const std::string &lineFormat = std::string("b-"),
            const std::string &plotName = std::string("plotXY")) {
    this->internal_plot_interface(y, lineFormat, plotName);
  }
  ...
  void GUI_IMPEXP plotEllipse(
      ...
      const mrpt::math::CMatrixTemplateNumeric<T> &cov22, const float quantiles,
      const std::string &lineFormat = std::string("b-"),
  ...
  void GUI_IMPEXP plotEllipse(
      ...
      const float quantiles, const std::string &lineFormat = std::string("b-"),
      const std::string &plotName = std::string("plotEllipse"),
      bool showName = false);
   ...
   * plot name).
    ...
    * \sa axis, axis_equal, axis_fit, hold_on, hold_off
    */
  void image(const utils::CImage &img, const float &x_left,
