...
protected:
  ...
  mrpt::opengl::COpenGLScene::Ptr m_3Dscene; //!< Internal OpenGL object (see
                                             //!general discussion in about
                                             //!usage of this object)
  mutable std::recursive_mutex
      m_csAccess3DScene; //!< Critical section for accesing m_3Dscene
...
public:
  /** Constructor */
  CDisplayWindow3D(const std::string &windowCaption = std::string(),
                   unsigned int initialWindowWidth = 400,
                   unsigned int initialWindowHeight = 300);

  /** Class factory returning a smart pointer */
  static CDisplayWindow3D::Ptr Create(const std::string &windowCaption,
                                      unsigned int initialWindowWidth = 400,
                                      unsigned int initialWindowHeight = 300);
  ...
  /** Gets a reference to the smart shared pointer that holds the internal scene
   * (carefuly read introduction in gui::CDisplayWindow3D before use!)
  ...
  void forceRepaint(); //!< Repaints the window. forceRepaint, repaint and
                       //!updateWindow are all aliases of the same method
  void repaint()       //!< Repaints the window. forceRepaint, repaint and
                       //!updateWindow are all aliases of the same method
  {
    forceRepaint();
  }
  void updateWindow() //!< Repaints the window. forceRepaint, repaint and
                      //!updateWindow are all aliases of the same method
  {
    forceRepaint();
  }

  float getFOV() const; //!< Return the camera field of view (in degrees) (used
                        //!for gluPerspective)
  void setMinRange(double new_min); //!< Changes the camera min clip range (z)
                                    //!(used for gluPerspective). The window is
                                    //!not updated with this method, call
                                    //!"forceRepaint" to update the 3D view.
  void setMaxRange(double new_max); //!< Changes the camera max clip range (z)
                                    //!(used for gluPerspective. The window is
                                    //!not updated with this method, call
                                    //!"forceRepaint" to update the 3D view.
  void setFOV(float v); //!< Changes the camera field of view (in degrees) (used
                        //!for gluPerspective). The window is not updated with
                        //!this method, call "forceRepaint" to update the 3D
                        //!view.
  void resize(unsigned int width,
              unsigned int height) override; //!< Resizes the window, stretching
                                             //!the image to fit into the
                                             //!display area.
  void
  setPos(int x,
         int y) override; //!< Changes the position of the window on the screen.
  void setWindowTitle(
      const std::string &str) override; //!<  Changes the window title.
  void setCameraElevationDeg(
      float deg); //!< Changes the camera parameters programatically
  void setCameraAzimuthDeg(
      float deg); //!< Changes the camera parameters programatically
  void setCameraPointingToPoint(
      float x, float y,
      float z); //!< Changes the camera parameters programatically
  void
  setCameraZoom(float zoom); //!< Changes the camera parameters programatically
  void setCameraProjective(
      bool isProjective); //!< Sets the camera as projective, or orthogonal.
  float
  getCameraElevationDeg() const;     //!< Get camera parameters programatically
  float getCameraAzimuthDeg() const; //!< Get camera parameters programatically
  void getCameraPointingToPoint(float &x, float &y, float &z)
      const;                   //!< Get camera parameters programatically
  float getCameraZoom() const; //!< Get camera parameters programatically
  bool
  isCameraProjective() const; //!< Sets the camera as projective, or orthogonal
  void useCameraFromScene(bool useIt = true); //!< If set to true (default =
                                              //!false), the mouse-based scene
                                              //!navigation will be disabled and
                                              //!the camera position will be
                                              //!determined by the opengl
                                              //!viewports in the 3D scene
  bool getLastMousePositionRay(mrpt::math::TLine3D &ray)
      const; //!< Gets the 3D ray for the direction line of the pixel where the
             //!mouse cursor is at. \return False if the window is closed. \sa
             //!getLastMousePosition
  virtual bool getLastMousePosition(
      int &x, int &y) const override; //!< Gets the last x,y pixel coordinates
                                      //!of the mouse. \return False if the
                                      //!window is closed. \sa
                                      //!getLastMousePositionRay
  virtual void
  setCursorCross(bool cursorIsCross) override; //!< Set cursor style to default
                                               //!(cursorIsCross=false) or to a
                                               //!cross (cursorIsCross=true) \sa
                                               //!getLastMousePositionRay
   ...
   * mrpt::gui::mrptEvent3DWindowGrabImageFile() which you can subscribe to.
    ...
    */
  void
  grabImagesStart(const std::string &grab_imgs_prefix = std::string("video_"));
  ...
  /** Add 2D text messages overlapped to the 3D rendered scene. The string will
   * remain displayed in the 3D window
    *   until it's changed with subsequent calls to this same method, or all the
   * texts are cleared with clearTextMessages().
    *
    *  \param x The X position, interpreted as absolute pixels from the left if
   * X>=1, absolute pixels from the left if X<0 or as a width factor if in the
   * range [0,1[.
   ...
   * the range [0,1[.
    ...
    *  \param unique_index An "index" for this text message, so that subsequent
   * calls with the same index will overwrite this text message instead of
   * creating new ones.
    *
  ...
  /** Get the average Frames Per Second (FPS) value from the last 250 rendering
   * events */
   ...
   * recall to first lock the parent openglscene's critical section, then do the
   * update, then release the lock, and then issue a window repaint).
    *  Internally, the texture is drawn using a mrpt::opengl::CTexturedPlane
    *  The viewport can be reverted to behave like a normal viewport by calling
   * setNormalMode()
    * \sa setImageView_fast, COpenGLViewport
          * \note This method already locks/unlocks the 3D scene of the window,
   * so the user must NOT call get3DSceneAndLock() / unlockAccess3DScene()
...
protected:
  void internal_setRenderingFPS(double FPS); //!< Set the rendering FPS (users
                                             //!don't call this, the method is
                                             //!for internal MRPT objects only)
                                             //!\sa getRenderingFPS
  void internal_emitGrabImageEvent(
      const std::string
          &fil); //!< called by
                 //!CMyGLCanvas_DisplayWindow3D::OnPostRenderSwapBuffers
 ...
 * from the wxWidgets internal MRPT thread,
  ...
  */
class GUI_IMPEXP mrptEvent3DWindowGrabImageFile
    : public mrpt::utils::mrptEvent {
protected:
  virtual void do_nothing() override {
  } //!< Just to allow this class to be polymorphic
public:
  inline mrptEvent3DWindowGrabImageFile(CDisplayWindow3D *obj,
                                        const std::string &_img_file)
      : source_object(obj), img_file(_img_file) {}

  CDisplayWindow3D *source_object;
  const std::string
      &img_file; //!< The absolute path of the file that has been just saved.
};               // End of class def.
...
public:
  /** Acquires the lock of the 3D scene of the referenced window, and returns a
   * copy of the smart pointer to it. */
