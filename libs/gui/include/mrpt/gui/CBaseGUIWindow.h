...
protected:
  mutable std::promise<void> m_threadReady; //!< This semaphore will be signaled
                                            //!when the wx window is built and
                                            //!ready.
  mutable std::promise<void> m_windowDestroyed; //!< This semaphore will be
                                                //!signaled when the wx window
                                                //!is destroyed.
  ...
  void createWxWindow(unsigned int initialWidth,
                      unsigned int initialHeight); //!< Must be called by child
                                                   //!classes just within the
                                                   //!constructor.
  void destroyWxWindow(); //!< Must be called by child classes in their
                          //!destructors. The code cannot be put into this
                          //!class' destructor.
...
public:
  ...
  void notifyChildWindowDestruction(); //!< Called by wx main thread to set
                                       //!m_hwnd to NULL.
  void notifySemThreadReady(); //!< Called by wx main thread to signal the
                               //!semaphore that the wx window is built and
                               //!ready.

public:
  /** CMD_DESTROY_WIN can be 299,399,499... See WxSubsystem */

  CBaseGUIWindow(void *winobj_voidptr, int CMD_CREATE_WIN, int CMD_DESTROY_WIN,
   ...
   * wait. To get
    ...
    *
    * \param ignoreControlKeys If set to false, any push of shift, cmd, control,
   * etc... will make this method to return.
    * \param out_pushModifier If set to !=nullptr, the modifiers of the key
   * stroke will be saved here.
    * \return The virtual key code, as defined in mrptKeyCode (a replication of
   * wxWidgets key codes).
    *
  ...
  /** Returns true if a key has been pushed, without blocking waiting for a new
   * key being pushed.
...
public:
  inline mrptEventWindowChar(CBaseGUIWindow *obj, int _char_code,
                             mrptKeyModifier _key_mod)
      : source_object(obj), char_code(_char_code), key_modifiers(_key_mod) {}

  CBaseGUIWindow *source_object;
  int char_code; //!< The virtual key code, as defined in <mrpt/gui/keycodes.h>
...
public:
  inline mrptEventWindowResize(CBaseGUIWindow *obj, size_t _new_width,
                               size_t _new_height)
...
public:
  inline mrptEventMouseDown(CBaseGUIWindow *obj,
                            mrpt::utils::TPixelCoord _coords, bool _leftButton,
                            bool _rightButton)
      : source_object(obj), coords(_coords), leftButton(_leftButton),
