...
public:
   ...
   * MAIN THREAD, in order to wait for the shutdown of the wx thread if this was
   ...
   * instance.
    ...
    *  For GUI apps (MRPT-based Windows are a part of a user wxWidget apps), we
   * must leave the control of
    *   message dispatching to the current main loop, so we cannot create a
   * different threads, making things a little different (hence this variable).
    */
  ...
  public:
    ...
    /** Atomically increments the number of windows created with the main frame
     * as parent.
      * \return The updated number of windows.
      */
    static int notifyWindowCreation();

    /** Atomically decrements the number of windows created with the main frame
     * as parent.
  ...
  struct TWxMainThreadData {
    std::thread m_wxMainThreadId; //!< The thread ID of wxMainThread, or 0 if it
                                  //!is not running.
    std::promise<void> m_semWxMainThreadReady; //!< This is signaled when
                                               //!wxMainThread is ready.
    std::promise<void> m_done;
    std::mutex m_csWxMainThreadId; //!< The critical section for accessing
                                   //!"m_wxMainThreadId"
  };

  static TWxMainThreadData &GetWxMainThreadInstance();

  /**  This will be the "MAIN" of wxWidgets: It starts an application object and
   * does not end until all the windows are closed.
  ...
  struct GUI_IMPEXP TRequestToWxMainThread {
    TRequestToWxMainThread()
        : source2D(nullptr), source3D(nullptr), sourcePlots(nullptr),
          sourceCameraSelectDialog(false), voidPtr(nullptr), voidPtr2(nullptr),
          x(400), y(400), boolVal(false) {}
    ...
    /** Valid codes are:
      *  For CDisplayWindow:
      *     - 200: Create a new 2D window, with caption "str" and initial size
      *"x" & "y", and save the "wxFrame*" in the "void**" passed in voidPtr.
      *     - 201: Updates the image shown in the window, from a "wxImage*"
      *passed in voidPtr2. The wxImage object will be freed with delete after
      *that. voidPtr must be a "wxFrame*", a "CWindowDialog*" actually.
      ...
      *     - 300: Create a new 3D window, with caption "str" and initial size
      *"x" & "y", and save the "wxFrame*" in the "void**" passed in voidPtr.
      *     - 302: Set position to x,y
      *     - 303: Change size to x,y
      *     - 304: Change title to "str"
      *		- 350: Force refresh
      *		- 360: Add a 2D text message: vector_x: [0]:x, [1]:y, [2,3,4]:R G B,
      ...
      *size "x" & "y",and save the "wxFrame*" in the "void**" passed in voidPtr.
      ...
      *		- 420: Add/update a 2D line/points plot: x/y data= vector_x/vector_y,
      *format string=str, plot name =plotName.
      *		- 421: Add/update a 2D ellipse: format string=str, plot name =plotName,
      *vector_x[0,1]:X/Y center, vector_x[2]:quantiles, vector_y[0,1,2]:
      *Covariance matrix entries 00,11,01,  boolVal=showName?
      *		- 422: Add/update a bitmap: plot name =plotName, vector_x[0,1]:X/Y
      *corner, vector_x[2,3]: X/Y widths, voidPtr2: pointer to a newly created
      *wxImage with the bitmap.
      *		- 440: Insert submenu in the popup menu. plotName=menu label,
      *x=user-defined ID.
      *		- 700: Shows a camera-pick-dialog and wait for user selection.
      *"voidPtr" must point to a CSemaphore, which will be signaled twice (1st
      *upon construction, 2nd upon dialog close); voidPtr2 must point to a
      *"mrpt::gui::CPanelCameraSelection*" which will be filled with the
      *selection (the panel must be deleted by the caller)
      *
      */
    int OPCODE;
  };
...
public:
  ...
  public:
    wxMRPTImageControl(wxWindow *parent, wxWindowID winID, int x, int y,
    ...
    void AssignImage(wxBitmap *img); //!< Assigns this image. This object has
                                     //!the ownship of the image and will delete
                                     //!it when appropriate.
...
public:
  CWindowDialog(CDisplayWindow *win2D, WxSubsystem::CWXMainFrame *parent,
                wxWindowID id = -1, const std::string &caption =
                                        std::string("[MRPT-CDisplayWindow]"),
...
public:
  C3DWindowDialog(CDisplayWindow3D *win3D, WxSubsystem::CWXMainFrame *parent,
                  wxWindowID id = -1, const std::string &caption = std::string(
  ...
  void addTextMessage(const double x_frac, const double y_frac,
  ...
  void addTextMessage(const double x_frac, const double y_frac,
                      const std::string &text,
                      const mrpt::utils::TColorf &color,
                      const std::string &font_name, const double font_size,
...
public:
  ...
  bool
      m_firstSubmenu; //!< to know whether to insert a separator the first time.
  ...
  /** Redirected from CDisplayWindowPlots::plot
    */
  void plot(const mrpt::math::CVectorFloat &x,
            const mrpt::math::CVectorFloat &y, const std::string &lineFormat,
            const std::string &plotName);

  /** Redirected from CDisplayWindowPlots::plotEllipse
    */
  void plotEllipse(const mrpt::math::CVectorFloat &x,
  ...
  /** Redirected from CDisplayWindowPlots::image
    */
  void image(void *theWxImage, const float &x0, const float &y0, const float &w,
