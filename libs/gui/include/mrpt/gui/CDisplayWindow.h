...
protected:
  /** Enables or disables the visualization of cursor coordinates on the window
   * caption.
    */
...
public:
  ...
  static CDisplayWindow::Ptr Create(const std::string &windowCaption,
   ...
   *  It adapts the size of the window to that of the image.
   *  MATCHEDLIST can be of the class: mrpt::vision::CMatchedFeatureList, or any
   * STL container of pairs of anything having ".x" and ".y" (e.g.
   * mrpt::math::TPoint2D)
   */
  ...
  void showImagesAndMatchedPoints(
    ...
    for (typename MATCHEDLIST::const_iterator i = mList.begin();
      ...
      imgColor.drawCircle(round(i->second->x + w), round(i->second->y), 4,
                          color);
      // imgColor.line( round( i->first->x ), round( i->first->y ), round(
      // i->second->x + w ), round( i->second->y ), color );
      if (showNumbers) {
        char buf[15];
        mrpt::system::os::sprintf(buf, 15, "%d[%u]", nf,
                                  (unsigned int)i->first->ID);
        imgColor.textOut(round(i->first->x) - 10, round(i->first->y), buf,
                         color);
        mrpt::system::os::sprintf(buf, 15, "%d[%u]", nf,
                                  (unsigned int)i->second->ID);
        imgColor.textOut(round(i->second->x + w) + 10, round(i->second->y), buf,
                         color);
      }
    }
    showImage(imgColor);
  ...
  void showImagesAndMatchedPoints(
    ...
    for (typename FEATURELIST::const_iterator iL = leftList.begin(),
      ...
      imgColor.line(round((*iL)->x), round((*iL)->y), round((*iR)->x + w),
                    round((*iR)->y), color);
    }
    showImage(imgColor);
