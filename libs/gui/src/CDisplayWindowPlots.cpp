...
CWindowDialogPlots::CWindowDialogPlots(CDisplayWindowPlots *winPlots,
  ...
  Create(parent, id, _U(caption.c_str()), wxDefaultPosition, initialSize,
  ...
  wxMenuItem *MenuItemPrint =
      new wxMenuItem(Menu1, ID_MENU_PRINT, _("Print..."), _(""), wxITEM_NORMAL);
  ...
  wxMenuItem *MenuItem2 =
      new wxMenuItem(Menu2, ID_MENUITEM2, _("About..."), _(""), wxITEM_NORMAL);
  ...
  Connect(ID_MENU_PRINT, wxEVT_COMMAND_MENU_SELECTED,
  ...
  m_plot->Connect(wxEVT_CHAR,
                  (wxObjectEventFunction)&CWindowDialogPlots::OnChar, 0, this);
  m_plot->Connect(wxEVT_MOTION,
                  (wxObjectEventFunction)&CWindowDialogPlots::OnMouseMove, 0,
                  this);

  m_plot->Connect(wxEVT_LEFT_DOWN,
                  (wxObjectEventFunction)&CWindowDialogPlots::OnMouseDown,
                  nullptr, this);
  m_plot->Connect(wxEVT_RIGHT_DOWN,
                  (wxObjectEventFunction)&CWindowDialogPlots::OnMouseDown,
                  nullptr, this);
  ...
  if (m_winPlots) {
    try {
      m_winPlots->publishEvent(mrptEventWindowResize(
          m_winPlots, event.GetSize().GetWidth(), event.GetSize().GetHeight()));
    } catch (...) {
    }
  }
  ...
  if (m_winPlots) {
    try {
      m_winPlots->publishEvent(mrptEventMouseDown(
          m_winPlots, TPixelCoord(event.GetX(), event.GetY()), event.LeftDown(),
          event.RightDown()));
    } catch (...) {
    }
  }
...
void CWindowDialogPlots::OnMenuSelected(wxCommandEvent &ev) {
  ...
  if (it != m_ID2ID.end()) {
    if (m_winPlots && m_winPlots->m_callback)
      m_winPlots->m_callback(it->second, m_curCursorPos.x, m_curCursorPos.y,
...
void CWindowDialogPlots::plot(const CVectorFloat &x, const CVectorFloat &y,
  ...
  wxPen pen(wxColour(lineColor[0], lineColor[1], lineColor[2]), lineWidth,
            lineStyle);
  theLayer->SetPen(pen);
...
void CWindowDialogPlots::plotEllipse(const CVectorFloat &x,
  ...
  if (x.size() != 3 || y.size() != 3) {
    cerr << "[CWindowDialogPlots::plotEllipse] vectors do not have expected "
            "size!!"
         << endl;
    return;
  }
...
void CWindowDialogPlots::image(void *theWxImage, const float &x0,
                               const float &y0, const float &w, const float &h,
                               const std::string &plotName) {
...
CDisplayWindowPlots::Ptr
CDisplayWindowPlots::Create(const std::string &windowCaption,
...
/*---------------------------------------------------------------
                                        Constructor
 ---------------------------------------------------------------*/
CDisplayWindowPlots::CDisplayWindowPlots(const std::string &windowCaption,
                                         unsigned int initialWidth,
                                         unsigned int initialHeight)
    : CBaseGUIWindow(static_cast<void *>(this), 400, 499, windowCaption),
      m_holdon(false), m_holdon_just_disabled(false), m_holdon_cnt(0),
      m_callback(nullptr), m_callback_param(nullptr) {
...
/*---------------------------------------------------------------
                                        axis
 ---------------------------------------------------------------*/
void CDisplayWindowPlots::axis(float x_min, float x_max, float y_min,
                               float y_max, bool aspectRatioFix) {
#if MRPT_HAS_WXWIDGETS
  if (!isOpen())
    return;
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
template <typename T>
void CDisplayWindowPlots::plotEllipse(const T mean_x, const T mean_y,
...
/*---------------------------------------------------------------
                                        image
 ---------------------------------------------------------------*/
void CDisplayWindowPlots::image(const utils::CImage &img, const float &x_left,
                                const float &y_bottom, const float &x_width,
...
/*---------------------------------------------------------------
                                        internal_plot
 ---------------------------------------------------------------*/
void CDisplayWindowPlots::internal_plot(CVectorFloat &x, CVectorFloat &y,
                                        const std::string &lineFormat,
                                        const std::string &plotName) {
#if MRPT_HAS_WXWIDGETS
  MRPT_START
