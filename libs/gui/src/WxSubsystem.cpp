...
WxSubsystem::CAuxWxSubsystemShutdowner::~CAuxWxSubsystemShutdowner() {
  if (WxSubsystem::isConsoleApp) {
    ...
    try {
      ...
      // std::this_thread::sleep_for(100ms); // JL: I found no better way of
      // doing this, sorry :-(  See WxSubsystem::waitWxShutdownsIfNoWindows()
...
public:
  ...
  CDialogAskUserForCamera()
      : wxDialog(nullptr, wxID_ANY, wxT("Select image source"),
    ...
    wxButton *btnOk = new wxButton(this, ID_BTN_OK, wxT("Ok"),
                                   wxDefaultPosition, wxDefaultSize);
    wxButton *btnCancel = new wxButton(this, ID_BTN_CANCEL, wxT("Cancel"),
                                       wxDefaultPosition, wxDefaultSize);
    ...
    f2->Add(btnCancel, 1, wxALL | wxALIGN_BOTTOM | wxALIGN_CENTER_HORIZONTAL,
            5);

    Connect(ID_BTN_OK, wxEVT_COMMAND_BUTTON_CLICKED,
            (wxObjectEventFunction)&CDialogAskUserForCamera::OnBtnOk);
    Connect(ID_BTN_CANCEL, wxEVT_COMMAND_BUTTON_CLICKED,
            (wxObjectEventFunction)&CDialogAskUserForCamera::OnBtnCancel);

    SetSizer(f1);
    Fit();

    btnOk->SetFocus(); // So the default params can be accepted by just pressing
                       // ENTER.
  }
...
WxSubsystem::CWXMainFrame::CWXMainFrame(wxWindow *parent, wxWindowID id) {
  Create(parent, id, _("MRPT-dummy frame window"), wxDefaultPosition,
         wxSize(1, 1),
  ...
  // processed regularly:
  // ------------------------------------------------------------------------------------------
  Connect(ID_TIMER_WX_PROCESS_REQUESTS, wxEVT_TIMER,
...
#ifdef WXSUBSYSTEM_VERBOSE
    cout << "[WxSubsystem::pushPendingWxRequest] IGNORING request since app "
  ...
  try {
    ...
    while (nullptr != (msg = popPendingWxRequest())) {
      ...
      case 200:
        if (msg->source2D) {
          CWindowDialog *wnd =
              new CWindowDialog(msg->source2D, this, (wxWindowID)-1, msg->str,
                                wxSize(msg->x, msg->y));

          // Set the "m_hwnd" member of the window:
          *((void **)msg->voidPtr) = (void *)wnd;

          // Signal to the constructor (still waiting) that the window is now
          // ready so it can continue:
      ...
      case 201:
        if (msg->source2D) {
          CWindowDialog *wnd =
              (CWindowDialog *)msg->voidPtr; // msg->source2D->getWxObject();
          if (!wnd)
            break;
          ...
          wnd->m_image->AssignImage(
              new wxBitmap(*img)); // Memory will be freed by the object.
          ...
          delete img;
          wnd->m_image->Refresh(
              false); // false: Do NOT erase background: avoid flickering
        }
        break;
      // Set position
      case 202:
      ...
      case 300:
        if (msg->source3D) {
          C3DWindowDialog *wnd =
              new C3DWindowDialog(msg->source3D, this, (wxWindowID)-1, msg->str,
                                  wxSize(msg->x, msg->y));
      ...
      case 350:
        ...
        break;
      // Add a 2D text message: vector_x: [0]:x, [1]:y, [2,3,4]:R G B, "x": enum
      // of desired font. "y": unique index, "str": String.
      case 360:
        if (msg->source3D) {
          C3DWindowDialog *wnd =
              (C3DWindowDialog *)msg->source3D->getWxObject();
          if (wnd) {
            wnd->addTextMessage(
                ...
                mrpt::utils::TColorf(msg->vector_x[2], msg->vector_x[3],
                                     msg->vector_x[4]),
                size_t(msg->y), mrpt::opengl::TOpenGLFont(msg->x));
          }
        }
        break;
        ...
        if (msg->source3D) {
          ...
          if (wnd) {
            wnd->addTextMessage(
                ...
                mrpt::opengl::TOpenGLFontStyle(msg->x), size_t(msg->y),
                msg->vector_x[6], msg->vector_x[7], msg->vector_x[8] != 0,
                mrpt::utils::TColorf(msg->vector_x[9], msg->vector_x[10],
                                     msg->vector_x[11]));
          }
        }
        break;
      ...
      case 400:
        if (msg->sourcePlots) {
          CWindowDialogPlots *wnd =
              new CWindowDialogPlots(msg->sourcePlots, this, (wxWindowID)-1,
                                     msg->str, wxSize(msg->x, msg->y));
      ...
      case 402:
        if (msg->sourcePlots) {
          CWindowDialogPlots *wnd =
              (CWindowDialogPlots *)msg->sourcePlots->getWxObject();
          if (wnd)
            wnd->SetSize(msg->x, msg->y, wxDefaultCoord, wxDefaultCoord);
        }
        break;
        ...
        if (msg->sourcePlots) {
          ...
          if (wnd) {
            if (msg->vector_x.size() == 2 && msg->vector_y.size() == 2) {
              wnd->m_plot->Fit(msg->vector_x[0], msg->vector_x[1],
      ...
      case 421:
        if (msg->sourcePlots) {
          CWindowDialogPlots *wnd =
              (CWindowDialogPlots *)msg->sourcePlots->getWxObject();
          if (wnd)
            wnd->plotEllipse(msg->vector_x, msg->vector_y, msg->str,
      ...
      case 422:
        if (msg->sourcePlots) {
          CWindowDialogPlots *wnd =
              (CWindowDialogPlots *)msg->sourcePlots->getWxObject();
          if (wnd)
            wnd->image(msg->voidPtr2, msg->vector_x[0], msg->vector_x[1],
                       msg->vector_x[2], msg->vector_x[3], msg->plotName);
        }
        break;
      ...
      case 440:
        if (msg->sourcePlots) {
          ...
          if (wnd) {
            ...
            wxMenuItem *mnuTarget =
                new wxMenuItem(popupMnu, MENUITEM_ID, _U(msg->plotName.c_str()),
                               wxEmptyString, wxITEM_NORMAL);
            popupMnu->Insert(0, mnuTarget);

            wnd->Connect(
                MENUITEM_ID, wxEVT_COMMAND_MENU_SELECTED,
                (wxObjectEventFunction)&CWindowDialogPlots::OnMenuSelected);
          }
        }
        break;
      ...
      case 700:
        if (msg->sourceCameraSelectDialog) {
          ...
          std::promise<mrpt::gui::detail::TReturnAskUserOpenCamera> *promise =
              reinterpret_cast<
                  std::promise<mrpt::gui::detail::TReturnAskUserOpenCamera> *>(
                  msg->voidPtr2);
          mrpt::gui::detail::TReturnAskUserOpenCamera ret;

          // Parse selection as a config text block:
          dlg->panel->writeConfigFromVideoSourcePanel("CONFIG",
                                                      &ret.selectedConfig);
      ...
      case 999: {
        ...
        if (WxSubsystem::CWXMainFrame::oneInstance)
          ((WxSubsystem::CWXMainFrame
                *)(WxSubsystem::CWXMainFrame::oneInstance))
              ->Close();
#ifdef WXSUBSYSTEM_VERBOSE
...
// To avoid an error in wx, always resize the icon to the expected size:
  ...
  const wxSize iconsSize(::GetSystemMetrics(SM_CXICON),
...
void WxSubsystem::waitWxShutdownsIfNoWindows() {
  ...
  if (!nOpenWnds && WxSubsystem::isConsoleApp) {
#ifdef WXSUBSYSTEM_VERBOSE
    cout << "[WxSubsystem::waitWxShutdownsIfNoWindows] Waiting for WxWidgets "
            "thread to shutdown...\n";
#endif

    // Then we must be shutting down in the wx thread (we are in the main MRPT
    // application thread)...
    ...
    if (m_done.wait_for(std::chrono::milliseconds(maxTimeout)) ==
      ...
      cerr << "[WxSubsystem::waitWxShutdownsIfNoWindows] Timeout waiting for "
...
bool WxSubsystem::createOneInstanceMainThread() {
  ...
  if (app_con && wxmtd.m_wxMainThreadId.get_id() == std::thread::id()) {
    // We are NOT in a console application: There is already a wxApp instance
    // running and it's not us.
    ...
    if (wxmtd.m_wxMainThreadId.get_id() == std::thread::id()) {
#ifdef WXSUBSYSTEM_VERBOSE
      printf("[WxSubsystem::createOneInstanceMainThread] Launching "
             "wxMainThread() thread...\n");
#endif
      ...
      {
        cerr << "[WxSubsystem::createOneInstanceMainThread] Timeout waiting "
                "wxApplication to start up!"
             << endl;
        return false;
      }
    }
