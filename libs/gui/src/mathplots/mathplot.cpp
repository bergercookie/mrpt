...
void mpWindow::OnPrintMenu(wxCommandEvent &WXUNUSED(event)) {
  ...
  wxPreviewFrame *frame = new wxPreviewFrame(
      preview, nullptr, wxT("Print Plot"), wxPoint(100, 100), wxSize(600, 650));
  frame->Centre(wxBOTH);
  frame->Initialize();
  frame->Show();
}
...
void mpBitmapLayer::SetBitmap(const wxImage &inBmp, double x, double y,
  ...
  if (!inBmp.Ok()) {
    ...
    if (lx < 0) {
      wxLogError(wxT("[mpBitmapLayer::SetBitmap] Assigned lx is negative!!"));
    }
    if (ly < 0) {
      wxLogError(wxT("[mpBitmapLayer::SetBitmap] Assigned ly is negative!!"));
    }
