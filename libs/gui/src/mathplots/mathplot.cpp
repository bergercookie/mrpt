		...
		if (m_pen.GetWidth() <= 1)
		{
			for (wxCoord i = startPx; i < endPx; ++i)
			{
				...
				if (m_drawOutsideMargins || ((iy >= minYpx) && (iy <= maxYpx)))
					dc.DrawPoint(
						i, iy);  // (wxCoord) ((w.GetPosY() - GetY( (double)i /
	...
	if (m_visible)
	{
		...
		else
		{
			for (i = 0; i < w.GetScrY(); ++i)
			{
				...
				//             ((GetX(
				//             (double)i / w.GetScaleY() + w.GetPosY()) -
				//             w.GetPosX()) *
				//             w.GetScaleX());
				//             dc.DrawLine(c, i, c, i);
			}
		...
		if (!m_name.IsEmpty() && m_showName)
		{
			...
			dc.DrawText(
				m_name, w.x2p(GetX(w.p2y(ty))), ty);  // (wxCoord) ((GetX(
			// (double)i /
			// w.GetScaleY() +
			// w.GetPosY()) -
			// w.GetPosX()) *
...
void mpProfile::Plot(wxDC& dc, mpWindow& w)
{
	if (m_visible)
	{
		...
		{
			wxCoord c0 =
				w.y2p(GetY(w.p2x(i)));  // (wxCoord) ((w.GetYpos() - GetY(
			// (double)i / w.GetXscl() +
			// w.GetXpos()) ) * w.GetYscl());
			wxCoord c1 =
...
void mpScaleX::Plot(wxDC& dc, mpWindow& w)
{
	if (m_visible)
	{
		...
		tmp = -65535;
		int labelH =
			0;  // Control labels heigth to decide where to put axis name
