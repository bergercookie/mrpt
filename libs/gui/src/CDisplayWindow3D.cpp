...
public:
  ...
  static void display3D_processKeyEvent(CDisplayWindow3D *m_win3D,
                                        wxKeyEvent &ev);
};
}
}
...
void CMyGLCanvas_DisplayWindow3D::display3D_processKeyEvent(
  ...
  if (m_win3D) {
    if (ev.AltDown() && ev.GetKeyCode() == MRPTK_RETURN) {
      if (mrpt::system::timeDifference(m_win3D->m_lastFullScreen,
                                       mrpt::system::now()) > 0.2) {
      ...
      // Alt+Enter: Don't notify on this key stroke, since if we're switching to
      // fullscreen
      //  and the user is waiting for a key to close the window, a runtime crash
      //  will occur,
      //  so return now:
      return;
    }
  ...
  if (m_win3D) {
    try {
      m_win3D->publishEvent(
          mrptEventMouseDown(m_win3D, TPixelCoord(event.GetX(), event.GetY()),
...
CMyGLCanvas_DisplayWindow3D::~CMyGLCanvas_DisplayWindow3D() {
  m_openGLScene.reset(); // Avoid the base class to free this object (it's freed
                         // by CDisplayWindow3D)
}
...
void CMyGLCanvas_DisplayWindow3D::OnPostRenderSwapBuffers(double At,
                                                          wxPaintDC &dc) {
  if (m_win3D)
  ...
  if (m_win3D && (!grabFile.empty() || m_win3D->isCapturingImgs())) {
    ...
    // Save image directly from OpenGL - It could also use 4 channels and save
    // with GL_BGRA_EXT
...
C3DWindowDialog::C3DWindowDialog(CDisplayWindow3D *win3D,
                                 ...
                                 wxWindowID id, const std::string &caption,
                                 wxSize initialSize)
  ...
  m_canvas = new CMyGLCanvas_DisplayWindow3D(win3D, this, wxID_ANY,
  ...
  Connect(wxID_ANY, wxEVT_CHAR,
          (wxObjectEventFunction)&C3DWindowDialog::OnChar);
  ...
  if (m_win3D) {
    try {
      m_win3D->publishEvent(mrptEventWindowResize(
          m_win3D, event.GetSize().GetWidth(), event.GetSize().GetHeight()));
    } catch (...) {
    }
  }
  event.Skip(); // so it's processed by the wx system!
...
void C3DWindowDialog::addTextMessage(const double x_frac, const double y_frac,
                                     const std::string &text,
                                     const mrpt::utils::TColorf &color,
                                     const size_t unique_index,
                                     const mrpt::opengl::TOpenGLFont font) {
#if MRPT_HAS_OPENGL_GLUT
  m_canvas->m_text_msgs.addTextMessage(x_frac, y_frac, text, color,
                                       unique_index, font);
#endif
}
...
CDisplayWindow3D::CDisplayWindow3D(const std::string &windowCaption,
    ...
    : CBaseGUIWindow(static_cast<void *>(this), 300, 399, windowCaption),
      m_grab_imgs_prefix(), m_grab_imgs_idx(0), m_is_capturing_imgs(false),
      m_lastFullScreen(mrpt::system::now()), m_last_FPS(10) {
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CDisplayWindow3D::getCameraPointingToPoint(float &x, float &y,
                                                float &z) const {
#if MRPT_HAS_WXWIDGETS && MRPT_HAS_OPENGL_GLUT
  const C3DWindowDialog *win = (const C3DWindowDialog *)m_hwnd.get();
  if (win) {
...
/*---------------------------------------------------------------
                                        addTextMessage
 ---------------------------------------------------------------*/
void CDisplayWindow3D::addTextMessage(const double x_frac, const double y_frac,
                                      const std::string &text,
                                      const mrpt::utils::TColorf &color,
                                      const size_t unique_index,
  ...
  if (win) {
    ...
    //  vector_x: [0]:x, [1]:y, [2,3,4]:R G B, "x": enum of desired font. "y":
    //  unique index, "str": String.
    WxSubsystem::TRequestToWxMainThread *REQ =
        new WxSubsystem::TRequestToWxMainThread[1];
    REQ->source3D = this;
    REQ->OPCODE = 360;
