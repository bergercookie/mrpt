...
CWindowDialog::CWindowDialog(CDisplayWindow *win2D,
                             WxSubsystem::CWXMainFrame *parent, wxWindowID id,
  ...
  m_image = new CWindowDialog::wxMRPTImageControl(this, ID_IMAGE_BITMAP, 0, 0,
                                                  10, 10);
  ...
  m_image->Connect(wxID_ANY, wxEVT_KEY_DOWN,
                   (wxObjectEventFunction)&CWindowDialog::OnChar, nullptr,
                   this);
  // m_image->Connect(wxEVT_CHAR,(wxObjectEventFunction)&CWindowDialog::OnChar,nullptr,this);
  m_image->Connect(wxEVT_SIZE, (wxObjectEventFunction)&CWindowDialog::OnResize,
                   nullptr, this);

  m_image->Connect(wxEVT_LEFT_DOWN,
                   (wxObjectEventFunction)&CWindowDialog::OnMouseDown, nullptr,
                   this);
  m_image->Connect(wxEVT_RIGHT_DOWN,
                   (wxObjectEventFunction)&CWindowDialog::OnMouseDown, nullptr,
                   this);
  ...
  if (m_win2D) {
    try {
      m_win2D->publishEvent(mrptEventWindowResize(
          m_win2D, event.GetSize().GetWidth(), event.GetSize().GetHeight()));
    } catch (...) {
    }
  }
  ...
  if (m_win2D) {
    try {
      m_win2D->publishEvent(
          mrptEventMouseDown(m_win2D, TPixelCoord(event.GetX(), event.GetY()),
...
void CWindowDialog::OnMenuSave(wxCommandEvent &event) {
  wxFileDialog dialog(this, wxT("Save image as..."), wxT("."), wxT("image.png"),
...
/*---------------------------------------------------------------
                                        showImageAndPoints
 ---------------------------------------------------------------*/
void CDisplayWindow::showImageAndPoints(const CImage &img,
