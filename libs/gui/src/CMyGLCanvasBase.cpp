...
void CMyGLCanvasBase::OnWindowCreation(wxWindowCreateEvent &ev) {
  if (!m_gl_context)
    m_gl_context.reset(new wxGLContext(this));
}
...
void CMyGLCanvasBase::OnMouseMove(wxMouseEvent &event) {
  ...
  if (event.LeftIsDown()) {
    ...
    OnUserManuallyMovesCamera(cameraPointingX, cameraPointingY, cameraPointingZ,
                              cameraZoomDistance, cameraElevationDeg,
                              cameraAzimuthDeg);
...
CMyGLCanvasBase::CMyGLCanvasBase(wxWindow *parent, wxWindowID id,
                                 const wxPoint &pos, const wxSize &size,
                                 long style, const wxString &name)
    : wxGLCanvas(parent, id, WX_GL_ATTR_LIST, pos, size,
                 style | wxFULL_REPAINT_ON_RESIZE, name),
      m_init(false), m_mouseLastX(0), m_mouseLastY(0) {
  ...
  Connect(wxID_ANY, wxEVT_CHAR_HOOK,
          (wxObjectEventFunction)&CMyGLCanvasBase::OnChar);
...
void CMyGLCanvasBase::Render() {
  ...
  if (!m_gl_context) { /*cerr << "[CMyGLCanvasBase::Render] No GL Context!" <<
                          endl;*/
    return;
  } else
