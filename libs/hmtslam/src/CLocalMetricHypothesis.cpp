...
void CLocalMetricHypothesis::getAs3DScene(
  ...
  // -----------------------------------------------
  {
    ...
    for (TNodeIDSet::const_iterator n = m_neighbors.begin();
      ...
      CPose3D originPose;
      if (node->m_annotations.getElemental(NODE_ANNOTATION_REF_POSEID,
                                           poseID_origin, m_ID)) {
  ...
  for (it = lstPoses.begin(); it != lstPoses.end(); it++) {
    ...
    // Mean:
    ellip->setLocation(pdf.mean.x(), pdf.mean.y(),
                       pdf.mean.z() +
                           0.005); // To be above the ground gridmap...
  ...
  for (it = lstPoses.begin(); it != lstPoses.end(); it++) {
    if (it->first != m_currentRobotPose) {
      ...
      line->setLineCoords(pdf.mean.x(), pdf.mean.y(), pdf.mean.z(), areaPnt.x(),
                          areaPnt.y(), areaPnt.z());
      objs->insert(line);
    }
  } // end for it
     ...
     (itLink=it->second.m_links.begin();itLink!=it->second.m_links.end();itLink++)
                  {
                          ...
                          {
                                  CRobotPosesAuxiliaryGraph::const_iterator
     hisIt = m_robotPoses.find( itLink->first );
                                  ASSERT_( hisIt !=m_robotPoses.end() );

                                  const CPose3DPDFGaussian  *hisPdf = &
     hisIt->second.m_pose;
  ...
  */
  ...
  // ---------------------------------------------------------
  {
    ...
    for (itMeans = areas_mean.begin(); itMeans != areas_mean.end(); itMeans++) {
      ...
      if (itMeans->first ==
          m_nodeIDmemberships.find(m_currentRobotPose)
              ->second) { // Color of current area
        sphere->setColor(0.1, 0.1, 0.7);
      } else { // Color of other areas
        sphere->setColor(0.7, 0, 0);
      }

      sphere->setLocation(itMeans->second.x(), itMeans->second.y(),
                          itMeans->second.z() +
      ...
      txt->setLocation(itMeans->second.x(), itMeans->second.y(),
                       itMeans->second.z() +
                           m_parent->m_options.VIEW3D_AREA_SPHERES_HEIGHT);
    ...
    for (itMeans = areas_mean.begin(); itMeans != areas_mean.end(); itMeans++) {
      ...
      for (TArcList::const_iterator a = lstArcs.begin(); a != lstArcs.end();
        ...
        if ((*a)->getNodeFrom() == srcAreaID) {
          map<CHMHMapNode::TNodeID, CPoint3D>::const_iterator trgAreaPoseIt =
              areas_mean.find((*a)->getNodeTo());
          if (trgAreaPoseIt != areas_mean.end()) {
            ...
            line->setLineCoords(
                areas_mean[srcAreaID].x(), areas_mean[srcAreaID].y(),
                areas_mean[srcAreaID].z() +
                    m_parent->m_options.VIEW3D_AREA_SPHERES_HEIGHT,
                trgAreaPoseIt->second.x(), trgAreaPoseIt->second.y(),
...
void CLocalMetricHypothesis::clearRobotPoses() {
  ...
  for (CParticleList::iterator it = m_particles.begin();
    ...
    it->log_w = 0;
    it->d.reset(
        new CLSLAMParticleData(&m_parent->m_options.defaultMapsInitializers));
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
const CPose3D *
CLocalMetricHypothesis::getCurrentPose(const size_t &particleIdx) const {
...
/** Removes a given area from the LMH:
  ...
  *		- The weights of all particles are changed to remove the effects of
  *the removed metric observations.
...
void CLocalMetricHypothesis::updateAreaFromLMH(
  ...
  TPoseID poseID_origin;
  node->m_annotations.getElemental(NODE_ANNOTATION_REF_POSEID, poseID_origin,
                                   m_ID, true);
  ...
  CRobotPosesGraph::Ptr posesGraph;
  {
    ...
    if (!annot) {
      ...
      node->m_annotations.setMemoryReference(NODE_ANNOTATION_POSES_GRAPH,
                                             posesGraph, m_ID);
    } else {
  ...
  for (TNodeIDList::const_iterator it = lstPoseIDs.begin();
    ...
    if (*it != m_currentRobotPose) // The current robot pose has no SF
    {
      ...
      if (eraseSFsFromLMH) {
        poseInfo.sf.moveFrom(itSF->second); // This leaves m_SFs[*it] without
                                            // observations, but it is being
                                            // erased just now:
        m_SFs.erase(itSF);
      } else {
...
void CLocalMetricHypothesis::dumpAsText(utils::CStringList &st) const {
  ...
  for (TMapPoseID2Pose3D::const_iterator it = lst.begin(); it != lst.end();
    ...
    string s = format("  ID: %i \t AREA: %i \t %.03f,%.03f,%.03fdeg",
                      (int)it->first, (int)area->second, it->second.x(),
                      it->second.y(), RAD2DEG(it->second.yaw()));
    st << s;
  }
}
...
void CLocalMetricHypothesis::readFromStream(mrpt::utils::CStream &in,
  ...
  switch (version) {
  case 0: {
    in >> m_ID >> m_currentRobotPose >> m_neighbors >> m_nodeIDmemberships >>
...
/*---------------------------------------------------------------
        ...
        Implements the writing to a CStream capability of
          CSerializable objects
  ---------------------------------------------------------------*/
void CLSLAMParticleData::writeToStream(mrpt::utils::CStream &out,
                                       int *version) const {
  if (version)
    *version = 0;
  else {
