 ...
 *		  "Towards a Unified Bayesian Approach to Hybrid
 *Metric-Topological SLAM",
 *		    in  IEEE Transactions on Robotics (TRO), Vol. 24, No. 2, April
 *2008.
 *		- ...
 *
...
CHMTSLAM::CHMTSLAM() {
  ...
  // --------------------------------
  registerLoopClosureDetector("gridmaps",
                              &CTopLCDetector_GridMatching::createNewInstance);
  registerLoopClosureDetector("fabmap",
                              &CTopLCDetector_FabMap::createNewInstance);
...
CHMTSLAM::~CHMTSLAM() {
  ...
  if (!m_options.LOG_OUTPUT_DIR.empty()) {
    try {
      ...
      MRPT_LOG_WARN(
          mrpt::format("Ignoring exception at ~CHMTSLAM():\n%s", e.what()));
    } catch (...) {
...
void CHMTSLAM::pushObservation(const CObservation::Ptr &obs) {
  if (m_terminateThreads) { // Discard it:
    // delete obs;
    return;
  }
...
void CHMTSLAM::TOptions::loadFromConfigFile(
  ...
  MRPT_LOAD_CONFIG_VAR_CAST_NO_DEFAULT(SLAM_METHOD, int, TLSlamMethod, source,
                                       section);
...
void CHMTSLAM::initializeEmptyMap() {
  ...
  CHMHMapNode::TNodeID firstAreaID;
  {
    ...
    firstArea->m_annotations.set(NODE_ANNOTATION_METRIC_MAPS, emptyMap,
                                 newHypothID);
    firstArea->m_annotations.setElemental(NODE_ANNOTATION_REF_POSEID,
                                          POSEID_INVALID, newHypothID);
  ...
  default:
    THROW_EXCEPTION_FMT("Invalid selection for LSLAM method: %i",
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
CTopLCDetectorBase *
CHMTSLAM::loopClosureDetector_factory(const std::string &name) {
  MRPT_START
