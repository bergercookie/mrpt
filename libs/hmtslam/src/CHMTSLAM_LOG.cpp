...
void CHMTSLAM::generateLogFiles(unsigned int nIteration) {
  ...
  CLocalMetricHypothesis *bestLMH = nullptr;
  {
    ...
    {
      string filLocalAreas =
          format("%s/LSLAM_3D/mostLikelyLMH_LSLAM_%05u.3Dscene",
...
// Save the SSO matrix:
...
#if 1
  {
    ...
    static int CNT = 0;
    if ((CNT++ % 20) == 0) {
      string hmtmap_file(format("%s/HMTSLAM_state/state_%05u.hmtslam",
                                m_options.LOG_OUTPUT_DIR.c_str(), nIteration));
...
#if 1
  {
    ...
    if ((CNT++ % 5) == 0) {
      ...
      COpenGLScene sceneGlobalHMTMAP;
      {
        ...
        MRPT_LOG_INFO_STREAM("[LOG] HMT-map: " << m_map.nodeCount()
                                               << " nodes/ " << m_map.arcCount()
                                               << " arcs");

        m_map.getAs3DScene(sceneGlobalHMTMAP,             // Scene
      ...
      string hmtmap_file(format("%s/HMAP_3D/mostLikelyHMT_MAP_%05u.3Dscene",
                                m_options.LOG_OUTPUT_DIR.c_str(), nIteration));
