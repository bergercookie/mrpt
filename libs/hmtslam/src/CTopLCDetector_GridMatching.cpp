...
CPose3DPDF::Ptr CTopLCDetector_GridMatching::computeTopologicalObservationModel(
  ...
  const CPose2D initEstimate(0, 0,
                             0); // It's actually ignored by the grid-matching
  ...
  CMultiMetricMap::Ptr hMapRef = refArea->m_annotations.getAs<CMultiMetricMap>(
      NODE_ANNOTATION_METRIC_MAPS, hypID, false);
...
// --------------------
  ...
  if (!m_hmtslam->m_options.LOG_OUTPUT_DIR.empty()) {
    ...
    const std::string filStat =
        dbg_dir + format("/state_%05i_test_%i_%i.hmtslam", cnt,
                         (int)currentArea->getID(), (int)refArea->getID());
    const std::string filRes =
        dbg_dir + format("/state_%05i_test_%i_%i_result.txt", cnt,
                         (int)currentArea->getID(), (int)refArea->getID());

    m_hmtslam->logFmt(mrpt::utils::LVL_DEBUG,
