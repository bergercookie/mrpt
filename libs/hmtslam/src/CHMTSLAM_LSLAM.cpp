  ...
  try {
    ...
    while (!obj->m_terminateThreads) {
      ...
      if (!obj->isInputQueueEmpty()) {
        ...
        else if (nextObject->GetRuntimeClass() == CLASS_ID(CSensoryFrame))
          observations = std::dynamic_pointer_cast<CSensoryFrame>(nextObject);
        else
          THROW_EXCEPTION("Element in the queue is neither CActionCollection "
                          "nor CSensoryFrame!!");
...

        {
          ...
          for (it = obj->m_LMHs.begin(); it != obj->m_LMHs.end(); it++) {
            ...
            // ----------------------------------------------
            obj->m_LSLAM_method->processOneLMH(&it->second, // The LMH
            ...
            if (it->second.m_posesPendingAddPartitioner.size() >
                5) // Option: Do this only one out of N new added poses:
            {
              ...
              TMessageLSLAMfromAA::Ptr msgFromAA = CHMTSLAM::areaAbstraction(
                  &it->second, it->second.m_posesPendingAddPartitioner);

              obj->logFmt(mrpt::utils::LVL_DEBUG, "[AreaAbstraction] Took "
                                                  "%.03fms to insert %u new "
                                                  "poses.               AA\n",
              ...
              //     Create new areas, joining, adding/deleting arcs and nodes,
              //     etc...
              // -----------------------------------------------------------------------
            ...
            if (!it->second.m_areasPendingTBI.empty()) {
              for (TNodeIDList::const_iterator areaID =
                   ...
                   areaID != it->second.m_areasPendingTBI.end(); ++areaID) {
                ...
                TMessageLSLAMfromTBI::Ptr msgFromTBI =
                    CHMTSLAM::TBI_main_method(&it->second, *areaID);
                ...
                // -----------------------------------------------------------------------
                //   Process the set of (potentially) several topological
                //   hypotheses:
                // -----------------------------------------------------------------------
    ...
    if (e.what())
      obj->logFmt(mrpt::utils::LVL_DEBUG, "%s", e.what());
    ...
    obj->logFmt(
        ...
        "\n---------------------- EXCEPTION CAUGHT! ---------------------\n");
    obj->logFmt(mrpt::utils::LVL_DEBUG, " In "
                                        "CHierarchicalMappingFramework::thread_"
                                        "LSLAM. Unexpected runtime error!!\n");
...
void CHMTSLAM::LSLAM_process_message_from_AA(const TMessageLSLAMfromAA &myMsg) {
  ...
  CTicTac tictac;
  tictac.Tic();
  logFmt(mrpt::utils::LVL_INFO, "[LSLAM_proc_msg_AA] Beginning of Msg from AA "
                                "processing...              [\n");
  ...
  // Sanity checks:
  {
    ...
    for (map<TPoseID, CHMHMapNode::TNodeID>::const_iterator itA =
      ...
      if (LMH->m_currentRobotPose !=
          itA->first) // The current pose is not included in the AA method
      {
        bool found = false;
        for (vector<TPoseIDList>::const_iterator it = myMsg.partitions.begin();
        ...
        if (!found)
          THROW_EXCEPTION_FMT("LMH's pose %i not found in AA's partitions.",
                              (int)itA->first);
      }
    }
  }
  ...
  TPoseID poseID_origin;
  {
    ...
    if (!m_map.getNodeByID(itCur->second)
             ->m_annotations.getElemental(NODE_ANNOTATION_REF_POSEID,
                                          poseID_origin, LMH->m_ID))
  ...
  if (0) {
    ...
    utils::CStringList s;
    m_map.dumpAsText(s);
    s.saveToFile(format("%s/HMAP_txt/HMAP_%05i_before.txt",
  ...
  for (i = 0, it = myMsg.partitions.begin(); it != myMsg.partitions.end();
       ++it, i++)
    for (TPoseIDList::const_iterator itPose = it->begin(); itPose != it->end();
         ++itPose) {
  ...
  vector<CHMHMapNode::TNodeID> partIdx2Areas(myMsg.partitions.size(),
                                             AREAID_INVALID);
  ...
  for (size_t k = 0; k < myMsg.partitions.size(); k++) {
    if (votes[k].size() == 1) { // map< unsigned int, map<CHMHMapNode::TNodeID,
                                // unsigned int> >		votes;
                                // recall!
  ...
  for (i = 0; i < partIdx2Areas.size(); i++) {
    if (partIdx2Areas[i] == AREAID_INVALID) {
      ...
      newArea->m_annotations.setMemoryReference(NODE_ANNOTATION_METRIC_MAPS,
                                                emptyMap, LMH->m_ID);

      CRobotPosesGraph::Ptr emptyPoseGraph =
          std::make_shared<CRobotPosesGraph>();
      newArea->m_annotations.setMemoryReference(NODE_ANNOTATION_POSES_GRAPH,
                                                emptyPoseGraph, LMH->m_ID);
  ...
  for (TNodeIDSet::const_iterator pBef = neighbors_before.begin();
    ...
    if (LMH->m_neighbors.find(*pBef) == LMH->m_neighbors.end()) {
      ...
      if (!node) {
        logFmt(mrpt::utils::LVL_INFO, "[LSLAM_proc_msg_AA] Area %i has been "
                                      "removed from the neighbors & no longer "
                                      "exists in the HMAP.\n",
               (int)*pBef);
      } else {
        ...
        // ----------------------------------------------------------------------------
        //  Check if arcs to nodes out of the LMH must be modified coz this
        //  deletion
        // ----------------------------------------------------------------------------
        TArcList arcs;
        node->getArcs(arcs);
        ...
        for (TArcList::const_iterator a = arcs.begin(); a != arcs.end(); ++a) {
          ...
          if ((*a)->getNodeFrom() == *pBef) { // node to delete is: "from"
            nodeB = m_map.getNodeByID((*a)->getNodeTo());
          } else { // node to delete is: "to"
            nodeB = m_map.getNodeByID((*a)->getNodeFrom());
          }

          bool inNeib =
              LMH->m_neighbors.find(nodeB->getID()) != LMH->m_neighbors.end();
          bool inBefNeib =
              neighbors_before.find(nodeB->getID()) != neighbors_before.end();
        ...
        for (TArcList::const_iterator a = arcs.begin(); a != arcs.end(); ++a) {
          ...
          if (arc->getNodeFrom() == *pBef) { // node to delete is: "from"
            nodeB = m_map.getNodeByID((*a)->getNodeTo());
            dirA2B = true;
          } else { // node to delete is: "to"
            nodeB = m_map.getNodeByID((*a)->getNodeFrom());
          ...
          } else // The target was into the LMH, but not anymore.
          {
            ...
            for (TListNodesArcs::iterator na = lstWithinLMH.begin();
              ...
              // Now we have the arc "arc" from "node"<->"nodeB" in the
              // direction "dirA2B", which will be deleted next.
              // The arc "a<->c", being "node_c" a node within the LMH, is in
              // "arc_c_a".
              //   node_b -> outside LMH
              //   node_c -> within LMH
              // Then:
              ...
              TPoseID refPoseAt_b;
              {
                ...
                if (!dirA2B) {
                  ...
                  arc->m_annotations.getElemental(
                      ARC_ANNOTATION_DELTA_SRC_POSEID, refPoseAt_b, LMH->m_ID,
                      true);
                  arc->m_annotations.getElemental(
                      ARC_ANNOTATION_DELTA_TRG_POSEID, refPoseAt_a, LMH->m_ID,
                      true);
                } else {
                  ...
                  // Check valid reference poseIDs:
                  arc->m_annotations.getElemental(
                      ARC_ANNOTATION_DELTA_TRG_POSEID, refPoseAt_b, LMH->m_ID,
                      true);
                  arc->m_annotations.getElemental(
                      ARC_ANNOTATION_DELTA_SRC_POSEID, refPoseAt_a, LMH->m_ID,
                      true);
                }

                TPoseID node_refPoseAt_b;
                nodeB->m_annotations.getElemental(NODE_ANNOTATION_REF_POSEID,
                                                  node_refPoseAt_b, LMH->m_ID,
                                                  true);
                ASSERT_(node_refPoseAt_b == refPoseAt_b);

                TPoseID node_refPoseAt_a;
                node->m_annotations.getElemental(NODE_ANNOTATION_REF_POSEID,
                                                 node_refPoseAt_a, LMH->m_ID,
                                                 true);
              ...
              {
                CPose3DPDFGaussian::Ptr pdf =
                    arc_c_a->m_annotations.getAs<CPose3DPDFGaussian>(
                        ARC_ANNOTATION_DELTA, LMH->m_ID, false);
                TPoseID refPoseAt_a;
                if (arc_c_a->getNodeTo() == node_c->getID()) {
                  ...
                  arc_c_a->m_annotations.getElemental(
                      ...
                      true);
                  arc_c_a->m_annotations.getElemental(
                      ARC_ANNOTATION_DELTA_TRG_POSEID, refPoseAt_c, LMH->m_ID,
                      true);
                } else {
                  ...
                  arc_c_a->m_annotations.getElemental(
                      ...
                      true);
                  arc_c_a->m_annotations.getElemental(
                      ARC_ANNOTATION_DELTA_SRC_POSEID, refPoseAt_c, LMH->m_ID,
                      true);
                }

                TPoseID node_refPoseAt_c;
                node_c->m_annotations.getElemental(NODE_ANNOTATION_REF_POSEID,
                                                   node_refPoseAt_c, LMH->m_ID,
                                                   true);
              ...
              MRPT_LOG_DEBUG_STREAM("b_a: " << Delta_b_a.mean << endl
              ...
              // ------------------------------------------------
              // Did an arc already exist? Look into existing arcs, in both
              // directions:
              ...
              if (!newArc) {
                // Create a new one:
                newArc = std::make_shared<CHMHMapArc>(nodeB,     // Source
                                                      node_c,    // Target
                                                      LMH->m_ID, // Hypos
                                                      &m_map     // The graph
                                                      );
              ...
              if (!arcDeltaIsInverted) { // arc: b->c
                newArc->m_annotations.set(
                    ARC_ANNOTATION_DELTA,
                    CPose3DPDFGaussian::Ptr(new CPose3DPDFGaussian(Delta_b_c)),
                    LMH->m_ID);
                MRPT_LOG_DEBUG_STREAM(
                    "[LSLAM_proc_msg_AA] Setting arc "
                    << nodeB->getID() << " -> " << node_c->getID() << " : "
                    << Delta_b_c.mean
                    << " cov = " << Delta_b_c.cov.inMatlabFormat() << endl);
                ...
                MRPT_LOG_DEBUG_STREAM(
                    ...
                    << nodeB->getID() << " <- " << node_c->getID() << " : "
                    << Delta_b_c_inv.mean
                    << " cov = " << Delta_b_c_inv.cov.inMatlabFormat() << endl);
                newArc->m_annotations.set(
                    ARC_ANNOTATION_DELTA,
  ...
  if (curAreaID != oldAreaID) {
    ...
    if (++cntAddTBI > 4) {
      cntAddTBI = 0;
      LMH->m_areasPendingTBI.insert(curAreaID); // Add to TBI list
      logFmt(mrpt::utils::LVL_INFO, "[LSLAM_proc_msg_AA] Current area %i "
                                    "enqued for TBI (routine check).\n",
             (int)curAreaID);
    }
  }
  ...
  //   - Update the arcs' deltas between all the pairs of connected areas within
  //   the LMH
  // ---------------------------------------------------------------------------------------
  // List of all LMH's internal arcs to create or update:
  //  Each entry is a pair of areas to create an arc between, and the "->second"
  //  is
  ...
  if (partIdx2Areas.size() > 1) {
    ...
    for (size_t idx_area_a = 0; idx_area_a < partIdx2Areas.size();
      ...
      if (!area_a->m_annotations.getElemental(NODE_ANNOTATION_REF_POSEID,
                                              poseID_trg, LMH->m_ID)) {
        // No reference pose annotation yet: add it now:
        poseID_trg = myMsg.partitions[idx_area_a][0];

        area_a->m_annotations.setElemental(NODE_ANNOTATION_REF_POSEID,
                                           poseID_trg, LMH->m_ID);
        logFmt(mrpt::utils::LVL_INFO, "[LSLAM_proc_msg_AA] Changing reference "
                                      "poseID of area '%i' to pose '%i'\n",
               (int)area_a_ID, (int)poseID_trg);

        // Reconsider the arcs of this area again, since the ref. poseID has
        // changed:
        ...
        if (!found) {
          ...
          logFmt(mrpt::utils::LVL_INFO, "[LSLAM_proc_msg_AA] Changing "
                                        "reference poseID of area '%i' to pose "
                                        "'%i'\n",
                 (int)area_a_ID, (int)poseID_trg);

          // ------------------------------------------------------------------------
          // Look for existing arcs from "area_a"<->other areas outside the LMH
          // to
          //  fix the "Delta" annotations due to the change in reference poseID
          //  from
          ...
          for (TArcList::const_iterator a = arcs.begin(); a != arcs.end();
            ...
            // --------------------------------------------------------------------------------------------
            // Ok... we are here updating an existing arc "nodeFrom"->"nodeTo",
            // with only one of the
            //  extremes being within the LMH, to account for a change in the
            //  reference pose of the area
            //  within the LMH.
            ...
            if (nodeFrom == area_a_ID) {
              ...
              if (LMH->m_neighbors.find(nodeTo) ==
                  LMH->m_neighbors.end()) { // nodeTo is outside the LMH:
                // The source area is into the LMH.
                CPose3DPDFParticles Anew_old_parts;
                LMH->getRelativePose(poseID_trg, poseID_trg_old,
                                     Anew_old_parts);
                ...
                CPose3DPDFGaussian::Ptr oldDelta =
                    theArc->m_annotations.getAs<CPose3DPDFGaussian>(
                        ARC_ANNOTATION_DELTA, LMH->m_ID, false);
                ...
                MRPT_LOG_DEBUG_STREAM("[LSLAM_proc_msg_AA] Updating arc "
                                      << nodeFrom << " -> " << nodeTo
                                      << " OLD: " << oldDelta->mean << " cov = "
                                      << oldDelta->cov.inMatlabFormat()
                                      << endl);
                MRPT_LOG_DEBUG_STREAM("[LSLAM_proc_msg_AA] Updating arc "
                                      << nodeFrom << " -> " << nodeTo
                ...
                theArc->m_annotations.set(
                    ARC_ANNOTATION_DELTA,
                    CPose3DPDFGaussian::Ptr(new CPose3DPDFGaussian(newDelta)),
                    LMH->m_ID);
                theArc->m_annotations.setElemental(
                    ARC_ANNOTATION_DELTA_SRC_POSEID, poseID_trg, LMH->m_ID);
              }
            } else {
              // Is nodeFrom out of the LMH?
              if (LMH->m_neighbors.find(nodeFrom) ==
                  LMH->m_neighbors.end()) { // nodeFrom is outside the LMH:
                // The target area is into the LMH:
                CPose3DPDFParticles Aold_new_parts;
                LMH->getRelativePose(poseID_trg_old, poseID_trg,
                                     Aold_new_parts);
                ...
                theArc->m_annotations.set(
                    ...
                    LMH->m_ID);
                theArc->m_annotations.setElemental(
                    ARC_ANNOTATION_DELTA_TRG_POSEID, poseID_trg, LMH->m_ID);
              }
            }
      ...
      // Now, go thru all other areas to check whether they are neighbors of
      // "area_a":
      for (size_t idx_area_b = 0; idx_area_b < myMsg.partitions.size();
        ...
        // Get the "trg" pose at "area_a": Sweep over all the poses in the
        // "area_a", to find the closests poses to other clusters:
        ...
        if (closestDistPoseSrc < 5 * m_options.SLAM_MIN_DIST_BETWEEN_OBS) {
          ...
          if (!area_b->m_annotations.getElemental(NODE_ANNOTATION_REF_POSEID,
                                                  poseID_src, LMH->m_ID)) {
            // Add 'poseID_closests': this should happen when the closest area
            // is a new one:
            area_b->m_annotations.setElemental(NODE_ANNOTATION_REF_POSEID,
                                               poseID_closests, LMH->m_ID);
            poseID_src = poseID_closests;
            logFmt(mrpt::utils::LVL_INFO, "[LSLAM_proc_msg_AA] Changing "
                                          "reference poseID of area '%i' to "
                                          "pose '%i' (creat. annot)\n",
    ...
    // ------------------------------------------------------------------------------------------------------
    // If two areas are neighbors but above the distance threshold, no link will
    // be created between them:
    //  Check this situation by looking for doubtful neighbors for areas without
    //  any link:
    // ------------------------------------------------------------------------------------------------------
      ...
      if (areasWithLink.find(area_ID) == areasWithLink.end()) {
        ...
        if (lstClosestDoubtfulNeigbors.find(area_ID) !=
          ...
          lstInternalArcsToCreate.insert(
              TPairNodeIDs(area_ID, lstClosestDoubtfulNeigbors[area_ID].first));
  ...
  // -------------------------------------------------------------------------------------
  {
    ...
    for (list_searchable<TPairNodeIDs>::const_iterator arcCreat =
      ...
      logFmt(mrpt::utils::LVL_INFO, "[LSLAM_proc_msg_AA] Creating arc "
                                    "%i[ref:%i] -> %i[ref:%i] = "
                                    "(%.03f,%.03f,%.03fdeg)\n",
      ...
      bool arcDeltaIsInverted;
      CHMHMapArc::Ptr newArc = m_map.findArcOfTypeBetweenNodes(
          area_a_ID, area_b_ID, LMH->m_ID, "RelativePose", arcDeltaIsInverted);

      // If not found, create it now:
      if (!newArc) {
        newArc = std::make_shared<CHMHMapArc>(area_a_ID,   // Source
                                              area_b_ID,   // Target
                                              theArcHypos, // Hypos
                                              &m_map       // The graph
                                              );
      ...
      if (!arcDeltaIsInverted) {
        MRPT_LOG_DEBUG_STREAM("[LSLAM_proc_msg_AA] Updating int. arc "
                              << area_a_ID << " -> " << area_b_ID << " : "
                              << relPoseGauss.mean << " cov = "
                              << relPoseGauss.cov.inMatlabFormat() << endl);
        newArc->m_annotations.set(
            ARC_ANNOTATION_DELTA,
            CPose3DPDFGaussian::Ptr(new CPose3DPDFGaussian(relPoseGauss)),
            LMH->m_ID);
        newArc->m_annotations.setElemental(ARC_ANNOTATION_DELTA_SRC_POSEID,
                                           area_a_poseID_src, LMH->m_ID);
        newArc->m_annotations.setElemental(ARC_ANNOTATION_DELTA_TRG_POSEID,
                                           area_b_poseID_trg, LMH->m_ID);
      } else {
        ...
        MRPT_LOG_DEBUG_STREAM("[LSLAM_proc_msg_AA] Updating int. arc "
                              << area_a_ID << " <- " << area_b_ID << " : "
                              << relPoseInv.mean << " cov = "
                              << relPoseInv.cov.inMatlabFormat() << endl);
        newArc->m_annotations.set(
            ARC_ANNOTATION_DELTA,
  ...
  // ----------------------------------------------------------------
  {
    ...
    for (TNodeIDSet::const_iterator pNei = LMH->m_neighbors.begin();
      ...
      //   "lstAlreadyUpdated"
      for (TArcList::const_iterator a = lstArcs.begin(); a != lstArcs.end();
           ++a) {
        const CHMHMapNode::TNodeID nodeToID = (*a)->getNodeFrom() == nodeFromID
                                                  ? (*a)->getNodeTo()
                                                  : (*a)->getNodeFrom();
  ...
  if (0) {
    ...
    utils::CStringList s;
    m_map.dumpAsText(s);
    s.saveToFile(format("%s/HMAP_txt/HMAP_%05i_mid.txt",
                        m_options.LOG_OUTPUT_DIR.c_str(), DEBUG_STEP));
    logFmt(mrpt::utils::LVL_INFO,
  ...
  for (TNodeIDSet::iterator pNei1 = LMH->m_neighbors.begin();
    ...
    if (*pNei1 != curAreaID) {
      ...
      {
        std::lock_guard<std::mutex> lock(m_map_cs);
        m_map.findArcsOfTypeBetweenNodes(*pNei1, curAreaID, LMH->m_ID,
  ...
  // have been added
  //  to the particles, etc... but not their observations into the metric maps:
  //  this is delayed
  //  since in the case we would need to change coordinate origin, it would had
  //  been pointless.
  ...
  CHMHMapNode::Ptr currentArea;
  {
    ...
    currentArea->m_annotations.getElemental(NODE_ANNOTATION_REF_POSEID,
                                            refPoseCurArea_accordingAnnot,
                                            LMH->m_ID, true);

    TArcList arcsToCurArea;
    currentArea->getArcs(arcsToCurArea, "RelativePose", LMH->m_ID);
    for (TArcList::iterator a = arcsToCurArea.begin(); a != arcsToCurArea.end();
         ++a) {
      const CHMHMapArc::Ptr arc = (*a);
      const CHMHMapNode::TNodeID otherAreaID = arc->getNodeFrom() == curAreaID
                                                   ? arc->getNodeTo()
                                                   : arc->getNodeFrom();
      ...
      if (LMH->m_neighbors.find(otherAreaID) == LMH->m_neighbors.end()) {
        ...
        CRobotPosesGraph::Ptr pg = area->m_annotations.getAs<CRobotPosesGraph>(
        ...
        {
          CPose3DPDFGaussian::Ptr pdf =
              arc->m_annotations.getAs<CPose3DPDFGaussian>(ARC_ANNOTATION_DELTA,
                                                           LMH->m_ID, false);
        ...
        if (arc->getNodeTo() == curAreaID) {
          ...
          arc->m_annotations.getElemental(ARC_ANNOTATION_DELTA_SRC_POSEID,
                                          refPoseIDAtOtherArea, LMH->m_ID,
                                          true);
          arc->m_annotations.getElemental(ARC_ANNOTATION_DELTA_TRG_POSEID,
                                          refPoseIDAtCurArea, LMH->m_ID, true);
        } else {
          // It is NOT inverted.
          arc->m_annotations.getElemental(ARC_ANNOTATION_DELTA_TRG_POSEID,
                                          refPoseIDAtOtherArea, LMH->m_ID,
        ...
        logFmt(mrpt::utils::LVL_INFO, "[LSLAM_proc_msg_AA] Bringing in: "
                                      "refPoseCur=%i refPoseOther=%i -> "
                                      "Delta_c2a:(%.03f,%.03f,%.03fdeg)\n",
...
// Assure the arc's references are OK:
#if defined(_DEBUG) || (MRPT_ALWAYS_CHECKS_DEBUG)
        {
          ...
          area->m_annotations.getElemental(NODE_ANNOTATION_REF_POSEID,
        ...
        for (CRobotPosesGraph::iterator p = pg->begin(); p != pg->end(); ++p) {
          ...
          for (itSrc = poseInfo.pdf.m_particles.begin(),
            ...
            itTrg->d->robotPoses[poseID] =
                itTrg->d->robotPoses[refPoseIDAtCurArea] + Delta_c2a +
                *itSrc->d;
          }
        ...
        // Update m_robotPosesGraph: This will be done in the next iteration of
        // the LSLAM thread,
        //  now just add to the list of pending pose IDs:
        LMH->m_posesPendingAddPartitioner.insert(
            LMH->m_posesPendingAddPartitioner.end(), lstNewPoseIDs.begin(),
            lstNewPoseIDs.end());

        // Mark this new area as to pending for updating the metric map at the
        // end of this method:
  ...
  if (0) {
    utils::CStringList s;
    LMH->dumpAsText(s);
    s.saveToFile(format("%s/HMAP_txt/HMAP_%05i_LMH_mid.txt",
                        m_options.LOG_OUTPUT_DIR.c_str(), DEBUG_STEP));
    logFmt(mrpt::utils::LVL_INFO,
  ...
  if (0) {
    ...
    string filLocalAreas = format("%s/HMAP_txt/HMAP_%05i_LMH_mid.3Dscene",
                                  m_options.LOG_OUTPUT_DIR.c_str(), DEBUG_STEP);
  ...
  if (!currentArea->m_annotations.getElemental(NODE_ANNOTATION_REF_POSEID,
                                               new_poseID_origin, LMH->m_ID))
    THROW_EXCEPTION("New coordinate origin not found");

  if (new_poseID_origin !=
      poseID_origin) { // Change coords AND rebuild metric maps
    CTicTac tictac;
    tictac.Tic();
    ...
    for (TNodeIDSet::iterator areaID = areasDelayedMetricMapsInsertion.begin();
  ...
  if (0) {
    ...
    utils::CStringList s;
    m_map.dumpAsText(s);
    s.saveToFile(format("%s/HMAP_txt/HMAP_%05i_after.txt",
                        m_options.LOG_OUTPUT_DIR.c_str(), DEBUG_STEP));
    logFmt(mrpt::utils::LVL_INFO,
  ...
  if (0) {
    utils::CStringList s;
    LMH->dumpAsText(s);
    s.saveToFile(format("%s/HMAP_txt/HMAP_%05i_LMH_after.txt",
                        m_options.LOG_OUTPUT_DIR.c_str(), DEBUG_STEP));
    logFmt(mrpt::utils::LVL_INFO,
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CHMTSLAM::LSLAM_process_message_from_TBI(
    const TMessageLSLAMfromTBI &myMsg) {
  MRPT_START

  CTicTac tictac;
  tictac.Tic();
  logFmt(mrpt::utils::LVL_INFO, "[LSLAM_proc_msg_TBI] Beginning of Msg from "
                                "TBI processing...              [\n");

  // In case of multiple areas involved in a TLC, we need a mapping from the old
  // areaIDs to the new ones:
  std::map<CHMHMapNode::TNodeID, CHMHMapNode::TNodeID> alreadyClosedLoops;

  for (map<CHMHMapNode::TNodeID, TMessageLSLAMfromTBI::TBI_info>::const_iterator
           candidate = myMsg.loopClosureData.begin();
    ...
    if (alreadyClosedLoops.find(myMsg.cur_area) != alreadyClosedLoops.end()) {
      ...
      cout << "[LSLAM_proc_msg_TBI] Using " << myMsg.cur_area << " -> "
           << currentArea << "  due to area being already merged." << endl;
    }
    ...
    m_map.computeCoordinatesTransformationBetweenNodes(
        currentArea, // Pose of "candidate->first" as seen from "currentArea"
    ...
    // Increase the uncertainty to avoid too understimated covariances and make
    // the chi-test fail:
    ...
    map<double, CPose3DPDFGaussian>
        lstModesAndCompats; // first=log(e^-0.5*maha_dist)+log(likelihood); The
                            // list only contains those chi2 compatible
    ...
    if (!lstModesAndCompats.empty()) {
      ...
      // --------------------------------------------------------
      static CTicTac tictac;
      logFmt(mrpt::utils::LVL_INFO, "[LSLAM_proc_msg_TBI] Accepting TLC of "
                                    "areas: %u <-> %u  with an overall "
                                    "log(lik)=%f  \n",
             (unsigned)currentArea, (unsigned)candidate->first,
             lstModesAndCompats.rbegin()->first);

      tictac.Tic();
      this->perform_TLC(m_LMHs[myMsg.hypothesisID],
