...
void CHMTSLAM::perform_TLC(
  ...
  //	{
  //		map<TPoseID,CPose3D>::iterator itPos =
  //it->d->robotPoses.find(*n);
  ...
  // ----------------------------------------------------------------------------------------------
  {
    ...
    for (TArcList::iterator arc = lstArcs.begin(); arc != lstArcs.end(); ++arc)
      arc->reset(); // The "delete" will automatically remove the entry in
                    // "m_map". Other smrtpnts will be cleared too.

    m_map.getNodeByID(Ai)
        .reset(); // The "delete" will automatically remove the entry in "m_map"
  }
