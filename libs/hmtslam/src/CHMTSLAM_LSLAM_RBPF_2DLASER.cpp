...
void CLSLAM_RBPF_2DLASER::processOneLMH(CLocalMetricHypothesis *LMH,
  ...
  if (currentPoseID == POSEID_INVALID) {
    ...
    firstArea->m_annotations.setElemental(NODE_ANNOTATION_REF_POSEID,
                                          currentPoseID, LMH->m_ID);
    m_parent->m_map_cs.unlock();
  }

  bool insertNewRobotPose = false;
  if (sf) {
    if (LMH->m_nodeIDmemberships.size() < 2) // If there is just 1 node (the
                                             // current robot pose), then there
                                             // is no observation in the map
                                             // yet!
    { // Update map if this is the first observation!
      ...
      for (TMapPoseID2Pose3D::iterator it = lstRobotPoses.begin();
        ...
        if (it->first != currentPoseID) {
          ...
          float angDist =
              fabs(math::wrapToPi(it->second.yaw() - currentRobotPose->yaw()));
  ...
  if (insertNewRobotPose) {
    m_parent->logStr(mrpt::utils::LVL_INFO,
    ...
    m_parent->logFmt(mrpt::utils::LVL_INFO,
...
void CLSLAM_RBPF_2DLASER::prediction_and_update_pfAuxiliaryPFOptimal(
  ...
  //	static bool						accumRobotMovementIsValid =
  //false;
  ...
  if (actions != nullptr) {
    ...
    if (!act)
      THROW_EXCEPTION("Action list does not contain any CActionRobotMovement2D "
                      "derived object!");
  ...
  keep_max(LMH->m_accumRobotMovement.motionModelConfiguration.gaussianModel
               .minStdPHI,
  ...
  // max(2000,(int)(PF_options.pfAuxFilterOptimal_MaximumSearchSamples * 5.6574)
  // ) );
  ...
  CTicTac tictac;
  tictac.Tic();
  LMH->prepareFastDrawSample(PF_options, particlesEvaluator_AuxPFOptimal,
                             robotMovement, sf);
  ...
  // LMH->m_pfAuxiliaryPFOptimal_estimatedProb ); // For normalization purposes
  // only
    ...
    if (LMH->m_SFs.empty()) {
      ...
      do {
        ...
        if (!maxLikMovementDrawHasBeenUsed[k]) {
          ...
          movementDraw = LMH->m_movementDraws[LMH->m_movementDrawsIdx++ %
                                              size_movementDraws];
        }

        newPose.composeFrom(oldPose,
        ...
        statsTrialsCount++; // Stats
               ...
               randomGenerator.drawUniform(MIN_ACCEPT_UNIF_DISTRIB, 0.999f));
  ...
  // Now add the new robot pose to the paths: (this MUST be done after the above
  // loop, separately):
  for (newPartIt = newParticlesArray.begin(), i = 0;
       newPartIt != newParticlesArray.end(); newPartIt++, i++)
...
void CLSLAM_RBPF_2DLASER::loadTPathBinFromPath(
  ...
  if (path != nullptr) {
    // Fill the bin data:
    for (itSrc = path->begin(), itX = outBin.x.begin(), itY = outBin.y.begin(),
        itPHI = outBin.phi.begin();
         itSrc != path->end(); itSrc++, itX++, itY++, itPHI++) {
      *itX = (int)round(itSrc->second.x() /
                        m_parent->m_options.KLD_params.KLD_binSize_XY);
      *itY = (int)round(itSrc->second.y() /
                        m_parent->m_options.KLD_params.KLD_binSize_XY);
      *itPHI = (int)round(itSrc->second.yaw() /
  ...
  for (size_t i = 0; i < M; i++) {
    ...
    if (LMH->m_SFs.empty()) {
      ...
      CPosePDF::Ptr alignEst = icp.Align(
          mapalign, &localMapPoints, initialPoseEstimation, nullptr, &icpInfo);
