...
void CHierarchicalMHMap::clear() {
  ...
  std::for_each(nodes.begin(), nodes.end(),
                metaprogramming::ObjectClearSecond());
  ...
  case 0: {
    ...
    for (i = 0; i < n; i++) {
      CHMHMapNode::Ptr node =
          std::make_shared<CHMHMapNode>(this); // This insert the node in my
                                               // internal list via the callback
                                               // method
      in >> *node;
    }
    ...
    for (i = 0; i < n; i++) {
      ...
      CHMHMapArc::Ptr arc =
          std::make_shared<CHMHMapArc>(p1, p2, THypothesisIDSet(), this);
      in >> *arc;
    }
  ...
  for (size_t j = 0; j < numannot; j++) {
    ...
    if (nodeanotmapit != nodeanotmap.end()) {
      if (type == "placePose") {
        ...
        node->m_annotations.set(NODE_ANNOTATION_PLACE_POSE, o,
                                COMMON_TOPOLOG_HYP);
      }
    }
  }
...
void CHierarchicalMHMap::dumpAsXMLfile(std::string fileName) const {
  ...
  for (TNodeList::const_iterator it = m_nodes.begin(); it != m_nodes.end();
    ...
    tablenodes->set(i, "id",
                    format("%i", static_cast<int>(it->second->getID())));
  ...
  /*
     ...
     (int)it->second->getID(), it->second->m_label.c_str() );
                  ...
                  for (TArcList::const_iterator
     a=arcs.begin();a!=arcs.end();++a)
     ...
     (int)(*a)->getNodeTo() );
                  ...
                  for (CMHPropertiesValuesList::const_iterator ann =
     it->second->m_annotations.begin(); ann != it->second->m_annotations.end();
     ++ann)
                  {
     ...
     (*it)->m_annotations.get(ARC_ANNOTATION_DELTA, ann->ID );
                                  ...
                                  st << format("     VALUE: (%f,%f,%f ,
     %fdeg,%fdeg,%fdeg)",
                                          relativePoseAcordToArc.mean.x(),
                                          relativePoseAcordToArc.mean.y(),
                                          relativePoseAcordToArc.mean.z(),
                                          RAD2DEG(
     relativePoseAcordToArc.mean.yaw() ),
                                          RAD2DEG(
     relativePoseAcordToArc.mean.pitch() ),
                                          RAD2DEG(
     relativePoseAcordToArc.mean.roll() ) );
                          }
                  }
