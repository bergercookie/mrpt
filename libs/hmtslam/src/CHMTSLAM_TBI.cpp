...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
CHMTSLAM::TMessageLSLAMfromTBI::Ptr
CHMTSLAM::TBI_main_method(CLocalMetricHypothesis *LMH,
  ...
  // -------------------------------------------------------
  // But first: if the areas are within the LMH, then we have to update the maps
  // in the HMAP!
  ...
  for (CHierarchicalMapMHPartition::iterator a = obj->m_map.begin();
    ...
    if (a->second->m_hypotheses.has(LMH_ID)) {
      ...
      // Compute it:
      double match =
          obj->m_map.computeOverlapProbabilityBetweenNodes(areaID,   // From
                                                           a->first, // To
                                                           LMH_ID);
  ...
  std::set<CHMHMapNode::TNodeID> lstNodesToErase;
  {
    ...
    for (deque<CTopLCDetectorBase *>::const_iterator it =
      ...
      for (map<CHMHMapNode::TNodeID, TMessageLSLAMfromTBI::TBI_info>::iterator
               candidate = msg->loopClosureData.begin();
           candidate != msg->loopClosureData.end(); ++candidate) {
        // If the current log_lik of this area is reaaaally low, we could skip
        // the computation with other LC detectors...
        // ----------------------------------------------------------------------------------------------------------------
        // TODO: ...
        ...
        // This is because not all LC detector MUST return a pose PDF (i.e.
