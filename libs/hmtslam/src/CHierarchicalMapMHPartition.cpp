	...
	const THypothesisID& hypothesisID) const
{
	...
	/*
	   ...
	   The
	   mean
	   pose of the observations in the area
	...
	const unsigned int& numberOfIterations) const
{
		 ...
		 it_node != pose_graph.nodes.end(); ++it_node)
	{
		...
		new_pose.cov
			.setIdentity();  // *** At present, graphslam does not output
		// the uncertainty of poses... ***
	}
