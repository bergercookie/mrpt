...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
CHMHMapNode::Ptr
CHierarchicalMapMHPartition::getNodeByID(CHMHMapNode::TNodeID id) {
  MRPT_START
  if (id == AREAID_INVALID)
...
CHierarchicalMapMHPartition::getNodeByID(CHMHMapNode::TNodeID id) const {
  MRPT_START
  if (id == AREAID_INVALID)
    return CHMHMapNode::Ptr();
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
CHMHMapNode::Ptr
CHierarchicalMapMHPartition::getNodeByLabel(const std::string &label,
...
void CHierarchicalMapMHPartition::saveAreasDiagramForMATLAB(
  ...
  /*
     ...
     // The ref. pose of each area
          std::map<CHMHMapNode::TNodeID,CPose2D>
     ...
     2006\n");
          ...
          for (it = nodesMeanPoses.begin();it!=nodesMeanPoses.end();it++)
          {
                  ...
                  os::fprintf(f,"\n%% Node: %s\n", node->label.c_str() );
                  os::fprintf(f,"rectangle('Curvature',[1 1],'Position',[%f %f
     %f %f],'FaceColor',[1 1 1]);\n",
                                                  pose.x - nodesRadius,
                                                  pose.y - nodesRadius,
                                                  2*nodesRadius,
...
void CHierarchicalMapMHPartition::saveAreasDiagramWithEllipsedForMATLAB(
  ...
  /*	MRPT_START
     ...
     it->second.mean;
                  ...
                  const CHMHMapNode			*node = getNodeByID(
     it->first );
  ...
  for (arcsIt = arcsPath.begin(), lstIt = listSamples.begin();
    ...
    getNodeByID((*arcsIt)->getNodeFrom())
        ->m_annotations.getElemental(NODE_ANNOTATION_REF_POSEID,
                                     curNodeRefPoseID, hypothesisID, true);
    getNodeByID((*arcsIt)->getNodeTo())
        ->m_annotations.getElemental(NODE_ANNOTATION_REF_POSEID,
                                     nextNodeRefPoseID, hypothesisID, true);

    TPoseID srcRefPoseID, trgRefPoseID;
    (*arcsIt)->m_annotations.getElemental(ARC_ANNOTATION_DELTA_SRC_POSEID,
                                          srcRefPoseID, hypothesisID, true);
    (*arcsIt)->m_annotations.getElemental(ARC_ANNOTATION_DELTA_TRG_POSEID,
                                          trgRefPoseID, hypothesisID, true);
    ...
    for (poseIt = lstIt->begin(), samplIt = samples.begin();
      ...
      // Minimum added noise:
      poseIt->setFromValues((*samplIt)[0] +
...
void CHierarchicalMapMHPartition::findArcsBetweenNodes(
  ...
  for (itArc = lstArcs.begin(); itArc != lstArcs.end(); itArc++) {
    if ((*itArc)->m_hypotheses.has(hypothesisID))
      if ((*itArc)->m_nodeFrom == node2id || (*itArc)->m_nodeTo == node2id) {
        ret.push_back(*itArc);
      }
  }
    ...
    if ((*itArc)->m_hypotheses.has(hypothesisID))
      if ((*itArc)->m_nodeFrom == node2id || (*itArc)->m_nodeTo == node2id) {
        if (!requiredAnnotation)
          return true;
        else if ((*itArc)->m_annotations.get(requiredAnnotation, hypothesisID))
          return true;
      }
  }
...
void CHierarchicalMapMHPartition::getAs3DScene(
  ...
  outScene.clear();
  {
    mrpt::opengl::CGridPlaneXY::Ptr obj =
        std::make_shared<mrpt::opengl::CGridPlaneXY>(-500, 500, -500, 500, 0,
                                                     5);
  ...
  typedef std::map<
      CHMHMapNode::TNodeID, CPose3DPDFGaussian, std::less<CHMHMapNode::TNodeID>,
  ...
  typedef std::map<
      ...
      Eigen::aligned_allocator<std::pair<const CHMHMapNode::TNodeID, CPose2D>>>
      TMapID2Pose2D;
  ...
  for (it = nodesPoses.begin(); it != nodesPoses.end(); it++) {
    ...
    CRobotPosesGraph::Ptr posesGraph =
        node->m_annotations.getAs<CRobotPosesGraph>(NODE_ANNOTATION_POSES_GRAPH,
                                                    hypothesisID);
  ...
  for (it = nodesPoses.begin(), it2 = nodesMeanPoses.begin();
    ...
    mrpt::opengl::CSphere::Ptr objSphere =
        std::make_shared<mrpt::opengl::CSphere>();

    objSphere->setName(node->m_label);
    objSphere->setColor(0, 0, 1);
    objSphere->setLocation(meanPose.x(), meanPose.y(),
                           meanPose.z() + nodes_height);
    ...
    objText->setLocation(meanPose.x(), meanPose.y(),
                         meanPose.z() + nodes_height);
    ...
    if (posesGraph) {
      for (CRobotPosesGraph::const_iterator it = posesGraph->begin();
        ...
        mrpt::opengl::CDisk::Ptr glObj =
            std::make_shared<mrpt::opengl::CDisk>();
        ...
        mrpt::opengl::CSimpleLine::Ptr objLine =
            std::make_shared<mrpt::opengl::CSimpleLine>();

        objLine->setColor(1, 0, 0, 0.2);
        objLine->setLineWidth(1.5);

        objLine->setLineCoords(meanPose.x(), meanPose.y(), nodes_height,
                               auxPose.x(), auxPose.y(), 0);
    ...
    for (TArcList::const_iterator a = arcs.begin(); a != arcs.end(); ++a) {
      ...
      if (arc->getNodeFrom() == node->getID()) {
        ...
        objLine->setLineCoords(meanPose.x(), meanPose.y(),
                               meanPose.z() + nodes_height, poseTo.x(),
                               poseTo.y(), poseTo.z() + nodes_height);
...
void CHierarchicalMapMHPartition::computeGloballyConsistentNodeCoordinates(
  ...
  for (TArcList::const_iterator it_arc = m_arcs.begin(); it_arc != m_arcs.end();
       ++it_arc) {
    if (!(*it_arc)->m_hypotheses.has(hypothesisID))
      continue;
  ...
  graphslam::optimize_graph_spa_levmarq(pose_graph, out_info,
  ...
  for (graphs::CNetworkOfPoses3DInf::global_poses_t::const_iterator it_node =
    ...
    new_pose.cov.setIdentity(); // *** At present, graphslam does not output the
                                // uncertainty of poses... ***
  }
...
void CHierarchicalMapMHPartition::dumpAsText(utils::CStringList &st) const {
  ...
  for (TNodeList::const_iterator it = m_nodes.begin(); it != m_nodes.end();
       ++it) {
    std::string s;
    s += format("NODE ID: %i\t LABEL:%s\tARCS: ", (int)it->second->getID(),
    ...
    for (CMHPropertiesValuesList::const_iterator ann =
      ...
      s = format("   [HYPO ID #%02i] Annotation '%s' Class: ", (int)ann->ID,
                 ann->name.c_str());
      if (ann->value)
      ...
      if (ann->name == NODE_ANNOTATION_REF_POSEID) {
        ...
        st << format("     CRobotPosesGraph has %i poses:",
                     (int)posesGraph->size());
        CPose3D pdfMean;
...
double CHierarchicalMapMHPartition::computeOverlapProbabilityBetweenNodes(
  ...
  // Draw pose samples:
  computeCoordinatesTransformationBetweenNodes(nodeFrom, nodeTo, posePDF,
                                               hypothesisID, monteCarloSamples);
  /*0.15f,
  DEG2RAD(3.0f) );*/
