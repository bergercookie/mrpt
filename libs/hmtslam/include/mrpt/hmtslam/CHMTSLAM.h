 ...
 *		- Blanco J.L., Fernandez-Madrigal J.A., and Gonzalez J., "Towards
 *a Unified Bayesian Approach to Hybrid Metric-Topological SLAM", in  IEEE
...
protected:
  ...
  /** Message definition:
          - From: LSLAM
          - To: AA
          - Meaning: Reconsider the graph partition for the given LMH. Compute
     SSO for the new node(s) "newPoseIDs".
          */
  /*struct TMessageAA
  {
  ...
  struct TMessageLSLAMfromTBI {
    ...
    CHMHMapNode::TNodeID
        cur_area; //!< The area for who the loop-closure has been computed.
    ...
    struct TBI_info {
      ...
      /** Depending on the loop-closure engine, an guess of the relative pose of
       * "area_new" relative to "cur_area" is given here.
        *  If the SOG contains 0 modes, then the engine does not provide this
       * information.
        */
  ...
  utils::CMessageQueue m_LSLAM_queue; //!< LSLAM thread input queue, messages of
                                      //!type CHMTSLAM::TMessageLSLAMfromAA
   ...
   * method (it does NOT lock any critical section).
    */
  static TMessageLSLAMfromAA::Ptr
  areaAbstraction(CLocalMetricHypothesis *LMH, const TPoseIDList &newPoseIDs);

  /** The entry point for Topological Bayesian Inference (TBI) engines, invoked
   * from LSLAM.
    * \param LMH (IN) The LMH which to this query applies.
   ...
   * method (it does NOT lock any critical section).
    */
  static TMessageLSLAMfromTBI::Ptr
  TBI_main_method(CLocalMetricHypothesis *LMH,
...
public:
  ...
  /** Returns true if the input queue is empty (Note that the queue must not be
   * empty to the user to enqueue more actions/observaitions)
   ...
   * process).
    *  This class will delete the passed object when required, so DO NOT DELETE
   * the passed object after calling this.
    * \sa pushObservations,pushObservation
    */
  void pushAction(const mrpt::obs::CActionCollection::Ptr &acts);

  /** Here the user can enter observations into the system (will go to the SLAM
   * process).
    *  This class will delete the passed object when required, so DO NOT DELETE
   * the passed object after calling this.
...
protected:
  ...
  /** The queue of pending actions/observations supplied by the user waiting for
   * being processed. */
  ...
  /** @name HMT-SLAM sub-processes.
          @{ */
  void LSLAM_process_message(const mrpt::utils::CMessage &
                                 msg); //!< Auxiliary method within thread_LSLAM
  ...
  /** Topological Loop Closure: Performs all the required operations
          ...
          to be the same.
   */
  void perform_TLC(CLocalMetricHypothesis &LMH,
...
protected:
  ...
  /** The list of LC modules in operation - initialized by "initializeEmptyMap"
   * or "loadState".  */
...
public:
  /** Default constructor
    *  \param debug_out_stream If debug output messages should be redirected to
   * any other stream apart from std::cout
    */
  CHMTSLAM();
  ...
  /** @name The important data.
          @{ */
  CHierarchicalMHMap
      m_map; //!< The hiearchical, multi-hypothesis graph-based map.
  aligned_containers<THypothesisID, CLocalMetricHypothesis>::map_t
      m_LMHs; //!< The list of LMHs at each instant.
              /** @} */
...
protected:
  ...
  struct TOptions : public utils::CLoadableOptions {
    ...
    std::string LOG_OUTPUT_DIR; //!< [LOGGING] If it is not an empty string
                                //!(""), a directory with that name will be
                                //!created and log files save there.
    int LOG_FREQUENCY;          //!< [LOGGING] One SLAM iteration out of
                       //!"LOGGING_logFrequency", a log file will be generated.

    /** [LSLAM] The method to use for local SLAM
      */
    TLSlamMethod SLAM_METHOD;

    /** [LSLAM] Minimum distance (and heading) difference between observations
     * inserted in the map.
    */
    float SLAM_MIN_DIST_BETWEEN_OBS, SLAM_MIN_HEADING_BETWEEN_OBS;

    /** [LSLAM] Minimum uncertainty (1 sigma, meters) in x and y for odometry
     ...
     * uncertainty values will be used instead); x y: In meters, phi: radians
     * (but in degrees when loading from a configuration ini-file!)
      */
    ...
    */
    ...
    mrpt::maps::TSetOfMetricMapInitializers defaultMapsInitializers; //!< The
                                                                     //!default
                                                                     //!set of
                                                                     //!maps to
                                                                     //!be
    ...
    bayes::CParticleFilter::TParticleFilterOptions
        pf_options; //!< These params are used from every LMH object.
    mrpt::slam::TKLDParams KLD_params;

    int random_seed; //!< 0 means randomize, use any other value to have
                     //!repetitive experiments.

    /** A list of topological loop-closure detectors to use: can be one or more
     * from this list:
    ...
    CTopLCDetector_GridMatching::TOptions
        TLC_grid_options; //!< Options passed to this TLC constructor
    CTopLCDetector_FabMap::TOptions
        TLC_fabmap_options; //!< Options passed to this TLC constructor

  } m_options;

}; // End of class CHMTSLAM.
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CHMTSLAM,
...
public:
  ...
  /** Main entry point from HMT-SLAM: process some actions & observations.
    *  The passed action/observation will be deleted, so a copy must be made if
   * necessary.
   ...
   * SLAM algorithm.
    ...
    */
  virtual void processOneLMH(CLocalMetricHypothesis *LMH,
...
public:
   ...
   * SLAM algorithm.
    ...
    * \param sf    The observations to process (or nullptr).
    */
  void processOneLMH(CLocalMetricHypothesis *LMH,
                     const mrpt::obs::CActionCollection::Ptr &act,
...
protected:
  ...
  /** Fills out a "TPathBin" variable, given a path hypotesis and (if not set to
   * nullptr) a new pose appended at the end, using the KLD params in "options".
          */
  void loadTPathBinFromPath(TPathBin &outBin, TMapPoseID2Pose3D *path = nullptr,
  ...
  /** Auxiliary function that evaluates the likelihood of an observation, given
   * a robot pose, and according to the options in "CPosePDFParticles::options".
    */
