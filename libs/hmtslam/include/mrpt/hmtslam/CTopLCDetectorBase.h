...
public:
  ...
  /** If implemented, this method provides the evaluation of an additional term
   * to be added to the SSO between each pair of observations.
    * \param out_SSO The output, in the range [0,1].
    * \return true if computed SSO is meaningful. The default virtual method
   * returns false.
    */
  virtual bool computeSSOBetweenObservations(const THypothesisID &hypID,
   ...
   * maps.
    ...
    */
  virtual void OnNewPose(const TPoseID &poseID,
