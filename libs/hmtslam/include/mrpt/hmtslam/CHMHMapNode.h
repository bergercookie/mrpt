...
protected:
   ...
   * use the same smrt. pnt., but at destructors the objects don't know anything
   * but "this", thus the usage of plain pointers.
    */
  void onArcDestruction(CHMHMapArc *arc);

  /** Event handler for arc addition: It should be only called for arcs from/to
   * this node, although other cases have no effects.
    */
...
public:
  /** Constructor
   */
  CHMHMapNode(CHierarchicalMHMap *parent = nullptr,
  ...
  /** The annotations of the node, see the general description of the class for
  ...
  /** Returns the level of this node in the hierarchy of arcs "arcType_Belongs",
   * where level=0 is the ground level, 1=its parents, etc.
    */
   ...
   * hypothesis ID and of a given type.
    */
  void getArcs(TArcList &out, const char *arcType,
               const THypothesisID &hyp_id) const;
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CHMHMapNode,
