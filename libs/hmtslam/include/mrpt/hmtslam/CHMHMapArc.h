...
protected:
   ...
   *   When a node involved in the arc is delected, the corresponding pointer in
   * the arc will be set to nullptr and the arc is no longer a valid one.
   */
...
public:
  /** Private constructor (see ::Create class factory)
    */
  CHMHMapArc(const CHMHMapNode::TNodeID &from = 0,
  ...
  /** Alternative constructor, using pointers for convenience.
    */
  CHMHMapArc(CHMHMapNode::Ptr &from, CHMHMapNode::Ptr &to,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CHMHMapArc,
