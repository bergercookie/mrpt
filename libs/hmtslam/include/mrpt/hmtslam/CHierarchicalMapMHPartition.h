...
public:
  ...
  /** Returns the first node in the graph, or nullptr if it does not exist.
    * \return A pointer to the object. DO NOT DELETE this object, if you want to
   * modify it in someway, first obtain a copy by invoking
   ...
   * "CSerializable::duplicate"
    */
  CHMHMapNode::Ptr getNodeByLabel(const std::string &label,
  ...
  getNodeByLabel(const std::string &label,
                 const THypothesisID &hypothesisID) const;

  /** Returns a partition of this graph only with nodes at a given level in the
    *hierarchy (0=ground level,1=parent level,etc)
     *	- The partition may be empty if no node fulfills the condition.
     *	- All arcs STARTING at each node from the partition will be added to the
    *partition as well.
     *	- Levels in the hierarchy here stands for arcs of type "arcType_Belongs"
    *only.
     * \sa CHMHMapArc
     */
  ...
  /** Saves a MATLAB script that represents graphically the nodes with
    ...
    *with an annotation "RelativePose", added to the pose of the original node.
    *	- If the coordinates of any node can not be computed (no arcs,...), an
    *exception will be raised.
    */
  void saveAreasDiagramForMATLAB(const std::string &filName,
  ...
  /** Saves a MATLAB script that represents graphically the reconstructed
    ...
    */
  void
  saveGlobalMapForMATLAB(const std::string &filName,
                         const THypothesisID &hypothesisID,
   ...
   * path is found or when the starting and ending node coincide.
    */
  void findPathBetweenNodes(const CHMHMapNode::TNodeID &nodeFrom,
   ...
   * closure), according to the grid maps and pose uncertainty from information
   * in arcs (uses a Monte Carlo aproximation)
  ...
  float computeMatchProbabilityBetweenNodes(
      const CHMHMapNode::TNodeID &nodeFrom, const CHMHMapNode::TNodeID &nodeTo,
      float &maxMatchProb, mrpt::poses::CPose3DPDFSOG &estimatedRelativePose,
      const THypothesisID &hypothesisID,
      unsigned int monteCarloSamplesPose = 300);

  /** Returns all the arcs between a pair of nodes:
    */
  void findArcsBetweenNodes(const CHMHMapNode::TNodeID &node1,
  ...
  /** Returns the arcs between a pair of nodes of a given type.
    */
  void findArcsOfTypeBetweenNodes(const CHMHMapNode::TNodeID &node1id,
   ...
   * is in the opposite direction.
    ...
    */
  CHMHMapArc::Ptr findArcOfTypeBetweenNodes(const CHMHMapNode::TNodeID &node1id,
                                            const CHMHMapNode::TNodeID &node2id,
                                            const THypothesisID &hypothesisID,
                                            const std::string &arcType,
                                            bool &isInverted) const;

  /** Returns whether two nodes are "neightbour", i.e. have a direct arc between
   * them  */
  bool areNodesNeightbour(const CHMHMapNode::TNodeID &node1,
                          const CHMHMapNode::TNodeID &node2,
                          const THypothesisID &hypothesisID,
   ...
   * "idReferenceNode".
    * \exception std::exception If there is any node without a pose arc, invalid
   * (non invertible) matrixes, etc...
   ...
   * "numberOfIterationsForOptimalGlobalPoses"
    */
  void
  getAs3DScene(mrpt::opengl::COpenGLScene &outScene,
               const CHMHMapNode::TNodeID &idReferenceNode,
               const THypothesisID &hypothesisID,
   ...
   * etc...
    * \param margin_to_substract In meters, the area of each gridmap is "eroded"
   * this amount to compensate the area in excess usually found in gridmaps.
    */
  double computeOverlapProbabilityBetweenNodes(
      const CHMHMapNode::TNodeID &nodeFrom, const CHMHMapNode::TNodeID &nodeTo,
      const THypothesisID &hypothesisID, const size_t &monteCarloSamples = 100,
