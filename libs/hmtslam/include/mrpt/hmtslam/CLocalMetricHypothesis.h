...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CLSLAMParticleData,
...
public:
  ...
  MRPT_TODO("Separate the serializable class from this code, so we don't have "
            "to worry about copying locks")
  struct ThreadLocks {
  ...
  THypothesisID m_ID; //!< The unique ID of the hypothesis (Used for accessing
                      //!mrpt::slam::CHierarchicalMHMap).
  mrpt::utils::safe_ptr<CHMTSLAM>
      m_parent;               //!< For quick access to our parent object.
  TPoseID m_currentRobotPose; //!< The current robot pose (its global unique ID)
                              //!for this hypothesis.
  // TNodeIDList							m_neighbors;			//!< The list of all
  // areas sourronding the current one (this includes the current area itself).
  TNodeIDSet m_neighbors; //!< The list of all areas sourronding the current one
                          //!(this includes the current area itself).
  std::map<TPoseID, CHMHMapNode::TNodeID> m_nodeIDmemberships; //!< The hybrid
                                                               //!map node
                                                               //!membership for
                                                               //!each robot
                                                               //!pose.
  std::map<TPoseID, mrpt::obs::CSensoryFrame>
      m_SFs; //!< The SF gathered at each robot pose.
  TPoseIDList m_posesPendingAddPartitioner; //!< The list of poseIDs waiting to
                                            //!be added to the graph
                                            //!partitioner, what happens in the
                                            //!LSLAM thread main loop.
  TNodeIDList m_areasPendingTBI; //!< The list of area IDs waiting to be
                                 //!processed by the TBI (topological bayesian
                                 //!inference) engines to search for potential
                                 //!loop-closures. Set in
                                 //!CHMTSLAM::LSLAM_process_message_from_AA,
                                 //!read in

  double m_log_w; //!< Log-weight of this hypothesis.
  std::vector<std::map<TPoseID, double>> m_log_w_metric_history; //!< The
                                                                 //!historic
                                                                 //!log-weights
                                                                 //!of the
                                                                 //!metric
  ...
  mrpt::obs::CActionRobotMovement2D
      m_accumRobotMovement;         //!< Used in CLSLAM_RBPF_2DLASER
  ...
  struct TRobotPosesPartitioning {
    ...
    unsigned int pose2idx(const TPoseID &id)
        const; //!< Uses idx2pose to perform inverse searches.
  ...
  /** Returns the mean of each robot pose in this LMH, as computed from the set
   * of particles.
   ...
   * computed from the set of particles.
    ...
    */
  void getPoseParticles(const TPoseID &poseID,
   ...
   * must be part of the the LMH).
    * \sa getMeans, getPoseParticles
    */
  void getRelativePose(const TPoseID &reference, const TPoseID &pose,
                       mrpt::poses::CPose3DPDFParticles &outPDF) const;
   ...
   * to the area, and removed from the LMH.
    * \note The critical section m_map_cs is locked internally, unlock it before
   * calling this.
    */
  void updateAreaFromLMH(const CHMHMapNode::TNodeID areaID,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CLocalMetricHypothesis,
                                             mrpt::utils::CSerializable,
                                             HMTSLAM_IMPEXP)
