 ...
 *Gaussians; or
 *		- Initial PDF of beacons: SOG, after convergence, a single
 *Gaussian.
 *
 *   Refer to the papers: []
 *
...
public:
  ...
  /** This struct contains data for choosing the method by which new beacons are
   * inserted in the map.
   */
  ...
  public:
    ...
    /** Insert a new beacon as a set of montecarlo samples (default=true), or,
     * if false, as a sum of gaussians (see mrpt::maps::CBeacon).
      * \sa MC_performResampling
      */
    bool insertAsMonteCarlo;

    /** Minimum and maximum elevation angles (in degrees) for inserting new
     * beacons at the first observation: the default values (both 0), makes the
     * beacons to be in the same horizontal plane that the sensors, that is, 2D
     * SLAM - the min/max values are -90/90.
      */
    float maxElevation_deg, minElevation_deg;

    /** Number of particles per meter of range, i.e. per meter of the "radius of
     * the ring".
      */
    ...
    /** Threshold for the maximum difference from the maximun (log) weight in
     * the set of samples for erasing a given sample (default=5).
      */
    float MC_thresholdNegligible;

    /** If set to false (default), the samples will be generated the first time
     * a beacon is observed, and their weights just updated subsequently - if
     * set to "true", fewer samples will be required since the particles will be
     * resamples when necessary, and a small "noise" will be added to avoid
     * depletion.
      */
    ...
    /** Threshold for the maximum difference from the maximun (log) weight in
     * the SOG for erasing a given mode (default=20).
      */
    ...
    /** Constant used to compute the std. dev. int the tangent direction when
   ...
   */
  bool saveToMATLABScript3D(const std::string &file, const char *style = "b",
  ...
  void getAs3DObject(mrpt::opengl::CSetOfObjects::Ptr &outObj)
      const override; //!< Returns a 3D object representing the map.

  const CBeacon *getBeaconByID(CBeacon::TBeaconID id) const; //!< Returns a
                                                             //!pointer to the
                                                             //!beacon with the
                                                             //!given ID, or
                                                             //!nullptr if it
                                                             //!does not exist.
  CBeacon *getBeaconByID(CBeacon::TBeaconID id); //!< Returns a pointer to the
                                                 //!beacon with the given ID, or
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CBeaconMap, CMetricMap,
                                             MAPS_IMPEXP)
