...
public:
  ...
  /** Which one of the different 3D point PDF is currently used in this object:
   * montecarlo, gaussian, or a sum of gaussians.
    * \sa m_location
    */
  TTypePDF m_typePDF;
  ...
  /** The individual PDF, if m_typePDF=pdfGauss (publicly accesible for ease of
  ...
  /** An ID for the landmark (see details next...)
    *  This ID was introduced in the version 3 of this class (21/NOV/2006), and
    *its aim is
    ...
    *images): A different ID must be assigned to every landmark (e.g. a
    *sequential counter), BUT only in the case of being sure of the
    *correspondence of one landmark with another one in the past (e.g.
    *tracking).
   ...
   * very separate Gaussian modes (that will result in negligible components) in
   * SOGs will be dropped to reduce the number of modes in the output.
    */
  void bayesianFusion(const CPointPDF &p1, const CPointPDF &p2,
  ...
  /** This static method returns a SOG with ring-shape (or as a 3D sphere) that
   * can be used to initialize a beacon if observed the first time.
    *  sensorPnt is the center of the ring/sphere, i.e. the absolute position of
   * the range sensor.
    *  If clearPreviousContentsOutPDF=false, the SOG modes will be added to the
   * current contents of outPDF
    *  If the 3x3 matrix covarianceCompositionToAdd is provided, it will be add
   * to every Gaussian (to model the composition of uncertainty).
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CBeacon,
