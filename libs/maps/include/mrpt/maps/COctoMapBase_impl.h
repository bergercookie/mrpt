...
double COctoMapBase<OCTREE, OCTREE_NODE>::internal_computeObservationLikelihood(
  ...
  if (!internal_build_PointCloud_for_observation(obs, &takenFrom, sensorPt,
                                                 scan))
...
void COctoMapBase<OCTREE, OCTREE_NODE>::insertPointCloud(
  ...
  for (size_t i = 0; i < N; i++)
    m_octomap.insertRay(sensorPt, octomap::point3d(xs[i], ys[i], zs[i]),
...
bool COctoMapBase<OCTREE, OCTREE_NODE>::castRay(
  ...
  const bool ret =
      m_octomap.castRay(octomap::point3d(origin.x, origin.y, origin.z),
...
COctoMapBase<OCTREE, OCTREE_NODE>::TInsertionOptions::TInsertionOptions(
    ...
    : maxrange(-1.), pruning(true), m_parent(&parent),
      // Default values from octomap:
      occupancyThres(0.5), probHit(0.7), probMiss(0.4),
      clampingThresMin(0.1192), clampingThresMax(0.971) {}

template <class OCTREE, class OCTREE_NODE>
COctoMapBase<OCTREE, OCTREE_NODE>::TInsertionOptions::TInsertionOptions()
    : maxrange(-1.), pruning(true), m_parent(nullptr),
      // Default values from octomap:
      occupancyThres(0.5), probHit(0.7), probMiss(0.4),
      clampingThresMin(0.1192), clampingThresMax(0.971) {}
...
void COctoMapBase<OCTREE, OCTREE_NODE>::TRenderingOptions::readFromStream(
  ...
  case 0: {
    in >> generateGridLines >> generateOccupiedVoxels >>
        visibleOccupiedVoxels >> generateFreeVoxels >> visibleFreeVoxels;
  } break;
  default:
    MRPT_THROW_UNKNOWN_SERIALIZATION_VERSION(version)
  }
