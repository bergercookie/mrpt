 ...
 **/
#if defined(MRPT_IS_X86_AMD64) // Pragma defined to ensure no structure packing:
                               // since we'll serialize TRandomFieldVoxel to
                               // streams, we want it not to depend on compiler
                               // options, etc.
#pragma pack(push, 1)
#endif
struct MAPS_IMPEXP TRandomFieldVoxel {
  double mean_value, stddev_value; //!< Mean and sigma (standard deviation)
                                   //!estimated values for the voxel.
...
public:
  static bool ENABLE_GMRF_PROFILER; //!< [default:false] Enables a profiler to
                                    //!show a performance report at application
                                    //!end.
   ...
   * initialize the object before using it to insert observations.
    */
  CRandomFieldGridMap3D(double x_min = -2, double x_max = 2, double y_min = -2,
   ...
   * `x y z stddev_value`, if `filName_stddev` is provided.
    ...
    */
  bool saveAsCSV(const std::string &filName_mean,
  ...
  virtual void resize(double new_x_min, double new_x_max, double new_y_min,
                      double new_y_max, double new_z_min, double new_z_max,
   ...
   * `resolution_z`<0, the same resolution will be used for all dimensions x,y,z
   * as given in `resolution_xy` \sa resize.*/
  ...
  struct MAPS_IMPEXP ConnectivityDescriptor {
    ...
    /** Implement the check of whether node i=(icx,icy,icz) is connected with
    * node j=(jcx,jcy,jcy).
    ...
    */
    virtual bool getEdgeInformation(
        const CRandomFieldGridMap3D
            *parent, //!< The parent map on which we are running
        size_t icx,
        size_t icy, size_t icz,             //!< (cx,cy,cz) for node "i"
        size_t jcx, size_t jcy, size_t jcz, //!< (cx,cy,cz) for node "j"
        double &out_edge_information //!< Must output here the inverse of the
                                     //!variance of the constraint edge.
        ) = 0;
  };

  /** Sets a custom object to define the connectivity between voxels. Must call
  ...
  bool insertIndividualReading(
      ...
      const TVoxelInterpolationMethod method, //!< [in] Voxel interpolation
                                              //!method: how many voxels will be
                                              //!affected by the reading
      const bool update_map //!< [in] Run a global map update after inserting
                            //!this observation (algorithm-dependant)
      );

  void updateMapEstimation(); //!< Run the method-specific procedure required to
                              //!ensure that the mean & variances are up-to-date
                              //!with all inserted observations, using
                              //!parameters in insertionOptions
