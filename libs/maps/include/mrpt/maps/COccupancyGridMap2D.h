...
/** The type of the map cells: */
   ...
   protected:
	 ...
	 * likelihood
	 * values for LF method among others, at a high cost in memory (see
	 * TLikelihoodOptions::enableLikelihoodCache). */
	...
	* \sa CObservation::insertIntoGridMap */
	class MAPS_IMPEXP TInsertionOptions : public mrpt::utils::CLoadableOptions
	{
		...
		*/
		 ...
		 * when
		 * the previous and next rays are also an invalid ray. */
		 ...
		 * considered
		 * horizontal, then processed by calls to this class (default=0). */
		 ...
		 * of
		 * lasers, disabled: insert rays as simple lines (Default=false) */
	...
	class MAPS_IMPEXP TLikelihoodOptions : public mrpt::utils::CLoadableOptions
	{
	   public:
		 ...
		 * where
		 * the likelihood of the whole range scan is computed by "averaging" of
		 * individual ranges, instead of by the "product". */
		 ...
		 * in
		 * the insertion of scans, e.g. if set to 2.0 the MI will use twice the
		 * distance that the update distance. */
		 ...
		 * list of
		 * likelihood values (first one is the largest); the size of this vector
		 * determines the number of highest likelihood values to fuse. */
	...
	class TLikelihoodOutput
	{
	   public:
		...
		/** [OWA method] This will contain the ascending-ordered list of
		 * likelihood
		 * values for individual range measurements in the scan. */
	...
	*COccupancyGridMap2D::RAYTRACE_STEP_SIZE_IN_CELL_UNITS
	...
	struct MAPS_IMPEXP TLaserSimulUncertaintyParams
	{
		 ...
		 * angles
		 * at which ranges are measured (in radians) */
		float angleNoiseStd;
