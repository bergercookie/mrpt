...
/** The type of the map cells: */
  ...
  static double RAYTRACE_STEP_SIZE_IN_CELL_UNITS; //!< (Default:1.0) Can be set
                                                  //!to <1 if a more fine
                                                  //!raytracing is needed in
                                                  //!sonarSimulator() and
                                                  //!laserScanSimulator(), or >1
                                                  //!to speed it up.
...
protected:
  ...
  static CLogOddsGridMapLUT<cellType>
      m_logodd_lut; //!< Lookup tables for log-odds

  std::vector<cellType> map; //!< Store of cell occupancy values. Order: row by
                             //!row, from left to right
  uint32_t size_x, size_y;   //!< The size of the grid in cells
  float x_min, x_max, y_min,
      y_max;        //!< The limits of the grid in "units" (meters)
  float resolution; //!< Cell size, i.e. resolution of the grid map.

  std::vector<double>
      precomputedLikelihood; //!< Auxiliary variables to speed up the
                             //!computation of observation likelihood values for
                             //!LF method among others, at a high cost in memory
  ...
  virtual void OnPostSuccesfulInsertObs(
      const mrpt::obs::CObservation *) override; //!< See base class

  float voroni_free_threshold; //!< The free-cells threshold used to compute the
                               //!Voronoi diagram.

  static double H(double p); //!< Entropy computation internal function:
  static std::vector<float>
      entropyTable; //!< Internally used to speed-up entropy calculation
  ...
  double
  computeObservationLikelihood_Consensus(const mrpt::obs::CObservation *obs,
  ...
  /** One of the methods that can be selected for implementing
   * "computeObservationLikelihood" */
  double computeObservationLikelihood_MI(const mrpt::obs::CObservation *obs,
                                         const mrpt::poses::CPose2D &takenFrom);
  /** One of the methods that can be selected for implementing
   * "computeObservationLikelihood" */
  ...
  virtual void internal_clear() override; //!< Clear the map: It set all cells
                                          //!to their default occupancy value
                                          //!(0.5), without changing the
                                          //!resolution (the grid extension is
                                          //!reset to the default values).

  /** Insert the observation information into this map.
   *
   * \param obs The observation
   * \param robotPose The 3D pose of the robot mobile base in the map reference
   * system, or nullptr (default) if you want to use CPose2D(0,0,deg)
   *
...
public:
  ...
  struct MAPS_IMPEXP TUpdateCellsInfoChangeOnly {
    TUpdateCellsInfoChangeOnly(bool enabled = false, double I_change = 0,
                               int cellsUpdated = 0)
        : enabled(enabled), I_change(I_change), cellsUpdated(cellsUpdated),
          laserRaysSkip(1) {}
    bool enabled; //!< If set to false (default), this struct is not used. Set
                  //!to true only when measuring the info of an observation.
    double I_change; //!< The cummulative change in Information: This is updated
                     //!only from the "updateCell" method.
    int cellsUpdated;  //!< The cummulative updated cells count: This is updated
                       //!only from the "updateCell" method.
    int laserRaysSkip; //!< In this mode, some laser rays can be skips to
                       //!speep-up
  } updateInfoChangeOnly;

  void fill(float default_value =
                0.5f); //!< Fills all the cells with a default value.

  /** Constructor */
  COccupancyGridMap2D(float min_x = -20.0f, float max_x = 20.0f,
   ...
   * \sa ResizeGrid
   */
  void setSize(float x_min, float x_max, float y_min, float y_max,
               float resolution, float default_value = 0.5f);
   ...
   * \param additionalMargin If set to true (default), an additional margin of a
   * few meters will be added to the grid, ONLY if the new coordinates are
   * larger than current ones.
   * \sa setSize
   */
  void resizeGrid(float new_x_min, float new_x_max, float new_y_min,
                  float new_y_max, float new_cells_default_value = 0.5f,
  ...
  /** Scales a real valued probability in [0,1] to an integer representation of:
   * log(p)-log(1-p)  in the valid range of cellType */
  ...
  /** Returns "true" if cell is "static", i.e.if its occupancy is below a given
   * threshold */
  ...
  void copyMapContentFrom(const COccupancyGridMap2D &otherMap); //!< copy the
                                                                //!gridmap
                                                                //!contents, but
                                                                //!not all the
                                                                //!options, from
  ...
  struct MAPS_IMPEXP TEntropyInfo {
    TEntropyInfo()
        : H(0), I(0), mean_H(0), mean_I(0), effectiveMappedArea(0),
          effectiveMappedCells(0) {}
    double H; //!< The target variable for absolute entropy, computed
              //!as:<br><center>H(map)=Sum<sub>x,y</sub>{ -p(x,y)*ln(p(x,y))
              //!-(1-p(x,y))*ln(1-p(x,y)) }</center><br><br>
    double I; //!< The target variable for absolute "information", defining I(x)
              //!= 1 - H(x)
    double mean_H; //!< The target variable for mean entropy, defined as entropy
                   //!per cell: mean_H(map) = H(map) / (cells)
    double mean_I; //!< The target variable for mean information, defined as
                   //!information per cell: mean_I(map) = I(map) / (cells)
    double effectiveMappedArea; //!< The target variable for the area of cells
                                //!with information, i.e. p(x)!=0.5
  ...
  * \sa CObservation::insertIntoGridMap */
  ...
  public:
    ...
    */
     ...
     */
    void
    loadFromConfigFile(const mrpt::utils::CConfigFileBase &source,
                       const std::string &section) override; // See base docs
    void
    dumpToTextStream(mrpt::utils::CStream &out) const override; // See base docs

    float mapAltitude;   //!< The altitude (z-axis) of 2D scans (within a 0.01m
                         //!tolerance) for they to be inserted in this map!
    bool useMapAltitude; //!< The parameter "mapAltitude" has effect while
                         //!inserting observations in the grid only if this is
                         //!true.
    float maxDistanceInsertion; //!< The largest distance at which cells will be
                                //!updated (Default 15 meters)
    float maxOccupancyUpdateCertainty; //!< A value in the range [0.5,1] used
                                       //!for updating cell with a bayesian
                                       //!approach (default 0.8)
    bool considerInvalidRangesAsFreeSpace; //!< If set to true (default),
                                           //!invalid range values (no echo
                                           //!rays) as consider as free space
                                           //!until
                                           //!"maxOccupancyUpdateCertainty", but
                                           //!ONLY when the previous and next
                                           //!rays are also an invalid ray.
    uint16_t decimation;       //!< Specify the decimation of the range scan
                               //!(default=1 : take all the range values!)
    float horizontalTolerance; //!< The tolerance in rads in pitch & roll for a
                               //!laser scan to be considered horizontal, then
                               //!processed by calls to this class (default=0).
    float CFD_features_gaussian_size; //!< Gaussian sigma of the filter used in
                                      //!getAsImageFiltered (for features
                                      //!detection) (Default=1) (0:Disabled)
    float CFD_features_median_size;   //!< Size of the Median filter used in
                                      //!getAsImageFiltered (for features
                                      //!detection) (Default=3) (0:Disabled)
    bool wideningBeamsWithDistance;   //!< Enabled: Rays widen with distance to
                                    //!approximate the real behavior of lasers,
                                    //!disabled: insert rays as simple lines
                                    //!(Default=false)
  };

  TInsertionOptions insertionOptions; //!< With this struct options are provided
                                      //!to the observation insertion process
                                      //!\sa CObservation::insertIntoGridMap
  ...
  public:
    ...
    TLikelihoodMethod likelihoodMethod; //!< The selected method to compute an
                                        //!observation likelihood
    float LF_stdHit; //!< [LikelihoodField] The laser range "sigma" used in
                     //!computations; Default value = 0.35
    float LF_zHit, LF_zRandom; //!< [LikelihoodField]
    float LF_maxRange;      //!< [LikelihoodField] The max. range of the sensor
                            //!(Default= 81 m)
    uint32_t LF_decimation; //!< [LikelihoodField] The decimation of the points
                            //!in a scan, default=1 == no decimation
    float LF_maxCorrsDistance; //!< [LikelihoodField] The max. distance for
                               //!searching correspondences around each sensed
                               //!point
    bool LF_useSquareDist;     //!< [LikelihoodField] (Default:false) Use
                           //!`exp(dist^2/std^2)` instead of `exp(dist^2/std^2)`
    bool LF_alternateAverageMethod; //!< [LikelihoodField] Set this to "true" ot
                                    //!use an alternative method, where the
                                    //!likelihood of the whole range scan is
                                    //!computed by "averaging" of individual
                                    //!ranges, instead of by the "product".
    float MI_exponent; //!< [MI] The exponent in the MI likelihood computation.
                       //!Default value = 5
    uint32_t MI_skip_rays; //!< [MI] The scan rays decimation: at every N rays,
                           //!one will be used to compute the MI
    float MI_ratio_max_distance; //!< [MI] The ratio for the max. distance used
                                 //!in the MI computation and in the insertion
                                 //!of scans, e.g. if set to 2.0 the MI will use
                                 //!twice the distance that the update distance.
    bool rayTracing_useDistanceFilter; //!< [rayTracing] If true (default), the
                                       //!rayTracing method will ignore measured
                                       //!ranges shorter than the simulated
                                       //!ones.
    int32_t rayTracing_decimation;     //!< [rayTracing] One out of
                                       //!"rayTracing_decimation" rays will be
                                   //!simulated and compared only: set to 1 to
                                   //!use all the sensed ranges.
    float rayTracing_stdHit;         //!< [rayTracing] The laser range sigma.
    int32_t consensus_takeEachRange; //!< [Consensus] The down-sample ratio of
                                     //!ranges (default=1, consider all the
                                     //!ranges)
    float consensus_pow; //!< [Consensus] The power factor for the likelihood
                         //!(default=5)
    std::vector<float> OWA_weights; //!< [OWA] The sequence of weights to be
                                    //!multiplied to of the ordered list of
                                    //!likelihood values (first one is the
                                    //!largest); the size of this vector
                                    //!determines the number of highest
                                    //!likelihood values to fuse.

    bool enableLikelihoodCache; //!< Enables the usage of a cache of likelihood
                                //!values (for LF methods), if set to true
                                //!(default=false).
  } likelihoodOptions;

  typedef std::pair<double, mrpt::math::TPoint2D>
      TPairLikelihoodIndex; //!< Auxiliary private class.

  /** Some members of this struct will contain intermediate or output data after
   * calling "computeObservationLikelihood" for some likelihood functions */
  class TLikelihoodOutput {
  public:
    TLikelihoodOutput() : OWA_pairList(), OWA_individualLikValues() {}

    std::vector<TPairLikelihoodIndex> OWA_pairList; //!< [OWA method] This will
                                                    //!contain the
                                                    //!ascending-ordered list of
                                                    //!pairs:(likelihood values,
                                                    //!2D point in map
                                                    //!coordinates).
    std::vector<double> OWA_individualLikValues;    //!< [OWA method] This will
                                                    //!contain the
                                                    //!ascending-ordered list of
  ...
  void subSample(int downRatio); //!< Performs a downsampling of the gridmap, by
                                 //!a given factor: resolution/=ratio

  /** Computes the entropy and related values of this grid map.
   *  The entropy is computed as the summed entropy of each cell, taking them as
   * discrete random variables following a Bernoulli distribution:
   ...
   */
  void buildVoronoiDiagram(float threshold, float robot_size, int x1 = 0,
                           int x2 = 0, int y1 = 0, int y2 = 0);
...
public:
  ...
  /** Return the Voronoi diagram; each cell contains the distance to its closer
   * obstacle, or 0 if not part of the Voronoi diagram \sa buildVoronoiDiagram
   */
   ...
   * \param basis_x Target buffer for coordinates of basis, having a size of two
   * "ints".
   * \param basis_y Target buffer for coordinates of basis, having a size of two
   * "ints".
   ...
   */
  int computeClearance(int cx, int cy, int *basis_x, int *basis_y, int *nBasis,
                       bool GetContourPoint = false) const;

  /** An alternative method for computing the clearance of a given location (in
   * meters).
  ...
  *sensor pose relative to this robot pose must be specified in the observation
  *object.
   * \param threshold [IN] The minimum occupancy threshold to consider a cell to
  *be occupied (Default: 0.5f)
  ...
  *D, 2D, 3D, ... Default is D=1
   * \param angleNoiseStd [IN] The sigma of an optional Gaussian noise added to
  *the angles at which ranges are measured (in radians).
   *
  * \sa laserScanSimulatorWithUncertainty(), sonarSimulator(),
  *COccupancyGridMap2D::RAYTRACE_STEP_SIZE_IN_CELL_UNITS
   */
  void laserScanSimulator(mrpt::obs::CObservation2DRangeScan &inout_Scan,
                          const mrpt::poses::CPose2D &robotPose,
                          float threshold = 0.6f, size_t N = 361,
                          float noiseStd = 0, unsigned int decimation = 1,
   ...
   * parameters before calling, and will contain the simulated ranges on return.
   * \param robotPose [IN] The robot pose in this map coordinates. Recall that
   ...
   * COccupancyGridMap2D::RAYTRACE_STEP_SIZE_IN_CELL_UNITS
   */
  void sonarSimulator(mrpt::obs::CObservationRange &inout_observation,
                      const mrpt::poses::CPose2D &robotPose,
                      float threshold = 0.5f, float rangeNoiseStd = 0.f,
                      float angleNoiseStd = mrpt::utils::DEG2RAD(0.f)) const;

  /** Simulate just one "ray" in the grid map. This method is used internally to
   * sonarSimulator and laserScanSimulator. \sa
   * COccupancyGridMap2D::RAYTRACE_STEP_SIZE_IN_CELL_UNITS */
  void simulateScanRay(const double x, const double y,
                       const double angle_direction, float &out_range,
                       bool &out_valid, const double max_range_meters,
  ...
  struct MAPS_IMPEXP TLaserSimulUncertaintyParams {
    TLaserSimulUncertaintyMethod method; //!< (Default: sumMonteCarlo) Select
                                         //!the method to do the uncertainty
                                         //!propagation
    /** @name Parameters for each uncertainty method
        @{ */
    double UT_alpha, UT_kappa, UT_beta; //!< [sumUnscented] UT parameters.
                                        //!Defaults: alpha=0.99, kappa=0,
                                        //!betta=2.0
    size_t MC_samples; //!< [sumMonteCarlo] MonteCarlo parameter: number of
                       //!samples (Default: 10)
                       /** @} */

    /** @name Generic parameters for all methods
        @{ */
    mrpt::poses::CPosePDFGaussian robotPose; //!< The robot pose Gaussian, in
                                             //!map coordinates. Recall that
                                             //!sensor pose relative to this
                                             //!robot pose must be specified in
                                             //!the observation object
    float aperture; //!< (Default: M_PI) The "aperture" or field-of-view of the
                    //!range finder, in radians (typically M_PI = 180 degrees).
    bool rightToLeft; //!< (Default: true) The scanning direction:
                      //!true=counterclockwise; false=clockwise
    float maxRange; //!< (Default: 80) The maximum range allowed by the device,
                    //!in meters (e.g. 80m, 50m,...)
    mrpt::poses::CPose3D sensorPose; //!< (Default: at origin) The 6D pose of
                                     //!the sensor on the robot at the moment of
                                     //!starting the scan.
    size_t nRays;
    float rangeNoiseStd; //!< (Default: 0) The standard deviation of measurement
                         //!noise. If not desired, set to 0
    float angleNoiseStd; //!< (Default: 0) The sigma of an optional Gaussian
                         //!noise added to the angles at which ranges are
                         //!measured (in radians)
    unsigned int decimation; //!< (Default: 1) The rays that will be simulated
                             //!are at indexes: 0, D, 2D, 3D,...
    float threshold; //!< (Default: 0.6f) The minimum occupancy threshold to
                     //!consider a cell to be occupied
    /** @} */
  ...
  struct MAPS_IMPEXP TLaserSimulUncertaintyResult {
    mrpt::obs::CObservation2DRangeScanWithUncertainty
        scanWithUncert; //!< The scan + its uncertainty

    TLaserSimulUncertaintyResult();
  };

  /** Like laserScanSimulatorWithUncertainty() (see it for a discussion of most
  * parameters) but taking into account
   *  the robot pose uncertainty and generating a vector of predicted variances
  * for each ray.
   *  Range uncertainty includes both, sensor noise and large non-linear effects
  * caused by borders and discontinuities in the environment
  ...
  * COccupancyGridMap2D::RAYTRACE_STEP_SIZE_IN_CELL_UNITS
  ...
  double computeLikelihoodField_Thrun(
      const CPointsMap *pm, const mrpt::poses::CPose2D *relativePose = nullptr);
   ...
   * coordinates, or nullptr for (0,0,0).
    ...
    */
  double
  computeLikelihoodField_II(const CPointsMap *pm,
  ...
  /** Saves a composite image with two gridmaps and lines representing a set of
  ...
  /** Saves the gridmap as a graphical bitmap file, 8 bit gray scale, 1 pixel is
   * 1 cell, and with an overlay of landmarks.
   * \note The template parameter CLANDMARKSMAP is assumed to be
   * mrpt::maps::CLandmarksMap normally.
   * \return False on any error.
   */
  template <class CLANDMARKSMAP>
  bool saveAsBitmapFileWithLandmarks(const std::string &file,
                                     const CLANDMARKSMAP *landmarks,
                                     bool addTextLabels = false,
                                     const mrpt::utils::TColor &marks_color =
   ...
   * respectively.
    ...
    */
  void getAsImage(utils::CImage &img, bool verticalFlip = false,
   ...
   * respectively.
    * \sa getAsImage
    */
  void getAsImageFiltered(utils::CImage &img, bool verticalFlip = false,
  ...
  void getAsPointCloud(mrpt::maps::CSimplePointsMap &pm,
                       const float occup_threshold = 0.5f) const;

  /** Returns true upon map construction or after calling clear(), the return
    *  changes to false upon successful insertObservation() or any other method
   * to load data in the map.
    */
   ...
   * upwards</b> on the image) for the pixel which will be taken at coordinates
   * origin (0,0). (Default: the center of the image)
   * \return False on any error.
   * \sa loadFromBitmap
   */
  bool loadFromBitmapFile(const std::string &file, float resolution,
                          float xCentralPixel = -1, float yCentralPixel = -1);

  /** Load the gridmap from a image in a file (the format can be any supported
   * by CImage::loadFromFile).
   *  See loadFromBitmapFile() for the meaning of parameters */
  bool loadFromBitmap(const mrpt::utils::CImage &img, float resolution,
                      float xCentralPixel = -1, float yCentralPixel = -1);
   ...
   * NOTICE: That the "z" dimension is ignored in the points. Clip the points as
   * appropiated if needed before calling this method.
   *
   * \sa computeMatching3DWith
   */
  virtual void
  determineMatching2D(const mrpt::maps::CMetricMap *otherMap,
  ...
  float
  compute3DMatchingRatio(const mrpt::maps::CMetricMap *otherMap,
  ...
  struct MAPS_IMPEXP TCriticalPointsList {
    TCriticalPointsList()
        : x(), y(), clearance(), x_basis1(), y_basis1(), x_basis2(),
          y_basis2() {}

    std::vector<int> x, y; //!< The coordinates of critical point.
    std::vector<int>
        clearance; //!< The clearance of critical points, in 1/100 of cells.
    std::vector<int> x_basis1, y_basis1, x_basis2,
        y_basis2; //!< Their two first basis points coordinates.
  } CriticalPointsList;
...
private:
  ...
  float min_x, max_x, min_y, max_y,
      resolution; //!< See COccupancyGridMap2D::COccupancyGridMap2D
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(COccupancyGridMap2D, CMetricMap,
                                             MAPS_IMPEXP)

bool MAPS_IMPEXP operator<(const COccupancyGridMap2D::TPairLikelihoodIndex &e1,
