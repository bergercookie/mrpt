...
/** A 2D grid map representing the reflectivity of the environment (for example,
  ...
  *		- Automatic resizing of the map limits when inserting observations
  *close to the border.
  *
  *   Each cell contains the up-to-date average height from measured falling in
  *that cell. Algorithms that can be used:
  *		- mrSimpleAverage: Each cell only stores the current average
  *value.
...
protected:
  static CLogOddsGridMapLUT<cell_t>
...
public:
  ...
  /** Constructor  */
  CReflectivityGridMap2D(double x_min = -2, double x_max = 2, double y_min = -2,
...
protected:
  ...
  double min_x, max_x, min_y, max_y,
      resolution; //!< See
                  //!CReflectivityGridMap2DOptions::CReflectivityGridMap2DOptions
  mrpt::maps::CReflectivityGridMap2D::TInsertionOptions insertionOpts;
  MAP_DEFINITION_END(CReflectivityGridMap2D, MAPS_IMPEXP)
};
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CReflectivityGridMap2D, CMetricMap,
                                             MAPS_IMPEXP)
