...
public:
  /** Constructor
    */
  CGasConcentrationGridMap2D(TMapRepresentation mapType = mrAchim,
  ...
  struct MAPS_IMPEXP TInsertionOptions : public utils::CLoadableOptions,
    ...
    /** @name For all mapping methods
        @{ */
    std::string gasSensorLabel; //!< The label of the CObservationGasSensor used
                                //!to generate the map
    uint16_t enose_id; //!< id for the enose used to generate this map (must be
                       //!< gasGrid_count)
    uint16_t gasSensorType; //!< The sensor type for the gas concentration map
                            //!(0x0000 ->mean of all installed sensors, 0x2600,
                            //!0x6810, ...)
    std::string windSensorLabel; //!< The label of the WindSenor used to
                                 //!simulate advection

    //[Advection Options]
    bool useWindInformation; //! Indicates if wind information must be used to
                             //! simulate Advection
    float advectionFreq; //! Frequency for simulating advection (only used to
                         //! transform wind speed to distance)
    float std_windNoise_phi, std_windNoise_mod; //! The std to consider on wind
  ...
  virtual void
  getAs3DObject(mrpt::opengl::CSetOfObjects::Ptr &meanObj,
...
protected:
  ...
  */
  ...
  mrpt::maps::CGasConcentrationGridMap2D::TMapRepresentation
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CGasConcentrationGridMap2D,
                                             CRandomFieldGridMap2D, MAPS_IMPEXP)
