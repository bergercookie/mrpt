...
class COctoMapBase : public mrpt::maps::CMetricMap
{
	...
	*/
	struct TInsertionOptions : public utils::CLoadableOptions
	{
		 ...
		 * only
		 * in limited situations, since get*() methods don't work, etc. */
		TInsertionOptions();
		...
		/// threshold
		/// for occupancy clamping (sensor model) (Default=0.1192, -2 in log
		/// odds)
		...
		/// threshold
		/// for occupancy clamping (sensor model) (Default=0.971, 3.5 in log
		/// odds)
	   ...
	   private:
		...
		double
			occupancyThres;  // sets the threshold for occupancy (sensor model)
		// (Default=0.5)
		double
			probHit;  // sets the probablility for a "hit" (will be converted to
		// logodds) - sensor model (Default=0.7)
		double
			probMiss;  // sets the probablility for a "miss" (will be converted
		// to logodds) - sensor model (Default=0.4)
	...
	*/
	...
	struct TRenderingOptions
	{
		 ...
		 * "structure"
		 * of the octree, but computationally costly (Default: false) */
