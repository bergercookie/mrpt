...
public:
  typedef COctoMapBase<OCTREE, OCTREE_NODE>
      myself_t; //!< The type of this MRPT class
  typedef OCTREE
      octree_t; //!< The type of the octree class in the "octomap" library.
  typedef OCTREE_NODE octree_node_t; //!< The type of nodes in the octree, in
                                     //!the "octomap" library.
  ...
  */
  struct TInsertionOptions : public utils::CLoadableOptions {
    ...
    TInsertionOptions(); //!< Especial constructor, not attached to a real
                         //!COctoMap object: used only in limited situations,
                         //!since get*() methods don't work, etc.
    TInsertionOptions &operator=(const TInsertionOptions &o) {
      ...
      setOccupancyThres(o_has_parent ? o.getOccupancyThres()
                                     : o.occupancyThres);
      setProbHit(o_has_parent ? o.getProbHit() : o.probHit);
      setProbMiss(o_has_parent ? o.getProbMiss() : o.probMiss);
      setClampingThresMin(o_has_parent ? o.getClampingThresMin()
                                       : o.clampingThresMin);
      setClampingThresMax(o_has_parent ? o.getClampingThresMax()
                                       : o.clampingThresMax);
      return *this;
    }
    ...
    bool pruning; //!< whether the tree is (losslessly) pruned after insertion
                  //!(default: true)
    ...
    void setOccupancyThres(double prob) {
      if (m_parent.get())
        m_parent->m_octomap.setOccupancyThres(prob);
    }
    /// (key name in .ini files: "probHit")sets the probablility for a "hit"
    /// (will be converted to logodds) - sensor model (Default=0.7)
    void setProbHit(double prob) {
      if (m_parent.get())
        m_parent->m_octomap.setProbHit(prob);
    }
    /// (key name in .ini files: "probMiss")sets the probablility for a "miss"
    /// (will be converted to logodds) - sensor model (Default=0.4)
    void setProbMiss(double prob) {
      if (m_parent.get())
        m_parent->m_octomap.setProbMiss(prob);
    }
    /// (key name in .ini files: "clampingThresMin")sets the minimum threshold
    /// for occupancy clamping (sensor model) (Default=0.1192, -2 in log odds)
    void setClampingThresMin(double thresProb) {
      if (m_parent.get())
        m_parent->m_octomap.setClampingThresMin(thresProb);
    }
    /// (key name in .ini files: "clampingThresMax")sets the maximum threshold
    /// for occupancy clamping (sensor model) (Default=0.971, 3.5 in log odds)
  ...
  private:
    ...
    double occupancyThres; // sets the threshold for occupancy (sensor model)
                           // (Default=0.5)
    double probHit;  // sets the probablility for a "hit" (will be converted to
                     // logodds) - sensor model (Default=0.7)
    double probMiss; // sets the probablility for a "miss" (will be converted to
                     // logodds) - sensor model (Default=0.4)
    double clampingThresMin; // sets the minimum threshold for occupancy
                             // clamping (sensor model) (Default=0.1192, -2 in
                             // log odds)
    double clampingThresMax; // sets the maximum threshold for occupancy
                             // clamping (sensor model) (Default=0.971, 3.5 in
                             // log odds)
  };
  ...
  */
  struct TLikelihoodOptions : public utils::CLoadableOptions {
    ...
    void
    writeToStream(mrpt::utils::CStream &out) const; //!< Binary dump to stream
    ...
    uint32_t decimation; //!< Speed up the likelihood computation by considering
                         //!only one out of N rays (default=1)
  };
  ...
  struct TRenderingOptions {
    bool generateGridLines; //!< Generate grid lines for all octree nodes,
                            //!useful to draw the "structure" of the octree, but
                            //!computationally costly (Default: false)

    bool generateOccupiedVoxels; //!< Generate voxels for the occupied volumes
                                 //!(Default=true)
    ...
    TRenderingOptions()
        : generateGridLines(false), generateOccupiedVoxels(true),
          visibleOccupiedVoxels(true), generateFreeVoxels(true),
   ...
   * mrpt::opengl::COctoMapVoxels object.
          ...
          */
  virtual void
  getAsOctoMapVoxels(mrpt::opengl::COctoMapVoxels &gl_obj) const = 0;
   ...
   * "prob" is undefined. */
  bool getPointOccupancy(const float x, const float y, const float z,
   ...
   * insertionOptions */
  void updateVoxel(const double x, const double y, const double z,
                   bool occupied) {
   ...
   * CMetricMap::insertObservation()
    */
  void insertPointCloud(const CPointsMap &ptMap, const float sensor_x,
                        const float sensor_y, const float sensor_z);

  /** Just like insertPointCloud but with a single ray. */
  void insertRay(const float end_x, const float end_y, const float end_z,
                 const float sensor_x, const float sensor_y,
                 const float sensor_z) {
    m_octomap.insertRay(octomap::point3d(sensor_x, sensor_y, sensor_z),
                        octomap::point3d(end_x, end_y, end_z),
   ...
   */
  bool castRay(const mrpt::math::TPoint3D &origin,
...
protected:
  ...
  bool internal_build_PointCloud_for_observation(
      ...
      octomap::point3d &point3d_sensorPt, octomap::Pointcloud &ptr_scan) const;
