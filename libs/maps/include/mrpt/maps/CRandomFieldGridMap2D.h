...
struct MAPS_IMPEXP TRandomFieldCell {
  ...
  TRandomFieldCell(double kfmean_dm_mean = 1e-20, double kfstd_dmmeanw = 0)
      : kf_mean(kfmean_dm_mean), kf_std(kfstd_dmmeanw), dmv_var_mean(0),
        last_updated(mrpt::system::now()), updated_std(kfstd_dmmeanw) {}
  ...
  union {
    ...
    double dm_mean; //!< [Kernel-methods only] The cumulative weighted readings
                    //!of this cell
  ...
  union {
    double
        kf_std; //!< [KF-methods only] The standard deviation value of this cell
    double dm_mean_w; //!< [Kernel-methods only] The cumulative weights
                      //!(concentration = alpha * dm_mean / dm_mean_w +
                      //!(1-alpha)*r0 )
    double gmrf_std;
  };

  double dmv_var_mean; //!< [Kernel DM-V only] The cumulative weighted variance
                       //!of this cell

  mrpt::system::TTimeStamp last_updated; //!< [Dynamic maps only] The timestamp
                                         //!of the last time the cell was
                                         //!updated
  double updated_std; //!< [Dynamic maps only] The std cell value that was
                      //!updated (to be used in the Forgetting_curve
};
...
/** CRandomFieldGridMap2D represents a 2D grid map where each cell is associated
  ...
  *Lilienthal, A. and Duckett, T., Robotics and Autonomous Systems, v.48, 2004.
  *		- `mrKernelDMV`: A kernel-based method. See:
  *			- "A Statistical Approach to Gas Distribution Modelling with
  *Mobile Robots--The Kernel DM+ V Algorithm", Lilienthal, A.J. and Reggente, M.
  *and Trincavelli, M. and Blanco, J.L. and Gonzalez, J., IROS 2009.
  *		- `mrKalmanFilter`: A "brute-force" approach to estimate the entire
  *map with a dense (linear) Kalman filter. Will be very slow for mid or large
  *maps. It's provided just for comparison purposes, not useful in practice.
  *		- `mrKalmanApproximate`: A compressed/sparse Kalman filter
  *approach. See:
  *			- "A Kalman Filter Based Approach to Probabilistic Gas
  *Distribution Mapping", JL Blanco, JG Monroy, J Gonzalez-Jimenez, A
  *Lilienthal, 28th Symposium On Applied Computing (SAC), 2013.
  *		- `mrGMRF_SD`: A Gaussian Markov Random Field (GMRF) estimator,
  *with these constraints:
  *			- `mrGMRF_SD`: Each cell only connected to its 4 immediate
  *neighbors (Up, down, left, right).
  *			- (Removed in MRPT 1.5.0: `mrGMRF_G`: Each cell connected to a
  *square area of neighbors cells)
  *			- See papers:
  *				- "Time-variant gas distribution mapping with obstacle
  *information", Monroy, J. G., Blanco, J. L., & Gonzalez-Jimenez, J. Autonomous
  *Robots, 40(1), 1-16, 2016.
  *
...
public:
  ...
  /** The type of map representation to be used, see CRandomFieldGridMap2D for a
   * discussion.
    */
  enum TMapRepresentation {
    mrKernelDM = 0,      //!< Gaussian kernel-based estimator (see discussion in
                         //!mrpt::maps::CRandomFieldGridMap2D)
    mrAchim = 0,         //!< Another alias for "mrKernelDM", for backwards
                         //!compatibility (see discussion in
                         //!mrpt::maps::CRandomFieldGridMap2D)
    mrKalmanFilter,      //!< "Brute-force" Kalman filter (see discussion in
                         //!mrpt::maps::CRandomFieldGridMap2D)
    mrKalmanApproximate, //!< (see discussion in
                         //!mrpt::maps::CRandomFieldGridMap2D)
    mrKernelDMV, //!< Double mean + variance Gaussian kernel-based estimator
                 //!(see discussion in mrpt::maps::CRandomFieldGridMap2D)
    // Removed in MRPT 1.5.0: mrGMRF_G,   //!< Gaussian Markov Random Field,
    // Gaussian prior weights between neighboring cells up to a certain distance
    // (see discussion in mrpt::maps::CRandomFieldGridMap2D)
    mrGMRF_SD //!< Gaussian Markov Random Field, squared differences prior
              //!weights between 4 neighboring cells (see discussion in
              //!mrpt::maps::CRandomFieldGridMap2D)
  };

  /** Constructor */
  CRandomFieldGridMap2D(TMapRepresentation mapType = mrKernelDM,
  ...
  /** Returns true if the map is empty/no observation has been inserted (in this
   * class it always return false,
  ...
  /** Like saveAsBitmapFile(), but returns the data in matrix form (first row in
   * the matrix is the upper (y_max) part of the map) */
  ...
  struct MAPS_IMPEXP TInsertionOptionsCommon {
    ...
    void internal_dumpToTextStream_common(
        mrpt::utils::CStream &out) const; //!< See utils::CLoadableOptions
    ...
    double dm_sigma_omega; //!< [DM/DM+V methods] The scaling parameter for the
                           //!confidence "alpha" values (see the IROS 2009
                           //!paper; see CRandomFieldGridMap2D) */
                           /** @} */

    /** @name Kalman-filter methods (mrKalmanFilter, mrKalmanApproximate)
        @{ */
    float KF_covSigma; //!< The "sigma" for the initial covariance value between
                       //!cells (in meters).
    float KF_initialCellStd; //!< The initial standard deviation of each cell's
                             //!concentration (will be stored both at each
                             //!cell's structure and in the covariance matrix as
                             //!variances in the diagonal) (in normalized
                             //!concentration units).
    float KF_observationModelNoise; //!< The sensor model noise (in normalized
                                    //!concentration units).
    float KF_defaultCellMeanValue; //!< The default value for the mean of cells'
                                   //!concentration.
    uint16_t KF_W_size; //!< [mrKalmanApproximate] The size of the window of
                        //!neighbor cells.
                        /** @} */
    ...
    /** @name Gaussian Markov Random Fields methods (mrGMRF_SD)
        @{ */
    double
        GMRF_lambdaPrior; //!< The information (Lambda) of fixed map constraints
    double GMRF_lambdaObs; //!< The initial information (Lambda) of each
                           //!observation (this information will decrease with
                           //!time)
    double GMRF_lambdaObsLoss; //!< The loss of information of the observations
                               //!with each iteration

    bool GMRF_use_occupancy_information; //!< whether to use information of an
                                         //!occupancy_gridmap map for building
                                         //!the GMRF
    std::string
        GMRF_simplemap_file; //!< simplemap_file name of the occupancy_gridmap
    std::string
        GMRF_gridmap_image_file;   //!< image name of the occupancy_gridmap
    double GMRF_gridmap_image_res; //!< occupancy_gridmap resolution: size of
                                   //!each pixel (m)
    size_t GMRF_gridmap_image_cx;  //!< Pixel coordinates of the origin for the
                                   //!occupancy_gridmap
    size_t GMRF_gridmap_image_cy;  //!< Pixel coordinates of the origin for the
                                   //!occupancy_gridmap

    double GMRF_saturate_min, GMRF_saturate_max; //!< (Default:-inf,+inf)
                                                 //!Saturate the estimated mean
                                                 //!in these limits
    bool GMRF_skip_variance; //!< (Default:false) Skip the computation of the
                             //!variance, just compute the mean
                             /** @} */
  };

  /** Changes the size of the grid, maintaining previous contents. \sa setSize
   */
  virtual void resize(double new_x_min, double new_x_max, double new_y_min,
                      double new_y_max,
   ...
   * insertionOptions
    * \sa resize
    */
  virtual void setSize(const double x_min, const double x_max,
  ...
  struct MAPS_IMPEXP ConnectivityDescriptor {
     ...
     * updated in out_edge_information), false otherwise.
      */
    virtual bool
    getEdgeInformation(const CRandomFieldGridMap2D
                           *parent, //!< The parent map on which we are running
                       size_t icx,
                       size_t icy,                  //!< (cx,cy) for node "i"
                       size_t jcx, size_t jcy,      //!< (cx,cy) for node "j"
                       double &out_edge_information //!< Must output here the
  ...
  virtual void getAs3DObject(mrpt::opengl::CSetOfObjects::Ptr &meanObj,
                             mrpt::opengl::CSetOfObjects::Ptr &varObj) const;

  TMapRepresentation getMapType(); //!< Return the type of the random-field grid
                                   //!map, according to parameters passed on
                                   //!construction.
  ...
  void insertIndividualReading(
      ...
      const bool update_map = true,      //!< [in] Run a global map update after
                                         //!inserting this observatin
                                         //!(algorithm-dependant)
      const bool time_invariant = true,  //!< [in] Whether the observation
                                         //!"vanishes" with time (false) or not
                                         //!(true) [Only for GMRF methods]
      const double reading_stddev = .0   //!< [in] The uncertainty (standard
                                         //!deviation) of the reading.
                                         //!Default="0.0" means use the default
                                         //!settings per map-wide parameters.
      );
  ...
  virtual void predictMeasurement(
      ...
      bool do_sensor_normalization, //!< [in] Whether to renormalize the
                                    //!prediction to a predefined interval (`R`
                                    //!values in insertionOptions)
      const TGridInterpolationMethod interp_method =
          gimNearest //!< [in] Interpolation method
      );

  /** Return the mean and covariance vector of the full Kalman filter estimate
   * (works for all KF-based methods). */
  void getMeanAndCov(mrpt::math::CVectorDouble &out_means,
                     mrpt::math::CMatrixDouble &out_cov) const;

  /** Return the mean and STD vectors of the full Kalman filter estimate (works
   * for all KF-based methods). */
  void getMeanAndSTD(mrpt::math::CVectorDouble &out_means,
                     mrpt::math::CVectorDouble &out_STD) const;

  /** Load the mean and STD vectors of the full Kalman filter estimate (works
   * for all KF-based methods). */
  void setMeanAndSTD(mrpt::math::CVectorDouble &out_means,
                     mrpt::math::CVectorDouble &out_STD);

  void updateMapEstimation(); //!< Run the method-specific procedure required to
                              //!ensure that the mean & variances are up-to-date
                              //!with all inserted observations.
...
protected:
   ...
   * derived-class instance of "insertOptions" upon construction of this class.
   */
  ...
  TMapRepresentation m_mapType; //!< The map representation type of this map, as
                                //!passed in the constructor

  mrpt::math::CMatrixD m_cov; //!< The whole covariance matrix, used for the
                              //!Kalman Filter map representation.

  /** The compressed band diagonal matrix for the KF2 implementation.
    *   The format is a Nx(W^2+2W+1) matrix, one row per cell in the grid map
   * with the
    *    cross-covariances between each cell and half of the window around it in
   * the grid.
    */
  ...
  struct TObservationGMRF
    ...
    double
        Lambda; //!< "Information" of the observation (=inverse of the variance)
    bool time_invariant; //!< whether the observation will lose weight (lambda)
                         //!as time goes on (default false)
  ...
  // Important: converted to a std::list<> so pointers are NOT invalidated upon
  // deletion.
  std::vector<std::list<TObservationGMRF>>
      m_mrf_factors_activeObs; //!< Vector with the active observations and
                               //!their respective Information
  std::deque<TPriorFactorGMRF> m_mrf_factors_priors; //!< Vector with the
                                                     //!precomputed priors for
                                                     //!each GMRF model
   ...
   * models DM & DM+V.
    ...
    */
  void insertObservation_KernelDM_DMV(double normReading,
   ...
   * map model.
    ...
    * \param point Is the sensor location on the map
    */
  void insertObservation_KF(double normReading,
                            const mrpt::math::TPoint2D &point);

  /** The implementation of "insertObservation" for the Efficient Kalman Filter
   * map model.
    * \param normReading Is a [0,1] normalized concentration reading.
    * \param point Is the sensor location on the map
    */
  void insertObservation_KF2(double normReading,
                             const mrpt::math::TPoint2D &point);
   ...
   * Field map model.
    ...
    * \param point Is the sensor location on the map
    */
  void insertObservation_GMRF(double normReading,
                              const mrpt::math::TPoint2D &point,
                              const bool update_map, const bool time_invariant,
                              const double reading_information);

  /** solves the minimum quadratic system to determine the new concentration of
   * each cell */
  ...
  /** Computes the estimated variance of the cell concentration, or the overall
   * average variance if it has never been observed  */
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CRandomFieldGridMap2D, CMetricMap,
                                             MAPS_IMPEXP)
...
struct TEnumTypeFiller<maps::CRandomFieldGridMap2D::TMapRepresentation> {
  ...
  static void fill(bimap<enum_t, std::string> &m_map) {
    ...
    m_map.insert(maps::CRandomFieldGridMap2D::mrKalmanApproximate,
                 "mrKalmanApproximate");
