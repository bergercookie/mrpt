   ...
   public:
	...
	enum TMapRepresentation
	{
		...
		/** Double mean + variance Gaussian kernel-based estimator (see
		   discussion
		   in mrpt::maps::CRandomFieldGridMap2D) */
		mrKernelDMV,
		/** Gaussian Markov Random Field, Gaussian prior weights between
		   neighboring
		   cells up to a certain distance (see discussion in
		   mrpt::maps::CRandomFieldGridMap2D) */
		// Removed in MRPT 1.5.0: mrGMRF_G,
		/** Gaussian Markov Random Field, squared differences prior weights
		   between
		   4 neighboring cells (see discussion in
		   mrpt::maps::CRandomFieldGridMap2D)
		   */
		mrGMRF_SD
	};
	...
	virtual void predictMeasurement(
		...
		/** [in] Whether to renormalize the prediction to a predefined interval
		   (`R` values in insertionOptions) */
