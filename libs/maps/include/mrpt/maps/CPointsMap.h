							   ...
							   public mrpt::utils::PLY_Exporter
{
   ...
   protected:
	...
	struct MAPS_IMPEXP TLaserRange3DInsertContext
	{
		 ...
		 * local
		 * coordinates of the scan points being inserted right now. */
   ...
   public:
	...
	struct MAPS_IMPEXP TInsertionOptions : public utils::CLoadableOptions
	{
		 ...
		 * from
		 * the scan are loaded, clearing all previous content. Default is false.
		 */
		 ...
		 * inserted
		 * scan and inside the free space, are erased: i.e. they don't exist
		 * yet. */
		 ...
		 * considered
		 * horizontal, considered only when isPlanarMap=true (default=0). */
	...
	struct MAPS_IMPEXP TLikelihoodOptions : public utils::CLoadableOptions
	{
		 ...
		 * the
		 * likelihood (default= 0.5^2 meters) */
		double sigma_dist;
	 ...
	 *each
	 *one in a map, to be considered the same one and be fused rather than
	 *added.
	 ...
	 * ALL
	 * the points in the map (the results are buffered, such as, if the map is
	 * not
