...
protected:
  ...
  struct MAPS_IMPEXP TLaserRange2DInsertContext {
    ...
    mrpt::math::CMatrixDouble44
        HM; //!< Homog matrix of the local sensor pose within the robot
    const mrpt::obs::CObservation2DRangeScan &rangeScan;
    std::vector<float>
        fVars; //!< Extra variables to be used as desired by the derived class.
  ...
  struct MAPS_IMPEXP TLaserRange3DInsertContext {
    ...
    float scan_x, scan_y,
        scan_z; //!< In \a internal_loadFromRangeScan3D_prepareOneRange, these
                //!are the local coordinates of the scan points being inserted
                //!right now.
    std::vector<float>
        fVars; //!< Extra variables to be used as desired by the derived class.
    std::vector<unsigned int> uVars;
...
public:
  ...
  // --------------------------------------------
  /** @name Pure virtual interfaces to be implemented by any class derived from
     CPointsMap
          @{ */

  /** Reserves memory for a given number of points: the size of the map does not
   * change, it only reserves the memory.
   ...
   * bounds
    ...
    */
  virtual void getPointAllFieldsFast(const size_t index,
                                     std::vector<float> &point_data) const = 0;
   ...
   * bounds
    ...
    */
  virtual void setPointAllFieldsFast(const size_t index,
                                     const std::vector<float> &point_data) = 0;

protected:
  /** Auxiliary method called from within \a addFrom() automatically, to finish
   * the copying of class-specific data  */
  virtual void addFrom_classSpecific(const CPointsMap &anotherMap,
...
public:
   ...
   * correspondence in the map.
    */
  virtual float squareDistanceToClosestCorrespondence(float x0,
                                                      float y0) const override;

  inline float
  squareDistanceToClosestCorrespondenceT(const mrpt::math::TPoint2D &p0) const {
    return squareDistanceToClosestCorrespondence(static_cast<float>(p0.x),
  ...
  struct MAPS_IMPEXP TInsertionOptions : public utils::CLoadableOptions {
    ...
    float minDistBetweenLaserPoints; //!< The minimum distance between points
                                     //!(in 3D): If two points are too close,
                                     //!one of them is not inserted into the
                                     //!map. Default is 0.02 meters.
    bool addToExistingPointsMap; //!< Applicable to "loadFromRangeScan" only! If
                                 //!set to false, the points from the scan are
                                 //!loaded, clearing all previous content.
                                 //!Default is false.
    bool also_interpolate;       //!< If set to true, far points (<1m) are
                                 //!interpolated with samples at
                           //!"minDistSqrBetweenLaserPoints" intervals (Default
                           //!is false).
    bool disableDeletion; //!< If set to false (default=true) points in the same
                          //!plane as the inserted scan and inside the free
                          //!space, are erased: i.e. they don't exist yet.
    bool fuseWithExisting; //!< If set to true (default=false), inserted points
                           //!are "fused" with previously existent ones. This
                           //!shrink the size of the points map, but its slower.
    bool isPlanarMap; //!< If set to true, only HORIZONTAL (in the XY plane)
                      //!measurements will be inserted in the map (Default value
                      //!is false, thus 3D maps are generated). \sa
                      //!horizontalTolerance
    float horizontalTolerance; //!< The tolerance in rads in pitch & roll for a
                               //!laser scan to be considered horizontal,
                               //!considered only when isPlanarMap=true
                               //!(default=0).
    float maxDistForInterpolatePoints; //!< The maximum distance between two
                                       //!points to interpolate between them
                                       //!(ONLY when also_interpolate=true)
    bool insertInvalidPoints; //!< Points with x,y,z coordinates set to zero
                              //!will also be inserted

    void writeToStream(mrpt::utils::CStream &out) const; //!< Binary dump to
                                                         //!stream - for usage
                                                         //!in derived classes'
                                                         //!serialization
    void readFromStream(mrpt::utils::CStream &in); //!< Binary dump to stream -
                                                   //!for usage in derived
  ...
  /** Options used when evaluating "computeObservationLikelihood" in the derived
   * classes.
  ...
  struct MAPS_IMPEXP TLikelihoodOptions : public utils::CLoadableOptions {
    ...
    double sigma_dist; //!< Sigma squared (variance, in meters) of the
                       //!exponential used to model the likelihood (default=
                       //!0.5^2 meters)
    double max_corr_distance; //!< Maximum distance in meters to consider for
                              //!the numerator divided by "sigma_dist", so that
                              //!each point has a minimum (but very small)
                              //!likelihood to avoid underflows (default=1.0
                              //!meters)
    uint32_t decimation; //!< Speed up the likelihood computation by considering
                         //!only one out of N rays (default=10)
  };
  ...
  /** Adds all the points from \a anotherMap to this map, without fusing.
    ...
    *  \endcode
    * \note The method in CPointsMap is generic but derived classes may redefine
   * this virtual method to another one more optimized.
    */
   ...
   * \sa fuseWith, addFrom
   */
  void insertAnotherMap(const CPointsMap *otherMap,
  ...
  /** Load from a text file. Each line should contain an "X Y" coordinate pair,
  ...
  virtual bool savePCDFile(const std::string &filename,
                           bool save_as_binary) const;
   ...
   * \return The return value is the weight of the point (the times it has been
   * fused), or 1 if weights are not used.
  ...
  /** Access to a given point from map, and its colors, if the map defines them
   * (othersise, R=G=B=1.0). First index is 0.
   * \return The return value is the weight of the point (the times it has been
   * fused)
   * \exception Throws std::exception on index out of bound.
   */
  virtual void getPoint(size_t index, float &x, float &y, float &z, float &R,
  ...
  virtual void setPoint(size_t index, float x, float y, float z, float R,
                        float G, float B) {
    MRPT_UNUSED_PARAM(R);
    MRPT_UNUSED_PARAM(G);
    MRPT_UNUSED_PARAM(B);
  ...
  /// Gets the point weight, which is ignored in all classes (defaults to 1) but
  /// in those which actually store that field (Note: No checks are done for
  /// out-of-bounds index).  \sa setPointWeight
   ...
   * points in the map.
    */
  void getPointsBuffer(size_t &outPointsCount, const float *&xs,
                       const float *&ys, const float *&zs) const;
  ...
  void getAllPoints(VECTOR &xs, VECTOR &ys, VECTOR &zs,
   ...
   * saved in the output, effectively performing a subsampling of the points.
    * \sa setAllPoints
    */
  void getAllPoints(std::vector<float> &xs, std::vector<float> &ys,
                    size_t decimation = 1) const;

  inline void getAllPoints(std::vector<mrpt::math::TPoint2D> &ps,
                           size_t decimation = 1) const {
  ...
  /// overload (RGB data is ignored in classes without color information)
  virtual void insertPoint(float x, float y, float z, float R, float G,
                           float B) {
    MRPT_UNUSED_PARAM(R);
    MRPT_UNUSED_PARAM(G);
    MRPT_UNUSED_PARAM(B);
  ...
  inline void setAllPointsTemplate(const VECTOR &X, const VECTOR &Y,
   ...
   * getAllPoints */
  inline void setAllPoints(const std::vector<float> &X,
   ...
   * implementation class this can be [X Y Z] or [X Y Z R G B], etc...
    ...
    */
  void getPointAllFields(const size_t index,
                         std::vector<float> &point_data) const {
    ASSERT_BELOW_(index, this->size())
   ...
   * bounds
    * \sa setPointAllFields, getPointAllFields, getPointAllFieldsFast
    */
  void setPointAllFields(const size_t index,
  ...
  // See docs in base class
  virtual void
  determineMatching3D(const mrpt::maps::CMetricMap *otherMap,
  ...
  /** Computes the matchings between this and another 3D points map.
     This method matches each point in the other map with the centroid of the 3
   closest points in 3D
     from this map (if the distance is below a defined threshold).
   * \param  otherMap					  [IN] The other map to compute
   the matching with.
   * \param  otherMapPose				  [IN] The pose of the other map as
   seen from "this".
   * \param  maxDistForCorrespondence   [IN] Maximum 2D linear distance between
   two points to be matched.
   * \param  correspondences			  [OUT] The detected matchings
   pairs.
   ...
   */
  void compute3DDistanceToMesh(const mrpt::maps::CMetricMap *otherMap2,
                               const mrpt::poses::CPose3D &otherMapPose,
  ...
  /** Transform the range scan into a set of cartessian coordinated
    ...
    */
  virtual void
  loadFromRangeScan(const mrpt::obs::CObservation2DRangeScan &rangeScan,
                    const mrpt::poses::CPose3D *robotPose = nullptr) = 0;

  /** Overload of \a loadFromRangeScan() for 3D range scans (for example, Kinect
   * observations).
    *
    * \param rangeScan The scan to be inserted into this map
    * \param robotPose Default to (0,0,0|0deg,0deg,0deg). Changes the frame of
   ...
   * at each derived class.
    ...
    */
  virtual void
  loadFromRangeScan(const mrpt::obs::CObservation3DRangeScan &rangeScan,
                    const mrpt::poses::CPose3D *robotPose = nullptr) = 0;

  /** Like \a loadFromRangeScan() for Velodyne 3D scans. Points are translated
   * and rotated according to the \a sensorPose field in the observation and, if
   * provided, to the \a robotPose parameter.
    *
   ...
   * coordinates).
    ...
    */
  void loadFromVelodyneScan(const mrpt::obs::CObservationVelodyneScan &scan,
   ...
   * \param otherMap The other map whose points are to be inserted into this
   *one.
   * \param minDistForFuse Minimum distance (in meters) between two points, each
   *one in a map, to be considered the same one and be fused rather than added.
   ...
   */
  void fuseWith(CPointsMap *anotherMap, float minDistForFuse = 0.02f,
   ...
   * p_i \f$ by \f$ p'_i = b \oplus p_i \f$ (pose compounding operator).
    */
  void changeCoordinatesReference(const CPointsMap &other,
   ...
   * COLOR_3DSCENE_R,COLOR_3DSCENE_G,COLOR_3DSCENE_B
    ...
    */
  virtual void
  getAs3DObject(mrpt::opengl::CSetOfObjects::Ptr &outObj) const override;

  /** If the map is a simple points map or it's a multi-metric map that contains
   * EXACTLY one simple points map, return it.
          * Otherwise, return NULL
          */
  virtual const mrpt::maps::CSimplePointsMap *
  getAsSimplePointsMap() const override {
    return nullptr;
  }
   ...
   * points, such as a sphere centered at the origin with this radius cover ALL
   * the points in the map (the results are buffered, such as, if the map is not
   * modified, the second call will be much faster than the first one). */
  float getLargestDistanceFromOrigin() const;

  /** Like \a getLargestDistanceFromOrigin() but returns in \a output_is_valid =
   * false if the distance was not already computed, skipping its computation
   * then, unlike getLargestDistanceFromOrigin() */
   ...
   * calculations.
    */
  void boundingBox(float &min_x, float &max_x, float &min_y, float &max_y,
                   float &min_z, float &max_z) const;

  inline void boundingBox(mrpt::math::TPoint3D &pMin,
                          mrpt::math::TPoint3D &pMax) const {
   ...
   * provided radius and zmin/zmax values.
    */
  void extractCylinder(const mrpt::math::TPoint2D &center, const double radius,
  ...
  /** Extracts the points in the map within the area defined by two corners.
    ...
    */
  void extractPoints(const mrpt::math::TPoint3D &corner1,
                     const mrpt::math::TPoint3D &corner2, CPointsMap *outMap,
                     const double &R = 1, const double &G = 1,
                     const double &B = 1);
  ...
  /** Get the min/max Z levels for points to be actually inserted in the map \sa
   ...
   * (PointCloud<PointXYZ>).
    ...
    *  \endcode
    * \sa setFromPCLPointCloud, CColouredPointsMap::getPCLPointCloudXYZRGB (for
   * color data)
    */
  ...
  void setFromPCLPointCloud(const POINTCLOUD &cloud) {
    ...
    reserve(N);
    for (size_t i = 0; i < N; ++i)
      this->insertPointFast(cloud.points[i].x, cloud.points[i].y,
                            cloud.points[i].z);
  }
  ...
  template <typename BBOX> bool kdtree_get_bbox(BBOX &bb) const {
    float min_z, max_z;
    this->boundingBox(bb[0].low, bb[0].high, bb[1].low, bb[1].high, min_z,
                      max_z);
    if (bb.size() == 3) {
      bb[2].low = min_z;
      bb[2].high = max_z;
...
protected:
  ...
  mrpt::obs::CSinCosLookUpTableFor2DScans m_scans_sincos_cache; //!< Cache of
                                                                //!sin/cos
                                                                //!values for
                                                                //!the latest 2D
                                                                //!scan
  ...
  /** This is a common version of CMetricMap::insertObservation() for point maps
   * (actually, CMetricMap::internal_insertObservation),
   ...
   * which are accepted. */
  bool
  internal_insertObservation(const mrpt::obs::CObservation *obs,
  ...
  virtual void
  PLY_export_get_vertex(const size_t idx, mrpt::math::TPoint3Df &pt,
                        bool &pt_has_color,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CPointsMap, CMetricMap,
                                             MAPS_IMPEXP)
...
class PointCloudAdapter<mrpt::maps::CPointsMap>
    : public detail::PointCloudAdapterHelperNoRGB<mrpt::maps::CPointsMap,
                                                  float> {
private:
...
public:
  ...
  inline void setPointXYZ(const size_t idx, const coords_t x, const coords_t y,
                          const coords_t z) {
