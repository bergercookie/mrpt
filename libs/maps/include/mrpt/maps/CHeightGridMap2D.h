...
struct MAPS_IMPEXP THeightGridmapCell {
  ...
  float u;    //!< Auxiliary variable for storing the incremental mean value (in
              //!meters).
  float v;    //!< Auxiliary (in meters)
  uint32_t w; //!< [For mrSimpleAverage model] The accumulated weight: initially
              //!zero if un-observed, increased by one for each observation
...
public:
  ...
  /** Constructor */
  CHeightGridMap2D(TMapRepresentation mapType = mrSimpleAverage,
                   double x_min = -2, double x_max = 2, double y_min = -2,
                   double y_max = 2, double resolution = 0.1);

  bool isEmpty() const override; //!< Returns true if the map is empty/no
                                 //!observation has been inserted.
  ...
  struct MAPS_IMPEXP TInsertionOptions : public utils::CLoadableOptions {
    ...
    bool filterByHeight; //!< Whether to perform filtering by z-coordinate
                         //!(default=false): coordinates are always RELATIVE to
                         //!the robot for this filter.vvv
    float z_min, z_max;  //!< Only when filterByHeight is true: coordinates are
                         //!always RELATIVE to the robot for this filter.
  ...
  virtual bool dem_get_z_by_cell(const size_t cx, const size_t cy,
                                 double &z_out) const override;
  virtual bool dem_get_z(const double x, const double y,
                         double &z_out) const override;
  ...
  double min_x, max_x, min_y, max_y,
      resolution; //!< See CHeightGridMap2D::CHeightGridMap2D
  mrpt::maps::CHeightGridMap2D::TMapRepresentation
      mapType; //!< The kind of map representation (see
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CHeightGridMap2D, CMetricMap,
                                             MAPS_IMPEXP)
...
namespace utils {
template <> struct TEnumTypeFiller<maps::CHeightGridMap2D::TMapRepresentation> {
