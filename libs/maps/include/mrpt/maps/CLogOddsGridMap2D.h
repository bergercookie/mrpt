...
template <> struct logoddscell_traits<int8_t> {
  static const int8_t CELLTYPE_MIN = -127; // In mrpt <0.9.4 was -128 (!) - This
                                           // is to make it compatible with all
                                           // architectures.
...
struct CLogOddsGridMap2D : public detail::logoddscell_traits<TCELL> {
   ...
   * saturation.
    ...
    *  \param logodd_obs Observation of the cell, in log-odd form as transformed
   * by p2l.
    *  \param thres  This must be CELLTYPE_MIN+logodd_obs
    * \sa updateCell, updateCell_fast_free
   */
  inline static void
  updateCell_fast_occupied(const unsigned x, const unsigned y,
                           const cell_t logodd_obs, const cell_t thres,
                           cell_t *mapArray, const unsigned _size_x) {
   ...
   * by p2l.
    ...
    * \sa updateCell, updateCell_fast_free
   */
  inline static void updateCell_fast_occupied(cell_t *theCell,
   ...
   */
  inline static void updateCell_fast_free(const unsigned x, const unsigned y,
                                          const cell_t logodd_obs,
                                          const cell_t thres, cell_t *mapArray,
   ...
   * by p2l.
    ...
    * \sa updateCell_fast_occupied
   */
  inline static void updateCell_fast_free(cell_t *theCell,
                                          const cell_t logodd_obs,
                                          const cell_t thres) {
    if (*theCell < thres)
      *theCell += logodd_obs;
...
struct CLogOddsGridMapLUT : public detail::logoddscell_traits<TCELL> {
  ...
  inline float l2p(const cell_t l) {
    if (l < traits_t::CELLTYPE_MIN)
      return logoddsTable[0]; // This is needed since min can be -127 and int8_t
                              // can be -128.
    else
  ...
  inline uint8_t l2p_255(const cell_t l) {
    if (l < traits_t::CELLTYPE_MIN)
      return logoddsTable_255[0]; // This is needed since min can be -127 and
                                  // int8_t can be -128.
    else
   ...
   * log(p)-log(1-p)  in the valid range of cell_t.
    */
