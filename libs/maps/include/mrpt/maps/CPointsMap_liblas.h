...
struct LAS_HeaderInfo {
  ...
  std::string spatial_reference_proj4; //!< Proj.4 string describing the Spatial
                                       //!Reference System.
...
bool saveLASFile(const POINTSMAP &ptmap, const std::string &filename,
                 const LAS_WriteParams &params = LAS_WriteParams()) {
  std::ofstream ofs;
  ofs.open(filename.c_str(), std::ios::out | std::ios::binary);

  if (!ofs.is_open()) {
    std::cerr << "[saveLASFile] Couldn't write to file: " << filename
              << std::endl;
    return false;
  }
  ...
  for (size_t i = 0; i < nPts; i++) {
    ...
    if (!writer.WritePoint(pt)) {
      std::cerr << "[saveLASFile] liblas returned error writing point #" << i
                << " to file.\n";
      return false;
    }
  }
 ...
 * derived class mrpt::maps::CColouredPointsMap
        * \return false on any error */
template <class POINTSMAP>
bool loadLASFile(POINTSMAP &ptmap, const std::string &filename,
...
#else
  out_headerInfo.project_guid =
      boost::lexical_cast<std::string>(header.GetProjectId());
#endif
  ...
  while (reader.ReadNextPoint()) {
    ...
    if (has_color) {
      ...
      ptmap.insertPoint(p.GetX(), p.GetY(), p.GetZ(), col.GetRed() * col_fract,
  ...
  if (ptmap.size() != nPts)
    cerr << "[loadLASFile] Header says point count is " << nPts << " but only "
         << ptmap.size() << " were really parsed in.\n";
