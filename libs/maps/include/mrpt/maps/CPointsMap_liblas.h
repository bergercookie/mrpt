	...
	const LAS_WriteParams& params = LAS_WriteParams())
{
	...
	for (size_t i = 0; i < nPts; i++)
	{
		...
		{
			std::cerr << "[saveLASFile] liblas returned error writing point #"
					  << i << " to file.\n";
			return false;
		}
	}
	...
	const LAS_LoadParams& params = LAS_LoadParams())
{
	...
	if (!ifs.is_open())
	{
		std::cerr << "[loadLASFile] Couldn't open file: " << filename
				  << std::endl;
		return false;
	}
	...
	if (ptmap.size() != nPts)
		cerr << "[loadLASFile] Header says point count is " << nPts
			 << " but only " << ptmap.size() << " were really parsed in.\n";
