...
public:
   ...
   * colour is undefined. */
  bool getPointColour(const float x, const float y, const float z, uint8_t &r,
                      uint8_t &g, uint8_t &b) const;

  /** Manually update the colour of the voxel at (x,y,z) */
  void updateVoxelColour(const double x, const double y, const double z,
  ...
  virtual void
  getAsOctoMapVoxels(mrpt::opengl::COctoMapVoxels &gl_obj) const override;
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CColouredOctoMap, CMetricMap,
                                             MAPS_IMPEXP)
