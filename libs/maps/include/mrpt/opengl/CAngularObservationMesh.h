...
public:
  ...
  private:
    /**
      ...
      * If 1, it's specified by an initial and a final value, and a fixed size
     * of samples.
  ...
  public:
    ...
    /**
      * Creates a range of values from the initial value, the final value and
     * the increment.
      * \throw std::logic_error if the increment is zero.
      */
    inline static TDoubleRange CreateFromIncrement(double initial, double final,
                                                   double increment) {
      if (increment == 0)
    ...
    /**
      * Creates a range of values from the initial value, the final value and a
     * desired amount of samples.
      */
    inline static TDoubleRange CreateFromAmount(double initial, double final,
                                                size_t amount) {
      return TDoubleRange(initial, final, amount);
    }
    /**
      * Creates a zero-centered range of values from an aperture, an amount of
     * samples and a direction.
      */
    inline static TDoubleRange
    CreateFromAperture(double aperture, size_t amount, bool negToPos = true) {
    ...
    inline double aperture() const {
      ...
      case 0:
        return (mrpt::utils::sign(rangeData.mode0.increment) ==
                mrpt::utils::sign(rangeData.mode0.final -
    ...
    inline double initialValue() const {
      ...
      case 2:
        return rangeData.mode2.negToPos ? -rangeData.mode2.aperture / 2
        ...
        return (mrpt::utils::sign(rangeData.mode0.increment) ==
                mrpt::utils::sign(rangeData.mode0.final -
                   ...
                   ? 1 + static_cast<size_t>(ceil(
                             (rangeData.mode0.final - rangeData.mode0.initial) /
    ...
    /**
      * Returns the direction of the scan. True if the increment is positive,
     * false otherwise.
      * \throw std::logic_error on invalid range type.
      */
    inline bool negToPos() const {
      ...
      case 1:
        return mrpt::utils::sign(rangeData.mode1.final -
...
protected:
  void updateMesh() const; //!< Updates the mesh, if needed. It's a const
                           //!method, but modifies mutable content.
  mutable std::vector<CSetOfTriangles::TTriangle>
      triangles; //!< Actual set of triangles to be displayed.
  void addTriangle(const mrpt::math::TPoint3D &p1,
                   const mrpt::math::TPoint3D &p2,
                   const mrpt::math::TPoint3D &p3)
      const;       //!< Internal method to add a triangle to the mutable mesh.
  bool mWireframe; //!< Whether the mesh will be displayed wireframe or solid.
  mutable bool meshUpToDate; //!< Mutable variable which controls if the object
                             //!has suffered any change since last time the mesh
                             //!was updated.
  bool mEnableTransparency;  //!< Whether the object may present transparencies
                             //!or not.
  mutable mrpt::math::CMatrixTemplate<mrpt::math::TPoint3D>
      actualMesh; //!< Mutable object with the mesh's points.
  mutable mrpt::math::CMatrixB validityMatrix; //!< Scan validity matrix.
  std::vector<double> pitchBounds; //!< Observation pitch range. When containing
                                   //!exactly two elements, they represent the
                                   //!bounds.
  std::vector<mrpt::obs::CObservation2DRangeScan>
      scanSet; //!< Actual scan set which is used to generate the mesh.
public:
  /**
    * Basic constructor.
    */
  CAngularObservationMesh()
      : mWireframe(true), meshUpToDate(false), mEnableTransparency(true),
  ...
  /**
    * Traces a ray to the object, returning the distance to a given pose through
   * its X axis.
  ...
  /**
    ...
    */
  void getActualMesh(mrpt::math::CMatrixTemplate<mrpt::math::TPoint3D> &pts,
...
private:
  ...
  public:
    FTrace1D(const T &s, const mrpt::poses::CPose3D &p, std::vector<double> &v,
             std::vector<char> &v2)
  ...
  public:
    FTrace2D(const T &s, const mrpt::poses::CPose3D &p,
    ...
    void operator()(double pitch) {
      ...
      values.reserve(nY);
      valid.reserve(nY);
      for_each(yValues.begin(), yValues.end(),
...
public:
  /**
    * 2D ray tracing (will generate a 3D mesh). Given an object and two ranges,
   * realizes a scan from the initial pose and stores it in a
   ...
   * its.
    ...
    */
  template <class T>
  static void trace2DSetOfRays(const T &e, const mrpt::poses::CPose3D &initial,
                               CAngularObservationMesh::Ptr &caom,
                               const TDoubleRange &pitchs,
                               const TDoubleRange &yaws);
  /**
    * 2D ray tracing (will generate a vectorial mesh inside a plane). Given an
   * object and a range, realizes a scan from the initial pose and stores it in
   * a CObservation2DRangeScan object.
    * The objective may be a COpenGLScene, a CRenderizable or any children of
   * its.
    * \sa mrpt::opengl::CRenderizable,mrpt::opengl::COpenGLScene.
    */
  template <class T>
  static void trace1DSetOfRays(const T &e, const mrpt::poses::CPose3D &initial,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CAngularObservationMesh,
...
void CAngularObservationMesh::trace2DSetOfRays(
  ...
  for_each(pValues.begin(), pValues.end(),
