...
MAP_DEFINITION_REGISTER("CHeightGridMap2D,heightMap,dem",
...
void CHeightGridMap2D::TMapDefinition::dumpToTextStream_map_specific(
  ...
  out.printf(
      "MAP TYPE                                  = %s\n",
      mrpt::utils::TEnumType<CHeightGridMap2D::TMapRepresentation>::value2name(
          mapType)
          .c_str());
...
/*---------------------------------------------------------------
                                                Constructor
  ---------------------------------------------------------------*/
CHeightGridMap2D::CHeightGridMap2D(TMapRepresentation mapType, double x_min,
                                   double x_max, double y_min, double y_max,
                                   double resolution)
    : CDynamicGrid<THeightGridmapCell>(x_min, x_max, y_min, y_max, resolution),
      insertionOptions(), m_mapType(mapType) {}
...
bool CHeightGridMap2D::internal_insertObservation(const CObservation *obs,
    ...
    for (size_t x = 0; x < m_size_x; x++)
      for (size_t y = 0; y < m_size_y; y++) {
        ...
        if (c->w) {
          ...
          colormap(insertionOptions.colorMap, // cmJET, //cmGRAYSCALE,
                   col_idx, r, g, b);
...
bool CHeightGridMap2D::dem_get_z_by_cell(const size_t cx, const size_t cy,
                                         double &z_out) const {
...
bool CHeightGridMap2D::dem_get_z(const double x, const double y,
                                 double &z_out) const {
