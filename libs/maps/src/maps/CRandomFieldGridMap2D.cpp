...
/*---------------------------------------------------------------
                                                Constructor
  ---------------------------------------------------------------*/
CRandomFieldGridMap2D::CRandomFieldGridMap2D(TMapRepresentation mapType,
                                             double x_min, double x_max,
                                             double y_min, double y_max,
                                             double resolution)
    : CDynamicGrid<TRandomFieldCell>(x_min, x_max, y_min, y_max, resolution),
      COutputLogger("CRandomFieldGridMap2D"),
      m_rfgm_run_update_upon_clear(true), m_insertOptions_common(nullptr),
      m_mapType(mapType), m_cov(0, 0), m_hasToRecoverMeanAndCov(true),
      m_DM_lastCutOff(0), m_average_normreadings_mean(0),
      m_average_normreadings_var(0), m_average_normreadings_count(0) {
...
void CRandomFieldGridMap2D::setSize(const double x_min, const double x_max,
                                    const double y_min, const double y_max,
                                    const double resolution,
                                    const TRandomFieldCell *fill_value) {
  CDynamicGrid<TRandomFieldCell>::setSize(x_min, x_max, y_min, y_max,
...
void CRandomFieldGridMap2D::internal_clear() {
    ...
    for (size_t i = 0; i < m_cov.getRowCount(); i++) {
      ...
      for (size_t j = i; j < m_cov.getColCount(); j++) {
        ...
        if (i == j) {
          ...
          m_cov(i, j) =
              std0sqr *
              exp(-0.5 * (res2 * static_cast<double>(square(cx1 - cx2) +
                                                     square(cy1 - cy2))) /
                  KF_covSigma2);
          m_cov(j, i) = m_cov(i, j);
        }
  ...
  case mrKalmanApproximate: {
    ...
    MRPT_LOG_DEBUG("[CRandomFieldGridMap2D::clear] Resetting compressed cov. "
                   "matrix and cells\n");
    TRandomFieldCell def(
        m_insertOptions_common->KF_defaultCellMeanValue, // mean
        m_insertOptions_common->KF_initialCellStd        // std
      ...
      if (i == 0) {
        ...
        *ptr++ = std0sqr;
        ...
        for (Acx = 1; Acx <= W; Acx++)
          *ptr++ = std0sqr *
                   exp(-0.5 *
                       (res2 * static_cast<double>(square(Acx) + square(Acy))) /
                       KF_covSigma2);
    ...
    MRPT_LOG_DEBUG_FMT("[clear] %ux%u cells done in %.03fms\n",
                       unsigned(m_size_x), unsigned(m_size_y),
                       1000 * tictac.Tac());
  } break;
  ...
  case mrGMRF_SD: {
    ...
    MRPT_LOG_DEBUG_FMT("[clear] Initial map dimension: %u cells, x=(%.2f,%.2f) "
                       "y=(%.2f,%.2f)\n",
    ...
    if (this->m_insertOptions_common
            ->GMRF_use_occupancy_information) { // Load Occupancy Gridmap and
                                                // resize
      if (!m_insertOptions_common->GMRF_simplemap_file.empty()) {
        mrpt::maps::CSimpleMap simpleMap;
        CFileGZInputStream(this->m_insertOptions_common->GMRF_simplemap_file) >>
            simpleMap;
        ...
        res_coef = this->getResolution() /
                   this->m_insertOptions_common->GMRF_gridmap_image_res;
      } else {
        THROW_EXCEPTION("Neither `simplemap_file` nor `gridmap_image_file` "
      ...
      resize(m_Ocgridmap.getXMin(), m_Ocgridmap.getXMax(),
             m_Ocgridmap.getYMin(), m_Ocgridmap.getYMax(), def, 0.0);

      MRPT_LOG_DEBUG_FMT(
          "Occupancy Gridmap dimensions: x=(%.2f,%.2f)m y=(%.2f,%.2f)m \n",
          m_Ocgridmap.getXMin(), m_Ocgridmap.getXMax(), m_Ocgridmap.getYMin(),
          m_Ocgridmap.getYMax());
      MRPT_LOG_DEBUG_FMT(
          "Occupancy Gridmap dimensions: %u cells, cx=%i cy=%i\n\n",
          static_cast<unsigned int>(m_Ocgridmap.getSizeX() *
                                    m_Ocgridmap.getSizeY()),
    ...
    const size_t nPriorFactors =
        ...
        this->getSizeX() *
            (this->getSizeY() - 1); // L = (Nr-1)*Nc + Nr*(Nc-1); Full connected

    MRPT_LOG_DEBUG_STREAM("[clear] Generating "
                          << nPriorFactors
                          << " prior factors for a map size of N=" << nodeCount
                          << endl);
    ...
    if (!m_gmrf_connectivity &&
      ...
      // Use region growing algorithm to determine the cell interconnections
      // (Factors)
      size_t cx = 0;
      size_t cy = 0;
      ...
      std::multimap<size_t, size_t> cell_interconnections; // Store the
                                                           // interconnections
                                                           // (relations) of
                                                           // each cell with its
                                                           // neighbourds
      ...
      for (size_t j = 0; j < nodeCount; j++) // For each cell in the map
      {
        ...
        // If cell occpuped then add fake observation: to allow all cells having
        // a solution
        if (m_Ocgridmap.getCell(seed_cxo, seed_cyo) < 0.5) {
          ...
          m_gmrf.addConstraint(
              *m_mrf_factors_activeObs[j].rbegin()); // add to graph
        }
        ...
        for (int neighbor = 0; neighbor < 2; neighbor++) {
          ...
          // Check using Region growing if cell j is connected to cell i
          // (Occupancy gridmap)
          if (exist_relation_between2cells(&m_Ocgridmap, cxo_min, cxo_max,
                                           cyo_min, cyo_max, seed_cxo, seed_cyo,
      ...
      if (custom_connectivity != nullptr)
        MRPT_LOG_DEBUG("[CRandomFieldGridMap2D::clear] Initiating prior (using "
                       "user-supplied connectivity pattern)");
      else
        MRPT_LOG_DEBUG("[CRandomFieldGridMap2D::clear] Initiating prior (fully "
                       "connected)");
        ...
        for (int dir = 0; dir < 2; dir++) {
          ...
          if (custom_connectivity != nullptr) {
            const bool is_connected = custom_connectivity->getEdgeInformation(
                this, cx, cy, cx + (dir == 0 ? 1 : 0), cy + (dir == 1 ? 1 : 0),
                edge_lamdba);
            if (!is_connected)
              continue;
          } else {
    ...
    MRPT_LOG_DEBUG_STREAM("[clear] Prior built in " << tictac.Tac()
                                                    << " s ----------");
...
void CRandomFieldGridMap2D::insertObservation_KernelDM_DMV(
  ...
  if (m_DM_lastCutOff != m_insertOptions_common->cutoffRadius ||
    ...
    MRPT_LOG_DEBUG_FMT("[CRandomFieldGridMap2D::insertObservation_KernelDM_DMV]"
    ...
    for (unsigned cx = 0; cx < Ac_all; cx++) {
      for (unsigned cy = 0; cy < Ac_all; cy++) {
        dist = m_resolution *
               sqrt(static_cast<double>(square(Ac_cutoff + 1 - cx) +
...
CRandomFieldGridMap2D::TInsertionOptionsCommon::TInsertionOptionsCommon()
    : sigma(0.15f), cutoffRadius(sigma * 3.0), R_min(0), R_max(3),
      dm_sigma_omega(
      ...
      GMRF_lambdaPrior(0.01f), // [GMRF model] The information (Lambda) of fixed
                               // map constraints
      GMRF_lambdaObs(10.0f), // [GMRF model] The initial information (Lambda) of
                             // each observation (this information will decrease
                             // with time)
      GMRF_lambdaObsLoss(0.0f), //!< The loss of information of the observations
                                //!with each iteration

      GMRF_use_occupancy_information(false), GMRF_simplemap_file(""),
      GMRF_gridmap_image_file(""), GMRF_gridmap_image_res(0.01f),
      GMRF_gridmap_image_cx(0), GMRF_gridmap_image_cy(0),
...
void CRandomFieldGridMap2D::TInsertionOptionsCommon::
  ...
  KF_initialCellStd = iniFile.read_float(section.c_str(), "KF_initialCellStd",
                                         KF_initialCellStd);
  ...
  GMRF_lambdaPrior =
      iniFile.read_float(section.c_str(), "GMRF_lambdaPrior", GMRF_lambdaPrior);
  GMRF_lambdaObs =
      iniFile.read_float(section.c_str(), "GMRF_lambdaObs", GMRF_lambdaObs);
  GMRF_lambdaObsLoss = iniFile.read_float(section.c_str(), "GMRF_lambdaObsLoss",
                                          GMRF_lambdaObsLoss);
...
void CRandomFieldGridMap2D::getAsMatrix(
  ...
  for (unsigned int y = 0; y < m_size_y; y++) {
    for (unsigned int x = 0; x < m_size_x; x++) {
      ...
      mrpt::utils::saturate(c, m_insertOptions_common->GMRF_saturate_min,
...
/*---------------------------------------------------------------
                                        getAsBitmapFile
 ---------------------------------------------------------------*/
void CRandomFieldGridMap2D::getAsBitmapFile(
...
/*---------------------------------------------------------------
                                        resize
 ---------------------------------------------------------------*/
void CRandomFieldGridMap2D::resize(double new_x_min, double new_x_max,
  ...
  CDynamicGrid<TRandomFieldCell>::resize(new_x_min, new_x_max, new_y_min,
                                         new_y_max, defaultValueNewCells,
  ...
  if (m_size_x != old_sizeX || m_size_y != old_sizeY) {
    ...
    if (m_mapType == mrKalmanFilter) {
      ...
      printf("[CRandomFieldGridMap2D::resize] Resizing from %ux%u to %ux%u (%u "
             "cells)\n",
             static_cast<unsigned>(old_sizeX), static_cast<unsigned>(old_sizeY),
             static_cast<unsigned>(m_size_x), static_cast<unsigned>(m_size_y),
        ...
        if (C1_isFromOldMap) {
          for (j = i; j < N; j++) {
            ...
            if (C1_isFromOldMap && C2_isFromOldMap) {
              ...
              unsigned int idx_c1 =
                  ((cx1 - Acx_left) + old_sizeX * (cy1 - Acy_bottom));
              unsigned int idx_c2 =
                  ((cx2 - Acx_left) + old_sizeX * (cy2 - Acy_bottom));
              ...
              if (i == j && m_cov(i, i) < 0) {
                printf("\ni=%u \nj=%i \ncx1,cy1 = %u,%u \n cx2,cy2=%u,%u "
                       ...
                       static_cast<unsigned>(cx1), static_cast<unsigned>(cy1),
                       static_cast<unsigned>(cx2), static_cast<unsigned>(cy2),
                       static_cast<unsigned>(idx_c1),
                       static_cast<unsigned>(idx_c2),
                       static_cast<unsigned>(Acx_left),
                       static_cast<unsigned>(Acy_bottom),
      ...
      for (i = 0; i < N; i++) {
        ...
        for (j = i; j < N; j++) {
          ...
          // If both cells were NOT in the old map??? --> Introduce default
          // values:
          if (!C1_isFromOldMap || !C2_isFromOldMap) {
            ...
            if (i == j) {
              ...
              m_cov(i, j) =
                  K * exp(-0.5 *
                          square(dist / m_insertOptions_common->KF_covSigma));
              m_cov(j, i) = m_cov(i, j);
            }
      ...
      for (size_t i = N - 1; i < N; i--) // i<N will become false for "i=-1" ;-)
      {
        ...
        if (cell.kf_std < 0) {
          // "i" is a new cell, fix its std and fill out the compressed
          // covariance:
...
void CRandomFieldGridMap2D::insertObservation_KF(
  ...
  for (i = 0; i < N; i++) {
    ...
    for (j = i; j < N; j++) {
      ...
      if (i == j) {
        if (m_cov(i, i) < 0) {
          printf("Wrong insertion in KF! m_cov(%u,%u) = %.5f",
...
// Save as a bitmap:
  ...
  DIMs.saveToTextFile(filNamePrefix + std::string("_grid_limits.txt"),
                      MATRIX_FORMAT_FIXED, false /* add mrpt header */,
  ...
  case mrKernelDMV: {
    ...
    all_means.saveToTextFile(filNamePrefix + std::string("_mean.txt"),
                             MATRIX_FORMAT_FIXED);
    if (m_mapType == mrKernelDMV) {
      all_vars.saveToTextFile(filNamePrefix + std::string("_var.txt"),
                              MATRIX_FORMAT_FIXED);
      all_confs.saveToTextFile(filNamePrefix + std::string("_confidence.txt"),
                               MATRIX_FORMAT_FIXED);
    }
  } break;
  ...
  case mrKalmanApproximate: {
    ...
    MEAN.saveToTextFile(filNamePrefix + std::string("_mean.txt"),
                        MATRIX_FORMAT_FIXED);
    STDs.saveToTextFile(filNamePrefix + std::string("_cells_std.txt"),
                        MATRIX_FORMAT_FIXED);

    if (m_mapType == mrKalmanApproximate) {
      m_stackedCov.saveToTextFile(filNamePrefix +
                                      std::string("_mean_compressed_cov.txt"),
                                  MATRIX_FORMAT_FIXED);
    }
    if (m_mapType == mrKalmanFilter) {
    ...
    img_cov.saveToFile(filNamePrefix + std::string("_cells_std.png"),
                       true /* vertical flip */);
  ...
  case mrGMRF_SD: {
    ...
    STDs.saveToTextFile(filNamePrefix + std::string("_cells_std.txt"),
                        MATRIX_FORMAT_FIXED);
    XYZ.saveToTextFile(
        filNamePrefix + std::string("_xyz_and_std.txt"), MATRIX_FORMAT_FIXED,
        false,
...
void CRandomFieldGridMap2D::saveAsMatlab3DGraph(
  ...
  for (cy = 0; cy < m_size_y; cy++) {
    for (cx = 0; cx < m_size_x; cx++) {
      const TRandomFieldCell *cell = cellByIndex(cx, cy);
      ASSERT_(cell != nullptr);
      os::fprintf(f, "%e ", mrpt::utils::saturate_val(
  ...
  os::fprintf(f, "set(gca,'PlotBoxAspectRatio',[%f %f %f]);\n",
              m_x_max - m_x_min, m_y_max - m_y_min, 4.0);
  os::fprintf(f, "view(-40,30)\n");
  os::fprintf(f, "axis([%f %f %f %f 0 1]);\n", m_x_min, m_x_max, m_y_min,
              m_y_max);

  os::fprintf(f, "\nfigure; imagesc(xs,ys,z_mean);axis equal;title('Mean "
                 "value');colorbar;");
  os::fprintf(f, "\nfigure; imagesc(xs,ys,(z_upper-z_mean)/%f);axis "
                 "equal;title('Std dev of estimated value');colorbar;",
              std_times);
...
void CRandomFieldGridMap2D::getAs3DObject(
  ...
  case mrGMRF_SD: {
    ...
    // ---------------------------------------
    double maxVal_m = 0, minVal_m = 1, AMaxMin_m, maxVal_v = 0, minVal_v = 1,
           AMaxMin_v;
    double c, v;
    ...
    for (cy = 1; cy < m_size_y; cy++) {
      for (cx = 1; cx < m_size_x; cx++) {
        ...
        const TRandomFieldCell *cell_x_1y_1 = cellByIndex(cx - 1, cy - 1);
        ASSERT_(cell_x_1y_1 != nullptr);

        // MEAN values
        //-----------------
        double c_xy = mrpt::utils::saturate_val(
            cell_xy->kf_mean, m_insertOptions_common->GMRF_saturate_min,
            m_insertOptions_common->GMRF_saturate_max);
        double c_x_1y = mrpt::utils::saturate_val(
            cell_x_1y->kf_mean, m_insertOptions_common->GMRF_saturate_min,
            m_insertOptions_common->GMRF_saturate_max);
        double c_xy_1 = mrpt::utils::saturate_val(
            cell_xy_1->kf_mean, m_insertOptions_common->GMRF_saturate_min,
            m_insertOptions_common->GMRF_saturate_max);
        double c_x_1y_1 = mrpt::utils::saturate_val(
            cell_x_1y_1->kf_mean, m_insertOptions_common->GMRF_saturate_min,
            m_insertOptions_common->GMRF_saturate_max);
        ...
        col_x_1y =
            v_x_1y / 10; // min(1.0,max(0.0, (v_x_1y-minVal_v)/AMaxMin_v ) );
        col_xy_1 =
            v_xy_1 / 10; // min(1.0,max(0.0, (v_xy_1-minVal_v)/AMaxMin_v ) );
        col_x_1y_1 = v_x_1y_1 /
                     10; // min(1.0,max(0.0, (v_x_1y_1-minVal_v)/AMaxMin_v ) );
  ...
  case mrKernelDMV: {
    ...
    for (cy = 1; cy < m_size_y; cy++) {
      for (cx = 1; cx < m_size_x; cx++) {
        ...
        double v_x_1y =
            min(1.0, max(0.0, computeVarCellValue_DM_DMV(cell_x_1y)));
        double v_xy_1 =
            min(1.0, max(0.0, computeVarCellValue_DM_DMV(cell_xy_1)));
        double v_x_1y_1 =
            min(1.0, max(0.0, computeVarCellValue_DM_DMV(cell_x_1y_1)));
...
double CRandomFieldGridMap2D::computeConfidenceCellValue_DM_DMV(
  ...
  return 1.0 - std::exp(-square(cell->dm_mean_w /
                                m_insertOptions_common->dm_sigma_omega));
}
...
double CRandomFieldGridMap2D::computeMeanCellValue_DM_DMV(
  ...
  // A confidence measure:
  const double alpha =
      1.0 - std::exp(-square(cell->dm_mean_w /
                             m_insertOptions_common->dm_sigma_omega));
  const double r_val =
...
void CRandomFieldGridMap2D::predictMeasurement(
  ...
  case gimBilinear:
    if (x <= m_x_min + m_resolution * 0.5 ||
      ...
      // Weights:
      queries[0].coef =
          K_1 * (idx2x(queries[3].cx) - x) * (idx2y(queries[3].cy) - y);
      queries[1].coef =
          K_1 * (idx2x(queries[3].cx) - x) * (y - idx2y(queries[0].cy));
      queries[2].coef =
          K_1 * (x - idx2x(queries[0].cx)) * (idx2y(queries[3].cy) - y);
      queries[3].coef =
          K_1 * (x - idx2x(queries[0].cx)) * (y - idx2y(queries[0].cy));
    }
    break;
  default:
...
void CRandomFieldGridMap2D::insertObservation_KF2(
  ...
  MRPT_LOG_DEBUG_STREAM("Inserting KF2: (" << normReading << ") at Position"
                                           << point << endl);
  ...
  resize(point.x - Aspace, point.x + Aspace, point.y - Aspace, point.y + Aspace,
         defCell, Aspace);
  ...
  CVectorDouble cross_covs_c_i(W21sqr, 0); // Indexes are relative to the
                                           // (2W+1)x(2W+1) window centered at
                                           // "cellIdx".
  vector_int window_idxs(W21sqr, -1); // The real-map indexes for each element
                                      // in the window, or -1 if there are out
                                      // of the map (for cells close to the
                                      // border)
  ...
  for (size_t i = 0; i < W21sqr; i++) {
    ...
    for (size_t j = i; j < W21sqr; j++) {
      ...
      if (Ax > W)
        continue; // Next cell (may be more rows after the current one...)

      const int Ay = cy_j - cy_i;
      if (Ay > W)
        break; // We are absolutely sure out of i's window.
      ...
      if (i == j && cov_to_change < Delta_cov)
        THROW_EXCEPTION_FMT("Negative variance value appeared! Please increase "
...
/*---------------------------------------------------------------
                                        getMeanAndCov
  ---------------------------------------------------------------*/
void CRandomFieldGridMap2D::getMeanAndCov(CVectorDouble &out_means,
                                          CMatrixDouble &out_cov) const {
...
/*---------------------------------------------------------------
                                        getMeanAndSTD
  ---------------------------------------------------------------*/
void CRandomFieldGridMap2D::getMeanAndSTD(CVectorDouble &out_means,
                                          CVectorDouble &out_STD) const {
  const size_t N = BASE::m_map.size();
  out_means.resize(N);
  out_STD.resize(N);
...
/*---------------------------------------------------------------
                                        setMeanAndSTD
  ---------------------------------------------------------------*/
void CRandomFieldGridMap2D::setMeanAndSTD(CVectorDouble &in_means,
                                          CVectorDouble &in_std) {
...
void CRandomFieldGridMap2D::updateMapEstimation_GMRF() {
  ...
  ASSERT_(m_insertOptions_common->GMRF_skip_variance ||
          size_t(m_map.size()) == size_t(x_var.size()));

  // Update Mean-Variance in the base grid class
  for (size_t j = 0; j < m_map.size(); j++) {
    m_map[j].gmrf_std =
        m_insertOptions_common->GMRF_skip_variance ? .0 : std::sqrt(x_var[j]);
    m_map[j].gmrf_mean += x_incr[j];

    mrpt::utils::saturate(m_map[j].gmrf_mean,
                          m_insertOptions_common->GMRF_saturate_min,
                          m_insertOptions_common->GMRF_saturate_max);
  }
        ...
        if (matExp(row, col) == 1) {
          ...
          for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
              ...
              if ((int(row) + j >= 0) &&
                  (int(row) + j <= int(matExp.getRowCount() - 1)) &&
                  (int(col) + i >= 0) &&
                  (int(col) + i <= int(matExp.getColCount()) - 1)) {
                if (!((i == 0 && j == 0) || !(matExp(row + j, col + i) == 0))) {
                  // check if expand
                  if ((m_Ocgridmap->getCell(row + cxo_min, col + cyo_min) <
                       0.5) ==
                      (m_Ocgridmap->getCell(row + j + cxo_min,
                                            col + i + cyo_min) < 0.5)) {
...
void CRandomFieldGridMap2D::TObservationGMRF::evalJacobian(
    double &dr_dx) const {
  dr_dx = 1.0;
}
// ============ TPriorFactorGMRF ===========
