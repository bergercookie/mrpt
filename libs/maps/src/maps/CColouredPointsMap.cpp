...
MAP_DEFINITION_REGISTER("CColouredPointsMap,colourPointsMap",
...
void CColouredPointsMap::TMapDefinition::loadFromConfigFile_map_specific(
  ...
  colourOpts.loadFromConfigFile(source,
                                sectionNamePrefix + string("_colorOpts"));
}
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CColouredPointsMap::writeToStream(mrpt::utils::CStream &out,
                                       int *version) const {
  if (version)
    *version = 9;
  else {
 ...
 * \exception Throws std::exception on index out of bound.
 */
void CColouredPointsMap::setPointColor(size_t index, float R, float G,
                                       float B) {
  if (index >= this->x.size())
    THROW_EXCEPTION("Index out of bounds");
  this->m_color_R[index] = R;
...
void CColouredPointsMap::insertPoint(float x, float y, float z, float R,
                                     float G, float B) {
  this->x.push_back(x);
  this->y.push_back(y);
  this->z.push_back(z);
...
/*---------------------------------------------------------------
                                        getPoint
 ---------------------------------------------------------------*/
unsigned long CColouredPointsMap::getPoint(size_t index, float &x, float &y,
                                           float &z) const {
  if (index >= this->x.size())
    THROW_EXCEPTION("Index out of bounds");
...
/*---------------------------------------------------------------
                                        getPoint
 ---------------------------------------------------------------*/
void CColouredPointsMap::getPoint(size_t index, float &x, float &y, float &z,
...
/** Retrieves a point color (colors range is [0,1])
  */
void CColouredPointsMap::getPointColor(size_t index, float &R, float &G,
                                       float &B) const {
  if (index >= this->x.size())
    THROW_EXCEPTION("Index out of bounds");
...
/*---------------------------------------------------------------
                                        getPoint
 ---------------------------------------------------------------*/
bool CColouredPointsMap::colourFromObservation(const CObservationImage &obs,
  ...
  for (size_t k = 0; k < p_idx.size(); k++) {
    ...
    {
      TPixelCoordf px;
      aux_projectPoint_with_distortion(TPoint3D(x[idx], y[idx], z[idx]),
                                       obs.cameraParams, px, true);
  ...
  for (itProPoints = projectedPoints.begin(), k = 0;
    ...
    if (itProPoints->x >= 0 && itProPoints->x < imgW && itProPoints->y > 0 &&
        itProPoints->y < imgH) {
      unsigned int ii = p_idx[p_proj[k]];
      uint8_t *p =
          obs.image((unsigned int)itProPoints->x, (unsigned int)itProPoints->y);
...
void CColouredPointsMap::PLY_import_set_vertex(
  ...
  if (pt_color)
    this->setPoint(idx, pt.x, pt.y, pt.z, pt_color->R, pt_color->G,
                   pt_color->B);
  else
...
bool CColouredPointsMap::savePCDFile(const std::string &filename,
...
template <> struct pointmap_traits<CColouredPointsMap> {
  ...
  inline static void internal_loadFromRangeScan3D_init(
    ...
    ASSERT_NOT_EQUAL_(me.colorScheme.z_max, me.colorScheme.z_min)
    lric.fVars[3] =
        1.0 / (me.colorScheme.z_max - me.colorScheme.z_min); // Az_1_color = ...
    ...
    // If both range & intensity images coincide (e.g. SwissRanger), then we can
    // just
    // assign 3D points to image pixels one-to-one, but that's not the case if
    //
    simple_3d_to_color_relation =
        (std::abs(lric.rangeScan.relativePoseIntensityWRTDepth.norm()) < 1e-5);
    img_idx_x = 0;
    img_idx_y = 0;
    ...
    case CColouredPointsMap::cmFromIntensityImage: {
      ...
      if (simple_3d_to_color_relation) {
        ...
        lric.rangeScan.relativePoseIntensityWRTDepth.inverseComposePoint(
            lric.scan_x, lric.scan_y, lric.scan_z, pt.x, pt.y, pt.z);
      ...
      if (hasValidColor && hasColorIntensityImg) {
        const uint8_t *c =
            lric.rangeScan.intensityImage.get_unsafe(img_idx_x, img_idx_y, 0);
        pR = c[2] * K_8u;
        pG = c[1] * K_8u;
        pB = c[0] * K_8u;
  ...
  inline static void internal_loadFromRangeScan3D_postOneRange(
    ...
    // Advance the color pointer (just for simple cases, e.g. SwissRanger, not
    // for Kinect)
...
void CColouredPointsMap::loadFromRangeScan(
  ...
  mrpt::maps::detail::loadFromRangeImpl<
      CColouredPointsMap>::templ_loadFromRangeScan(*this, rangeScan, robotPose);
}
