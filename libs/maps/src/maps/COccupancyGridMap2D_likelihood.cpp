...
double COccupancyGridMap2D::computeObservationLikelihood_ConsensusOWA(
  ...
  // ------------------------------------------------------------
  std::sort(likelihoodOutputs.OWA_pairList.begin(),
...
double COccupancyGridMap2D::computeObservationLikelihood_CellsDifference(
  ...
  if (obs->GetRuntimeClass() == CLASS_ID(CObservation2DRangeScan)) {
    ...
    COccupancyGridMap2D compareGrid(takenFrom.x() - 10, takenFrom.x() + 10,
                                    takenFrom.y() - 10, takenFrom.y() + 10,
                                    resolution);
...
---------------------------------------------------------------*/
double
COccupancyGridMap2D::computeObservationLikelihood_MI(const CObservation *obs,
...
double COccupancyGridMap2D::computeObservationLikelihood_rayTracing(
  ...
  if (obs->GetRuntimeClass() == CLASS_ID(CObservation2DRangeScan)) {
    ...
    laserScanSimulator(simulatedObs, // The in/out observation
    ...
    for (int j = 0; j < nRays; j += decimation) {
      ...
      if (o->validRange[j]) {
        likelihood =
            0.1 / o->maxRange +
            0.9 *
                exp(-square(min((float)fabs(r_sim - r_obs), 2.0f) / stdSqrt2));
        ret += log(likelihood);
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
double
COccupancyGridMap2D::computeLikelihoodField_Thrun(const CPointsMap *pm,
                                                  const CPose2D *relativePose) {
  MRPT_START

  double ret;
  size_t N = pm->size();
  int K = (int)ceil(likelihoodOptions.LF_maxCorrsDistance /*m*/ /
      ...
      if (!likelihoodOptions.enableLikelihoodCache ||
        ...
        // Optimized code: this part will be invoked a *lot* of times:
        {
          ...
          for (int yy = yy1; yy <= yy2; yy++) {
            unsigned int Ay2 =
                square((unsigned int)(Ay)); // Square is faster with unsigned.
            signed short Ax = Ax0;
            cellType cell;
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
double
COccupancyGridMap2D::computeLikelihoodField_II(const CPointsMap *pm,
                                               const CPose2D *relativePose) {
  MRPT_START
  ...
  for (j = 0; j < N; j += likelihoodOptions.LF_decimation) {
    ...
    for (cx = cx_min; cx <= cx_max; cx++) {
      for (cy = cy_min; cy <= cy_max; cy++) {
        ...
        float termDist = exp(Q * (square(idx2x(cx) - pointGlobal.x) +
...
COccupancyGridMap2D::TLikelihoodOptions::TLikelihoodOptions()
    : likelihoodMethod(lmLikelihoodField_Thrun),
      ...
      LF_stdHit(0.35f), LF_zHit(0.95f), LF_zRandom(0.05f), LF_maxRange(81.0f),
      LF_decimation(5), LF_maxCorrsDistance(0.3f), LF_useSquareDist(false),
      LF_alternateAverageMethod(false),

      MI_exponent(2.5f), MI_skip_rays(10), MI_ratio_max_distance(1.5f),

      rayTracing_useDistanceFilter(true), rayTracing_decimation(10),
      rayTracing_stdHit(1.0f),

      consensus_takeEachRange(1), consensus_pow(5),
...
void COccupancyGridMap2D::TLikelihoodOptions::loadFromConfigFile(
  ...
  MRPT_LOAD_CONFIG_VAR_CAST(likelihoodMethod, int, TLikelihoodMethod, iniFile,
                            section);

  enableLikelihoodCache = iniFile.read_bool(section, "enableLikelihoodCache",
  ...
  MI_ratio_max_distance = iniFile.read_float(section, "MI_ratio_max_distance",
  ...
  consensus_takeEachRange = iniFile.read_int(section, "consensus_takeEachRange",
