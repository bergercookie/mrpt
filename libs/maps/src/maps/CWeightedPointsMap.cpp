...
MAP_DEFINITION_REGISTER("CWeightedPointsMap,weightedPointsMap",
...
void CWeightedPointsMap::readFromStream(mrpt::utils::CStream &in, int version) {
  ...
  case 2: {
    ...
    if (version >= 1) {
      ...
      insertionOptions.readFromStream(
          in); // version 9: insert options are saved with its own method
    } else {
...
void CWeightedPointsMap::loadFromRangeScan(
  ...
  mrpt::maps::detail::loadFromRangeImpl<
      CWeightedPointsMap>::templ_loadFromRangeScan(*this, rangeScan, robotPose);
}
