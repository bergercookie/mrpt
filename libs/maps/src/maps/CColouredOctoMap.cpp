...
MAP_DEFINITION_REGISTER("CColouredOctoMap,colourOctoMap,colorOctoMap",
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CColouredOctoMap::writeToStream(mrpt::utils::CStream &out,
                                     int *version) const {
  if (version)
    *version = 2;
  else {
...
void CColouredOctoMap::readFromStream(mrpt::utils::CStream &in, int version) {
  ...
  case 2: {
    ...
    if (version >= 1)
      this->renderingOptions.readFromStream(in);
    if (version >= 2)
      in >> genericMapParams;
...
bool CColouredOctoMap::internal_insertObservation(
  ...
  if (IS_CLASS(obs, CObservation2DRangeScan)) {
    ...
    ********************************************************************/
    ...
    // Insert rays:
    m_octomap.insertScan(scan, sensorPt, insertionOptions.maxrange,
    ...
    ********************************************************************/
    ...
    const_cast<CObservation3DRangeScan *>(o)->project3DPointsFromDepthImageInto(
        *pts, proj_params);
    ...
    m_octomap.computeUpdate(scan, sensorPt, free_cells, occupied_cells,
    ...
    for (size_t i = 0; i < sizeRangeScan; i++) {
      ...
      // Add to this map:
      if (pt.x != 0 || pt.y != 0 || pt.z != 0)
        this->updateVoxelColour(pt.x, pt.y, pt.z, uint8_t(pt.R * colF2B),
...
* undefined. */
bool CColouredOctoMap::getPointColour(const float x, const float y,
...
void CColouredOctoMap::updateVoxelColour(const double x, const double y,
                                         const double z, const uint8_t r,
...
void CColouredOctoMap::getAsOctoMapVoxels(
  ...
  const TColor general_color_u(general_color.R * 255, general_color.G * 255,
                               general_color.B * 255, general_color.A * 255);
  ...
  for (octomap::ColorOcTree::tree_iterator it = m_octomap.begin_tree(max_depth);
       it != it_end; ++it) {
    ...
    if (it.isLeaf()) {
      ...
      // Not leaf-nodes:
      const mrpt::math::TPoint3D pt_min(vx_center.x() - L, vx_center.y() - L,
                                        vx_center.z() - L);
      const mrpt::math::TPoint3D pt_max(vx_center.x() + L, vx_center.y() + L,
                                        vx_center.z() + L);
