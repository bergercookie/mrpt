...
/*---------------------------------------------------------------
                                                Constructor
  ---------------------------------------------------------------*/
CRandomFieldGridMap3D::CRandomFieldGridMap3D(double x_min, double x_max,
                                             double y_min, double y_max,
                                             double z_min, double z_max,
                                             double voxel_size,
                                             bool call_initialize_now)
    : CDynamicGrid3D<TRandomFieldVoxel>(x_min, x_max, y_min, y_max, z_min,
                                        z_max, voxel_size /*xy*/,
                                        voxel_size /*z*/),
...
void CRandomFieldGridMap3D::setSize(const double x_min, const double x_max,
  ...
  CDynamicGrid3D<TRandomFieldVoxel>::setSize(x_min, x_max, y_min, y_max, z_min,
                                             z_max, resolution_xy, resolution_z,
                                             fill_value);
  this->internal_initialize();
...
void CRandomFieldGridMap3D::internal_initialize(bool erase_prev_contents) {
  ...
  MRPT_LOG_DEBUG_STREAM("[internal_initialize] Creating priors for GMRF with "
    ...
    for (int dir = 0; dir < 3; dir++) {
      ...
      if (custom_connectivity != nullptr) {
        const bool is_connected = custom_connectivity->getEdgeInformation(
            this, cx, cy, cz, cx + (dir == 0 ? 1 : 0), cy + (dir == 1 ? 1 : 0),
            cz + (dir == 2 ? 1 : 0), edge_lamdba);
        if (!is_connected)
          continue;
      } else {
  ...
  MRPT_LOG_DEBUG_STREAM("[internal_initialize] Prior built in " << tictac.Tac()
                                                                << " s\n"
                                                                << std::endl);
}
...
void CRandomFieldGridMap3D::TInsertionOptions::loadFromConfigFile(
  ...
  GMRF_lambdaPrior = iniFile.read_double(section.c_str(), "GMRF_lambdaPrior",
                                         GMRF_lambdaPrior);
  GMRF_skip_variance = iniFile.read_bool(section.c_str(), "GMRF_skip_variance",
                                         GMRF_skip_variance);
}
...
void CRandomFieldGridMap3D::updateMapEstimation() {
  ASSERTMSG_(!m_mrf_factors_activeObs.empty(),
             "Cannot update a map with no observations!");

  Eigen::VectorXd x_incr, x_var;
  m_gmrf.updateEstimation(x_incr,
                          insertionOptions.GMRF_skip_variance ? NULL : &x_var);

  ASSERT_(size_t(m_map.size()) == size_t(x_incr.size()));
  ASSERT_(insertionOptions.GMRF_skip_variance ||
...
bool CRandomFieldGridMap3D::insertIndividualReading(
    ...
    const bool update_map //!< [in] Run a global map update after inserting this
                          //!observatin (algorithm-dependant)
    ) {
  MRPT_START;

  ASSERT_ABOVE_(sensorVariance, .0);
  ASSERTMSG_(m_mrf_factors_activeObs.size() == m_map.size(),
...
void CRandomFieldGridMap3D::TObservationGMRF::evalJacobian(
    double &dr_dx) const {
  dr_dx = 1.0;
}
// ============ TPriorFactorGMRF ===========
