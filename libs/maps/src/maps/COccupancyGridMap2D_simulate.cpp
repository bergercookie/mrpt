...
void COccupancyGridMap2D::laserScanSimulator(
  ...
  for (size_t i = 0; i < N; i += decimation, A += AA * decimation) {
    bool valid;
    float out_range;
    simulateScanRay(sensorPose.x(), sensorPose.y(), A, out_range, valid,
...
void COccupancyGridMap2D::sonarSimulator(CObservationRange &inout_observation,
                                         ...
                                         float threshold, float rangeNoiseStd,
                                         float angleNoiseStd) const {
  const float free_thres = 1.0f - threshold;
  ...
  for (CObservationRange::iterator itR = inout_observation.begin();
    ...
    double direction =
        sensorAbsolutePose.phi() - 0.5 * inout_observation.sensorConeApperture;
    ...
    for (size_t i = 0; i < nRays; i++, direction += Adir) {
      bool valid;
      float sim_rang;
      simulateScanRay(sensorAbsolutePose.x(), sensorAbsolutePose.y(), direction,
...
#define int_y2idx(_Y) (_Y >> INTPRECNUMBIT)
  ...
  int64_t rxi = static_cast<int64_t>(((start_x - x_min) / resolution) *
                                     (1L << INTPRECNUMBIT));
  int64_t ryi = static_cast<int64_t>(((start_y - y_min) / resolution) *
                                     (1L << INTPRECNUMBIT));

  const int64_t Arxi = static_cast<int64_t>(RAYTRACE_STEP_SIZE_IN_CELL_UNITS *
                                            Arx * (1L << INTPRECNUMBIT));
  const int64_t Aryi = static_cast<int64_t>(RAYTRACE_STEP_SIZE_IN_CELL_UNITS *
                                            Ary * (1L << INTPRECNUMBIT));
...
COccupancyGridMap2D::TLaserSimulUncertaintyParams::
    ...
    : method(sumUnscented), UT_alpha(0.99), UT_kappa(.0), UT_beta(2.0),
      MC_samples(10), aperture(M_PIf), rightToLeft(true), maxRange(80.f),
      nRays(361), rangeNoiseStd(.0f), angleNoiseStd(.0f), decimation(1),
      threshold(.6f) {}
...
static void func_laserSimul_callback(const Eigen::Vector3d &x_pose,
  ...
  for (size_t i = 0; i < N; i += fixed_param.params->decimation,
    ...
    fixed_param.grid->simulateScanRay(sensorPose.x(), sensorPose.y(), A, range,
                                      valid, fixed_param.params->maxRange,
                                      free_thres, .0 /*noiseStd*/,
                                      .0 /*angleNoiseStd*/);
...
void COccupancyGridMap2D::laserScanSimulatorWithUncertainty(
  ...
  case sumUnscented:
    mrpt::math::transform_gaussian_unscented(
        ...
        &func_laserSimul_callback, // void  (*functor)(const VECTORLIKE1
                                   // &x,const USERPARAM &fixed_param,
                                   // VECTORLIKE3 &y)
