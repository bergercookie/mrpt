...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void COccupancyGridMap2D::buildVoronoiDiagram(float threshold, float robot_size,
...
*/
  m_voronoi_diagram.setSize(x_min, x_max, y_min, y_max,
                            resolution); // assign(size_x*size_y,0);
...
void COccupancyGridMap2D::findCriticalPoints(float filter_distance) {
  ...
  m_basis_map.setSize(x_min, x_max, y_min, y_max,
...
/*---------------------------------------------------------------
 ...
 Sirve para calcular diagramas de Voronoi y crit. points,etc...
  ---------------------------------------------------------------*/
int COccupancyGridMap2D::computeClearance(int cx, int cy, int *basis_x,
                                          int *basis_y, int *nBasis,
...
#define N_CIRCULOS 100
  ...
  if (!tabla_construida) {
    ...
    for (int i = 0; i < N_CIRCULOS; i++) {
      int nPasos = round(
          1 + (M_2PI * i)); // Estimacion de # de entradas (luego seran menos)
      float A = 0;
  ...
  if (*nBasis >= 2) {
    if (GetContourPoint) {
      ...
      if (!(vec & (1 << dir_predilecta)))
        ...
        dir = 6;
      if (dir != -1) {
        vec = GetNeighborhood(basis_x[0] + direccion_vecino_x[dir],
      ...
      if (!(vec & (1 << dir_predilecta)))
        ...
        dir = 6;
      if (dir != -1) {
        vec = GetNeighborhood(basis_x[1] + direccion_vecino_x[dir],
...
/*---------------------------------------------------------------
  ...
  Asociacion de numero de bit a vecinos:
                ...
                5       6       7
  ---------------------------------------------------------------*/
inline unsigned char COccupancyGridMap2D::GetNeighborhood(int cx,
                                                          int cy) const {
  unsigned char res = 0;

  if (getCell(cx - 1, cy - 1) <= voroni_free_threshold)
    res |= (1 << 0);
  if (getCell(cx, cy - 1) <= voroni_free_threshold)
    res |= (1 << 1);
  if (getCell(cx + 1, cy - 1) <= voroni_free_threshold)
    res |= (1 << 2);
  if (getCell(cx - 1, cy) <= voroni_free_threshold)
    res |= (1 << 3);
  if (getCell(cx + 1, cy) <= voroni_free_threshold)
    res |= (1 << 4);
  if (getCell(cx - 1, cy + 1) <= voroni_free_threshold)
