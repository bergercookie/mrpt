...
CPointsMap::CPointsMap()
    : insertionOptions(), likelihoodOptions(), x(), y(), z(),
      m_largestDistanceFromOrigin(0), m_heightfilter_z_min(-10),
      m_heightfilter_z_max(10), m_heightfilter_enabled(false) {
  mark_as_modified();
}
...
unsigned long CPointsMap::getPoint(size_t index, float &x, float &y,
                                   float &z) const {
  ASSERT_BELOW_(index, this->x.size())
...
unsigned long CPointsMap::getPoint(size_t index, double &x, double &y,
                                   double &z) const {
  ASSERT_BELOW_(index, this->x.size())
...
/*---------------------------------------------------------------
                                                getPointsBuffer
 ---------------------------------------------------------------*/
void CPointsMap::getPointsBuffer(size_t &outPointsCount, const float *&xs,
...
void CPointsMap::determineMatching2D(
  ...
  ASSERT_BELOW_(params.offset_other_map_points,
  ...
  const TPose2D otherMapPose(otherMapPose_.x(), otherMapPose_.y(),
                             otherMapPose_.phi());
...
// Translate and rotate all local points:
  ...
  // We'll assume that the real allocated memory in the source buffers at least
  ...
  for (; nPackets; nPackets--, ptr_in_x += 4, ptr_in_y += 4, ptr_out_x += 4,
    ...
    const __m128 lxs =
        _mm_add_ps(x0_4val, _mm_sub_ps(_mm_mul_ps(xs, cos_4val),
                                       _mm_mul_ps(ys, sin_4val)));
    const __m128 lys =
        _mm_add_ps(y0_4val, _mm_add_ps(_mm_mul_ps(xs, sin_4val),
                                       _mm_mul_ps(ys, cos_4val)));
  ...
  this->boundingBox(global_x_min, global_x_max, global_y_min, global_y_max,
  ...
  for (localIdx = params.offset_other_map_points,
    ...
    maxDistForCorrespondenceSquared =
        square(params.maxAngularDistForCorrespondence *
                   std::sqrt(square(params.angularDistPivotPoint.x - x_local) +
  ...
  if (params.onlyUniqueRobust) {
    ASSERTMSG_(params.onlyKeepTheClosest, "ERROR: onlyKeepTheClosest must be "
                                          "also set to true when "
...
void CPointsMap::changeCoordinatesReference(const CPose2D &newBase) {
  ...
  for (size_t i = 0; i < N; i++)
    newBase3D.composePoint(x[i], y[i], z[i], // In
                           x[i], y[i], z[i]  // Out
                           );
...
void CPointsMap::changeCoordinatesReference(const CPose3D &newBase) {
  ...
  for (size_t i = 0; i < N; i++)
    newBase.composePoint(x[i], y[i], z[i], // In
                         x[i], y[i], z[i]  // Out
                         );
...
CPointsMap::TInsertionOptions::TInsertionOptions()
    : minDistBetweenLaserPoints(0.02f), addToExistingPointsMap(true),
      also_interpolate(false), disableDeletion(true), fuseWithExisting(false),
      isPlanarMap(false), horizontalTolerance(DEG2RAD(0.05)),
      maxDistForInterpolatePoints(2.0f), insertInvalidPoints(false) {}
...
void CPointsMap::TInsertionOptions::writeToStream(
  ...
  out << minDistBetweenLaserPoints << addToExistingPointsMap << also_interpolate
      << disableDeletion << fuseWithExisting << isPlanarMap
      << horizontalTolerance << maxDistForInterpolatePoints
...
void CPointsMap::getAs3DObject(mrpt::opengl::CSetOfObjects::Ptr &outObj) const {
  ...
  obj->setColor(CPointsMap::COLOR_3DSCENE_R, CPointsMap::COLOR_3DSCENE_G,
                CPointsMap::COLOR_3DSCENE_B, 1);
  obj->setPointSize(mrpt::global_settings::POINTSMAPS_3DOBJECT_POINTSIZE);
  obj->enableColorFromZ(true);

  obj->setGradientColors(TColorf(0.0, 0, 0),
                         TColorf(CPointsMap::COLOR_3DSCENE_R,
...
float CPointsMap::compute3DMatchingRatio(
  ...
  this->determineMatching3D(otherMap2->getAsSimplePointsMap(), otherMapPose,
...
/*---------------------------------------------------------------
                                                getAllPoints
---------------------------------------------------------------*/
void CPointsMap::getAllPoints(vector<float> &xs, vector<float> &ys,
...
// Just the closest point:
  ...
  kdTreeTwoClosestPoint2D(x0, y0, // The query
  ...
  if (d12 > 0.20f * 0.20f || d12 < 0.03f * 0.03f) {
    ...
    math::closestFromPointToLine(x0, y0,            // the point
...
/*---------------------------------------------------------------
                                boundingBox
---------------------------------------------------------------*/
void CPointsMap::boundingBox(float &min_x, float &max_x, float &min_y,
                             float &max_y, float &min_z, float &max_z) const {
  ...
  if (!m_boundingBoxIsUpdated) {
    if (!nPoints) {
      ...
      // capacities to be 4*N by design,
      // but will leave this code here just in case (for some edge cases?)
      if (x.capacity() < nPoints_4align || y.capacity() < nPoints_4align ||
...
void CPointsMap::determineMatching3D(
  ...
  for (unsigned int localIdx = params.offset_other_map_points;
    ...
    otherMapPose.composePoint(otherMap->x[localIdx], otherMap->y[localIdx],
                              otherMap->z[localIdx], x_local, y_local, z_local);
...
/*---------------------------------------------------------------
                                extractCylinder
---------------------------------------------------------------*/
void CPointsMap::extractCylinder(const TPoint2D &center, const double radius,
...
/*---------------------------------------------------------------
                                extractPoints
---------------------------------------------------------------*/
void CPointsMap::extractPoints(const TPoint3D &corner1, const TPoint3D &corner2,
                               CPointsMap *outMap, const double &R,
...
void CPointsMap::compute3DDistanceToMesh(
  ...
  float local_x_min, local_x_max, local_y_min, local_y_max, local_z_min,
      local_z_max;
  otherMap->boundingBox(local_x_min, local_x_max, local_y_min, local_y_max,
  ...
  for (it = _correspondences.begin(); it != _correspondences.end(); ++it) {
    ...
    if (best.find(i0) != best.end() && best[i0].find(i1) != best[i0].end() &&
        best[i0][i1].find(i2) !=
            best[i0][i1].end()) // if there is a match, check if it is better
    {
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
double
CPointsMap::internal_computeObservationLikelihood(const CObservation *obs,
  ...
  if (obs->GetRuntimeClass() == CLASS_ID(CObservation2DRangeScan)) {
    ...
    if (takenFrom.isHorizontal()) {
      ...
      for (size_t i = 0; i < N;
        ...
        kdTreeClosestPoint2D(xg, yg, // Look for the closest to this guy
                             closest_x,
                             closest_y,  // save here the closest match
      ...
      for (size_t i = 0; i < N;
        ...
        kdTreeClosestPoint3D(xg, yg, zg, // Look for the closest to this guy
                             closest_x, closest_y,
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
bool CPointsMap::internal_insertObservation(const CObservation *obs,
  ...
  if (IS_CLASS(obs, CObservation2DRangeScan)) {
    ...
    if (reallyInsertIt) {
      ...
      if (insertionOptions.fuseWithExisting) {
        ...
        auxMap.loadFromRangeScan(*o, // The laser range scan observation
        ...
        if (!insertionOptions.disableDeletion) {
          ...
          n = size();
          for (size_t i = 0; i < n; i++) {
            if (checkForDeletion[i]) // Default to true, unless a fused point,
                                     // which must be kept.
            {
              float x, y;
              getPoint(i, x, y);
        ...
        loadFromRangeScan(*o,          // The laser range scan observation
                          &robotPose3D // The robot pose
                          );
      }
    ...
    if (reallyInsertIt) {
      ...
      if (insertionOptions.fuseWithExisting) {
        ...
        fuseWith(&auxMap, // Fuse with this map
                 insertionOptions.minDistBetweenLaserPoints, // Min dist.
                 NULL // rather than &checkForDeletion which we don't need for
                      // 3D observations
                 );
      } else {
        // Don't fuse: Simply add
      ...
      // This could be implemented to check whether existing points fall into
      // empty-space 3D polygon
      // but performance for standard Swissranger scans (176*144 points) may be
      // too sluggish?
      // if (! insertionOptions.disableDeletion )   {
      // ....
      // }
    ...
    /********************************************************************
                            OBSERVATION TYPE: CObservationRange  (IRs, Sonars,
     etc.)
     ********************************************************************/
    mark_as_modified();
    ...
    this->reserve(this->size() +
                  o->sensedData.size() * 30); // faster push_back's.
...
"added". This prevents
 ...
 ---------------------------------------------------------------*/
void CPointsMap::fuseWith(CPointsMap *otherMap, float minDistForFuse,
  ...
  determineMatching2D(otherMap, // The other map
  ...
  for (size_t i = 0; i < nOther; i++) {
    ...
    for (TMatchingPairList::const_iterator corrsIt = correspondences.begin();
...
void CPointsMap::loadFromVelodyneScan(
  ...
  if (!insertionOptions.addToExistingPointsMap)
    resize(0); // Resize to 0 instead of clear() so the std::vector<> memory is
               // not actually deallocated and can be reused.
  ...
  for (size_t i = 0; i < nScanPts; i++) {
    ...
    this->setPoint(nOldPtsCount + i, gx, gy, gz, // XYZ
