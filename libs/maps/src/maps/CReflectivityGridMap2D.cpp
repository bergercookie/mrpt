...
MAP_DEFINITION_REGISTER("CReflectivityGridMap2D,reflectivityMap",
...
/*---------------------------------------------------------------
                                                Constructor
  ---------------------------------------------------------------*/
CReflectivityGridMap2D::CReflectivityGridMap2D(double x_min, double x_max,
  ...
  if (IS_CLASS(obs, CObservationReflectivity)) {
    ...
    ********************************************************************/
    ...
    if (!cell) {
      ...
      resize(new_x_min, new_x_max, new_y_min, new_y_max, default_cell,
             2.0 /* addit. margin */);
  ...
  if (IS_CLASS(obs, CObservationReflectivity)) {
    ...
    ********************************************************************/
    ...
    if (!cell)
      ...
      return -0.5 * square((m_logodd_lut.p2l(*cell) - o->reflectivityLevel) /
                           o->sensorStdNoise);
    }
  } else
    return 0;
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CReflectivityGridMap2D::writeToStream(mrpt::utils::CStream &out,
                                           int *version) const {
  if (version)
    *version = 2;
  else {
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CReflectivityGridMap2D::readFromStream(mrpt::utils::CStream &in,
                                            int version) {
  switch (version) {
  case 0:
  case 1:
...
/*---------------------------------------------------------------
                                        getAsImage
  ---------------------------------------------------------------*/
void CReflectivityGridMap2D::getAsImage(utils::CImage &img, bool verticalFlip,
                                        bool forceRGB) const {
  if (!forceRGB) { // 8bit gray-scale
