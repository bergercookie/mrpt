...
void CPointCloudFilterByDistance::filter(
    mrpt::maps::CPointsMap *pc, //!< [in,out] The input pointcloud, which will
                                //!be modified upon return after filtering.
    const mrpt::system::TTimeStamp
        pc_timestamp, //!< [in] The timestamp of the input pointcloud
    const mrpt::poses::CPose3D &cur_pc_pose, //!< [in] If nullptr, the PC is
                                             //!assumed to be given in global
                                             //!coordinates. Otherwise, it will
                                             //!be transformed from local
                                             //!coordinates to global using this
                                             //!transformation.
  ...
  CSimplePointsMap::Ptr original_pc = std::make_shared<CSimplePointsMap>();
  ...
  if (prev_pc.size() < static_cast<size_t>(options.previous_keyframes)) {
    ...
    for (int i = 0; can_do_filter && i < options.previous_keyframes; ++i) {
      if (mrpt::system::timeDifference(m_last_frames.rbegin()->first,
                                       pc_timestamp) >
  ...
  if (can_do_filter) {
    ...
    for (size_t i = 0; i < N; i++) {
      ...
      pc->kdTreeNClosestPoint3D(pt_k, 2 /*num queries*/, neig_k,
                                neig_sq_dist_k);

      // Look for neighbors in "time=k-i"
      std::vector<TPoint3D> neig_kmi(options.previous_keyframes);
      std::vector<float> neig_sq_dist_kmi(options.previous_keyframes,
                                          std::numeric_limits<float>::max());

      for (int k = 0; k < options.previous_keyframes; ++k) {
        for (int prev_tim_idx = 0; prev_tim_idx < options.previous_keyframes;
             prev_tim_idx++) {
      ...
      const double max_allowed_dist_sq =
          square(options.min_dist + options.angle_tolerance * pt_k.norm());
    ...
    if ((params == nullptr || params->do_not_delete == false) && N > 0 &&
        del_count / double(N) <
            options.max_deletion_ratio // If we are deleting too many points, it
                                       // may be that the filter is plainly
                                       // wrong
        ) {
...
CPointCloudFilterByDistance::TOptions::TOptions()
    : min_dist(0.10), angle_tolerance(mrpt::utils::DEG2RAD(5)),
      too_old_seconds(1.0), previous_keyframes(1), max_deletion_ratio(.4) {}
...
void CPointCloudFilterByDistance::TOptions::saveToConfigFile(
  ...
  MRPT_SAVE_CONFIG_VAR_COMMENT(previous_keyframes,
                               "(Default: 1) How many previous keyframes will "
                               "be compared with the latest pointcloud.");
  MRPT_SAVE_CONFIG_VAR_COMMENT(
      max_deletion_ratio,
      "(Default: 0.4) If the ratio [0,1] of points considered invalid "
      "(`deletion` ) is larger than this ratio, no point will be deleted since "
      "it'd be too suspicious and may indicate a failure of this filter.");
}
