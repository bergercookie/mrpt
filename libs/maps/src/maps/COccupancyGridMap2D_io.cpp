...
void COccupancyGridMap2D::readFromStream(mrpt::utils::CStream &in,
  ...
  case 6: {
    ...
    in >> new_size_x >> new_size_y >> new_x_min >> new_x_max >> new_y_min >>
        new_y_max >> new_resolution;
...
/*---------------------------------------------------------------
                                        loadFromBitmap
 ---------------------------------------------------------------*/
bool COccupancyGridMap2D::loadFromBitmap(const mrpt::utils::CImage &imgFl,
                                         float resolution, float xCentralPixel,
                                         float yCentralPixel) {
  MRPT_START
...
bool COccupancyGridMap2D::saveAsBitmapTwoMapsWithCorrespondences(
  ...
  img.filledRectangle(0, 0, img.getWidth() - 1, img.getHeight() - 1,
  ...
  for (i = 0; i < n; i++) {
    lineColor =
        TColor(static_cast<long>(randomGenerator.drawUniform(0, 255.0f)),
               static_cast<long>(randomGenerator.drawUniform(0, 255.0f)),
               static_cast<long>(randomGenerator.drawUniform(0, 255.0f)));

    img.line(m1->x2idx(corrs[i].this_x),
...
// ---------------------------------------------
  ...
  for (i = 0; i < n; i++) {
    ...
    emf.textOut(m1->x2idx(corrs[i].this_x) - 10,
                Ay1 + ly1 - 1 - m1->y2idx(corrs[i].this_y) - 25, str,
                TColor::black);

    emf.textOut(lx1 + 1 + m2->x2idx(corrs[i].other_x) - 10,
...
void COccupancyGridMap2D::saveMetricMapRepresentationToFile(
  ...
  LIMITS(0, 2) = y_min;
  LIMITS(0, 3) = y_max;
  LIMITS.saveToTextFile(fil, MATRIX_FORMAT_FIXED, false /* add mrpt header */,
