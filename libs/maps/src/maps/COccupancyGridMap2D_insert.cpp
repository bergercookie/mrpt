	...
	if (CLASS_ID(CObservation2DRangeScan) == obs->GetRuntimeClass())
	{
		...
		if (reallyInsert)
		{
			...
			cellType logodd_thres_free =
				OCCGRID_CELLTYPE_MAX - logodd_observation;
				...
				for (idx = 0; idx < nRanges; idx += K)
				{
					...
					for (int nStep = 0; nStep < nStepsRay; nStep++)
					{
						updateCell_fast_free(
							cx, cy, logodd_observation, logodd_thres_free,
			...
			else
			{
				...
				for (idx = 0; idx < nRanges; idx += K, A += dAK)
				{
					...
					theR -=
						resolution;  // Remove one cell of length, which will be
					...
					else
					{
						...
						if (!frAy_R1)
							frAy_R1 =
								2
								<< FRBITS;  // If Ay=0, force it to be >0 so the
		...
		if (reallyInsert)
		{
			...
			for (idx = 0; idx < nRanges; idx += K, A += dAK)
			{
				...
				if (P0.cy == P2.cy && P0.cy == P1.cy)
				{
					...
					for (int ccx = min_cx; ccx <= max_cx; ccx++)
						updateCell_fast_free(
							ccx, P0.cy, logodd_observation, logodd_thres_free,
							theMapArray, theMapSize_x);
				}
				else
				{
					...
					if (!frAy_R1)
						frAy_R1 =
							2 << FRBITS;  // If Ay=0, force it to be >0 so the
					// "do...while" loop below ends in ONE
					// iteration.
