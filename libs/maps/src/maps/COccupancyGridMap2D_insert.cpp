...
Insert the observation information into this map.
 ---------------------------------------------------------------*/
bool COccupancyGridMap2D::internal_insertObservation(const CObservation *obs,
...
#define FRBITS 9
  ...
  if (CLASS_ID(CObservation2DRangeScan) == obs->GetRuntimeClass()) {
    ...
    if (reallyInsert) {
      ...
      int K = updateInfoChangeOnly.enabled ? updateInfoChangeOnly.laserRaysSkip
                                           : decimation;
      ...
      if (!insertionOptions.wideningBeamsWithDistance) {
        ...
        // Reserve a temporary block of memory on the stack with "alloca": this
        // memory has NOT to be deallocated,
        ...
        for (idx = 0, scanPoint_x = scanPoints_x, scanPoint_y = scanPoints_y;
        ...
        for (idx = 0; idx < nRanges; idx += K) {
          if (!o->validRange[idx] && !invalidAsFree)
            continue;
          ...
          ASSERT_(static_cast<unsigned int>(trg_cx) < size_x &&
          ...
          for (int nStep = 0; nStep < nStepsRay; nStep++) {
            updateCell_fast_free(cx, cy, logodd_observation, logodd_thres_free,
        ...
        for (idx = 0; idx < nRanges; idx += K, A += dAK) {
          ...
          theR -= resolution; // Remove one cell of length, which will be filled
                              // with "occupied" later.
...
#if defined(_DEBUG) || (MRPT_ALWAYS_CHECKS_DEBUG)
          // The x> comparison implicitly holds if x<0
          ASSERT_(static_cast<unsigned int>(P0.cx) < size_x &&
                  static_cast<unsigned int>(P0.cy) < size_y);
          ASSERT_(static_cast<unsigned int>(P1.cx) < size_x &&
                  static_cast<unsigned int>(P1.cy) < size_y);
          ASSERT_(static_cast<unsigned int>(P2.cx) < size_x &&
          ...
          if (P0.cy == P2.cy && P0.cy == P1.cy) {
            ...
            for (int ccx = min_cx; ccx <= max_cx; ccx++)
              updateCell_fast_free(ccx, P0.cy, logodd_observation,
            ...
            // Use "fractional integers" to approximate float operations during
            // the ray tracing:
            // Integers store "float values * 128"
            ...
            // Start R1=R2 = P0... unlesss P0.cy == P1.cy, i.e. there is only
            // one row:
            if (P0.cy != P1.cy) {
              R2 = R1;
            ...
            do {
              if (last_insert_cy != R1.cy) // || last_insert_cx!=R1.cx)
              {
                ...
                for (int ccx = R1.cx; ccx <= R2.cx; ccx++)
                  updateCell_fast_free(ccx, R1.cy, logodd_observation,
                                       logodd_thres_free, theMapArray,
                                       theMapSize_x);
              }
            ...
            // struct { int frX,frY; int cx,cy; } R1,R2;	// Fractional coords of
            // the two rays:
            // R1, R2 follow edges P1->P2 & P1b->P2
            // R1 forced to be at the left hand
            frAy_R1 = frAcy12;
            if (!frAy_R1)
              frAy_R1 = 2 << FRBITS; // If Ay=0, force it to be >0 so the
                                     // "do...while" loop below ends in ONE
                                     // iteration.
    ...
    if (reallyInsert) {
      ...
      for (idx = 0; idx < nRanges; idx += K, A += dAK) {
        ...
        if (P0.cy == P2.cy && P0.cy == P1.cy) {
          ...
          // Start R1=R2 = P0... unlesss P0.cy == P1.cy, i.e. there is only one
          // row:
          if (P0.cy != P1.cy) {
            R2 = R1;
            //  R1 & R2 follow the edges: P0->P1  & P0->P1b
        ...
        if (o->sensedData[idx].sensedDistance < maxDistanceInsertion) {
          ...
          // Special case: Only one cell:
          if (P2.cx == P1.cx && P2.cy == P1.cy) {
            updateCell_fast_occupied(P1.cx, P1.cy, logodd_observation_occupied,
...
COccupancyGridMap2D::TInsertionOptions::TInsertionOptions()
    : mapAltitude(0), useMapAltitude(false), maxDistanceInsertion(15.0f),
      maxOccupancyUpdateCertainty(0.65f),
      considerInvalidRangesAsFreeSpace(true), decimation(1),
      horizontalTolerance(DEG2RAD(0.05)),

      CFD_features_gaussian_size(1), CFD_features_median_size(3),
...
void COccupancyGridMap2D::TInsertionOptions::loadFromConfigFile(
  ...
  MRPT_LOAD_CONFIG_VAR(considerInvalidRangesAsFreeSpace, bool, iniFile,
                       section);
