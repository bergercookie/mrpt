...
template <class MAP> void load_demo_9pts_map(MAP &pts) {
  pts.clear();
  for (size_t i = 0; i < demo9_N; i++)
    pts.insertPoint(demo9_xs[i], demo9_ys[i], demo9_zs[i]);
}

template <class MAP> void do_test_insertPoints() {
...
template <class MAP> void do_test_clipOutOfRangeInZ() {
  MAP pts0;
...
template <class MAP> void do_test_clipOutOfRange() {
  MAP pts0;
  load_demo_9pts_map(pts0);
