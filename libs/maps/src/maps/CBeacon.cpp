...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
CBeacon::CBeacon()
    : m_typePDF(pdfGauss), m_locationMC(1), m_locationGauss(), m_locationSOG(1),
...
/*---------------------------------------------------------------
                                        bayesianFusion
  ---------------------------------------------------------------*/
void CBeacon::bayesianFusion(const CPointPDF &p1, const CPointPDF &p2,
...
void CBeacon::getAs3DObject(mrpt::opengl::CSetOfObjects::Ptr &outObj) const {
  ...
  case pdfMonteCarlo: {
    ...
    for (size_t i = 0; i < N; i++)
      obj->setPoint(i, m_locationMC.m_particles[i].d->x,
...
void CBeacon::getAsMatlabDrawCommands(utils::CStringList &out_Str) const {
  ...
  case pdfMonteCarlo: {
    ...
    sy = "ys=[";
    for (i = 0; i < N; i++) {
      os::sprintf(auxStr, sizeof(auxStr), "%.3f%c",
  ...
  case pdfGauss: {
    ...
    os::sprintf(auxStr, sizeof(auxStr), "m=[%.3f %.3f];",
  ...
  case pdfSOG: {
    for (CPointPDFSOG::const_iterator it = m_locationSOG.begin();
      ...
      os::sprintf(auxStr, sizeof(auxStr), "m=[%.3f %.3f];", (it)->val.mean.x(),
                  (it)->val.mean.y());
      out_Str.add(std::string(auxStr));
      os::sprintf(auxStr, sizeof(auxStr), "C=[%e %e;%e %e];",
  ...
  os::sprintf(auxStr, sizeof(auxStr), "text(%f,%f,'#%i');", meanP.x(),
              meanP.y(), static_cast<int>(m_ID));
  out_Str.add(std::string(auxStr));
...
void CBeacon::generateObservationModelDistribution(
  ...
  for (CPointPDFSOG::const_iterator it = beaconPos->begin();
    ...
    CPoint3D ringCenter((it)->val.mean.x() - sensorPntOnRobot.x(),
...
/*---------------------------------------------------------------
                                        generateRingSOG
  ---------------------------------------------------------------*/
void CBeacon::generateRingSOG(const float &R, CPointPDFSOG &outPDF,
  ...
  ASSERT_(myBeaconMap->insertionOptions.minElevation_deg <=
  ...
  for (idxEl = 0; idxEl <= (1 + B / 2); idxEl++) {
    ...
    for (idxTh = 0; idxTh < nThSteps; idxTh++) {
      ...
      if (reallyCreateIt) {
        ...
        dir = dir - sensorPnt;
        CMatrixDouble33 H = CMatrixDouble33(math::generateAxisBaseFromDirection(
            dir.x(), dir.y(),
            dir.z())); // 3 perpendicular & normalized vectors.

        H.multiply_HCHt(S, outPDF.get(modeIdx).val.cov); // out = H * S * ~H;
        if (minEl == maxEl) {                            // We are in 2D:
          // 3rd column/row = 0
          CMatrixDouble33 &C33 = outPDF.get(modeIdx).val.cov;
          C33.get_unsafe(0, 2) = C33.get_unsafe(2, 0) = C33.get_unsafe(1, 2) =
              C33.get_unsafe(2, 1) = C33.get_unsafe(2, 2) = 0;
        }
