...
CBeaconMap::CBeaconMap()
    : m_beacons(), likelihoodOptions(), insertionOptions() {}
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
double
CBeaconMap::internal_computeObservationLikelihood(const CObservation *obs,
    ...
    for (it_obs = o->sensedData.begin(); it_obs != o->sensedData.end();
      ...
      if (beac != nullptr && it_obs->sensedDistance > 0 &&
        ...
        case CBeacon::pdfMonteCarlo: {
          ...
          for (it = beac->m_locationMC.m_particles.begin(),
              itLW = logWeights.begin(), itLL = logLiks.begin();
               ...
               ++it, ++itLW, ++itLL) {
            float expectedRange =
                sensor3D.distance3DTo(it->d->x, it->d->y, it->d->z);
          ...
          if (logWeights.size())
            ret += math::averageLogLikelihood(
                logWeights, logLiks); // A numerically-stable method to average
                                      // the likelihoods
        } break;
        // ------------------------------
        // PDF is Gaussian
        ...
        case CBeacon::pdfSOG: {
          ...
          for (it = beac->m_locationSOG.begin(), itLW = logWeights.begin(),
              itLL = logLiks.begin();
               it != beac->m_locationSOG.end(); ++it, ++itLW, ++itLL) {
            // Compute the Jacobian H and varZ
          ...
          if (logWeights.size())
            ret += math::averageLogLikelihood(
                logWeights, logLiks); // log( linear_lik / sumW );
        } break;
      ...
      if (sensedRange > 0) // Only sensible range values!
      {
        if (!beac) {
          ...
          if (insertionOptions.insertAsMonteCarlo) {
            ...
            ASSERT_(insertionOptions.minElevation_deg <=
            ...
            CBeacon::generateRingSOG(sensedRange,           // Sensed range
          ...
          case CBeacon::pdfMonteCarlo: {
            ...
            // Update weights:
            // --------------------
            for (CPointPDFParticles::CParticleList::iterator it =
                     beac->m_locationMC.m_particles.begin();
                 it != beac->m_locationMC.m_particles.end(); ++it) {
              float expectedRange =
                  sensorPnt.distance3DTo(it->d->x, it->d->y, it->d->z);
              // Add bias:
              // expectedRange += float(0.1*(1-exp(-0.16*expectedRange)));
              it->log_w += -0.5 * square((sensedRange - expectedRange) /
                                         likelihoodOptions.rangeStd);
              maxW = max(it->log_w, maxW);
              sumW += exp(it->log_w);
            } // end for it

            // Perform resampling (SIR filter) or not (simply accumulate
            // weights)??
            // ------------------------------------------------------------------------
            if (insertionOptions.MC_performResampling) {
              ...
              for (CPointPDFParticles::CParticleList::iterator it =
                   ...
                   it != beac->m_locationMC.m_particles.end();) {
                if (it->log_w <
                    (maxW - insertionOptions.MC_thresholdNegligible)) {
                  it->d.reset();
          ...
          case CBeacon::pdfGauss: {
            ...
            float expectedRange =
                sensorPnt.distanceTo(beac->m_locationGauss.mean);
            ...
            // if (useEKF_or_KF)
            {
              ...
              beac->m_locationGauss.cov =
                  (Eigen::Matrix<double, 3, 3>::Identity() - K * H) *
                  beac->m_locationGauss.cov;
          ...
          case CBeacon::pdfSOG: {
            ...
            // For each Gaussian mode:
            //  1) Update its weight (using the likelihood of the observation
            //  linearized at the mean)
            ...
            for (it = beac->m_locationSOG.begin();
              ...
              it->val.cov = (Eigen::Matrix<double, 3, 3>::Identity() - K * H) *
                            it->val.cov;
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
bool CBeaconMap::saveToMATLABScript3D(const string &file, const char *style,
...
CBeaconMap::TInsertionOptions::TInsertionOptions()
    : insertAsMonteCarlo(true), maxElevation_deg(0), minElevation_deg(0),
      MC_numSamplesPerMeter(1000), MC_maxStdToGauss(0.4f),
      MC_thresholdNegligible(5), MC_performResampling(false),
      MC_afterResamplingNoise(0.01f), SOG_thresholdNegligible(20.0f),
      SOG_maxDistBetweenGaussians(1.0f), SOG_separationConstant(3.0f) {}
...
void CBeaconMap::TInsertionOptions::loadFromConfigFile(
  ...
  MRPT_LOAD_CONFIG_VAR(MC_afterResamplingNoise, float, iniFile,
                       section.c_str());
  MRPT_LOAD_CONFIG_VAR(SOG_thresholdNegligible, float, iniFile,
                       section.c_str());
  MRPT_LOAD_CONFIG_VAR(SOG_maxDistBetweenGaussians, float, iniFile,
                       section.c_str());
...
void CBeaconMap::saveMetricMapRepresentationToFile(
  ...
  string fil4(filNamePrefix + string("_population.txt"));
  {
    ...
    if (f) {
      ...
      fprintf(f, "%u %u", static_cast<unsigned>(nParts),
...
float CBeaconMap::compute3DMatchingRatio(
  ...
  computeMatchingWith3DLandmarks(&modMap, matchList, out_corrsRatio,
                                 otherCorrespondences);
...
void CBeaconMap::saveToTextFile(const string &fil) const {
  ...
  for (const_iterator it = m_beacons.begin(); it != m_beacons.end(); ++it) {
    ...
    os::fprintf(f, "%i %f %f %f %e %e %e %e %e %e %e %e\n",
                static_cast<int>(it->m_ID), p.x(), p.y(), p.z(), C(0, 0),
