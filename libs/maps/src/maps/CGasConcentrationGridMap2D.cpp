...
IMPLEMENTS_SERIALIZABLE(CGasConcentrationGridMap2D, CRandomFieldGridMap2D,
                        mrpt::maps)
...
CGasConcentrationGridMap2D::CGasConcentrationGridMap2D(
  ...
  //  Also, calling clear() is mandatory to end initialization of our base class
  //  (read note in CRandomFieldGridMap2D::CRandomFieldGridMap2D)
  CMetricMap::clear();
    ...
    ********************************************************************/
    ...
    if (o->sensorLabel.compare(insertionOptions.gasSensorLabel) == 0) {
      ...
      if (o->sensorLabel.compare("MCEnose") == 0 ||
        ...
        sensorPose =
            CPose2D(CPose3D(robotPose2D) + CPose3D(it->eNosePoseOnTheRobot));
        ...
        if (insertionOptions.gasSensorType == 0x0000) { // compute the mean
          ...
          if (i < it->sensorTypes.size()) {
            ...
            cout << "Sensor especified not found, compute default mean value"
                 << endl;
            sensorReading = math::mean(it->readingsVoltage);
          }
        }
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CGasConcentrationGridMap2D::writeToStream(mrpt::utils::CStream &out,
                                               int *version) const {
  if (version)
    *version = 5;
  else {
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CGasConcentrationGridMap2D::readFromStream(mrpt::utils::CStream &in,
                                                int version) {
  switch (version) {
  case 0:
  case 1:
  ...
  case 5: {
    ...
    if (version < 2) { // Converter from old versions <=1
...
CGasConcentrationGridMap2D::TInsertionOptions::TInsertionOptions()
    :
      ...
      std_windNoise_phi(0.2f), std_windNoise_mod(0.2f),
      default_wind_direction(0.0f), default_wind_speed(1.0f) {}
...
void CGasConcentrationGridMap2D::TInsertionOptions::loadFromConfigFile(
  ...
  gasSensorLabel = iniFile.read_string(section.c_str(), "gasSensorLabel",
                                       "Full_MCEnose", true);
  ...
  // Read sensor type in hexadecimal
  {
    ...
    } else { // fall back to old name, or default to current value:
      gasSensorType = iniFile.read_int(section.c_str(), "KF_sensorType",
                                       gasSensorType, true);
    }
  }
  windSensorLabel = iniFile.read_string(section.c_str(), "windSensorLabel",
                                        "Full_MCEnose", true);
    ...
    for (size_t it_i = 0; it_i < N; it_i++) {
      ...
      for (size_t it_j = 0; it_j < N; it_j++) {
        if (m_map[it_j].kf_mean != 0 && A(it_i, it_j) != 0) {
          if (row_sum[it_i] >= 1)
            new_means[it_i] +=
                (A(it_i, it_j) / row_sum[it_i]) * m_map[it_j].kf_mean;
          else
      ...
      if (row_sum[it_i] < 1)
        new_variances[it_i] =
            (1 - row_sum[it_i]) * square(insertionOptions.KF_initialCellStd);
      ...
      for (size_t it_j = 0; it_j < N; it_j++) {
        if (A(it_i, it_j) != 0) {
          if (row_sum[it_i] >= 1)
            ...
            new_variances[it_i] +=
                A(it_i, it_j) * (m_stackedCov(it_j, 0) +
                                 square(m_map[it_j].kf_mean - new_means[it_i]));
        }
      }
    }
...
*
*	The LUT contains the values of the Gaussian Weigths and the references
to the cell indexes to be applied.
*	Since the LUT is independent of the wind direction and angle, it
generates the Gaussian Weights for different configurations
*	of wind angle and module values.
*
*	To increase precission, each cell of the grid is sub-divided in
subcells of smaller size.

*	cell_i --> Cell origin (We consider our reference system in the bottom
left corner of cell_i ).
                           Is the cell that contains the gas measurement which
will be propagated by the wind.
                           The wind propagates in the shape of a 2D Gaussian
with center in the target cell (cell_j)
*	cell_j --> Target cell. Is the cell where falls the center of the
Gaussian that models the propagation of the gas comming from cell_i.
*/

{
  ...
  LUT.std_r =
      insertionOptions.std_windNoise_mod /
      insertionOptions.advectionFreq; // Standard Deviation in wind module (cte)
  std::string filename =
      format("Gaussian_Wind_Weights_res(%f)_stdPhi(%f)_stdR(%f).gz",
  ...
  if (mrpt::system::fileExists(filename.c_str())) {
    ...
    for (size_t phi_indx = 0; phi_indx < LUT.phi_count; phi_indx++) {
      ...
      for (size_t r_indx = 0; r_indx < LUT.r_count; r_indx++) {
        ...
        /* ---------------------------------------------------------------------------------
                Generate bounding-box  (+/- 3std) to determine which cells to
        update
        ---------------------------------------------------------------------------------*/
        ...
        // Consider special case for high uncertainty in PHI. The shape of the
        // polygon is a donut.
        double std_phi_BBox = LUT.std_phi;
        if (std_phi_BBox > M_PI / 3) {
          std_phi_BBox = M_PI / 3; // To avoid problems generating the bounding
                                   // box. For std>pi/3 the shape is always a
                                   // donut.
        }
        ...
        for (int sd = (-3); sd <= (3); sd++) {
          vertex_x[indx] =
              cell_i_x + (r + sr * LUT.std_r) * cos(phi + sd * std_phi_BBox);
          ...
          vertex_y[indx] =
              cell_i_y + (r + sr * LUT.std_r) * sin(phi + sd * std_phi_BBox);
        ...
        /* ------------------------------------------------------------------------
           Determine range of cells to update according to the Bounding-Box
        limits.
        ...
        if (num_cells_affected == 1) {
          ...
          if (debug) {
            // Save to file (debug)
            fprintf(debug_file, "(%d,%d)=%.4f", gauss_info.cx, gauss_info.cy,
                    gauss_info.value);
          }
        } else {
          ...
          for (int scy = 0; scy < BB_y_subcells; scy++) {
            for (int scx = 0; scx < BB_x_subcells; scx++) {
              ...
              float r_ia = sqrt(square(subcell_a_x - cell_i_x) +
                                square(subcell_a_y - cell_i_y));
              float phi_ia =
                  atan2(subcell_a_y - cell_i_y, subcell_a_x - cell_i_x);
              ...
              // Since we work with a cell grid, approximate the weight of the
              // gaussian by the volume of the subcell_a
              if (r_ia != 0.0)
                w = (w * (subcell_pres_x * subcell_pres_y) / r_ia);

              // Determine cell index of the current subcell
              int cell_cx =
                  static_cast<int>(floor(subcell_a_x / LUT.resolution));
              int cell_cy =
                  static_cast<int>(floor(subcell_a_y / LUT.resolution));
          ...
          for (it = w_values.begin(); it != w_values.end(); it++) {
            ...
            if (w_final >= 0.001) {
              ...
              if (debug) {
                // Save to file (debug)
                fprintf(debug_file, "(%d,%d)=%.6f    ", gauss_info.cx,
          ...
          //		float w_cell_a = 0.0;	//initial Gaussian value of cell
          //afected

          //		// Estimate volume of the Gaussian under cell (a)
          //		// Partition each cell into (p x p) subcells and evaluate
          //the gaussian.
          //		int p = 40;
          ...
          //exp(-0.5*( square(r_ia-r)/square(LUT.std_r) +
          //square(phi_ia-phi)/square(LUT.std_phi) ) );
          //				w += (1/(2*M_PI*LUT.std_r*LUT.std_phi)) * exp(-0.5*(
          //square(r_ia-r)/square(LUT.std_r) +
          //square(phi_ia+2*M_PI-phi)/square(LUT.std_phi) ) );
          //				w += (1/(2*M_PI*LUT.std_r*LUT.std_phi)) * exp(-0.5*(
          //square(r_ia-r)/square(LUT.std_r) +
