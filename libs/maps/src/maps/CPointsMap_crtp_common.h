...
template <class Derived> struct loadFromRangeImpl {
  static inline void
  templ_loadFromRangeScan(Derived &obj,
    ...
    if (!obj.insertionOptions.addToExistingPointsMap)
      obj.resize(0); // Resize to 0 instead of clear() so the std::vector<>
                     // memory is not actually deadllocated and can be reused.
...

    {
      const size_t expectedMaxSize =
          nPointsAtStart +
          (sizeRangeScan * (obj.insertionOptions.also_interpolate ? 3 : 1));
    ...
    // ------------------------------------------------------
    // Use a LUT to convert ranges -> (x,y) ; Automatically computed upon first
    // usage.
    const mrpt::obs::CSinCosLookUpTableFor2DScans::TSinCosValues &sincos_vals =
                ...
                3); // The +3 is to assure there's room for "nPackets*4"
    {
      ...
      // Make sure the input std::vector<> has room enough for reads of 4-float
      // at a time:
      // Invalid reads should not be a problem, but just for safety...
      // JLBC: OCT/2016: rangeScan.scan() is now, by design, ensured to hold
      // vectors of 4*N capacity, so there is no need to call reserve() here.
      ...
      for (; nPackets; nPackets--, ptr_in_scan += 4, ptr_in_cos += 4,
        ...
        _mm_store_ps(
            ptr_out_x,
            _mm_add_ps(m03_4val, _mm_add_ps(_mm_mul_ps(xs, m00_4val),
                                            _mm_mul_ps(ys, m01_4val))));
        _mm_store_ps(
            ptr_out_y,
            _mm_add_ps(m13_4val, _mm_add_ps(_mm_mul_ps(xs, m10_4val),
                                            _mm_mul_ps(ys, m11_4val))));
        _mm_store_ps(
            ptr_out_z,
            _mm_add_ps(m23_4val, _mm_add_ps(_mm_mul_ps(xs, m20_4val),
                                            _mm_mul_ps(ys, m21_4val))));
      }
#else  // MRPT_HAS_SSE2
      // The "+3" is to assure the buffer has room for the SSE2 method which
      // works with 4-tuples of floats.
      ...
      const Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, 1>> scan_vals(
          const_cast<float *>(&rangeScan.scan[0]), rangeScan.scan.size(), 1);
      // SinCos table allocates N+4 floats for the convenience of SSE2: Map to
      // make it appears it has the correct size:
      const Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, 1>> ccos(
          const_cast<float *>(&sincos_vals.ccos[0]), rangeScan.scan.size(), 1);
      const Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, 1>> csin(
          const_cast<float *>(&sincos_vals.csin[0]), rangeScan.scan.size(), 1);
    ...
    for (int i = 0; i < sizeRangeScan; i++) {
      if (rangeScan.validRange[i]) {
        ...
        pointmap_traits<Derived>::internal_loadFromRangeScan2D_prepareOneRange(
            obj, lx, ly, lz, lric);
        ...
        if (thisIsTheFirst || pt_pass_min_dist) {
          ...
          if (obj.insertionOptions.also_interpolate && i > 1) {
            ...
            if ((lx != lx_1 || ly != ly_1) && (lx_1 != lx_2 || ly_1 != ly_2))
              changeInDirection =
                  atan2(ly - ly_1, lx - lx_1) - atan2(ly_1 - ly_2, lx_1 - lx_2);
            else
              changeInDirection = 0;
            ...
            if (d >= 2 * obj.insertionOptions.minDistBetweenLaserPoints &&
              ...
              for (int q = 1; q < nInterpol; q++) {
                ...
                if (!obj.m_heightfilter_enabled ||
                  ...
                  // point:
                  pointmap_traits<
                      Derived>::internal_loadFromRangeScan2D_postPushBack(obj,
                                                                          lric);
                } // end if
              }   // end for
            }     // End of interpolate:
          ...
          if (!obj.m_heightfilter_enabled || (lz >= obj.m_heightfilter_z_min &&
            ...
            pointmap_traits<Derived>::internal_loadFromRangeScan2D_postPushBack(
                obj, lric);
    ...
    if (lastPointWasValid && !lastPointWasInserted) {
      if (!obj.m_heightfilter_enabled ||
          (lz >= obj.m_heightfilter_z_min && lz <= obj.m_heightfilter_z_max)) {
        obj.x[nextPtIdx] = lx;
        obj.y[nextPtIdx] = ly;
        obj.z[nextPtIdx] = lz;
        nextPtIdx++;
  ...
  templ_loadFromRangeScan(Derived &obj,
    ...
    for (size_t i = 0; i < sizeRangeScan; i++) {
      ...
      if (rangeScan.points3D_x[i] != 0 || rangeScan.points3D_y[i] != 0 ||
        ...
        // Specialized work in derived classes:
        pointmap_traits<Derived>::internal_loadFromRangeScan3D_prepareOneRange(
            obj, lx, ly, lz, lric);
        ...
        if (thisIsTheFirst ||
          ...
          obj.z.push_back(lz);
          // Allow derived classes to add any other information to that point:
          pointmap_traits<Derived>::internal_loadFromRangeScan3D_postPushBack(
              obj, lric);
      ...
      pointmap_traits<Derived>::internal_loadFromRangeScan3D_postOneRange(obj,
                                                                          lric);
    }
