...
MAP_DEFINITION_REGISTER("CHeightGridMap2D_MRF,dem_mrf",
                        mrpt::maps::CHeightGridMap2D_MRF)

CHeightGridMap2D_MRF::TMapDefinition::TMapDefinition()
    : run_map_estimation_at_ctor(true), min_x(-2), max_x(2), min_y(-2),
      max_y(2), resolution(0.10f), mapType(mrGMRF_SD) {}
...
// Constructor
CHeightGridMap2D_MRF::CHeightGridMap2D_MRF(TMapRepresentation mapType,
...
bool CHeightGridMap2D_MRF::insertIndividualPoint(
  ...
  if (!cell)
    return false;
  this->insertIndividualReading(z, mrpt::math::TPoint2D(x, y),
                                params.update_map_after_insertion,
...
bool CHeightGridMap2D_MRF::dem_get_z_by_cell(const size_t cx, const size_t cy,
                                             double &z_out) const {
...
bool CHeightGridMap2D_MRF::dem_get_z(const double x, const double y,
                                     double &z_out) const {
  const TRandomFieldCell *cell = cellByPos(x, y);
  if (cell && cell->kf_mean) {
    z_out = cell->kf_mean;
