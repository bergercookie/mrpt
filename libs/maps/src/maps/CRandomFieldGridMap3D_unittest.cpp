...
TEST(CRandomFieldGridMap3D, insertCheckMapBounds) {
  ...
  grid3d.setSize(-4.0, 4.0, 0.0, 4.0, 0.0, 4.0,
  ...
  EXPECT_FALSE(grid3d.insertIndividualReading(val, var, TPoint3D(2.0, 6.0, 3.0),
                                              im, false));
...
TEST(CRandomFieldGridMap3D, insertPointsAndRead) {
  ...
  EXPECT_TRUE(grid3d.insertIndividualReading(val, var, TPoint3D(2.0, 3.0, 1.0),
                                             im, false));
  ...
  grid3d.resize(-5.0, 5.0, -1.0, 5.0, -1.0, 5.0,
                mrpt::maps::TRandomFieldVoxel(), .0);
  {
    const double map_value = grid3d.cellByPos(2.0, 3.0, 1.0)->mean_value;
    EXPECT_NEAR(map_value, val, 1e-6);
  }
