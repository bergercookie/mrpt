...
bool CHeightGridMap2D_Base::intersectLine3D(const mrpt::math::TLine3D &ray,
  ...
  const TPlane horz_plane_above(TPoint3D(0, 0, z_max + 1),
                                TPoint3D(1, 0, z_max + 1),
                                TPoint3D(0, 1, z_max + 1));
  const TPlane horz_plane_below(TPoint3D(0, 0, z_min - 1),
                                TPoint3D(1, 0, z_min - 1),
  ...
  // Now, go from pt_ab -> pt_be doing "ray-tracing" and find the collision with
  // a cell:
  TPoint3D pt = pt_ab;
