...
/*---------------------------------------------------------------
                                        getAsImage
  ---------------------------------------------------------------*/
void COccupancyGridMap2D::getAsImage(utils::CImage &img, bool verticalFlip,
                                     bool forceRGB, bool tricolor) const {
  if (!tricolor) {
...
// ----------------------------------------------------
  ...
  if (insertionOptions.CFD_features_median_size != 0)
    img.filterMedianInPlace(round(insertionOptions.CFD_features_median_size));
#if 0
...
void COccupancyGridMap2D::getAsPointCloud(mrpt::maps::CSimplePointsMap &pm,
                                          const float occup_threshold) const {
  pm.clear();
  pm.reserve(1000);
