...
MAP_DEFINITION_REGISTER("COccupancyGridMap2D,occupancyGrid",
                        mrpt::maps::COccupancyGridMap2D)

COccupancyGridMap2D::TMapDefinition::TMapDefinition()
    : min_x(-10.0f), max_x(10.0f), min_y(-10.0f), max_y(10.0f),
      resolution(0.10f) {}
...
/*---------------------------------------------------------------
                                                Constructor
  ---------------------------------------------------------------*/
COccupancyGridMap2D::COccupancyGridMap2D(float min_x, float max_x, float min_y,
                                         float max_y, float resolution)
    : map(), size_x(0), size_y(0), x_min(), x_max(), y_min(), y_max(),
      resolution(), precomputedLikelihood(),
      precomputedLikelihoodToBeRecomputed(true), m_basis_map(),
      m_voronoi_diagram(), m_is_empty(true), voroni_free_threshold(),
      updateInfoChangeOnly(), insertionOptions(), likelihoodOptions(),
      likelihoodOutputs(), CriticalPointsList() {
  MRPT_START
...
/*---------------------------------------------------------------
                                                setSize
  ---------------------------------------------------------------*/
void COccupancyGridMap2D::setSize(float x_min, float x_max, float y_min,
...
/*---------------------------------------------------------------
                                                ResizeGrid
  ---------------------------------------------------------------*/
void COccupancyGridMap2D::resizeGrid(float new_x_min, float new_x_max,
  ...
  if (new_x_min > new_x_max) {
    printf("[COccupancyGridMap2D::resizeGrid] Warning!! Ignoring call, since: "
           "x_min=%f  x_max=%f\n",
           new_x_min, new_x_max);
    return;
  }
  ...
  if (additionalMargin) {
    if (new_x_min < x_min)
      new_x_min = floor(new_x_min - 4);
    if (new_x_max > x_max)
      new_x_max = ceil(new_x_max + 4);
    if (new_y_min < y_min)
      new_y_min = floor(new_y_min - 4);
    if (new_y_max > y_max)
      new_y_max = ceil(new_y_max + 4);
  }
...
/*---------------------------------------------------------------
 ...
 (meters))
        out_mean_I The target variable for mean information, defined as
 information per square meter: mean_I(map) = I(map) / (Map length x
 (meters))(Map length y (meters))
 ---------------------------------------------------------------*/
void COccupancyGridMap2D::computeEntropy(TEntropyInfo &info) const {
  ...
  info.H = info.I = 0;
  info.effectiveMappedCells = 0;
  for (std::vector<cellType>::const_iterator it = map.begin(); it != map.end();
       ++it) {
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
bool mrpt::maps::
operator<(const COccupancyGridMap2D::TPairLikelihoodIndex &e1,
...
/*---------------------------------------------------------------
                                computePathCost
  ---------------------------------------------------------------*/
float COccupancyGridMap2D::computePathCost(float x1, float y1, float x2,
                                           float y2) const {
  float sumCost = 0;
