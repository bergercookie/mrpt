...
IMPLEMENTS_SERIALIZABLE(CPlanarLaserScan, CRenderizableDisplayList,
                        mrpt::opengl)
...
CPlanarLaserScan::CPlanarLaserScan()
    : m_scan(), m_cache_points(), m_cache_valid(false), m_line_width(1),
      m_line_R(1.f), m_line_G(0.f), m_line_B(0.f), m_line_A(0.5f),
      m_points_width(3), m_points_R(1.0f), m_points_G(0.0f), m_points_B(0.0f),
      m_points_A(1.0f), m_plane_R(0.01f), m_plane_G(0.01f), m_plane_B(0.6f),
      m_plane_A(0.6f), m_enable_points(true), m_enable_line(true),
...
void CPlanarLaserScan::render_dl() const {
  ...
  if (n > 1 && m_enable_surface) {
    ...
    for (i = 0; i < n - 1; i++) {
      glVertex3f(m_scan.sensorPose.x(), m_scan.sensorPose.y(),
                 m_scan.sensorPose.z());
      glVertex3f(x[i], y[i], z[i]);
      glVertex3f(x[i + 1], y[i + 1], z[i + 1]);
    }
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CPlanarLaserScan::writeToStream(mrpt::utils::CStream &out,
                                     int *version) const {
...
void CPlanarLaserScan::readFromStream(mrpt::utils::CStream &in, int version) {
  ...
  case 1: {
    ...
    in >> m_scan;
    in >> m_line_width >> m_line_R >> m_line_G >> m_line_B >> m_line_A >>
        m_points_width >> m_points_R >> m_points_G >> m_points_B >>
        m_points_A >> m_plane_R >> m_plane_G >> m_plane_B >> m_plane_A;
...
void CPlanarLaserScan::getBoundingBox(mrpt::math::TPoint3D &bb_min,
