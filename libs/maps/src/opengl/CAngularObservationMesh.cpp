...
IMPLEMENTS_SERIALIZABLE(CAngularObservationMesh, CRenderizableDisplayList,
                        mrpt::opengl)

void CAngularObservationMesh::addTriangle(const TPoint3D &p1,
                                          const TPoint3D &p2,
                                          const TPoint3D &p3) const {
...
void CAngularObservationMesh::updateMesh() const {
  ...
  if (pitchBounds.size() == 2) {
    ...
    for (size_t i = 0; i < numRows; i++)
      pitchs[i] =
          p1 +
          (p2 - p1) * static_cast<double>(i) / static_cast<double>(numRows - 1);
  } else
    for (size_t i = 0; i < numRows; i++)
  ...
  for (size_t i = 0; i < numRows; i++) {
    ...
    for (size_t j = 0; j < numCols; j++)
      if ((validityMatrix(i, j) = (valid[j] != 0))) {
        ...
        actualMesh(i, j) =
            (origin + CPose3D(0, 0, 0, rToL ? pYaw : -pYaw, pitchIncr)) +
            CPoint3D(scan[j], 0, 0);
      }
  }
  delete[] pitchs;
  ...
  for (size_t k = 0; k < numRows - 1; k++) {
    for (size_t j = 0; j < numCols - 1; j++) {
      ...
      case 3:
        if (!b1)
          addTriangle(actualMesh(k, j + 1), actualMesh(k + 1, j),
                      actualMesh(k + 1, j + 1));
        else if (!b2)
          addTriangle(actualMesh(k, j), actualMesh(k + 1, j),
                      actualMesh(k + 1, j + 1));
        else if (!b3)
          addTriangle(actualMesh(k, j), actualMesh(k, j + 1),
                      actualMesh(k + 1, j + 1));
        else if (!b4)
          addTriangle(actualMesh(k, j), actualMesh(k, j + 1),
                      actualMesh(k + 1, j));
      ...
      case 4:
        addTriangle(actualMesh(k, j), actualMesh(k, j + 1),
                    actualMesh(k + 1, j));
        addTriangle(actualMesh(k + 1, j + 1), actualMesh(k, j + 1),
                    actualMesh(k + 1, j));
      }
    }
  }
...
bool CAngularObservationMesh::traceRay(const mrpt::poses::CPose3D &o,
                                       double &dist) const {
  MRPT_UNUSED_PARAM(o);
...
void CAngularObservationMesh::setPitchBounds(const double initial,
                                             const double final) {
  CRenderizableDisplayList::notifyChange();
...
void CAngularObservationMesh::getPitchBounds(double &initial,
                                             double & final) const {
...
void CAngularObservationMesh::generatePointCloud(CPointsMap *out_map) const {
  ...
  /*	size_t numRows=scanSet.size();
     ...
     pitchs[i]=p1+(p2-p1)*static_cast<double>(i)/static_cast<double>(numRows-1);
          ...
          for (size_t i=0;i<numRows;i++)
     out_map->insertObservation(&scanSet[i]);
  */

  std::for_each(scanSet.begin(), scanSet.end(),
                CAngularObservationMesh_fnctr(out_map));
}
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CAngularObservationMesh::writeToStream(mrpt::utils::CStream &out,
                                            int *version) const {
  if (version)
    *version = 0;
  else {
...
/*---------------------------------------------------------------
        Implements the reading from a CStream capability of
                CSerializable objects
  ---------------------------------------------------------------*/
void CAngularObservationMesh::readFromStream(mrpt::utils::CStream &in,
                                             int version) {
  switch (version) {
  case 0:
...
void CAngularObservationMesh::getTracedRays(CSetOfLines::Ptr &res) const {
  ...
  for (size_t i = 0; i < actualMesh.getRowCount(); i++)
    for (size_t j = 0; j < actualMesh.getColCount(); j++)
      if (validityMatrix(i, j))
        res->appendLine(TPose3D(scanSet[i].sensorPose), actualMesh(i, j));
}
...
public:
  ...
  FAddUntracedLines(CSetOfLines::Ptr &l, const CPoint3D &p,
    ...
    for (size_t i = 0; i < obs.scan.size(); i++)
      if (!obs.validRange[i]) {
        ...
        lins->appendLine(
            ...
            TPoint3D(obs.sensorPose +
                     CPose3D(0, 0, 0, obs.rightToLeft ? yaw : -yaw,
...
void CAngularObservationMesh::getUntracedRays(CSetOfLines::Ptr &res,
                                              double dist) const {
  for_each(scanSet.begin(), scanSet.end(),
...
void CAngularObservationMesh::generateSetOfTriangles(
  ...
  transform(triangles.begin(), triangles.end(), res.begin(),
            createFromTriangle);
}
