...
void CEdgeCounter::addEdge(const std::string &name,
  ...
  if (search != m_name_to_edges_num.end()) {
    ...
    if (is_new) {
      std::string str_err = mrpt::format("Specified edge type [%s] already "
                                         "exists but is_new is also specified!",
                                         name.c_str());
      THROW_EXCEPTION(str_err)
    ...
    if (is_new) {
      ...
      std::string str_err = "No edge with such name exists. Specify is_new "
                            "parameter if you want to add it";
      THROW_EXCEPTION(str_err)
    }
  }
...
void CEdgeCounter::setTextMessageParams(
  ...
  ASSERTMSG_(m_win_manager, "Visualization of data was requested but no "
                            "CWindowManager pointer was provided");
...
void CEdgeCounter::updateTextMessages() const {
  ...
  for (std::map<std::string, double>::const_iterator it =
    ...
    m_win_manager->addTextMessage(5, -offset_y, title.str(),
                                  mrpt::utils::TColorf(1.0, 1.0, 1.0),
