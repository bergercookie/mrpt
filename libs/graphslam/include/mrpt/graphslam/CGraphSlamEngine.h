...
public:
   ...
   *
   * \param[in] Name of the generated graph file - Defaults to "output_graph" if
   * not
   * set by the user
   *
   * \sa saveGraph
   ...
   * \sa computeMap
   */
  void getMap(mrpt::maps::COccupancyGridMap2D::Ptr map,
              mrpt::system::TTimeStamp *acquisition_time = NULL) const;
  void getMap(mrpt::maps::COctoMap::Ptr map,
              mrpt::system::TTimeStamp *acquisition_time = NULL) const;
   ...
   */
  bool execGraphSlamStep(mrpt::obs::CObservation::Ptr &observation,
                         size_t &rawlog_entry);
   ...
   * (e.g. pressed ctrl-c), True otherwise
   **/
  virtual bool _execGraphSlamStep(mrpt::obs::CActionCollection::Ptr &action,
   ...
   */
  static void
  readGTFile(const std::string &fname_GT,
   ...
   * graphslam.
   */
  bool getGraphSlamStats(std::map<std::string, int> *node_stats,
  ...
  void resumeExec() const {
    ...
    if (m_enable_visuals) {
      this->m_win->addTextMessage(0.3, 0.8, "", mrpt::utils::TColorf(1.0, 0, 0),
  ...
  void pauseExec() {
    ...
    m_is_paused = true;
    if (m_enable_visuals) {
      this->m_win->addTextMessage(0.3, 0.8, m_paused_message,
...
protected:
   ...
   */
  void initClass();
  /**\brief Automate the creation and initialization of a results file relevant
   * to
   * the application.
   *
   ...
   */
  virtual mrpt::poses::CPose3D
  getLSPoseForGridMapVisualization(const mrpt::utils::TNodeID nodeID) const;
   ...
   */
  virtual void
  setObjectPropsFromNodeID(const mrpt::utils::TNodeID nodeID,
   ...
   */
  void
  updateMapVisualization(const std::map<mrpt::utils::TNodeID,
                                        mrpt::obs::CObservation2DRangeScan::Ptr>
   ...
   */
  void decimateLaserScan(mrpt::obs::CObservation2DRangeScan &laser_scan_in,
  ...
  getTimeStamp(const mrpt::obs::CActionCollection::Ptr action,
   ...
   */
  /**\{*/
  static double accumulateAngleDiffs(const mrpt::poses::CPose2D &p1,
                                     const mrpt::poses::CPose2D &p2);
  static double accumulateAngleDiffs(const mrpt::poses::CPose3D &p1,
   ...
   */
  virtual void monitorNodeRegistration(bool registered = false,
