...
public:
  ...
  /**\brief Wrapper around the CDisplayWindow3D::addTextMessage method, so that
   * the
  ...
  void addTextMessage(
    ...
    win->addTextMessage(x, y, text, color, m_font_name, m_font_size,
                        mrpt::opengl::NICE, unique_index);
  }
   ...
   */
  void assignViewportParameters(double *x, double *y, double *width,
                                double *height) {
    *x = m_viewp_x;
    *y = m_viewp_y;
