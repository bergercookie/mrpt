...
/** Optimize a graph of pose constraints using the Sparse Pose Adjustment (SPA)
  ...
  *iterations.
  *		- "scale_hessian": (default=0.1) Multiplies the Hessian matrix by
  *this scalar (may improve convergence speed).
  *		- "initial_lambda": (default=0) <=0 means auto guess, otherwise,
  *initial lambda value for the lev-marq algorithm.
  *		- "tau": (default=1e-3) Initial tau value for the lev-marq
  *algorithm.
  *		- "e1": (default=1e-6) Lev-marq algorithm iteration stopping
  *criterion #1: |gradient| < e1
  *		- "e2": (default=1e-6) Lev-marq algorithm iteration stopping
  *criterion #2: |delta_incr| < e2*(x_norm+e2)
  *
...
void optimize_graph_spa_levmarq(
  ...
  // Make list of node IDs to optimize, since the user may want only a subset of
  // them to be optimized:
  profiler.enter("optimize_graph_spa_levmarq.list_IDs"); // ---------------\  .
  const set<TNodeID> *nodes_to_optimize;
  set<TNodeID>
      nodes_to_optimize_auxlist; // Used only if in_nodes_to_optimize==nullptr
  if (in_nodes_to_optimize) {
    ...
    for (typename gst::graph_t::global_poses_t::const_iterator it =
      ...
      if (it->first != graph.root) // Root node is fixed.
        nodes_to_optimize_auxlist.insert(nodes_to_optimize_auxlist.end(),
                                         it->first); // Provide the "first
                                                     // guess" insert position
                                                     // for efficiency
  ...
  // The list of those edges that will be considered in this optimization (many
  // may be discarded
  ...
  vector<pair<size_t, size_t>>
      observationIndex_to_relatedFreeNodeIndex; // "relatedFreeNodeIndex" means
                                                // into [0,nFreeNodes-1], or
                                                // "-1" if that node is fixed,
                                                // as ordered in
                                                // "nodes_to_optimize"
  ...
  for (typename gst::map_pairIDs_pairJacobs_t::const_iterator itJ =
    ...
    observationIndex_to_relatedFreeNodeIndex.push_back(
        std::make_pair(mrpt::utils::find_in_vector(id1, *nodes_to_optimize),
  ...
  for (size_t iter = 0; iter < max_iters; ++iter) {
    ...
    if (have_to_recompute_H_and_grad) // This will be false only when the delta
                                      // leads to a worst solution and only a
                                      // change in lambda is needed.
    {
      ...
      // that is: g_i is the "dot-product" of the i'th (transposed) block-column
      // of J and the vector of errors "errs"
      profiler.enter(
...

      {
        ...
        for (idx_obs = 0, itJ = lstJacobians.begin(); itJ != lstJacobians.end();
             ++itJ, ++idx_obs) {
          ASSERTDEB_(
              itJ->first ==
              lstObservationData[idx_obs]
                  .edge->first) // make sure they're in the expected order!

          //  grad[k] += J^t_{i->k} * Inf.Matrix * errs_i
          //    k: [0,nFreeNodes-1]     <-- IDs.first & IDs.second
          //    i: [0,nObservations-1]  <--- idx_obs

          // Get the corresponding indices in the vector of "free variables"
          // being optimized:
          const size_t idx1 =
              observationIndex_to_relatedFreeNodeIndex[idx_obs].first;
          const size_t idx2 =
              observationIndex_to_relatedFreeNodeIndex[idx_obs].second;

          if (idx1 != string::npos)
            detail::AuxErrorEval<typename gst::edge_t, gst>::multiply_Jt_W_err(
                itJ->second.first /* J */,
                lstObservationData[idx_obs].edge /* W */,
                errs[idx_obs] /* err */, grad_parts[idx1] /* out */
                );
          ...
          if (idx2 != string::npos)
            detail::AuxErrorEval<typename gst::edge_t, gst>::multiply_Jt_W_err(
                itJ->second.second /* J */,
                lstObservationData[idx_obs].edge /* W */,
                errs[idx_obs] /* err */, grad_parts[idx2] /* out */
                );
        }
      }

      // build the gradient as a single vector:
      ::memcpy(&grad[0], &grad_parts[0],
      ...
      // End condition #1
      const double grad_norm_inf =
          math::norm_inf(grad); // inf-norm (abs. maximum value) of the gradient
      if (grad_norm_inf <= e1) {
        // Change is too small
        if (verbose)
          cout << "[" << __CURRENT_FUNCTION_NAME__ << "] "
               << mrpt::format(
                      "End condition #1: math::norm_inf(g)<=e1 :%f<=%f\n",
                      grad_norm_inf, e1);
        break;
      }
      ...
      //              Here "i" corresponds to [0,N-1] indices of appearance in
      //              the map "*nodes_to_optimize".
      //  - H_map[i][j] is the entry for the j'th row, with "j" also in the
      //  range [0,N-1] as ordered in "*nodes_to_optimize".
      // ======================================================================
      {
        size_t idxObs;
        ...
        for (idxObs = 0, itJacobPair = lstJacobians.begin();
          ...
          // We sort IDs such as "i" < "j" and we can build just the upper
          ...
          // Take references to both Jacobians (wrt pose "i" and pose "j"),
          ...
          if (is_i_free_node) {
            ...
            detail::AuxErrorEval<typename gst::edge_t, gst>::multiplyJtLambdaJ(
          ...
          if (is_i_free_node && is_j_free_node) {
            ...
            detail::AuxErrorEval<typename gst::edge_t, gst>::
                multiplyJ1tLambdaJ2(J1, J2, JtJ,
                                    lstObservationData[idxObs].edge);
      ...
      // Just in the first iteration, we need to calculate an estimate for the
      // first value of "lamdba":
      if (lambda <= 0 && iter == 0) {
          ...
          for (typename map_ID2matrix_VxV_t::const_iterator it =
            ...
            if (i == j) {
              for (size_t k = 0; k < DIMS_POSE; k++)
                mrpt::utils::keep_max(H_diagonal_max,
    ...
    if (verbose)
      cout << "[" << __CURRENT_FUNCTION_NAME__ << "] Iter: " << iter
           ...
           << std::sqrt(total_sqr_err / nObservations) << " lambda: " << lambda
           << endl;
    ...
    // Note: we only need to fill out the upper diagonal part, since Cholesky
    // will later on ignore the other part.
    CSparseMatrix sp_H(nFreeNodes * DIMS_POSE, nFreeNodes * DIMS_POSE);
    for (size_t i = 0; i < nFreeNodes; i++) {
      const size_t i_offset = i * DIMS_POSE;

      for (typename map_ID2matrix_VxV_t::const_iterator it = H_map[i].begin();
           it != H_map[i].end(); ++it) {
        const size_t j = it->first; // entry submatrix is for (i,j).
        const size_t j_offset = j * DIMS_POSE;

        // For i==j (diagonal blocks), it's different, since we only need to
        // insert their
        // upper-diagonal half and also we have to add the lambda*I to the
        // diagonal from the Lev-Marq. algorithm:
        if (i == j) {
          for (size_t r = 0; r < DIMS_POSE; r++) {
            // c=r: add lambda from LM
            sp_H.insert_entry_fast(j_offset + r, i_offset + r,
                                   it->second.get_unsafe(r, r) + lambda);
            // c>r:
            for (size_t c = r + 1; c < DIMS_POSE; c++)
              sp_H.insert_entry_fast(j_offset + r, i_offset + c,
    ...
    //          A         x   =  b         -->       x = A^{-1} * b
    //
    CVectorDouble delta; // The (minus) increment to be added to the current
                         // solution in this step
    try {
      profiler.enter("optimize_graph_spa_levmarq.sp_H:chol");
      if (!ptrCh.get())
        ptrCh =
            SparseCholeskyDecompPtr(new CSparseMatrix::CholeskyDecomp(sp_H));
      else
      ...
      if (verbose)
        cout << "[" << __CURRENT_FUNCTION_NAME__ << "] Got non-definite "
                                                    "positive matrix, retrying "
                                                    "with a larger lambda...\n";
      lambda *= v;
      v *= 2;
      if (lambda > 1e9) { // enough!
        break;
      }
    ...
    if (delta_norm < thres_norm) {
      ...
      // =====================================================================================
      // Accept this delta? Try it and look at the increase/decrease of the
      // error:
...

      {
        ...
        for (set<TNodeID>::const_iterator it = nodes_to_optimize->begin();
          ...
          for (size_t i = 0; i < DIMS_POSE; i++)
            exp_delta[i] = -*delta_ptr++; // The "-" sign is for the missing "-"
                                          // carried all this time from above
          gst::SE_TYPE::exp(exp_delta, exp_delta_pose);

          // new_x_i =  exp_delta_i (+) old_x_i
          typename gst::graph_t::global_poses_t::iterator it_old_value =
              graph.nodes.find(*it);
          old_poses_backup[*it] =
              it_old_value->second; // back up the old pose as a copy
          it_old_value->second.composeFrom(exp_delta_pose,
                                           it_old_value->second);
        }
      }
