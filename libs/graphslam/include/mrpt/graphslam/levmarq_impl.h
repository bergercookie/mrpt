...
double computeJacobiansAndErrors(
  ...
  for (size_t i = 0; i < nObservations; i++) {
    ...
    MRPT_ALIGN16 std::pair<mrpt::utils::TPairNodeIDs, typename gst::TPairJacobs>
        newMapEntry;
  ...
  // std::accumulate(...,mrpt::math::squareNorm_accum<>), but led to GCC errors
  // when enabling parallelization)
  double ret_err = 0.0;
  for (size_t i = 0; i < errs.size(); i++)
    ret_err += errs[i].squaredNorm();
  return ret_err;
}
