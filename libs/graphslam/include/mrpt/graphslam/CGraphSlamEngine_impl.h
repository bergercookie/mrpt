...
CGraphSlamEngine<GRAPH_T>::CGraphSlamEngine(
    ...
    : m_node_reg(node_reg), m_edge_reg(edge_reg), m_optimizer(optimizer),
      m_enable_visuals(win_manager != nullptr), m_config_fname(config_file),
      m_rawlog_fname(rawlog_fname), m_fname_GT(fname_GT), m_GT_poses_step(1),
      ...
      m_odometry_color(0, 0, 255), m_GT_color(0, 255, 0),
      m_estimated_traj_color(255, 165, 0), m_optimized_map_color(255, 0, 0),
      m_current_constraint_type_color(255, 255, 255), m_robot_model_size(1),
      m_class_name("CGraphSlamEngine"), m_is_first_time_node_reg(true) {
  this->initClass();
};

template <class GRAPH_T> CGraphSlamEngine<GRAPH_T>::~CGraphSlamEngine() {
...
template <class GRAPH_T> void CGraphSlamEngine<GRAPH_T>::initClass() {
  MRPT_START;
  using namespace mrpt;
  using namespace mrpt::utils;
  ...
  // Assert that the graph class used is supported.
  {
    ...
    bool found = (std::find(m_supported_constraint_types.begin(),
    ...
    if (found) {
      ...
      MRPT_LOG_WARN_STREAM("Given graph class "
                           << c_str << " has not been tested consistently yet."
  ...
  if (!m_use_GT) {
    MRPT_LOG_WARN_STREAM("Ground truth file was not provided. Switching the "
                         "related visualization parameters off...");
    m_visualize_GT = 0;
    m_visualize_SLAM_metric = 0;
  }
  ...
  try {
    ...
    int num_of_objects =
        std::atoi(m_info_params.fields["Overall number of objects"].c_str());
  ...
  if (m_enable_visuals) {
    this->m_win->addTextMessage(0.5, 0.3, "", mrpt::utils::TColorf(1.0, 0, 0),
...
bool CGraphSlamEngine<GRAPH_T>::_execGraphSlamStep(
  ...
  this->monitorNodeRegistration(registered_new_node, "NodeRegistrationDecider");
  ...
  this->monitorNodeRegistration(registered_new_node, "EdgeRegistrationDecider");
  ...
  if (m_use_GT) {
    if (mrpt::system::strCmpI(m_GT_file_format, "rgbd_tum")) { // 1/loop
      ...
      if (m_observation_only_dataset) { // 1/2loops
        if (rawlog_entry % 2 == 0) {
          if (m_enable_visuals) {
            this->updateGTVisualization(); // I have already taken care of the
                                           // step
          }
          m_GT_poses_index += m_GT_poses_step;
        }
...
void CGraphSlamEngine<GRAPH_T>::loadParams(const std::string &fname) {
  ...
  // map visualization
  m_visualize_map = cfg_file.read_bool("VisualizationParameters",
                                       "visualize_map", true, false);
  ...
  m_visualize_estimated_trajectory = cfg_file.read_bool(
      "VisualizationParameters", "visualize_estimated_trajectory", true, false);

  // GT configuration visualization
  m_visualize_GT = cfg_file.read_bool("VisualizationParameters",
                                      "visualize_ground_truth", true, false);
  // SLAM metric plot
...
template <class GRAPH_T> void CGraphSlamEngine<GRAPH_T>::printParams() const {
  MRPT_START;
...
// current time vars
  ...
  m_out_streams[fname]->printf("# Automatically generated file - %s: %s\n",
                               time_spec.c_str(), cur_date_str.c_str());
...
template <class GRAPH_T> void CGraphSlamEngine<GRAPH_T>::updateAllVisuals() {
  MRPT_START;
  std::lock_guard<std::mutex> m_graph_lock(m_graph_section);
  m_time_logger.enter("Visuals");
...
void CGraphSlamEngine<GRAPH_T>::updateRangeImageViewport() {
  ...
  if (m_last_laser_scan3D->hasRangeImage) {
    ...
    // TODO - make this a static class member - or at least a private member of
    // the class
...
template <class GRAPH_T> void CGraphSlamEngine<GRAPH_T>::initCurrPosViewport() {
  MRPT_START;
  ASSERT_(m_enable_visuals);
  using namespace mrpt::opengl;
  using namespace mrpt::utils;
...
void CGraphSlamEngine<GRAPH_T>::readGTFile(
  ...
  for (size_t line_num = 0; file_GT.readLine(curr_line); line_num++) {
    ...
    ASSERTMSG_(
        ...
        "GridMapNavSimul application.\n Either specify the GT file format or "
        "set "
    ...
    // pose
    pose_t curr_pose(atof(curr_tokens[1].c_str()), atof(curr_tokens[2].c_str()),
...
void CGraphSlamEngine<GRAPH_T>::readGTFileRGBD_TUM(
  ...
  for (; file_GT.readLine(curr_line);) {
    ...
    system::tokenize(curr_line, " ", curr_tokens);
    ASSERTMSG_(curr_tokens.size() == 8,
               "\nGround Truth File doesn't seem to contain data as specified "
               "in RGBD_TUM related "
               "datasets.\n Either specify correct the GT file format or set "
               "visualize_ground_truth to false in the .ini file\n");
...
void CGraphSlamEngine<GRAPH_T>::queryObserverForEvents() {
  MRPT_START;
  ASSERT_(m_enable_visuals);
  ASSERTMSG_(m_win_observer, "\nqueryObserverForEvents method was called even "
                             "though no Observer object was provided\n");
  using namespace mrpt::utils;
...
void CGraphSlamEngine<GRAPH_T>::updateMapVisualization(
  ...
  {
    if (full_update) {
      // for all the nodes get the node position and the corresponding laser
      // scan
  ...
  for (std::set<mrpt::utils::TNodeID>::const_iterator node_it =
    ...
    if (search != nodes_to_laser_scans2D.end() && search->second) {
      ...
      // if the scan doesn't already exist, add it to the map object, otherwise
      // just
      // adjust its pose
...
void CGraphSlamEngine<GRAPH_T>::decimateLaserScan(
  ...
  laser_scan_out->loadFromVectors(new_scan_size, &new_scan[0],
                                  &new_validRange[0]);

  MRPT_END;
}

template <class GRAPH_T> void CGraphSlamEngine<GRAPH_T>::initGTVisualization() {
  MRPT_START;
  ASSERT_(m_enable_visuals);
  using namespace mrpt::utils;
  using namespace mrpt::opengl;
  ...
  // assertions
  ASSERT_(m_has_read_config);
  ASSERT_(m_win && "Visualization of data was requested but no "
                   "CDisplayWindow3D pointer was provided");
...
void CGraphSlamEngine<GRAPH_T>::updateGTVisualization() {
  ...
  if (m_visualize_GT && m_GT_poses_index < m_GT_poses.size()) {
    ASSERT_(m_win && "Visualization of data was requested but no "
                     "CDisplayWindow3D pointer was given");
...
void CGraphSlamEngine<GRAPH_T>::updateOdometryVisualization() {
  MRPT_START;
  ASSERT_(m_enable_visuals);
  ASSERTMSG_(m_win, "Visualization of data was requested but no "
                    "CDisplayWindow3D pointer was given");
  using namespace mrpt::opengl;
...
void CGraphSlamEngine<GRAPH_T>::initEstimatedTrajectoryVisualization() {
  ...
  if (m_visualize_estimated_trajectory) {
    ...
    m_win_manager->addTextMessage(
        ...
        mrpt::format("Estimated trajectory"), TColorf(m_estimated_traj_color),
...
void CGraphSlamEngine<GRAPH_T>::updateEstimatedTrajectoryVisualization(
  ...
  if (m_visualize_estimated_trajectory) {
    ...
    for (std::set<mrpt::utils::TNodeID>::const_iterator it = nodes_set.begin();
         it != nodes_set.end(); ++it) {
...
void CGraphSlamEngine<GRAPH_T>::save3DScene(
    const std::string *fname_in /* = NULL */) const {
  MRPT_START;
  ASSERTMSG_(m_enable_visuals, "\nsave3DScene was called even though "
                               "enable_visuals flag is off.\nExiting...\n");
  using namespace mrpt::opengl;
  using namespace mrpt::utils;
...
template <class GRAPH_T>
void CGraphSlamEngine<GRAPH_T>::computeSlamMetric(mrpt::utils::TNodeID nodeID,
                                                  size_t gt_index) {
  MRPT_START;
  using namespace mrpt::utils;
  using namespace mrpt::math;
...
void CGraphSlamEngine<GRAPH_T>::generateReportFiles(
  ...
  if (m_use_GT) { // slam evaluation metric
    ...
    const std::string desc(
        "# File includes the evolution of the SLAM metric.  Implemented metric "
        "computes the \"deformation energy\" that is needed to transfer the "
        "estimated trajectory into the ground-truth trajectory (computed as "
        "sum of the difference between estimated trajectory and ground truth "
        "consecutive poses See \"A comparison of SLAM algorithms based on a "
        "graph of relations - W.Burgard et al.\", for more details on the "
        "metric.\n");
