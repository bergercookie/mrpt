...
template <class GRAPH_t> TUserOptionsChecker<GRAPH_t>::~TUserOptionsChecker() {
  using namespace std;
  ...
  for (vector<TOptimizerProps *>::iterator it = optimizers_descriptions.begin();
...
void TUserOptionsChecker<GRAPH_t>::createDeciderOptimizerMappings() {
  ...
  optimizers_map["CEmptyGSO"] = &createGraphSlamOptimizer<CLevMarqGSO<GRAPH_t>>;
...
void TUserOptionsChecker<GRAPH_t>::populateDeciderOptimizerProperties() {
  ...
  { // CFixedIntervalsNRD
    ...
    dec->description = "Register a new node if the distance from the previous "
                       "node surpasses a predefined distance threshold. Uses "
                       "odometry information for estimating the robot movement";
    dec->type = "Node";
  ...
  { // CICPCriteriaNRD
    ...
    dec->description = "Register a new node if the distance from the previous "
                       "node surpasses a predefined distance threshold. Uses "
                       "2D/3D RangeScans alignment for estimating the robot "
                       "movement";
    dec->type = "Node";
  ...
  { // CICPCriteriaERD
    ...
    dec->description = "Register a new edge by aligning the provided 2D/3D "
                       "RangeScans of 2 nodes. Uses the goodness of the ICP "
                       "Alignment as the criterium for adding a new edge";
    dec->type = "Edge";
    dec->rawlog_format = "Both";
  ...
  { // CLoopCloserERD
    ...
    dec->description = "Partition the map and register *sets* of edges based "
                       "on the Pairwise consistency matrix of each set.";
    dec->type = "Edge";
    dec->rawlog_format = "Both";
    dec->observations_used.push_back("CObservation2DRangeScan - Format #1, #2");
    dec->is_slam_2d = true;
