...
CGraphSlamHandler<GRAPH_T>::CGraphSlamHandler(
    ...
    : m_logger(logger), m_options_checker(options_checker),
      m_do_save_results(true), m_has_set_fnames(false),
...
//////////////////////////////////////////////////////////////////////////////
template <class GRAPH_T> CGraphSlamHandler<GRAPH_T>::~CGraphSlamHandler() {
  ...
  if (m_win) {
    m_logger->logFmt(
        LVL_WARN, "Application will exit when the display window is closed.");
...
void CGraphSlamHandler<GRAPH_T>::initOutputDir(
  ...
  m_logger->logFmt(LVL_INFO, "Setting up output directory: %s",
  ...
  if (directoryExists(output_dir_fname)) {
    ...
    default: {
      ...
      m_logger->logFmt(LVL_INFO, "Renaming directory to: %s",
                       dst_fname.c_str());
      string error_msg;
...
void CGraphSlamHandler<GRAPH_T>::readConfigFname(const std::string &fname) {
  ...
  m_save_graph_fname = cfg_file.read_string(
      "GeneralConfiguration", "save_graph_fname", "output_graph.graph", false);
...
template <class GRAPH_T> void CGraphSlamHandler<GRAPH_T>::printParams() const {
...
void CGraphSlamHandler<GRAPH_T>::setResultsDirName(const std::string &dirname) {
  ...
  m_logger->logFmt(LVL_WARN, "Overriding .ini Results directory -> %s...",
...
void CGraphSlamHandler<GRAPH_T>::saveResults(
  ...
  if (m_save_graph) {
    std::string save_graph_fname = output_dir_fname + "/" + m_save_graph_fname;
...
template <class GRAPH_T> void CGraphSlamHandler<GRAPH_T>::execute() {
  ...
  while (CRawlog::getActionObservationPairOrObservation(
    ...
    cont_exec = m_engine->_execGraphSlamStep(action, observations, observation,
                                             curr_rawlog_entry);
  }
...
//////////////////////////////////////////////////////////////////////////////
template <class GRAPH_T> void CGraphSlamHandler<GRAPH_T>::initVisualization() {
