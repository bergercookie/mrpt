...
CLoopCloserERD<GRAPH_T>::CLoopCloserERD()
    : m_visualize_curr_node_covariance(false),
      ...
      m_partitions_full_update(false), m_is_first_time_node_reg(true),
...
template <class GRAPH_T> CLoopCloserERD<GRAPH_T>::~CLoopCloserERD() {
  ...
  if (registered_new_node) {
    ...
    if (!this->m_override_registered_nodes_check) {
      if (!((num_registered == 1) ^
        ...
        MRPT_LOG_ERROR_STREAM("Invalid number of registered nodes since last "
                              "call to updateStates| "
                              "Found \""
    ...
    m_partitions_full_update = ((this->m_graph->nodeCount() %
    ...
    this->updateMapPartitions(m_partitions_full_update,
                              /* is_first_time_node_reg = */ num_registered ==
                                  2);
...
void CLoopCloserERD<GRAPH_T>::fetchNodeIDsForScanMatching(
  ...
  for (int nodeID_i = static_cast<int>(curr_nodeID) - 1;
       ((fetched_nodeIDs <= this->m_laser_params.prev_nodes_for_ICP) &&
        (nodeID_i >= 0)); // <----- I *have* to use int (instead of unsigned) if
...
void CLoopCloserERD<GRAPH_T>::addScanMatchingEdges(
  ...
  for (std::set<TNodeID>::const_iterator node_it = nodes_set.begin();
    ...
    MRPT_LOG_DEBUG_STREAM(
        "Fetching laser scan for nodes: " << *node_it << "==> " << curr_nodeID);
    ...
    double goodness_thresh = m_laser_params.goodness_threshold_win.getMedian() *
                             m_consec_icp_constraint_factor;
    bool accept_goodness = icp_info.goodness > goodness_thresh;
    MRPT_LOG_DEBUG_STREAM("Curr. Goodness: "
                          << icp_info.goodness
...
bool CLoopCloserERD<GRAPH_T>::getICPEdge(
  ...
  bool from_success =
      this->getPropsOfNodeID(from, &from_pose, from_scan, from_params); // TODO
  // to-node parameters
  ...
  MRPT_LOG_DEBUG_STREAM("from_pose: " << from_pose << "| to_pose: " << to_pose
                                      << "| init_estim: " << initial_estim);

  range_ops_t::getICPEdge(*from_scan, *to_scan, rel_edge, &initial_estim,
                          icp_info);
  MRPT_LOG_DEBUG_STREAM("*************");
...
bool CLoopCloserERD<GRAPH_T>::fillNodePropsFromGroupParams(
  ...
  typename std::map<mrpt::utils::TNodeID, node_props_t>::const_iterator search =
      group_params.find(nodeID);
  bool res = false;
  ...
  for (partitions_t::const_iterator partitions_it = m_curr_partitions.begin();
    ...
    if (m_lc_params.LC_check_curr_partition_only) {
      bool curr_node_in_curr_partition =
          ((find(partitions_it->begin(), partitions_it->end(),
    ...
    for (vector_uint::const_iterator it = partitions_it->begin() + 1;
      ...
      if ((curr_nodeID - prev_nodeID) > m_lc_params.LC_min_nodeid_diff) {
        ...
        if (num_after_nodes >= m_lc_params.LC_min_remote_nodes &&
            num_before_nodes >=
                m_lc_params.LC_min_remote_nodes) { // at least X LC nodes
          MRPT_LOG_WARN_STREAM(
              "Found potential loop closures:"
              << endl
              << "\tPartitionID: " << partitionID << endl
              << "\tPartition: "
              << getSTLContainerAsString(*partitions_it).c_str() << endl
...
void CLoopCloserERD<GRAPH_T>::evaluatePartitionsForLC(
  ...
  for (partitions_t::const_iterator p_it = partitions.begin();
    ...
    this->splitPartitionToGroups(partition, &groupA, &groupB,
    ...
    this->generatePWConsistenciesMatrix(groupA, groupB, hypots_pool,
                                        &consist_matrix);

    // evaluate resulting matrix - fill valid hypotheses
    hypotsp_t valid_hypots;
    this->evalPWConsistenciesMatrix(consist_matrix, hypots_pool, &valid_hypots);
...
void CLoopCloserERD<GRAPH_T>::evalPWConsistenciesMatrix(
  ...
  bool valid_lambda_ratio =
      this->computeDominantEigenVector(consist_matrix, &u,
  ...
  for (int i = 0; i != w.size(); ++i) {
    ...
    ss << mrpt::format("current: %f | potential_dot_product: %f", dot_product,
  ...
  *groupB = vector_uint(partition.begin() + index_to_split, partition.end());
  ...
  if (max_nodes_in_group != -1) {
    ...
    if (groupA->size() > static_cast<size_t>(max_nodes_in_group)) {
      *groupA =
          vector_uint(groupA->begin(), groupA->begin() + max_nodes_in_group);
    }
    // groupB
    if (groupB->size() > static_cast<size_t>(max_nodes_in_group)) {
      *groupB = vector_uint(groupB->end() - max_nodes_in_group, groupB->end());
    }
  }
...
void CLoopCloserERD<GRAPH_T>::generateHypotsPool(
  ...
  int invalid_hypots = 0; // just for keeping track of them.
  {
    ...
    for (vector_uint::const_iterator // B - from
      ...
      for (vector_uint::const_iterator // A - to
        ...
        if (ad_params) {
          ...
          fillNodePropsFromGroupParams(*b_it, ad_params->groupB_params,
                                       &icp_ad_params->from_params);
          // to_success = fillNodePropsFromGroupParams(
          fillNodePropsFromGroupParams(*a_it, ad_params->groupA_params,
        ...
        constraint_t edge;
        bool found_edge =
            this->getICPEdge(*b_it, *a_it, &edge, &icp_info, icp_ad_params);
    ...
    MRPT_LOG_DEBUG_STREAM("Generated pool of hypotheses...\tsize = "
...
bool CLoopCloserERD<GRAPH_T>::computeDominantEigenVector(
  ...
  if (use_power_method) {
    ...
    ASSERTMSG_(eigvecs.size() == eigvals.size() &&
               ...
               mrpt::format("Size of eigvecs \"%lu\","
                            "eigvalues \"%lu\","
  ...
  if (approximatelyEqual(0.0, lambda2, /**limit = */ 0.00001)) {
    MRPT_LOG_ERROR_STREAM("Bad lambda2 value: "
                          << lambda2 << " => Skipping current evaluation."
                          << endl);
    return false;
  }
  double curr_lambda_ratio = lambda1 / lambda2;
  MRPT_LOG_DEBUG_STREAM("lambda1 = " << lambda1 << " | lambda2 = " << lambda2
                                     << "| ratio = " << curr_lambda_ratio
                                     << endl);
...
void CLoopCloserERD<GRAPH_T>::generatePWConsistenciesMatrix(
  ...
  for (vector_uint::const_iterator b1_it = groupB.begin();
    ...
    // b2
    for (vector_uint::const_iterator b2_it = b1_it + 1; b2_it != groupB.end();
         ++b2_it) {
      TNodeID b2 = *b2_it;
          ...
          if (hypot_b2_a1->is_valid && hypot_b1_a2->is_valid) {
            ...
            if (curr_opt_paths) {
              ...
              if (groupA_opt_paths) { // a1 -> a2 optimal path
                const path_t *p = this->findPathByEnds(*groupA_opt_paths, a1,
                                                       a2, /*throw_exc=*/true);
              ...
              if (groupB_opt_paths) { // b1 -> b2 optimal path
                const path_t *p = this->findPathByEnds(*groupB_opt_paths, b1,
                                                       b2, /*throw_exc=*/true);
                curr_opt_paths->push_back(*p);
              } else { // empty
                curr_opt_paths->push_back(path_t());
...
CLoopCloserERD<GRAPH_T>::findPathByEnds(const paths_t &vec_paths,
  ...
  if (throw_exc && !res) {
    THROW_EXCEPTION(format("Path for %lu => %lu is not found. Exiting...\n",
...
CLoopCloserERD<GRAPH_T>::findHypotByEnds(const hypotsp_t &vec_hypots,
...
template <class GRAPH_T>
mrpt::graphs::detail::THypothesis<GRAPH_T> *
CLoopCloserERD<GRAPH_T>::findHypotByID(const hypotsp_t &vec_hypots,
                                       const size_t &id,
                                       bool throw_exc /*=true*/) {
  using namespace mrpt::graphslam::detail;
...
void CLoopCloserERD<GRAPH_T>::execDijkstraProjection(
  ...
  ASSERTMSG_(starting_node != ending_node,
  ...
  // initialize a pool of TUncertaintyPaths - draw the minimum-uncertainty path
  // during
  // execution
...
void CLoopCloserERD<GRAPH_T>::addToPaths(
  ...
  for (std::set<TNodeID>::const_iterator neigh_it = neighbors.begin();
    ...
    if (*neigh_it == second_to_last_node)
      continue;
...
void CLoopCloserERD<GRAPH_T>::getMinUncertaintyPath(
  ...
  ASSERTMSG_(this->m_graph->edgeExists(from, to) ||
...
bool CLoopCloserERD<GRAPH_T>::mahalanobisDistanceOdometryToICPEdge(
  ...
  // cout << "Suggested Edge: " << rel_edge.getMeanVal() << "|\tInitial Estim.:
  // " << initial_estim
...
template <class GRAPH_T>
void CLoopCloserERD<GRAPH_T>::registerNewEdge(const mrpt::utils::TNodeID &from,
                                              const mrpt::utils::TNodeID &to,
                                              const constraint_t &rel_edge) {
  MRPT_START;
  using namespace mrpt::utils;
...
void CLoopCloserERD<GRAPH_T>::updateMapPartitionsVisualization() {
  ...
  for (partitions_t::const_iterator p_it = m_curr_partitions.begin();
    ...
    if (std::find(nodes_list.begin(), nodes_list.end(),
    ...
    if (obj) {
      ...
      if (m_lc_params.LC_check_curr_partition_only) { // make all but the last
                                                      // partition invisible
      ...
      CSetOfLines::Ptr connecting_lines_obj = std::make_shared<CSetOfLines>();
...
void CLoopCloserERD<GRAPH_T>::initLaserScansVisualization() {
  ...
  if (m_laser_params.visualize_laser_scans) {
    ...
    laser_scan_viz->setSurfaceColor(
        m_laser_params.laser_scans_color.R, m_laser_params.laser_scans_color.G,
        m_laser_params.laser_scans_color.B, m_laser_params.laser_scans_color.A);
...
template <class GRAPH_T> void CLoopCloserERD<GRAPH_T>::initializeVisuals() {
  MRPT_START;
...
template <class GRAPH_T> void CLoopCloserERD<GRAPH_T>::updateVisuals() {
  MRPT_START;
...
void CLoopCloserERD<GRAPH_T>::loadParams(const std::string &source_fname) {
  ...
  m_consec_icp_constraint_factor =
      source.read_double("EdgeRegistrationDeciderParameters",
                         "consec_icp_constraint_factor", 0.90, false);
  m_lc_icp_constraint_factor =
      source.read_double("EdgeRegistrationDeciderParameters",
                         "lc_icp_constraint_factor", 0.70, false);

  // set the logging level if given by the user
  int min_verbosity_level = source.read_int("EdgeRegistrationDeciderParameters",
                                            "class_verbosity", 1, false);

  this->setMinLoggingLevel(mrpt::utils::VerbosityLevel(min_verbosity_level));
  MRPT_END;
}
template <class GRAPH_T> void CLoopCloserERD<GRAPH_T>::printParams() const {
  MRPT_START;
  using namespace std;
...
void CLoopCloserERD<GRAPH_T>::updateMapPartitions(
  ...
  for (typename nodes_to_scans2D_t::const_iterator it = nodes_to_scans.begin();
       it != nodes_to_scans.end(); ++it) {
    if (it->second) { // if laserScan invalid go to next...
      MRPT_LOG_WARN_STREAM("nodeID \"" << it->first
...
CLoopCloserERD<GRAPH_T>::TLaserParams::TLaserParams()
    : laser_scans_color(0, 20, 255), keystroke_laser_scans("l"),
...
CLoopCloserERD<GRAPH_T>::TLoopClosureParams::TLoopClosureParams()
    : keystroke_map_partitions("b"), balloon_elevation(3), balloon_radius(0.5),
      balloon_std_color(153, 0, 153), balloon_curr_color(62, 0, 80),
      connecting_lines_color(balloon_std_color), has_read_config(false) {}
...
void CLoopCloserERD<GRAPH_T>::TLoopClosureParams::loadFromConfigFile(
  ...
  LC_min_nodeid_diff =
      source.read_int("GeneralConfiguration", "LC_min_nodeid_diff", 30, false);
  LC_min_remote_nodes =
