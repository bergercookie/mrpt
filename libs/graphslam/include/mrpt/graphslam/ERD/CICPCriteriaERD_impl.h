...
CICPCriteriaERD<GRAPH_T>::CICPCriteriaERD()
    : params(*this), // pass reference to self when initializing the parameters
      m_search_disk_color(142, 142, 56), m_laser_scans_color(0, 20, 255),
...
template <class GRAPH_T> CICPCriteriaERD<GRAPH_T>::~CICPCriteriaERD() {}
...
bool CICPCriteriaERD<GRAPH_T>::updateState(
    ...
    if (registered_new_node) {
      if (m_last_laser_scan2D) {
        this->m_nodes_to_laser_scans2D[this->m_graph->nodeCount() - 1] =
            m_last_laser_scan2D;
        this->logFmt(LVL_DEBUG, "Added laser scans of nodeID: %lu",
  ...
  if (registered_new_node) {
    ...
    this->getNearbyNodesOf(&nodes_to_check_ICP, this->m_graph->nodeCount() - 1,
                           params.ICP_max_distance);
    this->logFmt(LVL_DEBUG, "Found * %lu * nodes close to nodeID %lu",
  ...
  if (curr_laser_scan) {
    ...
    for (std::set<mrpt::utils::TNodeID>::const_iterator node_it =
      ...
      if (search != this->m_nodes_to_laser_scans2D.end()) {
        ...
        pose_t initial_pose =
            this->m_graph->nodes[curr_nodeID] - this->m_graph->nodes[*node_it];

        this->m_time_logger.enter("CICPCriteriaERD::getICPEdge");
        this->getICPEdge(*prev_laser_scan, *curr_laser_scan, &rel_edge,
        ...
        MRPT_LOG_DEBUG_STREAM(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
                              ">>>>>>>>>>>>>>>>>>>");
        MRPT_LOG_DEBUG_STREAM("ICP constraint between NON-successive nodes: "
...
void CICPCriteriaERD<GRAPH_T>::checkRegistrationCondition3D(
  ...
  if (curr_laser_scan) {
    // try adding ICP constraints with each node in the previous set
    for (set<mrpt::utils::TNodeID>::const_iterator node_it = nodes_set.begin();
         node_it != nodes_set.end(); ++node_it) {
      ...
      if (search != m_nodes_to_laser_scans3D.end()) {
        ...
        this->m_time_logger.enter("CICPCriteriaERD::getICPEdge");
        this->getICPEdge(*prev_laser_scan, *curr_laser_scan, &rel_edge, nullptr,
                         &icp_info);
        this->m_time_logger.leave("CICPCriteriaERD::getICPEdge");
...
template <class GRAPH_T>
void CICPCriteriaERD<GRAPH_T>::registerNewEdge(const mrpt::utils::TNodeID &from,
...
template <class GRAPH_T> void CICPCriteriaERD<GRAPH_T>::initializeVisuals() {
  MRPT_START;
  using namespace mrpt::opengl;
  ...
  if (this->m_win && this->m_win_manager && params.ICP_max_distance > 0) {
    this->m_win_manager->assignTextMessageParameters(&m_offset_y_search_disk,
...
template <class GRAPH_T> void CICPCriteriaERD<GRAPH_T>::updateVisuals() {
  MRPT_START;
  this->m_time_logger.enter("CICPCriteriaERD::Visuals");
  using namespace mrpt::opengl;
  using namespace mrpt::utils;
  ...
  if (this->m_win && params.visualize_laser_scans &&
    ...
    if (search != this->m_graph->nodes.end()) {
      ...
      laser_scan_viz->setPose(CPose3D(laser_scan_viz->getPoseX(),
                                      laser_scan_viz->getPoseY(), -0.15,
...
template <class GRAPH_T> void CICPCriteriaERD<GRAPH_T>::printParams() const {
  MRPT_START;
...
template <class GRAPH_T> CICPCriteriaERD<GRAPH_T>::TParams::~TParams() {}
...
void CICPCriteriaERD<GRAPH_T>::TParams::loadFromConfigFile(
  ...
  ICP_max_distance = source.read_double(section, "ICP_max_distance", 10, false);
  ICP_goodness_thresh =
