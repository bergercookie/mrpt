...
public:
  ...
  virtual bool updateState(mrpt::obs::CActionCollection::Ptr action,
   ...
   * calling \a delete.
   */
  void generateHypotsPool(const vector_uint &groupA, const vector_uint &groupB,
   ...
   */
  void generatePWConsistenciesMatrix(const vector_uint &groupA,
   ...
   */
  void
  evalPWConsistenciesMatrix(const mrpt::math::CMatrixDouble &consist_matrix,
                            const hypotsp_t &hypots_pool,
...
protected:
   ...
   * data.
   */
  bool getPropsOfNodeID(const mrpt::utils::TNodeID &nodeID, global_pose_t *pose,
  ...
  void registerNewEdge(const mrpt::utils::TNodeID &from,
                       const mrpt::utils::TNodeID &to,
                       const constraint_t &rel_edge);
   ...
   */
  virtual void
  fetchNodeIDsForScanMatching(const mrpt::utils::TNodeID &curr_nodeID,
  ...
  bool
  computeDominantEigenVector(const mrpt::math::CMatrixDouble &consist_matrix,
   ...
   *
   * Given the transformation vector \f$ (x,y,\phi)\f$ of the above composition
   * (e.g. T) the
   ...
   * \sa generatePWConsistenciesMatrix
   */
  double generatePWConsistencyElement(const mrpt::utils::TNodeID &a1,
   ...
   *
   */
  static hypot_t *findHypotByEnds(const hypotsp_t &vec_hypots,
   ...
   * to true
   */
  static const path_t *findPathByEnds(const paths_t &vec_paths,
   ...
   * otherwies NULL.
   */
  static hypot_t *findHypotByID(const hypotsp_t &vec_hypots, const size_t &id,
                                bool throw_exc = true);
   ...
   * valid data.
   */
  virtual bool getICPEdge(const mrpt::utils::TNodeID &from,
                          const mrpt::utils::TNodeID &to,
   ...
   */
  void
  execDijkstraProjection(mrpt::utils::TNodeID starting_node = 0,
   ...
   * matrices
   */
  void getMinUncertaintyPath(const mrpt::utils::TNodeID from,
                             const mrpt::utils::TNodeID to, path_t *path) const;
   ...
   * the current path
   */
  void addToPaths(std::set<path_t *> *pool_of_paths, const path_t &curr_path,
   ...
   */
  void splitPartitionToGroups(vector_uint &partition, vector_uint *groupA,
                              vector_uint *groupB, int max_nodes_in_group = 5);
