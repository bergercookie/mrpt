...
const std::string
    CRegistrationDeciderOrOptimizer<GRAPH_T>::header_sep = std::string(80, '-');
template <class GRAPH_T>
const std::string
    CRegistrationDeciderOrOptimizer<GRAPH_T>::report_sep = std::string(2, '\n');

template <class GRAPH_T>
CRegistrationDeciderOrOptimizer<GRAPH_T>::CRegistrationDeciderOrOptimizer()
    : m_graph(NULL), m_graph_section(NULL), m_win_manager(NULL), m_win(NULL),
      m_win_observer(NULL), m_initialized_visuals(false),
      m_class_name("CRegistrationDeciderOrOptimizer"), is_mr_slam_class(false) {
}
