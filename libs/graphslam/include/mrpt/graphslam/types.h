 ...
 * mrpt::graphs::CNetworkOfPoses<...>
  */
template <class GRAPH_T> struct graphslam_traits {
  ...
  typedef typename SE_TYPE::array_t Array_O; // An array of the correct size for
                                             // an "observation" (i.e. a
  ...
  /** Auxiliary struct used in graph-slam implementation: It holds the relevant
   * information for each of the constraints being taking into account. */
  ...
  typedef void (*TFunctorFeedback)(const GRAPH_T &graph, const size_t iter,
...
struct TResultInfoSpaLevMarq {
  ...
  double final_total_sq_error; //!< The sum of all the squared errors for every
                               //!constraint involved in the problem.
};
