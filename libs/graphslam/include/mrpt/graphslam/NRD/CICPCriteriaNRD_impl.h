...
// m_mahal_distance_ICP_odom("Mahalanobis dist (ICP - odom)")
{
  ...
  // m_mahal_distance_ICP_odom.resizeWindow(1000); // use the last X mahalanobis
  // distance values
...
template <class GRAPH_T> CICPCriteriaNRD<GRAPH_T>::~CICPCriteriaNRD() {}
...
bool CICPCriteriaNRD<GRAPH_T>::updateState(
  ...
  if (observation) { // Observation-Only Rawlog
    ...
    if (observations->getObservationByClass<CObservation2DRangeScan>()) { // 2D
      ...
      registered_new_node = updateState2D(curr_laser_scan);
    } else if (observations->getObservationByClass<
                   CObservation3DRangeScan>()) { // 3D - EXPERIMENTAL, has not
                                                 // been tested
      CObservation3DRangeScan::Ptr curr_laser_scan =
...
bool CICPCriteriaNRD<GRAPH_T>::checkRegistrationCondition2D() {
  ...
  this->getICPEdge(*m_last_laser_scan2D, *m_curr_laser_scan2D, &rel_edge,
                   nullptr, &icp_info);
...
void CICPCriteriaNRD<GRAPH_T>::loadParams(const std::string &source_fname) {
  ...
  int min_verbosity_level = source.read_int("NodeRegistrationDeciderParameters",
...
template <class GRAPH_T> void CICPCriteriaNRD<GRAPH_T>::printParams() const {
  MRPT_START;
...
template <class GRAPH_T> CICPCriteriaNRD<GRAPH_T>::TParams::~TParams() {}
template <class GRAPH_T>
...
void CICPCriteriaNRD<GRAPH_T>::TParams::loadFromConfigFile(
  ...
  registration_max_angle = source.read_double(section, "registration_max_angle",
                                              10 /* degrees */, false);
