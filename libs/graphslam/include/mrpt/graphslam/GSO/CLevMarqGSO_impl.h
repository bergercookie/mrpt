...
CLevMarqGSO<GRAPH_T>::CLevMarqGSO()
    : m_first_time_call(false), m_has_read_config(false),
      m_autozoom_active(true), m_last_total_num_of_nodes(5),
      m_optimization_policy(FOP_USE_LC), m_curr_used_consec_lcs(0),
      m_curr_ignored_consec_lcs(0), m_just_fully_optimized_graph(false),
...
template <class GRAPH_T> CLevMarqGSO<GRAPH_T>::~CLevMarqGSO() {
...
template <class GRAPH_T> void CLevMarqGSO<GRAPH_T>::initializeVisuals() {
  MRPT_START;
...
template <class GRAPH_T> void CLevMarqGSO<GRAPH_T>::updateVisuals() {
  MRPT_START;
...
template <class GRAPH_T> void CLevMarqGSO<GRAPH_T>::toggleGraphVisualization() {
  MRPT_START;
  using namespace mrpt::opengl;
...
template <class GRAPH_T> void CLevMarqGSO<GRAPH_T>::fitGraphInView() {
  MRPT_START;
  using namespace mrpt::opengl;

  ASSERTMSG_(this->m_win, "\nVisualization of data was requested but no "
  ...
  if (obj_grid) {
    ...
    this->m_win->setCameraPointingToPoint(0.5 * (x_min + x_max),
...
CLevMarqGSO<GRAPH_T>::initOptDistanceVisualizationInternal(
  ...
  obj->setColor_u8(opt_params.optimization_distance_color.R,
...
template <class GRAPH_T> void CLevMarqGSO<GRAPH_T>::optimizeGraph() {
  MRPT_START;
  using namespace std;

  MRPT_LOG_DEBUG_STREAM("optimizeGraph:: ThreadID:"
                        << endl
...
void CLevMarqGSO<GRAPH_T>::_optimizeGraph(bool is_full_update /*=false*/) {
  ...
  if (is_full_update) {
    ...
    // been added
    this->getNearbyNodesOf(nodes_to_optimize, this->m_graph->nodeCount() - 1,
  ...
  this->logFmt(mrpt::utils::LVL_DEBUG, "Optimization of graph took: %fs",
               elapsed_time);
...
template <class GRAPH_T> bool CLevMarqGSO<GRAPH_T>::checkForLoopClosures() {
  MRPT_START;
  ...
  for (typename GRAPH_T::edges_map_t::const_iterator it =
    ...
    if (search == opt_params.last_pair_nodes_to_edge.end()) {
      ...
      if (std::abs(static_cast<int>(curr_pair.first) -
...
template <class GRAPH_T> bool CLevMarqGSO<GRAPH_T>::checkForFullOptimization() {
...
template <class GRAPH_T>
void CLevMarqGSO<GRAPH_T>::levMarqFeedback(const GRAPH_T &graph,
                                           const size_t iter,
...
template <class GRAPH_T> void CLevMarqGSO<GRAPH_T>::printParams() const {
  parent::printParams();
...
void CLevMarqGSO<GRAPH_T>::loadParams(const std::string &source_fname) {
  ...
  m_max_ignored_consec_lcs =
      source.read_int("OptimizerParameters",
                      "max_ignored_consecutive_loop_closures", 15, false);
...
CLevMarqGSO<GRAPH_T>::OptimizationParams::OptimizationParams()
    : optimization_distance_color(0, 201, 87),
      keystroke_optimization_distance("u"), keystroke_optimize_graph("w") {}
template <class GRAPH_T>
CLevMarqGSO<GRAPH_T>::OptimizationParams::~OptimizationParams() {}
template <class GRAPH_T>
...
void CLevMarqGSO<GRAPH_T>::OptimizationParams::loadFromConfigFile(
  ...
  optimization_on_second_thread =
      source.read_bool(section, "optimization_on_second_thread", false, false);
  LC_min_nodeid_diff =
  ...
  ASSERTMSG_(optimization_distance == -1 || optimization_distance > 0,
...
void CLevMarqGSO<GRAPH_T>::GraphVisualizationParams::loadFromConfigFile(
  ...
  cfg["nodes_point_color"] =
      source.read_int(section, "optimized_nodes_point_color", 10526880, false);
