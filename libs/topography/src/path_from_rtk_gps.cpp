...
template <class T> std::set<T> make_set(const T &v0, const T &v1) {
  std::set<T> s;
  s.insert(v0);
  s.insert(v1);
  return s;
...
void mrpt::topography::path_from_rtk_gps(
  ...
  TGeodeticCoords ref(memFil.read_double("GPS_ORIGIN", "lat_deg", 0),
  ...
  if (doUncertaintyCovs &&
    ...
    THROW_EXCEPTION("ERROR: W_star matrix for uncertainty estimation is "
                    "provided but it's not a 6x6 matrix.");
...
// ------------------------------------------
    ...
    case CRawlog::etObservation: {
      ...
      if (o->GetRuntimeClass() == CLASS_ID(CObservationGPS)) {
        ...
        if (obs->has_GGA_datum &&
          ...
          // map<string,TPoint3D> GPS_local_coords_on_vehicle;  // label ->
          // local pose on the vehicle
          if (GPS_local_coords_on_vehicle.find(obs->sensorLabel) ==
              GPS_local_coords_on_vehicle.end())
            GPS_local_coords_on_vehicle[obs->sensorLabel] =
                TPoint3D(obs->sensorPose);

          // map<string, map<TTimeStamp,TPoint3D> >	gps_paths;	// label ->
          // (time -> 3D local coords)
          gps_paths[obs->sensorLabel][obs->timestamp] =
              TPoint3D(obs->getMsgByClass<gnss::Message_NMEA_GGA>()
  ...
  if (doConsistencyCheck && GPS_local_coords_on_vehicle.size() == 3) {
    ...
    D_cov(0, 0) =
        2 * square(Ad_ij[make_set(D_cov_rev_indexes[0], D_cov_rev_indexes[1])]);
    D_cov(1, 1) =
        2 * square(Ad_ij[make_set(D_cov_rev_indexes[0], D_cov_rev_indexes[2])]);
    D_cov(2, 2) =
        2 * square(Ad_ij[make_set(D_cov_rev_indexes[1], D_cov_rev_indexes[2])]);
  ...
  if (N_GPSs) {
      ...
      for (TListGPSs::iterator i = F; i != E; ++i) {
        ...
        for (set<string>::iterator l = lstGPSLabels.begin();
          ...
          if (!disableGPSInterp && GPS_a1 && GPS_b1) {
            if (mrpt::system::timeDifference(GPS_b1->timestamp,
                                             GPS_a1->timestamp) < 0.5) {
              CObservationGPS::Ptr new_gps =
                  CObservationGPS::Ptr(new CObservationGPS(*GPS_a1));
              new_gps->sensorLabel = *l;

              // cout << mrpt::system::timeLocalToString(GPS_b1->timestamp) << "
              ...
              new_gps->getMsgByClass<gnss::Message_NMEA_GGA>()
                  .fields.longitude_degrees =
                  0.5 * (GPS_a1->getMsgByClass<gnss::Message_NMEA_GGA>()
                             .fields.longitude_degrees +
    ...
    for (TListGPSs::iterator i = list_gps_obs.begin(); i != list_gps_obs.end();
         ++i, idx_in_GPSs++) {
      // Now check if we have 3 gps with the same time stamp:
      if (i->second.size() >= 3) {
        const size_t N = i->second.size();
        ...
        if (!ref_valid) // get the reference lat/lon, if it's not set from
                        // rawlog configuration block.
        {
          ref_valid = true;
          ref = GPS.begin()
        ...
        for (k = 0, g_it = GPS.begin(); g_it != GPS.end(); ++g_it, ++k) {
          ...
          corrs.push_back(TMatchingPair(
              ...
              g_it->second->sensorPose.x(), g_it->second->sensorPose.y(),
              g_it->second->sensorPose.z() // "other"/local coordinates
              ));
        ...
        if (doConsistencyCheck && GPS.size() == 3) {
          ...
          TPoint3D P0(X[XYZidxs[D_cov_rev_indexes[0]]],
                      Y[XYZidxs[D_cov_rev_indexes[0]]],
                      Z[XYZidxs[D_cov_rev_indexes[0]]]);
          TPoint3D P1(X[XYZidxs[D_cov_rev_indexes[1]]],
                      Y[XYZidxs[D_cov_rev_indexes[1]]],
                      Z[XYZidxs[D_cov_rev_indexes[1]]]);
          TPoint3D P2(X[XYZidxs[D_cov_rev_indexes[2]]],
          ...
          double mahaD =
              mrpt::math::mahalanobisDistance(iGPSdist2, D_mean, D_cov_1);
          outInfoTemp.mahalabis_quality_measure[i->first] = mahaD;

          // cout << "x: " << iGPSdist2 << " MU: " <<  D_mean << " -> " << mahaD
          // << endl;
        } // end consistency
        ...
        mrpt::tfest::se3_l2(corrs, optimal_pose, optimal_scale,
                            true); // Force scale=1
        // cout << "optimal pose: " << optimal_pose << " " << optimal_scale <<
        // endl;
    ...
    if (PATH_SMOOTH_FILTER > 0 && robot_path.size() > 1) {
      ...
      for (auto i = robot_path.begin(); i != robot_path.end(); ++i) {
        ...
        for (int k = 0; k < PATH_SMOOTH_FILTER && q != robot_path.begin();
             k++) {
          --q;
        ...
        q = i;
        for (int k = 0; k < PATH_SMOOTH_FILTER && q != (--robot_path.end());
             k++) {
          ++q;
          if (abs(mrpt::system::timeDifference(q->first, i->first)) <
              MAX_DIST_TO_FILTER) {
  ...
  // Here we can set best_gps_path  (that with the max. number of RTK fixed/foat
  // readings):
  if (outInfo) {
    string bestLabel;
    size_t bestNum = 0;
    ...
    for (map<TTimeStamp, TPoint3D>::iterator i =
      ...
      mrpt::topography::geodeticToENU_WGS84(
          TGeodeticCoords(
              pl.x, pl.y,
              pl.z), // i->second.x,i->second.y,i->second.z, // lat, lon, heigh
          P,         // X Y Z
          ref);
