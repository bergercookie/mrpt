 ...
 * coincides with the direction of an increasing geodetic height.
  */
void TOPO_IMPEXP geodeticToENU_WGS84(const TGeodeticCoords &in_coords,
...
void TOPO_IMPEXP ENUToGeocentric(const mrpt::math::TPoint3D &in_ENU_point,
                                 const TGeodeticCoords &in_coords_origin,
 ...
 * geodeticToENU_WGS84 */
void TOPO_IMPEXP
geocentricToENU_WGS84(const mrpt::math::TPoint3D &in_geocentric_point,
                      mrpt::math::TPoint3D &out_ENU_point,
                      const TGeodeticCoords &in_coords_origin);
 ...
 * http://en.wikipedia.org/wiki/Reference_ellipsoid
  ...
  */
void TOPO_IMPEXP geodeticToGeocentric_WGS84(const TGeodeticCoords &in_coords,
 ...
 * X/Y/Z coordinates (with an specified geoid).
  ...
  */
void TOPO_IMPEXP geodeticToGeocentric(const TGeodeticCoords &in_coords,
 ...
 * ] [ X Y Z ]_local
  ...
  */
void TOPO_IMPEXP transform7params(const mrpt::math::TPoint3D &in_point,
                                  const TDatum7Params &in_datum,
                                  mrpt::math::TPoint3D &out_point);

void TOPO_IMPEXP transform7params_TOPCON(const mrpt::math::TPoint3D &in_point,
 ...
 * ] [ X-Xp Y-Yp Z-Zp ]_local  + [Xp Yp Zp]
  * \sa transform7params
  */
void TOPO_IMPEXP transform10params(const mrpt::math::TPoint3D &in_point,
 ...
 * sin(alpha) cos(alpha) ] [ X-Xp Y-Yp Z-Zp ]_local + [Xp Yp Zp]
  ...
  */
void TOPO_IMPEXP transformHelmert2D(const mrpt::math::TPoint2D &p,
                                    const TDatumHelmert2D &d,
                                    mrpt::math::TPoint2D &o);

void TOPO_IMPEXP transformHelmert2D_TOPCON(const mrpt::math::TPoint2D &p,
 ...
 * ] [ X Y Z ]_local
  ...
  */
void TOPO_IMPEXP transformHelmert3D(const mrpt::math::TPoint3D &p,
                                    const TDatumHelmert3D &d,
                                    mrpt::math::TPoint3D &o);

void TOPO_IMPEXP transformHelmert3D_TOPCON(const mrpt::math::TPoint3D &p,
...
/**  1D transformation:
  ...
  */
void TOPO_IMPEXP transform1D(const mrpt::math::TPoint3D &p,
                             const TDatum1DTransf &d, mrpt::math::TPoint3D &o);

/**  Interpolation:
  *   [ Z ]_WGS84 = (dy * X - dx * Y + Z)*(1+e)+DZ
  */
void TOPO_IMPEXP transfInterpolation(const mrpt::math::TPoint3D &p,
 ...
 * WGS84)
  ...
  */
void TOPO_IMPEXP
UTMToGeodetic(double X, double Y, int zone, char hem,
              double &out_lon /*degrees*/, double &out_lat /*degrees*/,
              const TEllipsoid &ellip = TEllipsoid::Ellipsoid_WGS84());
 ...
 * WGS84)
  */
inline void
UTMToGeodetic(const TUTMCoords &UTMCoords, const int &zone, const char &hem,
              TGeodeticCoords &GeodeticCoords,
              const TEllipsoid &ellip = TEllipsoid::Ellipsoid_WGS84()) {
  UTMToGeodetic(UTMCoords.x, UTMCoords.y, zone, hem,
                GeodeticCoords.lon.decimal_value,
...
void TOPO_IMPEXP
geodeticToUTM(const TGeodeticCoords &GeodeticCoords, TUTMCoords &UTMCoords,
              int &UTMZone, char &UTMLatitudeBand,
              const TEllipsoid &ellip = TEllipsoid::Ellipsoid_WGS84());
...
GeodeticToUTM(const TGeodeticCoords &GeodeticCoords, TUTMCoords &UTMCoords,
              int &UTMZone, char &UTMLatitudeBand,
              const TEllipsoid &ellip = TEllipsoid::Ellipsoid_WGS84()) {
  GeodeticToUTM(GeodeticCoords.lat, GeodeticCoords.lon, UTMCoords.x,
                UTMCoords.y, UTMZone, UTMLatitudeBand, ellip);
 ...
 * coincides with the direction of an increasing geodetic height.
  * \sa geodeticToENU_WGS84
  */
void TOPO_IMPEXP ENU_axes_from_WGS84(double in_longitude_reference_degrees,
...
/** \overload */
inline void ENU_axes_from_WGS84(const TGeodeticCoords &in_coords,
                                mrpt::math::TPose3D &out_ENU,
                                bool only_angles = false) {
  ENU_axes_from_WGS84(in_coords.lon, in_coords.lat, in_coords.height, out_ENU,
                      only_angles);
}
