...
public:
   ...
   * to std::cout.
    ...
    */
  CConsoleRedirector(const std::string &out_file, bool also_to_console = true,
                     bool also_cerr = true, bool append_file = false,
                     int bufferSize = 1000)
      : m_of(), sbOld(nullptr), sbOld_cerr(nullptr),
        m_also_to_console(also_to_console), m_cs() {
    // Open the file:
  ...
  virtual ~CConsoleRedirector() {
    ...
    if (sbOld_cerr != nullptr)
      std::cerr.rdbuf(sbOld_cerr);
    if (pbase())
      delete[] pbase();
  }
  ...
  virtual void writeString(const std::string &str) {
    if (m_also_to_console)
      sbOld->sputn(str.c_str(), str.size());
    m_of << str;
  }
