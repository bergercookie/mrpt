...
public:
   ...
   * (which is copied), and sets the current stream position at the beginning of
   * the data.
  ...
  void changeSize(uint64_t newSize); //!< Change size. This would be rarely
                                     //!used. Use ">>" operators for writing to
                                     //!stream \sa Stream
...
void BASE_IMPEXP free_fn_for_zmq(void *data, void *hint); //!< Used in
                                                          //!mrpt_send_to_zmq().
                                                          //!`hint` points to a
                                                          //!`TFreeFnDataForZMQ`
                                                          //!struct, to be freed
