...
public:
   ...
   * hypothesis ID, or a nullptr smart pointer if it does not exist.
    */
  CSerializable::Ptr get(const char *propertyName,
                         const int64_t &hypothesis_ID) const;

  /** Returns the value of the property (case insensitive) for some given
   * hypothesis ID checking its class in runtime, or a nullptr smart pointer if
   * it does not exist.
    */
  template <typename T>
  typename T::Ptr getAs(const char *propertyName, const int64_t &hypothesis_ID,
...
*/
   ...
   * hypothesis ID, making a copy of the object (or setting it to nullptr if it
   * is the passed value)
    * \sa setMemoryReference
    */
  void set(const char *propertyName, const CSerializable::Ptr &obj,
   ...
   * the object.
    * \sa set
    */
  void setMemoryReference(const char *propertyName,
   ...
   * hypothesis ID, from an elemental data type.
    */
  template <class T>
  void setElemental(const char *propertyName, const T &data,
                    const int64_t &hypothesis_ID) {
    MRPT_START
   ...
   * ID, retrieves it as an elemental data type (types must coincide, basic size
   * check is performed).
    * \return false if the property does not exist for the given hypothesis.
    */
  template <class T>
  bool getElemental(const char *propertyName, T &out_data,
                    const int64_t &hypothesis_ID,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE(CMHPropertiesValuesList,
