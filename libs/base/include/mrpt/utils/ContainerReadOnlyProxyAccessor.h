...
template <typename STLCONTAINER> struct ContainerReadOnlyProxyAccessor {
  ContainerReadOnlyProxyAccessor(STLCONTAINER &source)
      : m_source(source) {} //!< ctor: binds to source object
  ContainerReadOnlyProxyAccessor(
      const ContainerReadOnlyProxyAccessor<STLCONTAINER> &) =
      delete; //!< (Deleted ctor) ignore copies (keep reference to original
              //!object)
  ...
  ContainerReadOnlyProxyAccessor<STLCONTAINER> &
  operator=(const ContainerReadOnlyProxyAccessor<STLCONTAINER> &o) {
