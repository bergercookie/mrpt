...
#define THROW_TYPED_EXCEPTION_FMT(exceptionClass, _FORMAT_STRING, ...)         \
  THROW_TYPED_EXCEPTION(mrpt::format(_FORMAT_STRING, __VA_ARGS__),             \
                        exceptionClass)
...
/** \def THROW_STACKED_EXCEPTION_CUSTOM_MSG
  * \param e The caught exception.
  *	\param stuff Is a printf-like sequence of params, e.g: "The error
  *happens for x=%i",x
  */
...
#define ASSERTMSG_(f, __ERROR_MSG)                                             \
  {                                                                            \
    if (!(f))                                                                  \
      THROW_EXCEPTION(::std::string(__ERROR_MSG));                             \
  }
...
#define ASSERT_EQUAL_(__A, __B)                                                \
  {                                                                            \
    if (__A != __B) {                                                          \
      ...
      __s__ << "ASSERT_EQUAL_(" << #__A << "," << #__B << ") failed with\n"    \
            << #__A << "=" << __A << "\n"                                      \
            << #__B << "=" << __B;                                             \
...
#define ASSERT_BELOW_(__A, __B)                                                \
  {                                                                            \
    if (__A >= __B) {                                                          \
      std::ostringstream __s__;                                                \
      __s__ << "ASSERT_BELOW_(" << #__A << "," << #__B << ") failed with\n"    \
            << #__A << "=" << __A << "\n"                                      \
            << #__B << "=" << __B;                                             \
      THROW_EXCEPTION(__s__.str())                                             \
    }                                                                          \
...
#define ASSERT_ABOVE_(__A, __B)                                                \
  {                                                                            \
    if (__A <= __B) {                                                          \
      std::ostringstream __s__;                                                \
      __s__ << "ASSERT_ABOVE_(" << #__A << "," << #__B << ") failed with\n"    \
            << #__A << "=" << __A << "\n"                                      \
            << #__B << "=" << __B;                                             \
      THROW_EXCEPTION(__s__.str())                                             \
    }                                                                          \
...
#define ASSERT_BELOWEQ_(__A, __B)                                              \
  {                                                                            \
    if (__A > __B) {                                                           \
      std::ostringstream __s__;                                                \
      __s__ << "ASSERT_BELOWEQ_(" << #__A << "," << #__B << ") failed with\n"  \
            << #__A << "=" << __A << "\n"                                      \
            << #__B << "=" << __B;                                             \
      THROW_EXCEPTION(__s__.str())                                             \
    }                                                                          \
...
#define ASSERT_ABOVEEQ_(__A, __B)                                              \
  {                                                                            \
    if (__A < __B) {                                                           \
      std::ostringstream __s__;                                                \
      __s__ << "ASSERT_ABOVEEQ_(" << #__A << "," << #__B << ") failed with\n"  \
            << #__A << "=" << __A << "\n"                                      \
            << #__B << "=" << __B;                                             \
      THROW_EXCEPTION(__s__.str())                                             \
    }                                                                          \
...
#define ASSERT_FILE_EXISTS_(FIL)                                               \
  ASSERTMSG_(mrpt::system::fileExists(FIL),                                    \
             std::string("Assert file existence failed: ") +                   \
                 ::std::string(FIL))
#define ASSERT_DIRECTORY_EXISTS_(DIR)                                          \
  ASSERTMSG_(mrpt::system::directoryExists(DIR),                               \
...
#define MRPT_PROFILE_FUNC_START                                                \
  ::mrpt::utils::CProfilerProxy BOOST_JOIN(__dum_var, __LINE__)(               \
      __CURRENT_FUNCTION_NAME__);
#else
#define MRPT_PROFILE_FUNC_START
#endif
