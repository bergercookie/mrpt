...
/** A class for storing a list of text lines.
  *  This class is optimized for managing strings on a "per-line" basis,
 * although methods are also provided to obtain/set the
  *   whole string list as a single, multi-line string.
  *  There are methods for saving and loading to/from text files.
  *  You can access to lines directly by "CStringList::get" or through the
 * operator "CStringList::operator ()". The later can be
  *   used both, to read and to write elements.
  *  Also methods are provided for accesing the text by key if they are formated
 * as "key=value" lines.
...
public:
  ...
  explicit CStringList(const std::deque<std::string> &lines)
      : m_strings(lines) {}
  ...
  /** Inserts a new item at a given position (0=insert at the beggining,1=put
  ...
  /** Looks for a given string in the list, and returns its index, or returns
   * "false" otherwise.
    * \return true if string has been found.
    */
  bool find(const std::string &compareText, size_t foundIndex,
  ...
  /** Sets the value of a given key ("key=value"), overwritten previous value if
   * it existed.
    */
