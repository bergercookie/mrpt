 ...
 */
template <class TDATA, size_t STATE_LEN> class CProbabilityDensityFunction {
public:
  static const size_t state_length =
      STATE_LEN; //!< The length of the variable, for example, 3 for a 3D point,
                 //!6 for a 3D pose (x y z yaw pitch roll).
  ...
  /** Returns an estimate of the pose covariance matrix (STATE_LENxSTATE_LEN cov
   ...
   * matrix) and the mean, both at once.
    * \sa getMean, getInformationMatrix
    */
  inline void getCovarianceDynAndMean(mrpt::math::CMatrixDouble &cov,
                                      TDATA &mean_point) const {
  ...
  /** Returns whether the class instance holds the uncertainty in covariance or
   * information form.
  ...
  */
   ...
   * classes should implemented optimized method for each particular PDF.
    */
  virtual void
  drawManySamples(size_t N,
                  std::vector<mrpt::math::CVectorDouble> &outSamples) const {
    outSamples.resize(N);
    TDATA pnt;
   ...
   * stored one in the object.
    */
  virtual void
  changeCoordinatesReference(const mrpt::poses::CPose3D &newReferenceBase) = 0;
