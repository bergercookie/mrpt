...
extern int BASE_IMPEXP MRPT_SAVE_NAME_PADDING,
    MRPT_SAVE_VALUE_PADDING; //!< Default padding sizes for macros
                             //!MRPT_SAVE_CONFIG_VAR_COMMENT(), etc.
...
protected:
  /** A virtual method to write a generic string.
    */
  virtual void writeString(const std::string &section, const std::string &name,
                           const std::string &str) = 0;

  /** Write a generic string with optional padding and a comment field ("//
   * ...") at the end of the line. */
  void writeString(const std::string &section, const std::string &name,
                   const std::string &str, const int name_padding_width,
                   const int value_padding_width, const std::string &comment);

  /** A virtual method to read a generic string.
  * \exception std::exception If the key name is not found and "failIfNotFound"
  * is true. Otherwise the "defaultValue" is returned. */
  virtual std::string readString(const std::string &section,
...
public:
  ...
  virtual void getAllKeys(const std::string &section,
                          vector_string &keys) const = 0;
  ...
  void write(const std::string &section, const std::string &name,
    ...
    ss << value;
    writeString(section, name, ss.str(), name_padding_width,
   ...
   * not found and `failIfNotFound`=true
    * @{ */
  double read_double(const std::string &section, const std::string &name,
                     double defaultValue, bool failIfNotFound = false) const;
  float read_float(const std::string &section, const std::string &name,
                   float defaultValue, bool failIfNotFound = false) const;
  bool read_bool(const std::string &section, const std::string &name,
                 bool defaultValue, bool failIfNotFound = false) const;
  int read_int(const std::string &section, const std::string &name,
               int defaultValue, bool failIfNotFound = false) const;
  uint64_t read_uint64_t(const std::string &section, const std::string &name,
                         uint64_t defaultValue,
                         bool failIfNotFound = false) const;
  std::string read_string(const std::string &section, const std::string &name,
                          const std::string &defaultValue,
                          bool failIfNotFound = false) const;
  /** Reads a configuration parameter of type "string", and keeps only the first
   * word (this can be used to eliminate possible comments at the end of the
   * line) */
  std::string read_string_first_word(const std::string &section,
                                     const std::string &name,
                                     const std::string &defaultValue,
                                     bool failIfNotFound = false) const;
  ...
  void read_vector(const std::string &section, const std::string &name,
  ...
  void read_matrix(const std::string &section, const std::string &name,
  ...
  /** Reads an "enum" value, where the value in the config file can be either a
   * numerical value or the symbolic name, for example:
    *   In the code:
    *  \code
   ...
   * equivalent)
    ...
    *  \endcode
    *
    *  \note For an enum type to work with this template it is required that it
   * defines a specialization of mrpt::utils::TEnumType
    */
  template <typename ENUMTYPE>
  ENUMTYPE read_enum(const std::string &section, const std::string &name,
    ...
    if (::isdigit(sVal[0])) { // Seems a number:
 ...
 * bool,...
  */
#define MRPT_LOAD_CONFIG_VAR(variableName, variableType, configFileObject,     \
                             sectionNameStr)                                   \
  {                                                                            \
 ...
 * degrees */
#define MRPT_LOAD_CONFIG_VAR_DEGREES(variableName, configFileObject,           \
                                     sectionNameStr)                           \
  {                                                                            \
    variableName = mrpt::utils::DEG2RAD(configFileObject.read_double(          \
        sectionNameStr, #variableName, mrpt::utils::RAD2DEG(variableName)));   \
  }
...
#define MRPT_LOAD_CONFIG_VAR_CAST(variableName, variableType,                  \
                                  variableTypeCast, configFileObject,          \
                                  sectionNameStr)                              \
  {                                                                            \
    variableName =                                                             \
...
#define MRPT_LOAD_HERE_CONFIG_VAR(variableName, variableType, targetVariable,  \
...
#define MRPT_LOAD_HERE_CONFIG_VAR_NO_DEFAULT(variableName, variableType,       \
...
#define MRPT_LOAD_HERE_CONFIG_VAR_DEGREES(variableName, variableType,          \
                                          targetVariable, configFileObject,    \
                                          sectionNameStr)                      \
  targetVariable = mrpt::utils::DEG2RAD(configFileObject.read_##variableType(  \
      sectionNameStr, #variableName, mrpt::utils::RAD2DEG(targetVariable),     \
      false));
...
#define MRPT_LOAD_HERE_CONFIG_VAR_DEGREES_NO_DEFAULT(                          \
  ...
  {                                                                            \
    try {                                                                      \
      targetVariable =                                                         \
          mrpt::utils::DEG2RAD(configFileObject.read_##variableType(           \
              sectionNameStr, #variableName, targetVariable, true));           \
    } catch (std::exception &) {                                               \
      THROW_EXCEPTION(mrpt::format(                                            \
          "Value for '%s' not found in config file in section '%s'",           \
...
#define MRPT_LOAD_CONFIG_VAR_NO_DEFAULT(variableName, variableType,            \
                                        configFileObject, sectionNameStr)      \
  {                                                                            \
    try {                                                                      \
      variableName = configFileObject.read_##variableType(                     \
...
#define MRPT_LOAD_CONFIG_VAR_CAST_NO_DEFAULT(variableName, variableType,       \
                                             variableTypeCast,                 \
                                             configFileObject, sectionNameStr) \
  {                                                                            \
    try {                                                                      \
...
#define MRPT_LOAD_HERE_CONFIG_VAR_CAST(variableName, variableType,             \
                                       variableTypeCast, targetVariable,       \
                                       configFileObject, sectionNameStr)       \
  targetVariable =                                                             \
      static_cast<variableTypeCast>(configFileObject.read_##variableType(      \
...
#define MRPT_SAVE_CONFIG_VAR_DEGREES(variableName, configFileObject,           \
                                     sectionNameStr)                           \
  {                                                                            \
    configFileObject.write(sectionNameStr, #variableName,                      \
                           mrpt::utils::RAD2DEG(variableName));                \
  }

#define MRPT_SAVE_CONFIG_VAR_COMMENT(variableName, __comment)                  \
  {                                                                            \
    c.write(s, #variableName, variableName,                                    \
            mrpt::utils::MRPT_SAVE_NAME_PADDING,                               \
            mrpt::utils::MRPT_SAVE_VALUE_PADDING, __comment);                  \
  }
#define MRPT_SAVE_CONFIG_VAR_DEGREES_COMMENT(__entryName, __variable,          \
                                             __comment)                        \
  {                                                                            \
    c.write(s, __entryName, mrpt::utils::RAD2DEG(__variable),                  \
            mrpt::utils::MRPT_SAVE_NAME_PADDING,                               \
            mrpt::utils::MRPT_SAVE_VALUE_PADDING, __comment);                  \
  }
