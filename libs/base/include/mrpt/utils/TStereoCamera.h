...
public:
  TCamera leftCamera, rightCamera; //!< Intrinsic and distortion parameters of
                                   //!the left and right cameras
  mrpt::poses::CPose3DQuat rightCameraPose; //!< Pose of the right camera with
  ...
  /**  Save all params to a plain text config file in this format:
        ...
        *
        *  \endcode
        *   Notice that 3 different sections are read, of which "section" is
   * only the prefix.
        */
  void saveToConfigFile(const std::string &section,
                        mrpt::utils::CConfigFileBase &cfg) const;

  /**  Load all the params from a config source, in the same format that used in
   * saveToConfigFile().
