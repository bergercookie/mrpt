...
void mrpt_send_to_zmq(ZMQ_SOCKET_TYPE zmq_socket,
  ...
  if (!nBytes)
    throw std::runtime_error("[mrpt_send_to_zmq] Serialized object has 0 "
                             "bytes, which probably means something went "
                             "wrong...");
  unsigned int nPkts =
      (!max_packet_len)
          ? 1U
  ...
  for (unsigned int iPkt = 0; iPkt < nPkts; ++iPkt) {
    ...
    // Build ZMQ msg:
    zmq_msg_t message;
    if (0 != zmq_msg_init_data(&message, pkt_data, nBytesThisPkt,
...
bool mrpt_recv_from_zmq_buf(ZMQ_SOCKET_TYPE zmq_socket,
                            VECTOR_MSG_T &out_lst_msgs,
                            mrpt::utils::CMemoryStream &target_buf,
                            bool dont_wait, size_t *rx_obj_length_in_bytes) {
  if (rx_obj_length_in_bytes)
    *rx_obj_length_in_bytes = 0;
  out_lst_msgs.clear();
  target_buf.Clear();
  int64_t more;
  ...
  if (out_lst_msgs.size() > 1) {
    for (size_t i = 0; i < out_lst_msgs.size(); i++) {
      target_buf.WriteBuffer(zmq_msg_data(out_lst_msgs[i]),
                             zmq_msg_size(out_lst_msgs[i]));
    }
    if (rx_obj_length_in_bytes)
...
namespace internal {
template <typename VECTOR_MSG_T> void free_zmq_msg_lst(VECTOR_MSG_T &lst_msgs) {
...
mrpt_recv_from_zmq(ZMQ_SOCKET_TYPE zmq_socket, bool dont_wait = false,
  ...
  if (!mrpt_recv_from_zmq_buf(zmq_socket, lst_msgs_to_close, target_buf,
                              dont_wait, rx_obj_length_in_bytes))
    return obj;
  // De-serialize:
 ...
 * https://github.com/MRPT/mrpt/tree/master/doc/mrpt-zeromq-example
  */
template <typename ZMQ_SOCKET_TYPE>
bool mrpt_recv_from_zmq_into(ZMQ_SOCKET_TYPE zmq_socket,
