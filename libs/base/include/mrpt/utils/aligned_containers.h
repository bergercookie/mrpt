...
namespace mrpt {
/** Helper types for STL containers with Eigen memory allocators.  (in #include
 * <mrpt/utils/aligned_containers.h>)  */
template <class TYPE1, class TYPE2 = TYPE1> struct aligned_containers {
  ...
  typedef std::map<TYPE1, TYPE2, std::less<TYPE1>,
                   Eigen::aligned_allocator<std::pair<const TYPE1, TYPE2>>>
      map_t;
  typedef std::multimap<TYPE1, TYPE2, std::less<TYPE1>,
                        Eigen::aligned_allocator<std::pair<const TYPE1, TYPE2>>>
      multimap_t;
};
}
