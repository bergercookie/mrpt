 ...
 *		- Binary dump using the CSerializable interface(<< and >>
 *operators), just as most objects
 *          in the MRPT library. This format is not compatible with any
 *standarized image format.
 *		- Saving/loading from files of different formats (bmp,jpg,png,...)
 *using the methods CImage::loadFromFile and CImage::saveToFile.
 ...
 *
 * Additional notes:
 *		- The OpenCV "IplImage" format is used internally for
 *compatibility with all OpenCV functions. Use CImage::getAs<IplImage>() to
 *retrieve the internal structure. Example:
 *         \code
 *            CImage  img;
 *            ...
 ...
 *         \endcode
 *		- Only the unsigned 8-bit storage format for pixels (on each
 ...
 *CImage::copyFastFrom rather than the copy operator =.
...
public:
  ...
  /** Copy constructor, makes a full copy of the original image contents (unless
   * it was externally stored, in that case, this new image will just point to
   * the same image file). */
   ...
   * otherwise creating a new grayscale image and converting the original image
   * into it.
    *   It's <b>very important to keep in mind</b> that the original image can't
   * be destroyed before the new object being created with this constructor.
    * Example of usage:
    *   \code
    ...
    *   \endcode
    */
  inline CImage(const CImage &other_img,
   ...
   * values, in the range [0,1] (normalized=true) or [0,255] (normalized=false)
    * \sa setFromMatrix
    */
  template <typename Derived>
  explicit inline CImage(const Eigen::MatrixBase<Derived> &m,
   ...
   * grayscale images will be ZIP compressed if they are larger than 16Kb: this
   * flag can be turn on to disable ZIP compression and gain speed versus
   * occupied space.
    *  (Default = false) */
   ...
   * storing RAW image data, disable this feature by setting this flag to true.
    *  (Default = false) */
   ...
   * its current content, for that, see scaleImage).
    ...
    *  - originTopLeft: Is true if the top-left corner is (0,0). In other case,
   * the reference is the bottom-left corner.
    * \sa scaleImage
    */
  inline void resize(unsigned int width, unsigned int height,
                     TImageChannels nChannels, bool originTopLeft) {
  ...
  /** Scales this image to a new size, interpolating as needed.
    ...
    */
  void scaleImage(unsigned int width, unsigned int height,
   ...
   * image in a different output object.
    * \sa resize, rotateImage
    */
  void scaleImage(CImage &out_img, unsigned int width, unsigned int height,
   ...
   * an optional scale factor.
    ...
    */
  void rotateImage(double angle_radians, unsigned int center_x,
                   unsigned int center_y, double scale = 1.0);
   ...
   * virtual class)
    *  This method may be redefined in some classes implementing this interface
   * in a more appropiate manner.
    */
  void line(int x0, int y0, int x1, int y1, const mrpt::utils::TColor color,
  ...
  void drawCircle(int x, int y, int radius,
                  const mrpt::utils::TColor &color = mrpt::utils::TColor(255,
                                                                         255,
                                                                         255),
                  unsigned int width = 1) override;

  void equalizeHistInPlace(); //!< Equalize the image histogram, replacing the
                              //!original image. \note RGB images are first
                              //!converted to HSV color space, then equalized
                              //!for brightness (V)
  void equalizeHist(CImage &outImg) const; //!< Equalize the image histogram,
                                           //!saving the new image in the given
                                           //!output object.  \note RGB images
                                           //!are first converted to HSV color
                                           //!space, then equalized for
                                           //!brightness (V)
   ...
   * \exception std::exception if patch pasted on the pixel (_row, _column) jut
   * out
   * of the image.
   * \sa extract_patch
   */
  void update_patch(const CImage &patch, const unsigned int col,
   ...
   * contents will be overwritten).
    ...
    * \sa update_patch
    */
  void extract_patch(CImage &patch, const unsigned int col = 0,
                     const unsigned int row = 0, const unsigned int width = 1,
  ...
  * (by AJOGD @ DEC-2006)
  */
  float correlate(const CImage &img2int, int width_init = 0,
                  int height_init = 0) const;
  ...
  * \param out_corr_image  If a !=nullptr pointer is provided, it will be saved
  * here the correlation image. The size of the output image is
  * (this_width-patch_width+1, this_height-patch_height+1 )
  *  Note: By default, the search area is the whole (this) image.
  * (by AJOGD @ MAR-2007)
  */
  void cross_correlation(const CImage &patch_img, size_t &u_max, size_t &v_max,
                         double &max_val, int u_search_ini = -1,
                         int v_search_ini = -1, int u_search_size = -1,
                         int v_search_size = -1,
                         CImage *out_corr_image = nullptr) const;

  /**	Computes the correlation matrix between this image and another one.
  *   This implementation uses the 2D FFT for achieving reduced computation
  * time.
  * \param in_img The "patch" image, which must be equal, or smaller than "this"
  * image. This function supports gray-scale (1 channel only) images.
  * \param u_search_ini The "x" coordinate of the search window.
  * \param v_search_ini The "y" coordinate of the search window.
  * \param u_search_size The width of the search window.
  * \param v_search_size The height of the search window.
  ...
  */
  void cross_correlation_FFT(const CImage &in_img, math::CMatrixFloat &out_corr,
                             int u_search_ini = -1, int v_search_ini = -1,
                             int u_search_size = -1, int v_search_size = -1,
                             float biasThisImg = 0, float biasInImg = 0) const;
  ...
  void flipVertical(bool also_swapRB = false); //!< Flips the image vertically.
                                               //!\sa swapRB(), flipHorizontal()
  void flipHorizontal(); //!< Flips the image horizontally \sa swapRB(),
                         //!flipVertical()
   ...
   * returns an output un-distorted image.
    ...
    * \param cameraParams The input camera params (containing the intrinsic and
   * distortion parameters of the camera)
    * \sa mrpt::vision::CUndistortMap
    */
  void rectifyImage(CImage &out_img,
  ...
  /** Filter the image with a Gaussian filter with a window size WxH, returning
   * the filtered image in out_img  */
  void filterGaussianInPlace(int W = 3, int H = 3);

  /** Filter the image with a Gaussian filter with a window size WxH, replacing
   * "this" image by the filtered one. */
   ...
   * drawn then).
    ...
    */
  bool drawChessboardCorners(std::vector<TPixelCoordf> &cornerCoords,
                             unsigned int check_size_x,
                             unsigned int check_size_y,
                             unsigned int lines_width = 1,
                             unsigned int circles_radius = 4);

  /** Joins two images side-by-side horizontally. Both images must have the same
   * number of rows and be of the same type (i.e. depth and color mode)
    *
   ...
   * images (for efficiency it avoids converting to grayscale internally).
    *  See KLT_response_optimized for more details on the internal optimizations
   * of this method, but this graph shows a general view:
    *  <img src="KLT_response_performance_SSE2.png" >
    */
  float KLT_response(const unsigned int x, const unsigned int y,
  ...
  /** Copy operator (if the image is externally stored, the writen image will be
   * such as well).
    * \sa copyFastFrom
    */
   ...
   * image file will be actually loaded into memory in "this" object.
    * \sa operator =
    * \exception CExceptionExternalImageNotFound If the external image couldn't
   * be loaded.
    */
  ...
  void swap(CImage &o); //!< Very efficient swap of two images (just swap the
                        //!internal pointers)
  ...
  /** Returns a pointer to a const T* containing the image - the idea is to call
  ...
  /** Returns a pointer to a T* containing the image - the idea is to call like
   * "img.getAs<IplImage>()" so we can avoid here including OpenCV's headers. */
  ...
  /**  Access to pixels without checking boundaries - Use normally the ()
    ...
    \sa CImage::operator()
    */
  unsigned char *get_unsafe(unsigned int col, unsigned int row,
   ...
   * format: [0,255]->[0,1]
    ...
    * \sa operator()
    */
  float getAsFloat(unsigned int col, unsigned int row,
   ...
   * \exception std::exception On pixel coordinates out of bounds
   */
  unsigned char *operator()(unsigned int col, unsigned int row,
                            unsigned int channel = 0) const;
  ...
  size_t getWidth()
      const override; //!< Returns the width of the image in pixels \sa getSize
  size_t getHeight()
      const override; //!< Returns the height of the image in pixels \sa getSize

  void getSize(TImageSize &s)
      const; //!< Return the size of the image \sa getWidth, getHeight
  ...
  /** Returns the row stride of the image: this is the number of *bytes* between
   * two consecutive rows. You can access the pointer to the first row with
   * get_unsafe(0,0)
    * \sa getSize, get_unsafe */
  size_t getRowStride() const;

  /** Return the maximum pixel value of the image, as a float value in the range
   * [0,1]
    * \sa getAsFloat */
  ...
  /** Returns a string of the form "BGR","RGB" or "GRAY" indicating the channels
   * ordering. \sa setChannelsOrder, swapRB */
   ...
   * always the size of the image at output. If set to false, the matrix will be
   * enlarged to the size of the image, but it will not be cropped if it has
   * room enough (useful for FFT2D,...)
    *  \param x_min The starting "x" coordinate to extract (default=0=the first
   * column)
    *  \param y_min The starting "y" coordinate to extract (default=0=the first
   * row)
   ...
   * (default=-1=the last row)
    * \sa setFromMatrix
    */
  void getAsMatrix(mrpt::math::CMatrixFloat &outMatrix, bool doResize = true,
                   int x_min = 0, int y_min = 0, int x_max = -1,
                   int y_max = -1) const;
   ...
   * (default=-1=the last row)
    ...
    */
  void getAsRGBMatrices(mrpt::math::CMatrixFloat &outMatrixR,
                        mrpt::math::CMatrixFloat &outMatrixG,
                        mrpt::math::CMatrixFloat &outMatrixB,
                        bool doResize = true, int x_min = 0, int y_min = 0,
                        int x_max = -1, int y_max = -1) const;
   ...
   * file, which will be loaded only under demand.
    *   A CImage with external storage does not consume memory until some method
   * trying to access the image is invoked (e.g. getWidth(), isColor(),...)
   ...
   * setExternalStorage. This property remains after serializing the object.
    *   File names can be absolute, or relative to the CImage::IMAGES_PATH_BASE
   * directory. Filenames staring with "X:\" or "/" are considered absolute
   * paths.
   ...
   * to that file, because this method can be also called
    *    to let the object know where to load the image in case its contents are
   * required. Thus, for saving images in this format (not when loading)
  ...
  static std::string
      IMAGES_PATH_BASE; //!< By default, "."  \sa setExternalStorage

  /** See setExternalStorage(). */
  bool isExternallyStored() const noexcept { return m_imgIsExternalStorage; }

  inline std::string getExternalStorageFile() const
      noexcept //!< Only if isExternallyStored() returns true. \sa
  ...
  /** For external storage image objects only, this method makes sure the image
   * is loaded in memory. Note that usually images are loaded on-the-fly on
   * first access and there's no need to call this.
    * \unload
    */
   ...
   * memory for images that will not be used often.
    *  If called for an image without the flag "external storage", it is simply
   * ignored.
  ...
  /** Reads the image from raw pixels buffer in memory.
    */
  void loadFromMemoryBuffer(unsigned int width, unsigned int height, bool color,
   ...
   * increment.
    */
  void loadFromMemoryBuffer(unsigned int width, unsigned int height,
                            unsigned int bytesPerRow, unsigned char *red,
   ...
   * IplImage upon destruction,
    ...
    *   This method provides a fast method to grab images from a camera without
   * making a copy of every frame.
    */
  void setFromIplImage(void *iplImage);

  /** Reads the image from a OpenCV IplImage object (WITHOUT making a copy) and
   * from now on the image cannot be modified, just read.
    *  When assigning an IPLImage to this object with this method, the IPLImage
   * will NOT be released/freed at this object destructor.
    *   This method provides a fast method to grab images from a camera without
   * making a copy of every frame.
  ...
  /** Sets the internal IplImage pointer to that of another given image, WITHOUT
   * making a copy, and from now on the image cannot be modified in this object
   * (it will be neither freed, so the memory responsibility will still be of
  ...
  /** Set the image from a matrix, interpreted as grayscale intensity values, in
    *the range [0,1] (normalized=true) or [0,255] (normalized=false)
    *	Matrix indexes are assumed to be in this order: M(row,column)
    * \sa getAsMatrix
    */
  template <typename Derived>
  void setFromMatrix(const Eigen::MatrixBase<Derived> &m,
   ...
   * (normalized=true) or [0,255] (normalized=false)
    ...
    */
  template <typename Derived>
  void setFromRGBMatrices(const Eigen::MatrixBase<Derived> &m_r,
   ...
   * channels plus a separate gray-level image with A channel.
    ...
    */
  static bool loadTGA(const std::string &fileName, mrpt::utils::CImage &out_RGB,
   ...
   * - TIFF files - TIFF, TIF.
   *
   * \param jpeg_quality Only for JPEG files, the quality of the compression in
   * the range [0-100]. Larger is better quality but slower.
   ...
   * \exception std::exception On number of rows or cols equal to zero or other
   * errors.
   * \sa saveToJPEG
   */
  void saveToStreamAsJPEG(mrpt::utils::CStream &out,
                          const int jpeg_quality = 95) const;
  ...
  /** Returns a RGB version of the grayscale image, or itself if it is already a
   * RGB image.
    * \sa grayscale
    */
...
protected:
  ...
  mutable std::string
      m_externalFile; //!< The file name of a external storage image.
