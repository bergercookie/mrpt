...
*/
template <class T> class CDynamicGrid3D {
public:
  /** Constructor */
  CDynamicGrid3D(double x_min = -1.0, double x_max = 1.0, double y_min = -1.0,
                 double y_max = +1.0, double z_min = -1.0, double z_max = 1.0,
                 double resolution_xy = 0.5, double resolution_z = 0.5)
      : m_map() {
    setSize(x_min, x_max, y_min, y_max, z_min, z_max, resolution_xy,
            resolution_z);
  }
  ...
  */
  virtual void resize(double new_x_min, double new_x_max, double new_y_min,
    ...
    // resolution:
    if (fabs(new_x_min / m_resolution_xy - round(new_x_min / m_resolution_xy)) >
        0.05)
      new_x_min = m_resolution_xy * round(new_x_min / m_resolution_xy);
    if (fabs(new_y_min / m_resolution_xy - round(new_y_min / m_resolution_xy)) >
        0.05)
      new_y_min = m_resolution_xy * round(new_y_min / m_resolution_xy);
    if (fabs(new_z_min / m_resolution_z - round(new_z_min / m_resolution_z)) >
        0.05)
      new_z_min = m_resolution_z * round(new_z_min / m_resolution_z);
    if (fabs(new_x_max / m_resolution_xy - round(new_x_max / m_resolution_xy)) >
        0.05)
      new_x_max = m_resolution_xy * round(new_x_max / m_resolution_xy);
    if (fabs(new_y_max / m_resolution_xy - round(new_y_max / m_resolution_xy)) >
        0.05)
      new_y_max = m_resolution_xy * round(new_y_max / m_resolution_xy);
    if (fabs(new_z_max / m_resolution_z - round(new_z_max / m_resolution_z)) >
        0.05)
    ...
    for (z = 0; z < m_size_z; z++) {
      for (y = 0; y < m_size_y; y++) {
        for (x = 0,
            itSrc = (m_map.begin() + y * m_size_x + z * m_size_x_times_y),
            itDst = (new_map.begin() + extra_x_izq +
   ...
   * undefined (some will remain with
    *  their old values, the new ones will have the default voxel value, but the
   * location of old values
    *  may change wrt their old places).
    * If \a fill_value is not nullptr, it is assured that all cells will have a
   * copy of that value after resizing.
  ...
  /** Erase the contents of all the cells, setting them to their default values
   * (default ctor). */
  virtual void clear() {
    m_map.clear();
    m_map.resize(m_size_x * m_size_y * m_size_z);
   ...
   * x2idx(), y2idx(), z2idx() */
  inline size_t cellAbsIndexFromCXCYCZ(const int cx, const int cy,
                                       const int cz) const {
  ...
  /** Returns a pointer to the contents of a voxel given by its coordinates, or
   ...
   * or nullptr if it is out of the map extensions.
          */
  inline const T *cellByIndex(unsigned int cx, unsigned int cy,
                              unsigned int cz) const {
    const size_t cidx = cellAbsIndexFromCXCYCZ(cx, cy, cz);
    if (cidx == INVALID_VOXEL_IDX)
      return nullptr;
    return &m_map[cidx];
...
protected:
  ...
  double m_x_min, m_x_max, m_y_min, m_y_max, m_z_min, m_z_max, m_resolution_xy,
      m_resolution_z;
