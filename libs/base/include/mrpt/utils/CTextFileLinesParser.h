...
* line, along its line number.
  *  Lines are strip out of leading and trailing whitespaces.
  *  By default, lines starting with either "#", "//" or "%" are skipped
* (comment lines),
...
public:
  ...
  CTextFileLinesParser()
      : m_curLineNum(0), m_filter_MATLAB_comments(true),
        m_filter_C_comments(true), m_filter_SH_comments(true) {}

  /** Constructor for opening a file  \exception std::exception On error opening
   * file */
  CTextFileLinesParser(const std::string &fil)
      : m_filter_MATLAB_comments(true), m_filter_C_comments(true),
  ...
  /** Reads from the file and stores the next (non-comment) line into the given
   * stream buffer.
    * \return false on EOF.
    */
   ...
   * respectively. */
  inline void enableCommentFilters(bool filter_MATLAB_comments,
