...
struct BASE_IMPEXP TMatchingPair {
  TMatchingPair()
      : this_idx(0), other_idx(0), this_x(0), this_y(0), this_z(0), other_x(0),
        other_y(0), other_z(0), errorSquareAfterTransformation(0) {}

  TMatchingPair(unsigned int _this_idx, unsigned int _other_idx, float _this_x,
                float _this_y, float _this_z, float _other_x, float _other_y,
                float _other_z)
      : this_idx(_this_idx), other_idx(_other_idx), this_x(_this_x),
        this_y(_this_y), this_z(_this_z), other_x(_other_x), other_y(_other_y),
        other_z(_other_z), errorSquareAfterTransformation(0) {}
...
public:
   ...
   * correspondences, given the 2D transformation "q"
    ...
    *  Where \f$ e_i \f$ are the elements of the square error vector as computed
   * by computeSquareErrorVector
   ...
   * by computeSquareErrorVector
    ...
    */
  float overallSquareErrorAndPoints(const mrpt::poses::CPose2D &q,
                                    std::vector<float> &xs,
   ...
   * (global) point and the "other" (local) point transformed through "q":
    ...
    */
  void squareErrorVector(const mrpt::poses::CPose2D &q,
                         std::vector<float> &out_sqErrs) const;
   ...
   * (global) point and the "other" (local) point transformed through "q":
    ...
    */
  void squareErrorVector(const mrpt::poses::CPose2D &q,
                         std::vector<float> &out_sqErrs, std::vector<float> &xs,
                         std::vector<float> &ys) const;
   ...
   * correspondence which coincides
    * in both directions, i.e. the best pairing of element `i` in map `this` is
   * the best match for element `j` in map `other`,
    * and viceversa*/
  void filterUniqueRobustPairs(const size_t num_elements_this_map,
...
/** A comparison operator */
bool BASE_IMPEXP operator==(const TMatchingPairList &a,
