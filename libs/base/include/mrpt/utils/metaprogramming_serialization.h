...
public:
  inline ObjectReadFromStream(mrpt::utils::CStream *stream)
      : m_stream(stream) {}
  // T can be CSerializable::Ptr, CSerializable, or any other class implementing
  // ">>"
  template <typename T> inline void operator()(T &obj) { (*m_stream) >> obj; }
};

template <typename ptr_t> struct ObjectReadFromStreamToPtrs {
private:
  CStream *m_stream;
...
public:
  inline ObjectReadFromStreamToPtrs(mrpt::utils::CStream *stream)
      : m_stream(stream) {}
  template <typename ptr2ptr_t> inline void operator()(ptr2ptr_t &obj) {
    ptr_t p;
    (*m_stream) >> p;
    obj = p;
  }
...
public:
  ...
  // T can be CSerializable::Ptr, CSerializable, or any other class implementing
  // "<<"
  template <typename T> inline void operator()(const T &ptr) {
    (*m_stream) << ptr;
  }
};
