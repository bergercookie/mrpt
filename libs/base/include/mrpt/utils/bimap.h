...
public:
  ...
  /**  Get the value associated the given key, KEY->VALUE, raising an exception
   * if not present.
    *  \sa inverse, hasKey, hasValue
  ...
  /**  Get the key associated the given value, VALUE->KEY, raising an exception
   * if not present.
    *  \sa direct, hasKey, hasValue
    * \return false on value not found.
    */
  KEY inverse(const VALUE &v) const {
    const_iterator_inverse i = m_v2k.find(v);
    if (i == m_v2k.end())
      THROW_EXCEPTION("Value not found.");
    return i->second;
  }
