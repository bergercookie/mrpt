...
#endif
  std::string
      m_remotePartIP; //!< The IP address of the remote part of the connection.
  unsigned short
      m_remotePartPort; //!< The TCP port of the remote part of the connection.

  /** Introduces a virtual method responsible for reading from the stream (This
   * method BLOCKS)
...
public:
  ...
  /** Establishes a connection with a remote part.
    * \param remotePartAddress This string can be a host name, like "server" or
   * "www.mydomain.org", or an IP address "11.22.33.44".
   ...
   * textual description of the error.
    */
  void connect(const std::string &remotePartAddress,
               unsigned short remotePartTCPPort, unsigned int timeout_ms = 0);
  ...
  uint64_t Seek(uint64_t Offset,
                CStream::TSeekOrigin Origin = sFromBeginning) override {
    MRPT_START
   ...
   * the starting of data from the other side.
    * \param timeoutBetween_ms The maximum timeout (in milliseconds) to wait for
   * a chunk of data after a previous one.
    *  Set timeout's to -1 to block until the desired number of bytes are read,
   * or an error happens.
    *  \return The number of actually read bytes.
    */
  size_t readAsync(void *Buffer, const size_t Count,
   ...
   * written, or an error happens.
    ...
    */
  size_t writeAsync(const void *Buffer, const size_t Count,
   ...
   * goes fine.
    */
  bool receiveMessage(CMessage &inMsg, const unsigned int timeoutStart_ms = 100,
                      const unsigned int timeoutBetween_ms = 1000);

  /** Return the number of bytes already in the receive queue (they can be read
   * without waiting) */
  ...
  /** Set the size of the SO send buffer. This buffer is used to store data, and
   * is sended when is full.
