...
namespace mrpt {
namespace utils {
/** Pointer to IMPLementation auxiliary structure to make raw pointers movable,
 *copiable and automatically deleted.
 *\ingroup mrpt_base_grp
 */
template <typename T> struct pimpl {
  std::unique_ptr<T> ptr;

  // All these must be defined in a .cpp file with PIMPL_DEFINE(_TYPE), after
  // including the
...
#define PIMPL_IMPLEMENT(_TYPE)                                                 \
  ...
  template <>                                                                  \
  pimpl<_TYPE> &pimpl<_TYPE>::operator=(pimpl<_TYPE> &&op) noexcept {          \
    ptr = std::move(op.ptr);                                                   \
    return *this;                                                              \
  }                                                                            \
  /* Copyable: */                                                              \
  template <> pimpl<_TYPE>::pimpl(const pimpl<_TYPE> &op) {                    \
    if (op.ptr.get() == ptr.get())                                             \
      return;                                                                  \
    ptr.reset(new _TYPE());                                                    \
    *ptr = *op.ptr;                                                            \
  }                                                                            \
