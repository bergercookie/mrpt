...
struct ObjectDelete {
  template <typename T> inline void operator()(const T *ptr) { delete ptr; }
};
...
struct ObjectClear {
  template <typename T> inline void operator()(T &ptr) {
    if (ptr)
      ptr->clear();
  }
};
 ...
 * pointer or smart-pointer, intended for being used in STL algorithms. */
struct ObjectClear2 {
  template <typename T> inline void operator()(T &ptr) { ptr.reset(); }
};
...
struct ObjectClearSecond {
  template <typename T> inline void operator()(T obj) { obj.second.reset(); }
};
 ...
 * STL algorithms.
  ...
  *   \endcode
  */
template <typename TARGET_TYPE> struct ObjectConvert {
  template <typename T> inline TARGET_TYPE operator()(const T &val) {
...
struct ObjectPairMakeUnique {
  template <typename T> inline void operator()(T &ptr) {
    ptr.first.reset(dynamic_cast<typename T::first_type::element_type *>(
        ptr.first->clone()));
    ptr.second.reset(dynamic_cast<typename T::second_type::element_type *>(
        ptr.first->clone()));
  }
};
...
/** An object for making smart pointers unique (ie, making copies if necessary),
 * intended for being used in STL algorithms. */
template <typename T> struct ObjectClearUnique {
...
inline void copy_container_typecasting(const src_container &src,
                                       dst_container &trg) {
  trg.resize(src.size());
...
public:
  ...
  operator-(const MemoryBypasserIterator<T, U> &it) const {
...
public:
  ...
  inline BinaryMemberFunctionWrapper(V &o, MemberFunction f)
      : obj(o), func(f) {}
...
/** This function creates a function from an object and a member function.
  ...
  */
template <typename T, typename U1, typename U2, typename V>
inline BinaryMemberFunctionWrapper<T, U1, U2, V>
wrapMember(V &obj, T (V::*fun)(U1, U2)) {
