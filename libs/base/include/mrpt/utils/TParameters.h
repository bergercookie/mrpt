 ...
 * "10.0".
  ...
  */
template <typename T> struct TParameters : public std::map<std::string, T> {
  ...
  inline T operator[](const std::string &s) const {
    ...
    if (BASE::end() == it)
      throw std::logic_error(std::string("Parameter '") + s +
  ...
  void getAsString(std::string &s) const {
    size_t maxStrLen = 10;
    for (typename BASE::const_iterator it = BASE::begin(); it != BASE::end();
         ++it)
...
typedef TParameters<double>
    TParametersDouble; //!< See the generic template mrpt::utils::TParameters
typedef TParameters<std::string>
    TParametersString; //!< See the generic template mrpt::utils::TParameters
