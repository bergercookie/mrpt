...
public:
  /** Constructor that creates the socket, performs binding, and start listening
   * mode.
   ...
   * waiting for "accept" before new ones are rejected.
    ...
    * \sa isListening
    * \exception std::exception If there is any error creating the socket, with
  ...
  bool isListening(); //!< Returns true if the socket was successfully open and
                      //!it's bound to the desired port.

  /** Waits for an incoming connection (indefinitely, or with a given timeout)
    * The returned object represents the new connection, and MUST BE deleted by
...
/** The handle for the listening server TCP socket. */
  ...
  void setupSocket(unsigned short listenPort, const std::string &IPaddress,
