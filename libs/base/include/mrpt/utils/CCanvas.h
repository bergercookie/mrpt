...
protected:
  ...
  const uint32_t
      *m_selectedFontBitmaps; //!< Direct access to character bitmaps.
...
public:
  ...
  virtual void line(int x0, int y0, int x1, int y1,
                    const mrpt::utils::TColor color, unsigned int width = 1,
  ...
  /** Draws a triangle
    ...
    * \sa triangle
    */
  void triangle(int x0, int y0, int size, const mrpt::utils::TColor color,
   ...
   * in a more appropiate manner.
    * \sa rectangle
    */
  virtual void filledRectangle(int x0, int y0, int x1, int y1,
   ...
   * encoding.
    ...
    * \sa selectTextFont
    */
  virtual void textOut(int x0, int y0, const std::string &str,
                       const mrpt::utils::TColor color);
  ...
  /** Select the current font used when drawing text.
    ...
    *  - 9x15B
    *  - 10x20
    *  - 18x18ja (Asian characters for UTF-8 strings - Only available if MRPT is
   * built with MRPT_HAS_ASIAN_FONTS = true)
    *
  ...
  /** Draws an image as a bitmap at a given position.
    * \param x0 The top-left corner x coordinates on this canvas where the image
   * is to be drawn
    * \param y0 The top-left corner y coordinates on this canvas where the image
   * is to be drawn
  ...
  * \param type The cross type. It could be: 'x', '+' or ':'(like '+' but clear
  * at the center dot)
  * \param width The desired width of the cross (this is IGNORED yet)
  */
  void cross(int x0, int y0, const mrpt::utils::TColor color, char type,
             unsigned int size = 5, unsigned int width = 1);

  /** Draws an image as a bitmap at a given position, with some custom scale and
   * rotation changes.
    * \param x0 The top-left corner x coordinates on this canvas where the image
   * is to be drawn
    * \param y0 The top-left corner y coordinates on this canvas where the image
   ...
   * in a more appropiate manner.
    */
  virtual void drawImage(int x, int y, const utils::CImage &img, float rotation,
                         float scale);
  ...
  /** Draws an ellipse representing a given confidence interval of a 2D Gaussian
   * distribution.
   ...
   * 2->95%, 3=99.97%,...)
    ...
    * \param width The desired width of the line (this is IGNORED in this
   * virtual class)
    * \param nEllipsePoints The number of points to generate to approximate the
   * ellipse shape.
  ...
  /** Draws a set of marks onto the image, given a generic container of entities
   * having just "x" and "y" fields.
  ...
  void drawFeaturesSimple(const FEATURELIST &list,
  ...
  /** Draws a set of marks (or scaled circles for features with scale) onto the
   * image, given a generic container of features.
   ...
   */
  template <class FEATURELIST>
  void drawFeatures(const FEATURELIST &list, const TColor &color = TColor::red,
    ...
    for (size_t i = 0; i < list.size(); ++i) {
      ...
      if (showIDs)
        this->textOut(
            x, y, format("%u", static_cast<unsigned int>(list.getFeatureID(i))),
            TColor::red);
      if (showResponse)
        this->textOut(x, y + 10,
                      format("R:%u", static_cast<unsigned int>(
