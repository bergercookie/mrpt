...
protected:
  ...
  static void dumpVar_string(CStream &out, const char *varName,
                             const std::string &v);
...
public:
  ...
  /** Behaves like loadFromConfigFile, but you can pass directly a file name and
   * a temporary CConfigFile object will be created automatically to load the
   * file.
    * \sa loadFromConfigFile
    */
  void loadFromConfigFileName(const std::string &config_file,
   ...
   */
  virtual void saveToConfigFile(mrpt::utils::CConfigFileBase &target,
                                const std::string &section) const;

  /** Behaves like saveToConfigFile, but you can pass directly a file name and a
   * temporary CConfigFile object will be created automatically to save the
   * file.
    * \sa saveToConfigFile, loadFromConfigFileName
    */
  void saveToConfigFileName(const std::string &config_file,
                            const std::string &section) const;
...
#define LOADABLEOPTS_DUMP_VAR(variableName, variableType)                      \
  {                                                                            \
    dumpVar_##variableType(out, #variableName,                                 \
                           static_cast<variableType>(variableName));           \
  }
...
#define LOADABLEOPTS_DUMP_VAR_DEG(variableName)                                \
  {                                                                            \
    dumpVar_double(out, #variableName,                                         \
                   RAD2DEG(static_cast<double>(variableName)));                \
  }
