...
public:
  /** Unbound constructor: must bind this object to CConfigFileBase before usage
   * with \a bind() and \a setPrefixes() */
  CConfigFilePrefixer();
  /** Construct and bind to (wrap) a given object with given prefix texts */
  CConfigFilePrefixer(const CConfigFileBase &o,
                      const std::string &prefix_sections,
                      const std::string &prefix_keys);

  /** Make this object to wrap the given existing CConfigFileBase object. Can be
   * changed at any moment after construction */
  void bind(const CConfigFileBase &o);

  /** Change the prefix for sections and keys. Can be called at any moment. */
  void setPrefixes(const std::string &prefix_sections,
                   const std::string &prefix_keys);

  std::string getSectionPrefix() const;
  std::string getKeyPrefix() const;
  CConfigFileBase *getBoundConfigFileBase() const; //!< Returns the
                                                   //!currently-bounded config
                                                   //!source, or nullptr if
                                                   //!none.

  virtual ~CConfigFilePrefixer();

  void
  getAllSections(vector_string &sections) const override; // See base class docs
  void getAllKeys(const std::string &section,
                  vector_string &keys) const override; // See base class docs
