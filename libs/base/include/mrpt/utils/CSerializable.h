...
#if MRPT_HAS_MATLAB
typedef struct mxArray_tag mxArray; //!< Forward declaration for mxArray (avoid
                                    //!#including as much as possible to speed
                                    //!up compiling)
#endif
...
protected:
   ...
   * \sa CStream
   */
  virtual void writeToStream(mrpt::utils::CStream &out,
 ...
 * >Integration with BABEL</a>
 */
void BASE_IMPEXP StringToObject(const std::string &str,
 ...
 */
void BASE_IMPEXP ObjectToOctetVector(const CSerializable *o,
 ...
 */
void BASE_IMPEXP OctetVectorToObject(const vector_byte &in_data,
                                     CSerializable::Ptr &obj);
 ...
 */
void BASE_IMPEXP ObjectToRawString(const CSerializable *o,
                                   std::string &out_str);
 ...
 */
void BASE_IMPEXP RawStringToObject(const std::string &in_str,
                                   CSerializable::Ptr &obj);
...
#define DEFINE_SERIALIZABLE_CUSTOM_LINKAGE(                                    \
  ...
  DEFINE_MRPT_OBJECT_CUSTOM_LINKAGE(class_name, _STATIC_LINKAGE_,              \
                                    _VIRTUAL_LINKAGE_)                         \
protected:                                                                     \
...
#define DEFINE_SERIALIZABLE(class_name)                                        \
  DEFINE_SERIALIZABLE_CUSTOM_LINKAGE(class_name,                               \
                                     void /*no extra linkage keyword*/,        \
                                     static /*none*/, virtual /*none*/)
...
#define DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(class_name, base_name,    \
                                                     _LINKAGE_)                \
  DEFINE_MRPT_OBJECT_POST_CUSTOM_BASE_LINKAGE2(class_name, base_name,          \
                                               _LINKAGE_ class_name)

/**  This declaration must be inserted in all CSerializable classes definition,
 * before the class declaration. */

#define DEFINE_SERIALIZABLE_POST_CUSTOM_BASE(class_name, base_name)            \
  DEFINE_MRPT_OBJECT_POST_CUSTOM_BASE_LINKAGE(class_name, base_name,           \
                                              BASE_IMPEXP)
...
/** This must be inserted as implementation of some required members for
  ...
  */
#define IMPLEMENTS_VIRTUAL_SERIALIZABLE(class_name, base_class_name,           \
                                        NameSpace)                             \
...
#define DECLARE_MEXPLUS_FROM(complete_type)                                    \
  namespace mexplus {                                                          \
  template <typename T> mxArray *from(const T &value);                         \
  template <> mxArray *from(const complete_type &value);                       \
  }

#define IMPLEMENTS_MEXPLUS_FROM(complete_type)                                 \
  namespace mexplus {                                                          \
  template <> mxArray *from(const complete_type &var) {                        \
