...
public:
  ...
  uint64_t getTotalBytesCount() override; //!< Method for getting the total
                                          //!number of <b>compressed</b> bytes
                                          //!of in the file (the physical size
                                          //!of the compressed file).
  uint64_t getPosition() override; //!< Method for getting the current cursor
                                   //!position in the <b>compressed</b>, where 0
                                   //!is the first byte and TotalBytesCount-1
                                   //!the last one.
...
static_assert(!std::is_copy_constructible<CFileGZInputStream>::value &&
