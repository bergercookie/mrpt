 ...
 * rgb2hsv */
void BASE_IMPEXP hsv2rgb(float h, float s, float v, float &r, float &g,
                         float &b);

/** Transform RGB color components to HSV, all of them in the range [0,1] \sa
 * hsv2rgb */
void BASE_IMPEXP rgb2hsv(float r, float g, float b, float &h, float &s,
                         float &v);
...
void BASE_IMPEXP colormap(const TColormap &color_map, const float color_index,
