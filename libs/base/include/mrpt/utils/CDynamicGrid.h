...
/** A 2D grid of dynamic size which stores any kind of data at each cell.
        ...
        */
template <class T> class CDynamicGrid {
protected:
...
public:
  /** Constructor */
  CDynamicGrid(double x_min = -10.0, double x_max = 10.0, double y_min = -10.0f,
               double y_max = 10.0f, double resolution = 0.10f)
      : m_map(), m_x_min(), m_x_max(), m_y_min(), m_y_max(), m_resolution(),
        m_size_x(), m_size_y() {
   ...
   * undefined (some will remain with
    *  their old values, the new ones will have the default cell value, but the
   ...
   * copy of that value after resizing.
    * \sa resize, fill
    */
  void setSize(const double x_min, const double x_max, const double y_min,
  ...
  virtual void resize(double new_x_min, double new_x_max, double new_y_min,
                      double new_y_max, const T &defaultValueNewCells,
    ...
    for (y = 0; y < m_size_y; y++) {
      for (x = 0, itSrc = (m_map.begin() + y * m_size_x),
          itDst =
               (new_map.begin() + extra_x_izq + (y + extra_y_arr) * new_size_x);
  ...
  /** Returns a pointer to the contents of a cell given by its cell indexes, or
   * nullptr if it is out of the map extensions.
          */
  ...
  /** Transform a global (linear) cell index value into its corresponding (x,y)
   * cell indexes. */
   ...
   * converted to the type of the matrix elements (e.g. double).
          */
  template <class MAT> void getAsMatrix(MAT &m) const {
  ...
  /** The user must implement this in order to provide "saveToTextFile" a way to
   * convert each cell into a numeric value */
