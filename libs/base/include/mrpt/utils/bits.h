 ...
 * prefer fast code. */
template <class T> inline T hypot_fast(const T x, const T y) {
...
template <typename T> inline int sign(T x) { return x < 0 ? -1 : 1; }

/** Returns the sign of X as "0", "1" or "-1" */
template <typename T> inline int signWithZero(T x) {
...
template <typename T> T lowestPositive(const T a, const T b) {
  if (a > 0 && a <= b)
...
template <typename T> inline const T min3(const T &A, const T &B, const T &C) {
  return std::min<T>(A, std::min<T>(B, C));
}
template <typename T> inline const T max3(const T &A, const T &B, const T &C) {
  return std::max<T>(A, std::max<T>(B, C));
}

/** Rounds toward zero  */
template <typename T> inline int fix(T x) {
  return x > 0 ? static_cast<int>(floor(static_cast<double>(x)))
               : static_cast<int>(ceil(static_cast<double>(x)));
}

using mrpt::math::square; //!< Allow square() to be available under mrpt::math
                          //!and mrpt::utils

/** Utility to get a cast'ed pointer from a smart pointer */
template <class R, class SMART_PTR> inline R *getAs(SMART_PTR &o) {
  return static_cast<R *>(&(*o));
}

/** Utility to get a cast'ed pointer from a smart pointer */
template <class R, class SMART_PTR> inline const R *getAs(const SMART_PTR &o) {
...
template <class T> void delete_safe(T *&ptr) {
  if (ptr) {
    delete ptr;
    ptr = nullptr;
  }
...
template <class VECTOR_T> inline void vector_strong_clear(VECTOR_T &v) {
  VECTOR_T dummy;
  dummy.swap(v);
}

/** Returns the smaller number >=len such that it's a multiple of 4 */
template <typename T> T length2length4N(T len) {
  if (0 != (len & 0x03))
    len += (4 - (len & 0x03));
  return len;
}
