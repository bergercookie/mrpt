...
public:
  ...
  /** Return a description of the error if loadFromPlyFile() returned false, or
   * an empty string if the file was loaded without problems. */
...
protected:
   ...
   * info.
    */
  virtual void
  PLY_import_set_vertex(const size_t idx, const mrpt::math::TPoint3Df &pt,
...
public:
   ...
   * strings stored in the file will be returned.
    * \param[in] file_obj_info If provided (!=nullptr) the list of "object info"
   * strings stored in the file will be returned.
    * \return false on any error writing the file. To obtain more details on the
   * error you can call getSavePLYErrorString()
    */
  bool saveToPlyFile(const std::string &filename, bool save_in_binary = false,
...
protected:
   ...
   * info.
    */
  virtual void PLY_export_get_vertex(const size_t idx,
