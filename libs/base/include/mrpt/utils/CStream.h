...
protected:
  ...
  /** Read the object */
  void internal_ReadObject(CSerializable *newObj, const std::string &className,
                           bool isOldFormat, int8_t version);

  /** Read the object Header*/
  void internal_ReadObjectHeader(std::string &className, bool &isOldFormat,
                                 int8_t &version);
...
public:
   ...
   *bytes from the MRPT stream standard (little endianness) to the format of the
  ...
  /** Reads a block of bytes from the stream into Buffer, and returns the amound
   *of bytes actually read, without waiting for more extra bytes to arrive (just
   *those already enqued in the stream).
  ...
  /** Writes a sequence of elemental datatypes, taking care of reordering their
   * bytes from the running architecture to MRPT stream standard (little
   * endianness).
  ...
  /** Introduces a pure virtual method for moving to a specified position in the
   *streamed resource.
   ...
   *	- sFromCurrent		Offset is from the current position in the
   *resource. Seek moves to Position + Offset.
   *	- sFromEnd			Offset is from the end of the resource. Offset
   *must be <= 0 to indicate a number of bytes before the end of the file.
   * \return Seek returns the new value of the Position property.
   */
  virtual uint64_t Seek(uint64_t Offset,
                        CStream::TSeekOrigin Origin = sFromBeginning) = 0;

  /** Returns the total amount of bytes in the stream.
   */
  virtual uint64_t getTotalBytesCount() = 0;

  /** Method for getting the current cursor position, where 0 is the first byte
   * and TotalBytesCount-1 the last one.
   */
  ...
  /** Reads an object from stream, its class determined at runtime, and returns
   * a smart pointer to the object.
   * \exception std::exception On I/O error or undefined class.
   * \exception mrpt::utils::CExceptionEOF On an End-Of-File condition found at
   * a correct place: an EOF that abruptly finishes in the middle of one object
   * raises a plain std::exception instead.
   */

  CSerializable::Ptr ReadObject() { return ReadObject<CSerializable>(); }

  /** Reads an object from stream, its class determined at runtime, and returns
   * a smart pointer to the object. This version is similar to
   * std::make_shared<T>.
   * \exception std::exception On I/O error or undefined class.
   * \exception mrpt::utils::CExceptionEOF On an End-Of-File condition found at
   * a correct place: an EOF that abruptly finishes in the middle of one object
   * raises a plain std::exception instead.
   */
  template <typename T> typename T::Ptr ReadObject() {
    ...
    internal_ReadObject(obj.get() /* may be nullptr */, strClassName,
                        isOldFormat,
...
private:
  template <typename RET> RET ReadVariant_helper(CSerializable::Ptr &ptr) {
...
public:
  /** Reads a variant from stream, its class determined at runtime, and returns
   * a variant to the object.
   ...
   * raises a plain std::exception instead.
   */
  template <typename... T> typename mrpt::utils::variant<T...> ReadVariant() {
    CSerializable::Ptr obj;
    std::string strClassName;
    bool isOldFormat;
    int8_t version;
  ...
  void printf_vector(const char *fmt, const CONTAINER_TYPE &V,
                     char separator = ',') {
    this->printf("[");
  ...
  /** Tries to receive a message from the device.
    * \exception std::exception On communication errors
    * \returns True if successful, false if there is no new data from the device
   * (but communications seem to work fine)
...
CStream BASE_IMPEXP &operator<<(mrpt::utils::CStream &s,
...
CStream BASE_IMPEXP &operator<<(mrpt::utils::CStream &,
...
CStream BASE_IMPEXP &operator>>(mrpt::utils::CStream &in,
