 ...
 * Jan 30, 2004
 *
 * \note This class DOES NOT support mathematical operations on its elements:
*it's a generic container, it doesn't assume they are numerical.
 * \note For a summary and classification of all MRPT vector, array and matrix
*classes see:
*http://www.mrpt.org/Matrices_vectors_arrays_and_Linear_Algebra_MRPT_and_Eigen_classes
 *
* \sa mrpt::math::CArrayNumeric (for another, non-related base template class
*that DOES support maths)
  ...
  inline void resize(const size_t nElements) {
    if (nElements != N)
      throw std::logic_error(
          format("Try to change the size of a %u-CArray to %u.",
                 static_cast<unsigned>(N), static_cast<unsigned>(nElements)));
  }
...
#ifdef _DEBUG
    if (N != n)
      throw std::out_of_range("CArray<>: assign() of incorrect length");
#endif
    for (size_t i = 0; i < N; i++)
      elems[i] = value;
  }

  // assign a range of values corresponding to a pair of iterators (by PMO for
  // MRPT)
...
// reverse iterator support
  ...
  // assignment with type conversion
  template <typename T2> CArray<T, 0> &operator=(const CArray<T2, 0> &rhs) {
