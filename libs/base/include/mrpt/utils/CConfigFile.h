...
public:
  ...
  /** Constructor, does not open any file. You should call "setFileName" before
   * reading or writting or otherwise nothing will be read and write operations
  ...
  /** Associate this object with the given file, so future read/write operations
   * will be applied to that file (it's synchronized at destruction) */
  ...
  void discardSavingChanges(); //!< Discard saving (current) changes to physical
  ...
  virtual void getAllKeys(const std::string &section,
                          vector_string &keys) const override;
