 ...
 * (CMultiMetricMaps, CImage,...)
  */
void BASE_IMPEXP registerClassCustomName(const char *customName,
...
std::vector<const mrpt::utils::TRuntimeClassId *>
    BASE_IMPEXP getAllRegisteredClasses();
 ...
 * registered
  ...
  */
const TRuntimeClassId BASE_IMPEXP *
findRegisteredClass(const std::string &className);
...
template <typename T> struct IS_CLASS_impl {
  template <typename PTR> static bool check(const PTR p) {
...
public:
  ...
  /** Returns a copy of the object, indepently of its class, as a smart pointer
   * (the newly created object will exist as long as any copy of this smart
   * pointer). */
 ...
 * arguments */
#define DEFINE_MRPT_OBJECT_CUSTOM_LINKAGE(class_name, _STATIC_LINKAGE_,        \
                                          _VIRTUAL_LINKAGE_)                   \
  /*! @name RTTI stuff  */                                                     \
  /*! @{  */                                                                   \
protected:                                                                     \
...
#define DEFINE_MRPT_OBJECT(class_name)                                         \
  DEFINE_MRPT_OBJECT_CUSTOM_LINKAGE(class_name, static /*none*/,               \
                                    virtual /*none*/)

// This macro is a workaround to avoid possibly empty arguments to MACROS (when
// _LINKAGE_ evals to nothing...)
#define DEFINE_MRPT_OBJECT_POST_CUSTOM_BASE_LINKAGE(class_name, base_name,     \
                                                    _LINKAGE_)                 \
 ...
 * the class declaration.  */
#define DEFINE_MRPT_OBJECT_POST_CUSTOM_BASE_LINKAGE2(class_name, base_name,    \
                                                     class_name_LINKAGE_)
...
#define DEFINE_MRPT_OBJECT_POST_CUSTOM_LINKAGE2(class_name,                    \
                                                class_name_LINKAGE_)           \
  using class_name##Ptr = class_name::Ptr; //!< MRPT 1.X backwards-compatible
                                           //!smart pointer name (to be removed
                                           //!in a future version of MRPT! Use
                                           //!CFoo::Ptr instead)
#else
...
#define DEFINE_MRPT_OBJECT_POST(class_name)                                    \
  DEFINE_MRPT_OBJECT_POST_CUSTOM_LINKAGE(                                      \
      class_name,                                                              \
      BASE_IMPEXP) // This macro is valid for classes within mrpt-base only.
...
#define IMPLEMENTS_MRPT_OBJECT(class_name, base, NameSpace)                    \
  ...
  const mrpt::utils::TRuntimeClassId *NameSpace::class_name::GetRuntimeClass() \
      const {                                                                  \
