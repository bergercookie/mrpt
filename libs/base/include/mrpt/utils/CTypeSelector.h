...
/** This class represents a std::string derived class which is also
 * CSerializable
 * \sa CSerializable
...
public:
  /** Default constructor.
    * \param posibilitiesList The list of options, as a comma-separated-string,
   * for example: "type 1,other type 2,type_3"
    * \param defaultType Default type value
    */
  CTypeSelector(std::string posibilitiesList = "",
  ...
  /** Fast check for a given type, returns true if the selection is exactly the
   * specified type name.
    */
  ...
  /** Sets the currently selected type.
    ...
    * \exception std::exception On trying to select a type not in the list of
   * posible values.
    */
  void setType(const std::string &type);

  /** Returns the index of a given type within the list of all possible types,
   * or -1 if the given string is not a valid type.
    */
