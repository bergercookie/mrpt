...
#define MRPT_DECLARE_TTYPENAME_CONTAINER(_CONTAINER)                           \
  template <typename V> struct TTypeName<_CONTAINER<V>> {                      \
...
#define MRPT_DECLARE_TTYPENAME_CONTAINER_ASSOC(_CONTAINER)                     \
  template <typename K, typename V> struct TTypeName<_CONTAINER<K, V>> {       \
    static std::string get() {                                                 \
      return std::string(#_CONTAINER) + std::string("<") +                     \
...
template <typename T1, typename T2> struct TTypeName<std::pair<T1, T2>> {
  static std::string get() {
