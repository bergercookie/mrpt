...
public:
  uint32_t type; //!< An identifier of the message type (only the least-sig byte
                 //!is typically sent)
  std::vector<uint8_t> content; //!< The contents of the message (memory is
                                //!automatically handled by the std::vector
                                //!object)

  /** A method for serializing a MRPT's object into the content.
    *  Any modification to data in "content" after this will corrupt the object
   * serialization.
  ...
  /** A method that parse the data in the message into an existing object.
    *  Note that the class of the object must be known and must match the one of
   ...
   * Deleting this object when
    *   no longer required is the responsability of the user. Note that previous
   ...
   * cross-platform.
    ...
    */
  template <class T> void setContentFromStruct(const T &data) {
   ...
   * cross-platform.
    ...
    */
  template <class T> void getContentAsStruct(T &data) const {
    MRPT_START
