...
#define MRPTSTL_SERIALIZABLE_SEQ_CONTAINER(CONTAINER)                          \
  ...
  CStream &operator>>(mrpt::utils::CStream &in, CONTAINER<T, _Ax> &obj) {      \
    ...
    if (pref != #CONTAINER)                                                    \
      THROW_EXCEPTION_FMT(                                                     \
          "Error: serialized container %s<%s>'s preambles is wrong: '%s'",     \
          #CONTAINER, TTypeName<T>::get().c_str(), pref.c_str())               \
    in >> stored_T;                                                            \
    if (stored_T != mrpt::utils::TTypeName<T>::get())                          \
      THROW_EXCEPTION_FMT("Error: serialized container %s< %s != %s >",        \
                          #CONTAINER, stored_T.c_str(),                        \
                          TTypeName<T>::get().c_str())                         \
    uint32_t n;                                                                \
    in >> n;                                                                   \
    obj.resize(n);                                                             \
    std::for_each(obj.begin(), obj.end(),                                      \
                  mrpt::utils::metaprogramming::ObjectReadFromStream(&in));    \
    return in;                                                                 \
  }
...
#define MRPTSTL_SERIALIZABLE_ASSOC_CONTAINER(CONTAINER)                        \
  ...
  CStream &operator<<(mrpt::utils::CStream &out,                               \
                      const CONTAINER<K, V, _Pr, _Alloc> &obj) {               \
  ...
  template <class K, class V, class _Pr, class _Alloc>                         \
  CStream &operator>>(mrpt::utils::CStream &in,                                \
                      CONTAINER<K, V, _Pr, _Alloc> &obj) {                     \
    obj.clear();                                                               \
    std::string pref, stored_K, stored_V;                                      \
    in >> pref;                                                                \
    if (pref != #CONTAINER)                                                    \
      THROW_EXCEPTION(format(                                                  \
          "Error: serialized container %s<%s,%s>'s preamble is wrong: '%s'",   \
          #CONTAINER, TTypeName<K>::get().c_str(),                             \
          TTypeName<V>::get().c_str(), pref.c_str()))                          \
    in >> stored_K;                                                            \
    if (stored_K != TTypeName<K>::get())                                       \
      THROW_EXCEPTION(                                                         \
          format("Error: serialized container %s key type %s != %s",           \
                 #CONTAINER, stored_K.c_str(), TTypeName<K>::get().c_str()))   \
    in >> stored_V;                                                            \
    if (stored_V != TTypeName<V>::get())                                       \
      THROW_EXCEPTION(                                                         \
          format("Error: serialized container %s value type %s != %s",         \
                 #CONTAINER, stored_V.c_str(), TTypeName<V>::get().c_str()))   \
    uint32_t n;                                                                \
    in >> n;                                                                   \
...
MRPTSTL_SERIALIZABLE_ASSOC_CONTAINER(
    std::multimap) // Serialization for std::multimap

#define MRPTSTL_SERIALIZABLE_SIMPLE_ASSOC_CONTAINER(CONTAINER)                 \
  /** Template method to serialize an associative STL container  */            \
  template <class K, class _Pr, class _Alloc>                                  \
  CStream &operator<<(mrpt::utils::CStream &out,                               \
                      const CONTAINER<K, _Pr, _Alloc> &obj) {                  \
    out << std::string(#CONTAINER) << TTypeName<K>::get();                     \
    out << static_cast<uint32_t>(obj.size());                                  \
    for (typename CONTAINER<K, _Pr, _Alloc>::const_iterator it = obj.begin();  \
         it != obj.end(); ++it)                                                \
      out << *it;                                                              \
    return out;                                                                \
  }                                                                            \
  ...
  template <class K, class _Pr, class _Alloc>                                  \
  CStream &operator>>(mrpt::utils::CStream &in,                                \
                      CONTAINER<K, _Pr, _Alloc> &obj) {                        \
    obj.clear();                                                               \
    std::string pref, stored_K;                                                \
    in >> pref;                                                                \
    if (pref != #CONTAINER)                                                    \
      THROW_EXCEPTION(format(                                                  \
          "Error: serialized container %s<%s>'s preamble is wrong: '%s'",      \
          #CONTAINER, TTypeName<K>::get().c_str(), pref.c_str()))              \
    in >> stored_K;                                                            \
    if (stored_K != TTypeName<K>::get())                                       \
      THROW_EXCEPTION(                                                         \
          format("Error: serialized container %s key type %s != %s",           \
...
MRPTSTL_SERIALIZABLE_SIMPLE_ASSOC_CONTAINER(
    std::set) // Serialization for std::set
MRPTSTL_SERIALIZABLE_SIMPLE_ASSOC_CONTAINER(
    std::multiset) // Serialization for std::multiset
...
CStream &operator>>(mrpt::utils::CStream &in, std::pair<T1, T2> &obj) {
  ...
  if (pref != "std::pair")
    THROW_EXCEPTION(
        format("Error: serialized std::pair<%s,%s>'s preamble is wrong: '%s'",
               TTypeName<T1>::get().c_str(), TTypeName<T2>::get().c_str(),
               pref.c_str()))
  in >> stored_K;
  if (stored_K != TTypeName<T1>::get())
    THROW_EXCEPTION(format("Error: serialized std::pair first type %s != %s",
                           stored_K.c_str(), TTypeName<T1>::get().c_str()))
  in >> stored_V;
  if (stored_V != TTypeName<T2>::get())
    THROW_EXCEPTION(format("Error: serialized std::pair second type %s != %s",
                           stored_V.c_str(), TTypeName<T2>::get().c_str()))
