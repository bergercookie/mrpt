...
/** File open modes are used in CFileStream
  *  Posible values are:
        - fomRead
        - fomWrite (creates the file if it didn't exist, otherwise truncates
  it).
...
public:
  ...
  bool
  fileOpenCorrectly(); //!< Returns true if the file was open without errors.
  bool is_open() {
  ...
  uint64_t getTotalBytesCount() override; //!< Method for getting the total
                                          //!number of bytes writen to buffer.
  uint64_t getPosition() override; //!< Method for getting the current cursor
                                   //!position, where 0 is the first byte and
                                   //!TotalBytesCount-1 the last one
  uint64_t getPositionI(); //!< The current Input cursor position, where 0 is
                           //!the first byte
  uint64_t getPositionO(); //!< The current Input cursor position, where 0 is
                           //!the first byte
...
static_assert(!std::is_copy_constructible<CFileStream>::value &&
