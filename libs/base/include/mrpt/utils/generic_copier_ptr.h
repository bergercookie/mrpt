...
namespace internal {
template <typename T> struct CopyStatic {
  T *copy(const T *o) {
    if (!o)
      return nullptr;
    return new T(*o);
...
template <typename T> struct CopyCloner {
  T *copy(const T *o) {
    if (!o)
      return nullptr;
...
template <typename T, typename Copier> class generic_copier_ptr {
public:
  ...
  /** move operator */
  generic_copier_ptr<T, Copier> &
  operator=(const generic_copier_ptr<T, Copier> &&o) {
    if (this == &o)
      return *this;
    m_ptr = o.m_ptr;
    o.m_ptr = nullptr;
