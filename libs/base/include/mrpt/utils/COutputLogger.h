...
typedef void (*output_logger_callback_t)(
...
public:
  static mrpt::system::TConsoleColor logging_levels_to_colors
      [NUMBER_OF_VERBOSITY_LEVELS]; //! Map from VerbosityLevels to their
                                    //! corresponding
                                    //! mrpt::system::TConsoleColor. Handy for
                                    //! coloring the input based on the
                                    //! verbosity of the message
  static std::string
      logging_levels_to_names[NUMBER_OF_VERBOSITY_LEVELS]; //!< Map from
                                                           //!VerbosityLevels to
                                                           //!their
                                                           //!corresponding
                                                           //!names. Handy for
  ...
  COutputLogger(); //!< Default class constructor. Name of the logger is
                   //!initialized to "logStr"
  virtual ~COutputLogger(); //!< virtual dtor (so we can derive classes from
                            //!this one)
   ...
   * \sa log, logFmt
   */
  void logCond(const VerbosityLevel level, bool cond,
               const std::string &msg_str) const;

  void setLoggerName(const std::string &name); //!< Set the name of the
                                               //!COutputLogger instance.  \sa
                                               //!getLoggerName
  std::string getLoggerName() const; //!< Return the name of the COutputLogger
                                     //!instance.  \sa setLoggerName
  ...
  void getLogAsString(std::string &log_contents) const; //!< Fill the provided
                                                        //!string with the
                                                        //!contents of the
                                                        //!logger's history in
                                                        //!std::string
                                                        //!representation
  std::string getLogAsString() const; //!< Get the history of COutputLogger
                                      //!instance in a string representation.
  ...
  std::string getLoggerLastMsg()
      const; //!< Return the last Tmsg instance registered in the logger history
  void getLoggerLastMsg(std::string &msg_str) const; //!< Fill inputtted string
                                                     //!with the contents of the
                                                     //!last message in history
  void loggerReset(); //!< Reset the contents of the logger instance. Called
                      //!upon construction.

  bool logging_enable_console_output; //!< [Default=true] Set it to false in
                                      //!case you don't want the logged messages
                                      //!to be dumped to the output
                                      //!automatically.
  bool logging_enable_keep_record;    //!< [Default=false] Enables storing all
                                      //!messages into an internal list. \sa
                                      //!writeLogToFile, getLogAsString

  void logRegisterCallback(output_logger_callback_t userFunc,
                           void *userParam = NULL);
  void logDeregisterCallback(output_logger_callback_t userFunc,
                             void *userParam = NULL);
  /** @} */
  ...
  struct BASE_IMPEXP TCallbackEntry {
    ...
    bool
    operator==(const mrpt::utils::COutputLogger::TCallbackEntry &c2) const {
      return func == c2.func && userParam == c2.userParam;
    }
  };

protected:
  /** \brief Provided messages with VerbosityLevel smaller than this value shall
   * be ignored */
...
private:
  ...
  struct BASE_IMPEXP TMsg {
     ...
     * current message
     */
    TMsg(const mrpt::utils::VerbosityLevel level, const std::string &msg,
  ...
  std::string generateStringFromFormat(const char *fmt,
                                       va_list argp) const; //!< Helper method
                                                            //!for generating a
                                                            //!std::string
                                                            //!instance from
                                                            //!printf-like
...
#define MRPT_LOG_DEBUG(_STRING)                                                \
  INTERNAL_MRPT_LOG(::mrpt::utils::LVL_DEBUG, _STRING)
#define MRPT_LOG_INFO(_STRING)                                                 \
  INTERNAL_MRPT_LOG(::mrpt::utils::LVL_INFO, _STRING)
#define MRPT_LOG_WARN(_STRING)                                                 \
  INTERNAL_MRPT_LOG(::mrpt::utils::LVL_WARN, _STRING)
#define MRPT_LOG_ERROR(_STRING)                                                \
  INTERNAL_MRPT_LOG(::mrpt::utils::LVL_ERROR, _STRING)

/** Use: `MRPT_LOG_ONCE_DEBUG("once-only message");`  */
#define MRPT_LOG_ONCE_DEBUG(_STRING)                                           \
  INTERNAL_MRPT_LOG_ONCE(::mrpt::utils::LVL_DEBUG, _STRING)
#define MRPT_LOG_ONCE_INFO(_STRING)                                            \
  INTERNAL_MRPT_LOG_ONCE(::mrpt::utils::LVL_INFO, _STRING)
#define MRPT_LOG_ONCE_WARN(_STRING)                                            \
  INTERNAL_MRPT_LOG_ONCE(::mrpt::utils::LVL_WARN, _STRING)
#define MRPT_LOG_ONCE_ERROR(_STRING)                                           \
  INTERNAL_MRPT_LOG_ONCE(::mrpt::utils::LVL_ERROR, _STRING)
...
#define MRPT_LOG_DEBUG_STREAM(__CONTENTS)                                      \
  INTERNAL_MRPT_LOG_STREAM(::mrpt::utils::LVL_DEBUG, __CONTENTS)
#define MRPT_LOG_INFO_STREAM(__CONTENTS)                                       \
  INTERNAL_MRPT_LOG_STREAM(::mrpt::utils::LVL_INFO, __CONTENTS)
#define MRPT_LOG_WARN_STREAM(__CONTENTS)                                       \
  INTERNAL_MRPT_LOG_STREAM(::mrpt::utils::LVL_WARN, __CONTENTS)
#define MRPT_LOG_ERROR_STREAM(__CONTENTS)                                      \
  INTERNAL_MRPT_LOG_STREAM(::mrpt::utils::LVL_ERROR, __CONTENTS)

/** Usage: `MRPT_LOG_THROTTLE_DEBUG_STREAM(5.0, "Var=" << value << " foo=" <<
 * foo_var);` */
#define MRPT_LOG_THROTTLE_DEBUG_STREAM(_PERIOD_SECONDS, __CONTENTS)            \
  INTERNAL_MRPT_LOG_THROTTLE_STREAM(::mrpt::utils::LVL_DEBUG, _PERIOD_SECONDS, \
                                    __CONTENTS)
#define MRPT_LOG_THROTTLE_INFO_STREAM(_PERIOD_SECONDS, __CONTENTS)             \
  INTERNAL_MRPT_LOG_THROTTLE_STREAM(::mrpt::utils::LVL_INFO, _PERIOD_SECONDS,  \
                                    __CONTENTS)
#define MRPT_LOG_THROTTLE_WARN_STREAM(_PERIOD_SECONDS, __CONTENTS)             \
  INTERNAL_MRPT_LOG_THROTTLE_STREAM(::mrpt::utils::LVL_WARN, _PERIOD_SECONDS,  \
                                    __CONTENTS)
#define MRPT_LOG_THROTTLE_ERROR_STREAM(_PERIOD_SECONDS, __CONTENTS)            \
  INTERNAL_MRPT_LOG_THROTTLE_STREAM(::mrpt::utils::LVL_ERROR, _PERIOD_SECONDS, \
                                    __CONTENTS)

/** Usage: `MRPT_LOG_THROTTLE_DEBUG_FMT(5.0, "i=%u", i);` */
#define MRPT_LOG_THROTTLE_DEBUG_FMT(_PERIOD_SECONDS, _FMT_STRING, ...)         \
  INTERNAL_MRPT_LOG_THROTTLE_FMT(::mrpt::utils::LVL_DEBUG, _PERIOD_SECONDS,    \
                                 _FMT_STRING, __VA_ARGS__)
#define MRPT_LOG_THROTTLE_INFO_FMT(_PERIOD_SECONDS, _FMT_STRING, ...)          \
  INTERNAL_MRPT_LOG_THROTTLE_FMT(::mrpt::utils::LVL_INFO, _PERIOD_SECONDS,     \
                                 _FMT_STRING, __VA_ARGS__)
#define MRPT_LOG_THROTTLE_WARN_FMT(_PERIOD_SECONDS, _FMT_STRING, ...)          \
  INTERNAL_MRPT_LOG_THROTTLE_FMT(::mrpt::utils::LVL_WARN, _PERIOD_SECONDS,     \
                                 _FMT_STRING, __VA_ARGS__)
#define MRPT_LOG_THROTTLE_ERROR_FMT(_PERIOD_SECONDS, _FMT_STRING, ...)         \
  INTERNAL_MRPT_LOG_THROTTLE_FMT(::mrpt::utils::LVL_ERROR, _PERIOD_SECONDS,    \
                                 _FMT_STRING, __VA_ARGS__)
...
#define MRPT_UNSCOPED_LOGGER_START                                             \
  do {                                                                         \
    struct dummy_logger_ : public mrpt::utils::COutputLogger {                 \
      dummy_logger_() : mrpt::utils::COutputLogger("MRPT_log") {               \
...
namespace utils {
template <> struct TEnumTypeFiller<mrpt::utils::VerbosityLevel> {
