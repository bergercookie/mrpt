...
#define INTERNAL_MRPT_LOG(_LVL,_STRING) \
...
/** For calling any `MRPT_LOG_*()` macro from outside of an object inherited from COutputLogger.
  * Debug level is `DEBUG` if build with `_DEBUG` preprocessor flag, `INFO` otherwise.
  * Use:
  * \code
  *  MRPT_UNSCOPED_LOGGER_START;
  *  MRPT_LOG_WARN("blah");
  ...
  #define MRPT_UNSCOPED_LOGGER_START  \
	  do { \
		  struct dummy_logger_ : public mrpt::utils::COutputLogger { \
			  ...
			  void usercode() { \
				  do {} while (0) \
				  // Here comes the user code, which is run in the ctor, and will call the object log methods.
