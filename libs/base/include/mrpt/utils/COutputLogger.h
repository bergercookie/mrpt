...
class BASE_IMPEXP COutputLogger
{
   public:
	 ...
	 * printing
	 * the current message VerbosityLevel along with the actual content */
	 ...
	 * messages
	 * to be dumped to the output automatically. */
	...
	struct BASE_IMPEXP TCallbackEntry
	{
		...
		bool operator<(
			const mrpt::utils::COutputLogger::TCallbackEntry& e2) const
		{
			return func < e2.func;
		}
		bool operator==(
			const mrpt::utils::COutputLogger::TCallbackEntry& c2) const
		{
   ...
   private:
	...
	struct BASE_IMPEXP TMsg
	{
		 ...
		 */
		TMsg(
			const mrpt::utils::VerbosityLevel level, const std::string& msg,
...
#define INTERNAL_MRPT_LOG_STREAM(_LVL, __CONTENTS)                 \
	...
	{                                                              \
		if (this->isLoggingLevelVisible(_LVL))                     \
		{                                                          \
			::mrpt::utils::COutputLoggerStreamWrapper(_LVL, *this) \
				<< __CONTENTS;                                     \
		}                                                          \
	} while (0)
...
#define MRPT_LOG_THROTTLE_DEBUG(_PERIOD_SECONDS, _STRING) \
	INTERNAL_MRPT_LOG_THROTTLE(                           \
		::mrpt::utils::LVL_DEBUG, _PERIOD_SECONDS, _STRING)
#define MRPT_LOG_THROTTLE_INFO(_PERIOD_SECONDS, _STRING) \
	INTERNAL_MRPT_LOG_THROTTLE(                          \
		::mrpt::utils::LVL_INFO, _PERIOD_SECONDS, _STRING)
#define MRPT_LOG_THROTTLE_WARN(_PERIOD_SECONDS, _STRING) \
	INTERNAL_MRPT_LOG_THROTTLE(                          \
		::mrpt::utils::LVL_WARN, _PERIOD_SECONDS, _STRING)
#define MRPT_LOG_THROTTLE_ERROR(_PERIOD_SECONDS, _STRING) \
	INTERNAL_MRPT_LOG_THROTTLE(                           \
		::mrpt::utils::LVL_ERROR, _PERIOD_SECONDS, _STRING)
...
#define MRPT_LOG_THROTTLE_DEBUG_FMT(_PERIOD_SECONDS, _FMT_STRING, ...) \
	INTERNAL_MRPT_LOG_THROTTLE_FMT(                                    \
		::mrpt::utils::LVL_DEBUG, _PERIOD_SECONDS, _FMT_STRING, __VA_ARGS__)
#define MRPT_LOG_THROTTLE_INFO_FMT(_PERIOD_SECONDS, _FMT_STRING, ...) \
	INTERNAL_MRPT_LOG_THROTTLE_FMT(                                   \
		::mrpt::utils::LVL_INFO, _PERIOD_SECONDS, _FMT_STRING, __VA_ARGS__)
#define MRPT_LOG_THROTTLE_WARN_FMT(_PERIOD_SECONDS, _FMT_STRING, ...) \
	INTERNAL_MRPT_LOG_THROTTLE_FMT(                                   \
		::mrpt::utils::LVL_WARN, _PERIOD_SECONDS, _FMT_STRING, __VA_ARGS__)
#define MRPT_LOG_THROTTLE_ERROR_FMT(_PERIOD_SECONDS, _FMT_STRING, ...) \
	INTERNAL_MRPT_LOG_THROTTLE_FMT(                                    \
		::mrpt::utils::LVL_ERROR, _PERIOD_SECONDS, _FMT_STRING, __VA_ARGS__)
