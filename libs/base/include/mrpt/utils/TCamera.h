...
public:
  ...
  mrpt::math::CMatrixDouble33 intrinsicParams; //!< Matrix of intrinsic
                                               //!parameters (containing the
                                               //!focal length and principal
                                               //!point coordinates)
  mrpt::math::CArrayDouble<5> dist; //!< [k1 k2 t1 t2 k3] -> k_i: parameters of
                                    //!radial distortion, t_i: parameters of
                                    //!tangential distortion (default=0)
  double focalLengthMeters; //!< The focal length of the camera, in meters (can
                            //!be used among 'intrinsicParams' to determine the
                            //!pixel size).
  ...
  /**  Save as a config block:
    ...
    *  \endcode
    */
  void saveToConfigFile(const std::string &section,
   ...
   * saveToConfigFile(), that is:
    ...
    */
  void loadFromConfigFile(const std::string &section,
                          const mrpt::utils::CConfigFileBase &cfg);
  /** overload This signature is consistent with the rest of MRPT APIs */
  inline void loadFromConfigFile(const mrpt::utils::CConfigFileBase &cfg,
   ...
   * values of focal length and principal point coordinates (in pixels)
    */
  inline void setIntrinsicParamsFromValues(double fx, double fy, double cx,
                                           double cy) {
  ...
  inline void
  getDistortionParamsVector(mrpt::math::CMatrixDouble15 &distParVector) const {
  ...
  void
  setDistortionParamsVector(const mrpt::math::CMatrixDouble15 &distParVector) {
    for (size_t i = 0; i < 5; i++)
   ...
   * values of the distortion coefficients
    */
  inline void setDistortionParamsFromValues(double k1, double k2, double p1,
                                            double p2, double k3 = 0) {
    dist[0] = k1;
    dist[1] = k2;
    dist[2] = p1;
    dist[3] = p2;
...
bool BASE_IMPEXP operator==(const mrpt::utils::TCamera &a,
