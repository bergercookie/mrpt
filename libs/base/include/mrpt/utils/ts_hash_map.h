...
namespace mrpt {
namespace utils {
template <typename KEY, typename VALUE> struct ts_map_entry {
  bool used;
  KEY first;
  VALUE second;
...
void BASE_IMPEXP reduced_hash(
    const std::string &value,
    uint8_t &hash); //!< hash function used by ts_hash_map. Uses dbj2 method
void BASE_IMPEXP reduced_hash(
    const std::string &value,
    uint16_t &hash); //!< hash function used by ts_hash_map. Uses dbj2 method
void BASE_IMPEXP reduced_hash(
    const std::string &value,
    uint32_t &hash); //!< hash function used by ts_hash_map. Uses dbj2 method
void BASE_IMPEXP reduced_hash(
    const std::string &value,
    uint64_t &hash); //!< hash function used by ts_hash_map. Uses dbj2 method
...
public:
  ...
  public:
    ...
    const_iterator(const VECTOR_T &vec, const self_t &parent, int idx_outer,
                   int idx_inner)
        : m_vec(const_cast<VECTOR_T *>(&vec)),
          m_parent(const_cast<self_t *>(&parent)), m_idx_outer(idx_outer),
    ...
    inline const_iterator operator++(int) { /* Post: it++ */
      const_iterator aux = *this;
      ++(*this);
      return aux;
    }
    inline const_iterator &operator++() { /* pre: ++it */
      incr();
      return *this;
    }
  ...
  protected:
    ...
    void incr() {
      // This loop ends with the first used entry in the nested arrays, or an
  ...
  public:
    ...
    inline iterator operator++(int) { /* Post: it++ */
      iterator aux = *this;
      ++(*this);
      return aux;
    }
    inline iterator &operator++() { /* pre: ++it */
      const_iterator::incr();
      return *this;
    }
  };
...
public:
  ...
  VALUE &operator[](const KEY &key) {
    typename mrpt::utils::uint_select_by_bytecount<NUM_BYTES_HASH_TABLE>::type
        hash;
