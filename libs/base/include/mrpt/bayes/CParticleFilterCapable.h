...
namespace bayes
{
/** An invalid log-likelihood value, used to signal non-initialized likelihood
 * variables.
 */
...
class BASE_IMPEXP CParticleFilterCapable
{
   ...
   */
	 ...
	 * an internal array (m_fastDrawAuxiliary.alreadyDrawnIndexes) with the
	 *random
	 * indexes generated according to the selected resample scheme in
	 * TParticleFilterOptions. Those indexes are read sequentially by subsequent
	 * calls to fastDrawSample.
	 *		- <b>DYNAMIC SAMPLE SIZE=YES</b>: Then:
	 *			- If TParticleFilterOptions.resamplingMethod = prMultinomial,
	 * the internal buffers will be filled out (m_fastDrawAuxiliary.CDF,
	 * CDF_indexes & PDF) and then fastDrawSample can be called an arbitrary
	 * number of times to generate random indexes.
	 ...
	 * andshould
	 * not be invoked manually normally. To just obtaining the sequence of
	 * resampled indexes from a sequence of weights, use "resample"
	 *
	 ...
	 * output samples is the same than the input population.  This generic
	 * method
	 * just computes these indexes, to actually perform a resampling in a
	 * particle
	 * filter object, call performResampling \param[in] out_particle_count The
	 * desired number of output particles after resampling; 0 means don't modify
	 * the current number.
	 *
