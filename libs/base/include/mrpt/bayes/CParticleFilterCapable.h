...
public:
  ...
  /** A callback function type for evaluating the probability of m_particles of
   * being selected, used in "fastDrawSample".
  ...
  /** Prepares data structures for calling fastDrawSample method next.
    ...
    *an internal array (m_fastDrawAuxiliary.alreadyDrawnIndexes) with
    *			the random indexes generated according to the selected resample
    *scheme in TParticleFilterOptions. Those indexes are
    *			read sequentially by subsequent calls to fastDrawSample.
    *		- <b>DYNAMIC SAMPLE SIZE=YES</b>: Then:
    *			- If TParticleFilterOptions.resamplingMethod = prMultinomial, the
    *internal buffers will be filled out (m_fastDrawAuxiliary.CDF, CDF_indexes &
    *PDF) and
    *				then fastDrawSample can be called an arbitrary number of times
    *to generate random indexes.
    *			- For the rest of resampling algorithms, an exception will be
    *raised since they are not appropriate for a dynamic (unknown in advance)
    *number of particles.
    *
    * The function pointed by "partEvaluator" should take into account the
    *particle filter algorithm selected in "m_PFAlgorithm".
    * If called without arguments (defaultEvaluator), the default behavior is to
    *draw samples with a probability proportional to their current weights.
    *  The action and the observation are declared as "void*" for a greater
    *flexibility.
    *  For a more detailed information see the <a
    *href="http://www.mrpt.org/Particle_Filters" >Particle Filter tutorial</a>.
  ...
  /** Draws a random sample from the particle filter, in such a way that each
    ...
    *in the range [0,M-1]
    *		- You do not need to call "normalizeWeights" before calling
    *this.
    * \sa prepareFastDrawSample
    */
  ...
  /**  Performs the substitution for internal use of resample in particle filter
   ...
   * \param out_max_log_w If provided, will return with the maximum log_w before
   * normalizing, such as new_weights = old_weights - max_log_w.
   ...
   * "performSubstitution" to actually perform the particle replacement.
    * This method is called automatically by CParticleFilter::execute, andshould
   * not be invoked manually normally.
    * To just obtaining the sequence of resampled indexes from a sequence of
   * weights, use "resample"
    * \param[in] out_particle_count The desired number of output particles after
   * resampling; 0 means don't modify the current number.
    * \sa resample
    */
   ...
   * resampling a given set of particles, given their logarithmic weights, and a
   * resampling method.
   ...
   * resampling; 0 means don't modify the current number.
    ...
    */
  static void
  computeResampling(CParticleFilter::TParticleResamplingAlgorithm method,
   ...
   * weights from log-weights.
    * \sa performResampling
    */
  static void log2linearWeights(const std::vector<double> &in_logWeights,
                                std::vector<double> &out_linWeights);

protected:
  /** Performs the particle filter prediction/update stages for the algorithm
   * "pfStandardProposal" (if not implemented in heritated class, it will raise
  ...
  virtual void prediction_and_update_pfAuxiliaryPFStandard(
      ...
      const bayes::CParticleFilter::TParticleFilterOptions &PF_options);
  /** Performs the particle filter prediction/update stages for the algorithm
   * "pfOptimalProposal" (if not implemented in heritated class, it will raise a
  ...
  struct BASE_IMPEXP TFastDrawAuxVars {
    TFastDrawAuxVars()
        : CDF(), CDF_indexes(), PDF(), alreadyDrawnIndexes(),
