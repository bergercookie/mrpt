   ...
   public:
		...
		: public mrpt::utils::CLoadableOptions
	{
	   public:
		 ...
		 * should
		 * perform adative sample size (default=false). */
		 ...
		 * model
		 * to affect the variance of the particle weights, eg
		 ...
		 * a
		 * max_likelihood (from the a-priori estimate) below the maximum from
		 * all
		 * the samples - max_loglikelihood_dyn_range, then the particle is
		 * directly
		 * discarded.
		 ...
		 * for
		 * the "pfAuxiliaryPFOptimal" method, i.e. through a monte carlo
		 * simulation.
		 ...
		 * perform
		 * rejection sampling, but just the most-likely (ML) particle found in
		 * the
		 * preliminary weight-determination stage. */
