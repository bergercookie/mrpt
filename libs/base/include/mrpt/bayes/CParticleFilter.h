...
public:
  ...
  /** Defines the different resampling algorithms.
    ...
    *(draws M random uniform numbers)
  ...
  public:
    ...
    bool adaptiveSampleSize; //!< A flag that indicates whether the
                             //!CParticleFilterCapable object should perform
                             //!adative sample size (default=false).
    double BETA; //!< The resampling of particles will be performed when ESS (in
                 //!range [0,1]) < BETA (default is 0.5)
    unsigned int sampleSize; //!< The initial number of particles in the filter
                             //!(it can change only if adaptiveSampleSize=true)
                             //!(default=1)
     ...
     * samples for searching the maximum likelihood value and also to estimate
     * the "first stage weights" (see papers!) (default=100)
      */
    unsigned int pfAuxFilterOptimal_MaximumSearchSamples;
    double powFactor; //!< An optional step to "smooth" dramatic changes in the
                      //!observation model to affect the variance of the
                      //!particle weights, eg weight*=likelihood^powFactor
                      //!(default=1 = no effects).
    TParticleFilterAlgorithm PF_algorithm; //!< The PF algorithm to use
                                           //!(default=pfStandardProposal) See
                                           //!TParticleFilterAlgorithm for the
                                           //!posibilities.
    TParticleResamplingAlgorithm resamplingMethod; //!< The resampling algorithm
                                                   //!to use
                                                   //!(default=prMultinomial).

    /** Only for PF_algorithm=pfAuxiliaryPFOptimal: If a given particle has a
     * max_likelihood (from the a-priori estimate) below the maximum from all
     * the samples - max_loglikelihood_dyn_range, then the particle is directly
     * discarded.
      *  This is done to assure that the rejection sampling doesn't get stuck in
     * an infinite loop trying to get an acceptable sample.
      *  Default = 15 (in logarithmic likelihood)
      */
    double max_loglikelihood_dyn_range;

    /** Only for PF_algorithm==pfAuxiliaryPFStandard:
      * If false, the APF will predict the first stage weights just at the mean
     * of the prior of the next time step.
      * If true, these weights will be estimated as described in the papers for
     * the "pfAuxiliaryPFOptimal" method, i.e. through a monte carlo simulation.
      *  In that case, "pfAuxFilterOptimal_MaximumSearchSamples" is the number
     * of MC samples used.
      */
    bool pfAuxFilterStandard_FirstStageWeightsMonteCarlo;

    bool pfAuxFilterOptimal_MLE; //!< (Default=false) In the algorithm
...
* filtering algorithm.
   *    The member CParticleFilter::m_options must be set before calling this to
* settle the algorithm parameters.
   *
...
* CActionCollection, or nullptr if there is no action.
* \param observation	A pointer to observations in the form of a
* CSensoryFrame, or nullptr if there is no observation.
* \param stats An output structure for gathering statistics of the particle filter
* execution, or set to nullptr if you do not need it (see
* CParticleFilter::TParticleFilterStats).
   *
   * \sa CParticleFilterCapable, executeOn
   */
  void executeOn(CParticleFilterCapable &obj,
                 const mrpt::obs::CActionCollection *action,
                 const mrpt::obs::CSensoryFrame *observation,
                 TParticleFilterStats *stats = nullptr);

  /** The options to be used in the PF, must be set before executing any step of
   * the particle filter.
    */
