...
struct CParticleFilterDataImpl : public CParticleFilterCapable {
  ...
  double normalizeWeights(double *out_max_log_w = nullptr) override {
    ...
    for (typename particle_list_t::iterator it = derived().m_particles.begin();
  ...
  /** Replaces the old particles by copies determined by the indexes in "indx",
   * performing an efficient copy of the necesary particles only and allowing
   * the number of particles to change.*/
 ...
 * \ingroup mrpt_base_grp
 */
template <class T> class CParticleFilterData {
public:
  typedef T CParticleDataContent; //!< This is the type inside the corresponding
                                  //!CParticleData class
  typedef CProbabilityParticle<T> CParticleData; //!< Use this to refer to each
                                                 //!element in the m_particles
                                                 //!array.
  typedef std::deque<CParticleData> CParticleList; //!< Use this type to refer
  ...
  /** Free the memory of all the particles and reset the array "m_particles" to
   * length zero  */
  void clearParticles() { m_particles.clear(); }

  /** Dumps the sequence of particles and their weights to a stream (requires T
   * implementing CSerializable).
    * \sa readParticlesFromStream
    */
  template <class STREAM> void writeParticlesToStream(STREAM &out) const {
    MRPT_START
  ...
  /** Reads the sequence of particles and their weights from a stream (requires
   * T implementing CSerializable).
    * \sa writeParticlesToStream
    */
  template <class STREAM> void readParticlesFromStream(STREAM &in) {
    MRPT_START
