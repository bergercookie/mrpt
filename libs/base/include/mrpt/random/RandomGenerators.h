...
public:
  ...
  void randomize(
      const uint32_t seed); //!< Initialize the PRNG from the given random seed
  ...
  /** Returns a uniformly distributed pseudo-random number by joining two 32bit
   * numbers from \a drawUniform32bit() */
  uint64_t drawUniform64bit();

  /** You can call this overloaded method with either 32 or 64bit unsigned ints
   * for the sake of general coding. */
  ...
  void drawUniformUnsignedIntRange(T &ret_number, const U min_val,
                                   const V max_val) {
   ...
   * mrpt::math::CMatrixFixedNumeric
    ...
    */
  template <class MAT>
  void drawUniformMatrix(MAT &matrix, const double unif_min = 0,
                         const double unif_max = 1) {
    for (size_t r = 0; r < matrix.getRowCount(); r++)
      for (size_t c = 0; c < matrix.getColCount(); c++)
        matrix.get_unsafe(r, c) =
            static_cast<typename MAT::Scalar>(drawUniform(unif_min, unif_max));
  }
  ...
  /** Fills the given vector with independent, uniformly distributed samples.
    ...
    */
  template <class VEC>
  void drawUniformVector(VEC &v, const double unif_min = 0,
                         const double unif_max = 1) {
    const size_t N = v.size();
    for (size_t c = 0; c < N; c++)
      v[c] = static_cast<typename mrpt::math::ContainerType<VEC>::element_t>(
   ...
   * receive the likelihood of the given sample to have been obtained, that is,
   * the value of the normal pdf at the sample value.
   */
   ...
   * mrpt::math::CMatrixFixedNumeric
    ...
    */
  template <class MAT>
  void drawGaussian1DMatrix(MAT &matrix, const double mean = 0,
                            const double std = 1) {
    for (size_t r = 0; r < matrix.getRowCount(); r++)
      for (size_t c = 0; c < matrix.getColCount(); c++)
        matrix.get_unsafe(r, c) =
            static_cast<typename MAT::Scalar>(drawGaussian1D(mean, std));
  }
  ...
  drawDefinitePositiveMatrix(const size_t dim, const double std_scale = 1.0,
  ...
  /** Fills the given vector with independent, 1D-normally distributed samples.
    ...
    */
  template <class VEC>
  void drawGaussian1DVector(VEC &v, const double mean = 0,
                            const double std = 1) {
    const size_t N = v.size();
    for (size_t c = 0; c < N; c++)
      v[c] = static_cast<typename mrpt::math::ContainerType<VEC>::element_t>(
  ...
  template <typename T>
  void
  drawGaussianMultivariate(std::vector<T> &out_result,
  ...
  void drawGaussianMultivariate(VECTORLIKE &out_result, const COVMATRIX &cov,
                                const VECTORLIKE *mean = nullptr) {
    const size_t N = cov.rows();
    ASSERT_(cov.rows() == cov.cols())
    if (mean)
      ASSERT_EQUAL_(size_t(mean->size()), N)

    // Compute eigenvalues/eigenvectors of cov:
    Eigen::SelfAdjointEigenSolver<typename COVMATRIX::PlainObject> eigensolver(
        cov);
  ...
  void drawGaussianMultivariateMany(
    ...
    if (mean)
      ASSERT_EQUAL_(size_t(mean->size()), size_t(cov.cols()))
 ...
 * mrpt::math::CMatrixFixedNumeric
  ...
  */
template <class MAT>
void matrixRandomUni(MAT &matrix, const double unif_min = 0,
                     const double unif_max = 1) {
  for (size_t r = 0; r < matrix.getRowCount(); r++)
    for (size_t c = 0; c < matrix.getColCount(); c++)
...
/** Fills the given matrix with independent, uniformly distributed samples.
  ...
  */
template <class T>
void vectorRandomUni(std::vector<T> &v_out, const T &unif_min = 0,
                     const T &unif_max = 1) {
  size_t n = v_out.size();
 ...
 * mrpt::math::CMatrixFixedNumeric
  ...
  */
template <class MAT>
void matrixRandomNormal(MAT &matrix, const double mean = 0,
                        const double std = 1) {
  for (size_t r = 0; r < matrix.getRowCount(); r++)
    for (size_t c = 0; c < matrix.getColCount(); c++)
      matrix.get_unsafe(r, c) = static_cast<typename MAT::Scalar>(
...
/** Generates a random vector with independent, normally distributed samples.
  ...
  */
template <class T>
void vectorRandomNormal(std::vector<T> &v_out, const T &mean = 0,
                        const T &std = 1) {
  size_t n = v_out.size();
  for (size_t r = 0; r < n; r++)
 ...
 * vector are in the output but at random positions.
  */
template <class T>
void randomPermutation(const std::vector<T> &in_vector,
...
void randomNormalMultiDimensionalMany(const MATRIXLIKE &cov,
 ...
 */
template <typename T, typename MATRIXLIKE>
void randomNormalMultiDimensional(const MATRIXLIKE &cov,
                                  std::vector<T> &out_result) {
  randomGenerator.drawGaussianMultivariate(out_result, cov);
}
