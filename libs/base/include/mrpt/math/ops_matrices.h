...
inline const typename Eigen::MatrixBase<Derived>::AdjointReturnType
...
inline void
multiply_HCHt(const MAT_H &H, const MAT_C &C, MAT_R &R,
              bool accumResultInOutput) //	bool allow_submatrix_mult)
{
  if (accumResultInOutput)
    R += ((H * C.template selfadjointView<Eigen::Lower>()).eval() * H.adjoint())
             .eval()
...
template <typename MAT_H, typename MAT_C, typename MAT_R>
void multiply_HtCH(const MAT_H &H, const MAT_C &C, MAT_R &R,
                   bool accumResultInOutput) // bool allow_submatrix_mult)
{
  if (accumResultInOutput)
    R += ((H.adjoint() * C.template selfadjointView<Eigen::Lower>()).eval() * H)
             .eval()
             .template selfadjointView<Eigen::Lower>();
  else
    R = ((H.adjoint() * C.template selfadjointView<Eigen::Lower>()).eval() * H)
...
void multiply_A_skew3(const MAT_A &A, const SKEW_3VECTOR &v, MAT_OUT &out) {
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    out.set_unsafe(i, 1,
                   -A.get_unsafe(i, 0) * v[2] + A.get_unsafe(i, 2) * v[0]);
...
void multiply_skew3_A(const SKEW_3VECTOR &v, const MAT_A &A, MAT_OUT &out) {
  ...
  for (size_t i = 0; i < N; i++) {
    out.set_unsafe(0, i,
                   -A.get_unsafe(1, i) * v[2] + A.get_unsafe(2, i) * v[1]);
    out.set_unsafe(1, i, A.get_unsafe(0, i) * v[2] - A.get_unsafe(2, i) * v[0]);
    out.set_unsafe(2, i,
                   -A.get_unsafe(0, i) * v[1] + A.get_unsafe(1, i) * v[0]);
  }
  MRPT_END
}
