 ...
 *			- The type in the template especialization MUST be a class
 *with a proper default constructor.
 *			- Initially all entries are set to nullptr.
 *			- Pointers can be manually asigned, or automatically created
 *through a call to "CMatrixTemplateObjects<T>::allocAllObjects"
 *			- Independently of how pointers are asigned, memory will be
 *free by destroying objects for each non-NULL entry in the matrix. In some
 *special situations, the user can indicate not to free those objects by calling
 ...
 *			- Asignament operator with matrixes will COPY THE POINTERS,
 *thus a copy of objects is not performed.
 *			- WARNING: Objects are not deleted while shrinking the
 *matrix by calling "setSize", thus please call
 *""CMatrixTemplateObjects<T>::freeAllObjects" or manually delete objects before
 *shrinking.
 *
 * \ingroup mrpt_base_grp
 * \sa CMatrixTemplate
 */
template <class T> class CMatrixTemplateObjects : public CMatrixTemplate<T *> {
private:
  bool m_freeObjects;
