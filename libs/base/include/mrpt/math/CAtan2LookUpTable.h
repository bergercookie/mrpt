...
public:
  ...
  CAtan2LookUpTable(double xmin, double xmax, double ymin, double ymax,
                    double resolution) noexcept;
  void resize(double xmin, double xmax, double ymin, double ymax,
              double resolution) noexcept;

  /** Returns the precomputed value for atan2(y,x). \return false if out of grid
   * bounds. */
...
public:
  ...
  CAtan2LookUpTableMultiRes(
      const std::map<double, double>
          &lst_resolutions2extensions) noexcept; //!< See
                                                 //!CAtan2LookUpTableMultiRes
                                                 //!for a discussion of the
                                                 //!parameters
  void
  resize(const std::map<double, double>
             &lst_resolutions2extensions) noexcept; //!< See
                                                    //!CAtan2LookUpTableMultiRes
                                                    //!for a discussion of the
                                                    //!parameters
...
private:
  std::map<double, mrpt::utils::CDynamicGrid<double>> m_grids; //!< Maps from
                                                               //!maximum (X,Y)
                                                               //!coordinates to
                                                               //!LUT for
                                                               //![-L,L]x[-L,L]
