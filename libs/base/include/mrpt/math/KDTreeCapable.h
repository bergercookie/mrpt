...
public:
  ...
  TKDTreeSearchParams
      kdtree_search_params; //!< Parameters to tune the ANN searches
  ...
  /** KD Tree-based search for the closest point (only ONE) to some given 2D
    ...
    * \param out_dist_sqr The square distance between the query and the returned
    *point.
    *
    * \return The index of the closest point in the map array.
    *  \sa kdTreeClosestPoint3D, kdTreeTwoClosestPoint2D
    */
  inline size_t kdTreeClosestPoint2D(float x0, float y0, float &out_x,
                                     float &out_y, float &out_dist_sqr) const {
    MRPT_START
    ...
    m_kdtree2d_data.index->findNeighbors(
        resultSet, &m_kdtree2d_data.query_point[0], nanoflann::SearchParams());
  ...
  /// \overload
  inline size_t kdTreeClosestPoint2D(float x0, float y0,
  ...
  /// \overload
  inline size_t kdTreeClosestPoint2D(const TPoint2D &p0, TPoint2D &pOut,
                                     float &outDistSqr) const {
    float dmy1, dmy2;
    size_t res =
        kdTreeClosestPoint2D(static_cast<float>(p0.x), static_cast<float>(p0.y),
                             dmy1, dmy2, outDistSqr);
    pOut.x = dmy1;
    pOut.y = dmy2;
    return res;
  }
  ...
  inline float kdTreeClosestPoint2DsqrError(const TPoint2D &p0) const {
    return kdTreeClosestPoint2DsqrError(static_cast<float>(p0.x),
  ...
  /** KD Tree-based search for the TWO closest point to some given 2D
    ...
    *returned point.
    * \param out_dist_sqr2 The square distance between the query and the second
    *returned point.
    *
    *  \sa kdTreeClosestPoint2D
    */
  inline void kdTreeTwoClosestPoint2D(float x0, float y0, float &out_x1,
                                      float &out_y1, float &out_x2,
                                      float &out_y2, float &out_dist_sqr1,
  ...
  inline void kdTreeTwoClosestPoint2D(const TPoint2D &p0, TPoint2D &pOut1,
                                      TPoint2D &pOut2, float &outDistSqr1,
                                      float &outDistSqr2) const {
    float dmy1, dmy2, dmy3, dmy4;
    kdTreeTwoClosestPoint2D(p0.x, p0.y, dmy1, dmy2, dmy3, dmy4, outDistSqr1,
                            outDistSqr2);
  ...
  /** KD Tree-based search for the N closest point to some given 2D coordinates.
    ...
    *correspondences.
    * \param out_dist_sqr The vector containing the square distance between the
    *query and the returned points.
    *
  ...
  kdTreeNClosestPoint2D(float x0, float y0, size_t knn,
  ...
  inline std::vector<size_t>
  kdTreeNClosestPoint2D(const TPoint2D &p0, size_t N,
                        std::vector<TPoint2D> &pOut,
                        std::vector<float> &outDistSqr) const {
    std::vector<float> dmy1, dmy2;
    std::vector<size_t> res = kdTreeNClosestPoint2D(static_cast<float>(p0.x),
                                                    static_cast<float>(p0.y), N,
                                                    dmy1, dmy2, outDistSqr);
  ...
  /** KD Tree-based search for the N closest point to some given 2D coordinates
    ...
    *  \sa kdTreeClosestPoint2D
    */
  inline void kdTreeNClosestPoint2DIdx(float x0, float y0, size_t knn,
  ...
  inline void kdTreeNClosestPoint2DIdx(const TPoint2D &p0, size_t N,
                                       std::vector<size_t> &outIdx,
                                       std::vector<float> &outDistSqr) const {
    return kdTreeNClosestPoint2DIdx(static_cast<float>(p0.x),
                                    static_cast<float>(p0.y), N, outIdx,
                                    outDistSqr);
  }
  ...
  /** KD Tree-based search for the closest point (only ONE) to some given 3D
    ...
    *  \sa kdTreeClosestPoint2D
    */
  inline size_t kdTreeClosestPoint3D(float x0, float y0, float z0, float &out_x,
    ...
    m_kdtree3d_data.index->findNeighbors(
        resultSet, &m_kdtree3d_data.query_point[0], nanoflann::SearchParams());
  ...
  /// \overload
  inline size_t kdTreeClosestPoint3D(float x0, float y0, float z0,
                                     float &out_dist_sqr) const {
    MRPT_START
    rebuild_kdTree_3D(); // First: Create the 3D KD-Tree if required
    if (!m_kdtree3d_data.m_num_points)
  ...
  /// \overload
  inline size_t kdTreeClosestPoint3D(const TPoint3D &p0, TPoint3D &pOut,
                                     float &outDistSqr) const {
  ...
  /** KD Tree-based search for the N closest points to some given 3D
    ...
    */
  inline void kdTreeNClosestPoint3D(float x0, float y0, float z0, size_t knn,
  ...
  /** KD Tree-based search for the N closest points to some given 3D
    *coordinates.
                *  This method automatically build the "m_kdtree_data" structure
    *when:
                *		- It is called for the first time
                *		- The map has changed
                *		- The KD-tree was build for 2D.
    ...
    *correspondences.
                * \param out_z The vector containing the Z coordinates of the
    *correspondences.
                * \param out_idx The vector containing the indexes of the
    *correspondences.
                * \param out_dist_sqr The vector containing the square distance
    *between the query and the returned points.
                *
                *  \sa kdTreeNClosestPoint2D
                */
  ...
  inline void kdTreeNClosestPoint3D(const TPoint3D &p0, size_t N,
                                    std::vector<TPoint3D> &pOut,
                                    std::vector<float> &outDistSqr) const {
    std::vector<float> dmy1, dmy2, dmy3;
    kdTreeNClosestPoint3D(static_cast<float>(p0.x), static_cast<float>(p0.y),
                          static_cast<float>(p0.z), N, dmy1, dmy2, dmy3,
                          outDistSqr);
    pOut.resize(dmy1.size());
    for (size_t i = 0; i < dmy1.size(); i++) {
      pOut[i].x = static_cast<double>(dmy1[i]);
  ...
  inline size_t kdTreeRadiusSearch3D(
      const num_t x0, const num_t y0, const num_t z0, const num_t maxRadiusSqr,
    ...
    if (m_kdtree3d_data.m_num_points != 0) {
      ...
      m_kdtree3d_data.index->radiusSearch(
          &xyz[0], maxRadiusSqr, out_indices_dist, nanoflann::SearchParams());
    }
  ...
  /** KD Tree-based search for the N closest point to some given 3D coordinates
    *and returns their indexes.
    *  This method automatically build the "m_kdtree_data" structure when:
    *		- It is called for the first time
    *		- The map has changed
    ...
    */
  inline void kdTreeNClosestPoint3DIdx(float x0, float y0, float z0, size_t knn,
                                       std::vector<size_t> &out_idx,
                                       std::vector<float> &out_dist_sqr) const {
    MRPT_START
    rebuild_kdTree_3D(); // First: Create the 3D KD-Tree if required
  ...
  inline void kdTreeNClosestPoint3DIdx(const TPoint3D &p0, size_t N,
                                       std::vector<size_t> &outIdx,
                                       std::vector<float> &outDistSqr) const {
    kdTreeNClosestPoint3DIdx(static_cast<float>(p0.x), static_cast<float>(p0.y),
...
private:
  ...
  template <int _DIM = -1> struct TKDTreeDataHolder {
    inline TKDTreeDataHolder() {}
    /** Copy constructor: It actually does NOT copy the kd-tree, a new object
     * will be created if required!   */
    inline TKDTreeDataHolder(const TKDTreeDataHolder &) : TKDTreeDataHolder() {}
    /** Copy operator: It actually does NOT copy the kd-tree, a new object will
  ...
  mutable bool
      m_kdtree_is_uptodate; //!< whether the KD tree needs to be rebuilt or not.

  /// Rebuild, if needed the KD-tree for 2D (nDims=2), 3D (nDims=3), ... asking
  /// the child class for the data points.
