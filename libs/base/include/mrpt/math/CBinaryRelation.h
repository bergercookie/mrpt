...
private:
  ...
  typedef typename detail::MatrixWrapper<U, UIsObject>::MatrixType
      MatrixType; //!<Matrix type used to store the actual relation.
public:
  typedef U (*SimpleFunctionByReturnValue)(
      T, T); //!< Simple function type, used to initialize chunks of the matrix.
  typedef U (*FunctionByReturnValue)(const T &, const T &); //!<Function type
                                                            //!which obtains the
                                                            //!relation value by
                                                            //!a return value.
  typedef void (*FunctionByReferencePass)(const T &, const T &,
                                          U &); //!<Function type which obtains
                                                //!the relation value by
                                                //!reference pass.
  typedef typename std::set<T>::const_iterator
      const_iterator; //!<Constant iterator through the set elements.
  typedef typename std::set<T>::const_reverse_iterator
      const_reverse_iterator; //!<Constant reverse iterator through the set
                              //!elements.
  typedef CMatrixRowAccessor<U> AccessorForFirstElement; //!<Accessor type to
                                                         //!every value related
                                                         //!to any element A,
                                                         //!i.e., f(A,x).
  typedef CMatrixColumnAccessor<U> AccessorForSecondElement; //!<Accessor type
                                                             //!to every value
                                                             //!related to any
                                                             //!element B, i.e.,
                                                             //!f(x,B).
  typedef CConstMatrixRowAccessor<U> ConstAccessorForFirstElement; //!<Const
                                                                   //!accessor
                                                                   //!type to
                                                                   //!every
                                                                   //!value
                                                                   ...
                                                                   //!i.e.,
                                                                   //!f(A,x).
  typedef CConstMatrixColumnAccessor<U>
      ConstAccessorForSecondElement; //!<Const accessor type to every value
                                     //!related to any element B, i.e., f(x,B).
private:
  std::set<T> elements; //!<Actual set of elements.
  MatrixType relation;  //!<Matrix storing the relation.

  /**
    * Template used to make the function interface independent from the function
   * type.
    *  (wrapper for the global method - needed to make this compile under GCC).
    */
  template <typename FunctionType>
  inline void applyFunction(FunctionType fun, size_t e1, size_t e2, const T &T1,
                            const T &T2) {
    detail::applyFunction<T, U, UIsObject, FunctionType>(*this, fun, e1, e2, T1,
                                                         T2);
  }
...
public:
  ...
  inline bool setRelationValue(const T &t1, const T &t2, const U &newVal) {
    ...
    if (it1 == e || it2 == e)
      return false;
    setRelationValue(static_cast<size_t>(std::distance(b, it1)),
  ...
  inline U getRelationValue(const T &t1, const T &t2) const {
    ...
    return getRelationValue(static_cast<size_t>(std::distance(b, it1)),
                            static_cast<size_t>(std::distance(b, it2)));
  }
  /**
    * Get a reference to a relation value given the elements, which allows both
   * querying and setting. Throws domain_error if any of the elements is not
    * present.
    */
  ...
  /**
    * Gets an accessor for every value related to an element A given its index,
   * i.e., every f(A,x). This accessor is iterable.
    */
  inline AccessorForFirstElement getRelationFrom(size_t i) {
    return AccessorForFirstElement(relation, i);
  }
  /**
    * Gets a constant accessor for every value related to an element A given its
  ...
  /**
    * Gets an accessor for every value related to an element B given its index,
   * i.e., every f(x,B). This accessor is iterable.
    */
  inline AccessorForSecondElement getRelationTo(size_t i) {
    return AccessorForSecondElement(relation, i);
  }
  /**
    * Gets a constant accessor for every value related to an element B given its
   * index, i.e., every f(x,B). This accessor is fully iterable.
    */
   ...
   * every f(A,x). A domain_error will be thrown if the element is not present.
    */
   ...
   * A, i.e., every f(A,x). A domain_error will be thrown if the element is not
    * present.
    */
   ...
   * every f(x,B). A domain_error will be thrown if the element is not present.
    */
   ...
   * B, i.e., every f(x,B). A domain_error will be thrown if the element is not
    * present.
    */
  ...
  /**
    * Removes an element. Returns false if the element was not present and thus
   * could'nt be eliminated.
    */
  ...
  /**
    * Inserts an element and initializes its relationship values, even if it was
   * already present.
    */
  template <typename FunctionType>
  ...
  size_t insertElements(const std::set<T> &els) {
    if (els.empty())
      return 0;
    // This code is much more complex than it should! Trying, for efficiency, to
    // avoid multiple calls to insertElement makes things a lot harder.
    // It raises the complexity level to N^2, but alleviates it greatly by
    // making a single memory allocation. Multiple calls to insertElement will
    // be
    // faster only if the number of elements in the set is really large.
    std::vector<size_t> added;
    // std::vector<size_t> exist;
    added.reserve(els.size());
    for (typename std::set<T>::const_iterator it = els.begin(); it != els.end();
         ++it) {
      ...
      if (ins.second) {
        ...
        // it2=exist.begin();it2!=exist.end();++it2) if (*it2>=dist) ++(*it2);
  ...
  size_t insertElements(const std::set<T> &els, FunctionType fun) {
    ...
    for (std::set<size_t>::const_iterator it = nPoss.begin(); it != nPoss.end();
         ++it) {
      const T &e1 = operator[](*it);
...
inline void applyFunction(CBinaryRelation<T, U, UIsObject> &o, FunctionType fun,
