...
EIGEN_STRONG_INLINE void set_unsafe(const size_t row, const size_t col,
                                    const Scalar val) {
#ifdef _DEBUG
  derived()(row, col) = val;
#else
 ...
 * methods of matrix classes themselves).
        ...
        * \param fileFormat See TMatrixTextFileFormat. The format of the numbers
 * in the text file.
        * \param appendMRPTHeader Insert this header to the file "% File
 * generated by MRPT. Load with MATLAB with: VAR=load(FILENAME);"
        * \param userHeader Additional text to be written at the head of the
 * file. Typically MALAB comments "% This file blah blah". Final end-of-line is
 * not needed.
...
EIGEN_STRONG_INLINE Scalar maximum() const {
  if (size() == 0)
    throw std::runtime_error("maximum: container is empty");
...
EIGEN_STRONG_INLINE Scalar minimum() const {
  if (size() == 0)
    throw std::runtime_error("minimum: container is empty");
...
/** [VECTORS ONLY] Compute the minimum and maximum of a container at once
  * \sa maximum, minimum
  * \exception std::exception On an empty input vector */
EIGEN_STRONG_INLINE void minimum_maximum(Scalar &out_min, Scalar &out_max,
                                         size_t *minIndex,
...
EIGEN_STRONG_INLINE void setSize(size_t row, size_t col) {
  ...
  ::mrpt::math::detail::TAuxResizer<Eigen::MatrixBase<Derived>,
                                    SizeAtCompileTime>::internal_resize(*this,
                                                                        row,
                                                                        col);
  if (nNewCols > 0)
    derived().block(0, oldCols, row, nNewCols).setZero();
  if (nNewRows > 0)
    derived().block(oldRows, 0, nNewRows, col).setZero();
}
...
void largestEigenvector(OUTVECT &x, Scalar resolution = Scalar(0.01),
                        size_t maxIterations = 6, int *out_Iterations = nullptr,
  ...
  if (out_Iterations)
    *out_Iterations = static_cast<int>(iter);
  if (out_estimatedResolution)
    *out_estimatedResolution = dif;
}
...
EIGEN_STRONG_INLINE double mean() const {
  if (size() == 0)
    throw std::runtime_error("mean: Empty container.");
 ...
 * or N for unbiased_variance=true or false, respectively.
  */
template <class VEC>
void meanAndStd(VEC &outMeanVector, VEC &outStdVector,
                const bool unbiased_variance = true) const {
  const size_t N = rows();
  if (N == 0)
    throw std::runtime_error("meanAndStd: Empty container.");
 ...
 * or N for unbiased_variance=true or false, respectively.
  */
void meanAndStdAll(double &outMean, double &outStd,
                   const bool unbiased_variance = true) const {
  const size_t N = size();
  if (N == 0)
    throw std::runtime_error("meanAndStdAll: Empty container.");
  const double N_ = unbiased_variance ? (N > 1 ? 1.0 / (N - 1) : 1.0) : 1.0 / N;
...
template <typename MAT>
EIGEN_STRONG_INLINE void insertMatrixTranspose(size_t r, size_t c,
                                               const MAT &m) {
...
removeColumns(const std::vector<size_t> &idxsToRemove) {
...
template <class MATRIX> EIGEN_STRONG_INLINE void inv(MATRIX &outMat) const {
...
EIGEN_STRONG_INLINE void multiply_HCHt(const MAT_C &C, MAT_R &R,
                                       bool accumResultInOutput = false) const
/*!< R = this * C * this<sup>T</sup>  */ {
...
template <typename MAT_C, typename MAT_R>
EIGEN_STRONG_INLINE void multiply_HtCH(const MAT_C &C, MAT_R &R,
                                       bool accumResultInOutput = false) const
/*!< R = this<sup>T</sup> * C * this  */ {
  if (accumResultInOutput)
...
EIGEN_STRONG_INLINE void multiply_subMatrix(const MAT_A &A, MAT_OUT &outResult,
                                            const size_t A_cols_offset,
                                            const size_t A_rows_offset,
                                            const size_t A_col_count) const {
  outResult =
      derived() *
...
void multiply_ABC(const MAT_A &A, const MAT_B &B,
                  const MAT_C &C) /*!< this = A*B*C  */ {
  *this = A * B * C;
}

template <class MAT_A, class MAT_B, class MAT_C>
void multiply_ABCt(const MAT_A &A, const MAT_B &B,
                   const MAT_C &C) /*!< this = A*B*(C<sup>T</sup>) */ {
  *this = A * B * C.adjoint();
}

template <class MAT_A, class MAT_B, class MAT_C>
void multiply_AtBC(const MAT_A &A, const MAT_B &B,
...
EIGEN_STRONG_INLINE void
multiply_ABt(const MAT_A &A, const MAT_B &B) /*!< this = A * B<sup>T</sup> */ {
  *this = A * B.adjoint();
}

template <class MAT_A>
EIGEN_STRONG_INLINE void
multiply_AAt(const MAT_A &A) /*!< this = A * A<sup>T</sup> */ {
  *this = A * A.adjoint();
}

template <class MAT_A>
EIGEN_STRONG_INLINE void
multiply_AtA(const MAT_A &A) /*!< this = A<sup>T</sup> * A */ {
  *this = A.adjoint() * A;
}
...
//! \overload
template <typename T>
inline void extractRow(size_t nRow, std::vector<T> &v,
                       size_t startingCol = 0) const {
  const size_t N = cols() - startingCol;
  v.resize(N);
  for (size_t i = 0; i < N; i++)
    v[i] = (*this)(nRow, startingCol + i);
}
/** Extract one row from the matrix into a column vector */
template <class VECTOR>
EIGEN_STRONG_INLINE void extractRowAsCol(size_t nRow, VECTOR &v,
                                         size_t startingCol = 0) const {
...
template <class VECTOR>
EIGEN_STRONG_INLINE void extractCol(size_t nCol, VECTOR &v,
                                    size_t startingRow = 0) const {
  v = derived().block(startingRow, nCol, rows() - startingRow, 1);
}
//! \overload
template <typename T>
inline void extractCol(size_t nCol, std::vector<T> &v,
                       size_t startingRow = 0) const {
  const size_t N = rows() - startingRow;
  v.resize(N);
  for (size_t i = 0; i < N; i++)
    v[i] = (*this)(startingRow + i, nCol);
}

template <class MATRIX>
EIGEN_STRONG_INLINE void extractMatrix(const size_t firstRow,
                                       const size_t firstCol, MATRIX &m) const {
  m = derived().block(firstRow, firstCol, m.rows(), m.cols());
}
template <class MATRIX>
EIGEN_STRONG_INLINE void
extractMatrix(const size_t firstRow, const size_t firstCol, const size_t nRows,
...
template <class MATRIX>
EIGEN_STRONG_INLINE void
extractSubmatrix(const size_t row_first, const size_t row_last,
                 const size_t col_first, const size_t col_last,
                 MATRIX &out) const {
  out.resize(row_last - row_first + 1, col_last - col_first + 1);
  out = derived().block(row_first, col_first, row_last - row_first + 1,
...
#endif
      out.block(dst_row_blk * block_size, dst_col_blk * block_size, block_size,
                block_size) =
          derived().block(block_indices[dst_row_blk] * block_size,
