...
#define NCOMPLEX // In MRPT we don't need complex numbers, so avoid the annoying
                 // warning: 'cs_ci_house' has C-linkage specified, but returns
                 // UDT 'std::complex<double>' which is incompatible with C
#include "cs.h"
#else
...
private:
  ...
  template <class MATRIX> void construct_from_mrpt_mat(const MATRIX &C) {
    std::vector<int> row_list,
        col_list; // Use "int" for convenience so we can do a memcpy below...
    ...
    ::memcpy(sparse_matrix.i, &row_list[0],
             sizeof(row_list[0]) * row_list.size());
    ::memcpy(sparse_matrix.p, &col_list[0],
             sizeof(col_list[0]) * col_list.size());
    ::memcpy(sparse_matrix.x, &content_list[0],
...
public:
   ...
   * elements.
                *  This constructor takes all the non-zero entries in "data" and
   * builds a column-compressed sparse representation.
                */
  template <typename T>
  inline CSparseMatrix(const CSparseMatrixTemplate<T> &data) {
    ASSERTMSG_(!data.empty(),
    ...
    // 2) Put data in:
    for (typename CSparseMatrixTemplate<T>::const_iterator it = data.begin();
         it != data.end(); ++it)
  ...
  void multiply_AB(const CSparseMatrix &A,
                   const CSparseMatrix &B); //!< this = A*B
  void
  multiply_Ab(const mrpt::math::CVectorDouble &b,
              mrpt::math::CVectorDouble &out_res) const; //!< out_res = this * b
  ...
  inline mrpt::math::CVectorDouble
  operator*(const mrpt::math::CVectorDouble &other) const {
  ...
  /** ONLY for TRIPLET matrices: insert a new non-zero entry in the matrix.
    ...
    *  The size of the matrix will be automatically extended if the indices are
   * out of the current limits.
    * \sa isTriplet, compressFromTriplet
    */
  void insert_entry(const size_t row, const size_t col, const double val);

  /** This was an optimized version, but is now equivalent to insert_entry() due
   * to the need to be compatible with unmodified CSparse system libraries. */
  inline void insert_entry_fast(const size_t row, const size_t col,
                                const double val) {
  ...
  inline void insert_submatrix(const size_t row, const size_t col,
                               const MATRIX &M) {
    if (!isTriplet())
      THROW_EXCEPTION("insert_entry() is only available for sparse matrix in "
                      "'triplet' format.")
...
*      m=D(1,1); n=D(1,2); nzmax=D(1,3);
  ...
  private:
    ...
    const CSparseMatrix *m_originalSM; //!< A const reference to the original
                                       //!matrix used to build this
                                       //!decomposition.

  public:
    /** Constructor from a square definite-positive sparse matrix A, which can
     * be use to solve Ax=b
    ...
    template <class VECTOR> inline VECTOR backsub(const VECTOR &b) const {
      VECTOR res;
      backsub(b, res);
      return res;
    }
     ...
     * input, square definite-positive sparse matrix.
      *  NOTE: This new matrix MUST HAVE exactly the same sparse structure than
     * the original one.
      */
    void update(const CSparseMatrix &new_SM);
  };
  static_assert(!std::is_copy_constructible<CholeskyDecomp>::value &&
