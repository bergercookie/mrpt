 ...
 *		- This class includes range checks with ASSERT_() if compiling
 *with "_DEBUG" or "MRPT_ALWAYS_CHECKS_DEBUG_MATRICES=1".
 *		- Please DO NOT use as template class type any other class. It can
 *be safely used the following types:
 *			- Elemental types (int,char,float,doble,...)
 *			- Data struct (Not classes!)
 *			- Any kind of pointers (user is responsible for allocating
 *and freeing the memory addressed by pointers).
 *
  ...
  void realloc(size_t row, size_t col, bool newElementsToZero = false) {
    if (row != m_Rows || col != m_Cols || m_Val == nullptr) {
      ...
      if (!row) {
        ...
        m_Val = static_cast<T **>(
            mrpt::system::os::aligned_realloc(m_Val, sizeof(T *) * row, 16));
      ...
      for (r = 0; r < row; r++) {
        if (r < m_Rows) {
          ...
          m_Val[r] = static_cast<T *>(
              mrpt::system::os::aligned_realloc(m_Val[r], row_size, 16));
          ...
          m_Val[r] =
              static_cast<T *>(mrpt::system::os::aligned_calloc(row_size, 16));
        }
      }
      // Done!
      m_Rows = row;
...
public:
  /**
    * Checks whether the rows [r-N,r+N] and the columns [c-N,c+N] are present in
   * the matrix.
    */
  template <size_t N> inline void ASSERT_ENOUGHROOM(size_t r, size_t c) const {
    ...
    ASSERT_((r >= N) && (r + N < getRowCount()) && (c >= N) &&
            (c + N < getColCount()));
#endif
  }
  ...
  */
  CMatrixTemplate(const CMatrixTemplate &m, const size_t cropRowCount,
  ...
  CMatrixTemplate(size_t row, size_t col, V (&theArray)[N])
    ...
    if (m_Rows * m_Cols != N)
      THROW_EXCEPTION(format(
          "Mismatch between matrix size %lu x %lu and array of length %lu",
          static_cast<long unsigned>(m_Rows),
          static_cast<long unsigned>(m_Cols), static_cast<long unsigned>(N)))
    size_t idx = 0;
    for (size_t i = 0; i < m_Rows; i++)
      for (size_t j = 0; j < m_Cols; j++)
   ...
   * std::list,...) with the initial values. The vector length must match cols x
   * row.
  */
  template <typename V>
  ...
  /** Assignment operator for initializing from a C array (The matrix must be
    ...
    */
  template <typename V, size_t N> CMatrixTemplate &operator=(V (&theArray)[N]) {
    MRPT_COMPILE_TIME_ASSERT(N != 0)
    if (m_Rows * m_Cols != N) {
      THROW_EXCEPTION(format(
          "Mismatch between matrix size %lu x %lu and array of length %lu",
...
#if defined(_DEBUG) || (MRPT_ALWAYS_CHECKS_DEBUG_MATRICES)
      if (ith >= m_Cols)
        THROW_EXCEPTION_FMT("Index %u out of range!",
...
// A columns matrix:
...
#ifdef _DEBUG
    if (row >= m_Rows)
      THROW_EXCEPTION(format("Row index %lu out of range. Matrix is %lux%lu",
  ...
  /** Subscript operator to get a submatrix
    */
  inline CMatrixTemplate<T> operator()(const size_t row1, const size_t row2,
                                       const size_t col1,
  ...
  /** Get a submatrix, given its bounds
    ...
    */
  void extractSubmatrix(const size_t row1, const size_t row2, const size_t col1,
  ...
  /** Gets a series of contiguous rows.
          ...
          * \sa extractColumns
          */
  inline void extractRows(size_t firstRow, size_t lastRow,
  ...
  /** Gets a series of contiguous columns.
          ...
          * \sa extractRows
          */
  inline void extractColumns(size_t firstCol, size_t lastCol,
                             CMatrixTemplate<T> &out) const {
  ...
  /** Gets a given column to a vector (without modifying the matrix)
          * \exception std::exception On index out of bounds
          */
  void extractCol(size_t nCol, CMatrixTemplate<T> &out,
                  int startingRow = 0) const {
    size_t i, n;
#if defined(_DEBUG) || (MRPT_ALWAYS_CHECKS_DEBUG_MATRICES)
    if (nCol >= m_Cols)
      THROW_EXCEPTION("extractCol: Column index out of bounds");
  ...
  /** Appends a new column to the matrix from a vector.
          * The length of the vector must match the number of rows of the
   * matrix, unless it is (0,0).
