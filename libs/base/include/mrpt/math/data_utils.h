...
typename MAT::Scalar mahalanobisDistance2(const VECTORLIKE1 &X,
 ...
 * covariance *inverse* COV_inv
  ...
  */
template <class VECTORLIKE1, class VECTORLIKE2, class MAT>
inline typename VECTORLIKE1::Scalar mahalanobisDistance(const VECTORLIKE1 &X,
                                                        const VECTORLIKE2 &MU,
                                                        const MAT &COV) {
...
typename MAT1::Scalar mahalanobisDistance2(const VECTORLIKE &mean_diffs,
                                           const MAT1 &COV1, const MAT2 &COV2,
 ...
 * and the vector with the difference of their means.
          *  \f[ d = \sqrt{ \Delta_\mu^\top (\Sigma_1 + \Sigma_2 - 2 \Sigma_12
 * )^{-1} \Delta_\mu } \f]
          */
template <class VECTORLIKE, class MAT1, class MAT2, class MAT3>
inline typename VECTORLIKE::Scalar
mahalanobisDistance(const VECTORLIKE &mean_diffs, const MAT1 &COV1,
                    const MAT2 &COV2, const MAT3 &CROSS_COV12) {
 ...
 * (\Sigma_1 + \Sigma_2 - 2 \Sigma_12)^{-1} \Delta_\mu)  \f]
  */
template <typename T>
T productIntegralTwoGaussians(const std::vector<T> &mean_diffs,
...
void mahalanobisDistance2AndLogPDF(const VECLIKE &diff_mean,
                                   const MATRIXLIKE &cov, T &maha2_out,
                                   T &log_pdf_out) {
  MRPT_START
  ASSERTDEB_(cov.isSquare())
 ...
 * (given by its difference wrt the mean) and a Gaussian.
  ...
  */
template <typename T, class VECLIKE, class MATRIXLIKE>
inline void mahalanobisDistance2AndPDF(const VECLIKE &diff_mean,
                                       const MATRIXLIKE &cov, T &maha2_out,
                                       T &pdf_out) {
...
* etc...
* \param weights_mean If !=nullptr, it must point to a vector of size()==number of
* elements, with normalized weights to take into account for the mean.
* \param weights_cov If !=nullptr, it must point to a vector of size()==number of
* elements, with normalized weights to take into account for the covariance.
...
inline void
covariancesAndMeanWeighted( // Done inline to speed-up the special case expanded
                            // in covariancesAndMean() below.
  ...
  for (size_t i = 0; i < DIM; i++) {
    T accum = 0;
    if (!elem_do_wrap2pi ||
        !elem_do_wrap2pi[i]) { // i'th dimension is a "normal", real number:
      if (weights_mean) {
    ...
    } else { // i'th dimension is a circle in [-pi,pi]: we need a little trick
             // here:
      ...
      for (size_t j = 0; j < nElms; j++) {
        ...
        if (fabs(ang) > 0.5 * M_PI) { // LEFT HALF: 0,2pi
          if (ang < 0)
            ang = (M_2PI + ang);
          accum_L += ang * w;
          Waccum_L += w;
        } else { // RIGHT HALF: -pi,pi
          accum_R += ang * w;
          Waccum_R += w;
        }
      }
  ...
  for (size_t i = 0; i < DIM; i++)
    for (size_t j = 0; j <= i; j++) // Only 1/2 of the matrix
    {
      ...
      if (i != j)
        covariances.get_unsafe(j, i) = elem;
    }
}
...
void covariancesAndMean(
    ...
    const bool *elem_do_wrap2pi =
        nullptr) { // The function below is inline-expanded here:
...
void weightedHistogram(const VECTORLIKE1 &values, const VECTORLIKE1 &weights,
                       float binWidth, VECTORLIKE2 &out_binCenters,
 ...
 * the whole vector sums up the unity.
  ...
  */
template <class VECTORLIKE1, class VECTORLIKE2>
void weightedHistogramLog(const VECTORLIKE1 &values,
                          const VECTORLIKE1 &log_weights, float binWidth,
  ...
  TNum totalSum = 0;
  typename VECTORLIKE1::const_iterator itVal, itW;
  for (itVal = values.begin(), itW = log_weights.begin(); itVal != values.end();
       ++itVal, ++itW) {
    int idx = round(((*itVal) - minBin) / binWidth);
    if (idx >= int(nBins))
      idx = nBins - 1;
 ...
 * page</a>.
  * \ingroup stats_grp
  */
double BASE_IMPEXP averageLogLikelihood(const CVectorDouble &logWeights,
