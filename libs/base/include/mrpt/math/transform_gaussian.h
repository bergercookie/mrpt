...
void transform_gaussian_unscented(
    ...
    void (*functor)(const VECTORLIKE1 &x, const USERPARAM &fixed_param,
                    VECTORLIKE3 &y),
  ...
  if (!valid)
    throw std::runtime_error("transform_gaussian_unscented: Singular "
                             "covariance matrix in Cholesky.");
  L *= sqrt(c);
  ...
  mrpt::math::covariancesAndMeanWeighted(Y, y_cov, y_mean, &W_mean, &W_cov,
                                         elem_do_wrap2pi);
  MRPT_END
}
...
* space and returns the mapped point in Y, optionally using an extra, constant
* parameter ("fixed_param") which remains constant.
* \param out_samples_y If !=nullptr, this vector will contain, upon return, the
* sequence of random samples generated and propagated through the functor().
  * \sa The example in MRPT/samples/unscented_transform_test
...
template <class VECTORLIKE1, class MATLIKE1, class USERPARAM, class VECTORLIKE2,
          class VECTORLIKE3, class MATLIKE2>
void transform_gaussian_linear(const VECTORLIKE1 &x_mean, const MATLIKE1 &x_cov,
                               void (*functor)(const VECTORLIKE1 &x,
                                               const USERPARAM &fixed_param,
                                               VECTORLIKE3 &y),
                               const USERPARAM &fixed_param,
                               VECTORLIKE2 &y_mean, MATLIKE2 &y_cov,
  ...
  Eigen::Matrix<double, VECTORLIKE3::RowsAtCompileTime,
      ...
      H;
  mrpt::math::jacobians::jacob_numeric_estimate(x_mean, functor, x_increments,
                                                fixed_param, H);
