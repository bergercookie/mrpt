...
extern double BASE_IMPEXP geometryEpsilon; //!< Global epsilon to overcome small
                                           //!precision errors (Default=1e-5)
...
public:
  ...
  TPolygon2D poly2D; //!< Polygon, after being projected to the plane using
                     //!inversePose. \sa inversePose
  TPolygonWithPlane(const TPolygon3D &p); //!< Constructor. Takes a polygon and
                                          //!computes each parameter.
   ...
   * TPolygonWithPlane  */
  static void getPlanes(const std::vector<TPolygon3D> &oldPolys,
...
/** Gets the intersection between two 3D segments. Possible outcomes:
  *		- Segments intersect: Return=true,
  ...
  * \sa TObject3D
  */
bool BASE_IMPEXP intersect(const TSegment3D &s1, const TSegment3D &s2,
                           TObject3D &obj);
...
/** Gets the intersection between a 3D segment and a plane. Possible outcomes:
  ...
  * \sa TObject3D
  */
bool BASE_IMPEXP intersect(const TSegment3D &s1, const TPlane &p2,
                           TObject3D &obj);
...
/** Gets the intersection between a 3D segment and a 3D line. Possible outcomes:
  ...
  * \sa TObject3D
  */
bool BASE_IMPEXP intersect(const TSegment3D &s1, const TLine3D &r2,
                           TObject3D &obj);
...
/** Gets the intersection between two 3D lines. Possible outcomes:
  ...
  * \sa TObject3D
  */
bool BASE_IMPEXP intersect(const TLine3D &r1, const TLine3D &r2,
                           TObject3D &obj);
...
/** Gets the intersection between two 2D lines. Possible outcomes:
  ...
  * \sa TObject2D
  */
bool BASE_IMPEXP intersect(const TLine2D &r1, const TLine2D &r2,
                           TObject2D &obj);
...
/** Gets the intersection between a 2D line and a 2D segment. Possible outcomes:
  ...
  * \sa TObject2D
  */
bool BASE_IMPEXP intersect(const TLine2D &r1, const TSegment2D &s2,
                           TObject2D &obj);
...
/** Gets the intersection between two 2D segments. Possible outcomes:
  ...
  * \sa TObject2D
  */
bool BASE_IMPEXP intersect(const TSegment2D &s1, const TSegment2D &s2,
                           TObject2D &obj);
 ...
 * the pose. An implicit constructor is used if a TPose3D is given.
  ...
  */
void BASE_IMPEXP createFromPoseAndVector(const mrpt::poses::CPose3D &p,
                                         const double (&vector)[3], TLine3D &r);
/**
 ...
 * given pose. An implicit constructor is used if a CPose2D is given.
  * \sa createFromPoseY,createFromPoseAndVector
  */
void BASE_IMPEXP createFromPoseAndVector(const TPose2D &p,
...
inline void project3D(const TPoint3D &point,
                      const mrpt::poses::CPose3D &newXYpose,
                      TPoint3D &newPoint) {
  newXYpose.composePoint(point.x, point.y, point.z, newPoint.x, newPoint.y,
                         newPoint.z);
}
/** Uses the given pose 3D to project a segment into a new base  */
inline void project3D(const TSegment3D &segment,
                      const mrpt::poses::CPose3D &newXYpose,
...
void BASE_IMPEXP project3D(const TLine3D &line,
                           const mrpt::poses::CPose3D &newXYpose,
                           TLine3D &newLine); //!< Uses the given pose 3D to
                                              //!project a line into a new base
void BASE_IMPEXP project3D(const TPlane &plane,
                           const mrpt::poses::CPose3D &newXYpose,
                           TPlane &newPlane); //!< Uses the given pose 3D to
                                              //!project a plane into a new base
void BASE_IMPEXP project3D(const TPolygon3D &polygon,
                           const mrpt::poses::CPose3D &newXYpose,
                           TPolygon3D &newPolygon); //!< Uses the given pose 3D
                                                    //!to project a polygon into
                                                    //!a new base
void BASE_IMPEXP project3D(const TObject3D &object,
                           const mrpt::poses::CPose3D &newXYPose,
                           TObject3D &newObject); //!< Uses the given pose 3D to
                                                  //!project any 3D object into
                                                  //!a new base.
...
template <class T>
void project3D(const T &obj, const TPlane &newXYPlane,
...
template <class T>
void project3D(const std::vector<T> &objs,
               const mrpt::poses::CPose3D &newXYpose, std::vector<T> &newObjs) {
  size_t N = objs.size();
  newObjs.resize(N);
...
void BASE_IMPEXP project2D(const TPoint2D &point,
                           const mrpt::poses::CPose2D &newXpose,
                           TPoint2D &newPoint);

/** Uses the given pose 2D to project a segment into a new base  */
inline void project2D(const TSegment2D &segment,
                      const mrpt::poses::CPose2D &newXpose,
...
void BASE_IMPEXP project2D(const TLine2D &line,
                           const mrpt::poses::CPose2D &newXpose,
                           TLine2D &newLine); //!< Uses the given pose 2D to
                                              //!project a line into a new base
void BASE_IMPEXP project2D(const TPolygon2D &polygon,
                           const mrpt::poses::CPose2D &newXpose,
                           TPolygon2D &newPolygon); //!< Uses the given pose 2D
                                                    //!to project a polygon into
                                                    //!a new base.
void BASE_IMPEXP project2D(const TObject2D &object,
                           const mrpt::poses::CPose2D &newXpose,
                           TObject2D &newObject); //!< Uses the given pose 2D to
                                                  //!project any 2D object into
                                                  //!a new base
 ...
 * inside the line, this projection will zero its Y coordinate.
  ...
  */
template <class T, class CPOSE2D>
void project2D(const T &obj, const TLine2D &newXLine, const TPoint2D &newOrigin,
               T &newObj) {
  CPOSE2D pose;
...
template <class T>
void project2D(const std::vector<T> &objs, const mrpt::poses::CPose2D &newXpose,
 ...
 */
bool BASE_IMPEXP intersect(const TPolygon2D &p1, const TSegment2D &s2,
                           TObject2D &obj);
/** Gets the intersection between a 2D polygon and a 2D line. \sa TObject2D  */
bool BASE_IMPEXP intersect(const TPolygon2D &p1, const TLine2D &r2,
                           TObject2D &obj);
/** Gets the intersection between two 2D polygons. \sa TObject2D */
bool BASE_IMPEXP intersect(const TPolygon2D &p1, const TPolygon2D &p2,
                           TObject2D &obj);
/** Gets the intersection between a 2D segment and a 2D polygon.  \sa TObject2D
 */
inline bool intersect(const TSegment2D &s1, const TPolygon2D &p2,
                      TObject2D &obj) {
 ...
 */
bool BASE_IMPEXP intersect(const TPolygon3D &p1, const TSegment3D &s2,
                           TObject3D &obj);
/** Gets the intersection between a 3D polygon and a 3D line. \sa TObject3D  */
bool BASE_IMPEXP intersect(const TPolygon3D &p1, const TLine3D &r2,
                           TObject3D &obj);
/** Gets the intersection between a 3D polygon and a plane. \sa TObject3D */
bool BASE_IMPEXP intersect(const TPolygon3D &p1, const TPlane &p2,
                           TObject3D &obj);
/** Gets the intersection between two 3D polygons. \sa TObject3D */
bool BASE_IMPEXP intersect(const TPolygon3D &p1, const TPolygon3D &p2,
                           TObject3D &obj);
/** Gets the intersection between a 3D segment and a 3D polygon. \sa TObject3D
 */
inline bool intersect(const TSegment3D &s1, const TPolygon3D &p2,
                      TObject3D &obj) {
  return intersect(p2, s1, obj);
}
...
size_t BASE_IMPEXP intersect(const std::vector<TPolygon3D> &v1,
...
size_t intersect(const std::vector<T> &v1, const std::vector<U> &v2,
...
bool BASE_IMPEXP intersect(const TObject2D &o1, const TObject2D &o2,
                           TObject2D &obj);
/** Gets the intersection between any pair of 3D objects.*/
bool BASE_IMPEXP intersect(const TObject3D &o1, const TObject3D &o2,
                           TObject3D &obj);
/** @}
 */
...
/** @name Distances
        @{
 */
double BASE_IMPEXP distance(const TPoint2D &p1,
                            const TPoint2D &p2); //!< Gets the distance between
                                                 //!two points in a 2D space.
double BASE_IMPEXP distance(const TPoint3D &p1,
                            const TPoint3D &p2); //!< Gets the distance between
                                                 //!two points in a 3D space.
double BASE_IMPEXP distance(
    const TLine2D &r1,
    const TLine2D &r2); //!<  Gets the distance between two lines in a 2D space.
double BASE_IMPEXP distance(
    const TLine3D &r1,
    const TLine3D &r2); //!<  Gets the distance between two lines in a 3D space.
double BASE_IMPEXP distance(const TPlane &p1,
                            const TPlane &p2); //!<  Gets the distance between
                                               //!two planes. It will be zero if
                                               //!the planes are not parallel.
double BASE_IMPEXP distance(const TPolygon2D &p1,
                            const TPolygon2D &p2); //!< Gets the distance
                                                   //!between two polygons in a
                                                   //!2D space.
double BASE_IMPEXP distance(const TPolygon2D &p1,
                            const TSegment2D &s2); //!< Gets the distance
                                                   //!between a polygon and a
...
double BASE_IMPEXP distance(const TPolygon2D &p1,
                            const TLine2D &l2); //!< Gets the distance between a
                                                //!polygon and a line in a 2D
                                                //!space.
inline double distance(const TLine2D &l1, const TPolygon2D &p2) {
  return distance(p2, l1);
}
double BASE_IMPEXP distance(const TPolygon3D &p1,
                            const TPolygon3D &p2); //!< Gets the distance
                                                   //!between two polygons in a
                                                   //!3D space.
double BASE_IMPEXP distance(const TPolygon3D &p1,
                            const TSegment3D &s2); //!< Gets the distance
                                                   //!between a polygon and a
                                                   //!segment in a 3D space.
...
double BASE_IMPEXP distance(const TPolygon3D &p1,
                            const TLine3D &l2); //!< Gets the distance between a
                                                //!polygon and a line in a 3D
                                                //!space.
...
double BASE_IMPEXP distance(
    const TPolygon3D &po,
    const TPlane &pl); //!< Gets the distance between a polygon and a plane.
...
void BASE_IMPEXP getRectangleBounds(const std::vector<TPoint2D> &poly,
                                    TPoint2D &pMin, TPoint2D &pMax);
/** Gets the prism bounds of a 3D polygon or set of 3D points. */
void BASE_IMPEXP getPrismBounds(const std::vector<TPoint3D> &poly,
                                TPoint3D &pMin, TPoint3D &pMax);
/** @}
 */
...
/**
  ...
  */
void BASE_IMPEXP createPlaneFromPoseXY(const mrpt::poses::CPose3D &pose,
                                       TPlane &plane);
/**
  * Given a pose, creates a plane orthogonal to its Y vector.
  * \sa createPlaneFromPoseXY,createPlaneFromPoseYZ,createPlaneFromPoseAndNormal
  */
void BASE_IMPEXP createPlaneFromPoseXZ(const mrpt::poses::CPose3D &pose,
                                       TPlane &plane);
/**
  * Given a pose, creates a plane orthogonal to its X vector.
  * \sa createPlaneFromPoseXY,createPlaneFromPoseXZ,createPlaneFromPoseAndNormal
  */
void BASE_IMPEXP createPlaneFromPoseYZ(const mrpt::poses::CPose3D &pose,
                                       TPlane &plane);
/**
 ...
 * the pose's coordinates.
  ...
  */
void BASE_IMPEXP createPlaneFromPoseAndNormal(const mrpt::poses::CPose3D &pose,
 ...
 * for y, 2 for z) corresponds to the provided vector.
  ...
  */
void BASE_IMPEXP generateAxisBaseFromDirectionAndAxis(const double (&vec)[3],
                                                      char coord,
 ...
 * Returns an estimation of the error.
  ...
  */
double BASE_IMPEXP getRegressionLine(const std::vector<TPoint2D> &points,
                                     TLine2D &line);
/**
 ...
 * Returns an estimation of the error.
  * \sa spline, leastSquareLinearFit
  */
double BASE_IMPEXP getRegressionLine(const std::vector<TPoint3D> &points,
                                     TLine3D &line);
/**
 ...
 * Returns an estimation of the error.
  * \sa spline, leastSquareLinearFit
  */
double BASE_IMPEXP getRegressionPlane(const std::vector<TPoint3D> &points,
                                      TPlane &plane);
/** @}
 */
...
/**
  ...
  */
void BASE_IMPEXP assemblePolygons(const std::vector<TSegment3D> &segms,
 ...
 * set of objects.
  */
void BASE_IMPEXP assemblePolygons(const std::vector<TObject3D> &objs,
                                  std::vector<TPolygon3D> &polys);
/**
  * Extracts all the polygons, including those formed from segments, from the
 * set of objects.
...
/**
  ...
  */
bool BASE_IMPEXP splitInConvexComponents(const TPolygon2D &poly,
...
/**
  ...
  */
bool BASE_IMPEXP splitInConvexComponents(const TPolygon3D &poly,
 ...
 * used if necessary)
  */
void BASE_IMPEXP getAngleBisector(const TLine2D &l1, const TLine2D &l2,
                                  TLine2D &bis);
/**
  * Gets the bisector of two lines or segments (implicit constructor will be
 * used if necessary)
  ...
  */
void BASE_IMPEXP getAngleBisector(const TLine3D &l1, const TLine3D &l2,
                                  TLine3D &bis);
...
/**
  ...
  */
bool BASE_IMPEXP traceRay(const std::vector<TPolygonWithPlane> &vec,
                          const mrpt::poses::CPose3D &pose, double &dist);
/**
  * Fast ray tracing method using polygons' properties.
  * \sa CRenderizable::rayTrace
  */
inline bool traceRay(const std::vector<TPolygon3D> &vec,
                     const mrpt::poses::CPose3D &pose, double &dist) {
...
//! \overload
template <class T>
inline void crossProduct3D(const std::vector<T> &v0, const std::vector<T> &v1,
                           std::vector<T> &v_out) {
  ASSERT_(v0.size() == 3)
  ASSERT_(v1.size() == 3);
  v_out.resize(3);
...
inline Eigen::Matrix<double, 3, 1> crossProduct3D(const VEC1 &v0,
                                                  const VEC2 &v1) {
...
inline bool vectorsAreParallel3D(const T &v1, const U &v2) {
  if (abs(v1[0] * v2[1] - v2[0] * v1[1]) >= geometryEpsilon)
    return false;
  if (abs(v1[1] * v2[2] - v2[1] * v1[2]) >= geometryEpsilon)
    return false;
...
/** Computes the closest point from a given point to a segment.
  ...
  */
void BASE_IMPEXP closestFromPointToSegment(const double &Px, const double &Py,
...
/** Computes the closest point from a given point to a (infinite) line.
  ...
  */
void BASE_IMPEXP closestFromPointToLine(const double &Px, const double &Py,
                                        const double &x1, const double &y1,
                                        const double &x2, const double &y2,
                                        double &out_x, double &out_y);
...
template <typename T>
T distanceBetweenPoints(const T x1, const T y1, const T z1, const T x2,
...
template <typename T>
T distanceSqrBetweenPoints(const T x1, const T y1, const T z1, const T x2,
                           const T y2, const T z2) {
 ...
 * minimum distance.
  */
template <typename T>
double minimumDistanceFromPointToSegment(const double Px, const double Py,
                                         const double x1, const double y1,
                                         const double x2, const double y2,
                                         T &out_x, T &out_y) {
  double ox, oy;
...
/** Returns the intersection point, and if it exists, between two segments.
  */
bool BASE_IMPEXP SegmentsIntersection(const double x1, const double y1,
                                      const double x2, const double y2,
...
/** Returns true if the 2D point (px,py) falls INTO the given polygon.
  ...
  */
bool BASE_IMPEXP pointIntoPolygon2D(const double &px, const double &py,
...
/** Specialized method to check whether a point (x,y) falls into a quadrangle.
  ...
  */
template <typename T>
bool pointIntoQuadrangle(T x, T y, T v1x, T v1y, T v2x, T v2y, T v3x, T v3y,
                         T v4x, T v4y) {
 ...
 * point is INTO the polygon or its perimeter.
  */
double BASE_IMPEXP distancePointToPolygon2D(const double &px, const double &py,
                                            unsigned int polyEdges,
                                            const double *poly_xs,
                                            const double *poly_ys);
...
/** Calculates the minimum distance between a pair of lines.
  ...
  to EPS) parallel.
 */
bool BASE_IMPEXP minDistBetweenLines(const double &p1_x, const double &p1_y,
                                     const double &p1_z, const double &p2_x,
                                     const double &p2_y, const double &p2_z,
                                     const double &p3_x, const double &p3_y,
                                     const double &p3_z, const double &p4_x,
                                     const double &p4_y, const double &p4_z,
