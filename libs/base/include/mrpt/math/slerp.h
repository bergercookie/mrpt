...
/** SLERP interpolation between two quaternions
  ...
  */
template <typename T>
void slerp(const CQuaternion<T> &q0, const CQuaternion<T> &q1, const double t,
           CQuaternion<T> &q) {
  ASSERTDEB_(t >= 0 && t <= 1)
  // See:
 ...
 * quaternions, but interpolates the [X,Y,Z] coordinates as well.
  ...
  * \exception std::exception Only in Debug, if t is not in the valid range.
  */
void BASE_IMPEXP slerp(const mrpt::poses::CPose3D &q0,
                       const mrpt::poses::CPose3D &q1, const double t,
                       mrpt::poses::CPose3D &p);

//! \overload
void BASE_IMPEXP slerp(const mrpt::poses::CPose3DQuat &q0,
                       const mrpt::poses::CPose3DQuat &q1, const double t,
 ...
 */
void BASE_IMPEXP slerp_ypr(const mrpt::math::TPose3D &q0,
                           const mrpt::math::TPose3D &q1, const double t,
                           mrpt::math::TPose3D &p);
