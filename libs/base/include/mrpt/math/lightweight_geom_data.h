...
struct BASE_IMPEXP TPoint2D {
  ...
  /** Returns a human-readable textual representation of the object (eg: "[0.02
   * 1.04]" )
    * \sa fromString
    */
...
struct BASE_IMPEXP TPose2D {
  ...
  mrpt::math::TPose2D
  operator+(const mrpt::math::TPose2D &b) const; //!< Operator "oplus" pose
                                                 //!composition: "ret=this
                                                 //!\oplus b"  \sa CPose2D
  mrpt::math::TPose2D
  operator-(const mrpt::math::TPose2D &b) const; //!< Operator "ominus" pose
                                                 //!composition: "ret=this
                                                 //!\ominus b"  \sa CPose2D
...
struct BASE_IMPEXP TPoint3D {
  ...
  /**
    * Point norm.
    */
  inline double norm() const { return sqrt(square(x) + square(y) + square(z)); }
  /**
    * Point scale.
    */
  ...
  /**
    * Transformation into vector.
    */
  template <class VECTORLIKE> void getAsVector(VECTORLIKE &v) const {
    v.resize(3);
    v[0] = x;
    v[1] = y;
    v[2] = z;
...
struct BASE_IMPEXP TPointXYZRGBu8 {
  ...
  inline TPointXYZRGBu8(double x, double y, double z, uint8_t R_val,
                        uint8_t G_val, uint8_t B_val)
...
struct BASE_IMPEXP TPointXYZfRGBu8 {
  ...
  inline TPointXYZfRGBu8(float x, float y, float z, uint8_t R_val,
                         uint8_t G_val, uint8_t B_val)
      : pt(x, y, z), R(R_val), G(G_val), B(B_val) {}
};
...
struct BASE_IMPEXP TPose3D {
  ...
  /**
    * Constructor from coordinates.
    */
  TPose3D(double _x, double _y, double _z, double _yaw, double _pitch,
          double _roll)
...
struct BASE_IMPEXP TPose3DQuat {
  ...
  /** Constructor from coordinates. */
  inline TPose3DQuat(double _x, double _y, double _z, double _qr, double _qx,
...
public:
  ...
  /**
    * Get a pose2D whose X axis corresponds to the line, forcing the base point
   * to one given.
    * \throw logic_error if the point is not inside the line.
    * \sa mrpt::poses::CPose2D.
    */
  void getAsPose2DForcingOrigin(const TPoint2D &origin,
...
public:
  ...
  /**
    ...
    * \throw std::logic_error if the line's director vector is orthogonal to the
   * XY plane.
    */
...
public:
  ...
  /**
    * Gets a pose whose XY plane corresponds to this, forcing an exact point as
   * its spatial coordinates.
    * \throw std::logic_error if the point is not inside the plane.
    */
  void getAsPose3DForcingOrigin(const TPoint3D &newOrigin,
   ...
   * its spatial coordinates.
    * \throw std::logic_error if the point is not inside the plane.
    */
  inline void getAsPose3DForcingOrigin(const TPoint3D &newOrigin,
                                       mrpt::poses::CPose3D &pose) const {
    TPlane p = *this;
  ...
  /**
    ...
    */
  TPlane(const TPoint3D &p1, const TPoint3D &p2,
...
public:
  double
  distance(const TPoint2D &point) const; //!< Distance to a point (always >=0)
  bool contains(const TPoint2D &point) const; //!< Check whether a point is
                                              //!inside (or within
                                              //!geometryEpsilon of a polygon
                                              //!edge). This works for concave
                                              //!or convex polygons.
  void getAsSegmentList(std::vector<TSegment2D> &v)
      const; //!< Gets as set of segments, instead of points.
  void generate3DObject(
      TPolygon3D &p) const;          //!< Projects into 3D space, zeroing the z.
  void getCenter(TPoint2D &p) const; //!< Polygon's central point.
  bool isConvex() const;             //!< Checks whether is convex.
  void removeRepeatedVertices();     //!< Erase repeated vertices.  \sa
                                     //!removeRedundantVertices
  void removeRedundantVertices();    //!< Erase every redundant vertex from the
                                     //!polygon, saving space. \sa
                                     //!removeRepeatedVertices
  void
  getPlotData(std::vector<double> &x,
              std::vector<double> &y) const; //!< Gets plot data, ready to use
                                             //!on a 2D plot. \sa
                                             //!mrpt::gui::CDisplayWindowPlots
  void getBoundingBox(TPoint2D &min_coords, TPoint2D &max_coords)
      const; //!< Get polygon bounding box. \exception On empty polygon
  /** Default constructor  */
   ...
   * less than three.
    */
  static void createRegularPolygon(size_t numEdges, double radius,
                                   TPolygon2D &poly);
   ...
   * less than three.
    */
  static inline void createRegularPolygon(size_t numEdges, double radius,
                                          TPolygon2D &poly,
...
public:
  double
  distance(const TPoint3D &point) const; //!< Distance to point (always >=0)
  bool contains(const TPoint3D &point) const; //!< Check whether a point is
                                              //!inside (or within
                                              //!geometryEpsilon of a polygon
                                              //!edge). This works for concave
                                              //!or convex polygons.
  void getAsSegmentList(std::vector<TSegment3D> &v)
      const; //!< Gets as set of segments, instead of set of points.
  bool getPlane(TPlane &p) const; //!< Gets a plane which contains the polygon.
                                  //!Returns false if the polygon is skew and
                                  //!cannot be fit inside a plane.
  ...
  bool isSkew() const; //!< Check whether the polygon is skew. Returns true if
                       //!there doesn't exist a plane in which the polygon can
                       //!fit. \sa getBestFittingPlane
  void removeRepeatedVertices();  //!< Remove polygon's repeated vertices.
  void removeRedundantVertices(); //!< Erase every redundant vertex, thus saving
                                  //!space.
  ...
  static void createRegularPolygon(size_t numEdges, double radius,
                                   TPolygon3D &poly);
  /** Static method to create a regular polygon, given its size and radius. The
   * center will be located on the given pose.
    * \throw std::logic_error if number of edges is less than three, or radius
   * is near zero.
    */
  static inline void createRegularPolygon(size_t numEdges, double radius,
...
private:
  ...
  /**
    ...
    */
  inline void destroy() {
    if (type == GEOMETRIC_TYPE_POLYGON)
      delete data.polygon;
...
public:
  /**
    ...
    */
  TObject3D(const TPoint3D &p) : type(GEOMETRIC_TYPE_POINT) { data.point = p; }
  /**
  ...
  /**
    ...
    */
  static void getPoints(const std::vector<TObject3D> &objs,
                        std::vector<TPoint3D> &pnts);
  /**
    * Static method to retrieve every segment included in a vector of objects.
    */
  static void getSegments(const std::vector<TObject3D> &objs,
                          std::vector<TSegment3D> &sgms);
  /**
    * Static method to retrieve every line included in a vector of objects.
    */
  static void getLines(const std::vector<TObject3D> &objs,
                       std::vector<TLine3D> &lins);
  /**
    * Static method to retrieve every plane included in a vector of objects.
    */
  static void getPlanes(const std::vector<TObject3D> &objs,
                        std::vector<TPlane> &plns);
  /**
    * Static method to retrieve every polygon included in a vector of objects.
    */
  static void getPolygons(const std::vector<TObject3D> &objs,
...
struct BASE_IMPEXP TTwist2D {
  ...
  bool operator!=(const TTwist2D &o) const;
  mrpt::math::TPose2D operator*(const double dt) const; //!< Returns the pose
                                                        //!increment of
                                                        //!multiplying each
                                                        //!twist component times
                                                        //!"dt" seconds.
  /** Returns a human-readable textual representation of the object (eg: "[vx vy
   * omega]", omega in deg/s)
     * \sa fromString
     */
  void asString(std::string &s) const;
  inline std::string asString() const {
...
struct BASE_IMPEXP TTwist3D {
  ...
  /** Constructor from components */
  inline TTwist3D(double vx_, double vy_, double vz_, double wx_, double wy_,
                  double wz_)
