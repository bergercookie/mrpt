...
enum TRobustKernelType {
  rkLeastSquares =
      0, //!< No robust kernel, use standard least squares: rho(r)= 1/2 * r^2
...
template <typename T> struct RobustKernel<rkLeastSquares, T> {
  T param_sq; //!< The kernel parameter (the "threshold") squared [Not used in
              //!this class, provided for consistency with the other classes]

  /** Evaluates the kernel function for the squared error r2 and returns
   * robustified squared error and derivatives of sqrt(2*rho(r)) at this point.
   */
 ...
 * r^2/delta^2 ) ) */
template <typename T> struct RobustKernel<rkPseudoHuber, T> {
