...
normalPDFInf(const VECTORLIKE1 &x, const VECTORLIKE2 &mu,
             const MATRIXLIKE &cov_inv, const bool scaled_pdf = false) {
  MRPT_START
  typedef typename MATRIXLIKE::Scalar T;
  ASSERTDEB_(cov_inv.isSquare())
  ASSERTDEB_(size_t(cov_inv.getColCount()) == size_t(x.size()) &&
             size_t(cov_inv.getColCount()) == size_t(mu.size()))
  T ret = ::exp(static_cast<T>(-0.5) *
                mrpt::math::multiply_HCHt_scalar((x - mu).eval(), cov_inv));
  return scaled_pdf ? ret
                    : ret * ::sqrt(cov_inv.det() /
                                   ::pow(static_cast<T>(M_2PI),
...
template <class VECTORLIKE1, class VECTORLIKE2, class MATRIXLIKE>
inline typename MATRIXLIKE::Scalar
normalPDF(const VECTORLIKE1 &x, const VECTORLIKE2 &mu, const MATRIXLIKE &cov,
...
typename MATRIXLIKE::Scalar normalPDF(const VECTORLIKE &d,
                                      const MATRIXLIKE &cov) {
  MRPT_START
  ASSERTDEB_(cov.isSquare())
  ASSERTDEB_(size_t(cov.getColCount()) == size_t(d.size()))
  return std::exp(static_cast<typename MATRIXLIKE::Scalar>(-0.5) *
                  mrpt::math::multiply_HCHt_scalar(d, cov.inverse())) /
         (::pow(static_cast<typename MATRIXLIKE::Scalar>(M_2PI),
...
double KLD_Gaussians(const VECTORLIKE1 &mu0, const MATRIXLIKE1 &cov0,
                     const VECTORLIKE2 &mu1, const MATRIXLIKE2 &cov1) {
  MRPT_START
  ASSERT_(size_t(mu0.size()) == size_t(mu1.size()) &&
...
26.3.32).
        The algorithm's running time is independent of the inputs. The accuracy
is only
        about 0.1 for few degrees of freedom, but reaches about 0.001 above dof
= 5.
...
*/
double BASE_IMPEXP noncentralChi2CDF(unsigned int degreesOfFreedom,
...
/*! Cumulative chi square distribution.
   ...
   degreesOfFreedom
        and tolerance \a accuracy at the given argument \a arg, i.e. the
   probability that
        a random number drawn from the distribution is below \a arg
        by calling <tt>noncentralChi2CDF(degreesOfFreedom, 0.0, arg,
   accuracy)</tt>.
 ...
 *	Computes the density of a chi square distribution with \a
 *degreesOfFreedom
 *	and tolerance \a accuracy at the given argument \a arg
 *	by calling <tt>noncentralChi2(degreesOfFreedom, 0.0, arg,
 *accuracy)</tt>.
 *	\note Function code from the Vigra project
 *(http://hci.iwr.uni-heidelberg.de/vigra/); code under "MIT X11 License", GNU
 *GPL-compatible.
 ...
 */
double BASE_IMPEXP chi2PDF(unsigned int degreesOfFreedom, double arg,
...
std::pair<double, double>
    BASE_IMPEXP noncentralChi2PDF_CDF(unsigned int degreesOfFreedom,
                                      double noncentrality, double arg,
                                      double eps = 1e-7);
