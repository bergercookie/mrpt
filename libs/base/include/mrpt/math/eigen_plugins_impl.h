...
template <int R, int C> struct MatOrVecResizer {
  template <typename S, int Opt, int MaxR, int MaxC>
  static inline void doit(Eigen::Matrix<S, R, C, Opt, MaxR, MaxC> &mat,
                          size_t new_rows, size_t new_cols) {
    ::mrpt::math::detail::TAuxResizer<
        Eigen::Matrix<S, R, C, Opt, MaxR, MaxC>,
        Eigen::Matrix<S, R, C, Opt, MaxR,
                      MaxC>::SizeAtCompileTime>::internal_resize(mat, new_rows,
                                                                 new_cols);
  }
};
// Specialization for column matrices:
template <int R> struct MatOrVecResizer<R, 1> {
  template <typename S, int Opt, int MaxR, int MaxC>
  static inline void doit(Eigen::Matrix<S, R, 1, Opt, MaxR, MaxC> &mat,
                          size_t new_rows, size_t) {
    ::mrpt::math::detail::TAuxResizer<
        Eigen::Matrix<S, R, 1, Opt, MaxR, MaxC>,
        Eigen::Matrix<S, R, 1, Opt, MaxR,
                      MaxC>::SizeAtCompileTime>::internal_resize(mat, new_rows);
  }
};
// Specialization for row matrices:
template <int C> struct MatOrVecResizer<1, C> {
  template <typename S, int Opt, int MaxR, int MaxC>
  static inline void doit(Eigen::Matrix<S, 1, C, Opt, MaxR, MaxC> &mat, size_t,
                          size_t new_cols) {
    ::mrpt::math::detail::TAuxResizer<
        Eigen::Matrix<S, 1, C, Opt, MaxR, MaxC>,
        Eigen::Matrix<S, 1, C, Opt, MaxR,
                      MaxC>::SizeAtCompileTime>::internal_resize(mat, new_cols);
  }
};
template <> struct MatOrVecResizer<1, 1> {
  template <typename S, int Opt, int MaxR, int MaxC>
  static inline void doit(Eigen::Matrix<S, 1, 1, Opt, MaxR, MaxC> &mat, size_t,
                          size_t new_cols) {
    ::mrpt::math::detail::TAuxResizer<
        Eigen::Matrix<S, 1, 1, Opt, MaxR, MaxC>,
        Eigen::Matrix<S, 1, 1, Opt, MaxR,
                      MaxC>::SizeAtCompileTime>::internal_resize(mat, new_cols);
  }
};
}
...
Eigen::MatrixBase<Derived>::eigenVectors(MATRIX1 &eVecs, MATRIX2 &eVals) const {
...
Eigen::MatrixBase<Derived>::eigenVectorsSymmetric(MATRIX1 &eVecs,
                                                  MATRIX2 &eVals) const {
  Matrix<Scalar, Dynamic, 1> evals;
...
template <class MATRIX1, class VECTOR1>
EIGEN_STRONG_INLINE void
Eigen::MatrixBase<Derived>::eigenVectorsSymmetricVec(MATRIX1 &eVecs,
                                                     VECTOR1 &eVals) const {
...
bool Eigen::MatrixBase<Derived>::fromMatlabStringFormat(
  ...
  while (i < end) {
    ...
    if (lstElements.empty()) {
      if (nRow > 0)
        ...
        // Else, this may be an empty matrix... if there is no next row, we'll
        // return with a (0,0) matrix
        if (Derived::RowsAtCompileTime == Eigen::Dynamic)
          (*this) = Derived();
      }
    } else {
      ...
      if ((nRow > 0 && size_t(cols()) != N) ||
        ...
        if (dump_errors_here)
          (*dump_errors_here) << "[fromMatlabStringFormat] Row " << nRow + 1
      ...
      else if (Derived::RowsAtCompileTime != Eigen::Dynamic &&
               int(nRow) >= Derived::RowsAtCompileTime) {
        if (dump_errors_here)
          (*dump_errors_here) << "[fromMatlabStringFormat] Read more rows than "
                                 "the capacity of the fixed sized matrix.\n";
        return false;
      }
  ...
  if (Derived::RowsAtCompileTime != Eigen::Dynamic &&
      int(nRow) != Derived::RowsAtCompileTime) {
    if (dump_errors_here)
      (*dump_errors_here) << "[fromMatlabStringFormat] Read less rows than the "
                             "capacity of the fixed sized matrix.\n";
    return false;
  }
  return true; // Ok
}
...
template <class Derived>
std::string
Eigen::MatrixBase<Derived>::inMatlabFormat(const size_t decimal_digits) const {
  std::stringstream s;
  s << "[" << std::scientific;
...
#endif
    fprintf(f, "%% File generated with %s at "
               "%s\n%%---------------------------------------------------------"
               "--------\n",
...
void Eigen::MatrixBase<Derived>::loadFromTextFile(const std::string &file) {
  ...
  if (f.fail())
    throw std::runtime_error(std::string("loadFromTextFile: can't open file:") +
                             file);
  loadFromTextFile(f);
}
...
void Eigen::MatrixBase<Derived>::loadFromTextFile(std::istream &f) {
  ...
  while (!f.eof() && !f.fail()) {
    ...
    if (str.size() && str[0] != '#' && str[0] != '%') {
      ...
      if ((Derived::ColsAtCompileTime != Eigen::Dynamic &&
        ...
        throw std::runtime_error("loadFromTextFile: The matrix in the text "
                                 "file does not match fixed matrix size");
      if (Derived::ColsAtCompileTime == Eigen::Dynamic && nRows > 0 &&
          Index(i) != cols())
        throw std::runtime_error("loadFromTextFile: The matrix in the text "
                                 "file does not have the same number of "
                                 "columns in all rows");
      ...
      if (Derived::RowsAtCompileTime == Eigen::Dynamic ||
        ...
        if (rows() < static_cast<int>(nRows + 1) ||
          ...
          internal_mrpt::MatOrVecResizer<
              Derived::RowsAtCompileTime,
              Derived::ColsAtCompileTime>::doit(derived(), nRows + extra_rows,
                                                i);
        }
      } else if (Derived::RowsAtCompileTime != Eigen::Dynamic &&
                 int(nRows) >= Derived::RowsAtCompileTime)
        throw std::runtime_error("loadFromTextFile: Read more rows than the "
                                 "capacity of the fixed sized matrix.");
  ...
  if (Derived::RowsAtCompileTime == Eigen::Dynamic ||
    ...
    internal_mrpt::MatOrVecResizer<Derived::RowsAtCompileTime,
                                   Derived::ColsAtCompileTime>::doit(derived(),
                                                                     nRows,
                                                                     cols());
