...
class BASE_IMPEXP CRuntimeCompiledExpression
{
   public:
	 ...
	 * exactly the problem.
		* \param[in] expression The expression to be compiled.
	  * \sa register_symbol_table()
		* \param[in] variables Map of variables/constants by `name` ->  `value`.
	 * The references to
		* the values in this map **must** be ensured to be valid thoughout all
	 * the
		* life of the compiled expression.
		* \param[in] expr_name_for_error_reporting A descriptive name of this
		* formula, to be used when generating error reports via an  exception,
	 * if
		* needed
	  */
	void compile(
