...
public:
  ...
  void compile(
      ...
      const std::map<std::string, double> &variables =
          std::map<std::string, double>(), //!< [in] Map of variables/constants
                                           //!by `name` ->  `value`. The
                                           //!references to the values in this
                                           //!map **must** be ensured to be
                                           //!valid thoughout all the life of
                                           //!the compiled expression.
      const std::string &expr_name_for_error_reporting =
          std::string() //!< A descriptive name of this formula, to be used when
                        //!generating error reports via an  exception, if needed
      );

  /** Can be used **before** calling compile() to register additional variables
   * by means of **pointers** instead of a std::map  */
  void register_symbol_table(const std::map<std::string, double *>
                                 &variables //!< [in] Map of variables/constants
                                            //!by `name` ->  `value`. The
                                            //!references to the values in this
                                            //!map **must** be ensured to be
                                            //!valid thoughout all the life of
  ...
  */
  ...
  bool is_compiled()
      const; //!< Returns true if compile() was called and ended without errors.
  const std::string &get_original_expression() const; //!< Returns the original
                                                      //!formula passed to
                                                      //!compile(), or an empty
                                                      //!string if still not
                                                      //!compiled.
  ...
  get_raw_exprtk_expr(); //!< Access raw exprtk expression object.
  const exprtk::expression<double> &
  get_raw_exprtk_expr() const; //!< Access raw exprtk expression object.
