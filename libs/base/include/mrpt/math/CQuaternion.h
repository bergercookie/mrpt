 ...
 */
template <class T> class CQuaternion : public CArrayNumeric<T, 4> {
...
public:
  ...
  inline CQuaternion(const T r, const T x, const T y, const T z) {
    ...
    (*this)[2] = y;
    (*this)[3] = z;
    ASSERTMSG_(std::abs(normSqr() - 1.0) < 1e-3,
               mrpt::format("Initialization data for quaternion is not "
                            "normalized: %f %f %f %f -> sqrNorm=%f",
                            r, x, y, z, normSqr()));
  }
   ...
   * rotation vector \f$ \mathbf{v} \f$:
    *   If \f$ \mathbf{v}=0 \f$, then the quaternion is \f$ \mathbf{q} = [1 ~ 0
   * ~ 0 ~ 0]^\top \f$, otherwise:
    ...
    *  \sa "Representing Attitude: Euler Angles, Unit Quaternions, and Rotation
   * Vectors (2006)", James Diebel.
    */
  template <class ARRAYLIKE3> void fromRodriguesVector(const ARRAYLIKE3 &v) {
    if (v.size() != 3)
      THROW_EXCEPTION("Vector v must have a length=3");
   ...
   * corresponding vector in the SO(3) Lie Algebra,
    *  which coincides with the so-called "rotation vector" (I don't have space
   * here for the proof ;-).
  ...
  template <class ARRAYLIKE3> inline void ln(ARRAYLIKE3 &out_ln) const {
    if (out_ln.size() != 3)
      out_ln.resize(3);
    this->ln_noresize(out_ln);
  }
  /** overload that returns by value */
  template <class ARRAYLIKE3> inline ARRAYLIKE3 ln() const {
    ARRAYLIKE3 out_ln;
    this->ln(out_ln);
    return out_ln;
  }
   ...
   * quaternion: this = q1 x q2
    *   After the operation, "this" will represent the composed rotations of q1
   * and q2 (q2 applied "after" q1).
    */
  inline void crossProduct(const CQuaternion &q1, const CQuaternion &q2) {
    ...
    const T new_x =
        q1.r() * q2.x() + q2.r() * q1.x() + q1.y() * q2.z() - q2.y() * q1.z();
    const T new_y =
        q1.r() * q2.y() + q2.r() * q1.y() + q1.z() * q2.x() - q2.z() * q1.x();
    const T new_z =
        q1.r() * q2.z() + q2.r() * q1.z() + q1.x() * q2.y() - q2.x() * q1.y();
    this->r(new_r);
    this->x(new_x);
    this->y(new_y);
    this->z(new_z);
  ...
  /** Rotate a 3D point (lx,ly,lz) -> (gx,gy,gz) as described by this quaternion
    */
  void rotatePoint(const double lx, const double ly, const double lz,
   ...
   * (conjugate) of this quaternion
    */
  void inverseRotatePoint(const double lx, const double ly, const double lz,
                          double &gx, double &gy, double &gz) const {
    const double t2 = -r() * x();
    const double t3 = -r() * y();
    const double t4 = -r() * z();
   ...
   * f(\cdot) = q_{this} \times r \f$, that is the 4x4 matrix \f$ \frac{\partial
   * f}{\partial q_{this} }  \f$.
    *  The output matrix can be a dynamic or fixed size (4x4) matrix.
    */
   ...
   * \mathbf{R} = \left(
    *    \begin{array}{ccc}
    *     q_r^2+q_x^2-q_y^2-q_z^2 	&  2(q_x q_y - q_r q_z)	&  	2(q_z q_x+q_r
   * q_y) \\
    *     2(q_x q_y+q_r q_z) 		& q_r^2-q_x^2+q_y^2-q_z^2 	& 2(q_y q_z-q_r
   * q_x) \\
   ...
   * q_z^2
    ...
    *
    */
  template <class MATRIXLIKE> inline void rotationMatrix(MATRIXLIKE &M) const {
    M.setSize(3, 3);
    rotationMatrixNoResize(M);
  }

  /** Fill out the top-left 3x3 block of the given matrix with the rotation
   * matrix associated to this quaternion (does not resize the matrix, for that,
   * see rotationMatrix). */
  template <class MATRIXLIKE>
  ...
  */
  inline void rpy(T &roll, T &pitch, T &yaw) const {
    rpy_and_jacobian(roll, pitch, yaw,
                     static_cast<mrpt::math::CMatrixDouble *>(nullptr));
  }
  ...
  */
  template <class MATRIXLIKE>
  void rpy_and_jacobian(T &roll, T &pitch, T &yaw,
    ...
    if (fabs(discr) > 0.49999) { // pitch = 90 deg
      pitch = 0.5 * M_PI;
      yaw = -2 * atan2(x(), r());
      roll = 0;
      if (out_dr_dq) {
      ...
      yaw =
          ::atan2(2 * (r() * z() + x() * y()), 1 - 2 * (y() * y() + z() * z()));
      pitch = ::asin(2 * discr);
      roll =
          ::atan2(2 * (r() * x() + y() * z()), 1 - 2 * (x() * x() + y() * y()));
      if (out_dr_dq) {
        // Auxiliary terms:
...
typedef CQuaternion<double>
    CQuaternionDouble; //!< A quaternion of data type "double"
typedef CQuaternion<float>
    CQuaternionFloat; //!< A quaternion of data type "float"
