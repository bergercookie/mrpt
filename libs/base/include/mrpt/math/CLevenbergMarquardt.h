...
public:
   ...
   * function which evaluates the error of a given point in the solution space.
    *  \param x The state point under examination.
    *  \param y The same object passed to "execute" as the parameter
   * "userParam".
    *  \param out The vector of (non-squared) errors, of the average square root
   * error, for the given "x". The functor code must set the size of this
   * vector.
    */
  typedef void (*TFunctorEval)(const VECTORTYPE &x, const USERPARAM &y,
                               VECTORTYPE &out);
   ...
   * Euclidean addition "x_new = x_old + x_incr" by any other operation.
    */
  typedef void (*TFunctorIncrement)(VECTORTYPE &x_new, const VECTORTYPE &x_old,
  ...
  struct TResultInfo {
    ...
    VECTORTYPE last_err_vector; //!< The last error vector returned by the
                                //!user-provided functor.
    matrix_t path; //!< Each row is the optimized value at each iteration.

    /** This matrix can be used to obtain an estimate of the optimal parameters
     * covariance matrix:
      *  \f[ COV = H M H^\top \f]
  ...
  /** Executes the LM-method, with derivatives estimated from
    *  \a functor is a user-provided function which takes as input two vectors,
    *in this order:
    ...
    *measured dimension, so the sum of the square of that output is the overall
    *square error.
    *
    ...
    *optimizations.
    */
  void execute(VECTORTYPE &out_optimal_x, const VECTORTYPE &x0,
               TFunctorEval functor, const VECTORTYPE &increments,
               const USERPARAM &userParam, TResultInfo &out_info,
...
typedef CLevenbergMarquardtTempl<mrpt::math::CVectorDouble>
    CLevenbergMarquardt; //!< The default name for the LM class is an
