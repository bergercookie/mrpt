 ...
 * but this will not raise any invalid memory access.
  ...
  */
template <class T> class CSparseMatrixTemplate {
  // Public typedefs
public:
  /**
...
public:
  ...
  /**
    * Returns an iterator which points to the starting point of the matrix. It's
   * a const_iterator, so that the usar isn't able to modify the matrix content
  ...
  /**
    * Returns an iterator which points to the end of the matrix, and can be used
   * to move backwards. It's a const_reverse_iterator, so that the usar isn't
   ...
   * although it's the upper limit of the matrix since it's a reverse iterator.
   * Also, it's a const_reverse_iterator, so that the usar isn't able to modify
   * the matrix content into an invalid state.
    * \sa begin,end,rbegin
    */
  inline const_reverse_iterator rend() const { return objectList.rend(); }
  /**
    * Inserts a full row into the matrix. The third argument is used to specify
   * a null object (which won't be inserted, since the matrix is sparse).
    * \sa getRow
    * \throw std::logic_error on out of range or wrong sized vector.
    */
  void setRow(size_t nRow, const std::vector<T> &vec,
    ...
    size_t N = vec.size();
    if (N != mColumns)
      throw std::logic_error("Wrong-sized vector");
   ...
   * sparse).
    ...
    * \throw std::logic_error on out of range or wrong sized vector.
    */
  void setColumn(size_t nCol, const std::vector<T> &vec,
                 const T &nullObject = T()) {
#if defined(_DEBUG) || (MRPT_ALWAYS_CHECKS_DEBUG_MATRICES)
    if (nCol >= mColumns)
      throw std::logic_error("Out of range");
  ...
  void resize(size_t nRows, size_t nCols) {
    // if (mRows<0||mColumns<0) throw std::logic_error("Invalid range"); // This
    // case never happens!
  ...
  /**
    ...
    */
  CSparseMatrixTemplate<T> operator()(size_t firstRow, size_t lastRow,
                                      size_t firstColumn,
  ...
  void purge(T nullObject = T()) {
    ...
    for (const_iterator it = begin(); it != end(); ++it)
      if (it->second == nullObject)
        nulls.push_back(it->first);
    for (std::vector<std::pair<size_t, size_t>>::const_iterator it =
             nulls.begin();
         it != nulls.end(); ++it)
      objectList.erase(*it);
...
public:
  ...
  inline T operator()(size_t r, size_t c) const {
    if (c < r)
      std::swap(r, c); // Symmetrical matrix
