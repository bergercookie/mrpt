...
private:
  ...
  void pickRandomIndex(std::set<size_t> p_set, size_t p_pick,
                       vector_size_t &p_ind);

public:
  template <typename TModelFit>
  bool ransacSingleModel(const TModelFit &p_state, size_t p_kernelSize,
                         const typename TModelFit::Real &p_fitnessThreshold,
                         typename TModelFit::Model &p_bestModel,
                         vector_size_t &p_inliers);

private:
  template <typename TModelFit> struct TSpecies {
...
public:
  template <typename TModelFit>
  bool geneticSingleModel(const TModelFit &p_state, size_t p_kernelSize,
                          const typename TModelFit::Real &p_fitnessThreshold,
