...
class CMatrixTemplateNumeric
    : public Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic,
                           // Use row major storage for backward compatibility
                           // with MRPT matrices in all cases (even in column
                           // vectors!)
...
public:
  ...
  MRPT_EIGEN_DERIVED_CLASS_CTOR_OPERATOR_EQUAL(
      CMatrixTemplateNumeric) // Implements ctor and "operator =" for any other
                              // Eigen class
  ...
  */
  ...
  inline CMatrixTemplateNumeric<T> &
   ...
   * that returns false if matrices are of different sizes instead of raising an
   * assert. */
  template <typename Derived>
  ...
  /** != comparison of two matrices; it differs from default Eigen operator in
   * that returns true if matrices are of different sizes instead of raising an
   * assert. */
  template <typename Derived>
  inline bool operator!=(const Eigen::MatrixBase<Derived> &m2) const {
    return !((*this) == m2);
...
public:
  ...
  inline static void insertInContainer(CMatrixTemplateNumeric<T> &mat, size_t r,
                                       size_t c, const T &t) {
