...
struct MatrixBlockSparseCols {
  ...
  typedef typename mrpt::aligned_containers<size_t, TEntry>::map_t
      col_t; //!< Each compressed sparse column
...
private:
  ...
  /** "remapped index" is the index of some global variable, interpreted by the
   * external user of this class. */
...
public:
  ...
  /** Clear all the entries in each column (do not change the number of columns,
   * though!) \sa getColCount */
  ...
  void saveToTextFileAsDense(const std::string &filename,
   ...
   * cols; false: compressed by rows
    */
  void getAsDense(mrpt::math::CMatrixDouble &D,
                  const bool force_symmetry = false,
                  const bool is_col_compressed = true) const {
