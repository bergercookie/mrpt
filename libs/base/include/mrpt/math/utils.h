...
void BASE_IMPEXP medianFilter(const std::vector<double> &inV,
                              std::vector<double> &outV, const int &winSize,
...
/** Normalize a vector, such as its norm is the unity.
  ...
  */
template <class VEC1, class VEC2> void normalize(const VEC1 &v, VEC2 &out_v) {
 ...
 * std::deque<std::list<double> >, std::list<CArrayDouble<5> >, etc. etc.
        *  - Output is the sequence:  data[0][idx],data[1][idx],data[2][idx],
 * etc..
        *
        *  For the sake of generality, this function does NOT check the limits
 * in the number of column, unless it's implemented in the [] operator of each
 * of the "rows".
        */
template <class VECTOR_OF_VECTORS, class VECTORLIKE>
inline void extractColumnFromVectorOfVectors(const size_t colIndex,
...
/** Round up to the nearest power of two of a given number
  */
template <class T> T round2up(T val) {
  T n = 1;
  while (n < val) {
    n <<= 1;
    if (n <= 1)
 ...
 * type of the numbers.
  ...
  *  \endcode
  */
template <size_t N, typename T> std::vector<T> make_vector(const T val1, ...) {
  MRPT_COMPILE_TIME_ASSERT(N > 0)
  std::vector<T> ret;
  ret.reserve(N);
...
bool saveEigenSparseTripletsToFile(const std::string &sFile,
                                   std::vector<TRIPLET> &tri) {
#if defined(_MSC_VER) &&                                                       \
    (_MSC_VER >= 1400) // Use a secure version in Visual Studio 2005+
  FILE *f;
  if (0 != ::fopen_s(&f, sFile.c_str(), "wt"))
    f = nullptr;
#else
