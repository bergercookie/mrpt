...
class CMatrixFixedNumeric
    : public Eigen::Matrix<
          ...
          Eigen::AutoAlign | ((NCOLS == 1 && NROWS != 1) ? Eigen::ColMajor
                                                         : Eigen::RowMajor)> {
public:
...
public:
  ...
  inline static void insertInContainer(CMatrixFixedNumeric<T, D, D> &mat,
                                       size_t r, size_t c, const T &t) {
...
struct TTypeName<mrpt::math::CMatrixFixedNumeric<T, N, M>> {
  static std::string get() {
    return mrpt::format("CMatrixFixedNumeric<%s,%u,%u>",
                        TTypeName<T>::get().c_str(), (unsigned int)N,
                        (unsigned int)M);
  }
};
}
