...
bool ModelSearch::ransacSingleModel(
  ...
  while (iter < softIterLimit && iter < hardIterLimit) {
    ...
    bool update_estim_num_iters = (iter == 0); // Always update on the first
                                               // iteration, regardless of the
                                               // result (even for ninliers=0)
    ...
    if (update_estim_num_iters) {
      // Update the estimation of maxIter to pick dataset with no outliers at
      // propability p
  ...
  while (iter < p_maxIteration) {
    if (iter > 0) {
      ...
      for (; i < elderCnt + siblingCnt; i++) {
        ...
        // better elders has more chance to mate as they are removed later from
        // the list
        int r1 = rand();
        int r2 = rand();
        int p1 = r1 % se;
