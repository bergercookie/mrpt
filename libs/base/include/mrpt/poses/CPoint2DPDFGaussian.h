...
public:
  ...
  CPoint2DPDFGaussian(
      ...
      const mrpt::math::CMatrixDouble22 &init_Cov); //!< Constructor
  ...
  void getCovarianceAndMean(mrpt::math::CMatrixDouble22 &out_cov,
                            CPoint2D &mean_point) const override {
    out_cov = this->cov;
    mean_point = this->mean;
  }

  void copyFrom(const CPoint2DPDF &o) override; //!< Copy operator, translating
   ...
   * stored one in the object. Both the mean value and the covariance matrix are
   * updated correctly. */
  ...
  /** Bayesian fusion of two points gauss. distributions, then save the result
    ...
    */
  void bayesianFusion(const CPoint2DPDFGaussian &p1,
   ...
   * both PDF.
    ...
    *  , with \f$ D^2 \f$ being the square Mahalanobis distance between the two
   * pdfs.
  ...
  void drawSingleSample(
      CPoint2D &outSample) const override; //!< Draw a sample from the pdf
   ...
   * SOGs will be dropped to reduce the number of modes in the output.
    */
  void bayesianFusion(const CPoint2DPDF &p1, const CPoint2DPDF &p2,
                      const double &minMahalanobisDistToDrop = 0) override;

  double mahalanobisDistanceTo(const CPoint2DPDFGaussian &other)
      const; //!< Returns the Mahalanobis distance from this PDF to another PDF,
             //!that is, it's evaluation at (0,0,0)
  double mahalanobisDistanceToPoint(const double x, const double y)
      const; //!< Returns the Mahalanobis distance from this PDF to some point
