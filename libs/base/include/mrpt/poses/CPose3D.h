...
public:
  mrpt::math::CArrayDouble<3> m_coords; //!< The translation vector [x,y,z]
                                        //!access directly or with x(), y(), z()
                                        //!setter/getter methods.
protected:
  mrpt::math::CMatrixDouble33 m_ROT; //!< The 3x3 rotation matrix, access with
                                     //!getRotationMatrix(), setRotationMatrix()
                                     //!(It's not safe to set this field as
                                     //!public)

  mutable bool m_ypr_uptodate; //!< Whether yaw/pitch/roll members are
                               //!up-to-date since the last rotation matrix
                               //!update.
  mutable double m_yaw, m_pitch, m_roll; //!< These variables are updated every
                                         //!time that the object rotation matrix
                                         //!is modified (construction, loading
                                         //!from values, pose composition, etc )
...
public:
  ...
  CPose3D(const double x, const double y, const double z, const double yaw = 0,
  ...
  //! \overload
  inline CPose3D(const mrpt::math::CMatrixDouble33 &rot,
  ...
  */
  ...
  CPose3D(const mrpt::math::CQuaternionDouble &q, const double x,
  ...
  /** Constructor from an array with these 12 elements: [r11 r21 r31 r12 r22 r32
  ...
  /** Computes the spherical coordinates of a 3D point as seen from the 6D pose
   * specified by this object. For the coordinate system see the top of this
   * page. */
  void sphericalCoordinates(const mrpt::math::TPoint3D &point,
                            double &out_range, double &out_yaw,
  ...
  /** An alternative, slightly more efficient way of doing \f$ G = P \oplus L
   * \f$ with G and L being 3D points and P this 6D pose.
    * \note local_point is passed by value to allow global and local point to be
   * the same variable
    */
  inline void composePoint(const mrpt::math::TPoint3D &local_point,
                           mrpt::math::TPoint3D &global_point) const {
    composePoint(local_point.x, local_point.y, local_point.z, global_point.x,
  ...
  /** An alternative, slightly more efficient way of doing \f$ G = P \oplus L
   * \f$ with G and L being 3D points and P this 6D pose.  */
  inline void composePoint(double lx, double ly, double lz, float &gx,
  ...
  /** \overload */
  inline void inverseComposePoint(const mrpt::math::TPoint3D &g,
                                  mrpt::math::TPoint3D &l) const {
  ...
  inline void inverseComposePoint(const mrpt::math::TPoint2D &g,
  ...
  /** Rebuild the internal matrix & update the yaw/pitch/roll angles within the
   * ]-PI,PI] range (Must be called after using addComponents)
    * \sa addComponents
    */
  void normalizeAngles();

  /** Scalar multiplication of x,y,z,yaw,pitch & roll (angles will be wrapped to
   * the ]-pi,pi] interval). */
  ...
  void setFromValues(const double x0, const double y0, const double z0,
                     const double yaw = 0, const double pitch = 0,
                     const double roll = 0);

  /** Set the pose from a 3D position (meters) and a quaternion, stored as [x y
   * z qr qx qy qz] in a 7-element vector.
  ...
  /** Set the 3 angles of the 3D pose (in radians) - This method recomputes the
   * internal rotation coordinates matrix.
    * \sa getYawPitchRoll, setFromValues
    */
  inline void setYawPitchRoll(const double yaw_, const double pitch_,
                              const double roll_) {
  ...
  /** Returns the three angles (yaw, pitch, roll), in radians, from the rotation
   * matrix.
  ...
  // CPose3D CANNOT have a write [] operator, since it'd leave the object in an
  // inconsistent state (outdated rotation matrix).
  // Use setFromValues() instead.
  // inline double &operator[](unsigned int i)

  /** Returns a human-readable textual representation of the object (eg: "[x y z
  ...
  void asString(std::string &s) const {
    ...
    updateYawPitchRoll();
    s = mrpt::format("[%f %f %f %f %f %f]", m_coords[0], m_coords[1],
                     m_coords[2], RAD2DEG(m_yaw), RAD2DEG(m_pitch),
                     RAD2DEG(m_roll));
  }
  ...
  void fromString(const std::string &s) {
    ...
    ASSERTMSG_(mrpt::math::size(m, 1) == 1 && mrpt::math::size(m, 2) == 6,
               "Wrong size of vector in ::fromString");
    this->setFromValues(m.get_unsafe(0, 0), m.get_unsafe(0, 1),
                        m.get_unsafe(0, 2), DEG2RAD(m.get_unsafe(0, 3)),
  ...
  /** \overload */
  static void exp(const mrpt::math::CArrayNumeric<double, 6> &vect,
                  CPose3D &out_pose, bool pseudo_exponential = false);

  /** Exponentiate a vector in the Lie algebra to generate a new SO(3) (a 3x3
   * rotation matrix). */
  static mrpt::math::CMatrixDouble33
  exp_rotation(const mrpt::math::CArrayNumeric<double, 3> &vect);

  /** Take the logarithm of the 3x4 matrix defined by this pose, generating the
   * corresponding vector in the SE(3) Lie Algebra.
    * \sa ln_jacob
    */
   ...
   * evaluated at a given 3x3 rotation matrix R.
    * \sa ln, ln_jacob
    */
  static void ln_rot_jacob(const mrpt::math::CMatrixDouble33 &R,
  ...
  static void jacob_dexpeD_de(const CPose3D &D,
   ...
   * http://ingmec.ual.es/~jlblanco/papers/jlblanco2010geometry3D_techrep.pdf */
  static void jacob_dAexpeD_de(const CPose3D &A, const CPose3D &D,
                               Eigen::Matrix<double, 12, 6> &jacob);
  ...
  typedef CPose3D type_value; //!< Used to emulate CPosePDF types, for example,
                              //!in mrpt::graphs::CNetworkOfPoses
  enum { is_3D_val = 1 };
  ...
  static inline void resize(const size_t n) {
    if (n != static_size)
      throw std::logic_error(format("Try to change the size of CPose3D to %u.",
