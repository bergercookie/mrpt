 ...
 * \ingroup poses_grp
 */
template <class DERIVEDCLASS> class CPoint : public CPoseOrPoint<DERIVEDCLASS> {
public:
   ...
   * operators in classes derived from "CPose"
    */
  template <class OTHERCLASS> inline void AddComponents(const OTHERCLASS &b) {
    const int dims = std::min(size_t(DERIVEDCLASS::static_size),
  ...
  */
  void asString(std::string &s) const {
    s = (!DERIVEDCLASS::is3DPoseOrPoint())
            ? mrpt::format("[%f %f]",
                           static_cast<const DERIVEDCLASS *>(this)->x(),
...
template <class DERIVEDCLASS>
bool operator==(const CPoint<DERIVEDCLASS> &p1,
