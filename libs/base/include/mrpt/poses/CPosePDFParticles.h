...
public:
  void clear(); //!< Free all the memory associated to m_particles, and set the
                //!number of parts = 0
   ...
   * \param particlesCount If this is set to 0 the number of m_particles remains
   * unchanged.
   ...
   * unchanged.
    ...
    */
  void resetUniform(const double &x_min, const double &x_max,
   ...
   * spread. Must contains at least one pose.
    * \param[in] num_particles_per_pose Number of particles to be spread around
   * each of the "spots" in list_poses. Must be >=1.
    *
   ...
   * "initial uncertainty".
    ...
    */
  void resetAroundSetOfPoses(const std::vector<mrpt::math::TPose2D> &list_poses,
                             const size_t num_particles_per_pose,
                             const double spread_x, const double spread_y,
   ...
   * window.
    ...
    */
  double evaluatePDF_parzen(const double &x, const double &y, const double &phi,
   ...
   * of the PDF as reconstructed by a Parzen window.
    ...
    */
  void saveParzenPDFToTextFile(const char *fileName, const double &x_min,
                               const double &x_max, const double &y_min,
                               const double &y_max, const double &phi,
