 ...
 * angle: the angle from the positive X angle.
...
public:
    ...
    if (b.is3DPoseOrPoint()) {
      if (is3DPoseOrPoint())
        return square(x() - b.x()) + square(y() - b.y()) +
               square(static_cast<const DERIVEDCLASS *>(this)->m_coords[2] -
  ...
  /** Returns the 2D distance from this pose/point to a 2D point (ignores Z, if
   * it exists). */
  ...
  */
  inline void getInverseHomogeneousMatrix(mrpt::math::CMatrixDouble44 &out_HM)
      const { // Get current HM & inverse in-place:
