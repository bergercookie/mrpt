...
public:
  ...
  void getMean(CPoint3D &p) const override; //!< Returns an estimate of the
                                            //!point, (the mean, or mathematical
  ...
  void getCovarianceAndMean(mrpt::math::CMatrixDouble33 &cov,
                            CPoint3D &mean_point) const override;
   ...
   * This is implemented as the integral from -inf to +inf of the product of
   * both PDF.
    * The resulting number is in the range [0,1]. This versions ignores the "z"
   * coordinate.
    *
    *  Note that the resulting value is in fact
    *  \f[ exp( -\frac{1}{2} D^2 ) \f]
  ...
  double mahalanobisDistanceTo(const CPointPDFGaussian &other,
