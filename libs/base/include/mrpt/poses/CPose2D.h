...
public:
  ...
  void getRotationMatrix(mrpt::math::CMatrixDouble22 &R)
      const; //!< Returns the SE(2) 2x2 rotation matrix
  void getRotationMatrix(mrpt::math::CMatrixDouble33 &R) const; //!< Returns the
                                                                //!equivalent
                                                                //!SE(3) 3x3
                                                                //!rotation
                                                                //!matrix, with
  ...
  typedef CPose2D type_value; //!< Used to emulate CPosePDF types, for example,
  ...
  static inline void resize(const size_t n) {
    if (n != static_size)
      throw std::logic_error(format("Try to change the size of CPose2D to %u.",
...
mrpt::math::TPoint2D BASE_IMPEXP operator+(
    const CPose2D &pose,
    const mrpt::math::TPoint2D &pnt); //!< Compose a 2D point from a new
                                      //!coordinate base given by a 2D pose.

bool BASE_IMPEXP operator==(const CPose2D &p1, const CPose2D &p2);
bool BASE_IMPEXP operator!=(const CPose2D &p1, const CPose2D &p2);

typedef mrpt::aligned_containers<CPose2D>::vector_t
    StdVector_CPose2D; //!< Eigen aligment-compatible container
typedef mrpt::aligned_containers<CPose2D>::deque_t
    StdDeque_CPose2D; //!< Eigen aligment-compatible container
