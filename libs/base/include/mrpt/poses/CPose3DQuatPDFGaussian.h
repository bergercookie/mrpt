...
public:
  ...
  /** Constructor which left all the member uninitialized, for using when speed
   * is critical - as argument, use UNINITIALIZED_QUATERNION. */
  CPose3DQuatPDFGaussian(
  ...
  void getMean(CPose3DQuat &mean_pose) const override; //!< Returns an estimate
                                                       //!of the pose, (the
                                                       //!mean, or mathematical
                                                       //!expectation of the
                                                       //!PDF). \sa
                                                       //!getCovariance
  void getCovarianceAndMean(
      mrpt::math::CMatrixDouble77 &cov,
      CPose3DQuat &mean_point) const override; //!< Returns an estimate of the
                                               //!pose covariance matrix (7x7
                                               //!cov matrix) and the mean, both
                                               //!at once. \sa getMean
  void copyFrom(const CPose3DQuatPDF &o) override; //!< Copy operator,
                                                   //!translating if necesary
                                                   //!(for example, between
                                                   //!particles and gaussian
                                                   //!representations)
  void copyFrom(const CPosePDF &o); //!< Copy operator, translating if necesary
                                    //!(for example, between particles and
                                    //!gaussian representations)
  void copyFrom(const CPose3DPDFGaussian &o); //!< Copy operator, translating if
                                              //!necesary (for example, between
                                              //!particles and gaussian
                                              //!representations)
  void saveToTextFile(const std::string &file) const override; //!< Save the PDF
                                                               //!to a text
                                                               //!file,
                                                               //!containing the
                                                               //!3D pose in the
  ...
  void drawSingleSample(CPose3DQuat &outPart)
  ...
  void drawManySamples(size_t N,
      ...
      const override; //!< Draws a number of samples from the distribution, and
                      //!saves as a list of 1x7 vectors, where each row contains
                      //!a (x,y,z,qr,qx,qy,qz) datum.
  ...
  void operator+=(const CPose3DQuat &Ap); //!< Makes: thisPDF = thisPDF + Ap,
                                          //!where "+" is pose composition (both
                                          //!the mean, and the covariance matrix
                                          //!are updated).
  void operator+=(const CPose3DQuatPDFGaussian &Ap); //!< Makes: thisPDF =
                                                     //!thisPDF + Ap, where "+"
                                                     //!is pose composition
                                                     //!(both the mean, and the
                                                     //!covariance matrix are
                                                     //!updated) (see formulas
                                                     //!in
  ...
  double evaluatePDF(
      const CPose3DQuat &x) const; //!< Evaluates the PDF at a given point.

  double evaluateNormalizedPDF(const CPose3DQuat &x) const; //!< Evaluates the
                                                            //!ratio PDF(x) /
                                                            //!PDF(MEAN), that
                                                            //!is, the
                                                            //!normalized PDF in
...
bool BASE_IMPEXP operator==(const CPose3DQuatPDFGaussian &p1,
                            const CPose3DQuatPDFGaussian &p2);
/** Pose composition for two 3D pose Gaussians  \sa
 * CPose3DQuatPDFGaussian::operator += */
CPose3DQuatPDFGaussian BASE_IMPEXP operator+(const CPose3DQuatPDFGaussian &x,
...
std::ostream BASE_IMPEXP &operator<<(std::ostream &out,
...
/** If set to true (default), a Scaled Unscented Transform is used instead of a
  ...
  *		- CPose3DQuatPDFGaussianInf::copyFrom(const CPose3DPDFGaussianInf
  *&o)
  */
