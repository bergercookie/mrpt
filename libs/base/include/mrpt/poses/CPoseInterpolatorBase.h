...
* \ingroup interpolation_grp poses_grp
*/
template <int DIM> class BASE_IMPEXP CPoseInterpolatorBase {
public:
  ...
  typedef typename mrpt::poses::SE_traits<DIM>::lightweight_pose_t
      pose_t; //!< TPose2D or TPose3D
  typedef typename mrpt::poses::SE_traits<DIM>::pose_t
      cpose_t; //!< CPose2D or CPose3D
  typedef typename mrpt::poses::SE_traits<DIM>::point_t
      point_t; //!< TPoint2D or TPoint3D
  ...
  void insert(mrpt::system::TTimeStamp t,
              const cpose_t &p); //!< Overload (slower)
   ...
   * the interpolation.
    ...
    */
  pose_t &interpolate(mrpt::system::TTimeStamp t, pose_t &out_interp,
                      bool &out_valid_interp) const;
  cpose_t &interpolate(mrpt::system::TTimeStamp t, cpose_t &out_interp,
                       bool &out_valid_interp) const; //!< \overload (slower)
  ...
  double getMaxTimeInterpolation(); //!< Set value of the maximum time to
  ...
  * \return true if pose was found, false otherwise */
  bool getPreviousPoseWithMinDistance(const mrpt::system::TTimeStamp &t,
                                      double distance, pose_t &out_pose);
  bool
  getPreviousPoseWithMinDistance(const mrpt::system::TTimeStamp &t,
                                 double distance,
                                 cpose_t &out_pose); //!< \overload (slower)
  ...
  /** Saves the points in the interpolator to a text file, with the same format
   * that saveToTextFile, but interpolating the path with the given period in
   * seconds.
    * \sa loadFromTextFile
    * \return true on success, false on any error.
    */
  ...
  void setInterpolationMethod(
      TInterpolatorMethod method); //!< Change the method used to interpolate
                                   //!the robot path. The default method at
                                   //!construction is "imSpline". \sa
                                   //!getInterpolationMethod()
  TInterpolatorMethod
  getInterpolationMethod() const; //!< Returns the currently set interpolation
                                  //!method.  \sa setInterpolationMethod()
   ...
   * interpolator. The width of the filter is set by the number of samples.
    * \param component	[IN]	The index of the component to filter: 0 (x), 1
   * (y), 2 (z), 3 (yaw), 4 (pitch) or 5 (roll)
...
protected:
  ...
  double maxTimeInterpolation; //!< Maximum time considered to interpolate. If
                               //!the difference between the desired timestamp
                               //!where to interpolate and the next timestamp
                               //!stored in the map is bigger than this value,
                               //!the interpolation will not be done.
  TInterpolatorMethod m_method;

  void impl_interpolation(const mrpt::math::CArrayDouble<4> &ts,
                          const TTimePosePair p1, const TTimePosePair p2,
                          const TTimePosePair p3, const TTimePosePair p4,
                          const TInterpolatorMethod method, double td,
...
namespace utils {
template <> struct TEnumTypeFiller<mrpt::poses::TInterpolatorMethod> {
