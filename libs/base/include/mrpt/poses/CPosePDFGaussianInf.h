...
public:
  ...
  mrpt::math::CMatrixDouble33 cov_inv; //!< The inverse of the 3x3 covariance
  ...
  /** Default constructor (mean=all zeros, inverse covariance=all zeros -> so be
   * careful!) */
  CPosePDFGaussianInf();
  ...
  /** Returns an estimate of the pose covariance matrix (3x3 cov matrix) and the
* mean, both at once.
* \sa getMean */
  ...
  virtual void
  getInformationMatrix(mrpt::math::CMatrixDouble33 &inf) const override {
    inf = cov_inv;
  }
...
*   "to project" the current pdf. Result PDF substituted the currently stored one
* in the object */
...
* in the object. */
   ...
   * \begin{array}{ccc} \cos\alpha & -\sin\alpha & 0 \\ \sin\alpha & \cos\alpha
   * & 0 \\ 0 & 0 & 1 \end{array}\right] \f$. */
  void rotateCov(const double ang);

  /** Set \f$ this = x1 \ominus x0 \f$ , computing the mean using the "-"
   * operator and the covariances through the corresponding Jacobians (For 'x0'
   * and 'x1' being independent variables!).  */
  void inverseComposition(const CPosePDFGaussianInf &x,
                          const CPosePDFGaussianInf &ref);

  /** Set \f$ this = x1 \ominus x0 \f$ , computing the mean using the "-"
   * operator and the covariances through the corresponding Jacobians (Given the
   * 3x3 cross-covariance matrix of variables x0 and x1). */
  void inverseComposition(const CPosePDFGaussianInf &x1,
...
*/
  ...
  /** Evaluates the ratio PDF(x) / PDF(MEAN), that is, the normalized PDF in the
   * range [0,1]. */
  ...
  /** Makes: thisPDF = thisPDF - Ap, where "-" is pose inverse composition (both
   * the mean, and the covariance matrix are updated) */
...
bool BASE_IMPEXP operator==(const CPosePDFGaussianInf &p1,
                            const CPosePDFGaussianInf &p2);
/** Pose compose operator: RES = A (+) B , computing both the mean and the
 * covariance */
CPosePDFGaussianInf BASE_IMPEXP operator+(const CPosePDFGaussianInf &a,
...
poses::CPosePDFGaussianInf BASE_IMPEXP operator+(
