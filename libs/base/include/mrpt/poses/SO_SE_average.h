...
public:
  ...
  void append(const double orientation_rad); //!< Adds a new orientation
                                             //!(radians) to the computation \sa
                                             //!get_average
  void append(const double orientation_rad,
              const double weight); //!< Adds a new orientation (radians) to the
                                    //!weighted-average computation \sa
                                    //!get_average
  ...
  bool enable_exception_on_undeterminate; //!< (Default=false) Set to true if
                                          //!you want to raise an exception on
...
public:
  SO_average();                          //!< Constructor
  void clear();                          //!< Resets the accumulator
  void append(const Eigen::Matrix3d &M); //!< Adds a new orientation to the
                                         //!computation \sa get_average
  void append(const Eigen::Matrix3d &M,
              const double weight); //!< Adds a new orientation to the
                                    //!weighted-average computation \sa
                                    //!get_average
                                    /** Returns the average orientation.
                                      * \exception std::logic_error If no data point were inserted.
                                      * \exception std::runtime_error Upon undeterminate average value (ie there
                                     * was a problem with the SVD) and \a enable_exception_on_undeterminate is set
                                     * to true (otherwise, the 0 orientation would be returned)
                                      * \sa append */
...
public:
  ...
  void clear(); //!< Resets the accumulator
  void append(const mrpt::poses::CPose2D
                  &p); //!< Adds a new pose to the computation \sa get_average
  void append(const mrpt::poses::CPose2D &p,
              const double weight); //!< Adds a new pose to the weighted-average
                                    //!computation \sa get_average
...
public:
  SE_average(); //!< Constructor
  void clear(); //!< Resets the accumulator
  void append(const mrpt::poses::CPose3D
                  &p); //!< Adds a new pose to the computation \sa get_average
