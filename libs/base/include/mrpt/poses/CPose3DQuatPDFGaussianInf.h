...
public:
  ...
  mrpt::math::CMatrixDouble77
      cov_inv; //!< The 7x7 information matrix (the inverse of the covariance)

  inline const CPose3DQuat &getPoseMean() const { return mean; }
  inline CPose3DQuat &getPoseMean() { return mean; }

  /** Returns an estimate of the pose, (the mean, or mathematical expectation of
   * the PDF)  \sa getCovariance */
  void getMean(CPose3DQuat &mean_pose) const override { mean_pose = mean; }
  bool isInfType() const override { return true; }

  /** Returns an estimate of the pose covariance matrix (7x7 cov matrix) and the
   * mean, both at once. \sa getMean */
  void getCovarianceAndMean(mrpt::math::CMatrixDouble77 &cov,
  ...
  /** Save the PDF to a text file, containing the 3D pose in the first line (x y
   * z qr qx qy qz), then the information matrix in the next 7 lines. */
  ...
  void operator+=(const CPose3DQuat &Ap); //!< Makes: thisPDF = thisPDF + Ap,
                                          ...
                                          //!are updated).
  void
  operator+=(const CPose3DQuatPDFGaussianInf &Ap); //!< Makes: thisPDF = thisPDF
                                                   //!+ Ap, where "+" is pose
                                                   //!composition (both the
                                                   //!mean, and the covariance
                                                   //!matrix are updated) (see
                                                   //!formulas in
                                                   //!jacobiansPoseComposition
                                                   //!).
  void operator-=(const CPose3DQuatPDFGaussianInf &Ap); //!< Makes: thisPDF =
                                                        //!thisPDF - Ap, where
                                                        //!"-" is pose inverse
                                                        //!composition (both the
                                                        //!mean, and the
