...
protected:
  void assureSymmetry(); //!< Ensures the symmetry of the covariance matrix
                         //!(eventually certain operations in the
                         //!math-coprocessor lead to non-symmetric matrixes!)
...
public:
   ...
   * preserving the total distribution.
    ...
    *  - "Kullback-Leibler Approach to Gaussian Mixture Reduction" AR Runnalls.
  ...
  void getMean(CPose2D &mean_pose) const override; //!< Returns an estimate of
                                                   //!the pose, (the mean, or
                                                   //!mathematical expectation
                                                   //!of the PDF) \sa
                                                   //!getCovariance
  void getCovarianceAndMean(mrpt::math::CMatrixDouble33 &cov,
                            CPose2D &mean_point) const override; //!< Returns an
                                                                 //!estimate of
                                                                 //!the pose
                                                                 //!covariance
                                                                 //!matrix (3x3
  ...
  void getMostLikelyCovarianceAndMean(
      mrpt::math::CMatrixDouble33 &cov,
      CPose2D &mean_point) const; //!< For the most likely Gaussian mode in the
                                  //!SOG, returns the pose covariance matrix
                                  //!(3x3 cov matrix) and the mean. \sa getMean
  void normalizeWeights(); //!< Normalize the weights in m_modes such as the
                           //!maximum log-weight is 0

  void copyFrom(const CPosePDF &o) override; //!< Copy operator, translating if
                                             //!necesary (for example, between
                                             //!particles and gaussian
                                             //!representations)
  ...
  void rotateAllCovariances(
      const double &ang); //!< Rotate all the covariance matrixes by replacing
                          //!them by \f$ \mathbf{R}~\mathbf{COV}~\mathbf{R}^t
                          //!\f$, where \f$ \mathbf{R} = \left[
                          //!\begin{array}{ccc} \cos\alpha & -\sin\alpha & 0 \\
                          //!\sin\alpha & \cos\alpha & 0 \\ 0 & 0 & 1
                          //!\end{array}\right] \f$
  void drawSingleSample(CPose2D &outPart)
      const override; //!< Draws a single sample from the distribution
  void drawManySamples(size_t N,
                       std::vector<mrpt::math::CVectorDouble> &outSamples)
      const override; //!< Draws a number of samples from the distribution, and
                      //!saves as a list of 1x3 vectors, where each row contains
                      //!a (x,y,phi) datum.
  void inverse(CPosePDF &o) const
      override; //!< Returns a new PDF such as: NEW_PDF = (0,0,0) - THIS_PDF

  void operator+=(const mrpt::poses::CPose2D &Ap); //!< Makes: thisPDF = thisPDF
                                                   //!+ Ap, where "+" is pose
                                                   //!composition (both the
                                                   //!mean, and the covariance
                                                   //!matrix are updated).

  double evaluatePDF(const mrpt::poses::CPose2D &x, bool sumOverAllPhis = false)
      const; //!< Evaluates the PDF at a given point.
  double evaluateNormalizedPDF(const mrpt::poses::CPose2D &x)
      const; //!< Evaluates the ratio PDF(x) / max_PDF(x*), that is, the
             //!normalized PDF in the range [0,1].

  /** Evaluates the PDF within a rectangular grid (and a fixed orientation) and
   * saves the result in a matrix (each row contains values for a fixed
   * y-coordinate value). */
  void evaluatePDFInArea(const double &x_min, const double &x_max,
                         const double &y_min, const double &y_max,
                         const double &resolutionXY, const double &phi,
   ...
   * object (WARNING: Currently p1 must be a mrpt::poses::CPosePDFSOG object and
   * p2 a mrpt::poses::CPosePDFGaussian object) */
  void bayesianFusion(const CPosePDF &p1, const CPosePDF &p2,
