...
public:
  ...
  mrpt::math::CMatrixDouble66
      cov_inv; //!< The inverse of the 6x6 covariance matrix
  ...
  /** Returns an estimate of the pose covariance matrix (6x6 cov matrix) and the
* mean, both at once.
* \sa getMean  */
  void getCovarianceAndMean(mrpt::math::CMatrixDouble66 &cov,
  ...
  virtual void
  getInformationMatrix(mrpt::math::CMatrixDouble66 &inf) const override {
    inf = cov_inv;
  }
...
*/
  ...
  void operator+=(const CPose3D &Ap); //!< Makes: thisPDF = thisPDF + Ap, where
                                      //!"+" is pose composition (both the mean,
                                      //!and the covariance matrix are updated)
  void operator+=(const CPose3DPDFGaussianInf &Ap); //!< Makes: thisPDF =
                                                    //!thisPDF + Ap, where "+"
                                                    //!is pose composition (both
                                                    //!the mean, and the
  ...
  void operator-=(const CPose3DPDFGaussianInf &Ap); //!< Makes: thisPDF =
                                                    ...
                                                    //!matrix are updated)
  double
  evaluatePDF(const CPose3D &x) const; //!< Evaluates the PDF at a given point
  double evaluateNormalizedPDF(const CPose3D &x) const; //!< Evaluates the ratio
                                                        //!PDF(x) / PDF(MEAN),
                                                        //!that is, the
                                                        //!normalized PDF in the
                                                        //!range [0,1]
  void getInvCovSubmatrix2D(mrpt::math::CMatrixDouble &out_cov)
      const; //!< Returns a 3x3 matrix with submatrix of the inverse covariance
             //!for the variables (x,y,yaw) only
...
bool BASE_IMPEXP operator==(const CPose3DPDFGaussianInf &p1,
                            const CPose3DPDFGaussianInf &p2);
/** Pose composition for two 3D pose Gaussians  \sa CPose3DPDFGaussian::operator
 * +=  */
CPose3DPDFGaussianInf BASE_IMPEXP operator+(const CPose3DPDFGaussianInf &x,
