...
public:
   ...
   * localization / SLAM.
    */
  void processUpdateNewPoseLocalization(const mrpt::math::TPose2D &newPose,
  ...
  void processUpdateNewOdometry(
      ...
      mrpt::system::TTimeStamp cur_tim, bool hasVelocities = false,
  ...
  * is too old or there is no valid data yet.
  ...
  */
  bool getCurrentEstimate(
      ...
      mrpt::system::TTimeStamp tim_query = mrpt::system::now()) const;
...
private:
  ...
  mrpt::math::TPose2D m_last_loc; //!< Last pose as estimated by the
                                  //!localization/SLAM subsystem.

  mrpt::math::TPose2D m_loc_odo_ref; //!< The interpolated odometry position for
                                     //!the last "m_robot_pose" (used as
                                     //!"coordinates base" for subsequent odo
                                     //!readings)

  mrpt::system::TTimeStamp m_last_odo_time;
  mrpt::math::TPose2D m_last_odo;
  mrpt::math::TTwist2D m_robot_vel_local; //!< Robot odometry-based velocity in
                                          //!a local frame of reference.

  /** An auxiliary method to extrapolate the pose of a robot located at "p" with
   * velocities (v,w) after a time delay "delta_time". */
  static void extrapolateRobotPose(const mrpt::math::TPose2D &p,
