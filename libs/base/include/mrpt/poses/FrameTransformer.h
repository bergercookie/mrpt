...
*/
template <int DIM> class BASE_IMPEXP FrameTransformerInterface {
public:
  typedef typename SE_traits<DIM>::pose_t
      pose_t; //!< This will be mapped to CPose2D (DIM=2) or CPose3D (DIM=3)
  typedef typename SE_traits<DIM>::lightweight_pose_t
      lightweight_pose_t; //!< This will be mapped to mrpt::math::TPose2D
                          //!(DIM=2) or mrpt::math::TPose3D (DIM=3)
...
public:
  ...
  virtual void sendTransform(
      ...
      const mrpt::system::TTimeStamp &timestamp = mrpt::system::now()) override;
  // See base docs
  virtual FrameLookUpStatus
  lookupTransform(const std::string &target_frame,
                  const std::string &source_frame,
  ...
  lookupTransform(const std::string &target_frame,
    ...
    FrameLookUpStatus ret = lookupTransform(target_frame, source_frame, p,
                                            query_time, timeout_secs);
    child_wrt_parent = typename base_t::pose_t(p);
    return ret;
  }

protected:
  // double m_max_extrapolation_time;  //!< for extrapolation in the past or in
  // the future [s]
  ...
  struct BASE_IMPEXP TF_TreeEdge {
    ...
    TF_TreeEdge(const typename base_t::pose_t &pose_,
