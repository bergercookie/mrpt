...
public:
  ...
  /** Default constructor, initialize translation to zeros and quaternion to no
   * rotation. */
  inline CPose3DQuat() : m_quat() {
    m_coords[0] = m_coords[1] = m_coords[2] = 0.;
  }

  /** Constructor which left all the quaternion members un-initialized, for use
   * when speed is critical; Use UNINITIALIZED_POSE as argument to this
   * constructor. */
  ...
  /** Constructor with initilization of the pose - the quaternion is normalized
   * to make sure it's unitary */
  inline CPose3DQuat(const double x, const double y, const double z,
   ...
   * qr qx qy qz]", angles in degrees.)
    * \sa fromString
    */
  void asString(std::string &s) const {
    s = mrpt::format("[%f %f %f %f %f %f %f]", m_coords[0], m_coords[1],
                     m_coords[2], m_quat[0], m_quat[1], m_quat[2], m_quat[3]);
  }
   ...
   * specified by this object.
    *  For the coordinate system see the top of this page.
            *  If the matrix pointers are not nullptr, the Jacobians will be
   * also computed for the range-yaw-pitch variables wrt the passed 3D point and
   * this 7D pose.
    */
...
public:
  typedef CPose3DQuat type_value; //!< Used to emulate CPosePDF types, for
                                  //!example, in mrpt::graphs::CNetworkOfPoses
  enum { is_3D_val = 1 };
  ...
  static inline void resize(const size_t n) {
    if (n != static_size)
      throw std::logic_error(
          format("Try to change the size of CPose3DQuat to %u.",
  ...
  public:
    ...
    inline iterator_base::difference_type operator-(const iterator &it) const {
      return m_cur_idx - it.m_cur_idx;
    }
    inline CPose3DQuat::reference
    operator[](iterator_base::difference_type off) const {
  ...
  public:
    ...
    inline iterator_base::difference_type
    operator-(const const_iterator &it) const {
      return m_cur_idx - it.m_cur_idx;
    }
    inline CPose3DQuat::const_reference
    operator[](iterator_base::difference_type off) const {
      return (*m_obj)[m_cur_idx + off];
    }
...
/**  Computes the 3D point L such as \f$ L = G \ominus this \f$.  \sa
 * inverseComposePoint    */
mrpt::math::TPoint3D BASE_IMPEXP operator-(const mrpt::math::TPoint3D &G,
                                           const CPose3DQuat &p);
