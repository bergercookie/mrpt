...
public:
  ...
  /** Return the Gaussian mode with the highest likelihood (or an empty Gaussian
   * if there are no modes in this SOG) */
  ...
  void copyFrom(const CPointPDF &o) override; //!< Copy operator, translating if
   ...
   * matrix (each row contains values for a fixed y-coordinate value).
    */
  void evaluatePDFInArea(float x_min, float x_max, float y_min, float y_max,
                         float resolutionXY, float z,
