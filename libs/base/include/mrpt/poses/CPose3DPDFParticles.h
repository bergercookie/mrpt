...
public:
  ...
  void copyFrom(const CPose3DPDF &o) override; //!< Copy operator, translating
                                               //!if necesary (for example,
                                               //!between m_particles and
  ...
  void getMean(CPose3D &mean_pose) const override; //!< Returns an estimate of
                                                   //!the pose, (the mean, or
                                                   //!mathematical expectation
                                                   //!of the PDF), computed as a
                                                   //!weighted average over all
                                                   //!m_particles. \sa
                                                   //!getCovariance
  void getCovarianceAndMean(mrpt::math::CMatrixDouble66 &cov,
                            CPose3D &mean_point) const override; //!< Returns an
                                                                 //!estimate of
                                                                 //!the pose
                                                                 //!covariance
                                                                 //!matrix (6x6
  ...
  CPose3D
  getParticlePose(int i) const; //!< Returns the pose of the i'th particle

  void saveToTextFile(const std::string &file) const override; //!< Save PDF's
                                                               //!m_particles to
                                                               //!a text file.
                                                               //!In each line
                                                               //!it will go: "x
  ...
  void drawSingleSample(CPose3D &outPart) const override; //!< Draws a single
                                                          //!sample from the
                                                          //!distribution
                                                          //!(WARNING: weights
                                                          //!are assumed to be
  ...
  void drawManySamples(size_t N,
      ...
      const override; //!< Draws a number of samples from the distribution, and
                      //!saves as a list of 1x6 vectors, where each row contains
                      //!a (x,y,phi) datum.
  void operator+=(const CPose3D &Ap);  //!< Appends (pose-composition) a given
                                       //!pose "p" to each particle
  void append(CPose3DPDFParticles &o); //!< Appends (add to the list) a set of
                                       //!m_particles to the existing ones, and
                                       //!then normalize weights.
  void inverse(CPose3DPDF &o) const
      override; //!< Returns a new PDF such as: NEW_PDF = (0,0,0) - THIS_PDF
  CPose3D getMostLikelyParticle()
      const; //!< Returns the particle with the highest weight.
  void bayesianFusion(const CPose3DPDF &p1,
                      const CPose3DPDF &p2) override; //!< Bayesian fusion
