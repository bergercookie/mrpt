 ...
 */
template <class T> class CPose2DGridTemplate {
protected:
...
public:
  ...
  /** Default constructor:
    */
  CPose2DGridTemplate(double xMin = -1.0f, double xMax = 1.0f,
                      double yMin = -1.0f, double yMax = 1.0f,
      ...
      : m_xMin(), m_xMax(), m_yMin(), m_yMax(), m_phiMin(), m_phiMax(),
        m_resolutionXY(), m_resolutionPhi(), m_sizeX(), m_sizeY(), m_sizePhi(),
        m_sizeXY(), m_idxLeftX(), m_idxLeftY(), m_idxLeftPhi(), m_data() {
    setSize(xMin, xMax, yMin, yMax, resolutionXY, resolutionPhi, phiMin,
            phiMax);
  }
