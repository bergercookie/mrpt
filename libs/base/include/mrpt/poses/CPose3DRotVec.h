...
public:
  ...
  inline CPose3DRotVec(const double vx, const double vy, const double vz,
  ...
  CPose3DRotVec(const mrpt::math::CQuaternionDouble &q, const double x,
  ...
  inline void
  setFromTransformationMatrix(const mrpt::math::CMatrixDouble44 &m) {
  ...
  void setFromXYZAndAngles(const double x, const double y, const double z,
                           const double yaw = 0, const double pitch = 0,
                           const double roll = 0);

  /** Computes the spherical coordinates of a 3D point as seen from the 6D pose
   * specified by this object. For the coordinate system see
   * mrpt::poses::CPose3D */
  ...
  /** Returns a human-readable textual representation of the object: "[x y z rx
   * ry rz]"
    * \sa fromString
    */
  void asString(std::string &s) const {
    s = mrpt::format("[%f %f %f %f %f %f]", m_coords[0], m_coords[1],
                     m_coords[2], m_rotvec[0], m_rotvec[1], m_rotvec[2]);
  }
  ...
  typedef CPose3DRotVec type_value; //!< Used to emulate CPosePDF types, for
  ...
  static inline void resize(const size_t n) {
    if (n != static_size)
      throw std::logic_error(
          format("Try to change the size of CPose3DRotVec to %u.",
