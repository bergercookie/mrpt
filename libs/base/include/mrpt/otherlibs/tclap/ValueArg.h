...
ValueArg<T>::ValueArg(const std::string &flag, const std::string &name,
    ...
    : Arg(flag, name, desc, req, true, v), _value(val), _typeDesc(typeDesc),
      _constraint(nullptr) {}
...
ValueArg<T>::ValueArg(const std::string &flag, const std::string &name,
    ...
    : Arg(flag, name, desc, req, true, v), _value(val),
...
template <class T> void ValueArg<T>::_extractValue(const std::string &val) {
  ...
  if (err == VALUE_ARG_HELPER::EXTRACT_FAILURE)
    throw(ArgParseException(
        "Couldn't read argument value from string '" + val + "'", toString()));

  if (err == VALUE_ARG_HELPER::EXTRACT_TOO_MANY)
    throw(ArgParseException("More than one valid value parsed from string '" +
                                val + "'",
                            toString()));
