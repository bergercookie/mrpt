 ...
 * \sa system::tokenize
 */
char BASE_IMPEXP *strtok(char *str, const char *strDelimit,
 ...
 */
void BASE_IMPEXP tokenize(const std::string &inString,
 ...
 * http://www.codeguru.com/cpp/misc/misc/multi-lingualsupport/article.php/c10451/.
  */
void BASE_IMPEXP decodeUTF8(const std::string &strUTF8,
 ...
 * \sa decodeBase64  */
void BASE_IMPEXP encodeBase64(const vector_byte &inputData,
 ...
 */
bool BASE_IMPEXP decodeBase64(const std::string &inString,
                              vector_byte &outData);
...
std::string BASE_IMPEXP unitsFormat(const double val, int nDecimalDigits = 2,
                                    bool middle_space = true);

/** Enlarge the string with spaces up to the given length. */
std::string BASE_IMPEXP rightPad(const std::string &str, const size_t total_len,
...
template <typename T> T str2num(std::string const &value) {
...
template <typename T> std::string num2str(T const &value) {
  std::stringstream sin;
  sin << value;
  return sin.str();
}
