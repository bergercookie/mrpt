...
/** A generic system for versatile memory pooling.
  ...
  * \ingroup mrpt_memory
  */
template <class DATA_PARAMS, class POOLABLE_DATA> class CGenericMemoryPool {
private:
  typedef std::list<std::pair<DATA_PARAMS, POOLABLE_DATA *>> TList;
  TList m_pool;
  std::mutex m_pool_cs;
  size_t m_maxPoolEntries;
  bool &m_was_destroyed; //!< With this trick we get rid of the "global
...
public:
  ...
  /** Construct-on-first-use (~singleton) pattern: Return the unique instance of
   * this class for a given template arguments,
    *  or nullptr if it was once created but it's been destroyed (which means
   * we're in the program global destruction phase).
    */
  static CGenericMemoryPool<DATA_PARAMS, POOLABLE_DATA> *
  getInstance(const size_t max_pool_entries = 5) {
    static bool was_destroyed = false;
    static CGenericMemoryPool<DATA_PARAMS, POOLABLE_DATA> inst(max_pool_entries,
                                                               was_destroyed);
   ...
   * params.
    *  Notice that the decision on the suitability of each pool'ed block is done
   * by DATA_PARAMS::isSuitable().
