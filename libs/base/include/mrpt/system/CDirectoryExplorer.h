...
public:
   ...
   * of files, which
    ...
    * \param path The path to examine (IT MUST BE A DIRECTORY), e.g "d:\temp\",
   * or "/usr/include/"
   ...
   * types do you want in the list (These values are platform-independent).
    ...
    * \sa sortByName
    */
  static void explore(const std::string &path, const unsigned long mask,
   ...
   * (without case) with the given one.
    ...
    */
  static void filterByExtension(TFileInfoList &lstFiles,
