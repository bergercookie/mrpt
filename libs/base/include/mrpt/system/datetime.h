 ...
 * UTC time or local time
  ...
  */
void BASE_IMPEXP timestampToParts(TTimeStamp t, TTimeParts &p,
...
double BASE_IMPEXP timeDifference(
    ...
    const mrpt::system::TTimeStamp t_later); //!< Returns the time difference
                                             //!from t1 to t2 (positive if t2 is
                                             //!posterior to t1), in seconds \sa
                                             //!secondsToTimestamp
...
mrpt::system::TTimeStamp BASE_IMPEXP
timestampAdd(const mrpt::system::TTimeStamp tim,
             const double num_seconds); //!< Shifts a timestamp the given amount
