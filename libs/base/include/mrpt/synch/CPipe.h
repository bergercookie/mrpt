...
public:
  ...
  static void initializePipe(CPipeReadEndPoint &outReadPipe,
...
public:
  ...
  unsigned int timeout_read_start_us;   //!< (Default=0) Timeout for read
                                        //!operations: microseconds (us) to wait
                                        //!for the first byte. 0 means infinite
                                        //!timeout.
  unsigned int timeout_read_between_us; //!< (Default=0) Timeout between burst
                                        //!reads operations: microseconds (us)
                                        //!to wait between two partial reads
                                        //!inside one large read. 0 means
                                        //!infinite timeout.

  /** Returns false if the pipe was closed due to some error. */
  inline bool isOpen() const { return m_pipe_file != 0; }

  /** Closes the pipe (normally not needed to be called by users, automatically
   * done at destructor) */
  void close();
...
protected:
  ...
  virtual uint64_t Seek(uint64_t Offset,
      ...
      override; //!< Without effect in this class
  virtual uint64_t
  getTotalBytesCount() override;           //!< Without effect in this class
  virtual uint64_t getPosition() override; //!< Without effect in this class
};                                         // end of CPipeBaseEndPoint
static_assert(!std::is_copy_constructible<CPipeBaseEndPoint>::value &&
...
private:
  CPipeReadEndPoint();
  void
  WriteBuffer(const void *Buffer,
              size_t Count); //!< Hide the write method in this read-only pipe.
...
private:
  CPipeWriteEndPoint();
  size_t
  ReadBuffer(void *Buffer,
             size_t Count); //!< Hide the read method in this write-only pipe.
