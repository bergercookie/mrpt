...
/*---------------------------------------------------------------
                                                compress
---------------------------------------------------------------*/
void mrpt::compress::zip::compress(void *inData, size_t inDataSize,
  ...
  ret = ::compress(&outData[0], &resSize, (unsigned char *)inData,
...
void mrpt::compress::zip::compress(const std::vector<unsigned char> &inData,
  ...
  resSize = (unsigned long)outData.size();
  ret = ::compress(&outData[0], &resSize, &inData[0],
...
/*---------------------------------------------------------------
                                                decompress
---------------------------------------------------------------*/
void mrpt::compress::zip::decompress(void *inData, size_t inDataSize,
                                     std::vector<unsigned char> &outData,
  ...
  ret = ::uncompress(&outData[0], &actualOutSize, (unsigned char *)inData,
                     (unsigned long)inDataSize);
...
void mrpt::compress::zip::decompress(void *inData, size_t inDataSize,
  ...
  ret = ::uncompress((unsigned char *)outData, &actualOutSize,
                     (unsigned char *)inData, (unsigned long)inDataSize);
...
/*---------------------------------------------------------------
                                                decompress
---------------------------------------------------------------*/
void mrpt::compress::zip::decompress(CStream &inStream, size_t inDataSize,
                                     void *outData, size_t outDataBufferSize,
                                     size_t &outDataActualSize) {
  int ret = 0;
  MRPT_START
  ...
  ret = ::uncompress((unsigned char *)outData, &actualOutSize, &inData[0],
                     (unsigned long)inDataSize);
...
// Create an anonymous pipe for writing the data to:
  ...
  try {
    // Write as gz
    {
      gzFile f = gzopen(pipe_file_name.c_str(),
                        format("wb%i", compress_level).c_str());
      if (f) {
