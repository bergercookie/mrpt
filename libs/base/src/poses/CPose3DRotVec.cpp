...
void CPose3DRotVec::setFromXYZAndAngles(const double x, const double y,
...
CArrayDouble<3>
CPose3DRotVec::rotVecFromRotMat(const math::CMatrixDouble44 &m) {
  // go through cpose3d
  CArrayDouble<3> out;
  CPose3D aux(m);
  out = aux.ln_rotation();
...
---------------------------------------------------------------*/
void CPose3DRotVec::sphericalCoordinates(const TPoint3D &point,
...
CPoint3D CPose3DRotVec::operator+(const CPoint3D &b) const {
  ...
  this->composePoint(b.m_coords[0], b.m_coords[1], b.m_coords[2],
                     outPoint.m_coords[0], outPoint.m_coords[1],
                     outPoint.m_coords[2]);
...
CPoint3D CPose3DRotVec::operator+(const CPoint2D &b) const {
  ...
  this->composePoint(b.m_coords[0], b.m_coords[1], 0, outPoint.m_coords[0],
...
void CPose3DRotVec::toQuatXYZ(CPose3DQuat &q) const {
  ...
  const double a = sqrt(this->m_rotvec[0] * this->m_rotvec[0] +
  ...
  if (a1 < 0.01 && a2 < 0.01) {
    ...
    if (out_jacobian_drvtC_drvtA || out_jacobian_drvtC_drvtB) {
      ...
      if (out_jacobian_drvtC_drvtB) // jacobian wrt B
      {
        ...
        (*out_jacobian_drvtC_drvtB)(3, 3) = (*out_jacobian_drvtC_drvtB)(4, 4) =
...
// Rotation part:
  ...
  if (out_jacobian_drvtC_drvtA || out_jacobian_drvtC_drvtB) {
    ...
    if (out_jacobian_drvtC_drvtB) {
      ...
      MRPT_ALIGN16 const double aux_valsQA[] = {qAr, -qAx, -qAy, -qAz, qAx, qAr,
                                                qAz, -qAy, qAy,  -qAz, qAr, qAx,
                                                qAz, qAy,  -qAx, qAr};
