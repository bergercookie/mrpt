...
void CPose3DInterpolator::writeToStream(mrpt::utils::CStream &out,
                                        int *version) const {
  if (version)
    *version = 1;
  else {
...
void CPose3DInterpolator::readFromStream(mrpt::utils::CStream &in,
                                         int version) {
  switch (version) {
  case 0: {
...
void CPoseInterpolatorBase<3>::impl_interpolation(
  ...
  case imLinear2Neig: {
    ...
    out_interp.yaw = math::interpolate2points(td, ts[1], yaw[1], ts[2], yaw[2],
                                              true); // Wrap 2pi
    out_interp.pitch =
        math::interpolate2points(td, ts[1], pitch[1], ts[2], pitch[2], true);
    out_interp.roll =
        math::interpolate2points(td, ts[1], roll[1], ts[2], roll[2], true);
  } break;
  ...
  case imLinear4Neig: {
    ...
    out_interp.yaw = math::leastSquareLinearFit<double, decltype(ts), 4>(
        td, ts, yaw, true); // Wrap 2pi
    out_interp.pitch = math::leastSquareLinearFit<double, decltype(ts), 4>(
        td, ts, pitch, true);
    out_interp.roll =
        math::leastSquareLinearFit<double, decltype(ts), 4>(td, ts, roll, true);
  } break;
  ...
  case imLinearSlerp: {
    ...
    mrpt::math::slerp_ypr(TPose3D(0, 0, 0, yaw[1], pitch[1], roll[1]),
                          TPose3D(0, 0, 0, yaw[2], pitch[2], roll[2]), ratio,
                          out_interp);
