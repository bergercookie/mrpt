...
/*---------------------------------------------------------------
                                                writeToStream
  ---------------------------------------------------------------*/
void CPosePDFGaussianInf::writeToStream(mrpt::utils::CStream &out,
                                        int *version) const {
  if (version)
    *version = 0;
  else {
...
/*---------------------------------------------------------------
                                                readFromStream
  ---------------------------------------------------------------*/
void CPosePDFGaussianInf::readFromStream(mrpt::utils::CStream &in,
                                         int version) {
  switch (version) {
  case 0: {
    TPose2D p;
...
void CPosePDFGaussianInf::copyFrom(const CPosePDF &o) {
  ...
  if (IS_CLASS(&o, CPosePDFGaussianInf)) { // It's my same class:
    const CPosePDFGaussianInf *ptr =
        static_cast<const CPosePDFGaussianInf *>(&o);
    mean = ptr->mean;
    cov_inv = ptr->cov_inv;
  } else { // Convert to gaussian pdf:
    o.getMean(mean);
...
void CPosePDFGaussianInf::copyFrom(const CPose3DPDF &o) {
  ...
  if (IS_CLASS(&o,
               CPose3DPDFGaussianInf)) { // Cov is already in information form:
...
void CPosePDFGaussianInf::inverse(CPosePDF &o) const {
  ...
  out->cov_inv.noalias() =
      (H * cov_inv * H.adjoint()).eval(); // o.cov = H * cov * Ht. It's the same
                                          // with inverse covariances.
}
...
double CPosePDFGaussianInf::mahalanobisDistanceTo(
  ...
  if (MU[0] == 0 && MU[1] == 0 && MU[2] == 0)
    return 0; // This is the ONLY case where we know the result, whatever COVs
              // are.
...
operator+(const mrpt::poses::CPose2D &A,
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CPosePDFGaussianInf::inverseComposition(const CPosePDFGaussianInf &xv,
...
void CPosePDFGaussianInf::operator+=(const CPosePDFGaussianInf &Ap) {
  ...
  CPosePDF::jacobiansPoseComposition(this->mean, // x
                                     Ap.mean,    // u
                                     df_dx, df_du);
