...
void run_test_so2_avrg(const double *angs, const size_t N,
...
void run_test_so3_avrg(const double *angs, const size_t N,
                       const Eigen::Matrix3d &correct_avr) {
  SO_average<3> so_avr;
  for (size_t i = 0; i < N; i++) {
    mrpt::poses::CPose3D rot(0, 0, 0, angs[3 * i + 0], angs[3 * i + 1],
                             angs[3 * i + 2]);
