...
TEST(CPose3DInterpolator, interp) {
  ...
  pose_path.insert(t0 + 2 * dt,
                   TPose3D(1. + 3., 2. + 4., 3. + 5., DEG2RAD(30.0 + 20.0),
                           DEG2RAD(.0), DEG2RAD(.0)));
  ...
  EXPECT_TRUE(valid);
  const TPose3D interp_good(1. + 1.5, 2. + 2.0, 3. + 2.5, DEG2RAD(30.0 + 10.0),
                            DEG2RAD(.0), DEG2RAD(.0));
  EXPECT_NEAR(.0, (CPose3D(interp_good).getHomogeneousMatrixVal() -
