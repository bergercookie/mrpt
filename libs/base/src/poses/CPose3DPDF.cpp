...
CPose3DPDF *CPose3DPDF::createFrom2D(const CPosePDF &o) {
  ...
  if (o.GetRuntimeClass() == CLASS_ID(CPosePDFGaussian)) {
    ...
    for (it1 = obj->begin(), it2 = newObj->begin(); it1 != obj->end();
         ++it1, ++it2) {
      it2->log_w = it1->log_w;
      it2->val.mean.setFromValues(it1->mean.x(), it1->mean.y(), 0,
                                  it1->mean.phi(), 0, 0);
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CPose3DPDF::jacobiansPoseComposition(const CPose3D &x, const CPose3D &u,
  ...
  CPose3DQuatPDF::jacobiansPoseComposition(quat_x, // x
                                           quat_u, // u
                                           quat_df_dx, quat_df_du);
