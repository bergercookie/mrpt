...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CPosePDFSOG::getCovarianceAndMean(CMatrixDouble33 &estCov,
                                       CPose2D &estMean2D) const {
...
void CPosePDFSOG::changeCoordinatesReference(const CPose3D &newReferenceBase_) {
  ...
  for (iterator it = m_modes.begin(); it != m_modes.end(); ++it) {
    ...
    // (it)->cov
    M.multiply_HCHt(CMatrixDouble33((it)->cov),
                    (it)->cov); // * (it)->cov * (~M);
  }
...
/*---------------------------------------------------------------
                                        bayesianFusion
 ---------------------------------------------------------------*/
void CPosePDFSOG::bayesianFusion(const CPosePDF &p1_, const CPosePDF &p2_,
  ...
  for (const_iterator it = p1->m_modes.begin(); it != p1->m_modes.end(); ++it) {
    ...
    //
    //      a = -1/2 ( dimensionality * log(2pi) - log(det(Cov^-1)) + (Cov^-1 *
    //      mu)^t * Cov^-1 * (Cov^-1 * mu) )
    //
    // ----------------------------------------------------------------------
    ...
    double a_i = -0.5 * (3 * log(M_2PI) - log(new_covInv_i.det()) +
...
/*---------------------------------------------------------------
                                                normalizeWeights
 ---------------------------------------------------------------*/
void CPosePDFSOG::evaluatePDFInArea(const double &x_min, const double &x_max,
                                    const double &y_min, const double &y_max,
                                    const double &resolutionXY,
                                    const double &phi, CMatrixD &outMatrix,
                                    bool sumOverAllPhis) {
  MRPT_START
...
void CPosePDFSOG::mergeModes(double max_KLd, bool verbose) {
  ...
  for (size_t i = 0; i < (N - 1);) {
    ...
    if (min_Bij < max_KLd) {
      ...
      Mij.mean = CPose2D(Wi_ * Mi.mean.x() + Wj_ * Mj.mean.x(),
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CPosePDFSOG::getMostLikelyCovarianceAndMean(CMatrixDouble33 &cov,
                                                 CPose2D &mean_point) const {
...
/*---------------------------------------------------------------
                                                getAs3DObject
 ---------------------------------------------------------------*/
// void  CPosePDFSOG::getAs3DObject( mrpt::opengl::CSetOfObjects::Ptr
// &outObj ) const
//{
//	outObj->clear();
//
