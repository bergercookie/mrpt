...
void CPosePDFParticles::copyFrom(const CPosePDF &o) {
  ...
  if (o.GetRuntimeClass() == CLASS_ID(CPosePDFParticles)) {
    ...
    for (itDest = m_particles.begin(), partsIt = parts.begin();
      ...
      itDest->log_w = 0;
      itDest->d.reset(new CPose2D((pdf->mean.x() + (*partsIt)[0]),
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CPosePDFParticles::getCovarianceAndMean(CMatrixDouble33 &cov,
                                             CPose2D &mean) const {
  cov.zeros();
  getMean(mean);
...
/*---------------------------------------------------------------
                                                resetUniform
 ---------------------------------------------------------------*/
void CPosePDFParticles::resetUniform(const double &x_min, const double &x_max,
...
void CPosePDFParticles::resetAroundSetOfPoses(
  ...
  for (i = 0, nSpot = 0; nSpot < list_poses.size(); nSpot++) {
    ...
    for (size_t k = 0; k < num_particles_per_pose; k++, i++) {
      m_particles[i].d.reset(new CPose2D());
      m_particles[i].d->x(randomGenerator.drawUniform(p.x - spread_x * 0.5,
                                                      p.x + spread_x * 0.5));
      m_particles[i].d->y(randomGenerator.drawUniform(p.y - spread_y * 0.5,
                                                      p.y + spread_y * 0.5));
      m_particles[i].d->phi(randomGenerator.drawUniform(
...
void CPosePDFParticles::saveToTextFile(const std::string &file) const {
  ...
  for (unsigned int i = 0; i < m_particles.size(); i++)
    os::fprintf(f, "%f %f %f %e\n", m_particles[i].d->x(),
                m_particles[i].d->y(), m_particles[i].d->phi(),
                m_particles[i].log_w);
...
/*---------------------------------------------------------------
                                        bayesianFusion
 ---------------------------------------------------------------*/
void CPosePDFParticles::bayesianFusion(const CPosePDF &p1, const CPosePDF &p2,
...
/*---------------------------------------------------------------
                                        evaluatePDF_parzen
 ---------------------------------------------------------------*/
double CPosePDFParticles::evaluatePDF_parzen(const double &x, const double &y,
                                             const double &phi,
                                             const double &stdXY,
