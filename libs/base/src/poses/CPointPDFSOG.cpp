...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CPointPDFSOG::getCovarianceAndMean(CMatrixDouble33 &estCov,
                                        CPoint3D &p) const {
  size_t N = m_modes.size();
...
void CPointPDFSOG::readFromStream(mrpt::utils::CStream &in, int version) {
  ...
  case 1: {
    ...
    for (it = m_modes.begin(); it != m_modes.end(); ++it) {
      ...
      if (version == 0)
        it->log_w = log(max(1e-300, it->log_w));
...
/*---------------------------------------------------------------
                                        bayesianFusion
 ---------------------------------------------------------------*/
void CPointPDFSOG::bayesianFusion(const CPointPDF &p1_, const CPointPDF &p2_,
  ...
  // Compute the new kernel means, covariances, and weights after multiplying to
  // the Gaussian "p2":
  ...
  for (CListGaussianModes::const_iterator it1 = p1->m_modes.begin();
    ...
    for (CListGaussianModes::const_iterator it2 = p2->m_modes.begin();
      ...
      if (minMahalanobisDistToDrop > 0) {
        ...
        float stdX2 = max(auxSOG_Kernel_i.cov.get_unsafe(0, 0),
                          (*it1).val.cov.get_unsafe(0, 0));
        mahaDist2 =
            square(auxSOG_Kernel_i.mean.x() - (*it1).val.mean.x()) / stdX2;

        float stdY2 = max(auxSOG_Kernel_i.cov.get_unsafe(1, 1),
                          (*it1).val.cov.get_unsafe(1, 1));
        mahaDist2 +=
            square(auxSOG_Kernel_i.mean.y() - (*it1).val.mean.y()) / stdY2;

        if (!is2D) {
          float stdZ2 = max(auxSOG_Kernel_i.cov.get_unsafe(2, 2),
                            (*it1).val.cov.get_unsafe(2, 2));
          mahaDist2 +=
              square(auxSOG_Kernel_i.mean.z() - (*it1).val.mean.z()) / stdZ2;
        }
      ...
      if (reallyComputeThisOne) {
        ...
        double new_a_i =
            ...
            (3 * log(M_2PI) - log(new_covInv_i.det()) +
             (new_eta_i.adjoint() * newKernel.val.cov * new_eta_i)(0, 0));

        newKernel.log_w = (it1)->log_w + (it2)->log_w + a + a_i - new_a_i;

        // Fix 2D case:
        if (is2D)
          newKernel.val.cov(2, 2) = 0;
...
/*---------------------------------------------------------------
                                                evaluatePDFInArea
 ---------------------------------------------------------------*/
void CPointPDFSOG::evaluatePDFInArea(float x_min, float x_max, float y_min,
                                     float y_max, float resolutionXY, float z,
                                     CMatrixD &outMatrix, bool sumOverAllZs) {
  MRPT_START
...
/*---------------------------------------------------------------
                                                getAs3DObject
 ---------------------------------------------------------------*/
// void  CPointPDFSOG::getAs3DObject( mrpt::opengl::CSetOfObjects::Ptr
// &outObj ) const
//{
