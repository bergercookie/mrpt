...
CPoint2D CPose2D::operator+(const CPoint2D &u) const {
  ...
  return CPoint2D(m_coords[0] + u.x() * m_cosphi - u.y() * m_sinphi,
...
void CPose2D::composePoint(double lx, double ly, double lz, double &gx,
...
void CPose2D::inverseComposePoint(const double gx, const double gy, double &lx,
                                  double &ly) const {
  update_cached_cos_sin();
...
CPoint3D CPose2D::operator+(const CPoint3D &u) const {
  ...
  return CPoint3D(m_coords[0] + u.x() * m_cosphi - u.y() * m_sinphi,
...
TPoint2D mrpt::poses::operator+(const CPose2D &pose, const TPoint2D &u) {
  ...
  return TPoint2D(pose.x() + u.x * ccos - u.y * ssin,
...
double CPose2D::distance2DFrobeniusTo(const CPose2D &p) const {
  return std::sqrt(square(p.x() - x()) + square(p.y() - y()) +
                   4 * (1 - cos(p.phi() - phi())));
}
