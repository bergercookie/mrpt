...
protected:
  ...
  void test_conversions() {
    ...
    EXPECT_NEAR(0, (poseRVT.getAsVectorVal() - newPoseRVT_1.getAsVectorVal())
                       .array()
                       .abs()
                       .sum(),
                1e-5)
    ...
    EXPECT_NEAR(0, (poseRVT.getAsVectorVal() - newPoseRVT_2.getAsVectorVal())
                       .array()
                       .abs()
                       .sum(),
                1e-5)
  ...
  void test_compose(double x1, double y1, double z1, double yaw1, double pitch1,
    ...
    EXPECT_NEAR(
        0, (p3.getAsVectorVal() - p1_c_p2.getAsVectorVal()).array().abs().sum(),
        1e-5)
        << "p3          : " << p3 << endl
...
TEST_F(Pose3DRotVecTests, Compose) {
  test_compose(25.0, 2.0, 3.0, DEG2RAD(-30), DEG2RAD(89), DEG2RAD(0), -10.0,
