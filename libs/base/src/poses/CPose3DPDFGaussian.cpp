...
#ifdef DO_TEST_JACOB
void ffff(const CVectorDouble &x, const CQuaternionDouble &Q,
          CVectorDouble &OUT) {
  OUT.resize(3);
...
void aux_posequat2poseypr(const CArrayDouble<7> &x, const double &dummy,
                          CArrayDouble<6> &y) {
...
//         [  0     | dr_dq_angles ]
    ...
    // Set the mean:
    this->mean.setFromValues(o.mean.x(), o.mean.y(), o.mean.z(), yaw, pitch,
                             roll);
    ...
    mrpt::math::transform_gaussian_unscented(
        ...
        elements_do_wrapPI);
    this->mean.setFromValues(y_mean[0], y_mean[1], y_mean[2], y_mean[3],
                             y_mean[4], y_mean[5]);
  }
  MRPT_END
}
...
void CPose3DPDFGaussian::changeCoordinatesReference(
  ...
  CPose3DPDF::jacobiansPoseComposition(newReferenceBase, // x
                                       this->mean,       // u
                                       df_dx, df_du);
...
void CPose3DPDFGaussian::drawSingleSample(CPose3D &outPart) const {
  ...
  outPart.setFromValues(mean.x() + v[0], mean.y() + v[1], mean.z() + v[2],
                        mean.yaw() + v[3], mean.pitch() + v[4],
                        mean.roll() + v[5]);
...
/*---------------------------------------------------------------
                                        bayesianFusion
 ---------------------------------------------------------------*/
void CPose3DPDFGaussian::bayesianFusion(const CPose3DPDF &p1_,
                                        const CPose3DPDF &p2_) {
...
void CPose3DPDFGaussian::inverse(CPose3DPDF &o) const {
  ...
  //  OUT = b - THIS
  CPose3DPDFGaussian p_zero(CPose3D(0, 0, 0, 0, 0, 0),
...
void CPose3DPDFGaussian::operator+=(const CPose3D &Ap) {
  ...
  CPose3DPDF::jacobiansPoseComposition(this->mean, // x
                                       Ap,         // u
                                       df_dx, df_du);
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
double
CPose3DPDFGaussian::mahalanobisDistanceTo(const CPose3DPDFGaussian &theOther) {
  MRPT_START
  ...
  for (int i = 0; i < 6; i++) {
    if (COV_.get_unsafe(i, i) == 0) {
      if (MU.get_unsafe(i, 0) != 0)
        ...
        COV_.get_unsafe(i, i) = 1; // Any arbitrary value since MU(i)=0, and
                                   // this value doesn't affect the result.
    }
  }
