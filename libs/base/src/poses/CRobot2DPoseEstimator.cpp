...
void CRobot2DPoseEstimator::processUpdateNewOdometry(
  ...
  if (m_last_odo_time != INVALID_TIMESTAMP) {
    const double dT = timeDifference(m_last_odo_time, cur_tim);
    if (dT <= 0)
      std::cerr << "[CRobot2DPoseEstimator::processUpdateNewOdometry] WARNING: "
                   "Diff. in timestamps between odometry should be >0, and "
                   "it's "
                << dT << "\n";
  }
  ...
  } else { // Note: JLBC 23/Nov/2016: I have removed an estimation of velocity
           // from increments of odometry
    // because it was really bad. Just don't make up things: if the user doesn't
    // provide us velocities,
...
bool CRobot2DPoseEstimator::getCurrentEstimate(
  ...
  const TPose2D p = TPose2D(CPose2D(m_last_loc) +
...
void CRobot2DPoseEstimator::extrapolateRobotPose(
  ...
  if (velLocal.vx == 0 && velLocal.vy == 0 && velLocal.omega == 0) { // Still
    new_p = p;
  } else if (std::abs(velLocal.vy) > 1e-2) { // non-Ackermann-like vehicle:
