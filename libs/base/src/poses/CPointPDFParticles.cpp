...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CPointPDFParticles::getCovarianceAndMean(CMatrixDouble33 &cov,
                                              CPoint3D &mean) const {
  MRPT_START
...
void CPointPDFParticles::changeCoordinatesReference(
  ...
  for (CParticleList::iterator it = m_particles.begin();
       it != m_particles.end(); ++it) {
    newReferenceBase.composePoint(it->d->x, it->d->y, it->d->z, // In
                                  pt.x, pt.y, pt.z              // Out
                                  );
    it->d->x = pt.x;
    it->d->y = pt.y;
...
/*---------------------------------------------------------------
                                                writeToStream
  ---------------------------------------------------------------*/
void TSimple3DPoint::writeToStream(mrpt::utils::CStream &out,
                                   int *version) const {
