...
template <typename T> void reverseBytesInPlace_2b(T &v_in_out) {
  uint16_t org;
  ::memcpy(&org, &v_in_out,
  ...
  ::memcpy(&v_in_out, &val_rev, sizeof(T)); // This avoid deref. puned pointer
                                            // warning with:
                                            // *reinterpret_cast<const
                                            // T*>(&val_rev);
}

template <typename T> void reverseBytesInPlace_4b(T &v_in_out) {
  uint32_t org;
  ::memcpy(&org, &v_in_out,
...
template <typename T> void reverseBytesInPlace_8b(T &v_in_out) {
  uint64_t org;
  ::memcpy(&org, &v_in_out,
...
void mrpt::utils::reverseBytesInPlace(long double &v_in_out) {
  uint64_t dat[2];
  ::memcpy(&dat[0], &v_in_out, sizeof(long double)); // This avoid deref. puned
