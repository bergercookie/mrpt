...
{
	uint16_t org;
	::memcpy(&org, &v_in_out, sizeof(T));  // Was: =
	// *reinterpret_cast<uint16_t*>(&v_in_out);
	// but caused SIGBUS in some archs
	const uint16_t val_rev = ((org & 0xff00) >> 8) | ((org & 0x00ff) << 8);
	::memcpy(
		&v_in_out, &val_rev, sizeof(T));  // This avoid deref. puned pointer
	// warning with:
...
void reverseBytesInPlace_4b(T& v_in_out)
{
	uint32_t org;
	::memcpy(&org, &v_in_out, sizeof(T));  // Was: = =
	// *reinterpret_cast<uint32_t*>(&v_in_out);
	// but caused SIGBUS in some archs
...
void reverseBytesInPlace_8b(T& v_in_out)
{
	uint64_t org;
	::memcpy(&org, &v_in_out, sizeof(T));  // Was: =
// *reinterpret_cast<uint64_t*>(&v_in_out);
// but caused SIGBUS in some archs
#if defined(_MSC_VER)
