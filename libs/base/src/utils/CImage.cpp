...
CImage &CImage::operator=(const CImage &o) {
  ...
  if (!o.m_imgIsExternalStorage) { // A normal image
#if MRPT_HAS_OPENCV
    ASSERTMSG_(o.img != nullptr,
               "Source image in = operator has nullptr IplImage*")
    img = cvCloneImage((IplImage *)o.img);
#endif
  } else { // An externally stored image:
...
void CImage::copyFromForceLoad(const CImage &o) {
  ...
  } else { // It's not external storage.
    *this = o;
  }
}
...
#if MRPT_HAS_OPENCV
    if (!o.img)
      THROW_EXCEPTION("Origin image is empty! (o.img==nullptr)")
#endif
    // Erase current image:
    releaseIpl();
...
/*---------------------------------------------------------------
                                                changeSize
 ---------------------------------------------------------------*/
void CImage::changeSize(unsigned int width, unsigned int height,
                        TImageChannels nChannels, bool originTopLeft) {
  MRPT_START

#if MRPT_HAS_OPENCV
  // If we're resizing to exactly the current size, do nothing and avoid wasting
  // mem allocs/deallocs!
  if (img) {
    ...
    if (static_cast<unsigned int>(ipl->width) == width &&
        ...
        ipl->nChannels == nChannels && ipl->origin == (originTopLeft ? 0 : 1)) {
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CImage::loadFromMemoryBuffer(unsigned int width, unsigned int height,
                                  bool color, unsigned char *rawpixels,
                                  bool swapRedBlue) {
  MRPT_START
  ...
  if (color && swapRedBlue) {
    ...
    if (((IplImage *)img)->widthStep ==
      ...
      // Copy the image data:
      memcpy(((IplImage *)img)->imageData, rawpixels,
...
/*---------------------------------------------------------------
                                                operator()
 ---------------------------------------------------------------*/
unsigned char *CImage::operator()(unsigned int col, unsigned int row,
  ...
  if (row >= (unsigned int)ipl->height || col >= (unsigned int)ipl->width ||
    ...
    THROW_EXCEPTION(format("Pixel coordinates/channel out of bounds: row=%u/%u "
                           "col=%u/%u chan=%u/%u",
                           row, ipl->height, col, ipl->width, channel,
                           ipl->nChannels));
  }
#endif
...
/*---------------------------------------------------------------
                                                get_unsafe()
 ---------------------------------------------------------------*/
unsigned char *CImage::get_unsafe(unsigned int col, unsigned int row,
                                  unsigned int channel) const {
#if MRPT_HAS_OPENCV
  makeSureImageIsLoaded(); // For delayed loaded images stored externally
  IplImage *ipl = ((IplImage *)img);
...
void CImage::writeToStream(mrpt::utils::CStream &out, int *version) const {
  ...
  if (version)
    *version = 8;
    ...
    if (m_imgIsExternalStorage) {
      out << m_externalFile;
    } else { // Normal image loaded in memory:
      ASSERT_(img != nullptr);
      ...
      if (!hasColor) {
        ...
        if (imageStoredAsZip) {
          ...
          compress::zip::compress(((IplImage *)img)->imageData, // Data
          ...
          out.WriteBuffer(((IplImage *)img)->imageData,
                          ((IplImage *)img)->imageSize);
        }
      } else {
...
void CImage::readFromStream(mrpt::utils::CStream &in, int version) {
  ...
  if (version == 100) {
    ...
    if (m_imgIsExternalStorage)
      ...
      THROW_EXCEPTION("[CImage] Cannot deserialize image since MRPT has been "
  ...
  case 8: {
    ...
    if (m_imgIsExternalStorage) {
      // Just the file name:
      in >> m_externalFile;
    } else { // Normal, the whole image data:
      ...
      if (!hasColor) {
        ...
        if (version == 2) {
          ...
          if (imageIsZIP) {
            ...
            compress::zip::decompress(in, zipDataLen,
            ...
            // Raw bytes:
            in.ReadBuffer(((IplImage *)img)->imageData,
                          ((IplImage *)img)->imageSize);
          }
        }
      } else {
            ...
            if (width < 0 && height < 0) {
              ...
              for (int y = 0; y < ipl->height; y++) {
                const size_t nRead = in.ReadBuffer(
                    &ipl->imageData[y * ipl->widthStep], bytes_per_row);
...
/*---------------------------------------------------------------
                                                getAsFloat
 ---------------------------------------------------------------*/
float CImage::getAsFloat(unsigned int col, unsigned int row,
                         unsigned int channel) const {
  makeSureImageIsLoaded(); // For delayed loaded images stored externally
  // [0,255]->[0,1]
...
// If possible, use SSE optimized version:
  ...
  if (is_aligned<16>(img_src->imageData) && (img_src->width & 0xF) == 0 &&
    ...
    image_SSSE3_bgr_to_gray_8u((const uint8_t *)img_src->imageData,
                               (uint8_t *)img_dest->imageData, img_src->width,
                               img_src->height);
    return img_dest;
  }
#endif
...
// If possible, use SSE optimized version:
  ...
  if (img_src->nChannels == 3 && is_aligned<16>(img_src->imageData) &&
      ...
      img_src->widthStep == img_src->width * img_src->nChannels &&
      img_dest->widthStep == img_dest->width * img_dest->nChannels) {
    image_SSSE3_scale_half_3c8u((const uint8_t *)img_src->imageData,
                                (uint8_t *)img_dest->imageData, w, h);
  ...
  if (img_src->nChannels == 1 && is_aligned<16>(img_src->imageData) &&
      ...
      img_src->widthStep == img_src->width * img_src->nChannels &&
      img_dest->widthStep == img_dest->width * img_dest->nChannels) {
    image_SSE2_scale_half_1c8u((const uint8_t *)img_src->imageData,
                               (uint8_t *)img_dest->imageData, w, h);
...
// If possible, use SSE optimized version:
  ...
  if (img_src->nChannels == 1 && is_aligned<16>(img_src->imageData) &&
      ...
      img_src->widthStep == img_src->width * img_src->nChannels &&
      img_dest->widthStep == img_dest->width * img_dest->nChannels) {
    image_SSE2_scale_half_smooth_1c8u((const uint8_t *)img_src->imageData,
                                      (uint8_t *)img_dest->imageData, w, h);
...
/*---------------------------------------------------------------
                                                line
 ---------------------------------------------------------------*/
void CImage::line(int x0, int y0, int x1, int y1,
                  const mrpt::utils::TColor color, unsigned int width,
                  TPenStyle penStyle) {
#if MRPT_HAS_OPENCV
  ...
  cvLine(ipl, cvPoint(x0, y0), cvPoint(x1, y1),
...
/*---------------------------------------------------------------
                                                drawCircle
 ---------------------------------------------------------------*/
void CImage::drawCircle(int x, int y, int radius,
                        const mrpt::utils::TColor &color, unsigned int width) {
#if MRPT_HAS_OPENCV
  makeSureImageIsLoaded(); // For delayed loaded images stored externally
  IplImage *ipl = ((IplImage *)img);
  ...
  cvCircle(ipl, cvPoint(x, y), radius, CV_RGB(color.R, color.G, color.B),
           width);
#endif
} // end
...
/*---------------------------------------------------------------
                                               update_patch
 --------------------------------------------------------------*/
void CImage::update_patch(const CImage &patch, const unsigned int col_,
  ...
  for (unsigned int i = 0; i < patch.getHeight(); i++) {
    memcpy(&ipl_int->imageData[(i + row_) * ipl_int->widthStep +
...
/*---------------------------------------------------------------
                                                extract_patch
 ---------------------------------------------------------------*/
void CImage::extract_patch(CImage &patch, const unsigned int col_,
                           const unsigned int row_, const unsigned int col_num,
  ...
  if ((ipl_int->width < (int)(col_ + col_num)) ||
    ...
    THROW_EXCEPTION(
        format("Trying to extract patch out of image boundaries: Image "
...
/*---------------------------------------------------------------
                                                correlate
 ---------------------------------------------------------------*/
float CImage::correlate(const CImage &img2, int width_init,
                        int height_init) const {
#if MRPT_HAS_OPENCV
  makeSureImageIsLoaded(); // For delayed loaded images stored externally
  ...
  for (i = 0; i < img2.getHeight(); i++) {
    for (j = 0; j < img2.getWidth(); j++) {
      m1 += *(*this)(
          j + width_init,
          i + height_init); //(double)(ipl1->imageData[i*ipl1->widthStep + j ]);
...
/*---------------------------------------------------------------
                                        cross_correlation
 ---------------------------------------------------------------*/
void CImage::cross_correlation(const CImage &patch_img, size_t &x_max,
                               size_t &y_max, double &max_val, int x_search_ini,
                               int y_search_ini, int x_search_size,
                               int y_search_size,
  ...
  if (!entireImg) {
    IplImage *aux = cvCreateImage(cvSize(patch_im->width + x_search_size,
    ...
    for (unsigned int i = 0; i < (unsigned int)y_search_size; i++) {
      memcpy(&aux->imageData[i * aux->widthStep],
             ...
             aux->width * aux->nChannels); // widthStep);  <-- JLBC: widthstep
                                           // SHOULD NOT be used as the length
                                           // of each row (the last one may be
                                           // shorter!!)
    }
    ipl_ext = aux;
  } else {
...
void CImage::normalize() {
  ...
  ASSERT_(ipl)
  ASSERTMSG_(ipl->nChannels == 1,
             "CImage::normalize() only defined for grayscale images.")

  uint8_t min_ = 255, max_ = 1;
  for (int y = 0; y < ipl->height; y++) {
    const uint8_t *ptr =
        reinterpret_cast<const uint8_t *>(ipl->imageData + y * ipl->widthStep);
  ...
  uint8_t lut[256];
  for (int v = 0; v < 256; v++)
    lut[v] = static_cast<uint8_t>((v - min_) * s);
...
/*---------------------------------------------------------------
                                                getAsMatrix
 ---------------------------------------------------------------*/
void CImage::getAsMatrix(CMatrixFloat &outMatrix, bool doResize, int x_min,
                         int y_min, int x_max, int y_max) const {
#if MRPT_HAS_OPENCV
  MRPT_START
  ...
  // Set sizes:
  if (x_max == -1)
    x_max = ((IplImage *)img)->width - 1;
  if (y_max == -1)
    y_max = ((IplImage *)img)->height - 1;
...
void CImage::getAsRGBMatrices(mrpt::math::CMatrixFloat &outMatrixR,
                              ...
                              bool doResize, int x_min, int y_min, int x_max,
                              int y_max) const {
#if MRPT_HAS_OPENCV
  MRPT_START
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CImage::cross_correlation_FFT(const CImage &in_img, CMatrixFloat &out_corr,
                                   int u_search_ini, int v_search_ini,
                                   int u_search_size, int v_search_size,
                                   float biasThisImg, float biasInImg) const {
  MRPT_START
  ...
  if (v_search_ini == -1)
    v_search_ini = 0;
  if (u_search_size == -1)
    u_search_size = getWidth();
  if (v_search_size == -1)
    v_search_size = getHeight();
  ...
  // We fill the images with the bias, such as when we substract the bias later
  // on,
  ...
  getAsMatrix(i2, false, u_search_ini, v_search_ini,
...
void CImage::makeSureImageIsLoaded() const
  ...
  if (m_imgIsExternalStorage) {
    ...
    // These are removed by "loadFromFile", and that's good, just fix it here
    // and carry on.
    m_imgIsExternalStorage = true;
    ...
    if (!ret)
      THROW_TYPED_EXCEPTION_FMT(
          CExceptionExternalImageNotFound,
          "Error loading externally-stored image from: %s", wholeFile.c_str());
  } else
...
void CImage::rectifyImageInPlace(const mrpt::utils::TCamera &cameraParams) {
  ...
  CvMat inMat = cvMat(cameraMatrix.getRowCount(), cameraMatrix.getColCount(),
                      CV_64F, aux1);
...
/*---------------------------------------------------------------
                        scaleImage
 ---------------------------------------------------------------*/
void CImage::scaleImage(unsigned int width, unsigned int height,
...
/*---------------------------------------------------------------
                        scaleImage
 ---------------------------------------------------------------*/
void CImage::scaleImage(CImage &out_img, unsigned int width,
                        unsigned int height,
...
/*---------------------------------------------------------------
                        rotateImage
 ---------------------------------------------------------------*/
void CImage::rotateImage(double angle_radians, unsigned int center_x,
                         unsigned int center_y, double scale) {
#if MRPT_HAS_OPENCV
  makeSureImageIsLoaded(); // For delayed loaded images stored externally
  ASSERT_(img != nullptr);
...
bool CImage::drawChessboardCorners(std::vector<TPixelCoordf> &cornerCoords,
                                   ...
                                   unsigned int lines_width, unsigned int r) {
#if MRPT_HAS_OPENCV
  ...
  for (y = 0, i = 0; y < check_size_y; y++) {
    ...
    for (x = 0; x < check_size_x; x++, i++) {
      ...
      if (i != 0)
        cvLine(ipl, prev_pt, pt, color, lines_width);

      cvLine(ipl, cvPoint(pt.x - r, pt.y - r), cvPoint(pt.x + r, pt.y + r),
             color, lines_width);
      cvLine(ipl, cvPoint(pt.x - r, pt.y + r), cvPoint(pt.x + r, pt.y - r),
             color, lines_width);
      ...
      if (i == 0 || i == cornerCoords.size() - 1)
        CCanvas::textOut(pt.x + 5, pt.y - 5, mrpt::format("%u", i),
...
void CImage::joinImagesHorz(const CImage &im1, const CImage &im2) {
  ...
  IplImage *out = cvCreateImage(cvSize(_im1->width + _im2->width, _im1->height),
                                _im1->depth, _im1->nChannels);
  ...
  IplImage *out2;
  if ((int)_im1->nChannels !=
      (int)this
          ->getChannelCount()) // Convert the input to the output channel format
  {
    out2 = cvCreateImage(cvSize(_im1->width + _im2->width, _im1->height),
                         _im1->depth, this->getChannelCount());
...
void CImage::equalizeHist(CImage &outImg) const {
  ...
  if (srcImg->nChannels == 1) { // Grayscale:
...
void image_KLT_response_template(const uint8_t *in, const int widthStep, int x,
                                 int y, int32_t &_gxx, int32_t &_gyy,
                                 int32_t &_gxy) {
...
float CImage::KLT_response(const unsigned int x, const unsigned int y,
                           const unsigned int half_window_size) const {
#if MRPT_HAS_OPENCV
  const IplImage *srcImg = this->getAs<IplImage>();
  ASSERT_(srcImg != nullptr)
  ASSERTMSG_(srcImg->nChannels == 1,
  ...
  ASSERTMSG_(min_x < img_w && max_x < img_w && min_y < img_h && max_y < img_h,
...
// (FreeBSD license)
bool CImage::loadTGA(const std::string &fileName, mrpt::utils::CImage &out_RGB,
