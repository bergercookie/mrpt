	...
	bool originTopLeft)
{
	...
	if (img)
	{
		...
		{
			// nothing to do, we're already right with the current IplImage!
			return;
		}
	}
	...
	if (!entireImg)
	{
		...
		for (unsigned int i = 0; i < (unsigned int)y_search_size; i++)
		{
			memcpy(
				...
				aux->width *
					aux->nChannels);  // widthStep);  <-- JLBC: widthstep
			// SHOULD NOT be used as the length of
			// each row (the last one may be
			// shorter!!)
		}
		ipl_ext = aux;
	}
