...
#define SAVE_COMPRESSED(ARR)                                                   \
  {                                                                            \
    ...
    memcpy(&list_registered_fonts[#ARR][0], mrpt_font_##ARR,                   \
           sizeof(mrpt_font_##ARR));                                           \
    cout << #ARR << " -> " << sizeof(mrpt_font_##ARR) << endl;                 \
    CFileGZOutputStream f(string("mrpt_font_") + string(#ARR) +                \
                          string(".gz"));                                      \
...
#define LOAD_FONT(FONTNAME)                                                    \
  {                                                                            \
    ...
    memcpy(&tmpBuf[0], mrpt_font_gz_##FONTNAME,                                \
...
/*---------------------------------------------------------------
                                                Constructor
---------------------------------------------------------------*/
CCanvas::CCanvas() : m_selectedFont("9x15"), m_selectedFontBitmaps(nullptr) {}

/*---------------------------------------------------------------
                                                line
---------------------------------------------------------------*/
void CCanvas::line(int x0, int y0, int x1, int y1,
  ...
  if (y0 < 0 && y1 < 0)
    return;
  if (x0 >= (int)getWidth() && x1 >= (int)getWidth())
    return;
  if (y0 >= (int)getHeight() && y1 >= (int)getHeight())
    return;
...
/*---------------------------------------------------------------
                                                rectangle
---------------------------------------------------------------*/
void CCanvas::rectangle(int x0, int y0, int x1, int y1,
                        const mrpt::utils::TColor color, unsigned int width) {
...
/*---------------------------------------------------------------
                                                triangle
---------------------------------------------------------------*/
void CCanvas::triangle(int x0, int y0, int size,
                       const mrpt::utils::TColor color, bool inferior,
                       unsigned int width) {
...
/*---------------------------------------------------------------
                                                filledRectangle
---------------------------------------------------------------*/
void CCanvas::filledRectangle(int x0, int y0, int x1, int y1,
...
/*---------------------------------------------------------------
                                                drawImage
---------------------------------------------------------------*/
void CCanvas::drawImage(int x, int y, const utils::CImage &img, float rotation,
                        float scale) {
  MRPT_UNUSED_PARAM(x);
  MRPT_UNUSED_PARAM(y);
...
/*---------------------------------------------------------------
                                                cross
---------------------------------------------------------------*/
void CCanvas::cross(int x0, int y0, const mrpt::utils::TColor color, char type,
...
/*---------------------------------------------------------------
                                                drawCircle
---------------------------------------------------------------*/
void CCanvas::drawCircle(int x, int y, int radius,
                         const mrpt::utils::TColor &color, unsigned int width) {
  if (radius < 0)
    radius = -radius;
...
/*---------------------------------------------------------------
                                                textOut
---------------------------------------------------------------*/
void CCanvas::textOut(int x0, int y0, const std::string &str,
                      const mrpt::utils::TColor color) {
  MRPT_START
