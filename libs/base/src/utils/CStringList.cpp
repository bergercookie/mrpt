...
bool CStringList::find(
	const string& compareText, size_t foundIndex, bool caseSensitive) const
{
	MRPT_START
	...
	if (caseSensitive)
	{
		for (deque<string>::const_iterator it = m_strings.begin();
			...
			if (!os::_strcmp(compareText.c_str(), it->c_str())) return true;
	}
	else
	{
		for (deque<string>::const_iterator it = m_strings.begin();
			 it != m_strings.end(); ++it, foundIndex++)
			if (!os::_strcmpi(compareText.c_str(), it->c_str())) return true;
	}
...
/*---------------------------------------------------------------
						writeToStream
 ---------------------------------------------------------------*/
void CStringList::writeToStream(
