...
TEST(SerializeTestBase, LoadDemoFile)
{
	...
	TRegs L;
	const string fil =
		MRPT_GLOBAL_UNITTEST_SRC_DIR + string("/tests/serialize_test_data.bin");
	...
	else
	{
		...
		gg >> L.v1 >> L.v2 >> L.v3 >> L.v4 >> L.v5 >> L.v6 >> L.v7 >> L.v8 >>
			L.v9;
...
const mrpt::utils::TRuntimeClassId* lstClasses[] = {
	// Misc:
	CLASS_ID(CPose2D), CLASS_ID(CPose3D), CLASS_ID(CPose3DQuat),
	CLASS_ID(CPoint2D), CLASS_ID(CPoint3D),
	// Poses:
	CLASS_ID(CPose3DPDFGaussian), CLASS_ID(CPose3DQuatPDFGaussian),
	// Others:
...
TEST(SerializeTestBase, CopyOperator)
{
	for (size_t i = 0; i < sizeof(lstClasses) / sizeof(lstClasses[0]); i++)
	{
		...
		{
			GTEST_FAIL() << "Exception during copy operator test for class '"
						 << lstClasses[i]->className << "':\n"
						 << e.what() << endl;
		}
	}
			...
			for (size_t i = 0; i < N; i++)
			{
				...
				for (size_t j = 0; j < M; j++)
				{
					std::set<std::string>& dd =
						d[rng.drawGaussian1D_normalized()];
					const size_t L = rng.drawUniform(2, 15);
					for (size_t k = 0; k < L; k++)
						dd.insert(
							mrpt::format(
								"%f", rng.drawGaussian1D_normalized()));
				}
				a.push_back(d);
			}
