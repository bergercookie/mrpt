...
CTimeLogger::CTimeLogger(bool enabled /*=true*/,
    ...
    : COutputLogger("CTimeLogger"), m_tictac(), m_enabled(enabled),
      m_name(name) {
  m_tictac.Tic();
}
...
CTimeLogger::CTimeLogger(const CTimeLogger &o)
    : COutputLogger(o), m_enabled(o.m_enabled), m_name(o.m_name),
      m_data(o.m_data) {}
...
std::string CTimeLogger::getStatsAsText(const size_t column_width) const {
  ...
  // append dashes to the header to reach column_width
  {
    ...
    if (dashes_half.size() %
        2) { // what if column_width-top_header.size() is odd?
      top_header += '-';
    }
  }

  std::string middle_header("           FUNCTION                         "
                            "#CALLS  MIN.T  MEAN.T MAX.T TOTAL ");
  ...
  for (const auto i : m_data) {
    ...
    const string sMeanT = unitsFormat(
        i.second.n_calls ? i.second.mean_t / i.second.n_calls : 0, 1, false);
...
CTimeLogger::TCallData::TCallData()
    : n_calls(0), min_t(0), max_t(0), mean_t(0), last_t(0),
      has_time_units(true) {}
...
CTimeLoggerEntry::CTimeLoggerEntry(CTimeLogger &logger,
