...
CMemoryStream::CMemoryStream()
    : m_memory(nullptr), m_size(0), m_position(0), m_bytesWritten(0),
      m_alloc_block_size(0x1000), m_read_only(false) {}
...
void CMemoryStream::resize(uint64_t newSize) {
  if (m_read_only)
    THROW_EXCEPTION("[CMemoryStream::resize] Cannot change memory block size "
                    "since it was set with 'assign'")

  if (!newSize) { // Free buffer:
    if (m_memory.get())
      free(m_memory.get());
    m_memory = nullptr;
    m_size = 0;
