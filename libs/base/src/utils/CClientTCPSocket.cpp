...
/*---------------------------------------------------------------
                                                sendMessage
 ---------------------------------------------------------------*/
bool CClientTCPSocket::sendMessage(const CMessage &outMsg,
...
/*---------------------------------------------------------------
                                                receiveMessage
 ---------------------------------------------------------------*/
bool CClientTCPSocket::receiveMessage(CMessage &inMsg,
  ...
  actRead = readAsync(&inMsg.content[0], toRead, timeoutBetween_ms,
                      timeoutBetween_ms);
  if (actRead != toRead)
    return false; // Error!
...
void CClientTCPSocket::connect(const std::string &remotePartAddress,
  ...
  if (!net::DNS_resolve_async(remotePartAddress, solved_IP,
                              DNS_LOOKUP_TIMEOUT_MS))
    THROW_EXCEPTION_FMT("DNS lookup failed for '%s'",
                        remotePartAddress.c_str());
...
// Set to NON-BLOCKING:
  ...
  // Try to connect:
  int r = ::connect(m_hSock, (struct sockaddr *)&otherAddress,
                    sizeof(otherAddress));
#ifdef MRPT_OS_WINDOWS
...
#endif
    THROW_EXCEPTION(format("Error connecting to %s:%hu. Error: %s [%d]",
  ...
  if (sel_ret == 0)
    THROW_EXCEPTION(format("Timeout connecting to '%s:%hu':\n%s",
                           remotePartAddress.c_str(), remotePartTCPPort,
                           getLastErrorStr().c_str()));
  if (sel_ret == -1)
    THROW_EXCEPTION(format("Error connecting to '%s:%hu':\n%s",
                           remotePartAddress.c_str(), remotePartTCPPort,
                           getLastErrorStr().c_str()));
...
#ifdef MRPT_OS_WINDOWS
  if (valopt)
    THROW_EXCEPTION(format("Error connecting to %s:%hu. Error: %i.",
                           remotePartAddress.c_str(), remotePartTCPPort,
                           valopt));
#else
  if (valopt)
    THROW_EXCEPTION(format("Error connecting to %s:%hu. Error: %s.",
                           remotePartAddress.c_str(), remotePartTCPPort,
                           strerror(valopt)));
#endif
// Connected!
...
/*---------------------------------------------------------------
                                                readAsync
 ---------------------------------------------------------------*/
size_t CClientTCPSocket::readAsync(void *Buffer, const size_t Count,
  ...
  while (alreadyRead < Count && !timeoutExpired) {
    ...
    int selRet = ::select(m_hSock + 1, // __nfds
    ...
    if (selRet == INVALID_SOCKET)
      THROW_EXCEPTION_FMT("Error reading from socket: %s",
                          getLastErrorStr().c_str());
    ...
    if (selRet == 0) {
      ...
      // Receive bytes:
      readNow =
          ::recv(m_hSock, ((char *)Buffer) + alreadyRead, (int)remainToRead, 0);
...
/*---------------------------------------------------------------
                                                writeAsync
 ---------------------------------------------------------------*/
size_t CClientTCPSocket::writeAsync(const void *Buffer, const size_t Count,
                                    const int timeout_ms) {
  MRPT_START
  ...
  while (alreadyWritten < Count && !timeoutExpired) {
    ...
    if (selRet == INVALID_SOCKET)
      THROW_EXCEPTION_FMT("Error writing to socket: %s",
                          getLastErrorStr().c_str());
    ...
    if (selRet == 0) {
      ...
      // Receive bytes:
      writtenNow = ::send(m_hSock, ((char *)Buffer) + alreadyWritten,
                          (int)remainToWrite, 0);
...
int CClientTCPSocket::setTCPNoDelay(const int &newValue) {
  ...
  return setsockopt(m_hSock, IPPROTO_TCP, TCP_NODELAY, (char *)&newValue,
                    length);
}
