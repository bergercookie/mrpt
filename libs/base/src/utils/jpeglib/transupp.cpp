			 ...
			 dst_blk_y += compptr->v_samp_factor)
		{
			...
			for (offset_y = 0; offset_y < compptr->v_samp_factor; offset_y++)
			{
				...
				else
				{
					...
					jcopy_block_row(
						src_buffer[offset_y], dst_buffer[offset_y],
					 ...
					 dst_blk_x += compptr->h_samp_factor)
				{
						 ...
						 offset_x++)
					{
						...
						if (dst_blk_y < comp_height)
						{
							/* Block is within the mirrorable area. */
							src_ptr =
								src_buffer[offset_x][comp_height - dst_blk_y -
													 offset_y - 1];
							for (i = 0; i < DCTSIZE; i++)
							{
								for (j = 0; j < DCTSIZE; j++)
			...
			for (offset_y = 0; offset_y < compptr->v_samp_factor; offset_y++)
			{
				...
				else
				{
					...
					for (; dst_blk_x < compptr->width_in_blocks; dst_blk_x++)
					{
						dst_ptr = dst_row_ptr[dst_blk_x];
						src_ptr = src_row_ptr[dst_blk_x];
						for (i = 0; i < DCTSIZE2; i++) *dst_ptr++ = *src_ptr++;
					}
				}
			}
		}
...
jtransform_adjust_parameters(
	j_decompress_ptr, j_compress_ptr dstinfo, jvirt_barray_ptr* src_coef_arrays,
...
jcopy_markers_execute(
	j_decompress_ptr srcinfo, j_compress_ptr dstinfo, JCOPY_OPTION)
{
	 ...
	 */
	for (marker = srcinfo->marker_list; marker != nullptr;
		 marker = marker->next)
	{
...
#else
		jpeg_write_marker(
			dstinfo, marker->marker, marker->data, marker->data_length);
#endif
	}
}
