	...
	switch (pass_mode)
	{
		case JBUF_PASS_THRU:
			if (cinfo->quantize_colors)
			{
				 ...
				 * 2-pass
				 * color quantization; in that case, jinit_d_post_controller did
				 * not
	...
	JDIMENSION* out_row_ctr, JDIMENSION out_rows_avail)
{
	...
	(*cinfo->upsample->upsample)(
		cinfo, input_buf, in_row_group_ctr, in_row_groups_avail, post->buffer,
		&num_rows, max_rows);
	...
	JDIMENSION* out_row_ctr, JDIMENSION out_rows_avail)
{
	...
	if (post->next_row > old_next_row)
	{
		...
		(*cinfo->cquantize->color_quantize)(
			cinfo, post->buffer + old_next_row, (JSAMPARRAY) nullptr,
			(int)num_rows);
		*out_row_ctr += num_rows;
	}
...
/* We round up the number of rows to a multiple of the strip height. */
			...
			post->whole_image = (*cinfo->mem->request_virt_sarray)(
				...
				(JDIMENSION)jround_up(
					(long)cinfo->output_height, (long)post->strip_height),
				post->strip_height);
#else
			ERREXIT(cinfo, JERR_BAD_BUFFER_MODE);
#endif /* QUANT_2PASS_SUPPORTED */
