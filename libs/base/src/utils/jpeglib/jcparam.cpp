 ...
 */
{
	...
	jpeg_add_quant_table(
		cinfo, 0, std_luminance_quant_tbl, scale_factor, force_baseline);
	jpeg_add_quant_table(
		cinfo, 1, std_chrominance_quant_tbl, scale_factor, force_baseline);
}
...
LOCAL(void)
add_huff_table(
	j_compress_ptr cinfo, JHUFF_TBL** htblptr, const UINT8* bits,
	const UINT8* val)
...
/* IMPORTANT: these are only valid for 8-bit data precision! */
{
	...
	add_huff_table(
		cinfo, &cinfo->dc_huff_tbl_ptrs[0], bits_dc_luminance,
		val_dc_luminance);
	add_huff_table(
		cinfo, &cinfo->ac_huff_tbl_ptrs[0], bits_ac_luminance,
		val_ac_luminance);
	add_huff_table(
		cinfo, &cinfo->dc_huff_tbl_ptrs[1], bits_dc_chrominance,
		val_dc_chrominance);
	add_huff_table(
		cinfo, &cinfo->ac_huff_tbl_ptrs[1], bits_ac_chrominance,
		val_ac_chrominance);
}
...
jpeg_default_colorspace(j_compress_ptr cinfo)
{
	switch (cinfo->in_color_space)
	{
		...
		case JCS_CMYK:
			jpeg_set_colorspace(
				cinfo, JCS_CMYK); /* By default, no translation */
			break;
		case JCS_YCCK:
...
/* Support routine: generate interleaved DC scan if possible, else N scans */
{
	...
	if (ncomps <= MAX_COMPS_IN_SCAN)
	{
		...
		for (ci = 0; ci < ncomps; ci++) scanptr->component_index[ci] = ci;
