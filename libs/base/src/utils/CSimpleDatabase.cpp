...
/*---------------------------------------------------------------
                                                getTable
 ---------------------------------------------------------------*/
CSimpleDatabaseTable::Ptr
CSimpleDatabase::getTable(const std::string &tableName) {
  MRPT_START
...
/*---------------------------------------------------------------
                                                set
 ---------------------------------------------------------------*/
void CSimpleDatabaseTable::set(size_t recordIndex, size_t fieldIndex,
                               string value) {
  MRPT_START
...
bool CSimpleDatabase::loadFromXML(const string &fileName) {
  try {
    ...
    if (root.isEmpty()) {
      cerr << "[CSimpleDatabase::loadFromXML] Loaded XML file does not have a "
              "'CSimpleDatabase-MRPT-Object' tag";
      return false;
    }
    ...
    for (i = 0; i < nTables; i++) {
      ...
      for (size_t k = 0; k < nRecs; k++) {
        ...
        for (j = 0; j < nFields; j++) {
          XMLCSTR str =
              recNod.getChildNode(t->getFieldName(j).c_str()).getText();
  ...
  if (it == m_tables.end())
...

    {
      ...
      if (itNew != m_tables.end())
        THROW_EXCEPTION_FMT("A table with the name '%s' already exists",
                            newTableName.c_str())
    }
