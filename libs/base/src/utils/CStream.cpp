...
// Write:
CStream &utils::operator<<(mrpt::utils::CStream &s,
...
CStream &utils::operator<<(mrpt::utils::CStream &s,
                           const mrpt::math::CVectorFloat &v) {
  const uint32_t n = static_cast<uint32_t>(v.size());
  s << n;
  if (n)
    s.WriteBufferFixEndianness(&v[0], n);
...
CStream &utils::operator<<(mrpt::utils::CStream &s,
                           const mrpt::math::CVectorDouble &v) {
  const uint32_t n = static_cast<uint32_t>(v.size());
  s << n;
  if (n)
    s.WriteBufferFixEndianness(&v[0], n);
...
void CStream::internal_ReadObjectHeader(std::string &strClassName,
  ...
  try {
    // First, read the class name: (exception is raised here if ZERO bytes read
    // -> possibly an EOF)
    ...
    if (!(lengthReadClassName & 0x80)) {
      ...
      if (buf[0] || buf[1] || buf[2])
        THROW_EXCEPTION("Expecting 0x00 00 00 while parsing old streaming "
                        "header (Perhaps it's a gz-compressed stream? Use a "
                        "GZ-stream for reading)");
    } else {
      isOldFormat = false;
    }
    ...
    if (lengthReadClassName > 120)
      THROW_EXCEPTION("Class name has more than 120 chars. This probably means "
                      "a corrupted binary stream.");
    ...
    if (isOldFormat) {
      ...
      version = int8_t(version_old);
               ...
               sizeof(version) !=
                   ReadBuffer((void *)&version, sizeof(version))) {
      THROW_EXCEPTION("Cannot read object streaming version from stream!");
    }
...
void CStream::internal_ReadObject(CSerializable *obj,
  ...
  try {
    ...
    if (!isOldFormat) {
      ...
      if (endFlag != SERIALIZATION_END_FLAG)
        THROW_EXCEPTION_FMT("end-flag missing: There is a bug in the "
                            "deserialization method of class: '%s'",
                            strClassName.c_str());
    }
  } catch (std::bad_alloc &) {
    throw;
...
void CStream::ReadObject(CSerializable *existingObj) {
  ...
  if (!id2)
    THROW_EXCEPTION_FMT("Stored object has class '%s' which is not registered!",
                        strClassName.c_str());
  if (id != id2)
    THROW_EXCEPTION(format("Stored class does not match with existing "
                           "object!!:\n Stored: %s\n Expected: %s",
  ...
  for (;;) {
    if (nBytesInFrame < 4)
      ...
      if (buf[0] == 0x69) {
        ...
        payload_len =
            MAKEWORD16B(buf[3] /*low*/, buf[2] /*hi*/); // Length of the content
