...
void CEnhancedMetaFile::drawImage(int x, int y, const utils::CImage &img) {
  try {
    ...
    pBmpInfo->bmiHeader.biXPelsPerMeter = pBmpInfo->bmiHeader.biYPelsPerMeter =
        0;
    ...
    HBITMAP hBitmap =
        CreateDIBitmap((HDC)m_hdc.get(), &pBmpInfo->bmiHeader, CBM_INIT,
                       ptrBits, pBmpInfo, DIB_RGB_COLORS);
    ...
    BitBlt((HDC)m_hdc.get(), x, y, (int)(m_scale * imgWidth),
...
/*---------------------------------------------------------------
                                                rectangle
---------------------------------------------------------------*/
void CEnhancedMetaFile::rectangle(int x0, int y0, int x1, int y1, TColor color,
                                  unsigned int width) {
