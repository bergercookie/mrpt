...
void COutputLogger::logFmt(const VerbosityLevel level, const char *fmt,
                           ...) const {
...
void COutputLogger::logCond(const VerbosityLevel level, bool cond,
                            const std::string &msg_str) const {
  if (!cond)
    return;
...
COutputLogger::TMsg::TMsg(const mrpt::utils::VerbosityLevel level,
...
void COutputLogger::TMsg::dumpToConsole() const {
  ...
  const bool dump_to_cerr =
      (level ==
       LVL_ERROR); // LVL_ERROR alternatively dumped to stderr instead of stdout
...
void COutputLogger::logDeregisterCallback(output_logger_callback_t userFunc,
                                          void *userParam) {
  ASSERT_(userFunc != nullptr);
  TCallbackEntry cbe;
  cbe.func = userFunc;
