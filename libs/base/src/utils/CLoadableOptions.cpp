...
void CLoadableOptions::dumpVar_float(CStream &out, const char *varName,
                                     float v) {
  out.printf("%-*s= %f\n", LOADABLEOPTS_COLUMN_WIDTH, varName, v);
}

void CLoadableOptions::dumpVar_double(CStream &out, const char *varName,
                                      double v) {
  out.printf("%-*s= %f\n", LOADABLEOPTS_COLUMN_WIDTH, varName, v);
}

void CLoadableOptions::dumpVar_bool(CStream &out, const char *varName, bool v) {
  out.printf("%-*s= %s\n", LOADABLEOPTS_COLUMN_WIDTH, varName,
             v ? "YES" : "NO");
}

void CLoadableOptions::dumpVar_string(CStream &out, const char *varName,
                                      const std::string &v) {
