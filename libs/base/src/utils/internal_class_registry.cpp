      ...
      if (it != registeredClasses.cend()) {
        if (it->second != &id) {
          std::cerr << mrpt::format(
              "[MRPT class registry] Warning: overwriting already registered "
              "className=`%s` with different `TRuntimeClassId`!\n",
              className.c_str());
        }
      }
 ...
 * (CMultiMetricMaps, CImage,...)
  */
void utils::registerClassCustomName(const char *customName,
...
utils::getAllRegisteredClassesChildrenOf(const TRuntimeClassId *parent_id) {
