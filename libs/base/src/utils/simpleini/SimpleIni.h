...
CSimpleIniTempl<SI_CHAR, SI_STRLESS, SI_CONVERTER>::CSimpleIniTempl(
    ...
    : m_pData(0), m_uDataLen(0), m_pFileComment(NULL),
      m_bAllowMultiKey(a_bAllowMultiKey), m_bAllowMultiLine(a_bAllowMultiLine),
      m_nOrder(0) {}
...
template <class SI_CHAR, class SI_STRLESS, class SI_CONVERTER>
SI_Error
CSimpleIniTempl<SI_CHAR, SI_STRLESS, SI_CONVERTER>::Load(const char *a_pData,
                                                         size_t a_uDataLen) {
...
public:
   ...
   */
  virtual size_t SizeFromStore(const char *a_pInputData,
   ...
   *                      converted.
   */
  virtual bool ConvertFromStore(const char *a_pInputData,
                                size_t a_uInputDataLen, SI_CHAR *a_pOutputData,
...
struct MRPT_IniFileParser : public SI_ConvertA<char> {
  ...
  virtual bool ConvertFromStore(const char *a_pInputData,
                                size_t a_uInputDataLen, char *a_pOutputData,
...
private:
  ...
  void parse_process_var_define(ParseContext &pc, const std::string &var_name,
      ...
      if (c == '\\' && i < in_len - 1 &&
        ...
        if (in_len > i + 4 && in_str[i] == '$' && in_str[i + 1] == '{') {
          ...
          if (it == pc.defined_vars.end())
            throw std::runtime_error(
                mrpt::format("Line %u: Unknown variable `${%s}`", pc.line_count,
                             varname.c_str()));
