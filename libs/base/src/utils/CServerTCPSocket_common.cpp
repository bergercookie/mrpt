...
CClientTCPSocket *CServerTCPSocket::accept(int timeout_ms) {
  ...
  int selRet = ::select(m_serverSock + 1, // __nfds
  ...
  if (selRet == 0) {
    ...
    int aceptdSock =
        ::accept(m_serverSock, (struct sockaddr *)&otherPart, &otherPartSize);
    ...
    MRPT_LOG_DEBUG(
        format("[CServerTCPSocket::accept] Connection accepted from %s:%u\n",
