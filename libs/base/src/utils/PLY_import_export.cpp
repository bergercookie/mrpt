...
int get_binary_item(FILE *, int, int, int *, unsigned int *,
                    double *); // return 0 on error
...
PlyFile *ply_open_for_writing(const char *name,
                              const vector<string> &elem_names, int file_type,
                              float *version) {
  PlyFile *plyfile;
  FILE *fp;
...
void ply_describe_element(PlyFile *plyfile, const string &elem_name, int nelems,
...
void ply_describe_property(PlyFile *plyfile, const char *elem_name,
...
PlyFile *ply_open_for_reading(const char *filename, vector<string> &elem_names,
...
void ply_get_property(PlyFile *plyfile, const string &elem_name,
                      const PlyProperty *prop) {
  PlyElement *elem;
...
PlyProperty *find_property(PlyElement *elem, const std::string &prop_name,
                           int *index) {
...
void ascii_get_element(PlyFile *plyfile, char *elem_ptr) {
  ...
  for (size_t j = 0; j < elem->props.size(); j++) {
    ...
    if (prop->is_list) { /* a list */
      ...
      get_ascii_item(words[which_word++].c_str(), prop->external_type, &int_val,
                     &uint_val, &double_val);
      if (store_it) {
        item = elem_data + prop->offset;
...
void binary_get_element(PlyFile *plyfile, char *elem_ptr) {
  ...
  for (size_t j = 0; j < elem->props.size(); j++) {
    ...
    if (prop->is_list) { /* a list */
      ...
      /* get and store the number of items in the list */
      if (!get_binary_item(fp, bin_file_type, prop->count_external, &int_val,
                           &uint_val, &double_val)) {
        // Error...
        fprintf(stderr,
      ...
      if (list_count == 0) {
        ...
        /* read items and store them into the array */
        for (int k = 0; k < list_count; k++) {
          if (!get_binary_item(fp, bin_file_type, prop->external_type, &int_val,
                               &uint_val, &double_val)) {
            // Error...
            fprintf(stderr,
                    "RPly::binary_get_element: Error reading binary file!\n");
...
int get_binary_item(FILE *fp, int bin_file_type, int type, int *int_val,
...
void get_ascii_item(const char *word, int type, int *int_val,
                    unsigned int *uint_val, double *double_val) {
  switch (type) {
  case PLY_CHAR:
  case PLY_UCHAR:
    ...
    for (size_t i = 0; i < elist.size(); i++) {
      ...
      if ("vertex" == elem_name) {
        ...
        for (int j = 0; j < num_elems; j++) {
          ...
          if (pt.intensity != VAL_NOT_SET) { // Grayscale
            const TColorf col(pt.intensity, pt.intensity, pt.intensity);
            this->PLY_import_set_vertex(j, xyz, &col);
          } else if (pt.r != VAL_NOT_SET && pt.g != VAL_NOT_SET &&
                     pt.b != VAL_NOT_SET) { // RGB
...
bool PLY_Exporter::saveToPlyFile(const std::string &filename,
  ...
  try {
    ...
    float version;
    PlyFile *ply = ply_open_for_writing(filename.c_str(), elem_names,
                                        save_in_binary ?
#if MRPT_IS_BIG_ENDIAN
                                                       PLY_BINARY_BE
#else
