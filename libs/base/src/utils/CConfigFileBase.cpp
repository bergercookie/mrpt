...
void CConfigFileBase::write(const std::string &section, const std::string &name,
                            double value, const int name_padding_width,
                            const int value_padding_width,
                            const std::string &comment) {
  writeString(
      section, name,
      format(((std::abs(value) > 1e-4 && std::abs(value) < 1e4) || value == .0)
                 ? "%f"
                 : "%e",
             value),
...
void CConfigFileBase::write(const std::string &section, const std::string &name,
                            float value, const int name_padding_width,
                            const int value_padding_width,
                            const std::string &comment) {
  writeString(section, name,
              format(((std::abs(value) > 1e-4f && std::abs(value) < 1e4f) ||
                      value == .0f)
                         ? "%f"
                         : "%e",
                     value),
...
void CConfigFileBase::writeString(const std::string &section,
  ...
  if (name_padding_width >= 1)
    name_pad =
        mrpt::format("%*s", -name_padding_width,
  ...
  if (value_padding_width >= 1)
    value_pad =
        mrpt::format(" %*s", -value_padding_width,
...
uint64_t CConfigFileBase::read_uint64_t(const std::string &section,
                                        ...
                                        bool failIfNotFound) const {
  string s =
      readString(section, name, format("%lu", (long unsigned int)defaultValue),
                 failIfNotFound);
...
std::string CConfigFileBase::read_string_first_word(
  ...
  if (auxStrs.empty()) {
    if (failIfNotFound) {
      THROW_EXCEPTION(format("Value '%s' seems to be present in section '%s' "
                             "but, are all whitespaces??",
