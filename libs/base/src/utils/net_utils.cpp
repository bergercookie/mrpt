...
ERRORCODE_HTTP BASE_IMPEXP mrpt::utils::net::http_get(
  ...
  vector_byte data;
  ERRORCODE_HTTP ret =
      http_get(url, data, out_errormsg, port, auth_user, auth_pass,
...
ERRORCODE_HTTP BASE_IMPEXP mrpt::utils::net::http_request(
  ...
  // Reset output data:
  out_content.clear();
  if (out_http_responsecode)
    *out_http_responsecode = 0;
  if (out_headers)
    out_headers->clear();
  ...
  try {
    ...
    while (!content_length_read ||
      ...
      // Read until "Content-Length: XXX \r\n" is read or the whole message is
      // read,
      ...
      if (!all_headers_read) {
        const char *ptr =
            ::strstr(reinterpret_cast<const char *>(&buf[0]), "\r\n\r\n");
        if (ptr) {
          ...
          const char *ptr_len = ::strstr(
              reinterpret_cast<const char *>(&buf[0]), "Content-Length:");
          if (ptr_len) {
    ...
    if (out_http_responsecode)
      *out_http_responsecode = http_code;
    if (out_headers)
      *out_headers = rx_headers;
...
ERRORCODE_HTTP BASE_IMPEXP mrpt::utils::net::http_get(
    ...
    int *out_http_responsecode, mrpt::utils::TParameters<string> *extra_headers,
    mrpt::utils::TParameters<string> *out_headers, int timeout_ms) {
  return http_request("GET", "", url, out_content, out_errormsg, port,
                      auth_user, auth_pass, out_http_responsecode,
                      extra_headers, out_headers, timeout_ms);
}
 ...
 * This method has a timeout for the maximum time to wait for the DNS server.
  ...
  */
bool net::DNS_resolve_async(const std::string &server_name, std::string &out_ip,
...
bool mrpt::utils::net::Ping(const std::string &address, const int &max_attempts,
...
// different "count" argument for Windows and *NIX  systems
  ...
  string cmd_full_str =
      mrpt::format("%s %s %s %s", cmd_str.c_str(), count_arg_str.c_str(),
                   address.c_str(), redirection_str.c_str());
