...
string mrpt::system::lowerCase(const string &str) {
  ...
  transform(outStr.begin(), outStr.end(), // In
            outStr.begin(),               // Out
            (int (*)(int))tolower);
  return outStr;
}
 ...
 */
std::string mrpt::system::unitsFormat(const double val, int nDecimalDigits,
                                      bool middle_space) {
  char prefix;
  double mult;
  ...
  return format(middle_space ? "%.*f %c" : "%.*f%c", nDecimalDigits, val * mult,
                prefix);
}
...
/*---------------------------------------------------------------
                                                strtok
---------------------------------------------------------------*/
char *mrpt::system::strtok(char *str, const char *strDelimit,
...
void my_tokenize(const std::string &inString, const std::string &inDelimiters,
...
bool mrpt::system::strStarts(const std::string &s1, const std::string &s2) {
  return !mrpt::system::os::_strncmp(
      s1.c_str(), s2.c_str(), s2.size()); // if s1 is shorter it's not a problem
}
