...
CFileSystemWatcher::CFileSystemWatcher(const std::string &path)
  ...
  // Create watcher:
  m_wd = inotify_add_watch(m_fd, path.c_str(), IN_CLOSE_WRITE | IN_DELETE |
...
void CFileSystemWatcher::thread_win32_watch() {
  ...
  while (ReadDirectoryChangesW(
    ...
    for (;;) {
      ...
      ASSERTMSG_(fni->FileNameLength < 10000,
                 "Name length >10K... this is probably an error")

      int reqLen = WideCharToMultiByte(CP_UTF8, 0, fni->FileName,
                                       fni->FileNameLength >> 1, nullptr, 0,
                                       nullptr, nullptr);
      std::vector<char> tmpBuf(reqLen);
      int actLen = WideCharToMultiByte(CP_UTF8, 0, fni->FileName,
                                       fni->FileNameLength >> 1, &tmpBuf[0],
