...
/*---------------------------------------------------------------
      ...
      complete path plus name plus extension.
  ---------------------------------------------------------------*/
string mrpt::system::extractFileExtension(const string &filePath,
                                          bool ignore_gz) {
  if (filePath.size() < 2)
    return string("");
...
void mrpt::system::deleteFiles(const string &s) {
  ...
  int res = ::system(aux);
  if (res) {
    cerr << "[mrpt::system::deleteFiles] Warning: error invoking: " << aux
         << endl;
  }
  delete[] aux;
  MRPT_END
...
---------------------------------------------------------------*/
bool mrpt::system::deleteFilesInDirectory(const string &path,
                                          bool deleteDirectoryAsWell) {
  if (!directoryExists(path))
    return false;

  CDirectoryExplorer::TFileInfoList lstFiles;
  CDirectoryExplorer::explore(path, FILE_ATTRIB_ARCHIVE | FILE_ATTRIB_DIRECTORY,
                              lstFiles);
 ...
 * is passed in error_msg, a description of the error is saved there.
  */
bool mrpt::system::renameFile(const string &oldFileName,
...
std::string
mrpt::system::fileNameChangeExtension(const std::string &filePath,
...
bool mrpt::system::copyFile(const std::string &sourceFile,
  ...
  if (dir_exs) {
    if (outErrStr)
      *outErrStr = string("Is source a directory?: ") + org;
    return false;
  }
  ...
  if (!f_src) {
    if (outErrStr)
      *outErrStr = string("Source file exists but cannot open it... is file "
                          "being used?:  ") +
                   org;
    return false;
  }
...
// It exists, but cannot overwrite it:
      ...
      if (dwProp == INVALID_FILE_ATTRIBUTES) {
        if (outErrStr)
          *outErrStr = string("Cannot get file attributes for target file, "
      ...
      if (!f_trg) {
        if (outErrStr)
          *outErrStr = string("Cannot overwrite target file, even after "
      ...
      if (chmod(trg.c_str(), S_IRWXU | S_IRGRP | S_IROTH)) {
        if (outErrStr)
          *outErrStr = string("Cannot set file permissions for target file, "
                              "trying to remove a possible read-only attribute "
                              "after first attempt of copy failed, for: ") +
                       trg;
        fclose(f_src);
  ...
  while (0 != (nBytes = fread(buf, 1, 64 * 1024, f_src))) {
    if (nBytes != fwrite(buf, 1, nBytes, f_trg)) {
      if (outErrStr)
        *outErrStr = string("Error writing the contents of the target file "
                            "(disk full?): ") +
                     trg;
      fclose(f_src);
      fclose(f_trg);
  ...
  if (copyAttribs) {
    ...
    if (dwPropSrc == INVALID_FILE_ATTRIBUTES) {
      if (outErrStr)
        *outErrStr =
            string("Cannot get the file attributes for source file:  ") + org;
      return false;
    }
    ...
    if (dwPropTrg == INVALID_FILE_ATTRIBUTES) {
      if (outErrStr)
        *outErrStr =
            string("Cannot get the file attributes for target file:  ") + trg;
      return false;
    }
    ...
    dwPropTrg &= ~(FILE_ATTRIBUTE_ARCHIVE | FILE_ATTRIBUTE_HIDDEN |
                   FILE_ATTRIBUTE_NORMAL | FILE_ATTRIBUTE_NOT_CONTENT_INDEXED |
                   FILE_ATTRIBUTE_OFFLINE | FILE_ATTRIBUTE_READONLY |
                   FILE_ATTRIBUTE_SYSTEM | FILE_ATTRIBUTE_TEMPORARY);
    dwPropTrg |= dwPropSrc;
    ...
    if (!SetFileAttributesA(trg.c_str(), dwPropTrg)) {
      if (outErrStr)
        *outErrStr =
            string("Cannot set file attributes for target file: ") + trg;
      return false;
    }
#else
...
// ---------------------------------------------------------------
std::string
mrpt::system::filePathSeparatorsToNative(const std::string &filePath) {
...
std::string mrpt::system::getShareMRPTDir() {
  ...
  if (is_first) {
    ...
    // Install dir:
    sPaths.push_back(string(MRPT_CMAKE_INSTALL_PREFIX) +
                     string("/share/mrpt/"));
