	...
	std::string* outErrStr, bool copyAttribs)
{
	...
	if (!f_src)
	{
		if (outErrStr)
			*outErrStr =
				string(
					"Source file exists but cannot open it... is file being "
					"used?:  ") +
				org;
		return false;
	}
...
// It exists, but cannot overwrite it:
				...
				if (outErrStr)
					*outErrStr =
						string(
							...
							"trying "
							"to remove a possible read-only attribute after "
							"first attempt of copy failed, for: ") +
						trg;
				fclose(f_src);
				return false;
			}
			...
			if (!f_trg)
			{
				if (outErrStr)
					*outErrStr =
						string(
							"Cannot overwrite target file, even after changing "
							"file attributes! : ") +
						trg;
				fclose(f_src);
				return false;
	...
	while (0 != (nBytes = fread(buf, 1, 64 * 1024, f_src)))
	{
		if (nBytes != fwrite(buf, 1, nBytes, f_trg))
		{
			if (outErrStr)
				*outErrStr =
					string(
						"Error writing the contents of the target file (disk "
						"full?): ") +
					trg;
			fclose(f_src);
			fclose(f_trg);
