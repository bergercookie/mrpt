...
/*---------------------------------------------------------------
                                        vsprintf
---------------------------------------------------------------*/
int os::vsprintf(char *buf, size_t bufSize, const char *format,
...
/*---------------------------------------------------------------
                                vsnprintf
---------------------------------------------------------------*/
int os::vsnprintf(char *buf, size_t bufSize, const char *format,
                  va_list args) noexcept {
#if defined(_MSC_VER)
#if (_MSC_VER >= 1400)
  // Use a secure version in Visual Studio 2005:
...
void os::fclose(FILE *f) {
  if (!f)
    THROW_EXCEPTION("Trying to close a nullptr 'FILE*' descriptor")
  ::fclose(f);
}
...
/*---------------------------------------------------------------
                                                memcpy
---------------------------------------------------------------*/
void os::memcpy(void *dest, size_t destSize, const void *src,
...
/*---------------------------------------------------------------
                                        _strtoull
  ---------------------------------------------------------------*/
uint64_t mrpt::system::os::_strtoull(const char *nptr, char **endptr,
                                     int base) {
#ifdef MRPT_OS_WINDOWS
...
const char *sLicenseTextF = "                     Mobile Robot Programming "
                            "Toolkit (MRPT)                \n"
                            "                          http://www.mrpt.org/    "
                            "                          \n"
                            "                                                  "
                            "                          \n"
                            " Copyright (c) 2005-%Y, Individual contributors, "
                            "see AUTHORS file         \n"
                            " See: http://www.mrpt.org/Authors - All rights "
                            "reserved.                   \n"
...
const std::string &mrpt::system::getMRPTLicense() {
  ...
  if (!sLicenseTextReady) {
    // Automatically update the last year of the copyright to the compilation
    // date:
    time_t rawtime;
    struct tm *timeinfo;
    time(&rawtime);
...

{
  ...
  if (CreateProcessA(NULL, (LPSTR)command.c_str(), NULL, NULL, true, 0, NULL,
                     NULL, &SI, &PI))
...

  {
    ...
    std::cerr << "[launchProcess] Couldn't spawn process. Error msg: " << str
              << std::endl;
  ...
  if (mrpt_shared_first_call) {
    ...
    for (int attempt = 0;; attempt++) {
      ...
      case 2: {
        ...
        dir = mrpt::system::extractFileDirectory(std::string(curExe)) +
              "/../share/mrpt/";
      } break;
#endif
