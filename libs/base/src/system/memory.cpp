...
void *mrpt::system::os::aligned_malloc(size_t bytes, size_t alignment) {
  ...
  if (ret)
    THROW_EXCEPTION("posix_memalign returned an error.");
  return ptr;
#else
 ...
 * alignment required.
  ...
  */
void *mrpt::system::os::aligned_realloc(void *old_ptr, size_t bytes,
                                        size_t alignment) {
#if defined(HAVE_ALIGNED_MALLOC)
#if defined(__GNUC__) && !defined(__MINGW32__)
...
public:
  ...
  CAuxPSAPI_Loader() {
    ...
    if (m_dll) {
      m_ptr =
          (TGetProcessMemoryInfo)GetProcAddress(m_dll, "GetProcessMemoryInfo");
    }
  }
  ~CAuxPSAPI_Loader() {
    if (m_dll) {
...
unsigned long mrpt::system::getMemoryUsage() {
  ...
  if (PSAPI_LOADER.GetProcessMemoryInfo(GetCurrentProcess(), &pmc,
                                        sizeof(pmc))) {
