...
void CParticleFilterCapable::performResampling(
  ...
  computeResampling(PF_options.resamplingMethod, log_ws, indxs,
                    out_particle_count);
...
void CParticleFilterCapable::computeResampling(
  ...
  case CParticleFilter::prResidual: {
    ...
    size_t N_rnd =
        out_particle_count >= R
            ? (out_particle_count - R)
            : 0; // # of particles to be drawn randomly (the "residual" part)
    ...
    if (N_rnd) // If there are "residual" part (should be virtually always!)
    {
      ...
      // perform resampling:
      vector<double> Q;
      mrpt::math::cumsum_tmpl<vector<double>, vector<double>, double>(linW_mod,
                                                                      Q);
      Q[M - 1] = 1.1;
  ...
  default:
    THROW_EXCEPTION(
        format("ERROR: Unknown resampling method selected: %i", method));
  };
...
void CParticleFilterCapable::prediction_and_update(
  ...
  case CParticleFilter::pfAuxiliaryPFStandard:
    prediction_and_update_pfAuxiliaryPFStandard(action, observation,
                                                PF_options);
    break;
...
void CParticleFilterCapable::prediction_and_update_pfAuxiliaryPFStandard(
  ...
  MRPT_UNUSED_PARAM(observation);
  MRPT_UNUSED_PARAM(PF_options);
  THROW_EXCEPTION("Algorithm 'pfAuxiliaryPFStandard' is not implemented in "
                  "inherited class!");
}
/*---------------------------------------------------------------
                                        prediction_and_update_...
...
void CParticleFilterCapable::prediction_and_update_pfAuxiliaryPFOptimal(
  ...
  MRPT_UNUSED_PARAM(observation);
  MRPT_UNUSED_PARAM(PF_options);
  THROW_EXCEPTION("Algorithm 'pfAuxiliaryPFOptimal' is not implemented in "
                  "inherited class!");
}
...
void CParticleFilterCapable::prepareFastDrawSample(
  ...
  if (PF_options.adaptiveSampleSize) {
    ...
    if (PF_options.resamplingMethod != CParticleFilter::prMultinomial)
      THROW_EXCEPTION("resamplingMethod must be 'prMultinomial' for a dynamic "
                      "number of particles!");
    ...
    // Prepare buffers:
    m_fastDrawAuxiliary.CDF.resize(1 + PARTICLE_FILTER_CAPABLE_FAST_DRAW_BINS,
                                   0);
...
size_t CParticleFilterCapable::fastDrawSample(
  ...
  if (PF_options.adaptiveSampleSize) {
    ...
    MRPT_END_WITH_CLEAN_UP(printf("\n[CParticleFilterCapable::fastDrawSample] "
                                  "DEBUG: draw=%f, CDF=%f CDF_next=%f\n",
                                  draw, CDF, CDF_next););
  } else {
    // --------------------------------------------------------
    // CASE: Static number of particles:
    ...
    if (m_fastDrawAuxiliary.alreadyDrawnNextOne >=
      ...
      THROW_EXCEPTION("Have you called 'fastDrawSample' more times than the "
                      "sample size? Did you forget calling 'prepareFastCall' "
                      "before?");
