...
TEST(SLERP_tests, correctShortestPath) {
  { // Both poses at (0,0,0) angles:
    ...
    const CPose3D pose_b(0, 0, 0, DEG2RAD(0), DEG2RAD(0), DEG2RAD(0));
    {
      ...
      EXPECT_NEAR(0, (pose_interp.getHomogeneousMatrixVal() -
                      expected.getHomogeneousMatrixVal())
                         .array()
                         .abs()
                         .sum(),
                  1e-4)
  ...
  { // Poses at yaw=-+179deg
    ...
    EXPECT_NEAR(.0, (CPose3D(expected).getHomogeneousMatrixVal() -
                     CPose3D(pose_interp).getHomogeneousMatrixVal())
                        .array()
                        .abs()
                        .sum(),
                1e-4)
