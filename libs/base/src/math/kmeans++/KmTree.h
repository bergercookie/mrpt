...
public:
  ...
  // Given k cluster centers, this runs a full k-means iterations, choosing the
  // next set of
  ...
  // Choose k initial centers for k-means using the kmeans++ seeding procedure.
  // The resulting
...
private:
  struct Node {
    ...
    Scalar
        opt_cost; // Min cost for putting all points in this node in 1 cluster
    Node *lower_node, *upper_node;  // Child nodes
    mutable int kmpp_cluster_index; // The cluster these points are assigned to
                                    // or -1 if variable
  };

  // Helper functions for constructor
  Node *BuildNodes(Scalar *points, int first_index, int last_index,
                   char **next_node_data);
  Scalar GetNodeCost(const Node *node, Scalar *center) const;

  // Helper functions for DoKMeans step
  Scalar DoKMeansStepAtNode(const Node *node, int k, int *candidates,
                            Scalar *centers, Scalar *sums, int *counts,
                            int *assignment) const;
  bool ShouldBePruned(Scalar *box_median, Scalar *box_radius, Scalar *centers,
  ...
  Scalar SeedKmppUpdateAssignment(const Node *node, int new_cluster,
                                  Scalar *centers, Scalar *dist_sq) const;
