...
Scalar KmTree::DoKMeansStep(int k, Scalar *centers, int *assignment) const {
  ...
  // Find nodes
  Scalar result = DoKMeansStepAtNode(top_node_, num_candidates, candidates,
...
KmTree::Node *KmTree::BuildNodes(Scalar *points, int first_index,
                                 int last_index, char **next_node_data) {
  // Allocate the node
  ...
  node->lower_node =
      BuildNodes(points, first_index, first_index + size1 - 1, next_node_data);
  node->upper_node =
...
Scalar KmTree::DoKMeansStepAtNode(const Node *node, int k, int *candidates,
                                  Scalar *centers, Scalar *sums, int *counts,
  ...
  if (node->lower_node != 0) {
    ...
    for (int i = 0; i < k; i++)
      if (!ShouldBePruned(node->median, node->radius, centers, closest_i,
                          candidates[i]))
    ...
    if (new_k > 1) {
      Scalar result =
          DoKMeansStepAtNode(node->lower_node, new_k, new_candidates, centers,
...
bool KmTree::ShouldBePruned(Scalar *box_median, Scalar *box_radius,
                            Scalar *centers, int best_index,
...
Scalar KmTree::SeedKMeansPlusPlus(int k, Scalar *centers) const {
  ...
  for (int new_cluster = 1; new_cluster < k; new_cluster++) {
    ...
    memcpy(centers + new_cluster * d_, points_ + point_indices_[i] * d_,
           d_ * sizeof(Scalar));
    total_cost =
...
Scalar KmTree::SeedKmppUpdateAssignment(const Node *node, int new_cluster,
                                        Scalar *centers,
                                        Scalar *dist_sq) const {
  // See if we can assign all points in this node to one cluster
  if (node->kmpp_cluster_index >= 0) {
    if (ShouldBePruned(node->median, node->radius, centers,
                       node->kmpp_cluster_index, new_cluster))
    ...
    if (ShouldBePruned(node->median, node->radius, centers, new_cluster,
      ...
      for (int i = node->first_point_index;
           i < node->first_point_index + node->num_points; i++)
        dist_sq[i] = PointDistSq(points_ + point_indices_[i] * d_,
                                 centers + new_cluster * d_, d_);
      return GetNodeCost(node, centers + new_cluster * d_);
    }

    // It may be that the a leaf-node point is equidistant from the new center
    // or old
    if (node->lower_node == 0)
      return GetNodeCost(node, centers + node->kmpp_cluster_index * d_);
  }
  ...
  // Recurse
  Scalar cost =
      SeedKmppUpdateAssignment(node->lower_node, new_cluster, centers,
                               dist_sq) +
