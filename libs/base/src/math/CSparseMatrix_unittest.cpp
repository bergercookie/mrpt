...
void generateRandomSparseMatrix(size_t N, size_t M, size_t nEntries,
                                CSparseMatrix &MAT) {
  MAT.clear();
  ...
  for (size_t i = 0; i < nEntries; i++) {
    MAT.insert_entry(mrpt::random::randomGenerator.drawUniform32bit() % N,
...
typedef void (*TMatrixSMOperator)(const CSparseMatrix &M1,
                                  const CSparseMatrix &M2, CSparseMatrix &res);
typedef void (*TMatrixDenseOperator)(const CMatrixDouble &M1,
                                     const CMatrixDouble &M2,
                                     CMatrixDouble &res);

void do_matrix_op_test(size_t nRows1, size_t nCols1, size_t nNonZeros1,
...
void op_sparse_add(const CSparseMatrix &M1, const CSparseMatrix &M2,
                   CSparseMatrix &res) {
  res = M1 + M2;
}
void op_dense_add(const CMatrixDouble &M1, const CMatrixDouble &M2,
                  CMatrixDouble &res) {
  res = M1 + M2;
}
...
void op_sparse_multiply_AB(const CSparseMatrix &M1, const CSparseMatrix &M2,
                           CSparseMatrix &res) {
  res = M1 * M2;
}
void op_dense_multiply_AB(const CMatrixDouble &M1, const CMatrixDouble &M2,
                          CMatrixDouble &res) {
  res = M1 * M2;
}

TEST(SparseMatrix, Op_Multiply_AB) {
  do_matrix_op_test(1, 1, 0, 1, 1, 0, &op_sparse_multiply_AB,
                    &op_dense_multiply_AB);
  do_matrix_op_test(1, 1, 1, 1, 1, 1, &op_sparse_multiply_AB,
                    &op_dense_multiply_AB);
  do_matrix_op_test(2, 2, 1, 2, 2, 1, &op_sparse_multiply_AB,
                    &op_dense_multiply_AB);
  do_matrix_op_test(10, 20, 33, 20, 15, 33, &op_sparse_multiply_AB,
                    &op_dense_multiply_AB);
  do_matrix_op_test(8, 34, 100, 34, 3, 100, &op_sparse_multiply_AB,
                    &op_dense_multiply_AB);
}
