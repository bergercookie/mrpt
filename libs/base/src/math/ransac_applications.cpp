...
namespace math {
template <typename T>
void ransac3Dplane_fit(const CMatrixTemplateNumeric<T> &allData,
                       const vector_size_t &useIndices,
                       vector<CMatrixTemplateNumeric<T>> &fitModels) {
  ASSERT_(useIndices.size() == 3);

  TPoint3D p1(allData(0, useIndices[0]), allData(1, useIndices[0]),
              allData(2, useIndices[0]));
  TPoint3D p2(allData(0, useIndices[1]), allData(1, useIndices[1]),
              allData(2, useIndices[1]));
  TPoint3D p3(allData(0, useIndices[2]), allData(1, useIndices[2]),
...
template <typename T>
void ransac3Dplane_distance(const CMatrixTemplateNumeric<T> &allData,
  ...
  for (size_t i = 0; i < N; i++) {
    const double d = plane.distance(TPoint3D(allData.get_unsafe(0, i),
                                             allData.get_unsafe(1, i),
...
/** Return "true" if the selected points are a degenerate (invalid) case.
  */
template <typename T>
bool ransac3Dplane_degenerate(const CMatrixTemplateNumeric<T> &allData,
...
void mrpt::math::ransac_detect_3D_planes(
  ...
  for (;;) {
    ...
    ransac.execute(remainingPoints, ransac3Dplane_fit, ransac3Dplane_distance,
    ...
    if (this_best_inliers.size() >= min_inliers_for_valid_plane) {
      ...
      out_detected_planes.push_back(std::make_pair(
          this_best_inliers.size(),
          TPlane(double(this_best_model(0, 0)), double(this_best_model(0, 1)),
...
EXPLICIT_INST_ransac_detect_3D_planes(float)
  ...
  namespace math {
  template <typename T>
  void ransac2Dline_fit(const CMatrixTemplateNumeric<T> &allData,
                        const vector_size_t &useIndices,
                        vector<CMatrixTemplateNumeric<T>> &fitModels) {
  ...
  template <typename T>
  void
  ransac2Dline_distance(const CMatrixTemplateNumeric<T> &allData,
                        const vector<CMatrixTemplateNumeric<T>> &testModels,
                        const T distanceThreshold,
                        unsigned int &out_bestModelIndex,
                        vector_size_t &out_inlierIndices) {
    ...
    if (testModels.empty())
      return; // No model, no inliers.

    ASSERTMSG_(testModels.size() == 1,
               format("Expected testModels.size()=1, but it's = %u",
  ...
  /** Return "true" if the selected points are a degenerate (invalid) case.
    */
  template <typename T>
  bool ransac2Dline_degenerate(const CMatrixTemplateNumeric<T> &allData,
                               const mrpt::vector_size_t &useIndices) {
    MRPT_UNUSED_PARAM(allData);
    MRPT_UNUSED_PARAM(useIndices);
    return false;
...
void mrpt::math::ransac_detect_2D_lines(
  ...
  while (size(remainingPoints, 2) >= 2) {
    ...
    if (this_best_inliers.size() >= min_inliers_for_valid_line) {
      // Add this plane to the output list:
      out_detected_lines.push_back(std::make_pair(
          this_best_inliers.size(),
          TLine2D(double(this_best_model(0, 0)), double(this_best_model(0, 1)),
                  double(this_best_model(0, 2)))));
