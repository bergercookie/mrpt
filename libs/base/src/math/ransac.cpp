...
bool RANSAC_Template<NUMTYPE>::execute(
  ...
  while (N > trialcount) {
    ...
    if (!degenerate) {
      dist_func(data, MODELS, NUMTYPE(distanceThreshold), bestModelIdx,
                inliers);
    ...
    if (update_estim_num_iters) {
      ...
      MRPT_LOG_DEBUG(format("Iter #%u Estimated number of iters: %u  "
    ...
    MRPT_LOG_DEBUG(format("trial %u out of %u \r", (unsigned int)trialcount,
  ...
  if (size(out_best_model, 1) > 0) { // We got a solution
