...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void math::closestFromPointToSegment(const double &Px, const double &Py,
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void math::closestFromPointToLine(const double &Px, const double &Py,
                                  const double &x1, const double &y1,
                                  const double &x2, const double &y2,
                                  double &out_x, double &out_y) {
  if (x1 == x2 && y1 == y2) {
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
double
math::closestSquareDistanceFromPointToLine(const double &Px, const double &Py,
                                           const double &x1, const double &y1,
                                           const double &x2, const double &y2) {
  if (x1 == x2 && y1 == y2) {
...
/*---------------------------------------------------------------
                                                Intersect
  ---------------------------------------------------------------*/
bool math::SegmentsIntersection(const double x1, const double y1,
                                const double x2, const double y2,
                                const double x3, const double y3,
                                const double x4, const double y4, double &ix,
                                double &iy) {
...
bool math::SegmentsIntersection(const double x1, const double y1,
                                const double x2, const double y2,
                                const double x3, const double y3,
                                const double x4, const double y4, float &ix,
                                float &iy) {
  double x, y;
...
/*---------------------------------------------------------------
                                                Intersect
  ---------------------------------------------------------------*/
bool math::pointIntoPolygon2D(const double &px, const double &py,
                              unsigned int polyEdges, const double *poly_xs,
  ...
  for (i = 0; i < polyEdges; i++) {
        ...
        (poly_ys[j] <= py && py < poly_ys[i]))   // a downward crossing
    {
      ...
      if (px - poly_xs[i] < ((poly_xs[j] - poly_xs[i]) * (py - poly_ys[i]) /
                             (poly_ys[j] - poly_ys[i])))
        res = !res;
    }
    j = i;
...
/*---------------------------------------------------------------
                                                Intersect
  ---------------------------------------------------------------*/
double math::distancePointToPolygon2D(const double &px, const double &py,
  ...
  for (i = 0; i < polyEdges; i++) {
    ...
    double d = minimumDistanceFromPointToSegment(px, py, poly_xs[j], poly_ys[j],
                                                 poly_xs[i], poly_ys[i],
                                                 closestX, closestY);
...
/*---------------------------------------------------------------
                                        minDistBetweenLines
 --------------------------------------------------------------- */
bool math::minDistBetweenLines(const double &p1_x, const double &p1_y,
  ...
  dist = (double)sqrt(square(pa_x - pb_x) + square(pa_y - pb_y) +
                      square(pa_z - pb_z));
...
bool math::RectanglesIntersection(
  ...
  for (int idx = 0; idx < 4; idx++) {
    if (math::SegmentsIntersection(R1_x_min, R1_y_min, R1_x_max, R1_y_min,
                                   xs[idx], ys[idx], xs[(idx + 1) % 4],
                                   ys[(idx + 1) % 4], ix, iy))
      return true;
    if (math::SegmentsIntersection(R1_x_max, R1_y_min, R1_x_max, R1_y_max,
                                   xs[idx], ys[idx], xs[(idx + 1) % 4],
                                   ys[(idx + 1) % 4], ix, iy))
      return true;
    if (math::SegmentsIntersection(R1_x_max, R1_y_max, R1_x_min, R1_y_max,
                                   xs[idx], ys[idx], xs[(idx + 1) % 4],
                                   ys[(idx + 1) % 4], ix, iy))
      return true;
    if (math::SegmentsIntersection(R1_x_min, R1_y_max, R1_x_min, R1_y_min,
                                   xs[idx], ys[idx], xs[(idx + 1) % 4],
                                   ys[(idx + 1) % 4], ix, iy))
      return true;
  }
...
bool intersectInCommonPlane(const T3D &o1, const U3D &o2,
...
bool intersectInCommonLine(const mrpt::math::TSegment3D &s1,
  ...
  size_t i1 = 0;
  while (abs(lin.director[i1]) < geometryEpsilon)
    i1++;
...
bool intersectInCommonLine(const TSegment2D &s1, const TSegment2D &s2,
                           const TLine2D &lin, TObject2D &obj) {
  // Move in a free coordinate, searching for minima and maxima
...
inline void unsafeProjectPoint(const TPoint3D &point, const CPose3D &pose,
                               TPoint2D &newPoint) {
  double dummy;
  pose.composePoint(point.x, point.y, point.z, newPoint.x, newPoint.y, dummy);
}
void unsafeProjectPolygon(const TPolygon3D &poly, const CPose3D &pose,
                          TPolygon2D &newPoly) {
  size_t N = poly.size();
  newPoly.resize(N);
...
bool intersect(const TPolygonWithPlane &p1, const TLine3D &l2, double &d,
               double bestKnown) {
...
bool intersect(const TPolygonWithPlane &p1, const TPolygonWithPlane &p2,
               TObject3D &obj) {
...
void math::TPolygonWithPlane::getPlanes(const vector<TPolygon3D> &oldPolys,
...
bool math::intersect(const TSegment3D &s1, const TSegment3D &s2,
                     TObject3D &obj) {
  TObject3D irr;
...
bool math::intersect(const TSegment3D &s1, const TLine3D &r1, TObject3D &obj) {
  ...
  if (obj.isLine()) {
    ...
    // Segment's line and the other line cross in a point, which may be or not
    // be inside the segment.
    TPoint3D p;
    if (!obj.getPoint(p))
      return false;
    return s1.contains(p);
...
bool math::intersect(const TPlane &p1, const TPlane &p2, TObject3D &obj) {
  ...
  if ((abs(lin.director[0]) < geometryEpsilon) &&
    ...
    while (abs(lin.director[i1]) < geometryEpsilon)
      i1++;
    // At this point, i1 points to a coordinate (0->x, 1->y, 2->z) in which we
    // can move freely.
    // If we arbitrarily assign this coordinate to 0, we'll find a suitable base
    // point by solving both planes' equations.
    size_t c1 = (i1 + 1) % 3, c2 = (i1 + 2) % 3;
    lin.pBase[i1] = 0.0;
    lin.pBase[c1] = (p2.coefs[3] * p1.coefs[c2] - p1.coefs[3] * p2.coefs[c2]) /
                    lin.director[i1];
    lin.pBase[c2] = (p2.coefs[c1] * p1.coefs[3] - p1.coefs[c1] * p2.coefs[3]) /
                    lin.director[i1];
    lin.unitarize();
    obj = lin;
    return true;
...
bool math::intersect(const TSegment2D &s1, const TSegment2D &s2,
                     TObject2D &obj) {
  ...
  if (obj.isLine())
    return intersectInCommonLine(s1, s2, lin,
                                 obj); // Segments' lines are parallel
  else if (obj.getPoint(p))
    return s1.contains(p) && s2.contains(p); // Segments' lines cross in a point
  return false;
}
...
double math::getAngle(const TPlane &s1, const TPlane &s2) {
  ...
  double s = sqrt(n1 * n2);
  if (s < geometryEpsilon)
    throw std::logic_error("Invalid plane(s)");
  if (abs(s) < abs(c))
    return (c / s < 0) ? M_PI : 0;
  else
    return acos(c / s);
...
double math::getAngle(const TPlane &s1, const TLine3D &r2) {
  ...
  double s = sqrt(n1 * n2);
  if (s < geometryEpsilon)
    throw std::logic_error("Invalid plane or line");
  if (abs(s) < abs(c))
...
double math::getAngle(const TLine3D &r1, const TLine3D &r2) {
  ...
  double s = sqrt(n1 * n2);
  if (s < geometryEpsilon)
    throw std::logic_error("Invalid line(s)");
  if (abs(s) < abs(c))
    return (c / s < 0) ? M_PI : 0;
  else
    return acos(c / s);
...
void math::createFromPoseAndVector(const CPose3D &p, const double (&vector)[3],
                                   TLine3D &r) {
  CMatrixDouble44 m = p.getHomogeneousMatrixVal();
  for (size_t i = 0; i < 3; i++) {
    r.pBase[i] = m.get_unsafe(i, 3);
...
void math::createFromPoseAndVector(const TPose2D &p, const double (&vector)[2],
                                   TLine2D &r) {
  double c = cos(p.phi);
  double s = sin(p.phi);
...
void math::project3D(const TLine3D &line, const CPose3D &newXYpose,
                     TLine3D &newLine) {
  newXYpose.composePoint(line.pBase.x, line.pBase.y, line.pBase.z,
                         newLine.pBase.x, newLine.pBase.y, newLine.pBase.z);
...
void math::project3D(const TPlane &plane, const CPose3D &newXYpose,
                     TPlane &newPlane) {
  CMatrixDouble44 mat = newXYpose.getHomogeneousMatrixVal();
  for (size_t i = 0; i < 3; i++) {
    newPlane.coefs[i] = 0;
  ...
  // VORSICHT! NO INTENTEN HACER ESTO EN SUS CASAS (nota: comentar sí o sí, más
  // tarde)
...
void math::project3D(const TPolygon3D &polygon, const CPose3D &newXYpose,
...
void math::project3D(const TObject3D &object, const CPose3D &newXYpose,
                     TObject3D &newObject) {
...
void math::project2D(const TPoint2D &point,
                     const mrpt::poses::CPose2D &newXpose, TPoint2D &newPoint) {
  newPoint = newXpose + mrpt::poses::CPoint2D(point);
}

void math::project2D(const TLine2D &line, const CPose2D &newXpose,
                     TLine2D &newLine) {
...
void math::project2D(const TPolygon2D &line, const CPose2D &newXpose,
                     TPolygon2D &newLine) {
  size_t N = line.size();
  newLine.resize(N);
  for (size_t i = 0; i < N; i++)
...
void math::project2D(const TObject2D &obj, const CPose2D &newXpose,
                     TObject2D &newObject) {
  switch (obj.getType()) {
  case GEOMETRIC_TYPE_POINT: {
    TPoint2D p, p2;
...
bool math::intersect(const TPolygon2D &p1, const TSegment2D &s2,
                     TObject2D &obj) {
...
bool intersect(const TSegmentWithLine &s1, const TSegmentWithLine &s2,
               TObject2D &obj) {
...
bool math::intersect(const TPolygon2D & /*p1*/, const TPolygon2D & /*p2*/,
                     TObject2D & /*obj*/) {
...
bool math::intersect(const TPolygon3D &p1, const TSegment3D &s2,
                     TObject3D &obj) {
  TPlane p;
  if (!p1.getPlane(p))
    return false;
...
// Auxiliary function2
bool intersectAux(const TPolygon3D &p1, const TPlane &pl1, const TPolygon3D &p2,
...
bool compatibleBounds(const TPoint3D &min1, const TPoint3D &max1,
                      const TPoint3D &min2, const TPoint3D &max2) {
  for (size_t i = 0; i < 3; i++)
...
bool math::intersect(const TPolygon3D &p1, const TPolygon3D &p2,
                     TObject3D &obj) {
...
// Auxiliary function
inline void getPlanes(const std::vector<TPolygon3D> &polys,
...
void getMinAndMaxBounds(const std::vector<TPolygon3D> &v1,
  ...
  maxP.reserve(N);
  TPoint3D p1, p2;
  for (std::vector<TPolygon3D>::const_iterator it = v1.begin(); it != v1.end();
       ++it) {
...
size_t math::intersect(const std::vector<TPolygon3D> &v1,
  ...
  for (size_t i = 0; i < M; i++)
    for (size_t j = 0; j < N; j++)
      if (!compatibleBounds(minBounds1[i], maxBounds1[i], minBounds2[j],
                            maxBounds2[j]))
        continue;
...
void math::getRectangleBounds(const std::vector<TPoint2D> &poly, TPoint2D &pMin,
                              TPoint2D &pMax) {
  size_t N = poly.size();
  if (N < 1)
    throw logic_error("Empty polygon");
  pMin = poly[0];
  pMax = poly[0];
  for (size_t i = 1; i < N; i++) {
...
double math::distance(const TLine3D &r1, const TLine3D &r2) {
  if (abs(getAngle(r1, r2)) < geometryEpsilon)
    ...
    p.coefs[3] = -(p.coefs[0] * r1.pBase[0] + p.coefs[1] * r1.pBase[1] +
...
void math::getPrismBounds(const std::vector<TPoint3D> &poly, TPoint3D &pMin,
                          TPoint3D &pMax) {
  size_t N = poly.size();
  if (N < 1)
    throw logic_error("Empty polygon");
...
void createPlaneFromPoseAndAxis(const CPose3D &pose, TPlane &plane,
                                size_t axis) {
  plane.coefs[3] = 0;
...
void math::createPlaneFromPoseAndNormal(const CPose3D &pose,
...
void math::generateAxisBaseFromDirectionAndAxis(const double (&vec)[3],
                                                char coord,
  ...
  matrix(3, 3) = 1.0;
  for (size_t i = 0; i < 3; i++)
    matrix.set_unsafe(i, coord, vec[i]);
  ...
  matrix.set_unsafe(0, coord2,
                    matrix.get_unsafe(1, coord) * matrix.get_unsafe(2, coord1) -
                        matrix.get_unsafe(2, coord) *
                            matrix.get_unsafe(1, coord1));
  matrix.set_unsafe(1, coord2,
                    matrix.get_unsafe(2, coord) * matrix.get_unsafe(0, coord1) -
                        matrix.get_unsafe(0, coord) *
                            matrix.get_unsafe(2, coord1));
  matrix.set_unsafe(2, coord2,
                    matrix.get_unsafe(0, coord) * matrix.get_unsafe(1, coord1) -
                        matrix.get_unsafe(1, coord) *
                            matrix.get_unsafe(0, coord1));
}
...
double math::getRegressionLine(const vector<TPoint2D> &points, TLine2D &line) {
  ...
  return sqrt(eigenVal.get_unsafe(1 - selected, 1 - selected) /
...
double math::getRegressionLine(const vector<TPoint3D> &points, TLine3D &line) {
  ...
  covars.eigenVectors(eigenVec, eigenVal);
  size_t selected =
      getIndexOfMax(eigenVal.get_unsafe(0, 0), eigenVal.get_unsafe(1, 1),
  ...
  return sqrt((eigenVal.get_unsafe(i1, i1) + eigenVal.get_unsafe(i2, i2)) /
              eigenVal.get_unsafe(selected, selected));
}
...
double math::getRegressionPlane(const vector<TPoint3D> &points, TPlane &plane) {
  ...
  size_t selected =
      getIndexOfMin(eigenVal.get_unsafe(0, 0), eigenVal.get_unsafe(1, 1),
                    eigenVal.get_unsafe(2, 2));
  plane.coefs[3] = 0;
  for (size_t i = 0; i < 3; i++) {
  ...
  size_t i1 = (selected + 1) % 3, i2 = (selected + 2) % 3;
  return sqrt(eigenVal.get_unsafe(selected, selected) /
              (eigenVal.get_unsafe(i1, i1) + eigenVal.get_unsafe(i2, i2)));
}

void math::assemblePolygons(const std::vector<TSegment3D> &segms,
...
bool depthFirstSearch(const CSparseMatrixTemplate<unsigned char> &mat,
                      ...
                      std::vector<bool> &used, size_t searching,
                      unsigned char mask,
      ...
      if (!match)
        ...
        if (current.size() >= 2 && current[0].seg1 == i) {
          if (s2p != current[0].seg1Point) {
            ...
            continue; // Strange shape... not a polygon, although it'll be
...
void depthFirstSearch(const CSparseMatrixTemplate<unsigned char> &mat,
  ...
  for (size_t i = 0; i < used.size(); i++)
    if (!used[i])
      if (depthFirstSearch(mat, res, used, i, 0xf, cur))
        cur.clear();
}
void math::assemblePolygons(const std::vector<TSegment3D> &segms,
                            std::vector<TPolygon3D> &polys,
  ...
  transform(results.begin(), results.end(), polys.begin(),
            FCreatePolygon(segms));
  for (size_t i = 0; i < N; i++)
    if (!usedSegments[i])
      remainder.push_back(tmp[i]);
}

void math::assemblePolygons(const std::vector<TObject3D> &objs,
                            std::vector<TPolygon3D> &polys) {
...
inline bool intersect(const TSegmentWithLine &s1, const TLine2D &l2,
                      TObject2D &obj) {
  return intersect(l2, s1, obj);
}

bool math::splitInConvexComponents(const TPolygon2D &poly,
    ...
    for (size_t j = ii; j != i_; j = (j + 1) % N)
      if (intersect(segms[i].line, segms[j], obj) && obj.getPoint(pnt)) {
        TSegmentWithLine sTmp = TSegmentWithLine(
            pnt, segms[i].segment[(distance(pnt, segms[i].segment.point1) <
        ...
        for (size_t k = 0; (k < N) && !cross; k++)
          if (intersect(sTmp, segms[k], obj)) {
            if (obj.getPoint(pTmp) &&
                (distance(pTmp, sTmp.segment[0]) >= geometryEpsilon) &&
                (distance(pTmp, sTmp.segment[1]) >= geometryEpsilon))
              cross = true;
          }
        if (cross)
          continue;
        // At this point, we have a suitable point, although we must check if
        // the division is right.
        // We do this by evaluating, in the expanded segment's line, the next
        // and previous points. If both signs differ, proceed.
        ...
        if (splitInConvexComponents(p1, tempComps))
          components.insert(components.end(), tempComps.begin(),
                            tempComps.end());
        else
...
bool math::splitInConvexComponents(const TPolygon3D &poly,
                                   vector<TPolygon3D> &components) {
  TPlane p;
  if (!poly.getPlane(p))
    throw std::logic_error("Polygon is skew");
  CPose3D pose1, pose2;
  ...
  if (splitInConvexComponents(poly2D, components2D)) {
    ...
    transform(components2D.begin(), components2D.end(), components.begin(),
...
void math::getAngleBisector(const TLine2D &l1, const TLine2D &l2,
                            TLine2D &bis) {
  TPoint2D p;
  TObject2D obj;
...
void math::getAngleBisector(const TLine3D &l1, const TLine3D &l2,
                            TLine3D &bis) {
...
bool math::traceRay(const vector<TPolygonWithPlane> &vec, const CPose3D &pose,
                    double &dist) {
  dist = HUGE_VAL;
  double nDist = 0;
  TLine3D lin;
