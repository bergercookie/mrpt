...
protected:
  ...
  void test_gimbalLock(double YAW, double PITCH, double ROLL) {
    ...
    EXPECT_NEAR(0, std::abs((CPose3D(q1, 0, 0, 0).getAsVectorVal() -
  ...
  void test_toYPRAndBack(double YAW, double PITCH, double ROLL) {
    ...
    EXPECT_NEAR(
        0,
        (p1.getRotationMatrix() - p2.getRotationMatrix()).array().abs().sum(),
        1e-4)
    ...
    EXPECT_NEAR(
        0,
        (p1.getRotationMatrix() - p3.getRotationMatrix()).array().abs().sum(),
        1e-4)
...
TEST_F(QuaternionTests, crossProduct) {
  ...
  EXPECT_NEAR(
      0,
      std::abs(
          (p3.getAsVectorVal() - CPose3D(q3, 0, 0, 0).getAsVectorVal()).sum()),
      1e-6)
      << "q1 = " << q1 << endl
...
TEST_F(QuaternionTests, LnAndExpMatches) {
  ...
  for (size_t i = 0; i < N; i++)
    test_lnAndExpMatches(DEG2RAD(list_test_YPR_angles_degrees[i][0]),
...
TEST_F(QuaternionTests, ExpAndLnMatches) {
  ...
  for (size_t i = 0; i < N; i++)
    test_ExpAndLnMatches(list_test_XYZ[i][0], list_test_XYZ[i][1],
                         list_test_XYZ[i][2]);
}
