...
CAtan2LookUpTable::CAtan2LookUpTable(double xmin, double xmax, double ymin,
                                     double ymax, double resolution) noexcept {
  this->resize(xmin, xmax, ymin, ymax, resolution);
}

void CAtan2LookUpTable::resize(double xmin, double xmax, double ymin,
                               double ymax, double resolution) noexcept {
  const double def = .0;
...
void CAtan2LookUpTableMultiRes::resize(
  ...
  for (const auto &it : lst_resolutions2extensions) {
    ...
    mrpt::utils::CDynamicGrid<double> &dg = m_grids[exten] =
        mrpt::utils::CDynamicGrid<double>(-exten, exten, -exten, exten, res);
...
bool CAtan2LookUpTableMultiRes::atan2(double y, double x,
                                      double &out_atan2) const noexcept {
