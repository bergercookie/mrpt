...
void TPoint2D::fromString(const std::string &s) {
  ...
  ASSERTMSG_(mrpt::math::size(m, 1) == 1 && mrpt::math::size(m, 2) == 2,
...
void TPose2D::fromString(const std::string &s) {
  ...
  if (!m.fromMatlabStringFormat(s))
    THROW_EXCEPTION("Malformed expression in ::fromString");
  ASSERTMSG_(mrpt::math::size(m, 1) == 1 && mrpt::math::size(m, 2) == 3,
             "Wrong size of vector in ::fromString");
  x = m.get_unsafe(0, 0);
  y = m.get_unsafe(0, 1);
  phi = DEG2RAD(m.get_unsafe(0, 2));
}
mrpt::math::TPose2D mrpt::math::TPose2D::
operator+(const mrpt::math::TPose2D &b) const {
...
void TTwist3D::asString(std::string &s) const {
  s = mrpt::format("[%f %f %f  %f %f %f]", vx, vy, vz, mrpt::utils::RAD2DEG(wx),
...
TPoint3D::TPoint3D(const mrpt::poses::CPoint3D &p)
    : x(p.x()), y(p.y()), z(p.z()) {}
TPoint3D::TPoint3D(const mrpt::poses::CPose3D &p)
    : x(p.x()), y(p.y()), z(p.z()) {}
...
void TPose3D::asString(std::string &s) const {
  s = mrpt::format("[%f %f %f %f %f %f]", x, y, z, RAD2DEG(yaw), RAD2DEG(pitch),
                   RAD2DEG(roll));
}
...
TPose3DQuat::TPose3DQuat(const mrpt::poses::CPose3DQuat &p)
    : x(p.x()), y(p.y()), z(p.z()), qr(p.quat().r()), qx(p.quat().x()),
      qy(p.quat().y()), qz(p.quat().z()) {}
void TPose3DQuat::fromString(const std::string &s) {
  CMatrixDouble m;
  if (!m.fromMatlabStringFormat(s))
    THROW_EXCEPTION("Malformed expression in ::fromString");
  ASSERTMSG_(mrpt::math::size(m, 1) == 1 && mrpt::math::size(m, 2) == 7,
             "Wrong size of vector in ::fromString");
...
CStream &operator<<(mrpt::utils::CStream &out,
...
mrpt::utils::CStream &operator>>(mrpt::utils::CStream &in,
                                 mrpt::math::TSegment3D &s) {
  return in >> s.point1 >> s.point2;
}
mrpt::utils::CStream &operator<<(mrpt::utils::CStream &out,
...
double TSegment2D::signedDistance(const TPoint2D &point) const {
  ...
  // segment or not (being the longest segment one between the point and either
  // end of TSegment2D).
...
double TSegment3D::distance(const TPoint3D &point) const {
  return min(min(math::distance(point, point1), math::distance(point, point2)),
...
void TLine2D::getAsPose2DForcingOrigin(const TPoint2D &origin,
                                       mrpt::poses::CPose2D &outPose) const {
  if (!contains(origin))
...
TLine2D::TLine2D(const TPoint2D &p1,
                 const TPoint2D &p2) throw(std::logic_error) {
  if (p1 == p2)
...
TLine3D::TLine3D(const TPoint3D &p1,
                 const TPoint3D &p2) throw(std::logic_error) {
...
bool TPlane::contains(const TLine3D &line) const {
  if (!contains(line.pBase))
    return false; // Base point must be contained
...
void TPlane::getAsPose3DForcingOrigin(const TPoint3D &newOrigin,
  ...
  generateAxisBaseFromDirectionAndAxis(normal, 2, AXIS);
  for (size_t i = 0; i < 3; i++)
    AXIS.set_unsafe(i, 3, newOrigin[i]);
  pose = mrpt::poses::CPose3D(AXIS);
}
TPlane::TPlane(const TPoint3D &p1, const TPoint3D &p2,
               const TPoint3D &p3) throw(std::logic_error) {
...
TPlane::TPlane(const TLine3D &r1, const TLine3D &r2) throw(std::logic_error) {
  ...
  if (abs(coefs[0]) < geometryEpsilon && abs(coefs[1]) < geometryEpsilon &&
    ...
    // Use a line's director vector and both pBase's difference to create the
    // plane.
    double d[3];
    for (size_t i = 0; i < 3; i++)
...
template <class T> inline void removeUnusedVertices(T &poly) {
  size_t N = poly.size();
  if (N < 3)
    return;
...
template <class T> inline void removeRepVertices(T &poly) {
  size_t N = poly.size();
  if (N < 3)
    return;
...
inline double isLeft(const mrpt::math::TPoint2D &P0,
...
template <class T, int N> class FAddPoint {
public:
  T &object;
...
void TPolygon2D::createRegularPolygon(size_t numEdges, double radius,
                                      TPolygon2D &poly) {
...
inline void TPolygon2D::createRegularPolygon(size_t numEdges, double radius,
                                             TPolygon2D &poly,
...
bool TPolygon3D::contains(const TPoint3D &point) const {
  ...
  if (point.x + geometryEpsilon < pMin.x ||
      ...
      point.y > pMax.y + geometryEpsilon || point.z > pMax.z + geometryEpsilon)
    return false;
  TPlane plane;
  if (!getPlane(plane))
    throw std::logic_error("Polygon does not conform a plane");
...
void TPolygon3D::createRegularPolygon(size_t numEdges, double radius,
                                      TPolygon3D &poly) {
  if (numEdges < 3 || abs(radius) < geometryEpsilon)
    throw std::logic_error("Invalid arguments for regular polygon creations");
  poly.resize(numEdges);
...
inline void TPolygon3D::createRegularPolygon(size_t numEdges, double radius,
                                             TPolygon3D &poly,
                                             const mrpt::poses::CPose3D &pose) {
  createRegularPolygon(numEdges, radius, poly);
  for (size_t i = 0; i < numEdges; i++)
    pose.composePoint(poly[i][0], poly[i][1], poly[i][2], poly[i][0],
                      poly[i][1], poly[i][2]);
}
...
void TObject2D::getPoints(const std::vector<TObject2D> &objs,
...
void TObject2D::getSegments(const std::vector<TObject2D> &objs,
...
void TObject2D::getLines(const std::vector<TObject2D> &objs,
...
void TObject2D::getPolygons(const std::vector<TObject2D> &objs,
...
BASE_IMPEXP mrpt::utils::CStream &operator>>(mrpt::utils::CStream &in,
                                             mrpt::math::TTwist2D &o);
BASE_IMPEXP mrpt::utils::CStream &operator<<(mrpt::utils::CStream &out,
                                             const mrpt::math::TTwist2D &o);
