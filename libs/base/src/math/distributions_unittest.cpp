...
TEST(distributions, normalPDF_vector) {
  ...
  EXPECT_NEAR(normalPDF(x0, x0, COV), 0.02592116832548877620,
              eps); // sprintf('%.20f',mvnpdf([0;0;0],[0;0;0],S))
  EXPECT_NEAR(normalPDF(x2, x2, COV), 0.02592116832548877620,
              eps); // sprintf('%.20f',mvnpdf([0;0;0],[0;0;0],S))

  EXPECT_NEAR(normalPDF(x1, x0, COV), 0.02061240910323311470,
              eps); // sprintf('%.20f',mvnpdf([1;0;0],[0;0;0],S))
  EXPECT_NEAR(normalPDF(x2, x0, COV), 0.00008423820480102986,
              eps); // sprintf('%.20f',mvnpdf([1;2;3],[0;0;0],S))

  EXPECT_NEAR(normalPDF(x1, COV), 0.02061240910323311470,
              eps); // sprintf('%.20f',mvnpdf([1;0;0],[0;0;0],S))
  EXPECT_NEAR(normalPDF(x2, COV), 0.00008423820480102986,
              eps); // sprintf('%.20f',mvnpdf([1;0;0],[0;0;0],S))
}
...
TEST(distributions, noncentralChi2PDF_CDF) {
  ...
  EXPECT_NEAR(mrpt::math::noncentralChi2PDF_CDF(1, 1, 1.0).second,
              0.477249868051821, eps2);
  EXPECT_NEAR(mrpt::math::noncentralChi2PDF_CDF(1, 1, 2.0).second,
              0.652756536682270, eps2);
  EXPECT_NEAR(mrpt::math::noncentralChi2PDF_CDF(1, 1, 3.0).second,
              0.764784149631031, eps2);
  // MATLAB: ncx2pdf(1:3,1,1)
  EXPECT_NEAR(mrpt::math::noncentralChi2PDF_CDF(1, 1, 1.0).first,
              0.226466623457311, eps2);
  EXPECT_NEAR(mrpt::math::noncentralChi2PDF_CDF(1, 1, 2.0).first,
              0.137103272271503, eps2);
  EXPECT_NEAR(mrpt::math::noncentralChi2PDF_CDF(1, 1, 3.0).first,
              0.090852330823658, eps2);

  // MATLAB: ncx2cdf(1:3,2,3)
  EXPECT_NEAR(mrpt::math::noncentralChi2PDF_CDF(2, 3, 1.0).second,
              0.121825497229364, eps2);
  EXPECT_NEAR(mrpt::math::noncentralChi2PDF_CDF(2, 3, 2.0).second,
              0.252206942426039, eps2);
  EXPECT_NEAR(mrpt::math::noncentralChi2PDF_CDF(2, 3, 3.0).second,
              0.378499822919087, eps2);
  // MATLAB: ncx2pdf(1:3,2,3)
  EXPECT_NEAR(mrpt::math::noncentralChi2PDF_CDF(2, 3, 1.0).first,
              0.128765424775546, eps2);
  EXPECT_NEAR(mrpt::math::noncentralChi2PDF_CDF(2, 3, 2.0).first,
              0.129923687128879, eps2);
  EXPECT_NEAR(mrpt::math::noncentralChi2PDF_CDF(2, 3, 3.0).first,
              0.121500177080913, eps2);
}
