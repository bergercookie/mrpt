...
void realft(float data[], unsigned long n) {
  ...
  for (i = 2; i <= (n >> 2); i++) // Case i=1 done separately below.
  {
    ...
    h1r = c1 *
          (data[i1] +
           data[i3]); // The two separate transforms are separated out of data.
    h1i = c1 * (data[i2] - data[i4]);
    h2r = -c2 * (data[i2] + data[i4]);
    h2i = c2 * (data[i1] - data[i3]);
    data[i1] = (float)(h1r + wr * h2r - wi * h2i); // Here they are recombined
                                                   // to form the true transform
...
otherwise
  ...
  */
void rdft2d(int n1, int n2, int isgn, FFT_TYPE **a, FFT_TYPE *t, int *ip,
            FFT_TYPE *w) {
...
*/
void cdft2d(int n1, int n2, int isgn, FFT_TYPE **a, FFT_TYPE *t, int *ip,
            FFT_TYPE *w) {
  void makewt(int nw, int *ip, FFT_TYPE *w);
  void cdft(int n, int isgn, FFT_TYPE *a, int *ip, FFT_TYPE *w);
  int n, i, j, i2;
...
/*---------------------------------------------------------------
                                                fft_real
 ---------------------------------------------------------------*/
void math::fft_real(CVectorFloat &in_realData, CVectorFloat &out_FFT_Re,
...
/*---------------------------------------------------------------
                                                dft2_real
 ---------------------------------------------------------------*/
void math::dft2_real(const CMatrixFloat &in_data, CMatrixFloat &out_real,
...
/*---------------------------------------------------------------
                                                idft2_real
 ---------------------------------------------------------------*/
void math::idft2_real(const CMatrixFloat &in_real, const CMatrixFloat &in_imag,
...
/*---------------------------------------------------------------
        ...
        sign ->  -1: DFT, 1:IDFT
 ---------------------------------------------------------------*/
void myGeneralDFT(int sign, const CMatrixFloat &in_real,
                  const CMatrixFloat &in_imag, CMatrixFloat &out_real,
                  CMatrixFloat &out_imag) {
  ASSERT_(in_real.getRowCount() == in_imag.getRowCount())
  ASSERT_(in_real.getColCount() == in_imag.getColCount())
...
/*---------------------------------------------------------------
                                                dft2_complex
 ---------------------------------------------------------------*/
void math::dft2_complex(const CMatrixFloat &in_real,
                        const CMatrixFloat &in_imag, CMatrixFloat &out_real,
                        CMatrixFloat &out_imag) {
  MRPT_START
...
/*---------------------------------------------------------------
                                                idft2_complex
 ---------------------------------------------------------------*/
void math::idft2_complex(const CMatrixFloat &in_real,
                         const CMatrixFloat &in_imag, CMatrixFloat &out_real,
                         CMatrixFloat &out_imag) {
  MRPT_START
...
double math::averageLogLikelihood(const CVectorDouble &logWeights,
...
double math::averageWrap2Pi(const CVectorDouble &angles) {
  ...
  if (W_phi_L)
    phi_L /= static_cast<double>(W_phi_L); // [0,2pi]
  if (W_phi_R)
    phi_R /= static_cast<double>(W_phi_R); // [-pi,pi]
...
string math::MATLAB_plotCovariance2D(const CMatrixFloat &cov,
                                     const CVectorFloat &mean,
                                     const float &stdCount, const string &style,
...
string math::MATLAB_plotCovariance2D(const CMatrixDouble &cov,
  ...
  return str;
  MRPT_END_WITH_CLEAN_UP(cerr << "The matrix that led to error was: " << endl
                              << cov << endl;)
}
...
double mrpt::math::interpolate2points(const double x, const double x0,
...
void mrpt::math::medianFilter(const std::vector<double> &inV,
                              std::vector<double> &outV, const int &_winSize,
  ...
  for (size_t k = 0; k < sz; ++k) {
    ...
    outV[k] = (auxSz % 2) ? (aux[auxMPoint])
                          : (0.5 * (aux[auxMPoint - 1] +
                                    aux[auxMPoint])); // If the window is even,
...
template <class T>
void noncentralChi2OneIteration(T arg, T &lans, T &dans, T &pans,
                                unsigned int &j) {
  double tol = -50.0;
  if (lans < tol) {
...
mrpt::math::noncentralChi2PDF_CDF(unsigned int degreesOfFreedom,
                                  double noncentrality, double arg,
                                  double eps) {
  ASSERTMSG_(noncentrality >= 0.0 && arg >= 0.0 && eps > 0.0,
  ...
  return std::make_pair(0.5 * ao * density,
                        std::min(1.0, std::max(0.0, ao * probability)));
}

double mrpt::math::chi2PDF(unsigned int degreesOfFreedom, double arg,
                           double accuracy) {
  return mrpt::math::noncentralChi2PDF_CDF(degreesOfFreedom, 0.0, arg, accuracy)
      .first;
}
