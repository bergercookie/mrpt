...
TEST(Matrices, inv_4x4_fix) {
  ...
  CHECK_AND_RET_ERROR((AInv - C).array().abs().sum() > 1e-4,
...
TEST(Matrices, inv_6x6_fix) {
  ...
  CHECK_AND_RET_ERROR(std::isnan(C(0, 0)) || !std::isfinite(C(0, 0)) ||
...
TEST(Matrices, fromMatlabStringFormat) {
  ...
  const char *mat2 =
      " [ 	  -8.2	 9.232 ; -2e+2		+6 ; 1.000  7 ] "; // With tabs
                                                                   // and
                                                                   // spaces...
...

  {
    ...
    if (!M1b.fromMatlabStringFormat(mat1) ||
        (CMatrixFixedNumeric<double, 2, 3>(vals1) - M1b).array().abs().sum() >
            1e-4)
      GTEST_FAIL() << mat1;
  }
...

  {
    ...
    if (!M2b.fromMatlabStringFormat(mat2) ||
        (CMatrixFixedNumeric<double, 3, 2>(vals2) - M2b).array().abs().sum() >
            1e-4)
      GTEST_FAIL() << mat2;
  }
