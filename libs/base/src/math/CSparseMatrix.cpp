...
void CSparseMatrix::insert_entry(const size_t row, const size_t col,
                                 const double val) {
  if (!isTriplet())
...
void CSparseMatrix::add_AB(const CSparseMatrix &A, const CSparseMatrix &B) {
  ASSERT_(A.getColCount() == B.getColCount() &&
...
void CSparseMatrix::multiply_Ab(const mrpt::math::CVectorDouble &b,
...
void CSparseMatrix::cs2dense(const cs &SM, CMatrixDouble &d_M) {
  ...
  {               // It's in triplet form.
    for (int idx = 0; idx < SM.nz; ++idx)
      d_M(SM.i[idx], SM.p[idx]) += SM.x[idx]; // += since the convention is that
                                              // duplicate (i,j) entries add to
                                              // each other.
  } else {                                    // Column compressed format:
    ...
    for (int j = 0; j < SM.n; j++) {
      ...
      for (int p = p0; p < p1; p++)
        d_M(SM.i[p], j) += SM.x[p]; // += since the convention is that duplicate
                                    // (i,j) entries add to each other.
    }
  }
}
...
CSparseMatrix::CholeskyDecomp::CholeskyDecomp(const CSparseMatrix &SM)
    : m_symbolic_structure(nullptr), m_numeric_structure(nullptr),
      m_originalSM(&SM) {
...
/** Return the vector from a back-substitution step that solves: Ux=b   */
void CSparseMatrix::CholeskyDecomp::backsub(const double *b, double *sol,
  ...
  cs_pvec(m_symbolic_structure->pinv, &tmp[0], &sol[0],
          N); /* sol = PERMUT'*tmp */
...
void CSparseMatrix::CholeskyDecomp::update(const CSparseMatrix &new_SM) {
  ASSERTMSG_(m_originalSM->sparse_matrix.nzmax == new_SM.sparse_matrix.nzmax,
             "New matrix doesn't have the same sparse structure!");
  ASSERTMSG_(m_originalSM->sparse_matrix.n == new_SM.sparse_matrix.n,
             "New matrix doesn't have the same sparse structure!");
