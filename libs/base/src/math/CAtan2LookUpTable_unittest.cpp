...
void atan2_lut_test(const LUT_CLASS &atan2lut, const double SIZE,
                    const double max_deg_errors, const double skip_area) {
  for (int i = 0; i < 1000; i++) {
    const double x =
        mrpt::random::randomGenerator.drawUniform(-.5 * SIZE, .5 * SIZE);
    const double y =
        mrpt::random::randomGenerator.drawUniform(-.5 * SIZE, .5 * SIZE);
    ...
    EXPECT_LT(std::abs(atan2_good - atan2_lut),
...
TEST(CAtan2LookUpTable, ValidValidTest) {
  ...
  mrpt::math::CAtan2LookUpTable atan2lut(-.5 * SIZE, .5 * SIZE, -.5 * SIZE,
                                         .5 * SIZE, RES);
