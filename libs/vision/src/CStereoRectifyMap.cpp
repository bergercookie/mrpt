...
CStereoRectifyMap::CStereoRectifyMap()
    : m_alpha(-1), m_resize_output(false),
      m_enable_both_centers_coincide(false), m_resize_output_value(0, 0),
...
void CStereoRectifyMap::setFromCamParams(
  ...
  const cv::Size trg_size =
      m_resize_output
          ? cv::Size(m_resize_output_value.x,
...
CV_EXPORTS void stereoRectify( const Mat& cameraMatrix1, const Mat& distCoeffs1,
                                                           const Mat&
cameraMatrix2, const Mat& distCoeffs2,
...
CV_OUT Mat& R2,
                                                                 CV_OUT Mat& P1,
CV_OUT Mat& P2, CV_OUT Mat& Q,
                                                                 double alpha,
Size newImageSize=Size(),
                                                                 CV_OUT Rect*
validPixROI1=0, CV_OUT Rect* validPixROI2=0,
                                                                 int
flags=CALIB_ZERO_DISPARITY );
...
CV_EXPORTS void stereoRectify( InputArray cameraMatrix1, InputArray distCoeffs1,
                                                           InputArray
cameraMatrix2, InputArray distCoeffs2,
                                                           Size imageSize,
...
*/
  ...
  cv::stereoRectify(K1, D1, K2, D2, img_size, R, T, R1, R2, P1, P2, Q,
                    m_enable_both_centers_coincide ? cv::CALIB_ZERO_DISPARITY
                                                   : 0);
#elif MRPT_OPENCV_VERSION_NUM < 0x230
  // OpenCV 2.1.X - 2.2.X
  cv::stereoRectify(K1, D1, K2, D2, img_size, R, T, R1, R2, P1, P2, Q, m_alpha,
...
// Rest of arguments -> default
  ...
  cv::initUndistortRectifyMap(K1, D1, R1, P1, real_trg_size, CV_16SC2,
                              _mapx_left, _mapy_left);
  cv::initUndistortRectifyMap(K2, D2, R2, P2, real_trg_size, CV_16SC2,
                              _mapx_right, _mapy_right);
...
void CStereoRectifyMap::rectify(const mrpt::utils::CImage &in_left_image,
  ...
  const CvSize trg_size =
      m_resize_output ? cvSize(m_resize_output_value.x, m_resize_output_value.y)
                      : cvSize(ncols, nrows);

  out_left_image.resize(trg_size.width, trg_size.height,
                        in_left_image.isColor() ? 3 : 1,
                        in_left_image.isOriginTopLeft());
  out_right_image.resize(trg_size.width, trg_size.height,
                         in_left_image.isColor() ? 3 : 1,
                         in_left_image.isOriginTopLeft());
...
void CStereoRectifyMap::rectify(mrpt::utils::CImage &left_image,
  ...
  if (use_internal_mem_cache) {
    m_cache1.resize(trg_size.width, trg_size.height,
                    left_image.isColor() ? 3 : 1, left_image.isOriginTopLeft());
    m_cache2.resize(trg_size.width, trg_size.height,
                    right_image.isColor() ? 3 : 1,
  ...
  if (use_internal_mem_cache) {
    ...
    left_image.resize(trg_size.width, trg_size.height,
                      left_image.isColor() ? 3 : 1,
                      left_image.isOriginTopLeft());
    right_image.resize(trg_size.width, trg_size.height,
                       right_image.isColor() ? 3 : 1,
                       right_image.isOriginTopLeft());
    ...
    // Move the internal pointers: but we deallocate the old contents and needed
    // to allocate this one
...
void CStereoRectifyMap::rectify(
  ...
  // Rectify images:
  this->rectify(stereo_image_observation.imageLeft,
                stereo_image_observation.imageRight, use_internal_mem_cache);
...
void CStereoRectifyMap::rectify_IPL(const void *srcImg_left,
                                    const void *srcImg_right, void *outImg_left,
  ...
  const CvMat mapx_left = cvMat(nrows_out, ncols_out, CV_16SC2,
                                const_cast<int16_t *>(&m_dat_mapx_left[0]));
  const CvMat mapy_left = cvMat(nrows_out, ncols_out, CV_16UC1,
                                const_cast<uint16_t *>(&m_dat_mapy_left[0]));
  const CvMat mapx_right = cvMat(nrows_out, ncols_out, CV_16SC2,
                                 const_cast<int16_t *>(&m_dat_mapx_right[0]));
  const CvMat mapy_right = cvMat(nrows_out, ncols_out, CV_16UC1,
  ...
  cv::remap(src1, dst1, mapx1, mapy1, static_cast<int>(m_interpolation_method),
            cv::BORDER_CONSTANT, cvScalarAll(0));
  cv::remap(src2, dst2, mapx2, mapy2, static_cast<int>(m_interpolation_method),
            cv::BORDER_CONSTANT, cvScalarAll(0));
#endif
  MRPT_END
}

const mrpt::utils::TStereoCamera &
CStereoRectifyMap::getRectifiedImageParams() const {
  if (!isSet())
    THROW_EXCEPTION("Error: setFromCamParams() must be called before.")
  return m_rectified_image_params;
}

const mrpt::utils::TCamera &
CStereoRectifyMap::getRectifiedLeftImageParams() const {
  if (!isSet())
    THROW_EXCEPTION("Error: setFromCamParams() must be called before.")
...
const mrpt::utils::TCamera &
CStereoRectifyMap::getRectifiedRightImageParams() const {
  if (!isSet())
    THROW_EXCEPTION("Error: setFromCamParams() must be called before.")
