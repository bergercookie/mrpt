...
**
************************************************************************************************/
void CFeatureExtraction::extractFeaturesORB(const mrpt::utils::CImage &inImg,
                                            CFeatureList &feats,
...
// The detector and descriptor
  ...
  Ptr<cv::ORB> orb =
      cv::ORB::create(n_feats_2_extract, options.ORBOptions.scale_factor,
                      options.ORBOptions.n_levels);
  orb->detectAndCompute(cvImg, Mat(), cv_feats, cv_descs,
                        use_precomputed_feats);
#endif
    ...
    for (size_t k = 0; k < n_feats; ++k) {
      ...
      */
      ...
      if (options.ORBOptions.extract_patch && options.patchSize > 0) {
        inImg.extract_patch(feats[k]->patch, round(feats[k]->x) - patch_size_2,
  ...
  // The "min-distance" filter is done by means of a 2D binary matrix where each
  // cell is marked when one
  // feature falls within it. This is not exactly the same than a pure
  // "min-distance" but is pretty close
  // and for large numbers of features is much faster than brute force search of
  // kd-trees.
  // (An intermediate approach would be the creation of a mask image updated for
  // each accepted feature, etc.)
  ...
  while (c_feats < n_max_feats && k < n_feats) {
    ...
    if (do_filter_min_dist) {
      ...
      if (section_idx_x > 0)
        occupied_sections.set_unsafe(section_idx_x - 1, section_idx_y, true);
      if (section_idx_y > 0)
        occupied_sections.set_unsafe(section_idx_x, section_idx_y - 1, true);
    ...
    if (options.ORBOptions.extract_patch && options.patchSize > 0) {
      ...
      inImg.extract_patch(
          ft->patch, round(ft->x) - patch_size_2, round(ft->y) - patch_size_2,
          options.patchSize,
...
#if MRPT_OPENCV_VERSION_NUM < 0x300
  Ptr<Feature2D> orb = Algorithm::create<Feature2D>("Feature2D.ORB");
  orb->operator()(cvImg, Mat(), cv_feats, cv_descs,
                  true /* use_precomputed_feats */);
#else
  Ptr<cv::ORB> orb = cv::ORB::create(n_feats, options.ORBOptions.scale_factor,
                                     options.ORBOptions.n_levels);
  orb->detectAndCompute(cvImg, Mat(), cv_feats, cv_descs,
                        true /* use_precomputed_feats */);
#endif
