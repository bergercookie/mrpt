...
//			saveToConfigFile
// --------------------------------------------------
void TMultiResDescOptions::saveToConfigFile(mrpt::utils::CConfigFileBase &cfg,
  ...
  if (!descriptors.hasDescriptorMultiSIFT())
    ...
    for (int k = 0; k < (int)multiScales.size(); ++k) {
      ...
      for (int m = 0; m < (int)multiOrientations[k].size(); ++m) {
        ...
        out.printf(" ·· [D] ");
        for (int n = 0; n < (int)descriptors.multiSIFTDescriptors[k][m].size();
             ++n)
...
// CONSTRUCTOR
CFeature::CFeature()
    : x(0.0f), y(0.0f), ID(0), patchSize(21), type(featNotDefined),
      track_status(status_IDLE), response(0.0), orientation(0.0), scale(0.0),
      user_flags(0), nTimesSeen(1), nTimesNotSeen(0), nTimesLastSeen(0),
      depth(0), initialDepth(0), p3D(), multiScales(), multiOrientations(),
      multiHashCoeffs(), descriptors() {}

// Ctor
CFeature::TDescriptors::TDescriptors()
    : SIFT(), SURF(), SpinImg(), SpinImg_range_rows(0), PolarImg(0, 0),
      LogPolarImg(0, 0), polarImgsNoRotation(false), ORB() {}
...
float CFeature::descriptorDistanceTo(const CFeature &oFeature,
  ...
  if (descriptorToUse == descAny) {
    if (descriptors.hasDescriptorSIFT())
      ...
      THROW_EXCEPTION("Feature has no descriptors and descriptorToUse=descAny")
  }
  ...
  case descLogPolarImages: {
    float minAng;
    return descriptorLogPolarImgDistanceTo(oFeature, minAng,
                                           normalize_distances);
  }
  case descORB:
    return float(descriptorORBDistanceTo(oFeature));
  default:
    THROW_EXCEPTION_FMT("Unknown value for 'descriptorToUse'=%u",
...
float CFeature::descriptorSIFTDistanceTo(const CFeature &oFeature,
  ...
  ASSERT_(this->descriptors.hasDescriptorSIFT() &&
  ...
  if (normalize_distances)
    dist /= this->descriptors.SIFT.size();
  dist = sqrt(dist);
  if (normalize_distances)
    dist /= 64.0f;
  return dist;
...
float CFeature::descriptorSURFDistanceTo(const CFeature &oFeature,
  ...
  ASSERT_(this->descriptors.hasDescriptorSURF() &&
  ...
  if (normalize_distances)
    dist /= this->descriptors.SURF.size();
  dist = sqrt(dist);
  if (normalize_distances)
    dist /= 0.20f; // JL: Ad-hoc value! Investigate where does this come from...
  return dist;
...
float CFeature::descriptorSpinImgDistanceTo(
  ...
  ASSERT_(this->descriptors.SpinImg.size() ==
          oFeature.descriptors.SpinImg.size());
  ASSERT_(this->descriptors.hasDescriptorSpinImg() &&
...
// --------------------------------------------------
float CFeature::internal_distanceBetweenPolarImages(const CMatrix &desc1,
                                                    const CMatrix &desc2,
                                                    float &minDistAngle,
  ...
  for (delta = 0; delta < height; delta++) {
    ...
    for (i = 0; i < height; i++) {
      ...
      for (j = 0; j < width; j++) {
        ...
        dist += square(desc1.get_unsafe(i, j) - desc1_mean -
...
#ifdef LM_CORR_BIAS_MEAN
        dist += abs(desc1.get_unsafe(i, j) - desc1_mean -
                    desc2.get_unsafe(ii, j) + desc2_mean);
#else
...
float CFeature::descriptorPolarImgDistanceTo(const CFeature &oFeature,
  ...
  ASSERT_(size(descriptors.PolarImg, 1) ==
          size(oFeature.descriptors.PolarImg, 1))
  ASSERT_(size(descriptors.PolarImg, 2) ==
          size(oFeature.descriptors.PolarImg, 2))
  ASSERT_(this->descriptors.hasDescriptorPolarImg() &&
          oFeature.descriptors.hasDescriptorPolarImg())
  ASSERT_(size(descriptors.PolarImg, 1) > 1 &&
          size(descriptors.PolarImg, 2) > 1)
...
float CFeature::descriptorLogPolarImgDistanceTo(
  ...
  ASSERT_(size(descriptors.LogPolarImg, 1) ==
          size(oFeature.descriptors.LogPolarImg, 1))
  ASSERT_(size(descriptors.LogPolarImg, 2) ==
          size(oFeature.descriptors.LogPolarImg, 2))
  ASSERT_(this->descriptors.hasDescriptorLogPolarImg() &&
          oFeature.descriptors.hasDescriptorLogPolarImg())
  ASSERT_(size(descriptors.LogPolarImg, 1) > 1 &&
...
uint8_t CFeature::descriptorORBDistanceTo(const CFeature &oFeature) const {
  ASSERT_(this->descriptors.hasDescriptorORB() &&
  ...
  for (uint8_t k = 0; k < t_desc.size(); ++k) {
    ...
    uint8_t count; // from : Wegner, Peter (1960), "A technique for counting
                   // ones in a binary computer", Communications of the ACM 3
                   // (5): 322, doi:10.1145/367236.367286
...
void CFeature::saveToTextFile(const std::string &filename, bool APPEND) {
  ...
  //    HAS_SURF [SURF] HAS_MULTI [MULTI_i] HAS_ORB [ORB]"
  //    "%% \\---------------------- feature ------------------/ \\---------
  //    descriptors -------/\n"
  //    "%% with:\n"
  ...
  f.printf("%5u %2d %7.3f %7.3f %6.2f %6.2f %2d %6.3f ", (unsigned int)this->ID,
           (int)this->get_type(), this->x, this->y, this->orientation,
           this->scale, (int)this->track_status, this->response);
  ...
  if (this->descriptors.hasDescriptorMultiSIFT()) {
    for (int k = 0; k < int(this->multiScales.size()); ++k) {
      for (int m = 0; m < int(this->multiOrientations[k].size()); ++m) {
        f.printf("%.2f %6.2f ", this->multiScales[k],
                 this->multiOrientations[k][m]);
        f.printf("%4d ",
                 int(this->descriptors.multiSIFTDescriptors[k][m].size()));
        for (unsigned int n = 0;
             n < this->descriptors.multiSIFTDescriptors[k][m].size(); ++n)
...
void CFeatureList::saveToTextFile(const std::string &filename, bool APPEND) {
  ...
  if (!f.open(filename, APPEND))
    THROW_EXCEPTION("[CFeatureList::saveToTextFile] ERROR: File could not be "
                    "open for writing");

  f.printf("%% Dump of mrpt::vision::CFeatureList. Each line format is:\n"
           "%% ID TYPE X Y ORIENTATION SCALE TRACK_STATUS RESPONSE HAS_SIFT "
           "[SIFT] HAS_SURF [SURF]\n"
           "%% \\---------------------- feature ------------------/ "
           "\\--------- descriptors -------/\n"
           "%% with:\n"
           "%%  TYPE  : The used detector: 0:KLT, 1: Harris, 2: BCD, 3: SIFT, "
           "4: SURF, 5: Beacon, 6: FAST\n"
           "%%  HAS_* : 1 if a descriptor of that type is associated to the "
           "feature. \n"
           "%%  SIFT  : Present if HAS_SIFT=1: N DESC_0 ... DESC_N-1 \n"
           "%%  SURF  : Present if HAS_SURF=1: N DESC_0 ... DESC_N-1 \n"
  ...
  for (CFeatureList::iterator it = this->begin(); it != this->end(); ++it) {
    f.printf("%5u %2d %7.3f %7.3f %6.2f %6.2f %2d %6.3f ",
             (unsigned int)(*it)->ID, (int)(*it)->get_type(), (*it)->x,
             (*it)->y, (*it)->orientation, (*it)->scale,
...
void CFeatureList::loadFromTextFile(const std::string &filename) {
  ...
  while (parser.getNextLine(line)) {
    try {
      ...
      if (!(line >> feat->x >> feat->y))
        throw std::string("x,y");
      if (!(line >> feat->orientation))
        throw std::string("orientation");
      if (!(line >> feat->scale))
        throw std::string("scale");

      int _track_st;
      if (!(line >> _track_st))
        throw std::string("track_status");
      feat->track_status = TFeatureTrackStatus(_track_st);

      if (!(line >> feat->response))
        throw std::string("response");

      int hasSIFT;
      if (!(line >> hasSIFT))
        throw std::string("hasSIFT");
      if (hasSIFT) {
        size_t N;
        if (!(line >> N))
          throw std::string("SIFT-len");
        feat->descriptors.SIFT.resize(N);
        for (size_t i = 0; i < N; i++) {
          int val;
          line >> val;
          feat->descriptors.SIFT[i] = val; // DON'T read directly SIFT[i] since
                                           // it's a uint8_t, interpreted as a
                                           // cha
        }

        if (!line)
          throw std::string("SIFT-data");
      }

      int hasSURF;
      if (!(line >> hasSURF))
        throw std::string("hasSURF");
      if (hasSURF) {
        size_t N;
        if (!(line >> N))
          throw std::string("SURF-len");
        feat->descriptors.SURF.resize(N);
        for (size_t i = 0; i < N; i++)
          line >> feat->descriptors.SURF[i];
        if (!line)
          throw std::string("SURF-data");
      }

      push_back(feat_ptr);
    } catch (std::string &msg) {
      THROW_EXCEPTION(format("%s:%d: Error parsing features text file (%s).",
                             filename.c_str(),
                             (int)parser.getCurrentLineNumber(), msg.c_str()))
    }
  }
...
// getByID()
// --------------------------------------------------
void CFeatureList::getByMultiIDs(const vector<TFeatureID> &IDs,
                                 vector<CFeature::Ptr> &out,
...
// nearest(x,y)
// --------------------------------------------------
CFeature::Ptr CFeatureList::nearest(const float x, const float y,
                                    double &dist_prev) const {
  if (this->empty())
    return CFeature::Ptr();
...
// getMaxID()
// --------------------------------------------------
void CMatchedFeatureList::getMaxID(const TListIdx &idx, TFeatureID &firstListID,
...
// --------------------------------------------------
bool CFeature::getFirstDescriptorAsMatrix(
    mrpt::math::CMatrixFloat &desc) const {
  if (descriptors.hasDescriptorSIFT()) {
