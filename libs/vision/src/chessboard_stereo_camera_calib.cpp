...
bool mrpt::vision::checkerBoardStereoCalibration(
  ...
  try {
    ...
    vector<size_t> valid_image_pair_indices; // Indices in images[] which are
                                             // valid pairs to be used in the
                                             // optimization
    ...
    for (size_t i = 0; i < images.size(); i++) {
      ...
      for (int lr = 0; lr < 2; lr++) {
        ...
        if (!i) {
          ...
          if (imgSize[lr].y != (int)img_gray.getHeight() ||
            ...
            std::cout << "ERROR: All the images in each left/right channel "
        ...
        corners_found[lr] = mrpt::vision::findChessboardCorners(
            img_gray, dat.detected_corners, p.check_size_x, p.check_size_y,
      ...
      // Only if corners were detected perfectly in BOTH images, add this image
      // pair as a good one for optimization:
      if (corners_found[0] && corners_found[1]) {
        valid_image_pair_indices.push_back(i);

        // Consistency between left/right pair: the corners MUST BE ORDERED so
        // they match to each other,
        // and the criterion must be the same in ALL pairs to avoid rubbish
        // optimization:

        // Key idea: Generate a representative vector that goes along rows and
        // columns.
        // Check the angle of those director vectors between L/R images. That
        // can be done
        // via the dot product. Swap rows/columns order as needed.
        bool has_to_redraw_corners = false;

        const mrpt::math::TPoint2D pt_l0 = images[i].left.detected_corners[0],
        ...
        if (Al.x * Ar.x + Al.y * Ar.y < 0) {
          ...
          // Invert all corners:
          std::reverse(images[i].right.detected_corners.begin(),
        ...
        if (has_to_redraw_corners) {
          ...
          images[i].right.img_checkboard.drawChessboardCorners(
              images[i].right.detected_corners, check_size.x, check_size.y);
        }
      }
    ...
    if (p.verbose)
      std::cout << valid_image_pair_indices.size() << " valid image pairs."
                << std::endl;
    if (valid_image_pair_indices.empty()) {
      std::cout << "ERROR: No valid images. Perhaps the checkerboard size is "
                   "incorrect?"
                << std::endl;
      return false;
    }
    ...
    for (unsigned int y = 0; y < p.check_size_y; y++)
      for (unsigned int x = 0; x < p.check_size_x; x++)
        obj_points.push_back(TPoint3D(p.check_squares_length_X_meters * x,
    ...
    const size_t nObs =
        2 * N * CORNERS_COUNT; // total number of valid observations (px,py)
    ...
    // Initial state:
    // Within lm_stat: CArrayDouble<9> left_cam_params, right_cam_params; // [fx
    // fy cx cy k1 k2 k3 t1 t2]
    ...
    Eigen::MatrixXd H; // Hessian matrix  (Declared here so it's accessible as
    ...
    for (int calibRound = 0; calibRound < 2; calibRound++) {
      ...
      // Select which cam. parameters to optimize (for each camera!) and which
      // to leave fixed:
      ...
      if (calibRound == 1) {
        ...
        if (p.optimize_k2)
          vars_to_optimize.push_back(5);
        if (p.optimize_t1)
          vars_to_optimize.push_back(6);
        if (p.optimize_t2)
          vars_to_optimize.push_back(7);
        if (p.optimize_k3)
          vars_to_optimize.push_back(8);
      }
      ...
      err = recompute_errors_and_Jacobians(
          lm_stat, res_jacob, false /* no robust */, p.robust_kernel_param);
      ...
      while (iter < p.maxIters && !done) {
        ...
        if (llt.info() != Eigen::Success) {
          ...
          if (p.verbose && !done)
            cout << "LM iter#" << iter
                 << ": Couldn't solve LLt, retrying with larger lambda="
                 << lambda << endl;
          continue;
        }
        ...
        if (eps_norm < t2 * (eps_norm + t2)) {
          if (p.verbose)
            cout << "Termination criterion: eps_norm < t2*(eps_norm+t2): "
        ...
        double err_new = recompute_errors_and_Jacobians(
            new_lm_stat, new_res_jacob, use_robust, p.robust_kernel_param);
    ...
    for (size_t i = 0; i < valid_image_pair_indices.size(); i++) {
      ...
      // //!< At output, only will have an empty vector if the checkerboard was
      // not found in this image, or the predicted (reprojected) corners, which
      // were used to estimate the average square error.
      // std::vector<TPixelCoordf>		projectedPoints_undistorted;
      // //!< At output, like projectedPoints_distorted but for the undistorted
      // image.
      ...
      // Draw corners:
      dat_l.img_rectified.drawChessboardCorners(dat_l.projectedPoints_distorted,
                                                check_size.x, check_size.y);
      dat_r.img_rectified.drawChessboardCorners(dat_r.projectedPoints_distorted,
                                                check_size.x, check_size.y);
    }
...
void jacob_db_dp(const TPoint3D &p, // 3D coordinates wrt the camera
...
void jacob_deps_D_p_deps(const TPoint3D &p_D, // D (+) p
...
void jacob_dA_eps_D_p_deps(const CPose3D &A, const CPose3D &D,
                           const TPoint3D &p,
  ...
  const Eigen::Matrix<double, 1, 3> v(P.dot(dr1) + D.x(), P.dot(dr2) + D.y(),
                                      P.dot(dr3) + D.z());
...
void mrpt::vision::build_linear_system(const TResidualJacobianList &res_jac,
  ...
  for (size_t i = 0; i < N_Cs; i++) {
    for (size_t j = 0; j < N_Cs; j++) {
      ...
      H(nUnkPoses + N_Cs + i, nUnkPoses + N_Cs + j) =
          H_tot(nUnkPoses + 9 + var_indxs[i], nUnkPoses + 9 + var_indxs[j]);
    }
  }
...
struct TNumJacobData {
  ...
  TNumJacobData(const lm_stat_t &_lm_stat, const TPoint3D &_obj_point,
                const CPose3D &_left_cam, const CPose3D &_right2left,
                const Eigen::Matrix<double, 4, 1> &_real_obs)
      : lm_stat(_lm_stat), obj_point(_obj_point), left_cam(_left_cam),
        right2left(_right2left), real_obs(_real_obs) {}
};

void numeric_jacob_eval_function(const CArrayDouble<30> &x,
  ...
  TPixelCoordf px_l;
  project_point(dat.obj_point, cam_params.leftCamera, incrPose_l + dat.left_cam,
                px_l);
  TPixelCoordf px_r;
  project_point(dat.obj_point, cam_params.rightCamera,
                incrPose_rl + dat.right2left + incrPose_l + dat.left_cam, px_r);

  const Eigen::Matrix<double, 4, 1> predicted_obs(px_l.x, px_l.y, px_r.x,
                                                  px_r.y);
...
void eval_h_b(const CArrayDouble<2> &X, const TCamera &params,
              CArrayDouble<2> &out) {
  // Radial distortion:
...
void eval_deps_D_p(const CArrayDouble<6> &eps, const TPoint3D &D_p,
                   CArrayDouble<3> &out) {
...
void eval_dA_eps_D_p(const CArrayDouble<6> &eps, const TEvalData_A_eps_D_p &dat,
                     CArrayDouble<3> &out) {
  const CArrayDouble<6> incr(&eps[0]);
  const CPose3D incrPose = CPose3D::exp(incr);
...
double mrpt::vision::recompute_errors_and_Jacobians(
  ...
  for (size_t k = 0; k < N; k++) {
    ...
    for (size_t i = 0; i < nPts; i++) {
      ...
      const Eigen::Matrix<double, 4, 1> obs(px_obs_l.x, px_obs_l.y, px_obs_r.x,
                                            px_obs_r.y);
      ...
      project_point(lm_stat.obj_points[i], camparam_l,
                    lm_stat.left_cam_poses[k_idx], px_l);
      project_point(lm_stat.obj_points[i], camparam_r,
...
// ---------------------------------------------------------------------------------
      ...
      lm_stat.left_cam_poses[k_idx].composePoint(lm_stat.obj_points[i],
                                                 pt_wrt_left);
      (lm_stat.right2left_pose + lm_stat.left_cam_poses[k_idx])
      ...
      jacob_dh_db_and_dh_dc(pt_wrt_left, lm_stat.left_cam_params, dhl_dbl,
                            dhl_dcl);
      jacob_dh_db_and_dh_dc(pt_wrt_right, lm_stat.right_cam_params, dhr_dbr,
                            dhr_dcr);
      ...
      jacob_dA_eps_D_p_deps(lm_stat.right2left_pose,
                            lm_stat.left_cam_poses[k_idx],
...
// ---- end of theoretical Jacobians ----
      ...
      CArrayDouble<30>
          x0; // eps_l (6) + eps_lr (6) + l_camparams (9) + r_camparams (9)
      x0.setZero();
      ...
      TNumJacobData dat(lm_stat, lm_stat.obj_points[i],
                        lm_stat.left_cam_poses[k_idx], lm_stat.right2left_pose,
                        obs);
...
TStereoCalibParams::TStereoCalibParams()
    : check_size_x(7), check_size_y(9), check_squares_length_X_meters(0.02),
      check_squares_length_Y_meters(0.02), normalize_image(true),
      skipDrawDetectedImgs(false), verbose(true), maxIters(2000),
      optimize_k1(true), optimize_k2(true), optimize_k3(false),
      optimize_t1(false), optimize_t2(false), use_robust_kernel(false),
      robust_kernel_param(10), callback(nullptr), callback_user_param(nullptr) {
}
