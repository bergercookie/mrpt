	...
	TStereoCalibResults& out)
{
	try
	{
		...
		vector<size_t>
			valid_image_pair_indices;  // Indices in images[] which are
		...
		{
			std::cout
				<< "ERROR: No valid images. Perhaps the checkerboard size is "
				   "incorrect?"
				<< std::endl;
			return false;
		}
		...
		const size_t nObs =
			2 * N *
			CORNERS_COUNT;  // total number of valid observations (px,py)
		...
		Eigen::MatrixXd
			H;  // Hessian matrix  (Declared here so it's accessible as
		...
		for (size_t i = 0; i < valid_image_pair_indices.size(); i++)
		{
			...
			/** At output, only will have an empty vector if the checkerboard
			 * was not
			 * found in this image, or the predicted (reprojected) corners,
			 * which were
			 * used to estimate the average square error. */
			// std::vector<TPixelCoordf>		projectedPoints_distorted;
			/** At output, like projectedPoints_distorted but for the
			 * undistorted
			 * image. */
