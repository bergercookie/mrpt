...
/*-------------------------------------------------------------
                                        insertHashCoeffs
-------------------------------------------------------------*/
void vision::insertHashCoeffs(const CFeature::Ptr &feat,
      ...
      if (qTable.find(key1) != qTable.end() &&
        ...
        for (int n = 0; n < int(qTable[key1][key2][key3].size()); ++n) {
          ...
          if (thisID == feat->ID && thisScale == feat->multiScales[k])
            found = true;
          //                            cout << "Inserting in: " << key1 << ","
          //                            << key2 << "," << key3 << endl;
        }         // end-for
      }           // end-if
      if (!found) // Insert the new coefficients if they haven't been inserted
                  // before
...
/*-------------------------------------------------------------
                                        saveQTableToFile
-------------------------------------------------------------*/
void vision::saveQTableToFile(const TQuantizationTable &qTable,
  ...
  for (it1 = qTable.begin(); it1 != qTable.end(); ++it1)
    for (it2 = it1->second.begin(); it2 != it1->second.end(); ++it2)
      for (it3 = it2->second.begin(); it3 != it2->second.end(); ++it3) {
        mrpt::system::os::fprintf(f, "%d\t%d\t%d\t", it1->first, it2->first,
                                  it3->first);
  ...
  for (it = currentList.begin(); it != currentList.end(); ++it, ++curCounter) {
    ...
    if (!(*it)->descriptors.hasDescriptorMultiSIFT()) {
      ...
      if ((*it)->x + hpSize > (w - 1) || (*it)->y + hpSize > (h - 1) ||
        ...
        cout << "[relocalizeMultiDesc] WARNING: Feature too close to the "
                "border. MultiDescriptor computation skipped."
             << endl;
        continue;
      }
    ...
    for (int k = 0; k < int((*it)->multiOrientations[0].size()); ++k) {
              ...
              for (int m3 = c3mn; m3 < c3mx; ++m3) {
                ...
                if (it3 != it2->second.end()) {
                  for (int n = 0; n < int(it3->second.size()); ++n) {
                    featsToCompareMap[qTable[m1][m2][m3][n].first].push_back(
      ...
      // Use all orientations for each scale (we don't know a priori what's the
        ...
        for (int k1 = 0; k1 < int(baseFeat->multiScales.size()); ++k1) {
          ...
          for (int k2 = 0; k2 < int(nit->second.size()); ++k2) {
                ...
                nit->second[k2]) // if this scale has been selected
            {
              ...
              for (int k3 = 0; k3 < int(baseFeat->multiOrientations[k1].size());
                   ++k3) {
                // for each orientation of the feature 2
                for (int k4 = 0; k4 < int((*it)->multiOrientations[0].size());
                     ++k4) {
                  // check orientation
                  if (fabs(baseFeat->multiOrientations[k1][k3] -
                           (*it)->multiOrientations[0][k4]) > DEG2RAD(10))
                    continue;
                  //                                    cout << "Orientation
                  //                                    check passed" << endl;
                  //                                    cout << "HASH: " <<
                  ...
                  int dist = 0;
                  for (int d = 0; d < int(baseFeat->descriptors
                                              .multiSIFTDescriptors[k1][k3]
                                              .size());
                       ++d)
                    dist += fabs(
                        float(baseFeat->descriptors.multiSIFTDescriptors[k1][k3]
                                                                        [d]) -
                        float(
                            (*it)->descriptors.multiSIFTDescriptors[0][k4][d]));
...
void vision::updateBaseList(CFeatureList &baseList,
...
void vision::checkScalesAndFindMore(
  ...
  for (itBase = baseList.begin(); itBase != baseList.end(); ++m) {
    ...
    //        cout << m << " was last seen " << itBase->->first->nTimesLastSeen
    //        << " frames ago and has been seen " << itBase->first->nTimesSeen
    //        << " times so far." << endl;
    if (itBase->first->nTimesLastSeen > matchOpts.lastSeenThreshold &&
      ...
      //            {
      //                cout << m << " vs " <<
      //                (int)output.firstListCorrespondences.size() << endl;
      ...
      if (tidx != FEAT_FREE) {
        if (output.firstListFoundScales[m] == 0) {
          ...
          int res = computeMoreDescriptors(currentImage, currentList[tidx],
                                           itBase->first, true, computeOpts);
          ...
          if (output.firstListFoundScales[m] == (int)nscales) {
            ...
            int res = computeMoreDescriptors(currentImage, currentList[tidx],
                                             itBase->first, false, computeOpts);
            if (res == 0)
...
// x = col, y = row
bool vision::computeGradient(const CImage &image, const unsigned int x,
...
bool vision::computeMainOrientations(const CImage &image, const unsigned int x,
    ...
    for (int jj = -hPatchSize; jj <= hPatchSize; ++jj) {
      if (computeGradient(image, mx + ii, my + jj, mag, ori)) {
        ...
        if (dif > 0)
          nxbin = bin == NBINS - 1
                      ? 0
                      : bin + 1; // the other involved bin is the next one
        else
          nxbin = bin == 0
                      ? NBINS - 1
                      : bin - 1; // the other involved bin is the previous one
        ...
        oris[bin] +=
            w * fabs(dif2) /
            bin_size; // dif2 == 0 means that "ori" is equal to "bin_center"
        oris[nxbin] +=
            w * fabs(dif) /
            bin_size; // dif == 0 means that "ori" is equal to "nbin_center"
      }               // end-if
      else
        return false;
    } // end
  ...
  for (unsigned int k = 0; k < oris.size(); ++k) {
    ...
    double nv =
        k == oris.size() - 1 ? 0 : oris[k + 1]; // Next peak of the histogram

    if (oris[k] > pv && oris[k] > nv &&
        oris[k] > hist_mag_th) // It this peak is maximum and is over 0.8 of the
                               // maximum peak
    {
      // Check this formulae:
      ...
      double int_bin =
          k + 0.5 * (pv - nv) / (pv - 2.0 * oris[k] + nv); // Hess formulae
      int_bin = (int_bin < 0) ? NBINS + int_bin
...
-------------------------------------------------------------*/
void vision::interpolateHistEntry(vector<double> &oris, const double &cbin,
                                  const double &rbin, const double &obin,
                                  const double &mag, const int d, const int n) {
    ...
    for (int m = 0; m < 2; ++m)
      for (int l = 0; l < 2; ++l)
        if (ncbin_i + k >= 0 && ncbin_i + k < d && nrbin_i + m >= 0 &&
            nrbin_i + m < d) {
          int idx =
              ((nobin_i + l) % n) + n * (ncbin_i + k) + n * d * (nrbin_i + m);
          oris[idx] += mag * (1 - fabs(d_c + k)) * (1 - fabs(d_r + m)) *
                       (1 - fabs(d_o + l));
        }
  ...
  //    {
  //        double binCenter    = bin-1.5;                          // Center of
  //        the bin
  //        double colDistance  = fabs( cbin - binCenter );         // Distance
  //        to the center of the bin
  //        if( colDistance < 1.0 )                                 // If it is
  //        close enough
  //        {
  ...
  //        }
  //
  //        if( colIndex.size() == 2 && rowIndex.size() == 2 )       // We have
  //        found all we need (stop looping)
  //            break;
  //    } // end for
  //
...
void vision::computeHistogramOfOrientations(
  ...
  if (opts.computeHashCoeffs) {
    ...
    normalizeImage(
        thePatch, nimage); // Normalize to zero-mean and unit standard deviation
  }
...
TMultiResMatchingOutput vision::matchMultiResolutionFeatures(
  ...
  // depths)
  // We perform a prediction of the "List1" features and try to find its matches
  // within List2
  // --------------------------------------------------------
  // Algortihm summary:
  // --------------------------------------------------------
  ...
  for (leftFeatCounter = 0, it1 = list1.begin(); it1 != list1.end();
    ...
    double sRegionX0 = max(
        (*it1)->x - min((hWindow + 2 * (*it1)->nTimesLastSeen), mxsize), 0.0f);
    double sRegionX1 =
        min((*it1)->x + min((hWindow + 2 * (*it1)->nTimesLastSeen), mxsize),
            (float)imageW);
    double sRegionY0 = max(
        (*it1)->y - min((hWindow + 2 * (*it1)->nTimesLastSeen), mxsize), 0.0f);
    double sRegionY1 =
    ...
    for (rightFeatCounter = 0, it2 = list2.begin(); it2 != list2.end();
      ...
      if (!vision::computeMainOrientations(rightImage, (*it2)->x, (*it2)->y,
                                           patchSize, thisOris,
                                           computeOpts.sg2)) {
        //                if( (*it2)->ID == 193 )
        //                {
        //                    cout << (*it2)->ID << " bad orientation computed"
        //                    << endl;
        //                    mrpt::system::pause();
        //                }
        continue;
      ...
      if (matchOpts.useDepthFilter)
        ...
        lastScale =
            min((int)(*it1)->multiScales.size() - 1, (int)matchOpts.highScl1);
      }

      //            if( (*it2)->ID == 193 )
      //            {
      //                cout << "Searching in scales: " << firstScale << " to "
      //                << lastScale << endl;
      //                mrpt::system::pause();
      //            }
      ...
      for (int k1 = firstScale; k1 <= lastScale; ++k1)
        for (int k2 = 0; k2 < (int)(*it1)->multiOrientations[k1].size(); ++k2)
          for (int k3 = 0; k3 < (int)thisOris.size();
            ...
            if (fabs((*it1)->multiOrientations[k1][k2] - thisOris[k3]) <
                    matchOpts.oriThreshold ||
                fabs(M_2PI - fabs((*it1)->multiOrientations[k1][k2] -
              ...
              if (list2.size() >= 8) {
                ...
                list2.kdTreeNClosestPoint2DIdx((*it2)->x, (*it2)->y, 8, out_idx,
                                               out_dist_sqr);
                for (size_t kdcounter = 0; kdcounter < out_idx.size();
                     ++kdcounter) {
              ...
              // Candidate found at scale "k1" and orientation "k2" (list1) and
              // orientation "k3" (list2)
              // Compute descriptor for these conditions!

              //                            vector<int> desc, hashC;
              //                            TMultiResDescOptions auxOpts =
              //                            computeOpts;
              //                            auxOpts.computeHashCoeffs = false;

              // All the filters have been passed -> we can add all the
              // information into the feature
              // If it has a previous computed descriptor it is substituted by
              // this one
              ...
              bool oriFound = false;
              int wh = 0;
              for (int co = 0; co < (int)(*it2)->multiOrientations[0].size();
                   ++co)
              ...
              int dist = 0;
              if (!oriFound) // The feature hasn't got this orientation already
                             // -> compute the descriptor & coeffs
              {
                //                                if( (*it2)->ID == 193 )
                //                                    cout << "Orientation not
                //                                    found -> compute the
                //                                    descriptor." << endl;
                ...
                computeHistogramOfOrientations(rightImage, (*it2)->x, (*it2)->y,
                                               patchSize, thisOris[k3],
                                               thisDesc, computeOpts, thisHash);
                ...
                for (int n = 0; n < int(thisDesc.size()); n++)
                  dist += square(
                      (*it1)->descriptors.multiSIFTDescriptors[k1][k2][n] -
                      thisDesc[n]);
              } // end if
              else {
                //                                if( (*it2)->ID == 193 )
                ...
                for (int n = 0;
                     n <
                     (int)(*it2)->descriptors.multiSIFTDescriptors[0]
                                                                  [wh].size();
                     n++)
                  dist += square(
                      (*it1)->descriptors.multiSIFTDescriptors[k1][k2][n] -
                      (*it2)->descriptors.multiSIFTDescriptors[0][wh][n]);
              } // end else
              ...
              //                            if( (*it2)->ID == 193 )
              //                            {
              //                                cout << "Matching entre 193 y
              //                                "<< (*it2)->ID << " entre
              //                                escalas: " << firstScale << " y
              //                                " << lastScale << endl;
              //                                cout << "OR1: " <<
    ...
    if (minDist < matchOpts.matchingThreshold) {
      ...
      //            minIdx;
      //            cout << "] at scale " << minScale << " [" << minauxfscale <<
      //            "," << minauxlscale << "]";
      //            cout << " with depths: [" << mindepth1 << "," << mindepth2
      //            << "]" << endl;
    } // end if
  }   // end for it1
...
int vision::matchMultiResolutionFeatures(
  ...
  // Left image
  output1 = matchMultiResolutionFeatures(baseList1, auxList1, leftImage,
                                         matchOpts, computeOpts);
  updateBaseList(baseList1, auxList1,
  ...
  //            '+' );
  //            auxImg1.textOut( baseList1[k]->x, baseList1[k]->y, format("%d",
  //            scales1[k]), TColor::red );
  ...
  // Right image
  output2 = matchMultiResolutionFeatures(baseList2, auxList2, rightImage,
                                         matchOpts, computeOpts);
  updateBaseList(baseList2, auxList2,
  ...
  //            '+' );
  //            auxImg2.textOut( baseList2[k]->x, baseList2[k]->y, format("%d",
  //            scales2[k]), TColor::red );
  ...
  cout << "Left matches: " << output1.nMatches << " out of " << baseList1.size()
       << "," << auxList1.size() << endl;
  ...
  for (m = 0, itMatch = mList1.begin(); itMatch != mList1.end(); ++m) {
    ...
    if (itMatch->first->nTimesLastSeen > matchOpts.lastSeenThreshold ||
      ...
      if (tidx != FEAT_FREE) {
        if (scales1[m] == 0) {
          cout << "Left feature " << m << " found in scale 0!" << endl;
          int res = computeMoreDescriptors(leftImage, auxList1[tidx],
                                           itMatch->first, true, computeOpts);
          if (res == 0)
            cout << "LF LOWSCALES Out of bounds!!" << endl;
          // mrpt::system::pause();
        } else {
          ...
          if (scales1[m] == nScales - 1) {
            cout << "Left feature " << m << " found in last scale!"
                 << endl; // computeMoreDescriptors( auxList1[k1], leftImage,
                          // false, itMatch->first );
            cout << "tidx=" << tidx << endl;
            int res = computeMoreDescriptors(
                leftImage, auxList1[tidx], itMatch->first, false, computeOpts);
            if (res == 0)
              cout << "LF HIGHSCALES Out of bounds!!" << endl;
            //                    mrpt::system::pause();
          }
      ...
      if (tidx != FEAT_FREE) {
        if (scales2[m] == 0) {
          cout << "Right feature " << m << " found in scale 0!"
               << endl; // computeMoreDescriptors( auxList2[k2], rightImage,
                        // true, itMatch->second );
          int res = computeMoreDescriptors(rightImage, auxList2[tidx],
                                           itMatch->second, true, computeOpts);
          if (res == 0)
          ...
          if (scales2[m] == nScales - 1) {
            cout << "Right feature " << m << " found in scale!"
                 << endl; // computeMoreDescriptors( auxList2[k2], rightImage,
                          // false, itMatch->second );
            int res =
                computeMoreDescriptors(rightImage, auxList2[tidx],
                                       itMatch->second, false, computeOpts);
            if (res == 0)
...
-------------------------------------------------------------*/
int vision::computeMoreDescriptors(const CImage &image,
  ...
  cv::GaussianBlur(inImg1, tempImg1, cvSize(0, 0), opts.sg1 /*sigmaX*/,
                   opts.sg1 /*sigmaY*/);
  aux1 = tempImg1;
  ...
  if (lowerScales) {
    ...
    for (int k = 1; k >= 0; --k) {
      ...
      npSize =
          npSize % 2 ? npSize : npSize + 1; // round to the closest odd number
      hpSize = npSize / 2;                  // half size of the patch

      CImage tPatch;
      // LEFT IMAGE:
      if (npSize)
        smLeftImg.extract_patch(tPatch, inputFeat->x - hpSize,
                                inputFeat->y - hpSize, npSize, npSize);

      cv::Mat out_mat_patch;
      // The size is a+2xa+2 because we have to compute the gradient (magnitude
      // and orientation) in every pixel within the axa patch so we need
      // one more row and column. For instance, for a 23x23 patch we need a
      // 25x25 patch.
      cv::resize(cv::cvarrToMat(tPatch.getAs<IplImage>(), false), out_mat_patch,
                 cv::Size(a + 2, a + 2));
      ...
      if (!vision::computeMainOrientations(rsPatch, a / 2 + 1, a / 2 + 1, a,
      ...
      for (unsigned int m = 0; m < auxOriVector.size(); ++m) {
        ...
        //                auxOriVector[m];
        computeHistogramOfOrientations(rsPatch, a / 2 + 1, a / 2 + 1, a,
                                       auxOriVector[m], auxDescVector[m], opts,
                                       auxHashCoeffs[m]);
        //                cout << " ...done" << endl;
      } // end-for
    ...
    outputFeat->multiScales.push_back(thisScales[0] *
                                      outputFeat->multiScales[nCurrScales - 1]);
    outputFeat->multiScales.push_back(thisScales[1] *
                                      outputFeat->multiScales[nCurrScales - 1]);
    outputFeat->multiScales.push_back(thisScales[2] *
                                      outputFeat->multiScales[nCurrScales - 1]);
    outputFeat->multiScales.push_back(thisScales[3] *
                                      outputFeat->multiScales[nCurrScales - 1]);

    //        for( int k = nCurrScales; k < (int)outputFeat->multiScales.size();
    //        ++k )
    for (int k = 0; k < (int)thisScales.size(); ++k) {
      ...
      vector<double> auxOriVector;
      vision::computeMainOrientations(rsPatch, a / 2 + 1, a / 2 + 1, a,
                                      auxOriVector, opts.sg2);
...
/*-------------------------------------------------------------
                                        setProperScales
-------------------------------------------------------------*/
void vision::setProperScales(const CFeature::Ptr &feat1,
                             const CFeature::Ptr &feat2, int &firstScale,
                             int &lastScale) {
...
void vision::computeMultiResolutionDescriptors(
  ...
  cv::GaussianBlur(inImg2, tempImg2, cvSize(0, 0), opts.sg1 /*sigmaX*/,
                   opts.sg1 /*sigmaY*/);
  aux2 = tempImg2;
  ...
  for (CMatchedFeatureList::iterator itMatch = matchedFeats.begin();
    ...
    if (opts.computeDepth) {
      ...
      itMatch->second->depth =
          sqrt((x3D - opts.baseline) * (x3D - opts.baseline) + y3D * y3D +
               z3D * z3D);
    }
    ...
    memcpy(&itMatch->first->multiScales[0], &opts.scales[0],
           opts.scales.size() * sizeof(double));
    memcpy(&itMatch->second->multiScales[0], &opts.scales[0],
           opts.scales.size() * sizeof(double));
    ...
    for (unsigned int k = 0; k < opts.scales.size(); ++k) {
      ...
      smLeftImg.extract_patch(tPatch, itMatch->first->x - hpSize,
                              itMatch->first->y - hpSize, npSize, npSize);
      ...
      // histograms
      // Orientation vector will be resize inside the function, so don't reserve
      // space for it
      vision::computeMainOrientations(rsPatch, a / 2 + 1, a / 2 + 1, a,
                                      itMatch->first->multiOrientations[k],
                                      opts.sg2);
      ...
      // RIGHT IMAGE:
      tlogger.enter("extract & resize");
      imageRight.extract_patch(tPatch, itMatch->second->x - hpSize,
      ...
      tlogger.enter("main orientations");
      vision::computeMainOrientations(rsPatch2, a / 2 + 1, a / 2 + 1, a,
                                      itMatch->second->multiOrientations[k],
                                      opts.sg2);
      tlogger.leave("main orientations");
...
bool vision::computeMultiResolutionDescriptors(
    const CImage &image, CFeature::Ptr &feat,
    const TMultiResDescOptions &opts) {
#if MRPT_HAS_OPENCV && MRPT_OPENCV_VERSION_NUM >= 0x211
  ...
  if (feat->x + hLargestSize > (w - 1) || feat->x - hLargestSize < 0 ||
    ...
    cout << endl
         << "[computeMultiResolutionDescriptors] WARNING: Feature is too close "
            "to the border. MultiDescriptor computation skipped."
         << endl;
    return false;
  }
  ...
  for (int k = 0; k < maxScale; ++k) {
    ...
    //        image.getHeight()-1 || feat->x-hpSize < 0 || feat->y-hpSize < 0 )
    //            cout << "(" << feat->x << "," << feat->y << ") and hpSize: "
    //            << hpSize << " with scales ";
    //            cout << opts.scales << " imSize: " << image.getWidth() << "x"
    //            << image.getHeight() << endl;
    image.extract_patch(tPatch, feat->x - hpSize, feat->y - hpSize, npSize,
                        npSize);

    cv::Mat out_mat_patch;
    // The size is a+2xa+2 because we have to compute the gradient (magnitude
    // and orientation) in every pixel within the axa patch so we need
    // one more row and column. For instance, for a 23x23 patch we need a 25x25
    // patch.
    cv::resize(cv::cvarrToMat(tPatch.getAs<IplImage>(), false), out_mat_patch,
               cv::Size(a + 2, a + 2));
    IplImage aux_img = IplImage(out_mat_patch);
    ...
    // Compute the main orientations for the axa patch, taking into account that
    // the actual patch has a size of a+2xa+2
    // (being the center point the one with coordinates a/2+1,a/2+1).
    // A sigma = opts.sg2 is used to smooth the entries in the orientations
    // histograms
    // Orientation vector will be resized inside the function, so don't reserve
    // space for it
    vision::computeMainOrientations(rsPatch, a / 2 + 1, a / 2 + 1, a,
    ...
    for (unsigned int m = 0; m < nMainOris; ++m) {
      if (opts.computeHashCoeffs) {
        computeHistogramOfOrientations(
            rsPatch, a / 2 + 1, a / 2 + 1, a, feat->multiOrientations[k][m],
...
vector<bool> vision::computeMultiResolutionDescriptors(
  ...
  if (opts.blurImage) {
    ...
    cv::GaussianBlur(inImg, tempImg, cvSize(0, 0), opts.sg1 /*sigmaX*/,
                     opts.sg1 /*sigmaY*/);
    aux = tempImg;
...
/*-------------------------------------------------------------
                                        computeMultiResolutionDescriptors
-------------------------------------------------------------*/
void vision::computeMultiOrientations(const CImage &image, CFeatureList &list,
                                      const TMultiResDescOptions &opts) {
#if MRPT_HAS_OPENCV && MRPT_OPENCV_VERSION_NUM >= 0x211
  MRPT_START
  CTimeLogger tlogger;
  ...
  for (CFeatureList::iterator it = list.begin(); it != list.end(); ++it) {
    ...
    // Copy the scale values within the feature.
    memcpy(&(*it)->multiScales[0], &opts.scales[0],
           opts.scales.size() * sizeof(double));
    tlogger.leave("cp scales");
    ...
    for (unsigned int k = 0; k < opts.scales.size(); ++k) {
      ...
      // LEFT IMAGE:
      tlogger.enter("extract & resize");
      smLeftImg.extract_patch(tPatch, (*it)->x - hpSize, (*it)->y - hpSize,
                              npSize, npSize);
