	...
	const TMultiResDescMatchOptions& matchOpts)
{
	...
	for (itBase = baseList.begin(); itBase != baseList.end(); ++m)
	{
		...
		//        itBase->->first->nTimesLastSeen
		//        << " frames ago and has been seen " <<
		//        itBase->first->nTimesSeen
		//        << " times so far." << endl;
	...
	const double& sigma)
{
	...
	for (unsigned int k = 0; k < oris.size(); ++k)
	{
		...
		if (oris[k] > pv && oris[k] > nv &&
			oris[k] >
				hist_mag_th)  // It this peak is maximum and is over 0.8 of
		// the maximum peak
		{
			// Check this formulae:
	...
	const TMultiResDescOptions& opts)
{
	...
	else
	{
		...
		//        (int)outputFeat->multiScales.size();
		//        ++k )
