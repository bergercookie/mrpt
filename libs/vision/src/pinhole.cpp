...
void mrpt::vision::pinhole::projectPoints_no_distortion(
  ...
  projectPoints_with_distortion(in_points_3D, cameraPose, intrinsicParams,
                                distortion_dummy, projectedPoints,
...
void mrpt::vision::pinhole::projectPoints_with_distortion(
  ...
  for (size_t i = 0; i < N; i++)
    cameraPose.inverseComposePoint(in_points_3D[i].x, in_points_3D[i].y,
                                   in_points_3D[i].z, objPoints[i].x,
                                   objPoints[i].y, objPoints[i].z);
  ...
  cv::projectPoints(object_points, rotvec, _translation_vector, camera_matrix,
                    dist_coeffs, image_points);

  for (size_t i = 0; i < N; i++) {
    if (accept_points_behind ||
        objPoints[i].z > 0) { // Valid point or we accept them:
      projectedPoints[i].x = image_points[i].x;
      projectedPoints[i].y = image_points[i].y;
    } else { // Invalid point behind the camera:
...
void mrpt::vision::pinhole::undistort_points(
    ...
    const std::vector<double> &Dk) { // Hub function:
  TCamera cam;
  cam.intrinsicParams = A;
  ASSERT_(Dk.size() <= cam.dist.static_size)
  for (size_t i = 0; i < cam.dist.static_size; i++)
    cam.dist[i] = Dk[i];
...
void mrpt::vision::pinhole::projectPoints_with_distortion(
  ...
  for (itPoints = P.begin(), itPixels = pixels.begin(); itPoints != P.end();
    ...
    TPoint3D nP;
    cameraPose.inverseComposePoint(itPoints->x, itPoints->y, itPoints->z, nP.x,
                                   nP.y, nP.z);
...
///* distorted pixels in image */
//	const mrpt::math::CMatrixDouble33
//&intrinsicParams,	/* intrinsic parameters of the camera */
//	const std::vector<double>
//&distortionParams,	/* k1 k2 p1 p2 */
//	const unsigned int								&resX,
///* X-resolution of the image */
//	const unsigned int								&resY,
///* Y-resolution of the image */
//	const double
//&pixelSize,			/* pixel size (square)*/
//	std::vector<mrpt::utils::TPixelCoordf>			&outputPixels
///* estimated undistorted pixels in image */
//    )
//{
//	MRPT_START
//
...
//	std::vector<mrpt::utils::TPixelCoordf>				grid;
//// The 43x43 grid with distorted
...
//
//			const double ncx =
//dx*(1+delta)+2*p1*dx*dy+p2*(r2+2*dx*dx);
//			const double ncy =
//dy*(1+delta)+p1*(r2+2*dy*dy)+2*p2*dx*dy;
//
