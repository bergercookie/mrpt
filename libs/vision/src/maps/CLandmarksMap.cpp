...
void CLandmarksMap::TMapDefinition::loadFromConfigFile_map_specific(
  ...
  insertionOpts.loadFromConfigFile(source,
                                   sectionNamePrefix + string("_insertOpts"));
  likelihoodOpts.loadFromConfigFile(source, sectionNamePrefix +
                                                string("_likelihoodOpts"));
}
...
mrpt::maps::CMetricMap *CLandmarksMap::internal_CreateFromMapDefinition(
  ...
  for (std::deque<CLandmarksMap::TMapDefinition::TPairIdBeacon>::const_iterator
           p = def.initialBeacons.begin();
...
CLandmarksMap::CLandmarksMap()
    : landmarks(), insertionOptions(), likelihoodOptions(), insertionResults(),
      fuseOptions() {
  landmarks.clear();
  //_mEDD.clear();
  //_mapMaxID = 0;
  ...
  if (CLASS_ID(CObservation2DRangeScan) == obs->GetRuntimeClass() &&
    ...
    for (it = o->sensedData.begin(); it != o->sensedData.end(); it++) {
      ...
      for (lm_it = landmarks.begin(); !found && lm_it != landmarks.end();
           lm_it++) {
        if ((lm_it->getType() == featBeacon) && (lm_it->ID == sensedID) &&
...
/*---------------------------------------------------------------
                                                insertObservation
  ---------------------------------------------------------------*/
bool CLandmarksMap::internal_insertObservation(const CObservation *obs,
...
void CLandmarksMap::computeMatchingWith2D(
  ...
  computeMatchingWith3DLandmarks(otherMap2, correspondences,
                                 correspondencesRatio, otherCorrespondences);
...
void CLandmarksMap::loadSiftFeaturesFromImageObservation(
  ...
  vision::CFeatureList
      siftList; // vision::TSIFTFeatureList			siftList;
...
void CLandmarksMap::loadSiftFeaturesFromStereoImageObservation(
  ...
  // Extract SIFT features:
  fExt.options = feat_options; // OLD: fExt.options.SIFTOptions.implementation =
                               // vision::CFeatureExtraction::Hess;

  // Default: Hess implementation
  fExt.detectFeatures(obs.imageLeft, leftSiftList, fID,
                      insertionOptions.SIFTs_numberOfKLTKeypoints);
  fExt.detectFeatures(obs.imageRight, rightSiftList, fID,
                      insertionOptions.SIFTs_numberOfKLTKeypoints);
  ...
  vision::matchFeatures(leftSiftList, rightSiftList, matchesList,
                        matchingOptions);

  if (insertionOptions.PLOT_IMAGES) {
    std::cerr << "Warning: insertionOptions.PLOT_IMAGES has no effect since "
                 "MRPT 0.9.1\n";
  }
...
void CLandmarksMap::computeMatchingWith3DLandmarks(
  ...
  case 0:
    // Our method: Filter out by the likelihood of the 3D position and compute
    // the likelihood of the Euclidean descriptor distance
    ...
    for (k = 0, otherIt = anotherMap->landmarks.begin();
      ...
      if (otherIt->getType() == featSIFT) {
        ...
        */
        ...
        for (j = 0, thisIt = landmarks.begin(); thisIt != landmarks.end();
             thisIt++, j++) {
          if (thisIt->getType() == featSIFT &&
            ...
            lik_dist =
                exp(K_dist *
                    distMahaFlik2); // Likelihood regarding the spatial distance
            ...
            if (lik_dist > 1e-2) {
              ...
              // ONLY COMPUTE THE EUCLIDEAN DISTANCE BETWEEN DESCRIPTORS IF IT
              ...
              if (CLandmarksMap::_mEDD[mPair] == 0) {
                ...
                desc = 0;
                for (i = 0; i < n; i++)
                  desc += square(otherIt->features[0]->descriptors.SIFT[i] -
                                 thisIt->features[0]->descriptors.SIFT[i]);

                CLandmarksMap::_mEDD[mPair] = desc;
                // std::cout << "[fuseWith] - Nueva entrada! - (LIK): " << exp(
                // K_desc * desc ) << " -> " << "(" << mPair.first << "," <<
                // mPair.second << ")" << std::endl;

              } // end if
              else {
                desc = CLandmarksMap::_mEDD[mPair];
                // std::cout << "[fuseWith] - Ya esta calculado!: " << "(" <<
                // mPair.first << "," << mPair.second << ")"  << ": " << desc <<
                // std::endl;
              }

              lik_desc =
                  exp(K_desc * desc); // Likelihood regarding the descriptor
            } else {
              lik_desc = 1e-3;
            }
        ...
        if (maxLik > insertionOptions.SiftLikelihoodThreshold) {
          ...
          // ****************************************************************
          // If a previous correspondence for this LM was found, discard this
          // one!
  ...
  case 1:
    ...
    // IMPLEMENTATION OF THE METHOD DESCRIBED IN [VISION-BASED SLAM USING THE
    // RBPF][SIM, ELINAS, GRIFFIN, LITTLE]
    ...
    unsigned int dLen =
        anotherMap->landmarks.begin()->features[0]->descriptors.SIFT.size();
    for (k = 0, otherIt = anotherMap->landmarks.begin();
         otherIt != anotherMap->landmarks.end(); otherIt++, k++) {
      double mEDD = -1.0;
      ...
      for (j = 0, thisIt = landmarks.begin(); thisIt != landmarks.end();
        ...
        double EDD = 0.0;
        for (i = 0; i < dLen; i++)
          EDD += square(otherIt->features[0]->descriptors.SIFT[i] -
                        thisIt->features[0]->descriptors.SIFT[i]);
      ...
      if (mEDD > insertionOptions.SiftEDDThreshold) {
        ...
        if (!thisLandmarkAssigned[mEDDidx]) // If there is not multiple
                                            // correspondence
        {
...
bool CLandmarksMap::saveToTextFile(std::string file) {
  ...
  for (TSequenceLandmarks::iterator it = landmarks.begin();
    ...
    os::fprintf(
        ...
        it->pose_mean.z, static_cast<int>(it->getType()), it->seenTimesCount,
        it->timestampLastSeen == INVALID_TIMESTAMP
            ? 0
            : mrpt::system::extractDayTimeFromTimestamp(it->timestampLastSeen));

    if (it->getType() == featSIFT) {
      ASSERT_(!it->features.empty() && it->features[0])

      for (unsigned int i = 0; i < it->features[0]->descriptors.SIFT.size();
           i++)
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
bool CLandmarksMap::saveToMATLABScript3D(std::string file, const char *style,
  ...
  for (TSequenceLandmarks::const_iterator it = landmarks.begin();
       it != landmarks.end(); ++it) {
    os::fprintf(f, "m=[%.4f %.4f %.4f];", it->pose_mean.x, it->pose_mean.y,
                it->pose_mean.z);
    ...
    os::fprintf(
        f, "error_ellipse(c,m,'conf',%f,'style','%s','numPointsEllipse',10);\n",
        confInterval, style);
  }
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
bool CLandmarksMap::saveToMATLABScript2D(std::string file, const char *style,
                                         float stdCount) {
  FILE *f = os::fopen(file.c_str(), "wt");
  if (!f)
    return false;
...
double CLandmarksMap::computeLikelihood_RSLC_2007(const CLandmarksMap *s,
  ...
  for (itOther = s->landmarks.begin(); itOther != s->landmarks.end();
    ...
    cx_2 = min(static_cast<int>(grid->getSizeX()) - 1,
               grid->x2idx(itOther->pose_mean.x + 0.10f));
    cy_1 = max(0, grid->y2idx(itOther->pose_mean.y - 0.10f));
    cy_2 = min(static_cast<int>(grid->getSizeY()) - 1,
               grid->y2idx(itOther->pose_mean.y + 0.10f));

    // The likelihood of no correspondence starts at "1" and will be modified
    // next:
    PrNoCorr = 1;
    ...
    for (cx = cx_1; cx <= cx_2; cx++)
      for (cy = cy_1; cy <= cy_2; cy++) {
        ...
        if (!corrs->empty())
          for (vector_int::iterator it = corrs->begin(); it != corrs->end();
               ++it) {
            lm = landmarks.get(*it);
            ...
            // The likelihood of no corresp. is proportional to the product of
            // all "1-CORRij":
            // -------------------------------------------------------------
            PrNoCorr *= 1 - corr;
...
const CLandmark *
CLandmarksMap::TCustomSequenceLandmarks::get(unsigned int indx) const {
  return &m_landmarks[indx];
}

void CLandmarksMap::TCustomSequenceLandmarks::isToBeModified(
    unsigned int indx) {
  vector_int *cell = m_grid.cellByPos(m_landmarks[indx].pose_mean.x,
                                      m_landmarks[indx].pose_mean.y);
...
void CLandmarksMap::TCustomSequenceLandmarks::hasBeenModified(
    unsigned int indx) {
  vector_int dummyEmpty;
...
// *theMap )
{
  ...
  case 0: // Our method
         ...
         lm1 += decimation, idx1 += decimation) // Other theMap LM1
    {
      if (lm1->getType() == featSIFT) {
        ...
        for (idx2 = 0, lm2 = landmarks.begin(); lm2 != landmarks.end();
             lm2++, idx2++) // This theMap LM2
        {
          if (lm2->getType() == featSIFT) {
            ...
            // std::cout << "ED POSICION: " << sqrt( dij(0,0)*dij(0,0) +
            // dij(0,1)*dij(0,1) + dij(0,2)*dij(0,2) ) << std::endl;
            // Equivalent covariance from "i" to "j":
            ...
            if (likByDist > 1e-2) {
              // If the EUCLIDEAN distance is not too large, we compute the
              // Descriptor distance
              ...
              if (CLandmarksMap::_mEDD[mPair] == 0) {
                ...
                ASSERT_(lm1->features[0]->descriptors.SIFT.size() ==
                        lm2->features[0]->descriptors.SIFT.size())

                for (it1 = lm1->features[0]->descriptors.SIFT.begin(),
                    it2 = lm2->features[0]->descriptors.SIFT.begin();
                     it1 != lm1->features[0]->descriptors.SIFT.end();
                     it1++, it2++)
                  distDesc += square(*it1 - *it2);

                // Insert into the vector of Euclidean distances
                CLandmarksMap::_mEDD[mPair] = distDesc;
                // std::cout << "Nueva entrada! - (LIK): " << exp( K_desc *
                // distDesc ) << " -> " << "(" << mPair.first << "," <<
                // mPair.second << ")" << std::endl;

              } // end if
              else {
                distDesc = (unsigned long)CLandmarksMap::_mEDD[mPair];
                // std::cout << "Ya esta calculado!: " << "(" << mPair.first <<
                // "," << mPair.second << ")"  << ": " << distDesc << std::endl;
              }
              ...
              // idx2, likByDist, likByDesc, distDesc );
              ...
              //																	idx2,
              //lm2->pose_mean.x, lm2->pose_mean.y, lm2->pose_mean.z );
              //	mrpt::system::pause();
              //}

              lik_i += likByDist * likByDesc; // Cumulative Likelihood
            } else {
              // If the EUCLIDEAN distance is too large, we assume that the
              // cumulative likelihood is (almost) zero.
              lik_i += 1e-10f;
            }
          } // end if

        } // end for "lm2"
        // lik += (0.1 + 0.9*lik_i);		// (CONSENSUS) Total likelihood
        // (assuming independent probabilities)
        // lik += lik_i;					// (CONSENSUS) Total likelihood
        // (assuming independent probabilities)
        lik *= (0.1 + 0.9 * lik_i); // (TRADITIONAL) Total likelihood (assuming
                                    // independent probabilities)
        // lik *= lik_i;				// (TRADITIONAL) Total likelihood
        // (assuming independent probabilities)
      }
    } // end for "lm1"
  ...
  case 1: // SIM, ELINAS, GRIFFIN, LITTLE
    ...
    if (correspondences == nullptr)
      THROW_EXCEPTION("When using this method with SIFTLikelihoodMethod = 1, "
                      "TMatchingPairList *correspondence can not be NULL");

    if (otherCorrespondences == nullptr)
      THROW_EXCEPTION("When using this method with SIFTLikelihoodMethod = 1, "
                      "std::vector<bool> *otherCorrespondences can not be "
                      "NULL");

    for (itCorr = correspondences->begin(); itCorr != correspondences->end();
         itCorr++) {
      ...
      distMahaFlik2 =
          dij.multiply_HCHt_scalar(Cij_1); // ( dij * Cij_1 * (~dij) )(0,0);

      dist = min((double)likelihoodOptions.SIFTnullCorrespondenceDistance,
                 distMahaFlik2);
...
CLandmarksMap::TInsertionOptions::TInsertionOptions()
    : insert_SIFTs_from_monocular_images(true),
      ...
      insert_Landmarks_from_range_scans(true), SiftCorrRatioThreshold(0.4f),
      SiftLikelihoodThreshold(0.5f),

      SiftEDDThreshold(200.0f), SIFTMatching3DMethod(0),
      SIFTLikelihoodMethod(0),

      SIFTsLoadDistanceOfTheMean(3.0f), SIFTsLoadEllipsoidWidth(0.05f),

      SIFTs_stdXY(2.0f), SIFTs_stdDisparity(1.0f),

      SIFTs_numberOfKLTKeypoints(60), SIFTs_stereo_maxDepth(15.0f),

      SIFTs_epipolar_TH(1.5f), PLOT_IMAGES(false),
...
void CLandmarksMap::TInsertionOptions::loadFromConfigFile(
  ...
  insert_SIFTs_from_monocular_images =
      iniFile.read_bool(section.c_str(), "insert_SIFTs_from_monocular_images",
                        insert_SIFTs_from_monocular_images);
  insert_SIFTs_from_stereo_images =
      iniFile.read_bool(section.c_str(), "insert_SIFTs_from_stereo_images",
                        insert_SIFTs_from_stereo_images);
  insert_Landmarks_from_range_scans =
      iniFile.read_bool(section.c_str(), "insert_Landmarks_from_range_scans",
                        insert_Landmarks_from_range_scans);
  ...
  SiftEDDThreshold =
      iniFile.read_float(section.c_str(), "SiftEDDThreshold", SiftEDDThreshold);
  ...
  SIFTsLoadDistanceOfTheMean =
      iniFile.read_float(section.c_str(), "SIFTsLoadDistanceOfTheMean",
                         SIFTsLoadDistanceOfTheMean);
  ...
  SIFTs_stdDisparity = iniFile.read_float(section.c_str(), "SIFTs_stdDisparity",
                                          SIFTs_stdDisparity);
  SIFTs_numberOfKLTKeypoints =
      iniFile.read_int(section.c_str(), "SIFTs_numberOfKLTKeypoints",
                       SIFTs_numberOfKLTKeypoints);
  SIFTs_stereo_maxDepth = iniFile.read_float(
      section.c_str(), "SIFTs_stereo_maxDepth", SIFTs_stereo_maxDepth);
  SIFTs_epipolar_TH = iniFile.read_float(section.c_str(), "SIFTs_epipolar_TH",
                                         SIFTs_epipolar_TH);
...
CLandmarksMap::TLikelihoodOptions::TLikelihoodOptions()
    : rangeScan2D_decimation(20), SIFTs_sigma_euclidean_dist(0.30f),
      SIFTs_sigma_descriptor_dist(100.0f), SIFTs_mahaDist_std(4.0f),
      SIFTnullCorrespondenceDistance(4.0f), SIFTs_decimation(1),
      SIFT_feat_options(vision::featSIFT), beaconRangesStd(0.08f),
      beaconRangesUseObservationStd(false), extRobotPoseStd(0.05f), GPSOrigin(),
      GPS_sigma(1.0f) {}

CLandmarksMap::TLikelihoodOptions::TGPSOrigin::TGPSOrigin()
    : longitude(-4.47763833333333), latitude(36.71559000000000), altitude(42.3),
      ang(0), x_shift(0), y_shift(0), min_sat(4) {}
...
void CLandmarksMap::TLikelihoodOptions::loadFromConfigFile(
  ...
  SIFTs_sigma_euclidean_dist =
      iniFile.read_double(section.c_str(), "SIFTs_sigma_euclidean_dist",
                          SIFTs_sigma_euclidean_dist);
  SIFTs_sigma_descriptor_dist =
      iniFile.read_double(section.c_str(), "SIFTs_sigma_descriptor_dist",
                          SIFTs_sigma_descriptor_dist);
  SIFTs_mahaDist_std = iniFile.read_float(section.c_str(), "SIFTs_mahaDist_std",
                                          SIFTs_mahaDist_std);
  SIFTs_decimation =
      iniFile.read_int(section.c_str(), "SIFTs_decimation", SIFTs_decimation);
  SIFTnullCorrespondenceDistance =
      iniFile.read_float(section.c_str(), "SIFTnullCorrespondenceDistance",
                         SIFTnullCorrespondenceDistance);

  GPSOrigin.latitude = iniFile.read_double(section.c_str(), "GPSOriginLatitude",
                                           GPSOrigin.latitude);
  GPSOrigin.longitude = iniFile.read_double(
      section.c_str(), "GPSOriginLongitude", GPSOrigin.longitude);
  GPSOrigin.altitude = iniFile.read_double(section.c_str(), "GPSOriginAltitude",
                                           GPSOrigin.altitude);
  GPSOrigin.ang =
      iniFile.read_double(section.c_str(), "GPSOriginAngle", GPSOrigin.ang) *
      M_PI / 180;
  GPSOrigin.x_shift = iniFile.read_double(section.c_str(), "GPSOriginXshift",
                                          GPSOrigin.x_shift);
  GPSOrigin.y_shift = iniFile.read_double(section.c_str(), "GPSOriginYshift",
                                          GPSOrigin.y_shift);
  GPSOrigin.min_sat =
  ...
  beaconRangesUseObservationStd =
      iniFile.read_bool(section.c_str(), "beaconRangesUseObservationStd",
                        beaconRangesUseObservationStd);
...
const CLandmark *
CLandmarksMap::TCustomSequenceLandmarks::getByBeaconID(unsigned int ID) const {
  for (size_t indx = 0; indx < m_landmarks.size(); indx++) {
    if (m_landmarks[indx].ID == ID)
      return &m_landmarks[indx];
  }
 ...
 * \param  otherMap					  [IN] The other map to
 compute the matching with.
 * \param  otherMapPose				  [IN] The 6D pose of the
 other map as seen from "this".
 * \param  maxDistForCorr			  [IN] The minimum distance
 between 2 non-probabilistic map elements for counting them as a correspondence.
 * \param  maxMahaDistForCorr		  [IN] The minimum Mahalanobis
 distance between 2 probabilistic map elements for counting them as a
 correspondence.
 *
...
float CLandmarksMap::compute3DMatchingRatio(
  ...
  // ---------------------------------------------------------------------------------------------------------------
  // Is there any "contact" between the spheres that contain all the points from
  // each map after translating them??
  ...
  for (itOther = otherMap->landmarks.begin(),
    ...
    for (itThis = landmarks.begin(), itPoseThis = poses3DThis.begin();
      ...
      TPoint3D D((*itPoseThis)->mean.x() - (*itPoseOther)->mean.x(),
      ...
      float distMaha =
          sqrt(d.multiply_HCHt_scalar(COV.inv())); //(d*COV.inv()*(~d))(0,0) );
        ...
        if (!itThis->features.empty() && !itOther->features.empty() &&
          ...
          for (it1 = itThis->features[0]->descriptors.SIFT.begin(),
              it2 = itOther->features[0]->descriptors.SIFT.begin();
               it1 != itThis->features[0]->descriptors.SIFT.end(); it1++, it2++)
            descrDist += square(*it1 - *it2);

          float descrDist_f = sqrt(static_cast<float>(descrDist)) /
                              itThis->features[0]->descriptors.SIFT.size();
...
void CLandmarksMap::simulateRangeBearingReadings(
  ...
  if (spurious_count_std != 0 || spurious_count_mean != 0)
    nSpurious =
        static_cast<size_t>(mrpt::utils::round_long(std::max(0.0, fSpurious)));

  // For each spurios reading to generate:
  // ------------------------------------------
  for (size_t i = 0; i < nSpurious; i++) {
    // Make up yaw,pitch,range out from thin air:
    // (the conditionals on yaw & pitch are to generate 2D observations if we
    // are in 2D, which we learn from a null std.dev.)
