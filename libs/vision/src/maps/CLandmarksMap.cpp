	...
	const mrpt::vision::CFeatureExtraction::TOptions& feat_options)
{
	...
	// Extract SIFT features:
	fExt.options =
		feat_options;  // OLD: fExt.options.SIFTOptions.implementation
	...
	{
		std::cerr
			<< "Warning: insertionOptions.PLOT_IMAGES has no effect since "
			   "MRPT 0.9.1\n";
	}
				...
				if (lm1->getType() == featSIFT)
				{
						 ...
						 lm2++, idx2++)  // This theMap LM2
					{
						if (lm2->getType() == featSIFT)
						{
							...
							// dij(0,0)*dij(0,0) +
							// dij(0,1)*dij(0,1) + dij(0,2)*dij(0,2) ) <<
							// std::endl;
							// Equivalent covariance from "i" to "j":
							...
							if (likByDist > 1e-2)
							{
								...
								//																	idx2,
								// lm2->pose_mean.x,
								// lm2->pose_mean.y, lm2->pose_mean.z );
								//	mrpt::system::pause();
								//}
					...
					// (assuming independent probabilities)
					lik *=
						(0.1 + 0.9 * lik_i);  // (TRADITIONAL) Total likelihood
					// (assuming independent probabilities)
