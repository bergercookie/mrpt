...
mrpt::vision::pnp::rpnp::rpnp(Eigen::MatrixXd obj_pts_,
                              Eigen::MatrixXd img_pts_, Eigen::MatrixXd cam_,
                              int n0) {
  obj_pts = obj_pts_;
  img_pts = img_pts_;
  cam_intrinsic = cam_;
...
bool mrpt::vision::pnp::rpnp::compute_pose(Eigen::Ref<Eigen::Matrix3d> R_,
  ...
  for (int i = 0; i < cnt; i++) {
    ...
    for (int j = 0; j < n; j++) {
      double ui = img_pts(j, 0), vi = img_pts(j, 1), xi = P(0, j), yi = P(1, j),
             zi = P(2, j);
      D.row(2 * j) << -r(1) * yi + ui * (r(7) * yi + r(8) * zi) - r(2) * zi,
          -r(2) * yi + ui * (r(8) * yi - r(7) * zi) + r(1) * zi, -1, 0, ui,
          ui * r(6) * xi - r(0) * xi;

      D.row(2 * j + 1) << -r(4) * yi + vi * (r(7) * yi + r(8) * zi) - r(5) * zi,
          -r(5) * yi + vi * (r(8) * yi - r(7) * zi) + r(4) * zi, 0, -1, vi,
          vi * r(6) * xi - r(3) * xi;
    }
...
void mrpt::vision::pnp::rpnp::calcampose(Eigen::MatrixXd &XXc,
  ...
  Eigen::JacobiSVD<Eigen::MatrixXd> svd(SXY, Eigen::ComputeThinU |
                                                 Eigen::ComputeThinV);
...
Eigen::VectorXd mrpt::vision::pnp::rpnp::getp3p(double l1, double l2, double A5,
                                                double C1, double C2, double D1,
                                                double D2, double D3) {
