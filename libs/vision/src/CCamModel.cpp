...
/**********************************************************************************************************************/
void CCamModel::jacob_undistor_fm(const mrpt::utils::TPixelCoordf &p,
...
void CCamModel::jacob_undistor(const mrpt::utils::TPixelCoordf &p,
                               mrpt::math::CMatrixDouble &J_undistor) {
...
void CCamModel::distort_a_point(const mrpt::utils::TPixelCoordf &p,
  ...
  const double dx = (p.x - cam.cx()); // / cam.fx;   //JL: commented out
                                      // cam.fxy... (dx,dy) units must be pixels
...
void CCamModel::undistort_point(const mrpt::utils::TPixelCoordf &p,
  ...
  mrpt::vision::pinhole::undistort_points(in_p, out_p, cam.intrinsicParams,
...
void CCamModel::jacobian_project_with_distortion(
  ...
  */
  const double ux =
      (p3D.x / p3D.z) * cam.fx(); // coordinates with (0,0) at the image center
...
void CCamModel::loadFromConfigFile(const mrpt::utils::CConfigFileBase &source,
  ...
  // Read camera parameters: They are mandatory, we'll raise an exception if not
  // found:
