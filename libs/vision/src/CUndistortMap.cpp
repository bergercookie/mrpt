...
void CUndistortMap::setFromCamParams(const mrpt::utils::TCamera &campar) {
  ...
  cv::initUndistortRectifyMap(inMat, distM, cv::Mat(), inMat, _mapx.size(),
                              _mapx.type(), _mapx, _mapy);
#else
 ...
 * setFromCamParams() must have been set prior to calling this.
  */
void CUndistortMap::undistort(const mrpt::utils::CImage &in_img,
...
#if MRPT_HAS_OPENCV && MRPT_OPENCV_VERSION_NUM >= 0x200
  CvMat mapx = cvMat(m_camera_params.nrows, m_camera_params.ncols, CV_16SC2,
                     const_cast<int16_t *>(
                         &m_dat_mapx[0])); // Wrappers on the data as a CvMat's.
  CvMat mapy = cvMat(m_camera_params.nrows, m_camera_params.ncols, CV_16UC1,
  ...
  cvRemap(srcImg, outImg, &mapx, &mapy); // cv::remap(src, dst_part, map1_part,
                                         // map2_part, INTER_LINEAR,
                                         // BORDER_CONSTANT );
