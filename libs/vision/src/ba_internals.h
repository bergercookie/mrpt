...
// way.
template <int FrameDof, int PointDof, int ObsDim> struct JacData {
  inline JacData()
      : J_frame(mrpt::math::UNINITIALIZED_MATRIX),
        J_point(mrpt::math::UNINITIALIZED_MATRIX), J_frame_valid(false),
        J_point_valid(false) {}
  ...
  MRPT_MAKE_ALIGNED_OPERATOR_NEW // Needed by any struct having Eigen::Matrix<>
                                 // fields
};
...
void frameJac(const mrpt::utils::TCamera &camera_params,
  ...
  if (POSES_ARE_INVERSE)
    cam_pose.composePoint(landmark_global.x, landmark_global.y,
                          landmark_global.z, x, y, z);
  else
    cam_pose.inverseComposePoint(landmark_global.x, landmark_global.y,
                                 landmark_global.z, x, y, z);
  ...
  if (POSES_ARE_INVERSE) {
    ...
    const double jac_proj_vals[] = {fx_z, 0, -fx_z2 * x, 0, fy_z, -fy_z2 * y};
...
*/
template <bool POSES_ARE_INVERSE>
void pointJac(const mrpt::utils::TCamera &camera_params,
              const mrpt::poses::CPose3D &cam_pose,
              const mrpt::math::TPoint3D &landmark_global,
  ...
  if (POSES_ARE_INVERSE)
    cam_pose.composePoint(landmark_global.x, landmark_global.y,
                          landmark_global.z, l.x, l.y, l.z, &dp_point);
  else
    cam_pose.inverseComposePoint(landmark_global.x, landmark_global.y,
                                 landmark_global.z, l.x, l.y, l.z, &dp_point);
...
void ba_compute_Jacobians(
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    if (i_f >= num_fix_frames) {
      frameJac<POSES_ARE_INVERSE>(camera_params, frame_poses[i_f],
                                  landmark_points[i_p], D.J_frame);
      D.J_frame_valid = true;
    }

    if (i_p >= num_fix_points) {
      pointJac<POSES_ARE_INVERSE>(camera_params, frame_poses[i_f],
                                  landmark_points[i_p], D.J_point);
