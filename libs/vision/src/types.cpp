...
void TSequenceFeatureObservations::loadFromTextFile(
  ...
  while (!f.fail()) {
    ...
    if (!(s >> camID >> featID >> px.x >> px.y))
      THROW_EXCEPTION(format("%s:%u: Error parsing line: '%s'", filName.c_str(),
                             linNum, lin.c_str()))
...
bool TSequenceFeatureObservations::saveAsSBAFiles(
  ...
  for (std::map<TLandmarkID,
    ...
    f << pts[it->first].x << " " << pts[it->first].y << " " << pts[it->first].z
      << " " << m.size() << " ";
    for (std::map<TCameraPoseID, TPixelCoordf>::const_iterator itO = m.begin();
         itO != m.end(); ++itO)
...
void TSequenceFeatureObservations::compressIDs(
  ...
  for (BASE::const_iterator it = BASE::begin(); it != BASE::end(); ++it) {
    ...
    if (lmIDs.find(f_ID) == lmIDs.end()) {
      TLandmarkID nextID = lmIDs.size(); // *IMPORTANT* Separate in 2 lines,
                                         // otherwise lmIDs[] is called first
                                         // (!?)
      lmIDs[f_ID] = nextID;
    }
    if (camIDs.find(c_ID) == camIDs.end()) {
      TCameraPoseID nextID = camIDs.size(); // *IMPORTANT* Separate in 2 lines,
                                            // otherwise camIDs[] is called
                                            // first (!?)
      camIDs[c_ID] = nextID;
    }
  }
  ...
  if (old2new_camIDs)
    old2new_camIDs->swap(camIDs);
  if (old2new_lmIDs)
    old2new_lmIDs->swap(lmIDs);
}
