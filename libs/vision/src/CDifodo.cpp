          ...
          if ((v > 0) && (v < rows_i - 1) && (u > 0) && (u < cols_i - 1)) {
            if (dcenter > 0.f) {
              ...
              for (int l = -2; l < 3; l++)
                for (int k = -2; k < 3; k++) {
                  const float abs_dif =
                      abs(depth[i_1](v2 + k, u2 + l) - dcenter);
                  if (abs_dif < max_depth_dif) {
                    const float aux_w =
                        g_mask[2 + k][2 + l] * (max_depth_dif - abs_dif);
                    weight += aux_w;
  ...
  for (unsigned int j = 0; j < cols_i; j++)
    for (unsigned int i = 0; i < rows_i; i++) {
      ...
      if (z > 0.f) {
        ...
        const float depth_w =
            acu_trans(0, 0) * z + acu_trans(0, 1) * xx[image_level](i, j) +
            acu_trans(0, 2) * yy[image_level](i, j) + acu_trans(0, 3);
        const float x_w =
            acu_trans(1, 0) * z + acu_trans(1, 1) * xx[image_level](i, j) +
            acu_trans(1, 2) * yy[image_level](i, j) + acu_trans(1, 3);
        const float y_w =
            acu_trans(2, 0) * z + acu_trans(2, 1) * xx[image_level](i, j) +
        ...
        // The warped pixel (which is not integer in general) contributes to all
    ...
    for (unsigned int v = 0; v < rows_i; v++) {
      if ((depth_old[image_level](v, u)) == 0.f ||
        ...
        depth_inter[image_level](v, u) =
            0.5f *
            (depth_old[image_level](v, u) + depth_warped[image_level](v, u));
        xx_inter[image_level](v, u) =
            0.5f * (xx_old[image_level](v, u) + xx_warped[image_level](v, u));
        yy_inter[image_level](v, u) =
            0.5f * (yy_old[image_level](v, u) + yy_warped[image_level](v, u));
        null(v, u) = false;
...
void CDifodo::calculateDepthDerivatives() {
  ...
  for (unsigned int u = 0; u < cols_i - 1; u++)
    for (unsigned int v = 0; v < rows_i; v++)
      if (null(v, u) == false) {
        rx_ninv(v, u) = sqrtf(square(xx_inter[image_level](v, u + 1) -
                                     xx_inter[image_level](v, u)) +
                              square(depth_inter[image_level](v, u + 1) -
                                     depth_inter[image_level](v, u)));
      }
  ...
  for (unsigned int u = 0; u < cols_i; u++)
    for (unsigned int v = 0; v < rows_i - 1; v++)
      if (null(v, u) == false) {
        ry_ninv(v, u) = sqrtf(square(yy_inter[image_level](v + 1, u) -
                                     yy_inter[image_level](v, u)) +
                              square(depth_inter[image_level](v + 1, u) -
                                     depth_inter[image_level](v, u)));
      }
  ...
  for (unsigned int v = 0; v < rows_i; v++) {
    for (unsigned int u = 1; u < cols_i - 1; u++)
      if (null(v, u) == false)
        du(v, u) = (rx_ninv(v, u - 1) * (depth_inter[image_level](v, u + 1) -
                                         depth_inter[image_level](v, u)) +
  ...
  for (unsigned int u = 0; u < cols_i; u++) {
    for (unsigned int v = 1; v < rows_i - 1; v++)
      if (null(v, u) == false)
        dv(v, u) = (ry_ninv(v - 1, u) * (depth_inter[image_level](v + 1, u) -
                                         depth_inter[image_level](v, u)) +
...
void CDifodo::computeWeights() {
  ...
  // Obtain the velocity associated to the rigid transformation estimated up to
  // the present level
  ...
  for (unsigned int u = 1; u < cols_i - 1; u++)
    for (unsigned int v = 1; v < rows_i - 1; v++)
      if (null(v, u) == false) {
        ...
        // const float j1 = -2.f*inv_d*inv_d*(xx_inter[image_level](v,u)*dycomp
        ...
        //yy_inter[image_level](v,u)*kai_level[3]) + inv_d*dzcomp*(kai_level[2]
        //+ xx_inter[image_level](v,u)*kai_level[3]);
        // const float j2 = inv_d*dycomp*(kai_level[0] +
        // yy_inter[image_level](v,u)*kai_level[4] -
        // 2.f*xx_inter[image_level](v,u)*kai_level[5]) - dzcomp*kai_level[3];
        // const float j3 = inv_d*dzcomp*(kai_level[0] +
        // 2.f*yy_inter[image_level](v,u)*kai_level[4] -
        // xx_inter[image_level](v,u)*kai_level[5]) + dycomp*kai_level[3];

        const float j4 = 1.f;
        const float j5 =
            xx_inter[image_level](v, u) * inv_d * inv_d * f_inv *
                (kai_level[0] + yy_inter[image_level](v, u) * kai_level[4] -
                 xx_inter[image_level](v, u) * kai_level[5]) +
            inv_d * f_inv * (-kai_level[1] - z * kai_level[5] +
                             yy_inter[image_level](v, u) * kai_level[3]);
        const float j6 =
            yy_inter[image_level](v, u) * inv_d * inv_d * f_inv *
                (kai_level[0] + yy_inter[image_level](v, u) * kai_level[4] -
                 xx_inter[image_level](v, u) * kai_level[5]) +
            inv_d * f_inv * (-kai_level[2] + z * kai_level[4] -
                             xx_inter[image_level](v, u) * kai_level[3]);
        ...
        //-----------------------------------------------------------------------
        const float ini_du =
            depth_old[image_level](v, u + 1) - depth_old[image_level](v, u - 1);
        const float ini_dv =
            depth_old[image_level](v + 1, u) - depth_old[image_level](v - 1, u);
        ...
        const float dvu =
            dv(v, u + 1) - dv(v, u - 1); // Completely equivalent to compute duv
  ...
  for (unsigned int u = 1; u < cols_i - 1; u++)
    for (unsigned int v = 1; v < rows_i - 1; v++)
      if (null(v, u) == false) {
        ...
        A(cont, 4) =
            tw * (y + dycomp * inv_d * y * x + dzcomp * (y * y * inv_d + d));
        A(cont, 5) =
            tw * (-x - dycomp * (x * x * inv_d + d) - dzcomp * inv_d * y * x);
        B(cont, 0) = tw * (-dt(v, u));
...
void CDifodo::filterLevelSolution() {
  ...
  // Important: we have to substract the previous levels' solutions from the old
  // velocity.
  Matrix4f acu_trans;
  acu_trans.setIdentity();
  for (unsigned int i = 0; i < level; i++)
...
void CDifodo::getDepthDerivatives(MatrixXf &cur_du, MatrixXf &cur_dv,
                                  MatrixXf &cur_dt) {
