...
struct lm_stat_t {
  ...
  mrpt::aligned_containers<mrpt::poses::CPose3D>::vector_t
      left_cam_poses; // Poses of the origin of coordinates of the pattern wrt
                      // the left camera
  ...
  // Ctor
  lm_stat_t(const TCalibrationStereoImageList &_images,
            const std::vector<size_t> &_valid_image_pair_indices,
            const std::vector<mrpt::math::TPoint3D> &_obj_points)
      : images(_images), valid_image_pair_indices(_valid_image_pair_indices),
        obj_points(_obj_points) {
    left_cam_poses.assign(images.size(),
                          mrpt::poses::CPose3D(0, 0, 1, 0, 0, 0)); // Initial
  }
...
struct TResidJacobElement {
  Eigen::Matrix<double, 4, 1>
      predicted_obs; //!< [u_l v_l  u_r v_r]: left/right camera pixels
  Eigen::Matrix<double, 4, 1> residual; //!<  = predicted_obs - observations
  Eigen::Matrix<double, 4, 30> J; //!< Jacobian. 4=the two predicted pixels;
                                  //!30=Read below for the meaning of these 30
                                  //!variables
};
...
double recompute_errors_and_Jacobians(const lm_stat_t &lm_stat,
                                      TResidualJacobianList &res_jac,
                                      bool use_robust_kernel,
                                      double kernel_param);
void build_linear_system(const TResidualJacobianList &res_jac,
                         const vector_size_t &var_indxs,
                         Eigen::VectorXd &minus_g, Eigen::MatrixXd &H);
void add_lm_increment(const Eigen::VectorXd &eps,
                      const vector_size_t &var_indxs, lm_stat_t &new_lm_stat);
}
}
