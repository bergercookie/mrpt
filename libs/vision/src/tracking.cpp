...
inline void trackFeatures_checkResponses<CFeatureList>(
  ...
  for (CFeatureList::iterator itFeat = featureList.begin(); itFeat != itFeatEnd;
       ++itFeat) {
    ...
    if (x > KLT_response_half_win && y > KLT_response_half_win && x < max_x &&
        y < max_y) { // Update response:
      ft->response = cur_gray.KLT_response(x, y, KLT_response_half_win);

      // Is it good enough?
      // http://grooveshark.com/s/Goonies+Are+Good+Enough/2beBfO?src=5
      if (ft->response < minimum_KLT_response) { // Nope!
...
inline void trackFeatures_checkResponses<TSimpleFeatureList>(
  ...
  trackFeatures_checkResponses_impl_simple<TSimpleFeatureList>(
      featureList, cur_gray, minimum_KLT_response, KLT_response_half_win, max_x,
      max_y);
}
template <>
...
template <typename FEATLIST>
inline void trackFeatures_updatePatch(FEATLIST &featureList,
                                      const CImage &cur_gray);

template <>
inline void trackFeatures_updatePatch<CFeatureList>(CFeatureList &featureList,
                                                    const CImage &cur_gray) {
  for (CFeatureList::iterator itFeat = featureList.begin();
    ...
    if (patch_width > 0 && patch_height > 0) {
      try {
        ...
        cur_gray.extract_patch(ft->patch, round(ft->x) - offset,
                               round(ft->y) - offset, patch_width,
                               patch_height);
      } catch (std::exception &) {
...
template <>
inline void
trackFeatures_updatePatch<TSimpleFeatureList>(TSimpleFeatureList &featureList,
                                              const CImage &cur_gray) {
...
template <>
inline void
trackFeatures_updatePatch<TSimpleFeaturefList>(TSimpleFeaturefList &featureList,
                                               const CImage &cur_gray) {
  MRPT_UNUSED_PARAM(featureList);
  MRPT_UNUSED_PARAM(cur_gray);
  // This list type does not have patch stored explicitly
...
inline void trackFeatures_addNewFeats<CFeatureList>(
  ...
  for (size_t i = 0; i < nNewToCheck && featureList.size() < maxNumFeatures;
    ...
    if (feat.response < minimum_KLT_response_to_add)
      continue;
    ...
    if (min_dist_sqr > threshold_sqr_dist_to_add_new && feat.pt.x > offset &&
        feat.pt.y > offset && feat.pt.x < w_off && feat.pt.y < h_off) {
      // Add new feature:
...
inline void trackFeatures_addNewFeats_simple_list(
  ...
  const int grid_cell_log2 = round(
      std::log(std::sqrt(threshold_sqr_dist_to_add_new) * 0.5) / std::log(2.0));
  ...
  for (size_t i = 0; i < nNewToCheck && featureList.size() < maxNumFeatures;
    ...
    newFeat.response = feat.response;
    newFeat.octave = 0;
    newFeat.track_status = status_IDLE; //!< Inactive: right after detection,
                                        //!and before being tried to track
  }
#else
...
template <typename FEATLIST>
inline size_t
trackFeatures_deleteOOB(FEATLIST &trackedFeats, const size_t img_width,
...
template <>
inline size_t
trackFeatures_deleteOOB(TSimpleFeatureList &trackedFeats,
...
template <>
inline size_t
trackFeatures_deleteOOB(TSimpleFeaturefList &trackedFeats,
                        const size_t img_width, const size_t img_height,
                        const int MIN_DIST_MARGIN_TO_STOP_TRACKING) {
...
template <>
inline size_t
trackFeatures_deleteOOB(CFeatureList &trackedFeats, const size_t img_width,
                        const size_t img_height,
                        const int MIN_DIST_MARGIN_TO_STOP_TRACKING) {
...
/** Perform feature tracking from "old_img" to "new_img", with a (possibly
  ...
  *be also added to the existing ones in areas of the image poor of features.
  ...
  */
template <typename FEATLIST>
void CGenericFeatureTracker::internal_trackFeatures(const CImage &old_img,
  ...
  if (check_KLT_response_every > 0 &&
    ...
    detail::trackFeatures_checkResponses(featureList, cur_gray,
                                         minimum_KLT_response,
                                         KLT_response_half_win, max_x, max_y);
  ...
  if (remove_lost_features) {
    ...
    const size_t nRemoved = detail::trackFeatures_deleteOOB(
        featureList, img_width, img_height, MIN_DIST_MARGIN_TO_STOP_TRACKING);
  ...
  if (add_new_features) {
    ...
    // Look for new features and save in "m_newly_detected_feats", if they're
    ...
    const size_t desired_num_features = extra_params.getWithDefaultVal(
        "desired_num_features_adapt", size_t((img_width * img_height) >> 9));
    ...
    //      features "m_newly_detected_feats"
...
void CGenericFeatureTracker::trackFeatures(const CImage &old_img,
                                           const CImage &new_img,
...
void CGenericFeatureTracker::updateAdaptiveNewFeatsThreshold(
  ...
  if (nNewlyDetectedFeats < hysteresis_min_num_feats)
    m_detector_adaptive_thres =
        std::max(2.0, std::min(m_detector_adaptive_thres - 1.0,
                               m_detector_adaptive_thres * 0.8));
  else if (nNewlyDetectedFeats > hysteresis_max_num_feats)
    m_detector_adaptive_thres = std::max(m_detector_adaptive_thres + 1.0,
                                         m_detector_adaptive_thres * 1.2);
}
  ...
  for (itLeft = leftList.begin(), itRight = rightList.begin();
    ...
    if ((*itLeft)->x < 0 || (*itLeft)->y < 0 ||   // Out of bounds
        ...
        fabs((*itLeft)->y - (*itRight)->y) >
            options.epipolar_TH) // Not fulfillment of the epipolar constraint
    {
      // Show reason
      std::cout << "Bad tracked match:";
      if ((*itLeft)->x < 0 || (*itLeft)->y < 0 || (*itRight)->x < 0 ||
          (*itRight)->y < 0)
        std::cout << " Out of bounds: (" << (*itLeft)->x << "," << (*itLeft)->y
                  << " & (" << (*itRight)->x << "," << (*itRight)->y << ")"
                  << std::endl;
...
-------------------------------------------------------------*/
void vision::filterBadCorrsByDistance(TMatchingPairList &feat_list,
