...
**
************************************************************************************************/
void CFeatureExtraction::extractFeaturesSIFT(const CImage &img,
                                             CFeatureList &feats,
  ...
  // use a smart pointer so we just copy the pointer if the image is grayscale,
  // or we'll create a new one if it was RGB:
  CImage img_grayscale(img, FAST_REF_OR_CONVERT_TO_GRAY); // Was: auxImg::Ptr;
  if (usingROI) {
    ASSERT_(ROI.xMin >= 0 && ROI.xMin < ROI.xMax && ROI.xMax < img.getWidth() &&
  ...
  case CSBinary: {
    ...
    if (onlyDesc) {
      ...
      os::strcat(filFeat, 1000, "temp_feats.txt"); // KEYPOINTS INPUT FILE
    ...
    if (!ret)
      THROW_EXCEPTION("[extractFeaturesSIFT] Could not launch external "
    ...
    std::cout << "[computeSiftFeatures] " << aux.getRowCount() << " features."
              << std::endl;
    ...
    remove(filOut);
#else
    THROW_EXCEPTION("Unfortunately, this SIFT Implementation only runs in "
                    "Windows OS, try Hess implementation");
#endif
    break;
...
// --------------------------------------------------------------------------------------
  ...
  case LoweBinary: // Binary by Lowe
  {
    ...
    GetTempPathA(1000, filImg);
    os::strcat(filImg, 1000,
               "temp_img.pgm"); // INPUT IMAGE (PGM) FOR ORIGINAL BINARY BY LOWE
    ...
    os::strcpy(
        paramImg, 1000,
        format("cmd /C gmic.exe %s -o %s -quiet", filImg, filImg).c_str());
    ...
    for (size_t i = 0; i < nFeats; i++) {
      ...
      // IMPORTANTE NOTE: Lowe format stores first the 'y' coordinate and then
      // the 'x' one
      float fx, fy, fo, fs;
  ...
  case Hess: // Implementation by Robert Hess
  {
    ...
    THROW_EXCEPTION("Method not available since MRPT has been compiled without "
                    "Hess' SIFT library")
    ...
    compute_descriptors(features, gauss_pyr, SIFT_DESCR_WIDTH,
    ...
    if (nDesiredFeatures > 0) {
      if (nDesiredFeatures < (unsigned int)features->total)
        ...
        cout << "[Warning] Detected less features than the requested "
             << features->total << " vs " << nDesiredFeatures << endl;
    } // end if
  ...
  case OpenCV: {
    ...
    while (cont != nMax && i != N) {
      ...
      if (options.patchSize == 0 ||
        ...
        memcpy(&(ft->descriptors.SIFT[0]), &desc.data[128 * i],
               128 * sizeof(ft->descriptors.SIFT[0])); // The descriptor
...
//} // end for features
  ...
  default: {
    cout << "SIFT Extraction method not supported for features with already "
...
// ------------------------------------------------------------------------------------
void CFeatureExtraction::insertCvSeqInCFeatureList(void *features_,
                                                   CFeatureList &list,
                                                   unsigned int init_ID) const {
#if MRPT_HAS_OPENCV && MRPT_HAS_SIFT_HESS
  CvSeq *features = reinterpret_cast<CvSeq *>(features_);
  ...
  for (itFeats = featList.begin(); itFeats != featList.end(); itFeats++) {
    ...
    for (o = 0; o < octvs; o++) {
      ...
      for (i = 1; i <= intvls; i++) {
        ...
        // Get number of times the point is bigger or lower than the
        // surroundings
...
**
************************************************************************************************/
double CFeatureExtraction::getLaplacianValue(void *dog_pyr_, int octv,
                                             int intvl, float row,
                                             float col) const {
#if MRPT_HAS_OPENCV && MRPT_HAS_SIFT_HESS
  IplImage ***dog_pyr = reinterpret_cast<IplImage ***>(dog_pyr_);
  ...
  for (j = -1; j <= 1; j++)
    for (k = -1; k <= 1; k++) {
      ...
      if (k == 0 && j == 0)
        value +=
            8 * pixval32f(dog_pyr[octv][intvl], (int)(row + j), (int)(col + k));
      else
        value -=
            pixval32f(dog_pyr[octv][intvl], (int)(row + j), (int)(col + k));
    }
  return value;
#else
  THROW_EXCEPTION(
...
void CFeatureExtraction::getTimesExtrema(void *dog_pyr_, int octv, int intvl,
                                         float row, float col,
                                         unsigned int &nMin,
  ...
  for (i = -1; i <= 1; i++)
    for (j = -1; j <= 1; j++)
      for (k = -1; k <= 1; k++) {
        ...
        // endl;
        if (val >
            pixval32f(dog_pyr[octv][intvl + i], (int)(row + j), (int)(col + k)))
          nMax++;
        if (val <
            pixval32f(dog_pyr[octv][intvl + i], (int)(row + j), (int)(col + k)))
          nMin++;
