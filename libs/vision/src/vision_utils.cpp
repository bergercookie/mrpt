...
#if MRPT_HAS_OPENCV
	cvFlip(
		img.getAs<IplImage>());  // More params exists, they could be added in
	// the future?
	...
	const TStereoSystemParams& params)
{
		 ...
		 ++itList1, ++lFeat)
	{
		...
		switch (options.matching_method)
		{
			case TMatchingOptions::mmDescriptorSIFT:
				cond1 =
					minDist1 < options.maxEDD_TH;  // Maximum Euclidean Distance
			...
			case TMatchingOptions::mmCorrelation:
				...
				cond2 = (maxCC2 / maxCC1) <
						options.rCC_TH;  // Ratio between the two
				// highest cross
				// correlation values
				minVal = 1 - maxCC1;
				break;
			case TMatchingOptions::mmDescriptorSURF:
				cond1 = minDist1 <
						options.maxEDSD_TH;  // Maximum Euclidean Distance
				// between SURF descriptors
				// (EDSD)
				cond2 =
					(minDist1 / minDist2) <
