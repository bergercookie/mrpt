...
-------------------------------------------------------------*/
void vision::openCV_cross_correlation(const CImage &img,
                                      const CImage &patch_img, size_t &x_max,
                                      size_t &y_max, double &max_val,
                                      int x_search_ini, int y_search_ini,
                                      int x_search_size, int y_search_size) {
  MRPT_START
  ...
  bool entireImg = (x_search_ini < 0 || y_search_ini < 0 || x_search_size < 0 ||
                    y_search_size < 0);
  ...
  IplImage *result = cvCreateImage(cvSize(x_search_size + 1, y_search_size + 1),
                                   IPL_DEPTH_32F, 1);
  ...
  if (entireImg) {
    ...
    im.extract_patch(img_region_to_search,
  ...
  cvMatchTemplate(img_region_to_search.getAs<IplImage>(),
                  patch_im.getAs<IplImage>(), result, CV_TM_CCORR_NORMED);
...
#if MRPT_HAS_OPENCV
  cvFlip(img.getAs<IplImage>()); // More params exists, they could be added in
                                 // the future?
...
-------------------------------------------------------------*/
CMatrixDouble33 vision::buildIntrinsicParamsMatrix(const double focalLengthX,
...
-------------------------------------------------------------*/
CMatrixDouble33 vision::defaultIntrinsicParamsMatrix(unsigned int camIndex,
                                                     unsigned int resX,
                                                     unsigned int resY) {
  float fx, fy, cx, cy;
  ...
  return buildIntrinsicParamsMatrix(resX * fx, resY * fy, resX * cx, resY * cy);
}
...
/*------------------------------------------------------------
                                        rowChecking
-------------------------------------------------------------*/
void vision::rowChecking(CFeatureList &leftList, CFeatureList &rightList,
                         float threshold) {
...
/*------------------------------------------------------------
                                        getDispersion
-------------------------------------------------------------*/
void vision::getDispersion(const CFeatureList &list, CVectorFloat &std,
                           CVectorFloat &mean) {
  std.assign(2, 0);
  mean.assign(2, 0);
...
/*-------------------------------------------------------------
                    cloudsToMatchedList
-------------------------------------------------------------*/
void vision::cloudsToMatchedList(const CObservationVisualLandmarks &cloud1,
...
-------------------------------------------------------------*/
float vision::computeMainOrientation(const CImage &image, unsigned int x,
                                     unsigned int y) {
  MRPT_START
  float orientation = 0;
  if ((int(x) - 1 >= 0) && (int(y) - 1 >= 0) && (x + 1 < image.getWidth()) &&
      (y + 1 < image.getHeight()))
    orientation =
        (float)atan2((double)*image(x, y + 1) - (double)*image(x, y - 1),
...
void vision::normalizeImage(const CImage &image, CImage &nimage) {
  ...
  nimage.resize(image.getWidth(), image.getHeight(), 1,
...
size_t vision::matchFeatures(const CFeatureList &list1,
  ...
  for (lFeat = 0, itList1 = list1.begin(); itList1 != list1.end();
         ...
         ++itList2, ++rFeat) // ... compare with all the features in list2.
    {
      ...
      bool c2 = options.useXRestriction ? ((*itList1)->x - (*itList2)->x) > 0
      ...
      if (c1 && c2) {
        ...
        case TMatchingOptions::mmDescriptorSIFT: {
          ...
          ASSERT_((*itList1)->descriptors.hasDescriptorSIFT() &&
        ...
        case TMatchingOptions::mmCorrelation: {
          ...
          vision::openCV_cross_correlation((*itList1)->patch, (*itList2)->patch,
                                           u, v, res);
        ...
        case TMatchingOptions::mmDescriptorSURF: {
          ...
          ASSERT_((*itList1)->descriptors.hasDescriptorSURF() &&
        ...
        case TMatchingOptions::mmDescriptorORB: {
          // Ensure that both features have SURF descriptors
          ASSERT_((*itList1)->descriptors.hasDescriptorORB() &&
        ...
        case TMatchingOptions::mmSAD: {
          ...
          if ((*itList1)->patch.isColor() && (*itList2)->patch.isColor()) {
            ...
            aux1 = cvCreateImage(cvSize((*itList1)->patch.getHeight(),
                                        (*itList1)->patch.getWidth()),
                                 IPL_DEPTH_8U, 1);
            aux2 = cvCreateImage(cvSize((*itList2)->patch.getHeight(),
          ...
          //					for( unsigned int ii = 0; ii < (unsigned
          ...
          // NEW CODE
          double res = 0;
          for (unsigned int ii = 0; ii < (unsigned int)aux1->height;
               ++ii) // Rows
            for (unsigned int jj = 0; jj < (unsigned int)aux1->width;
                 ++jj) // Cols
              res +=
                  fabs((double)(aux1->imageData[ii * aux1->widthStep + jj]) -
    ...
    switch (options.matching_method) {
    case TMatchingOptions::mmDescriptorSIFT:
      cond1 = minDist1 < options.maxEDD_TH; // Maximum Euclidean Distance
                                            // between SIFT descriptors (EDD)
    ...
    case TMatchingOptions::mmCorrelation:
      ...
      cond2 =
          (maxCC2 / maxCC1) <
          options
              .rCC_TH; // Ratio between the two highest cross correlation values
      minVal = 1 - maxCC1;
      break;
    case TMatchingOptions::mmDescriptorSURF:
      cond1 = minDist1 < options.maxEDSD_TH; // Maximum Euclidean Distance
                                             // between SURF descriptors (EDSD)
      cond2 = (minDist1 / minDist2) <
    ...
    if (cond1 && cond2) // The minimum distance must be below a threshold
    {
      ...
      if (auxIdx != FEAT_FREE) {
        if (distCorrs[auxIdx] > minVal) {
          ...
          //                    auxIdx << "(" << distCorrs[ auxIdx ] << ")";
          //                    cout << " now is with [L] " << minLeftIdx << "("
          //                    << minVal << ")" << endl;
  ...
  for (int vCnt = 0; vCnt < (int)idxLeftList.size(); ++vCnt) {
    if (idxLeftList[vCnt] != FEAT_FREE) {
      ...
      if (options.estimateDepth && options.parallelOpticalAxis) {
        projectMatchedFeature(list1[vCnt], list2[idxLeftList[vCnt]], p3D,
                              params);
        ...
        dp2 = sqrt((p3D.x - params.baseline) * (p3D.x - params.baseline) +
...
void vision::generateMask(const CMatchedFeatureList &mList, CMatrixBool &mask1,
                          CMatrixBool &mask2, int wSize) {
...
double vision::computeSAD(const CImage &patch1, const CImage &patch2) {
  ...
  for (unsigned int ii = 0; ii < (unsigned int)im1->height; ++ii)  // Rows
    for (unsigned int jj = 0; jj < (unsigned int)im1->width; ++jj) // Cols
      res += fabs((double)(im1->imageData[ii * im1->widthStep + jj]) -
...
/*-------------------------------------------------------------
                                        addFeaturesToImage
-------------------------------------------------------------*/
void vision::addFeaturesToImage(const CImage &inImg,
                                const CFeatureList &theList, CImage &outImg) {
  outImg = inImg; // Create a copy of the input image
  for (CFeatureList::const_iterator it = theList.begin(); it != theList.end();
       ++it)
    outImg.rectangle((*it)->x - 5, (*it)->y - 5, (*it)->x + 5, (*it)->y + 5,
                     TColor(255, 0, 0));
}
...
/*-------------------------------------------------------------
                                        projectMatchedFeatures
-------------------------------------------------------------*/
void vision::projectMatchedFeatures(const CFeatureList &leftList,
...
/*-------------------------------------------------------------
                                        projectMatchedFeatures
-------------------------------------------------------------*/
void vision::projectMatchedFeature(const CFeature::Ptr &leftFeat,
  ...
  for (;;) {
    ...
    xt = C * (params.F.get_unsafe(0, 0) * xhd +
              params.F.get_unsafe(0, 1) * yhd + f0 * params.F.get_unsafe(0, 2));
    yt = C * (params.F.get_unsafe(1, 0) * xhd +
              params.F.get_unsafe(1, 1) * yhd + f0 * params.F.get_unsafe(1, 2));
    xtd = C * (params.F.get_unsafe(0, 0) * xh + params.F.get_unsafe(1, 0) * yh +
               f0 * params.F.get_unsafe(2, 0));
    ytd = C * (params.F.get_unsafe(0, 1) * xh + params.F.get_unsafe(1, 1) * yh +
...
/*-------------------------------------------------------------
                                        projectMatchedFeatures
-------------------------------------------------------------*/
void vision::projectMatchedFeatures(CMatchedFeatureList &mfList,
  ...
  for (itList = mfList.begin(); itList != mfList.end();) {
    ...
    else // This
    {
      ...
      if ((z3D < param.minZ) || (z3D > param.maxZ))
        ...
        // If the matched landmarks has a (SIFT or SURF) descriptor, asign the
        // left one to the landmark.
        ...
        case TStereoSystemParams::Prop_Linear: {
          ...
          lm.pose_cov_11 =
              ...
              stdDisp2 * base2 * square(itList->first->x - c0) / square(disp2);
          lm.pose_cov_12 = stdDisp2 * base2 * (itList->first->x - c0) *
                           (itList->first->y - r0) / square(disp2);
          lm.pose_cov_13 = stdDisp2 * base2 * sqrt(foc2) *
                           (itList->first->x - c0) / square(disp2);
          lm.pose_cov_22 =
              stdPixel2 * base2 / disp2 +
              stdDisp2 * base2 * square(itList->first->y - r0) / square(disp2);
        ...
        case TStereoSystemParams::Prop_UT: {
          ...
          for (i = 1; i <= 2 * Na; i++) {
            ...
            x3D =
                (myPoint[0] - param.K(0, 2)) * ((param.baseline)) / myPoint[2];
            y3D =
                (myPoint[1] - param.K(1, 2)) * ((param.baseline)) / myPoint[2];
...
/*-------------------------------------------------------------
                                        projectMatchedFeatures
-------------------------------------------------------------*/
void vision::projectMatchedFeatures(CFeatureList &leftList,
  ...
  for (itListL = leftList.begin(), itListR = rightList.begin();
    ...
    if (disp < 1e-9)                              // Filter out too far points
    {
      itListL = leftList.erase(
          itListL); // Erase the match : itListL = leftList.erase( itListL );
      itListR = rightList.erase(
          itListR); // Erase the match : itListR = rightList.erase( itListR );
    } else          // This
    {
      // Too much distant features are not taken into account
      ...
      if ((z3D < param.minZ) || (z3D > param.maxZ)) {
        ...
        case TStereoSystemParams::Prop_Linear: {
          ...
          lm.pose_cov_11 =
              stdPixel2 * base2 / disp2 +
              stdDisp2 * base2 * square((*itListL)->x - c0) / square(disp2);
          lm.pose_cov_12 = stdDisp2 * base2 * ((*itListL)->x - c0) *
                           ((*itListL)->y - r0) / square(disp2);
          lm.pose_cov_13 = stdDisp2 * base2 * sqrt(foc2) *
                           ((*itListL)->x - c0) / square(disp2);
          lm.pose_cov_22 =
              stdPixel2 * base2 / disp2 +
              stdDisp2 * base2 * square((*itListL)->y - r0) / square(disp2);
          lm.pose_cov_23 = stdDisp2 * base2 * sqrt(foc2) *
                           ((*itListL)->y - r0) / square(disp2);
          lm.pose_cov_33 = stdDisp2 * foc2 * base2 / square(disp2);
        } // end case 'Prop_Linear'
...
/* -------------------------------------------------------
                                StereoObs2RBObs #1
   ------------------------------------------------------- */
void vision::StereoObs2BRObs(const CMatchedFeatureList &inMatches,
                             const CMatrixDouble33 &intrinsicParams,
                             const double &baseline, const CPose3D &sensorPose,
  ...
  for (CMatchedFeatureList::const_iterator itMatchList = inMatches.begin();
    ...
    // Projection equations according to a standard camera coordinate axis (+Z
    // forward & +Y downwards)
    // -------------------------------------------------------------------------------------------------------
    double X = (x - x0) * b / d;
    double Y = (y - y0) * b / d;
    ...
    // Projection equations according to a standard coordinate axis (+X forward
    // & +Z upwards)
    // -------------------------------------------------------------------------------------------------------
    // double X	= f * b / d;
    // double Y	= ( x0 - x ) * b / d;
    ...
    // Jacobian equations according to a standard coordinate axis (+X forward &
    // +Z upwards)
    // -------------------------------------------------------------------------------------------------------
    ...
    JG.set_unsafe(2, 0,
                  Z * X / (square(m.range) * sqrt(square(X) + square(Y))));
    JG.set_unsafe(2, 1,
                  Z * Y / (square(m.range) * sqrt(square(X) + square(Y))));
  ...
  // Indicate that the covariances have been calculated (for compatibility with
  // earlier versions)
...
/* -------------------------------------------------------
                                StereoObs2RBObs #2
   ------------------------------------------------------- */
void vision::StereoObs2BRObs(const CObservationStereoImages &inObs,
                             const vector<double> &sg,
                             CObservationBearingRange &outObs) {
  // Local variables
...
/* -------------------------------------------------------
                                StereoObs2RBObs #3
   ------------------------------------------------------- */
void vision::StereoObs2BRObs(const CObservationVisualLandmarks &inObs,
                             CObservationBearingRange &outObs) {
  ...
  for (itCloud = inObs.landmarks.landmarks.begin();
    ...
    CObservationBearingRange::TMeasurement m;
    m.range = sqrt(square(itCloud->pose_mean.x) + square(itCloud->pose_mean.y) +
    ...
    // The reference system is assumed to be that typical robot operation: +X
    // forward and +Z upwards.
...
void vision::computeStereoRectificationMaps(
  ...
  // OpenCV 2.0.X
  cv::stereoRectify(K1, D1, K2, D2, nSize, R, T, R1, R2, P1, P2, Q,
                    cv::CALIB_ZERO_DISPARITY);
#elif MRPT_OPENCV_VERSION_NUM < 0x230
  // OpenCV 2.1.X - 2.2.X
  cv::stereoRectify(K1, D1, K2, D2, nSize, R, T, R1, R2, P1, P2, Q, alpha,
...
// Rest of arguments -> default
  ...
  cv::Size sz1, sz2;
  cv::initUndistortRectifyMap(K1, D1, R1, P1, cv::Size(resX, resY), CV_32FC1,
                              *mapx1, *mapy1);
  cv::initUndistortRectifyMap(K2, D2, R2, P2, cv::Size(resX, resY), CV_32FC1,
                              *mapx2, *mapy2);
/**/
#else
  THROW_EXCEPTION("The MRPT has been compiled with MRPT_HAS_OPENCV = 0 or "
                  "OpenCV version is < 2.1.1!");
#endif
...
TStereoSystemParams::TStereoSystemParams()
    : uncPropagation(Prop_Linear), baseline(0.119f), // Bumblebee
      stdPixel(1), stdDisp(1), maxZ(20.0f),          // Indoor
      minZ(0.5f),                                    // Indoor
      maxY(3.0f),                                    // Indoor
      factor_k(1.5f), factor_a(1e-3f), factor_b(2.0f) {
...
-------------------------------------------------------------*/
void TStereoSystemParams::loadFromConfigFile(const CConfigFileBase &iniFile,
...
TMatchingOptions::TMatchingOptions()
    :                               // General
      ...
      // SAD
      maxSAD_TH(0.4), SAD_RATIO(0.5),

      // For estimating depth
      estimateDepth(false), maxDepthThreshold(15.0) {
} // end constructor TMatchingOptions

/*-------------------------------------------------------------
                        TMatchingOptions: loadFromConfigFile
-------------------------------------------------------------*/
void TMatchingOptions::loadFromConfigFile(const CConfigFileBase &iniFile,
                                          const string &section) {
  int mm =
  ...
  useDisparityLimits = iniFile.read_bool(section.c_str(), "useDisparityLimits",
                                         useDisparityLimits);
  ...
  maxDepthThreshold = iniFile.read_float(section.c_str(), "maxDepthThreshold",
                                         maxDepthThreshold);
