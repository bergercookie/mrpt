...
inline void reprojectionResidualsElement(
  ...
  if (use_robust_kernel) {
    ...
    if (out_kernel_1st_deriv)
      *out_kernel_1st_deriv = kernel_1st_deriv;
  } else {
    sum += sum_2;
  }
}
...
double mrpt::vision::reprojectionResiduals(
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    if (frame_poses_are_inverse)
      reprojectionResidualsElement<true>(camera_params, OBS, out_residuals[i],
                                         frame, point, sum, use_robust_kernel,
                                         kernel_param, ptr_1st_deriv);
    else
      reprojectionResidualsElement<false>(camera_params, OBS, out_residuals[i],
                                          frame, point, sum, use_robust_kernel,
                                          kernel_param, ptr_1st_deriv);
  }
...
void mrpt::vision::add_se3_deltas_to_frames(const TFramePosesVec &frame_poses,
