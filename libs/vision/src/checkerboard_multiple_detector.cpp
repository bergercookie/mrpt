...
bool find_chessboard_corners_multiple(
  ...
  for (int dilations = 0; !last_dilation; dilations++) {
    ...
    for (size_t i = 0; i < quads.size(); i++) {
      ...
      quad_centers[i][0] = 0.25 * (q->corners[0]->pt.x + q->corners[1]->pt.x +
                                   q->corners[2]->pt.x + q->corners[3]->pt.x);
      quad_centers[i][1] = 0.25 * (q->corners[0]->pt.y + q->corners[1]->pt.y +
    ...
    for (size_t nClusters = 1; nClusters < MAX_NUM_CLUSTERS; nClusters++) {
...
#if VIS
      {
        ...
        for (size_t i = 0; i < quad_centers.size(); i++) {
          static const TColor colors[4] = {TColor(255, 0, 0), TColor(0, 0, 255),
                                           TColor(255, 0, 255),
                                           TColor(0, 255, 0)};
          im.cross(quad_centers[i][0], quad_centers[i][1],
      ...
      for (size_t i = 0; i < nClusters; i++) {
        ...
        for (int group_idx = 0;; group_idx++) {
          ...
          if (count == expected_quads_count) {
            ...
            {
              static mrpt::gui::CDisplayWindow win;
              win.setWindowTitle(format("Candidate group #%i (%i)",
                                        (int)group_idx,
                                        (int)quad_group.size()));
              mrpt::utils::CImage im;
              img.colorImage(im);
              for (size_t i = 0; i < quad_group.size(); i++) {
                static const TColor colors[4] = {
                    TColor(255, 0, 0), TColor(0, 0, 255), TColor(255, 0, 255),
                    TColor(0, 255, 0)};
                const double x = 0.25 * (quad_group[i]->corners[0]->pt.x +
                                         quad_group[i]->corners[1]->pt.x +
                                         quad_group[i]->corners[2]->pt.x +
                                         quad_group[i]->corners[3]->pt.x);
                const double y = 0.25 * (quad_group[i]->corners[0]->pt.y +
            ...
            // Add this set of quads as a good result to be returned to the
            // user:
  ...
  for (list<vector<CvCBQuad::Ptr>>::const_iterator it =
    ...
    for (size_t i = 0; i < it->size();
         i++) { // JL: Avoid the normalizations of all the averages, since it
                // really doesn't matter for our purpose.
      boardCenter += TPoint2D(
          /*0.25* */ (*it)[i]->corners[0]->pt.x + (*it)[i]->corners[1]->pt.x +
              (*it)[i]->corners[2]->pt.x + (*it)[i]->corners[3]->pt.x,
          /*0.25* */ (*it)[i]->corners[0]->pt.y + (*it)[i]->corners[1]->pt.y +
              (*it)[i]->corners[2]->pt.y + (*it)[i]->corners[3]->pt.y);
    }
