...
double mrpt::vision::bundle_adj_full(
  ...
  ba_compute_Jacobians<INV_POSES_BOOL>(frame_poses, landmark_points,
                                       camera_params, jac_data_vec,
  ...
  ba_build_gradient_Hessians(observations, residual_vec, jac_data_vec, H_f,
                             eps_frame, H_p, eps_point, num_fix_frames,
                             num_fix_points,
                             use_robust_kernel ? &kernel_1st_deriv : nullptr);
  ...
  for (size_t iter = 0; iter < max_iters; iter++) {
    ...
    if (user_feedback)
      (*user_feedback)(iter, res, max_iters, observations, frame_poses,
                       landmark_points);
    ...
    do {
      ...
      aligned_containers<Matrix_FxF>::vector_t U_star(num_free_frames,
                                                      I_muFrame);
      ...
      for (TSequenceFeatureObservations::const_iterator it_obs =
        ...
        if (jac_iter->J_frame_valid && jac_iter->J_point_valid) {
          const CMatrixFixedNumeric<double, ObsDim, FrameDof> &J_frame =
              jac_iter->J_frame;
          const CMatrixFixedNumeric<double, ObsDim, PointDof> &J_point =
              jac_iter->J_point;
      ...
      aligned_containers<pair<TCameraPoseID, TLandmarkID>, Matrix_FxF>::map_t
          YW_map;
      for (size_t i = 0; i < H_f.size(); ++i)
        YW_map[std::pair<TCameraPoseID, TLandmarkID>(i, i)] = U_star[i];

      CVectorDouble delta(len_free_frames +
                          len_free_points); // The optimal step
      ...
      for (size_t j = 0; j < num_free_frames; ++j)
        ::memcpy(
            ...
            sizeof(e[0]) *
                FrameDof); // e.slice(j*FrameDof,FrameDof) = AT(eps_frame,j);
      ...
      for (WMap::iterator Y_ij = Y.begin(); Y_ij != Y.end(); ++Y_ij) {
        ...
        // for (size_t k=0; k<num_free_frames; ++k)
        {
          ...
          Matrix_FxF YWt(
              UNINITIALIZED_MATRIX); //-(Y_ij->second) * (W_ik->second).T();
      ...
      try {
        ...
        ::memcpy(&delta[0], &bck_res[0],
                 bck_res.size() *
                     sizeof(bck_res[0])); // delta.slice(0,...) = Ch.backsub(e);
      ...
      ::memcpy(
          &g[0], &e[0],
          len_free_frames *
              sizeof(g[0])); // g.slice(0,FrameDof*(num_frames-num_fix_frames))
                             // = e;
      ...
      for (size_t i = 0; i < num_free_points; ++i) {
        ...
        for (size_t j = 0; j < num_free_frames; ++j) {
          WMap::iterator W_ij;
          W_ij = W.find(make_pair(TCameraPoseID(j + num_fix_frames),
        ...
        ::memcpy(&delta[len_free_frames + i * PointDof], &Vi_tmp[0],
                 sizeof(Vi_tmp[0]) * PointDof);
        ::memcpy(&g[len_free_frames + i * PointDof], &eps_point[i][0],
      ...
      add_se3_deltas_to_frames(frame_poses, delta, 0, len_free_frames,
                               new_frame_poses, num_fix_frames);
      profiler.leave("add_se3_deltas_to_frames");

      profiler.enter("add_3d_deltas_to_points");
      add_3d_deltas_to_points(landmark_points, delta, len_free_frames,
                              len_free_points, new_landmark_points,
                              num_fix_points);
      ...
      double res_new = mrpt::vision::reprojectionResiduals(
          observations, camera_params, new_frame_poses, new_landmark_points,
          new_residual_vec,
          INV_POSES_BOOL, // are poses inverse?
          use_robust_kernel, kernel_param,
