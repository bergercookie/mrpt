...
/************************************************************************************\
    ...
    <a href="mailto:pgruebele@cox.net">pgruebele@cox.net</a>
        ...
        His code was adapted for use with low resolution and omnidirectional
cameras
...
int cvFindChessboardCorners3(const mrpt::utils::CImage &img_,
  ...
  vector<CvCBCorner::Ptr> corners;  // CvCBCorner *corners			=
                                    // 0;
  vector<CvCBQuad::Ptr>
  ...
  cvAdaptiveThreshold(img.getAs<IplImage>(), thresh_img.getAs<IplImage>(), 255,
...
// VISUALIZATION--------------------------------------------------------------
  ...
  for (int dilations = 0; !last_dilation; dilations++) {
    ...
    // Dilate squares:
    last_dilation = do_special_dilation(thresh_img, dilations, kernel_cross,
                                        kernel_rect, kernel_diag1, kernel_diag2,
                                        kernel_horz, kernel_vert);
    ...
    quad_count =
        icvGenerateQuads(quads, corners, thresh_img, flags, dilations, true);
    if (quad_count <= 0)
      continue;
...
// VISUALIZATION--------------------------------------------------------------
        ...
        cvCreateImage(cvGetSize(thresh_img.getAs<IplImage>()), 8, 3);
    {
      ...
      cnt++;
      cvSaveImage(
          mrpt::format("./DBG_dilation=%i_quads.png", (int)dilations).c_str(),
          imageCopy22);
      ...
      cvSaveImage(mrpt::format("./DBG_allFoundNeighbors_%05i.png", cnt).c_str(),
                  imageCopy3);
    }
#endif
    // END------------------------------------------------------------------------
...
// VISUALIZATION--------------------------------------------------------------
        ...
        static int cnt = 0;
        cvSaveImage(
            format("./DBG_CornersIncreasingOrder_%05i.png", cnt++).c_str(),
            imageCopy11);
// cvWaitKey(0);
#endif
        // END------------------------------------------------------------------------
  ...
  // If enough corners have been found already, then there is no need for PART 2
  // ->EXIT
...
// VISUALIZATION--------------------------------------------------------------
      ...
      for (size_t kkk = 0; kkk < max_count; kkk++) {
        ...
        cvFillConvexPoly(imageCopy23, pt, 4,
...
// cvWaitKey(0);
      ...
      quad_count =
          icvGenerateQuads(quads, corners, thresh_img, flags, dilations, false);
      if (quad_count <= 0)
        continue;
...
// cvWaitKey(0);
      ...
      int feedBack = -1;
      while (feedBack == -1) {
        feedBack = mrAugmentBestRun(quads, dilations, output_quad_group,
                                    max_dilation_run_ID);
...
double triangleArea(double x0, double y0, double x1, double y1, double x2,
                    double y2) {
...
void icvCleanFoundConnectedQuads(std::vector<CvCBQuad::Ptr> &quad_group,
  ...
  for (size_t i = 0; i < nQuads; i++) {
    ...
    // Quad area:
    const double a = triangleArea(q->corners[0]->pt.x, q->corners[0]->pt.y,
...
void mrLabelQuadGroup(std::vector<CvCBQuad::Ptr> &quad_group,
                      const CvSize &pattern_size, bool firstRun) {
      ...
      if ((quad_group[i])->labeled == false) {
        ...
        for (size_t j = 0; j < 4; j++) {
          ...
          if ((quad_group[i])->neighbors[j]) {
            CvCBQuad::Ptr &quadNeighborJ = quad_group[i]->neighbors[j];
            ...
            if (quadNeighborJ->labeled == true) {
              ...
              CvCBCorner::Ptr &conCorner =
                  quadNeighborJ->corners[connectedNeighborCornerId];
              CvCBCorner::Ptr &conCornerCW1 =
                  quadNeighborJ->corners[(connectedNeighborCornerId + 1) % 4];
              CvCBCorner::Ptr &conCornerCW2 =
                  quadNeighborJ->corners[(connectedNeighborCornerId + 2) % 4];
              CvCBCorner::Ptr &conCornerCW3 =
                  quadNeighborJ->corners[(connectedNeighborCornerId + 3) % 4];
            ...
            if (number == 1) {
              ...
              if (delta_x != 0 || delta_y != 0) {
                ...
                (quad_group[k])->corners[l]->pt.x =
                    (quad_group[k])->corners[l]->pt.x - delta_x / 2;
                (quad_group[quadID])->corners[cornerID]->pt.x =
                    (quad_group[quadID])->corners[cornerID]->pt.x + delta_x / 2;
                (quad_group[k])->corners[l]->pt.y =
                    (quad_group[k])->corners[l]->pt.y - delta_y / 2;
                (quad_group[quadID])->corners[cornerID]->pt.y =
                    (quad_group[quadID])->corners[cornerID]->pt.y + delta_y / 2;
              }
            } else if (number > 2) {
  ...
  //  -	If both are set to false -> There is a possibility that the larger side
  //  is
...
int mrAugmentBestRun(std::vector<CvCBQuad::Ptr> &new_quads, int new_dilation,
  ...
  // conservative assumption that dilation was performed using the 3x3 CROSS
  // kernel, which coresponds to the 4-neighborhood.
  const float thresh_dilation = (float)(2 * new_dilation + 3) *
                                (2 * old_dilation + 3) *
                                2; // the "*2" is for the x and y component
  // int idx, i, k, j;																// the "3"
  // is for initial corner mismatch
  float dx, dy, dist;
  ...
  for (size_t idx = 0; idx < old_quads.size(); idx++) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (closest_corner_idx >= 0 && min_dist < FLT_MAX) {
        ...
        newQuad->group_idx = cur_quad->group_idx; // the same as the current
                                                  // quad
...
int icvGenerateQuads(vector<CvCBQuad::Ptr> &out_quads,
                     ...
                     const mrpt::utils::CImage &image, int flags, int dilation,
                     bool firstRun) {
  MRPT_UNUSED_PARAM(dilation);
  // Initializations
  int quad_count = 0;
...
// found quadrangles
  ...
  scanner = cvStartFindContours(const_cast<IplImage *>(image.getAs<IplImage>()),
                                temp_storage, sizeof(CvContourEx),
  ...
  while ((src_contour = cvFindNextContour(scanner)) != 0) {
    ...
    if (CV_IS_SEQ_HOLE(src_contour) && rect.width * rect.height >= min_size) {
      ...
      int approx_level;
      for (approx_level = min_approx_level; approx_level <= max_approx_level;
           approx_level++) {
  ...
  for (int idx = 0; idx < root->total; idx++) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      CvCBCorner::Ptr &corner =
          out_corners[quad_count * 4 + i]; // &(*out_corners)[quad_count*4 + i];
...
int myQuads2Points(const std::vector<CvCBQuad::Ptr> &output_quads,
  ...
  for (size_t i = 0; i < output_quads.size(); i++) {
    ...
    for (int j = 0; j < 4; j++) {
      if ((q->corners[j])->column == max_column &&
          (q->corners[j])->row != min_row && (q->corners[j])->row != max_row) {
