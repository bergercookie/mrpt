...
/* ----------------------------------------------------------
                                                open
   ---------------------------------------------------------- */
bool CVideoFileWriter::open(const std::string &out_file, double fps,
                            const mrpt::utils::TImageSize &frameSize,
                            const std::string &fourcc, bool isColor) {
#if MRPT_HAS_OPENCV
  close();
  ...
  m_video =
      cvCreateVideoWriter(out_file.c_str(), cc, fps,
                          cvSize(frameSize.x, frameSize.y), isColor ? 1 : 0);
...
/* ----------------------------------------------------------
                                                operator <<
   ---------------------------------------------------------- */
const CVideoFileWriter &CVideoFileWriter::
operator<<(const mrpt::utils::CImage &img) const {
  if (!m_video.get())
    THROW_EXCEPTION("Call open first")

  if ((size_t)m_img_size.x != img.getWidth() ||
      (size_t)m_img_size.y != img.getHeight())
    THROW_EXCEPTION(format("Video frame size is %ix%i but image is %ux%u",
...
bool CVideoFileWriter::writeImage(const mrpt::utils::CImage &img) const {
  ...
  if ((size_t)m_img_size.x != img.getWidth() ||
      (size_t)m_img_size.y != img.getHeight()) {
    std::cout << format("[CVideoFileWriter::writeImage] Error: video frame "
