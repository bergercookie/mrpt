...
**
************************************************************************************************/
void CFeatureExtraction::extractFeaturesFAST(const mrpt::utils::CImage &inImg,
                                             CFeatureList &feats,
...
//  outside.
  ...
  FastFeatureDetector fastDetector(options.FASTOptions.threshold,
  ...
  FAST(inImg_gray.getAs<IplImage>(), cv_feats, options.FASTOptions.threshold,
       options.FASTOptions.nonmax_suppression);
#endif
  ...
  while (cont != nMax && i != N) {
    ...
    if (!(xBorderSup < (int)imgW && xBorderInf > 0 && yBorderSup < (int)imgH &&
          yBorderInf > 0))
      continue; // nope, skip.
