...
**
************************************************************************************************/
void CFeatureExtraction::detectFeatures(const CImage &img, CFeatureList &feats,
                                        const unsigned int init_ID,
                                        const unsigned int nDesiredFeatures,
                                        const TImageROI &ROI) const {
  switch (options.featsType) {
  case featHarris:
    MRPT_TODO("Refactor: check if OpenCV's tile method can be directly called "
              "to save space here?")
    if (options.harrisOptions.tile_image) {
      ...
      if (!(ROI.xMax == 0 && ROI.xMin == 0 && ROI.yMax == 0 &&
        ...
        std::cout << "Warning: Image ROI is not taken into account, as "
                     "harrisOptions.tile is set to YES"
                  << std::endl;
      ...
      for (unsigned int k = 0; k < 4; k++) // Search over the 2x4 tiled image
      {
        ...
        tictac.Tic();
        extractFeaturesKLT(img, aux_feats[k], init_ID, nDesiredFeatures,
                           newROI);
        cout << "Tiempo en extraer una tile: " << tictac.Tac() * 1000.0f
             << endl;
        ...
        tictac.Tic();
        extractFeaturesKLT(img, aux_feats[k + 4], init_ID, nDesiredFeatures,
                           newROI);
        cout << "Tiempo en extraer una tile: " << tictac.Tac() * 1000.0f
             << endl;
      ...
      feats.clear();
      if (tt > new_nDesiredFeatures) // We have found too many features, we have
                                     // to select them
      {
      ...
      else      // We have found less features than the desired
      {
        ...
        for (unsigned int m = 0; m < 8; m++)
          for (itList = aux_feats[m].begin(); itList != aux_feats[m].end();
               itList++)
...
**
************************************************************************************************/
void CFeatureExtraction::extractFeaturesBCD(const CImage &img,
                                            CFeatureList &feats,
    ...
    : featsType(_featsType) // Default Method: Kanade-Lucas-Tomasi
{
  ...
  LogPolarImagesOptions.num_angles = 16; // Log-Polar image patch will have
                                         // dimensions WxH, with:  W=num_angles,
                                         // H= rho_scale * log(radius)
...
void CFeatureExtraction::TOptions::loadFromConfigFile(
  ...
  MRPT_LOAD_CONFIG_VAR_CAST(SIFTOptions.implementation, int,
                            TSIFTImplementation, iniFile, section)
  ...
  MRPT_LOAD_CONFIG_VAR(SpinImagesOptions.hist_size_distance, int, iniFile,
                       section)
  MRPT_LOAD_CONFIG_VAR(SpinImagesOptions.hist_size_intensity, int, iniFile,
                       section)
  ...
  MRPT_LOAD_CONFIG_VAR(LogPolarImagesOptions.rho_scale, double, iniFile,
                       section)
}
