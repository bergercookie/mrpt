...
void my_cv2eigen(
  ...
  if (!(dst.Flags & Eigen::RowMajorBit)) {
    Mat _dst(src.cols, src.rows, DataType<_Tp>::type, dst.data(),
    ...
    Mat _dst(src.rows, src.cols, DataType<_Tp>::type, dst.data(),
             (size_t)(dst.stride() * sizeof(_Tp)));
    src.convertTo(_dst, _dst.type());
    CV_DbgAssert(_dst.data == (uchar *)dst.data());
  }
...
bool mrpt::vision::checkerBoardCameraCalibration(
  ...
  try {
      ...
      for (y = 0, k = 0; y < check_size_y; y++) {
        for (unsigned int x = 0; x < check_size_x; x++, k++) {
          pattern_obj_points[k].x = -check_squares_length_X_meters *
                                    x; // The "-" is for convenience, so the
                                       // camera poses appear with Z>0
    ...
    for (i = 0, it = images.begin(); it != images.end(); it++, i++) {
      ...
      vector<cv::Point2f> this_img_pts(CORNERS_COUNT); // Temporary buffer for
                                                       // points, to be added if
                                                       // the points pass the
                                                       // checks.
      ...
      if (corners_found) {
        ...
        for (y = 0, k = 0; y < check_size.height; y++)
          for (x = 0; x < check_size.width; x++, k++)
            dat.detected_corners.push_back(mrpt::utils::TPixelCoordf(
        ...
        if (!dat.img_original.isExternallyStored()) {
          ...
          for (y = 0, k = 0; y < check_size.height; y++) {
            ...
            for (x = 0; x < check_size.width; x++, k++) {
              ...
              if (k != 0)
                cvLine(rgb_img, prev_pt, pt, color);
    ...
    const double cv_calib_err =
        cv::calibrateCamera(objectPoints, imagePoints, imgSize, cameraMatrix,
                            distCoeffs, rvecs, tvecs, 0 /*flags*/);
    ...
    for (i = 0; i < valid_detected_imgs; i++) {
      ...
      if (dat.detected_corners.size() != CORNERS_COUNT)
        continue;
      ...
      for (unsigned int p = 0; p < CORNERS_COUNT; p++)
        lstPatternPoints[p] =
            TPoint3D(pattern_obj_points[p].x, pattern_obj_points[p].y,
      ...
      for (unsigned int p = 0; p < CORNERS_COUNT; p++) {
        ...
        if (!dat.img_original.isExternallyStored()) {
          if (px >= 0 && px < imgSize.width && py >= 0 && py < imgSize.height)
            cvCircle(dat.img_rectified.getAs<IplImage>(), cvPoint(px, py), 4,
                     CV_RGB(0, 0, 255));
        }
        ...
        sqrErr +=
            ...
            square(py_d -
                   dat.detected_corners[p]
                       .y); // Error relative to the original (distorted) image.
      }
    }
