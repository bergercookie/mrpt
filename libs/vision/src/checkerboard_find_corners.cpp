...
bool mrpt::vision::findChessboardCorners(
  ...
  if (!useScaramuzzaMethod) {
    ...
    corners_found = 0 != cv::findChessboardCorners(cvImg, check_size, pointbuf,
                                                   find_chess_flags);
    ...
    for (size_t i = 0; i < corners_list.size(); i++) {
      ...
      for (unsigned int y = 0, k = 0; y < check_size_y; y++)
        for (unsigned int x = 0; x < check_size_x; x++, k++)
          cornerCoords[i].push_back(
              TPixelCoordf(corners_list[i][k].x, corners_list[i][k].y));

      // Consistency of the counter-clockwise XYZ reference system and corners
      // ORDER.
      ...
      if (Az[2] > 0) {
        ...
        for (unsigned int y = 0; y < check_size_y; y++)
          std::reverse(cornerCoords[i].begin() + y * check_size_x,
