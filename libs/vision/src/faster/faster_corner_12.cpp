...
template <int I, int N> struct BitCheck {
  template <class C>
  static inline void eval(unsigned int three, const uint8_t *p, const int w,
...
template <int N> struct BitCheck<N, N> {
  template <class C>
  static inline void eval(unsigned int, const uint8_t *, const int, const int,
                          C &) {}
};

template <int CHUNKS, class C>
inline void process_16(unsigned int three, const uint8_t *p, const int w,
                       const int barrier, C &corners) {
  three |= (three >> 16);
...
void faster_corner_detect_12(const IplImage *I,
  ...
  for (Passed::iterator it = passed.begin(); it != passed.end(); ++it) {
    if (*it == 0) {
      ...
      row_start =
          (const uint8_t *)I->imageData + I->widthStep * (++row); //  I[++row];
      continue;
    }
    int x = *it - row_start;
    if (x > 2 && x < w - 3)
