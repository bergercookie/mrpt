...
void faster_corner_detect_9(const IplImage *I,
                            mrpt::vision::TSimpleFeatureList &corners,
                            int barrier, uint8_t octave,
  ...
  for (int y = 3; y < I->height - 3; y++) {
    ...
    for (int x = 3; x < 16; x++)
      if (is_corner_9<Less>((const uint8_t *)I->imageData + I->widthStep * y +
                                x,
                            I->widthStep, barrier) ||
          is_corner_9<Greater>((const uint8_t *)I->imageData +
                                   I->widthStep * y + x,
                               I->widthStep, barrier))
        corners.push_back_fast(x << octave, y << octave);

    for (int x = 16; x < xend; x += 16) {
      const uint8_t *p = (const uint8_t *)I->imageData + I->widthStep * y +
                         x; //(const uint8_t*)I->imageData+I->widthStep*y+x;
      __m128i lo, hi;
      {
      ...
      unsigned int ans_15, ans_1;
      {
        ...
        __m128i c =
            _mm_insert_epi16(_mm_srli_si128(a, 2),
      ...
      unsigned int ans_9, ans_7;
      {
        ...
        __m128i f =
            _mm_insert_epi16(_mm_srli_si128(d, 2),
                             *(const unsigned short *)(p + 15 + stride), 7);
...
void fast_corner_detect_9(const IplImage *I,
                          mrpt::vision::TSimpleFeatureList &corners,
                          int barrier, uint8_t octave,
                          std::vector<size_t> *out_feats_index_by_row) {
  if (I->width < 22) {
