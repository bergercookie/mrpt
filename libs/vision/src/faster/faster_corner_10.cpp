	...
	uint8_t octave, std::vector<size_t>* out_feats_index_by_row)
{
	...
	for (int y = 3; y < I->height - 3; y++)
	{
		...
		for (int x = 3; x < 16; x++)
			if (is_corner_10<Less>(
					(const uint8_t*)I->imageData + I->widthStep * y + x,
					I->widthStep, barrier) ||
				is_corner_10<Greater>(
					(const uint8_t*)I->imageData + I->widthStep * y + x,
					I->widthStep, barrier))
		...
		for (int x = 16; x < xend; x += 16)
		{
			...
			unsigned int ans_a, ans_c;
			{
				...
				__m128i c = _mm_insert_epi16(
					_mm_srli_si128(a, 2),
					*(const unsigned short*)(p + 15 - stride), 7);
			...
			{
				if (possible & (1 << 0)) corners.push_back_fast(x + 0, y);
				if (possible & (1 << 1)) corners.push_back_fast(x + 1, y);
				if (possible & (1 << 2)) corners.push_back_fast(x + 2, y);
				if (possible & (1 << 3)) corners.push_back_fast(x + 3, y);
				if (possible & (1 << 4)) corners.push_back_fast(x + 4, y);
				if (possible & (1 << 5)) corners.push_back_fast(x + 5, y);
				if (possible & (1 << 6)) corners.push_back_fast(x + 6, y);
				if (possible & (1 << 7)) corners.push_back_fast(x + 7, y);
			}
			// if(possible & 0xf0) //Does this mak( ,  fast)r?
			{
				if (possible & (1 << 8)) corners.push_back_fast(x + 8, y);
				if (possible & (1 << 9)) corners.push_back_fast(x + 9, y);
				if (possible & (1 << 10)) corners.push_back_fast(x + 10, y);
				if (possible & (1 << 11)) corners.push_back_fast(x + 11, y);
				if (possible & (1 << 12)) corners.push_back_fast(x + 12, y);
				if (possible & (1 << 13)) corners.push_back_fast(x + 13, y);
				if (possible & (1 << 14)) corners.push_back_fast(x + 14, y);
				if (possible & (1 << 15)) corners.push_back_fast(x + 15, y);
			}
		}
	...
	uint8_t octave, std::vector<size_t>* out_feats_index_by_row)
{
	...
	if (mrpt::system::is_aligned<16>(I->imageData) &&
		...
		faster_corner_detect_10<true>(
			I, corners, barrier, octave, out_feats_index_by_row);
	else
		faster_corner_detect_10<false>(
			I, corners, barrier, octave, out_feats_index_by_row);
#else
	fast_corner_detect_plain_10(
		I, corners, barrier, octave, out_feats_index_by_row);
