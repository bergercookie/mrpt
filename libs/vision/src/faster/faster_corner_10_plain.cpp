...
void fast_corner_detect_plain_10(
	const IplImage* i, TSimpleFeatureList& corners, int b, uint8_t octave,
	std::vector<size_t>* out_feats_index_by_row)
{
	...
	int pixel[16] = {
		0 + i->widthStep * 3,   1 + i->widthStep * 3,   2 + i->widthStep * 2,
		3 + i->widthStep * 1,   3 + i->widthStep * 0,   3 + i->widthStep * -1,
		2 + i->widthStep * -2,  1 + i->widthStep * -3,  0 + i->widthStep * -3,
		-1 + i->widthStep * -3, -2 + i->widthStep * -2, -3 + i->widthStep * -1,
		-3 + i->widthStep * 0,  -3 + i->widthStep * 1,  -2 + i->widthStep * 2,
		-1 + i->widthStep * 3,
	};
	...
	for (y = 3; y < i->height - 3; y++)
	{
		...
		cache_0 =
			(const uint8_t*)i->imageData + i->widthStep * y + 3;  // &i[y][3];
		line_min = cache_0 - 3;
		line_max = (const uint8_t*)i->imageData + i->widthStep * y + i->width -
				   3;  //&i[y][i.size().x - 3];
