...
void CFeatureExtraction::internal_computeSpinImageDescriptors(
  ...
  for (CFeatureList::iterator it = in_features.begin(); it != in_features.end();
       ++it) {
...
// square(k_idx2dis) );
        ...
        const int bin_int_low =
            max(0, static_cast<int>(ceil(
                       k_int2idx *
                       (pix_val -
                        STD_TIMES * options.SpinImagesOptions.std_intensity))));
        ...
        if (center_bin_dist < static_cast<int>(HIST_N_DIS)) // this accounts for
                                                            // the "square" or
                                                            // "circle" shapes
                                                            // of the area to
                                                            // account for
        {
          ...
          const int bin_dist_hi = min(static_cast<int>(HIST_N_DIS - 1),
          ...
          for (bin_dist = bin_dist_low; bin_dist <= bin_dist_hi;
            ...
            for (bin_int = bin_int_low; bin_int <= bin_int_hi;
              ...
              //								_2var_dist
              //* square(pix_dist - bin_dist*k_idx2dis ) +
              //								_2var_int
