...
void CFeatureTracker_KL::trackFeatures_impl_templ(const CImage &old_img,
  ...
  ASSERT_(old_img.getWidth() == new_img.getWidth() &&
  ...
  if (nFeatures > 0) {
    ...
    cvCalcOpticalFlowPyrLK(
        prev_gray_ipl, cur_gray_ipl, pPyr, cPyr, &points[0][0], &points[1][0],
        nFeatures, cvSize(window_width, window_height), LK_levels, &status[0],
        track_error, cvTermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS,
    ...
    for (size_t i = 0; i < nFeatures; ++i) {
      ...
      else // Feature could not be tracked
      {
        ...
        featureList.setTrackStatus(i, trck_err_too_large ? status_LOST
                                                         : status_OOB);
      } // end else
    }   // end for
...
void CFeatureTracker_KL::trackFeatures_impl(const CImage &old_img,
                                            const CImage &new_img,
