...
public:
  ...
  mrpt::poses::CPose3D refCameraPose;  //!< The 3D pose of the reference camera
                                       //!relative to robot coordinates.
  mrpt::maps::CLandmarksMap landmarks; //!< The landmarks, with coordinates
                                       //!origin in the camera reference system.

  /** Implements the virtual method in charge of finding the likelihood between
    *this
    ...
    */
  float
  likelihoodWith(const mrpt::obs::CObservation *anotherObs,
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CObservationVisualLandmarks,
                                             CObservation, VISION_IMPEXP)
