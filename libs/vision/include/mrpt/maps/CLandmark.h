...
public:
  ...
  std::vector<mrpt::vision::CFeature::Ptr>
      features; //!< The set of features from which the landmark comes.

  mrpt::math::TPoint3D pose_mean; //!< The mean of the landmark 3D position.
  mrpt::math::TPoint3D normal; //!< The "normal" to the landmark, i.e. a unitary
                               //!3D vector towards the viewing direction, or a
                               //!null vector if not applicable
  ...
  mrpt::system::TTimeStamp
      timestampLastSeen; //!< The last time that this landmark was observed.
  uint32_t
      seenTimesCount; //!< The number of times that this landmark has been seen.
  ...
  /** Gets the type of the first feature in its feature vector. The vector must
   * not be empty.
    */
  ...
  void createOneFeature() {
    features.assign(1,
                    mrpt::vision::CFeature::Ptr(new mrpt::vision::CFeature()));
  }
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CLandmark,
