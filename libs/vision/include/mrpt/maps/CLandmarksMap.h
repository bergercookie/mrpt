   ...
   public:
	...
	struct VISION_IMPEXP TCustomSequenceLandmarks
	{
	   ...
	   public:
		 ...
		 * the
		 * points, such as a sphere centered at the origin with this radius
		 * cover
		 * ALL the points in the map (the results are buffered, such as, if the
		 * map
		 * is not modified, the second call will be much faster than the first
		 * one).
		  */
	...
	struct VISION_IMPEXP TInsertionOptions : public utils::CLoadableOptions
	{
	   public:
		 ...
		 * of
		 * the ellipsoid in the axis perpendicular to the main directiom
		 * (Default =
		 * 0.05f)
		  */
	...
	struct VISION_IMPEXP TLikelihoodOptions : public utils::CLoadableOptions
	{
	   public:
		 ...
		 * (default=0.08)
		 * [meters] \sa beaconRangesUseObservationStd */
	 ...
	 * "other"
	 * will be inserted into "this" without checking for possible
	 * correspondences
