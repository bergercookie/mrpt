 ...
 *		- For "visual landmarks" from images: features with associated
 descriptors.
 *		- For laser scanners: each of the range measuremnts, as
 "occupancy" landmarks.
 *		- For grid maps: "Panoramic descriptor" feature points.
 *		- For range-only localization and SLAM: Beacons. It is also
 supported the simulation of expected beacon-to-sensor readings, observation
 likelihood,...
 * <br>
 ...
 *
 ...
 added, with an initial uncertainty according to insertionOptions</td> </tr>
...
public:
  ...
  static mrpt::utils::TColorf
      COLOR_LANDMARKS_IN_3DSCENES; //!< The color of landmark ellipsoids in
                                   //!CLandmarksMap::getAs3DObject
  ...
  public:
    ...
    /** The object is copied, thus the original copy passed as a parameter can
     * be released.
      */
  ...
  public:
    ...
    /** If set to true (default), the insertion of a CObservationImage in the
     * map will insert SIFT 3D features.
      */
    bool insert_SIFTs_from_monocular_images;

    /** If set to true (default), the insertion of a CObservationStereoImages in
     * the map will insert SIFT 3D features.
      */
    bool insert_SIFTs_from_stereo_images;

    /** If set to true (default), inserting a CObservation2DRangeScan in the map
     * will generate landmarks for each range.
      */
    ...
    /** [For SIFT landmarks only] The minimum likelihood value of a match to set
     * as correspondence (Default=0.5)
      */
    ...
    /** [For SIFT landmarks only] Method to compute the likelihood (Default = 0
     * (Our method))
      * 0: Our method -> Euclidean Distance between Descriptors and 3D position
    ...
    /** [For SIFT landmarks only] The distance (in meters) of the mean value of
     * landmarks, for the initial position PDF (Default = 3m)
      */
    float SIFTsLoadDistanceOfTheMean;

    /** [For SIFT landmarks only] The width (in meters, standard deviation) of
     * the ellipsoid in the axis perpendicular to the main directiom (Default =
     * 0.05f)
      */
    float SIFTsLoadEllipsoidWidth;

    /** [For SIFT landmarks only] The standard deviation (in pixels) for the
     * SIFTs detector (This is used for the Jacobbian to project stereo images
     * to 3D)
      */
    ...
    /** Maximum depth of 3D landmarks when loading a landmarks map from a stereo
     * image observation.
      */
    float SIFTs_stereo_maxDepth;

    /** Maximum distance (in pixels) from a point to a certain epipolar line to
     * be considered a potential match.
      */
    float SIFTs_epipolar_TH;

    /** Indicates if the images (as well as the SIFT detected features) should
     ...
     * likelihoodOptions member.
      *  \note All parameters of this field can be loaded from a config file.
     * See mrpt::vision::CFeatureExtraction::TOptions for the names of the
     * expected fields.
      */
  ...
  public:
    ...
    /** @name Parameters for: 2D LIDAR scans
      * @{ */
    unsigned int rangeScan2D_decimation; //!< The number of rays from a 2D range
                                         //!scan will be decimated by this
    ...
    int SIFTs_decimation; //!< Considers 1 out of "SIFTs_decimation" visual
                          //!landmarks in the observation during the likelihood
                          //!computation.
    /** Parameters of the SIFT feature detector/descriptors while inserting
     * images in the landmark map.
      *  \note There exists another \a SIFT_feat_options field in the \a
    ...
    /** @name Parameters for: Range-only observation
      * @{ */
    float beaconRangesStd; //!< The standard deviation used for Beacon ranges
                           //!likelihood (default=0.08) [meters] \sa
                           //!beaconRangesUseObservationStd
    bool beaconRangesUseObservationStd; //!< (Default: false) If true,
                                        //!`beaconRangesStd` is ignored and each
                                        //!individual
                                        //!`CObservationBeaconRanges::stdError`
                                        //!field is used instead.
                                        /** @} */

    /** @name Parameters for: External robot poses observation
      * @{ */
    float extRobotPoseStd; //!< The standard deviation used for external robot
                           //!poses likelihood (default=0.05) [meters]
                           /** @} */

    /** @name Parameters for: GPS readings
      * @{ */

    /** This struct store de GPS longitude, latitude (in degrees ) and altitude
     * (in meters) for the first GPS observation
    ...
    public:
      ...
      /** These 3 params allow rotating and shifting GPS coordinates with other
       * 2D maps (e.g. gridmaps).
        * - ang : Map rotation [deg]
        * - x_shift, y_shift: (x,y) offset [m] */
      double ang, x_shift, y_shift;
      unsigned int
          min_sat; //!< Minimum number of sats to take into account the data
    } GPSOrigin;

    float
        GPS_sigma; //!< A constant "sigma" for GPS localization data (in meters)
    /** @} */
  ...
  struct VISION_IMPEXP TFuseOptions {
    ...
    /** Required number of times of a landmark to be seen not to be removed, in
  ...
  /** Save to a text file.
   *  In line "i" there are the (x,y,z) mean values of the i'th landmark + type
   * of landmark + # times seen + timestamp + RGB/descriptor + ID
   *
   ...
   *  \param stdCount The ellipsoids will be drawn from the center to "stdCount"
   *times the "standard deviations". (default is 2std = 95% confidence
   *intervals)
   *
   *  \return Returns false if any error occured, true elsewere.
   */
  bool saveToMATLABScript2D(std::string file, const char *style = "b",
                            float stdCount = 2.0f);
   ...
   *  \return Returns false if any error occured, true elsewere.
   */
  bool saveToMATLABScript3D(std::string file, const char *style = "b",
  ...
  /** Computes the (logarithmic) likelihood function for a sensed observation
    ...
    */
  double computeLikelihood_RSLC_2007(const CLandmarksMap *s,
   ...
   * (insertionOptions.SIFTsLoadDistanceOfTheMean)
    *
    *  \param feat_options Optionally, you can pass here parameters for changing
   * the default SIFT detector settings.
    */
   ...
   * observation consisting of a pair of stereo-image (Previous contents of map
   * will be erased)
    *  The robot is assumed to be at the origin of the map.
    *  Some options may be applicable from "insertionOptions"
    *
  ...
  // See docs in base class
  void computeMatchingWith2D(const mrpt::maps::CMetricMap *otherMap,
  ...
  /** Perform a search for correspondences between "this" and another lansmarks
   * map:
    *  In this class, the matching is established solely based on the landmarks'
   * IDs.
  ...
  /** Changes the reference system of the map "otherMap" and save the result in
   * "this" map.
    */
  void changeCoordinatesReference(const mrpt::poses::CPose3D &newOrg,
                                  const mrpt::maps::CLandmarksMap *otherMap);
   ...
   * with the result.
    *  This process involves fusing corresponding landmarks, then adding the new
   * ones.
    *  \param other The other landmarkmap, whose landmarks will be inserted into
   * "this"
    *  \param justInsertAllOfThem If set to "true", all the landmarks in "other"
   * will be inserted into "this" without checking for possible correspondences
   * (may appear duplicates ones, etc...)
    */
  ...
  /** Simulates a noisy reading toward each of the beacons in the landmarks map,
   * if any.
    * \param in_robotPose This robot pose is used to simulate the ranges to each
   * beacon.
   ...
   * and "CObservationBeaconRanges::stdError" MUST BE FILLED OUT before calling
   * this function.
  ...
  /** Simulates a noisy bearing-range observation of all the beacons (landamrks
   * with type glBeacon) in the landmarks map, if any.
   ...
   * sensorDetectsIDs=false. Spurious readings are assigned a std::string::npos
   * (=-1) index.
   ...
   * mean and the std are zero (the default) no spurious readings are generated.
   ...
   * and "CObservationBearingRange::minSensorDistance" MUST BE FILLED OUT before
   ...
   * sensor_std_* members correctly set to their values.
    * An observation will be generated for each beacon in the map, but notice
   * that some of them may be missed if out of the sensor maximum range or field
   * of view-
    */
  ...
  std::deque<TPairIdBeacon> initialBeacons; //!< Initial contents of the map,
                                            //!especified by a set of 3D Beacons
                                            //!with associated IDs
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CLandmarksMap, CMetricMap,
                                             VISION_IMPEXP)
