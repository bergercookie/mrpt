...
size_t find_descriptor_pairings(
  ...
  typedef typename DESCRIPTOR_KDTREE::kdtree_t::ElementType
      KDTreeElementType; // The expected data type of elements for the kd-tree
  ...
  if (pairings_1_to_multi_2)
    pairings_1_to_multi_2->assign(N, vector_size_t()); // Reset output container
  if (pairings_1_to_2) {
  ...
  if (!N)
    return overall_pairs; // No features -> nothing to do

  if (descriptor == descSIFT) {
    ASSERTMSG_(feats_img1[0]->descriptors.hasDescriptorSIFT(),
               "Request to match SIFT features but feats_img1 has no SIFT "
               "descriptors!")
    ASSERTMSG_(
        sizeof(KDTreeElementType) == sizeof(feats_img1[0]->descriptors.SIFT[0]),
        "Incorrect data type kd_tree::ElementType for SIFT (should be uint8_t)")
  } else if (descriptor == descSURF) {
    ASSERTMSG_(feats_img1[0]->descriptors.hasDescriptorSURF(),
               "Request to match SURF features but feats_img1 has no SURF "
               "descriptors!")
    ASSERTMSG_(
        sizeof(KDTreeElementType) == sizeof(feats_img1[0]->descriptors.SURF[0]),
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    // Include all correspondences below the absolute and the relative threshold
    // (indices comes ordered by distances):
