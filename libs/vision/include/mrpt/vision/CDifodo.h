...
/** This abstract class implements a method called "Difodo" to perform Visual
  ...
  *	It is based on the range flow equation and assumes that the scene is
  *rigid.
  *	It can work with different image resolutions (640 x 480, 320 x 240 or
  *160 x 120) and a different number of
  ...
  *"loadFrame(...)" according to the user application.
  *		  This method has to load the depth image into the variable
  *"depth_wf".
  *		- Call loadFrame();
...
protected:
  ...
  /** Matrix which indicates whether the depth of a pixel is zero (null = 1) or
   * not (null = 00).*/
  ...
  /** Number of coarse-to-fine levels. I has to be consistent with the number of
   * rows and cols, because the
  ...
  /** Speed filter parameters:
    * Previous_speed_const_weight - Directly weights the previous speed in order
   * to calculate the filtered velocity. Recommended range - (0, 0.5)
...
public:
  ...
  /** Get the rows and cols of the depth image that are considered by the visual
   * odometry method. */
  inline void getRowsAndCols(unsigned int &num_rows,
                             unsigned int &num_cols) const {
    num_rows = rows;
    num_cols = cols;
  }

  /** Get the number of coarse-to-fine levels that are considered by the visual
   * odometry method. */
   ...
   */
  inline void getPointsCoord(Eigen::MatrixXf &x, Eigen::MatrixXf &y,
                             Eigen::MatrixXf &z);

  /** Get the depth derivatives (last level) respect to u,v and t respectively
   */
  inline void getDepthDerivatives(Eigen::MatrixXf &cur_du,
