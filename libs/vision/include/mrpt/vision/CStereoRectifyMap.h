...
public:
  ...
  getRectifiedLeftImageParams() const; //!< Just like \a
                                       //!getRectifiedImageParams() but for the
                                       //!left camera only
  const mrpt::utils::TCamera &
  getRectifiedRightImageParams() const; //!< Just like \a
                                        //!getRectifiedImageParams() but for the
                                        //!right camera only
  ...
  /** If enabled, the computed maps will rectify images to a size different than
   * their original size.
    * \note Call this method before building the rectification maps, otherwise
   * they'll be marked as invalid.
    */
  void enableResizeOutput(bool enable, unsigned int target_width = 0,
  ...
  /** Change remap interpolation method (default=Lineal). This parameter can be
   * safely changed at any instant without consequences. */
  ...
  void setRectifyMaps(const std::vector<int16_t> &left_x,
                      const std::vector<uint16_t> &left_y,
                      const std::vector<int16_t> &right_x,
                      const std::vector<uint16_t> &right_y);

  /** Direct input access to rectify maps. This method swaps the vectors so the
   * inputs are no longer available.*/
  void setRectifyMapsFast(std::vector<int16_t> &left_x,
   ...
   * images - \a setFromCamParams() must have been set prior to calling this.
    * The previous contents of the output images are completely ignored, but if
   * they are already of the
   ...
   * parameters of these images.
    * \exception std::exception If the rectification maps have not been
   * computed.
    * \note The same image CANNOT be at the same time input and output, in which
   * case an exception will be raised (but see the overloaded version for
   * in-place rectification)
    */
  void rectify(const mrpt::utils::CImage &in_left_image,
   ...
   * invoke this method simultaneously.
    */
  void rectify(mrpt::utils::CImage &left_image,
   ...
   * left camera).
    */
  void rectify(mrpt::obs::CObservationStereoImages &stereo_image_observation,
               const bool use_internal_mem_cache = true) const;
  ...
  void rectify_IPL(const void *in_left_image, const void *in_right_image,
...
private:
  ...
  mutable mrpt::utils::CImage m_cache1,
      m_cache2; //!< Memory caches for in-place rectification speed-up.
  ...
  mrpt::utils::TStereoCamera
      m_camera_params; //!< A copy of the data provided by the user
  mrpt::utils::TStereoCamera
      m_rectified_image_params; //!< Resulting images params

  mrpt::poses::CPose3DQuat m_rot_left,
      m_rot_right; //!< The rotation applied to the left/right camera so their
                   //!virtual image plane is the same after rectification.
