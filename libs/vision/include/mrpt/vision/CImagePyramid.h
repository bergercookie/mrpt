...
public:
  ...
  /** Fills the vector \a images with the different octaves built from the input
   * image.
    *  \param[in] img The input image. Can be either color or grayscale.
    *  \param[in] nOctaves Number of octaves to build. 1 means just the original
   * image, 2 means the original plus the 1/2 image, etc.
   ...
   * even for color input images.
    * \sa buildPyramidFast
    */
  void buildPyramid(const mrpt::utils::CImage &img, const size_t nOctaves,
   ...
   * converted from RGB to grayscale, the image data buffer is *reutilized*
    ...
    * \sa buildPyramid
    */
  void buildPyramidFast(mrpt::utils::CImage &img, const size_t nOctaves,
                        const bool smooth_halves = true,
                        const bool convert_grayscale = false);
