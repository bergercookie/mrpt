...
struct VISION_IMPEXP CGenericFeatureTracker {
  ...
  inline CGenericFeatureTracker()
      : m_timlog(false), m_update_patches_counter(0), m_check_KLT_counter(0),
  ...
  inline CGenericFeatureTracker(mrpt::utils::TParametersDouble extraParams)
      : extra_params(extraParams), m_timlog(false), m_update_patches_counter(0),
        m_check_KLT_counter(0), m_detector_adaptive_thres(10) {}
  /** Dtor */
  ...
  /** Perform feature tracking from "old_img" to "new_img", with a (possibly
    ...
    *"add_new_features"!=0.
    */
  void trackFeatures(const mrpt::utils::CImage &old_img,
   ...
   * original list of features unmodified and returns the tracked ones in a new
   * list. */
  inline void trackFeaturesNewList(const mrpt::utils::CImage &old_img,
                                   const mrpt::utils::CImage &new_img,
                                   const vision::CFeatureList &in_featureList,
                                   vision::CFeatureList &out_featureList) {
    out_featureList = in_featureList;
    std::for_each(out_featureList.begin(), out_featureList.end(),
  ...
  struct VISION_IMPEXP TExtraOutputInfo {
    size_t raw_FAST_feats_detected; //!< In the new_img with the last adaptive
                                    //!threshold
    size_t num_deleted_feats; //!< The number of features which were deleted due
                              //!to OOB, bad tracking, etc... (only if
                              //!"remove_lost_features" is enabled)
  };

  TExtraOutputInfo
      last_execution_extra_info; //!< Updated with each call to trackFeatures()

protected:
  /** The tracking method implementation, to be implemented in children classes.
   */
  virtual void trackFeatures_impl(const mrpt::utils::CImage &old_img,
                                  const mrpt::utils::CImage &new_img,
                                  TSimpleFeaturefList &inout_featureList);
  ...
  mrpt::utils::CTimeLogger
      m_timlog; //!< the internal time logger, disabled by default.
  ...
  void updateAdaptiveNewFeatsThreshold(const size_t nNewlyDetectedFeats,
...
private:
  ...
  void internal_trackFeatures(const mrpt::utils::CImage &old_img,
                              const mrpt::utils::CImage &new_img,
...
/** Track a set of features from old_img -> new_img using sparse optimal flow
  ...
  *		- "LK_levels" (Default=3) Number of pyramids to build for LK
  *tracking (this parameter only has effects when tracking with CImage's, not
  *with CImagePyramid's).
  *		- "LK_max_iters" (Default=10) Max. number of iterations in LK
  *tracking.
  *		- "LK_epsilon" (Default=0.1) Minimum epsilon step in interations of
  *LK_tracking.
  *		- "LK_max_tracking_error" (Default=150.0) The maximum "tracking
  *error" of LK tracking such as a feature is marked as "lost".
  *
...
protected:
  virtual void
  trackFeatures_impl(const mrpt::utils::CImage &old_img,
                     const mrpt::utils::CImage &new_img,
...
private:
  template <typename FEATLIST>
  void trackFeatures_impl_templ(const mrpt::utils::CImage &old_img,
                                const mrpt::utils::CImage &new_img,
                                FEATLIST &inout_featureList);
};
...
/** Filter bad correspondences by distance
  * ...
  */
void VISION_IMPEXP filterBadCorrsByDistance(
