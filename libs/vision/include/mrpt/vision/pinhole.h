...
/** Project a single 3D point with global coordinates P into a camera at pose F,
  ...
  *"F" is stored:
  *		- INVERSE_CAM_POSE:false -> The local coordinates of the feature
  *wrt the camera F are: \f$ P \ominus F \f$
  *		- INVERSE_CAM_POSE:true  -> The local coordinates of the feature
  *wrt the camera F are: \f$ F \oplus P \f$
  */
template <bool INVERSE_CAM_POSE>
inline mrpt::utils::TPixelCoordf
projectPoint_no_distortion(const mrpt::utils::TCamera &cam_params,
  ...
  ASSERT_(z != 0)
  // Pinhole model:
  return mrpt::utils::TPixelCoordf(cam_params.cx() + cam_params.fx() * x / z,
...
inline void
projectPoint_no_distortion(const POINT &in_point_wrt_cam,
 ...
 * projected normally.
  ...
  */
void VISION_IMPEXP
projectPoint_with_distortion(const mrpt::math::TPoint3D &in_point_wrt_cam,
...
//! \overload
void VISION_IMPEXP
projectPoints_with_distortion(const std::vector<mrpt::math::TPoint3D> &P,
 ...
 * parameters.
  * \sa undistort_points
  */
void VISION_IMPEXP undistort_point(const mrpt::utils::TPixelCoordf &inPt,
