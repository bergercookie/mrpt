...
typedef mrpt::aligned_containers<TCameraPoseID, mrpt::poses::CPose3D>::map_t
    TFramePosesMap; //!< A list of camera frames (6D poses) indexed by unique
                    //!IDs.
typedef mrpt::aligned_containers<mrpt::poses::CPose3D>::vector_t
    TFramePosesVec; //!< A list of camera frames (6D poses), which assumes
                    //!indexes are unique, consecutive IDs.

typedef std::map<TLandmarkID, mrpt::math::TPoint3D>
    TLandmarkLocationsMap; //!< A list of landmarks (3D points) indexed by
                           //!unique IDs.
typedef std::vector<mrpt::math::TPoint3D> TLandmarkLocationsVec; //!< A list of
                                                                 //!landmarks
                                                                 //!(3D points),
                                                                 //!which
                                                                 //!assumes
...
enum TFeatureType {
  featNotDefined =
      -1,       //!< Non-defined feature (also used for Occupancy features)
  ...
  featBeacon,   //!< A especial case: this is not an image feature, but a 2D/3D
                //!beacon (used for range-only SLAM from mrpt::maps::CLandmark)
  featFAST,     //!< FAST feature detector, OpenCV's implementation ("Faster and
                //!better: A machine learning approach to corner detection", E.
                //!Rosten, R. Porter and T. Drummond, PAMI, 2009).
  featFASTER9,  //!< FASTER-9 detector, Edward Rosten's libcvd implementation
                //!optimized for SSE2.
  featFASTER10, //!< FASTER-9 detector, Edward Rosten's libcvd implementation
                //!optimized for SSE2.
  featFASTER12, //!< FASTER-9 detector, Edward Rosten's libcvd implementation
                //!optimized for SSE2.
  featORB //!< ORB detector and descriptor, OpenCV's implementation ("ORB: an
          //!efficient alternative to SIFT or SURF", E. Rublee, V. Rabaud, K.
          //!Konolige, G. Bradski, ICCV, 2012).
};
...
enum TDescriptorType {
  descAny =
      0, //!< Used in some methods to mean "any of the present descriptors"
...
enum TFeatureTrackStatus {
  // Init value
  status_IDLE =
      0, //!< Inactive (right after detection, and before being tried to track)

  // Ok:
  status_TRACKED = 5, //!< Feature correctly tracked

  // Bad:
  status_OOB = 1,  //!< Feature fell Out Of Bounds (out of the image limits, too
                   //!close to image borders)
  status_LOST = 10 //!< Unable to track this feature (mismatch is too high for
                   //!the given tracking window: lack of texture? oclussion?)
};
...
struct VISION_IMPEXP TFeatureObservation {
  ...
  inline TFeatureObservation(const TLandmarkID _id_feature,
  ...
  TCameraPoseID id_frame; //!< A unique ID of a "frame" (camera position) from
                          //!where the feature was observed.
  mrpt::utils::TPixelCoordf
      px; //!< The pixel coordinates of the observed feature
};
...
struct TRelativeFeaturePos {
  ...
  inline TRelativeFeaturePos(const mrpt::vision::TCameraPoseID _id_frame_base,
                             const mrpt::math::TPoint3D &_pos)
      : id_frame_base(_id_frame_base), pos(_pos) {}

  mrpt::vision::TCameraPoseID id_frame_base; //!< The ID of the camera frame
                                             //!which is the coordinate
                                             //!reference of \a pos
  mrpt::math::TPoint3D pos; //!< The (x,y,z) location of the feature, wrt to the
                            //!camera frame \a id_frame_base
};
...
struct VISION_IMPEXP TSequenceFeatureObservations
   ...
   * #FRAME_ID  #FEAT_ID  #PIXEL_X  #PIXEL_Y
    * The file is self-descripting, since the first line contains a comment line
   * (starting with '%') explaining the format.
    * Generated files can be loaded from MATLAB.
    * \sa loadFromTextFile \exception std::exception On I/O error  */
  void saveToTextFile(const std::string &filName,
                      bool skipFirstCommentLine = false) const;
   ...
   * poses to a pair of files in the format
    ...
    */
  bool saveAsSBAFiles(const TLandmarkLocationsVec &pts,
                      const std::string &pts_file, const TFramePosesVec &cams,
  ...
  /** Remove all but one out of \a decimate_ratio camera frame IDs from the list
   * (eg: from N camera pose IDs at return there will be just N/decimate_ratio)
   ...
   * stored here. */
  void
  compressIDs(std::map<TCameraPoseID, TCameraPoseID> *old2new_camIDs = nullptr,
              std::map<TLandmarkID, TLandmarkID> *old2new_lmIDs = nullptr);
};

/** Parameters associated to a stereo system
  */
struct VISION_IMPEXP TStereoSystemParams
    : public mrpt::utils::CLoadableOptions {
  /** Initilization of default parameters */
  TStereoSystemParams();

  void loadFromConfigFile(const mrpt::utils::CConfigFileBase &source,
                          const std::string &section) override; // See base docs
  void
  dumpToTextStream(mrpt::utils::CStream &out) const override; // See base docs

  /** Method for propagating the feature's image coordinate uncertainty into 3D
   * space. Default value: Prop_Linear
    */
...
struct VISION_IMPEXP TMatchingOptions : public mrpt::utils::CLoadableOptions {
  ...
  // For determining
  bool useEpipolarRestriction; //!< Whether or not take into account the
                               //!epipolar restriction for finding
                               //!correspondences
  bool hasFundamentalMatrix;   //!< Whether or not there is a fundamental matrix
  bool parallelOpticalAxis; //!< Whether or not the stereo rig has the optical
                            //!axes parallel
  bool useXRestriction; //!< Whether or not employ the x-coord restriction for
                        //!finding correspondences (bumblebee camera, for
                        //!example)
  bool addMatches; //!< Whether or not to add the matches found into the input
                   //!matched list (if false the input list will be cleared
                   //!before being filled with the new matches)
  bool useDisparityLimits; //!< Whether or not use limits (min,max) for the
                           //!disparity, see also 'min_disp, max_disp'
  bool enable_robust_1to1_match; //!< Whether or not only permit matches that
                                 //!are consistent from left->right and
                                 //!right->left
  ...
  float minDCC_TH; //!< Minimum Difference Between the Maximum Cross Correlation
                   //!Values
  ...
  // SAD
  double maxSAD_TH; //!< Minimum Euclidean Distance Between Sum of Absolute
                    //!Differences
  ...
  //			// To estimate depth
  bool estimateDepth; //!< Whether or not estimate the 3D position of the real
                      //!features for the matches (only with parallelOpticalAxis
                      //!by now).
  double maxDepthThreshold; //!< The maximum allowed depth for the matching. If
                            //!its computed depth is larger than this, the match
                            //!won't be considered.
  //            double  fx,cx,cy,baseline;          //!< Intrinsic parameters of
  //            the stereo rig
...
#define CHECK_MEMBER(_m) this->_m == o._m
  ...
  bool operator==(const TMatchingOptions &o) const {
    return CHECK_MEMBER(useXRestriction) && CHECK_MEMBER(useDisparityLimits) &&
...
struct VISION_IMPEXP TMultiResMatchingOutput {
  ...
  std::vector<int> firstListCorrespondences; //!< Contains the indexes within
                                             //!the second list corresponding to
                                             //!the first one.
  std::vector<int> secondListCorrespondences; //!< Contains the indexes within
                                              //!the first list corresponding to
                                              //!the second one.
  std::vector<int> firstListFoundScales; //!< Contains the scales of the first
                                         //!list where the correspondence was
                                         //!found.
  std::vector<double>
      firstListDistance; //!< Contains the distances between the descriptors.

  TMultiResMatchingOutput()
      : nMatches(0), firstListCorrespondences(), secondListCorrespondences(),
...
struct VISION_IMPEXP TMultiResDescMatchOptions
    : public mrpt::utils::CLoadableOptions {
  bool
      useOriFilter; //!< Whether or not use the filter based on orientation test
  double oriThreshold; //!< The threshold for the orientation test

  bool
      useDepthFilter; //!< Whether or not use the filter based on the depth test

  double matchingThreshold; //!< The absolute threshold in descriptor distance
                            //!for considering a match
  double matchingRatioThreshold; //!< The ratio between the two lowest distances
                                 //!threshold for considering a match
  uint32_t lowScl1, lowScl2;   //!< The lowest scales in the two features to be
                               //!taken into account in the matching process
  uint32_t highScl1, highScl2; //!< The highest scales in the two features to be
                               //!taken into account in the matching process

  uint32_t
      searchAreaSize; //!< Size of the squared area where to search for a match.
  uint32_t lastSeenThreshold;  //!< The allowed number of frames since a certain
                               //!feature was seen for the last time.
  uint32_t timesSeenThreshold; //!< The minimum number of frames for a certain
                               //!feature to be considered stable.

  uint32_t minFeaturesToFind; //!< The minimum number of features allowed in the
                              //!system. If current number is below this value,
                              //!more features will be found.
  uint32_t minFeaturesToBeLost; //!< The minimum number of features allowed in
                                //!the system to not be considered to be lost.

  /** Default constructor
    */
  TMultiResDescMatchOptions()
      : useOriFilter(true), oriThreshold(0.2), useDepthFilter(true),
        matchingThreshold(1e4), matchingRatioThreshold(0.5), lowScl1(0),
        lowScl2(0), highScl1(6), highScl2(6), searchAreaSize(20),
        lastSeenThreshold(10), timesSeenThreshold(5), minFeaturesToFind(30),
  ...
  TMultiResDescMatchOptions(
      ...
      : useOriFilter(_useOriFilter), oriThreshold(_oriThreshold),
        useDepthFilter(_useDepthFilter), matchingThreshold(_th),
        matchingRatioThreshold(_th2), lowScl1(_lwscl1), lowScl2(_lwscl2),
        highScl1(_hwscl1), highScl2(_hwscl2), searchAreaSize(_searchAreaSize),
        lastSeenThreshold(_lsth), timesSeenThreshold(_tsth),
        minFeaturesToFind(_minFeaturesToFind),
        minFeaturesToBeLost(_minFeaturesToBeLost) {}

  void loadFromConfigFile(const mrpt::utils::CConfigFileBase &cfg,
                          const std::string &section) override;
  void saveToConfigFile(mrpt::utils::CConfigFileBase &cfg,
...
struct VISION_IMPEXP TMultiResDescOptions
  ...
  bool blurImage; //!< Whether or not to blur the image previously to compute
                  //!the descriptors
  double fx, cx, cy, baseline; //!< Intrinsic stereo pair parameters for
                               //!computing the depth of the feature
  bool computeHashCoeffs;      //!< Whether or not compute the coefficients for
                               //!mantaining a HASH table of descriptors (for
                               //!relocalization)

  double cropValue; //!< The SIFT-like descriptor is cropped at this value
                    //!during normalization

  /** Default constructor
    */
  TMultiResDescOptions()
      : basePSize(23), sg1(0.5), sg2(7.5), sg3(8.0), computeDepth(true),
        blurImage(true), fx(0.0), cx(0.0), cy(0.0), baseline(0.0),
        computeHashCoeffs(false), cropValue(0.2) {
    scales.resize(7);
    scales[0] = 0.5;
    scales[1] = 0.8;
    scales[2] = 1.0;
  ...
  TMultiResDescOptions(const unsigned int &_basePSize,
                       const std::vector<double> &_scales,
                       const unsigned int &_comLScl,
                       const unsigned int &_comHScl, const double &_sg1,
                       const double &_sg2, const double &_sg3,
                       const bool &_computeDepth, const bool _blurImage,
                       const double &_fx, const double &_cx, const double &_cy,
                       const double &_baseline, const bool &_computeHashCoeffs,
                       const double &_cropValue)
      : basePSize(_basePSize), comLScl(_comLScl), comHScl(_comHScl), sg1(_sg1),
        sg2(_sg2), sg3(_sg3), computeDepth(_computeDepth),
        blurImage(_blurImage), fx(_fx), cx(_cx), cy(_cy), baseline(_baseline),
        computeHashCoeffs(_computeHashCoeffs), cropValue(_cropValue) {
  ...
  void loadFromConfigFile(const mrpt::utils::CConfigFileBase &source,
                          const std::string &section) override; // See base docs
  void
  saveToConfigFile(mrpt::utils::CConfigFileBase &cfg,
