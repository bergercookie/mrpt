...
struct VISION_IMPEXP TImageStereoCallbackData {
  int calibRound; //!< =-1:Processing images;  =0: Initial calib without
                  //!distortion, =1: Calib of all parameters
  size_t current_iter;
  double
      current_rmse; //!< Current root-mean square reprojection error (in pixels)
  unsigned int nImgsProcessed, nImgsToProcess; //!< Info for calibRound==-1
};

/** Prototype of optional user callback function. */
typedef void (*TSteroCalibCallbackFunctor)(const TImageStereoCallbackData &d,
                                           void *user_data);

/** Input parameters for mrpt::vision::checkerBoardStereoCalibration */
struct VISION_IMPEXP TStereoCalibParams {
  unsigned int check_size_x, check_size_y; //!< The number of squares in the
                                           //!checkerboard in the "X" & "Y"
                                           //!direction.
  double check_squares_length_X_meters,
      check_squares_length_Y_meters; //!< The size of each square in the
                                     //!checkerboard, in meters, in the "X" &
                                     //!"Y" axes.
  bool normalize_image;
  bool skipDrawDetectedImgs;
  bool verbose; //!< Show progress messages to std::cout console (default=true)
  size_t
      maxIters; //!< Maximum number of iterations of the optimizer (default=300)
   ...
   * mrpt::utils::TCamera).
    ...
    * \note Default values are to only assume distortion via k1 and k2 (the rest
   * are zeros).
    */
  bool optimize_k1, optimize_k2, optimize_k3, optimize_t1, optimize_t2;

  bool use_robust_kernel;     //!< Employ a Pseudo-Huber robustifier kernel
                              //!(Default: false)
  double robust_kernel_param; //!< The parameter of the robust kernel, in pixels
                              //!(only if use_robust_kernel=true) (Default=10)

  TSteroCalibCallbackFunctor callback; //!< If set to !=nullptr, this function
                                       //!will be called within each Lev-Marq.
                                       //!iteration (don't do heavy stuff here
                                       //!since performance will degrade)
  void *callback_user_param; //!< If using a callback function, you can use this
                             //!to pass custom data to your callback.
...
struct VISION_IMPEXP TStereoCalibResults {
  ...
  mrpt::utils::TStereoCamera
      cam_params; //!< Recovered parameters of the stereo camera
  mrpt::poses::CPose3D right2left_camera_pose; //!< The pose of the left camera
                                               //!as seen from the right camera
  ...
  std::vector<bool> image_pair_was_used; //!< true if a checkerboard was
                                         //!correctly detected in both
                                         //!left/right images. false if it
                                         //!wasn't, so the image pair didn't
                                         //!make it to the optimization.

  double final_rmse;  //!< Final reprojection square Root Mean Square Error (in
                      //!pixels).
  size_t final_iters; //!< Final number of optimization iterations executed.
  size_t final_number_good_image_pairs; //!< Number of image pairs in which
                                        //!valid checkerboards were correctly
                                        //!detected.
 ...
 * href="http://www.mrpt.org/Application:kinect-calibrate" >kinect-calibrate</a>
