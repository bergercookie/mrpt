...
/** The central class from which images can be analyzed in search of different
  ...
  *		- BCD (Binary Corner Detector): A detector (no descriptor vector)
  *(Not implemented yet).
  ...
  *		- The FASTER (9,10,12) detectors (Edward Rosten's libcvd
  *implementation optimized for SSE2).
  *
  ...
  *		- SURF descriptor (OpenCV's implementation - Requires OpenCV 1.1.0
  *from SVN or later).
  *		- Intensity-domain spin images (SpinImage): Creates a vector
  *descriptor with the 2D histogram as a single row.
  ...
  *		- A log-polar image patch (Log-polar images): The matrix descriptor
  *is the 2D log-polar image centered at the interest point.
  *
  *
...
public:
  ...
  struct VISION_IMPEXP TOptions : public utils::CLoadableOptions {
    ...
    */
    ...
    /** Whether to use a mask for determining the regions where not to look for
     * keypoints (default=false).
      */
    bool useMask;

    /** Whether to add the found features to the input feature list or clear it
     * before adding them (default=false).
      */
    bool addNewFeatures;

    /** Indicates if subpixel accuracy is desired for the extracted points (only
     * applicable to KLT and Harris features)
      */
    bool FIND_SUBPIXEL;
    ...
    struct VISION_IMPEXP TKLTOptions {
      ...
      float threshold;    // (default=0.1) for rejecting weak local maxima (with
                          // min_eig < threshold*max(eig_image))
      float min_distance; // minimum distance between features
      bool tile_image; // splits the image into 8 tiles and search for the best
                       // points in all of them (distribute the features over
                       // all the image)
    } KLTOptions;

    /** Harris Options */
    struct VISION_IMPEXP THarrisOptions {
      float threshold; // (default=0.005) for rejecting weak local maxima (with
                       // min_eig < threshold*max(eig_image))
    ...
    struct VISION_IMPEXP TFASTOptions {
      ...
      float min_distance; //!< (default=5) minimum distance between features (in
                          //!pixels)
      bool nonmax_suppression; //!< Default = true
      bool use_KLT_response;   //!< (default=false) If true, use
                             //!CImage::KLT_response to compute the response at
                             //!each point instead of the FAST "standard
                             //!response".
    } FASTOptions;
    ...
    struct VISION_IMPEXP TORBOptions {
      TORBOptions()
          : n_levels(8), min_distance(0), scale_factor(1.2f),
            extract_patch(false) {}
    ...
    struct VISION_IMPEXP TSIFTOptions {
      ...
      TSIFTImplementation implementation; //!< Default: Hess (OpenCV should be
                                          //!preferred, but its nonfree module
    ...
    struct VISION_IMPEXP TSURFOptions {
      TSURFOptions()
          : rotation_invariant(true), hessianThreshold(600), nOctaves(2),
            nLayersPerOctave(4) {}

      /** SURF Options
        */
      bool rotation_invariant; //!< Compute the rotation invariant SURF
                               //!(dim=128) if set to true (default), or the
                               //!smaller uSURF otherwise (dim=64)
    ...
    struct VISION_IMPEXP TSpinImagesOptions {
      /** SpinImages Options
        */
      unsigned int hist_size_intensity; //!< Number of bins in the "intensity"
                                        //!axis of the 2D histogram
                                        //!(default=10).
      unsigned int hist_size_distance;  //!< Number of bins in the "distance"
                                       //!axis of the 2D histogram (default=10).
      float std_dist; //!< Standard deviation in "distance", used for the "soft
                      //!histogram" (default=0.4 pixels)
      float std_intensity; //!< Standard deviation in "intensity", used for the
                           //!"soft histogram" (default=20 units [0,255])
      unsigned int radius; //!< Maximum radius of the area of which the
                           //!histogram is built, in pixel units (default=20
                           //!pixels)
    } SpinImagesOptions;
    ...
    struct VISION_IMPEXP TPolarImagesOptions {
      unsigned int bins_angle; //!< Number of bins in the "angular" axis of the
                               //!polar image (default=8).
      unsigned int bins_distance; //!< Number of bins in the "distance" axis of
                                  //!the polar image (default=6).
      unsigned int radius; //!< Maximum radius of the area of which the polar
                           //!image is built, in pixel units (default=20 pixels)
    } PolarImagesOptions;

    /** LogPolarImagesOptions Options
      */
    struct VISION_IMPEXP TLogPolarImagesOptions {
      unsigned int radius; //!< Maximum radius of the area of which the log
                           //!polar image is built, in pixel units (default=30
                           //!pixels)
      unsigned int num_angles; //!< (default=16) Log-Polar image patch will have
                               //!dimensions WxH, with:  W=num_angles,  H=
                               //!rho_scale * log(radius)
      double rho_scale;        //!< (default=5) Log-Polar image patch will have
                        //!dimensions WxH, with:  W=num_angles,  H= rho_scale *
                        //!log(radius)
  ...
  */
  void detectFeatures(const mrpt::utils::CImage &img, CFeatureList &feats,
  ...
  * \param inout_features (input/output) The list of features whose descriptors
  ...
  *  \endcode
  *
  * \note The SIFT descriptors for already located features can only be computed
  * through the Hess and
  ...
  */
  void computeDescriptors(const mrpt::utils::CImage &in_img,
   ...
   * fields are left <b>uninitialized</b> and their content is <b>undefined</b>.
   ...
   * adding the newly detected feats.
    *
    * If a valid pointer is provided for \a out_feats_index_by_row, upon return
   * you will find a vector with
   ...
   * disregarding the value of \a octave).
    ...
    *  the first feature that falls in that line of the image. This index can be
   * used to fasten looking for correspondences.
    *
    * \ingroup mrptvision_features
    */
  static void detectFeatures_SSE2_FASTER9(
      const mrpt::utils::CImage &img, TSimpleFeatureList &corners,
      const int threshold = 20, bool append_to_list = false, uint8_t octave = 0,
...
private:
  ...
  * \param in_img (input) The image from where to compute the descriptors.
  * \param in_features (input/output) The list of features whose descriptors are
  ...
  *
  * \note Additional parameters from
  * CFeatureExtraction::TOptions::LogPolarImagesOptions are used in this method.
  */
  void
  internal_computeLogPolarImageDescriptors(const mrpt::utils::CImage &in_img,
                                           CFeatureList &in_features) const;
  ...
  * authomatic.
  */
  void extractFeaturesKLT(const mrpt::utils::CImage &img, CFeatureList &feats,
                          unsigned int init_ID = 0,
  ...
  */
  void extractFeaturesBCD(const mrpt::utils::CImage &img, CFeatureList &feats,
                          unsigned int init_ID = 0,
                          unsigned int nDesiredFeatures = 0,
                          const TImageROI &ROI = TImageROI()) const;
  ...
  * \param ROI (op. input) Region of Interest. Default: All the image.
  */
  void extractFeaturesSIFT(const mrpt::utils::CImage &img, CFeatureList &feats,
                           unsigned int init_ID = 0,
                           unsigned int nDesiredFeatures = 0,
                           const TImageROI &ROI = TImageROI()) const;
  ...
  * authomatic.
  */
  void extractFeaturesORB(const mrpt::utils::CImage &img, CFeatureList &feats,
                          const unsigned int init_ID = 0,
                          const unsigned int nDesiredFeatures = 0,
                          const TImageROI &ROI = TImageROI()) const;
  ...
  * authomatic.
  */
  void extractFeaturesSURF(const mrpt::utils::CImage &img, CFeatureList &feats,
                           unsigned int init_ID = 0,
                           unsigned int nDesiredFeatures = 0,
                           const TImageROI &ROI = TImageROI()) const;
  ...
  * authomatic.
  */
  void extractFeaturesFAST(const mrpt::utils::CImage &img, CFeatureList &feats,
                           unsigned int init_ID = 0,
                           unsigned int nDesiredFeatures = 0,
  ...
  void extractFeaturesFASTER_N(const int N, const mrpt::utils::CImage &img,
                               CFeatureList &feats, unsigned int init_ID = 0,
                               unsigned int nDesiredFeatures = 0,
                               const TImageROI &ROI = TImageROI()) const;
  ...
  */
  void *my_scale_space_extrema(CFeatureList &featList, void *dog_pyr, int octvs,
                               int intvls, double contr_thr, int curv_thr,
                               void *storage) const;
  ...
  */
  ...
  /** Gets the number of times that a point in the image is higher or lower than
  * the surroundings in the image-scale space
  * \param dog_pyr Pyramid of images.
  * \param octvs Number of considered octaves.
  * \param intvls Number of intervales in octaves.
  ...
  */
  void getTimesExtrema(void *dog_pyr, int octvs, int intvls, float row,
                       float col, unsigned int &nMin, unsigned int &nMax) const;
  ...
  * \param col The column of the feature in the original image.
  */
  double getLaplacianValue(void *dog_pyr, int octvs, int intvls, float row,
                           float col) const;
  ...
  */
  void insertCvSeqInCFeatureList(void *features, CFeatureList &list,
                                 unsigned int init_ID = 0) const;
  ...
  * ROI -> not used).
  */
  void convertCvSeqInCFeatureList(void *features, CFeatureList &list,
                                  unsigned int init_ID = 0,
                                  const TImageROI &ROI = TImageROI()) const;
