 ...
 * integers or floats.
  ...
  */
template <typename PIXEL_COORD_TYPE> struct TSimpleFeature_templ {
  typedef PIXEL_COORD_TYPE pixel_coords_t; //!< The type of \a pt
  typedef typename PIXEL_COORD_TYPE::pixel_coord_t
      pixel_coord_t; //!< The type of pt.x and pt.y
  ...
  float response; //!< A measure of the "goodness" of the feature (typically,
                  //!the KLT_response value)
  uint8_t octave; //!< The image octave the image was found in: 0=original
                  //!image, 1=1/2 image, 2=1/4 image, etc.
  ...
  explicit TSimpleFeature_templ(const OTHER_TSIMPLEFEATURE &o)
      : pt(o.pt.x, o.pt.y), ID(o.ID), track_status(o.track_status),
        response(o.response), octave(o.octave), user_flags(o.user_flags) {}
};
 ...
 * TSimpleFeaturefList
  */
template <typename FEATURE> struct TSimpleFeatureList_templ {
public:
  ...
  TFeatureID getMaxID() const {
    ...
    for (; N; --N)
      mrpt::utils::keep_max(maxID, m_feats[N].ID);
    return maxID;
  }
   ...
   * doesn't fill this out, it will remain empty and useless.
    *  \note FASTER detectors do fill this out. In general, a feature list that
   * dynamically changes will not use this LUT.
    */
  ...
  /** @name getFeature*() methods for template-based access to feature list
      @{ */
  inline typename TSimpleFeatureTraits<FEATURE>::coord_t
  ...
  inline void setFeatureX(size_t i,
                          typename TSimpleFeatureTraits<FEATURE>::coord_t x) {
    m_feats[i].pt.x = x;
  }
  inline void setFeatureY(size_t i,
...
private:
  ...
  std::vector<size_t> m_first_index_per_row; //!< A LUT of the first feature
                                             //!index per row, to efficiently
                                             //!look for neighbors, etc.
