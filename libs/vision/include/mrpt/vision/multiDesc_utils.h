...
void VISION_IMPEXP saveQTableToFile(const TQuantizationTable &qTable,
                                    const std::string &filename);

void VISION_IMPEXP insertHashCoeffs(const CFeature::Ptr &feat,
                                    TQuantizationTable &qTable);

TMultiResMatchingOutput VISION_IMPEXP
relocalizeMultiDesc(const mrpt::utils::CImage &image, CFeatureList &baseList,
...
void VISION_IMPEXP updateBaseList(CFeatureList &baseList,
...
/** Computes the gradient of certain pixel within the image.
        ...
        * \return True if the gradient could be computed and False if the pixel
 * is located outside the image or at its border (where the gradient cannot be
 * computed)
        */
bool VISION_IMPEXP computeGradient(const mrpt::utils::CImage &image,
 ...
 * computing the orientation histogram.
        * \param orientations [OUT]   A vector containing the main orientations
 * of the image point.
        * \param sigma        [IN]    The sigma value of the Gaussian kernel
 * used to smooth the orientation histogram (typically 7.5 px).
        */
bool VISION_IMPEXP computeMainOrientations(const mrpt::utils::CImage &image,
 ...
 * neighbourhood into the histogram of orientations. This value can
        ...
        * \param d    [IN]        The number of row (and column) bins used in
 * the histogram (typically 4).
        * \param n    [IN]        The number of orienation bins used in the
 * histogram (typically 8).
        */
void VISION_IMPEXP interpolateHistEntry(std::vector<double> &hist,
                                        const double &cbin, const double &rbin,
                                        const double &obin, const double &mag,
 ...
 * descriptor.
        * \param hashCoeffs   [OUT]   A vector containing the computed
 * coefficients for the HASH table used in relocalization.
 ...
 * descriptors
        * at a set of different scales. The second list doesn't need to contain
 * such information because it will be computed if necessary according to the
        * the fulfillment of some restriction regarding the matching process.
 * This function will try to find the best matches within list2 corresponding to
 * the features
 ...
 * necessary.
        * \param output           [OUT]       The output structure with the
 * matching information.
        * \param matchOpts        [IN]        The options structure for the
 * matching process.
 ...
 * TMultiResMatchingOutput
        */
TMultiResMatchingOutput VISION_IMPEXP
matchMultiResolutionFeatures(const CFeatureList &list1, CFeatureList &list2,
 ...
 * This is performed for both the "left" and "right" lists
        * The first matched list is taken as a base, i.e. its features must
 * contain multi-resolution descriptors
        * at a set of different scales. The second list doesn't need to contain
 * such information because it will be computed if necessary according to the
        * the fulfillment of some restriction regarding the matching process.
 * This function will try to find the best matches within list2 corresponding to
 ...
 * position in a new image. This
        * is called when we have found a match between a feature and itself in a
 * new frame but it has been found in
 ...
 * descriptors for that feature.
        ...
        * \param outputFeat   [OUT]   The base feature (detected in the base
 * frame).
        * \param lowerScales  [IN]    If we should find descriptors for lower
 * scales or for higher ones.
        * \param opts         [IN]    The options for computing the new
 * descriptors.
        */
int VISION_IMPEXP computeMoreDescriptors(const mrpt::utils::CImage &image,
 ...
 * between multi-resolution features.
        ...
        * \param feat1        [IN]    The base feature which MUST contain a set
 * of different scales.
        * \param feat2        [IN]    The other feature which must be computed
 * at base scale (1.0).
 ...
 * feat1->multiScale.size()-1]) where to look.
        */
void VISION_IMPEXP setProperScales(const CFeature::Ptr &feat1,
 ...
 * features
        ...
        * \param matchedFeats [IN/OUT]    The list of matched features. They
 * will be updated with the multi-scales, multi-orientations, multi-descriptors
 * and depth information.
        * \param opts         [IN]        The options structure for the
 * descriptor computation process.
        * \sa TMultiResDescOptions
*/
...
/** Computes the multi-resolution SIFT-like descriptor of a features
        ...
        * \param feat         [IN/OUT]    The feature. It will be updated with
 * the multi-scales, multi-orientations, multi-descriptors
        * \param opts         [IN]        The options structure for the
 * descriptor computation process.
        * \sa TMultiResDescOptions
*/
 ...
 * computation process.
        * \sa TMultiResDescOptions
        */
void VISION_IMPEXP computeMultiOrientations(const mrpt::utils::CImage &image,
                                            CFeatureList &list,
                                            const TMultiResDescOptions &opts);
