...
public:
  ...
  uint16_t patchSize; //!< Size of the patch (patchSize x patchSize) (it must be
                      //!an odd number)
  ...
  uint8_t user_flags; //!< A field for any other flags needed by the user (this
                      //!has not a predefined meaning)
  uint16_t nTimesSeen; //!< Number of frames it has been seen in a sequence of
                       //!images.
  uint16_t nTimesNotSeen; //!< Number of frames it has not been seen in a
                          //!sequence of images.
  uint16_t
      nTimesLastSeen; //!< Number of frames since it was seen for the last time.

  double depth; //!< The estimated depth in 3D of this feature wrt the camera in
                //!the current frame
  ...
  std::deque<std::vector<double>> multiOrientations; //!< A vector of main
                                                     //!orientations (there is a
                                                     //!vector of orientations
                                                     //!for each scale)
  std::deque<std::vector<std::vector<int32_t>>> multiHashCoeffs; //!< A set of
                                                                 //!vectors
                                                                 //!containing
                                                                 //!the
                                                                 //!coefficients
                                                                 ...
                                                                 //!descriptors
  bool
  isPointFeature() const; //!< Return false only for Blob detectors (SIFT, SURF)
  ...
  struct VISION_IMPEXP TDescriptors {
    ...
    uint16_t SpinImg_range_rows; //!< The number of rows (corresponding to range
                                 //!bins in the 2D histogram) of the original
                                 //!matrix from which SpinImg was extracted as a
                                 //!vector.
    mrpt::math::CMatrix
        PolarImg; //!< A polar image centered at the interest point
    mrpt::math::CMatrix
        LogPolarImg; //!< A log-polar image centered at the interest point
    bool polarImgsNoRotation; //!< If set to true (manually, default=false) the
                              //!call to "descriptorDistanceTo" will not
                              //!consider all the rotations between polar image
                              //!descriptors (PolarImg, LogPolarImg)
    std::deque<std::vector<std::vector<int32_t>>>
        multiSIFTDescriptors; //!< A set of SIFT-like descriptors for each
                              //!orientation and scale of the multiResolution
                              //!feature (there is a vector of descriptors for
                              //!each scale)
    ...
    bool hasDescriptorMultiSIFT() const {
      return (multiSIFTDescriptors.size() > 0 &&
  ...
  /** Computes the normalized cross-correlation between the patches of this and
   * another feature (normalized in the range [0,1], such as 0=best, 1=worst).
    *  \note If this or the other features does not have patches or they are of
   * different sizes, an exception will be raised.
    * \sa descriptorDistanceTo
    */
  float patchCorrelationTo(const CFeature &oFeature) const;

  /** Computes the Euclidean Distance between this feature's and other feature's
   * descriptors, using the given descriptor or the first present one.
   ...
   * present in one of the features, an exception will be raised.
    ...
    */
  float descriptorDistanceTo(const CFeature &oFeature,
   ...
   */
  float descriptorSIFTDistanceTo(const CFeature &oFeature,
                                 bool normalize_distances = true) const;

  /** Computes the Euclidean Distance between "this" and the "other" descriptors
   */
  float descriptorSURFDistanceTo(const CFeature &oFeature,
                                 bool normalize_distances = true) const;

  /** Computes the Euclidean Distance between "this" and the "other" descriptors
   */
  float descriptorSpinImgDistanceTo(const CFeature &oFeature,
                                    bool normalize_distances = true) const;
   ...
   * distance is found.
    ...
    */
  float descriptorPolarImgDistanceTo(const CFeature &oFeature,
                                     float &minDistAngle,
                                     bool normalize_distances = true) const;
   ...
   * distance is found.
    * \return The distance for the best orientation (minimum distance).
    */
  float descriptorLogPolarImgDistanceTo(const CFeature &oFeature,
                                        float &minDistAngle,
                                        bool normalize_distances = true) const;
...
*|---------------------- descriptors ------------------------|"
*    "%% with:\n"
*    "%%  TYPE  : The used detector: 0:KLT, 1: Harris, 2: BCD, 3: SIFT, 4: SURF,
*5: Beacon, 6: FAST, 7: ORB\n"
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CFeature,
...
public:
  ...
  void getByMultiIDs(const std::vector<TFeatureID> &IDs,
  ...
  *   \param max_dist [IN/OUT] At input: The maximum distance to search for. At
  * output: The actual distance to the feature.
...
public:
  ...
  /** Returns a smart pointer to the feature with the provided ID or a empty one
   * if not found */
  CFeature::Ptr getByID(const TFeatureID &ID, const TListIdx &idx);

  /** Returns the maximum ID of the features in the list. If the max ID has been
     already set up, this method just returns it.
      Otherwise, this method finds, stores and returns it.*/
  void getMaxID(const TListIdx &idx, TFeatureID &firstListID,
