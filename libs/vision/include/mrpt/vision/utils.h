...
/** Builds the intrinsic parameters matrix A from parameters:
  ...
  */
mrpt::math::CMatrixDouble33 VISION_IMPEXP
buildIntrinsicParamsMatrix(const double focalLengthX, const double focalLengthY,
 ...
 * valid matched are within this threshold.
  */
void VISION_IMPEXP rowChecking(CFeatureList &leftList, CFeatureList &rightList,
                               float threshold = 1.0);
...
/** Transform two clouds of 3D points into a matched list of points
  ...
  */
void VISION_IMPEXP
cloudsToMatchedList(const mrpt::obs::CObservationVisualLandmarks &cloud1,
 ...
 * points.
  ...
  */
float VISION_IMPEXP computeMainOrientation(const mrpt::utils::CImage &image,
 ...
 * to zero and its standard deviation to unit.
  ...
  */
void VISION_IMPEXP normalizeImage(const mrpt::utils::CImage &image,
 ...
 * type.
  ...
  */
size_t VISION_IMPEXP
matchFeatures(const CFeatureList &list1, const CFeatureList &list2,
 ...
 * patches. Both patches must have the same size.
  ...
  */
void VISION_IMPEXP generateMask(const CMatchedFeatureList &mList,
 ...
 * patches. Both patches must have the same size.
  ...
  */
double VISION_IMPEXP computeSAD(const mrpt::utils::CImage &patch1,
 ...
 * features.
  */
void VISION_IMPEXP addFeaturesToImage(const mrpt::utils::CImage &inImg,
                                      const CFeatureList &theList,
                                      mrpt::utils::CImage &outImg);

void VISION_IMPEXP
projectMatchedFeatures(const CMatchedFeatureList &matches,
 ...
 * from the lists.
  ...
  */
void VISION_IMPEXP projectMatchedFeatures(CMatchedFeatureList &mfList,
...
/** Project a pair of feature lists into the 3D space, using the provided
  ...
  * \sa TStereoSystemParams, CLandmarksMap
  */
void VISION_IMPEXP projectMatchedFeatures(CFeatureList &leftList,
...
/** Converts a stereo images observation into a bearing and range observation.
        ...
        \param sg		[IN]	The sigma of the row, col, and
   disparity variables involved in the feature detection.
        \param outObs	[OUT]	The output bearing and range
   observation (including covariances).
*/
...
/** Converts a matched feature list into a bearing and range observation (some
   of the stereo camera system must be provided).
        \param inMatches		[IN]	The input list of matched
   features.
        \param intrinsicParams	[IN]	The intrisic params of the
   reference (left) camera of the stereo system.
        \param baseline			[IN]	The distance among
   the X axis of the right camera wrt the reference (left) camera.
        \param sg				[IN]	The sigma of the
   row, col, and disparity variables involved in the feature detection.
        \param outObs			[OUT]	The output bearing and
   range observation (including covariances).
*/
void VISION_IMPEXP StereoObs2BRObs(
...
*/
void VISION_IMPEXP
StereoObs2BRObs(const mrpt::obs::CObservationVisualLandmarks &inObs,
                mrpt::obs::CObservationBearingRange &outObs);
...
cameras and the relative pose of the second one wrt the first one.
        ...
        \param rightCameraPose      [IN]    The change in pose of the second
camera wrt the first one
        \param outMap1x,outMap1y    [OUT]   The x-and-y maps corresponding to
cam1 (should be converted to *cv::Mat)
        \param outMap2x,outMap2y    [OUT]   The x-and-y maps corresponding to
cam2 (should be converted to *cv::Mat)
