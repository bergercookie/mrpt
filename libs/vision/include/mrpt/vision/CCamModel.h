...
public:
  ...
  void jacob_undistor_fm(const mrpt::utils::TPixelCoordf &uvd,
                         math::CMatrixDouble &J_undist);

  /** Calculate the image coordinates undistorted
   */
  void jacob_undistor(const mrpt::utils::TPixelCoordf &p,
                      mrpt::math::CMatrixDouble &J_undist);

  /**	Return the pixel position distorted by the camera
   */
  void distort_a_point(const mrpt::utils::TPixelCoordf &p,
   ...
   *(undistorted)
   */
  void undistort_point(const mrpt::utils::TPixelCoordf &p,
...
*/
  void project_3D_point(const mrpt::math::TPoint3D &p3D,
   ...
   * \sa project_3D_point
   * \note Of course, there is a depth ambiguity, so the returned 3D point must
   * be considered a direction from the camera focus, or a vector, rather than a
   * meaninful physical point.
*/
  void unproject_3D_point(const mrpt::utils::TPixelCoordf &distorted_p,
                          mrpt::math::TPoint3D &p3D) const;

  /** Jacobian of the projection of 3D points (with distortion), as done in
  project_3D_point \f$ \frac{\partial h}{\partial y} \f$, evaluated at the point
  p3D (read below the full explanation)

  We define \f$ h = (h_x ~ h_y) \f$ as the projected point in pixels (origin at
  the top-left corner),
  ...
  \f[
  \frac{\partial h}{\partial y} =  \frac{\partial h}{\partial u} \frac{\partial
  u}{\partial y}
  \f]
  ...
  where \f$ f_x, f_y \f$ is the focal length in units of pixel sizes in x and y,
  respectively.
  And, if we define:
  ...
  \sa project_3D_point
  */
  void jacobian_project_with_distortion(const mrpt::math::TPoint3D &p3D,
                                        math::CMatrixDouble &dh_dy) const;
  ...
  \sa unproject_3D_point
  */
  void jacobian_unproject_with_distortion(const mrpt::utils::TPixelCoordf &p,
                                          math::CMatrixDouble &dy_dh) const;
  ...
  void getTemporaryVariablesForTransform(const POINT &p,
                                         CameraTempVariables<T> &v) const {
    v.x_ = p[1] / p[0];
    v.y_ = p[2] / p[0];
    v.x_2 = square(v.x_);
    v.y_2 = square(v.y_);
  ...
  inline void getFullProjectionT(const CameraTempVariables<T> &tmp,
                                 PIXEL &pOut) const {
  ...
  template <typename T, typename POINT, typename MATRIX>
  void getFullJacobianT(const POINT &pIn, const CameraTempVariables<T> &tmp,
                        MATRIX &mOut) const {
    T x_ = 1 / pIn[0];
    T x_2 = square(x_);
    // First two jacobians...
...
public:
  ...
  void getFullInverseModelWithJacobian(const POINTIN &pIn, POINTOUT &pOut,
                                       MAT22 &jOut) const {
    // Temporary variables (well, there are some more, but these are the basics)
    // WARNING!: this shortcut to avoid repeated initialization makes the method
    // somewhat
    ...
    static mrpt::math::CMatrixFixedNumeric<double, 2, 3>
        J4; // This is not initialized in a special way, although declaring it
    mrpt::math::CArrayNumeric<double, 4> tmp1;
    mrpt::math::CArrayNumeric<double, 2> tmp2; // This would be a
                                               // CArray<double,3>, but to avoid
                                               // copying, we let "R2" lie in
                                               // tmp1.
    // Camera Parameters
    ...
    // Third step: radial distortion. Really simple, since most work has been
    ...
    // As fast as possible, and without more temporaries, let the jacobian be
    // J4*J3*J2*J1;
    jOut.multiply_ABC(J4, J3, J2); // Note that using the other order is not
                                   // possible due to matrix sizes (jOut may,
                                   // and most probably will, be fixed).
