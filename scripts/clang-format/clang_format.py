...
logger = logging.getLogger("ClangFormatter")
...
CLANG_FORMAT_SOURCE_TAR_BASE = \
    string.Template("clang+llvm-$version-$tar_path/bin/" +
    ...
    def __init__(self, clang_path, cache_dir):
        ...
        if clang_path is not None:
            if os.path.isfile(clang_path):
                ...
                logger.warn("Could not find clang-format %s" % (clang_path))
        ...
        if self.clang_path is None:
            ...
            programs = [CLANG_FORMAT_PROGNAME + "-" +
                        CLANG_FORMAT_VERSION,
                        CLANG_FORMAT_PROGNAME + "-" +
        ...
        if self.clang_path is None:
            ...
            self.clang_path = os.clang_path.join(cache_dir,
                                                 CLANG_FORMAT_PROGNAME + "-" +
                                                 CLANG_FORMAT_VERSION +
                                                 clang_format_progname_ext)
...
def get_repos():
    """Return a Repo object of the MRPT git repo."""
    # TODO - fix this
    return [Repo(os.path.curdir)]
...
class Repo(object):
    ...
    def _git_ls_files(self, cmd):
        ...
        file_list = [line.rstrip()
                     ...
                     if (line.startswith("libs") or
                         line.startswith("apps") or
                         line.startswith("samples") or
                         line.startswith("python") and
                         "otherlibs" not in line)]
...
def get_files_to_check_working_tree():
    ...
    logging.info("Files that clang-format will operate on:\n%s"
...
def get_files_to_check_from_patch(patches):
    """Take a patch file generated by git diff, and scan the patch for a list
    of files to check.
    """
    candidates = []
...
def _get_build_dir():
    """Get the location of a build directory in case we need to
...
def _lint_files(clang_format, files):
    ...
    lint_clean = parallel_process([os.path.abspath(f)
                                   for f in files], clang_format.lint)
...
def _format_files(clang_format, files):
    ...
    format_clean = parallel_process([os.path.abspath(f)
                                     for f in files], clang_format.format)
...
def reformat_branch(clang_format, commit_prior_to_reformat,
    ...
    # We assume the target branch is master, it could be a different branch if
    # needed for testing
    ...
    if repo.does_branch_exist(new_branch):
        raise ValueError("The branch '%s' already exists. Please delete the "
                         "branch '%s', or rename the current branch."
                         % (new_branch, new_branch))

    commits = get_list_from_lines(repo.log(
    ...
    # Go through all the commits the user made on the local branch and migrate
    # to a new branch that is based on post_reformat commits instead
    for commit_hash in commits:
        ...
        for commit_file in commit_files:
            ...
            if not os.path.exists(commit_file):
                logger.info("Skipping file '%s' since it has been deleted in commit '%s'" % (
        ...
        # Note: we will not name as a branch until we are done with all commits
        # on the local branch
        repo.checkout(["--quiet", previous_commit_base])

        # Copy each file from the reformatted commit on top of the post
        # reformat
        diff_files = get_list_from_lines(repo.diff(
...
def usage():
    """Print usage."""
    logger.info("clang-format.py supports 5 commands "
                "[ lint, lint-all, lint-patch, format, reformat-branch].")
...
def main():
    ...
    For now you have to run the script from the toplevel of your project if you
    want it to affect the entire codebase.
    """
    logger.info("Initializing script...")


    parser = OptionParser()
    parser.add_option("-c", "--clang-format",
                      type="string", dest="clang_format")
    ...
    if len(args) > 1:
        ...
        if command == "lint":
            ...
            if len(args) < 3:
                logger.error("reformat-branch takes two parameters: "
