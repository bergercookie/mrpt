...
TCLAP::CmdLine cmd("velodyne-view", ' ',
                   mrpt::system::MRPT_getVersion().c_str());

TCLAP::ValueArg<std::string>
    arg_out_rawlog("o", "out-rawlog", "If set, grab dataset in rawlog format",
                   false, "", "out.rawlog", cmd);
...
TCLAP::SwitchArg arg_nologo("n", "nologo", "Skip the logo at startup", cmd,
                            false);
TCLAP::SwitchArg arg_verbose("v", "verbose", "Verbose debug output", cmd,
                             false);
  ...
  try {
    ...
    if (arg_out_rawlog.isSet()) {
      if (!f_out_rawlog.open(arg_out_rawlog.getValue()))
        THROW_EXCEPTION_FMT("Error creating output rawlog file: %s",
    ...
    while (!hard_error && !p.quit) {
      ...
      if (obs_gps)
        std::atomic_store(&p.new_obs_gps, obs_gps);
  ...
  while (win3D.isOpen() && !thrPar.quit) {
    ...
    if (possiblyNewObsGps &&
      ...
      if (last_obs_gps->has_RMC_datum) {
        rmc_datum =
            mrpt::format("Lon=%.09f deg  Lat=%.09f deg  Valid?: '%c'\n",
      ...
      win3D.addTextMessage(
          5, 40,
          format("POS. frame rx at %s, RMC=%s",
    ...
    if (possiblyNewObs && possiblyNewObs->timestamp != INVALID_TIMESTAMP &&
      ...
      if (!last_obs->scan_packets.empty()) {
        ...
        win3D.addTextMessage(
            5, 55,
            format("LIDAR scan rx at %s with %u packets",
      ...
      win3D.get3DSceneAndLock();
      win3D.addTextMessage(-150, -20, format("%.02f Hz", thrPar.Hz),
                           TColorf(1, 1, 1), 100,
