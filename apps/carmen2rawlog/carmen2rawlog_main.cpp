  ...
  try {
    ...
    while (carmen_log_parse_line(input_stream, importedObservations,
      ...
      for (size_t i = 0; i < importedObservations.size(); i++) {
        ...
        // by the way: if we have an "odometry" observation but it's not alone,
        // it's probably
      ...
      if (verbose && ++decimateUpdateConsole > 10) {
        ...
        cout << "\rProgress: [" << string(nBlocks, '#')
             ...
             << format("] %6.02f%% (%u objects)", progress_ratio * 100,
                       nSavedObs);
        cout.flush();
      }
    };
    ...
    if (!groundTruthPoses.empty()) {
      ...
      if (!gt_file.is_open())
        throw std::runtime_error(
            format("Couldn't open output file for ground truth: '%s'",
                   gt_filename.c_str()));
      ...
      for (map<TTimeStamp, TPose2D>::const_iterator it =
        ...
        gt_file << format("   %12.06f %9.03f %9.03f %9.04f\n",
                          ...
                          it->second.x, it->second.y, it->second.phi);
    }
