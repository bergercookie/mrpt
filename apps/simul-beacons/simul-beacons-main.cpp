...
int main(int argc, char **argv) {
  try {
    ...
    os::sprintf(auxStr, sizeof(auxStr), "%s/%s", outDir.c_str(),
                outFile.c_str());
    ...
    for (i = 0; i < nSteps; i++) {
      ...
      if (i >= stopSteps) {
        if (circularPath) {
          ...
          if ((i % squarePathLength) > (squarePathLength - 5))
            ...
            incPose = CPose2D(0.20f, 0, 0);
        }
      } else
        incPose = CPose2D(0, 0, 0);
      ...
      for (size_t q = 0; q < obs->sensedData.size(); q++) {
        if (randomGenerator.drawUniform(0.0f, 1.0f) < probability_corrupt) {
          obs->sensedData[q].sensedDistance += randomGenerator.drawUniform(
