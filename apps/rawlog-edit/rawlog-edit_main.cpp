...
typedef void (*TOperationFunctor)(mrpt::utils::CFileGZInputStream &in_rawlog,
                                  TCLAP::CmdLine &cmdline, bool verbose);
...
TCLAP::CmdLine cmd("rawlog-edit", ' ',
                   mrpt::format("%s - Sources timestamp: %s\n",
                                MRPT_getVersion().c_str(),
                                MRPT_getCompilationDate().c_str()));

TCLAP::ValueArg<std::string>
    arg_input_file("i", "input", "Input dataset (required) (*.rawlog)", true,
                   "", "dataset.rawlog", cmd);
TCLAP::ValueArg<std::string> arg_output_file("o", "output",
                                             "Output dataset (*.rawlog)", false,
                                             "", "dataset_out.rawlog", cmd);

TCLAP::ValueArg<std::string>
    arg_outdir("", "out-dir", "Output directory (used by some commands only)",
               false, ".", ".", cmd);

TCLAP::ValueArg<std::string> arg_external_img_extension("", "image-format",
                                                        "External image format",
                                                        false, "jpg",
                                                        "jpg,png,pgm,...", cmd);
TCLAP::SwitchArg arg_txt_externals("", "txt-externals",
                                   "When externalizing CObservation3DRangeScan "
                                   "objects, switched from binary files "
                                   "(default) to plain text.",
                                   cmd, false);
TCLAP::ValueArg<std::string> arg_img_size("", "image-size",
                                          "Resize output images", false, "",
                                          "COLSxROWS", cmd);
TCLAP::SwitchArg arg_rectify_centers("", "rectify-centers-coincide",
                                     "In stereo rectification, force that both "
                                     "image centers after coincide after "
                                     "rectifying.",
                                     cmd, false);

TCLAP::ValueArg<std::string> arg_out_text_file("", "text-file-output",
                                               "Output for a text file", false,
                                               "out.txt", "out.txt", cmd);

TCLAP::ValueArg<uint64_t> arg_from_index("", "from-index",
                                         "Starting index for --cut", false, 0,
                                         "N0", cmd);
TCLAP::ValueArg<uint64_t> arg_to_index("", "to-index", "End index for --cut",
                                       false, 0, "N1", cmd);

TCLAP::ValueArg<double> arg_from_time("", "from-time",
                                      "Starting time for --cut, as UNIX "
                                      "timestamp, optionally with fractions of "
                                      "seconds.",
                                      false, 0, "T0", cmd);
TCLAP::ValueArg<double> arg_to_time("", "to-time", "End time for --cut, as "
                                                   "UNIX timestamp, optionally "
                                                   "with fractions of seconds.",
                                    false, 0, "T1", cmd);

TCLAP::ValueArg<double> arg_odo_KL("", "odo-KL", "Constant from encoder ticks "
                                                 "to meters (left wheel), used "
                                                 "in --recalc-odometry.",
                                   false, 0, "KL", cmd);
TCLAP::ValueArg<double> arg_odo_KR("", "odo-KR", "Constant from encoder ticks "
                                                 "to meters (right wheel), "
...
TCLAP::SwitchArg arg_overwrite("w", "overwrite",
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    arg_ops.push_back(new TCLAP::SwitchArg(
        "", "externalize", "Op: convert to external storage.\n"
    ...
    arg_ops.push_back(new TCLAP::SwitchArg(
        "", "info", "Op: parse input file and dump information and statistics.",
        cmd, false));
    ...
    arg_ops.push_back(
        new TCLAP::SwitchArg("", "list-timestamps",
                             "Op: generates a list with all the observations' "
                             "timestamp, sensor label and C++ class name.\n"
                             "Optionally the output text file can be changed "
    ...
    arg_ops.push_back(new TCLAP::SwitchArg(
        ...
        "sensor label of GPS observations in the dataset. "
        "The generated .kml files will be saved in the same path than the "
        "input rawlog, with the same "
    ...
    arg_ops.push_back(
        new TCLAP::SwitchArg("", "export-gps-txt",
                             "Op: Export GPS GPGGA messages to TXT files.\n"
                             "Generates one .txt file for each different "
                             "sensor label of GPS observations in the dataset. "
                             "The generated .txt files will be saved in the "
                             "same path than the input rawlog, with the same "
                             "filename + each sensorLabel.",
                             cmd, false));
    ...
    arg_ops.push_back(new TCLAP::SwitchArg(
        ...
        "The generated .txt files will be saved in the same path than the "
        "input rawlog, with the same "
        "filename + each sensorLabel.",
        cmd, false));
    ...
    arg_ops.push_back(new TCLAP::SwitchArg(
        "", "recalc-odometry", "Op: Recomputes odometry increments from new "
    ...
    arg_ops.push_back(
        new TCLAP::SwitchArg("", "export-2d-scans-txt",
                             "Op: Export 2D scans to TXT files.\n"
                             "Generates two .txt files for each different "
                             "sensor label of 2D scan observations, one with "
    ...
    arg_ops.push_back(new TCLAP::SwitchArg(
        "", "generate-3d-pointclouds", "Op: (re)generate the 3D pointclouds "
                                       "within CObservation3DRangeScan objects "
                                       "that have range data.\n"
                                       "Requires: -o (or --output)\n",
        cmd, false));
    ...
    arg_ops.push_back(new TCLAP::SwitchArg(
        ...
        "for each sensor observation that can be converted into"
    ...
    arg_ops.push_back(new TCLAP::ValueArg<std::string>(
        "", "sensors-pose", "Op: batch change the poses of sensors from a "
                            "rawlog-grabber-like configuration file that "
                            "specifies the pose of sensors by their "
                            "sensorLabel names.\n"
                            "Requires: -o (or --output)\n",
        false, "", "file.ini", cmd));
    ...
    arg_ops.push_back(new TCLAP::ValueArg<std::string>(
        "", "camera-params", "Op: change the camera parameters of all "
                             "CObservationImage's with the given SENSOR_LABEL, "
                             "with new params loaded from the given file, "
                             "section '[CAMERA_PARAMS]' "
                             "for monocular cameras, or '[CAMERA_PARAMS_LEFT]' "
                             "and '[CAMERA_PARAMS_RIGHT]' for stereo.\n"
                             "Requires: -o (or --output)\n",
    ...
    arg_ops.push_back(new TCLAP::ValueArg<std::string>(
        ...
        "CObservationStereoImages with the given SENSOR_LABEL, using the "
        "camera parameters stored in the "
        "observations (which must be a valid calibration) and with the given "
        "alpha value. Alpha can be -1 for auto, or otherwise be in the range "
        "[0,1] (see OpenCV's docs for cvStereoRectify).\n"
        "Requires: -o (or --output)\n"
        "Optional: --image-format to set image format (default=jpg), \n"
        "          --image-size to resize output images (example: --image-size "
        "640x480) \n",
    ...
    if (!mrpt::system::fileExists(input_rawlog))
      throw runtime_error(
          format("Input file doesn't exist: '%s'", input_rawlog.c_str()));
    ...
    if (mrpt::system::directoryExists(CImage::IMAGES_PATH_BASE)) {
      ...
      VERBOSE_COUT << "Warning: No external storage directory was found (not "
                      "an issue if the rawlog does not contain delayed-load "
                      "images).\n";
    }
    ...
    // ------------------------------------
    ASSERTMSG_(ops_functors.find(selected_op) != ops_functors.end(),
...
TOutputRawlogCreator::TOutputRawlogCreator() {
  if (!arg_output_file.isSet())
    throw runtime_error("This operation requires an output file. Use '-o file' "
                        "or '--output file'.");
  ...
  if (fileExists(out_rawlog_filename) && !arg_overwrite.getValue())
    throw runtime_error(
        ...
        out_rawlog_filename +
        string("\n. Select a different output path, remove the file or force "
               "overwrite with '-w' or '--overwrite'."));

  if (!out_rawlog.open(out_rawlog_filename))
    throw runtime_error(string("*ABORTING*: Cannot open output file: ") +
                        out_rawlog_filename);
}
...
template <typename T>
bool getArgValue(TCLAP::CmdLine &cmdline, const std::string &arg_name,
                 T &out_val) {
  using namespace TCLAP;
...
template bool getArgValue<>(TCLAP::CmdLine &cmdline,
                            const std::string &arg_name, std::string &out_val);
template bool getArgValue<>(TCLAP::CmdLine &cmdline,
                            const std::string &arg_name, double &out_val);
template bool getArgValue<>(TCLAP::CmdLine &cmdline,
                            const std::string &arg_name, size_t &out_val);
template bool getArgValue<>(TCLAP::CmdLine &cmdline,
                            const std::string &arg_name, int &out_val);
