...
DECLARE_OP_FUNCTION(op_camera_params) {
  ...
  public:
    ...
    CRawlogProcessor_CamParams(CFileGZInputStream &in_rawlog,
      ...
      if (lstTokens.size() != 2)
        throw std::runtime_error("--camera-params op: argument must be in the "
                                 "format: --camera-params LABEL,file.ini");
      ...
      if (!fileExists(fil))
        throw std::runtime_error(
            string("--camera-params op: config file can't be open:") + fil);
      ...
      if (!sErrorCam.empty()) {
        ...
        try {
          ...
          throw std::runtime_error(
              string("--camera-params op: Error loading monocular camera "
                     "params:\n") +
              sErrorCam +
