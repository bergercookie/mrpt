...
DECLARE_OP_FUNCTION(op_remap_timestamps) {
  ...
  public:
    CRawlogProcessor_RemapTimestamps(CFileGZInputStream &in_rawlog,
  ...
  if (!getArgValue<string>(cmdline, "remap-timestamps", sAB_params) ||
      sAB_params.empty())
    throw std::runtime_error("remap-timestamps: This operation needs two "
                             "arguments in the format 'a;b'.");
