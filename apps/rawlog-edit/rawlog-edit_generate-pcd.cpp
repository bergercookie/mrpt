...
DECLARE_OP_FUNCTION(op_generate_pcd) {
  ...
  class CRawlogProcessor_GeneratePCD
  ...
  public:
    ...
    CRawlogProcessor_GeneratePCD(CFileGZInputStream &in_rawlog,
      ...
      if (!mrpt::system::directoryExists(m_outdir))
        throw std::runtime_error(
            string("ERROR: Output directory does not exist: ") + m_outdir);
    }
      ...
      if (IS_CLASS(obs, CObservation3DRangeScan)) {
        ...
        if (obs3D->hasPoints3D) {
          ...
          if (!map.savePCDFile(label_time, false /* not bin format*/))
            throw std::runtime_error(string("ERROR: While saving file: ") +
                                     label_time);
          entries_done++;
        }
