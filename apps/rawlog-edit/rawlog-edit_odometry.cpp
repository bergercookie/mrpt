		...
		bool processOneObservation(CObservation::Ptr& o)
		{
			...
			if (it == lstFiles.end())  // A new file for this sensorlabel??
			{
				...
				::fprintf(
					...
					"%18s %18s %18s "  // HAS, V,W
					"\n",
					"Time", "GLOBAL_ODO_X", "GLOBAL_ODO_Y",
					"GLOBAL_ODO_PHI_RAD", "HAS_ENCODERS", "LEFT_ENC_INCR_TICKS",
					"RIGHT_ENC_INCR_TICKS", "HAS_VELOCITIES",
					"LIN_SPEED_MetPerSec", "ANG_SPEED_RadPerSec");
			}
			else
				f_this = it->second;
