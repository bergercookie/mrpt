...
DECLARE_OP_FUNCTION(op_export_odometry_txt) {
  ...
  class CRawlogProcessor_ExportODO_TXT
  ...
  public:
    ...
    CRawlogProcessor_ExportODO_TXT(CFileGZInputStream &in_rawlog,
    ...
    bool processOneObservation(CObservation::Ptr &o) {
      ...
      if (it == lstFiles.end()) // A new file for this sensorlabel??
      {
        ...
        if (!f_this)
          THROW_EXCEPTION_FMT("Cannot open output file for write: %s",
                              fileName.c_str());
        ...
        ::fprintf(f_this,
                  ...
                  "%18s %18s %18s " // HAS, V,W
                  "\n",
                  "Time", "GLOBAL_ODO_X", "GLOBAL_ODO_Y", "GLOBAL_ODO_PHI_RAD",
                  "HAS_ENCODERS", "LEFT_ENC_INCR_TICKS", "RIGHT_ENC_INCR_TICKS",
                  "HAS_VELOCITIES", "LIN_SPEED_MetPerSec",
                  "ANG_SPEED_RadPerSec");
      } else
        f_this = it->second;

      // For each entry in this sequence: Compute the timestamp and save all 15
      // values:
      ...
      ::fprintf(f_this,
                ...
                "\n",
                sampleTime, obs->odometry.x(), obs->odometry.y(),
                obs->odometry.phi(),
                static_cast<int>(obs->hasEncodersInfo ? 1 : 0),
                static_cast<int>(obs->encoderLeftTicks),
                static_cast<int>(obs->encoderRightTicks),
                static_cast<int>(obs->hasVelocities ? 1 : 0),
                obs->velocityLocal.vx, obs->velocityLocal.omega);
      m_entriesSaved++;
      return true; // All ok
    }
    ...
    ~CRawlogProcessor_ExportODO_TXT() {
      ...
      // Save the joint file:
      // -------------------------
      VERBOSE_COUT << "Number of different odometry sensorLabels  : "
                   << lstFiles.size() << endl;
...
DECLARE_OP_FUNCTION(op_recalc_odometry) {
  ...
  public:
    ...
    CRawlogProcessor_RecalcODO(CFileGZInputStream &in_rawlog,
                               TCLAP::CmdLine &cmdline, bool verbose)
        : CRawlogProcessorOnEachObservation(in_rawlog, cmdline, verbose),
          m_odo_accum_valid(false), m_entriesSaved(0) {
    ...
    bool processOneObservation(CObservation::Ptr &o) {
      ...
      if (!obs->hasEncodersInfo)
        throw std::runtime_error("CObservationOdometry entry does not contain "
                                 "encoder info, cannot recalculate odometry!");
