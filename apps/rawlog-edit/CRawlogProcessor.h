...
public:
  ...
  // Ctor
  CRawlogProcessor(mrpt::utils::CFileGZInputStream &_in_rawlog,
                   TCLAP::CmdLine &_cmdline, bool _verbose)
      : m_in_rawlog(_in_rawlog), m_cmdline(_cmdline), verbose(_verbose),
        m_last_console_update(mrpt::system::now()), m_rawlogEntry(0) {
  ...
  void doProcessRawlog() {
    ...
    while (mrpt::obs::CRawlog::getActionObservationPairOrObservation(
      ...
      if (!process_ret) {
        ...
        std::cerr << "\nParsing stopped due to request from Rawlog filter "
                     "implementation.\n";
        break;
      }
    }; // end while
  ...
  virtual bool processOneEntry(mrpt::obs::CActionCollection::Ptr &actions,
                               mrpt::obs::CSensoryFrame::Ptr &SF,
...
public:
  CRawlogProcessorOnEachObservation(mrpt::utils::CFileGZInputStream &in_rawlog,
  ...
  virtual bool processOneEntry(mrpt::obs::CActionCollection::Ptr &actions,
                               ...
                               mrpt::obs::CObservation::Ptr &obs) {
    MRPT_UNUSED_PARAM(actions);
    // Process each observation individually, either from "obs" or each within a
    // "SF":
...
public:
  ...
  bool m_we_are_done_with_this_rawlog; //!< Set to true to indicate that we are
                                       //!sure we don't have to keep on reading.
  ...
  CRawlogProcessorFilterObservations(
      ...
      : CRawlogProcessorOnEachObservation(in_rawlog, cmdline, verbose),
        m_out_rawlog(out_rawlog), m_entries_removed(0), m_entries_parsed(0),
