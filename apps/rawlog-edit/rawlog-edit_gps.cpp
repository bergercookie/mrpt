...
DECLARE_OP_FUNCTION(op_export_gps_kml) {
  ...
  public:
    CRawlogProcessor_ExportGPS_KML(CFileGZInputStream &in_rawlog,
    ...
    void generate_KML() {
      ...
      static const char *COLOR_CODES[] = {"a000ffff", "a00000ff", "a0ff0000",
                                          "a0707070", "a0000000"};

      for (size_t i = 0; i < NCOLORS; i++) {
        f.printf("    <Style id=\"gpscolor%i\">\n"
                 "      <LineStyle>\n"
                 "        <color>%s</color>\n"
                 "        <width>%i</width>\n"
                 "      </LineStyle>\n"
        ...
        f.printf("    <Style id=\"gpscolor%i_thick\">\n"
                 "      <LineStyle>\n"
                 "        <color>%s</color>\n"
                 "        <width>%i</width>\n"
                 "      </LineStyle>\n"
      ...
      const string LineString_START =
          format("      <LineString>\n"
                 "        %s\n"
                 "       <coordinates> \n",
                 save_altitude ? "<altitudeMode>absolute</altitudeMode>" : "");
      const string LineString_END = "        </coordinates>\n"
                                    "      </LineString>\n";

      // For each sensor label:
      int color_idx = 0;
      for (map<string, TDataPerGPS>::const_iterator it = m_gps_paths.begin();
        ...
        f.printf("    <Placemark>\n"
                 ...
                 "      <description>%s: All received points (for all quality "
                 "levels)</description>\n"
        ...
        if (hasSomeRTK) {
          ...
          for (map<TTimeStamp, TGPSDataPoint>::const_iterator itP =
            ...
            if (last_valid.lat != 0 && last_valid.lon != 0) {
              ...
              const double dist =
                  6.371e6 * DEG2RAD(::hypot(last_valid.lon - d.lon,
                                            last_valid.lat - d.lat));

              // If the distance is above a threshold, finish the line and start
              // another one:
...
DECLARE_OP_FUNCTION(op_export_gps_txt) {
  ...
  protected:
    ...
    void doSaveJointFile(map<TTimeStamp, map<string, CPoint3D>> &lstXYZallGPS,
                         set<string> &lstAllGPSlabels,
      ...
      const string jointFilName =
          format("%s_JOINT%s_%s.txt", m_filPrefix.c_str(), gpsKindLabel,
                 joint_name.c_str());

      VERBOSE_COUT << "Writing joint GPS file: " << jointFilName << endl;

      MAT.saveToTextFile(jointFilName, MATRIX_FORMAT_ENG, false,
                         "% For N GPS sensors, each line has 1+3*N entries: \n"
                         "% timestamp (UNIX time_t with sec fractions) + N*[ "
                         "ENU_X ENU_Y ENU_Z ] \n"
                         "% "
                         "-----------------------------------------------------"
                         "-------------- \n");
      ...
      MAT_REF.saveToTextFile(
          format("%s_JOINTREF%s_%s.txt", m_filPrefix.c_str(), gpsKindLabel,
                 joint_name.c_str()),
  ...
  public:
    ...
    CRawlogProcessor_ExportGPS_TXT(CFileGZInputStream &in_rawlog,
                                   TCLAP::CmdLine &cmdline, bool verbose)
        : CRawlogProcessorOnEachObservation(in_rawlog, cmdline, verbose),
    ...
    bool processOneObservation(CObservation::Ptr &o) {
      ...
      if (obs->has_GGA_datum) {
        ...
        // If available, Cartessian X Y Z, VX VY VZ, as supplied by the GPS
        // itself:
        ...
        if (obs->has_PZS_datum) {
          const mrpt::obs::gnss::Message_TOPCON_PZS &pzs =
              obs->getMsgByClass<mrpt::obs::gnss::Message_TOPCON_PZS>();
        ...
        ::fprintf(
            ...
            DEG2RAD(gga.fields.longitude_degrees), gga.fields.altitude_meters,
            gga.fields.fix_quality, gga.fields.satellitesUsed,
            obs->has_RMC_datum
                ? DEG2RAD(
                      obs->getMsgByClass<mrpt::obs::gnss::Message_NMEA_RMC>()
                          .fields.speed_knots)
                : 0.0,
            obs->has_RMC_datum
                ? DEG2RAD(
            ...
            geo.x, geo.y, geo.z, cart_pos.x, cart_pos.y, cart_pos.z, cart_vel.x,
            cart_vel.y, cart_vel.z, cart_vel_local.x, cart_vel_local.y,
            cart_vel_local.z,
...
DECLARE_OP_FUNCTION(op_export_gps_all) {
  ...
  public:
    ...
    CRawlogProcessor_ExportGPS_ALL(CFileGZInputStream &in_rawlog,
                                   TCLAP::CmdLine &cmdline, bool verbose)
        : CRawlogProcessorOnEachObservation(in_rawlog, cmdline, verbose),
          m_GPS_entriesSaved(0) {
      getArgValue<string>(cmdline, "input", m_inFile);
    ...
    bool processOneObservation(CObservation::Ptr &o) {
      ...
      for (CObservationGPS::message_list_t::const_iterator it =
        ...
        ::fprintf(
            ...
            mrpt::system::timestampTotime_t(obs->originalReceivedTimestamp),
            buf.str().c_str());
        m_GPS_entriesSaved++;
