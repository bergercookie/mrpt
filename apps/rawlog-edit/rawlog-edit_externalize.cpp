...
DECLARE_OP_FUNCTION(op_externalize) {
  ...
  public:
    ...
    CRawlogProcessor_Externalize(CFileGZInputStream &in_rawlog,
      ...
      if (directoryExists(outDir))
        throw runtime_error(
            string("*ABORTING*: Output directory for images already exists: ") +
            outDir +
            string(
    ...
    bool processOneObservation(CObservation::Ptr &obs) {
      const string label_time = format("%s_%f", obs->sensorLabel.c_str(),
      ...
      if (IS_CLASS(obs, CObservationStereoImages)) {
        ...
        if (!obsIm->image.isExternallyStored()) {
          const string fileName =
              string("img_") + label_time + string(".") + imgFileExtension;
