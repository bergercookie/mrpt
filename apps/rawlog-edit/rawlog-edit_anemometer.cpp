...
DECLARE_OP_FUNCTION(op_export_anemometer_txt) {
  ...
  class CRawlogProcessor_ExportANEMOMETER_TXT
  ...
  public:
    ...
    CRawlogProcessor_ExportANEMOMETER_TXT(CFileGZInputStream &in_rawlog,
    ...
    bool processOneObservation(CObservation::Ptr &o) {
      ...
      ::fprintf(f_this,
                ...
                "\n",
                sampleTime, obs->speed, obs->direction);
      m_entriesSaved++;
      return true; // All ok
    }
    ...
    ~CRawlogProcessor_ExportANEMOMETER_TXT() {
      ...
      // Save the joint file:
      // -------------------------
      VERBOSE_COUT << "Number of different anemometer sensorLabels  : "
                   << lstFiles.size() << endl;
