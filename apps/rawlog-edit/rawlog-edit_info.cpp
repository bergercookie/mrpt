...
struct TInfoPerSensorLabel {
  TInfoPerSensorLabel()
      : occurrences(0), tim_first(INVALID_TIMESTAMP),
...
DECLARE_OP_FUNCTION(op_info) {
  ...
  public:
    ...
    CRawlogProcessor_Info(CFileGZInputStream &in_rawlog,
    ...
    virtual bool processOneEntry(CActionCollection::Ptr &actions,
                                 CSensoryFrame::Ptr &SF,
                                 CObservation::Ptr &obs) {
      // Rawlog format: Normally only one of both should exist simultaneously!
      if (actions || SF)
        has_actSF_format = true;
      if (obs)
        has_obs_format = true;
      if (actions)
        nActions++;
      ...
      // Process each observation individually, either from "obs" or each within
      // a "SF":
  ...
  for (map<string, TInfoPerSensorLabel>::const_iterator it =
    ...
    if (it != proc.infoPerSensorLabel.begin())
      cout << ", ";
    cout << it->first;
  }
  cout << "\n";
  ...
  for (map<std::string, TInfoPerSensorLabel>::const_iterator it =
    ...
    if (tf != INVALID_TIMESTAMP && tl != INVALID_TIMESTAMP) {
      ...
      Hz = double(it->second.occurrences > 1 ? it->second.occurrences - 1 : 1) /
           dur;
    }
