...
DECLARE_OP_FUNCTION(op_export_enose_txt) {
  ...
  public:
    ...
    // Default Constructor
    CRawlogProcessor_ExportENOSE_TXT(CFileGZInputStream &in_rawlog,
    ...
    bool processOneObservation(CObservation::Ptr &o) {
      ...
      if (it == lstFiles.end()) // A new file for this sensorlabel??
      {
        ...
        for (size_t j = 0; j < obs->m_readings.size(); j++) {
          ...
          for (size_t k = 0; k < obs->m_readings[j].readingsVoltage.size(); k++)
            ::fprintf(f_this, " S%u_%u ", static_cast<unsigned int>(j),
      ...
      // For each entry in this sequence: Compute the timestamp and save all
      // values:
