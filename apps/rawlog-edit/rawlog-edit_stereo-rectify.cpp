...
DECLARE_OP_FUNCTION(op_stereo_rectify) {
  ...
  class CRawlogProcessor_StereoRectify
  ...
  public:
    ...
    CRawlogProcessor_StereoRectify(CFileGZInputStream &in_rawlog,
      ...
      if (lstTokens.size() != 2)
        throw std::runtime_error("--stereo-rectify op: argument must be in the "
                                 "format: --stereo-rectify LABEL,ALPHA_VALUE");
      ...
      if (rectify_alpha != -1 && !(rectify_alpha >= 0 && rectify_alpha <= 1))
        throw std::runtime_error("--stereo-rectify op: Invalid ALPHA value. "
                                 "Use '-1' for auto guess.");
      ...
      outDir =
          (out_rawlog_basedir.empty() ? string()
                                      : (out_rawlog_basedir + string("/"))) +
          extractFileName(outrawlog.out_rawlog_filename) + string("_Images");
      if (directoryExists(outDir))
        throw runtime_error(
            string("*ABORTING*: Output directory for rectified images already "
                   "exists: ") +
            outDir +
            string(
      ...
      if (!fileExists(outDir))
        throw runtime_error(string("*ABORTING*: Couldn't create directory: ") +
                            outDir);
      ...
      if (getArgValue<string>(cmdline, "image-size", str)) {
        ...
        if (lstTokens.size() != 2)
          throw std::runtime_error(
              "--stereo-rectify op: Expected format: --image-size NCOLSxNROWS");

        const int nCols = atoi(lstTokens[0].c_str());
        const int nRows = atoi(lstTokens[1].c_str());
        VERBOSE_COUT << "Will rectify and resize to " << nCols << "x" << nRows
                     << " simultaneously.\n";
      ...
      if (strCmpI(obs->sensorLabel, target_label)) {
        if (IS_CLASS(obs, CObservationStereoImages)) {
          ...
          try {
            ...
            // This is needed to raise an exception of the correct type that
            // reveal any missing external file:
            ...
            const string label_time = format("%s_%f", o->sensorLabel.c_str(),
            ...
            if (m_num_external_files_failures < MAX_FAILURES) {
              ...
              cerr << "\n *WARNING*: Dropping one observation due to missing "
                      "external image file at rawlog entry "
                   << m_rawlogEntry << endl;
            } else {
              throw std::runtime_error("*ERROR* Too many external images "
                                       "missing, this doesn't seem spureous "
                                       "missings!");
            }
          }
        }
    ...
    // stream.
    virtual void OnPostProcess(mrpt::obs::CActionCollection::Ptr &actions,
