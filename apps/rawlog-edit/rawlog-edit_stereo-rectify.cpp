			...
			: CRawlogProcessorOnEachObservation(in_rawlog, cmdline, verbose)
		{
			...
			if (lstTokens.size() != 2)
				throw std::runtime_error(
					"--stereo-rectify op: argument must be in the format: "
					"--stereo-rectify LABEL,ALPHA_VALUE");
			...
			if (rectify_alpha != -1 &&
				!(rectify_alpha >= 0 && rectify_alpha <= 1))
				throw std::runtime_error(
					"--stereo-rectify op: Invalid ALPHA value. Use '-1' for "
					"auto "
					"guess.");
			...
			if (getArgValue<string>(cmdline, "image-size", str))
			{
				...
				if (lstTokens.size() != 2)
					throw std::runtime_error(
						"--stereo-rectify op: Expected format: --image-size "
						"NCOLSxNROWS");

				const int nCols = atoi(lstTokens[0].c_str());
				const int nRows = atoi(lstTokens[1].c_str());
				VERBOSE_COUT << "Will rectify and resize to " << nCols << "x"
							 << nRows << " simultaneously.\n";
			...
			{
				VERBOSE_COUT
					<< "Will rectify such that both image centers coincide.\n";
			...
			if (strCmpI(obs->sensorLabel, target_label))
			{
				if (IS_CLASS(obs, CObservationStereoImages))
				{
					...
					catch (mrpt::utils::CExceptionExternalImageNotFound&)
					{
						...
						{
							throw std::runtime_error(
								"*ERROR* Too many external images missing, "
								"this doesn't seem "
								"spureous missings!");
						}
					}
