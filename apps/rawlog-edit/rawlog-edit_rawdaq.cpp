    ...
    if (obs->AIN_interleaved) {                                                \
      for (i = 0, tim = tim0;                                                  \
        ...
        for (size_t j = 0; j < obs->AIN_channel_count; j++) {                  \
          ::fprintf(                                                           \
              f_this, _FRMT " ",                                               \
              (_TYPECAST)obs->_VECTOR_NAME[i * obs->AIN_channel_count + j]);   \
          m_entriesSaved++;                                                    \
        }                                                                      \
        ::fprintf(f_this, "\n");                                               \
      }                                                                        \
...
DECLARE_OP_FUNCTION(op_export_rawdaq_txt) {
  ...
  class CRawlogProcessor_ExportRAWDAQ_TXT
  ...
  public:
    ...
    CRawlogProcessor_ExportRAWDAQ_TXT(CFileGZInputStream &in_rawlog,
