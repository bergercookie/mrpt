...
string rawlog_ext_imgs_dir; // Directory where to save externally stored images,
                            // only for CCameraSensor's.
...
int main(int argc, char **argv) {
  try {
    ...
    MRPT_LOAD_CONFIG_VAR(time_between_launches, int, iniFile,
                         GLOBAL_SECTION_NAME);
    ...
    MRPT_LOAD_CONFIG_VAR(rawlog_GZ_compress_level, int, iniFile,
                         GLOBAL_SECTION_NAME);
      ...
      if (use_sensoryframes) {
        ...
        for (CGenericSensor::TListObservations::iterator it =
          ...
          if (IS_DERIVED(it->second, CAction)) {
            ...
            out_file << curSF;
            cout << "[" << dateTimeToString(now()) << "] Saved SF with "
                 << curSF.size() << " objects." << endl;
            curSF.clear();
            ...
            // First, check if inserting this OBS into the SF would overflow
            // "SF_max_time_span":
            if (curSF.size() != 0 &&
                timeDifference(curSF.getObservationByIndex(0)->timestamp,
              ...
              if (hwdrivers_verbose) {
                ...
                CObservationIMU::Ptr imu =
                    curSF.getObservationByClass<CObservationIMU>();
                if (imu) {
                  cout << format("   IMU angles (degrees): "
            ...
            THROW_EXCEPTION(
                "*** ERROR *** Class is not an action or an observation");
        }
      } else {
        // ---------------------------
        ...
        for (CGenericSensor::TListObservations::iterator it =
          ...
          if (hwdrivers_verbose) {
            if ((it->second)->GetRuntimeClass() == CLASS_ID(CObservationGPS)) {
              CObservationGPS::Ptr gps =
                  std::dynamic_pointer_cast<CObservationGPS>(it->second);
              if (gps->has_GGA_datum)
                cout << "  GPS mode: "
                     << (int)gps
                            ->getMsgByClass<mrpt::obs::gnss::Message_NMEA_GGA>()
                            .fields.fix_quality
                     << " label: " << gps->sensorLabel << endl;
            } else if ((it->second)->GetRuntimeClass() ==
        ...
        if (hwdrivers_verbose) {
          // Show IMU angles:
          if (imu) {
            cout << format("   IMU angles (degrees): (yaw,pitch,roll)=(%.06f, "
                           "%.06f, %.06f)",
                           RAD2DEG(imu->rawMeasurements[IMU_YAW]),
                           RAD2DEG(imu->rawMeasurements[IMU_PITCH]),
                           RAD2DEG(imu->rawMeasurements[IMU_ROLL]))
        ...
        if (!copy_of_global_list_obs.empty()) {
          cout << "[" << dateTimeToString(now()) << "] Saved "
               << copy_of_global_list_obs.size() << " objects." << endl;
        }
      }
...
void SensorThread(TThreadParams params) {
  try {
    string driver_name =
        params.cfgFile->read_string(params.sensor_label, "driver", "", true);

    CGenericSensor::Ptr sensor = CGenericSensor::createSensorPtr(driver_name);
    if (!sensor) {
      cerr << endl
           << "***ERROR***: Class name not recognized: " << driver_name << endl;
      allThreadsMustExit = true;
    }
