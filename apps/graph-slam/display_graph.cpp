...
const char *MSG_HELP_WINDOW = "These are the supported commands:\n"
                              " - h: Toogle help view\n"
...
template <class GRAPHTYPE> void display_graph(const GRAPHTYPE &g) {
  ...
  win.setCameraElevationDeg(75);
  {
    ...
    if (obj_grid) {
      ...
      win.setCameraPointingToPoint(0.5 * (x_min + x_max), 0.5 * (y_min + y_max),
                                   z_min);
      win.setCameraZoom(
  ...
  struct Win3D_observer : public mrpt::utils::CObserver {
    ...
    Win3D_observer(const GRAPHTYPE &g)
        : m_graph(g), request_to_refresh_3D_view(false), request_to_quit(false),
  ...
  protected:
    ...
    virtual void OnEvent(const mrptEvent &e) override {
      if (e.isOfType<mrpt::gui::mrptEventWindowChar>()) {
        ...
        if (rebuild_3d_obj) {
          m_new_3dobj =
              mrpt::opengl::graph_tools::graph_visualize(m_graph, params);
        ...
        if (showing_help || hiding_help) {
          ...
          mrpt::opengl::gl_utils::renderMessageBox(
              ...
              MSG_HELP_WINDOW,
              0.02f, // text size
              mrpt::utils::TColor(190, 190, 190,
                                  200 * transparency),            // background
  ...
  while (win.isOpen()) {
    if (win_feedback.request_to_refresh_3D_view) {
      ...
      *objGraph = *win_feedback.m_new_3dobj; // This overwrites the object
                                             // POINTER BY the smart pointer
                                             // "objGraph".
      win.unlockAccess3DScene();
      win.repaint();
...
template void
display_graph<CNetworkOfPoses2DInf>(const CNetworkOfPoses2DInf &g);
template void
display_graph<CNetworkOfPoses3DInf>(const CNetworkOfPoses3DInf &g);
