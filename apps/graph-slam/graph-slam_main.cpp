...
#define DECLARE_OP_FUNCTION(_NAME)                                             \
  void _NAME(const std::string &in_file, bool is3D, TCLAP::CmdLine &cmdline,   \
             bool verbose)

#define IMPLEMENT_OP_FUNCTION(_NAME)                                           \
  template <class GRAPHTYPE>                                                   \
  void _NAME##impl(const std::string &in_file, bool is3D,                      \
 ...
 * parentheses
 */
typedef void (*TOperationFunctor)(const std::string &in_file, bool is3D,
                                  TCLAP::CmdLine &cmdline, bool verbose);
...
TCLAP::ValueArg<std::string>
    arg_input_file("i", "input", "Input file (required) (*.graph,*.graphbin)",
                   true, "", "test.graph", cmd);
TCLAP::ValueArg<std::string>
    arg_output_file("o", "output",
                    "Output file (optional) (*.graph,*.graphbin)", false, "",
                    "result.graph", cmd);

TCLAP::SwitchArg arg_2d("", "2d",
                        "Use 2D poses (Must use exactly one of --2d and --3d)",
                        cmd, false);
TCLAP::SwitchArg arg_3d("", "3d",
                        "Use 3D poses (Must use exactly one of --2d and --3d)",
                        cmd, false);

TCLAP::SwitchArg arg_view("", "view",
                          "Op: Visualize the graph in a 3D view. If used "
                          "alone, represent VERTEX2 or VERTEX3 poses directly "
                          "as stored in the input file. "
                          "If used together with another operation, the final "
                          "obtained graph after the operation will be shown, "
                          "not the input original one.\n",
                          cmd, false);

TCLAP::SwitchArg arg_quiet("q", "quiet", "Terse output", cmd, false);
TCLAP::ValueArg<int> arg_max_iters("", "max-iters",
                                   "Maximum number of iterations (optional)",
                                   false, 100, "N", cmd);
TCLAP::ValueArg<double>
    arg_initial_lambda("", "initial-lambda",
                       "Initial lambda parameter (optional, lev-marq)", false,
                       0, "val", cmd);
TCLAP::SwitchArg
    arg_no_span("", "no-span",
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    arg_ops.push_back(new TCLAP::SwitchArg(
        "", "levmarq", "Op: Optimizes the graph with sparse "
                       "Levenberg-Marquartd using global coordinates (via "
                       "mrpt::graphslam::optimize_graph_spa_levmarq).\n"
                       "   Can be used together with: --view, --output, "
                       "--max-iters, --no-span, --initial-lambda",
        cmd, false));
    ops_functors["levmarq"] = &op_levmarq;

    arg_ops.push_back(new TCLAP::SwitchArg(
        "", "dijkstra", "Op: Executes "
                        "CNetworkOfPoses::dijkstra_nodes_estimate() to "
                        "estimate the global pose of nodes from a Dijkstra "
                        "tree and the edge relative poses.\n"
                        "   Can be used together with: --view, --output",
        cmd, false));
    ...
    if ((arg_2d.isSet() && arg_3d.isSet()) ||
        (!arg_2d.isSet() && !arg_3d.isSet()))
...
IMPLEMENT_OP_FUNCTION(op_dijkstra) {
  ...
  if (!save_to_file && !display_3D)
    std::cerr << "\n ** WARNING **: Neither --view nor --output specified. The "
                 "result will not be saved anywhere.\n\n";
