...
void Test_Kinect() {
  ...
  globalPtsMap.colorScheme.scheme =
      CColouredPointsMap::cmFromIntensityImage; // Take points color from RGB+D
                                                // observations
  // globalPtsMap.colorScheme.scheme =
  ...
  const double aspect_ratio =
      480.0 / 640.0; // kinect.getRowCount() / double( kinect.getColCount() );
  ...
  while (win3D.isOpen() && !thrPar.quit) {
    ...
    if (possiblyNewObs && possiblyNewObs->timestamp != INVALID_TIMESTAMP &&
      ...
      for (CFeatureList::iterator itFeat = trackedFeats.begin();
        ...
        if (d > 0.05 && d < 10.0) {
          ASSERT_(size_t(last_obs->rangeImage.cols() *
                         last_obs->rangeImage.rows()) ==
                  last_obs->points3D_x.size())
          const size_t nPt = last_obs->rangeImage.cols() * y + x;
          curVisibleFeats[(*itFeat)->ID] =
              TPoint3D(last_obs->points3D_x[nPt], last_obs->points3D_y[nPt],
      ...
      if (!lastVisibleFeats.empty()) {
        ...
        for (map<TFeatureID, TPoint3D>::const_iterator itCur =
          ...
          if (itFound != lastVisibleFeats.end()) {
            corrs.push_back(TMatchingPair(itFound->first, itCur->first,
                                          itFound->second.x, itFound->second.y,
                                          itFound->second.z, itCur->second.x,
                                          itCur->second.y, itCur->second.z));
          }
        }
        ...
        if (corrs.size() >= 3) {
          ...
          try {
            ...
            /* Cannot find a minimum number of matches, inconsistent parameters
             * due to very reduced numberof matches,etc. */
          }

          const CPose3D relativePose = CPose3D(results.transformation);

          str_status = mrpt::format(
              "%d corrs | inliers: %d | rel.pose: %s ", int(corrs.size()),
              int(results.inliers_idx.size()), relativePose.asString().c_str());
          str_status2 = string(results.inliers_idx.size() == 0
          ...
          if (register_ok && std::abs(results.scale - 1.0) < 0.1) {
            ...
            if ((relativePose.norm() > KEYFRAMES_MIN_DISTANCE ||
              ...
              const CPose3D new_keyframe_global =
                  CPose3D(*camera_key_frames_path.rbegin()) + relativePose;

              camera_key_frames_path.push_back(TPose3D(new_keyframe_global));

              gl_keyframes_must_refresh = true;

              currentCamPose_wrt_last = CPose3D(); // It's (0,0,0) since the
                                                   // last key-frame is the
                                                   // current pose!
              lastVisibleFeats = curVisibleFeats;

              cout << "Adding new key-frame: pose=" << new_keyframe_global
                   << endl;
      ...
      // Show 3D points & current visible feats, at the current camera 3D pose
      // "currentCamPose_wrt_last"
      // ---------------------------------------------------------------------
      if (last_obs->hasPoints3D) {
        const CPose3D curGlobalPose =
            CPose3D(*camera_key_frames_path.rbegin()) + currentCamPose_wrt_last;
        win3D.get3DSceneAndLock();
        // All 3D points:
        ...
        for (map<TFeatureID, TPoint3D>::const_iterator it =
          ...
          mrpt::opengl::CBox::Ptr box = std::make_shared<mrpt::opengl::CBox>(
      ...
      win3D.get3DSceneAndLock();
      win3D.addTextMessage(-100, -20, format("%.02f Hz", thrPar.Hz),
                           TColorf(0, 1, 1), 100,
    ...
    if (gl_keyframes_must_refresh) {
      ...
      // cout << "Updating gl_keyframes with " << camera_key_frames_path.size()
      // << " frames.\n";
      win3D.get3DSceneAndLock();
      gl_keyframes->clear();
    ...
    win3D.addTextMessage(2, -30,
                         format("'s':save point cloud, 'r': reset, 'o'/'i': "
                                "zoom out/in, mouse: orbit 3D, ESC: quit"),
                         TColorf(1, 1, 1), 110, MRPT_GLUT_BITMAP_HELVETICA_12);
    win3D.addTextMessage(2, -50, str_status, TColorf(1, 1, 1), 111,
                         MRPT_GLUT_BITMAP_HELVETICA_12);
    win3D.addTextMessage(2, -70, str_status2, TColorf(1, 1, 1), 112,
                         MRPT_GLUT_BITMAP_HELVETICA_18);
    win3D.unlockAccess3DScene();
