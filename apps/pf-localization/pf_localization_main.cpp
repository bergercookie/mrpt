...
// Forward declaration:
void do_pf_localization(const std::string &iniFilename,
                        const std::string &cmdline_rawlog_file);
void getGroundTruth(CPose2D &expectedPose, size_t rawlogEntry,
                    const CMatrixDouble &GT, const TTimeStamp &cur_time);
...
//				do_pf_localization
// ------------------------------------------------------
void do_pf_localization(const std::string &ini_fil,
                        const std::string &cmdline_rawlog_file) {
  ASSERT_FILE_EXISTS_(ini_fil)

  CConfigFile cfg(ini_fil);

  vector_int particles_count; // Number of initial particles (if size>1, run the
                              // experiments N times)
  ...
  // Mandatory entries:
  cfg.read_vector(sect, "particles_count", vector_int(1, 0), particles_count,
  ...
  if (cmdline_rawlog_file.empty())
    RAWLOG_FILE =
        cfg.read_string(sect, "rawlog_file", "", /*Fail if not found*/ true);
  else
  ...
  // Default odometry uncertainty parameters in "dummy_odom_params" depending on
  // how fast the robot moves, etc...
  ...
  if (MAP_FILE.size()) {
    ...
    string mapExt = lowerCase(
        extractFileExtension(MAP_FILE, true)); // Ignore possible .gz extensions
    ...
    if (!mapExt.compare("simplemap")) {
      ...
      THROW_EXCEPTION_FMT("Map file has unknown extension: '%s'",
                          mapExt.c_str());
    }
  }
  ...
  // Create the 3D scene and get the map only once, later we'll modify only the
  // particles, etc..
  COpenGLScene scene;
  ...
  if (metricMap.m_gridMaps.size()) {
    ...
    gridInfo.effectiveMappedArea =
        (init_PDF_max_x - init_PDF_min_x) * (init_PDF_max_y - init_PDF_min_y);
  }

  {
    scene.insert(
        std::make_shared<mrpt::opengl::CGridPlaneXY>(-50, 50, -50, 50, 0, 5));
    ...
    for (int repetition = 0; repetition < NUM_REPS; repetition++) {
      ...
      if (!SAVE_STATS_ONLY) {
        ...
        if (!metricMap.m_gridMaps.empty()) {
          ...
          CFileOutputStream(format("%s/gridmap_limits.txt", sOUT_DIR.c_str()))
              .printf("%f %f %f %f", metricMap.m_gridMaps[0]->getXMin(),
      ...
      // -----------------------------
      tictac.Tic();
      if (!cfg.read_bool(sect, "init_PDF_mode", false,
                         /*Fail if not found*/ true))
        pdf.resetUniformFreeSpace(
            metricMap.m_gridMaps[0].get(), 0.7f, PARTICLE_COUNT, init_PDF_min_x,
            init_PDF_max_x, init_PDF_min_y, init_PDF_max_y,
            DEG2RAD(cfg.read_float(sect, "init_PDF_min_phi_deg", -180)),
            DEG2RAD(cfg.read_float(sect, "init_PDF_max_phi_deg", 180)));
      else
        pdf.resetUniform(
            init_PDF_min_x, init_PDF_max_x, init_PDF_min_y, init_PDF_max_y,
            DEG2RAD(cfg.read_float(sect, "init_PDF_min_phi_deg", -180)),
            DEG2RAD(cfg.read_float(sect, "init_PDF_max_phi_deg", 180)),
            PARTICLE_COUNT);
      ...
      while (!end) {
        ...
        if (obs) {
          // It's an observation-only rawlog: build an auxiliary pair of
          // action-SF, since
          ...
          // TODO: Another good idea would be to take CObservationOdometry
          // objects and use that information, if available.
        ...
        if (step >= rawlog_offset) {
          ...
          if (step > rawlog_offset) {
            ...
            if (SHOW_PROGRESS_3D_REAL_TIME) {
              ...
              if (rawlogEntry >= 2)
                getGroundTruth(expectedPose, rawlogEntry - 2, GT,
                               cur_obs_timestamp);
              ...
              win3D->addTextMessage(
                  10, 33, mrpt::format("#particles= %7u",
                                       static_cast<unsigned int>(pdf.size())),
                  mrpt::utils::TColorf(.8f, .8f, .8f), "mono", 15,
                  mrpt::opengl::NICE, 6002);

              win3D->addTextMessage(10, 55,
                                    mrpt::format("mean pose (x y phi_deg)= %s",
                                                 meanPose.asString().c_str()),
                                    mrpt::utils::TColorf(.8f, .8f, .8f), "mono",
                                    15, mrpt::opengl::NICE, 6003);

              {
                CRenderizable::Ptr grid_ground =
                    ptrScene->getByName("ground_lines");
                if (!grid_ground) {
                  grid_ground = std::make_shared<mrpt::opengl::CGridPlaneXY>(
                      -50, 50, -50, 50, 0, 5);
              ...
              ptrScene->createViewport("small_view"); // Create, or get existing
              one.
            ...
            tictac.Tic();
            if (!SAVE_STATS_ONLY)
              printf("Step %u -- Executing ParticleFilter on %u particles....",
                     (unsigned int)step, (unsigned int)pdf.particlesCount());
          ...
          if (best_mov_estim)
            odometryEstimation =
                odometryEstimation + best_mov_estim->poseChange->getMeanVal();
...
// mrpt::system::timestampTotime_t(cur_obs_timestamp)) << expectedPose << endl;
          ...
          { // Averaged error to GT
            ...
            for (size_t k = 0; k < pdf.size(); k++)
              sumW += exp(pdf.getW(k));
            for (size_t k = 0; k < pdf.size(); k++)
              locErr += expectedPose.distanceTo(pdf.getParticlePose(k)) *
                        exp(pdf.getW(k)) / sumW;
          ...
          pdf.getCovarianceAndMean(current_pdf_gaussian.cov,
                                   current_pdf_gaussian.mean);
          ...
          if (!SAVE_STATS_ONLY) {
            ...
            cout << "         PDF est: " << pdfEstimation
                 ...
                 << " tr(cov): " << std::sqrt(current_pdf_gaussian.cov.trace())
                 << "\n";
            if (GT.getRowCount() > 0)
          ...
          if (DO_RELIABILITY_ESTIMATE) {
            ...
            if (observations)
              obs_scan =
                  observations->getObservationByClass<CObservation2DRangeScan>(
                      0); // Get the 0'th scan, if several are present.
            ...
            if (obs_scan && gridmap) // We have both, go on:
            {
              ...
              obs_reliability_estim =
                  ssu_out.scanWithUncert.evaluateScanLikelihood(*obs_scan,
                                                                evalParams);
              ...
              if (DO_SCAN_LIKELIHOOD_DEBUG) {
                ...
                for (size_t i = 0;
                     i < ssu_out.scanWithUncert.rangeScan.scan.size(); i++)
                  ranges_mean.push_back(
                      ssu_out.scanWithUncert.rangeScan.scan[i]);
                ...
                win.setWindowTitle(mrpt::format("obs_reliability_estim: %f",
                                                obs_reliability_estim));
                win.axis_fit();
              }
            }
          ...
          if (!SAVE_STATS_ONLY && SCENE3D_FREQ > 0 &&
            ...
            // Generate 3D scene:
            // ------------------------------
            // MRPT_TODO("Someday I should clean up this mess, since two
            // different 3D scenes are built -> refactor code")
            ...
            /*COpenGLViewport::Ptr view2= scene.createViewport("small_view"); //
          ...
          if (!SAVE_STATS_ONLY && SCENE3D_FREQ != -1 &&
              (step % SCENE3D_FREQ) == 0) {
            // Save 3D scene:
            CFileGZOutputStream(format("%s/progress_%05u.3Dscene",
                                       sOUT_DIR_3D.c_str(), (unsigned)step))
                << scene;

            // Generate text files for matlab:
            // ------------------------------------
            pdf.saveToTextFile(format("%s/particles_%05u.txt",
                                      sOUT_DIR_PARTS.c_str(), (unsigned)step));
          }
    ...
    math::confidenceIntervals(covergenceErrors, covergenceErrorMean,
    ...
    // Save overall results:
    {
      ...
      CFileOutputStream f(format("%s_SUMMARY.txt", OUT_DIR_PREFIX.c_str()),
                          true /* append */);

      f.printf("%% Ratio_covergence_success  #particles  "
               "average_time_per_execution  convergence_mean_error "
               "convergence_error_conf_int_inf convergence_error_conf_int_sup "
               "\n");
      if (!nConvergenceTests)
        nConvergenceTests = 1;
