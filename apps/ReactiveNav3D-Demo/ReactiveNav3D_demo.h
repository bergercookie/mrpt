...
public:
  ...
  void KinectScan(const std::vector<mrpt::maps::COccupancyGridMap2D> &m_maps,
    ...
    for (unsigned int k = 0; k < m_maps.size(); k++) {
      ...
      m_maps[k].laserScanSimulator(m_auxlaser, scanpose2d, 0.5f,
                                   acc_factor * m_columns, m_std_error, 1, 0);
      ...
      for (unsigned int i = 0; i < m_rows; i++) {
        for (unsigned int j = 0; j < m_columns; j++) {
          if (row_points.size() > acc_factor * j) {
            ...
            // Points which belong to their height level are inserted. Otherwise
            // they are deleted.
            if (m_maps.size() == 1) {
              ...
              if (k == 0) {
                ...
                if ((point.z >= h) && (point.z < h + heights[k])) {
                  m_points.insertPoint(point);
                }
              }
            }
...
public:
  ...
  void updateObsGrids(float incrx, float incry, float phi,
    ...
    } else if (sqrt(square(incrx) + square(incry)) >
               2.6 * obsgrids[0].getXMax())
    // The displacement is too big so the grid is reset
    {
      for (unsigned int i = 0; i < obsgrids.size(); i++) {
        obsgrids[i].setSize(obsgrids[0].getXMin(), obsgrids[0].getXMax(),
    ...
    float incr_grid_reactive =
        0.2 / obsgrids[0].getResolution(); // This number marks distance in
                                           // meters (but it's transformed into
                                           // an index)
    ...
    for (unsigned int n = 0; n < obsgrids.size(); n++) {
      ...
      // Vector obs_in is filled with 0 or 1 depending on the presence of any
      // obstacle at each cell (of the grid)
      for (unsigned int i = 0; i < kinects[0].m_points.size(); i++) {
        kinects[0].m_points.getPoint(i, paux);

        // Points rotation and translation
        aux_xpass =
            paux.x * cos(angrot) + paux.y * sin(angrot) + robot_ingrid.x;
        paux.y = -paux.x * sin(angrot) + paux.y * cos(angrot) + robot_ingrid.y;
        paux.x = aux_xpass;

        // Set binary occupancy of the cells (1 - there is at least one point, 0
        // - there isn't any point)
        if ((paux.x >= -xylim) && (paux.x <= xylim) && (paux.y >= -xylim) &&
            (paux.y <= xylim) && (paux.z < level_height + heights[n]) &&
            (paux.z > level_height + 0.000001)) {
          index =
              obsgrids[n].x2idx(paux.x) + num_col * obsgrids[n].y2idx(paux.y);
          obs_in[index] = 1;
        }
      }
      ...
      for (unsigned int i = 0; i < num_col; i++) {
        for (unsigned int j = 0; j < num_col; j++) {
          ...
          else if (((i < lim_visionxn) || (i > lim_visionxp)) ||
                   ((j < lim_visionyn) || (j > lim_visionyp))) {
            // The angle between the advance direction of the robot and the cell
            // is calculated
            angle_cell = atan2(obsgrids[n].idx2y(j) - robot_ingrid.y,
          ...
          if (((i >= lim_visionxn - incr_grid_reactive) &&
            ...
            if (obsgrids[n].getCell(i, j) > occupancy_threshold) {
              paux.x = (obsgrids[n].idx2x(i) - robot_ingrid.x) * cos(-angrot) +
                       (obsgrids[n].idx2y(j) - robot_ingrid.y) * sin(-angrot);
...
public:
  ...
  bool senseObstacles(mrpt::maps::CSimplePointsMap &obstacles,
    ...
    for (unsigned int i = 0; i < kinects.size(); i++) {
      ...
      kinects[i].KinectScan(maps, robotShape.getHeights(), robotpose3d,
                            kinectrelpose);
      obstacles.insertAnotherMap(&kinects[i].m_points,
                                 CPose3D(0, 0, 0, 0, 0, 0));
    }
    ...
    if (stm.is_active) {
      ...
      stm.updateObsGrids(incrx, incry, float(new_pose.phi()), kinects,
  ...
  void loadConfiguration(const utils::CConfigFileBase &ini) {
    ...
    for (unsigned int i = 1; i <= num_lasers; i++) {
      ini.read_vector("LASER_CONFIG", format("LASER%d_POSE", i),
                      std::vector<double>(0), lasercoord, true);
      mrpt::obs::CObservation2DRangeScan &scan = lasers[i - 1].m_scan;
      scan.maxRange = ini.read_float("LASER_CONFIG",
                                     format("LASER%d_MAX_RANGE", i), 50, true);
      ...
      scan.sensorPose.setFromValues(lasercoord[0], lasercoord[1], lasercoord[2],
                                    lasercoord[3], lasercoord[4],
                                    lasercoord[5]);
      lasers[i - 1].m_level =
          ini.read_int("LASER_CONFIG", format("LASER%d_LEVEL", i), 1, true);
    ...
    for (unsigned int i = 1; i <= num_kinects; i++) {
      kinects[i - 1].m_level =
          ini.read_int("KINECT_CONFIG", format("KINECT%d_LEVEL", i), 1, true);
      kinects[i - 1].m_xrel =
          ini.read_float("KINECT_CONFIG", format("KINECT%d_X", i), 0, true);
      kinects[i - 1].m_yrel =
          ini.read_float("KINECT_CONFIG", format("KINECT%d_Y", i), 0, true);
      kinects[i - 1].m_zrel =
          ini.read_float("KINECT_CONFIG", format("KINECT%d_Z", i), 0, true);
      kinects[i - 1].m_phi = DEG2RAD(
          ini.read_float("KINECT_CONFIG", format("KINECT%d_PHI", i), 0, true));
      ...
      kinects[i - 1].m_rows =
          ini.read_int("KINECT_CONFIG", format("KINECT%d_ROWS", i), 10, true);
    ...
    for (unsigned int i = 1; i <= num_levels; i++) {
      robotShape.setHeight(i - 1, ini.read_double("ROBOT_CONFIG",
                                                  format("LEVEL%d_HEIGHT", i),
                                                  1.0, true));
      robotShape.setRadius(i - 1, ini.read_double("ROBOT_CONFIG",
                                                  format("LEVEL%d_RADIUS", i),
                                                  0.5, false));
      ini.read_vector("ROBOT_CONFIG", format("LEVEL%d_VECTORX", i),
                      std::vector<double>(0), xaux, false);
      ini.read_vector("ROBOT_CONFIG", format("LEVEL%d_VECTORY", i),
    ...
    float delay =
        0.f; // ini.read_float("ReactiveParams","ROBOTMODEL_DELAY", 0, true);
    ...
    if (stm.is_active) {
      ...
      for (unsigned int i = 0; i < stm.obsgrids.size(); i++) {
        stm.obsgrids[i].setSize(-grid_length, grid_length, -grid_length,
                                grid_length, grid_resolution, 0.5);
      }
    }
  }
  ...
  void initializeScene() {
    ...
    // The robot is inserted
    {
      ...
      for (unsigned int i = 0; i < robotShape.size(); i++) {
        ...
        robotpose3d.z(h);
        CPolyhedron::Ptr obj;
        obj = opengl::CPolyhedron::CreateCustomPrism(robotShape.polygon(i),
                                                     robotShape.getHeight(i));
  ...
  CAbstractNavigator::TNavigationParams
  createNewTarget(float x, float y, float targetAllowedDistance,
