...
const char *default_cfg_txt =
    ...
    ";						KINECTX_MINRANGE, "
    "KINECTX_MAXRANGE (METERS), KINECTX_ROWS, KINECTX_COLUMNS, "
    "KINECTX_STD_ERROR  \n\n"
    ...
    "robotMax_V_mps = 0.70			; Speed limits - mps \n"
    "robotMax_W_degps = 60			; dps \n"
    "MAX_REFERENCE_DISTANCE = 2		; Marks the maximum distance "
    "regarded by the reactive navigator (m) \n"

    "X0 = 2						; Initial location "
    "(meters) \n"
    "Y0 = 0 \n"
    "PHI0 = -90					; In degrees \n"
    ...
    "WIDE_GAP_SIZE_PERCENT = 0.25			; The robot travels "
    "nearer to obstacles if this parameter is small. \n"
    "										"
    ...
    "RISK_EVALUATION_DISTANCE = 0.7			; Parameter used to "
    "decrease speed if obstacles are closer than this threshold \n"
    "										"
    ...
    "TOO_CLOSE_OBSTACLE = 0.03				; In normalized "
    "ps-meters [0,1] \n\n\n"
    ...
    "TARGET_SLOW_APPROACHING_DISTANCE = 0.8	; Used to decrease speed "
    "gradually when the target is going to be reached \n"
    "TARGET_ATTRACTIVE_FORCE = 7.5			; Use it to control the "
    "relative weight of the target respect to the obstacles \n\n\n"
    ...
    "Vision_limit = 0.5			; Min. Limit of vision of the "
    "RGBD sensor \n"
    ...
    while (!stop) {
      ...
      if (observer.mouse_click == 1) {
        ...
        if (moving_target == 1) {
          ...
          const CAbstractNavigator::TNavigationParams nav_params =
              ReactInterface.createNewTarget(last_Target_Pos.x,
                                             last_Target_Pos.y, 0.3f, 0);
      ...
      if (moving_target == 1) {
        ...
        if (ReactInterface.window.getLastMousePosition(mouse_x, mouse_y)) {
          ...
          math::TLine3D ray;
          ReactInterface.scene->getViewport("main")->get3DRayForPixelCoord(
              mouse_x, mouse_y, ray);

          // Create a 3D plane, e.g. Z=0
          const math::TPlane ground_plane(TPoint3D(0, 0, 0), TPoint3D(1, 0, 0),
                                          TPoint3D(0, 1, 0));
          ...
          if (inters.getPoint(last_Target_Pos)) {
            ...
            ReactInterface.scene->getByClass<CDisk>(0)->setLocation(
                last_Target_Pos.x, last_Target_Pos.y, last_Target_Pos.z);
          }
        }
      }
    }
