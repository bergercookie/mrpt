...
bool DemoFeatures() {
  ...
  default: { cerr << "Descriptor specified is not handled yet" << endl; } break;
  }
  ...
  for (unsigned int i1 = 0; i1 < feats1.size() && winPlots.isOpen() &&
    ...
    winPlots.plot(CVectorDouble(1, (double)min_dist_idx),
                  CVectorDouble(1, min_dist), ".8b", "best_dists");

    winPlots.setWindowTitle(format("Distances feat #%u -> all others ", i1));

    // Display the current descriptor in its window and the best descriptor from
    // the other image:
    switch (desc_to_compute) {
    case descAny: // Patch
    case descPolarImages:
    ...
    case descSpinImages: {
      ...
      if (desc_to_compute == descAny) {
      ...
      } else if (desc_to_compute == descSpinImages) {
        {
          ...
          const size_t nC = feats1[i1]->descriptors.SpinImg.size() /
      ...
      while (auxImg1.getWidth() < 100 && auxImg1.getHeight() < 100)
        auxImg1.scaleImage(auxImg1.getWidth() * 2, auxImg1.getHeight() * 2,
                           IMG_INTERP_NN);
      while (auxImg2.getWidth() < 100 && auxImg2.getHeight() < 100)
        auxImg2.scaleImage(auxImg2.getWidth() * 2, auxImg2.getHeight() * 2,
                           IMG_INTERP_NN);
    ...
    for (unsigned int i2 = 0; i2 < feats2.size(); i2++) {
      if (distances[i2] < min_dist + 0.1 * dist_std) {
        ...
        img2_show_base.textOut(feats2[i2]->x + 10, feats2[i2]->y - 10,
        ...
        img2_show_base.cross(feats2[i2]->x, feats2[i2]->y, TColor::gray, '+',
                             3);
      }
    }
    ...
    // Show new images in win1 / win2, but with a catchy animation to focus on
    // the features:
    // ------------------------------------------------------------------------------------------
    ...
    for (unsigned anim_loops = 36; anim_loops > 0; anim_loops -= 2) {
      ...
      img1_show.drawCircle(feats1[i1]->x, feats1[i1]->y, 7 + anim_loops,
                           TColor::blue);

      img2_show = img2_show_base;
      for (unsigned int i2 = 0; i2 < feats2.size(); i2++) {
        if (distances[i2] < min_dist + 0.1 * dist_std) {
          img2_show.drawCircle(feats2[i2]->x, feats2[i2]->y, 7 + anim_loops,
                               TColor::blue);
        }
      }
