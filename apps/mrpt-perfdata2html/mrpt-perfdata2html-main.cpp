...
/*---------------------------------------------------------------
        APPLICATION: mrpt-perfdata2html
        PURPOSE: Convert peformance results into HTML pages
  ---------------------------------------------------------------*/
...
int run_build_tables(const std::string &PERF_DATA_DIR) {
  ...
  for (size_t i = 0; i < fils.size(); i++) {
    if (!strStarts(fils[i].name, "perf-results"))
      continue;
  ...
  std::sort(lstConfigurations.begin(), lstConfigurations.end(),
            func_comp_entries);
  ...
  //                  index.html
  // ====================================================
  const string out_fil_index = PERF_DATA_DIR + string("/perf-html/"
                                                      "index.html");
  ...
  fo.printf("<br>\n"
            "This is a historic repository of performance tests executed "
            "against the MRPT libraries along time.\n"
            "Each entry represents a different MRPT version, different "
            "compilers or architectures."
            "Below follows the list of available performance tests (see also "
            "the <a href=\"#matrix\">comparison tables matrix</a>):<br>\n"
            "<ul>\n");
  ...
  for (size_t i = 0; i < lstConfigurations.size(); i++) {
    ...
    fo.printf(" <tr><td align=\"right\"><b>%s</b></td>",
  ...
  for (size_t i = 0; i < lstConfigurations.size(); i++) {
    const TPerfField &P1 = lstConfigurations[i];
    // class=\"my_rotated\"
    fo.printf("<td align=\"center\" width=\"20em\"><b><div >%s</div></b></td>",
              P1.config_name.c_str());
  }
  fo.printf(" </tr>\n");
  fo.printf("</table></div>\n");

  fo.printf("<hr><small>Page generated automatically at %s with %s by "
  ...
  for (size_t i = 0; i < lstConfigurations.size(); i++) {
    const TPerfField &P = lstConfigurations[i];
    const string out_fil = PERF_DATA_DIR + format("/perf-html/results_%s.html",
                                                  P.config_name.c_str());
    cout << "Generating: " << out_fil << "...\n";
    CFileOutputStream fo(out_fil);

    fo.printf("<html><head><title>mrpt-performance results - Configuration: "
              "%s</title></head>\n"
              "<body style=\"background-color:#E6E6FA\">\n\n",
              P.config_name.c_str());

    fo.printf("<div align=\"center\"><h2>Results for: %s</h2></div>",
              P.config_name.c_str());
    fo.printf("<hr> <br><br>\n");
         ...
         j++) // vector<pair<string,double> >
    {
      ...
      fo.printf("<tr> <td>%s</td> <td align=\"right\">%s</td> <td "
  ...
  for (size_t i = 0; i < lstConfigurations.size(); i++) {
    ...
    for (size_t j = 0; j < lstConfigurations.size(); j++) {
      ...
      const string out_fil =
          PERF_DATA_DIR + format("/perf-html/comparison_%s_vs_%s.html",
                                 P1.config_name.c_str(),
      ...
      fo.printf("<html><head><title>mrpt-performance results - %s vs "
                "%s</title></head>\n"
                "<body style=\"background-color:#E6E6FA\">\n\n",
                P1.config_name.c_str(), P2.config_name.c_str());

      fo.printf("<div align=\"center\"><h2>Comparison: %s vs. %s</h2></div>",
                P1.config_name.c_str(), P2.config_name.c_str());
      fo.printf("<hr> <br><br>\n");

      fo.printf("<div align=\"center\"><table border=\"1\">\n");
      fo.printf("<tr> <td rowspan=\"2\" align=\"center\"><b>Test "
                "description</b></td> "
           ...
           k++) // vector<pair<string,double> >
      {
        ...
        if (!P2_has_this_one) {
          ...
          str_Hz_res =
              format("<div style=\"color:%s;\">%s %.02f%%</div>",
                     AHz > 0 ? "blue" : "red", AHz > 0 ? "&Delta;" : "&nabla;",
                     100 * AHz / (1.0 / t2));
        }
...
int main(int argc, char **argv) {
  try {
    if (argc != 2) {
      cerr << "Usage: " << argv[0] << " <PATH_TO_MRPT_PERF-DATA>\n";
      return 1;
    }

    const std::string PERF_DATA_DIR = std::string(argv[1]);

    return run_build_tables(PERF_DATA_DIR);
