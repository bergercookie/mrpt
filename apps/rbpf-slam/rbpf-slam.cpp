...
int main(int argc, char **argv) {
  try {
    ...
    RAWLOG_FILE = !override_rawlog_file.empty()
                      ...
                      : iniFile.read_string("MappingApplication", "rawlog_file",
                                            "", /*Force existence:*/ true);
    rawlog_offset = iniFile.read_int("MappingApplication", "rawlog_offset", 0);
    OUT_DIR_STD = iniFile.read_string("MappingApplication", "logOutput_dir",
                                      "log_out", /*Force existence:*/ true);
    LOG_FREQUENCY = iniFile.read_int("MappingApplication", "LOG_FREQUENCY", 5,
                                     /*Force existence:*/ true);
    GENERATE_LOG_JOINT_H =
        iniFile.read_bool("MappingApplication", "GENERATE_LOG_JOINT_H", false);
    GENERATE_LOG_INFO =
    ...
    METRIC_MAP_CONTINUATION_START_POSE.phi = DEG2RAD(
        iniFile.read_double("MappingApplication",
                            "METRIC_MAP_CONTINUATION_START_POSE_PHI_DEG", .0));

    MRPT_LOAD_CONFIG_VAR(PROGRESS_WINDOW_WIDTH, int, iniFile,
                         "MappingApplication");
    MRPT_LOAD_CONFIG_VAR(PROGRESS_WINDOW_HEIGHT, int, iniFile,
                         "MappingApplication");
...
void MapBuilding_RBPF() {
  ...
  rbpfMappingOptions.loadFromConfigFile(CConfigFile(INI_FILENAME),
                                        "MappingApplication");
  ...
  if (!METRIC_MAP_CONTINUATION_GRIDMAP_FILE.empty()) {
    ...
    for (CMultiMetricMapPDF::CParticleList::iterator it =
      ...
      ASSERTMSG_(it_grid, "No gridmap in multimetric map definition, but "
                          "metric map continuation was set (!)");
  ...
  // ---------------------------------
  // mapBuilder.setVerbosityLevel(  mrpt::utils::LVL_DEBUG );  // default value:
  // as loaded from config file
  ...
  f_info.printf("%% EMI    H    EMMI    effecMappedArea  effecMappedCells \n"
                "%%-------------------------------------------------------\n");

  f_pathOdo.printf("%% time_step  x  y z yaw pitch roll timestamp \n"
                   "%%--------------------------------------------\n");

  f_pathOdo.printf("%% time_step  x  y z yaw pitch roll \n"
                   "%%----------------------------------\n");

  f_partStats.printf("%% time_step   #particles   ESS \n"
                     "%%------------------------------\n");
  ...
  if (SHOW_PROGRESS_IN_WINDOW) {
    win3D = new CDisplayWindow3D("RBPF-SLAM @ MRPT C++ Library",
  ...
  for (;;) {
    ...
    if (rawlogEntry >= rawlog_offset) {
      ...
      f_log.printf("%u %f %i %i\n", static_cast<unsigned int>(step),
                   ...
                   mapBuilder.m_statsLastIteration.observationsInserted
                       ? int(1)
                       : int(0));
      ...
      if (0 == (step % LOG_FREQUENCY)) {
        ...
        if (GENERATE_LOG_INFO) {
          ...
          printf("Ok\n EMI = %.04f    EMMI=%.04f (in %.03fms)\n", entropy.I,
                 entropy.mean_I, 1000.0f * tictac_JH.Tac());
        }
        ...
        if (SAVE_3D_SCENE || SHOW_PROGRESS_IN_WINDOW) {
          ...
          mrpt::opengl::CGridPlaneXY::Ptr groundPlane =
              std::make_shared<mrpt::opengl::CGridPlaneXY>(-200, 200, -200, 200,
                                                           0, 5);
          ...
          for (int k = (int)dummyPath.size() - 1; k >= 0; k--) {
            ...
            if (meanPose.distanceTo(lastMeanPose) > minDistBtwPoses) {
              ...
              objEllip->setLocation(meanPose.x(), meanPose.y(),
                                    meanPose.z() + 0.001);
        ...
        if (SAVE_3D_SCENE) { // Save as file:
          CFileGZOutputStream(
              format("%s/buildingmap_%05u.3Dscene", OUT_DIR_3D.c_str(), step))
              << *scene;
        }
      ...
      // ------------------------------------------------------------------
      {
        ...
        FILE *f =
            os::fopen(format("%s/log_MemoryUsage.txt", OUT_DIR).c_str(), "at");
        if (f) {
          os::fprintf(f, "%u\t%lu\n", step, memUsage);
          os::fclose(f);
        }
      ...
      f_partStats.printf("%u %u %f\n", (unsigned int)step,
                         (unsigned int)curPDF.size(), curPDF.ESS());
      ...
      f_path.printf("%i %f %f %f %f %f %f %f\n", (int)rawlogEntry, meanPose.x(),
                    meanPose.y(), meanPose.z(), meanPose.yaw(),
                    meanPose.pitch(), meanPose.roll(),
                    observations_timestamp_double);

      f_pathOdo.printf("%i\t%f\t%f\t%f\t%f\t%f\t%f\n", step, odoPose.x(),
                       odoPose.y(), odoPose.z(), odoPose.yaw(), odoPose.pitch(),
                       odoPose.roll());
  ...
  if (mostLikMap->m_gridMaps.size() > 0) {
    ...
    auxMat.saveToTextFile(format("%s/finalGridmapSize.txt", OUT_DIR),
                          MATRIX_FORMAT_FIXED);
  }
