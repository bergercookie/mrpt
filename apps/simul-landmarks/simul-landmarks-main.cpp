...
int main(int argc, char **argv) {
  try {
    ...
    CPose3D sensorPoseOnRobot(
        sensorPose_x, sensorPose_y, sensorPose_z, DEG2RAD(sensorPose_yaw_deg),
        DEG2RAD(sensorPose_pitch_deg), DEG2RAD(sensorPose_roll_deg));
    ...
    do {
      ...
      for (size_t i = 0; i < nLandmarks; i++) {
        ...
        // Random coordinates:
        pt3D.mean = CPoint3D(randomGenerator.drawUniform(min_x, max_x),
    ...
    for (size_t i = 0; i < nSteps; i++) {
      ...
      if (obs->sensedData.empty())
        nWarningsNoSight++;

      SF.push_back(obs);

      // Simulate odometry, from "incPose3D" with noise:
      if (!incPose_is_3D) { // 2D odometry:
        CActionRobotMovement2D act;
        CPose2D incOdo(incPose3D);
        if (incPose3D.x() != 0 || incPose3D.y() != 0 || incPose3D.yaw() != 0) {
          incOdo.x_incr(randomGenerator.drawGaussian1D_normalized() *
                        odometryNoiseXY_std);
          incOdo.y_incr(randomGenerator.drawGaussian1D_normalized() *
                        odometryNoiseXY_std);
          incOdo.phi_incr(randomGenerator.drawGaussian1D_normalized() *
        ...
        if (incPose3D.x() != 0 || incPose3D.y() != 0 || incPose3D.yaw() != 0) {
          noisyIncPose.x_incr(randomGenerator.drawGaussian1D_normalized() *
                              odometryNoiseXY_std);
          noisyIncPose.y_incr(randomGenerator.drawGaussian1D_normalized() *
                              odometryNoiseXY_std);
          noisyIncPose.z_incr(randomGenerator.drawGaussian1D_normalized() *
                              odometryNoiseXY_std);
    ...
    GT_path.saveToTextFile(
        format("%s/%s_ground_truth_robot_path.txt", outDir.c_str(),
               outFile.c_str()),
    ...
    if (show_in_3d && size(GT_path, 1) > 1) {
      ...
      for (size_t i = 0; i < N - 1; i++)
        pathLines->setLineByIndex(i, GT_path(i, 0), GT_path(i, 1),
                                  GT_path(i, 2), GT_path(i + 1, 0),
                                  GT_path(i + 1, 1), GT_path(i + 1, 2));
      ...
      for (size_t i = 0; i < N; i++) {
        ...
        corner->setPose(TPose3D(GT_path(i, 0), GT_path(i, 1), GT_path(i, 2),
