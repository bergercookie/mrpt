...
void do_simulation();
void simulOdometry(const CPose2D &real_pose, CPose2D &last_pose, CPose2D &Apose,
...
int main(int argc, char **argv) {
  try {
    ...
    TCLAP::ValueArg<std::string> arg_poses(
        "p", "poses", "poses text file, one 'time x y phi' line per pose", true,
        "", "poses.txt", cmd);
    ...
    TCLAP::ValueArg<int> arg_ranges(
        "r", "ranges", "number of laser ranges per scan (default=361)", false,
        361, "icp_goodness.txt", cmd);
    TCLAP::ValueArg<double> arg_span(
        "s", "span", "span of the laser scans (default=180 degrees)", false,
        180, "span", cmd);
    TCLAP::ValueArg<double> arg_std_r("R", "std_r",
                                      "range noise sigma (default=0.01 meters)",
                                      false, 0.01, "std_r", cmd);
    TCLAP::ValueArg<double> arg_std_b(
        "B", "std_b", "bearing noise sigma (default=0.05 degrees)", false, 0.05,
        "std_b", cmd);

    TCLAP::SwitchArg arg_nologo("n", "nologo", "skip the logo at startup", cmd,
                                false);
...
void do_simulation() {
  ...
  if (have_in_rawlog) {
    ...
    for (size_t i = 1; i < N; i++) {
      ...
      the_grid.laserScanSimulator(*the_scan, real_pose, 0.5f, LASER_N_RANGES,
                                  LASER_STD_ERROR, 1, LASER_BEARING_STD_ERROR);
      sf->insert(the_scan);
...
void simulOdometry(
  ...
  // Noise:
  Apose.x_incr(randomGenerator.drawGaussian1D_normalized() *
               odo_opts.gaussianModel.minStdXY);
  Apose.y_incr(randomGenerator.drawGaussian1D_normalized() *
               odo_opts.gaussianModel.minStdXY);
  Apose.phi_incr(randomGenerator.drawGaussian1D_normalized() *
