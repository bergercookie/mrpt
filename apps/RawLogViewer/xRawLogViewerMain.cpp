	...
	: m_fileHistory(9 /* Max file list*/)
{
	...
	FlexGridSizer15->Add(
		btnToolbarOpen, 1, wxALL | wxFIXED_MINSIZE | wxALIGN_CENTER_HORIZONTAL |
	...
	FlexGridSizer15->Add(
		Button1, 1, wxALL | wxFIXED_MINSIZE | wxALIGN_CENTER_HORIZONTAL |
						wxALIGN_CENTER_VERTICAL,
		1);
	...
	FlexGridSizer15->Add(
		Button2, 1, wxALL | wxFIXED_MINSIZE | wxALIGN_CENTER_HORIZONTAL |
						wxALIGN_CENTER_VERTICAL,
		1);
	...
	FlexGridSizer15->Add(
		Button3, 1, wxALL | wxFIXED_MINSIZE | wxALIGN_CENTER_HORIZONTAL |
						wxALIGN_CENTER_VERTICAL,
		1);
	...
	FlexGridSizer15->Add(
		Button4, 1, wxALL | wxFIXED_MINSIZE | wxALIGN_CENTER_HORIZONTAL |
						wxALIGN_CENTER_VERTICAL,
		1);
	...
	Button5->SetBitmapDisabled(
		wxArtProvider::GetBitmap(
			wxART_MAKE_ART_ID_FROM_STR(_T("ICON_ICP")), wxART_TOOLBAR));
	Button5->SetMargins(wxSize(5, 5));
	FlexGridSizer15->Add(
		Button5, 1, wxALL | wxFIXED_MINSIZE | wxALIGN_CENTER_HORIZONTAL |
						wxALIGN_CENTER_VERTICAL,
		1);
	...
	FlexGridSizer15->Add(
		Button6, 1, wxALL | wxFIXED_MINSIZE | wxALIGN_CENTER_HORIZONTAL |
						wxALIGN_CENTER_VERTICAL,
		1);
	...
	FlexGridSizer15->Add(
		Button7, 1, wxALL | wxFIXED_MINSIZE | wxALIGN_CENTER_HORIZONTAL |
						wxALIGN_CENTER_VERTICAL,
		1);
	...
	FlexGridSizer15->Add(
		Button8, 1, wxALL | wxFIXED_MINSIZE | wxALIGN_CENTER_HORIZONTAL |
						wxALIGN_CENTER_VERTICAL,
		1);
	...
	FlexGridSizer15->Add(
		Button9, 1, wxALL | wxFIXED_MINSIZE | wxALIGN_CENTER_HORIZONTAL |
						wxALIGN_CENTER_VERTICAL,
		1);
	...
	memo = new wxTextCtrl(
		...
		wxSize(327, 140), wxTE_MULTILINE | wxTE_READONLY | wxTE_WORDWRAP |
...
bool xRawLogViewerFrame::AskForOpenRawlog(std::string& fil)
{
	...
	wxString wildcard = wxT(
		"RawLog files (*.rawlog,*.rawlog.gz)|*.rawlog;*.rawlog.gz|All files "
		"(*.*)|*.*");
...
void xRawLogViewerFrame::loadRawlogFile(const string& str, int first, int last)
{
	...
	while (keepLoading)
	{
		...
		catch (std::bad_alloc&)
		{
			...
			wxString s = _(
				"OUT OF MEMORY: The last part of the rawlog has been freed to "
				"allow the program to continue.\n");
#if MRPT_WORD_SIZE == 32
			s << _(
				"  This is a 32bit machine, so the maximum memory available is "
				"2Gb "
				"despite of the real RAM installed.");
#endif
			wxMessageBox(s);
	...
	if (!rawlog.size())
	{
		wxMessageBox(
			_("If the file is not really empty, perhaps the format is invalid "
			  "or a "
			  "more recent\nversion of RawLogViewer is required to load the "
		...
		switch (idx % N_TIPS)
		{
			case 0:
				return _(
					...
					"portion "
					"of it, or use the 'decimation' slide.");
			case 1:
				return _(
			...
			case 3:
				return _(
					...
					"a "
					"summary of the log, including approximate overall "
					"distance "
					"traveled by the vehicle, overall time, and the frequency "
					"of each "
					"sensor.");
			case 4:
				return _(
				...
				for (unsigned int k = 0; k < sf->size(); k++)
				{
						...
						CLASS_ID(CObservation2DRangeScan))
					{
						...
						std::pair<FILE*, std::pair<FILE*, FILE*>>* files =
							nullptr;
						std::map<string,
								 std::pair<FILE*, std::pair<FILE*, FILE*>>>::
							iterator it;
			 ...
			 it != rawlog.end() && keepDoing; it++, countLoop++)
		{
			...
			while (true)
			{
				...
				if (obs)
				{
					...
					std::map<std::string,
							 mrpt::obs::CObservationImage>::iterator c =
