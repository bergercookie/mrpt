...
CFormRawMap::CFormRawMap(wxWindow* parent, wxWindowID)
{
	...
	StaticText5 = new wxStaticText(
		Panel2, ID_STATICTEXT7, wxEmptyString, wxDefaultPosition, wxDefaultSize,
		0, _T("ID_STATICTEXT7"));
	FlexGridSizer3->Add(
		StaticText5, 1,
	...
	StaticText2 = new wxStaticText(
		Panel2, ID_STATICTEXT5, wxEmptyString, wxDefaultPosition, wxDefaultSize,
		0, _T("ID_STATICTEXT5"));
	FlexGridSizer3->Add(
		StaticText2, 1,
		wxALL | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL, 5);
	StaticText1 = new wxStaticText(
		Panel2, ID_STATICTEXT1, _("First entry:"), wxDefaultPosition,
	...
	FlexGridSizer3->Add(
		edFirst, 1, wxALL | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL,
		5);
	StaticText3 = new wxStaticText(
		Panel2, ID_STATICTEXT3, _("Last entry:"), wxDefaultPosition,
	...
	FlexGridSizer3->Add(
		edLast, 1, wxALL | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL,
		5);
	StaticText6 = new wxStaticText(
		Panel2, ID_STATICTEXT10, _("Decimation:"), wxDefaultPosition,
	...
	FlexGridSizer11->Add(
		lbCount, 1, wxALL | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL,
		5);
	lbLength = new wxStaticText(
		Panel2, ID_STATICTEXT2, wxEmptyString, wxDefaultPosition, wxDefaultSize,
		0, _T("ID_STATICTEXT2"));
	FlexGridSizer11->Add(
		lbLength, 1,
		wxALL | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL, 5);
			 ...
			 it != robot_path.end(); ++it)
		{
			...
			else if (last_t != INVALID_TIMESTAMP)
			{
				...
				if (i1 != rtk_path_info.best_gps_path.end())
				{
					...
					{
						obj2->appendLine(
							x0, y0, z0, t->second.x, t->second.y, t->second.z);
						x0 = t->second.x;
						y0 = t->second.y;
						z0 = t->second.z;
					}
				...
				case CRawlog::etActionCollection:
				{
					...
					for (size_t j = 0; j < acts->size(); j++)
					{
							...
							CLASS_ID(CActionRobotMovement2D))
						{
							...
							// Load an odometry estimation, but only if it is
							// the only
							// movement
							//  estimation source: any other may be a better
							//  one:
							if (!poseIncrementLoaded ||
								mov->estimationMethod !=
									CActionRobotMovement2D::emOdometry)
...
void CFormRawMap::OnGenerateFromRTK(wxCommandEvent&)
{
	...
	wxProgressDialog progDia2(
		wxString::Format(
			wxT("Building map - %u GPS points"), (unsigned)robot_path.size()),
...
void CFormRawMap::OnbtnSavePathClick(wxCommandEvent&)
{
			 ...
			 i != robot_path.end(); ++i)
		{
			...
			if (Wit != rtk_path_info.vehicle_uncertainty.end())
			{
				...
				f.printf(
					...
					C(1, 1), C(1, 2), C(1, 3), C(1, 4), C(1, 5), C(2, 2),
					C(2, 3), C(2, 4), C(2, 5), C(3, 3), C(3, 4), C(3, 5),
					C(4, 4), C(4, 5), C(5, 5));
			}
			else
				f.printf(
					...
					1e-4, 0.0, 0.0, 0.0, 1e-4, 0.0, 0.0, 1e-4, 0.0, 1e-4);
		}
	...
	{
		wxFileDialog dialog(
			this, wxT("Save path as binary file..."), defaultDir,
	...
	// ---------------------------------------------
	{
		wxFileDialog dialog(
			...
			wxT("GT_path_vehicle_interp.txt"),
			wxT("Text files (*.txt)|*.txt|All files (*.*)|*.*"),
			wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
	...
	if (!rtk_path_info.mahalabis_quality_measure.empty())
	{
		wxFileDialog dialog(
			...
			wxT("GT_path_vehicle_confidence.txt"),
			wxT("Text files (*.txt)|*.txt|All files (*.*)|*.*"),
			wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
		if (dialog.ShowModal() != wxID_OK) return;
					 ...
					 itStr != the_labels.end(); itStr++, itOutFiles++)
				{
					if (obs->sensorLabel == *itStr)
					{
						...
						CPose3DPDFGaussian global_sensor_pose =
							veh_pose + sensor_on_the_vehicle;  // Global sensor
						// pose with
						// uncert.
						...
						// save the composed path:
						{
							...
							mrpt::system::os::fprintf(
								*itOutFiles,
								...
								C(0, 5), C(1, 1), C(1, 2), C(1, 3), C(1, 4),
								C(1, 5), C(2, 2), C(2, 3), C(2, 4), C(2, 5),
								C(3, 3), C(3, 4), C(3, 5), C(4, 4), C(4, 5),
								C(5, 5));
						}
					}  // end if
				}  // end for
