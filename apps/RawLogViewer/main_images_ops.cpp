      ...
      case CRawlog::etObservation: {
        ...
        if (IS_CLASS(o, CObservationStereoImages)) {
          ...
          obsSt->imageLeft.saveToFile(format(
              ...
              obsSt->sensorLabel.c_str(), imgSaved, imgFileExtension.c_str()));
          ...
          obsIm->image.saveToFile(format(
              ...
              obsIm->sensorLabel.c_str(), imgSaved, imgFileExtension.c_str()));
          imgSaved++;
        }
      } break;
...
void xRawLogViewerFrame::OnMenuMono2Stereo(wxCommandEvent &event) {
  ...
  wxString sNewLabel =
      wxGetTextFromUser(_("New stereo observation label:"),
                        _("Stereo observations"), _("STEREO_CAM"), this);
  ...
  for (unsigned int countLoop = 0; countLoop < nEntries; countLoop++) {
    ...
    case CRawlog::etObservation: {
      // CObservation::Ptr o = rawlog.getAsObservation(countLoop);
      THROW_EXCEPTION("Operation not implemented for observations-only "
                      "rawlogs. Please convert into SensoryFrame-based first.");
    } break;
...
*/
    ...
    try {
        ...
        for (unsigned int k = 0; k < SF->size(); k++) {
          if (SF->getObservationByIndex(k)->GetRuntimeClass() ==
            ...
            if (obsIm->cameraParams.k1() != 0 ||
              ...
              // Set distortion parameters to zero -> indicating that the image
              // is now rectified
      ...
      case CRawlog::etObservation: {
        ...
        if (IS_CLASS(o, CObservationImage)) {
          ...
          if (obsIm->cameraParams.k1() != 0 || obsIm->cameraParams.k2() != 0 ||
              obsIm->cameraParams.p1() != 0 || obsIm->cameraParams.p2() != 0) {
            string p;
            obsIm->image.getExternalStorageFileAbsolutePath(p);

            obsIm->image.rectifyImageInPlace(obsIm->cameraParams);

            // Set distortion parameters to zero -> indicating that the image is
            // now rectified
            obsIm->cameraParams.setDistortionParamsFromValues(0, 0, 0, 0);
  ...
  // Set error msg:
  wxMessageBox(_U(format("Images rectified: %i", N).c_str()), _("Done"), wxOK,
               this);
...
void renameExternalImageFile(CObservationImage::Ptr o) {
  ...
  if (imgFileExistsNow) { // Rename the actual file:
    string strErr;
...
void xRawLogViewerFrame::OnMenuRenameImageFiles(wxCommandEvent &event) {
  ...
  wxString auxStr;
  wxProgressDialog progDia(wxT("Progress"), wxT("Processing rawlog..."),
                           nEntries, // range
                           this,     // parent
                           wxPD_CAN_ABORT | wxPD_APP_MODAL | wxPD_SMOOTH |
                               wxPD_AUTO_HIDE | wxPD_ELAPSED_TIME |
    ...
    try {
      ...
      case CRawlog::etSensoryFrame: {
        ...
        for (unsigned int k = 0; k < SF->size(); k++) {
          ...
          else if (IS_CLASS(SF->getObservationByIndex(k),
