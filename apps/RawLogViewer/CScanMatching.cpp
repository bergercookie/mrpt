...
CScanMatching::CScanMatching(wxWindow* parent, wxWindowID)
{
	...
	StaticText2 = new wxStaticText(
		Panel1, ID_STATICTEXT2, _("Enter the rawlog indexes of the "
	...
	edOptRefPnt = new wxTextCtrl(
		...
		_("; Configuration for CPointsMap::TInsertionOptions - See MRPT "
		  ...
		  "The "
		  "minimum distance between points (in 3D): If two points are too "
		  "close, "
		  ...
		  "false).\ndisableDeletion=1 ; If set to false (default) points in "
		  "the "
	...
	edOptRefGrid = new wxTextCtrl(
		...
		_("; Configuration for COccupancyGridMap2D::TInsertionOptions - See "
		  ...
		  "--------------------------------------------------------------------"
		  "--"
		  "-----------\n[Construction]\nresolution=0.05 ; The grid cell size, "
		  "in "
		  ...
		  "inserted in the map.\nmaxDistanceInsertion=40 ; The maximum "
		  "distance "
	...
	plotMaps = new mpWindow(
		Panel3, ID_CUSTOM1, wxDefaultPosition, wxSize(341, 359), 0);
	FlexGridSizer9->Add(
		plotMaps, 1,
		wxALL | wxEXPAND | wxFIXED_MINSIZE | wxALIGN_LEFT | wxALIGN_TOP, 0);
	Panel5 = new wxPanel(
...
void CScanMatching::OnbtnICPClick(wxCommandEvent&)
{
	...
	if (useGridMap)
	{
		...
		lyRefGrid->SetBitmap(
			*newBmp, refMapGrid.getXMin(), refMapGrid.getYMin(), lx, ly);
...

	{
		...
		while (curStep <= maxSteps)
		{
			...
			lyNewRobotPose->SetCoordinateBase(
				estMean.x(), estMean.y(), estMean.phi());

			lyEstimateEllipse->SetCoordinateBase(estMean.x(), estMean.y());
			lyEstimateEllipse->SetCovarianceMatrix(
				estCov(0, 0), estCov(0, 1), estCov(1, 1));
			...
			cout << " std(phi) = " << RAD2DEG(sqrt(estCov(2, 2))) << " deg. "
				 << endl;

			cout << format(
				"Output PDF class is: %s\n",
				poseEst->GetRuntimeClass()->className);
			if (poseEst->GetRuntimeClass() == CLASS_ID(CPosePDFSOG))
			{
				CPosePDFSOG::Ptr SOG =
					std::dynamic_pointer_cast<CPosePDFSOG>(poseEst);
				size_t i, n = SOG->size();
				cout << format("# of gaussians in SOG: %i\n", (int)n);
				for (i = 0; i < n; i++)
				{
					cout << format(
								"SOG[%02i]:w=%e mean=", (int)i,
								SOG->get(i).log_w)
						 << SOG->get(i).mean << endl;
				}
			}
...
void CScanMatching::OnbtnHelpClick(wxCommandEvent&)
{
	wxString s;
	s << _(
		"Scan matching tries to register two maps. Each of them is created by "
		"inserting observations,\n");
	s << _(
		" typically laser scans, into point or grid maps. You must enter here "
		"the indexes of the observations\n");
	s << _(
		" to be inserted in each map. You can select just one index, or "
		"several "
		"ones separated by spaces.\n");
