...
void parseGeneralVector(const std::string &str, size_t idx,
...
void xRawLogViewerFrame::OnImportCARMEN(wxCommandEvent &event) {
  ...
  iniFile->write(iniFileSect, "LastDir",
                 string(dialog.GetDirectory().mb_str()));
  WX_END_TRY
  ...
  wxString strMaxLaserRange = wxGetTextFromUser(_("Maximum valid laser range:"),
                                                _("Enter parameter:"), _("30"));
  ...
  rawlog.clear();
  wxProgressDialog progDia(_("Importing rawlog..."), _("Processing..."),
                           n,    // range
                           this, // parent
  ...
  while (i < n && !end) {
    ...
    if (isFrontLaser || isRearLaser) {
      ...
      obsScan->sensorPose =
          isFrontLaser ? CPose3D(frontLaserPose) : CPose3D(rearLaserPose);
...
void xRawLogViewerFrame::OnImportSequenceOfImages(wxCommandEvent &event) {
  ...
  if (rawlog.size())
    if (wxYES != wxMessageBox(_("This will overwrite your currently loaded "
                                "rawlog. Proceed anyway?"),
                              _("Import rawlog"), wxYES_NO, this))
      return;

  // Select directory:
  wxMessageBox(_("Please, select a directory which contain the images "
                 "(*.bmp,*.jpg,*.tif,*.png,...) to be imported\n They will be "
                 "loaded by filename ascending order."),
               _("Import rawlog"), wxOK, this);

  wxDirDialog dirDialog(this,
                        _("Choose the directory that contains the images"),
  ...
  wxString strFPS =
      wxGetTextFromUser(_("Provide frame rate (fps, or Hz):"),
  ...
  wxString auxStr;
  wxProgressDialog progDia(wxT("Progress"), wxT("Parsing files..."),
                           nEntries, // range
                           this,     // parent
                           wxPD_CAN_ABORT | wxPD_APP_MODAL | wxPD_SMOOTH |
                               wxPD_AUTO_HIDE | wxPD_ELAPSED_TIME |
  ...
  wxMessageBox(_U(format("%u images have been imported to rawlog format",
...
void xRawLogViewerFrame::OnMenuExportALOG(wxCommandEvent &event) {
  ...
  if (dialog.ShowModal() == wxID_OK) {
    ...
    wxString strRobot =
        wxGetTextFromUser(_("Enter the name of the robot to use in ALOG:"),
                          _("Robot name:"), _("ROBOT"));
    ...
    wxProgressDialog progDia(wxT("Exporting rawlog to ALOG"), wxT("Saving..."),
                             n,    // range
                             this, // parent
                             wxPD_CAN_ABORT | wxPD_APP_MODAL | wxPD_SMOOTH |
                                 wxPD_AUTO_HIDE | wxPD_ELAPSED_TIME |
    ...
    for (i = 0; i < n; i++) {
      ...
      if (rawlog.getType(i) == CRawlog::etActionCollection &&
        ...
        if (act) {
          ...
          CPose2D MOOS_odometry(-globalOdometry.y(), globalOdometry.x(),
                                globalOdometry.phi());
          ...
          ::fprintf(f, "%f   %s_ODOMETRY   iPlatform    "
                       "time=%f,Pose=[3x1]{%.03f,%.03f,%.03f}\n",
                    time_odo, robot_name.c_str(), time_odo, MOOS_odometry.x(),
                    MOOS_odometry.y(), MOOS_odometry.phi());
        }
        ...
        if (obs) {
          ...
          if (firstTime == INVALID_TIMESTAMP)
            firstTime = lastTime;
          ...
          ::fprintf(f, "%f    LMS_LASER_2D     LMS2D  "
                       "time=%f,elevation=0.000000,endElevation=0.000000,"
                       "angRes=1.00,offset=0.00,subRange=0,minAngle=%.02f,"
                       "maxAngle=%.02f,range=[%u]{",
                    time_obs, time_obs, RAD2DEG(-obs->aperture / 2),
                    RAD2DEG(obs->aperture / 2), 181);
        ...
        THROW_EXCEPTION(
            "Export to ALOG only implemented for SF-based rawlogs.");
    ...
    while (!f.eof()) {
      ...
      if (strLine.size() > 3) {
        if (strLine[0] == 'P') {
          //					size_t idx;
          //					if ((idx =
          //strLine.find("Max_scan_distance"))!=strLine.npos )
          //					{
          //						max_laser_range = atof( strLine.c_str() +
          //idx + strlen("Max_scan_distance") + 1 );
          //					}
          // else if
        } else if (strLine[0] == 'O') {
          ...
          if (rawdata.size() > 10) {
            ...
            for (size_t i = 0; i < N; i++) {
              float rng =
                  sqrt(square(rawdata[1 + i * 4 + 2] - rawdata[1 + i * 4 + 0]) +
                       square(rawdata[1 + i * 4 + 3] - rawdata[1 + i * 4 + 1]));
...
// [N]={X1,X2,...}
void parseMOOSVector(const std::string &str, size_t idx,
                     vector<float> &outVector) {
  ASSERT_(str[idx] == '[');
...
void xRawLogViewerFrame::OnMenuImportALOG(wxCommandEvent &event) {
      ...
      if (idx != string::npos) {
        ...
        if (string::npos != (idx = strLine.find("_ODOMETRY"))) {
          if (string::npos != (idx = strLine.find("Pose=[3x1]{"))) {
            newRecord.data.resize(6);
            if (3 == sscanf(strLine.c_str() + idx + 11, "%f,%f,%f",
                            &newRecord.data[0], &newRecord.data[1],
                            &newRecord.data[2])) {
              if (string::npos != (idx = strLine.find("Vel=[3x1]{"))) {
                if (3 == sscanf(strLine.c_str() + idx + 10, "%f,%f,%f",
        ...
        } else if (string::npos != strLine.find("LMS_LASER_2D") ||
                   string::npos != strLine.find("LMS_LASER_3D")) {
      ...
      if (string::npos != strLine.find("CAMERA_GRAB")) {
        if (string::npos != (idx = strLine.find(".jpg"))) {
          ...
          if (idx_slash != string::npos) {
            ...
            //  ^       ^
            newRecord.imgFile =
                strLine.substr(idx_slash + 1, idx + 3 - idx_slash);
            newRecord.type = 3;
            ...
            if (string::npos != idx_time) {
              ...
              if (idx_pan != string::npos) {
                ...
                newRecord.data[0] =
                    DEG2RAD(atof(strLine.c_str() + idx_pan + 4));
  ...
  string dir_for_images(".");
  if (nImgs) {
    ::wxMessageBox(_("The alog contains images. Please pick next the directory "
    ...
    if (wxYES ==
        wxMessageBox(_("No images found in the alog. Do you want to provide an "
                       "additional directory with image files?"),
      ...
      if (dirDialog.ShowModal() == wxID_OK) {
        ...
        CDirectoryExplorer::TFileInfoList lstFiles;
        CDirectoryExplorer::explore(dir_for_images, FILE_ATTRIB_ARCHIVE,
                                    lstFiles);

        CDirectoryExplorer::filterByExtension(lstFiles, "jpg");
        CDirectoryExplorer::sortByName(lstFiles);

        for (CDirectoryExplorer::TFileInfoList::iterator it = lstFiles.begin();
             it != lstFiles.end(); ++it) {
...
void xRawLogViewerFrame::saveImportedLogToRawlog(
  ...
  wxProgressDialog progDia(wxT("Importing rawlog"), wxT("Importing..."),
                           progress_N, // range
                           this,       // parent
                           wxPD_CAN_ABORT | wxPD_APP_MODAL | wxPD_SMOOTH |
                               wxPD_AUTO_HIDE | wxPD_ELAPSED_TIME |
  ...
  for (progress_i = 0, it = theAlog.begin(); it != theAlog.end(); it++) {
    ...
    if (it->second.type != 0) {
      ...
      case 2: {
        ...
        obs->sensorPose.setFromValues(0, 0, 0, 0,
                                      -DEG2RAD(it->second.startElev), 0);
      } break;
      ...
      case 3: {
        ...
        if (fileExists(img_file)) {
          ...
          obs->cameraPose.setFromValues(0, 0, 0, it->second.data[0], 0, 0);
      ...
      CPose2D curOdo(it->second.data[0], it->second.data[1],
  ...
  if (dialog.ShowModal() == wxID_OK) {
    ...
    if (!genTimes)
      ::wxMessageBox(_("It seems that there are no valid timestamps in the "
                       "rawlog. Timestamps will be zero."));
    ...
    for (i = 0; i < n; i++) {
      ...
      case CRawlog::etObservation: {
        ...
        if (IS_CLASS(o, CObservationWirelessPower)) {
          obs = std::dynamic_pointer_cast<CObservationWirelessPower>(
              o); // Get the GAS observation
        }
      } break;
      ...
      if (obs) {
        ...
        /*	for (size_t j=0;j<obs->m_readings.size();j++)
           ...
           it=obs->m_readings[j].readingsVoltage.begin();it!=obs->m_readings[j].readingsVoltage.end();++it)
                        {
                                ::fprintf(f,"%f ",*it);
                                lineCount++;
                        }
...
void xRawLogViewerFrame::OnMenuItemImportBremenDLRLog(wxCommandEvent &event) {
  ...
  wxString wildcard = wxT("Uni-bremen DLR circles dataset "
                          "(*.circles)|*.circles|All files (*.*)|*.*");
  wxString defaultDir(
      _U(iniFile->read_string(iniFileSect, "LastDir", ".").c_str()));
  ...
  http://www.informatik.uni-bremen.de/agebv/en/DlrSpatialCognitionDataSet
    ...
    CIRCLEMARKDETECTOR <cameraCalibrationFile> <useHalfImages> <radius>
  <threshold>
  ...
  Defines the parameters for the circular fiducial detector.
     ...
     frame image and it's y resolution must be divided by two to match the
  images.

     <radius> metrical radius of the fiducials (meter)

     <threshold> the visual fiducial detector assigns a quality measure between
  [0..1]
     to every fiducial found. Only those above <threshold> are passed to the
  SLAM algorithm.

    STEP dlr-spatial_cognition-c.0000 -0.03752 -0.10467 -0.10807 0.000014396673
  -0.000000184001 0.000013237216 0.000015157225 -0.000001814284 0.000246181200
    STEP <image> <dX> <dY> <dTheta>, <cXX>, <cXY>, <cYY>, <cXTheta>, <cYTheta>,
  <cThetaTheta>

  Defines a single step of the robot trajectory, that is an odometry measurement
  plus an image. Such
  a line defines, that the robot first moved by <dX>, <dY>, <dTheta> relative to
  it's previous pose and
  then took the image saved in <image>. <cXX>...<cThetaTheta> are the
  covariances of <dX>, <dY>, <dTheta>.
  ...
  */
  ...
  const bool use_SF_format =
      (wxYES == wxMessageBox(_("Use Actions-SensoryFrames format (YES) or the "
                               "Observation-only format (NO)?"),
                             _("Import rawlog"), wxYES_NO, this));

  const bool use_ground_truth_IDs =
      (wxYES == wxMessageBox(_("Employ landmark IDs in file (YES) or make the "
                               "sensor unable to identify any landmarks (NO)?"),
                             _("Import rawlog"), wxYES_NO, this));
  ...
  CSensoryFrame set_of_obs; // Set of observations after the last odometry;
                            // saved BEFORE processing the next odometry.
  ...
  int32_t next_outlier_ID = 10000; // In DLR datasets no real landmark has such
                                   // a high ID... use these numbers for
                                   // outliers.
  ...
  while (fileParser.getNextLine(line)) {
    ...
    if (words[0] == "STEP") {
      ...
      if (set_of_obs.empty()) {
        // Always create an range-bearing observation, for the cases of images
        // without any detected landmark
      ...
      //  -0.000001814284 0.000246181200
      //  STEP <image> <dX> <dY> <dTheta>, <cXX>, <cXY>, <cYY>, <cXTheta>,
      //  <cYTheta>, <cThetaTheta>
      ...
      odoParams.gaussianModel.a1 = odoParams.gaussianModel.a2 =
          odoParams.gaussianModel.a3 = odoParams.gaussianModel.a4 = 0;
      odoParams.gaussianModel.minStdXY =
          std::sqrt(atof(words[5].c_str()) + atof(words[7].c_str()));
      ...
      // LANDMARK_C 0.67972 -2.87676 0.600235 0.00176143 -0.000314459 0.00334762
      // -1
      ...
      const double lm_x = -atof(words[2].c_str()); // Ey! Yes, I flipped the
                                                   // coord. system to match
                                                   // MRPT's standard
      const double lm_y =
          atof(words[1].c_str()); //  of +X pointing fordward, +Y to the left.
      ...
      const double dat_C_xy[3 * 3] = {Cxy_xx, Cxy_xy, 0, Cxy_xy, Cxy_yy,
                                      0,      0,      0, 0};
      const CMatrixDouble33 C_xy(dat_C_xy);

      // Jacobian:
      const double dat_H[3 * 3] = {
          lm_x / r, lm_y / r, 0, -lm_y / (r * r), lm_x / (r * r), 0, 0, 0, 0};
      ...
      if (use_ground_truth_IDs) {
        ...
        if (ID_in_file < 0) {
          ...
          //  assign a unique ID in a range that will not collide with real
          //  landmarks:
  ...
  if (stats_stdRanges.size() != 0) {
    ...
    cout << "[ImportBremenDLRLog] Computed stdRange (m): mean= "
         << stdRange_mean << " std= " << stdRange_std << endl;
    cout << "[ImportBremenDLRLog] Computed stdYaw (deg): mean= "
         << RAD2DEG(stdYaw_mean) << " std= " << RAD2DEG(stdYaw_std) << endl;
    ...
    cout << "[ImportBremenDLRLog] Computed stdX (m): mean= " << stdX_mean
         << " std= " << stdX_std << endl;
    cout << "[ImportBremenDLRLog] Computed stdY (m): mean= " << stdY_mean
         << " std= " << stdY_std << endl;
  }
...
void xRawLogViewerFrame::OnGenerateIMUTextFile(wxCommandEvent &event) {
  ...
  if (dialog.ShowModal() == wxID_OK) {
    ...
    ::fprintf(f, "%% TIMESTAMP IMU_X_ACC IMU_Y_ACC IMU_Z_ACC IMU_YAW_VEL "
                 "IMU_PITCH_VEL IMU_ROLL_VEL IMU_X_VEL IMU_Y_VEL IMU_Z_VEL "
                 "IMU_YAW IMU_PITCH IMU_ROLL IMU_X IMU_Y IMU_Z MAG_X MAG_Y "
                 "MAG_Z PRESS ALTIT TEMP\n"
                 "%% "
                 "-------------------------------------------------------------"
                 "-------------------------------------------------------------"
                 "-------------------------------\n");
    ...
    for (i = 0; i < n; i++) {
        ...
        do {
          ...
          if (obs) {
            ...
            if (obs->rawMeasurements.size() > 0) {
              ...
              ASSERT_(obs->rawMeasurements.size() ==
                      obs->rawMeasurements.size());
              for (size_t idx = 0; idx < nValuesPerRow; idx++)
                ::fprintf(f, "%f ", obs->dataIsPresent[idx]
      ...
      case CRawlog::etObservation: {
        ...
        if (oo->GetRuntimeClass() == CLASS_ID(CObservationIMU)) {
          CObservationIMU::Ptr obs =
              std::dynamic_pointer_cast<CObservationIMU>(oo);
          // For each entry in this sequence:
          if (obs->rawMeasurements.size() > 0) {
            ASSERT_(obs->dataIsPresent.size() == obs->rawMeasurements.size());
            size_t nValuesPerRow = obs->dataIsPresent.size();
    ...
    wxMessageBox(_U(format("%u IMU data entries saved!", M).c_str()), _("Done"),
                 wxOK, this);
  }
  ...
  if (dialog.ShowModal() == wxID_OK) {
    ...
    for (i = 0; i < n; i++) {
      if (rawlog.getType(i) == CRawlog::etSensoryFrame) {
        CSensoryFrame::Ptr sf = rawlog.getAsObservations(i);
        CObservationBearingRange::Ptr obs =
            sf->getObservationByClass<CObservationBearingRange>();
        if (obs) {
          // For each entry in this sequence:
          for (size_t q = 0; q < obs->sensedData.size(); q++) {
            M++;
            ::fprintf(f, "%u %i %f %f %f\n", i, obs->sensedData[q].landmarkID,
    ...
    wxMessageBox(_U(format("%u bearing-range data entries saved!", M).c_str()),
                 _("Done"), wxOK, this);
  }
