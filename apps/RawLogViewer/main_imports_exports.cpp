...
void parseGeneralVector(
	const std::string& str, size_t idx, vector<float>& outVector);
...
void xRawLogViewerFrame::OnImportCARMEN(wxCommandEvent& event)
{
	...
	WX_START_TRY
	iniFile->write(
		iniFileSect, "LastDir", string(dialog.GetDirectory().mb_str()));
	WX_END_TRY
	...
	while (i < n && !end)
	{
		...
		if (isFrontLaser || isRearLaser)
		{
			...
			if (isRearLaser) thisOdoReading.phi(-thisOdoReading.phi());
...
void xRawLogViewerFrame::OnImportSequenceOfImages(wxCommandEvent& event)
{
	...
	// Select directory:
	wxMessageBox(
		_("Please, select a directory which contain the images "
	...
	wxDirDialog dirDialog(
		this, _("Choose the directory that contains the images"), _("."), 0,
		wxDefaultPosition);
	...
	wxString auxStr;
	wxProgressDialog progDia(
		wxT("Progress"), wxT("Parsing files..."),
		nEntries,  // range
		this,  // parent
		wxPD_CAN_ABORT | wxPD_APP_MODAL | wxPD_SMOOTH | wxPD_AUTO_HIDE |
			wxPD_ELAPSED_TIME | wxPD_ESTIMATED_TIME | wxPD_REMAINING_TIME);
		...
		try
		{
				...
				!os::_strcmpi("ras", filExt.c_str()))
			{
				...
				im->cameraParams.intrinsicParams(0, 2) =
					im->image.getWidth() / 2;
				im->cameraParams.intrinsicParams(1, 2) =
					im->image.getHeight() / 2;
...
void xRawLogViewerFrame::OnMenuExportALOG(wxCommandEvent& event)
{
	...
	if (dialog.ShowModal() == wxID_OK)
	{
		...
		wxString strRobot = wxGetTextFromUser(
			_("Enter the name of the robot to use in ALOG:"), _("Robot name:"),
			_("ROBOT"));
		...
		for (i = 0; i < n; i++)
		{
				...
				lastTime != INVALID_TIMESTAMP)
			{
				...
				{
					globalOdometry =
						globalOdometry + act->poseChange->getMeanVal();

					// MOOS uses a different coordinate system:
					CPose2D MOOS_odometry(
						-globalOdometry.y(), globalOdometry.x(),
						globalOdometry.phi());
					...
					::fprintf(
						f,
						"%f   %s_ODOMETRY   iPlatform    "
						"time=%f,Pose=[3x1]{%.03f,%.03f,%.03f}\n",
						time_odo, robot_name.c_str(), time_odo,
						MOOS_odometry.x(), MOOS_odometry.y(),
						MOOS_odometry.phi());
				}
			}
			else if (rawlog.getType(i) == CRawlog::etSensoryFrame)
			{
				...
				if (obs)
				{
					...
					::fprintf(
						f,
						"%f    LMS_LASER_2D     LMS2D  "
						"time=%f,elevation=0.000000,endElevation=0.000000,"
						"angRes=1.00,"
						"offset=0.00,subRange=0,minAngle=%.02f,maxAngle=%.02f,"
						"range=[%u]"
						"{",
...
void xRawLogViewerFrame::OnMenuImportALOG(wxCommandEvent& event)
{
		...
		while (!f.eof())
		{
			...
			if (idx != string::npos)
			{
				...
				if (string::npos != (idx = strLine.find("_ODOMETRY")))
				{
					...
					string::npos != strLine.find("LMS_LASER_2D") ||
					string::npos != strLine.find("LMS_LASER_3D"))
				{
					if (string::npos != (idx = strLine.find("endElevation=")))
					{
						...
						{
							newRecord.startElev =
								atof(strLine.c_str() + idx + 10);

							if (string::npos != (idx = strLine.find("range=")))
							{
								parseMOOSVector(
									strLine, idx + 6, newRecord.data);
								if (string::npos !=
									strLine.find("LMS_LASER_2D"))
								{
									newRecord.type = 1;
									newRecord.label = "LASER_2D";
								}
	...
	{
		::wxMessageBox(
			_("The alog contains images. Please pick next the directory "
			  "containing "
			  "all the images."));
		wxDirDialog dirDialog(
			this,
	...
	else
	{
		...
		if (wxYES ==
			wxMessageBox(
				_("No images found in the alog. Do you want to provide an "
				...
				_("Additional images?"), wxYES_NO))
		{
			...
			if (dirDialog.ShowModal() == wxID_OK)
			{
				...
				CDirectoryExplorer::explore(
					dir_for_images, FILE_ATTRIB_ARCHIVE, lstFiles);
		...
		if (it->second.type != 0)
		{
			...
			switch (it->second.type)
			{
				...
				case 2:
				{
					...
					obs->deltaPitch =
						-DEG2RAD(it->second.endElev - it->second.startElev);

					obs->sensorPose.setFromValues(
						0, 0, 0, 0, -DEG2RAD(it->second.startElev), 0);
				}
				break;
				...
				case 3:
				{
					...
					// ----------------
					string img_file =
						dir_for_images + string("/") + it->second.imgFile;
			...
			if (newObs)
			{
				...
				if (!sfAccum.size()) firstSFtime = tim;
				...
				if (genTimes)
				{
					if (obs->timestamp != INVALID_TIMESTAMP)
						t = mrpt::system::timeDifference(
							rawlog_first_timestamp, obs->timestamp);
				}
		...
		for (i = 0; i < n; i++)
		{
			...
			switch (rawlog.getType(i))
			{
				case CRawlog::etSensoryFrame:
				{
					CSensoryFrame::Ptr sf = rawlog.getAsObservations(i);
					obs =
						sf->getObservationByClass<CObservationWirelessPower>();
				}
				break;
...
void xRawLogViewerFrame::OnMenuItemImportBremenDLRLog(wxCommandEvent& event)
{
	...
	images.
	   ...
	   <threshold> the visual fiducial detector assigns a quality measure
	between
	[0..1]
	...
	Defines a single step of the robot trajectory, that is an odometry
	measurement
	plus an image. Such
	a line defines, that the robot first moved by <dX>, <dY>, <dTheta> relative
	to
	...
	*/
	...
	int32_t next_outlier_ID =
		10000;  // In DLR datasets no real landmark has such
	...
	while (fileParser.getNextLine(line))
	{
		...
		if (words[0] == "STEP")
		{
			...
			if (set_of_obs.empty())
			{
				// Always create an range-bearing observation, for the cases of
				// images
			...
			odoParams.gaussianModel.minStdPHI =
				std::sqrt(atof(words[10].c_str()));
		...
		else if (words[0] == "LANDMARK_C")
		{
			...
			const double lm_x =
				-atof(words[2].c_str());  // Ey! Yes, I flipped the
			...
			const double dat_H[3 * 3] = {
				lm_x / r, lm_y / r, 0, -lm_y / (r * r), lm_x / (r * r), 0,
				0,		  0,		0};
			...
			if (use_ground_truth_IDs)
			{
				...
				if (ID_in_file < 0)
				{
					// If the ground-truth says this landmark is an outlier
					// (ID=-1), then
					//  assign a unique ID in a range that will not collide with
					//  real
					//  landmarks:
	...
	// finally, we have to repeat this part not to lost the last few
	// observations:
	if (!set_of_obs.empty())
	{
		if (use_SF_format)
...
void xRawLogViewerFrame::OnGenerateIMUTextFile(wxCommandEvent& event)
{
	...
	if (dialog.ShowModal() == wxID_OK)
	{
		...
		::fprintf(
			f,
			"%% TIMESTAMP IMU_X_ACC IMU_Y_ACC IMU_Z_ACC IMU_YAW_VEL "
			"IMU_PITCH_VEL "
			"IMU_ROLL_VEL IMU_X_VEL IMU_Y_VEL IMU_Z_VEL IMU_YAW IMU_PITCH "
			"IMU_ROLL "
			"IMU_X IMU_Y IMU_Z MAG_X MAG_Y MAG_Z PRESS ALTIT TEMP\n"
			"%% "
			"------------------------------------------------------------------"
			"----"
			"------------------------------------------------------------------"
				...
				case CRawlog::etSensoryFrame:
				{
					...
					do
					{
						...
						if (obs)
						{
							...
							{
								ASSERT_(
									obs->dataIsPresent.size() ==
									obs->rawMeasurements.size());
								size_t nValuesPerRow =
									obs->dataIsPresent.size();

								// For each entry in this sequence: Compute the
								// timestamp and
								// save all 15 values:
