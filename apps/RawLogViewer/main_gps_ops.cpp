...
void xRawLogViewerFrame::OnMenuDrawGPSPath(wxCommandEvent &event) {
  ...
  if (wxYES != wxMessageBox(_("Do you want to take the GPS reference "
                              "automatically from the first found entry?"),
                            _("GPS path"), wxYES_NO)) {
    wxString s = wxGetTextFromUser(_("Reference Latitude (degrees):"),
                                   _("GPS reference"), _("0.0"), this);
    if (s.IsEmpty())
      return;
    ...
    s = wxGetTextFromUser(_("Reference Longitude (degrees):"),
                          _("GPS reference"), _("0.0"), this);
    if (s.IsEmpty())
      return;
    ...
    s = wxGetTextFromUser(_("Reference Height (meters):"), _("GPS reference"),
                          _("0.0"), this);
    if (s.IsEmpty())
      return;
  ...
  for (i = 0; i < n; i++) {
    ...
    if (gga && (!only_rtk || gga->fields.fix_quality == 4 ||
      ...
      if (!xs.empty())
        overall_distance += sqrt(square(X_ENU.x - *xs.rbegin()) +
  ...
  // Window 3d:
  winGPSPath = std::make_shared<CDisplayWindow3D>(
      format("GPS path, %i points (%s) %.03f meters length", int(M),
...
void fixGPStimestamp(CObservationGPS::Ptr &obs, CVectorDouble &time_changes,
...
void xRawLogViewerFrame::OnMenuRegenerateGPSTimestamps(wxCommandEvent &event) {
  ...
  for (i = 0; i < n; i++) {
    ...
    default:
      wxMessageBox(
          _("Error: this command is for rawlogs without sensory frames."));
      return;
      break;
    ...
    case CRawlog::etSensoryFrame: {
      ...
      for (CSensoryFrame::iterator it = sf->begin(); it != sf->end(); ++it) {
        if ((*it)->GetRuntimeClass() == CLASS_ID(CObservationGPS) &&
            find_in_vector((*it)->sensorLabel, the_labels) != string::npos) {
          CObservationGPS::Ptr obs =
  ...
  if (wxYES ==
      wxMessageBox(
          wxString::Format(_("%u changes, average/std time shift is: %f/%f "
                             "sec. Do you want to re-order by timestamp?"),
...
void xRawLogViewerFrame::OnMenuDistanceBtwGPSs(wxCommandEvent &event) {
  ...
  wxMessageBox(
      _("It will be measured the distance between two GPSs, assuming they are "
  ...
  wxString ret = wxGetSingleChoice(_("Choose the first GPS:"),
                                   _("Sensor Labels"), lstLabels, this);
  if (ret.IsEmpty())
    return;

  string gps1 = string(ret.mb_str());

  ret = wxGetSingleChoice(_("Choose the second GPS:"), _("Sensor Labels"),
                          lstLabels, this);
  if (ret.IsEmpty())
    return;
  ...
  for (i = 0; i < n; i++) {
    ...
    if (last_GPS1 && last_GPS2) {
      if (last_GPS1->getMsgByClass<gnss::Message_NMEA_GGA>().fields.UTCTime ==
          last_GPS2->getMsgByClass<gnss::Message_NMEA_GGA>().fields.UTCTime) {
        // Compute distance:
        TPoint3D p1;
        ...
        // Fix offset:
        p1.x += memFil.read_double(string("OFFSET_") + last_GPS1->sensorLabel,
                                   "x", 0);
        p1.y += memFil.read_double(string("OFFSET_") + last_GPS1->sensorLabel,
                                   "y", 0);
        p1.z += memFil.read_double(string("OFFSET_") + last_GPS1->sensorLabel,
                                   "z", 0);

        p2.x += memFil.read_double(string("OFFSET_") + last_GPS2->sensorLabel,
                                   "x", 0);
        p2.y += memFil.read_double(string("OFFSET_") + last_GPS2->sensorLabel,
                                   "y", 0);
        p2.z += memFil.read_double(string("OFFSET_") + last_GPS2->sensorLabel,
                                   "z", 0);
  ...
  if (dists.empty()) {
    wxMessageBox(_("No valid GPS observations were found."), _("Done"), wxOK,
                 this);
  } else {
    double d_mean, d_std;
    mrpt::math::meanAndStd(dists, d_mean, d_std);

    wxMessageBox(_U(format("The distance between GPS sensors is %.04fm, with\n "
  ...
  for (i = 0; i < n; i++) {
      ...
      if (obs)
        if (obs->has_GGA_datum) {
          ASSERT_(
              obs->getMsgByClass<gnss::Message_NMEA_GGA>().fields.fix_quality <=
              8);
          histogramGPSModes[obs->getMsgByClass<gnss::Message_NMEA_GGA>()
                                .fields.fix_quality]++;
        }
    } break;
    ...
    case CRawlog::etObservation: {
      ...
      if (IS_CLASS(o, CObservationGPS)) {
        ...
        if (obs)
          if (obs->has_GGA_datum) {
            ASSERT_(obs->getMsgByClass<gnss::Message_NMEA_GGA>()
...
void xRawLogViewerFrame::OnGenGPSTxt(wxCommandEvent &event) {
  ...
  if (dialog.ShowModal() == wxID_OK) {
    ...
    if (ref_valid) {
      wxMessageBox(
          _("GPS origin coordinates taken from rawlog configuration block"),
          _("Export GPS data"));
    }

    // Ask the user for the reference?
    if (!ref_valid &&
        wxYES != wxMessageBox(_("Do you want to take the GPS reference "
                                "automatically from the first found entry?"),
      ...
      TPose3D _local_ENU;
      mrpt::topography::ENU_axes_from_WGS84(refCoords.lon, refCoords.lat,
        ...
        do {
          ...
          if (obs) {
            ...
            if (obs->has_GGA_datum) // && obs->has_RMC_datum )
            {
              ...
              if (!ref_valid) {
                ref_valid = true;
                refCoords = obs->getMsgByClass<gnss::Message_NMEA_GGA>()
                                .getAsStruct<TGeodeticCoords>();

                // Local coordinates reference:
                TPose3D _local_ENU;
                mrpt::topography::ENU_axes_from_WGS84(refCoords, _local_ENU,
                                                      true);
                local_ENU = CPose3D(_local_ENU);
              }
              ...
              ::fprintf(
                  f_this, "%.4f %.16f %.16f %f %u %u %f %f %.16f %.16f %f %i "
                          "%.4f %.4f %.4f\n",
                  tim, DEG2RAD(obs->getMsgByClass<gnss::Message_NMEA_GGA>()
                                   .fields.latitude_degrees),
                  DEG2RAD(obs->getMsgByClass<gnss::Message_NMEA_GGA>()
                  ...
                  obs->has_RMC_datum
                      ? obs->getMsgByClass<gnss::Message_NMEA_RMC>()
                            .fields.speed_knots
                      : 0.0,
                  obs->has_RMC_datum
                      ? DEG2RAD(obs->getMsgByClass<gnss::Message_NMEA_RMC>()
          ...
          if (obs) {
            ...
            if (it == lstFiles.end()) // A new fiile for this sensorlabel??
            {
              ...
              ::fprintf(f_this,
                        ...
                        "%14s "              // SAT Time
                        "\n",
                        "Time", "Lat", "Lon", "Alt", "fix", "#sats", "speed",
                        "dir", "Local X", "Local Y", "Local Z", "rawlog ID",
                        "Geocen X", "Geocen Y", "Geocen Z", "GPS X", "GPS Y",
                        "GPS Z", "GPS VX", "GPS VY", "GPS VZ", "Local VX",
                        "Local VY", "Local VZ", "SAT Time");
            } else
              f_this = it->second;
            ...
            if (obs->has_GGA_datum) // && obs->has_RMC_datum )
            {
              ...
              if (!ref_valid) {
                ref_valid = true;
                refCoords.lon = obs->getMsgByClass<gnss::Message_NMEA_GGA>()
                                    .fields.longitude_degrees;
                refCoords.lat = obs->getMsgByClass<gnss::Message_NMEA_GGA>()
                                    .fields.latitude_degrees;
                refCoords.height = obs->getMsgByClass<gnss::Message_NMEA_GGA>()
                                       .fields.altitude_meters;
              ...
              if (obs->has_PZS_datum &&
                  obs->getMsgByClass<gnss::Message_TOPCON_PZS>()
                      .hasCartesianPosVel) {
              ...
              ::fprintf(
                  ...
                  mrpt::system::timestampTotime_t(
                      obs->getMsgByClass<gnss::Message_NMEA_GGA>()
                          .fields.UTCTime.getAsTimestamp(obs->timestamp)));
              M++;
    ...
    wxMessageBox(_U(format("%u entries saved!", (unsigned)M).c_str()),
                 _("Done"), wxOK, this);
  }
...
void filter_delGPSNan(mrpt::obs::CActionCollection *acts,
...
void xRawLogViewerFrame::OnMenuGPSDeleteNaN(wxCommandEvent &event) {
  ...
  dlgEdit.executeOperationOnRawlog(filter_delGPSNan,
                                   "GPS deleted with NaN values:");
