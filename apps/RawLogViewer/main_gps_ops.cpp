...
void xRawLogViewerFrame::OnMenuDrawGPSPath(wxCommandEvent& event)
{
	...
	if (wxYES !=
		wxMessageBox(
			_("Do you want to take the GPS reference automatically from "
...
void xRawLogViewerFrame::OnMenuRegenerateGPSTimestamps(wxCommandEvent& event)
{
	...
	if (wxYES ==
		wxMessageBox(
			wxString::Format(
				_("%u changes, average/std time shift is: %f/%f sec. Do "
				  "you want to re-order by timestamp?"),
