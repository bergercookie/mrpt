...
void xRawLogViewerFrame::OnMenuCompactRawlog(wxCommandEvent &event) {
  ...
  bool onlyOnePerLabel =
      (wxYES == wxMessageBox(_("Keep only one observation of each label within "
                               "each sensoryframe?"),
  ...
  wxProgressDialog progDia(wxT("Compacting rawlog"), wxT("Processing..."),
                           progress_N, // range
                           this,       // parent
  ...
  for (progress_i = 0; it != rawlog.end(); progress_i--) {
    ...
    if (it.getType() == CRawlog::etActionCollection) {
      ...
      if (lastAct) {
        ...
        ASSERT_(act);
        lastAct->computeFromOdometry(lastAct->rawOdometryIncrementReading +
        ...
        // Before leaving the last SF, leave only one observation for each
        // sensorLabel:
  ...
  string str = format("%u actions deleted\n%u sensory frames deleted",
                      nActionsDel, nEmptySFDel);
  ...
  for (i = 0; i < N; i++) {
    ...
    if (rawlog.getType(i) == CRawlog::etActionCollection) {
      ...
      if (mov) {
        ...
        // If we have previous observations, shift them according to this new
        // increment:
        if (accum_sf) {
...
void xRawLogViewerFrame::OnMenuConvertExternallyStored(wxCommandEvent &event) {
  ...
  if (fileExists(outDir)) {
    wxMessageBox(
        _U(format("*ABORTING*: Output directory for images already exists. "
  ...
  if (!fileExists(outDir)) {
    wxMessageBox(
        _U(format("*ABORTING*: Cannot create directory:\n%s", outDir.c_str())
               .c_str()));
    return;
  }
      ...
      if (newObj->GetRuntimeClass() == CLASS_ID(CSensoryFrame)) {
        ...
        for (unsigned k = 0; k < SF->size(); k++) {
          if (SF->getObservationByIndex(k)->GetRuntimeClass() ==
            ...
            string fileName = format("img_stereo_%u_left_%05u.%s", k, imgSaved,
            ...
            // save image to file & convert into external storage:
            fileName = format("img_stereo_%u_right_%05u.%s", k, imgSaved,
                              imgFileExtension.c_str());
          ...
          if (SF->getObservationByIndex(k)->GetRuntimeClass() ==
            ...
            // save image to file & convert into external storage:
            string fileName = format("img_monocular_%u_%05u.%s", k, imgSaved,
                                     imgFileExtension.c_str());
...
void xRawLogViewerFrame::OnMenuResortByTimestamp(wxCommandEvent &event) {
  ...
  bool useSensorTimestamp =
      (wxYES == wxMessageBox(_("Yes: use sensor-based UTC timestamp. No: use "
  ...
  for (i = 0; i < n; i++) {
    ...
    case CRawlog::etObservation: {
      ...
      TTimeStamp tim = useSensorTimestamp ? o->getTimeStamp()
                                          : o->getOriginalReceivedTimeStamp();

      if (tim == INVALID_TIMESTAMP) {
        wxMessageBox(wxString::Format(
            _("Error: Element %u does not have a valid timestamp."),
            (unsigned int)i));
        return;
      }
...
void xRawLogViewerFrame::OnMenuShiftTimestampsByLabel(wxCommandEvent &event) {
  ...
  wxString s = wxGetTextFromUser(_("Enter the number of seconds to shift (can "
                                   "have fractions, be positive or negative)"),
  ...
  if (wxYES ==
      wxMessageBox(
          wxString::Format(
              _("%u changes. Do you want to re-order by timestamp?"), nChanges),
...
void xRawLogViewerFrame::OnMenuConvertSF(wxCommandEvent &event) {
  ...
  wxString strMaxL =
      wxGetTextFromUser(_("Maximum length of each sensoryframe (seconds):"),
  ...
  for (unsigned int countLoop = 0; countLoop < nEntries; countLoop++) {
    if (countLoop % 20 == 0) {
      if (!progDia.Update(countLoop,
                          wxString::Format(wxT("Parsing rawlog... %u objects"),
                                           countLoop))) {
        return;
      }
      wxTheApp->Yield(); // Let the app. process messages
    ...
    case CRawlog::etObservation: {
      ...
      if (SF_len > maxLengthSF && SF_new.size() != 0) {
        ...
        if (last_sf_odo && cur_sf_odo) {
          ...
          act.computeFromOdometry(cur_sf_odo->odometry - last_sf_odo->odometry,
                                  opts);
          acts.insert(act);
        }
