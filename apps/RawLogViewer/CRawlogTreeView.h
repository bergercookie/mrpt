...
enum TRawlogTreeViewEvent { evSelected };
...
public:
  /** Constructor
    */
  CRawlogTreeView(wxWindow *parent = nullptr, wxWindowID id = -1,
  ...
  /** Sets the rawlog to be rendered in the control (It's kept as a pointer, so
   * the original object cannot be destroyed).
  ...
  void SetSelectedItem(int index, bool force_refresh = false); //!< Changes the
                                                               //!selected item,
                                                               //!if different,
                                                               //!and raises
                                                               //!user callback,
...
protected:
  ...
  std::vector<TNodeData> m_tree_nodes; //!< The nuimber of rows to display for
                                       //!the rawlog, used to compute the height
