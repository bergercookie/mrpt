...
/* ------------------------------------------------------------
                                                CRawlogTreeView
   ------------------------------------------------------------ */
CRawlogTreeView::CRawlogTreeView(wxWindow *parent, wxWindowID id,
                                 const wxPoint &pos, const wxSize &size,
                                 long style, const wxString &name)
    : wxScrolledWindow(parent, id, pos, size,
                       style | wxVSCROLL | wxFULL_REPAINT_ON_RESIZE, name),
      m_rawlog(nullptr), m_imageList(nullptr), m_selectedItem(-1),
      m_event_select_change(nullptr), m_win_parent(nullptr),
      m_rawlog_start(INVALID_TIMESTAMP), m_rawlog_last(INVALID_TIMESTAMP),
      m_tree_nodes() {}
  ...
  if (m_rawlog) {
    ...
    for (CRawlog::iterator it = m_rawlog->begin(); it != end_it;
      ...
      if ((*it)->GetRuntimeClass() == CLASS_ID(CSensoryFrame)) {
        ...
        for (CActionCollection::iterator a = acts->begin(); a != acts->end();
             ++a) {
          m_tree_nodes.push_back(TNodeData());
          TNodeData &d = m_tree_nodes.back();
          d.level = 2;
        ...
        if (o->timestamp != INVALID_TIMESTAMP) {
          ...
          // tims.push_back( mrpt::system::timeDifference(m_rawlog_start,
          // o->timestamp));
        }
      }
    }
...
void CRawlogTreeView::OnDraw(wxDC &dc) {
  ...
  wxRect visibleArea(xc0 * ROW_HEIGHT, y0 * ROW_HEIGHT, w * ROW_HEIGHT,
                     h * ROW_HEIGHT);
  ...
  static wxFont font_normal(11, wxFONTFAMILY_ROMAN, wxFONTSTYLE_NORMAL,
                            wxFONTWEIGHT_NORMAL);
  static wxFont font_bold(11, wxFONTFAMILY_ROMAN, wxFONTSTYLE_NORMAL,
                          wxFONTWEIGHT_BOLD);
...
void CRawlogTreeView::ConnectSelectedItemChange(
    wxRawlogTreeEventFunction func) {
  ...
  if (sel_item < (int)m_tree_nodes.size()) {
    if (sel_item != m_selectedItem || force_refresh) {
      ...
      if (m_event_select_change && m_win_parent) {
        (*m_event_select_change)(m_win_parent, this, evSelected, sel_item,
