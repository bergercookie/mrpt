...
struct TInfoPerSensorLabel {
  TInfoPerSensorLabel()
      : max_ellapsed_tim_between_obs(.0), first(INVALID_TIMESTAMP),
  ...
  void addOcurrence(mrpt::system::TTimeStamp obs_tim,
...
public:
  wxStaticBitmapPopup() {}
  wxStaticBitmapPopup(wxWindow *parent, wxWindowID id, const wxBitmap &img,
...
public:
  ...
  AskForObservationByLabelMultiple(const std::string &title);

  void saveImportedLogToRawlog(const std::string &target_filename,
...
private:
  ...
  static void
  OntreeViewSelectionChanged(wxWindow *me, CRawlogTreeView *the_tree,
                             TRawlogTreeViewEvent ev, int item_index,
