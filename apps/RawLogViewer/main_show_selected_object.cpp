...
// Update 3D view ==========
				...
				this->m_gl3DRangeScan->m_openGLScene->insert(
					std::make_shared<mrpt::opengl::CAxis>(
				...
				if (obs->hasPoints3D)
				{
						...
						0)  // This includes when there is no confidence image.
					{
						...
						pnts->setPose(
							mrpt::poses::CPose3D());  // No need to further
						// transform 3D points
					}
					else
					{
						...
						// relative to the
						// camera, but the camera may be translated wrt the
						// robot (our 0,0,0
						// here):
...
catch (mrpt::utils::CExceptionExternalImageNotFound& e)
{
	...
	if (wxYES == wxMessageBox(
					 _U(format(
							...
							"is:\n%s\n\nDo you "
							"want to set it to a different one?",
