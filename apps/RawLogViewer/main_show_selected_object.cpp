...
void xRawLogViewerFrame::SelectObjectInTreeView(
  ...
  if (sel_obj) {
    ...
    { // scope for redirector
      ...
      // Select the panel by class (cannot make "wxWindow::FindWindowByName" to
      // run right!!! :-(
      ...
      if (classID->derivedFrom(CLASS_ID(CObservation))) {
        ...
        if (IS_CLASS(sel_obj, CObservation2DRangeScan)) {
          ...
          cout << "2D coordinates of valid points (wrt to robot/vehicle frame, "
               << pts.size() << " points)\n";
          cout << "pts=[";
      ...
      if (classID == CLASS_ID(CObservationBearingRange)) {
        ...
        for (size_t k = 0; k < nPts; k++) {
          ...
          CPoint3D local(R * cos(yaw) * cos(pitch), R * sin(yaw) * cos(pitch),
                         R * sin(pitch));
      ...
      if (classID == CLASS_ID(CObservation3DRangeScan)) {
        ...
        obs->load(); // Make sure the 3D point cloud, etc... are all loaded in
                     // memory.

        const bool generate3Donthefly =
            !obs->hasPoints3D && mnuItemEnable3DCamAutoGenPoints->IsChecked();
        ...
        if (generate3Donthefly)
          cout << "NOTICE: The stored observation didn't contain 3D points, "
                  "but these have been generated on-the-fly just for "
                  "visualization purposes.\n"
                  "(You can disable this behavior from the menu Sensors->3D "
                  "depth cameras\n\n";
...
// Update 3D view ==========
        ...
        this->m_gl3DRangeScan->m_openGLScene->insert(
            std::make_shared<mrpt::opengl::CAxis>(-20, -20, -20, 20, 20, 20, 1,
                                                  2, true));
        ...
        if (obs->hasPoints3D) {
              ...
              0) // This includes when there is no confidence image.
          {
            ...
            pointMap.insertObservation(
                obs.get()); // This transform points into vehicle-frame
            pnts->loadFromPointsMap(&pointMap);

            pnts->setPose(mrpt::poses::CPose3D()); // No need to further
                                                   // transform 3D points
          } else {
            pnts->clear();
            ...
            // Translate the 3D cloud since sensed points are relative to the
            // camera, but the camera may be translated wrt the robot (our 0,0,0
            // here):
