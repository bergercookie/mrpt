...
TCLAP::ValueArg<std::string> arg_output_file("o", "output",
                                             "Output file (*.simplemap)", true,
                                             "", "map.simplemap", cmd);
TCLAP::ValueArg<std::string> arg_input_file("i", "input",
                                            "Input dataset (required) (*.log)",
                                            true, "", "carmen.log", cmd);

TCLAP::ValueArg<int> arg_gz_level("z", "compress-level",
                                  "Output GZ-compress level (optional)", false,
                                  5, "0: none, 1-9: min-max", cmd);

// Declarations:
#define VERBOSE_COUT                                                           \
  if (verbose)                                                                 \
  cout << "[carmen2simplemap] "
  ...
  try {
    ...
    if (mrpt::system::fileExists(output_file) && !overwrite)
      throw runtime_error(format(
          "Output file already exist: '%s' (Use --overwrite to override)",
          output_file.c_str()));
    ...
    while (carmen_log_parse_line(input_stream, importedObservations,
                                 base_timestamp)) {
      CPose2D gt_pose;
      ...
      for (size_t i = 0; i < importedObservations.size(); i++) {
        ...
        // probably
        //  a "corrected" odometry from some SLAM program, so save it as ground
        //  truth:
      ...
      if (has_gt_pose) {
        ...
        for (size_t i = 0; i < importedObservations.size(); i++) {
          if (!IS_CLASS(importedObservations[i],
                        CObservationOdometry)) // Odometry was already used as
                                               // positioning...
          {
        ...
        CPosePDFGaussian::Ptr pos = std::make_shared<CPosePDFGaussian>();
        pos->mean = gt_pose;
      ...
      if (verbose && ++decimateUpdateConsole > 10) {
        ...
        cout << "\rProgress: [" << string(nBlocks, '#')
             ...
             << format("] %6.02f%% (%u frames)", progress_ratio * 100,
    ...
    // Save final map object:
    {
      ...
      if (!out_map.open(output_file, compress_level))
        throw runtime_error(
            format("Error opening for write: '%s'", output_file.c_str()));
