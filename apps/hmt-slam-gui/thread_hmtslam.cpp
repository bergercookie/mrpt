  ...
  try {
    ...
    while (true) {
      try {
        ...
        // If we are running SLAM, read actions/observations and feed them into
        // the SLAM engine:
        // ----------------------------------------------------------------------------------------
        ...
        if (is_running_slam) {
          ...
          try {
            ...
            is_running_slam = false;
            cout << endl
                 << "=============== END OF RAWLOG FILE: ENDING HMT-SLAM "
                    "==============\n";
            continue;
          } catch (...) {
          ...
          if (IS_CLASS(objFromRawlog, CActionCollection)) {
            m_hmtslam->pushAction(std::dynamic_pointer_cast<CActionCollection>(
            ...
            m_hmtslam->pushObservation(std::dynamic_pointer_cast<CObservation>(
                objFromRawlog)); // Memory will be freed in mapping class
          } else
