...
struct TThreadParam {
  TThreadParam()
      : quit(false), terminated(false), tilt_ang_deg(0), Hz(0),
        select_IR_channel(false), flag_grab_depth(false) {}
...
// Thread for (async) live detecting corners: Do in another thread so the GUI
// doesn't freeze.
struct TThreadDetectCornerParam {
  TThreadDetectCornerParam()
      : quit(false), terminated(false), ready_for_new_images(true),
        image_timestamp(INVALID_TIMESTAMP), detected_corners_done(false) {}
...
class kinect_calibrate_guiDialog : public wxDialog {
  friend void myCalibCallback(const mrpt::vision::TImageStereoCallbackData &d,
                              void *user_data);
...
private:
  ...
  void fillGridLine(int r, const char *label_prefix, const char *label,
