...
private:
  ...
  mrpt::vector_size_t m_lastObservation_GT_indices; //!< Ground truth of the
                                                    //!indices in the landmark
                                                    //!map of the sensed
                                                    //!landmarks.
...
*/
  ...
  std::set<size_t> m_realIDX_already_mapped; //!< At least inserted in the map
                                             //!once (or more if due to errors
                                             //!it's more than once).
  ...
  struct THistoric {
    THistoric()
        : run_time(0), da_true_pos(0), da_true_neg(0), da_false_pos(0),
          da_false_neg(0) {}
  ...
  std::vector<THistoric, Eigen::aligned_allocator<THistoric>>
      m_historicData; //!< A registry of all data for the simulation, used to
                      //!compute errors, etc..
  ...
  void executeOneStep(); //!< Executes 1 step of the simulator (does NOT update
                         //!the graphs)
  ...
  struct TSimulationOptions : public mrpt::utils::CLoadableOptions {
    ...
    double sensor_fov;
    bool sensorDistingishesLandmarks; //!< If false (default), data associatin
                                      //!must be done

    double path_square_len;   //!< Used to simulate the robot path
    double robot_step_length; //!< The length (in meters) of each robot forward
                              //!step.
    ...
    double spurious_count_mean, spurious_count_std; //!< Mean and std of
                                                    //!spurious readings per
                                                    //!"sensor observation".
  };
