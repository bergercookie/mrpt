...
bool slamdemoApp::doCommandLineProcess() {
  ...
  TCLAP::CmdLine cmd("2d-slam-demo", ' ',
                     mrpt::system::MRPT_getVersion().c_str());

  TCLAP::ValueArg<std::string> arg_cfgFil("c", "config", "Config file to load",
                                          false, "", "params.ini", cmd);
...
#ifdef MRPT_OS_WINDOWS
    wxMessageBox(wxT("The indicated config file does not exist"),
                 _("2d-slam-demo"));
#endif
    return false;
  } else {
