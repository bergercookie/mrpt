...
void generateRandomMaskImage(mrpt::math::CMatrix &m, const unsigned int nrows,
...
double obs3d_test_depth_to_3d(int a, int b) {
  ...
  if (b & 0x01) {
    generateRandomMaskImage(minF, obs1.rangeImage.rows(),
                            obs1.rangeImage.cols());
    fp.rangeMask_min = &minF;
  }
  if (b & 0x02) {
    generateRandomMaskImage(maxF, obs1.rangeImage.rows(),
                            obs1.rangeImage.cols());
...
void register_tests_CObservation3DRangeScan() {
  if (mrpt::system::fileExists(rgbd_test_rawlog_file)) {
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->3D (no LUT,w/o SSE2)",
                 obs3d_test_depth_to_3d, 0x00, 0));
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->3D (no LUT,w/SSE2)",
                 obs3d_test_depth_to_3d, 0x02, 0));
    lstTests.push_back(TestData("3DRangeScan: 320x240 Depth->3D (LUT,w/o SSE2)",
                                obs3d_test_depth_to_3d, 0x01, 0));
    lstTests.push_back(TestData("3DRangeScan: 320x240 Depth->3D (LUT,w/SSE2)",
                                obs3d_test_depth_to_3d, 0x03, 0));

    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->3D (no LUT,w/o SSE2,minFilter)",
                 obs3d_test_depth_to_3d, 0x00, 0x01));
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->3D (no LUT,w/SSE2,minFilter)",
                 obs3d_test_depth_to_3d, 0x02, 0x01));
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->3D (LUT,w/o SSE2,minFilter)",
                 obs3d_test_depth_to_3d, 0x01, 0x01));
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->3D (LUT,w/SSE2,minFilter)",
                 obs3d_test_depth_to_3d, 0x03, 0x01));

    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->3D (no LUT,w/o SSE2,maxFilter)",
                 obs3d_test_depth_to_3d, 0x00, 0x02));
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->3D (no LUT,w/SSE2,maxFilter)",
                 obs3d_test_depth_to_3d, 0x02, 0x02));
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->3D (LUT,w/o SSE2,maxFilter)",
                 obs3d_test_depth_to_3d, 0x01, 0x02));
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->3D (LUT,w/SSE2,maxFilter)",
                 obs3d_test_depth_to_3d, 0x03, 0x02));

    lstTests.push_back(TestData(
        "3DRangeScan: 320x240 Depth->3D (no LUT,w/o SSE2,min/maxFilter)",
        obs3d_test_depth_to_3d, 0x00, 0x03));
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->3D (no LUT,w/SSE2,min/maxFilter)",
                 obs3d_test_depth_to_3d, 0x02, 0x03));
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->3D (LUT,w/o SSE2,min/maxFilter)",
                 obs3d_test_depth_to_3d, 0x01, 0x03));
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->3D (LUT,w/SSE2,min/maxFilter)",
                 obs3d_test_depth_to_3d, 0x03, 0x03));

    lstTests.push_back(TestData("3DRangeScan: 320x240 Depth->2D scan",
                                obs3d_test_depth_to_2d_scan));
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->2D scan + min_filter",
                 obs3d_test_depth_to_2d_scan, 1, 0));
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->2D scan + max_filter",
                 obs3d_test_depth_to_2d_scan, 0, 1));
    lstTests.push_back(
        TestData("3DRangeScan: 320x240 Depth->2D scan + min/max_filters",
