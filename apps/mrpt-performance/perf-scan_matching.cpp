...
void generate_points(TPoints &pA, TPoints &pB) {
  ...
  for (unsigned int i = 0; i < 5; ++i) {
    pB[i].resize(3);
    qPose.inverseComposePoint(pA[i][0], pA[i][1], pA[i][2], pB[i][0], pB[i][1],
                              pB[i][2]);
  }
...
// ------------------------------------------------------
void generate_list_of_points(const TPoints &pA, const TPoints &pB,
...
// ------------------------------------------------------
void generate_vector_of_points(const TPoints &pA, const TPoints &pB,
...
void register_tests_scan_matching() {
  ...
  lstTests.push_back(
      TestData("tfest: se2_l2 [x1000 corrs]", scan_matching_test_4, 1000, 1e5));
}
