...
double poses_test_compose3Dpoint3(int a1, int a2) {
  ...
  for (long i = 0; i < N; i++) {
    a.composePoint(b.x(), b.y(), b.z(), x, y, z, &df_dpoint, &df_dpose,
                   &df_dse3);
  }
  double T = tictac.Tac() / N;
  dummy_do_nothing_with_string(mrpt::format("%f", x));
...
void register_tests_poses() {
  ...
  lstTests.push_back(TestData("poses: CPose3D.composePoint()+Jacobs",
                              poses_test_compose3Dpoint3));

  lstTests.push_back(
      TestData("poses: CPoint3D (-) CPose3D", poses_test_invcompose3Dpoint));
  lstTests.push_back(TestData("poses: CPose3D.inverseComposePoint()",
  ...
  lstTests.push_back(
      TestData("poses: CPose3DQuat (+) CPose3DQuat", poses_test_compose3DQuat));
  lstTests.push_back(
      TestData("poses: CPose3DQuat.composeFrom()", poses_test_compose3DQuat2));
  lstTests.push_back(TestData("poses: CPose3DQuat (+) CPoint3D",
                              poses_test_compose3DQuatpoint));
  lstTests.push_back(TestData("poses: CPose3DQuat.composePoint()",
                              poses_test_compose3DQuatpoint2));

  lstTests.push_back(TestData("poses: CPose3DQuat.invcomposePoint()",
                              poses_test_invcompose3DQuatpoint2));

  lstTests.push_back(TestData("poses: Conv CPose3DQuat <- CPose3D",
                              poses_test_convert_quat_ypr));
  lstTests.push_back(TestData("poses: Conv CPose3D -> CPose3DQuat",
                              poses_test_convert_ypr_quat));

  lstTests.push_back(
      TestData("poses: Conv CPose3DQuat Gauss <- CPose3D Gauss (DEF)",
               poses_test_convert_quat_ypr_pdf, -1));
  lstTests.push_back(
      TestData("poses: Conv CPose3DQuat Gauss <- CPose3D Gauss (Lin)",
               poses_test_convert_quat_ypr_pdf, 0));
  lstTests.push_back(
      TestData("poses: Conv CPose3DQuat Gauss <- CPose3D Gauss (SUT)",
               poses_test_convert_quat_ypr_pdf, 1));
  lstTests.push_back(
      TestData("poses: Conv CPose3D Gauss <- CPose3DQuat Gauss (DEF)",
               poses_test_convert_ypr_quat_pdf, -1));
  lstTests.push_back(
      TestData("poses: Conv CPose3D Gauss <- CPose3DQuat Gauss (Lin)",
               poses_test_convert_ypr_quat_pdf, 0));
  lstTests.push_back(
      TestData("poses: Conv CPose3D Gauss <- CPose3DQuat Gauss (SUT)",
               poses_test_convert_ypr_quat_pdf, 1));
}
