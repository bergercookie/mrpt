...
double graphslam_levmarq_solve(int nVertices, int N) {
  ...
  for (long i = 0; i < N; i++) {
    ...
    graphslam::optimize_graph_spa_levmarq(graph0, levmarq_info, nullptr,
                                          params);
    timer.leave("test");
  }
...
void register_tests_graphslam() {
  ...
  lstTests.push_back(TestData("graphslam(2d): levmarq 50 KFs/101 edges",
                              graphslam_levmarq_solve<CNetworkOfPoses2D>, 50,
                              50));
  lstTests.push_back(TestData("graphslam(2d): levmarq 100 KFs/451 edges",
                              graphslam_levmarq_solve<CNetworkOfPoses2D>, 100,
                              2));
  lstTests.push_back(TestData("graphslam(3d): levmarq 50 KFs/101 edges",
                              graphslam_levmarq_solve<CNetworkOfPoses3D>, 50,
                              10));
  lstTests.push_back(TestData("graphslam(3d): levmarq 100 KFs/451 edges",
                              graphslam_levmarq_solve<CNetworkOfPoses3D>, 100,
                              2));
}
