...
void register_tests_math() {
  ...
  lstTests.push_back(
      TestData("math: std::hypot(float)",
               std::bind(math_test_FUNC<float, decltype(std::hypotf)>, _1, _2,
                         std::hypotf)));
  lstTests.push_back(TestData(
      "math: mrpt::math::hypot_fast(float)",
      std::bind(math_test_FUNC<float, decltype(mrpt::math::hypot_fast<float>)>,
                _1, _2, mrpt::math::hypot_fast<float>)));
  lstTests.push_back(TestData(
      "math: mrpt::math::hypot_fast(double)",
      std::bind(
          math_test_FUNC<double, decltype(mrpt::math::hypot_fast<double>)>, _1,
          _2, mrpt::math::hypot_fast<double>)));
}
