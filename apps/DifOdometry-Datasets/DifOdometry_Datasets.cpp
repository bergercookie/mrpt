...
void CDifodoDatasets::loadConfiguration(const utils::CConfigFileBase &ini) {
  ...
  if (f_gt.fail())
    throw std::runtime_error("\nError finding the groundtruth file: it should "
                             "be contained in the same folder than the rawlog "
                             "file");
...
void CDifodoDatasets::updateScene() {
  ...
  for (unsigned int y = 0; y < cols; y++)
    for (unsigned int z = 0; z < rows; z++)
      cam_points->push_back(depth[repr_level](z, y), xx[repr_level](z, y),
  ...
  if ((first_pose == true) && ((gt_pose - gt_oldpose).norm() < 0.5)) {
    ...
    traj_lines_odo->appendLine(cam_oldpose.x(), cam_oldpose.y(),
                               cam_oldpose.z(), cam_pose.x(), cam_pose.y(),
                               cam_pose.z());
    ...
    traj_lines_gt->appendLine(gt_oldpose.x(), gt_oldpose.y(), gt_oldpose.z(),
