...
int main(int argc, char** argv)
{
	try
	{
		...
		{
			printf(
				"Use: kf-slam <config_file> [dataset.rawlog]\n\nPush any key "
				"to "
				"exit...\n");
			os::getch();
			return -1;
		...
		if (argc == 3)
			...
			rawlogFileName = cfg.read_string(
...
void Run_KF_SLAM(CConfigFile& cfgFile, const std::string& rawlogFileName)
{
	...
	mrpt::utils::bimap<int, int>
		DA2GTDA_indices;  // Landmark indices bimapping:
	// SLAM DA <---> GROUND TRUTH
	// DA
					...
					for (size_t i = 0; i < obsRB->sensedData.size(); i++)
					{
						...
						if (itDA != GT_DA.end())
						{
							...
							if (it != da.results.associations.end())
							{
								...
								// // Landmark
								// indices bimapping: SLAM DA <---> GROUND TRUTH
								// DA
