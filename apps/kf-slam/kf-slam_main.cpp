...
int main(int argc, char **argv) {
  try {
    ...
    if (argc == 3)
      ...
      rawlogFileName = cfg.read_string("MappingApplication", "rawlog_file",
...
template <> struct kfslam_traits<CRangeBearingKFSLAM2D> {
  ...
  template <class ARR> static void landmark_to_3d(const ARR &lm, TPoint3D &p) {
    p.x = lm[0];
    p.y = lm[1];
    p.z = 0;
  }

  static void doPartitioningExperiment(ekfslam_t &mapping,
  ...
  static void doPartitioningExperiment(ekfslam_t &mapping,
    ...
    if (mapping.options.doPartitioningExperiment) {
      ...
      for (size_t i = 0; i < STEPS; i++) {
        ...
        mapping.getLastPartitionLandmarks(landmarksMembership);
        ERRS_SWEEP[i] = mapping.computeOffDiagonalBlocksApproximationError(
            landmarksMembership);
      }
...
void Run_KF_SLAM(CConfigFile &cfgFile, const std::string &rawlogFileName) {
  ...
  const bool SAVE_MAP_REPRESENTATIONS =
      cfgFile.read_bool("MappingApplication", "SAVE_MAP_REPRESENTATIONS", true);
  bool SHOW_3D_LIVE =
  ...
  string OUT_DIR =
      cfgFile.read_string("MappingApplication", "logOutput_dir", "OUT_KF-SLAM");
  string ground_truth_file =
      cfgFile.read_string("MappingApplication", "ground_truth_file", "");
  string ground_truth_file_robot =
      cfgFile.read_string("MappingApplication", "ground_truth_file_robot", "");
  ...
  mrpt::utils::bimap<int, int> DA2GTDA_indices; // Landmark indices bimapping:
                                                // SLAM DA <---> GROUND TRUTH DA
  if (!ground_truth_data_association.empty() &&
    ...
    for (size_t i = 0; i < mGT_DA.getRowCount(); i++) {
      ...
      if (v.size() <= mGT_DA(i, 1))
        v.resize(mGT_DA(i, 1) + 1);
  ...
  {
    const std::string f =
        std::string(OUT_DIR + std::string("/data_association_performance.log"));
    out_da_performance_log.open(f.c_str());
    ASSERTMSG_(out_da_performance_log.is_open(),
               std::string("Error writing to: ") + f)

    // Header:
    out_da_performance_log << "%           TIMESTAMP                "
                              "INDEX_IN_OBS    TruePos FalsePos TrueNeg "
                              "FalseNeg  NoGroundTruthSoIDontKnow \n"
                           << "%-----------------------------------------------"
                              "------------------------------------------------"
                              "-----------------\n";
  }
  ...
  if (SHOW_3D_LIVE) {
    win3d = std::make_shared<mrpt::gui::CDisplayWindow3D>("KF-SLAM live view",
                                                          800, 500);

    win3d->addTextMessage(0.01, 0.96, "Red: Estimated path",
                          TColorf(0.8f, 0.8f, 0.8f), 100,
                          MRPT_GLUT_BITMAP_HELVETICA_10);
    win3d->addTextMessage(0.01, 0.93, "Black: Ground truth path",
                          TColorf(0.8f, 0.8f, 0.8f), 101,
                          MRPT_GLUT_BITMAP_HELVETICA_10);
  }
  ...
  for (;;) {
    ...
    // --------------------------------------------------
    if (!CRawlog::readActionObservationPair(rawlogFile, action, observations,
                                            rawlogEntry))
      break; // file EOF
      ...
      if (SAVE_DA_LOG) {
        ...
        if (obs) {
          ...
          for (size_t i = 0; i < obsRB->sensedData.size(); i++) {
            std::map<observation_index_t, prediction_index_t>::const_iterator
                it = da.results.associations.find(i);
          ...
          for (size_t i = 0; i < obsRB->sensedData.size(); i++) {
            ...
            if (itDA != GT_DA.end()) {
              ...
              std::map<observation_index_t, prediction_index_t>::const_iterator
                  it = da.results.associations.find(i);
              if (it != da.results.associations.end()) {
                // This observation was assigned the already existing LM in the
                // map: "it->second"
                // TruePos -> If that LM index corresponds to that in the GT
                // (with index mapping):

                // mrpt::utils::bimap<int,int>  DA2GTDA_indices; // Landmark
                // indices bimapping: SLAM DA <---> GROUND TRUTH DA
                if (DA2GTDA_indices.hasKey(it->second)) {
                  ...
                  // Is this case possible? Assigned to an index not ever seen
                  // for the first time with a GT....
                  //  Just in case:
                  is_FP = true;
                }
              } else {
                ...
                if (itNewLM != da.newly_inserted_landmarks.end()) {
                  ...
                  if (DA2GTDA_indices.hasValue(GT_ASSOC)) {
                    // GT says this LM was already observed, so it shouldn't
                    // appear here as new:
                    is_FN = true;
                  } else {
                  ...
                  // Not associated neither inserted: Shouldn't really never
                  // arrive here.
                }
              }
            }
            ...
            out_da_performance_log
                << format("%35.22f %13i %8i %8i %8i %8i %8i\n", tim, (int)i,
      ...
      if (win3d || (SAVE_3D_SCENES && !(step % SAVE_LOG_FREQUENCY))) {
        COpenGLScene::Ptr scene3D = std::make_shared<COpenGLScene>();
        {
          opengl::CGridPlaneXY::Ptr grid =
              std::make_shared<opengl::CGridPlaneXY>(-1000, 1000, -1000, 1000,
                                                     0, 5);
        ...
        if (size(GT_PATH, 2) == 6 || size(GT_PATH, 2) == 3) {
          ...
          if (size(GT_PATH, 2) == 6) {
            ...
            for (size_t i = 0; i < N; i++) {
              const CPose3D p(GT_PATH(i, 0), GT_PATH(i, 1), GT_PATH(i, 2),
                              GT_PATH(i, 3), GT_PATH(i, 4), GT_PATH(i, 5));

              GT_path->appendLine(gtx0, gty0, gtz0, p.x(), p.y(), p.z());
              gtx0 = p.x();
              gty0 = p.y();
              gtz0 = p.z();
            }
        ...
        // Draw latest data association:
        {
          ...
          for (std::map<observation_index_t, prediction_index_t>::const_iterator
            ...
            // Line: robot -> landmark:
            lins->appendLine(robotPoseMean3D.x(), robotPoseMean3D.y(),
        ...
        if (win3d) {
          ...
          win3d->addTextMessage(
              0.02, 0.02, format("Step %u - Landmarks in the map: %u",
                                 (unsigned int)step, (unsigned int)LMs.size()),
              TColorf(1, 1, 1), 0, MRPT_GLUT_BITMAP_HELVETICA_12);

          win3d->addTextMessage(
              0.02, 0.06,
              format(is_pose_3d ? "Estimated pose: (x y z qr qx qy qz) = %s"
          ...
          win3d->addTextMessage(
              0.02, 0.14, format("Execution rate: %7sHz",
        ...
        if (SAVE_3D_SCENES && !(step % SAVE_LOG_FREQUENCY)) {
          // Save to file:
          CFileGZOutputStream(
              OUT_DIR + format("/kf_state_%05u.3Dscene", (unsigned int)step))
              << *scene3D;
        }
      }
  ...
  if (ground_truth_file.size() && fileExists(ground_truth_file)) {
    ...
    if (GT.getRowCount() > 0 && !LMs.empty()) {
      ...
      for (size_t i = 0; i < LMs.size(); i++) {
        ...
        for (size_t r = 0; r < GT.getRowCount(); r++) {
          if (LM_IDs[i] == GT(r, 6)) {
            ...
            ERRS.push_back(gtPt.distanceTo(
                CPoint3D(TPoint3D(LMs[i])))); // All these conversions are to
        ...
        if (!found) {
          cerr << "Ground truth entry not found for landmark ID:" << LM_IDs[i]
               << endl;
        }
      }
