...
int main(int argc, char **argv) {
  try {
    ...
    TCLAP::CmdLine cmd("image2gridmap", ' ',
                       mrpt::system::MRPT_getVersion().c_str());

    TCLAP::ValueArg<std::string> arg_input_file(
        "i", "input", "Input image file (required) (*.png,*.jpg,...)", true, "",
        "map_image.png", cmd);
    TCLAP::ValueArg<std::string> arg_output_file(
        "o", "output", "Name of the output file (*.gridmap, *.gridmap.gz)",
        false, "", "map.gridmap.gz", cmd);
    TCLAP::ValueArg<double> arg_res(
        "r", "res",
        "Resolution: size (in meters) of one pixel in the image (required)",
        true, 0.1, "0.1", cmd);
    TCLAP::ValueArg<double> arg_cx("", "cx", "(Use either --cx or --px) X "
                                             "coordinate of the image central "
                                             "pixel (Default:0)",
                                   false, 0.0, "0.0", cmd);
    TCLAP::ValueArg<double> arg_cy("", "cy", "(Use either --cy or --py) Y "
                                             "coordinate of the image central "
                                             "pixel (Default:0)",
                                   false, 0.0, "0.0", cmd);
    TCLAP::ValueArg<double> arg_px(
        "", "px", "(Use either --cx or --px) Pixel horizontal coordinate of "
                  "the origin of coordinates in the image",
        false, 0.0, "0.0", cmd);
    TCLAP::ValueArg<double> arg_py("", "py", "(Use either --cx or --px) Pixel "
                                             "verticl coordinate of the origin "
    ...
    if ((arg_px.isSet() && !arg_py.isSet()) ||
        (!arg_px.isSet() && arg_py.isSet()))
      throw std::runtime_error(
          "You cannot set only one of --px & --py arguments!");

    if ((arg_cx.isSet() && !arg_cy.isSet()) ||
        (!arg_cx.isSet() && arg_cy.isSet()))
      throw std::runtime_error(
          "You cannot set only one of --cx & --cy arguments!");
    ...
    if (mrpt::system::fileExists(sOutFile) && !arg_overwrite.isSet()) {
      std::cerr << "Output file already exists, aborting. Use `-w` flag to "
                   "overwrite.\n";
      return 1;
    }
