...
void TestParticlesLocalization() {
  ...
  MRPT_LOAD_CONFIG_VAR(SHOW_3D_FRANCO_POSITION, bool, (*iniFile),
                       "ro-localization");
  ...
  */
  ...
  for (float range_Pc = Pc_range_ini; range_Pc <= Pc_range_end;
    ...
    for (int repetition = 0; repetition < NUM_REPS; repetition++) {
      ...
      // PDF Options:
      // ------------------
      pdf.options.KLD_binSize_PHI =
          DEG2RAD(iniFile->read_float("KLD_options", "KLD_binSize_PHI_deg", 5));
      ...
      pdf.options.probabilityChangingBias =
          iniFile->read_float("RO_MODEL", "probabilityChangingBias", 0.1f);
      ...
      // Create Directory
      OUT_DIR = format("%s_Pc_%.06f/REP_%03u", OUT_DIR_PREFIX.c_str(), range_Pc,
      ...
      CPose2D odometryEstimation(
          ...
          DEG2RAD(
              iniFile->read_float("OdometryEstimation", "odometry_PHI", 0)));
      ...
      pdf.resetUniform(
          ...
          state_min, state_max, -M_PIf, M_PIf, PARTICLE_COUNT);
...
// 3D World
      ...
      if (mrpt::system::fileExists(MAP_FILE)) {
...
#ifdef SHOW_REAL_TIME_3D
        {
          ...
          obj->setLocation(initialPoseExperiment.x(), initialPoseExperiment.y(),
                           0);
          sceneTR->insert(obj);
        }
#endif
      ...
      // Floor
      {
        opengl::CGridPlaneXY::Ptr obj =
            std::make_shared<opengl::CGridPlaneXY>(-20, 20, -10, 10, 0, 0.5);
      ...
      if (metricMap.m_landmarksMap) {
        for (size_t k = 0; k < metricMap.m_landmarksMap->size(); k++) {
          opengl::CSphere::Ptr parts = std::make_shared<opengl::CSphere>();
      ...
      while (rawlogEntry < (rawlogEntries - 1)) {
        ...
        // ----------------------------------------
        tictac.Tic();
        printf("Executing ParticleFilter on %u particles....",
        ...
        odometryEstimation =
            odometryEstimation +
            action->getBestMovementEstimation()->poseChange->getMeanVal();
        pdf.getMean(pdfEstimation);

        CPose2D GT_Pose;
        if (groundTruth.getRowCount() > size_t(step)) {
          GT_Pose = CPose2D(groundTruth(step, 1), groundTruth(step, 2),
                            groundTruth(step, 3));
...
// The particles:
          ...
          if (!obj)
            parts = std::make_shared<opengl::CPointCloud>();
          else
          ...
          for (size_t i = 0; i < pdf.size(); i++)
            parts->setPoint(i, pdf.m_particles[i].d->pose.x(),
...
// The particles' cov:
          ...
          if (!obj)
            scanPts = std::make_shared<opengl::CPointCloud>();
          else
          ...
          if (metricMap.m_landmarksMap && dist && !dist->sensedData.empty()) {
            for (size_t k = 0; k < dist->sensedData.size(); k++) {
              string beacon_name =
                  format("ring%u", unsigned(dist->sensedData[k].beaconID));
              const mrpt::maps::CLandmark *lm =
                  metricMap.m_landmarksMap->landmarks.getByBeaconID(
              ...
              if (lm) {
                ...
                float R = square(dist->sensedData[k].sensedDistance) -
                          square(dist->sensedData[k].sensorLocationOnRobot.z() -
                                 lm->pose_mean.z);
          ...
          if (SHOW_3D_FRANCO_POSITION) {
            ...
            CObservationBeaconRanges::Ptr dist =
                observations->getObservationByClass<CObservationBeaconRanges>();
            if (dist) {
              sphere->setLocation(dist->auxEstimatePose.x(),
          ...
          // GPS pose
          {
            CObservationGPS::Ptr o =
                observations->getObservationByClass<CObservationGPS>();
            if (o && metricMap.m_landmarksMap) {
              ...
              if (o->hasMsgClass<mrpt::obs::gnss::Message_NMEA_GGA>()) {
                ...
                x = DEG2RAD((gga.fields.longitude_degrees -
                             metricMap.m_landmarksMap->likelihoodOptions
                                 .GPSOrigin.longitude)) *
                    6371000 * 1.03;
                y = DEG2RAD((gga.fields.latitude_degrees -
                             metricMap.m_landmarksMap->likelihoodOptions
                                 .GPSOrigin.latitude)) *
                    6371000 * 1.15;
                sphere->setLocation(
                    (x * cos(metricMap.m_landmarksMap->likelihoodOptions
                                 .GPSOrigin.ang) +
                     y * sin(metricMap.m_landmarksMap->likelihoodOptions
                                 .GPSOrigin.ang) +
                     metricMap.m_landmarksMap->likelihoodOptions.GPSOrigin
                         .x_shift),
                    (-x * sin(metricMap.m_landmarksMap->likelihoodOptions
                                  .GPSOrigin.ang) +
                     y * cos(metricMap.m_landmarksMap->likelihoodOptions
                                 .GPSOrigin.ang) +
                     metricMap.m_landmarksMap->likelihoodOptions.GPSOrigin
                         .y_shift),
                    0);
        ...
        if ((step % SCENE3D_FREQ) == 0)
          CFileGZOutputStream(
              format("%s/3Dscene_%03u.3Dscene", OUT_DIR.c_str(), step))
              << *scene;
      ...
      real_ranges.saveToTextFile(format("%s/GT_ranges.txt", OUT_DIR.c_str()),
                                 MATRIX_FORMAT_FIXED);

      real_offsets.setSize(real_offsets_rows, real_offsets.getColCount());
      real_offsets.saveToTextFile(format("%s/GT_offsets.txt", OUT_DIR.c_str()),
                                  MATRIX_FORMAT_FIXED);
    ...
    if (Pc_range_ini != Pc_range_end) {
      CFileStream fo(
          format("Pc_%.06f_%.06f_RESULTs.txt", Pc_range_ini, Pc_range_end),
          fomAppend);
