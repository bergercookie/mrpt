...
public:
  ...
  struct TPredictionParams {
    ...
    /** In the algorithm "CParticleFilter::pfAuxiliaryPFOptimal", the number of
     * samples for searching the maximum likelihood value (see papers!)
      */
...
public:
  ...
  /** Copy operator, translating if necesary (for example, between particles and
   * gaussian representations)
    */
  void copyFrom(const CPosePDF &o) override;

  /** Reset the PDF to a single point: All particles will be set exactly to the
   * supplied pose.
   * \param location The location to set all the particles.
   * \param particlesCount If this is set to -1 the number of particles remains
   * unchanged.
   *  \sa resetUniform, resetUniformFreeSpace
   */
  void resetDeterministic(TExtendedCPose2D &location, int particlesCount = -1);
  ...
  /** Reset the PDF to an uniformly distributed one, inside of the defined cube.
    ...
    */
  void resetUniform(float x_min, float x_max, float y_min, float y_max,
                    mrpt::math::CVectorFloat state_min,
                    mrpt::math::CVectorFloat state_max, float phi_min = -M_PI,
                    float phi_max = M_PI, int particlesCount = -1);
  ...
  /** Returns an estimate of the pose-state, i.e. a "mean value", computed as a
   * weighted average over all particles.
   */
   ...
   * x,y,phi variables)
    */
  void getCovarianceAndMean(mrpt::math::CMatrixDouble33 &C,
                            CPose2D &p) const override;

  /** Returns the pose of the i'th particle.
    */
  CPose2D getParticlePose(int i) const;

  /** Update the particles, predicting the posterior of robot pose and map after
   * a movement command.
  ...
  /** Draws a single sample from the distribution (WARNING: weights are assumed
   * to be normalized!)
    */
  ...
  /** Bayesian fusion.
    */
  void bayesianFusion(const mrpt::poses::CPosePDF &p1,
                      const mrpt::poses::CPosePDF &p2,
                      const double &minMahalanobisDistToDrop = 0) override;

  /** Evaluates the PDF at a given arbitrary point as reconstructed by a Parzen
   * window.
    * \sa saveParzenPDFToTextFile
    */
  double evaluatePDF_parzen(float x, float y, float phi, float stdXY,
                            float stdPhi) const;
   ...
   * of the PDF as reconstructed by a Parzen window.
    ...
    */
  void saveParzenPDFToTextFile(const char *fileName, float x_min, float x_max,
                               float y_min, float y_max, float phi,
                               float stepSizeXY, float stdXY,
                               float stdPhi) const;

  void bayesianFusion(mrpt::poses::CPosePDF &p1, mrpt::poses::CPosePDF &p2,
...
private:
   ...
   * a robot pose, and according to the options in
...
DEFINE_SERIALIZABLE_POST_CUSTOM_BASE_LINKAGE(CPosePDFParticlesExtended,
                                             mrpt::poses::CPosePDF,
                                             DUMMY_LINKAGE)
