...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CPosePDFParticlesExtended::getCovarianceAndMean(CMatrixDouble33 &cov,
                                                     CPose2D &mean) const {
  getMean(mean);
  cov.zeros();
...
/*---------------------------------------------------------------
                                                writeToStream
  ---------------------------------------------------------------*/
void CPosePDFParticlesExtended::writeToStream(mrpt::utils::CStream &out,
                                              int *version) const {
  if (version)
    *version = 0;
  else {
...
/*---------------------------------------------------------------
                                                readFromStream
  ---------------------------------------------------------------*/
void CPosePDFParticlesExtended::readFromStream(mrpt::utils::CStream &in,
                                               int version) {
  switch (version) {
  case 0: {
...
void CPosePDFParticlesExtended::prediction_and_update_pfStandardProposal(
  ...
  if (sf != nullptr) {
    ...
    for (it = m_particles.begin(), i = 0; it != m_particles.end(); it++, i++)
      it->log_w += auxiliarComputeObservationLikelihood(PF_options, this, i, sf,
                                                        it->d.get()) *
...
void CPosePDFParticlesExtended::prediction_and_update_pfAuxiliaryPFOptimal(
  ...
  robotMovement =
      actions->getBestMovementEstimation(); // Find a robot movement estimation:
  ...
  tictac.Tic();
  prepareFastDrawSample(PF_options, particlesEvaluator_AuxPFOptimal,
                        &mean_movement, sf);
    ...
    for (i = 0; i < M; i++) {
      ...
      if (maxLik_k < 0) {
        ...
        for (unsigned int q = 0;
             q < PF_options.pfAuxFilterOptimal_MaximumSearchSamples; q++) {
          // Draw new robot pose:
          newPose.pose =
          ...
          // Likelihood:
          double lik = auxiliarComputeObservationLikelihood(PF_options, this, k,
                                                            sf, &newPose);
          if (lik > maxLik_k) {
            maxLik_k = lik;
      ...
      // And its weight:
      newParticlesWeight[i] =
          1; // newPoseLikelihood / m_pfAuxiliaryPFOptimal_estimatedProb[k];
    }        // for i
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CPosePDFParticlesExtended::resetDeterministic(TExtendedCPose2D &location,
                                                   int particlesCount) {
  CParticleList::iterator it;
...
/*---------------------------------------------------------------
                                        evaluatePDF_parzen
 ---------------------------------------------------------------*/
double CPosePDFParticlesExtended::evaluatePDF_parzen(float x, float y,
                                                     float phi, float stdXY,
                                                     float stdPhi) const {
  double difPhi, ret = 0;
