...
void do_grid_align() {
  ...
  f_log.printf("%% std_noise_xy std_noise_phi std_noise_laser  covDet:mean&std "
  ...
  size_t N1 = max(
      40, (int)round(grid1->getArea() * gridAlign.options.featsPerSquareMeter));

  COccupancyGridMapFeatureExtractor gridfeatextract;

  CLandmarksMap lm1;

  gridfeatextract.extractFeatures(*grid1, lm1, N1,
...

  {
    ...
    for (unsigned int iter = 0; iter < N_ITERS; iter++) {
      ...
      for (unsigned int q = 0; q < map2noisy.size(); q++) {
        ...
        if (NOISE_IN_POSE) {
          ...
          newPDF->mean.phi_incr(
              randomGenerator.drawGaussian1D(0, DEG2RAD(STD_NOISE_PHI)));
      ...
      size_t N2 = max(40, (int)round(grid2->getArea() *
                                     gridAlign.options.featsPerSquareMeter));
      ...
      if (!NOSAVE &&
          (is_match || is_detect_test)) // If it's only a
                                        // SAVE_CORR_AND_NONCORR_DISTS test,
                                        // save time
      {
        // Save maps:
        grid2->saveAsBitmapFile(
            format("%s/map2_noise_%f.png", RESULTS_DIR.c_str(), STD_NOISE_XY));
        grid2->saveAsBitmapFileWithLandmarks(format("%s/map2_LM_noise_%f.png",
                                                    RESULTS_DIR.c_str(),
                                                    STD_NOISE_XY),
                                             &lm2, true);
        CImage img;
        grid2->getAsImageFiltered(img);
        img.saveToFile(format("%s/map2_filt_noise_%f.png", RESULTS_DIR.c_str(),
                              STD_NOISE_XY));
      }

      // Only if the case of "save-corr-dists" we can do NOT align the maps,
      // since we're not interested in that
      if (is_match || is_detect_test) {
        // --------------------------
        //        DO ALIGN
        // --------------------------
        CPosePDF::Ptr parts = gridAlign.Align(&the_map1, &the_map2,
                                              CPose2D(0, 0, 0), &tim, &info);
        ...
        float AphiBrute =
            fabs(math::wrapToPi(info.noRobustEstimation.phi() - GT_Aphi_rad));
        ...
        if (IS_CLASS(parts, CPosePDFParticles)) {
          ...
          partsPdf->saveToTextFile(format("%s/particles_noise_%.03f.txt",
          ...
          if (!SAVE_ICP_GOODNESS_FIL.empty()) {
            mrpt::system::vectorToTextFile(info.icp_goodness_all_sog_modes,
                                           SAVE_ICP_GOODNESS_FIL,
                                           true); // append & as column
          }
          ...
          if (SAVE_SOG_ALL_MAPS_OVERLAP_HYPOTHESES) {
            ...
            grid1->resizeGrid(
                min(grid1->getXMin(), -60.0f), max(grid1->getXMax(), 60.0f),
                min(-40.0f, grid1->getYMin()), max(30.0f, grid1->getYMax()));
            ...
            for (nNode = 0, it = pdf_SOG->begin(); it != pdf_SOG->end();
                 it++, nNode++) {
              CPose2D x = it->mean;
              ...
              imgCanvas.saveToFile(format("%s/_OVERLAP_MAPS_SOG_MODE_%04u.png",
                                          RESULTS_DIR.c_str(),
                                          (unsigned int)nNode));
              ...
              CFileGZOutputStream(
                  format("%s/_OVERLAP_MAPS_SOG_MODE_%04u.3Dscene",
              ...
              /*{
                      ...
                      CEnhancedMetaFile::LINUX_IMG_WIDTH = grid1->getSizeX() +
              grid2->getSizeX() + 50;
          ...
          gridLimits.saveToTextFile(format("%s/SOG_grid_limits_noise_%f.txt",
                                           RESULTS_DIR.c_str(), STD_NOISE_XY));

          CMatrixD evalGrid;
          pdf_SOG->evaluatePDFInArea(gridLimits(0, 0), gridLimits(0, 1),
                                     gridLimits(0, 2), gridLimits(0, 3), 0.002f,
                                     0, evalGrid,
                                     true // Sum over all phis
                                     );

          evalGrid.saveToTextFile(format("%s/SOG_grid_noise_%f.txt",
                                         RESULTS_DIR.c_str(), STD_NOISE_XY));
#endif
        } // end if is SOG
      ...
      // Save the descriptor distances for corresponding and non-corresponding
      // features
      ...
      if (SAVE_CORR_AND_NONCORR_DISTS) {
        ...
        for (size_t i1 = 0; i1 < lm1->landmarks.size(); i1++) {
          CVectorDouble D(
              lm2->landmarks.size()); // Distances in descriptor space
          ...
          if (dErrs[best_match] < 0.20) {
            ...
            gt_corrs.push_back(TMatchingPair(i1, best_match, l1->pose_mean.x,
      ...
      /*			printf("\n------------------------------------------------------------------------\n");
                              printf("Noise:%f -> ratio:%.02f%%
         ERRs:(%.03f,%.03f) STDs:(%.03f %.03f)\n",
    ...
    f_log.printf(
        ...
        STD_NOISE_PHI, STD_NOISE_LASER, math::mean(stats_covDet),
        math::stddev(stats_covDet), math::mean(stats_stdPhi),
        math::stddev(stats_stdPhi), math::mean(stats_errorXY),
        math::stddev(stats_errorXY), math::mean(stats_errorPhi),
        math::stddev(stats_errorPhi), math::mean(stats_bruteErrorXY),
        math::stddev(stats_bruteErrorXY), math::mean(stats_bruteErrorPhi),
        math::stddev(stats_bruteErrorPhi), math::mean(stats_GT_likelihood),
...
int main(int argc, char **argv) {
  try {
    ...
    TCLAP::CmdLine cmd("grid-matching", ' ',
                       mrpt::system::MRPT_getVersion().c_str());

    TCLAP::SwitchArg arg_match("m", "match", "Operation: match two maps", cmd,
                               false);
    TCLAP::SwitchArg arg_detect("d", "detect-test",
                                "Operation: Quality of match with one map", cmd,
                                false);

    TCLAP::ValueArg<std::string> arg_filgrid1("1", "map1",
                                              "Map #1 to align (*.simplemap)",
                                              true, "", "map1.simplemap", cmd);
    TCLAP::ValueArg<std::string> arg_filgrid2("2", "map2",
                                              "Map #2 to align (*.simplemap)",
                                              false, "", "map2.simplemap", cmd);
    ...
    TCLAP::SwitchArg arg_savesogall("a", "save-sog-all",
                                    "Save all the map overlaps", cmd, false);
    ...
    TCLAP::ValueArg<double> arg_Ax("X", "Ax",
                                   "In detect-test mode, displacement in X (m)",
                                   false, 4, "X", cmd);
    TCLAP::ValueArg<double> arg_Ay("Y", "Ay",
    ...
    TCLAP::SwitchArg arg_noise_pose("O", "noise-pose",
                                    "detect-test mode: enable noise in pose",
                                    cmd, false);
    TCLAP::SwitchArg arg_noise_laser("L", "noise-laser",
                                     "detect-test mode: enable noise in laser",
                                     cmd, false);

    TCLAP::SwitchArg arg_verbose("v", "verbose", "Verbose output", cmd, false);
    TCLAP::SwitchArg arg_nologo("g", "nologo", "skip the logo at startup", cmd,
                                false);
    TCLAP::SwitchArg arg_nosave("n", "nosave", "skip saving map images", cmd,
                                false);
    TCLAP::SwitchArg arg_skip_icp("s", "noicp", "skip ICP optimization stage",
                                  cmd, false);
    ...
    if (is_match) {
      ...
      if (!arg_filgrid1.isSet() || !arg_filgrid2.isSet()) {
        cout << endl
             << "Error: Two maps must be passed: --map1=xxx and --map2=xxx"
             << endl;
        TCLAP::StdOutput so;
        so.usage(cmd);
        return 1;
