...
mrpt::gui::CDisplayWindow3D::Ptr win; // This is global such as an exception
                                      // within the main program do not abruptly
                                      // closes the window
...
int DoTrackingDemo(CCameraSensor::Ptr cam, bool DO_SAVE_VIDEO) {
  win = std::make_shared<mrpt::gui::CDisplayWindow3D>("Tracked features", 800,
                                                      600);

  mrpt::vision::CVideoFileWriter vidWritter;

  bool hasResolution = false;
  TCamera cameraParams; // For now, will only hold the image resolution on the
                        // arrive of the first frame.
  ...
  while (win->isOpen()) {
    ...
    } catch (CExceptionEOF &) { // End of a rawlog file.
      break;
    }
    ...
    { // FPS:
      ...
      theImg.textOut(3, 22, format("# feats: %u - Adaptive threshold: %i",
                                   (unsigned int)trackedFeats.size(),
                                   current_adapt_thres),
                     TColor(200, 200, 0));
      ...
      theImg.textOut(
          ...
          format("# raw feats: %u - Removed: %u",
                 (unsigned int)
                     tracker->last_execution_extra_info.raw_FAST_feats_detected,
                 (unsigned int)
    ...
    if (SHOW_FEAT_TRACKS) {
      ...
      for (size_t i = 0; i < trackedFeats.size(); ++i) {
        ...
        if (seq.size() > 1) {
          ...
          for (; it != it_end; ++it) {
            theImg.line(it_prev->x, it_prev->y, it->x, it->y,
                        TColor(190, 190, 190));
            it_prev = it;
          }
        }
...
void showUsage(char *cmd) {
  cout
      ...
      << cmd << " --help          -> Show this information.\n"
                "  "
      << cmd
      << " If added --save-video, an video file will be created with results.\n"
         "\n";
}
