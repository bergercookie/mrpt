...
bool CDifodoCamera::openCamera() {
  ...
  for (int i = 0;
    ...
    cout << "depth mode :" << vm.getResolutionX() << "x" << vm.getResolutionY()
         << "@" << vm.getFps() << "Hz using format " << vm.getPixelFormat()
         << endl;
  }
...
void CDifodoCamera::updateScene() {
  ...
  traj_lines_odo->appendLine(cam_oldpose.x(), cam_oldpose.y(), cam_oldpose.z(),
