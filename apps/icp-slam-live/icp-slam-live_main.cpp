...
void SensorThread(TThreadParams params) {
  ...
  try {
    string driver_name =
        params.cfgFile->read_string(params.section_name, "driver", "", true);
    CGenericSensor::Ptr sensor = CGenericSensor::createSensorPtr(driver_name);
    if (!sensor)
      throw std::runtime_error(
          string("***ERROR***: Class name not recognized: ") + driver_name);
...
void MapBuilding_ICP_Live(const string &INI_FILENAME) {
  ...
  if (allThreadsMustExit)
    throw std::runtime_error("\n\n==== ABORTING: It seems that we could not "
                             "connect to the LIDAR. See reported errors. ==== "
                             "\n");
  ...
  const string OUT_DIR_STD =
      iniFile.read_string("MappingApplication", "logOutput_dir", "log_out",
  ...
  const bool SAVE_POSE_LOG = iniFile.read_bool(
      "MappingApplication", "SAVE_POSE_LOG", false, /*Force existence:*/ true);
  const bool SAVE_3D_SCENE = iniFile.read_bool(
      "MappingApplication", "SAVE_3D_SCENE", false, /*Force existence:*/ true);
  ...
  while (!allThreadsMustExit) {
    ...
    CObservation2DRangeScan::Ptr observation;
    {
      ...
      for (mrpt::hwdrivers::CGenericSensor::TListObservations::reverse_iterator
               it = obs_copy.rbegin();
      ...
      if (out_rawlog.fileOpenCorrectly()) {
        for (mrpt::hwdrivers::CGenericSensor::TListObservations::iterator it =
                 obs_copy.begin();
             it != obs_copy.end(); ++it)
          if (it->second && IS_CLASS(it->second, CObservation2DRangeScan))
    ...
    if (0 == (step % LOG_FREQUENCY) || (SAVE_3D_SCENE || win3D)) {
      ...
      mrpt::opengl::CGridPlaneXY::Ptr groundPlane =
          std::make_shared<mrpt::opengl::CGridPlaneXY>(-200, 200, -200, 200, 0,
                                                       5);
      ...
      if (win3D) {
        ...
        // Move camera:
        win3D->setCameraPointingToPoint(curRobotPose.x(), curRobotPose.y(),
                                        curRobotPose.z());
