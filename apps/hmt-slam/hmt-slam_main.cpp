...
int main(int argc, char **argv) {
  try {
    CConsoleRedirector dbg_out_file("./DEBUG_log_streaming.txt", true, true,
                                    false, 0);
...
void Run_HMT_SLAM() {
  ...
  // ----------------------------------------
  rawlogFileName =
      cfgFile.read_string("HMT-SLAM", "rawlog_file", std::string("log.rawlog"));
  ...
  //			INITIALIZATION
  // ----------------------------------------
  // utils::CRandomGenerator::Randomize( );	// Not necesary (called inside the
  // classes)
    ...
    if (rawlogEntry >= rawlog_offset) {
      ...
      if (IS_CLASS(objFromRawlog, CActionCollection)) {
        mapping.pushAction(std::dynamic_pointer_cast<CActionCollection>(
            objFromRawlog)); // Memory will be freed in mapping class
      } else if (IS_CLASS(objFromRawlog, CSensoryFrame)) {
        mapping.pushObservations(std::dynamic_pointer_cast<CSensoryFrame>(
            objFromRawlog)); // Memory will be freed in mapping class
